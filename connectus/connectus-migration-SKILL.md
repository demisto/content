...
> _Auth parity is enforced **inside `set-auth`** — the candidate `Auth Details` JSON is run through [`check_auth_parity.py`](check_auth_parity.py) before the cell is committed, and the write is rejected unless parity passes or short-circuits via an interpolated structural skip. **AUTH-PARITY GATE STRICTNESS FIX (2026-06-03): the ONLY codes that commit are `ERROR_ALL_INTERPOLATED` / `ERROR_CONNECTION_INTERPOLATED`.** Every "cannot verify" code (`ERROR_NON_PYTHON`, `ERROR_NO_BASECLIENT`, `APIMODULE_INTEGRATION_CANNOT_VERIFY`, `ERROR_INTEGRATION_REJECTS_HTTP`, `MULTI_SECRET_PASSTHROUGH`) now **BLOCKS** the write — including all non-Python (`.js` / `.ps1`) integrations. The required operator action for those is to mark the offending auth(s) `interpolated: true`, after which they flow through the `ERROR_ALL_INTERPOLATED` clean path. A successful `set-auth` therefore *means* "parity has been verified (or was all-interpolated)"; see [§1.12 Auth-parity gate inside `set-auth`](#112-auth-parity-gate-inside-set-auth)._
...
**When the gate BLOCKS:** the core rule is — mark the offending auth(s) `interpolated: true` (they then flow through the `ERROR_ALL_INTERPOLATED` clean path and the gate allows), OR make parity runnable (provide docker/env) and re-run. Always `--dry-run` first and iterate on the dry-run (it is free and side-effect-free) before the real apply.

> Per-failure-class fixes — UCP non-standard-header overrides (`_apply_ucp_*`), startup-validator `is_ucp_enabled()` gating, boto3/feed permanent-interpolated cases, UCP-strip crashes, the full `--seed-param` recovery reference, and the sentinel grammar — are in **[`connectus/auth-parity-troubleshooting.md`](auth-parity-troubleshooting.md)**.
