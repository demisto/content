...
def process(path):
    ...
    if promote_tenant and tenant_sample is not None:
        gf = {"id": "tenant_id", "title": strip_title(tenant_sample.get("title", "Tenant ID")),
    ...
    order = (["proxy"] if "proxy" in movable_samples else []) + \
    ...
    if gen_fields:
        ...
        if not gc:
            gc = {"description": "Common configuration shared by all connection profiles",
    ...
    print(f"  {path}: promoted_tenant={promote_tenant} moved={sorted(movable_samples)} "
