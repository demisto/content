...
| 14 | `shadowed command test passes` | Checkpoint ✅ | Verify no conflicting commands in the same connector |
| 15 | `requires auth parity test` | Flag | `YES`, `NO`, or `N/A` |
| 16 | `auth parity test passes` | Checkpoint ✅ | Auth parity test passes (auto `N/A` if flag is `NO`) |
| 17 | `code reviewed` | Checkpoint ✅ | Code review completed |
| 18 | `code merged` | Checkpoint ✅ | Code merged to branch |
...
2. **Sequential order** — Checkpoint columns 7–18 must be completed in order. You cannot mark "unit tests passed" before "wrote code" is done. The script will reject the attempt and tell you what the current step is.
...
8. **Auth parity flag** — Column 15 is a flag, not a checkpoint. When set to `NO` or `N/A`, column 16 is automatically set to `N/A` and skipped.
