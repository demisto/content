...
class VectraClient(BaseClient):
        ...
        **kwargs,
        ...
        try:
            ...
            raise DemistoException(
                ERRORS["INVALID_OBJECT"].format(response_type, res.content),  # type: ignore[str-bytes-safe]
                exception,
                res,
            )
    ...
    def list_users_request(self, username: Optional[str], role=Optional[str], last_login_timestamp=Optional[datetime]) -> dict:
        """
        List users.
    ...
    def get_entity_request(self, entity_id: int = None, entity_type: str = None) -> dict:
        ...
        entity = self.http_request(
            method="GET", url_suffix="{}/{}".format(ENDPOINTS["ENTITY_ENDPOINT"], entity_id), params=params, response_type="json"
        )
        return entity
    ...
    def mark_or_unmark_detection_fixed_request(self, detection_ids: List[str], mark: str) -> dict:
        ...
        data = {"detectionIdList": detection_ids, "mark_as_fixed": mark}

        res = self.http_request(method="PATCH", url_suffix=ENDPOINTS["DETECTION_ENDPOINT"], json_data=data, response_type="json")
        return res
    ...
    def add_entity_assignment_request(
        ...
        res = self.http_request(method="POST", url_suffix=ENDPOINTS["ASSIGNMENT_ENDPOINT"], data=body, response_type="json")
        return res

    def update_entity_assignment_request(
        self, assign_to_user_id: Optional[int] = None, assignment_id: Optional[int] = None
    ) -> dict:
        """
    ...
    def resolve_entity_assignment_request(
        self,
        ...
        res = self.http_request(
            method="PUT",
            url_suffix=ENDPOINTS["RESOLVE_ASSIGNMENT_ENDPOINT"].format(assignment_id),
            data=body,
            response_type="json",
        )
        return res
    ...
    def list_assignment_outcomes_request(self, page: int = None, page_size: int = None) -> dict:
        ...
        res = self.http_request(
            method="GET", url_suffix=ENDPOINTS["ASSIGNMENT_OUTCOME_ENDPOINT"], params=params, response_type="json"
        )
        return res
...
def urgency_score_to_severity(entity_urgency_score: Optional[int], params: dict[str, Any]) -> int:
    ...
    """
    try:
        urgency_score_lt = validate_urgency_score(
            urgency_score=params.get("urgency_score_low_threshold", "30").strip(), score_name="urgency_score_lt"
        )
    except ValueError:
        demisto.error("Provided urgency_score_low_threshold is not a valid number. Falling back to default threshold value 30.")
        urgency_score_lt = DEFAULT_URGENCY_SCORE_LOW_THRESHOLD

    try:
        urgency_score_mt = validate_urgency_score(
            urgency_score=params.get("urgency_score_medium_threshold", "50").strip(), score_name="urgency_score_mt"
        )
    except ValueError:
        demisto.error(
    ...
    try:
        urgency_score_ht = validate_urgency_score(
            urgency_score=params.get("urgency_score_high_threshold", "80").strip(), score_name="urgency_score_ht"
        )
    except ValueError:
        demisto.error("Provided urgency_score_high_threshold is not a valid number. Falling back to default threshold value 80.")
...
def get_list_entity_detections_command_hr(detections: dict[Any, Any], page: Optional[int], page_size: Optional[int], count: int):
    """
...
def get_assignment_list_command_hr(assignments: dict, page: Optional[int], page_size: Optional[int], count: int):
    """
...
def get_assignment_outcome_list_command_hr(outcomes: dict, page: Optional[int], page_size: Optional[int], count: int):
    """
...
def fetch_incidents(client: VectraClient, params: dict[str, Any]) -> List:
    ...
    if not last_run:  # If it's the first time running
        ...
        new_last_run = {
            "time": arg_to_datetime(first_fetch_time, arg_name="First Fetch Time").strftime(DATE_FORMAT)  # type: ignore
        }
    ...
    if entities and not all(id_to_check in already_fetched for id_to_check in ids_to_check):
        ...
        if len(entities) == max_fetch and next_url:
            new_last_run.update({"next_url": next_url, "already_fetched": already_fetched})
        else:
            new_last_run.update(
                {"time": entities[-1].get("last_modified_timestamp"), "next_url": None, "already_fetched": already_fetched}
            )
    else:
...
def vectra_detections_mark_fixed_command(client: VectraClient, args: dict[str, Any]):
    ...
    if (
        res.get("_meta", {}).get("level") == "Success"
        and res.get("_meta").get(  # type: ignore
            "message"
        )
...
def vectra_assignment_outcome_list_command(client: VectraClient, args: dict[str, Any]):
    ...
    human_readable, outcomes = get_assignment_outcome_list_command_hr(
        outcomes=outcomes, page=page, page_size=page_size, count=count
    )
    ...
    return CommandResults(
        ...
        outputs_prefix="Vectra.Entity.Assignments.Outcomes",
        outputs_key_field=["id"],
    )
...
def get_modified_remote_data_command(client: VectraClient, args: dict) -> GetModifiedRemoteDataResponse:
    ...
    while True:
        ...
        try:
            response = client.list_entities_request(
                last_modified_timestamp=command_last_run_date,  # type: ignore
                page=page,
                page_size=page_size,
                state="",
            )
...
def update_remote_system_command(client: VectraClient, args: dict) -> str:
    ...
    if "closingUserId" in delta_keys and parsed_args.incident_changed and parsed_args.inc_status == IncidentStatus.DONE:
        ...
        if len(close_notes) > MAX_OUTGOING_NOTE_LIMIT:
            ...
            client.add_entity_note_request(entity_id=mirror_entity_id, entity_type=remote_entity_type, note=closing_note)
