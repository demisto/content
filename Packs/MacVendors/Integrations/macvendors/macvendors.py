...
def test_module(client: BaseClient):
    try:
        ...
        client._http_request("GET", resp_type="response", url_suffix=test_mac, ok_codes=[200])
        return_results("ok")
...
def lookup_mac_command(client: BaseClient, args: dict[str, Any]):
    ...
    res = client._http_request("GET", url_suffix=urllib.parse.quote(str(mac_address)), ok_codes=[200, 404], resp_type="response")
    command_results = CommandResults(outputs_prefix="MacVendors", outputs_key_field="address")
    if res.status_code == 200:
        command_results.outputs = {"address": mac_address, "vendor": res.text}
        command_results.readable_output = f"{mac_address} - {res.text}"
    else:
        command_results.outputs = {"address": mac_address, "vendor": "Unknown"}
