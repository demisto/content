...
def restcall(method, api, **kwargs):
    ...
    try:
        response = request_func(url, headers=header, verify=VERIFY_CERT, **kwargs)
    except Exception as e:
        return_error(f"Error Connecting to server. Details: {e!s}")
    ...
    if end_time is not None and start_time is None:
        ...
        if unit == "minutes" or unit == "minute":
            temp_time_ago = datetime.strftime(
                datetime.strptime(end_time, "%Y-%m-%d %H:%M:%S.000") - timedelta(minutes=number), "%Y-%m-%d %H:%M:%S.000"
            )
        if unit == "hours" or unit == "hour":
            temp_time_ago = datetime.strftime(
                datetime.strptime(end_time, "%Y-%m-%d %H:%M:%S.000") - -timedelta(hours=number), "%Y-%m-%d %H:%M:%S.000"
            )
        if unit == "days" or unit == "day":
            temp_time_ago = datetime.strftime(
                datetime.strptime(end_time, "%Y-%m-%d %H:%M:%S.000") - -timedelta(days=number), "%Y-%m-%d %H:%M:%S.000"
            )
        if unit == "months" or unit == "month":
            temp_time_ago = datetime.strftime(
                datetime.strptime(end_time, "%Y-%m-%d %H:%M:%S.000") - -timedelta(days=number * 30), "%Y-%m-%d %H:%M:%S.000"
            )
        if unit == "years" or unit == "year":
            temp_time_ago = datetime.strftime(
                datetime.strptime(end_time, "%Y-%m-%d %H:%M:%S.000") - -timedelta(days=number * 365), "%Y-%m-%d %H:%M:%S.000"
            )
    else:
        temp_time_ago, now = parse_date_range(
            timeago,
            date_format="%Y-%m-%d \
%H:%M:%S.000",
        )
...
def uptycs_run_query():
    ...
    if demisto.args().get("query_type") == "global":
        ...
        if demisto.args().get("asset_id") is not None:
            _id = {"_id": {"equals": demisto.args().get("asset_id")}}
        elif demisto.args().get("host_name_is") is not None:
            _id = {"host_name": {"equals": demisto.args().get("host_name_is")}}
        elif demisto.args().get("host_name_like") is not None:
            _id = {"host_name": {"like": "%{}%".format(demisto.args().get("host_name_like"))}}
        else:
            _id = {"host_name": {"like": "%%"}}

        post_data = {"query": query, "type": "realtime", "filtering": {"filters": _id}}
...
def uptycs_run_query_command():
    ...
    human_readable = tableToMarkdown("Uptycs Query Result", query_results.get("items"))
    ...
    entry = {
        ...
        "EntryContext": {"Uptycs.QueryResults": context},
    }
...
def uptycs_get_assets():
    ...
    like_cuts = {"host_name": demisto.args().get("host_name_like")}
...
def uptycs_get_assets_command():
    ...
    entry = {
        ...
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Assets(val.id == obj.id)": context},
    }
        ...
        for index in range(len(context)):
            ...
            if bool(json.loads(context[index].get("metadata")).get("indicatorId")):
                ...
                context[index]["threat_source_name"] = (
                    json.loads(context[index].get("metadata")).get("indicatorSummary").get("threatSourceName")
                )
            else:
...
this alert"
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Alerts(val.id == obj.id)": context},
    }
...
def uptycs_get_events_command():
    ...
    human_readable = tableToMarkdown(
        "Uptycs Events", query_results.get("items"), ["host_name", "grouping", "event_time", "description", "value", "severity"]
    )
    entry = {
        "ContentsFormat": formats["json"],
        "Type": entryTypes["note"],
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Events(val.id == obj.id)": query_results.get("items")},
    }
...
def uptycs_get_alert_rules_command():
    ...
    human_readable = tableToMarkdown(
        "Uptycs Alert Rules", query_results.get("items"), ["name", "description", "grouping", "enabled", "updatedAt", "code"]
    )
...
def uptycs_get_event_rules_command():
    ...
    human_readable = tableToMarkdown(
        "Uptycs Event Rules", query_results.get("items"), ["name", "description", "grouping", "enabled", "updatedAt", "code"]
    )
...
def uptycs_get_process_open_files():
    ...
    if time is not None:
        day = time.replace(" ", "-")
        day_list = day.split("-")
        uptday = int(f"{day_list[0]!s}{day_list[1]!s}{day_list[2]!s}")
    ...
    like_cuts = {"upt_hostname": demisto.args().get("host_name_like")}
    query = apply_like_cuts(query, like_cuts)
    ...
    if time is None and (time_ago is not None or (start_window is not None or end_window is not None)):
        begin, end = uptycs_parse_date_range(time_ago, start_window, end_window)
...
def uptycs_get_process_open_files_command():
    ...
    human_readable = tableToMarkdown(
        "Process which has opened a file", query_results.get("items"), ["upt_hostname", "pid", "path", "fd", "upt_time"]
    )
    context = query_results.get("items")
    context_entries_to_keep = ["upt_hostname", "upt_asset_id", "pid", "path", "fd", "upt_time"]
    if context is not None:
        remove_context_entries(context, context_entries_to_keep)
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Files": context},
    }
...
def uptycs_get_process_open_sockets_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Sockets": context},
    }
...
def uptycs_get_socket_events_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.SocketEvents": context},
    }
...
ORDER BY upt_time DESC LIMIT 1".format(uptday, time, demisto.args().get("ip"))
    ...
    equal_cuts = {"upt_asset_id": demisto.args().get("asset_id"), "upt_hostname": demisto.args().get("host_name_is")}
...
def uptycs_get_socket_event_information_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.SocketEvent": context},
    }
...
def uptycs_get_processes_command():
    ...
    human_readable = tableToMarkdown(
        "Processes", query_results.get("items"), ["upt_hostname", "pid", "name", "path", "upt_time", "parent", "cmdline"]
    )
    context = query_results.get("items")
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Process": context},
    }
...
def uptycs_get_process_events_command():
    ...
    human_readable = tableToMarkdown(
        "Process events", query_results.get("items"), ["upt_hostname", "pid", "path", "upt_time", "parent", "cmdline"]
    )
    context = query_results.get("items")

    context_entries_to_keep = ["upt_hostname", "upt_asset_id", "pid", "path", "upt_time", "parent", "cmdline", "cwd"]
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.ProcessEvents": context},
    }
...
def uptycs_get_process_information_command():
    ...
    human_readable = tableToMarkdown(
        "Process information", query_results.get("items"), ["upt_hostname", "parent", "pid", "name", "path", "cmdline"]
    )
    context = query_results.get("items")
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Proc": context},
    }
...
def uptycs_get_process_event_information_command():
    ...
    human_readable = tableToMarkdown(
        ...
        query_results.get("items"),
        ["upt_hostname", "parent", "pid", "path", "cmdline", "ancestor_list"],
    )
    context = query_results.get("items")
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.ProcEvent": context},
    }
...
def uptycs_get_parent_information_command():
    ...
    human_readable = tableToMarkdown(
        "Parent process information", query_results.get("items"), ["upt_hostname", "parent", "pid", "name", "path", "cmdline"]
    )
    context = query_results.get("items")
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Parent": context},
    }
...
def uptycs_get_parent_event_information_command():
    ...
    human_readable = tableToMarkdown(
        "Parent process event information", query_results.get("items"), ["upt_hostname", "parent", "pid", "path", "cmdline"]
    )
    context = query_results.get("items")
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.ParentEvent": context},
    }
...
def uptycs_get_process_child_processes_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Children": context},
    }
...
def uptycs_set_alert_status_command():
    ...
    human_readable = tableToMarkdown("Uptycs Alert Status", query_results, ["id", "code", "status", "createdAt", "updatedAt"])
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.AlertStatus": context},
    }
...
def uptycs_get_asset_id_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Asset": context},
    }
...
def uptycs_get_asset_tags_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.AssetTags": context},
    }
...
def uptycs_set_asset_tag_command():
    ...
    human_readable = tableToMarkdown("Uptycs Asset Tag", query_results, ["hostName", "tags"])
    context = query_results
...
def uptycs_get_users_command():
    ...
    human_readable = tableToMarkdown(
        "Uptycs Users", query_results.get("items"), ["name", "email", "id", "admin", "active", "createdAt", "updatedAt"]
    )
    context = query_results.get("items")
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Users": context},
    }
...
def uptycs_get_user_information_command():
    ...
    human_readable = tableToMarkdown("Uptycs User Information", query_results, ["name", "email", "id"])
    context = query_results
    context["userRoles"] = {context.get("userRoles")[0].get("role").get("name"): context.get("userRoles")[0].get("role")}
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.UserInfo": context},
    }
    ...
    for user_id in user_ids:
        ...
        for obj_group in obj_groups:
            if obj_group.get("objectGroupId") == asset_group_id:
                users_in_group[user_info.get("name")] = {"email": user_info.get("email"), "id": user_info.get("id")}
...
def uptycs_get_user_asset_groups_command():
    ...
    human_readable = tableToMarkdown("Uptycs User Asset Groups", query_results)
    context = query_results
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.UserGroups": context},
    }
...
def uptycs_get_asset_groups_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.AssetGroups": context},
    }
...
def uptycs_get_threat_indicators_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.ThreatIndicators": context},
    }
...
def uptycs_get_threat_indicator_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.ThreatIndicator": context},
    }
...
def uptycs_get_threat_sources_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.ThreatSources": context},
    }
...
def uptycs_get_threat_source_command():
    ...
    human_readable = tableToMarkdown(
        "Uptycs Threat Sources", query_results, ["name", "description", "url", "enabled", "custom", "createdAt", "lastDownload"]
    )
    context = query_results
    context_entries_to_keep = ["name", "description", "url", "enabled", "custom", "createdAt", "lastDownload"]
...
def uptycs_get_threat_vendors_command():
    ...
    human_readable = tableToMarkdown("Uptycs Threat Vendors", context)
    ...
    entry = {
        ...
        "Contents": context,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.ThreatVendors": context},
    }
...
def uptycs_post_threat_source():
    ...
    response = requests.post(url, headers=header, data=post_data, files=files, verify=VERIFY_CERT)
...
def uptycs_get_saved_queries_command():
    ...
    human_readable = tableToMarkdown(
        "Uptycs Saved Queries", context, ["name", "description", "query", "executionType", "grouping", "id"]
    )
    ...
    entry = {
        ...
        "Contents": context,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.SavedQueries": context},
    }
...
def uptycs_run_saved_query():
    ...
    if query_results.get("executionType") == "realtime":
        api_call = "/assets/query"
        if demisto.args().get("asset_id") is not None:
            _id = {"id": {"equals": demisto.args().get("asset_id")}}
        elif demisto.args().get("host_name_is") is not None:
            _id = {"host_name": {"equals": demisto.args().get("host_name_is")}}
        elif demisto.args().get("host_name_like") is not None:
            _id = {"host_name": {"like": "%" + demisto.args().get("host_name_like") + "%"}}
        else:
            _id = {"host_name": {"like": "%%"}}

        post_data = {"type": "realtime", "query": query, "filtering": {"filters": _id}}
    else:
        post_data = {"query": query}
        api_call = "/query"
...
def uptycs_run_saved_query_command():
    ...
    entry = {
        ...
        "Contents": context,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.RunQuery": context},
    }
...
def uptycs_post_saved_query_command():
    ...
    if query_results.get("status") == 500:
        return_error(
            "Internal Server Error, check whether a query with this \
        name has already been saved"
        )

    human_readable = tableToMarkdown(
        "Uptycs Posted Query", query_results, ["name", "type", "description", "query", "executionType", "grouping", "custom"]
    )
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.PostedQuery": query_results},
    }
...
def uptycs_get_carves_link_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.CravesLink": query_results},
    }
...
def get_carve_headers(url):
    ...
    keys = ["x-amz-server-side-encryption-customer-key", "x-amz-server-side-encryption-customer-key-MD5"]
...
def uptycs_get_carves_source_command():
    ...
    human_readable = tableToMarkdown("Uptycs Carves", context, context_entries_to_keep)
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.Carves(val.id == obj.id)": context},
    }
    return entry
...
def uptycs_get_tags_source_command():
    ...
    human_readable = tableToMarkdown("Uptycs tags", context, context_entries_to_keep)
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.tags(val.id == obj.id)": context},
    }
    return entry
...
def uptycs_get_tag_with_id_source_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.tag": context},
    }
    return entry
...
def uptycs_get_lookuptables_command():
    ...
    human_readable = tableToMarkdown("Uptycs look up tables", context, context_entries_to_keep)
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.lookuptables(val.id == obj.id)": context},
    }
    return entry
...
def uptycs_get_lookuptable_command():
    ...
    entry = {
        ...
        "Contents": query_results,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.lookuptable": context},
    }
    return entry
...
def uptycs_edit_lookuptable_command():
    ...
    entry = {
        ...
        "Contents": response,
        "HumanReadable": human_readable,
        "EntryContext": {"Uptycs.lookuptable": response},
    }
...
def uptycs_delete_lookuptable_command():
    ...
    entry = {"ContentsFormat": formats["json"], "Type": entryTypes["note"], "Contents": response, "HumanReadable": human_readable}
...
def uptycs_delete_assets_tags():
    ...
    asset_id = demisto.args().get("asset_id")
    post_data = {"tagId": demisto.args().get("tag_id"), "filters": {"id": {"in": [asset_id]}}}
...
def uptycs_post_lookuptable_data_source(table_id=None):
    ...
    response = requests.post(url, headers=header, files=files, verify=VERIFY_CERT)
    return response
