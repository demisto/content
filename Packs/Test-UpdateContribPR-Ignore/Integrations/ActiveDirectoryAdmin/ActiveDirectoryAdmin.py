...
from ldap3 import ALL_ATTRIBUTES, NTLM, SUBTREE, Connection, Entry, Server, Tls
...
def account_entry(person_object, custome_attributes):
    ...
    account = {
        ...
        'sAMAccountName': person_object.get('sAMAccountName'),
...
def read_samccountname_dn_input(args, default_base_dn):
    search_base = args.get('base-dn') or default_base_dn

    # get the  dn of the user
    args_err = "Please provide either sAMAccountName or user-dn"

    if args.get('sAMAccountName') and args.get('user-dn'):
        # both arguments passed
        raise Exception(args_err)
    if args.get('sAMAccountName'):
        dn = user_dn(args['sAMAccountName'], search_base)
    elif args.get('user-dn'):
        dn = args.get('user-dn')
    else:
...
def search_users(default_base_dn, page_size):
    ...
    if args.get('sAMAccountName'):
        query = "(&(objectClass=User)(objectCategory=person)(sAMAccountName={}))".format(args['sAMAccountName'])
...
def create_user():
    ...
    if custom_attributes:
        try:
            custome_attributes = json.loads(custom_attributes)
        except Exception as e:
...
def update_user(default_base_dn):
    ...
    dn = read_samccountname_dn_input(args, default_base_dn)
    ...
    try:
        ...
        demisto.error(traceback.format_exc())
        raise Exception(
            "Failed to parse attributes argument. Please see an example of this argument in the description."
        )
    ...
    demisto.log(json.dumps(modification))
    # modify user
    ...
    demisto_entry = {
        'ContentsFormat': formats['text'],
        'Type': entryTypes['note'],
        'Contents': "Updated user attributes: {} ".format(json.dumps(attributes))
    }
    demisto.results(demisto_entry)
...
def modify_user_ou(default_base_dn):
    assert conn is not None
    args = demisto.args()
    ...
    success = conn.modify_dn(dn, cn, new_superior=args.get('new-ou'))
    if not success:
    ...
    demisto_entry = {
        'ContentsFormat': formats['text'],
        'Type': entryTypes['note'],
        'Contents': "Moved user {} to {}".format(dn, args.get('new-ou'))
    }
    demisto.results(demisto_entry)
...
def enable_user(default_base_dn):
    ...
    # set user custom attributes if there is any
    if custom_attributes:
        try:
    ...
    demisto_entry = {
        'ContentsFormat': formats['text'],
        'Type': entryTypes['note'],
        'Contents': "User {} was enabled".format(dn)
    }
    demisto.results(demisto_entry)
...
def disable_user(default_base_dn):
    ...
    demisto_entry = {
        'ContentsFormat': formats['text'],
        'Type': entryTypes['note'],
        'Contents': "User {} was disabled".format(dn)
    }
    demisto.results(demisto_entry)
...
def add_member_to_group(default_base_dn):
    ...
    member_dn = read_samccountname_dn_input(args, default_base_dn)
    ...
    if not success:
        raise Exception("Failed to add {} to group {}".format(
            args.get('sAMAccountName') or args.get('computer-name'),
            args.get('group_name')
        ))
...
def unlock_account(default_base_dn):
    ...
    if not success:
        raise Exception("Failed to unlock user {}".format(dn))

    demisto_entry = {
        'ContentsFormat': formats['text'],
        'Type': entryTypes['note'],
        'Contents': "Unlocked user {}".format(dn)
    }
    demisto.results(demisto_entry)
...
def main():
    ...
    sAMAccountName = demisto.params().get('credentials')['identifier']
    ...
    try:
        ...
        if NTLM_AUTH:
            ...
            domain_user = SERVER_IP + '\\' + sAMAccountName if '\\' not in sAMAccountName else sAMAccountName
            conn = Connection(server, user=domain_user, password=PASSWORD, authentication=NTLM)
        else:
            # here sAMAccountName should be the user dn
            conn = Connection(server, user=sAMAccountName, password=PASSWORD)
        ...
        try:
            ...
            message = "Failed to access LDAP server. " + exc_msg
        ...
        demisto.debug('Established connection with AD LDAP server')
        ...
        demisto.debug('Verified base DN "{}"'.format(DEFAULT_BASE_DN))
        ...
        if demisto.command() == 'ad-modify-user-ou':
            modify_user_ou(DEFAULT_BASE_DN)
