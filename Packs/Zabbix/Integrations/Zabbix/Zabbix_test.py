...
class TestZabbix:
    def test_get_host(self, mocker):
        mocker.patch(
            "demistomock.params",
            return_value={"url": "http://localhost", "credentials": {"identifier": "user", "password": "password"}},
        )
        zapi = ZabbixApiStub()
        spy = mocker.spy(zapi, "do_request")
        args = {"method": "host.get", "params": "{}"}
    ...
    def test_main_get_host(self, mocker):
        ...
        mocker.patch("demistomock.args", return_value={"method": "host.get"})
