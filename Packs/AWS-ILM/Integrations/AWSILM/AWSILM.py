...
AWS_DEFAULT_SCHEMA_MAPPING = {
    ...
    "active": "active",
    "password": "password",
    "emails": [{"type": "type", "value": "value", "primary": "primary"}],
    "phoneNumbers": [{"type": "type", "value": "value"}],
    "addresses": [
        {
            "formatted": "formatted",
...
class Client(BaseClient):
    ...
    def get_user(self, iam_attribute: str, iam_attribute_val: str) -> Optional["IAMUserAppData"]:
        ...
        res = self._http_request(method="GET", url_suffix=url_suffix, params=params)
        user_app_data = None
        ...
        if user_app_data:
            ...
            return IAMUserAppData(
                user_id, username, is_active, user_app_data, email=get_first_primary_email_by_scim_schema(user_app_data)
            )
        return None
    ...
    def create_group(self, data):
        return self._http_request(
            method="POST", url_suffix=groupUri, json_data=data, resp_type="response", ok_codes=(200, 201, 409)
        )
...
def get_group_command(client, args):
    ...
    if not group_id:
        try:
            ...
            if res.status_code == 200:
                if res_json.get("totalResults") == 0:
                    ...
                    generic_iam_context = OutputContext(
                        success=True, iden=group_id, displayName=group_name, details=res_json["Resources"][0]
                    )
        ...
        try:
            ...
            if res.status_code == 200:
                generic_iam_context = OutputContext(
                    success=True, iden=res_json.get("id"), displayName=res_json.get("displayName"), details=res_json
                )
        except DemistoException as exc:
...
def update_group_command(client, args):
    ...
    if member_ids_to_add:
        ...
        for member_id in member_ids_to_add:
            operation = {"op": "add", "path": "members", "value": [{"value": member_id}]}
    ...
    if member_ids_to_delete:
        ...
        for member_id in member_ids_to_delete:
            operation = {"op": "remove", "path": "members", "value": [{"value": member_id}]}
            group_input = {"schemas": [patchSchema], "Operations": [operation]}
            try:
                res = client.update_group(group_id, group_input)
            except DemistoException as exc:
...
def get_mapping_fields() -> GetMappingFieldsResponse:
    ...
    """
    incident_type_scheme = SchemeTypeMapping(type_name=IAMUserProfile.DEFAULT_INCIDENT_TYPE, fields=AWS_DEFAULT_SCHEMA_MAPPING)
