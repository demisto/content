    ...
    assert response == [
        {
            ...
            "ETag": "cb4b5d90134c462ec98a1839f430ee2c",
            "Size": 68768,
...
def test_main_proxy_error(mocker):
    ...
    """
    params = {"accessKey": "accessKey", "secretKey": "secretKey", "feedType": ["Domain"], "proxy": True}
...
def test_main(mocker):
    ...
    params = {"accessKey": "accessKey", "secretKey": "secretKey", "feedType": ["Domain"]}
    ...
    # Sis get indicators
    fetch_indicators_response = [
        {
            "value": "007blog.icu",
            "type": "Domain",
            "rawJSON": OrderedDict([("value", "007blog.icu"), ("Timestamp", "1590810346"), ("type", "Domain")]),
            "fields": {
                "threattypes": {"threatcategory": "Domain"},
                "region": "us-west-1",
                "service": "S3",
...
def test_build_iterator(mocker):
    ...
    mocker.patch.object(
        gzip, "open", return_value=["007blog.icu\t1590810346", "0122312.com\t1590809115", "0666639.cn\t1590812070"], create=True
    )
    ...
    mocker.patch("botocore.client.BaseClient._make_api_call", return_value=expected_response)
    assert isinstance(
        next(CLIENT.build_iterator(feed_type="Domain", key="key", limit="1", is_get_indicators=True)), csv.DictReader
    )
...
def test_build_iterator_client_error(mocker):
    ...
    mocker.patch("botocore.client.BaseClient._make_api_call", side_effect=botocore.exceptions.ClientError(expected_response, ""))
    with pytest.raises(ValueError) as v_error:
...
def test_build_iterator_proxy_error(mocker):
    ...
    """
    mocker.patch(
        "botocore.client.BaseClient._make_api_call", side_effect=botocore.exceptions.ProxyConnectionError(proxy_url="proxy_url")
    )
    with pytest.raises(ValueError) as v_error:
        next(CLIENT.build_iterator(feed_type="bucket", key="key"))
...
def test_build_iterator_http_client_error(mocker):
    ...
    """
    mocker.patch("botocore.client.BaseClient._make_api_call", side_effect=botocore.exceptions.HTTPClientError(error=""))
    with pytest.raises(ValueError) as v_error:
        next(CLIENT.build_iterator(feed_type="bucket", key="key"))
    assert str(v_error.value.args[0]) == MESSAGES["HTTP_CLIENT_ERROR"] + "An HTTP Client raised an unhandled exception: "
...
def test_build_iterator_error(mocker):
    ...
    """
    mocker.patch("botocore.client.BaseClient._make_api_call", side_effect=Exception(""))
    with pytest.raises(ValueError) as v_error:
        next(CLIENT.build_iterator(feed_type="bucket", key="key"))
...
def test_get_last_key_from_integration_context(mocker):
    ...
    integration_context = [{"Domain": "key"}]
...
def test_fetch_indicators_command(mocker):
    ...
    mocker.patch(
        "SecurityIntelligenceServicesFeed.Client.request_list_objects",
        return_value=[{"Key": "key1.gz", "LastModified": datetime.now(timezone.utc)}],
    )

    mocker.patch(
        "SecurityIntelligenceServicesFeed.Client.build_iterator",
        return_value=[csv.DictReader(f=["007blog.icu\t1590810346"], fieldnames=["value", "Timestamp"], delimiter="\t")],
    )

    assert (
        next(fetch_indicators_command(client=CLIENT, feed_types=["domain"], first_fetch_interval="1 day", tags=["s3", "s4"]))
        == expected_response
    )

    # When no latest key found.
    mocker.patch("SecurityIntelligenceServicesFeed.Client.request_list_objects", return_value=[])
    mocker.patch("SecurityIntelligenceServicesFeed.get_last_key_from_integration_context_dict", return_value="key1")
    mocker.patch(
        "SecurityIntelligenceServicesFeed.Client.build_iterator",
        return_value=[csv.DictReader(f=["007blog.icu\t1590810346"], fieldnames=["value", "Timestamp"], delimiter="\t")],
    )
    assert (
        next(
            fetch_indicators_command(
                client=CLIENT, feed_types=["domain"], first_fetch_interval="0 day", limit="1", tags=["s3", "s4"]
            )
        )
        == expected_response
    )
...
def test_get_indicators_command(mocker):
    ...
    mocker.patch(
        ...
        return_value=[
            csv.DictReader(
                f=["007blog.icu\t1590810346"], fieldnames=["value", "Timestamp"], delimiter="\t", quoting=csv.QUOTE_NONE
            )
        ],
    )
    args = {"feed_type": "Domain", "limit": 1}
...
def test_indicator_field_mapping():
    ...
    assert (
        indicator_field_mapping("domain", {"value": "007blog.icu", "Timestamp": "1590810346"}, tags=["s3"], tlp_color="")
        == expected_res
    )

    expected_res = {
        "service": "Passive Total",
        "siscategory": "category",
