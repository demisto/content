...
#         results.append(get_entry_for_object('The requested scan was launched successfully', 'TenableIO.Scan', replace_keys(res), LAUNCH_SCAN_HEADERS))
...
def pause_scan_command():
    ...
    # for scan_id in scan_ids:
    #     demisto.results("the scan id is"+scan_id)
    ...
    #             "Status": "Pausing"
    #         }
    #         results.append(get_entry_for_object("The requested scan was paused successfully", "TenableIO.Scan", replace_keys(resumed_scan), ["Id", "Status"]))

    #     else:
    #         results.append("Command 'tenable-io-pause-scans' cannot be called while scan status is {} for scanID {}".format(scan_status["Status"], scan_id))
...
#                 "Status": "Resuming"
#             }
#             results.append(get_entry_for_object("The requested scan was resumed successfully", "TenableIO.Scan", replace_keys(resumed_scan), ["Id", "Status"]))

#         else:
#             results.append("Command 'tenable-io-resume-scans' cannot be called while scan status is {} for scanID {}".format(scan_status["Status"], scan_id))
...
#         for scan in scans:
#             post_response = send_request(str(scan).replace("u'", '"').replace("'", '"'), endpoint="scans", method="POST", endpoint_base="")
