...
| detailed | If detailed is true, the report will contain remediations and hosts information as well for the given scan. Otherwise the report will only have vulnerabilities. Possible values are: yes, no. Default is no. | Optional | 
| info | Return the basic details of the given scan. Possible values are: yes, no. Default is no. | Optional | 
