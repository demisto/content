    ...
    "vulnerabilities": [
        {
            ...
            "accepted_count": 0,
            "recasted_count": 0,
            "counts_by_severity": [{"count": 117, "value": 2}],
            "severity": 2,
        },
    ]
}
...
def test_get_scans_command(mocker, requests_mock):
    ...
    requests_mock.get(
        MOCK_PARAMS["url"] + "scans/?folder_id=1", json={"scans": [{"status": "running", "id": "1"}], "info": {"id": "1"}}
    )
...
def test_get_asset_details_command(mocker, requests_mock):
    from test_data.response_and_results import EXPECTED_ASSET_INFO_RESULTS, MOCK_RAW_ASSET_ATTRIBUTES, MOCK_RAW_ASSET_BY_IP
...
@pytest.mark.parametrize(
    "range_str, expected_lower_range_bound,expected_upper_range_bound",
    [
        ("2.5-3", 2.5, 3),
        ("2.5-3.5", 2.5, 3.5),
        ("0.1-3", 0.1, 3),
...
@pytest.mark.parametrize(
    "date_str, expected_date", [("1 day ago", 1326326400), ("2 days ago", 1326240000), ("1326232800", 1326232800)]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"vprScoreOperator": "eq", "vprScoreValue": "", "vprScoreRange": ""},
            "Please specify vprScoreValue and vprScoreOperator",
        ),
        (
            {"vprScoreOperator": "", "vprScoreValue": "3.5", "vprScoreRange": ""},
            "Please specify vprScoreValue and vprScoreOperator",
        ),
    ],
)
...
@pytest.mark.parametrize(
    "args, expected_vpr_score",
    [
        (
            {"vprScoreOperator": "equal", "vprScoreValue": "3.5"},
            {"eq": [3.5], "neq": None, "gt": None, "lt": None, "gte": None, "lte": None},
        ),
        (
            {"vprScoreOperator": "not equal", "vprScoreValue": "3.5"},
            {"eq": None, "neq": [3.5], "gt": None, "lt": None, "gte": None, "lte": None},
        ),
        (
            {"vprScoreOperator": "lt", "vprScoreValue": "3.5"},
            {"eq": None, "neq": None, "gt": None, "lt": 3.5, "gte": None, "lte": None},
        ),
        (
            {"vprScoreOperator": "lte", "vprScoreValue": "3.5"},
            {"eq": None, "neq": None, "gt": None, "lt": None, "gte": None, "lte": 3.5},
        ),
        (
            {"vprScoreOperator": "gt", "vprScoreValue": "3.5"},
            {"eq": None, "neq": None, "gt": 3.5, "lt": None, "gte": None, "lte": None},
        ),
        (
            {"vprScoreOperator": "gte", "vprScoreValue": "3.5"},
            {"eq": None, "neq": None, "gt": None, "lt": None, "gte": 3.5, "lte": None},
        ),
...
def test_export_vulnerabilities_build_command_result():
    ...
    assert (
        command_result.readable_output == "### Vulnerabilities"
...
@pytest.mark.parametrize(
    "args, return_value_export_request_with_export_uuid",
    [
...
def test_export_assets_command(mocker, args, return_value_export_request_with_export_uuid):
    ...
    mocker.patch.object(
        Tenable_io, "export_request_with_export_uuid", return_value={"export_uuid": "XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX"}
    )
    mocker.patch.object(Tenable_io, "export_request_with_export_uuid", return_value=return_value_export_request_with_export_uuid)
    mocker.patch.object(Tenable_io, "get_chunks_request", return_value=export_assets_response)
    ...
    if not args.get("exportUuid"):
        ...
        assert (
            response.readable_output == "### Assets\n|ASSET ID|DNS NAME (FQDN)|SYSTEM TYPE|OPERATING SYSTEM|IPV4 ADDRESS"
            "|NETWORK|FIRST SEEN|LAST SEEN|LAST LICENSED SCAN|SOURCE|TAGS|\n|"
            "---|---|---|---|---|---|---|---|---|---|---|\n|"
...
def test_export_vulnerabilities_command(mocker, args, return_value_export_request_with_export_uuid):
    ...
    mocker.patch.object(Tenable_io, "export_request_with_export_uuid", return_value=return_value_export_request_with_export_uuid)
    mocker.patch.object(Tenable_io, "get_chunks_request", return_value=export_vulnerabilities_response)
    mocker.patch.object(demisto, "args", return_value=args)
    ...
    if not args.get("exportUuid"):
        ...
        assert (
            response.readable_output == "### Vulnerabilities"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"sortFields": "", "sortOrder": "asc", "excludeRollover": False, "limit": 10, "offset": 0},
            {
                "sort": "",
        ...
        (
            ...
            {"sort": "name:asc,date:asc,status:asc", "exclude_rollover": False, "limit": 50, "offset": 0},
        ),
        ...
        (
            ...
            {"sort": "name:asc,date:desc,status:asc", "exclude_rollover": False, "limit": 50, "offset": 0},
        ),
    ],
)
...
def test_get_scan_history_command(mocker):
    ...
    request.assert_called_with(*test_data["called_with"]["args"], **test_data["called_with"]["kwargs"])
...
def test_download_export_scan(mocker):
    ...
    request.assert_called_with("GET", "scans/scan_id/export/file_id/download", resp_type="content")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"scanId": "", "format": "", "filterSearchType": ""},
            {"status": "error"},
...
@pytest.mark.parametrize(
    "args, expected_result",
    (
        ({"page": "10", "pageSize": "5", "limit": "50"}, {"limit": 5, "offset": 45}),
        ({"limit": "50", "page": "23"}, {"limit": "50", "offset": 0}),
    ),
)
...
def test_set_index_audit_logs(
    dt_now: datetime, dt_start_date: datetime, audit_logs: List[dict], last_index_fetched: int, new_last_index_fetched: int
):
    """
    Given:
...
def test_fetch_assets(requests_mock):
    ...
    # Verify snapshot_id is generated for XSIAM dataset snapshots
    assert last_run.get("snapshot_id") is not None
    assert last_run.get("total_assets") == 0
    original_snapshot_id = last_run.get("snapshot_id")
...
FETCH_ASSETS_EXAMPLES = [
    ...
    ([{"id": "asset_id_one", "name": "asset_name_one"}], [{"id": "asset_id_one", "name": "asset_name_one"}], {}, False),
    # export uuid 111 is not valid, so new export uuid 222 is generated (snapshot_id should be regenerated)
    (
        {"status": 404, "message": "Export expired or not found"},
        [],
        {"assets_export_uuid": "222", "nextTrigger": "30", "type": 1},
        True,  # snapshot_id should be regenerated
    ),
    # # export uuid is valid, but chunk is not valid (snapshot_id should be regenerated)
    ({"status": 404, "message": "invalid chunk"}, [], {"assets_export_uuid": "222", "nextTrigger": "30", "type": 1}, True),
]


@pytest.mark.parametrize("api_response, expected_assets, expected_last_run, should_regenerate_snapshot", FETCH_ASSETS_EXAMPLES)
def test_handle_assets_chunks(requests_mock, api_response, expected_assets, expected_last_run, should_regenerate_snapshot):
    """
    Given:
    ...
    # Verify snapshot_id handling
    if should_regenerate_snapshot:
        # When export uuid is regenerated due to 404, snapshot_id should also be regenerated
        assert new_last_run.get("snapshot_id") is not None
...
def test_should_seal_empty_assets_snapshot_first_time_after_assets_complete():
    ...
    """
    from Tenable_io import should_seal_empty_assets_snapshot
