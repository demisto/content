...
def enrich_incident_items(parent, childrens, feed_tags, tlp_color):
    ...
    for children in childrens:
        ...
        indicator = {
            ...
            "occurred": datetime.strptime(parent.get("created"), "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%m/%d/%Y, %H:%M:%S"),
...
def enrich_malware_items(parent, childrens, feed_tags, tlp_color):
    ...
    for children in childrens:
        ...
        if indicator_type:
            ...
            indicator = {
                "value": value,
                "occurred": datetime.strptime(children["created"], "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%m/%d/%Y, %H:%M:%S"),
                "type": indicator_type,
                "rawJSON": dict(children),
                "fields": {
                    "category": "Malware",
    ...
    if parent:
        ...
        modified_parent = {
            "value": parent["name"],
            "occurred": datetime.strptime(parent["created"], "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%m/%d/%Y, %H:%M:%S"),
...
class Client(BaseClient):
    ...
    def fetch_access_token(self):
        ...
        req_data = parse.urlencode(
            {"grant_type": "client_credentials", "client_id": self.luminar_client_id, "client_secret": self.luminar_client_secret}
        )
        ...
        for luminar_feed in self.fetch_luminar_api_feeds(is_fetch_command=True):
            ...
            for key, group in relationships.items():
                ...
                if parent and parent.get("type") == "malware":
                    indicators_list = []
                    modified_parent, modified_childrens = enrich_malware_items(parent, children, self.feed_tags, self.tlp_color)
                    ...
                    incidents_list = []
                    modified_parent, modified_childrens = enrich_incident_items(parent, children, self.feed_tags, self.tlp_color)
...
def cognyte_luminar_get_leaked_records(client: Client, args: dict):
    ...
    if leaked_records:
        readable_output = tableToMarkdown(
            name=LEAKED_CREDENTIALS_TABLE_HEADER,
            t=leaked_records,
