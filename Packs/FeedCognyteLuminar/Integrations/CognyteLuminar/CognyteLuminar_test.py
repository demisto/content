...
def test_fetch_indicators_command(mocker):
    mocker.patch.object(client, "fetch_luminar_indicators", side_effect=[INDICATOR_LIST])
...
def test_generic_item_finder(expected_optput):
    response = generic_item_finder(INDICATOR_LIST, "indicator--f0680d81-7ce7-4a04-8315-0d38f792b908")
    result = {}
    for i in response:
        result.update(i)
...
def test_enrich_malware_items(expected_optput):
    ...
    parent, child = enrich_malware_items(malware[0], indicator, ["TT1", "TT2"], "RED")
...
def test_enrich_incident_items(expected_optput):
    user_account_list = [ele for ele in INDICATOR_LIST if ele["type"] == "user-account"]
    incident = {
        "type": "incident",
        "spec_version": "2.1",
        "id": "incident--9907916e-f213-4c08-bd91-6c2bf109e509",
...
def test_get_luminar_indicators_list(mocker, requests_mock):
    ...
    mocker.patch.object(client, "fetch_luminar_api_feeds", side_effect=gen_response)
