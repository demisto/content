...
def delete_entries_command(args):
    resource_id = args.get('resourceId')
    entries_arg = args.get('entries')
    query_path = f'detect-api/rest/activelists/{resource_id}/deleteEntries'
    fields = []
    entries = []
...
def main():
    ...
    try:
        if demisto.command() == 'test-module':
            ...
            if use_detect_api:
                delete_entries_command(demisto.args())
            else:
...
register_module_line('ArcSight ESM v2_copy', 'end', __line__())
