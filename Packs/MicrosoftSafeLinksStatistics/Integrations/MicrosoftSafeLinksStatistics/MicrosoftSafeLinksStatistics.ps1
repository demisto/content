...
$script:COMMAND_PREFIX = "o365-safelinks-stats"
$script:INTEGRATION_ENTRY_CONTEXT = "O365SafeLinksStats"
...
class ExchangeOnlineClient {
    ...
    [Array]SearchSafeLinksStats(
        [string]$start_date,
        [string]$end_date,
        [string]$click_id,
        [string]$recipient_address,
        [String]$url_or_domain
    ){
        ...
        if ($click_id){
            $cmd_args.ClickId = $click_id
        }
        if ($recipient_address){
            $cmd_args.RecipientAddress = $recipient_address
        }
        if ($url_or_domain){
            $cmd_args.UrlOrDomain = $url_or_domain
        }
...
function SearchSafeLinksStatsCommand{
    [CmdletBinding()]
    Param(
    ...
    try
    {
        ...
        $raw_response = $client.SearchSafeLinksStats(
                $start_date,
                $end_date,
                $kwargs.click_id,
                $kwargs.recipient_address,
                $kwargs.url_or_domain
        )
        if ($raw_response){
            ...
            foreach ($item in $raw_response)
            {
                 $stats =@{
                    ...
                    "ClickTime" = $item.Clicked.DateTime
                    "Recipient" = $item.RecipientAddress
                    "URL" = $item.Url
                    "UrlBlocked" = $item.UrlBlocked
                    "UrlClicked" = $item.UrlClicked
                    "Workload" = $item.Workload}
                $list.add($stats)
            }
            $context = @{"$script:INTEGRATION_ENTRY_CONTEXT" = $list}
            $human_readable = TableToMarkdown $list.ToArray() "Stats from $start_date to $end_date"
            Write-Output $human_readable, $context, $list
        } else {
            $human_readable = "Stats from $start_date to $end_date is empty"
    ...
    try
    {
        ...
        switch ($command)
        {
            ...
            "$script:COMMAND_PREFIX-search" {
                ($human_readable, $entry_context, $raw_response) = SearchSafeLinksStatsCommand $exo_client $command_arguments
            }
