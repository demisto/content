...
def mocked_client(requests_mock, mocker):
    ...
    mock_tenant_url = "https://mock-tenant.epm.cyberark.cloud"

    requests_mock.get(f"{mock_tenant_url}/epm/api/Sets", json=mock_response_sets)
    requests_mock.get(
        f"{mock_tenant_url}/epm/api/Sets/id1/AdminAudit?dateFrom=2023-01-01T00:00:00Z&limit=10",
        json=mock_response_admin_audits,
    )
    requests_mock.get(
        f"{mock_tenant_url}/epm/api/Sets/id2/AdminAudit?dateFrom=2023-01-01T00:00:00Z&limit=10",
        json=mock_response_admin_audits,
    )
    requests_mock.get(
        f"{mock_tenant_url}/epm/api/Sets/id1/AdminAudit?dateFrom=2023-12-12T07:45:27.141Z&limit=10",
        json=mock_response_admin_audits,
    )
    requests_mock.get(
        f"{mock_tenant_url}/epm/api/Sets/id2/AdminAudit?dateFrom=2023-12-12T07:45:27.141Z&limit=10",
        json=mock_response_admin_audits,
    )
    requests_mock.post(
        f"{mock_tenant_url}/epm/api/Sets/id1/policyaudits/search?nextCursor=start&limit=10", json=mock_response_policy_audits
    )
    requests_mock.post(
        f"{mock_tenant_url}/epm/api/Sets/id1/policyaudits/search?nextCursor=1700097106000&limit=10",
        json=mock_response_no_more_events,
    )
    requests_mock.post(
        f"{mock_tenant_url}/epm/api/Sets/id2/policyaudits/search?nextCursor=start&limit=10", json=mock_response_policy_audits
    )
    requests_mock.post(
        f"{mock_tenant_url}/epm/api/Sets/id2/policyaudits/search?nextCursor=1700097106000&limit=10",
        json=mock_response_no_more_events,
    )
    requests_mock.post(
        f"{mock_tenant_url}/epm/api/Sets/id1/Events/Search?nextCursor=start&limit=10", json=mock_response_events
    )
    requests_mock.post(
        f"{mock_tenant_url}/epm/api/Sets/id1/Events/Search?nextCursor=1702360757618&limit=10",
        json=mock_response_no_more_events,
    )
    requests_mock.post(
        f"{mock_tenant_url}/epm/api/Sets/id2/Events/Search?nextCursor=start&limit=10", json=mock_response_events
    )
    requests_mock.post(
        f"{mock_tenant_url}/epm/api/Sets/id2/Events/Search?nextCursor=1702360757618&limit=10",
        json=mock_response_no_more_events,
    )

    client = Client(
        base_tenant_url="https://api-na.epm.cyberark.cloud",
        identity_url="https://tenant.id.cyberark.cloud",
        client_id="test_client_id",
        client_secret="test_client_secret",
        web_app_id="test_web_app",
        policy_audits_event_type=["a", "b", "c"],
    )
    mocker.patch.object(client, "_get_access_token", return_value="mock_access_token")
    mocker.patch.object(client, "_get_tenant_url", return_value=mock_tenant_url)
...
def test_get_set_ids_by_set_names(mocker, requests_mock):
    ...
    client = mocked_client(requests_mock, mocker)
...
def test_get_events_command(mocker, requests_mock, event_type):
    """
    Given:
...
def test_fetch_events(mocker, requests_mock):
    ...
    events, next_run = fetch_events(mocked_client(requests_mock, mocker), last_run, 10, True)
