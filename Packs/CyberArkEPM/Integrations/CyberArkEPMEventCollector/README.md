...
| EPM API Version | Applies to the Idira OAuth authentication method only. The EPM API version used as a URL path segment in the data-call paths (`/EPM/API/<Version>/Sets`). CyberArk documents the format as `x.x.x.x` (four dot-separated numbers); shorter forms such as `26.7.0` are also accepted. Defaults to `26.7.0`. Change this only if your tenant requires a different version. | False |
