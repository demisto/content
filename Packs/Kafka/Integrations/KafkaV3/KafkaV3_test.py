...
from confluent_kafka import TIMESTAMP_CREATE_TIME, TIMESTAMP_NOT_AVAILABLE, KafkaError, TopicPartition
...
@pytest.mark.parametrize(
    "demisto_params, cluster_tree, first_offset, last_offset",
    [
...
def test_passing_test_module_with_fetch_and_offset_as_num(mocker, demisto_params, cluster_tree, first_offset, last_offset):
    """
    Given:
        - initialized KafkaCommunicator
    When:
...
@pytest.mark.parametrize(
    "demisto_params, cluster_tree, expected_failure",
    [
        (
            {"isFetch": True, "topic": "some-topic", "partition": "2"},
            {"some-topic": [1]},
            "Partition 2 is not assigned to kafka topic some-topic",
        ),
        (
            {"isFetch": True, "topic": "some-topic", "partition": "1", "offset": "some-bad-offset"},
            {"some-topic": [1]},
...
@pytest.mark.parametrize(
    "demisto_params, cluster_tree, first_offset, last_offset, expected_failure",
    [
        (
            {"isFetch": True, "topic": "some-topic", "partition": "1", "offset": "5"},
            {"some-topic": [1]},
            6,
            7,
        ...
        (
            {"isFetch": True, "topic": "some-topic", "partition": "1", "offset": "10"},
            {"some-topic": [1]},
            6,
            7,
        ...
        (
            {"isFetch": True, "topic": "some-topic", "partition": "1", "offset": "7"},
            {"some-topic": [1]},
            6,
            7,
...
def test_failing_test_module_with_fetch_and_offset_as_num(
    mocker, demisto_params, cluster_tree, first_offset, last_offset, expected_failure
):
    """
    Given:
        - initialized KafkaCommunicator
...
@pytest.mark.parametrize(
    "demisto_args, first_offset, last_offset",
    [({"include_offsets": "true"}, 0, 1), ({"include_offsets": "true"}, 1, 5), ({"include_offsets": "true"}, 0, 2)],
)
def test_print_topics_with_offsets(mocker, demisto_args, first_offset, last_offset):
    ...
    mocker.patch.object(KConsumer, "get_watermark_offsets", return_value=(first_offset, last_offset))
    result = print_topics(KAFKA, demisto_args)
    expected = {"Name": "some-topic", "Partitions": [{"ID": 1, "EarliestOffset": first_offset, "OldestOffset": last_offset}]}
    from CommonServerPython import CommandResults
...
@pytest.mark.parametrize(
    "demisto_args, cluster_tree, topic",
    [
...
@pytest.mark.parametrize(
    "demisto_args, topic_partitions",
    [
        ({"topic": "some-topic", "partition": 0, "offset": 0}, [TopicPartition(topic="some-topic", partition=0, offset=0)]),
        ({"topic": "some-topic", "partition": 0, "offset": 1}, [TopicPartition(topic="some-topic", partition=0, offset=1)]),
        (
            {"topic": "some-topic", "partition": 0, "offset": "latest"},
            [TopicPartition(topic="some-topic", partition=0, offset=1)],
        ),
        ({"topic": "some-topic", "partition": 0}, [TopicPartition(topic="some-topic", partition=0, offset=0)]),
    ],
)
...
@pytest.mark.parametrize(
    "demisto_args, topic_partitions, cluster",
    [
        ({"topic": "some-topic"}, [TopicPartition(topic="some-topic", partition=1, offset=0)], {"some-topic": [1]}),
        (
            {"topic": "some-topic"},
            [
...
def test_produce_message(mocker, partition_number):
    ...
    produce_mock.assert_called_once_with(
        topic="some-topic", partition=partition_number, value="some-value", on_delivery=KAFKA.delivery_report
    )
...
def test_produce_message_with_Schema(mocker, value, value_schema_type, value_schema_str, value_schema_subject_name):
    """
    Given:
        - initialized KafkaCommunicator
    When:
    ...
    produce_mock.assert_called_once_with(topic="some-topic", partition=0, value=value, on_delivery=KAFKA.delivery_report)
...
def test_produce_error_message(mocker):
    ...
    produce_mock.assert_called_once_with(topic="some-topic", partition=1, value="some-value", on_delivery=KAFKA.delivery_report)
    flush_mock.assert_called_once()
...
def test_produce_schema_error(mocker, value_schema_str, value_schema_subject_name, exception_message):
    """
    Given:
        - initialized KafkaCommunicator
    When:
...
def test_produce_schema_registry_none_error(mocker):
    """
    Given:
        - initialized KafkaCommunicator
    When:
    ...
    demisto_args = {"topic": "some-topic", "value": "some-value", "value_schema_type": "AVRO", "value_schema_str": "Test"}
    produce_mock = mocker.patch.object(KProducer, "produce")
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            {"topic": "some-topic", "partition": "0", "first_fetch": "earliest", "max_fetch": "1"},
            {},
            {"some-topic": [0]},
            [TopicPartition(topic="some-topic", partition=0, offset=0)],
            [
            ...
            [MessageMock(message="polled_msg", partition=0, offset=0, timestamp=(TIMESTAMP_NOT_AVAILABLE, 0))],
            [(0, 2)],
            id="first run",
        ),
        pytest.param(
        ...
        pytest.param(
            {"topic": "some-topic", "partition": "0", "first_fetch": "earliest", "max_fetch": "2"},
            {"last_fetched_offsets": {"0": 0}, "last_topic": "some-topic"},
            {"some-topic": [0]},
            [TopicPartition(topic="some-topic", partition=0, offset=1)],
            [
            ...
            [(0, 2), (0, 2)],
            id="1 partition 2/2 messages",
        ),
        pytest.param(
            {"topic": "some-topic", "partition": "0", "first_fetch": "earliest", "max_fetch": "3"},
            {"last_fetched_offsets": {"0": 0}, "last_topic": "some-topic"},
            {"some-topic": [0]},
            [TopicPartition(topic="some-topic", partition=0, offset=1)],
            [
        ...
        pytest.param(
            {"topic": "some-topic", "partition": "0,1", "first_fetch": "earliest", "max_fetch": "2"},
            {"last_fetched_offsets": {"0": 0}, "last_topic": "some-topic"},
            {"some-topic": [0, 1]},
            [
                TopicPartition(topic="some-topic", partition=0, offset=1),
            ...
            [(0, 3), (0, 3), (0, 3)],
            id="2 partitions, 1 message each",
        ),
        pytest.param(
            {"topic": "some-topic", "partition": "0", "first_fetch": "2", "max_fetch": "1"},
            {},
            {"some-topic": [0]},
            [TopicPartition(topic="some-topic", partition=0, offset=3)],
            [
                {
        ...
        pytest.param(
            ...
            [(0, 2)],
            id="first run add timestamp",
        ),
        pytest.param(
            {"topic": "some-topic", "partition": "", "first_fetch": "0", "max_fetch": "1"},
            {},
            {"some-topic": [0]},
            [TopicPartition(topic="some-topic", partition=0, offset=1)],
            [
        ...
        pytest.param(
            ...
            {"last_fetched_offsets": {"0": 1}, "last_topic": "some-topic"},
            {"some-topic": [0]},
            [TopicPartition(topic="some-topic", partition=0, offset=2)],
            [
                {
                    "name": "Kafka some-topic partition:0 offset:2",
                    "details": "polled_msg",
        ...
        pytest.param(
            {"topic": "some-topic", "partition": "", "first_fetch": "earliest", "max_fetch": "1"},
            {"last_fetched_offsets": {"0": 1}, "last_topic": "some-topic"},
            {"some-topic": [0, 1]},
            [
                TopicPartition(topic="some-topic", partition=0, offset=2),
        ...
        pytest.param(
            {"topic": "some-topic", "partition": "0,1", "first_fetch": "1", "max_fetch": "1"},
            {},
            {"some-topic": [0, 1]},
            [TopicPartition(topic="some-topic", partition=1, offset=2)],
            [
                {
            ...
            [MessageMock(message="polled_msg", partition=1, offset=2, timestamp=(TIMESTAMP_NOT_AVAILABLE, 0))],
            [(0, 2), (0, 3), (0, 3), (0, 3), (0, 3), (0, 3)],
            id="2 partitions, one to skip due to offset out of bounds",
        ),
        pytest.param(
            {"topic": "some-topic", "partition": "", "first_fetch": "", "max_fetch": "1"},
            {},
            {"some-topic": [0, 1]},
            [
                TopicPartition(topic="some-topic", partition=0, offset=4),
                TopicPartition(topic="some-topic", partition=1, offset=0),
            ],
            [
                {
        ...
        pytest.param(
            ...
            [MessageMock(message="polled_msg", partition=0, offset=1, timestamp=(TIMESTAMP_NOT_AVAILABLE, 0))],
            [(0, 2), (0, 2), (0, 2)],
            id="first run, offset is 0",
        ),
    ],
)
def test_fetch_incidents(
    mocker, demisto_params, last_run, cluster_tree, topic_partitions, incidents, next_run, polled_msgs, offsets
):
    """
    Given:
        - initialized KafkaCommunicator
...
@pytest.mark.parametrize(
    "demisto_params, last_run, cluster_tree",
    [
        pytest.param(
            {"topic": "some-topic", "partition": "0", "first_fetch": "earliest", "max_fetch": "1"},
            {"last_fetched_offsets": {"0": 1}, "last_topic": "some-topic"},
            {"some-topic": [0]},
            id="out of bounds offset",
        )
    ],
)
