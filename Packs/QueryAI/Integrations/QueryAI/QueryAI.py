...
class Client(BaseClient):  # type: ignore
    def __init__(
        ...
        self._headers = {"Content-Type": "application/json", "Accept": "application/json", "Authorization": self._api_token}

    def queryai_http_request(self, method, url_suffix, json_data=None, params=None, data=None, **kwargs):
        if not json_data:
            json_data = {}
        return self._http_request(
            method=method, url_suffix=url_suffix, params=params, json_data=json_data, data=data, timeout=self.timeout, **kwargs
        )

    def run_query(
        self, query: str, connection_params: str = None, workflow_params: str = None, time_text: str = None, alias: str = None
    ) -> dict[str, Any]:
...
def queryai_run_query_command(client: Client, args: dict[str, Any]) -> CommandResults:  # type: ignore
    ...
    try:
        result = client.run_query(
            query=query, connection_params=connection_params, workflow_params=workflow_params, time_text=time_text, alias=alias
        )
        ...
        return CommandResults(
            readable_output=readable_output, outputs_prefix="QueryAI.query", outputs_key_field="", outputs=reply
        )
