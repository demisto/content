...
def is_advanced_filters_contain_all_fields(filters_list: list) -> bool:
    ...
    """
    return all("name" in filter_str and "value" in filter_str and "comparison" in filter_str for filter_str in filters_list)
...
def is_valid_advanced_filters(string_filters: str) -> bool:
    ...
    return is_advanced_filters_contain_all_fields(filters_list) and is_advanced_filter_fields_in_right_order(filters_list)
...
def parse_filter_field(string_filters) -> dict:
    ...
    if is_valid_advanced_filters(string_filters):
        try:
            ...
            filters = {
                split_str[0].split("=")[1]: [
                    {"Value": split_str[1].split("=")[1], "Comparison": split_str[2].split("=")[1].upper()}
                ]
...
def create_filters_list_dictionaries(arr: List[str], compare_param: str) -> List[Dict]:
    ...
    result_arr = []
    for item in arr:
        d = {"Comparison": compare_param, "Value": item}
        result_arr.append(d)
    return result_arr
...
def build_severity_label_obj(label: str) -> List:
    ...
    """
    severity_dict = {"Informational": 0, "Low": 1, "Medium": 2, "High": 3, "Critical": 4}
    severity_label_obj = []
    ...
    for lbl in severity_dict:
        ...
        if key >= num:
            severity_label_obj.append({"Comparison": "EQUALS", "Value": lbl.upper()})
...
def update_findings_command(client, args):
    kwargs = {
        "Filters": {
            "Id": [
                {"Value": args.get("findingId"), "Comparison": "EQUALS"},
            ]
        },
...
def batch_update_findings_command(client, args):
    ...
    outputs = {"AWS-SecurityHub.ProcessedFindings": response["ProcessedFindings"]}
    del response["ResponseMetadata"]
...
def fetch_incidents(
    client, aws_sh_severity, archive_findings, additional_filters, mirror_direction, finding_types, workflow_status, product_name
):
    ...
    filters = {"CreatedAt": [{"Start": last_run, "End": now.isoformat()}]}
    if aws_sh_severity:
    ...
    for finding in findings:
        finding.update({"mirror_direction": mirror_direction, "mirror_instance": demisto.integrationInstance()})
        incidents.append(
            {
    ...
    demisto.setLastRun({"lastRun": last_run, "next_token": next_token})
    demisto.incidents(incidents)

    if archive_findings and findings:
        kwargs = {
            "FindingIdentifiers": [{"Id": finding["Id"], "ProductArn": finding["ProductArn"]} for finding in findings],
            "Workflow": {
                "Status": "NOTIFIED",
            },
            "Note": {"Text": "Archived by Demisto", "UpdatedBy": "Demisto"},
        }
...
def get_remote_data_command(client: boto3.client, args: Dict[str, Any]) -> GetRemoteDataResponse:  # type: ignore
    ...
    filters = {"Id": [{"Comparison": "EQUALS", "Value": remote_incident_id}]}
...
def update_remote_system_command(client: boto3.client, args: Dict[str, Any], resolve_findings: bool) -> str:  # type: ignore
    ...
    if parsed_args.incident_changed and delta:
        demisto.debug(
            f"Got the following delta keys {list(delta.keys())}.\n"
            f"Incident id {remote_incident_id} and incident change: "
            f"{parsed_args.incident_changed}"
        )
        kwargs = {
            "FindingIdentifiers": [
                {"Id": data.get("FindingIdentifiers.Id"), "ProductArn": data.get("FindingIdentifiers.ProductArn")}
            ],
            "Severity": {"Label": delta.get("Severity.Label")},
            ...
            "Workflow": {"Status": delta.get("Workflow.Status")[0] if delta.get("Workflow.Status") else None},
        }
