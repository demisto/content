...
PARSED_INSIGHT_HIT = {
    ...
    "investigationIDs": ["57ec1eb4-454e-4561-8059-a9beb3f830c0"],
...
def test_hit_to_indicator_enrichment_excluded():
    ...
    ioc = esf.hit_to_indicator(
        MockHit(CUSTOM_HIT), CUSTOM_VAL_KEY, CUSTOM_TYPE_KEY, None, ["tag1", "tag2"], "AMBER", enrichment_excluded=True
    )
...
def test_extract_indicators_from_generic_hit(mocker):
    ...
    ioc_lst = esf.extract_indicators_from_generic_hit(
        CUSTOM_HIT, CUSTOM_VAL_KEY, CUSTOM_TYPE_KEY, None, ["tag1", "tag2"], "AMBER"
    )
...
def test_last_run():
    ...
    ioc_lst = [
        {"id": "1", "calculatedTime": "2023-01-17T14:30:00.000Z"},
...
@pytest.mark.parametrize("client_version", [("Elasticsearch"), ("OpenSearch"), ("Elasticsearch_v8")])
def test_get_indicators_by_elastic_version_generic_feed(mocker, client_version):
    ...
    esf.get_indicators_command(
        client, feed_type="Generic Feed", src_val="indicatorValue", src_type="indicatorType", default_type="IP"
    )
...
def test_get_indicators_by_elastic_version_demisto_feed(mocker, client_version):
    ...
    mock_get_demisto_indicators_elastic_v7 = mocker.patch.object(esf, "get_demisto_indicators_elastic_v7", return_value=([], []))
...
def test_fetch_indicators_by_elastic_version(mocker, client_version):
    ...
    client = esf.ElasticsearchClient()
    esf.fetch_indicators_command(
        client,
        feed_type="Generic Feed",
        src_val="indicatorValue",
        src_type="indicatorType",
        default_type="IP",
        last_fetch={},
        fetch_limit=1000,
    )
...
def test_fetch_demisto_indicators_elastic_v7(mocker):
    ...
    mocked_search_indicators_res_e7 = {
        "took": 1,
        "timed_out": False,
    ...
    ioc_lst, _ = esf.fetch_indicators_elastic_v7(
        client,
        last_fetch_timestamp="",
        feed_type="Cortex XSOAR MT Shared Feed",
        fetch_limit=10000,
        src_type=None,
        src_val=None,
        default_type=None,
...
def test_feed_main_enrichment_excluded(mocker):
    ...
    params = {"tlp_color": "RED", "enrichmentExcluded": False, "time_field": "test", "feed_type": ["test"]}
