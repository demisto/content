...
def initialize_global_values():
    global URL, MAX_INCIDENTS_TO_FETCH, COOKIE, AUTH_HEADERS, CLIENT_ID, CLIENT_SECRET, AUTH_HEADERS, DOMAIN, AUTHORIZATION
...
def callAPI(queryClient, payload, loginClient):
    res = queryClient._http_request(method="POST", json_data=payload, url_suffix="graphql-third-party")
    return res
...
class LoginClient(BaseClient):
    def fetch_api_token(self):
        res = self._http_request(method="GET", url_suffix="api/v1/login")
...
def filter_user_information(answers):
    ...
    result = {"internal-users": int_users, "external_users": ext_users, "group_users": grp_users, "orphan_users": orphan_users}
    return result
...
def fetch_file_information(loginClient: LoginClient, queryClient: QueryClient, path: str, name: str):
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="ConcentricAI.FileInfo", outputs_key_field="ownerDetails", outputs=target
    )
...
def get_file_sharing_details(loginClient: LoginClient, queryClient: QueryClient, cid: str):
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="ConcentricAI.FileSharingInfo", outputs_key_field="info", outputs=results
    )
...
def get_users_overview(loginClient: LoginClient, queryClient: QueryClient, max_users: int):
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="ConcentricAI.UserInfo", outputs_key_field="info", outputs=result
    )
...
def get_user_details(loginClient: LoginClient, queryClient: QueryClient, user: str):
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="ConcentricAI.UserDetails", outputs_key_field="info", outputs=results
    )
...
def main() -> None:
    ...
    loginClient = LoginClient(base_url=base_url, verify=verify_certificate, headers=headers, proxy=proxy)

    global QUERY_HEADERS
    if QUERY_HEADERS is None:
        fetch_token(loginClient)

    queryClient = QueryClient(base_url=base_url, headers=QUERY_HEADERS, proxy=proxy)
    ...
    try:
        ...
        # Set and define the fetch incidents command to run after activated via integration settings.
            ...
            max_results = arg_to_int(arg=demisto.params().get("max_fetch"), arg_name="max_fetch", required=True)
