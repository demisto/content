...
class Client(BaseClient):
    def __init__(self, app_user_id, app_user_secret, external_api_key, *args, **kwargs):
...
def test_module(client):
    ...
    try:
        headers={"Authorization": f"Bearer {get_jwt_token(client.app_user_id, client.app_user_secret)}"}

        if client.external_api_key:
            headers["X-Console-API-Key"] = client.external_api_key
...
def main():  # pragma: no cover
    ...
    try:
        client = Client(app_user_id=app_user_id, app_user_secret=app_user_secret, base_url=base_url, verify=verify_certificate, external_api_key=external_api_key)
