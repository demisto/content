...
def client():
    ...
    client.get_data_type_findings = MagicMock(
        return_value=[
...
def test_dspm_get_risk_findings_with_valid_args(client):
    ...
    args = {"cloud_provider_equal": "AWS", "affects_equal": "SECURITY", "status_equal": "OPEN", "sort": "records,desc"}
    result = get_list_risk_findings(client, args, page=0)
...
def test_get_list_of_assets_with_valid_args(client):
    ...
    args = {"cloud_provider_equal": "AWS", "service_type_equal": "RDS", "lifecycle_equal": "RUNNING", "sort": "status,ASC"}
    result = get_list_of_assets(client, args, page=0)
...
def test_get_data_type_findings_with_valid_args(client):
    args = {
        "cloud_provider_in": "AWS,AZURE",
        "service_type_in": "DYNAMODB,RDS",
        "lifecycle_in": "DELETED,STOPPED",
        "sort": "records,DESC",
    }
    ...
    args = {"cloud_provider_equal": "AWS", "service_type_equal": "DYNAMODB", "lifecycle_equal": "DELETED", "sort": "records,ASC"}
    result = get_data_type_findings(client, args, page=0)
    ...
    mock_response = [
        {
            ...
            "cloudProvider": "AWS",
            "destinationProjects": {"188619942792": "Redlock"},
            ...
            "eventSource": "*.**.**.***",
            "policyFrameWorks": ["MITRE-T1074", "MITRE-T1537"],
            "eventRawData": "",
        }
    ]
...
sample_data_multiple_strings = ["AADHAAR_INDIVIDUAL_IDENTIFICATION", "PII", "CREDIT_CARD", "SSN"]

sample_data_single = [{"dataTypeName": "AADHAAR_INDIVIDUAL_IDENTIFICATION"}]

sample_data_single_string = ["AADHAAR_INDIVIDUAL_IDENTIFICATION"]
...
def test_get_data_type_findings_command_multiple_strings(client):
    ...
    assert isinstance(result, List)
    assert result == ["AADHAAR_INDIVIDUAL_IDENTIFICATION", "PII", "CREDIT_CARD", "SSN"]
...
def test_get_data_type_findings_command_single_type(client):
    ...
    result = get_data_type_findings(client, args, page=0)
    assert isinstance(result, List)
    assert result == [{"dataTypeName": "AADHAAR_INDIVIDUAL_IDENTIFICATION"}]
...
def test_get_data_type_findings_command_single_string(client):
    ...
    result = get_data_type_findings(client, args, page=0)
    assert isinstance(result, List)
    assert result == ["AADHAAR_INDIVIDUAL_IDENTIFICATION"]


def test_get_asset_files_by_id(client):
    # Mock response data
    mock_responses = [
        {"files": [{"filename": "file1.txt", "size": 1234}, {"filename": "file2.txt", "size": 5678}], "filesCount": 2},
        {"files": [], "filesCount": 0},
    ]
    ...
    args = {"asset_id": "asset1", "types": ["type1", "type2"], "page": 1, "size": 20}
    ...
    assert result.outputs == {
        "files": [{"filename": "file1.txt", "size": 1234}, {"filename": "file2.txt", "size": 5678}],
        "filesCount": 2,
    }
...
def test_dspm_get_list_of_asset_fields_command(client):
    ...
    mock_responses = [
        {
            "fields": [{"name": "maidenname", "databaseName": "dummy"}, {"name": "salary", "databaseName": "dummy"}],
            "fieldsCount": 2,
        },
        {"fields": [], "filesCount": 0},
    ]
    ...
    # Define the arguments for the command
    args = {"assetId": "asset1", "page": 1, "size": 20}
...
mock_assets_page_1 = [
    {
        ...
        "openToWorld": False,
        "tags": {"example_tag_key": "example_tag_value"},
        "assetDigTags": [{"digTagId": 1, "key": "tag1", "value": "value1"}, {"digTagId": 2, "key": "tag2", "value": "value2"}],
    }
]
...
mock_assets_page_2 = [
    {
        ...
        "assetDigTags": [{"digTagId": 3, "key": "tag3", "value": "value3"}, {"digTagId": 4, "key": "tag4", "value": "value4"}],
    }
]


@pytest.mark.parametrize(
    "mock_responses, expected_outputs",
    [
        (
