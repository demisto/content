...
import pytest
import sqlalchemy
from GenericSQL import Client, generate_default_port_by_dialect, sql_query_execute
...
ARGS1 = {"query": "select Name from city", "limit": 5, "skip": 0}

ARGS2 = {"query": "select * from mysql.user", "limit": 1, "skip": 0}

ARGS3 = {"query": "select Name from city where 1=2", "limit": 5, "skip": 0}
...
EXPECTED_OUTPUT1 = {
    "GenericSQL(val.Query && val.Query === obj.Query)": {
        "GenericSQL": {
            "Result": [{"Name": "Kabul"}, {"Name": "Qandahar"}, {"Name": "Herat"}, {"Name": "Mazar-e-Sharif"}],
            "Headers": HEADER1,
...
@pytest.mark.parametrize(
    "command, args, response, expected_result, header",
    [
...
@pytest.mark.parametrize(
    "connect_parameters, dialect, expected_response",
    [
        ...
        (
            "arg1=value1&arg2=value2&autocommit=False",
            "Microsoft SQL Server",
...
@pytest.mark.parametrize(
    "table, params, response, headers, expected_incidents, expected_last_run",
    [
        (
            input_data.TABLE_1,
            input_data.PARAMS_1,
            input_data.RESPONSE_1,
            input_data.HEADERS_1,
            input_data.EXPECTED_INCIDENTS_1,
            input_data.EXPECTED_LAST_RUN_1,
        )
    ],
)
def test_fetch_incident_by_id_simple_query(table, params, response, headers, expected_incidents, expected_last_run, mocker):
    """
    Given
...
@pytest.mark.parametrize(
    "table, params, response, headers, last_run_before_fetch",
    [(input_data.TABLE_2, input_data.PARAMS_2, input_data.RESPONSE_2, input_data.HEADERS_2, input_data.LAST_RUN_BEFORE_FETCH_2)],
)
...
@pytest.mark.parametrize(
    "table, params, response, headers, last_run_before_second_fetch, expected_incidents",
    [
        (
            input_data.TABLE_3,
            input_data.PARAMS_3,
            input_data.RESPONSE_3,
            input_data.HEADERS_3,
            input_data.LAST_RUN_BEFORE_SECOND_FETCH_3,
            input_data.EXPECTED_INCIDENTS_3,
        )
    ],
)
def test_fetch_incident_avoiding_duplicates(
    table, params, response, headers, last_run_before_second_fetch, expected_incidents, mocker
):
    """
    Given
...
@pytest.mark.parametrize(
    ...
    [
        (
            input_data.TABLE_4_1,
            input_data.TABLE_4_2,
            input_data.PARAMS_4,
            input_data.RESPONSE_4_1,
            input_data.RESPONSE_4_2,
            input_data.HEADERS_4,
            input_data.EXPECTED_LAST_RUN_4_1,
            input_data.EXPECTED_LAST_RUN_4_2,
        )
    ],
)
def test_fetch_incident_update_last_run(
    table_first_cycle,
    table_second_cycle,
    params,
    response_first_cycle,
    response_second_cycle,
    headers,
    expected_last_run_4_1,
...
@pytest.mark.parametrize(
    "table_first_cycle, table_second_cycle, table_third_cycle, params, response_first_cycle, "
    "response_second_cycle, response_third_cycle, headers, expected_last_run_5_1, "
    "expected_last_run_5_2, expected_last_run_5_3",
    [
        (
            input_data.TABLE_5_1,
            input_data.TABLE_5_2,
            input_data.TABLE_5_3,
            input_data.PARAMS_5,
            input_data.RESPONSE_5_1,
            input_data.RESPONSE_5_2,
            input_data.RESPONSE_5_3,
            input_data.HEADERS_5,
            input_data.EXPECTED_LAST_RUN_5_1,
            input_data.EXPECTED_LAST_RUN_5_2,
            input_data.EXPECTED_LAST_RUN_5_3,
        )
    ],
)
def test_fetch_incidents_de_duplication(
    table_first_cycle,
    table_second_cycle,
    table_third_cycle,
    params,
    response_first_cycle,
    response_second_cycle,
    response_third_cycle,
    headers,
    expected_last_run_5_1,
    expected_last_run_5_2,
    expected_last_run_5_3,
    mocker,
):
    """
    Given
...
GENERATE_VARS_NAMES_MSSQL = (
    [1, 123],
    "SELECT * FROM TABLE WHERE ID = ? and EMPLOYEE = ?",
    "Microsoft SQL Server",
    (
...
GENERATE_VARS_NAMES_MYSQL = (
    [1, 123],
    "SELECT * FROM TABLE WHERE ID = %s and EMPLOYEE = %s",
    "MySQL",
    (
        {"bind_variable_1": 1, "bind_variable_2": 123},
        "SELECT * FROM TABLE WHERE ID = :bind_variable_1 and EMPLOYEE = :bind_variable_2",
...
@pytest.mark.parametrize(
    "bind_variables_values_list, query, dialect, expected_result",
    [GENERATE_VARS_NAMES_MSSQL, GENERATE_VARS_NAMES_MYSQL, GENERATE_VARS_NAMES_POSTGRES, GENERATE_VARS_NAMES_DO_NOTHING],
)
def test_generate_variable_names_and_mapping(
    bind_variables_values_list: list, query: str, dialect: str, expected_result: tuple[dict[str, Any], str | Any]
):
    """
    Given
...
def test_create_engine_and_connect_engine_exists(mocker):
    ...
    client = MockedClient(
        mocker=mocker,
        global_cache={"1": Engine("Demo Engine")},
        cache_string="1",
        dialect="Teradata",
        host="host",
        username="username",
