...
## Connection Pooling
By default the integration does not pool database connections. Thus a connections is created and closed for each command run by the integration. When connection pooling is enabled, each docker container will maintain a single connection open for the _Connection Pool Time to Live_ value (default: 600 seconds). Once the time to live expires and upon the arrival of a new command, the database connection will close and a new connection will be created. 
