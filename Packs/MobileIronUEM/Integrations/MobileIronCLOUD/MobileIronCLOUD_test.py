...
MOCK_PARAMS = {"partition_id": "12345"}
...
def client():
    ...
    return MobileIronCloudClient(base_url=MOCK_URL, verify=False, auth=("test", "p"))
...
class TestGetPartitionId:
    ...
    def test_get_partition_id_return_api(mocker, requests_mock, client):
        ...
        mocker.patch.object(demisto, "params", return_value={"credentials": {"identifier": "useremail"}})
    ...
    def test_get_partition_id_return_stored(mocker, client):
        ...
        mocker.patch.object(demisto, "params", return_value={"credentials": {"identifier": "useremail"}})
        mocker.patch.object(
            demisto, "getIntegrationContext", return_value={"for_user": "useremail", "default_partition_id": "10101"}
        )
        result = get_partition_id(client)
        assert result == "10101"
...
class TestClientGetDevicesData:
    ...
    def prepare_mock(self, requests_mock):
        ...
        requests_mock.register_uri(
            "GET",
            f"{MOCK_URL}/api/v1/device",
            [{"json": mock_response_page_one, "status_code": 200}, {"json": mock_response_page_two, "status_code": 200}],
        )
...
def test_get_devices_data_command(mocker):
    ...
    mocker.patch.object(demisto, "params", return_value=MOCK_PARAMS)
    mocker.patch.object(demisto, "args", return_value={"query": "mock query", "max_fetch": "5"})

    client = mocker.Mock(name="client")
    client.get_devices_data.return_value = []
    result = execute_get_devices_data_command(client)
    client.get_devices_data.assert_called_once_with(query="mock query", max_fetch=5, partition_id="12345")
...
def test_execute_get_device_by_field_command(mocker):
    ...
    client.get_devices_data.assert_called_once_with(query="name_of_field=value_of_field", partition_id="12345")
...
def test_execute_device_action_command(client, requests_mock, mocker):
    ...
    mocker.patch.object(demisto, "params", return_value=MOCK_PARAMS)
    mocker.patch.object(demisto, "args", return_value={"device_id": "device_id_value"})
...
def test_fetch_incidents(client, requests_mock):
    from MobileIronCLOUD import SEVERITY_HIGH, fetch_incidents
...
def test_compose_non_compliance_message():
    ...
    mock_device = {"complianceState": False, "violatedPolicies": ["Out of Contact", "Compromised"]}
    message = compose_non_compliance_message(mock_device)
    assert message == "Non Compliant Device - Out of Contact, Compromised"

    mock_device = {"complianceState": False, "violatedPolicies": None}
    message = compose_non_compliance_message(mock_device)
