...
def client():
    ...
    return MobileIronCoreClient(base_url=MOCK_URL, verify=False, auth=("test", "p"))
...
class TestClientGetDevicesData:
    ...
    def prepare_mock(self, requests_mock):
        ...
        requests_mock.register_uri(
            "GET",
            f"{MOCK_URL}/api/v2/devices",
            [{"json": mock_response_one, "status_code": 200}, {"json": mock_response_two, "status_code": 200}],
        )
...
def test_execute_get_devices_data_command(mocker):
    ...
    from MobileIronCORE import STANDARD_DEVICE_FIELDS, execute_get_devices_data_command

    mocker.patch.object(demisto, "params", return_value=MOCK_PARAMS)
    mocker.patch.object(
        demisto, "args", return_value={"query": "mock query", "additional_fields": "common.other", "max_fetch": "1000"}
    )
    ...
    client.get_devices_data.assert_called_once_with(
        query="mock query", admin_space_id="1", max_fetch=1000, fields=f"{STANDARD_DEVICE_FIELDS},common.other"
    )
...
def test_send_message_command(client, mocker, requests_mock):
    ...
    mocker.patch.object(
        demisto,
        "args",
        return_value={"device_id": "device_id_value", "message": "my message", "subject": "my subject", "message_type": "email"},
    )
    ...
    spy.assert_called_once_with(
        device_id="device_id_value", admin_space_id="1", message="my message", message_mode="email", message_subject="my subject"
    )


def test_fetch_incidents(client, requests_mock):
    from MobileIronCORE import SEVERITY_HIGH, fetch_incidents
