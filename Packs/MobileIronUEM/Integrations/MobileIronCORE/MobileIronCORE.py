...
STANDARD_DEVICE_FIELDS = ",".join(
    [
        ...
        "ios.iPhone MAC_ADDRESS_EN0",
        "ios.Current MCC",
        "ios.iPhone UDID",
        "common.current_country_code",
        "common.home_country_code",
...
class MobileIronCoreClient(BaseClient):
    ...
    def get_device_data_page(
        ...
        response = self._http_request(
            ...
            params={"adminDeviceSpaceId": admin_space_id, "query": query, "fields": fields, "limit": per_page, "offset": page},
        )
    ...
    def execute_device_action(self, device_id: str, admin_space_id: str, command_action: str) -> dict[str, Any]:
        ...
        return self._http_request(
            ...
            params={"adminDeviceSpaceId": admin_space_id, "actionType": command_action},
            json_data=payload,
        )
    ...
    def ping(self):
        ...
        :rtype:
        """
        return self._http_request(method="GET", url_suffix="/api/v2/ping")
...
def validate_action_response(response) -> str:
    ...
    if validation:
        ...
        raise DemistoException("API indicated that the request was not successful. Make sure the device id provided is valid")


def execute_device_action_command(client: MobileIronCoreClient, action: str) -> str:
    """
...
def execute_fetch_incidents_command(client):
    ...
    incidents = fetch_incidents(client=client, admin_space_id=admin_space_id, incident_type=incident_type, max_fetch=max_fetch)
...
def fetch_incidents(
    ...
    devices = client.get_devices_data(
        admin_space_id=admin_space_id, max_fetch=max_fetch, query=FETCH_INCIDENTS_DEVICE_QUERY, fields=STANDARD_DEVICE_FIELDS
    )
...
def execute_get_device_by_field_command(client: MobileIronCoreClient, field_name: str, field_value: str) -> CommandResults:
    ...
    return CommandResults(outputs_prefix="MobileIronCore.Device", outputs_key_field="common_id", outputs=device)
...
def execute_get_devices_data_command(client: MobileIronCoreClient, query: str) -> CommandResults:
    ...
    devices_data_response = client.get_devices_data(
        admin_space_id=admin_space_id, query=query, fields=fields, max_fetch=max_fetch
    )
...
def execute_send_message_command(client: MobileIronCoreClient) -> str:
    ...
    response = client.send_message_action(
        device_id=device_id, admin_space_id=admin_space_id, message=message, message_mode=message_mode, message_subject=subject
    )
    return validate_action_response(response)
...
def main():
    ...
    try:
        client = MobileIronCoreClient(base_url=base_url, auth=(username, password), verify=verify_certificate, proxy=proxy)

        command_methods = {
            "test-module": lambda: execute_test_module_command(client),
            "fetch-incidents": lambda: execute_fetch_incidents_command(client),
            "mobileiron-core-get-devices-data": lambda: execute_get_devices_data_command(client, query=args.get("query")),
            "mobileiron-core-get-device-by-uuid": lambda: execute_get_device_by_field_command(
                client, field_name="common.uuid", field_value=args.get("device_uuid")
            ),
            "mobileiron-core-get-device-by-serial": lambda: execute_get_device_by_field_command(
                client, field_name="common.SerialNumber", field_value=args.get("device_serial")
            ),
            "mobileiron-core-get-device-by-mac": lambda: execute_get_device_by_field_command(
                client, field_name="common.wifi_mac_address", field_value=args.get("device_mac")
            ),
            "mobileiron-core-get-device-by-ip": lambda: execute_get_device_by_field_command(
                client, field_name="common.ip_address", field_value=args.get("device_ip")
            ),
            "mobileiron-core-unlock-device-only": lambda: execute_device_action_command(client, "UNLOCK_DEVICE_ONLY"),
            "mobileiron-core-enable-voice-roaming": lambda: execute_device_action_command(client, "ENABLE_VOICE_ROAMING"),
            "mobileiron-core-disable-voice-roaming": lambda: execute_device_action_command(client, "DISABLE_VOICE_ROAMING"),
            "mobileiron-core-enable-data-roaming": lambda: execute_device_action_command(client, "ENABLE_DATA_ROAMING"),
            "mobileiron-core-disable-data-roaming": lambda: execute_device_action_command(client, "DISABLE_DATA_ROAMING"),
            "mobileiron-core-enable-personal-hotspot": lambda: execute_device_action_command(client, "ENABLE_PERSONAL_HOTSPOT"),
            "mobileiron-core-disable-personal-hotspot": lambda: execute_device_action_command(client, "DISABLE_PERSONAL_HOTSPOT"),
            ...
            "mobileiron-core-unlock-app-connect-container": lambda: execute_device_action_command(
                client, "UNLOCK_APP_CONNECT_CONTAINER"
            ),
