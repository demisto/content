...
class Client:
    ...
    def get_collection(self, collection: str) -> Collection:
        ...
        raise DemistoException(f"Collection '{collection}' has not found in database '{self.db.name}'")
    ...
    def query(self, collection: str, query: dict, limit: int = 50, sort_str: str = "", fields: str | None = None) -> Cursor[Any]:
        collection_obj = self.get_collection(collection)
        if fields:
...
def handle_bulk_update_string_query_arguments(
    argument: tuple[str, str], filter_valid_input_example: str, update_valid_input_example: str
) -> list[dict]:
    ...
    if not argument_value.startswith("[") or not argument_value.endswith("]"):
        ...
        raise DemistoException(
            brackets_syntax_error_msg.format(
                f"`{argument_name}`", filter_valid_input_example if argument_name == "filter" else update_valid_input_example
            )
        )
...
def parse_and_validate_bulk_update_arguments(filter: str | list, update: str | list) -> tuple[list, list]:
    ...
    filters = (
        handle_bulk_update_string_query_arguments(("filter", filter), filter_valid_input_example, update_valid_input_example)
        if isinstance(filter, str)
        else filter
    )
    updates = (
        handle_bulk_update_string_query_arguments(("update", update), filter_valid_input_example, update_valid_input_example)
...
def get_entry_by_id_command(client: Client, collection: str, object_id: str, **kwargs) -> tuple[str, dict, list]:
...
def search_query(
    client: Client, collection: str, query: str, limit: str, sort: str = "", fields: str = None, **kwargs
...
def insert_entry_command(client: Client, collection: str, entry: str, **kwargs) -> tuple[str, dict, list]:
    ...
    try:
        ...
        if not results.acknowledged:
            ...
            outputs = {CONTEXT_KEY: [{"_id": _id, "collection": collection} for _id in object_ids]}
...
def update_entry_command(
    ...
    **kwargs,
    ...
    response = client.update_entry(collection, json_filter, json_update, argToBoolean(update_one), argToBoolean(upsert))
...
def delete_entry_command(client: Client, collection, filter, delete_one, **kwargs) -> tuple[str, None]:
    try:
...
def list_collections_command(client: Client, **kwargs) -> tuple[str, dict, list]:
    ...
    if raw_response:
        readable_outputs = tableToMarkdown("MongoDB: All collections in database", raw_response, headers=["Collection"])
        outputs = {"MongoDB.Collection(val.Name === obj.Name)": [{"Name": collection} for collection in raw_response]}
        return readable_outputs, outputs, raw_response
    else:
        return "MongoDB: No results found", {}, raw_response


def drop_collection_command(client: Client, collection: str, **kwargs) -> tuple[str, None]:
    response = client.drop_collection(collection)
    if (hasattr(response, "acknowledged") and not response.acknowledged) or (
        isinstance(response, dict) and not response.get("ok") == 1.0
    ):
...
def pipeline_query_command(
    client: Client, collection: str, pipeline: str, limit: str = "50", offset: str = "0", **kwargs
) -> tuple[str, dict, list]:
