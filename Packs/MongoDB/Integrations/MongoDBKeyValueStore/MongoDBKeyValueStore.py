...
def write_key_value_command():
    ...
    if not cursor:
        ...
        context = {"ID": str(entry_id), "Incident": incident, "Modified": timestamp, "Key": key, "Value": value}
        ec = {"MongoDB.Entry(val.ID === obj.ID)": context}
        return f'Incident "{incident}" - key/value collection - 1 document added', ec, {}

    # Modify Existing Record
    object_id = cursor.get("_id")
    COLLECTION.update_one({"_id": object_id}, {"$set": {incident: {"key": key, "value": value, "modified": timestamp}}})
    context = {"ID": str(object_id), "Incident": incident, "Modified": timestamp, "Key": key, "Value": value}
    ec = {"MongoDB.Entry(val.ID === obj.ID)": context}
...
def num_keys_command():
    ...
    for i in cursor:
        if incident in i:
            count += 1
    return f"The count of the key/value pairs for the incident - {count!s}", {}, {}
...
def list_key_values_command():
    ...
    if cursor is None:
        ...
        for i in cursor:
            if incident in i:
                found = True
                return_json.append({"Key": i[incident]["key"], "Value": i[incident]["value"]})
                context.append({"Incident": incident, "Key": i[incident]["key"], "Value": i[incident]["value"]})
...
def purge_entries_command():
    ...
    for i in cursor:
        if incident in i:
            ...
            deleted += 1
    if deleted == 1:
        return f'Incident "{incident}" key/value pairs purged - {deleted!s} document/record deleted', {}, {}
    return f'Incident "{incident}" key/value pairs purged - {deleted!s} documents/records deleted', {}, {}
...
def list_incidents_command():
    ...
    for incident in cursor:
        ...
        for i in incidents:
            if i not in results:
                results.append(i)
    human_readable = tableToMarkdown(f"List of incidents in collecion {COLLECTION_NAME}", results, headers=["Incidents"])
