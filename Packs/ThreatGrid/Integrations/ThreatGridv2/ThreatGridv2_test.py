...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"sample_id": "sample_id", "command_name": "threat-grid-sample-list"},
            "ThreatGrid.Sample",
        ),
        (
            f"/{API_VERSION2_URL}/samples/sample_id/report.html",
            {"sample_id": "sample_id", "artifact": "report.html", "command_name": "threat-grid-sample-list"},
            "ThreatGrid.Sample",
        ),
        (
            f"/{API_VERSION2_URL}/samples/sample_id/summary",
            {"sample_id": "sample_id", "command_name": "threat-grid-sample-summary-get"},
            "ThreatGrid.SampleAnalysisSummary",
        ),
        (
            f"/{API_VERSION2_URL}/samples",
            {"offset": 1, "limit": 2, "command_name": "threat-grid-sample-list"},
            "ThreatGrid.Sample",
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"domain": "domain", "command_name": "threat-grid-domain-samples-list"},
            "ThreatGrid.DomainAssociatedSample",
        ),
        (
            f"/{API_VERSION2_URL}/registry_keys/registry_key/samples",
            {"registry_key": "registry_key", "command_name": "threat-grid-registry-key-samples-list"},
            "ThreatGrid.RegistryKeyAssociatedSample",
        ),
        (
            f"/{API_VERSION2_URL}/paths/path/samples",
            {"offset": 1, "limit": 2, "path": "path", "command_name": "threat-grid-path-samples-list"},
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"sample_id": "sample_id", "command_name": "threat-grid-analysis-annotations-get"},
            "SampleAnnotations",
        ),
        (
            f"/{API_VERSION2_URL}/samples/sample_id/analysis/artifacts/artifact",
            {"sample_id": "sample_id", "artifact_id": "artifact", "command_name": "threat-grid-analysis-artifacts-get"},
            "ArtifactAnalysis",
        ),
        (
            f"/{API_VERSION2_URL}/samples/sample_id/analysis/iocs",
            {"sample_id": "sample_id", "ioc": "ioc", "command_name": "threat-grid-analysis-iocs-get"},
            "IOCAnalysis",
        ),
        (
            f"/{API_VERSION2_URL}/samples/sample_id/analysis/metadata",
            {"sample_id": "sample_id", "command_name": "threat-grid-analysis-metadata-get"},
            "AnalysisMetadata",
        ),
        (
        ...
        (
            ...
            {"sample_id": "sample_id", "process_id": "process_id", "command_name": "threat-grid-analysis-processes-get"},
            "ProcessAnalysis",
        ),
    ],
)
...
def test_analysis_sample_command_no_response(requests_mock, mock_client):
    ...
    args = {"sample_id": "sample_id", "command_name": "threat-grid-analysis-annotations-get"}
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"domain": "domain", "command_name": "threat-grid-domain-associated-ips"},
            "DomainAssociatedIp",
        ),
        (
            f"/{API_VERSION2_URL}/domains/domain/urls",
            {"domain": "domain", "command_name": "threat-grid-domain-associated-urls"},
            "DomainAssociatedUrl",
        ),
        (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"network_stream": "network_stream", "command_name": "threat-grid-feeds-network-stream"},
            "NetworkStreams",
        ),
        (
        ...
        (f"/{API_VERSION2_URL}/iocs/feeds/ips", {"ip": "ip", "command_name": "threat-grid-feeds-ip"}, "Ip"),
        (
...
def test_upload_sample_command(requests_mock, mock_client):
    ...
    result = upload_sample_command(mock_client, {"url": "url", "private": True})
...
@pytest.mark.parametrize(
    "url_prefix ,args, outputs_prefix",
    [
        (
            f"urls/{URL_SHA256}",
            {
...
@pytest.mark.parametrize(
    "args,outputs_key_field",
    [
        ({"command_name": "ip", "ip": "ip"}, "ip"),
        ({"command_name": "url", "url": "url"}, "url"),
        ({"command_name": "domain", "domain": "domain"}, "domain"),
        ({"command_name": "file", "file": "file"}, "md5"),
    ],
)
def test_reputation_command(
    ...
    if args["command_name"] == "ip":
        mock_response = load_mock_response("sample_analysis.json")
        requests_mock.get(url=f"/{API_VERSION2_URL}/samples/sample_id/analysis/annotations", json=mock_response)
...
def test_schedule_command_sample_upload_when_state_is_fail(
    ...
    mocker.patch("ThreatGridv2.sample_state_get_command", return_value=CommandResults(raw_response=mock_raw_response))
