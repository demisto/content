...
def test_error_message(requests_mock):
    ...
    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {":output": "json", "command": "ERROR"}
...
def test_epo_help_command(requests_mock):
    ...
    args = {":output": "json", "search": "clienttask.find"}
...
def test_epo_get_current_dat_command(requests_mock):
    ...
    requests_mock.get(
        f"{EPO_URL}/remote/repository.findPackages?:output=json&searchText=VSCANDAT1000", text=epo_get_current_dat_file_response
    )
...
def test_epo_command_command(requests_mock):
    ...
    requests_mock.get(
        f"{EPO_URL}/remote/repository.findPackages?:output=json&searchText=VSCANDAT1000", text=epo_command_command_response
    )

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {":output": "json", "command": "repository.findPackages", "searchText": "VSCANDAT1000"}
...
def test_epo_update_client_dat_command(requests_mock):
    ...
    requests_mock.get(
        f"{EPO_URL}/remote/clienttask.run?names=192.168.1.102&%3Aoutput=json&productId=EPOAGENTMETA"
        f"&taskId=33&retryAttempts=1&retryIntervalInSeconds=5&abortAfterMinutes=5&stopAfterMinutes=20"
        f"&randomizationInterval=0",
        text="OK:\nSucceeded",
    )
    requests_mock.get(
        f"{EPO_URL}/remote/clienttask.find?searchText=VSEContentUpdateDemisto&%3Aoutput=json", text=epo_get_client_task_id_by_name
    )
...
def test_epo_get_system_tree_groups_command(requests_mock):
    ...
    requests_mock.get(
        f"{EPO_URL}/remote/system.findGroups?%3Aoutput=json&searchText=workgroup", text=epo_get_system_tree_groups_response
    )

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {":output": "json", "search": "workgroup"}
...
def test_epo_find_systems_command(requests_mock):
    ...
    requests_mock.get(f"{EPO_URL}/remote/epogroup.findSystems?%3Aoutput=json&groupId=2", text=epo_find_systems_command_response)

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {":output": "json", "groupId": "2"}

    result = epo_find_systems_command(client, args)

    assert len(result) == 7
    assert result[0].outputs[0] == {
        "AutoID": 2,
        "CPUSerialNum": "",
        "CPUSpeed": 0,
        "CPUType": "",
...
def test_epo_find_system_command(requests_mock):
    ...
    requests_mock.get(
        f"{EPO_URL}/remote/system.find?%3Aoutput=json&searchText=192.168.1.102", text=epo_find_system_command_response
    )
    requests_mock.get(
        f"{EPO_URL}/remote/system.find?%3Aoutput=json&searchText=192.168.1.103", text=epo_find_system_command_response_key_renamed
    )
    requests_mock.get(f"{EPO_URL}/remote/system.find?%3Aoutput=json&searchText=192.168.1.105", text="OK:\n[ ]")

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {":output": "json", "searchText": "192.168.1.102"}
    ...
    assert result.outputs[0] == {
        ...
        "IsPortable": -1,
        "LastAgentHandler": 1,
        "LastUpdate": "2021-11-21T13:11:42-08:00",
        "ManagedState": 1,
    ...
    args = {":output": "json", "searchText": "192.168.1.105"}
    ...
    args = {":output": "json", "searchText": "192.168.1.102", "verbose": "true"}
    ...
    args = {":output": "json", "searchText": "192.168.1.103"}
...
def test_epo_wakeup_agent_command(requests_mock):
    ...
    """
    requests_mock.get(
        f"{EPO_URL}/remote/system.wakeupAgent?%3Aoutput=json&names=192.168.1.102",
        text='OK:\n"completed: 1\\nfailed: 0\\nexpired: 0"',
    )
    requests_mock.get(
        f"{EPO_URL}/remote/system.wakeupAgent?%3Aoutput=json&names=192.168.1.105",
        text='OK:\n"No systems found to wake up.  Please enter valid computer ids\u002fnames."',
    )

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {":output": "json", "names": "192.168.1.102"}
    ...
    args = {":output": "json", "names": "192.168.1.105"}
...
def test_epo_apply_tag_command(requests_mock):
    ...
    test Passed or Failed
    """
    requests_mock.get(f"{EPO_URL}/remote/system.applyTag?%3Aoutput=json&names=192.168.1.102&tagName=Server", text="OK:\n0")

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {":output": "json", "names": "192.168.1.102", "tagName": "Server"}
...
def test_epo_clear_tag_command(requests_mock):
    ...
    test Passed or Failed
    """
    requests_mock.get(f"{EPO_URL}/remote/system.clearTag?%3Aoutput=json&names=192.168.1.102&tagName=Server", text="OK:\n0")
...
def test_epo_list_tag_command(requests_mock):
    ...
    requests_mock.get(f"{EPO_URL}/remote/system.findTag?%3Aoutput=json&searchText=TIESERVER", text=epo_list_tag_command_response)
    ...
    assert (
        result.readable_output == "### ePO Tags\n|tagId|tagName|tagNotes|\n|---|---|---|\n| 4 | TIESERVER | Apply "
        "Tag to TIEServers |\n"
    )
...
def test_epo_get_tables_command(requests_mock):
    ...
    requests_mock.get(f"{EPO_URL}/remote/core.listTables?%3Aoutput=json&table=Tags", text=epo_get_tables_command_response)
...
def test_epo_query_table_command(requests_mock):
    ...
    requests_mock.get(
        f"{EPO_URL}/remote/core.executeQuery?target=EPOEvents&select=(select EPOEvents.AutoID "
...
def test_epo_get_version_command(requests_mock):
    ...
    Returns:
       test Passed or Failed
    """
    requests_mock.get(f"{EPO_URL}/remote/epo.getVersion?:output=json", text='OK:\n"5.3.2"')
...
def test_epo_move_system_command(requests_mock):
    ...
    Returns:
       test Passed or Failed
    """
    requests_mock.get(f"{EPO_URL}/remote/system.move?names=TIE&parentGroupId=2", text="OK:\ntrue")

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {"names": "TIE", "parentGroupId": "2"}
...
def test_epo_advanced_command(requests_mock):
    ...
    epo_advanced_command_command_response_second = load_test_data_txt("test_data/epo_advanced_command_command_second.txt")

    requests_mock.get(
        f"{EPO_URL}/remote/clienttask.find?searchText=On-demand&%3Aoutput=json", text=epo_advanced_command_command_response
    )

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {"command": "clienttask.find", "commandArgs": "searchText:On-demand"}
    ...
    requests_mock.get(
        f"{EPO_URL}/remote/clienttask.run?names=TIE&productId=ENDP_AM_1000&taskId=28&retryAttempts=2"
...
def test_epo_find_client_task_command(requests_mock):
    ...
    requests_mock.get(
        f"{EPO_URL}/remote/clienttask.find?searchText=endp&%3Aoutput=json", text=epo_find_client_task_command_response
    )

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {"searchText": "endp"}
...
def test_epo_find_policy_command(requests_mock):
    ...
    requests_mock.get(f"{EPO_URL}/remote/policy.find?searchText=endp&%3Aoutput=json", text=epo_find_policy_command_response)
...
def test_epo_assign_policy_to_group_command(requests_mock):
    ...
    """
    requests_mock.get(
        f"{EPO_URL}/remote/policy.assignToGroup?%3Aoutput=json&groupId=2&productId=ENDP_AM_1000"
    ...
    args = {"groupId": "2", "productId": "ENDP_AM_1000", "objectId": "90"}
...
def test_epo_assign_policy_to_system_command(requests_mock):
    ...
    requests_mock.get(
        f"{EPO_URL}/remote/policy.assignToSystem?%3Aoutput=json&names=192.168.1.102&"
        f"productId=ENDP_FW_META&typeId=29&objectId=63",
        text=epo_assign_policy_to_system_response,
    )
    requests_mock.get(
        f"{EPO_URL}/remote/policy.assignToSystem?%3Aoutput=json&names=192.168.1.105&"
        f"productId=ENDP_FW_META&typeId=29&objectId=63",
    ...
    args = {"names": "192.168.1.102", "productId": "ENDP_FW_META", "objectId": "63", "typeId": "29"}
    result = epo_assign_policy_to_system(client, args)

    assert result.outputs is None
    assert result.readable_output.find("Assign policy succeeded") >= 0

    args = {"names": "192.168.1.105", "productId": "ENDP_FW_META", "objectId": "63", "typeId": "29"}
    result = epo_assign_policy_to_system(client, args)
...
def test_epo_list_issues(requests_mock):
    ...
    requests_mock.get(f"{EPO_URL}/remote/issue.listIssues?%3Aoutput=json", text=epo_list_issues_command_response)
...
def test_epo_delete_issue(requests_mock):
    ...
    Returns:
        test Passed or Failed
    """
    requests_mock.get(f"{EPO_URL}/remote/issue.deleteIssue?%3Aoutput=json&id=2", text="OK:\n9")
...
def test_epo_update_issue(requests_mock):
    ...
    Returns:
        test Passed or Failed
    """
    requests_mock.get(f"{EPO_URL}/remote/issue.updateIssue?id=9&name=WISSAM&desc=Test1&state=NEW", text="OK:\n9")

    client = Client(base_url=f"{EPO_URL}/remote", headers={}, auth=("", ""))

    args = {"id": "9", "name": "WISSAM", "description": "Test1", "state": "NEW"}
...
def test_epo_create_issue(requests_mock):
    ...
    """
    requests_mock.get(
        f"{EPO_URL}/remote/issue.createIssue?%3Aoutput=json&name=auto-gen-epo-integration&desc"
    ...
    args = {"name": "auto-gen-epo-integration", "description": "automatically generated by dmst Integration v2"}
