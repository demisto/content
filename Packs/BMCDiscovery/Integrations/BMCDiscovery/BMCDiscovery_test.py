...
def mock_client(mocker, http_request_result=None):
    client = Client(url=URL, verify=False, proxy=False, api_token=VALID_TOKEN)
...
def test_discovery_search_custom_command(mocker):
    mocker.patch.object(
        client, "discovery_search_custom", return_value=util_load_json("test_data/discovery_search_custom_resp.json")
    )
    query = "SEARCH FLAGS(no_segment) DeviceInfo WHERE #DiscoveryResult:DiscoveryAccessResult:DiscoveryAccess:DiscoveryAccess._last_marker TRAVERSE Primary:Inference:InferredElement: SHOW kind(#), name, #InferredElement:Inference:Associate:DiscoveryAccess.endpoint"  # noqa: E501
...
def test_discovery_scan_status_list_command_by_id(mocker):
    mocker.patch.object(
        client, "discovery_scan_status_list", return_value=util_load_json("test_data/discovery_scan_status_list_id_resp.json")
    )
...
def test_discovery_scan_status_list_command_single(mocker):
    mocker.patch.object(
        client, "discovery_scan_status_list", return_value=util_load_json("test_data/discovery_scan_status_list_single_resp.json")
    )
...
def test_discovery_scan_status_list_command_multiple(mocker):
    mocker.patch.object(
        client, "discovery_scan_status_list", return_value=util_load_json("test_data/discovery_scan_status_list_multi_resp.json")
    )
    results = discovery_scan_status_list_command(client)
...
def test_discovery_scan_create_command(mocker):
    mocker.patch.object(client, "discovery_scan_create", return_value=util_load_json("test_data/discovery_scan_create_resp.json"))
...
def test_discovery_scan_summary_command_single(mocker):
    mocker.patch.object(
        client, "discovery_scan_summary", return_value=util_load_json("test_data/discovery_scan_summary_single_resp.json")
    )
    run_id = "f5ac176243062b2b14c1a75a6e446973636f7665727952756e"
...
def test_discovery_scan_summary_command_multiple(mocker):
    mocker.patch.object(
        client, "discovery_scan_summary", return_value=util_load_json("test_data/discovery_scan_summary_multi_resp.json")
    )
...
def test_discovery_search_command_ip_fail(mocker):
    mocker.patch.object(
        client, "discovery_search", return_value=util_load_json("test_data/discovery_search_fail_192.168.11.1_resp.json")
    )
...
def test_discovery_search_command_hostname_multiple(mocker):
    mocker.patch.object(
        client, "discovery_search", return_value=util_load_json("test_data/discovery_search_success_multi_ais-blade_resp.json")
    )
    kind = "Host"
    hostname = "ais-blade"
