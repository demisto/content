...
def test_url_enrichment_script_end_to_end(mocker):
    ...
    command_results = url_enrichment_script(
        url_list=url_list, external_enrichment=True, verbose=True, enrichment_brands=["brand1", "brand2", "WildFire-v2"]
    )
    outputs = command_results.outputs

    # --- Assert ---
    # Convert the list of results to a dictionary for easier access
    enrichment_map = {item["Value"]: item for item in outputs.get("URLEnrichment(val.Value && val.Value == obj.Value)", [])}
