...
class TestArgumentValidationSanitization:
 ...
 'the integrations are enabled.') in str(e.value)
    ...
    class TestFileHash:
        ...
        @pytest.mark.parametrize("unsupported_hash", [
...
class TestEndpointBrandMapper:
    ...
    def test_group_by_brand_end_to_end(self, mocker):
        ...
        mock_response = [
            {
                ...
                'EntryContext': {
                    'EndpointData(val.ID && val.ID == obj.ID)': [
                        {"ID": "xdr-online-1", "Brand": Brands.CORTEX_XDR_IR, "Status": "Online",
                         "Message": "Command successful"},
                        {"ID": "core-online-1", "Brand": Brands.CORTEX_CORE_IR, "Status": "Online",
                         "Message": "Command successful"},
                        {"ID": "offline-1", "Brand": Brands.CORTEX_XDR_IR, "Status": "Offline", "Message": "Command successful"},
                        {"ID": "not-found-1", "Brand": Brands.CORTEX_XDR_IR, "Status": "Unknown", "Message": "Command failed - no endpoint found"},
                        {"ID": "xdr-online-2", "Brand": Brands.CORTEX_XDR_IR, "Status": "Online",
                         "Message": "Command successful"}
                    ]
                }
            }
        ...
        # Assert Grouping
        expected_groups = {
            Brands.CORTEX_XDR_IR: ['xdr-online-1', 'xdr-online-2'],
            Brands.CORTEX_CORE_IR: ['core-online-1']
        }
        ...
        assert not_found_result.message == QuarantineResult.Messages.FAILED_WITH_REASON.format(reason="Command failed - no endpoint found")
        ...
        mock_response = [
            {
                ...
                'EntryContext': {
                    'EndpointData(val.ID && val.ID == obj.ID)': [
                        ...
                        {"ID": "duplicate-id", "Brand": Brands.CORTEX_XDR_IR, "Status": "Online", "Message": "Command successful"},
                        {"ID": "duplicate-id", "Brand": Brands.CORTEX_XDR_IR, "Status": "Offline", "Message": "Command successful"},
                    ]
                }
            }
        ]
    ...
    def test_empty_api_response_raises_exception(self, mocker):
        ...
        args = {"endpoint_id": "any-id"}
        mock_response = [
            {'Type': 1, 'Contents': {}, 'EntryContext': {'EndpointData(val.ID && val.ID == obj.ID)': []}}
        ]
        mocker.patch.object(demisto, 'executeCommand', return_value=mock_response)
        mapper = self._get_mapper(args)
