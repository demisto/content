...
class ContextPaths(Enum):
    ...
    VIRUS_TOTAL_domain = "VirusTotal.domain(val.id && val.id == obj.id)"
...
class Command:
    ...
    def execute(self) -> tuple[list[dict], list[CommandResults]]:
        ...
        execution_results = demisto.executeCommand(self.name, self.args, brand=self.brand)
...
def classify_hashes_by_type(domain_hashes: list[str]) -> dict[str, list[str]]:
    """Creates a dictionary of hash type (key) and hashes list (value)

    Args:
        domain_hashes (list[str]): A list of domain hashes.

    Returns:
        dict[str, list[str]]: Classified domain hashes.
    """
    hashes_by_type: dict[str, list] = defaultdict(list)

    for domain_hash in domain_hashes:
        hash_type = get_hash_type(domain_hash).upper()
        if domain_hash not in hashes_by_type.get(hash_type, []):
            hashes_by_type[hash_type].append(domain_hash)
...
def get_domain_from_ioc_custom_fields(ioc_custom_fields: dict[str, Any]) -> dict[str, Any]:
    """
    Gets the `domain` context dictionary using the `CustomFields` value in the Indicator of Compromise (IOC) record.
    ...
    Returns:
        dict: Transformed `domain` indicator dictionary.
    """
    output: dict[str, Any] = {}
...
def merge_context_outputs(per_command_context: dict[str, dict], include_additional_fields: bool) -> dict[str, list]:
    """
    Merges multiple dictionaries into a single `domainEnrichment` context dictionary.
    ...
    Returns:
        dict[str, list]: A merged `domain` indicator dictionary.
    """
    merged_context_output: dict[str, list] = {ContextPaths.DBOT_SCORE.value: [], ContextPaths.domain_ENRICHMENT.value: []}
    ...
    for command_name, command_context_output in per_command_context.items():
        ...
        for domain_context in command_context_output.get("domainEnrichment", []):
            if not include_additional_fields:
                domain_context.pop("AdditionalFields", {})
            merged_context_output[ContextPaths.domain_ENRICHMENT.value].append(domain_context)

    return assign_params(**merged_context_output)


def find_matching_dbot_score(dbot_scores: list[dict], domain_context: dict[str, Any]) -> dict:
    """
    Finds the associated "DBotScore" object whose indicator value matches any of the hashes if the "domain" Context.
    This is needed since multiple "DBotScore" and "domain" objects can be returned if more than one domain hash is specified.

    Args:
        dbot_scores (list[dict]): List of "DBotScore" objects.
        domain_context (dict[str, Any]): Context output from a single war room result entry.

    Returns:
        dict: The matching "DBotScore" object if found, otherwise an empty dictionary.
    """
    hashes_in_context = {domain_context.get(hash_type, "").casefold() for hash_type in HASH_TYPES if hash_type in domain_context}
...
def execute_domain_reputation(command: Command) -> tuple[dict[str, list], list[CommandResults]]:
    """
    Executes the `!domain` command and transforms the entry context into a standard context output by extracting the `domain` and
    `DBotScore` dictionaries.

    Args:
        command (Command): The `!domain` command to be executed.

    Returns:
        tuple[dict[str, list], list[CommandResults]]: The transformed context output and human-readable command results.
    """
    demisto.debug(f"Starting to execute domain reputation command with args: {command.args}.")
    entry_context, readable_command_results = command.execute()

    demisto.debug(
        "Extracting domain and DBotScore from domain reputation entry context. "
    ...
    for context_item in entry_context:
        ...
        if domains_context := get_from_context(context_item, ContextPaths.domain):
            for domain_context in domains_context:
                dbot_score = find_matching_dbot_score(dbot_scores, domain_context)
                domain_context["Brand"] = dbot_score.get("Vendor", "Unknown")
                domain_context["Score"] = dbot_score.get("Score", Common.DBotScore.NONE)
                context_output["domainEnrichment"].append(domain_context)
...
def execute_wildfire_verdict(command: Command) -> tuple[dict[str, list], list[CommandResults]]:
    ...
    the `domain` and `DBotScore` dictionaries.
...
def execute_ir_hash_analytics(command: Command) -> tuple[dict[str, list], list[CommandResults]]:
    ...
    extracting the `domain` and `DBotScore` dictionaries. It also adds source brand fields to additional entry context items.
...
def enrich_with_command(
    ...
    """
    Calls the relevant domain enrichment command execution function if the command brand has an enabled integration
    ...
    command_execution_function_mapping: dict[str, Callable] = {
        "domain": execute_domain_reputation,
...
def search_domain_indicator(
    ...
    the Indicator of Compromise (IOC) into a standard context output by extracting the `domain` dictionary. It also adds source
...
def run_external_enrichment(
    ...
    """
    Runs the external domain enrichment flow by executing the relevant commands from multiple source brands.

    Args:
        domains (dict[str, list]): The list of domains to enrich.
        brands (list[str]): The enabled brands on the tenant intersected with the given brands to run (if given) enrichment on.
        per_command_context (dict[str, dict]): Dictionary of the entry context (value) of each command name (key).
        verbose_command_results (list[CommandResults]): : List of CommandResults with human-readable output.
    """

    demisto.debug(f"Starting to run external enrichment flow on the following domains: {domains}.")

    # A. Run domain reputation command - using all relevant brands or according to `enrichment_brands` argument
    domain_reputation_command = Command(
        name="domain",
        args=assign_params(**{"domain": domains, "using-brand": ",".join(brands)}),
    )
    enrich_with_command(
        command=domain_reputation_command,
        per_command_context=per_command_context,
        verbose_command_results=verbose_command_results,
    )

    if "Cortex Core - IR" in brands:
        domain_analytics_prevalence_command = Command(
            name="core-get-domain-analytics-prevalence",
            args=assign_params(**{"domain_name": domains}),
        )
        enrich_with_command(
            command=domain_analytics_prevalence_command,
            per_command_context=per_command_context,
            verbose_command_results=verbose_command_results,
        )

    else:
        demisto.debug("Skipping running command 'core-get-domain-analytics-prevalence'.")
...
def summarize_command_results(
    ...
    Args:
        hashes_by_type (dict[str, list]): Dictionary of domain hashes (value) classified by the hash type (key).
        per_command_context (dict[str, dict]): A dictionary of the entry context (value) of each command name (key).
        verbose_command_results (list[CommandResults]): List of CommandResults with human-readable output.
        external_enrichment (bool): Whether to enrich the domain indicator from external source brands.
        include_additional_fields (bool): Whether to ...
    ...
    merged_domain_context = merged_context.get(ContextPaths.domain_ENRICHMENT.value, [])

    demisto.debug(f"Found information on hashes from {len(merged_domain_context)} sources.")

    summaries: list[dict] = []
    # Write summary Message
    for hash_type, domain_hashes in hashes_by_type.items():
        for domain_hash in domain_hashes:
            domain_context_for_hash = [
                d for d in merged_domain_context if hash_type in d and d[hash_type].casefold() == domain_hash.casefold()
            ]

            summary = {"domain": domain_hash}
            summary["Status"] = "Done" if domain_context_for_hash else "Not Found"
            summary["Result"] = result
            tim_score = next(
                iter(context.get("Score") for context in domain_context_for_hash if context.get("Brand") == Brands.TIM.value),
                Common.DBotScore.NONE,
            )
            summary["TIM Verdict"] = DBOT_SCORE_TO_VERDICT.get(tim_score, "Unknown")

            if domain_context_for_hash:
                brands = {domain_context.get("Brand") for domain_context in domain_context_for_hash}
                summary["Message"] = f"Found data on domain from {len(brands)} brands."
                summary["Brands"] = ", ".join(sorted(brands))
            else:
                summary["Message"] = "Could not find data on domain."
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(name=f"domain Enrichment result for {', '.join(domain_hashes)}", t=summaries),
    )
...
def warn_about_invalid_args(
    ...
    """
    Returns a warning war room entry about invalid `domain_hash` and `enrichment_brands` argument values, if found.

    Args:
        invalid_hashes (list[str]): List of domain hashes where the type was classified as "UNKNOWN".
        enrichment_brands (list[str]): List of brand names to run, as given in the `enrichment_brands` argument.
...
def domain_enrichment_script(args: dict[str, Any]) -> list[CommandResults]:
    """
    Implements `!domain-enrichment` script by searching for the Domain indicator in TIM and (optionally) running external enrichment.
    It then outputs a human-readable summary of the command execution results and returns a consolidated merged `Domain` context.
    ...
    Raises:
        ValueError: If the domain hash is not valid.
    ...
    if external_enrichment:
        ...
        if brands:
            demisto.debug(f"Got a list of specified brands to run on, Validating overlap between enrichment brands: {brands} and enabled integration brands.")

            if not (brands := list(set(brands).intersection(enabled_brands))):
        ...
        demisto.debug(f"Running Step 2: External enrichment commands on {domains} using brands: {brands}.")
    ...
    demisto.debug(f"Running Step 3: Summarizing command results on {domain_hashes} and consolidating context output.")
    ...
    if verbose:
        ...
        executed_brands = [
            domain_context.get("Brand")
            for domain_context in summary_command_results.outputs.get(ContextPaths.domain_ENRICHMENT.value, [])  # type: ignore [attr-defined]
        ]
        # Warn about partially invalid `domain_hash` and `enrichment_brands` argument values
...
def main():  # pragma: no cover
    demisto.debug("Stating to run domain-enrichment script.")
    try:
        args = demisto.args()
        command_results = domain_enrichment_script(args)
        return_results(command_results)
        demisto.debug(f"Finishing running domain-enrichment script. Got context output: {command_results[0].outputs}.")

    except Exception as e:
        demisto.error(f"Encountered error during execution of domain-enrichment script: {traceback.format_exc()}.")
        return_error(f"Failed to execute domain-enrichment script. Error: {str(e)}")
