...
class Client(BaseClient):
    ...
    def get_list_with_query(self, list_type: str, start: int | None = None, page_size: int | None = None,
        ...
        new_query = self.rest_api_new_query if list_type == 'incidents' else True
    ...
    def update_incident(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        endpoint = f'/incidents/id/{args["id"]}' if args.get('id', None) else f'/incidents/number/{args["number"]}'
    ...
    def add_filter_to_query(query: str | None, filter_name: str, filter_arg: str,
        ...
        if filter_name and filter_arg:
            ...
            query = f'{query}{filter_name}=={filter_arg}' if use_new_query else f'{query}{filter_name}={filter_arg}'
    ...
    def add_query_to_request(query: str | None, url_suffix: str, new_query: bool,
        ...
        if query:
            ...
            url_suffix = f'{url_suffix}&{query}' if inline_parameters else f'{url_suffix}?{query}'
...
def attachment_upload_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    if isinstance(file_name, list):  # If few files
        file_name = args.get('file_name') if args.get('file_name', None) and args.get('file_name') in file_name else file_name[0]
        ...
        if float(last_fetch_datetime.timestamp()) < float(incident_created_time.timestamp()):
            ...
            for action in actions:
                ...
                if action["operator"]:
                    ...
                    name = "Unknown"
                date_time = entry_date.strftime(DATE_FORMAT) if entry_date else incident_created_time.strftime(DATE_FORMAT)
