...
def client(requests_mock):
    ...
    requests_mock.get("https://test.com/api/version", json={"version": "3.1.4"})
    return Client(base_url="https://test.com/api/", verify=False, auth=("some_username", "some_password"))


@pytest.mark.parametrize(
    "outputs, expected_capitalized_output",
    [
...
def test_list_command(client, requests_mock, command, command_api_url, mock_response, expected_results):
    ...
    """
    requests_mock.get(command_api_url, json=mock_response)
...
@pytest.mark.parametrize(
    "command, args, command_api_url, mock_response, expected_results",
    [
        (
            entry_types_command,
            {"limit": "1"},
        ...
        (
            ...
            "https://test.com/api/incidents/call_types",
            [
                {"id": "1st-id", "name": "call-type-1"},
                {"id": "2st-id", "name": "call-type-2"},
        ...
        (
            ...
            ],
            {
                ...
                "outputs_key_field": "Id",
                "outputs": [{"id": "1st-id-sub", "name": "subcategory-1", "category": {"id": "1st-id", "name": "category-1"}}],
            },
        ),
        (
            escalation_reasons_command,
            ...
            "https://test.com/api/incidents/escalation-reasons",
            [
                {"id": "1st-id", "name": "escalation-name-1"},
                {"id": "2st-id", "name": "escalation-name-2"},
        ...
        (
            ...
            [{"id": "1st-id", "name": "deescalation-name-1"}, {"id": "2st-id", "name": "deescalation-name-2"}],
            {
                ...
                "outputs_key_field": "Id",
                "outputs": [{"id": "1st-id", "name": "deescalation-name-1"}, {"id": "2st-id", "name": "deescalation-name-2"}],
            },
        ),
        (
            archiving_reasons_command,
...
@pytest.mark.parametrize(
    "command, command_api_url, mock_response_file, override_nodes, expected_results",
    [
        (
            list_persons_command,
            ...
            {"outputs_prefix": f"{INTEGRATION_NAME}.Person", "outputs_key_field": "Id"},
        ),
        (
            list_operators_command,
            ...
            {"outputs_prefix": f"{INTEGRATION_NAME}.Operator", "outputs_key_field": "Id"},
        ),
        (
            branches_command,
            ...
            {"outputs_prefix": f"{INTEGRATION_NAME}.Branch", "outputs_key_field": "Id"},
        ),
        (
            get_incidents_list_command,
            ...
            {"outputs_prefix": f"{INTEGRATION_NAME}.Incident", "outputs_key_field": "Id"},
        ),
    ],
)
def test_large_output_list_command(
    client, requests_mock, command, command_api_url, mock_response_file, override_nodes, expected_results
):
    """Unit test
    Given
        - a command that returns a list
    ...
    requests_mock.get(command_api_url, json=mock_topdesk_response)
    command_results = command(client, {})
    assert command_results.outputs_prefix == expected_results["outputs_prefix"]
    assert command_results.outputs_key_field == expected_results["outputs_key_field"]
...
@pytest.mark.parametrize(
    "action, command_args, command_api_url, mock_response_file, override_node",
    [
        (
            "escalate",
...
def test_incident_do_commands(client, requests_mock, action, command_args, command_api_url, mock_response_file, override_node):
    """Unit test
    Given
    ...
    requests_mock.put(command_api_url, json=response_incident)

    command_results = incident_do_command(client=client, args=command_args, action=action)
...
@pytest.mark.parametrize(
    "command_args, command_api_url, command_api_body",
    [
        (
        ...
        ({"id": "incident_id", "file": "some_entry_id"}, "https://test.com/api/incidents/id/incident_id/attachments", {}),
    ],
)
def test_attachment_upload_command(client, mocker, requests_mock, command_args, command_api_url, command_api_body):
    """Unit test
    Given
    ...
    requests_mock.post(command_api_url, json=response_attachment)
    ...
    command_results = attachment_upload_command(client=client, args=command_args)
...
@pytest.mark.parametrize(
    "command_args, command_api_url, response_override",
    [
        (
            ...
            [
                {
                    "id": "attachment-id-1",
                    "downloadUrl": "/api/incidents/id/incident_id/attachments/attachment-id-1",
                    "expected": True,
                }
            ],
        ),
        (
            ...
            [
                ...
                {
                    "id": "attachment-id-2",
                    "downloadUrl": "/api/incidents/id/incident_id/attachments/attachment-id-2",
                    "expected": False,
                },
            ],
        ),
...
def test_attachment_list_command(client, requests_mock, command_args, command_api_url, response_override):
    """Unit test
    Given
        - command args: id, file, description, invisible_for_caller
    When
    ...
    command_results = list_attachments_command(client=client, args=command_args)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "https://test.com/api/incidents/",
            "test_data/topdesk_incident.json",
            {"callerLookup": {"id": "some_caller"}, "briefDescription": "some_change"},
        ),
        (
            True,  # Create
            ...
            "https://test.com/api/incidents/",
            "test_data/topdesk_incident.json",
            {"callerLookup": {"id": "some_caller"}, "briefDescription": "some_change", "category": {"name": "some_category_id"}},
        ),
        (
            False,  # Update
...
def test_caller_lookup_incident_touch_commands(
    client, requests_mock, create_func, command_args, command_api_url, mock_response_file, expected_last_request_body
):
    """Unit test
    Given
    ...
    command_results = incident_touch_command(client=client, args=command_args, client_func=client_func, action=action)
...
def test_non_registered_caller_incident_touch_commands(
    client, requests_mock, create_func, command_args, command_api_url, mock_response_file, expected_last_request_body
):
    """Unit test
    Given
...
@pytest.mark.parametrize(
    "command, command_args, command_api_request",
    [
        (branches_command, {"page_size": 2}, "https://test.com/api/branches?page_size=2"),
        (branches_command, {"start": 2}, "https://test.com/api/branches?start=2"),
        (branches_command, {"query": "id==1st-branch-id"}, "https://test.com/api/branches?query=id==1st-branch-id"),
        (
            branches_command,
        ...
        (branches_command, {"fields": "id,name"}, "https://test.com/api/branches?$fields=id,name"),
        (list_operators_command, {"page_size": 2}, "https://test.com/api/operators?page_size=2"),
        (list_operators_command, {"start": 2}, "https://test.com/api/operators?start=2"),
        (list_operators_command, {"query": "id==1st-operator-id"}, "https://test.com/api/operators?query=id==1st-operator-id"),
        (
            list_operators_command,
        ...
        (list_persons_command, {"page_size": 2}, "https://test.com/api/persons?page_size=2"),
        (list_persons_command, {"start": 2}, "https://test.com/api/persons?start=2"),
        (list_persons_command, {"query": "id==1st-person-id"}, "https://test.com/api/persons?query=id==1st-person-id"),
        (
            list_persons_command,
        ...
        (list_persons_command, {"fields": "id,status"}, "https://test.com/api/persons?$fields=id,status"),
        (get_incidents_list_command, {"page_size": 2}, "https://test.com/api/incidents?page_size=2"),
        (get_incidents_list_command, {"start": 2}, "https://test.com/api/incidents?start=2"),
        (get_incidents_list_command, {"query": "id=1st-incident-id"}, "https://test.com/api/incidents?id=1st-incident-id"),
        (
            get_incidents_list_command,
        ...
        (get_incidents_list_command, {"fields": "id,number"}, "https://test.com/api/incidents?fields=id,number"),
    ],
)
def test_large_output_list_command_with_args(client, requests_mock, command, command_args, command_api_request):
    """Unit test
    Given
...
@pytest.mark.parametrize(
    "command_args, command_api_request, call_count",
    [
        (
            {
                "max_fetch": 2,
            ...
            [
                (
                    "https://test.com/api/incidents?page_size=2&id=1st-incident-id",
                    {"creation_date_start": "2020-02-10T06:32:36Z", "creation_date_end": "2020-03-10T06:32:36Z"},
                )
            ],
            1,
        ),
...
def test_get_incidents_with_pagination(client, requests_mock, command_args, command_api_request, call_count):
    """Unit test
    Given
    ...
    for request in command_api_request:
        requests_mock.get(request[0], json=[{}])
    get_incidents_with_pagination(
        client=client,
...
@pytest.mark.parametrize(
    "command, new_query, command_args, command_api_request",
    [
        (
            list_persons_command,
...
def test_old_new_query(requests_mock, command, new_query, command_args, command_api_request):
    """Unit test
    Given
        - command args
    ...
    requests_mock.get("https://test.com/api/version", json={"version": version})
    client = Client(base_url="https://test.com/api", verify=False, auth=("some_username", "some_password"))
    requests_mock.get(command_api_request, json=[{}])
...
@pytest.mark.parametrize(
    "command_args", [({"category": "blah"}), ({"subcategory": "blah"}), ({"call_type": "blah"}), ({"entry_type": "blah"})]
)
...
@pytest.mark.parametrize(
    "topdesk_incidents_override, last_fetch_time, updated_fetch_time",
    [
        (
            [
            ...
            "2020-01-11T06:32:36.303000+0000",
            "2020-02-10T06:32:36.303000+0000",
        ),
        (
            [
            ...
            "2020-02-11T06:32:36.303000+0000",
            "2020-03-10T06:32:36.303000+0000",
        ),
        (
            [
...
def test_fetch_incidents(client, requests_mock, topdesk_incidents_override, last_fetch_time, updated_fetch_time):
    ...
    requests_mock.get("https://test.com/api/incidents/id/some-test-id-1/actions", json=mock_actions)
    ...
    requests_mock.get("https://test.com/api/incidents", json=mock_topdesk_response)

    last_run = {"last_fetch": last_fetch_time}
    last_fetch, incidents = fetch_incidents(client=client, last_run=last_run, demisto_params={"mirror_direction": "both"})
...
def test_fetch_incidents_with_no_actions(client, requests_mock, topdesk_incidents_override, last_fetch_time, updated_fetch_time):
    ...
    mock_topdesk_incident = util_load_json("test_data/topdesk_incident.json")
    mock_topdesk_response = []
    requests_mock.get("https://test.com/api/incidents/id/some-test-id-1/actions", text="")
...
def test_get_mapping_fields(client):
    ...
    expected_mapping = {"TOPdesk Incident": {"processingStatus": "", "priority": "", "urgency": "", "impact": ""}}
    assert expected_mapping == res.extract_mapping()


@pytest.mark.parametrize(
    "command_args",
    [({"incident_id": "some-id", "incident_number": None}), ({"incident_id": None, "incident_number": "some-number"})],
)
def test_incident_actions_list(client, requests_mock, command_args):
    ...
    if command_args["incident_id"]:
        requests_mock.get("https://test.com/api/incidents/id/some-id/actions", json=mock_incident_actions)
    elif command_args["incident_number"]:
        requests_mock.get("https://test.com/api/incidents/number/some-number/actions", json=mock_incident_actions)

    list_actions_command(client, command_args)
    assert requests_mock.called


@pytest.mark.parametrize("args", [({"id": "some-id", "lastUpdate": "2022-04-26T08:21:48.520Z"})])
def test_get_remote_data_command(client, requests_mock, args):
    ...
    mock_actions = util_load_json("test_data/topdesk_actions.json")
    requests_mock.get("https://test.com/api/incidents/id/some-id", json=mock_incident)
    requests_mock.get("https://test.com/api/incidents/id/some-id/actions", json=mock_actions)
