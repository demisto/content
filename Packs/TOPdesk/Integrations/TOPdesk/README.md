...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| start | The offset at which to start listing the operators at. Must be greater or equal to 0. Default is 0. | Required | 
| page_size | The amount of operators to be returned per request. Must be between 1 and 100. Default is 10. Default is 50. | Required | 
| filter | Supports a set of OData filters which can be combined with 'and' and 'or' operators. Filter expressions can also be grouped by parentheses. https://developers.topdesk.com/explorer/?page=assets#/Assets/getAssets. | Optional | 
| search_term | Search assets by name. | Optional | 
| archived | List only Archived or only Active devices. Leave empty for all. Possible values are: true, false. | Optional | 
| fields | Additional fields to include in the response. Possible values are: archived,etag,id,text. | Optional | 
