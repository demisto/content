...
def get_client_detail_from_context(client):
    ...
    client_detail = {"ClientName": client_name, "Id": client_id}
...
def prepare_filter_payload(args, projection=None):
    ...
    if fieldname or value or operator or exclusive_operator:
        ...
        filters.append({"field": fieldname, "exclusive": exclusive_operator, "operator": operator, "value": value.lower()})
...
def add_filter_to_request(data, additional_fieldname, exclusive_operator, operator, value):
    ...
    filtr = {"field": additional_fieldname, "exclusive": exclusive_operator, "operator": operator, "value": value}
...
def get_host_hr(resp_host):
    ...
    """
    owner = [
        owner.get("value", "") for owner in resp_host.get("configurationManagementDB", []) if owner.get("key", "") == "owned_by"
    ]
    return {
        "ID": resp_host.get("id", ""),
...
def prepare_payload_for_detail_commands(args):
    ...
    field = ""
    value = ""
    argument = {"host": "hostname", "host_id": "id", "application_id": "id", "hostfinding_id": "id"}
...
def get_host_detail_hr(host_detail_dict):
    ...
    hr += tableToMarkdown(
        "Host Details:",
        get_host_details_hr(host_detail_dict),
    ...
    hr += "\n" + tableToMarkdown(
        "Findings Distribution:", findings_distribution, ["Total", "Critical", "High", "Medium", "Low", "Info"], removeNull=True
    )
...
def get_group_detail(groups):
    ...
    :rtype: list
    """
    return [{"ID": group.get("id", ""), "Name": group.get("name", "")} for group in groups]
...
def get_port_detail(ports):
    ...
    :rtype: list
    """
    return [{"ID": port.get("id", ""), "Number": port.get("number", "")} for port in ports]
...
def get_network_detail(network):
    ...
    :rtype: dict
    """
    return {"ID": network.get("id", ""), "Name": network.get("name", ""), "Type": network.get("type", "")}
...
def get_assessment_detail(assessments):
    ...
    """
    return [
        {"ID": assessment.get("id", ""), "Name": assessment.get("name", ""), "Date": assessment.get("date", "")}
...
def get_patch_detail(patches):
    ...
    :rtype: list
    """
    return [{"Name": patch.get("name", ""), "Url": patch.get("url", "")} for patch in patches]
...
def get_icon_detail(icon_details):
    ...
    """
    return [
        {"Type": icon_detail.get("type", ""), "OverlayText": icon_detail.get("overlayText", "")} for icon_detail in icon_details
    ]
...
def get_host_finding_detail_hr(host_finding_detail):
    ...
    hr += tableToMarkdown(
        ...
        get_host_finding_details_hr(host_finding_detail),
    ...
    hr += "\n" + tableToMarkdown(
        "Status:",
        get_host_finding_status_hr(status_detail),
    ...
    hr += "\n" + tableToMarkdown(
        "Tag(s) (" + str(len(tags)) + "):", get_tag_details_hr(tags), ["Name", "Category", "Created", "Updated"], removeNull=True
    )

    manual_report = host_finding_detail.get("manualFindingReports", [])
    hr += "\n" + tableToMarkdown(
        "Manual Finding Report(s) (" + str(len(manual_report)) + "):",
    ...
    hr += "\n" + tableToMarkdown(
        "Ticket(s) (" + str(len(ticket_detail)) + "):",
        get_ticket_detail_hr(ticket_detail),
        ["Ticket Number", "Ticket Status", "Deep Link", "Type", "Connector Name", "Detailed Status"],
        removeNull=True,
    )

    assessment_detail = host_finding_detail.get("assessments", [])
    hr += "\n" + tableToMarkdown(
        "Assessment(s) (" + str(len(assessment_detail)) + "):",
        get_assessment_detail_hr(assessment_detail),
        ["Name", "Date"],
        removeNull=True,
    )
...
def get_app_detail_hr(app_detail_dict):
    ...
    hr += tableToMarkdown(
        "Application Details:",
        get_app_details_hr(app_detail_dict),
    ...
    hr += tableToMarkdown(
        "Tag(s) (" + str(len(app_detail_dict.get("tags", []))) + "):",
    ...
    hr += tableToMarkdown(
        "Ticket(s) (" + str(len(app_detail_dict.get("tickets", []))) + "):",
...
def get_cve_context(cve_list):
    ...
    """
    return [
        {"ID": cve_dict.get("cve", ""), "CVSS": cve_dict.get("baseScore", ""), "Description": cve_dict.get("summary", "")}
...
def get_ticket_context(ticket_list):
    ...
    """
    return [
        {"ID": ticket_dict.get("ticketNumber", ""), "State": ticket_dict.get("ticketStatus", "")} for ticket_dict in ticket_list
    ]
...
def validate_values_for_between_operator(args):
    ...
    if operator == "BETWEEN":
        ...
        if not (
            bool(re.match(REGEX_FOR_INR_OR_FLOAT, values[0]))
...
def prepare_payload_for_create_tag(tag_name, client, propagate_to_all_findings):
    ...
    return {
        "fields": [
            {"uid": "TAG_TYPE", "value": "CUSTOM"},
            {"uid": "NAME", "value": tag_name},
            {"uid": "DESCRIPTION", "value": "Tag Created for " + tag_name},
            {"uid": "OWNER", "value": get_user_id_from_integration_context(client)},
            {"uid": "COLOR", "value": "#648d9f"},
            {"uid": "PROPAGATE_TO_ALL_FINDINGS", "value": propagate_to_all_findings},
        ]
    }
...
def get_apply_tag_context(resp, tag_name):
    ...
    return {"AssociationID": resp.get("id", ""), "Created": resp.get("created", ""), "TagName": tag_name}
...
def get_hosts_command(client, args):
    ...
    if resp and "_embedded" in resp:
        ...
        ec = {"Host(val.ID == obj.ID)": host_context, "RiskSense.Host(val.ID == obj.ID)": risksense_host_context}
    ...
    if resp and "_embedded" in resp:
        ...
        for host_finding in host_findings:
            ...
            hr += tableToMarkdown(
                ...
                get_vulnerabilities_hr(vulnerabilities),
                [
                    "Name",
                    "V2/Score",
                    "Attack Vector",
        ...
        ec = {
            "RiskSense.UniqueVulnerabilities(val.Cve == obj.Cve && val.HostFindingID == obj.HostFindingID)": host_findings_context
        }
...
def get_host_findings_command(client, args):
    ...
    if resp and "_embedded" in resp:
        ...
        for resp_hostfinding in resp_list_hostfinding:
            ...
            # creating Demisto's standard CVE context
            host_finding_cve_context.extend(get_cve_context(resp_hostfinding.get("vulnerabilities", {}).get("vulnInfoList", [])))

        # human Readable
        hr += (
            "### Total " + status.lower() + " host findings: " + str(total_element)
            if status
            else "### Total host findings: " + str(total_element)
        )
        ...
        # context data
        ec = {"RiskSense.HostFinding(val.ID == obj.ID)": risksense_host_finding_context, "Host(val.ID == obj.ID)": host_context}
...
def get_unique_open_findings_command(client, args):
    ...
    if resp and "_embedded" in resp:
        ...
        hr += tableToMarkdown(
            ...
            unique_open_finding_hr,
            ["Title", "Severity", "Asset Count", "Source", "Source ID"],
            removeNull=True,
        )

        ec = {"RiskSense.UniqueHostFinding": unique_open_finding_context}
    else:
...
def get_apps_command(client, args):
    ...
    if resp and "_embedded" in resp:
        ...
        ec = {"RiskSense.Application(val.ID == obj.ID)": apps_context}
...
def get_host_finding_detail_command(client, args):
    ...
    if resp and "_embedded" in resp:
        ...
        host_finding_cve_context = []  # type: List[Dict[str, Any]]
        host_context = []  # type: List[Dict[str, Any]]
        risksense_host_finding_context = [get_risksense_host_finding_context(host_finding_detail)]  # type: List[Dict[str, Any]]
        ...
        # creating Demisto's standard CVE context
        host_finding_cve_context.extend(get_cve_context(host_finding_detail.get("vulnerabilities", {}).get("vulnInfoList", [])))
...
def get_app_detail_command(client, args):
    ...
    if resp and "_embedded" in resp:
        ...
        ec = {"RiskSense.Application(val.ID == obj.ID)": app_detail_context}
        if app_ticket_context:
            ec["Ticket(val.ID == obj.ID)"] = app_ticket_context
    else:
...
def apply_tag_command(client, args):
    ...
    if resp:
        ...
        # Context.
        ec = {"RiskSense.TagAssociation(val.AssociationID == obj.AssociationID)": get_apply_tag_context(resp, tag_name)}
    else:
