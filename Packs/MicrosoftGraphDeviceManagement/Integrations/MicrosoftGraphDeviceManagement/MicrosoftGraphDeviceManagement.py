...
SPECIAL_HEADERS: dict = {"id": "ID", "userId": "User ID", "osVersion": "OS Version", "imei": "IMEI", "meid": "MEID"}
...
class MsGraphClient:
    ...
    def list_managed_devices(
        self, limit: int, page_size: int | None, next_link: str | None = None
    ...
    def wipe_device(
        self, keep_enrollment_data: bool, keep_user_data: bool, mac_os_unlock_code: str, device_id: str, action: str
    ) -> None:
        body: dict = {"keepEnrollmentData": keep_enrollment_data, "keepUserData": keep_user_data}
    ...
    def update_windows_device_account(
        self,
        device_account_password: str,
        password_rotation_enabled: bool,
        calendar_sync_enabled: bool,
        device_account_email: str,
        exchange_server: str,
        session_initiation_protocal_address: str,
        device_id: str,
        action: str,
    ) -> None:
        body: dict = {
            "updateWindowsDeviceAccountActionParameter": {
                ...
                "deviceAccount": {"@odata.type": "microsoft.graph.windowsDeviceAccount", "password": device_account_password},
...
def build_device_object(raw_device: dict) -> dict:
    ...
    """
    device_action_results: list = raw_device.get("deviceActionResults", []) if raw_device.get("deviceActionResults") else []
    conf_manager_client_enabled_features: dict = (
        raw_device.get("configurationManagerClientEnabledFeatures", {})
        if raw_device.get("configurationManagerClientEnabledFeatures")
        else {}
    )
    device_health_attestation_state: dict = (
        raw_device.get("deviceHealthAttestationState", {}) if raw_device.get("deviceHealthAttestationState") else {}
    )
    return assign_params(
        ...
        HealthAttestationState=assign_params(
            ...
            EarlyLaunchAntiMalwareDriverProtection=device_health_attestation_state.get("earlyLaunchAntiMalwareDriverProtection"),
            ...
            SecureBootConfigurationPolicyFingerPrint=device_health_attestation_state.get(
...
def get_managed_device_command(client: MsGraphClient, args: dict) -> None:
    ...
    if device:
        human_readable = tableToMarkdown(
            name=f"Managed device {device_name}",
            t=raw_response,
            headers=HEADERS["raw_device"],
            headerTransform=lambda h: SPECIAL_HEADERS.get(h, pascalToSpace(h)),
            removeNull=True,
...
def get_managed_device_physical_memory_command(client: MsGraphClient, args: dict) -> None:
    ...
    if device:
        human_readable = tableToMarkdown(
            name=f"Managed device {device_name}", t=device, removeNull=True, headers=["PhysicalMemoryInBytes"]
        )
...
def update_windows_device_account_command(client: MsGraphClient, args: dict) -> None:
    ...
    device_id: str = str(args.get("device_id"))
    client.update_windows_device_account(
...
def main():
    ...
    certificate_thumbprint: str = params.get("credentials_certificate_thumbprint", {}).get("password") or params.get(
        "certificate_thumbprint", ""
    )
