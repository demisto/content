...
def panorama_install_file_content_update(version: str, category: str):
    params = {
        'type': 'op',
        'cmd': (f'<request><{category}><upgrade><install><file>{version}</file></install></upgrade></{category}></request>'),
        'key': API_KEY
    }
    result = http_request(
        URL,
...
def panorama_upload_content_update_file_command():
    ...
    try:
        with open(fileName, 'rb') as file:
            ...
            response = requests.request("POST", url, data=payload, headers=headers, params=params, files=files, verify=False)
