...
PRIORITY_TO_SEVERITY_MAP = {"5": 0, "4": 1, "3": 2, "2": 3, "1": 4}
...
ALL_INSTANCE_TYPE: dict[str, str] = {"all_classes": "All Classes", "asset_classes": "Asset Classes", "ci_classes": "CI Classes"}
...
INCIDENT_PREFIX = {"Incident": "IN", "Service Request": "SR"}

QUEUE_TYPES = {"Incident/Service Request": "BMCServiceDesk__Incident__c"}
...
class Client(BaseClient):
    ...
    def http_request(self, method, url_suffix, headers=None, json_data=None, params=None):
        ...
        default_headers = {"Authorization": f"Bearer {session_id}"}
        ...
        with http_exception_handler():
            resp = self._http_request(
                ...
                resp_type="response",
                ok_codes=(200, 201, 202, 204, 400, 401, 403, 404, 500),
    ...
    def get_salesforce_session(self):
        ...
        with http_exception_handler():
            return self._http_request(
                "POST",
                "",
                full_url=SOAP_LOGIN_URL,
                data=request_payload,
                headers=headers,
                timeout=self._request_timeout,
                ok_codes=(200, 201, 202, 204, 400, 401, 404, 500),
                resp_type="response",
            )
...
def prepare_query_for_fetch_incidents(params: dict[str, str], start_time: int) -> str:
    ...
    if params.get("query", ""):
        ...
        if where_count > 1:
            ...
            if query.count("from"):
                ...
                return query[:from_search_end] + f" where LastModifiedDate > {start_time}" + query[from_search_end:]
        ...
        return query[:where_search_end] + f" LastModifiedDate > {start_time} and" + query[where_search_end:]
    ...
    return SALESFORCE_QUERIES["FETCH_INCIDENT_QUERY"].format(fields, *fetch_type, start_time, max_incidents)
...
def prepare_date_or_markdown_fields_for_fetch_incidents(fields: dict[str, Any]) -> None:
    ...
    fields["BMCServiceDesk__closeDateTime__c"] = prepare_iso_date_string(fields.get("BMCServiceDesk__closeDateTime__c", ""))
    fields["BMCServiceDesk__dueDateTime__c"] = prepare_iso_date_string(fields.get("BMCServiceDesk__dueDateTime__c", ""))
    ...
    fields["BMCServiceDesk__openDateTime__c"] = prepare_iso_date_string(fields.get("BMCServiceDesk__openDateTime__c", ""))
    ...
    fields["Attachments"] = tableToMarkdown(
        "", fields.get("attachments", []), headers=["File", "Download Link", DATE_AND_TIME, "Created By"]
    )
    fields["Notes"] = tableToMarkdown(
        "", fields.get("notes", []), ["Incident History ID", "Action~", DATE_AND_TIME, "Sender", "Description", "Note"]
    )
...
def prepare_incident_for_fetch_incidents(record: dict[str, Any], params: dict[str, Any]) -> dict[str, Any]:
    ...
    incident = {"name": name, "rawJSON": json.dumps(record), "details": json.dumps(record) if params.get("query", "") else ""}
...
def prepare_query_for_queue_details_get(args: dict[str, Any]) -> str:
    ...
    if queue_type:
        ...
        return (
            SALESFORCE_QUERIES["GET_QUEUE_DETAIL_FOR_SPECIFIC_TYPE"].format(QUEUE_TYPES.get(queue_type, queue_type)) + queue_name
        )
...
def prepare_queue_details_get_output(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [
        {
            "Id": record.get("QueueId", "") if record.get("QueueId", "") else record.get("Id", ""),
...
def process_single_service_request_definition(res) -> dict:
    ...
    filtered_questions = [
        {key: value for (key, value) in question.items() if key in required_question_fields}
    ...
    fields = {
        ...
        if "Name" in field and "Value" in field and field["Name"].strip().lower() not in standard_key_set
    }
...
def process_single_service_request_definition_output(res) -> dict:
    ...
    for question in res.get("Questions", []):
        questions += (
            ...
            + question.get("Text", "")
            + "\nIs Required: "
            + ("Yes" if question.get("IsRequired", False) else "No")
        )
...
def prepare_context_for_get_service_request_definitions(resp: dict) -> list:
    ...
    return [
        process_single_service_request_definition(resp.get("Result"))
        if isinstance(resp.get("Result"), dict)
        else process_single_service_request_definition(res)
...
def input_data_create_note(summary: str, notes: str) -> dict:
    ...
    :return Dict
    """
    return {"ActivityLog": [{"Summary": summary, "Notes": notes}]}
...
def create_service_request(
    client: Client,
    service_request_definition: str,
    ...
    body = {
        "Fields": [
            {"Name": "requestDefinitionId", "Value": service_request_definition},
            {"Name": "client", "Value": client_id if client_id else ""},
        ],
        "Answers": answers,
    }
    headers = {"Content-Type": "application/json"}
    response = client.http_request(method="POST", url_suffix=URL_SUFFIX["SERVICE_REQUEST"], headers=headers, json_data=body)
    if response and response.get("Success"):
        ...
        if resp and resp.get("message"):
            ...
            hr_output = {OUTPUT_PREFIX["SERVICE_REQUEST_WARNING"]: outputs}

            return_warning(
                message=markdown_message, exit=True, warning=markdown_message, outputs=hr_output, ignore_auto_extract=True
            )
...
def update_service_request(
    ...
    response = client.http_request(method="GET", url_suffix=URL_SUFFIX["SALESFORCE_QUERY"], params={"q": endpoint_to_get_id})
    ...
    resp = update_incident(client, service_request_id, params=request_params)
    ...
    if resp.get("message"):
        ...
        context_output = {OUTPUT_PREFIX["SERVICE_REQUEST_WARNING"]: resp["outputs"]}

        return_error(message=readable_output, error=readable_output, outputs=context_output)
    else:
        if excluded_fields:
            markdown_message = "{}".format(MESSAGES["UPDATE_SERVICE_REQUEST_WARNING"]).format(
                service_request_number, ", ".join(excluded_fields), MESSAGES["UNEXPECTED_ERROR"]
            )
            outputs = {OUTPUT_PREFIX["SERVICE_REQUEST_WARNING"]: resp["outputs"]}

            return_warning(
                message=markdown_message, exit=True, warning=markdown_message, outputs=outputs, ignore_auto_extract=True
            )
        else:
            return_results(
                CommandResults(
                    ...
                    readable_output="{}".format(HR_MESSAGES["SERVICE_REQUEST_UPDATE_SUCCESS"]).format(service_request_number),
                )
            )
...
def update_incident(client: Client, incident_id: str, params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        http_response = client.http_request(method="PATCH", url_suffix=endpoint, headers=headers, json_data=body)
...
def create_hr_context_output(result: list) -> list:
    ...
    for result_row in result:
        hr_context_output_list.append({"Id": result_row.get("Id", ""), "Name": result_row.get("Name", "")})
...
def get_update_incident_payload(args: dict[str, str]) -> tuple[dict[str, Any], list[str]]:
    ...
    update_request_body = {
        MAPPING_OF_FIELDS_WITH_SALESFORCE_COLUMNS[key]: value
        for (key, value) in args.items()
        if len(value.strip()) > 0 and key in MAPPING_OF_FIELDS_WITH_SALESFORCE_COLUMNS and key != "additional_fields"
    }
...
def validate_incident_update_payload(payload: dict[str, Any]) -> None:
    ...
    """
    opened_date = validate_and_get_date_argument(payload, MAPPING_OF_FIELDS_WITH_SALESFORCE_COLUMNS["opened_date"], "opened_date")
    due_date = validate_and_get_date_argument(payload, MAPPING_OF_FIELDS_WITH_SALESFORCE_COLUMNS["due_date"], "due_date")
    outage_start = validate_and_get_date_argument(
        payload, MAPPING_OF_FIELDS_WITH_SALESFORCE_COLUMNS["outage_start"], "outage_start"
    )
    outage_end = validate_and_get_date_argument(payload, MAPPING_OF_FIELDS_WITH_SALESFORCE_COLUMNS["outage_end"], "outage_end")
...
def create_asset_query(asset_name: str, instance_type: str) -> str:
    ...
    if instance_type == ALL_INSTANCE_TYPE["asset_classes"]:
        ...
        append_query = append_query + f"and BMCServiceDesk__InstanceType__c='{instance_type}' "
...
def process_attachment_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    download_url = demisto.params()["url"] + URL_SUFFIX["DOWNLOAD_ATTACHMENT"].format(record.get("ContentDocumentId", ""))
    attachment = {
...
def get_attachments_for_incident(client: Client, incident_id: str) -> list[dict[str, Any]]:
    ...
    response = client.http_request(
        "GET", url_suffix=URL_SUFFIX["SALESFORCE_QUERY"], params={"q": SALESFORCE_QUERIES["GET_ATTACHMENTS"].format(incident_id)}
    )
...
def get_notes_for_incident(client: Client, incident_number: str) -> list[dict[str, Any]]:
    ...
    if not incident_number or len(incident_number.strip()) < 1:
        return notes
    response = client.http_request(
        "GET", url_suffix=URL_SUFFIX["SALESFORCE_QUERY"], params={"q": SALESFORCE_QUERIES["GET_NOTES"].format(incident_number)}
    )
...
@logger
def fetch_incidents(
    client: Client, params: dict[str, Any], last_run: dict[str, Any], first_fetch: int
        ...
        if isinstance(safe_load_json(additional_fields), dict):
            ...
            if invalid_fields:
                raise AttributeError(
                    "{}".format(MESSAGES["INVALID_FIELDS_ERROR"]).format(", ".join(invalid_fields), "additional_fields")
                )
        else:
            raise ValueError(
                "{}".format(MESSAGES["INVALID_FORMAT_ERROR"]).format("additional_fields", "field_id1=value_1; field_2=value_2")
            )
    update_service_request(
        client, service_request_number, excluded_fields=excluded_fields, additional_fields=additional_fields, default_args=args
    )
...
def bmc_remedy_create_service_request_command(client: Client, args: dict[str, str]) -> None:
    ...
    answers, excluded_answers = get_valid_arguments(
    ...
    if answers and isinstance(safe_load_json(answers), dict):
        ...
        raise ValueError(
            "{}".format(MESSAGES["INVALID_FORMAT_ERROR"]).format(
...
def bmc_remedy_incident_create_command(client: Client, args: dict[str, str]) -> None:
    ...
    if not api_response.get("Success", False) or not create_result or "Id" not in create_result:
        raise DemistoException(
            HR_MESSAGES["CREATE_INCIDENT_FAILURE"].format(api_response.get("ErrorMessage", MESSAGES["UNEXPECTED_ERROR"]))
        )

    try:
        id_suffix = "/{}".format(create_result.get("Id", ""))
        update_api_response = client.http_request("PATCH", URL_SUFFIX["UPDATE_INCIDENT"] + id_suffix, json_data=update_payload)
        ...
        readable_output = HR_MESSAGES["CREATE_INCIDENT_WARNING"].format(
            create_result.get("Number", ""), ", ".join(fields), str(e)
        )
        warning_output = create_result
        context_output = {OUTPUT_PREFIX["INCIDENT_WARNING"]: warning_output}
        demisto.error(MESSAGES["TRACEBACK_MESSAGE"].format(demisto.command()) + traceback.format_exc())  # print the traceback
        return_warning(
            message=readable_output, exit=True, warning=readable_output, outputs=context_output, ignore_auto_extract=True
        )
...
def bmc_remedy_incident_update_command(client: Client, args: dict[str, str]) -> None:
    ...
    context_output = {"Id": incident_id, "Number": incident_number}
...
def bmc_remedy_service_request_definition_get_command(client: Client, args: dict[str, str]) -> CommandResults | None:
    ...
    if "service_request_definition_name" in args:
        ...
        if service_request_definition_id == "":
            return CommandResults(
                readable_output=HR_MESSAGES["NOT_FOUND_SERVICE_REQUEST_DEF"].format(args.get("service_request_definition_name"))
            )
        else:
            service_request_definition_suffix = "/" + service_request_definition_id

    # call api
    api_response = client.http_request(
        "GET", url_suffix=URL_SUFFIX["GET_SERVICE_REQUEST_DEFINITION"] + service_request_definition_suffix
    )
    ...
    if not success:
        ...
        # prepare output
        output_header = MESSAGES["GET_OUTPUT_MESSAGE"].format(
            "service request definition(s)",
            1 if isinstance(api_response.get("Result"), dict) else len(api_response.get("Result", [])),
        )
        ...
        readable_output = tableToMarkdown(
            output_header,
            output_content,
            headers=["Service Request Definition Id", "Service Request Definition Name", "Questions"],
            removeNull=True,
        )
...
def bmc_remedy_template_details_get_command(client: Client, args: dict[str, str]) -> CommandResults | str | None:
    ...
    params = {"q": query + SALESFORCE_QUERIES["ORDER_BY_NAME"]}
    url_suffix = URL_SUFFIX.get("SALESFORCE_QUERY", "")
...
def bmc_remedy_service_offering_details_get_command(client: Client, args: dict) -> CommandResults | str | None:
    ...
    if result:
        ...
        readable_output = tableToMarkdown(
            HR_MESSAGES["GET_COMMAND_DETAILS_SUCCESS"].format("service offering(s)", len(service_offering_result_list)),
...
def bmc_remedy_asset_details_get_command(client: Client, args: dict[str, str]) -> CommandResults | str | None:
    ...
    if result:
        ...
        readable_output = tableToMarkdown(
            ...
            headers=["Id", "Name", "Description", "Asset #", "Class Name", "Instance Type"],
            removeNull=True,
        )
        return CommandResults(
...
def bmc_remedy_impact_details_get_command(client: Client, args: dict[str, str]) -> CommandResults | str | None | None:
    ...
    api_response = client.http_request("GET", url_suffix=URL_SUFFIX["SALESFORCE_QUERY"], params={"q": endpoint_to_get_impacts})
    ...
    if records:
        ...
        readable_output = tableToMarkdown(markdown, outputs, headers=["Id", "Name"], removeNull=True)
        return CommandResults(
...
def bmc_remedy_account_details_get_command(client: Client, args: dict[str, str]) -> CommandResults | str | None:
    ...
    if account_name:
        query = query + f" and name ='{account_name}'"
    ...
    if result:
        ...
        readable_output = tableToMarkdown(
            HR_MESSAGES["GET_COMMAND_DETAILS_SUCCESS"].format("account(s)", len(result)),
            accounts_result_list,
            headers=["Id", "Name"],
            removeNull=True,
        )
        return CommandResults(
...
def bmc_remedy_urgency_details_get_command(client: Client, args: dict[str, str]) -> CommandResults | str | None:
    ...
    if result:
        ...
        readable_output = tableToMarkdown(
            HR_MESSAGES["GET_COMMAND_DETAILS_SUCCESS"].format("urgencies", len(urgency_result_list)),
            urgency_result_list,
            headers=["Id", "Name"],
            removeNull=True,
        )
...
def bmc_remedy_category_details_get_command(client: Client, args: dict[str, str]) -> CommandResults | str | None | None:
    ...
    if category_name:
        endpoint_to_get_category = f"{endpoint_to_get_category} and name='{category_name}'"
    ...
    if category_type in POSSIBLE_CATEGORY_TYPES:
        ...
        raise ValueError(
            "{}".format(MESSAGES["INVALID_TYPE_FOR_CATEGORIES"]).format("type", "type", ", ".join(POSSIBLE_CATEGORY_TYPES))
        )

    api_response = client.http_request("GET", url_suffix=URL_SUFFIX["SALESFORCE_QUERY"], params={"q": endpoint_to_get_category})
    records = api_response.get("records")
    if records:
        ...
        readable_output = tableToMarkdown(markdown, hr_output, headers=["Id", "Name", "Children Count"], removeNull=True)
        return CommandResults(
...
def bmc_remedy_queue_details_get_command(client: Client, args: dict[str, Any]) -> CommandResults | str:
    ...
    readable_output = tableToMarkdown(
        HR_MESSAGES["GET_COMMAND_DETAILS_SUCCESS"].format("queue(s)", response.get("totalSize", 0)),
        custom_ec,
        ["Id", "Name", "Email"],
        removeNull=True,
    )
    return CommandResults(
        outputs_prefix=OUTPUT_PREFIX["QUEUE"],
        outputs_key_field="Id",
        outputs=custom_ec,
        readable_output=readable_output,
        raw_response=response,
...
def bmc_remedy_user_details_get_command(client: Client, args: dict[str, Any]) -> CommandResults | str:
    ...
    custom_ec = createContext(output, removeNull=True)
    readable_output = tableToMarkdown(
        HR_MESSAGES["GET_COMMAND_DETAILS_SUCCESS"].format("user(s)", response.get("totalSize", 0)),
        custom_ec,
        [
            "Id",
            "Username",
...
def bmc_remedy_broadcast_details_get_command(client: Client, args: dict[str, str]) -> CommandResults | str | None:
    ...
    response = client.http_request(
        method="GET", url_suffix=URL_SUFFIX["SALESFORCE_QUERY"], params={"q": endpoint_to_get_broadcast}
    )

    if response.get("records"):
        output = prepare_broadcast_details_get_output(response.get("records"))
        custom_ec = createContext(output, removeNull=True)
        markdown_message = HR_MESSAGES["GET_COMMAND_DETAILS_SUCCESS"].format("broadcast(s)", len(response.get("records")))
        readable_output = tableToMarkdown(
            markdown_message,
            custom_ec,
        ...
        return CommandResults(
            outputs_prefix=OUTPUT_PREFIX["BROADCAST"],
            outputs_key_field="Id",
            outputs=custom_ec,
            readable_output=readable_output,
            raw_response=response,
...
def bmc_remedy_incident_get_command(client: Client, args: dict[str, str]) -> CommandResults | str | None:
    ...
    if maximum_incident:
        ...
        if not (1 <= int(maximum_incident_int) <= 500):
            raise ValueError(MESSAGES["MAX_INCIDENT_LIMIT"].format("maximum_incident"))
        final_query = final_query + f" LIMIT {maximum_incident_int}"
    ...
    if response and response.get("records", ""):
        ...
        readable_output = tableToMarkdown(
            HR_MESSAGES["GET_COMMAND_DETAILS_SUCCESS"].format("incident(s)", len(incident_result_output)),
...
def bmc_remedy_service_request_get_command(client: Client, args: dict[str, Any]) -> CommandResults | str | None:
    ...
    if response and response.get("records"):
        ...
        markdown_message = HR_MESSAGES["GET_COMMAND_DETAILS_SUCCESS"].format("service request(s)", len(records))
        readable_output = tableToMarkdown(
            markdown_message,
            custom_ec,
            headers=["Number", "Priority", "Description", "ClientID", "Status", "Staff", "Queue"],
