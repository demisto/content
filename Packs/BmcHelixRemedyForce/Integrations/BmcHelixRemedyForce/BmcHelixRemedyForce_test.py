...
import pytest
import requests
from BmcHelixRemedyForce import (
    DISPLAY_DATE_FORMAT,
    HR_MESSAGES,
    INCIDENT_CATEGORY_OBJECT,
    MESSAGES,
    OUTPUT_PREFIX,
    ...
    Client,
    DemistoException,
    bmc_remedy_account_details_get_command,
    bmc_remedy_asset_details_get_command,
    bmc_remedy_broadcast_details_get_command,
    bmc_remedy_category_details_get_command,
    bmc_remedy_create_service_request_command,
    bmc_remedy_impact_details_get_command,
    bmc_remedy_service_offering_details_get_command,
    bmc_remedy_service_request_get_command,
    bmc_remedy_template_details_get_command,
    bmc_remedy_update_service_request_command,
    bmc_remedy_urgency_details_get_command,
    date_to_timestamp,
    get_request_params,
    get_service_request_details,
    get_valid_arguments,
    prepare_outputs_for_get_service_request,
    remove_prefix,
    update_incident,
)
...
def prepare_expected_response_for_fetch_incidents():
    ...
    """
    from BmcHelixRemedyForce import prepare_date_or_markdown_fields_for_fetch_incidents, remove_empty_elements
...
class BmcHelixRemedyForceTestCase(unittest.TestCase):
    ...
    msg_for_invalid_format = "{}".format(MESSAGES["INVALID_DATA_FORMAT"]).format("additional_fields")
    ...
    def test_get_session_id_valid_context(self, mocker_set_context, mocker_session, mocker_get_context):
        ...
        mocker_get_context.return_value = {"sessionId": self.valid_session_id, "validUntil": some_future_time}
        mocker_set_context.return_value = {}
    ...
    def test_http_request_unauthorized_response(
        self, mocker_set_context, mocker_get_context, mocker_get_session_id, mocker_http_request
    ):
        """
    ...
    def test_http_request_bad_request(self, mocker_get_session_id, mocker_http_request):
        ...
        response_data = [{"message": "custom bad request error message"}]
    ...
    def test_get_request_timeout_valid(self, mocker_params):
        ...
        :return: None
        """
        mocker_params.return_value = {"request_timeout": 25}
    ...
    def test_get_request_timeout_invalid(self, mocker_params):
        ...
        :return: None
        """
        mocker_params.return_value = {"request_timeout": "25asd"}
    ...
    def test_get_request_timeout_exceeding(self, mocker_params):
        ...
        :return: None
        """
        mocker_params.return_value = {"request_timeout": 92233720361}
    ...
    @patch("demistomock.params")
    def test_main_success(self, mocker_params, mocker_return_results, mocker_http_request, mocker_command, mocker_sr_results):
        """
        ...
        mocker_params.return_value = {"request_timeout": 50, "credentials": {}, "url": "some_base_url", "first_fetch": "10 hours"}
    ...
    def test_main_failure(self, mocker_params, mocker_return_error, mocker_http_request, mocker_command, mocker_error):
        ...
        mocker_params.return_value = {"request_timeout": 50, "credentials": {}, "url": "some_base_url"}
    ...
    def test_prepare_query_for_fetch_incidents(self):
        ...
        """
        from BmcHelixRemedyForce import SALESFORCE_QUERIES, prepare_query_for_fetch_incidents, timestamp_to_datestring

        # Without query
        params = {"category": "category", "impact": "impact", "max_fetch": "10", "type": "Incident"}
        fields = "BMCServiceDesk__Category_ID__c='category' and BMCServiceDesk__Impact_Id__c='impact' and "
        start_time = 1594250101000
        fetch_type = ("true", "Yes")
        assert prepare_query_for_fetch_incidents(params, start_time) == SALESFORCE_QUERIES["FETCH_INCIDENT_QUERY"].format(
    ...
    def test_fetch_incidents(self, http_request, demisto_params_url, mocker_get_attachments, mocker_sr_details):
        ...
        # With first_fetch
        next_run, incidents = fetch_incidents(client=self.client, params=params, last_run={}, first_fetch=1594250101000)
        assert next_run == {"start_time": 1594250101000}

        assert incidents == expected_incidents

        # With last_run
        next_run, incidents = fetch_incidents(
            client=self.client, params=params, last_run={"start_time": 1582584487}, first_fetch=0
        )
        assert next_run == {
            "start_time": date_to_timestamp(
                json.loads(expected_incidents[0]["rawJSON"])["LastModifiedDate"], date_format="%Y-%m-%dT%H:%M:%S.%f%z"
            )
        }
    ...
    def test_input_data_create_note(self):
        ...
        notes = "test note"
        expected_output = {"ActivityLog": [{"Summary": summary, "Notes": notes}]}
    ...
    def test_remedy_note_create_command_success(self, mocker_id_from_incident_number, mocker_http_request):
        ...
        mocker_http_request.return_value = expected_res
        args = {"request_number": "IN00000123", "summary": "Summary", "notes": "Notes"}
    ...
    @patch("BmcHelixRemedyForce.Client.http_request")
    @patch("BmcHelixRemedyForce.get_id_from_incident_number")
    def test_remedy_note_create_command_failed_raise_exception_with_message(
        self, mocker_id_from_incident_number, mocker_http_request
    ):
        """
        ...
        mocker_id_from_incident_number.return_value = "a123bcd"
        args = {"summary": "Summary", "notes": "Notes"}
    ...
    def test_bmc_remedy_template_details_get_command_success(self, mock_http_res):
        ...
        BmcHelixRemedyForce.create_template_output = MagicMock(side_effect=BmcHelixRemedyForce.create_template_output)
    ...
    def test_update_incident_exception(self, req_obj, mocker_get_session_id):
        ...
        params = {"category_id": "abc", "broadcast_id": None}
        expected_response = {"outputs": {}, "message": MESSAGES["AUTHENTICATION_ERROR"]}
        actual_response = update_incident(self.client, "123", params=params)
    ...
    @patch("BmcHelixRemedyForce.Client.get_session_id")
    @patch("requests.Session.request")
    def test_bmc_remedy_update_service_request_command_invalid_additional_data(self, req_obj, mocker_get_session_id, get_mock):
        """
        ...
        try:
            ...
            msg = "{}".format(MESSAGES["INVALID_FORMAT_ERROR"]).format("additional_fields", "field_id1=value_1; field_2=value_2")
            assert str(e) == msg
    ...
    def test_update_service_request(self, req_obj, mocker_get_session_id, mocker_update_incident, mocker_context):
        ...
        """
        mocker_update_incident.return_value = {"outputs": {"key": "value"}}
        mocker_get_session_id.return_value = self.valid_session_id
        resp1 = Mock(spec=requests.Response)
        resp1.status_code = 200
        ...
        for call in mocker_context.call_args_list:
            ...
            assert args[0].readable_output == "{}".format(HR_MESSAGES["SERVICE_REQUEST_UPDATE_SUCCESS"]).format("345")
    ...
    def test_update_service_request_warning(
        self, req_obj, mocker_get_session_id, mocker_update_incident, get_mock, mocker_return_warning, mocker_error
    ):
        """
    ...
    @patch("BmcHelixRemedyForce.Client.get_session_id")
    @patch("requests.Session.request")
    def test_update_service_request_failure(self, req_obj, mocker_get_session_id, mocker_update_incident, mocker_context):
        """
        ...
        :param mocker_context: mocker object of return_results method of CommonServerPython
        """
        mocker_update_incident.return_value = {"outputs": {"key": "value"}, "message": "Failure message"}
        mocker_get_session_id.return_value = self.valid_session_id
        resp1 = Mock(spec=requests.Response)
        resp1.status_code = 200
        resp1.json = fetch_dummy_response
    ...
    @patch("BmcHelixRemedyForce.Client.get_session_id")
    @patch("requests.Session.request")
    def test_create_service_request(self, req_obj, mocker_get_session_id, mocker_update_incident, mocker_context, mocker_error):
        """
        ...
        for call in mocker_context.call_args_list:
            ...
            assert args[0].readable_output == "{}".format(HR_MESSAGES["SERVICE_REQUEST_CREATE_SUCCESS"]).format("00000132")
    ...
    @patch("BmcHelixRemedyForce.Client.get_session_id")
    @patch("requests.Session.request")
    def test_create_service_request_failure(self, req_obj, mocker_get_session_id, mocker_update_incident, mocker_context):
        """
    ...
    def test_create_service_request_fail_to_update(
        self, req_obj, mocker_get_session_id, mocker_update_incident, mocker_context, mocker_return_warning, mocker_error
    ):
        """
        ...
        :param mocker_error: mocker object for demistomock.error
        """
        mock_return_value = {"outputs": {"key": "value"}, "message": "Fail to update"}
        ...
        resp1.json = fetch_dummy_response
        req_obj.side_effect = [resp1]
        args = {"service_request_definition_id": "345", "service_request_definition_params": "abc=xyz", "category_id": "sd"}
        bmc_remedy_create_service_request_command(self.client, args)
        for call in mocker_return_warning.call_args_list:
            args, kwargs = call
    ...
    def test_create_service_request_invalid_additional_fields(
        ...
        resp1.json = fetch_dummy_response
        req_obj.side_effect = [resp1]
        args = {"service_request_definition_id": "345", "additional_fields": "broadcast_id= ;service_id="}
        try:
            bmc_remedy_create_service_request_command(self.client, args)
        except Exception as e:
    ...
    def test_get_request_params(self):
        ...
        """
        data = {"category_id": "abc", "queue_id": "xyz", "client_id": "def"}
    ...
    def test_bmc_remedy_service_offering_details_get_command_success(self, mock_http_res):
        ...
        BmcHelixRemedyForce.create_hr_context_output = MagicMock(side_effect=BmcHelixRemedyForce.create_hr_context_output)

        with open("TestData/get_service_offering_details.json", encoding="utf-8") as f:
            expected_res = json.load(f)

        mock_http_res.return_value = expected_res
        args = {"service_offering_name": "name"}
    ...
    def test_bmc_remedy_service_offering_details_get_command_no_record_found(self, mock_http_res):
        ...
        mock_http_res.return_value = expected_res
        args = {"service_offering_name": "sof1"}
    ...
    def test_bmc_remedy_incident_create_command_warning(self, mocker_http_request, mocker_return_warning, mocker_error):
        ...
        bmc_remedy_incident_create_command(
            self.client, {"client_id": "SOME_CLIENT_ID", "additional_fields": "service_id=SOME_ID"}
        )
        ...
        for call in mocker_return_warning.call_args_list:
            args, kwargs = call
            expected_msg = HR_MESSAGES["CREATE_INCIDENT_WARNING"].format(
                "SUCCESS_NUMBER", "client_id, service_id", "SOME_ISSUE_WITH_UPDATE"
            )
    ...
    def test_bmc_remedy_incident_create_command_input_error(self):
        ...
        with pytest.raises(DemistoException) as e:
            bmc_remedy_incident_create_command(self.client, {"client_id": "CLIENT_ID", "additional_fields": "description=test"})
    ...
    def test_bmc_remedy_asset_details_get_command_success(self, mock_http_res):
        ...
        BmcHelixRemedyForce.create_asset_output = MagicMock(side_effect=BmcHelixRemedyForce.create_asset_output)

        with open("TestData/get_assets_details_records.json", encoding="utf-8") as f:
            expected_res = json.load(f)

        mock_http_res.return_value = expected_res
        args = {"assets_name": "ADP", "instance_type": "Asset Classes"}
    ...
    def test_bmc_remedy_asset_details_get_command_no_record_found(self, mock_http_res):
        ...
        mock_http_res.return_value = expected_res
        args = {"asset_name": "aset1", "instance_type": "CI Classes"}
    ...
    def test_bmc_remedy_category_details_get_command_invalid_type(self):
        ...
        try:
            ...
            assert str(e) == "{}".format(MESSAGES["INVALID_TYPE_FOR_CATEGORIES"]).format(
    ...
    def test_bmc_remedy_category_details_get_command_invalid_type_with_category(self, req_obj, mocker_get_session_id):
        ...
        req_obj.side_effect = [resp1]
        args = {"type": "abc", "category_name": "category"}
        try:
            bmc_remedy_category_details_get_command(self.client, args)
        except ValueError as e:
            assert str(e) == "{}".format(MESSAGES["INVALID_TYPE_FOR_CATEGORIES"]).format(
    ...
    def test_bmc_remedy_queue_details_get_command(self, http_request, demisto_args):
        ...
        args = {"type": "Incident/Service Request"}
    ...
    def test_prepare_query_for_queue_details_get():
        ...
        """
        from BmcHelixRemedyForce import QUEUE_TYPES, SALESFORCE_QUERIES, prepare_query_for_queue_details_get

        # Without type and queue_name
        assert prepare_query_for_queue_details_get({}) == SALESFORCE_QUERIES["GET_QUEUE_DETAIL"].format("")

        # With queue name
        assert prepare_query_for_queue_details_get({"queue_name": "queue1"}) == SALESFORCE_QUERIES["GET_QUEUE_DETAIL"].format(
            " and name = 'queue1'"
        )
        ...
        assert prepare_query_for_queue_details_get({"type": queue_type}) == SALESFORCE_QUERIES[
            "GET_QUEUE_DETAIL_FOR_SPECIFIC_TYPE"
        ].format(QUEUE_TYPES[queue_type])
        ...
        # With type and name
        assert (
            prepare_query_for_queue_details_get({"type": "type", "queue_name": "q1"})
            == SALESFORCE_QUERIES["GET_QUEUE_DETAIL_FOR_SPECIFIC_TYPE"].format("type") + " and queue.name = 'q1'"
        )
    ...
    def test_prepare_query_for_user_details_get(self):
        ...
        args = {"email": "email", "queue_name": "queue_name", "is_staff": "true", "username": "username"}
        expected_query = (
            "select id,name, firstname, lastname, username,"
    ...
    def test_validate_and_get_date_argument(self):
        ...
        assert isinstance(validate_and_get_date_argument({"date_key": "2020-08-23T08:53:00.000Z"}, "date_key", "date"), datetime)
        assert isinstance(
            validate_and_get_date_argument({"date1_key": "2020-08-23T08:53:00.000+0530"}, "date1_key", "date1"), datetime
        )
    ...
    def test_get_notes_for_incident_success(self, mock_http_res):
        ...
        BmcHelixRemedyForce.process_notes_record = MagicMock(side_effect=BmcHelixRemedyForce.process_notes_record)
    ...
    def test_get_service_request_details(self, mock_http_res):
        ...
        resp1 = {
            "Success": True,
            "Result": {
                "Answers": [{"QuestionText": "question1", "Text": "answer1"}, {"QuestionText": "question2", "Text": "answer2"}]
            },
        }
        mock_http_res.return_value = resp1
        actual_response = get_service_request_details(self.client, "abc")
        assert actual_response == {"question1": "answer1", "question2": "answer2"}
    ...
    def test_bmc_remedy_incident_get_command_success(self, mock_http_res):
        ...
        BmcHelixRemedyForce.create_output_for_incident = MagicMock(side_effect=BmcHelixRemedyForce.create_output_for_incident)

        with open("TestData/fetch_incident_response.json", encoding="utf-8") as f:
            expected_res = json.load(f)

        mock_http_res.return_value = expected_res
        args = {"time": "1 day", "incident_number": "123"}
    ...
    def test_bmc_remedy_incident_get_command_no_incident_found(self, mock_http_res):
        ...
        args = {"time": "2 months", "incident_number": "321", "maximum_incident ": "50"}
        result = BmcHelixRemedyForce.bmc_remedy_incident_get_command(self.client, args)
    ...
    def test_prepare_outputs_for_get_service_request(self):
        ...
        """
        outputs, hr_outputs = prepare_outputs_for_get_service_request(self.get_service_request_response["input_records"])
