...
class Client(BaseClient):
    ...
    def fetch_password(self, method, resource_id, account_id, reason, ticket_id):
        ...
        headers = {"APP_AUTHTOKEN": self._app_token, "APP_TYPE": "17"}
        query = {"operation": {"Details": {"REASON": reason, "TICKETID": ticket_id}}}
    ...
    def update_account_password(self, method, resource_id, account_id, new_password, reset_type, reason, ticket_id):
        ...
        query = {
            "operation": {
                "Details": {"NEWPASSWORD": new_password, "RESETTYPE": reset_type, "REASON": reason, "TICKETID": ticket_id}
            }
        }
        params = {"INPUT_DATA": f"{query}"}
        return self._http_request(method, URL_SUFFIX, headers=headers, data=params)
...
def pam360_fetch_password(client: Client, resource_id: str = "", account_id: str = "", reason: str = "", ticket_id: str = ""):
...
def pam360_update_resource(
    ...
    update_resource = client.update_resource(
        "PUT",
        resource_id,
        resource_name,
        resource_type,
...
def pam360_update_account(
    ...
    update_account = client.update_account(
        "PUT", resource_id, account_id, account_name, notes, owner_name, account_password_policy
    )
...
def pam360_fetch_account_details(client: Client, resource_id: str = "", account_id: str = ""):
...
def pam360_list_accounts(client: Client, resource_id: str = ""):
...
def pam360_fetch_resource_account_id(client: Client, resource_name: str = "", account_name: str = ""):
