...
def list_secrets_command(client, args, gids=None):
    ...
    if not res:
        ...
        return CommandResults(outputs_prefix="No secrets found", readable_output=readable_output, outputs={}, raw_response={})
    ...
    if kid and res:
        ...
        return CommandResults(outputs_prefix="Fortanix.Secret", readable_output=readable_output, outputs=res, raw_response=res)
    else:
        mapped_secrets = [
            {"Name": secret["name"], "ID": secret["kid"], "Group": secret["group_id"]}
            for secret in res
            if state is None or (state == secret["state"].lower() or state == secret["enabled"])
        ]
...
def fetch_secret_command(client, args):
    ...
    return CommandResults(
        outputs_prefix="Fortanix.Secret.Value", readable_output=readable_output, outputs=mapped_value, raw_response=res
    )
...
def delete_secret_command(client, args):
    ...
    readable_output = tableToMarkdown("Fortanix DSM Secret", mapped_value)
    return CommandResults(outputs_prefix="Fortanix.Secret.Result", readable_output=readable_output, outputs=mapped_value)
...
def invoke_plugin_command(client, args):
    ...
    return CommandResults(
        outputs_prefix="Fortanix.Plugin.Output", readable_output=readable_output, outputs=value, raw_response=res
    )
...
def encrypt_command(client, args, pkey=None, pmode=None):
    ...
    return CommandResults(
        outputs_prefix="Fortanix.Data.Cipher", readable_output=readable_output, outputs=mapped_value, raw_response=res
    )
...
def decrypt_command(client, args, pkey=None, pmode=None):
    ...
    mapped_value = [{"Plain": value}]
    readable_output = tableToMarkdown("Fortanix DSM Decryption", mapped_value)
    return CommandResults(
        outputs_prefix="Fortanix.Data.Plain", readable_output=readable_output, outputs=mapped_value, raw_response=res
    )
...
def main() -> None:
    ...
    try:
        ...
        client = Client(base_url=endpoint, verify=verify_ssl, headers=get_headers(username, password, apikey), proxy=proxy)
