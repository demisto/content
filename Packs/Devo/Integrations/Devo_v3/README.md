## Overview
  ---
  The Devo_v3 Integration adds pagination support for single table query and custom table query support.
  ...
  * OAuth `apiv2` token type with the `*.**` scope.
  ...
  * `CHAIN CA`
  ...
  ## Configure Devo_v3 on Cortex XSOAR
  ---
  1. Navigate to __Settings__ > __Integrations__ > __Servers & Services__.
  2. Search for Devo_v3
  ...
  - Under the Server Configuration section, add these two parameters by clicking on "+ Add Server Configuration":
      - key = <name_of_integration>.devo-run-query.timeout, value = 1440
      - key = <name_of_integration>.devo-multi-table-query.timeout, value = 1440
  - The <name_of_integration> is the name of the Devo integration, but lowercased and replacing spaces with underscores: if its name its "Devo v3", then it is transformed to "devo_v3".
  ...
  By default, incidents are fetched from "siem.logtrust.alert.info" but, user can fetch incidents from custom alerts table as well. 
  Note: If user wants to fetch data from custom table they will have to provide column name prefix as well. User can also control the amount of incidents to fetch in a set duration. The default is 50 if nothing is provided. 
  ...
  **A Cortex XSOAR instance configured with the correct OAuth token that has permission to query the target tables**
  ...
  If user provides a table name, alerts are based off that table else, `siem.logtrust.alert.info` found in your Devo account. Please refer to this table
  ...
  **Requires a Devo OAuth token that has read permission on siem.logtrust.alert.info table**
  ...
  | **Argument Name** | **Description**                                                           | **Required** |
  |-------------------|---------------------------------------------------------------------------|--------------|
  | table_name              | Table to fetch incidents from                                        | Optional     |
    | prefix              | Column prefix for custom tables                                        | Optional     |
  ...
  ##### __table_name__ note:
  If user provides a table name in table_name argument, it will be used to fetch incidents from, else "siem.logtrust.alert.info" will be used.
  ...
  ##### Command Example
  ```
  !devo-get-alerts table_name="my.app.alerts.info" from=1576845233.193244 to=1576845293.193244 items_per_page=10
  ```
