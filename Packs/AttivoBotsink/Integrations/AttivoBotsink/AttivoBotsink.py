...
class BSAPI:
    ...
    def login(self, bs_user, bs_pass):
        url = "/auth/login"
        login_data = {
            "userName": base64.b64encode(bs_user.encode()).decode(),
...
if demisto.command() == "attivo-get-events":
    ...
    entry = {
        ...
        "EntryContext": {"Attivo.Events.Count": len(events["eventdata"]), "Attivo.Events.List": context},
    }
...
if demisto.command() == "fetch-incidents":
    ...
    events = attivo_api.get_events(
        timestamp_start=last_run_time, timestamp_end="now", severity_start=severity_start, severity_end=severity_end
    )
    if "error" in events:
...
if demisto.command() == "attivo-list-playbooks":
    ...
    for playbook in all_playbooks["pb"]:
        brief_playbook = {"ID": playbook["id"], "Name": playbook["name"]}
...
if demisto.command() == "attivo-run-playbook":
    ...
    if not playbook_id:
        ...
        demisto.info(f"Running Attivo playbook named {playbook_name} ({playbook_id}) with attacker IP {attacker_ip}")
    ...
    entry = {
        ...
        "EntryContext": {"Attivo.Playbook.Status": status, "Attivo.Playbook.Message": status_message},
    }
...
if demisto.command() == "attivo-deploy-decoy":
    ...
    entry = {
        ...
        "HumanReadable": status_message,
        "ReadableContentsFormat": formats["text"],
        "EntryContext": {"Attivo.DeployDecoy.Status": status, "Attivo.DeployDecoy.Message": status_message},
    }
...
if demisto.command() == "attivo-check-user":
    ...
    entry = {
        ...
        "EntryContext": {"Attivo.User.Name": user, "Attivo.User.IsDeceptive": is_deceptive, "Attivo.User.Groups": in_groups},
    }

    demisto.info(f"User {user}, deceptive = {is_deceptive}, group(s) = {in_groups}")
    demisto.results(entry)
    logout_status = attivo_api.logout()
...
if demisto.command() == "attivo-list-hosts":
    ...
    if (
        td_monitoring.get("forwarder_vm_monitoring_rules") is not None
    ...
    if (
        bs_monitoring.get("cfg_monitoring_rules") is not None
    ...
    entry = {
        ...
        "HumanReadable": tableToMarkdown(
            f"Attivo deceptive hosts (network decoys): {len(all_hosts)}", all_hosts, headers=headers
        ),
        "ReadableContentsFormat": formats["markdown"],
        "EntryContext": {},
    }
...
if demisto.command() == "attivo-check-host":
    ...
    bs_monitoring = attivo_api.get_monitoring_rules()
    if (
        bs_monitoring is not None
        and bs_monitoring.get("cfg_monitoring_rules") is not None
        and bs_monitoring.get("cfg_monitoring_rules").get("monitoringRules") is not None
    ):
        for rule in bs_monitoring["cfg_monitoring_rules"]["monitoringRules"]:
    ...
    if not is_deceptive:
        ...
        td_monitoring = attivo_api.get_threatdirect_rules()
        if (
            td_monitoring is not None
            and td_monitoring.get("forwarder_vm_monitoring_rules") is not None
            and td_monitoring.get("forwarder_vm_monitoring_rules").get("forwarderVmMonitoringRules") is not None
        ):
            for rule in td_monitoring["forwarder_vm_monitoring_rules"]["forwarderVmMonitoringRules"]:
                this_ip = rule["ip"]
    ...
    entry = {
        ...
        "HumanReadable": output_table,
        "ReadableContentsFormat": formats["table"],
        "EntryContext": {"Attivo.Host.HostInfo": host_info, "Attivo.Host.IsDeceptive": is_deceptive},
    }
