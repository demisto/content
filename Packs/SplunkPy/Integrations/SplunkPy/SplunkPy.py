...
class Notable:
    ...
    def failed_to_submit(self):
        ...
        return all(enrichment.status == Enrichment.FAILED for enrichment in self.enrichments) \
               and len(self.enrichments) == len(ENABLED_ENRICHMENTS)

    def handled(self):
        """ Returns an indicator on whether all notable's enrichments were handled or not """
        return all(enrichment.status in Enrichment.HANDLED for enrichment in self.enrichments) \
               or any(enrichment.status == Enrichment.EXCEEDED_TIMEOUT for enrichment in self.enrichments)
