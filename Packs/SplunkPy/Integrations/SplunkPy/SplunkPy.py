...
def splunk_job_share(service: client.Service, args: dict) -> list[CommandResults]:
    ...
    for sid in sids:
        try:
            ...
            if str(error) == "HTTP 404 Not Found -- Unknown sid.":
                raise DemistoException(
                    f"Not found job for SID: {sid}"
                ) from error
            else:
                raise DemistoException(
                    f"Querying splunk for SID: {sid} resulted in the following error {str(error)}"
                ) from error
        else:
            try:
                ttl_results = True
                ...
                ttl_results = False
                raise DemistoException(
                    f"Error increasing TTL for SID: {sid} resulted in the following error {str(error)}"
                ) from error
            ...
            try:
                ...
                share_results = False
                raise DemistoException(
                    f"Error changing permissions for SID: {sid} resulted in the following error {str(error)}"
                ) from error
