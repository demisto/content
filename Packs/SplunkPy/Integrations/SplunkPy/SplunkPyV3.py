...
class UserMappingObject:
    ...
    update_xsoar_user_in_notables = update_xsoar_user_in_findings
...
def build_fetch_query(params, search_offset: int | str =0 , earliest_time=None, latest_time='now'):
    """Build the fetch query, replacing placeholders with actual values.
    
    Args:
        params (dict): The integration parameters
        search_offset (str|int): The offset for pagination (default: 0)
        earliest_time (str): The earliest time for the query (optional)
        latest_time (str): The latest time for the query (optional)
    
    Returns:
        str: The complete fetch query with placeholders replaced
    """
    fetch_query = params["fetchQuery"]
    
    # Replace placeholders with actual values
    # | rest "/servicesNS/nobody/missioncontrol/public/v2/findings?earliest=EARLIEST_PLACEHOLDER&latest=LATEST_PLACEHOLDER&limit=LIMIT_PLACEHOLDER&offset=OFFSET_PLACEHOLDER&sort=_time:asc&search_format=true"
    fetch_query = fetch_query.replace("LIMIT_PLACEHOLDER", str(FETCH_LIMIT))
    fetch_query = fetch_query.replace("OFFSET_PLACEHOLDER", str(search_offset))
    fetch_query = fetch_query.replace("LATEST_PLACEHOLDER", latest_time)
    
    if earliest_time:
        fetch_query = fetch_query.replace("EARLIEST_PLACEHOLDER", earliest_time)
...
def fetch_findings(
    ...
    # Build the fetch query with placeholders replaced
    fetch_query = build_fetch_query(params, search_offset, occured_start_time, latest_time)
    ...
    if late_indexed_pagination := last_run_data.get("late_indexed_pagination"):
        ...
        if last_run_fetched_ids:
            ...
            fetch_query = fetch_query.replace(f"offset={search_offset}",'offset=0')
    ...
    if fetched_items:
        ...
        get_splunk_notes_data(service, finding_id_to_item, comment_tag_to_splunk, occured_start_time_timestamp, is_fetch=True)

    for item in fetched_items:
        extensive_log(f"[SplunkPy] Incident data before parsing to finding: {item}")
        finding_incident = Finding(data=item)
        inc = finding_incident.to_incident(mapper, comment_tag_to_splunk, comment_tag_from_splunk)
...
class Finding:
    ...
    @staticmethod
    def create_incident(
        finding_data, occurred, mapper: UserMappingObject, comment_tag_to_splunk: str, comment_tag_from_splunk: str
    ):
        ...
        if demisto.get(finding_data, "comment"):
            comments = argToList(finding_data.get("comment", []))
        ...
        finding_data["SplunkComments"] = comment_entries
...
class Cache:
    ...
    def __init__(self, not_yet_submitted_findings=None, submitted_findings=None):
        ...
        self.not_yet_submitted_notables = self.not_yet_submitted_findings
        self.submitted_notables = self.submitted_findings
...
# Backward compatibility aliases
get_notable_field_and_value = get_finding_field_and_value
...
def handle_submitted_findings(service: client.Service, cache_object: Cache) -> list[Finding]:
    ...
    total = len(findings)
    demisto.debug(f"Trying to handle {len(findings[:MAX_HANDLE_NOTABLES])}/{total} open enrichments")

    for finding in findings[:MAX_HANDLE_NOTABLES]:
    ...
    cache_object.submitted_notables = cache_object.submitted_findings  # Update backward compatibility alias
...
# Backward compatibility aliases
handle_submitted_notables = handle_submitted_findings
...
# Backward compatibility aliases
handle_submitted_notable = handle_submitted_finding
...
def submit_findings(service: client.Service, cache_object: Cache) -> tuple[list[Finding], list[Finding]]:
    ...
    total = len(findings)
    if findings:
        demisto.debug(f"Enriching {len(findings[:MAX_SUBMIT_NOTABLES])}/{total} fetched findings")

    for finding in findings[:MAX_SUBMIT_NOTABLES]:
    ...
    # Update backward compatibility aliases
    cache_object.not_yet_submitted_notables = cache_object.not_yet_submitted_findings
    cache_object.submitted_notables = cache_object.submitted_findings
...
# Backward compatibility aliases
submit_notables = submit_findings
...
# Backward compatibility aliases
submit_notable = submit_finding


def create_incidents_from_findings(
    findings_to_be_created: list[Finding], mapper: UserMappingObject, comment_tag_to_splunk: str, comment_tag_from_splunk: str
):
    ...
    for finding in findings_to_be_created:
        ...
        incidents.append(finding.to_incident(mapper, comment_tag_to_splunk, comment_tag_from_splunk))
...
# Backward compatibility aliases
create_incidents_from_notables = create_incidents_from_findings
...
def run_enrichment_mechanism(
    ...
    try:
        ...
        incidents = create_incidents_from_findings(
            handled_findings + failed_findings, mapper, comment_tag_to_splunk, comment_tag_from_splunk
        )
    except Exception as e:
        ...
        cache_object.dump_to_integration_context()
        incidents += [
            finding.to_incident(mapper, comment_tag_to_splunk, comment_tag_from_splunk)
...
def reset_enriching_fetch_mechanism():
    ...
    integration_context = get_integration_context()
    for field in (INCIDENTS, CACHE, MIRRORED_ENRICHING_FINDINGS):
...
def get_comment_entry(content: str, comment_tag_from_splunk: str, finding_id: str, format: str) -> dict:
    return {
...
def get_splunk_notes_data(
    ...
    """Get finding notes using the query-based approach for Splunk ES 8.2+.
    
    This implementation uses a search query to find note IDs associated with findings,
    ...
    Args:
        ...
        last_update_splunk_timestamp (str): Last update timestamp to filter notes
    ...
    # Create the search query to find note IDs
    ...
    demisto.debug(f"get_splunk_notes_data: Running {search_query=} to find note IDs for {len(finding_ids)} findings")
    ...
    try:
        ...
        demisto.debug(
            f"get_splunk_notes_data: Search query completed in {query_time:.3f} sec, "
            f"found {len(note_ids)} note IDs"
        )
        
        if not note_ids:
            demisto.debug("get_splunk_notes_data: No note IDs found")
            return war_room_notes
        ...
        query_time = time.time() - start_time
        demisto.debug(
            f"get_splunk_notes_data: mc_notes KV store query completed in {query_time:.3f} sec, "
            f"retrieved {len(mc_notes)} notes"
        )
        
    except Exception as e:
        demisto.error(f"get_splunk_notes_data: Failed to query notes: {e}")
        return war_room_notes
    ...
    for note in mc_notes:
        ...
        if not finding_id:
            demisto.debug(f"get_splunk_notes_data: Skipping note without notable_id: {note}")
            continue
        ...
        if (
            ...
            war_room_notes.append(get_comment_entry(markdown_content, comment_tag_from_splunk, finding_id, EntryFormat.MARKDOWN))

    # Sort comments by update time and update the comments in each finding
    extensive_log(f"get_splunk_notes_data: finding_notes = {finding_notes}")
    for finding_id, comments in finding_notes.items():
        ...
        # Used by Finding.create_incident() during fetch operations and after enriching findings mechanism.
        id_to_finding_map[finding_id]["comment"] = [comment["comment"] for comment in sorted_comments]

        if not is_fetch:
            # in mirror-in
            id_to_finding_map[finding_id]["SplunkComments"] = [{"Comment": comment["comment"]} for comment in sorted_comments]
...
# Backward compatibility alias
handle_enriching_notables = handle_enriching_findings


def handle_closed_finding(finding, finding_id, close_extra_labels, close_end_statuses, entries):
    ...
    if should_close:
        ...
        demisto.info(f"mirror-in: closing incident for {finding_id} (status_label={status_label}, status_end={status_end})")
        ...
        demisto.debug(
            f"mirror-in: Not closing incident {finding_id}. status_label={status_label}, "
            f"status_end={status_end}, close_extra_labels={close_extra_labels}, close_end_statuses={close_end_statuses}"
        )


# Backward compatibility alias
handle_closed_notable = handle_closed_finding
...
def get_modified_remote_data_command(
    ...
    if modified_findings_map:
        # Since we know ES version is 8.2+, notes are in the mc_notes KV Store
        # We use the query-based approach to fetch notes
        comment_entries = get_splunk_notes_data(service, modified_findings_map, comment_tag_from_splunk, original_last_update)
    ...
    if close_incident:
        for finding_id, finding in modified_findings_map.items():
            status_label = finding.get("status_label")
            status_end = finding.get("status_end")
            demisto.debug(
                f"Evaluating closure for {finding_id}: status_label={status_label}, status_end={status_end}, "
                f"close_extra_labels={close_extra_labels}, close_end_statuses={close_end_statuses}"
            )
            handle_closed_finding(finding, finding_id, close_extra_labels, close_end_statuses, entries)
...
def update_remote_system_command(args, params, service: client.Service, auth_token, mapper, comment_tag_to_splunk):
    ...
    Args:
        ...
        comment_tag_to_splunk (str) - tag of comment from xsoar
    ...
    if parsed_args.incident_changed and delta:
        ...
        if any(changed_data.values()):
            ...
            try:
                ...
                response_info = update_finding_events(
                    baseurl=base_url,
                ...
                if "success" not in response_info or not response_info["success"]:
                    demisto.error(f"Failed updating finding {finding_id}: {response_info!s}")
                else:
                    demisto.debug(f"update-remote-system for finding {finding_id}: {response_info.get('message')}")
            ...
            if comment_tag_to_splunk in entry_tags:
                ...
                try:
                    ...
                    if "success" not in response_info or not response_info["success"]:
                        ...
                        demisto.debug("update-remote-system for finding {}: {}".format(finding_id, response_info.get("message")))
                except Exception as e:
                    demisto.error(
...
def get_mapping_fields_command(
    ...
    for item in reader:
        if isinstance(item, dict):
            ...
            total_parsed_results.append(finding.to_incident(mapper, comment_tag_to_splunk, comment_tag_from_splunk))
...
def update_finding_events(
    ...
    """
    Update finding events (Splunk ES 8.2+).

    Arguments:
    comment -- A description of the change or some information about the finding events
    ...
    eventIDs -- A list of finding event IDs (must be provided if a search ID is not provided)
    ...
    # If you want to manipulate the finding events returned by a search then include the search ID
    ...
    mod_findings = requests.post(
        f"{baseurl}services/finding_update",
        data=args,
        headers=auth_header,
        verify=VERIFY_CERTIFICATE,
    )

    return mod_findings.json()


# Backward compatibility alias
update_notable_events = update_finding_events
...
def splunk_edit_finding_command(base_url: str, token: str, auth_token: str | None, args: dict) -> None:
    """Edit finding events in Splunk ES.
    """
    ...
    if "success" not in response_info or not response_info["success"]:
        return_error(f'Could not update finding events: {args.get("eventIDs", "")}: {response_info!s}')
    else:
        return_results(f'Splunk ES Finding events: {response_info.get("message")}')
...
def main():  # pragma: no cover
    ...
    if command == "test-module":
        ...
        splunk_edit_finding_command(base_url, token, auth_token, args)
