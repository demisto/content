...
SSL_ARGS = {"field": "serialNumber", "query": "dummy serial number"}

PDNS_ARGS = {"query": "dummy domain", "start": "2020-01-01 00:00:00", "end": "2020-01-31"}

HOST_ATTRIBUTE_ARGS = {
    "component_by_domain": {"query": "dummy domain", "start": "2020-05-25 00:05:25"},
    "component_by_ip": {"query": "dummy ip", "end": "2020-05-25 00:05:25"},
    "tracker_by_domain": {"query": "dummy domain", "start": "2020-05-25 00:05:25"},
    "tracker_by_ip": {"query": "dummy ip", "end": "2020-05-25 00:05:25"},
    "host_pair": {"query": "dummy child", "direction": "parents", "start": "2020-05-25 00:05:25"},
}
...
def test_http_request_proxy_error_based_on_status(mock_base_http_request, client):
    ...
    # Assert
    assert (
        str(e.value) == "Proxy Error - cannot connect to proxy. Either try clearing the 'Use system proxy' check-box "
...
def test_http_request_internal_server_error(mock_base_http_request, client):
    ...
    # Assert
    assert str(e.value) == "The server encountered an internal error for PassiveTotal and was unable to complete your request."
...
def test_function_success(request_mocker, client):
    ...
    mock_response = {"results": 0, "domains": []}
...
def test_get_components_command_main_success(mocker, client):
    ...
    mocker.patch.object(
        PassiveTotal_v2, "get_components_command", return_value="No component(s) were found for the given argument(s)."
    )
    PassiveTotal_v2.main()
...
def test_get_trackers_command_main_success(mocker, client):
    ...
    mocker.patch.object(
        PassiveTotal_v2, "get_trackers_command", return_value="No tracker(s) were found for the given argument(s)."
    )
    PassiveTotal_v2.main()
...
def test_get_host_pairs_command_main_success(mocker, client):
    ...
    mocker.patch.object(
        PassiveTotal_v2, "get_host_pairs_command", return_value="No host pair(s) were found for the given argument(s)."
    )
    PassiveTotal_v2.main()
...
def test_get_common_arguments_invalid_value_for_query():
    ...
    # Configure
    args = {"query": "", "start": "2016-02-02 02:32:44"}
...
def test_get_valid_whois_search_invalid_value_for_field_arguemnts():
    ...
    # Configure
    args = {"query": "test-query@test.com", "field": "field"}
    ...
    # Assert
    assert (
        str(e.value) == "Invalid field type field. Valid field types are domain, email, name, organization, address, phone, "
        "nameserver."
    )
...
def test_pt_whois_search_command_success(request_mocker, mock_cr, client):
    ...
    API endpoint
    """
    from PassiveTotal_v2 import get_context_for_whois_commands, get_human_readable_for_whois_commands, pt_whois_search_command

    # Configure
    args = {"query": "test-query@test.com", "field": "email"}
...
def test_get_pdns_details_command_success(mocker_http_request, client):
    ...
    assert result[0].readable_output == expected_hr
    assert (
        result[0].outputs_prefix == "PassiveTotal.PDNS(val.resolve == obj.resolve && val.recordType == "
        "obj.recordType"
...
def test_http_request_proxy_error(mock_base_http_request, client):
    ...
    # Assert
    assert (
        str(e.value) == "Proxy Error - cannot connect to proxy. Either try clearing the 'Use system proxy' check-box or"
...
def test_http_request_connection_error(mock_base_http_request, client):
    ...
    # Assert
    assert (
        str(e.value) == "Connectivity failed. Check your internet connection, the API URL or try increasing "
...
def test_domain_reputation_command_success(mocker_http_request, client):
    ...
    """
    from PassiveTotal_v2 import domain_reputation_command, get_context_for_whois_commands
...
@pytest.mark.parametrize("args", [{"ip": "1234"}, {"ip": "1.1.1"}, {"ip": "124.2222.2.2"}, {"ip": "abc"}])
...
def test_pt_get_whois_command_success(request_mocker, mock_cr, client):
    ...
    API endpoint
    """
    from PassiveTotal_v2 import get_context_for_get_whois_commands, get_human_readable_for_whois_commands, pt_get_whois_command

    # Configure
    args = {"query": "test-query@test.com", "history": "true"}
    with open("test_data/whois_command/whois_command_response.json", "rb") as f:
        dummy_response = json.load(f)
    with open("test_data/whois_command/whois_custom_context.json", "rb") as f:
        dummy_custom_context = json.load(f)
...
@pytest.mark.parametrize(
    "query,search_by,sort,order,page",
    [
...
def test_get_cookies_failures(client, query, search_by, sort, order, page):
    ...
    args = {"query": query, "search_by": search_by, "sort": sort, "order": order, "page": page}
...
def test_get_data_card_command_failure(client):
    ...
    from PassiveTotal_v2 import MESSAGES, get_data_card_summary_command
...
def test_get_reputation_command_failure(client):
    ...
    from PassiveTotal_v2 import MESSAGES, get_reputation_command
...
def test_pt_list_intel_profile_command_success(request_mocker, client):
    ...
    # Configure
    args = {"indicator_value": "conglomeratoid.com", "type": "actor", "source": "osint", "category": "network"}
...
def test_list_intel_profile_indicators_command_when_empty_response_is_returned(request_mocker, client):
    ...
    """
    from PassiveTotal_v2 import MESSAGES, list_intel_profile_indicators_command
...
def test_list_my_asi_assets_command_success(request_mocker, client):
    ...
    args = {"id": "40466", "segment_by": "savedfilter_metric_29644"}
    # Execute
...
def test_list_my_asi_vulnerable_components_command_success(request_mocker, client):
    ...
    args = {"page_size": 2, "page_number": 0}
    # Execute
...
def test_list_my_asi_vulnerable_components_command_when_empty_response_is_returned(request_mocker, client):
    ...
    """
    from PassiveTotal_v2 import MESSAGES, list_my_asi_vulnerable_components_command
...
def test_list_my_asi_vulnerabilities_command_when_empty_response_is_returned(request_mocker, client):
    ...
    """
    from PassiveTotal_v2 import MESSAGES, list_my_asi_vulnerabilities_command
...
def test_list_my_asi_observations_command_success(request_mocker, client):
    ...
    args = {"cve_id": "CVE-2021-23017", "page_size": 2, "page_number": 0}
    # Execute
...
def test_list_third_party_asi_assets_command_success(request_mocker, client):
    ...
    args = {"vendor_id": 45998, "id": 123, "segment_by": "savedfilter_metric_29644"}
    # Execute
...
def test_list_third_party_asi_vulnerable_components_command_success(request_mocker, client):
    ...
    with open("test_data/ASI_VulnerableComponents/third_party_asi_vulnerable_components_command_context.json", "rb") as f:
        dummy_context = json.load(f)
    ...
    args = {"id": 45998, "page_size": 2, "page_number": 0}
    # Execute
...
def test_list_third_party_asi_vulnerable_components_command_when_empty_response_is_returned(request_mocker, client):
    ...
    response.
    """
    from PassiveTotal_v2 import MESSAGES, list_third_party_asi_vulnerable_components_command
...
@patch("PassiveTotal_v2.Client.http_request")
def test_list_third_party_attack_surface_vulnerabilities_command_when_empty_response_is_returned(request_mocker, client):
    """
    Test case scenario for successful execution of pt-list-third-party-attack-surface-vulnerabilities command with an
    empty response.
    """
    from PassiveTotal_v2 import MESSAGES, list_third_party_attack_surface_vulnerabilities_command

    request_mocker.return_value = {"cves": []}
    command_results = list_third_party_attack_surface_vulnerabilities_command(client, {"id": 45998, "page_number": "100"})
...
def test_list_third_party_asi_observations_command_success(request_mocker, client):
    ...
    args = {"id": 45998, "cve_id": "CVE-2021-23017", "page_size": 2, "page_number": 0}
    # Execute
...
def test_list_third_party_asi_observations_command_success_object_not_present(request_mocker, client):
    ...
    with open("test_data/ASI_Observations/third_party_asi_observation_command_object_not_present_context.json", "rb") as f:
        dummy_context = json.load(f)
...
@patch("PassiveTotal_v2.Client.http_request")
def test_list_third_party_attack_surface_observations_command_when_empty_response_is_returned(request_mocker, client):
    """
    Test case scenario for successful execution of pt-list-third-party-attack-surface-observations command with an
    empty response.
    """
    from PassiveTotal_v2 import MESSAGES, list_third_party_asi_observations_command
...
def test_list_my_asi_observations_command_when_empty_response_is_returned(request_mocker, client):
    ...
    empty response.
    """
    from PassiveTotal_v2 import MESSAGES, list_my_asi_observations_command
