...
def path_finder(querystring):
    ...
    try:
        ...
        entry["HumanReadable"] = tableToMarkdown(
            "Tufin Topology Search for {} to {} via Service {}. Traffic is {}".format(
                querystring["src"],
                querystring["dst"],
                querystring["service"],
                ("**Denied**", "**Allowed**")[o["path_calc_results"]["traffic_allowed"]],
            ),
            {
                "Start": querystring["src"],
                "Devices in Path": "-->".join(
                    ["**" + d["name"] + "**" + " ({})".format(d["vendor"]) for d in o["path_calc_results"]["device_info"]]
                ),
                "End": querystring["dst"],
            },
            ["Start", "Devices in Path", "End"],
        )
...
def object_lookup(querystring):
    ...
    if total > 0:
        ...
        for obj in objs:
            ...
            return_json["objects"].append(
                {
                    "object_name": obj["display_name"],
                    "device": device_name(device_json, obj["device_id"]),
                    "comment": obj["comment"],
                }
            )
    else:
    ...
    entry["HumanReadable"] = tableToMarkdown(
        "Object Lookup for {}".format(querystring["exact_subnet"]),
        return_json["objects"],
        ["object_name", "device", "comment"],
...
def object_lookup_command():
    ...
    querystring = {"filter": "subnet", "count": "50", "exact_subnet": demisto.args()["ip"]}
...
def policy_search(querystring, max_rules_per_device=100):
    ...
    if not len(search_devices):
        ...
        entry["HumanReadable"] = tableToMarkdown(
            "Policy Search Results for {}".format(querystring["search_text"]),
            rule_return,
            ["Device", "Source", "Source Service", "Destination", "Destination Service", "Action"],
            removeNull=True,
        )
        # Send back to Demisto inside function
        return entry
...
def zone_match(ipaddr):
        ...
        for zone in zone_list["zones"]["zone"]:
            ...
            for subnet in zone_subnets["zone_entries"]["zone_entry"]:
                ...
                if IPAddress(ipaddr) in IPNetwork(ipnet):
                    ...
                    entry["Contents"] = zone
                    entry["HumanReadable"] = tableToMarkdown(
                        f"Tufin Zone Search for {ipaddr}",
    ...
    entry["HumanReadable"] = tableToMarkdown(
        f"Tufin Zone Search for {ipaddr}", {"Name": "Not Found", "ID": "0"}, ["Name", "ID"], removeNull=True
    )
    return entry
...
def change_req(req_type, subj, priority, src, dst="", proto="", port="", action="", comment=""):
    ...
    try:
        if req_type.lower() == "firewall change request":
            ...
            (
                req["ticket"]["steps"]["step"][0]["tasks"]["task"]["fields"]["field"]["access_request"]["sources"]["source"][0][
                    "ip_address"
                ]
            ) = src
            (
                req["ticket"]["steps"]["step"][0]["tasks"]["task"]["fields"]["field"]["access_request"]["destinations"][
                    "destination"
                ][0]["ip_address"]
            ) = dst
            (
                req["ticket"]["steps"]["step"][0]["tasks"]["task"]["fields"]["field"]["access_request"]["services"]["service"][0][
                    "protocol"
                ]
            ) = proto.upper()
            (
            ...
            (
                req["ticket"]["steps"]["step"][0]["tasks"]["task"]["fields"]["field"]["access_request"]["action"]
            ) = action.capitalize()
            ...
            entry["HumanReadable"] = tableToMarkdown(
                f"{req_type} ticket request", {"status": "Success"}, ["status"], removeNull=True
            )
            ...
            req["ticket"]["priority"] = priority.capitalize()
            (
                req["ticket"]["steps"]["step"][0]["tasks"]["task"]["fields"]["field"]["server_decommission_request"]["servers"][
                    "server"
                ]["ip_address"]
            ) = src
            req["ticket"]["comment"] = comment
...
def dev_search(name="", ip="", vendor="", model=""):
    ...
    try:
        ...
        if devices["devices"]["count"] > 0:
            ...
            entry["HumanReadable"] = tableToMarkdown(
                "Device Search Results", dev_list, ["ID", "Name", "IP", "Vendor", "Model"], removeNull=True
            )
    except Exception as e:
        return_error(f"Error submitting request: {str(e)}")
    return entry
    ...
    try:
        ...
        entry["HumanReadable"] = tableToMarkdown(
            f"Ticket ID {ticket_id}",
            {
                ...
                "Requester": ticket["ticket"]["requester"],
                "Comments": comments,
            },
            ["ID", "Subject", "Priority", "Status", "CurrentStep", "Requester", "Comments"],
        )
    except Exception as e:
        return_error(f"Error submitting request: {str(e)}")
    return entry
...
def app_search(name=""):
    ...
    try:
            ...
            app_list.append(
                {
                    ...
                    "Name": app["name"],
                    "Status": app["status"],
                    "Decommissioned": app["decommissioned"],
        ...
        entry["HumanReadable"] = tableToMarkdown(
            "Application Search Results", app_list, ["ID", "Name", "Status", "OwnerName", "Comments"], removeNull=True
        )
    except Exception as e:
        return_error(f"Error submitting request: {str(e)}")
    return entry
...
def app_conns(app_id):
    ...
    try:
        ...
        for conn in conns["connections"]["connection"]:
            conn_md = "{}\n#### Connection: {} ({})\nStatus: **{}**\nExternal: **{}**\nComment: **{}**".format(
                conn_md, conn["name"], conn["id"], conn["status"], conn["external"], conn["comment"]
            )
            # Get sources
            source_list = []
            conn_md = f"{conn_md}\n\n**Source:**\nid|type|name\n---|---|---"
