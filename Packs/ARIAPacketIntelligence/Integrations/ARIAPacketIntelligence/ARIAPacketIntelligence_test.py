...
class TestARIA:
    ...
    def _ip(ip1=None, ip2=None, ip3=None, ip4=None):
        ...
        return f"{ip1!s}.{ip2!s}.{ip3!s}.{ip4!s}"

    def _mock_request(self, function_name, demisto_args):
        # mock data
        mock_fqn = (
            "<sds_cluster_0>.<sds_node_sia_mock>.<sds_component_PacketIntelligence>."
        ...
        mock_response_ruleforward = {"endpoints": [mock_endpoint_entry], "timestamp": 123456789}
        mock_tcl_entry = {"PC_TRID": mock_trid, "status": "SUCCESS - The transaction completed successfully."}
        mock_response_trid = {"tclCount": 1, "tclList": [mock_tcl_entry]}
    ...
    def test_process_ip_address(self):
        ...
        ip_addr_invalid = [
            "0",
            "11,22,33,44",
            TestARIA()._ip(111, 222, 333, 444),
            f"{TestARIA()._ip(1, 2, 3, 4)}|32",
            f"{TestARIA()._ip(25, 89, 125, 255)}/33",
            TestARIA()._ip(10, 20, 30, 256),
        ]

        for i in range(len(ip_addr_invalid)):
        ...
        ip_addr_valid = [
            "1.2. 3.4",
            f"{TestARIA()._ip(1, 2, 3, 4)}  /32",
            f"{TestARIA()._ip(255, 255, 255, 255)}/ 24",
            "10.20.30. 40",
        ]

        for i in range(len(ip_addr_valid)):
    ...
    def test_process_port_range(self):
        ...
        for i in range(len(port_range_invalid)):
            with pytest.raises(ValueError):
        ...
        for i in range(len(port_range_valid)):
    ...
    def test_build_alert_instruction(self):
        ...
        for i in range(len(transport_type)):
            with pytest.raises(ValueError):
                ARIA(self.sdso_url)._build_alert_instruction(
                    transport_type={transport_type[i]},
                    tti_index={tti_index[i]},
    ...
    def test_unblock_conversation_command(self):
        demisto_args = {"rule_name": "block_conversation", "rcs": self.rcs}
    ...
    def test_stop_recording_conversation_command(self):
        demisto_args = {"rule_name": "record_conversation", "rcs": self.rcs}
    ...
    def test_stop_recording_dest_port_command(self):
        demisto_args = {"rule_name": "record_dest_port", "rcs": self.rcs}
    ...
    def test_mute_alert_src_port_command(self):
        demisto_args = {"rule_name": "alert_src_port", "rcs": self.rcs}
        self._mock_request("aria-mute-alert-src-port", demisto_args)

    def test_block_dest_subnet_command(self):
        demisto_args = {"rule_name": "block_dest_subnet", "target_ip": f"{TestARIA()._ip(1, 2, 3, 4)}/32", "rcs": self.rcs}
        self._mock_request("aria-block-dest-subnet", demisto_args)

    def test_unblock_dest_subnet_command(self):
        demisto_args = {"rule_name": "block_dest_subnet", "rcs": self.rcs}
    ...
    def test_stop_recording_dest_subnet_command(self):
        demisto_args = {"rule_name": "record_dest_subnet", "rcs": self.rcs}
    ...
    def test_mute_alert_dest_subnet_command(self):
        demisto_args = {"rule_name": "alert_dest_subnet", "rcs": self.rcs}
        self._mock_request("aria-mute-alert-dest-subnet", demisto_args)

    def test_block_src_subnet_command(self):
        demisto_args = {"rule_name": "block_src_subnet", "src_ip": f"{TestARIA()._ip(2, 3, 4, 5)}/32", "rcs": self.rcs}
        self._mock_request("aria-block-src-subnet", demisto_args)

    def test_unblock_src_subnet_command(self):
        demisto_args = {"rule_name": "block_src_subnet", "rcs": self.rcs}
    ...
    def test_stop_recording_src_subnet_command(self):
        demisto_args = {"rule_name": "record_src_subnet", "rcs": self.rcs}
    ...
    def test_mute_alert_src_subnet_command(self):
        demisto_args = {"rule_name": "alert_src_subnet", "rcs": self.rcs}
