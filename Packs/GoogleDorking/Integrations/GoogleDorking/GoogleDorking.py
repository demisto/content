...
class Client(BaseClient):
    ...
    def search(self, q, start=0, num=10) -> dict:
        params = assign_params(cx=self._cx, key=self._api_key, q=q, filter=1, start=start, num=num)
...
def get_search_results(client: Client, parser: Callable) -> list | str:
    ...
    while not all_pages_found and current_page < pages:
        ...
        total = int(demisto.get(page, "searchInformation.totalResults", demisto.get(page, "queries.request.totalResults", 0)))
...
def item_to_incident(client: Client, item: dict) -> list:
    ...
    if link := item.get("link"):
        try:
            ...
            demisto.debug(f"Failed fetching file from {link}. {e!s}")
    return [assign_params(name=item.get("title", ""), attachment=files, rawJSON=json.dumps(item))]
...
def item_to_result(client: Client, item: dict) -> list:
    ...
    results.append(CommandResults(outputs_prefix="GoogleDorking", outputs=item))
    return results
...
def fetch_incidents(client: Client, last_run: dict | None) -> list:
    ...
    demisto.setLastRun({LAST_RUN_TIME_KEY: (now - timedelta(days=1)).strftime(GOOGLE_TIME_FORMAT)})
...
def main() -> None:
    ...
    first_fetch_time = arg_to_datetime(arg=params.get("first_fetch"), arg_name="First fetch time", required=False)
    max_fetch = max(arg_to_number(params.get("max_fetch")) or MAX_INCIDENTS_TO_FETCH, MAX_INCIDENTS_TO_FETCH)
