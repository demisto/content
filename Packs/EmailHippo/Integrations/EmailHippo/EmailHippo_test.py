...
import json
import EmailHippo
from EmailHippo import Client, DemistoException, Common
...
class TestHappyPath:
    def test_get_email_reputation_success(self, requests_mock, client):
        ...
        requests_mock.get('https://test.com/v3/more/json/test/test@example.com',
                          json=load_test_data('test_data/get_email_output.json'))
        ...
        output_keys = [Common.EMAIL.CONTEXT_PATH, Common.DBotScore.CONTEXT_PATH]

        assert command_res
        assert all(key in command_res[0].readable_output for key in hr_keys)
        assert all(key in command_res[0].to_context()['EntryContext'] for key in output_keys)
    ...
    def test_domain_reputation_command_success(self, requests_mock, client):
        ...
        requests_mock.get('https://test.com/v1/test/example.com',
                          json=load_test_data('test_data/get_domain_output.json'))
        ...
        output_keys = [Common.Domain.CONTEXT_PATH, Common.DBotScore.CONTEXT_PATH]

        assert command_res
        entry_context = command_res[0].to_context()['EntryContext']
        assert all(key in command_res[0].readable_output for key in hr_keys)
        assert all(key in entry_context for key in output_keys)

        # test that the date parsed correct
        assert entry_context[Common.Domain.CONTEXT_PATH][0]['CreationDate'] == '2018-07-20T13:27:00.000Z'
        assert entry_context[Common.Domain.CONTEXT_PATH][0]['UpdatedDate'] == '2023-11-05T14:28:35.000Z'
...
class TestFailure:
    ...
    def test_get_email_reputation_failure_quota_limit(self, requests_mock, client):
        ...
        """
        requests_mock.get('https://test.com/v3/more/json/test/test@example.com', status_code=429)
