...
def get_email_quota_command(client: Client) -> CommandResults:
    ...
    readable_output = tableToMarkdown('Email quota',
                                      {
                                          'Email Quota used': quota.get('quotaUsed'),
                                          'Email Quota remaining': quota.get('quotaRemaining')
                                      })
    return CommandResults(
        readable_output=readable_output,
        outputs_prefix='EmailHippo.Quota',
        outputs_key_field='accountId',
        outputs=quota
    )


def email_reputation_command(client: Client, args: dict[str, Any],
                             reliability: DBotScoreReliability, create_relationships: bool = False) -> list[CommandResults]:
    """
...
def domain_reputation_command(client: Client, args: dict[str, Any],
