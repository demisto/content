...
def test_performance_large_input(mocker):
    ...
    mocker.patch.object(demisto, "args", return_value={"value": large_value, "keys": "SourceIP,DestinationIP"})
    mocker.patch.object(DedupBy, "return_results")
...
def test_performance_comparison_no_keys(mocker):
    ...
    for i in range(500):
        for _ in range(5):  # 5 duplicates each
            large_value.append({"id": i, "data": {"nested": {"deep": {"value": f"item_{i}", "list": [1, 2, 3, 4, 5]}}}})
