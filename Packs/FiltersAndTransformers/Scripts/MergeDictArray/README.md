...
| conflict_strategy | How to merge the values if the key conflicts. 'destination' means data of 'merge_with', 'source' means data of 'array_path'. Default is 'merge'.|
| overwrite_by_source | The list of keys to overwrite by source value if the key conflicts. |
| overwrite_by_destination | The list of keys to overwrite by destination value if the key conflicts. |
