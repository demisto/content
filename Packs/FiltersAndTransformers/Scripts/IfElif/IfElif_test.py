...
def test_parse_conditions(expression, expected_result):
    ...
    if_elif = ConditionParser(
        context=None,
        conditions=json.dumps([{"condition": expression, "return": True}, {"default": False}]),
    )
...
@pytest.mark.parametrize(
    "expression", ["unknown_word or 1", '__import__("os").system("RM -RF /")', "1 if 0 else 2", "sys.exit()"]
)
