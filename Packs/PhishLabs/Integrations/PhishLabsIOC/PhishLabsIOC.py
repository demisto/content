...
class Client:
    def __init__(self, base_url: str, user_name: str, password: str, use_ssl: bool, reliability: str = DBotScoreReliability.B):
    ...
    def http_request(self, method: str, path: str, params: dict = None, data: dict = None) -> dict:
        ...
        try:
            ...
            connection_error = f"Could not connect to PhishLabs IOC Feed: {e!s}"
            if RAISE_EXCEPTION_ON_ERROR:
...
def populate_context(
    dbot_scores: list, domain_entries: list, file_entries: list, url_entries: list, email_entries: list = None
) -> dict:
    """
    ...
    if url_entries:
        ...
        context["PhishLabs.URL(val.ID && val.ID === obj.ID)"] = createContext([u[1] for u in url_entries], removeNull=True)
    if domain_entries:
        context[outputPaths["domain"]] = createContext([d[0] for d in domain_entries])
        context["PhishLabs.Domain(val.ID && val.ID === obj.ID)"] = createContext([d[1] for d in domain_entries], removeNull=True)
    if file_entries:
        context[outputPaths["file"]] = createContext([f[0] for f in file_entries])
        context["PhishLabs.File(val.ID && val.ID === obj.ID)"] = createContext([f[1] for f in file_entries], removeNull=True)
    if email_entries:
        context["Email"] = createContext([e[0] for e in email_entries])
        context["PhishLabs.Email(val.ID && val.ID === obj.ID)"] = createContext([e[1] for e in email_entries], removeNull=True)
    if dbot_scores:
...
def create_domain_context(indicator: dict, classification: str) -> dict:
    ...
    """
    domain_object = {"Name": indicator.get("value")}

    if classification == "Malicious":
        domain_object["Malicious"] = {"Vendor": "PhishLabs", "Description": "Domain in PhishLabs feed"}
...
def create_url_context(indicator: dict, classification: str) -> dict:
    ...
    url_object: dict = {"Data": indicator.get("value")}

    if classification == "Malicious":
        url_object["Malicious"] = {"Vendor": "PhishLabs", "Description": "URL in PhishLabs feed"}
...
def create_phishlabs_object(indicator: dict) -> dict:
    ...
    return {
        ...
        "Attribute": [
            {"Name": a.get("name"), "Type": a.get("type"), "Value": a.get("value"), "CreatedAt": a.get("createdAt")}
    ...
    if results:
        ...
        for result in results:
            ...
            if indicator_type == "URL":
                ...
                context_object = {"Name": file_name, "Type": file_type, "MD5": file_md5}
        ...
        human_readable: str = tableToMarkdown(
            "PhishLabs Global Feed", contents, headers=indicator_headers, removeNull=True, headerTransform=pascalToSpace
        )
    else:
...
@logger
def get_global_feed_request(
    client: Client,
    since: str = None,
    limit: str = None,
    indicator: list = None,
    remove_protocol: str = None,
    remove_query: str = None,
        ...
        if results:
            ...
            for result in indicators:
                human_readable += tableToMarkdown(
                    "Indicator",
                    create_indicator_content(result),
                ...
                if indicator_type == "URL":
                    ...
                    context_object = {"To": email_to, "From": email_from, "Body": email_body, "Subject": result.get("value")}
...
def fetch_incidents(client: Client, fetch_time, fetch_limit):
    ...
    last_fetch_time: datetime = (
        datetime.strptime(last_fetch, "%Y-%m-%dT%H:%M:%SZ") if last_fetch else datetime.strptime(NONE_DATE, "%Y-%m-%dT%H:%M:%SZ")
    )
...
def main():
    ...
    server: str = params.get("url")[:-1] if (params.get("url") and params.get("url").endswith("/")) else params.get("url")
