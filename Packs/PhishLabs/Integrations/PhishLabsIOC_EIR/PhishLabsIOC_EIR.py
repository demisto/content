...
class Client(BaseClient):
    ...
    def get_incidents(
        ...
        params: dict[str, Any] = {}
        if period:
            created_after, created_before = parse_date_range(date_range=period, date_format="%Y-%m-%dT%H:%M:%SZ")
        ...
        params.update({"status": status, "sort": sort, "direction": direction, "limit": limit, "offset": offset})
        demisto.debug(f"Params sent to API are: {params!s}")
        return self._http_request("GET", url_suffix=suffix, params=assign_params(**params))
...
def indicator_ec(indicator: dict, type_ec: AnyStr) -> dict:
    ...
    if type_ec == "url-phishlabs":
        ...
        ec = {"To": indicator.get("emailReportedBy"), "From": indicator.get("sender"), "Body/HTML": indicator.get("emailBody")}
...
def indicator_dbot_ec(client: Client, indicator: dict, type_ec: AnyStr) -> tuple[dict, dict]:
    ...
    ec: dict = {}
    if type_ec == "url-ec":
        ec = {"Data": indicator.get("url"), "Malicious": {"Vendor": INTEGRATION_NAME, "Description": indicator.get("malicious")}}
        dbotscore = {
        ...
        ec = {
            ...
            "SHA256": indicator.get("sha256"),
            "MD5": indicator.get("md5"),
            "Malicious": {"Vendor": INTEGRATION_NAME, "Description": indicator.get("malicious")},
        }
        dbotscore = {
    ...
    for incident in incidents:
        ...
        phishlabs: dict = {
            ...
            "Email": {
                ...
                "Attachment": indicators_to_list_ec(client, sc_incident.get("attachments", []), type_ec="attach-phishlabs"),
            },
        }
...
def fetch_incidents_per_status(
    client: Client,
    created_after: str,
    offset: int,
    limit_page: int,
    ...
    """
    total: int = 0
    raw_response = client.get_incidents(
        created_after=created_after, offset=offset, limit=limit_page, sort=sort, direction=direction, status=status
    )
    while raw_response.get("metadata", {}).get("count") and total < limit_incidents:
        ...
        raw_response = client.get_incidents(
            offset=offset, created_after=created_after, limit=limit_page, sort=sort, direction=direction, status=status
        )
...
def fetch_incidents_command(
    client: Client, fetch_time: str, limit: str, last_ids: set, last_run: str | None = None
    ...
    raws, incidents_raw = fetch_incidents_per_status(
        client=client,
    ...
    demisto.debug(
        f"finished processing incidents. returning {len(incidents_report)} incidents,"
        f"with {datetime_new_last_run!s} as last run,"
        f"and {last_ids!s} as lastIds"
    )
...
def get_incidents_command(client: Client, **kwargs: dict) -> tuple[object, dict, list | dict]:
    ...
    if raw_response:
        ...
        context_entry: dict = {
            ...
            f"val.EIR.Modified && val.EIR.Modified === obj.EIR.Modified)": {"EIR": phishlabs_ec},
        }
        human_readable = tableToMarkdown(
            name=title,
            t=phishlabs_ec,
            headers=["ID", "Title", "Status", "Created", "Classification", "SubClassification", "EmailReportedBy"],
            removeNull=True,
        )
