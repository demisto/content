...
def create_connection(client_cert, client_key, root_ca):
    ...
    if client_cert or client_key or root_ca:
        ...
        conn.set_ssl(for_hosts=[(HOSTNAME, PORT)], key_file=client_key, cert_file=client_path)
...
def subscribe(client, conn, subscription_id, topic_name, queue_name):
    ...
    if queue_name:
        ...
        conn.subscribe(
            "/topic/" + topic_name, subscription_id, ack="client-individual", headers={"activemq.subscriptionName": client}
        )
...
def main():
    ...
    conn = create_connection(client_cert=CLIENT_CERT, client_key=CLIENT_KEY, root_ca=ROOT_CA)
