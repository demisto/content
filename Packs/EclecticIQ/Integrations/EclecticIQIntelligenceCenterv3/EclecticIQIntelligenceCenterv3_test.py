...
from EclecticIQIntelligenceCenterv3 import (
    ...
    get_indicators,
    ip_command,
    observable_id_from_url,
    parse_reputation_results,
    prepare_entity_observables,
    request_delete,
    request_get,
    request_patch,
    request_post,
    request_put,
    taxonomie_id_from_url,
    url_command,
)
...
def test_get_outh_token_success(requests_mock):
    ...
    client = EclecticIQ_api(
        baseurl=SERVER, eiq_api_version=API_VERSION, username="", password=PASSWORD, verify_ssl=USE_SSL, init_cred_test=False
    )
...
def test_entity(mocker):
    ...
    client = EclecticIQ_api(baseurl=SERVER, eiq_api_version=API_VERSION, username="", password=PASSWORD, verify_ssl=USE_SSL)
...
def test_create_sighting(mocker):
    ...
    mocker.patch.object(
        ...
        return_value={
            "observable_type": "ipv4",
            "observable_value": "1.1.1.1",
            "sighting_title": "EIQ-title",
            "sighting_description": "sighting",
...
def test_prepare_entity_observables(mocker):
    response = prepare_entity_observables(
        "1.1.1.1", "ipv4", "Malicious (High confidence)", '[{"type":"ipv4", "value": "2.2.2.2", "maliciousness": "medium"}]'
    )
        ...
        "data": [
            {
                ...
                "name": "Testing Group",
                "source": ["https://ic-playground.eclecticiq.com/api/v2/sources/95c654c9-e1de-4639-9d17-b12a883164b6"],
                "users": [
...
def test_get_feed_content_blocks(mocker):
    ...
    mock_response.json.return_value = {
        ...
        "data": {
            ...
            "id": 11,
            "items_count": 25,
            "meta": {"destination": "a80e31f1-3123-4f96-a15c-944e5f72f302"},
...
def test_eiq_get_entity_by_id(mocker):
    ...
    mock_response.text = json.dumps(
        {
            "data": {
                ...
                "observables": ["https://ic-playground.eclecticiq.com/api/v2/observables/651905"],
                "outgoing_feeds": [],
                "relevancy": "1",
                "sources": ["https://ic-playground.eclecticiq.com/api/v2/sources/2411da8d-9b07-4507-8e9e-ed64abd179c8"],
            }
        }
    )
    ...
    response = client.get_entity_by_id(
        "2c082ad0-cd32-47c2-8ff1-7c6adde759d4", observables_lookup=False, relationships_lookup=False
    )
...
def test_eiq_get_enrichers_list(mocker):
    mock_response = mocker.Mock()
    mock_response.json.return_value = {"data": [{"name": "enricher1", "is_active": True}]}
...
def test_eiq_get_status_red_component(mocker):
    ...
    service_return = {
        "health": "GREEN",
        "celery_states": [{"health": "RED", "component": "celery"}],
...
def test_eiq_create_incoming_feed(mocker):
    ...
    response = client.create_incoming_feed(
        feed_title="new_feed", content_type="CSV Observable", password="secret", username="user"
    )
...
def test_eiq_get_full_feed_info(mocker):
    mock_response = mocker.Mock()
    mock_response.text = json.dumps(
        {"data": [{"id": 1, "blobs": ["123-123-123", "123-123-124", "123-123-125"], "content_type": "CSV"}]}
    )
        ...
        "data": [
            {
                ...
                "sources": ["https://ic-playground.eclecticiq.com/api/v2/sources/0a80459d-282d-40c1-8cff-e2980535283c"],
                "strict_stix_1": False,
                "strict_stix_2": True,
            }
        ]
...
def test_eiq_get_entity_realtionships(mocker):
    ...
    mock_response_2.text = json.dumps(
        {"data": {"data": {"type": "indicator", "title": "New Entity"}, "id": "123-12-12", "observables": []}}
    )
    ...
    entity = [
        {
            "title": "test entity",
            "type": "indicator",
            "observables_list": [{"value": "1.1.1.1", "type": "ipv4"}],
            "relationships_list": [],
        }
    ]
    return entity
