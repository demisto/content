...
def mock_rest_client():
    ...
    return RestClient(base_url=BASE_URL, verify=False)
...
def test_fetch_incidents_does_not_get_most_recent_event_again(mocker, requests_mock):
    ...
    last_run = {"Tenant 1": {"time": 1593044883}}

    requests_mock.get(
        f"{BASE_URL}/session/tenantIdMapping?tempId={API_TOKEN}", json={"dev1": "Tenant 1", "dev1_tenant2": "Tenant 2"}
    )
...
def test_get_incident_command(requests_mock):
    ...
    full_incidents_response = load_test_data("test_data/full_incidents_response_single_full_incident.json")
    ...
    requests_mock.post(f"{BASE_URL}/graphql?tempId={API_TOKEN}&tenantId=dev1", json=full_incidents_response)
    args = {"tenant_id": "Tenant 1", "incident_id": 6}
...
def test_fetch_incidents(mocker, requests_mock):
    ...
    get_ids_response = [{"id": "8", "dateCreated": "1234566789"}, {"id": "14", "dateCreated": "12345676789"}]
    get_full_incidents_response = load_test_data("test_data/full_incidents.json")

    client = mock_rest_client()

    requests_mock.get(f"{BASE_URL}/session/tenantIdMapping?tempId={API_TOKEN}", json={"dev1": "Tenant 1"})
    mocker.patch.object(client, "construct_and_send_get_incident_ids_query", return_value=get_ids_response)
    mocker.patch.object(client, "construct_and_send_full_incidents_query", return_value=get_full_incidents_response)
...
def test_remove_user(mocker, requests_mock):
    ...
    requests_mock.post(f"{BASE_URL}/graphql?tempId={API_TOKEN}&tenantId=dev1", json=remove_user_response)
    requests_mock.get(f"{BASE_URL}/api/v0/users?tempId={API_TOKEN}", json=get_all_users_response)
    requests_mock.get(f"{BASE_URL}/session/tenantIdMapping?tempId={API_TOKEN}", json={"dev1": "Tenant 1"})
    requests_mock.get(
...
def test_assign_user(mocker, requests_mock):
    ...
    assign_user_response = {"data": {"addUserToIncident": {"id": "5", "userIds": ["675ad53a-d8f4-4ae7-9a3a-59de6c70b912"]}}}
    get_all_users_response = load_test_data("test_data/users.json")
    ...
    requests_mock.post(f"{BASE_URL}/graphql?tempId={API_TOKEN}&tenantId=dev1", json=assign_user_response)
...
def test_close_incident(mocker, requests_mock):
    ...
    mocker.patch.object(rest_client, "construct_and_send_full_incidents_query", return_value=single_full_incident_response)
    ...
    requests_mock.post(f"{BASE_URL}/graphql?tempId={API_TOKEN}&tenantId=dev1", json=close_incident_response)
    ...
    args = {
        ...
        "feedback_selected_options": [
            {"id": "4", "key": "unmonitoredAssets", "value": "true"},
    ...
    res = close_incident_command(rest_client, args)
    assert "incident closed and/or feedback updated for incident with id 6 on tenant Tenant 1" in res
...
def test_remove_user_raises_exception(mocker, requests_mock):
    ...
    args = {"tenant_id": "Tenant 1", "incident_id": 5, "username": "qa-user4@respond-software.com"}
    with pytest.raises(Exception):
        remove_user_command(rest_client, args)

    demisto.error.assert_called_once_with("no user found with email qa-user4@respond-software.com")
...
def test_close_incident_with_bad_responses(mocker, requests_mock):
    ...
    demisto.error.assert_any_call("error closing incident and/or updating feedback: type 'Exception' is not subscriptable")
    ...
    expected_result = [
        {
            ...
            "escalationreasons": [{"label": "Multiple Network IPS Signatures Triggered by Same Internal Asset"}],
            "assignedUsers": ["user1"],
            "feedback": {
            ...
            "owner": "user1",
            "externalsystems": [{"hostname": "host2", "ipaddress": "10.150.0.22", "isinternal": False}],
            "malware": [
                {"name": "name1", "type": "Ransomware", "vendor": "vendor"},
                {"name": "name2", "type": "RAT", "vendor": "vendor"},
            ],
            "hashes": [{"hash": "44d88612fea8a8f36de82e1278abb02f"}],
            "accounts": [
                {"domain": None, "name": "svc_adminscom3"},
                {"domain": None, "name": "svc_adminscom"},
                {"domain": "test", "name": "svc_adminscom2"},
                {"domain": None, "name": "svc_adminscom2"},
                {"domain": "test", "name": "svc_adminscom3"},
                {"domain": "test", "name": "svc_adminscom"},
                {"domain": None, "name": "Unknown"},
            ],
            "signatures": [],
            "domains": [],
        },
        {
            "Contents": {"closeNotes": "blah blah blah", "closeReason": "Non-Actionable", "dbotIncidentClose": True},
            "ContentsFormat": "json",
            "Type": 1,
        },
    ]
...
def test_get_escalations_no_new(requests_mock, mocker):
    ...
    requests_mock.post(f"{BASE_URL}/graphql?tempId={API_TOKEN}&tenantId=dev1", json=escalation_query_response)
    args = {"incident_id": "1"}
    rest_client = mock_rest_client()
    requests_mock.get(f"{BASE_URL}/session/tenantIdMapping?tempId={API_TOKEN}", json={"dev1": "Tenant 1"})
