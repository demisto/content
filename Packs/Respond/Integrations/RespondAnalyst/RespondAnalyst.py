...
RESPOND_INCIDENT_FIELDS = {
    ...
    "title": {"description": "incident title", "xsoar_field_name": "Title"},
    "feedback outcome": {"description": "the outcome of the incident close", "xsoar_field_name": "feedbackoutcome"},
}
...
class RestClient(BaseClient):
    ...
    def get_tenant_mappings(self):
        ...
        res = self._http_request(
            method="GET", url_suffix=f"/session/tenantIdMapping?tempId={API_TOKEN}", retries=3, resp_type="none"
        )
    ...
    def get_current_user(self):
        return self._http_request(method="GET", url_suffix=f"/session/activeUser?tempId={API_TOKEN}", retries=3)

    def get_all_users(self):
        return self._http_request(method="GET", url_suffix=f"/api/v0/users?tempId={API_TOKEN}", retries=3)
    ...
    def construct_and_send_close_incident_mutation(
        self, tenant_id, feedback_status, feedback_selected_options, feedback_optional_text, incident_id, user
    ):
    ...
    def construct_and_send_update_description_mutation(self, tenant_id, incident_id, description):
        mutation = {
...
def get_max_number_of_ids_by_least_recently_created(all_ids, max_fetch):
    ...
    for _i in range(len(all_ids) - max_fetch):
        sorted_ids.pop()
...
def fetch_incidents_for_tenant(rest_client, internal_tenant_id, external_tenant_id, from_time, max_fetch):
    ...
    try:
        response = rest_client.construct_and_send_get_incident_ids_query(internal_tenant_id, from_time)
        ...
        raw_incidents = rest_client.construct_and_send_full_incidents_query(internal_tenant_id, id_list)
...
def format_raw_incident(raw_incident, external_tenant_id, internal_tenant_id):
    ...
    formatted_incident = {
        ...
        "URL": BASE_URL + "/secure/incidents/" + raw_incident.get("id") + "?tenantId=" + internal_tenant_id,
        "closeURL": BASE_URL + "/secure/incidents/feedback/" + raw_incident.get("id") + "?tenantId=" + internal_tenant_id,
...
def get_formatted_incident(rest_client, args):
    ...
    if external_tenant_id is None:
        internal_tenant_id, external_tenant_id = get_tenant_map_if_single_tenant(user_tenant_mappings)
    else:
        internal_tenant_id = get_internal_tenant_from_mapping_with_external(user_tenant_mappings, external_tenant_id)

    incident_id = int(args["incident_id"])

    raw_incident = rest_client.construct_and_send_full_incidents_query(internal_tenant_id, [incident_id])[0]
...
def remove_user_command(rest_client, args):
    ...
    try:
        res = rest_client.construct_and_send_remove_user_from_incident_mutation(internal_tenant_id, user_to_remove, incident_id)
        return (
            "user with email: "
            + user_to_remove
            + " removed from incident with id "
            + res["id"]
            + " on tenant "
...
def assign_user_command(rest_client, args):
    ...
    try:
        res = rest_client.construct_and_send_add_user_to_incident_mutation(internal_tenant_id, user_to_add, incident_id)
        return (
            "user with email: "
            + user_to_add
            + " added to incident with id "
            + res.get("id")
            + " on tenant "
            + str(external_tenant_id)
        )
...
def close_incident_command(rest_client, args):
    ...
    try:
        incident = rest_client.construct_and_send_full_incidents_query(internal_tenant_id, [incident_id])[0]
        ...
        res = rest_client.construct_and_send_close_incident_mutation(
            internal_tenant_id,
            feedback_status,
            feedback_selected_options,
        ...
        return (
            "incident closed and/or feedback updated for incident with id "
            + str(incident_id)
            + " on tenant "
            + external_tenant_id
            + ":\n"
    ...
    try:
        ...
        while more_data:
            if datetime.now().timestamp() - start > fourMinutes:
                demisto.debug(f'exiting safely for incident {args["incident_id"]} on {external_tenant_id} starting at {start}')
                entries.append(
                    {
                        "Type": EntryType.NOTE,
                ...
                break
            all_escalations = rest_client.construct_and_send_new_escalations_query(internal_tenant_id, args["incident_id"])
            for escalation in all_escalations:
                valid_entry = {"Type": EntryType.NOTE, "Contents": escalation, "ContentsFormat": EntryFormat.JSON}
        ...
        demisto.debug(
            ...
            f'{args["incident_id"]} Error message: {e!s}'
        )
        raise e
    if len(entries) == 0:
        entries.append({"Type": EntryType.NOTE, "Contents": "No new escalations", "ContentsFormat": EntryFormat.TEXT})

    demisto.debug(f'returning escalations for incident {args["incident_id"]} on {external_tenant_id}: {entries}')
    return entries
...
def update_remote_system_command(rest_client, args):
    ...
    try:
        if remote_args.delta:
            ...
            demisto.debug(
                f"Got the following delta keys {list(remote_args.delta.keys())!s} to "
                f" update Respond incident {remote_args.remote_incident_id}"
            )

            if remote_args.delta.get("title"):
                demisto.debug(f'changed title for {remote_args.remote_incident_id}: {remote_args.delta["title"]}')
                rest_client.construct_and_send_update_title_mutation(internal_tenant_id, incident_id, remote_args.delta["title"])

            if remote_args.delta.get("description"):
                demisto.debug(f'changed description for {remote_args.remote_incident_id}: {remote_args.delta["description"]}')
                rest_client.construct_and_send_update_description_mutation(
                    internal_tenant_id, incident_id, remote_args.delta["description"]
                )
            ...
            if remote_args.delta.get("owner"):
                ...
                assigned_user_args = {"tenant_id": tenant_id, "incident_id": incident_id, "username": user_email}
                assign_user_command(rest_client, assigned_user_args)

    except Exception as e:
        demisto.debug(f"Error in Respond outgoing mirror for incident {remote_args.remote_incident_id} Error message: {e!s}")
...
def get_mapping_fields_command():
    ...
    for field in RESPOND_INCIDENT_FIELDS:
        respond_incident_type_scheme.add_field(name=field, description=RESPOND_INCIDENT_FIELDS[field].get("description"))
...
def fetch_incidents(rest_client, last_run):
    ...
    for internal_tenant_id, external_tenant_id in tenant_mappings.items():
        ...
        raw_incidents = fetch_incidents_for_tenant(
            rest_client, internal_tenant_id, external_tenant_id, from_time, max_fetch_per_tenant
        )

        raw_incidents.sort(key=lambda x: x.get("dateCreated"))
        for raw_incident in raw_incidents:
            try:
                formatted_incident = format_raw_incident(raw_incident, external_tenant_id, internal_tenant_id)
                new_incident = {
                ...
                demisto.error(
                    "Exception thrown collecting specific incident for tenant: "
                    + external_tenant_id
                    + str(err)
                    + "\n incident: "
                    + str(raw_incident)
