...
from test_data.test_responses import (
...
test_data = [
    (delete_host, {"host_id": "1"}, {}, {"id": "1", "Deleted": True}, "AnsibleAWX.Host(val.id && val.id == obj.id)"),
    (
        job_template_launch,
        {"job_template_id": "1"},
        JOB_TEMPLATE_LAUNCH_RES,
...
remove_fields_test_responses = [
    (
        {
            "results": [
                {"related": {"all_groups": "/api/v2/hosts/4/all_groups/"}, "summary_fields": {}, "name": "inventory", "id": 1}
            ]
        }
    ),
    (
