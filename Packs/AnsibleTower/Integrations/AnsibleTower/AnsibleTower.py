    ...
    if print_output:
        ...
        if text_filter:
            for line in content.split("\n"):
                if re.search(rf"{text_filter.lower()}", line.lower()):
...
def inventories_list(client: Client, args: dict) -> CommandResults:
    ...
    if not results:
        return CommandResults(
            readable_output=f"No results were found for the following arguments {str(args)}", raw_response=response
        )
...
def hosts_list(client: Client, args: dict) -> CommandResults:
    ...
    if not results:
        return CommandResults(
            readable_output=f"No results were found for the following argument {str(args)}", raw_response=response
        )
    context_data = results_output_data(results)
    headers = get_headers(context_data)
    return CommandResults(
...
def job_template_launch(client: Client, args: dict) -> CommandResults:
    ...
    data = {
        "inventory": args.get("inventory_id", None),
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(
            name=f"Job: {job_id} status is: {job_id_status}", t=context_data, removeNull=True, headers=headers
        ),
        raw_response=response,
    )
...
def job_relaunch(client: Client, args: dict):
    ...
    data = {"hosts": args.get("relaunch_hosts", "all"), "credential": args.get("credentials_id", None)}
    body = {key: data[key] for key in data if data[key]}
    demisto.debug(f"Request body is: {str(body)}")
    response = client.api_request(method="POST", url_suffix=url_suffix, json_data=body, ok_codes=[201])
    context_data = output_data(response)
    ...
    return CommandResults(
        ...
        outputs=context_data,
        readable_output=tableToMarkdown(
            name=f"Job - {job_id} status - {job_id_status}", t=context_data, removeNull=True, headers=headers
        ),
        raw_response=response,
    )
...
def job_stdout(client: Client, args: dict) -> CommandResults:
    ...
    output_text = output_content(context_data.get("content", ""), print_output, text_filter, f"### Job {job_id} output ### \n\n")
    return CommandResults(
...
def create_ad_hoc_command(client: Client, args: dict) -> CommandResults:
    ...
    body = {"inventory": inventory_id, "credential": credential_id, "module_name": module_name, "module_args": module_args}
    ...
    return CommandResults(
        ...
        outputs=context_data,
        readable_output=tableToMarkdown(
            name=f"Ad hoc command - {command_id} status - {command_status}", t=context_data, removeNull=True, headers=headers
        ),
        raw_response=response,
    )
...
def ad_hoc_command_stdout(client: Client, args: dict) -> CommandResults:
    ...
    output_text = output_content(
        context_data.get("content", ""), print_output, text_filter, f"### Ad hoc command {command_id} output ### \n\n"
    )
    return CommandResults(
