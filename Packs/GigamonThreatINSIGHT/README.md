# Gigamon ThreatINSIGHT integration
...
The integration files can be downloaded [here](https://assets.ctfassets.net/yjhod2jd8xdy/1ofxPPxEVmT0QLvzEgUg9x/6f1512d09eefdcd163ec398ef4d21599/Gigamon_ThreatINSIGHT.yml).


## Installation

To install the integration file:

1. Navigate to the **Settings** section of the interface and click the Upload button on the Settings page.
2. Once installed, find the Gigamon ThreatINSIGHT app.
3. Click the **Add Instance** link to create a new instance of the add-on.
4. In the settings, enter a **Name** for the instance.
5. Enter a valid **API Key** (required). To generate an API token, go to your: [Profile Settings](https://portal.icebrg.io/help/portal/profile-settings) page in ThreatINSIGHT.
6. Choose either Use **Single Engine** or **Use Load-Balancing** Group depending on your Demisto deployment.
7. Click the **Test** button to test the instance. If the instance can connect to the APIs, you should see a successful message like the one above.
8. Lastly, click the **Done** button to complete the installation of the integration.

- Optional - Select the **Fetches Incidents** option to have the integration periodically pull new detections from ThreatINSIGHT into Demisto.
- Optional - Choose the default **Incident Type** category for new incidents. The integration automatically categorizes incidents based on their detection rule category.
