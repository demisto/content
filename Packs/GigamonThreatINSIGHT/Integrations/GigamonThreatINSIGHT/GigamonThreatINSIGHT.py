...
class SensorClient(Client):
    ...
    def createTasks(self, data=None) -> Dict[str, Any]:
        ...
        return self._http_request(method="POST", url_suffix="pcaptasks", data=json.dumps(data))
    ...
    def getTelemetry(self, telemetry: str, args: str) -> Dict[str, Any]:
        ...
        return self._http_request(method="GET", url_suffix="telemetry/" + telemetry.lower() + args)
...
class DetectionClient(Client):
    ...
    def getDetectionRuleEvents(self, rule_uuid: str, args: str) -> Dict[str, Any]:
        ...
        return self._http_request(method="GET", url_suffix="rules/" + rule_uuid + "/events" + args)
    ...
    def createDetectionRule(self, data) -> Dict[str, Any]:
        ...
        return self._http_request(method="POST", url_suffix="/rules", data=json.dumps(data))
    ...
    def resolveDetection(self, detection_id: str, data=None) -> Dict[str, Any]:
        ...
        return self._http_request(method="Put", url_suffix="detections/" + detection_id + "/resolve", data=json.dumps(data))
...
def commandGetSensors(sensorClient: SensorClient, args):
    ...
    return CommandResults(outputs_prefix=prefix, outputs_key_field=key, outputs=result.get(key))
...
def commandGetEventsTelemetry(sensorClient: SensorClient, args):
    ...
    return CommandResults(outputs_prefix=prefix, outputs_key_field=key, outputs=formatEvents(result))
...
def commandFetchIncidents(detectionClient: DetectionClient, account_uuid, params, last_run) -> tuple[Dict[str, int], List[dict]]:
    ...
    args = {
        "created_or_shared_start_date": last_fetch.strftime(DATE_FORMAT),
        "include": "rules",
        "sort_by": "first_seen",
        "sort_order": "asc",
        "limit": max_results,
...
def commandGetDetections(detectionClient: DetectionClient, args):
    ...
    result: Dict[str, Any] = detectionClient.getDetections(encodeArgsToURL(args, ["include", "status", "rule_uuid"]))

    # if there are more detections to be retrieved, pull the
    # remaining detections incrementally
    if (
        "total_count" in result
        and int(result["total_count"]) > MAX_DETECTIONS
...
def commandGetDetectionRules(detectionClient: DetectionClient, args):
    ...
    result: Dict[str, Any] = detectionClient.getDetectionRules(encodeArgsToURL(args, ["confidence", "severity", "category"]))
...
def main():
    ...
    try:
        ...
        if command == "test-module":
            ...
            next_run, incidents = commandFetchIncidents(detectionClient, account_uuid, params, demisto.getLastRun())
            ...
            return_results(commandGetEventsTelemetry(sensorClient, encodeArgsToURL(args)))

        elif command == "insight-get-telemetry-network":
            return_results(commandGetNetworkTelemetry(sensorClient, encodeArgsToURL(args)))

        elif command == "insight-get-telemetry-packetstats":
            return_results(commandGetPacketstatsTelemetry(sensorClient, encodeArgsToURL(args)))
