...
class Client(BaseClient):
    ...
    def __init__(
        self, base_url, verify=True, proxy=False, ok_codes=(), headers=None, username=None, password=None, manager_email=None
    ):
    ...
    def get_user_by_id(self, user_id):
        ...
        return self._http_request(method="GET", url_suffix=uri, params=params, headers=self.headers)
    ...
    def get_user(self, filter_name: str, filter_value: str) -> Optional[IAMUserAppData]:
        ...
        data = {"email": filter_value, "includeSuspendedUsers": True}
        if filter_name == "id":
            res = self.get_user_by_id(f"/User/{filter_value}")
            user_app_data = res
        else:
            res = self._http_request(method="POST", url_suffix=uri, json_data=data, headers=self.headers)
    ...
    def create_user(self, user_data: Dict[str, Any]) -> IAMUserAppData:
        ...
        res = self._http_request(method="PUT", url_suffix=uri, json_data=user_data, headers=self.headers)
    ...
    def update_user(self, user_id: str, user_data: Dict[str, Any]) -> IAMUserAppData:
        ...
        if self.manager_id:
            user_data["DirectManager"] = self.manager_id
        user_data = {k: v for k, v in user_data.items() if k in self.app_fields}
        self._http_request(method="POST", url_suffix=uri, json_data=user_data, headers=self.headers)
