...
def fetch_incidents(client: Client, max_fetch: int) -> Tuple[str, List[dict]]:
    ...
    if last_run is None:
        ...
        first_fetch = demisto.params().get('first_fetch')
        last_run_obj = dateparser.parse(first_fetch, [DATE_TIME_FORMAT])  # type: ignore[attr-defined]
        last_run = last_run_obj.strftime(DATE_TIME_FORMAT)  # type: ignore[attr-defined]
