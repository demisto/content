...
def main() -> None:
    ...
    try:
        ...
        if not radar_files_added:
            html = f"<div style={DIV_HTML_STYLE}><h1 style={GREEN_HTML_STYLE}{radar_files_added!s}</h1></div>"
        else:
            html = f"<div style={DIV_HTML_STYLE}><h1 style={RED_HTML_STYLE}{radar_files_added!s}</h1></div>"
