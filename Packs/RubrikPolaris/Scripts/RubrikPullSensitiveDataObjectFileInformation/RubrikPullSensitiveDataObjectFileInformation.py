...
def sync_the_sensitive_data_object_file_information(args: dict[str, Any]) -> list:
    """
    Sync the Sensitive Data Object file information from RSC.
    ...
    command_args = {"object_id": object_id, "snapshot_id": snapshot_id, "limit": limit}

    command_results = demisto.executeCommand("rubrik-sonar-file-context-list", command_args)
    ...
    if not command_result:
        raise ValueError(f"Failed to get sensitive data object file information: {command_results[0].get('Contents')}")
    ...
    if file_data:
        ...
        results.append(
            CommandResults(readable_output="#### Sensitive data object file information has been synchronized successfully.")
        )
...
def main():
    try:
        return_results(sync_the_sensitive_data_object_file_information(demisto.args()))
    except Exception as ex:
        demisto.error(traceback.format_exc())  # print the traceback
        return_error(f"Failed to execute RubrikPullSensitiveDataObjectFileInformation-RubrikSecurityCloud. Error: {ex!s}")
