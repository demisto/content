...
from RubrikSetIncidentSeverityUsingWorkLoadRiskLevel import main, set_incident_severity_using_risk_level_command
...
@pytest.mark.parametrize(
    "args,incident_severity,severity",
    [
        ...
        (
            {"threat_hunt_malicious": ["Matches Found"], "anomaly_severities": ["Critical"], "increase_severity_by": 1},
            2,
            "Critical",
        ),
...
def test_set_incident_severity_using_risk_level_command_with_success_using_command(mocker, args, incident_severity, severity):
    ...
    # Mock the demisto.incident() function to return an incident with mocked values.
    mocker.patch.object(demisto, "incident", return_value={"severity": incident_severity})
...
def test_set_incident_severity_using_risk_level_command_with_high_current_severity_compared_to_new_severity(mocker):
    ...
    response = set_incident_severity_using_risk_level_command({"risk_levels": "Low"})
