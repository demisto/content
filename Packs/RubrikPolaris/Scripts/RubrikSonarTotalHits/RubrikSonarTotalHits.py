...
def main() -> None:
    ...
    try:
        ...
        if not sonar_total_hits:
            html = f"<div style={DIV_HTML_STYLE}><h1 style={GREEN_HTML_STYLE}{sonar_total_hits!s}</h1></div>"
        else:
            html = f"<div style={DIV_HTML_STYLE}><h1 style={RED_HTML_STYLE}{sonar_total_hits!s}</h1></div>"
