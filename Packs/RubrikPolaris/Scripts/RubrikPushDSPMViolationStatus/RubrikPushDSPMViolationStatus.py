...
def sync_the_violation_status(args: dict[str, Any]) -> dict[str, Any]:
    ...
    incident_violation_status = demisto.get(incident_info, "CustomFields.rubrikviolationstatus")
    ...
    command_results = demisto.executeCommand("rubrik-data-security-violation-status-update", command_args)
...
def main() -> None:
    try:
        ...
        return_error(f"Failed to execute RubrikPushDSPMViolationStatus-RubrikSecurityCloud. Error: {ex!s}")
