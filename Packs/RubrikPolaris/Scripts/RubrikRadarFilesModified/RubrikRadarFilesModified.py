...
def main() -> None:
    ...
    try:
        ...
        if not radar_files_modified:
            html = f"<div style={DIV_HTML_STYLE}><h1 style={GREEN_HTML_STYLE}{radar_files_modified!s}</h1></div>"
        else:
            html = f"<div style={DIV_HTML_STYLE}><h1 style={RED_HTML_STYLE}{radar_files_modified!s}</h1></div>"
