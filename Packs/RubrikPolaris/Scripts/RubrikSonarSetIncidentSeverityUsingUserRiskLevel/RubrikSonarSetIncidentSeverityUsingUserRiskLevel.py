...
def set_incident_severity_using_risk_level_command(args: dict[str, Any]) -> CommandResults:
    ...
    if new_severity > current_severity:
        ...
        return CommandResults(
            readable_output="No users with a risk level higher than the current incident "
...
def main():
    try:
        ...
        return_error(f"Failed to execute RubrikSonarSetIncidentSeverityUsingUserRiskLevel-RubrikPolaris. Error: {str(ex)}")
