...
def test_fetch_incidents(requests_mock):
    ...
    requests_mock.post("https://localhost/intelligence/data/graphql", json=mock_response)

    client = Client(base_url="https://localhost", verify=False, headers={"Authorization": "Bearer some_api_key"})

    first_fetch_time = 1600000000
    last_run = {}
    min_severity = 0
    next_run, incidents = fetch_incidents(
        client=client, events_limit=10, first_fetch_time=first_fetch_time, last_run=last_run, min_severity=min_severity
    )
