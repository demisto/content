...
- New: This trigger is responsible for handling the 'Suspicious LDAP search query executed (High severity)' and 'Possible LDAP enumeration by unsigned process (Medium severity)' alerts via the 'Suspicious LDAP search query' playbook<~XSIAM> (Available from Cortex XSIAM %%XSIAM_VERSION%%).</~XSIAM>
