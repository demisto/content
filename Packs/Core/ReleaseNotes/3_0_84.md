...
- **Endpoint Isolation**: Isolate the endpoint in specific high-risk cases to prevent further compromise.<~XSIAM> (Available from Cortex XSIAM %%XSIAM_VERSION%%).</~XSIAM>
...
- New: This trigger is responsible for handling several the 'Unsigned and unpopular process performed an injection' alerts<~XSIAM> (Available from Cortex XSIAM %%XSIAM_VERSION%%).</~XSIAM>
