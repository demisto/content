...
- The playbook checks for suspicious behaviors, including whether an Exchange admin created the rule outside of working hours, from an unusual geolocation, or if the user who created the rule has a high-risk score. It then aggregates all evidence collected during the investigation.
...
- If two or more pieces of suspicious evidence are found, the playbook will initiate soft response actions. These include signing the user out of their account and removing the forwarding email address from their mailbox. The user will be notified of these actions via email.
