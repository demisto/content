...
def test_get_incident_list(requests_mock):
    from CortexCoreIR import get_incidents_command, Client
    ...
    expected_output = {
        'PaloAltoNetworksCore.Incident(val.incident_id==obj.incident_id)':
...
def test_fetch_incidents(requests_mock, mocker):
    from CortexCoreIR import fetch_incidents, Client, sort_all_list_incident_fields
    import copy
    ...
    requests_mock.post(f'{Core_URL}/public_api/v1/incidents/get_incidents/', json=get_incidents_list_response)
    requests_mock.post(f'{Core_URL}/public_api/v1/incidents/get_incident_extra_data/', json=raw_incident)
    ...
    assert incidents[0]['name'] == "Core Incident 1 - 'Local Analysis Malware' generated by Core Agent detected on host" \
...
def test_fetch_incidents_filtered_by_status(requests_mock, mocker):
    ...
    """
    from CortexCoreIR import fetch_incidents, Client
    ...
    assert incidents[0]['name'] == "Core Incident 1 - 'Local Analysis Malware' generated by Core Agent detected on host "\
                                   "AAAAA involving user Administrator"
    assert incidents[1]['name'] == "Core Incident 2 - 'Local Analysis Malware' generated by Core Agent detected on host "\
...
def test_fetch_incidents_with_rate_limit_error(requests_mock, mocker):
    ...
    mocker.patch('CortexCoreIR.get_incident_extra_data_command', side_effect=return_extra_data_result)
...
def test_get_incident_extra_data(requests_mock):
    from CortexCoreIR import get_incident_extra_data_command, Client

    get_incident_extra_data_response = load_test_data('./test_data/get_incident_extra_data_host_id_array.json')
    requests_mock.post(f'{Core_URL}/public_api/v1/incidents/get_incident_extra_data/',
    ...
    expected_output = {
        'PaloAltoNetworksCore.Incident(val.incident_id==obj.incident_id)': expected_incident,
...
def test_insert_parsed_alert(requests_mock):
    from CortexCoreIR import insert_parsed_alert_command, Client

    insert_alerts_response = load_test_data('./test_data/create_alerts.json')
    requests_mock.post(f'{Core_URL}/public_api/v1/alerts/insert_parsed_alerts/', json=insert_alerts_response)
...
def test_insert_cef_alerts(requests_mock):
    from CortexCoreIR import insert_cef_alerts_command, Client

    insert_cef_alerts_response = load_test_data('./test_data/insert_cef_alerts.json')
    requests_mock.post(f'{Core_URL}/public_api/v1/alerts/insert_cef_alerts/', json=insert_cef_alerts_response)
...
def test_blacklist_files_command_with_more_than_one_file(requests_mock):
    ...
    from CortexCoreIR import blacklist_files_command, Client
    ...
    requests_mock.post(f'{Core_URL}/public_api/v1/hash_exceptions/blacklist/', json=test_data['api_response'])
...
def test_sort_all_list_incident_fields():
    ...
    """
    from CortexCoreIR import sort_all_list_incident_fields
...
def test_get_mapping_fields_command():
    ...
    """
    from CortexCoreIR import get_mapping_fields_command
    expected_mapping = {"Cortex Core Incident": {
...
def test_get_remote_data_command_should_update(requests_mock, mocker):
    ...
    """
    from CortexCoreIR import get_remote_data_command, Client, sort_all_list_incident_fields
    client = Client(
        base_url=f'{Core_URL}/public_api/v1', headers={}
    )
    args = {
    ...
    mocker.patch('CortexCoreIR.get_last_mirrored_in_time', return_value=0)
    mocker.patch('CortexCoreIR.check_if_incident_was_modified_in_core', return_value=True)
...
def test_get_remote_data_command_with_rate_limit_exception(mocker):
    ...
    Then
        ...
        incident.
    """
    from CortexCoreIR import get_remote_data_command, Client
    client = Client(
        base_url=f'{Core_URL}/public_api/v1', headers={}
    )
    args = {
    ...
    mocker.patch('CortexCoreIR.get_incident_extra_data_command', side_effect=Exception("Rate limit exceeded"))
...
def test_get_remote_data_command_should_not_update(requests_mock, mocker):
    ...
    # make sure get-extra-data is returning an incident
    mocker.patch('CortexCoreIR.get_last_mirrored_in_time', return_value=0)
    mocker.patch('CortexCoreIR.check_if_incident_was_modified_in_core', return_value=False)
...
def test_get_remote_data_command_should_close_issue(requests_mock, mocker):
    ...
    Given:
        -  an Core client
        - arguments (id and lastUpdate time set to a lower than incident modification time)
        - a raw incident (get-extra-data results) indicating the incident was closed on Core side
    When
        - running get_remote_data_command
    Then
        - the mirrored_object in the GetRemoteDataResponse is the same as the modified raw incident
...
def test_get_modified_remote_data_command(requests_mock):
    ...
    """
    from CortexCoreIR import get_modified_remote_data_command, Client
...
def test_decode_dict_values():
    ...
    Then
        - Verify expected output
    """
    from CortexCoreIR import decode_dict_values
...
def test_filter_general_fields():
    ...
    Then
        - Verify expected output
    """
    from CortexCoreIR import filter_general_fields
    alert = {
...
def test_filter_general_fields_no_event(mocker):
    ...
    err = mocker.patch('CortexCoreIR.return_warning')
    filter_general_fields(alert)
    assert err.call_args[0][0] == "No Core cloud analytics event."
...
def test_filter_vendor_fields():
    ...
    """
    from CortexCoreIR import filter_vendor_fields
