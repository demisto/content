...
def test_wrap_list_items_in_double_quotes(input_arg, expected):
    ...
    """
    response = CortexCoreXQLQueryEngine.wrap_list_items_in_double_quotes(input_arg)
...
def test_get_file_event_query():
    ...
    response = CortexCoreXQLQueryEngine.get_file_event_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_get_process_event_query():
    ...
    response = CortexCoreXQLQueryEngine.get_process_event_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_get_dll_module_query():
    ...
    response = CortexCoreXQLQueryEngine.get_dll_module_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_get_network_connection_query():
    ...
    response = CortexCoreXQLQueryEngine.get_network_connection_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_get_registry_query():
    ...
    response = CortexCoreXQLQueryEngine.get_registry_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_get_event_log_query():
    ...
    response = CortexCoreXQLQueryEngine.get_event_log_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_get_dns_query():
    ...
    response = CortexCoreXQLQueryEngine.get_dns_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_get_file_dropper_query():
    ...
    response = CortexCoreXQLQueryEngine.get_file_dropper_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_get_process_instance_network_activity_query():
    ...
    response = CortexCoreXQLQueryEngine.get_process_instance_network_activity_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_get_process_causality_network_activity_query():
    ...
    response = CortexCoreXQLQueryEngine.get_process_causality_network_activity_query(endpoint_ids=ENDPOINT_IDS, args=args)
...
def test_convert_timeframe_string_to_json(time_to_convert, expected):
    ...
    response = CortexCoreXQLQueryEngine.convert_timeframe_string_to_json(time_to_convert=time_to_convert)
...
def test_start_xql_query_valid(mocker):
    ...
    response = CortexCoreXQLQueryEngine.start_xql_query(CLIENT, args=args)
...
def test_start_xql_query_with_tenant_id_and_tenant_ids(mocker, tenant_id, expected):
    ...
    CortexCoreXQLQueryEngine.start_xql_query(CLIENT, args=args)
...
def test_get_xql_query_results_success_under_1000(mocker):
    ...
    response, file_data = CortexCoreXQLQueryEngine.get_xql_query_results(CLIENT, args=args)
    assert response == {
        "status": "SUCCESS",
        "number_of_results": 1,
        "query_cost": {"376699223": 0.0031591666666666665},
...
def test_get_xql_query_results_polling_command_success_under_1000(mocker):
    ...
    mocker.patch("CoreXQLApiModule.get_xql_query_results", return_value=(mock_response, None))
    mocker.patch.object(demisto, "command", return_value="xdr-xql-generic-query")
    command_results = CortexCoreXQLQueryEngine.get_xql_query_results_polling_command(
        CLIENT,
        {
            "query": query,
        },
...
def test_get_xql_query_results_polling_command_success_more_than_1000_results_parse_to_context(mocker):
    ...
    # Creates the mocked data which returns from 'CortexCoreXQLQueryEngine.get_xql_query_results' command:
    mock_file_data = b""
    ...
    mocker.patch.object(demisto, "command", return_value="xdr-xql-generic-query")
    results = CortexCoreXQLQueryEngine.get_xql_query_results_polling_command(
...
def test_get_xql_quota_is_core_available_true(mock_api_call):
    ...
    Then:
    - Ensure the request for get_xql_quota use the demisto.apiCall.
    ...
    mock_api_call.assert_called_once_with(
        method="POST", path="some_mock_url/xql/get_quota", data=None, headers={}, timeout=None, response_data_type=None
    )
    assert response == {"reply": {"additional_purchased_quota": 0.0, "eval_quota": 0.0, "license_quota": 1, "used_quota": 0.0}}
...
def test_start_xql_query_is_core_available_true(mock_api_call):
    ...
    Then:
    - Ensure the request for start_xql_query use the demisto.apiCall.

    """
    mock_api_call.return_value = {
        "name": "/api/webapp/public_api/v1/xql/start_xql_query",
        "status": 200,
        "data": '{"reply": "aaa"}',
    }
    response = CLIENT.start_xql_query({})
    mock_api_call.assert_called_once_with(
        method="POST", path="some_mock_url/xql/start_xql_query", data=None, headers={}, timeout=None, response_data_type=None
    )
    assert response == "aaa"
...
def test_get_xql_query_results_is_core_necessary_true(mock_api_call):
    ...
    Then:
    - Ensure the request for get_xql_query_results use the demisto.apiCall.
    ...
    response = CLIENT.get_xql_query_results({})
    mock_api_call.assert_called_once_with(
        method="POST", path="some_mock_url/xql/get_query_results", data=None, headers={}, timeout=None, response_data_type=None
    )
    assert response == "aaa"
...
def test_start_xql_query_platform(mocker):
    ...
    mock_http_request = mocker.patch.object(CLIENT, "_http_request", return_value=mock_execution_id)

    response = CortexCoreXQLQueryEngine.start_xql_query_platform(CLIENT, query, timeframe)

    assert response == mock_execution_id
    mock_http_request.assert_called_once()
    call_args = mock_http_request.call_args
    assert call_args[1]["use_platform_api"] is True
...
def test_start_xql_query_platform_adds_limit(mocker):
    ...
    CortexCoreXQLQueryEngine.start_xql_query_platform(CLIENT, query, timeframe)
...
def test_get_xql_query_results_platform_success(mocker):
    ...
    response = CortexCoreXQLQueryEngine.get_xql_query_results_platform(CLIENT, execution_id)
...
def test_get_xql_query_results_platform_failure(mocker):
    ...
    mock_error_details = {"error": "Query execution failed"}

    mocker.patch.object(CLIENT, "_http_request", return_value=mock_info_response)
    mocker.patch.object(CLIENT, "get_xql_query_results", return_value=mock_error_details)
...
def test_xql_query_platform_command_no_wait(mocker):
    ...
    mocker.patch.object(CortexCoreXQLQueryEngine, "start_xql_query_platform", return_value=mock_execution_id)
    mocker.patch.object(demisto, "demistoUrls", return_value={"server": "https://test.server"})
    get_results_polling_mock = mocker.patch.object(CortexCoreXQLQueryEngine, "get_xql_query_results_platform_polling")

    res = CortexCoreXQLQueryEngine.xql_query_platform_command(CLIENT, args)
...
def test_xql_query_platform_command_with_wait(mocker):
    ...
    mocker.patch.object(CortexCoreXQLQueryEngine, "start_xql_query_platform", return_value=mock_execution_id)
    mocker.patch.object(CortexCoreXQLQueryEngine, "get_xql_query_results_platform_polling", return_value=mock_results)
    mocker.patch.object(demisto, "demistoUrls", return_value={"server": "https://test.server"})
...
def test_main_success_generic_command(
    mock_return_error,
    mock_return_results,
    mock_Client,
    mock_demisto_params,
