...
> **Note:** Keys in the query results that represent timestamp fields (ending with `_time`, `timestamp`, or exactly named `time`) will have their values automatically converted to a date format.
...
| parse_result_file_to_context | Whether to extract more than 1000 query results as JSON data to context instead of returning a .gz file. | Optional |
| source_id | The ID of the source that initiated the query. Relevant only when running from a playbook. When provided, this value is reported instead of the automatically detected playbook ID. | Optional |
| source_name | The name of the source that initiated the query. Relevant only when running from a playbook. When provided, this value is reported instead of the automatically detected playbook name. | Optional |
