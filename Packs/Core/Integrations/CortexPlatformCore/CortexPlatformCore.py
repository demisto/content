...
def validate_custom_fields(fields_to_validate: dict, client: Client) -> tuple[dict, list[str]]:
    ...
    for field_name, field_value in fields_to_validate.items():
        if field_name in system_fields:
            ...
            error_messages.append(f"Field '{field_name}' does not exist. Available custom fields are: {available_fields}")
    ...
    try:
        if command == "test-module":
            ...
            if issues_command_results.outputs:
                outputs = issues_command_results.outputs
                issues_command_results.outputs = [alert_to_issue(o) for o in outputs]  # type: ignore[attr-defined,arg-type]

            # Apply output_keys filtering if specified
            if output_keys and issues_command_results.outputs:
                outputs = issues_command_results.outputs
                issues_command_results.outputs = filter_context_fields(output_keys, outputs)  # type: ignore[attr-defined,arg-type]
