...
class CipherTrustClient(BaseClient):
    ...
    def create_auth_token(self, username: str, password: str):
        return self._http_request(
            ...
            json_data={"grant_type": "password", "username": username, "password": password},
        )
...
def load_content_from_file(entry_id: str) -> str:
    ...
    try:
        ...
        raise ValueError(f"Failed to load the file {entry_id}: {e!s}")
...
def local_ca_markdown(raw_response: dict[str, Any]) -> str:
    ...
    return tableToMarkdown(
        raw_response.get("subject", ""),
        {key: raw_response.get(key, "") for key in keys},
...
def group_list_markdown(data: dict) -> str:
    skip_limit_total_hr = hr_skip_limit_to_markdown(data.get("skip", 0), data.get("limit", 0), data.get("total", 0), "Groups")
    ...
    return (
        tableToMarkdown("Groups", transformed_data, headerTransform=underscoreToCamelCase, sort_headers=False)
        + "\n"
        + skip_limit_total_hr
    )
...
def certificate_list_markdown(data: dict, title: str) -> str:
    ...
    transformed_data = [
        {key: certificate.get(key, "") for key in CERTIFICATE_LIST_KEYS} for certificate in data.get("resources", [])
    ]
...
def external_ca_list_markdown(data: dict) -> str:
    skip_limit_total_hr = hr_skip_limit_to_markdown(
        data.get("skip", 0), data.get("limit", 0), data.get("total", 0), "External Certificate Authorities"
    )
    transformed_data = [
        {
            "Name": ca.get("name", ""),
    ...
    t = tableToMarkdown(
        "External Certificate Authorities", transformed_data, headerTransform=underscoreToCamelCase, sort_headers=False
    )
    return t + "\n" + skip_limit_total_hr
...
def group_list_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    skip, limit = derive_skip_and_limit_for_pagination(args.get(LIMIT), args.get(PAGE), args.get(PAGE_SIZE))
...
def group_create_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    request_data = assign_params(name=args.get(NAME), description=args.get(DESCRIPTION))
...
def group_delete_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    client.delete_group(group_name=args.get(GROUP_NAME, ""), request_data=request_data)
    return CommandResults(readable_output=f"{args.get(GROUP_NAME)} has been deleted successfully!")


def group_update_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    request_data = assign_params(description=args.get(DESCRIPTION), name=args.get(NEW_GROUP_NAME))
    raw_response = client.update_group(group_name=args.get(GROUP_NAME, ""), request_data=request_data)
    return CommandResults(
        outputs_prefix=GROUP_CONTEXT_OUTPUT_PREFIX,
        outputs=raw_response,
        raw_response=raw_response,
...
def user_to_group_add_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    raw_response = client.add_user_to_group(group_name=args.get(GROUP_NAME, ""), user_id=args.get(USER_ID, ""))
    return CommandResults(
        outputs_prefix=GROUP_CONTEXT_OUTPUT_PREFIX,
        outputs=raw_response,
        raw_response=raw_response,
...
def user_update_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    add_empty_list_param(request_data, args.get(ALLOWED_CLIENT_TYPES), "allowed_client_types")
    add_prevent_ui_login(request_data, optional_arg_to_bool(args.get(PREVENT_UI_LOGIN)))
    raw_response = client.update_user(user_id=args.get(USER_ID, ""), request_data=request_data)
    return CommandResults(
        outputs_prefix=USERS_CONTEXT_OUTPUT_PREFIX,
        outputs=raw_response,
        raw_response=raw_response,
...
def user_delete_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(readable_output=f"{args.get(USER_ID)} has been deleted successfully!")
...
def user_password_change_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(readable_output=f"Password has been changed successfully for {args.get(USERNAME)}!")
...
def local_ca_create_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    request_data = assign_params(
        ...
        name=args.get(NAME),
        names=optional_safe_load_json(args.get(NAME_FIELDS_RAW_JSON), args.get(NAME_FIELDS_JSON_ENTRY_ID)),
...
def local_ca_list_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    if local_ca_id := args.get(LOCAL_CA_ID):  # filter by local_ca_id if provided, in other words - get a single local CA
        ...
        outputs = [
            remove_key_from_outputs(local_ca_entry, ["csr", "cert"])[0] for local_ca_entry in raw_response.get("resources", [])
        ]
...
def local_ca_update_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    raw_response = client.update_local_ca(local_ca_id=args.get(LOCAL_CA_ID, ""), params=params)
    outputs, removed_values = remove_key_from_outputs(raw_response, ["csr", "cert"])
    for data, filename in zip(removed_values, ["CSR.pem", "Certificate.pem"]):
        return_results(fileResult(filename, data, EntryType.ENTRY_INFO_FILE))
...
def local_ca_delete_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(readable_output=f"{args.get(LOCAL_CA_ID)} has been deleted successfully!")
...
def local_ca_self_sign_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    raw_response = client.self_sign_local_ca(local_ca_id=args.get(LOCAL_CA_ID, ""), request_data=request_data)
    outputs, removed_values = remove_key_from_outputs(raw_response, ["csr", "cert"])
    for data, filename in zip(removed_values, ["CSR.pem", "Certificate.pem"]):
        return_results(fileResult(filename, data, EntryType.ENTRY_INFO_FILE))
...
def local_ca_install_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    raw_response = client.install_local_ca(local_ca_id=args.get(LOCAL_CA_ID, ""), request_data=request_data)
    outputs, removed_values = remove_key_from_outputs(raw_response, ["csr", "cert"])
    for data, filename in zip(removed_values, ["CSR.pem", "Certificate.pem"]):
        return_results(fileResult(filename, data, EntryType.ENTRY_INFO_FILE))
...
def certificate_issue_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    raw_response = client.issue_certificate(ca_id=args.get(CA_ID, ""), request_data=request_data)
...
def certificate_list_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    raw_response = client.get_certificates_list(ca_id=args.get(CA_ID, ""), params=params)
    if args.get(ID):
        ...
        outputs = [remove_key_from_outputs(certificate, ["csr", "cert"])[0] for certificate in raw_response.get("resources", [])]
...
def local_certificate_delete_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    client.delete_certificate(ca_id=args.get(CA_ID, ""), local_ca_id=args.get(LOCAL_CA_ID, ""))
    return CommandResults(readable_output=f"{args.get(LOCAL_CA_ID)} has been deleted successfully!")
...
def certificate_revoke_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    raw_response = client.revoke_certificate(ca_id=args.get(CA_ID, ""), cert_id=args.get(CERT_ID, ""), request_data=request_data)
    outputs, cert = remove_key_from_outputs(raw_response, "cert")
    return_results(fileResult("Certificate.pem", cert, EntryType.ENTRY_INFO_FILE))
...
def certificate_resume_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    raw_response = client.resume_certificate(ca_id=args.get(CA_ID, ""), cert_id=args.get(CERT_ID, ""))
    outputs, cert = remove_key_from_outputs(raw_response, "cert")
    return_results(fileResult("Certificate.pem", cert, EntryType.ENTRY_INFO_FILE))
    return CommandResults(
        outputs_prefix=CA_CERTIFICATE_CONTEXT_OUTPUT_PREFIX,
...
def external_ca_delete_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(readable_output=f"{args.get(EXTERNAL_CA_ID)} has been deleted successfully!")
...
def external_ca_update_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    ...
    raw_response = client.update_external_ca(external_ca_id=args.get(EXTERNAL_CA_ID, ""), request_data=request_data)
    outputs, cert = remove_key_from_outputs(raw_response, "cert")
    return_results(fileResult("Certificate.pem", cert, EntryType.ENTRY_INFO_FILE))
    return CommandResults(
        outputs_prefix=EXTERNAL_CA_CONTEXT_OUTPUT_PREFIX,
...
def external_ca_list_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    if external_ca_id := args.get(EXTERNAL_CA_ID):
        ...
        outputs = [
            remove_key_from_outputs(external_ca_entry, ["cert"])[0] for external_ca_entry in raw_response.get("resources", [])
        ]
...
def csr_generate_command(client: CipherTrustClient, args: dict[str, Any]) -> CommandResults:
    if not args.get(ENCRYPTION_PASSWORD) and not args.get(PRIVATE_KEY_FILE_PASSWORD):
        raise ValueError(
            'Either the "encryption_password" or "private_key_file_password" argument must be provided. '
    ...
    if private_key_file_password := args.get(PRIVATE_KEY_FILE_PASSWORD):
        return_password_protected_zip_file_result(
            "privateKey", "privateKey.pem", outputs.pop("key", ""), private_key_file_password
        )
    else:
...
def main():
    ...
    try:
        client = CipherTrustClient(username=username, password=password, server_url=server_url, verify=verify, proxy=proxy)
