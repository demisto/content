...
def test_azure_sql_servers_list_command_with_not_found(mocker):
    ...
    args = {"resource_group_name": "resource-group", "list_by_resource_group": "true"}
...
def test_azure_sql_db_audit_policy_create_update_command(mocker):
    ...
    client = mock_client(mocker, util_load_json("test_data/azure_sql_db_audit_policy_create_update_command_result.json"))
    args = {
    ...
    results = azure_sql_db_audit_policy_create_update_command(client=client, args=args, resource_group_name="resourceGroupName")
    assert (
        "### Create Or Update Database Auditing Settings for resource_group_name='resourceGroupName'" in results.readable_output
    )
...
def test_azure_sql_db_threat_policy_create_update_command(mocker):
    ...
    client = mock_client(mocker, util_load_json("test_data/azure_sql_db_threat_policy_create_update_command_result.json"))
    args = {
        "server_name": "integration",
        "db_name": "integration-db",
        "state": "Enabled",
    ...
    results = azure_sql_db_threat_policy_create_update_command(client=client, args=args, resource_group_name="resourceGroupName")
    assert (
        "### Create Or Update Database Threat Detection Policies for resource_group_name='resourceGroupName'"
        in results.readable_output
    )
    assert results.outputs.get("retentionDays") == 5
