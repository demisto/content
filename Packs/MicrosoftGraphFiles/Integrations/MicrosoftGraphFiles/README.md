...
| MsGraphFiles.ItemActivity.Value.Action | Unknown | The action components of the activity, for example Access, Edit, Share, Rename, Move, Delete, Restore, Comment or Version. Each facet is present only when the activity included that action. | 
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| object_type | The MS Graph resource. Required together with `object_type_id`, unless `share_url` is used. Possible values are: drives, groups, sites, users. | Optional | 
