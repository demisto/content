...
register_module_line('Microsoft_Graph_Files', 'start', __line__())
...
class MsGraphClient:
    ...
    def search_sharepoint_sites(self, keyword):
        """
        This function searches the tenant sites
...
def list_sharepoint_sites_command(client: MsGraphClient, *_):
    ...
    :return: human_readable, context, result
    """
    result = client.search_sharepoint_sites('*')
...
def search_sharepoint_sites_command(client: MsGraphClient, args):
    """
    This function runs search tenant site command
    :return: human_readable, context, result
    """
    if args.get('keyword'):
        ...
        keyword = '*'
    result = client.search_sharepoint_sites(keyword)
    ...
    context = {f"{INTEGRATION_NAME}.SearchSites(val.ID === obj.ID)": context_entry}
...
def main():
    ...
    try:
        ...
        elif demisto.command() == "msgraph-search-sharepoint-sites":
            return_outputs(*search_sharepoint_sites_command(client, demisto.args()))
...
register_module_line('Microsoft_Graph_Files', 'end', __line__())
