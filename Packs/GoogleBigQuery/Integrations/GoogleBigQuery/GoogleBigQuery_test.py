...
def test_test_module_no_region_success(mocker):
    """
    Given:
    - Valid credentials in the integration parameters and no region.
    ...
    Then:
    - Ensure the bigquery client is initialized.
    - Ensure the query is called with the correct SQL and location is None.
    - Ensure the result is "ok".
    """
    from GoogleBigQuery import test_module

    # Mock demisto.params
    mock_params = {"credentials_google_service": {"password": '{"project_id": "test-project"}'}}
    mocker.patch.object(demisto, "params", return_value=mock_params)
    ...
    # Verify query was called with correct parameters
    mock_client.query.assert_called_once_with("SELECT 1", location=None)
    # Verify results
    results_mock.assert_called_once_with("ok")
