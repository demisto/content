...
def validate_args_for_query_job_config(
    ...
    if write_disposition and not (
        write_disposition == "WRITE_TRUNCATE" or write_disposition == "WRITE_APPEND" or write_disposition == "WRITE_EMPTY"
    ):
...
def build_query_job_config(
    ...
    validate_args_for_query_job_config(
        allow_large_results, priority, use_query_cache, use_legacy_sql, dry_run, destination_table, write_disposition
    )
...
def query(
    ...
    query_job = bigquery_client.query(
        query=query_string, job_config=job_config, location=location, job_id=job_id, project=project_id
    )
    ...
    if dry_run and str_to_bool(dry_run):
        ...
        for row in query_results:
            ...
            if context_key_format == "underscore":
                ...
                row_context = {underscoreToCamelCase(k): convert_to_string(v, datetime_format) for k, v in row.items()}
            rows_contexts.append(row_context)

        if rows_contexts:
            context["BigQuery(val.Query && val.Query == obj.Query)"] = {"Query": args["query"], "Row": rows_contexts}
            title = "BigQuery Query Results"
            human_readable = tableToMarkdown(title, rows_contexts, removeNull=True)

    return_outputs(readable_output=human_readable, outputs=context, raw_response=rows_contexts)
...
def fetch_incidents():
    ...
    demisto.debug(f"[BigQuery Debug] new_incidents is: {new_incidents}\nbigquery_rows is: {len(bigquery_rows)}")
    ...
    next_run = {"last_date": latest_incident_time_str, "found_ids": found_incidents_ids}
