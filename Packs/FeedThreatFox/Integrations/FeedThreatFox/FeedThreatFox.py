...
def create_query(
    ...
    query_dict = {"search_term": "search_ioc", "id": "ioc", "hash": "search_hash", "tag": "taginfo", "malware": "malwareinfo"}
    ...
    for indicator in indicators:
        res.append(
            assign_params(
                ...
                MalwareFamilyTags=indicator.get("malware_printable")
                if indicator.get("malware_printable") != "Unknown malware"
                else None,
...
def parse_indicator_for_fetch(indicator: dict, with_ports: bool, create_relationship: bool, tlp_color: str) -> dict[str, Any]:
    ...
    relationships = (
        create_relationships(ioc_value, indicator["ioc_type"], indicator.get("malware_printable"), demisto_ioc_type)
    ...
    return assign_params(value=ioc_value, type=demisto_ioc_type, fields=fields, relationships=relationships, rawJSON=indicator)
...
def create_relationships(value: str, type: str, related_malware: Optional[str], demisto_ioc_type: str) -> list:
    ...
    if related_malware and related_malware != "Unknown malware":
        name = (
            EntityRelationship.Relationships.COMMUNICATED_BY
            if type == "domain" or type == "ip:port" or type == "url"
            else EntityRelationship.Relationships.RELATED_TO
        )
        reverse_name = (
            EntityRelationship.Relationships.COMMUNICATED_WITH
            if type == "domain" or type == "ip:port" or type == "url"
            else EntityRelationship.Relationships.RELATED_TO
        )
        return [
            EntityRelationship(
                entity_a=value,
                entity_a_type=demisto_ioc_type,
                name=name,
                entity_b=related_malware,
...
def main() -> None:
    ...
    try:
        ...
        if command == "test-module":
            ...
            next_run, res = fetch_indicators_command(
                client=client,
