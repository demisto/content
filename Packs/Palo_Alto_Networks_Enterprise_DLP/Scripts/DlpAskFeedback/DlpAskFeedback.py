...
def send_slack_message(entitlement, task, user_id, message):
    ...
    send_notification_args = {"ignoreAddURL": "true", "using-brand": "SlackV3"}
    ...
    send_notification_args["blocks"] = json.dumps(
        {"blocks": blocks, "entitlement": entitlement_string, "reply": reply, "expiry": expiry, "default_response": "NoResponse"}
    )
        ...
        "content": {
            ...
            "actions": [
                ...
                },
                {
                    "type": "Action.Submit",
                    "title": "No",
                    "data": {"response": "No", "entitlement": entitlement, "investigation_id": investigation_id, "task_id": task},
                },
            ],
        },
    }

    command_arguments: dict = {"adaptive_card": json.dumps(adaptive_card), "using-brand": "Microsoft Teams"}
...
def main():
    ...
    res = demisto.executeCommand(
        "addEntitlement", {"persistent": demisto.get(args, "persistent"), "replyEntriesTag": demisto.get(args, "replyEntriesTag")}
    )
    if isError(res[0]):
        demisto.results(res)
        sys.exit(0)
