...
from CSCDomainManager import (
    ...
    csc_domains_availability_check_command,
    csc_domains_configuration_search_command,
    csc_domains_search_command,
    domain,
    get_domain_hr_fields,
    get_domains_availability_check_hr_fields,
    get_domains_configurations_hr_fields,
    get_domains_search_hr_fields,
...
def create_mock_client():
    return Client(base_url=EXAMPLE_BASE_URL, verify=VERIFY, apikey="test", token="test")


def test_create_params_string():
    args = {"domain_name": "csc-panw", "registry_expiry_date": "22-Apr-2025"}
    params_str = create_params_string(args)
    assert params_str == "filter=domain==csc-panw,registryExpiryDate==22-Apr-2025"

    args = {"domain_name": "csc-panw", "registry_expiry_date": "22/04/2025"}

    params_str = create_params_string(args)
    assert params_str == "filter=domain==csc-panw,registryExpiryDate==22-Apr-2025"

    args = {"domain_name": "csc-panw", "registry_expiry_date": "22-Apr-2025", "page": "2"}
    params_str = create_params_string(args)
    assert params_str == "filter=domain==csc-panw,registryExpiryDate==22-Apr-2025&page=2"

    args = {"admin_email": "example@panw.com", "email": "example@panwcom", "organization": "panw"}
    params_str = create_params_string(args)
...
def test_csc_domains_search(mocker):
    ...
    args = {"domain_name": "csc-panw", "registry_expiry_date": "22-Apr-2025 UTC"}
...
def test_csc_domains_search_with_operator(mocker):
    client = create_mock_client()
    args = {"registration_date": "ge=22-Apr-2024"}
    mocker.patch.object(client, "send_get_request", return_value=GET_REQUEST_EXAMPLE1)
    result = csc_domains_search_command(client, args)
    result_output = result.to_context().get("Contents")
    assert len(result_output) == 2
...
def test_csc_domains_availability_check(mocker):
    client = create_mock_client()
    args = {"domain_name": "cscpanw.org,csc-panw.info"}
