...
HR_HEADERS_FOR_DOMAIN_CONFI_LIST = [
    "Domain",
    "Domain Label",
    "Domain Status Code",
    "Domain extension",
...
class Client(BaseClient):
    ...
    def __init__(self, base_url, verify: bool, token: str = "", apikey: str = ""):
        headers = {"Authorization": f"{BEARER_PREFIX}{token}", "apikey": apikey, "Accept": ACCEPT_VAL}
        super().__init__(base_url=base_url, verify=verify, headers=headers)

    def send_get_request(self, url_suffix, params) -> Any:
        try:
            results = self._http_request(method="GET", url_suffix=url_suffix, params=params, headers=self._headers)
...
def csc_domains_search_command(client: Client, args) -> CommandResults:
    ...
    results = CommandResults(
        readable_output=tableToMarkdown(
            "Filtered Domains", domains_with_required_fields, headers=HR_HEADERS_FOR_DOMAINS_SEARCH, removeNull=True
        ),
...
def csc_domains_availability_check_command(client: Client, args) -> CommandResults:
    ...
    results = CommandResults(
        readable_output=tableToMarkdown("Domains Availability", hr_output, headers=HR_HEADERS_FOR_AVAILABILITY, removeNull=True),
...
def main():
    ...
    try:
        ...
        client = Client(base_url=base_url, verify=verify, token=token, apikey=api_key)
