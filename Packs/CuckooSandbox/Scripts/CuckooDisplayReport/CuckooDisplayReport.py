        ...
        if "reportdata" in demisto.args():
            ...
            try:
                if "reportfilepath" in demisto.args():
                    ...
                    filePath = demisto.executeCommand("getFilePath", {"id": demisto.args()["reportentryid"]})[0]["Contents"][
                        "path"
                    ]
    ...
    for s in showSections:
        try:
            ...
            if s == "info":
                ...
                res.append(
                    {"Type": entryTypes["note"], "ContentsFormat": formats["table"], "Contents": {"Malscore": report["malscore"]}}
                )
            elif s == "strings":
                ...
                if t and isinstance(t, list):
                    for subtable in t:
                        if t[subtable]:
                            ...
                            res.append(
                                {"Type": entryTypes["note"], "ContentsFormat": formats["table"], "Contents": behavior_data}
                            )
            else:
