...
LANGUAGE_TO_CODE = {"english": "en", "russian": "ru"}
FILE_TYPE = "F"
URL_TYPE = "U"
...
class Client(BaseClient):
        ...
        if self._check_report_available(file):
            try:
                report = self._http_request(
                    "get",
                    REPORT.format(tds_analysis_id, file["analgin_result"]["commit"], file["analgin_result"]["reports"][0]["id"]),
                )
                if "data" in report:
    ...
    def get_url(self, file):
        if self._check_report_available(file):
            return self._http_request(method="get", url_suffix=file.get("file_url")[1:], resp_type="content").decode()
    ...
    def export_report(self, tds_analysis_id):
        ...
        if self._check_report_available(file):
            return self._http_request(
                method="get",
                url_suffix=REPORT_EXPORT.format(
                    tds_analysis_id, file["analgin_result"]["commit"], file["analgin_result"]["reports"][0]["id"]
                ),
                resp_type="content",
            )
    ...
    def export_video(self, tds_analysis_id):
        ...
        if self._check_report_available(file):
            return self._http_request(
                method="get",
                url_suffix=VIDEO_EXPORT.format(
                    tds_analysis_id, file["analgin_result"]["commit"], file["analgin_result"]["reports"][0]["id"]
                ),
                resp_type="content",
            )
    ...
    def get_hash_reputation(self, hash_type, hash_value):
        return self._http_request(method="get", url_suffix=HASH_REPUTATION.format(hash_type, hash_value)).get("data", {})
...
def serialize_report_info(report, analysis_type):
    ...
    if analysis_type == FILE_TYPE:
        res.update({"Type": report.get("target", {}).get("file", {}).get("type")})
    else:
        res.update({"URL": report.get("target", {}).get("url")})
    return res
...
def get_human_readable_analysis_info(analysis_info):
    return tableToMarkdown(f"Analysis {analysis_info.get('ID')}", analysis_info, removeNull=True)
...
def get_main_indicator(report, analysis_type):
    ...
    if report.get("info", {}).get("verdict"):
        ...
        malicious = "Verdict probability: {}%".format(report.get("info", {}).get("probability"))
        signatures: list = []
...
def get_network_indicators(res):
    ...
    for host in network.get("hosts", []) + [h[0] for h in network.get("dead_hosts", [])]:
        ...
        command_results.append(
            CommandResults(
                readable_output=tableToMarkdown(f"New IP indicator was created {ip.ip}", {}), indicator=ip, raw_response=res
            )
        )
    for http in network.get("http", []):
        ...
        command_results.append(
            CommandResults(
                readable_output=tableToMarkdown(f"New URL indicator was created {url.url}", {}), indicator=url, raw_response=res
            )
        )
    return command_results
    ...
    for p in report.get("goo_monitor", {}).get("processes", []):
        ...
        for regkey in p.get("regkeys", []):
            if regkey.get("action") == "regkey_written":
                reg = RegistryKey(path=regkey.get("ioc"), value=str(regkey.get("value")))
                command_results.append(
                    CommandResults(
...
def get_report_indicators(res, analysis_type):
    ...
    command_results.append(CommandResults(readable_output=human_readable, indicator=indicator, raw_response=res))
...
def export_report_command(client, args):
    ...
    demisto.results(fileResult(filename="report.tar", data=report))


def export_pcap_command(client, args):
    tds_analysis_id = drop_prefix(args.get("tds_analysis_id"))
    pcap = client.export_pcap(tds_analysis_id)
    demisto.results(fileResult(filename="dump.pcap", data=pcap))
...
def export_video_command(client, args):
    ...
    if not video:
        ...
        demisto.results(fileResult(filename="video.webm", data=video))
...
def upload_file_command(client, args):
    ...
    res = f"{FILE_TYPE}{res}"
    outputs = {"ID": res, "EntryID": file_id, "FileName": file_obj.get("name"), "Status": "In Progress"}
    results = CommandResults(
...
def main():
    ...
    try:
        client = Client(base_url=base_url, api_key=api_key, verify=verify_certificate, language=report_language)
        commands = {
