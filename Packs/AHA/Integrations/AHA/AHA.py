...
class Client(BaseClient):
    ...
    def __init__(self, headers: dict, base_url: str, proxy: bool, verify: bool, url: str):
        super().__init__(base_url=base_url, proxy=proxy, verify=verify, headers=headers)
        self.url = url
        self._headers["Content-Type"] = "application/json"

    def get(self, aha_type: AHA_TYPE, name: str, fields: str, from_date: str, page: str, per_page: str) -> dict:
        """
...
def get_command(
    ...
    response = client.get(
        aha_type=aha_type, name=aha_object_name, fields=req_fields, from_date=from_date, page=page, per_page=per_page
    )
    if response:
        ...
        human_readable = tableToMarkdown(f"Aha! get {aha_type.get_type_plural()}", message, removeNull=True)
    else:
        human_readable = ""
...
def edit_command(client: Client, aha_type: AHA_TYPE, aha_object_name: str, fields: str = "{}") -> CommandResults:
    message: List = []
    ...
    if response:
        ...
        human_readable = tableToMarkdown(f"Aha! edit {aha_type.get_type_singular()}", message, removeNull=True)
    else:
        human_readable = ""
        demisto.debug(f"{response=} -> {human_readable=}")
    return CommandResults(
...
def main() -> None:
    ...
    try:
        ...
        client = Client(headers=headers, base_url=base_url, proxy=proxy, verify=verify, url=url)
        ...
        if command == "test-module":
            ...
            command_result = get_command(client, aha_type=AHA_TYPE.FEATURES, aha_object_name=args.pop("feature_name", ""), **args)
            return_results(command_result)
        elif command == "aha-edit-feature":
            command_result = edit_command(
                client, aha_type=AHA_TYPE.FEATURES, aha_object_name=args.pop("feature_name", ""), **args
            )
            return_results(command_result)
        elif command == "aha-get-ideas":
            command_result = get_command(
                client=client, aha_type=AHA_TYPE.IDEAS, aha_object_name=args.pop("idea_name", ""), **args
            )
            return_results(command_result)
        elif command == "aha-edit-idea":
            command_result = edit_command(client, aha_type=AHA_TYPE.IDEAS, aha_object_name=args.pop("idea_name", ""), **args)
            return_results(command_result)
        else:
