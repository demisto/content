...
TICKETS_OBJECTS = {
    ...
    "category": {"Network": 1, "Other": 2, "Software": 4, "Hardware": 3},
    "priority": {"Medium": 1, "High": 2, "Low": 3},
    "status": {
        "Opened": 1,
        "Closed": 2,
        "Need More Info": 3,
...
class Client(BaseClient):
    ...
    def get_cookie(self, res_cookie: dict) -> str:
        ...
        cookie = (
            f"KACE_LAST_USER_SECURE={KACE_LAST_USER_SECURE}; KACE_LAST_ORG_SECURE={KACE_LAST_ORG_SECURE};"
...
def get_machines_list_command(client, args) -> tuple[str, dict, dict]:
    ...
    human_readable_markdown = tableToMarkdown(
        "Quest Kace Machines",
        context,
        removeNull=True,
        headers=[
    ...
    context = {"QuestKace.Machine(val.ID === obj.ID)": context}
...
def get_assets_list_command(client, args) -> tuple[str, dict, dict]:
    ...
    context = {"QuestKace.Asset(val.ID === obj.ID)": context}
    return human_readable_markdown, context, raw_response
...
def get_queues_list_command(client, args) -> tuple[str, dict, dict]:
    ...
    context = parse_response(raw_response)
    human_readable_markdown = tableToMarkdown("Quest Kace Queues", context, removeNull=True, headers=["ID", "Name", "Fields"])
    context = {"QuestKace.Queue(val.ID === obj.ID)": context}
    return human_readable_markdown, context, raw_response
...
def get_tickets_list_command(client, args) -> tuple[str, dict, dict]:
    ...
    human_readable_markdown = tableToMarkdown(
        "Quest Kace Tickets", context, removeNull=True, headers=["ID", "Title", "Created", "Modified", "HdQueueID", "DueDate"]
    )
    context = {"QuestKace.Ticket(val.ID === obj.ID)": context}
    return human_readable_markdown, context, raw_response
...
def fetch_incidents(
    ...
    last_run: dict,
    fetch_limit: str,
    fetch_queue_id: list | None = None,
    fetch_filter: str | None = None,
) -> list:
    """
    ...
    demisto.info(
        f"Fetching Incident has Started,\n"
        f"Fetch filter is {fetch_filter_for_query}\n"
        f"Last fetch was on {parsed_last_time!s}"
    )
    client.update_token()
    ...
    demisto.info(
        f"Fetching Incident has Finished\n"
        f"Fetch limit was {fetch_limit}"
        f"Last fetch was on {last_incident_time!s}\n"
...
def shaping_fetch(client: Client, fetch_queue_id: list) -> str:
    ...
    if integration_context:
        ...
        if time_now < valid_until:
            ...
            integration_context = {"shaping_fields": fetch_shaping, "valid_until": int(time.time()) + 3600 * 24}
...
def parse_incidents(items: list, fetch_limit: str, time_format: str, parsed_last_time: datetime) -> tuple[list, Any]:
    """
    ...
    for item in items:
        ...
        incident = {"name": item["title"], "occurred": incident_created_time.strftime(time_format), "rawJSON": json.dumps(item)}
...
def main():
    ...
    try:
        client = Client(url=base_url, username=username, password=password, verify=verify_certificate, proxy=proxy)
        ...
        if command in commands:
            ...
            incidents = fetch_incidents(
                ...
                fetch_shaping=fetch_shaping,
                fetch_filter=fetch_filter,
                fetch_limit=fetch_limit,
                fetch_queue_id=fetch_queue_id,
