...
def validate_input_values(arguments_values_to_verify, available_values):
    for value in arguments_values_to_verify:
        if value not in available_values:
            return_error("Argument: '{}' is not one of the possible values: {}".format(value, ", ".join(available_values)))
...
def get_credentials(additional_scopes=None, delegated_user=ADMIN_EMAIL):
    ...
    try:
        ...
        cred = service_account.ServiceAccountCredentials.from_json_keyfile_dict(json_keyfile, scopes=scopes)
...
def connect():
    ...
    try:
        ...
        err_msg = f"There was an error creating the Vault service - {e!s}"
        return_error(err_msg)
    return service
...
def create_mail_export_query(
    ...
    if export_mbox.upper() == "TRUE":
        export_format = "MBOX"
    mail_options = {"exportFormat": export_format}
...
def create_drive_export_query(
    ...
    terms,
    org_unit="",
    search_method="Specific Accounts(requires emails argument)",
...
def create_groups_export_query(
    export_name,
    emails,
    time_frame,
    start_time,
    ...
    if export_mbox.upper() == "TRUE":
        export_format = "MBOX"
    groups_options = {"exportFormat": export_format}
...
def connect_to_storage():
    try:
        ...
        err_msg = f"There was an error creating the Storage service - {e!s}"
        return_error(err_msg)
    return service
...
def build_document_dict(document):
    ...
    newDocumentDict = {"DocType": os.path.splitext(file_info["@FileName"])[1][1:].strip().lower(), "MD5": file_info["@Hash"]}
...
def get_current_matter_from_context(matter_id):
    ...
    if not context_matter:
        context_matter = {"MatterID": matter_id, "Export": []}
    ...
    try:
        ...
        if not matters:
            ...
            for matter in matters:
                output.append(
                    {"Matter Name": matter.get("name"), "Matter ID": matter.get("matterId"), "Matter State": matter.get("state")}
                )
            ...
            demisto.results(
                {
                    ...
                    "EntryContext": {"GoogleVault.Matter(val.MatterID === obj.MatterID)": context_output},
                }
            )
    except Exception as ex:
        err_msg = str(ex)
    ...
    try:
        ...
        if matter_description:
            markdown = "Matter: {} was created successfully with description: {}.\nID: {}.".format(
                matter_name, matter_description, matter.get("matterId")
            )
        else:
            markdown = "Matter: {} was created successfully without a description.\nID: {}.".format(
                matter_name, matter.get("matterId")
            )
        ...
        markdown += tableToMarkdown(
            title, markdown_matter, ["Matter Name", "Matter ID", "Matter State"]
        )  # Why is the title displayed in a weird way?

        output_context = []
        output_context.append({"Name": matter.get("name"), "MatterID": matter.get("matterId"), "State": matter.get("state")})
        ...
        demisto.results(
            {
                ...
                "Contents": matter,
                "HumanReadable": markdown,
                "EntryContext": {"GoogleVault.Matter(val.MatterID === obj.MatterID)": output_context},
            }
        )
    except Exception as ex:
        err_msg = str(ex)
...
def add_account_to_hold_command():  # Todo: Not sure if context is good (It works, but maybe not according to conventions)
    try:
        ...
        context_output = []
        context_output.append({"ID": hold_id, "matterID": matter_id, "HeldAccount": {"accountID": account_id, "IsHeld": True}})
        ...
        demisto.results(
            {
                ...
                "ContentsFormat": formats["text"],
                "Contents": msg_to_usr,
                "EntryContext": {"GoogleVault.Hold(val.ID === obj.ID)": context_output},
            }
        )
    except Exception as ex:
        err_msg = str(ex)
    ...
    try:
        ...
        for matter in matters:
            if matter.get("name").lower() == wanted_name or matter.get("matterId").lower() == wanted_id:
                ...
                output.append({"Name": matter.get("name"), "MatterID": matter.get("matterId"), "State": matter.get("state")})
            ...
            demisto.results(
                {
                    ...
                    "HumanReadable": markdown,
                    "EntryContext": {"GoogleVault.Matter(val.MatterID === obj.MatterID)": output},
                }
            )
    except Exception as ex:
        err_msg = str(ex)
...
def list_holds_command():
    try:
        ...
        if not holds:
            ...
            context_output = []
            for hold in holds:
                output.append({"Matter ID": matter_id, "Hold Name": hold.get("name"), "Hold ID": hold.get("holdId")})
                context_output.append({"name": hold.get("name"), "ID": hold.get("holdId"), "MatterID": matter_id})
            markdown = ""  # Use this to add extra line
...
def create_hold_command():
    ...
    output = []
    context_output = []
    output.append({"Hold Name": hold_name, "Hold ID": hold_id})
    context_output.append({"name": hold_name, "ID": hold_id, "matterID": matter_id})
    markdown = ""  # Use this to add extra line
...
def create_mail_export_command():
    ...
    validate_input_values(
        [search_method],
        ["All Accounts", "Specific Accounts(requires emails argument)", "Organizational Unit(requires ou argument)"],
    )

    query = create_mail_export_query(
        export_name,
        emails,
        time_frame,
        start_time,
    ...
    new_export = {"MatterID": matter_id, "ExportID": export_id, "Name": export_name, "CreateTime": create_time}
    ...
    demisto.results(
        {
            ...
            "Contents": response,
            "HumanReadable": markdown,
            "EntryContext": {f'GoogleVault.Matter(val.MatterID === "{matter_id}")': new_matter},
        }
    )
...
def create_drive_export_command():
    ...
    validate_input_values(
        [search_method],
        ["Team Drive", "Specific Accounts(requires emails argument)", "Organizational Unit(requires ou argument)"],
    )

    query = create_drive_export_query(
        export_name,
        emails,
        team_drives,
        time_frame,
...
def create_groups_export_command():
    ...
    query = create_groups_export_query(
        export_name, emails, time_frame, start_time, end_time, terms, search_method, export_pst, export_mbox, data_scope
    )
    try:
        response = create_export(service, matter_id, query)
    except Exception as ex:
    ...
    demisto.results(
        {
            ...
            "Contents": "",
            "Type": entryTypes["note"],
            "EntryContext": {f'GoogleVault.Matter(val.MatterID === "{matter_id}")': current_matter},
        }
    )
        ...
        demisto.results(
            {
                ...
                "HumanReadable": markdown,
                "EntryContext": {"GoogleVault.Matter.Export(val.ExportID === obj.ExportID)": contextOutput},
            }
        )
