...
class Layout:
    ...
    def generate_new_tab_id(tab):
        ...
        # update this to always gen valid uuids
        def mod_uid(uid): return uid.replace(uid[random.randrange(len(uid))], random.choice(string.ascii_lowercase))
        def gen_i_id(uid): return f"{tab.get('id')}-{mod_uid(uid[0])}-{'-'.join(uid[1:])}"
        def gen_item_id(uid): return f"{uid[1]}-{mod_uid(uid[2])}-{'-'.join(uid[3:])}"
