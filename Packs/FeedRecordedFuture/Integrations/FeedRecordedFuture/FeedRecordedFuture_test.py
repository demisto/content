...
GET_INDICATOR_TYPE_INPUTS = [
    (
        "ip",
        OrderedDict(
            [("Name", "192.168.1.1"), ("Risk", "89"), ("RiskString", "5/12"), ("EvidenceDetails", '{"EvidenceDetails": []}')]
        ),
        "IP",
    ),
    (
        "ip",
        OrderedDict(
            [("Name", "192.168.1.1/32"), ("Risk", "89"), ("RiskString", "5/12"), ("EvidenceDetails", '{"EvidenceDetails": []}')]
        ),
        "CIDR",
    ),
    (
    ...
    (
        ...
        OrderedDict(
            [
                ("Name", "42a5e275559a1651b3df8e15d3f5912499f0f2d3d1523959c56fc5aea6371e59"),
                ("Algorithm", "SHA-256"),
                ("Risk", "89"),
                ("RiskString", "5/12"),
                ("EvidenceDetails", '{"EvidenceDetails": []}'),
    ...
    (
        ...
        OrderedDict(
            [
                ("Name", "c8092abd8d581750c0530fa1fc8d8318"),
                ("Algorithm", "MD5"),
                ("Risk", "89"),
                ("RiskString", "5/12"),
                ("EvidenceDetails", '{"EvidenceDetails": []}'),
    ...
    (
        "domain",
        OrderedDict(
            [("Name", "domaintools.com"), ("Risk", "89"), ("RiskString", "5/12"), ("EvidenceDetails", '{"EvidenceDetails": []}')]
        ),
        "Domain",
    ),
    (
...
build_iterator_answer_domain = [
    [{"EvidenceDetails": '{"EvidenceDetails": []}', "Name": "domaintools.com", "Risk": "97", "RiskString": "4/37"}]
]

build_iterator_answer_domain_glob = [
    [{"EvidenceDetails": '{"EvidenceDetails": []}', "Name": "*domaintools.com", "Risk": "92", "RiskString": "4/37"}]
]

build_iterator_answer_ip = [
    [{"EvidenceDetails": '{"EvidenceDetails": []}', "Name": "192.168.1.1", "Risk": "50", "RiskString": "4/37"}]
]
...
build_iterator_answer_url = [
    [{"EvidenceDetails": '{"EvidenceDetails": []}', "Name": "www.securityadvisor.io", "Risk": "97", "RiskString": "4/37"}]
]

build_iterator_no_evidence_details_value = [
    [{"EvidenceDetails": None, "Name": "192.168.1.1", "Risk": "50", "RiskString": "4/37"}]
]

build_iterator_answer_vulnerability = [
    [{"EvidenceDetails": '{"EvidenceDetails": []}', "Name": "CVE-2014-1111", "Risk": "90", "RiskString": "4/37"}]
]
...
@pytest.mark.parametrize("indicator_type, risk_rules, build_iterator_answer, value, type", GET_INDICATORS_BY_RISK_RULES_INPUTS)
def test_get_indicators_command_by_risk_rules(mocker, indicator_type, risk_rules, build_iterator_answer, value, type):
    ...
    for rule in risk_rules_list:
        assert (
            f"Indicators from RecordedFuture Feed for {rule} risk rule" in hr
...
@pytest.mark.parametrize(
    "indicator_type, risk_rules, service, expected_err_msg",
    [
        ("url", "dhsAis", "fusion", "You entered a risk rule but the 'connectApi' service is not chosen."),
...
def test_risk_rule_validations(mocker, indicator_type, risk_rules, service, expected_err_msg):
    ...
    """
    mocker.patch(
        "FeedRecordedFuture.Client.get_risk_rules",
