...
class Client(BaseClient):
    ...
    PARAMS = {"output_format": "csv/splunk", "download": 1}  # for faster download
    headers = {"X-RF-User-Agent": "Demisto", "content-type": "application/json"}
    ...
    def __init__(
        ...
        services: list,
        risk_rule: str = None,
        fusion_file_path: str = None,
        insecure: bool = False,
        polling_timeout: int = 20,
        proxy: bool = False,
        malicious_threshold: int = 65,
    ...
    def _build_request(self, service, indicator_type, risk_rule: str | None = None) -> requests.PreparedRequest:
        ...
        if service == "connectApi":
            ...
            response = requests.Request("GET", url + fusion_path, headers=self.headers, params=self.PARAMS)
        else:
                ...
                if chunk:
                    ...
                    for bytes_to_cut in [0, 1, 2, 3]:
                        try:
                            ...
                            demisto.debug(
                                f"RF: Decoding chunk number {chunks_counter} with {bytes_to_cut} bytes cut off using "
                                "UTF-8 failed due to cut-off character while streaming."
                                " Going to cut off one more byte and decode"
                            )
        demisto.debug(
            f"RF: {CHUNK_SIZE=}. Number of chunks received = {chunks_counter}."
    ...
    def run_parameters_validations(self):
            ...
            if "connectApi" not in self.services:
                ...
                for risk_rule in self.risk_rule:
                    if not is_valid_risk_rule(self, risk_rule):
                        return_error(
                            f"The given risk rule: {risk_rule} does not exist,"
        ...
        if self.fusion_file_path is not None and "fusion" not in self.services:
            return_error(
                "You entered a fusion file path but the 'fusion' service is not chosen. "
    ...
    def get_risk_rules(self, indicator_type: str | None = None) -> dict:
        ...
        return self._http_request(
            method="GET", url_suffix=indicator_type + "/riskrules", params=self.PARAMS, headers=self.headers
        )
...
def get_indicators_command(client, args) -> tuple[str, dict[Any, Any], list[dict]]:  # pragma: no cover
    ...
    if client.risk_rule:
        ...
        for risk_rule in client.risk_rule:
            ...
            hr = tableToMarkdown(
                f"Indicators from RecordedFuture Feed for {risk_rule} risk rule:",
                entry_result,
                headers=["Value", "Type"],
                removeNull=True,
            )
        ...
        human_readable = tableToMarkdown(
            "Indicators from RecordedFuture Feed:", entry_results, headers=["Value", "Type"], removeNull=True
        )
...
def get_risk_rules_command(client: Client, args) -> tuple[str, dict, dict]:
    ...
    for entry in result["data"]["results"]:
        entry_result.append(
            {"Name": entry.get("name"), "Description": entry.get("description"), "Criticality": entry.get("criticalityLabel")}
        )
...
def main():  # pragma: no cover
    ...
    client = Client(
        ...
        params.get("insecure"),
        params.get("polling_timeout"),
        params.get("proxy"),
        params.get("threshold"),
        params.get("suspicious_threshold"),
