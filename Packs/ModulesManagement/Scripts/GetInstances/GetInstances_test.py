...
ARGS_SYSTEM_FILTER = {'is_enabled': 'both'}
ARGS_BRAND_FILTER = {'brand': 'EWS v2, splunk', 'is_enabled': 'both'}
ARGS_IS_ENABLED_FILTER = {'is_enabled': 'active'}
ARGS_ALL_FILTERS = {'brand': 'EWS v2, splunk', 'is_enabled': 'active'}
PREPARED_ARGS_ALL_FILTERS = {
    'is_enabled': ARGS_ALL_FILTERS['is_enabled'],
...
data_test_prepare_args_with_invalid_value = [{}, {'is_enabled': 'test'}]
...
def test_prepare_args_with_invalid_value(input_args):
    try:
        ...
        assert str(error) == "is_enabled should be one of the following 'active', 'both', 'disabled'"


def test_without_any_filter():
    assert filter_config({'brand': 'EWS v2'}, is_enabled='both')
    assert not filter_config({'brand': 'Scripts'}, is_enabled='both')
    assert not filter_config({'brand': 'Builtin'}, is_enabled='both')
    assert not filter_config({'brand': 'testmodule'}, is_enabled='both')


def test_with_enabled_filter():
    assert filter_config({'state': 'active'}, is_enabled='active')
    assert not filter_config({'state': 'disabled'}, is_enabled='active')
