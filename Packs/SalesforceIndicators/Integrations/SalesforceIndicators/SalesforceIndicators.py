...
class Client(BaseClient):
    def __init__(
        ...
        self._headers = {"Authorization": f"Bearer {self.session_data['access_token']}", "Content-Type": "application/json"}
        ...
        self.score = (
            1
            if self.feedReputation == "Good"
            else 2
            if self.feedReputation == "Suspicious"
            else 3
            if self.feedReputation == "Bad"
            else 0
        )
    ...
    def get_new_token(self):
        ...
        res = self._http_request(
            "POST", "/services/oauth2/token", headers={"Content-Type": "application/x-www-form-urlencoded"}, data=body
        )
        return res
    ...
    def raw_query(self, query, raw_query=False):
        ...
        if raw_query:
            res = self._http_request("GET", f"{query}", timeout=600)
        else:
            res = self._http_request("GET", "query", params=params, timeout=600)
        return res
...
def fetch_indicators_command(client: Client, manual_run: bool = False):
    ...
    if not manual_run:
        ...
        demisto.results({"SFDC.Indicators": indicators, "Count": len(indicators)})
