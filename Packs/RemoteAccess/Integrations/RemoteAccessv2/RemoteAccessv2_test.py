...
@pytest.mark.parametrize(
    "cipher_arg, key_arg, server_ciphers, server_key_algorithms",
    [
        (set(), set(), set(), set()),
        (set(), {"diffie-hellman-group14-sha256"}, set(), {"diffie-hellman-group14-sha256", "diffie-hellman-group14-sha128"}),
...
@pytest.mark.parametrize(
    "command, mock_std_output, mock_std_error, success",
    [
        ("echo lol", "lol", "", True),
...
def test_execute_shell_command(mocker, command, mock_std_output, mock_std_error, success):
    ...
    from RemoteAccessv2 import CommandResults, execute_shell_command, tempfile
    ...
    assert results.outputs == [{"output": mock_std_output, "error": mock_std_error, "success": success, "command": command}]
...
def test_copy_to_command_invalid_entry_id(mocker):
    ...
    with pytest.raises(DemistoException, match="Could not find given entry ID path. Please assure given entry ID is correct."):
...
def test_copy_to_command_invalid_arguments():
    ...
    mock_client: SSHClient = SSHClient()
    with pytest.raises(
        DemistoException, match='Please provide at most one of "dest-dir" argument or "destination_path", not both.'
    ):
...
investigation_data = {
    ...
    "systems": [
        {
            "ciphers": None,
            "credentials": "ssh - test",
            "engineId": "",
            "host": "11.234.1.17",
            "integrationinstanceid": "",
            "issharedagent": False,
            "name": "i-0fb7ben3de5e85283",
            "os": "linux",
            "servicesID": "",
            "terminalOptions": {
                "Echo": 0,
...
@pytest.mark.parametrize(
    "given_systems, given_hosts, investigation, expected_message",
    [
        system_found,
        host_found,
