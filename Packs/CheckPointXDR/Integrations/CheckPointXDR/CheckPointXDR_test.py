...
from CheckPointXDR import (Client, update_remote_system_command, fetch_incidents, 
                           get_instances_id, map_severity, parse_incidents, 
                           test_module as check_module, get_mapping_fields_command,
                           main, OUTGOING_MIRRORED_FIELDS)
...
def test_client_login_failure(mocker):
    ...
    try:
        client._login()
        assert False, "Expected DemistoException was not raised"
...
def test_main_test_module(mocker):
    ...
    mocker.patch('CheckPointXDR.demisto.params', return_value={
        'url': 'https://test.url',
...
def test_client_get_incidents(mocker):
    ...
    mock_response.json.return_value = {
        "data": {
            "incidents": util_load_json('./test_data/checkpointxdr-get_incidents.json').get('objects')
        }
    }
