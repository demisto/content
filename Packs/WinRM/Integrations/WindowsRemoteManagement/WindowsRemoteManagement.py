...
class Client:
    ...
    def run(self):
        s: Session
        if self.auth == "ntlm":
            s = winrm.Session(target=self.hostname, auth=(self.username, self.password), transport=self.auth)
        else:  # self.auth == "kerberos":
            demisto.debug(f"{self.auth=}. Should be kerberos")
            s = winrm.Session(target=self.hostname, auth=(self.username, self.password), transport=self.auth, realm=self.realm)
