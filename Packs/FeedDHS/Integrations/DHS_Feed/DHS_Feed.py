...
class SetList(list):
    def __init__(self, extract: Optional[Callable[[Any], Hashable]] = None):
        super().__init__()
        self._set: Set = set()
        self._extract = extract

    def append(self, element: Any):
        temp = self._extract(element) if self._extract else element
        try:
            _hash = hash(temp)
        except TypeError:
            raise DemistoException('please add extract function.')
        if _hash not in self._set:
            self._set.add(_hash)
            super(SetList, self).append(element)
    ...
    def __len__(self):
        return super(SetList, self).__len__()
...
class TempFile:
    def __init__(self, data, suffix=None):
        self.path = tempfile.mkstemp(suffix=suffix)[1]
