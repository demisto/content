...
class TestHelpers:
    ...
    data_test_ssl_files_checker = [("test_data/rsa/2048b-rsa-example-keypair.pem", "test_data/rsa/2048b-rsa-example-cert.pem")]
...
class TestSafeDataGet:
    data_test_safe_data_get = [("without_list", "one_level_get"), (["standard_list", "test"], "multi_level_get")]
    ...
    def test_safe_data_get(self, get_list, expected_output):
        dict_data = {"without_list": "one_level_get", "standard_list": {"test": "multi_level_get"}}
    ...
    data_test_get_none_existing_path_with_prefix = [
        ({"TEST:standard_list": {"TEST:test": "multi_level_get"}}, ["standard_list1", "test"]),
        ({"TEST:standard_list": {"TEST:test": "multi_level_get"}}, ["standard_list", "test1"]),
...
class TestIndicators:
    ...
    def test_tlp_color_from_header(self, data_type, tlp_colors):
        blocks_headers = list(map(self.get_stix_header, self.read_json(f"test_data/blocks/blocks_from_{data_type}_data.json")))
        test_tlp_colors = list(map(Indicators._tlp_color_from_header, blocks_headers))
        assert test_tlp_colors == tlp_colors

    data_test_source_from_header = [(IP, ["Infoblox Inc"] * 3), (File, ["Reversing Labs"] * 3)]

    @pytest.mark.parametrize("data_type, sources", data_test_source_from_header)
    def test_source_from_header(self, data_type, sources):
        sources_headers = list(map(self.get_stix_header, self.read_json(f"test_data/blocks/blocks_from_{data_type}_data.json")))
