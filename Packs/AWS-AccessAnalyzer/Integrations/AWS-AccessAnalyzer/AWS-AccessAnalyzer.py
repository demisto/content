...
def list_analyzed_resource_command(aws_client, args):
    ...
    analyzed_resources = [resource | {"analyzerArn": analyzer_arn} for resource in response["analyzedResources"]]
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(
            "AWS Access Analyzer Resources", analyzed_resources, headers=headers, headerTransform=pascalToSpace
        ),
...
def list_findings_command(aws_client, args):
    ...
    findings = [finding | {"analyzerArn": args.get("analyzerArn")} for finding in response["findings"]]
    headers = ["id", "resource", "principal", "condition", "updatedAt", "status"]
    return CommandResults(
        outputs_prefix="AWS.AccessAnalyzer.Finding",
        outputs_key_field="id",
        outputs=findings,
        readable_output=tableToMarkdown("AWS Access Analyzer Findings", findings, headers=headers, headerTransform=pascalToSpace),
        raw_response=response,
    )
...
def update_findings_command(aws_client, args):
    client = get_aws_session(aws_client, args)
    kwargs = {"analyzerArn": args.get("analyzerArn"), "ids": argToList(args.get("findingIds")), "status": args.get("status")}
...
def get_filters(args):
    return {field: {"eq": [args.get(field)]} for field in ("resourceType", "status") if args.get(field)}


def get_findings(aws_client, args):
    kwargs = {"analyzerArn": args.get("analyzerArn"), "sort": {"attributeName": "UpdatedAt", "orderBy": "DESC"}}
...
def main():
    ...
    try:
        ...
        return_error(f"Error has occurred in AWS Access Analyzer Integration: {e!s}")
