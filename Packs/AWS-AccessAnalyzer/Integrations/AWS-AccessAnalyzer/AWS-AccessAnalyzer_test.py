...
def test_test_module(mocker):
    ...
    mock_required_fields(
        mocker, command_name="test-module", mocked_method_name="list_analyzers", method_return_value=mocked_results
    )
...
def test_start_resource_scan_command(mocker):
    mock_required_fields(mocker, command_name="aws-access-analyzer-start-resource-scan", mocked_method_name="start_resource_scan")
...
def test_update_findings_command(mocker):
    mock_required_fields(mocker, command_name="aws-access-analyzer-update-findings", mocked_method_name="update_findings")
...
@pytest.mark.parametrize(
    argnames="lats_run_obj, expected_inc_number", argvalues=[(None, 1), ({"time": "1676316480000"}, 2), ({"time": "1000"}, 3)]
)
def test_fetch_incidents_command__various_last_run(mocker, lats_run_obj, expected_inc_number):
    ...
    mock_required_fields(
        mocker, command_name="fetch-incidents", mocked_method_name="list_findings", method_return_value=mocked_results
    )
...
def test_fetch_incidents_command__with_next_token(mocker, next_token, expected_call_count):
    ...
    mock_required_fields(mocker, command_name="fetch-incidents", mocked_method_name="list_findings")
...
def test_exception_in_test_module(mocker):
    ...
    mock_required_fields(mocker, command_name="test-module", mocked_method_name="list_analyzers")
