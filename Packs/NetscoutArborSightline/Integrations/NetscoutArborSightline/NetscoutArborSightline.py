...
IMPORTANCE_DICTIONARY = {"Low": "0", "Medium": "1", "High": "2"}
ONGOING_DICTIONARY = {
    "Ongoing": "true",
    "Not Ongoing": "false",
}
IP_DICTIONARY = {"IPv4": 4, "IPv6": 6}
...
MANAGED_OBJECTS_HR_HEADERS = ["id", "name", "tags", "match_type", "match_enabled", "match", "family", "autodetected"]
...
class NetscoutClient(BaseClient):
    ...
    RELATIONSHIP_TO_TYPE = {"routers": "router"}
    ...
    def http_request(
        ...
        return_empty_response: Optional[bool] = None,
    ...
    def build_relationships(self, **kwargs) -> dict:
        ...
        for key, val in kwargs.items():
            if val:
                ...
                if key == "routers":
                    relationships[key] = {"data": [{"type": _type, "id": val[0]}]}
                else:
                    relationships[key] = {"data": {"type": _type, "id": val}}
        ...
        for key, val in attributes_dict.items():
            ...
            if key not in operator_names and val:
                ...
                if operator_name := self.OPERATOR_NAME_DICTIONARY.get(key):
                    operator = attributes_dict.get(operator_name, "") if attributes_dict.get(operator_name) else "="
    ...
    def create_mitigation(self, data: dict) -> dict:
        return self.http_request(method="POST", url_suffix="mitigations/", json_data=data)

    def delete_mitigation(self, mitigation_id: str):
        self.http_request(method="DELETE", url_suffix=f"mitigations/{mitigation_id}", return_empty_response=True)

    def mitigation_template_list(self) -> dict:
        return self.http_request(method="GET", url_suffix="mitigation_templates/")

    def router_list(self) -> dict:
        return self.http_request(method="GET", url_suffix="routers/")

    def managed_object_list(self, page: Optional[int] = None, page_size: Optional[int] = None) -> dict:
        return self.http_request(method="GET", url_suffix="managed_objects/", params=assign_params(page=page, perPage=page_size))

    def tms_group_list(self) -> dict:
        return self.http_request(method="GET", url_suffix="tms_groups/")
...
def build_output(data: dict, extend_data: bool = False, key_to_flat: str = "attributes", keys_to_remove: list = None) -> dict:
...
def list_alerts_command(client: NetscoutClient, args: dict):
    ...
    if alert_id:
        ...
        data_relationships_filter = (
            f"AND /data/relationships/managed_object/data/id={managed_object_id}" if managed_object_id else ""
        )
    ...
    return CommandResults(
        outputs_prefix="NASightline.Alert",
...
def alert_annotation_list_command(client: NetscoutClient, args: dict):
    ...
    return CommandResults(
        outputs_prefix="NASightline.AlertAnnotation",
...
def mitigation_list_command(client: NetscoutClient, args: dict):
    ...
    mitigations = [
        build_output(data=mitigation, keys_to_remove=["relationships", "subobject"], extend_data=extend_data)
        for mitigation in data
    ]
    return CommandResults(
        outputs_prefix="NASightline.Mitigation",
        outputs_key_field="id",
        outputs=mitigations,
...
def mitigation_create_command(client: NetscoutClient, args: dict):
    ...
    if not ip_version:
        raise DemistoException(
            'ip_version value can be one of the following: '
    ...
    relationships = client.build_relationships(
        alert=alert_id,
        managed_object=managed_object_id,
        mitigation_template=mitigation_template_id,
        routers=router_ids,
        tms_group=tms_group_id,
    )
    attributes = assign_params(
        description=description, ip_version=ip_version, name=name, ongoing=ongoing, subtype=sub_type, subobject=sub_object
    )
...
def mitigation_template_list_command(client: NetscoutClient, args: dict):
    ...
    return CommandResults(
        outputs_prefix="NASightline.MitigationTemplate",
        outputs_key_field="id",
...
def router_list_command(client: NetscoutClient, args: dict):
    ...
    return CommandResults(
        outputs_prefix="NASightline.Router",
        outputs_key_field="id",
        outputs=routers,
        readable_output=tableToMarkdown("Router list", hr, headers=ROUTERS_HR_HEADERS, removeNull=True),
        raw_response=raw_result,
    )
...
def managed_object_list_command(client: NetscoutClient, args: dict):
    ...
    return CommandResults(
        outputs_prefix="NASightline.ManagedObject",
        outputs_key_field="id",
        outputs=objects,
        readable_output=tableToMarkdown("Managed object list", hr, headers=MANAGED_OBJECTS_HR_HEADERS, removeNull=True),
        raw_response=raw_result,
    )
...
def tms_group_list_command(client: NetscoutClient, args: dict):
    ...
    return CommandResults(
        outputs_prefix="NASightline.TMSGroup",
        outputs_key_field="id",
        outputs=groups,
...
def main() -> None:
    try:
        ...
        headers: dict = {"X-Arbux-APIToken": api_token}
