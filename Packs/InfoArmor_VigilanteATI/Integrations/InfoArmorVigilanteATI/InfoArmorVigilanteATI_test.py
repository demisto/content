...
@pytest.mark.parametrize(
    "mock_response, expected_readable_output, expected_context",
    [({"results": {}}, "No entries.", {}), ({"results": {"test_results": "test"}}, "test", {"test_results": "test"})],
)
...
def test_get_leak_command(mocker):
    ...
    expected_readable_output = (
        "### Accounts related to leak aa66573902ed4f4bfb2ae08ebac390c3\n|email|domain|"
...
def test_query_domains_command(mocker):
    ...
    expected_readable_output = (
        "### Accounts related to domain: None\n|leak_id|email|password|source_type|type_id|"
...
def test_watchlist_add_accounts_command(mocker):
    ...
    mock_response = {"added": ["1", "2"], "already on watchlist": ["3", "4"], "invalid": ["5", "6"]}
...
def test_watchlist_remove_accounts_command(mocker):
    ...
    mock_response = {"removed": ["1", "2"], "not on watchlist": ["3", "4"]}
...
def test_get_watchlist_accounts_command(mocker):
    ...
    mock_response = {"identifiers": {"id1": "111", "id2": "222", "id3": "333"}, "token": "test_token"}

    expected_context = {
        "VigilanteATI.WatchlistQueryToken(true==true)": "test_token",
        "VigilanteATI.Watchlist(val.identifier == obj.identifier)": {"id1": "111", "id2": "222", "id3": "333"},
    }
...
def test_usage_info_command(mocker):
    ...
    mock_response = {"num_queries_left": 1, "num_queries_allotted": 2}
...
@pytest.mark.parametrize(
    "params_to_test, expected_params",
    [
