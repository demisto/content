    ...
    demisto.results(
        {
            ...
            "Contents": results,
            "HumanReadable": markdown,
            "EntryContext": {"VigilanteATI.ElasticsearchResults": results.get("results")},
        }
    )
    ...
    demisto.results(
        {
            ...
            "Contents": results,
            "HumanReadable": markdown,
            "EntryContext": {"SearchResults": results.get("results")},
        }
    )
    ...
    demisto.results(
        {
            ...
            "HumanReadable": markdown,
            "EntryContext": {"Hosts(val.ip === obj.ip)": raw_host_data.get("hosts")},
        }
    )
...
def get_leak(leak_id, limit, domains, token):
    ...
    """
    params = remove_none_params({"leak_id": leak_id, "limit": limit, "domains": domains, "token": token})
...
def get_leak_command(args):
    ...
    markdown = tableToMarkdown(
        f"Accounts related to leak {leak_id}", leak.get("accounts"), ["email", "domain", "password", "type_id"]
    )
    outputs = {"VigilanteATI.Leaks(val.leak_id === obj.leak_id)": {"leak_id": leak_id, "accounts": accounts}}
    if leak.get("token"):
...
def query_ecrime_intelligence_database(query, q_forum, q_start_data, limit, re_token=None):
    params = remove_none_params(
        {"query": query, "q_forum": q_forum, "q_start_data": q_start_data, "limit": limit, "re_token": re_token}
    )
...
def query_ecrime_intelligence_database_command(args):
    ...
    outputs = {"VigilanteATI.ECrimePosts": posts}
...
def query_accounts(account_identifier, limit, days_ago):
    ...
    body = remove_none_params({"account_identifier": account_identifier, "limit": limit, "daysAgo": days_ago})
...
def query_accounts_command(args):
    ...
    markdown = tableToMarkdown(
        "Leaks related to email accounts \n{}".format("\n".join(emails)),
        accounts,
...
def domain_info(domain_identifier, subdomains, days_ago):
    params = remove_none_params({"domain_identifier": domain_identifier, "subdomains": subdomains, "daysAgo": days_ago})
...
def query_domains(domain, days_ago, limit, token=None):
    params = remove_none_params({"domain_identifier": domain, "limit": limit, "daysAgo": days_ago, "token": token})
...
def query_domains_command(args):
    ...
    for i, _ in enumerate(accounts):
        ...
        del account["plain"]
    markdown = tableToMarkdown(
        f"Accounts related to domain: {domain}", accounts, ["leak_id", "email", "password", "source_type", "type_id"]
    )

    outputs = {
        "VigilanteATI.Domain(val.domain == obj.domain)": {"domain": query_results.get("domain_identifier"), "accounts": accounts}
    }
...
def get_report(date, account_identifier, limit, token=None):
    params = remove_none_params({"date": date, "account_identifier": account_identifier, "limit": limit, "token": token})
...
def watchlist_add_accounts(account_identifiers, _type, tag):
    body = remove_none_params({"account_identifiers": account_identifiers, "type": _type, "tag": tag})

    params = {"account_identifiers": json.dumps(account_identifiers), "type": _type, "tag": tag}
...
def watchlist_add_accounts_command(args):
    ...
    demisto.results(
        {"Type": entryTypes["note"], "ContentsFormat": formats["json"], "Contents": result, "HumanReadable": markdown}
    )


def watchlist_remove_accounts(account_identifiers):
    params = remove_none_params({"account_identifiers": json.dumps(account_identifiers)})
...
def get_watchlist_accounts(limit, token=None):
    params = remove_none_params({"limit": limit, "token": token})
...
def usage_info_command():
    ...
    demisto.results({"Type": entryTypes["note"], "ContentsFormat": formats["json"], "Contents": usage, "HumanReadable": markdown})
...
def main():  # pragma: no coverage
    ...
    if command == "test-module":
        query_infected_host_data(days_ago=0, q_address="8.8.8.8", limit=1)
        usage_info()
