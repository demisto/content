...
class MainTester:
                ...
                filter(
                    ...
                    (
                        ...
                        for v1, v2 in itertools.zip_longest(
                            sorted(
                                obj1, key=functools.cmp_to_key(functools.partial(MainTester.compare_obj, skip_keys=skip_keys))
                            ),
                            sorted(
                                obj2, key=functools.cmp_to_key(functools.partial(MainTester.compare_obj, skip_keys=skip_keys))
                            ),
                        )
                    ),
                ),
    ...
    def __get_list_from_content_bundle(
        ...
        with tarfile.open(bundle_file, "r") as t:
            ...
            file_path = next((x for x in t.getnames() if os.path.basename(x) == file_name), None)
            if not file_path:
                return None
        ...
        if isinstance(conf, dict):
            ...
            if _type == "file":
                ...
                self.__xql_last_resp = {
                    ...
                    "Contents": {},
                    "EntryContext": {ec_key: resp},
                }
            elif _type == "data":
                self.__xql_last_resp = {
                    "Type": 1,  # EntryType.NOTE
                    "Contents": {},
                    "EntryContext": {ec_key: conf.get("data")},
                }
            elif _type == "error":
                self.__xql_last_resp = {
    ...
    def __demisto_dt(
        ...
        if path == "recordset":
            ...
            if var == func:
                return "\n".join(" - " + x.get("text") for x in recordset)
    ...
    def __execute_command(
        ...
        if command == "getList":
            ...
            for ent in filter(None, [lists.get(list_name)] + to_list(lists.get("*"))):
                data_type = ent.get("type")
                if data_type == "raw":
                    return ent.get("data")
                elif data_type == "content-bundle":
                    list_data = self.__get_list_from_content_bundle(bundle_file=ent.get("data"), list_name=list_name)
    ...
    def __return_error(
        ...
        if errors := self.__config.get("errors"):
            assert any(re.match(fnmatch.translate(m), str(message)) for m in to_list(errors.get("messages")))
    ...
    def main(
        ...
        if self.__config.get("errors"):
            ...
            if expected_entry is not None:
                skip_keys = []
                if to_bool(demisto.get(self.__config, "config.validation.skip-color", "false")):
                    skip_keys = ["color"]
...
```
    ...
    def test_triple_quotes(self, mocker, triple_quotes_to_string, input_template, output_template) -> None:
        args = {
