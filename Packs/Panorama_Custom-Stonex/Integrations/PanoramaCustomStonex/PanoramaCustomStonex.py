...
CONSTANT_PACK_VERSION = '2.5.1'
demisto.debug('pack id = PAN-OS, pack version = 2.5.1')
...
class PanosResponse:
    ...
    def handle_illegal_chars(self, dictionary: dict, illegal_chars: set | None = None):
        if not illegal_chars:
            return dictionary
        return {key.replace(char, ""): val for key, val in dictionary.items() for char in illegal_chars}
    ...
    def to_class(self, response, ignored_keys: set | None = None, illegal_chars: set | None = None) -> PanosNamespace:
        ...
        return json.loads(
            ...
            object_hook=lambda d: self.PanosNamespace(
                **self.handle_illegal_chars(d, illegal_chars=illegal_chars), ignored_keys=ignored_keys
            ),
        )
...
def http_request(
    ...
    if ("response" not in json_result or "@code" not in json_result["response"]) and not json_result["response"][
        "@status"
    ] != "success":
        return json_result
    ...
    if json_result["response"]["@status"] != "success":
        if "msg" in json_result["response"] and "line" in json_result["response"]["msg"]:
            ...
            if response_msg == "No such node":
                raise Exception("Object was not found, verify that the name is correct and that the instance was committed.")
            ...
            # catch non valid jobID errors and display a meaningful message
                ...
                isinstance(json_result["response"]["msg"]["line"], str)
                and json_result["response"]["msg"]["line"].find("job") != -1
                and (
                    json_result["response"]["msg"]["line"].find("not found") != -1
                    or json_result["response"]["msg"]["line"].find("No such query job")
                )
                != -1
            ):
            ...
            # catch already registered ip tags and return this as an entry.note
                ...
                if isinstance(json_result["response"]["msg"]["line"]["uid-response"]["payload"]["register"]["entry"], list):
                    ips = [
                        o["@ip"] for o in json_result["response"]["msg"]["line"]["uid-response"]["payload"]["register"]["entry"]
                    ]
                else:
                    ips = json_result["response"]["msg"]["line"]["uid-response"]["payload"]["register"]["entry"]["@ip"]
                return_results("IP " + str(ips) + " already exist in the tag. All submitted IPs were not registered to the tag.")
                sys.exit(0)
        ...
        if "@code" in json_result["response"]:
            raise Exception(
                "Request Failed.\nStatus code: "
                + str(json_result["response"]["@code"])
                + "\nWith message: "
                + str(json_result["response"]["msg"]["line"])
            )
        else:
...
def parse_pan_os_un_committed_data(dictionary, keys_to_remove):
    ...
    for key in dictionary:
        ...
        elif isinstance(dictionary[key], list) and isinstance(dictionary[key][0], dict) and dictionary[key][0].get("#text"):
            temp_list = []
...
def do_pagination(
    entries: list, page: Optional[int] = None, page_size: int = DEFAULT_LIMIT_PAGE_SIZE, limit: int = DEFAULT_LIMIT_PAGE_SIZE
):
...
def prepare_security_rule_params(
    api_action: str = None,
    rulename: str = None,
    source: Any = None,
    destination: Any = None,
    negate_source: str = None,
    negate_destination: str = None,
    action: str = None,
    service: List[str] = None,
    disable: str = None,
    application: List[str] = None,
    source_user: List[str] = None,
    category: List[str] = None,
    from_: str = None,
    to: str = None,
    description: str = None,
    target: str = None,
    log_forwarding: str = None,
    disable_server_response_inspection: str = None,
    tags: List[str] = None,
    profile_setting: str = None,
    where: str = "bottom",
    dst: str = None,
) -> Dict:
    ...
    if DEVICE_GROUP:
        if not PRE_POST:
            raise Exception("Please provide the pre_post argument when configuring " "a security rule in Panorama instance.")
        else:
...
def get_pan_os_version() -> str:
    ...
    """
    params = {"type": "version", "key": API_KEY}
...
def panorama_test(fetch_params):
    ...
    http_request(URL, "GET", params=params)
...
def get_device_groups_names():
    ...
    """
    params = {"action": "get", "type": "config", "xpath": "/config/devices/entry/device-group/entry", "key": API_KEY}
...
def start_tsf_export():
    ...
    """
    params = {"type": "export", "category": "tech-support", "key": API_KEY}
    result = http_request(URL, "GET", params=params)
    return result
...
def get_tsf_export_status(job_id: str):
    ...
    """
    params = {"type": "export", "category": "tech-support", "action": "status", "job-id": job_id, "key": API_KEY}
    result = http_request(URL, "GET", params=params)
    return result
...
def download_tsf(job_id: str):
    ...
    """
    params = {"type": "export", "category": "tech-support", "action": "get", "job-id": job_id, "key": API_KEY}
...
def export_tsf_command(args: dict):
    ...
    if job_id := args.get("job_id"):
        job_status = dict_safe_get(get_tsf_export_status(job_id), ["response", "result", "job", "status"])
        return PollResult(
...
def device_group_test():
    ...
    if DEVICE_GROUP not in device_group_names:
        raise Exception(
            f'Device Group: {DEVICE_GROUP} does not exist.'
...
def template_test(template: str):
    ...
    if template not in template_names:
        raise Exception(
            f'Template: {template} does not exist.' f' The available Templates for this instance: {", ".join(template_names)}.'
        )
...
def panorama_commit(args):
    ...
    params = {"type": "commit", "cmd": f"<commit>{command}</commit>", "key": API_KEY}
    if is_partial:
    ...
    if job_id := args.get("commit_job_id"):
        ...
        return PollResult(
            ...
            partial_result=CommandResults(
                ...
                else f"Waiting for commit job ID {job_id} to finish..."
            ),
        )
    ...
    return_results(
        {
            ...
            "HumanReadable": tableToMarkdown(
                "Commit status:", commit_status_output, ["JobID", "Status", "Details", "Warnings"], removeNull=True
            ),
...
def panorama_push_to_device_group_command(args: dict):
    ...
    if push_job_id := args.get("push_job_id"):
        ...
        if job_id:
            ...
            push_output = CommandResults(readable_output=result.get("response", {}).get("msg") or "There are no changes to push.")
            continue_to_poll = False
...
def panorama_push_to_template_command(args: dict):
    ...
    if "result" in result["response"]:
        ...
        push_output = {"Template": TEMPLATE, "JobID": result["response"]["result"]["job"], "Status": "Pending"}
        return_results(
            {
                "Type": entryTypes["note"],
                "ContentsFormat": formats["json"],
                "Contents": result,
                "ReadableContentsFormat": formats["markdown"],
                "HumanReadable": tableToMarkdown("Push to Template:", push_output, ["JobID", "Status"], removeNull=True),
                "EntryContext": {"Panorama.Push(val.JobID == obj.JobID)": push_output},
            }
        )
    else:
...
def panorama_push_to_template_stack_command(args: dict):
    ...
    if "result" in result["response"]:
        # commit has been given a jobid
        push_output = {"TemplateStack": template_stack, "JobID": result["response"]["result"]["job"], "Status": "Pending"}
        return_results(
            {
                "Type": entryTypes["note"],
                "ContentsFormat": formats["json"],
...
def panorama_list_addresses(tag: Optional[str] = None):
    params = {"action": "get", "type": "config", "xpath": XPATH_OBJECTS + "address/entry", "key": API_KEY}
    ...
    return_results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": tableToMarkdown(
                "Addresses:", addresses_output, ["Name", "IP_Netmask", "IP_Range", "IP_Wildcard", "FQDN", "Tags"], removeNull=True
            ),
            "EntryContext": {"Panorama.Addresses(val.Name == obj.Name)": addresses_output},
        }
    )
    ...
    return_results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": tableToMarkdown(
                "Address:", address_output, ["Name", "IP_Netmask", "IP_Range", "FQDN", "Tags"], removeNull=True
            ),
            "EntryContext": {"Panorama.Addresses(val.Name == obj.Name)": address_output},
        }
    )


@logger
def panorama_create_address(
    address_name: str,
    fqdn: str | None = None,
...
def panorama_create_address_command(args: dict):
    ...
    if tags := set(argToList(args.get("tag", []))):
        result = http_request(
            URL, "GET", params={"type": "config", "action": "get", "key": API_KEY, "xpath": f"{XPATH_OBJECTS}tag"}
        )
    ...
    if sum(map(bool, [(fqdn is not None), (ip_netmask is not None), (ip_range is not None), (ip_wildcard is not None)])) != 1:
        raise DemistoException(f"Please specify exactly one of the following arguments: fqdn, ip_netmask, ip_range, ip_wildcard.")
...
def pan_os_edit_address_command(args):
    ...
    return CommandResults(raw_response=raw_response, readable_output=f"Address {address_name} was edited successfully.")
...
def prettify_address_groups_arr(address_groups_arr: list) -> List:
    ...
    for address_group in address_groups_arr:
        pretty_address_group = {"Name": address_group["@name"], "Type": "static" if "static" in address_group else "dynamic"}
        if DEVICE_GROUP:
...
def panorama_list_address_groups(tag: str | None = None):
    params = {"action": "get", "type": "config", "xpath": XPATH_OBJECTS + "address-group/entry", "key": API_KEY}
    ...
    return_results(
        {
            ...
            "HumanReadable": tableToMarkdown(
                "Address groups:",
                address_groups_output,
                ["Name", "Type", "Addresses", "Match", "Description", "Tags"],
                removeNull=True,
            ),
            "EntryContext": {"Panorama.AddressGroups(val.Name == obj.Name)": address_groups_output},
        }
    )
    ...
    return_results(
        {
            ...
            "HumanReadable": tableToMarkdown(
                "Address group:",
                prettify_address_group(result),
                ["Name", "Type", "Addresses", "Match", "Description", "Tags"],
                removeNull=True,
            ),
            "EntryContext": {"Panorama.AddressGroups(val.Name == obj.Name)": prettify_address_group(result)},
        }
    )


@logger
def panorama_create_static_address_group(
    address_group_name: str, addresses: list, description: str | None = None, tags: list | None = None
):
    params = {
        "action": "set",
        "type": "config",
...
def panorama_create_dynamic_address_group(
    address_group_name: str, match: Optional[str], description: str | None = None, tags: list | None = None
):
    params = {
        "action": "set",
        "type": "config",
        "xpath": XPATH_OBJECTS + "address-group/entry[@name='" + address_group_name + "']",
        "element": "<dynamic>"
        + add_argument(match, "filter", False)
        + "</dynamic>"
        + add_argument(description, "description", False)
        + add_argument_list(tags, "tag", True),
        "key": API_KEY,
...
def panorama_create_address_group_command(args: dict):
    ...
    address_group_output = {"Name": address_group_name, "Type": type_}
    if DEVICE_GROUP:
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.AddressGroups(val.Name == obj.Name)": address_group_output},
        }
    )
...
def panorama_edit_address_group_command(args: dict):
    ...
    element_to_remove = argToList(args["element_to_remove"]) if "element_to_remove" in args else None
    ...
    if type_ == "static":
        if (element_to_add and element_to_remove) or (not element_to_add and not element_to_remove):
            raise Exception(
                "To edit a Static Address group,"
    ...
    params = {"action": "edit", "type": "config", "key": API_KEY, "xpath": "", "element": ""}
...
def panorama_list_services(tag: str | None = None):
    params = {"action": "get", "type": "config", "xpath": XPATH_OBJECTS + "service/entry", "key": API_KEY}
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.Services(val.Name == obj.Name)": services_output},
        }
    )
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.Services(val.Name == obj.Name)": service_output},
        }
    )


@logger
def panorama_create_service(
    service_name: str,
    protocol: str,
...
def panorama_create_service_command(args: dict):
    ...
    service_output = {"Name": service_name, "Protocol": protocol, "DestinationPort": destination_port}
    if DEVICE_GROUP:
...
def prettify_service_groups_arr(service_groups_arr: list):
    ...
    for service_group in service_groups_arr:
        pretty_service_group = {"Name": service_group["@name"], "Services": service_group["members"]["member"]}
        if DEVICE_GROUP:
...
def panorama_list_service_groups(tag: str | None = None):
    params = {"action": "get", "type": "config", "xpath": XPATH_OBJECTS + "service-group/entry", "key": API_KEY}
    ...
    return_results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": tableToMarkdown(
                "Service groups:", service_groups_output, ["Name", "Services", "Tags"], removeNull=True
            ),
            "EntryContext": {"Panorama.ServiceGroups(val.Name == obj.Name)": service_groups_output},
        }
    )
    ...
    return_results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": tableToMarkdown(
                "Service group:", pretty_service_group, ["Name", "Services", "Tags"], removeNull=True
            ),
            "EntryContext": {"Panorama.ServiceGroups(val.Name == obj.Name)": pretty_service_group},
        }
    )
...
def panorama_create_service_group(service_group_name: str, services: list, tags: list):
    params = {
        ...
        "xpath": XPATH_OBJECTS + "service-group/entry[@name='" + service_group_name + "']",
        "element": "<members>"
        + add_argument_list(services, "member", True)
        + "</members>"
        + add_argument_list(tags, "tag", True),
        "key": API_KEY,
    }
...
def panorama_create_service_group_command(args: dict):
    ...
    service_group_output = {"Name": service_group_name, "Services": services}
    if DEVICE_GROUP:
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.ServiceGroups(val.Name == obj.Name)": service_group_output},
        }
    )
...
def panorama_edit_service_group_command(args: dict):
    ...
    services_to_remove = argToList(args["services_to_remove"]) if "services_to_remove" in args else None
    ...
    return_results(
        {
            ...
            "HumanReadable": tableToMarkdown(
                "Custom URL Category:",
                custom_url_category_output,
                ["Name", "Type", "Categories", "Sites", "Description"],
                removeNull=True,
            ),
            "EntryContext": {"Panorama.CustomURLCategory(val.Name == obj.Name)": custom_url_category_output},
        }
    )


@logger
def panorama_create_custom_url_category(
...
def panorama_create_custom_url_category_command(args: dict):
    ...
    custom_url_category, custom_url_category_output = panorama_create_custom_url_category(
        custom_url_category_name, type_, sites, categories, description
    )
    return_results(
        {
            "Type": entryTypes["note"],
...
@logger
def panorama_edit_custom_url_category(custom_url_category_name: str, type_: str, items: list, description: Optional[str] = None):
    major_version = get_pan_os_major_version()
    ...
    custom_url_category_output: Dict[str, Any] = {"Name": custom_url_category_name, "Type": type_}
    if DEVICE_GROUP:
        custom_url_category_output["DeviceGroup"] = DEVICE_GROUP
    if description:
        custom_url_category_output["Description"] = description
    ...
    return_results(
        {
            ...
            "HumanReadable": tableToMarkdown(
                "Updated Custom URL Category:",
                custom_url_category_output,
                ["Name", "Categories", "Sites", "Description"],
                removeNull=True,
            ),
            "EntryContext": {"Panorama.CustomURLCategory(val.Name == obj.Name)": custom_url_category_output},
        }
...
def panorama_get_url_category(url_cmd: str, url: str, target: Optional[str] = None) -> List[str]:
    params = {"action": "show", "type": "op", "key": API_KEY, "cmd": f"<test><{url_cmd}>{url}</{url_cmd}></test>"}
    ...
    for url in urls:
        ...
        try:
            ...
            for category in categories:
                ...
                current_dbot_score = calculate_dbot_score(category.lower(), additional_suspicious, additional_malicious)
            ...
            url_obj = Common.URL(url=url, dbot_score=dbot_score, category=url_dbot_score_category)
            readable_output = err_readable_output or tableToMarkdown(
                "URL", url_obj.to_context(), headerTransform=lambda x: x.partition("(")[0]
            )
            command_results.append(CommandResults(indicator=url_obj, readable_output=readable_output))
        except InvalidUrlLengthException as e:
            score = 0
            category = None
            err_readable_output = str(e)
            dbot_score = Common.DBotScore(
                indicator=url, indicator_type=DBotScoreType.URL, integration_name="PAN-OS", score=score, reliability=reliability
            )
            url_obj = Common.URL(url=url, dbot_score=dbot_score, category=category)
            readable_output = err_readable_output
            command_results.append(CommandResults(indicator=url_obj, readable_output=readable_output))

    url_category_output_hr = []
    for key, value in categories_dict_hr.items():
        url_category_output_hr.append({"Category": key, "URL": value})

    url_category_output = []
    for key, value in categories_dict.items():
        url_category_output.append({"Category": key, "URL": value})
...
def prettify_get_url_filter(url_filter: dict):
    ...
    for category in alert_category_list:
        pretty_url_filter["Category"].append({"Name": category, "Action": "alert"})
    block_category_list = argToList(block_category_list)
    for category in block_category_list:
        pretty_url_filter["Category"].append({"Name": category, "Action": "block"})
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.URLFilter(val.Name == obj.Name)": url_filter_output},
        }
    )
...
def create_url_filter_params(
    ...
    element = (
        add_argument_list(url_category_list, action, True)
...
def panorama_create_url_filter(
    ...
    params = create_url_filter_params(
        url_filter_name, action, url_category_list, override_allow_list, override_block_list, description
    )
...
def panorama_create_url_filter_command(args: dict):
    ...
    result = panorama_create_url_filter(
        url_filter_name, action, url_category_list, override_allow_list, override_block_list, description
    )
    ...
    for category in url_category_list:
        url_filter_output["Category"].append({"Name": category, "Action": action})
...
def verify_edit_url_filter_args(major_version: int, element_to_change: str) -> None:
    if major_version >= 9:  # only url categories are allowed, e.g gambling, abortion
        if element_to_change not in ("allow_categories", "block_categories", "description"):
            raise DemistoException(
                "Only the allow_categories, block_categories, description properties can be changed"
...
@logger
def panorama_edit_url_filter(
    url_filter_name: str, element_to_change: str, element_value: str, add_remove_element: Optional[str] = None
):
...
def prettify_rule(rule: dict):
    ...
    if not isinstance(entries, list):
        entries = [entries]
    rule_devices = [entry.get("@name") for entry in entries if isinstance(entry, dict)]
    ...
    rule_profiles = {key: dict_safe_get(profiles, [key, "member"], "") for key in profile_keys}
...
def panorama_list_rules(xpath: str, name: str | None = None, filters: dict | None = None, query: str | None = None):
    params = {"action": "get", "type": "config", "xpath": xpath, "key": API_KEY}
...
def panorama_list_rules_command(args: dict):
    ...
    filters = assign_params(tags=argToList(args.get("tags")), action=args.get("action"))
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.SecurityRule(val.Name == obj.Name)": pretty_rules},
        }
    )
...
def panorama_edit_rule_items(rulename: str, element_to_change: str, element_value: List[str], behaviour: str):
    ...
    params = {"type": "config", "action": "edit", "key": API_KEY}
    ...
    if element_to_change == "profile-setting":
        ...
        current_objects_items = panorama_get_current_element(element_to_change, params["xpath"], is_commit_required=False)
        if behaviour == "add":
            ...
            if not_existing_values := [item for item in element_value if item not in current_objects_items]:
                return_warning(
                    f'The following {element_to_change}s do not exist: {", ".join(not_existing_values)}',
    ...
    rule_output = {"Name": rulename, ELEM_TO_CONTEXT[element_to_change]: values}
    if DEVICE_GROUP:
        rule_output["DeviceGroup"] = DEVICE_GROUP
...
def build_audit_comment_params(name: str, pre_post: str, audit_comment: str = "", policy_type="security", xml_type="set") -> dict:
    """
    ...
    return {"type": "op", "cmd": build_audit_comment_cmd(_xpath, audit_comment, xml_type), "key": API_KEY}
...
def panorama_edit_rule_command(args: dict):
    ...
    if behaviour != "replace":
        ...
        rule_output = {"Name": rulename, ELEM_TO_CONTEXT[element_to_change]: element_value}
        if DEVICE_GROUP:
            rule_output["DeviceGroup"] = DEVICE_GROUP
...
def panorama_custom_block_rule_command(args: dict):
    ...
    custom_block_output = {"Name": rulename, "Direction": direction, "Disabled": False}
    if DEVICE_GROUP:
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.SecurityRule(val.Name == obj.Name)": custom_block_output},
        }
    )
...
def panorama_list_pcaps_command(args: dict):
    ...
    params = {"type": "export", "key": API_KEY, "category": pcap_type}
    ...
    if json_result["@status"] != "success":
        raise Exception(
            "Request to get list of Pcaps Failed.\nStatus code: "
            + str(json_result["response"]["@code"])
            + "\nWith message: "
            + str(json_result["response"]["msg"]["line"])
        )
        ...
        return_results(
            {
                ...
                "EntryContext": {"Panorama.Pcaps(val.Name == obj.Name)": pcap_list},
            }
        )
...
def panorama_get_pcap_command(args: dict):
    ...
    if result.headers["Content-Type"] != "application/octet-stream":
        ...
        if (json_result.get("@status") or "") == "error":
            errors = "\n".join([f"{error_key}: {error_val}" for error_key, error_val in (json_result.get("msg") or {}).items()])
...
def panorama_list_applications(args: Dict[str, str], predefined: bool) -> Union[List[dict], dict]:
    major_version = get_pan_os_major_version()
    params = {"type": "config", "action": "get", "key": API_KEY}
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.Applications(val.Name == obj.Name)": applications_arr_output},
        }
    )
...
def prettify_edls_arr(edls_arr: Union[list, dict]):
    ...
    for edl in edls_arr:
        pretty_edl = {"Name": edl["@name"], "Type": "".join(edl["type"])}
...
def panorama_list_edls():
    params = {"action": "get", "type": "config", "xpath": XPATH_OBJECTS + "external-list/entry", "key": API_KEY}
    result = http_request(
        URL,
        "GET",
        params=params,
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.EDL(val.Name == obj.Name)": edls_output},
        }
    )
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.EDL(val.Name == obj.Name)": edl_output},
        }
    )


@logger
def panorama_create_edl(
    edl_name: str, url: str, type_: str, recurring: str, certificate_profile: Optional[str], description: Optional[str]
):
    params = {
        "action": "set",
        "type": "config",
    ...
    params["element"] = (
        add_argument(url, "url", False)
        + "<recurring><"
        + recurring
        + "/></recurring>"
        + add_argument(certificate_profile, "certificate-profile", False)
        + add_argument(description, "description", False)
    )
...
def panorama_create_edl_command(args: Dict[str, str]):
    ...
    edl_output = {"Name": edl_name, "URL": url, "Type": type_, "Recurring": recurring}
...
def panorama_refresh_edl(edl_name: str, edl_type: str, location: str, vsys: str):
    ...
    # if refreshing an EDL on the Panorama
        ...
        if not edl_type or not location or not vsys:
            raise Exception(
                "To refresh an EDL from the Firewall on Panorama" " please use the: edl_type, location and vsys arguments."
            )

    params["cmd"] = (
        f"<request><system><external-list><refresh><type><{edl_type}><name>{edl_name}"
...
def panorama_register_ip_tag_command(args: dict):
    ...
    if persistent == "1":
        ...
        registered_ip = {"Tag": tag, "IPs": all_ips}
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.DynamicTags(val.Tag == obj.Tag)": registered_ip},
        }
    )
...
def panorama_unregister_ip_tag(tag: str, ips: list):
    ...
    params = {
        "type": "user-id",
        "cmd": "<uid-message><version>2.0</version><type>update</type><payload><unregister>"
        + entry
...
def panorama_register_user_tag_command(args: dict):
    ...
    registered_user = {"Tag": tag, "Users": all_users}
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.DynamicTags(val.Tag == obj.Tag)": registered_user},
        }
    )
...
@logger
def panorama_query_traffic_logs(
    number_of_logs: str,
    direction: str,
    query: str,
    source: str,
    ...
    params = {"type": "log", "log-type": "traffic", "key": API_KEY}
...
def panorama_query_traffic_logs_command(args: dict):
    ...
    if query and (source or destination or receive_time or application or to_port or action):
        raise Exception(
            "Use the query argument or the "
            "source, destination, receive_time, application, to_port, action arguments to build your query"
        )

    result = panorama_query_traffic_logs(
        number_of_logs, direction, query, source, destination, receive_time, application, to_port, action
    )
    ...
    query_traffic_output = {"JobID": result["response"]["result"]["job"], "Status": "Pending"}
    ...
    return_results(
        {
            ...
            "Contents": result,
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": tableToMarkdown("Query Traffic Logs:", query_traffic_output, ["JobID", "Status"], removeNull=True),
...
def panorama_get_traffic_logs(job_id: str):
    params = {"action": "get", "type": "log", "job-id": job_id, "key": API_KEY}
    result = http_request(
        URL,
        "GET",
        params=params,
...
def panorama_check_traffic_logs_status_command(job_id: str):
    ...
    query_traffic_status_output = {"JobID": job_id, "Status": "Pending"}

    if (
        "response" not in result
        or "result" not in result["response"]
        or "job" not in result["response"]["result"]
    ...
    return_results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": tableToMarkdown(
                "Query Traffic Logs status:", query_traffic_status_output, ["JobID", "Status"], removeNull=True
            ),
...
def panorama_get_traffic_logs_command(job_id: str):
    ...
    query_traffic_logs_output = {"JobID": job_id, "Status": "Pending"}
    ...
    if result["response"]["result"]["job"]["status"] != "FIN":
        return_results(
            {
                ...
                "ReadableContentsFormat": formats["markdown"],
                "HumanReadable": tableToMarkdown(
                    "Query Traffic Logs status:", query_traffic_logs_output, ["JobID", "Status"], removeNull=True
                ),
        ...
        if (
            "response" not in result
            or "result" not in result["response"]
            or "log" not in result["response"]["result"]
            ...
            return_results(
                {
                    ...
                    "HumanReadable": tableToMarkdown(
                        "Query Traffic Logs:",
                        pretty_traffic_logs,
                        ["JobID", "Source", "SourcePort", "Destination", "DestinationPort", "Application", "Action"],
                        removeNull=True,
                    ),
                    "EntryContext": {"Panorama.TrafficLogs(val.JobID == obj.JobID)": query_traffic_logs_output},
                }
...
def build_logs_query(
    address_src: Optional[str],
    address_dst: Optional[str],
    ip_: Optional[str],
    zone_src: Optional[str],
    zone_dst: Optional[str],
    ...
    time_generated_after: Optional[str],
):
    query = ""
    if address_src:
...
def panorama_query_logs(
    ...
    params = {"type": "log", "log-type": log_type, "key": API_KEY}
...
def panorama_query_logs_command(args: dict):
    ...
    if not job_id:
        ...
        poll_result = PollResult(
            ...
            partial_result=CommandResults(
                readable_output=f"Fetching {log_type} logs for job ID {result.ns.response.result.job}...", raw_response=result.raw
            ),
        )
        ...
        parsed: PanosResponse = PanosResponse(
            panorama_get_traffic_logs(job_id), illegal_chars=illegal_chars, ignored_keys=ignored_keys
        )
        if parsed.ns.response.status == "error":
            if parsed.ns.response.result.msg.line:
                raise Exception(f"Query logs failed. Reason is: {parsed.ns.response.result.msg.line}")
            else:
                raise Exception(f"Query logs failed.")

        if not parsed.ns.response.result.job.id:
            raise Exception("Missing JobID status in response.")

        query_logs_output = {"JobID": job_id, "LogType": log_type}
        ...
        if parsed.ns.response.result.job.status.upper() == "FIN":
            ...
            if parsed.ns.response.result.log.logs.count == "0":
                ...
                readable_output = tableToMarkdown(
                    ...
                    ["ReceiveTime", "Name", "FromZone", "ToZone", "SourceAddress", "DestinationAddress",
                        "DestinationPort", "Application", "Action", "Severity", "Rule", "URLOrFilename", "TimeGenerated"],
                    removeNull=True,
                )
...
def panorama_check_logs_status_command(job_id: str):
    ...
    for job_id in job_ids:
        ...
        query_logs_status_output = {"JobID": job_id, "Status": "Pending"}
        ...
        return_results(
            {
                ...
                "ReadableContentsFormat": formats["markdown"],
                "HumanReadable": tableToMarkdown(
                    "Query Logs status:", query_logs_status_output, ["JobID", "Status"], removeNull=True
                ),
...
def panorama_get_logs_command(args: dict):
    ...
    for job_id in job_ids:
        ...
        query_logs_output = {"JobID": job_id, "Status": "Pending"}
            ...
            return_results(
                {
                    ...
                    "ReadableContentsFormat": formats["markdown"],
                    "HumanReadable": tableToMarkdown(
                        "Query Logs status:", query_logs_output, ["JobID", "Status"], removeNull=True
                    ),
...
def panorama_security_policy_match(
    application: Optional[str] = None,
    category: Optional[str] = None,
    destination: Optional[str] = None,
    destination_port: Optional[str] = None,
    ...
    params = {
        ...
        "target": target,
        "vsys": vsys,
        "cmd": build_policy_match_query(
            application, category, destination, destination_port, from_, to_, protocol, source, source_user
        ),
    }
...
def devices(targets=None, vsys_s=None):
    ...
    if VSYS:  # for FW intstances
        ...
        res = http_request(
            URL, "GET", params={"key": API_KEY, "type": "op", "cmd": "<show><devices><all></all></devices></show>"}
        )
...
def panorama_security_policy_match_command(args: dict):
    ...
    for target, vsys in devices(targets=argToList(args.get("target")), vsys_s=argToList(args.get("vsys"))):
        matching_rules = panorama_security_policy_match(
            application, category, destination, destination_port, from_, to_, protocol, source, source_user, target, vsys
        )
        if matching_rules:
            ...
            context = {
                ...
                "QueryFields": prettify_query_fields(
                    application, category, destination, destination_port, from_, to_, protocol, source, source_user
                ),
                "Query": build_policy_match_query(
                    application, category, destination, destination_port, from_, to_, protocol, source, source_user
                ),
            }
            if device:
...
def panorama_add_static_route_command(args: dict):
    ...
    static_route = panorama_add_static_route(
        xpath_network, virtual_router, static_route_name, destination, nexthop_type, nexthop_value, interface, metric
    )
...
def panorama_override_vulnerability(threatid: str, vulnerability_profile: str, drop_mode: str):
    xpath = "{}profiles/vulnerability/entry[@name='{}']/threat-exception/entry[@name='{}']/action".format(
        XPATH_OBJECTS, vulnerability_profile, threatid
    )
    params = {"action": "set", "type": "config", "xpath": xpath, "key": API_KEY, "element": "<{0}></{0}>".format(drop_mode)}
    ...
    return_results(
        {
            ...
            "EntryContext": {"Panorama.Vulnerability(val.Name == obj.Name)": threat_output},
        }
    )
...
def panorama_delete_static_route_command(args: dict):
    ...
    entry_context = {"Name": route_name, "Deleted": True}
    return_results(
        {
            "Type": entryTypes["note"],
            "ContentsFormat": formats["json"],
...
def panorama_show_device_version(target: str | None = None):
    params = {"type": "op", "cmd": "<show><system><info/></system></show>", "key": API_KEY}
    if target:
        params["target"] = target
    result = http_request(URL, "GET", params=params)
...
def panorama_content_update_download_status_command(args: dict):
    ...
    content_download_status = {"JobID": result["response"]["result"]["job"]["id"]}
...
def panorama_content_update_install_status_command(args: dict):
    ...
    content_install_status = {"JobID": result["response"]["result"]["job"]["id"]}
...
def panorama_check_latest_panos_software_command(target: Optional[str] = None):
    ...
    if len(versions) > 5:
        versions = versions[:5]
    human_readable = tableToMarkdown(
        "5 latest pan-os software releases",
        versions,
        ["version", "filename", "size", "released-on", "downloaded", "current", "latest", "uploaded"],
        removeNull=True,
    )
    return CommandResults(
        readable_output=human_readable, outputs=to_context, raw_response=result, outputs_prefix="Panorama.LatestVersions"
    )
...
def panorama_download_panos_version_command(args: dict):
    ...
    if "result" in result["response"]:
        # download has been given a jobid
        panos_version_download = {"JobID": result["response"]["result"]["job"]}
...
def panorama_download_panos_status_command(args: dict):
    ...
    panos_download_status = {"JobID": result["response"]["result"]["job"]["id"]}
    if result["response"]["result"]["job"]["status"] in ["FIN", "ACT", "FAIL"]:
        status_res = result["response"]["result"]["job"]["result"]
        if status_res == "OK":
    ...
    human_readable = tableToMarkdown(
        "PAN-OS download status:", panos_download_status, ["JobID", "Status", "Details"], removeNull=True
    )
...
def panorama_install_panos_version_command(args: dict):
    ...
    if "result" in result["response"]:
        ...
        panos_install = {"JobID": result["response"]["result"]["job"]}
...
def panorama_install_panos_status_command(args: dict):
    ...
    panos_install_status = {"JobID": result["response"]["result"]["job"]["id"]}
    if result["response"]["result"]["job"]["status"] == "FIN":
        if result["response"]["result"]["job"]["result"] == "OK":
    ...
    human_readable = tableToMarkdown(
        "PAN-OS installation status:", panos_install_status, ["JobID", "Status", "Details"], removeNull=True
    )
    return_results(
        {
            "Type": entryTypes["note"],
...
def panorama_device_reboot_command(args: dict):
    params = {"type": "op", "cmd": "<request><restart><system></system></restart></request>", "key": API_KEY}
...
def panorama_show_location_ip(ip_address: str):
    params = {"type": "op", "cmd": f"<show><location><ip>{ip_address}</ip></location></show>", "key": API_KEY}
    result = http_request(URL, "GET", params=params)
...
def panorama_show_location_ip_command(ip_address: str):
    ...
    if "response" in result and "result" in result["response"] and "entry" in result["response"]["result"]:
        ...
        show_location_output = {"ip_address": ip_address, "status": "NotFound"}
...
def panorama_get_license() -> Dict:
    params = {"type": "op", "cmd": "<request><license><info/></license></request>", "key": API_KEY}
    result = http_request(URL, "GET", params=params)
...
def prettify_data_filtering_rule(rule: Dict) -> Dict:
    ...
    """
    pretty_rule = {"Name": rule.get("@name")}
    ...
    if "spyware" in security_profiles and security_profiles["spyware"] is not None:
        ...
        if isinstance(profiles, list):
            for profile in profiles:
                ...
                spyware_content.append({"Name": profile["@name"], "Rules": spyware_rules})
        else:
            rules = profiles.get("rules", {}).get("entry", [])
            spyware_rules = prettify_profiles_rules(rules)
            spyware_content = [{"Name": profiles["@name"], "Rules": spyware_rules}]
    ...
    if "virus" in security_profiles and security_profiles["virus"] is not None:
        ...
        if isinstance(profiles, list):
            for profile in profiles:
                ...
                virus_content.append({"Name": profile["@name"], "Decoder": antivirus_rules})
        else:
            rules = profiles.get("decoder", {}).get("entry", [])
            antivirus_rules = prettify_profiles_rules(rules)
            virus_content = [{"Name": profiles["@name"], "Rules": antivirus_rules}]

        human_readable += tableToMarkdown(
            "Antivirus Profiles", virus_content, headers=["Name", "Decoder", "Rules"], removeNull=True
        )
    ...
    if "file-blocking" in security_profiles and security_profiles["file-blocking"] is not None:
        ...
        if isinstance(profiles, list):
            for profile in profiles:
                ...
                file_blocking_content.append({"Name": profile["@name"], "Rules": file_blocking_rules})
        else:
            rules = profiles.get("rules", {}).get("entry", [])
            file_blocking_rules = prettify_profiles_rules(rules)
            file_blocking_content = [{"Name": profiles["@name"], "Rules": file_blocking_rules}]
    ...
    if "vulnerability" in security_profiles and security_profiles["vulnerability"] is not None:
        ...
        if isinstance(profiles, list):
            for profile in profiles:
                ...
                vulnerability_content.append({"Name": profile["@name"], "Rules": vulnerability_rules})
        else:
            rules = profiles.get("rules", {}).get("entry", [])
            vulnerability_rules = prettify_profiles_rules(rules)
            vulnerability_content = [{"Name": profiles["@name"], "Rules": vulnerability_rules}]
    ...
    if "data-filtering" in security_profiles and security_profiles["data-filtering"] is not None:
        ...
        if isinstance(profiles, list):
            for profile in profiles:
                ...
                data_filtering_content.append({"Name": profile["@name"], "Rules": data_filtering_rules})
        else:
            rules = profiles.get("rules", {}).get("entry", [])
            data_filtering_rules = prettify_data_filtering_rules(rules)
            data_filtering_content = [{"Name": profiles["@name"], "Rules": data_filtering_rules}]
    ...
    if "url-filtering" in security_profiles and security_profiles["url-filtering"] is not None:
        ...
        if isinstance(profiles, list):
            for profile in profiles:
                ...
                url_filtering_content.append({"Name": profile["@name"], "Rules": url_filtering_rules})
        else:
            url_filtering_rules = prettify_get_url_filter(profiles)
            url_filtering_content = [{"Name": profiles["@name"], "Rules": url_filtering_rules}]
    ...
    if "wildfire-analysis" in security_profiles and security_profiles["wildfire-analysis"] is not None:
        ...
        if isinstance(profiles, list):
            for profile in profiles:
                ...
                wildfire_analysis_content.append({"Name": profile["@name"], "Rules": wildfire_rules})
        else:
            rules = profiles.get("rules", {}).get("entry", [])
            wildfire_rules = prettify_wildfire_rules(rules)
            wildfire_analysis_content = [{"Name": profiles["@name"], "Rules": wildfire_rules}]
...
def apply_security_profile(xpath: str, profile_name: str, profile_type: str) -> Dict:
    ...
    profile_types_result = dict_safe_get(
        result, ["response", "result", "entry", "profile-setting", "profiles"], default_return_value={}
    )
...
def prettify_profile_rule(rule: Dict) -> Dict:
    ...
    """
    pretty_rule = {"Name": rule["@name"], "Action": rule["action"]}
    if isinstance(rule.get("application"), dict) and "member" in rule["application"]:
        pretty_rule["Application"] = rule["application"]["member"]
    if isinstance(rule.get("file-type"), dict) and "member" in rule["file-type"]:
        pretty_rule["File-type"] = rule["file-type"]["member"]
...
def get_anti_spyware_best_practice() -> Dict:
    params = {"action": "get", "type": "config", "xpath": "/config/predefined/profiles/spyware", "key": API_KEY}
...
def get_anti_spyware_best_practice_command():
    ...
    sinkhole_content = []
    if sinkhole:
        sinkhole_content = [{"ipv6-address": sinkhole["ipv6-address"], "ipv4-address": sinkhole["ipv4-address"]}]
...
def get_file_blocking_best_practice() -> Dict:
    params = {"action": "get", "type": "config", "xpath": "/config/predefined/profiles/file-blocking", "key": API_KEY}
...
def get_file_blocking_best_practice_command():
    ...
    human_readable = tableToMarkdown(
        "File Blocking Profile Best Practice", rules, ["Name", "Action", "File-type", "Application"], removeNull=True
    )
...
def get_antivirus_best_practice() -> Dict:
    params = {"action": "get", "type": "config", "xpath": "/config/predefined/profiles/virus", "key": API_KEY}
...
def get_antivirus_best_practice_command():
    ...
    human_readable = tableToMarkdown(
        "Antivirus Best Practice Profile", rules, ["Name", "Action", "WildFire-action"], removeNull=True
    )
...
def get_vulnerability_protection_best_practice() -> Dict:
    params = {"action": "get", "type": "config", "xpath": "/config/predefined/profiles/vulnerability", "key": API_KEY}
...
def get_wildfire_best_practice() -> Dict:
    params = {"action": "get", "type": "config", "xpath": "/config/predefined/profiles/wildfire-analysis", "key": API_KEY}
...
def get_wildfire_best_practice_command():
    ...
    wildfire_schedule = {"Recurring": "every-minute", "Action": "download-and-install"}
    ...
    human_readable = tableToMarkdown(
        "WildFire Best Practice Profile", rules, ["Name", "Analysis", "Application", "File-type"], removeNull=True
    )
    ...
    human_readable += tableToMarkdown(
        "Wildfire System Settings\n report-grayware-file: yes", system_settings, ["Name", "File-size"]
    )
...
def set_xpath_wildfire(template: str | None = None) -> str:
    ...
    if template:
        xpath_wildfire = (
            f"/config/devices/entry[@name='localhost.localdomain']/template/entry[@name="
    ...
    return_results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": human_readable,
            "EntryContext": {"Panorama.WildFire(val.Name == obj.Name)": file_size, "Panorama.WildFire.Schedule": schedule},
        }
    )
    ...
    return_results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": human_readable,
            "EntryContext": {"Panorama.URLFilter": rules, "Panorama.URLFilter.Header": headers_best_practice},
        }
    )
...
def show_user_id_interface_config_request(args: dict):
    ...
    if VSYS:  # firewall instance xpath
        ...
        xpath = (
            f"/config/devices/entry[@name='localhost.localdomain']/template/entry[@name='{TEMPLATE}']/config"
    ...
    params = {"action": "show", "type": "config", "xpath": xpath, "key": API_KEY}
    result = http_request(
        URL,
        "GET",
        params=params,
        ...
        return_results(
            CommandResults(
                ...
                readable_output=tableToMarkdown(
                    "Zone Configuration:",
                    formatted_results,
                    ["Name", "Network", "EnableUserIdentification", "ZoneProtectionProfile", "LogSetting"],
                    removeNull=True,
                ),
                raw_response=raw_response,
            )
...
def list_configured_user_id_agents_request(args: dict, version):
    ...
    if not VSYS and not TEMPLATE and not template_stack:
        raise DemistoException(
            "In order to show the the User ID Agents in your Panorama, "
            "supply either the template or the template_stack arguments."
        )
    ...
    if VSYS:
        if version < 10:
            ...
            xpath = f"/config/devices/entry[@name='localhost.localdomain']/vsys/entry[@name='{vsys}']/" "redistribution-agent"
    ...
    if raw_response:
        ...
        return_results(
            CommandResults(
                ...
                outputs_key_field="Name",
                outputs=formatted_results,
                readable_output=tableToMarkdown("User ID Agents:", formatted_results, headers, removeNull=True),
                raw_response=raw_response,
            )
        )
    else:
...
def panorama_install_file_content_update(version: str, category: str, validity: str):
    ...
    if category == "content":
        ...
        params = {
            "type": "op",
            "cmd": (
                f"<request><{category}><upgrade><install><file>{version}" f"</file></install></upgrade></{category}></request>"
            ),
            "key": API_KEY,
        }
    result = http_request(URL, "GET", params=params)
...
class URLFilteringProfile(VersionedPanObject):
    ...
    def _setup(self):
        ...
        self._params = (
            ...
            VersionedParamPath("credential_enforce_alert", vartype="member", path="credential-enforcement/alert"),
            VersionedParamPath("credential_enforce_block", vartype="member", path="credential-enforcement/block"),
        )
...
class Topology:
    ...
    def get_single_device(self, filter_string: str) -> Union[Firewall, Panorama]:
        ...
        raise DemistoException(
            f"filter_str {filter_string} is not the exact ID of a host in this topology; " + f"use a more specific filter string."
        )
    ...
    def build_from_string(
        ...
        for hostname in hostnames.split(","):
            try:
                if api_key:
                    device = PanDevice.create_from_device(hostname=hostname, api_key=api_key, port=port)
                else:
                    device = PanDevice.create_from_device(
                        hostname=hostname, api_username=username, api_password=password, port=port
                    )
                # Set the timeout
...
class HygieneRemediation:
    ...
    @staticmethod
    def fix_log_forwarding_profile_enhanced_logging(
        topology: Topology, issues: List[ConfigurationHygieneIssue]
    ...
    def fix_security_zone_no_log_setting(
        topology: Topology, issues: List[ConfigurationHygieneIssue], log_forwarding_profile: str
    ) -> List[ConfigurationHygieneFix]:
        """
    ...
    @staticmethod
    def get_all_rules_in_container(
        container: Union[Panorama, Firewall, DeviceGroup, Template, Vsys], object_class: Union[SecurityRule, NatRule]
    ):
        """
    ...
    @staticmethod
    def fix_secuity_rule_log_settings(
        topology: Topology, issues: List[ConfigurationHygieneIssue], log_forwarding_profile_name: str
    ) -> List[ConfigurationHygieneFix]:
        """
...
class ObjectGetter:
    ...
    def get_object_reference(
        ...
        object_references = []
        for device, container in topology.get_all_object_containers(device_filter_string, container_name=container_filter):
            unfiltered_objects = []
    ...
    def get_hygiene_check_register(cls, issue_codes: List[str]):
        ...
        check_register = {
            ...
            "BP-V-5": ConfigurationHygieneCheck(
                issue_code="BP-V-5", result=UNICODE_PASS, description="Fails if no spyware profile is configured for visibility."
            ),
...
class HygieneLookups:
    ...
    def check_log_forwarding_profiles(
        ...
        check_register = HygieneCheckRegister.get_hygiene_check_register(["BP-V-1", "BP-V-2", "BP-V-3"])
        ...
        if len(lf_profile_list) == 0:
            issues.append(
                ConfigurationHygieneIssue(
                    hostid="PLATFORM", container_name="", description="No log profiles configured!", name="", issue_code="BP-V-1"
                )
            )
    ...
    def get_conforming_threat_profiles(
        ...
        for profile in profiles:
            ...
            for rule in profile.children:
                block_actions = [rule.is_reset_both, rule.is_reset_client, rule.is_reset_server, rule.is_drop, rule.is_block_ip]
    ...
    def get_all_conforming_url_filtering_profiles(
        ...
        for device, container in topology.get_all_object_containers(device_filter_str):
            ...
            conforming_profiles = HygieneLookups.get_conforming_url_filtering_profiles(url_filtering_profiles)
...
class PanoramaCommand:
    ...
    def push_style(topology: Topology, device: Union[Firewall, Panorama], style: str, filter: Optional[List[str]] = None):
        ...
        for commit_group_name in commit_group_names:
            commit_command = PanoramaCommitAll(style=style, name=commit_group_name)
...
class UniversalCommand:
    ...
    def get_system_info(
        ...
        for device in topology.all(filter_string=device_filter_str, target=target):
            ...
            result_data.append(dataclass_from_element(device, ShowSystemInfoResultData, response.find("./result/system")))
            summary_data.append(dataclass_from_element(device, ShowSystemInfoSummaryData, response.find("./result/system")))
    ...
    def download_software(
        ...
        for device in topology.all(filter_string=device_filter_str, target=target):
            ...
            result.append(GenericSoftwareStatus(hostid=resolve_host_id(device), started=True))
...
class FirewallCommand:
    ...
    def get_arp_table(
        ...
        for firewall in topology.firewalls(filter_string=device_filter_str, target=target):
            ...
            summary_data.append(dataclass_from_element(firewall, ShowArpCommandSummaryData, response.find("./result")))
        ...
        return ShowArpCommandResult(result_data=result_data, summary_data=summary_data)
    ...
    def get_counter_global(
        ...
        return ShowCounterGlobalCommmandResult(result_data=result_data, summary_data=summary_data)
    ...
    def get_routing_summary(
        ...
        for firewall in topology.firewalls(filter_string=device_filter_str, target=target):
            ...
            summary_data.append(
                dataclass_from_element(firewall, ShowRoutingCommandSummaryData, response.find("./result/entry/All-Routes"))
            )

        return ShowRouteSummaryCommandResult(summary_data=summary_data, result_data=[])
    ...
    def get_bgp_peers(
        ...
        for firewall in topology.firewalls(filter_string=device_filter_str, target=target):
            ...
            summary_data.append(
                dataclass_from_element(firewall, ShowRoutingProtocolBGPPeersSummaryData, response.find("./result/entry"))
            )
            result_data.append(
                dataclass_from_element(firewall, ShowRoutingProtocolBGPPeersResultData, response.find("./result/entry"))
            )

        return ShowRoutingProtocolBGPCommandResult(summary_data=summary_data, result_data=result_data)
    ...
    def get_ha_status(
        ...
        for firewall in topology.all(filter_string=device_filter_str, target=target):
            ...
            if not peer_serial and DEVICE_GROUP:
                result.append(ShowHAState(hostid=firewall_host_id, status="HA Not enabled.", active=True, peer=""))
            else:
                ...
                if "active" in state:
                    result.append(ShowHAState(hostid=firewall_host_id, status=state, active=True, peer=peer_serial))
                else:
                    result.append(ShowHAState(hostid=firewall_host_id, status=state, active=False, peer=peer_serial))
    ...
    def get_routes(
        ...
        for firewall in topology.firewalls(filter_string=device_filter_str, target=target):
            ...
            for entry in response.findall("./result/entry"):
                result_data.append(dataclass_from_element(firewall, ShowRoutingRouteResultData, entry))
        ...
        for firewall_hostname, interfaces in count_data.items():
            for interface, route_count in interfaces.items():
                summary_data.append(
                    ShowRoutingRouteSummaryData(hostid=firewall_hostname, interface=interface, route_count=route_count)
                )
...
def check_log_forwarding(topology: Topology, device_filter_string: Optional[str] = None) -> ConfigurationHygieneCheckResult:
    """
...
def hygiene_issue_dict_to_object(issue_dicts: Union[List[dict], dict]) -> List[ConfigurationHygieneIssue]:
    ...
    for issue_dict in issue_dicts:
        ...
        issues.append(ConfigurationHygieneIssue(**issue_dict))
    return issues
...
def fix_security_zone_log_setting(
    topology: Topology, issue: List, log_forwarding_profile_name: str
) -> List[ConfigurationHygieneFix]:
    """
    ...
    return HygieneRemediation.fix_security_zone_no_log_setting(
        topology, log_forwarding_profile=log_forwarding_profile_name, issues=hygiene_issue_dict_to_object(issue)
    )
...
def fix_security_rule_log_setting(
    ...
    return HygieneRemediation.fix_secuity_rule_log_settings(
        topology, log_forwarding_profile_name=log_forwarding_profile_name, issues=hygiene_issue_dict_to_object(issue)
    )


def fix_security_rule_security_profile_group(
    topology: Topology, issue: List, security_profile_group_name: str
) -> List[ConfigurationHygieneFix]:
    """
    ...
    return HygieneRemediation.fix_security_rule_security_profile_group(
        topology, security_profile_group_name=security_profile_group_name, issues=hygiene_issue_dict_to_object(issue)
    )
...
def dataclasses_to_command_results(
    ...
    readable_output = tableToMarkdown(override_table_name or title, summary_list, removeNull=True, headers=override_table_headers)
    command_result = CommandResults(outputs_prefix=output_prefix, outputs=outputs, readable_output=readable_output, **extra_args)
...
def pan_os_get_running_config(args: dict):
    ...
    params = {"type": "op", "key": API_KEY, "cmd": "<show><config><running></running></config></show>"}
...
def pan_os_get_merged_config(args: dict):
    ...
    params = {"type": "op", "key": API_KEY, "cmd": "<show><config><merged></merged></config></show>"}
...
def pan_os_list_templates(template_name: Optional[str]):
    params = {"type": "config", "action": "get", "key": API_KEY, "xpath": build_template_xpath(template_name)}
...
def pan_os_list_templates_command(args):
    ...
    return CommandResults(
        raw_response=raw_response,
        outputs=templates,
        readable_output=tableToMarkdown("Templates:", table, removeNull=True),
...
def build_nat_xpath(
    name: Optional[str], pre_post: str, element: Optional[str] = None, filters: dict | None = None, query: str | None = None
):
...
def get_pan_os_nat_rules(
    show_uncommited: bool,
            ...
            if _source_translation := source_translation_object.get(_source_translation_type):
                ...
                if _source_translation_type == "dynamic-ip-and-port":
                    if interface := _source_translation.get("interface-address"):
                        return {pretty_context: {"InterfaceAddress": extract_objects_info_by_key(interface, "interface")}}

                return {
                    pretty_context: {"TranslatedAddress": extract_objects_info_by_key(_source_translation, "translated-address")}
                }
        return None
...
def pan_os_list_nat_rules_command(args):
    ...
    filters = assign_params(tags=argToList(args.get("tags")), nat_type=args.get("nat_type"))
    if nat_type := filters.pop("nat_type", None):  # Replace the key name from 'nat_type' to 'nat-type'.
        filters["nat-type"] = nat_type
    query = args.get("query")

    raw_response = get_pan_os_nat_rules(
        name=name, pre_post=pre_post, show_uncommited=show_uncommitted, filters=filters, query=query
    )
    result = raw_response.get("response", {}).get("result", {})
...
def create_nat_rule(args):
    def _set_up_body_request():
            ...
            if destination_translation_type != "none":
                ...
                if destination_translation_type == "static_ip":
                    if destination_dns_rewrite_direction := args.get("destination_dns_rewrite_direction"):
                        destination_translation_body_request["dns-rewrite"] = {"direction": destination_dns_rewrite_direction}
            ...
            if source_translation_type != "none":
                ...
                if source_translated_address_type == "translated-address":
                    ...
                    if source_translated_interface:
                        ...
                        raise DemistoException(
                            "source_translated_interface must be " "provided if source_translation_type == interface-address"
                        )
            return {}
        ...
        if negate_destination := args.get("negate_destination"):
            _body_request["target"] = {"negate": negate_destination}
...
def pan_os_create_nat_rule_command(args):
    ...
    return CommandResults(raw_response=raw_response, readable_output=f"Nat rule {rule_name} was created successfully.")
...
def pan_os_delete_nat_rule_command(args):
    ...
    return CommandResults(raw_response=raw_response, readable_output=f"Nat rule {rule_name} was deleted successfully.")


def pan_os_edit_nat_rule(rule_name, pre_post, behavior, element_to_change, element_value, object_name, is_listable=True):
...
def pan_os_edit_nat_rule_command(args):
    ...
    elements_to_change_mapping_pan_os_paths = {
        ...
        "source_translation_dynamic_ip": ("source-translation/dynamic-ip/translated-address", "translated-address", True),
        "source_translation_static_ip": ("source-translation/static-ip/translated-address", "translated-address", False),
        ...
        "destination_translation_dynamic_port": ("dynamic-destination-translation/translated-port", "translated-port", False),
        "destination_translation_dynamic_ip": ("dynamic-destination-translation/translated-address", "translated-address", False),
        "destination_translation_dynamic_distribution_method": (
    ...
    return CommandResults(raw_response=raw_response, readable_output=f"Nat rule {rule_name} was edited successfully.")
        ...
        human_readable.append(
            {
                ...
                "Static Route": extract_objects_info_by_key(
                    entry.get("routing-table", {}).get("ip", {}).get("static-route", {}).get("entry", {}), "@name"
                ),
...
def parse_pan_os_list_redistribution_profiles(entries):
    def extract_bgp_and_ospf_filters(_entry, _filters_types):
        ...
        filters_by_types = {
            ...
            for filter_type in _filters_types
            if _entry.get(filter_type)
        }
...
def pan_os_edit_redistribution_profile(
    virtual_router_name, redistribution_profile_name, element_to_change, element_value, object_name, is_listable, behavior
):
    xpath = build_redistribution_profile_xpath(virtual_router_name, redistribution_profile_name, element=element_to_change)
...
def pan_os_delete_redistribution_profile_command(args):
    ...
    return CommandResults(
        raw_response=raw_response, readable_output=f"Redistribution profile {redistribution_profile} was deleted successfully."
    )
...
def pan_os_list_pbf_rules_command(args):
    ...
    raw_response = pan_os_list_pbf_rules(
        name=name, pre_post=pre_post, show_uncommitted=show_uncommitted, filters=filters, query=query
    )
    result = raw_response.get("response", {}).get("result", {})
        ...
        def _set_up_action_body_request():
            ...
            if action := args.get("action"):
                if action == "forward":
                    ...
                    _action_body_request.update(prepare_pan_os_objects_body_request("action", action, is_empty_tag=True))
...
def pan_os_create_pbf_rule_command(args):
    ...
    return CommandResults(raw_response=raw_response, readable_output=f"PBF rule {rule_name} was created successfully.")


def pan_os_edit_pbf_rule(rule_name, element_value, pre_post, element_to_change, object_name, is_listable, behavior):
    xpath = build_pbf_xpath(name=rule_name, pre_post="rulebase" if VSYS else pre_post, element_to_change=element_to_change)
...
def pan_os_edit_pbf_rule_command(args):
    ...
    return CommandResults(raw_response=raw_response, readable_output=f"PBF {rule_name} was edited successfully.")
...
def pan_os_delete_pbf_rule_command(args):
    ...
    return CommandResults(raw_response=raw_response, readable_output=f"PBF rule {rule_name} was deleted successfully.")
...
def pan_os_list_application_groups_command(args):
    ...
    for entry in entries:
        ...
        application_groups.append(
            {"Name": extract_objects_info_by_key(entry, "@name"), "Applications": applications, "Members": len(applications)}
        )

    return CommandResults(
        raw_response=raw_response,
        outputs=application_groups,
        readable_output=tableToMarkdown(f"Application groups:", application_groups, removeNull=True),
...
def pan_os_edit_application_group_command(args):
    ...
    return CommandResults(
        ...
        outputs={"Name": application_group_name, "Applications": updated_applications, "Members": len(updated_applications)},
        outputs_key_field="Name",
        outputs_prefix="Panorama.ApplicationGroup",
    )
...
def pan_os_delete_application_group(name):
    params = {"xpath": build_application_groups_xpath(name), "action": "delete", "type": "config", "key": API_KEY}
...
def pan_os_delete_application_group_command(args):
    ...
    return CommandResults(
        raw_response=raw_response, readable_output=f"application-group {application_group_name} was deleted successfully."
    )
...
def pan_os_list_tag(is_shared: bool = False) -> tuple[dict, list]:
    ...
    """
    params = {"type": "config", "action": "get", "key": API_KEY, "xpath": build_tag_xpath(is_shared=is_shared)}
...
def pan_os_list_tag_command(args: dict) -> CommandResults:
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(f"Tags:", prettify_tags_list, ["Name", "Color", "Comment", "Location"]),
        outputs_prefix="Panorama.Tag",
        outputs_key_field="name",
    )


def pan_os_create_tag(tag_name: str, disable_override: bool, is_shared: bool, comment: str) -> dict:
...
def pan_os_create_tag_command(args: dict) -> CommandResults:
    ...
    raw_response = pan_os_create_tag(tag_name, disable_override, is_shared, comment)
...
def pan_os_edit_tag(tag_name: str, new_tag_name: str, disable_override: bool, comment: str, color: Optional[str]) -> dict:
...
def pan_os_edit_tag_command(args: dict) -> CommandResults:
    ...
    try:
        ...
        raise DemistoException(
            f"Can't find the tag with name '{tag_name}' in tags list.\n"
...
def pan_os_delete_tag(tag_name: str) -> dict:
    ...
    Returns:
        dict: The raw response from panorama's API.
    """
    params = {"xpath": build_tag_xpath(name=tag_name), "action": "delete", "type": "config", "key": API_KEY}
...
def pan_os_create_security_profile_group_command(args: dict) -> CommandResults:
    ...
    params = {
        ...
        "element": (
            add_argument(args.get("antivirus_profile"), "virus", True)
...
def pan_os_edit_security_profile_group_command(args: dict) -> CommandResults:
    ...
    if profile_value.lower() == "none":
        ...
        params.update({"xpath": xpath, "element": element})
    else:
...
def pan_os_get_audit_comment_command(args: dict) -> CommandResults:
    ...
    outputs = {"rule_name": rule_name, "rule_type": rule_type, "comment": comment}
...
def build_xpath_for_profile_exception_commands(
    profile_name: str, profile_type: str, device_group: str, action_type: str, extracted_id: Optional[str] = None
) -> str:
    """
...
def get_predefined_threats_list() -> list:
    ...
    predefined_threats += (
        result.get("response", {}).get("result", {}).get("threats", {}).get("vulnerability", {}).get("entry", [])
    )
...
def build_element_for_profile_exception_commands(
    extracted_id: str, action: str, packet_capture: str, exempt_ip: str, ip_track_by: str, ip_duration_sec: str
) -> str:
    """
...
def profile_exception_crud_requests(args: dict, action_type: str) -> Any:
    ...
    exceptions_ip_track_by_types_map = {"Source": "source", "Source And Destination": "source-and-destination"}

    exception_profile_types_map = {"Vulnerability Protection Profile": "vulnerability", "Anti Spyware Profile": "spyware"}
    ...
    if action_type in [ExceptionCommandType.ADD.value, ExceptionCommandType.EDIT.value]:
        ...
        params = {"type": "config", "action": action_type, "xpath": xpath, "key": API_KEY, "element": element}
...
def get_query_by_job_id_request(log_type: str, query: str, max_fetch: int, offset_fetch: int) -> str:
    ...
    params = assign_params(
        key=API_KEY,
        type="log",
        log_type=LOG_TYPE_TO_REQUEST[log_type],
        query=query,
        nlogs=max_fetch,
        dir="forward",
        skip=offset_fetch,
...
def get_query_entries(
    log_type: str, query: str, max_fetch: int, fetch_job_polling_max_num_attempts: int, offset_fetch: int
) -> List[Dict[Any, Any]]:
...
def get_fetch_start_datetime_dict(
    last_fetch_dict: LastFetchTimes, first_fetch: str, queries_dict: QueryMap
) -> Dict[str, datetime]:
    ...
    if queries_dict:
        last_fetch_dict |= {  # type: ignore[assignment, typeddict-item]
            log_type: "" for log_type in queries_dict if log_type not in last_fetch_dict
        }
...
def update_offset_dict(
    incident_entries_dict: dict[str, list[dict[str, Any]]], last_fetch_dict: LastFetchTimes, offset_fetch_dict: Offset
):
    """
    for each log type incident entries array, parse the raw incidents into context incidents.
    if necessary, update the latest fetch time and last ID values in their corresponding dictionaries.
    ...
    for log_type, incident_entries in incident_entries_dict.items():
        if incident_entries:
            ...
            for entry in incident_entries:
                ...
                if not log_type in offset_fetch_dict or not last_fetch_time or log_time > last_fetch_time:
...
def fetch_incidents(
    ...
    incident_entries_dict = fetch_incidents_request(
        queries_dict, max_fetch_dict, fetch_start_datetime_dict, fetch_job_polling_max_num_attempts, offset_dict
    )
    ...
    new_last_run = LastRun(
        ...
        last_id_dict=last_id_dict,  # type: ignore[typeddict-item]
...
def main():  # pragma: no cover
    try:
        ...
        # Get pre-defined threats list from the firewall
            ...
            command == "panorama-url-filtering-block-default-categories"
        ...
        elif command == "panorama-get-file-blocking-best-practice" or command == "pan-os-get-file-blocking-best-practice":
            ...
            topology = get_topology()
            return_results(
                dataclasses_to_command_results(get_arp_tables(topology, **demisto.args()), empty_result_message="No ARP entries.")
            )
        elif command == "pan-os-platform-get-route-summary":
            topology = get_topology()
            return_results(
                dataclasses_to_command_results(
                    get_route_summaries(topology, **demisto.args()), empty_result_message="Empty route summary result."
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    get_routes(topology, **demisto.args()), empty_result_message="Empty route summary result."
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    get_device_groups(topology, **demisto.args()), empty_result_message="No device groups found."
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    get_template_stacks(topology, **demisto.args()), empty_result_message="No template stacks found."
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    get_global_counters(topology, **demisto.args()), empty_result_message="No Global Counters Found"
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    get_bgp_peers(topology, **demisto.args()), empty_result_message="No BGP Peers found."
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    get_available_software(topology, **demisto.args()), empty_result_message="No Available software images found"
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    get_ha_state(topology, **demisto.args()), empty_result_message="No HA information available"
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    download_software(topology, **demisto.args()), empty_result_message="Software download not started"
                )
            )
        elif command == "pan-os-apply-dns-signature-policy":
            return_results(apply_dns_signature_policy_command(args))
        elif command == "pan-os-platform-install-software":
            topology = get_topology()
            return_results(
                dataclasses_to_command_results(
                    install_software(topology, **demisto.args()), empty_result_message="Software Install not started"
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    reboot(topology, **demisto.args()), empty_result_message="Device not rebooted, or did not respond."
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    fix_log_forwarding(topology, **demisto.args()), empty_result_message="Nothing to fix."
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    fix_security_zone_log_setting(topology, **demisto.args()), empty_result_message="Nothing to fix."
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    fix_security_rule_log_setting(topology, **demisto.args()), empty_result_message="Nothing to fix."
                )
            )
            ...
            return_results(
                dataclasses_to_command_results(
                    fix_security_rule_security_profile_group(topology, **demisto.args()), empty_result_message="Nothing to fix."
                )
            )
