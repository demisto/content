...
runtime = boto3.Session(
    aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key, region_name=params["AWSRegion"]
).client("runtime.sagemaker")  # type: ignore[call-overload]
...
def parse_results(result):
    res = []
    for r in result:
        res.append({"Label": r["label"][0], "Probability": r["probability"]})
    return res
...
if demisto.command() == "predict-phishing":
    ...
    context = {"DBotPhishingPrediction": predictions}
    demisto.results(
        {
