...
def main():
    ...
    if linked_incidents and not close_notes.startswith("Closed from parent Incident"):
        demisto.executeCommand("executeCommandAt", {"command":"closeInvestigation", "arguments": {"closeReason": close_reason, "closeNotes": f"Closed from parent Incident {incident_id}\n\nClose Notes:\n{close_notes}"}, "incidents": (",").join(linked_incidents)})
