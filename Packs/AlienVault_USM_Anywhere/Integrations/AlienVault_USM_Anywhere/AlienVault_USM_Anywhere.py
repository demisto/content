...
def fetch_incidents():
    ...
    # Log the timestamp range of returned alarms for debugging
    if items:
        first_ts = items[0].get("timestamp_occured_iso8601") or items[0].get("timestamp_occured")
        last_ts = items[-1].get("timestamp_occured_iso8601") or items[-1].get("timestamp_occured")
        alarm_ids = [item.get("uuid", "")[:8] for item in items]
        demisto.debug(
            f"Alarm timestamp range: first={first_ts}, last={last_ts}, "
            f"alarm_id_prefixes={alarm_ids}"
        )
    ...
    demisto.debug(
        f"fetch_incidents complete: {len(incidents)} incidents created, "
        f"setting last_fetch={last_fetch}"
    )
