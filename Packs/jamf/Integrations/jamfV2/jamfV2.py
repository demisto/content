...
class Client(BaseClient):
    ...
    def get_computers_request(self, computer_id: str = None, basic_subset: bool = False, match: str = None):
        ...
        uri = "/computers"
        if computer_id:
            res = self._classic_api_get(
                url_suffix=f"{uri}/id/{computer_id}/subset/General", error_handler=self._generic_error_handler
            )
        elif basic_subset:
            res = self._classic_api_get(url_suffix=f"{uri}/subset/basic", error_handler=self._generic_error_handler)
        elif match:
    ...
    if subset == "General":
        ...
        for profile in computers_response.get("configuration_profiles"):
            configuration_profiles.append(
                {"Configuration profile ID": profile.get("id"), "Is Removable": profile.get("is_removable")}
            )
...
def get_computer_id(client: Client, response, subset_name, identifier, identifier_value):
    ...
    if subset_name != "General":
        computer_id = (
            client.get_computer_subset_request(identifier, identifier_value, "General").get("computer").get("general").get("id")
        )
    else:
...
def endpoint_command(client, args):
    ...
    if endpoint_id_list:
        for endpoint_id in endpoint_id_list:
            computers_response = client.get_computer_subset_request(
                identifier="id", identifier_value=endpoint_id, subset="general"
            )
    ...
    if endpoint_hostname_list:
        for endpoint_hostname in endpoint_hostname_list:
            computers_response = client.get_computer_subset_request(
                identifier="name", identifier_value=endpoint_hostname, subset="general"
            )
            outputs.append(computers_response.get("computer").get("general"))
...
def check_authentication_parameters(
    ...
    """
    if (
        (not all([client_id and client_secret]) and not all([username and password]))
        or any([client_id, client_secret])
        and any([username, password])
    ):
