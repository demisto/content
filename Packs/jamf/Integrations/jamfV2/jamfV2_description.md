...
When using the first two methods, provide a username and password. The system will automatically attempt to generate a token using he Bearer token based on basic authentication method. If unsuccessful, it will fallback to using the Basic authentication method.