...
def get_machine_alerts_command(client: MsClient, args: dict):
    ...
    headers = ['ID', 'Title', 'Description', 'IncidentID', 'Severity', 'Status', 'Classification', 'Category',
               'ThreatFamilyName', 'MachineID']
    machine_id = args.get('machine_id')
    response = client.get_machine_alerts(machine_id)
    alerts_list = get_alerts_list(response)
    human_readable = tableToMarkdown(f'Machine {machine_id} related alerts Info:', alerts_list, headers=headers,
                                     removeNull=True)
    context_output = {
        'Machine': machine_id,
        'Alerts': alerts_list
    ...
    entry_context = {
        'MicrosoftATP.MachineAlert(val.Machine === obj.Machine)': context_output
    }
    return human_readable, entry_context, response
