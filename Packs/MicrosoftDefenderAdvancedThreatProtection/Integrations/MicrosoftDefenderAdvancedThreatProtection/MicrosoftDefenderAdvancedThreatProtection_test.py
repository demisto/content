...
client_mocker = MsClient(
    ...
    alert_severities_to_fetch="Informational,Low,Medium,High",
    alert_time_to_fetch="3 days",
    alert_status_to_fetch="New",
    max_fetch="10",
    auth_code="",
    auth_type="",
    redirect_uri="",
    endpoint_type="com",
    alert_detectionsource_to_fetch="",
)
...
def test_first_fetch_incidents(mocker):
    ...
    assert incidents[2].get("name") == "Microsoft Defender ATP Alert da636983472338927033_-2077013687"
...
def test_second_fetch_incidents(mocker):
    ...
    incidents, _ = fetch_incidents(
        client_mocker,
        {"last_alert_fetched_time": "2019-09-01T13:31:07", "existing_ids": ["da637029414680409372_735564929"]},
        False,
    )
    assert incidents == [
        {
            ...
            "occurred": "2019-09-01T13:31:08.0252869Z",
            "dbotMirrorId": "da637029414680409372_735564929",
        }
    ]
...
def test_third_fetch_incidents(mocker):
    ...
    incidents, _ = fetch_incidents(
        client_mocker,
        {"last_alert_fetched_time": "2019-09-01T13:29:37", "existing_ids": ["da637029413772554314_295039533"]},
        False,
    )
    assert incidents[0].get("name") == "Microsoft Defender ATP Alert da637029414680409372_735564929"
...
def test_get_machine_by_ip_with_limit(mocker, params, ip, timestamp, expected):
    ...
    mock_get_machines = mocker.patch.object(MsClient, "get_machines_for_get_machine_by_ip_command", return_value=raw_response)
...
def test_get_alert_related_ips_command(mocker):
    ...
    assert res["MicrosoftATP.AlertIP(val.AlertID === obj.AlertID)"] == {"AlertID": "123", "IPs": ["1.1.1.1"]}
...
def test_get_alert_related_user_command(mocker):
    ...
    assert res["MicrosoftATP.AlertUser(val.AlertID === obj.AlertID)"] == {"AlertID": "123", "User": USER_DATA}
...
def test_get_domain_alerts_command(mocker):
    ...
    assert res["MicrosoftATP.DomainAlert(val.Domain === obj.Domain)"] == {"Domain": "test", "Alerts": [ALERT_DATA]}
...
def test_get_domain_machine_command(mocker):
    ...
    assert res["MicrosoftATP.DomainMachine(val.Domain === obj.Domain)"] == {"Domain": "test", "Machines": [MACHINE_DATA]}
...
def test_get_ip_alerts_command(mocker):
    ...
    assert res["MicrosoftATP.IPAlert(val.IPAddress === obj.IPAddress)"] == {"IPAddress": "1.1.1.1", "Alerts": [ALERT_DATA]}
...
ALERT_RELATED_IPS_API_RESPONSE = {"value": [{"id": "1.1.1.1"}, {"id": "2.2.2.2"}]}

ALERT_RELATED_DOMAINS_API_RESPONSE = {"value": [{"host": "www.example.com"}, {"host": "www.example2.com"}]}
    ...
    "value": [
        {
            ...
            "machineId": "123abc",
            "comments": [
                {"comment": "test comment for docs", "createdBy": "test@test.com", "createdTime": "2019-11-05T14:08:37.8404534Z"}
            ],
        }
    ]
}
...
SINGLE_MACHINE_RESPONSE_API: dict = {
    ...
    "ipAddresses": [
        ...
        {"ipAddress": "fe80::2413:e4aa:a3f4:d5bf", "macAddress": "001122334418", "operationalStatus": "Up", "type": "Ethernet"},
        {"ipAddress": "192.0.2.10", "macAddress": "001122334436", "operationalStatus": "Up", "type": "Ethernet"},
        {"ipAddress": "fe80::55b9:7f5a:6e9c:30ed", "macAddress": "001122334436", "operationalStatus": "Up", "type": "Ethernet"},
        {"ipAddress": "192.0.2.11", "macAddress": "001122334422", "operationalStatus": "Up", "type": "Ethernet"},
        {"ipAddress": "fe80::c3:b878:f6fd:ae4b", "macAddress": "001122334422", "operationalStatus": "Up", "type": "Ethernet"},
        {"ipAddress": "192.0.2.12", "macAddress": "00112233442C", "operationalStatus": "Up", "type": "Ethernet"},
        {"ipAddress": "fe80::65a8:d227:e97b:8220", "macAddress": "00112233442C", "operationalStatus": "Up", "type": "Ethernet"},
    ],
    "isAadJoined": False,
...
MACHINE_ALERTS_OUTPUT = {
    ...
    "Category": "CommandAndControl",
    "Classification": "TruePositive",
    "Comments": [{"Comment": None, "CreatedBy": None, "CreatedTime": None}],
    "ComputerDNSName": None,
    "Description": "A network connection was made to a risky host which has exhibited malicious activity.",
    "DetectionSource": "WindowsDefenderAtp",
    "DetectorID": None,
...
def tests_get_future_time(mocker):
    ...
    mocker.patch(
        "MicrosoftDefenderAdvancedThreatProtection.parse_date_range", return_value=(datetime(1992, 3, 18), datetime(1992, 3, 21))
    )
...
def test_build_std_output_domain():
    ...
    res = build_std_output([{"domainName": domain}])
    assert res["Domain(val.Name && val.Name == obj.Name)"][0]["Name"] == domain


def test_build_std_output_ip():
    ip = "8.8.8.8"
    res = build_std_output([{"networkIPv4": ip}])
    assert res["IP(val.Address && val.Address == obj.Address)"][0]["Address"] == ip


def test_build_std_output_url():
    url = "https://www.example.com/"
    res = build_std_output([{"url": url}])
    assert res["URL(val.Data && val.Data == obj.Data)"][0]["Data"] == url


ip_addresses = [
    {"ipAddress": "ip1", "macAddress": "MAC1", "operationalStatus": "Up", "type": "Ethernet"},
    {"ipAddress": "ip2", "macAddress": "MAC2", "operationalStatus": "Up", "type": "Ethernet"},
    {"ipAddress": "ip3", "macAddress": "MAC1", "operationalStatus": "Up", "type": "Ethernet"},
]
ip_addresses_result = [
    {"MACAddress": "MAC1", "IPAddresses": ["ip1", "ip3"], "Type": "Ethernet", "Status": "Up"},
    {"MACAddress": "MAC2", "IPAddresses": ["ip2"], "Type": "Ethernet", "Status": "Up"},
]

print_ip_addresses_result = (
    "1. | MAC : MAC1 | IP Addresses : ip1,ip3 | Type : Ethernet | Status : Up\n"
...
human_readable_result = (
    "### Microsoft Defender ATP machines ['123abc'] details:\n"
...
@pytest.mark.parametrize(
    "fields_to_filter_by, field_key_from_type_list, expected_query",
    [
...
@pytest.mark.parametrize(
    "indicators_response, expected_result",
    [
        ({"value": []}, []),
        (
        ...
        (
            {
                "value": [
                    {"id": "1", "indicator": "2", "isFailed": "false", "failureReason": "", "name": "no"},
...
FIRST_RUN = {"arguments": "''", "comment": "testing", "machine_id": "machine_id_example", "scriptName": "test_script.ps1"}
SECOND_RUN = {
    "arguments": "''",
    "comment": "testing",
...
def test_run_script_polling(mocker, args, request_status, args_to_compare, expected_results):
    ...
    res = run_polling_command(
        client_mocker, args, "microsoft-atp-live-response-run-script", mock_action_command, mock_get_status, mock_post_process
    )
    ...
    (
        {"machine_id": "machine_id", "scriptName": "test_script.ps1", "comment": "testing", "arguments": "example_arg"},
        {
            "Commands": [
                {
                    "type": "RunScript",
                    "params": [{"key": "ScriptName", "value": "test_script.ps1"}, {"key": "Args", "value": "example_arg"}],
                }
            ],
            "Comment": "testing",
        },
...
GET_FILE_CASES = [
    (
        {"machine_id": "machine_id", "comment": "testing", "path": "C:\\Users\\example\\Desktop\\test.txt"},
        {
            "Commands": [{"type": "GetFile", "params": [{"key": "Path", "value": "C:\\Users\\example\\Desktop\\test.txt"}]}],
            "Comment": "testing",
        },
    ),
]
...
PUT_FILE_CASES = [
    (
        {"machine_id": "machine_id", "comment": "testing", "file_name": "test_script.ps1"},
        {"Commands": [{"type": "PutFile", "params": [{"key": "FileName", "value": "test_script.ps1"}]}], "Comment": "testing"},
    ),
]
...
EMPTY_LAST_RUN: dict = {}
OLD_LAST_RUN_WITH_IDS = {
    "last_alert_fetched_time": "2022-02-17T02:07:23",
    "existing_ids": ["da637806604436477417_-578430041", "da637806604436712653_-30042333"],
}
...
def test_fetch(mocker, case, expected_result):
    ...
    frozen_time = dateparser.parse("2022-02-17T14:39:01.391001Z", settings={"RETURN_AS_TIMEZONE_AWARE": True, "TIMEZONE": "UTC"})
...
""")  # noqa: E501
    ...
    assert (
        str(e.value) == f"Failed to fetch {client_mocker.max_alerts_to_fetch} alerts. "
...
QUERY_BUILDING_CASES = [
    ...
    (
        ...
        "2022-02-17T14:39:01.391001Z",
        None,
        {"$filter": "alertCreationTime+gt+2022-02-17T14:39:01.391001Z", "$orderby": "alertCreationTime asc", "$top": "5"},
    ),
    (
        "Resolved",
        "High",
    ...
    (
        ...
        None,
        {
            "$filter": "alertCreationTime+gt+2022-02-17T14:39:01.391001Z",
            "$orderby": "alertCreationTime asc",
            "$expand": "evidence",
            "$top": "5",
        },
...
@pytest.mark.parametrize(
    "status, severity, limit, evidence, last_fetch_time, detection_sources, expected_result", QUERY_BUILDING_CASES
)
...
class TestHuntingQueryBuilder:
    class TestHelperMethods:
        ...
        def test_build_generic_query(self):
            ...
            :return:
            """
            query_params = assign_params(a='("1")', b='("1","2")', c="", d=None, e=("test_op", '"1","2"'))
...
@pytest.mark.parametrize(
    "query, query_batch, hr_name, timeout",
    [
        (
            "",
            '[{"query": "DeviceInfo | where OnboardingStatus == Onboarded | limit 10'
            ' | distinct DeviceName", "name": "name1", "timeout": "20"}]',
            "name1",
            20,
        ),
...
def test_get_advanced_hunting_command(mocker, query, query_batch, hr_name, timeout):
    ...
    """
    args = {"timeout": "10", "time_range": "1 day", "name": "name", "query": query, "query_batch": query_batch}
    req = mocker.patch.object(
        client_mocker,
        "get_advanced_hunting",
        return_value={"Results": [{"DeviceName": "win2016-msde-agent.msde.lab.demisto"}, {"DeviceName": "ec2amaz-ua9hieu"}]},
    )
...
@pytest.mark.parametrize(
    "query, query_batch, exception, return_value",
    [
        (
            "",
            "",
        ...
        (
            "query",
            "query_batch",
            "Both query and query_batch were given, please provide just one",
            {"Results": [{"DeviceName": "win2016-msde-agent.msde.lab.demisto"}]},
        ),
    ],
)
                ...
                "value": [
                    {
                        ...
                        "cvssV3": 5.3,
                        "exposedMachines": 2,
                        "publishedOn": "2023-09-06T00:00:00Z",
...
@pytest.mark.parametrize(
    "filters_arg_list, name, expected_result",
    [
        (["id1"], "id", "id eq 'id1'"),
...
@pytest.mark.parametrize(
    "filters_arg_list, expected_result",
    [
        (
...
@pytest.mark.parametrize(
    "args_and_name_list, expected_result",
    [
...
def test_create_filter_list_vulnerabilities(
    id_and_severity, name_equal, name_contains, description, published_on, cvss, updated_on, expected_result
):
    from MicrosoftDefenderAdvancedThreatProtection import create_filter_list_vulnerabilities

    result = create_filter_list_vulnerabilities(
        id_and_severity, name_equal, name_contains, description, published_on, cvss, updated_on
    )
    assert result == expected_result


@pytest.mark.parametrize(
    "args, return_value_get_list_software,expected_human_readable,expected_outputs",
    [
        (
            {"vendor": "some_vendor"},
            {
...
@pytest.mark.parametrize(
    "data_to_escape_with_backslash, expected_result",
    [
        (
            [
                {
            ...
            [
                {
                    "id": "some\\_id",
                    "cveId": "CVE-3333-33333",
                    "machineId": "some\\_machine\\_id",
                    "fixingKbId": None,
                    "productName": "some\\_product\\_name",
                    "productVendor": "some\\_vendor",
                    "productVersion": "7.0.2.0",
                    "severity": "High",
                }
            ],
...
def test_test_module_command_with_managed_identities(mocker, requests_mock, client_id):
    ...
    from MicrosoftDefenderAdvancedThreatProtection import MANAGED_IDENTITIES_TOKEN_URL, main
