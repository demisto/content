...
def internal_request(method: str, uri: str, body: dict = {}) -> dict:
    ...
    """
    return demisto.executeCommand(f"core-api-{method.lower()}", {"uri": uri, "body": json.dumps(body)})[0]["Contents"]["response"]  # type: ignore
...
def set_instance(instance: dict, accounts: str) -> dict:
    ...
    accounts_param.update({"hasvalue": True, "value": accounts})
...
def remove_excluded_accounts(account_ids: list, accounts_to_exclude: str | None) -> list[str]:
    return list(set(account_ids) - set(argToList(accounts_to_exclude))) if accounts_to_exclude else account_ids
...
def main():
    try:
        ...
        account_ids, readable_output = get_account_ids(args.get("org_instance_name"), arg_to_number(args.get("max_accounts")))
