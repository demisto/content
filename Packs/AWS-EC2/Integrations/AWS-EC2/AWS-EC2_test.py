...
def validate_kwargs(*args, **kwargs):
    normal_kwargs = {
        "IpPermissions": [{"ToPort": 23, "FromPort": 23, "UserIdGroupPairs": [{}], "IpProtocol": "TCP"}],
        "GroupId": "sg-0566450bb5ae17c7d",
    }
    ippermsfull_kwargs = {
        "GroupId": "sg-0566450bb5ae17c7d",
...
def test_run_on_all_accounts(mocker):
    ...
    # dict as output
    result_func = AWS_EC2.run_on_all_accounts(
        lambda _: CommandResults(
            outputs={},
            readable_output="readable_output",
            outputs_prefix="prefix",
        )
        ...
        (
            {
                "IpamResourceDiscoveries": {
                    ...
                    "IsDefault": True,
                    "OperatingRegions": [{"RegionName": "ap-south-1"}],
        ...
        (
            {
                "IpamDiscoveredPublicAddresses": {
                    ...
                    "SecurityGroups": [{"GroupId": "sg-11111111111111111", "GroupName": "example_sg"}],
            ...
            "|Address|AddressAllocationId|AddressOwnerId|AddressRegion|AddressType|AssociationStatus|InstanceId|"
            "IpamResourceDiscover"
            "yId|NetworkBorderGroup|NetworkInterfaceDescription|NetworkInterfaceId|PublicIpv4PoolId|SampleTime|"
            "SecurityGroups|Subnet"
            "Id|Tags|VpcId|\n|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|\n"
            "| 1.1.1.1 | eipalloc-11111111111111111 | 222222222222 | us-east-1 | amazon-owned-eip | associated |"
            " i-11111111111111111"
            " | ipam-res-disco-11111111111111111 | us-east-1 |  | eni-11111111111111111 | amazon | 2023-11-26T02:00:45 |"
            " {'GroupId':"
...
def test_create_vpc_endpoint_command(mocker):
    ...
    return_boto = {
        "VpcEndpoint": {
            ...
            "Groups": [
                {"GroupId": "test", "GroupName": "test"},
            ],
            "IpAddressType": "ipv4",
            "DnsOptions": {"DnsRecordIpType": "ipv4", "PrivateDnsOnlyForInboundResolverEndpoint": True},
            ...
            "DnsEntries": [
                {"DnsName": "test", "HostedZoneId": "test"},
            ],
            ...
            "OwnerId": "test",
            "LastError": {"Message": "test", "Code": "test"},
        },
        "ClientToken": "test",
    }

    mocker.patch.object(Boto3Client, "create_vpc_endpoint", return_value=return_boto)
    results = AWS_EC2.create_vpc_endpoint_command(
        {"vpcId": "test_endpoint_id", "serviceName": "test", "tagSpecifications": '{"test": "test-tag"}'}
    )
