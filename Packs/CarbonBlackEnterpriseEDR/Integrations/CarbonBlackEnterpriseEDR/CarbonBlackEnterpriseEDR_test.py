...
CLIENT = cbe.Client(base_url="https://server_url.com", use_ssl=False, use_proxy=False, token=None, cb_org_key="123")
...
PROCESS_CASES = [
    (
        ...
        },  # args
        {
            ...
            "rows": 20,
            "start": 0,
            "time_range": {"end": "2020-11-04T13:34:14.758295Z", "start": "2020-11-03T13:34:14.758295Z"},
        },
        # expected
    ),
    (
        {
            "process_name": "svchost.exe,vmtoolsd.exe",
            "event_id": None,
            "query": None,
            "limit": 20,
            "start_time": "1 day",
...
EVENT_CASES = [
    ...
    (
        {"process_guid": "1234", "event_type": "modload", "query": None, "limit": 20, "start": 20, "start_time": "1 day"},  # args
        {
            "criteria": {"event_type": ["modload"]},
            "rows": 20,
            "start": 20,
...
MOCK_UPDATE_THREAT_TAGS_RESPONSE = {"tags": ["tag1", "tag2"]}
...
MOCK_CREATE_THREAT_NOTES_RESPONSE = {"notes": "These are threat notes"}
...
MOCK_GET_THREAT_TAGS_RESPONSE = {"list": [{"tag": "malware"}, {"tag": "suspicious"}]}
...
MOCK_UPDATE_ALERT_NOTES_RESPONSE = {"notes": "These are alert notes"}
...
def test_alert_workflow_update_request_good_arguments(mocker):
    ...
    """
    http_request = mocker.patch.object(CLIENT, "_http_request", return_value=[])
    CLIENT.alert_workflow_update_request(
        alert_id="1234",
        state="OPEN",
        comment="bla1",
        determination="NONE",
    ...
    assert "api/alerts/v7/orgs" in http_request.call_args[0][1]
    assert http_request.call_args.kwargs["json_data"] == {
        "time_range": {"start": "1", "end": "2", "range": "-2w"},
        "criteria": {"id": ["1234"]},
        "determination": "NONE",
        "closure_reason": "bla2",
        "status": "OPEN",
    ...
    (
        {
            "results": [  # response
                {
                    "id": "123",
                    "backend_timestamp": "2023-05-12T08:16:51.779Z",
                    "first_event_timestamp": "2000-04-12T08:14:51.779Z",
                },
                {
                    "id": "456",
                    "backend_timestamp": "2023-05-12T08:17:51.779Z",
                    "first_event_timestamp": "2000-04-12T08:14:51.779Z",
                },
                {
                    "id": "789",
    ...
    (
        {
            "results": [  # response
                ...
                {
                    "id": "123",
                    "backend_timestamp": "2023-06-12T08:17:51.779Z",
                    "first_event_timestamp": "2000-05-12T08:14:51.779Z",
                },
                {
                    "id": "345",
                    "backend_timestamp": "2023-07-12T08:17:51.779Z",
                    "first_event_timestamp": "2000-05-12T08:14:51.779Z",
                },
            ]
        },
...
def test_fetch_incidents__no_alerts(mocker):
    ...
    _, res = fetch_incidents(
        ...
        fetch_time="3 days",
        fetch_limit="50",
        last_run={"last_fetched_alert_create_time": "2000-07-16T05:26:05.491Z", "last_fetched_alerts_ids": ["123"]},
    )
    ...
    mocker.patch.object(
        ...
        return_value={
            "results": [
                {
                    "id": "123",
                    "backend_timestamp": "2000-07-16T05:26:05.491Z",
                    "first_event_timestamp": "2000-04-12T08:14:51.779Z",
                }
            ]
        },
