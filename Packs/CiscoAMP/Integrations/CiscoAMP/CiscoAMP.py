...
Pagination = namedtuple(
    'Pagination',
    ('page', 'page_size', 'limit', 'offset', 'number_of_requests', 'offset_multiplier', 'is_automatic', 'is_manual'),
...
class Client(BaseClient):
    ...
    def app_trajectory_query_list_request(self, ios_bid: str) -> Dict[str, Any]:
        ...
        return self._http_request(
            method='GET',
            url_suffix='/app_trajectory/queries',
            params=params
        )
...
def fetch_incidents(
    ...
    response = client.event_list_request(
    ...
    for item in items:
        ...
        if incident_severities and severity not in incident_severities:
            continue
        ...
        incident = remove_empty_elements({
            ...
            'severity': XSOAR_SEVERITY_BY_AMP_SEVERITY.get(severity),
...
def computer_delete_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    raw_response = client.computer_delete_request(
        connector_guid=connector_guid
    )
...
def event_list_command(client: Client, args: Dict[str, Any]) -> List[CommandResults]:
    ...
    event_type = [arg_to_number(et) for et in event_type if et is not None]  # type: ignore
    page = arg_to_number(args.get('page'))
    page_size = arg_to_number(args.get('page_size'))
    limit = arg_to_number(args.get('limit'))
...
def group_policy_update_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    has_no_policy_guid = (
        not windows_policy_guid
        and not mac_policy_guid
        and not android_policy_guid
        and not linux_policy_guid
    )
...
def groups_delete_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    raw_response = client.group_delete_request(
        group_guid=group_guid
    )
...
def vulnerability_list_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    for request_number in \
        ...
        if not sha256:
            ...
            raw_response_list.append(client.vulnerable_computers_list_request(
                sha256=sha256,
                group_guids=group_guid,
                start_time=start_time,
                end_time=end_time,
...
def combine_response_results(raw_response_list: List[Dict[str, Any]], is_automatic: bool = False) -> Dict[str, Any]:
    ...
    concatenated_raw_response['metadata']['results']['items_per_page'] = \
        concatenated_raw_response['metadata']['results']['current_item_count']
