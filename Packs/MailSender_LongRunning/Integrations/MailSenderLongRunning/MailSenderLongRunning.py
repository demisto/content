...
def extract_entitlement(entitlement):
    """
...
def main():
    ...
    try:
        if demisto.command() == 'long-running-execution':
            while True:
                time.sleep(1)
                SERVER.noop()  # if this fails (timeout) the loop would restart and re-auth
                try:
                    outbox = json.loads(get_integration_context().get('outbox'))
                    for msg in outbox:
                        try:
                            msg['remove'] = True
                            SERVER.sendmail(msg['from'], msg['to'], msg['msg'])
                            msg['response'] = 'Mail sent successfully from queue'
                        except Exception as e:
                            tb = "\n".join(traceback.format_exception(*sys.exc_info()))
                            msg['response'] = 'Error sending email: ' + tb + ' \n\nmsg: ' + str(msg)
                        finally:
                            guid, incident_id, task_id = extract_entitlement(msg['entitlement'])
                            demisto.handleEntitlementForUser(incident_id, guid, msg['user'], msg['response'], task_id)
        ...
        if demisto.command() == 'test-module':
            ...
            if demisto.getArg('async') == 'yes':
                if not demisto.getParam('longRunning'):
                    return_error('Sending emails in async is only supported with Long Running instances.')
                if not demisto.getArg('entitlement'):
                    return_warning('Async is not useful (but still works) without an entitlement, try using !QueuedSendEmail instead')
                msg = {
                    "user": demisto.investigation()['entryUsers'][0],
                    "entitlement": demisto.getArg('entitlement'),
                    "from": FROM,
                    "to": to + cc + bcc,
                    "msg": str_msg,
                }
                set_to_integration_context_with_retries({"outbox": [msg]}, {'outbox': 'entitlement'})
                return_outputs('Mail added to outbox successfully', {}, msg)
            else:
                SERVER.sendmail(FROM, to + cc + bcc, str_msg)  # type: ignore[union-attr]
                SERVER.quit()  # type: ignore[union-attr]
                return_outputs('Mail sent successfully')
        else:
...
if __name__ in ("__builtin__", "builtins", "main"):
    main()
