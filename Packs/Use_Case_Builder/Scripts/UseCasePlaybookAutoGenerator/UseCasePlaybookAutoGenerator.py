...
class Playbook:
    def __init__(self):
        ...
        # ADD COMMENT HERE
        self.taskSectionDict = {
            "Enrichment": "usecaseenrichmentsteps",
            "Containment": "usecasecontainmentsteps",
            "Initiator": "usecaseinitiator",
            "Type": "usecasetype",
    ...
    def addHeader(self, headerName):
        self.playbookTasks[str(self.currentTaskId)] = {
            ...
            "type": "title",
            "task": {"name": headerName, "isTitleTask": True, "type": "title"},
    ...
    def addTask(self, taskName):
        ...
        if taskType == "playbook":
            ...
            self.playbookTasks[str(self.currentTaskId)] = {
                "id": str(self.currentTaskId),
                "type": "regular",
                "task": {"name": taskName, "isCommand": True, "type": "regular", "scriptId": automationName},
                "nextTasks": {"#none#": [str(self.currentTaskId + 1)]},
            }  # Adds task to playbook

        elif taskType == "header":
            self.playbookTasks[str(self.currentTaskId)] = {
                "id": str(self.currentTaskId),
                "type": "title",
                "task": {"name": automationName, "isTitleTask": True, "type": "title"},
                "nextTasks": {"#none#": [str(self.currentTaskId + 1)]},
            }
            # Adds header as next task in playbook
    ...
    def endPlaybook(self):
        self.playbookTasks[str(self.currentTaskId)] = {
            "id": str(self.currentTaskId),
            "type": "title",
            "task": {"name": "Finished", "isTitleTask": True, "type": "title"},
            "nextTasks": {"#none#": []},
        }
    ...
    def createPlaybookJSON(self):
        playbookJSON = [
            {
                "name": self.useCaseName,
                "startTaskId": "0",
                "tasks": self.playbookTasks,
                "view": {"linkLabelsPosition": {}, "paper": {"dimensions": {"height": 380, "width": 385, "x": 50, "y": 50}}},
            }
        ]
        return playbookJSON
    ...
    def addTimers(self):
        self.playbookTasks[str(self.currentTaskId)] = {
            "id": str(self.currentTaskId),
            "type": "title",
            "task": {"name": "Start SLA Timers", "isTitleTask": True, "type": "title"},
            "nextTasks": {"#none#": [str(self.currentTaskId + 1)]},
            "timerTriggers": [
