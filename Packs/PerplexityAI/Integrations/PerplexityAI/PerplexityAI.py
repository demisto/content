...
class Client(BaseClient):
    ...
    def chat(self, args: dict):
        try:
            response = self._http_request(method="POST", url_suffix="/chat/completions", json_data=args)
...
def deepresearch_chat_command(client: Client, args: dict, thinking: str, citations: str, jsonout: str) -> CommandResults:
    ...
    if jsonout == "no":
        ...
        return CommandResults(
            outputs_prefix="DeepResearch", outputs=final_results, readable_output=DictMarkdown(final_results, "")
        )
...
def main() -> None:
    ...
    try:
        ...
        if command == "test-module":
            ...
            args["web_search_options"] = {"search_context_size": args.get("contextsize", "medium")}
            ...
            args["messages"] = [{"role": "system", "content": syscontent}, {"role": "user", "content": usrcontent}]
            for key in [
                "systemmessage",
                "usermessage",
                "citations",
