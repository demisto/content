...
def test_build_teams_message_url_suffix_channel():
    ...
    """
    from MicrosoftTeams import build_teams_message_url_suffix
...
def test_build_policy_violation_body_omits_verdict_when_empty():
    ...
    """
    from MicrosoftTeams import build_policy_violation_body
...
def test_validate_teams_message_target_args_invalid(chat_id, team_id, channel_id, parent_message_id, error_message):
    ...
    Then:
        - A ValueError with the specific message is raised.
    """
    from MicrosoftTeams import validate_teams_message_target_args
...
def test_update_teams_message_policy_violation_command_chat(mocker):
    ...
    When:
        - Running the microsoft-teams-message-update-policy-violation command.
    Then:
        - A PATCH is issued to the chat message path with the policy violation body.
    """
    from MicrosoftTeams import GRAPH_BASE_URL, update_teams_message_policy_violation_command
    ...
    http_mock = mocker.patch("MicrosoftTeams.http_request", return_value={})
    return_results = mocker.patch("MicrosoftTeams.return_results")
    ...
    http_mock.assert_called_once_with(
        "PATCH",
        f"{GRAPH_BASE_URL}/v1.0/chats/chat-1/messages/msg-1",
        json_={
            "policyViolation": {
    ...
    assert result.outputs_prefix == "MicrosoftTeams.TeamsMessagePolicyViolation"
...
def test_update_teams_message_policy_violation_command_channel_reply(mocker):
    ...
    http_mock.assert_called_once_with(
        "PATCH",
        f"{GRAPH_BASE_URL}/v1.0/teams/team-1/channels/chan-1/messages/parent-1/replies/msg-1?model=A",
        json_={
            "policyViolation": {
                "dlpAction": "BlockAccess",
...
def test_update_teams_message_policy_violation_command_invalid_target(mocker):
    ...
    """
    from MicrosoftTeams import update_teams_message_policy_violation_command
