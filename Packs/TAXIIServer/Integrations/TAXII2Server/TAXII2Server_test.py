...
def test_taxii20_collections(mocker, taxii2_server_v20):
    ...
    """
    collections = util_load_json(os.path.dirname(__file__) + '/test_data/collections20.json')
    mocker.patch('TAXII2Server.SERVER', taxii2_server_v20)
    with APP.test_client() as test_client:
...
def test_taxii21_collections(mocker, taxii2_server_v21):
    """
        ...
        Then
            Validate that collections returned as expected
    """
    collections = util_load_json(os.path.dirname(__file__) + '/test_data/collections21.json')
    mocker.patch('TAXII2Server.SERVER', taxii2_server_v21)
    with APP.test_client() as test_client:
        response = test_client.get('/threatintel/collections/', headers=HEADERS)
        assert response.status_code == 200
...
def test_taxii20_manifest(mocker, taxii2_server_v20):
    ...
    """
    iocs = util_load_json(os.path.dirname(__file__) + '/test_data/ip_iocs.json')
    manifest = util_load_json(os.path.dirname(__file__) + '/test_data/manifest20.json')
...
def test_taxii21_manifest(mocker, taxii2_server_v21):
    ...
    """
    iocs = util_load_json(os.path.dirname(__file__) + '/test_data/ip_iocs.json')
    manifest = util_load_json(os.path.dirname(__file__) + '/test_data/manifest21.json')
    mocker.patch.object(demisto, 'params', return_value={'res_size': '100'})
    mocker.patch('TAXII2Server.SERVER', taxii2_server_v21)
    mocker.patch.object(demisto, 'searchIndicators', return_value=iocs)
    with APP.test_client() as test_client:
...
def test_taxii20_objects(mocker, taxii2_server_v20):
    ...
    """
    iocs = util_load_json(os.path.dirname(__file__) + '/test_data/ip_iocs.json')
    objects = util_load_json(os.path.dirname(__file__) + '/test_data/objects20.json')
    mocker.patch('TAXII2Server.SERVER', taxii2_server_v20)
...
def test_taxii20_indicators_objects(mocker, taxii2_server_v20):
    ...
    """
    iocs = util_load_json(os.path.dirname(__file__) + '/test_data/ip_iocs.json')
    objects = util_load_json(os.path.dirname(__file__) + '/test_data/objects20-indicators.json')
    mocker.patch('TAXII2Server.SERVER', taxii2_server_v20)
...
def test_convert_sco_to_indicator_sdo_with_type_file(mocker):
    ...
    """
    xsoar_indicator = util_load_json(os.path.dirname(__file__) + '/test_data/sco_indicator_file.json').get('objects', {})[0]
    ioc = util_load_json(os.path.dirname(__file__) + '/test_data/objects21_file.json').get('objects', {})[0]
...
xsoar_indicators = util_load_json(os.path.dirname(__file__) + '/test_data/xsoar_sco_indicators.json').get('iocs', {})
sco_indicators = util_load_json(os.path.dirname(__file__) + '/test_data/stix_sco_indicators.json').get('objects', {})
...
def test_taxii21_objects_with_relationships(mocker, taxii2_server_v21):
    ...
    mock_iocs = util_load_json(os.path.dirname(__file__) + '/test_data/sort_ip_iocs.json')
    mock_entity_b_iocs = util_load_json(os.path.dirname(__file__) + '/test_data/entity_b_iocs.json')
...
def test_create_entity_b_stix_objects_with_file_object(mocker, taxii2_server_v21):
    ...
    relationships = util_load_json(os.path.dirname(__file__) + '/test_data/relationship_report_file.json')
    iocs = util_load_json(os.path.dirname(__file__) + '/test_data/ioc_for_report_relationship.json')
    mocker.patch.object(demisto, 'searchIndicators', return_value=iocs)
