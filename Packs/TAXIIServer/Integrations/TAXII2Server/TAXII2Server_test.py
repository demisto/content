...
from TAXII2Server import APP, MEDIA_TYPE_STIX_V20, MEDIA_TYPE_TAXII_V20, TAXII2Server, create_fields_list, create_query, uuid
...
def taxii2_server_v21(mocker):
    ...
    server = TAXII2Server(
        ...
        host="demisto",
        port=7000,
        collections={"Collection1": "type:IP", "Collection2": {"query": 'sourceBrands:"Some Feed"', "description": "Test desc"}},
        certificate="",
        private_key="",
        http_server=True,
        credentials={"identifier": "username", "password": "password"},
...
@pytest.mark.parametrize(
    ...
    [
        {"Authorization": _basic_auth_str("username", "password")},
        {"Authorization": _basic_auth_str("username", "password"), "Accept": "wrong_type"},
    ],
)
...
def test_taxii20_get_collections(mocker, taxii2_server_v20):
    ...
    integration_context = {"collections": collections["collections"]}
...
def test_taxii20_get_server_info_demisto_version(mocker):
    ...
    integration_context = {}
    integration_context["server_info"] = {
        "api_roots": ["https://www.example.com/path/to/resource"],
...
def test_taxii20_manifest(mocker, taxii2_server_v20):
    ...
    with APP.test_client() as test_client:
        response = test_client.get("/threatintel/collections/4c649e16-2bb7-50f5-8826-2a2d0a0b9631/manifest/", headers=headers)
        assert response.status_code == 200
        assert response.content_type == "application/vnd.oasis.taxii+json; version=2.0"
...
def test_taxii21_manifest(mocker, taxii2_server_v21):
    ...
    with APP.test_client() as test_client:
        response = test_client.get(
            "/threatintel/collections/4c649e16-2bb7-50f5-8826-2a2d0a0b9631/manifest/?limit=4", headers=HEADERS
        )
        assert response.status_code == 200
        assert response.content_type == "application/taxii+json;version=2.1"
        assert response.json == manifest
...
def test_taxii20_objects(mocker, taxii2_server_v20):
    ...
    mocker.patch.object(demisto, "params", return_value={"res_size": "100"})
    with APP.test_client() as test_client:
        response = test_client.get("/threatintel/collections/4c649e16-2bb7-50f5-8826-2a2d0a0b9631/objects/", headers=headers)
        assert response.status_code == 200
...
def test_taxii20_indicators_objects(mocker, taxii2_server_v20):
    ...
    mocker.patch("TAXII2Server.SERVER", taxii2_server_v20)
    mocker.patch(
        "TAXII2Server.SERVER.types_for_indicator_sdo",
        ["ipv4-addr", "domain-name", "ipv6-addr", "user-account", "email-addr", "windows-registry-key", "file", "url"],
    )
    mocker.patch.object(uuid, "uuid4", return_value="1ffe4bee-95e7-4e36-9a17-f56dbab3c777")
    headers = copy.deepcopy(HEADERS)
    headers["Content-Range"] = "items 0-2/5"
...
@pytest.mark.parametrize(
    "demisto_iocs_file,res_file,query_type",
    [
...
@pytest.mark.parametrize("api_request", ["objects", "manifest"])
...
@pytest.mark.parametrize(
    "res_file,fields,has_extension",
    [("objects21_no_extention_file", {"name", "type"}, False), ("objects21_spec_fields_file", {"sha1"}, True)],
)
...
def test_taxii21_objects_with_relationships(mocker, taxii2_server_v21):
    ...
    mocker.patch.object(demisto, "searchIndicators", side_effect=[mock_iocs, mock_entity_b_iocs])
