...
if queryValue and queryType:
    resp = queryBuilder(queryType, escapeSpecialCharacters(queryValue), attrs, nestedSearch,
                        demisto.get(demisto.args(), 'using'), userAccountControlOut)
else:
...
if isError(resp):
    ...
    for response in resp:
        ...
        if isinstance(data, str) or isinstance(data, unicode):
            ...
            context['Account(val.Email && val.Email === obj.Email || val.ID && val.ID === obj.ID || val.Username && val.Username === obj.Username)'] = createAccountEntities(
                data, attrs)
