...
def find_additional_ad_info(email: str, manager_attribute: str) -> dict:
    ...
    filter_str = rf"(&(objectClass=user)(mail={email}))"
    response = demisto.executeCommand("ad-search", {"filter": filter_str, "attributes": "displayName," + manager_attribute})

    if response and isError(response[0]):
        demisto.results(response)
        sys.exit(0)

    if not (type(response) is list and type(response[0].get("Contents")) is list and type(response[0]["Contents"][0]) is dict):
    ...
    # seems to be different from the original.
        ...
        data.get("attributes")
        and data["attributes"].get(manager_attribute)
    ...
    # This was added as the json structure returned from the AD search during testing
    # seems to be different from the original.
    elif data.get("attributes") and data["attributes"].get("displayName") and type(data["attributes"]["displayName"]) is list:
        additional_info["employee_name"] = data["attributes"]["displayName"][0]

    filter_str = rf'(&(objectClass=User)(distinguishedName={additional_info["manager_dn"]}))'
    ...
    if type(response) is list and type(response[0].get("Contents")) is list and type(response[0]["Contents"][0]) is dict:
        data = response[0]["Contents"][0]
        ...
        # This was added as the json structure returned from the AD search during testing
        # seems to be different from the original.
        elif data.get("attributes") and data["attributes"].get("mail") and type(data["attributes"]["mail"]) is list:
