...
DEFAULT_PERSON_ATTRIBUTES = ["name", "displayName", "memberOf", "mail", "sAMAccountName", "manager", "userAccountControl"]
...
def account_entry(person_object, custom_attributes):
    ...
    lower_cased_person_object_keys = {person_object_key.lower(): person_object_key for person_object_key in person_object}
...
def endpoint_entry(computer_object, custom_attributes):
    ...
    lower_cased_person_object_keys = {person_object_key.lower(): person_object_key for person_object_key in computer_object}
...
def group_entry(group_object, custom_attributes):
    ...
    lower_cased_person_object_keys = {person_object_key.lower(): person_object_key for person_object_key in group_object}
...
def get_user_activity_by_samaccountname(default_base_dn, samaccountname):
    ...
    entries = search_with_paging(query, default_base_dn, attributes=["userAccountControl"], size_limit=1, page_size=1)
...
def get_user_dn_by_email(default_base_dn, email):
    ...
    query = f"(&(objectClass=User)(objectCategory=person)(sAMAccountName={samaccountname}))"
    entries = search_with_paging(query, default_base_dn, attributes=["sAMAccountName"], size_limit=1, page_size=1)
...
def search_with_paging(search_filter, search_base, attributes=None, page_size=100, size_limit=0, time_limit=0, page_cookie=None):
    """
    find entries in the DIT
    ...
    return {"raw": raw, "flat": flat, "page_cookie": encode_cookie}
...
def convert_special_chars_to_unicode(search_filter):
    ...
    chars_to_replace = {"\\(": "\\28", "\\)": "\\29", "\\*": "\\2a", "\\/": "\\2f", "\\\\": "\\5c"}
...
def free_search(default_base_dn, page_size):
    ...
    ec = (
        {}
        if context_output == "no"
        else {
            "ActiveDirectory.Search(obj.dn == val.dn)": entries["flat"],
            "ActiveDirectory(true)": {"SearchPageCookie": entries["page_cookie"]},
        }
    )
    demisto_entry = {
...
def get_user_iam(default_base_dn, args, mapper_in, mapper_out):
    ...
    try:
        ...
        iam_user_profile = IAMUserProfile(
            ...
            user_profile_delta=user_profile_delta,
            mapper=mapper_out,
        ...
        entries = search_with_paging(query, default_base_dn, attributes=attributes, size_limit=1, page_size=1)
        ...
        iam_user_profile.set_result(success=False, error_code=error_code, error_message=str(e), action=IAMActions.GET_USER)
        return iam_user_profile
...
def search_group_members(default_base_dn, page_size):
    ...
    demisto_entry = {
        ...
        "EntryContext": {
            "ActiveDirectory.Groups(obj.dn ==" + group_dn + ")": {"dn": group_dn, "members": members},
    ...
    if member_type == "person":
        ...
        demisto_entry["EntryContext"]["Account"] = [account_entry(entry, custom_attributes) for entry in entries["flat"]]
    elif member_type == "computer":
        demisto_entry["EntryContext"]["ActiveDirectory.Computers(obj.dn == val.dn)"] = entries["flat"]
        demisto_entry["EntryContext"]["Endpoint"] = [endpoint_entry(entry, custom_attributes) for entry in entries["flat"]]
    elif member_type == "group":
        demisto_entry["EntryContext"]["ActiveDirectory.Groups(obj.dn == val.dn)"] = entries["flat"]
        demisto_entry["EntryContext"]["Group"] = [group_entry(entry, custom_attributes) for entry in entries["flat"]]
...
def create_user_iam(default_base_dn, args, mapper_out, disabled_users_group_cn):
    ...
    try:
        ...
        if not ad_user.get("ou"):
            raise DemistoException(
                "User must have an Organizational Unit (OU). Please make sure you've added a "
        ...
        if user_exists:
            ...
            if success:
                iam_user_profile.set_result(
                    ...
                    active=False,
                )  # the user should be activated with the IAMInitADUser script

            else:
                error_msg = (
                    "Please validate your instance configuration and make sure all of the "
...
def get_iam_user_profile(user_profile, mapper_out):
    ...
    if old_user_data:
        iam_old_user_profile = IAMUserProfile(
            user_profile=old_user_data, mapper=mapper_out, incident_type=IAMUserProfile.UPDATE_INCIDENT_TYPE
        )
        ad_old_user = iam_old_user_profile.map_object(mapper_name=mapper_out, incident_type=IAMUserProfile.UPDATE_INCIDENT_TYPE)
    ...
    try:
        ...
        if not user_exists:
            if create_if_not_exists:
                ...
                iam_user_profile.set_result(action=IAMActions.UPDATE_USER, skip=True, skip_reason=error_message)
        else:
        ...
        error_code, _ = IAMErrors.BAD_REQUEST
        iam_user_profile.set_result(success=False, error_code=error_code, error_message=str(e), action=IAMActions.UPDATE_USER)
        return iam_user_profile
...
def create_contact():
    ...
    if args.get("custom-attributes"):
        try:
            ...
            demisto.info(str(e))
            raise Exception("Failed to parse custom attributes argument. Please see an example of this argument in the argument.")
...
def create_group():
    ...
    if args.get("members"):
        ...
        attributes = {"samAccountName": group_name, "groupType": group_type, "member": members}
    else:
        attributes = {"samAccountName": group_name, "groupType": group_type}
    ...
    demisto_entry = {"ContentsFormat": formats["text"], "Type": entryTypes["note"], "Contents": f"Created group with DN: {dn}"}
    demisto.results(demisto_entry)
...
def expire_user_password(default_base_dn):
    ...
    demisto_entry = {"ContentsFormat": formats["text"], "Type": entryTypes["note"], "Contents": "Expired password successfully"}
    demisto.results(demisto_entry)
...
def set_user_password(default_base_dn, port):
    ...
    demisto_entry = {"ContentsFormat": formats["text"], "Type": entryTypes["note"], "Contents": "User password successfully set"}
    demisto.results(demisto_entry)
...
def enable_user(default_base_dn, default_page_size):
    ...
    # modify user
    modification = {"userAccountControl": [("MODIFY_REPLACE", turn_disable_flag_off(account_options))]}
    modify_object(dn, modification)
...
def enable_user_iam(default_base_dn, dn, disabled_users_group_cn):
    ...
    :param disabled_users_group_cn: The disabled group cn, the user will be removed from this group when enabled
    """
    modification = {"userAccountControl": [("MODIFY_REPLACE", PASSWORD_NOT_REQUIRED)]}
    modify_object(dn, modification)
...
def disable_user_iam(default_base_dn, disabled_users_group_cn, args, mapper_out):
    ...
    try:
        ...
        user_exists = check_if_user_exists_by_attribute(default_base_dn, "sAMAccountName", sam_account_name)
        if not user_exists:
            iam_user_profile.set_result(success=True, action=IAMActions.DISABLE_USER, skip=True, skip_reason="User doesn't exist")
            return iam_user_profile

        dn = user_dn(sam_account_name, default_base_dn)

        # modify user
        modification = {"userAccountControl": [("MODIFY_REPLACE", DISABLED_ACCOUNT)]}
        ...
        error_code, _ = IAMErrors.BAD_REQUEST
        iam_user_profile.set_result(success=False, error_code=error_code, error_message=str(e), action=IAMActions.DISABLE_USER)
        return iam_user_profile
...
def add_member_to_group(default_base_dn):
    ...
    if args.get("username"):
        ...
        computers = argToList(args.get("computer-name"))
        member_dns = []
        for c in computers:
...
def remove_member_from_group(default_base_dn):
    ...
    if not success:
        raise Exception(
            "Failed to remove {} from group {}".format(args.get("username") or args.get("computer-name"), args.get("group-cn"))
        )
...
def delete_group():
    ...
    demisto_entry = {"ContentsFormat": formats["text"], "Type": entryTypes["note"], "Contents": f"Deleted group with DN: {dn}"}
    demisto.results(demisto_entry)
...
def set_password_not_expire(default_base_dn):
    ...
    if user := entries.get("flat"):
        ...
        # Modify user
        modify_object(dn, modification)
        demisto_entry = {"ContentsFormat": formats["text"], "Type": entryTypes["note"], "Contents": content_output}
        demisto.results(demisto_entry)
...
def main():
    ...
    try:
        ...
        # IAM commands
            ...
            user_profile = update_user_iam(default_base_dn, args, create_if_not_exists, mapper_out, disabled_users_group_cn)
            return return_results(user_profile)
