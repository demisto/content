...
2. Click **Add instance** to create and configure a new integration instance.
    ...
    *  **Server IP address (for example, 192.168.0.1)**: The Server IP that should be used to access Active Directory.
...
| attributes | A CSV list of the object attributes to return. For example, "dn,memberOf". To get all object attributes, specify ‘ALL’. | Optional |
...
| context-output | Whether to output the search results to the context. | Optional |
...
| group-cn | The name of the group to add the user from. | Required |
...
| ActiveDirectory.Users.memberOf | string | Groups in which the user is member. |
...
| disable-nested-search | Whether to disable recursive retrieval of a user's group memberships. | Optional |
...
| IAM.Vendor.details | String | Tells the user if the API was successful, otherwise provides error information. |
...
| IAM.Vendor.instanceName | String | The name of the integration instance. |
