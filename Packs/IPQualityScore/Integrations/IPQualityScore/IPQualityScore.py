...
def email_command(client, args, email_suspicious_score_threshold, email_malicious_score_threshold, reliability):
    ...
    for email in emails:
        ...
        ip_context = Common.EMAIL(
            address=email, dbot_score=reputation, domain=result.get("sanitized_email", email).split("@")[-1]
        )
...
def url_command(client, args, url_suspicious_score_threshold, url_malicious_score_threshold, reliability):
    ...
    for url in urls:
        ...
        ip_context = Common.URL(url=url, dbot_score=reputation)
...
def main():
    ...
    try:
        if demisto.command() == "test-module":
            ...
            client = Client(base_url=base_url_ip if base_url_ip.endswith("/") else base_url_ip + "/", verify=verify_certificate, proxy=proxy)
            ...
            reliability = demisto.params().get("feedReliability")
            client = Client(base_url=base_url_email if base_url_email.endswith("/") else base_url_email + "/", verify=verify_certificate, proxy=proxy)
            return_results(
                email_command(
            ...
            reliability = demisto.params().get("feedReliability")
            client = Client(base_url=base_url_url if base_url_url.endswith("/") else base_url_url + "/", verify=verify_certificate, proxy=proxy)
            return_results(
