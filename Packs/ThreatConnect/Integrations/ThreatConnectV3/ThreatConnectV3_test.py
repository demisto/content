...
def groups_fixture() -> list:
    return [
        {"dateAdded": "2022-08-04T12:35:33Z", "id": 1},
        {"dateAdded": "2022-09-06T12:35:33Z", "id": 2},
        {"dateAdded": "2022-03-06T12:35:33Z", "id": 3},
        {"dateAdded": "2022-09-06T12:36:33Z", "id": 3},
        {"dateAdded": "2022-08-06T11:35:33Z", "id": 4},
    ]


@pytest.mark.parametrize(
    "last_run, expected_result", [("2022-07-04T12:35:33", "2022-09-06T12:36:33"), ("2023-07-04T12:35:33", "2023-07-04T12:35:33")]
)
    ...
    res = (
        {
            ...
            "URL(val.Data && val.Data == obj.Data)": [
                {"Data": "http://yourwebsite.com/opcache.php", "Malicious": {"Description": "", "Vendor": "ThreatConnect"}}
            ],
        },
        [
            {
...
def test_list_groups(mocker):
    ...
    assert (
        mock.call_args.args[1] == "/api/v3/groups?tql=a%2Cbtag%20like%20%22%25a%25%22%20AND%20tag%20like%"
    ...
    assert (
        mock.call_args.args[1] == "/api/v3/groups?tql=%28id%3Dtest%20%29a%2Cb%20AND%20tag%20like%20%22%25a%25"
...
def test_set_additional_data_with_mode():
    ...
    mode = "test_mode"
    expected_output = {"data": [{"name": "label1"}, {"name": "label2"}], "mode": mode}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "internet",
            None,
            {"address": "test@example.com", "networkType": "internet"},
        ),
    ],
)
...
ARGS_INCLUDE_ALL = {"include_all_metaData": True, "filter": "Test Filter", "victim_id": "test_victim_id", "limit": 20, "page": 1}

EXPECTED_URL_INCLUDE_ALL = (
    f"{VICTIM_API_PREFIX}/test_victim_id?"
...
@pytest.mark.parametrize(
    "args, expected_url",
    [(ARGS_INCLUDE_ASSETS_ATTRIBUTES, EXPECTED_URL_ASSETS_ATTRIBUTES), (ARGS_INCLUDE_ALL, EXPECTED_URL_INCLUDE_ALL)],
)
...
def test_list_victim_assets_command(mocker):
    ...
    Then:
        - The url request contains all given fields
    """
    args = {"filter": "Test Filter", "victim_asset_id": "test_victim_asset_id", "limit": 20, "page": 1}
...
def test_list_victim_attributes_command(mocker):
    ...
    Then:
        - The url request contains all given fields
    """
    args = {"filter": "Test Filter", "victim_attribute_id": "test_victim_attribute_id", "limit": 20, "page": 1}
...
def test_tc_add_indicator_command_with_description(mocker):
    ...
    tc_add_indicator_command(
        client, {"tags": [], "indicator": "1.1.1.1", "indicatorType": "Address", "description": "description"}
    )
    # Verifying if the client.make_request method was called with the expected arguments
