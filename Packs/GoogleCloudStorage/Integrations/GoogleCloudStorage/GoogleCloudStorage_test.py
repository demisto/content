...
def test_ec_key():
    ...
    assert ec_key("Foo.Bar.Baz", "ID1", "ID2") == "Foo.Bar.Baz(val.ID1 && val.ID1 === obj.ID1 && val.ID2 && val.ID2 === obj.ID2)"

    assert (
        ec_key("Foo.Bar.Baz", "ID1", "ID2", "ID3") == "Foo.Bar.Baz(val.ID1 && val.ID1 === obj.ID1"
...
def test_datetime2str():
    assert (
        datetime2str(datetime(year=2019, month=8, day=28, hour=11, minute=28, second=47, microsecond=165123))
        == "2019-08-28T11:28:47"
    )
    assert (
        datetime2str(datetime(year=2001, month=4, day=14, hour=23, minute=32, second=15, microsecond=999999))
        == "2001-04-14T23:32:15"
    )
    assert datetime2str(datetime(year=2030, month=11, day=7, hour=14, minute=7, second=0, microsecond=0)) == "2030-11-07T14:07:00"


def test_human_readable_table():
    # Verify that 1. header order is preserved, 2. spaces are added between (capitalized) header words
    assert (
        human_readable_table("My Table", {"HeaderOne": "value one", "HeaderTwo": "value two", "HeaderThree": "value three"})
        == "### My Table\n"
