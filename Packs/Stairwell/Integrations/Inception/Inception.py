...
class Client(BaseClient):
    def get_file_reputation(self, file_hash: str) -> Dict[str, Any]:
        return self._http_request(method="GET", url_suffix=file_hash)
...
def file_enrichment_command(client, file_hash):
    try:
        ...
        if "404" in str(err):
            results = CommandResults(readable_output="File not found: " + file_hash)
            return results
        else:
            raise err
...
def variant_discovery_command(client, file_hash):
    try:
        ...
        if "similarity" in response:
            ...
            results = CommandResults(readable_output="No variants discovered for: " + file_hash)
            return results
    except DemistoException as err:
