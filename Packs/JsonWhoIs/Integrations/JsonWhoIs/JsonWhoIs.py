...
def http_request(method, url_suffix, params=None, max_retry=3):
    for _ in range(max_retry):
        res = requests.request(method, urljoin(BASE_URL, url_suffix), verify=USE_SSL, params=params, headers=HEADERS)
...
def whois_command():
    ...
    return_outputs(readable_output=human_readable, outputs=ec, raw_response=raw)
