...
def test_fetch_events_command(mocker, requests_mock, fetch_networking):
    ...
    requests_mock.get(
        f"{BASE_URL}/auditlogs/v1/events", request_headers={"authorization": f"Bearer {TEST_TOKEN}"}, json=audit_response_mock
    )
    requests_mock.get(
        f"{BASE_URL}/monitoring/v2/events",
    ...
    mocker.patch(
        "HPEArubaCentralEventCollector.get_integration_context",
        return_value={"access_token": TEST_TOKEN, "expiry_time": FETCH_TIME + 1},
    )
    ...
    expected_events = (
        audit_response_mock["events"]
        if not fetch_networking
        else (audit_response_mock["events"] + networking_response_mock["events"])
    )
...
def test_get_access_token(mocker, requests_mock):
    ...
    def match_login_request(request):
        return request.qs.get("client_id")[0] == CLIENT_ID and request.json() == {
            "username": USER_NAME,
            "password": USER_PASSWORD,
        }

    requests_mock.post(
        f"{BASE_URL}/oauth2/authorize/central/api/login",
    ...
    def match_auth_code_request(request):
        return (
            request.qs.get("client_id")[0] == CLIENT_ID
            and request.qs.get("response_type")[0] == "code"
            and request.json() == {"customer_id": CUSTOMER_ID}
        )

    requests_mock.post(
        f"{BASE_URL}/oauth2/authorize/central/api",
        additional_matcher=match_auth_code_request,
        request_headers={
...
def test_fetch_with_duplicates(mocker, requests_mock):
    ...
    requests_mock.get(
        f"{BASE_URL}/auditlogs/v1/events", request_headers={"authorization": f"Bearer {TEST_TOKEN}"}, json=first_audit_response
    )
    requests_mock.get(
        f"{BASE_URL}/monitoring/v2/events",
        request_headers={"authorization": f"Bearer {TEST_TOKEN}"},
    ...
    send_events_to_xsiam_mock.assert_called_once_with(
        expected_audit_events + expected_networking_events, vendor=VENDOR, product=PRODUCT
    )

    # Mock next fetch to get all of the events, including the previously fetched
    requests_mock.get(
        f"{BASE_URL}/auditlogs/v1/events", request_headers={"authorization": f"Bearer {TEST_TOKEN}"}, json=full_audit_response
    )
    requests_mock.get(
        f"{BASE_URL}/monitoring/v2/events",
        request_headers={"authorization": f"Bearer {TEST_TOKEN}"},
...
def test_aruba_auth_test(mocker, should_fail):
    ...
    if should_fail:
        mocker.patch(
            "HPEArubaCentralEventCollector.fetch_events",
...
def test_validate_authentication_params_no_credentials_at_all():
    """
    Given:
    - Neither a Download Token nor Username/Password is provided.
    ...
    """
    with pytest.raises(DemistoException, match="No authentication credentials provided"):
        validate_authentication_params("", "", "", "")
...
def test_validate_authentication_params_userpass_missing_customer_id():
    """
    Given:
    - No Download Token, Username/Password provided, but no Customer ID.
    ...
    - A DemistoException is raised.
    """
    with pytest.raises(DemistoException, match="Customer ID is required"):
        validate_authentication_params("", USER_NAME, USER_PASSWORD, "")


def test_validate_authentication_params_valid_configs():
    """
    Given:
    - Valid configurations: token-based, or Username/Password with Customer ID.
    ...
    """
    # Token-based: no other credentials needed (even if user/pass/customer_id are empty).
    validate_authentication_params(DOWNLOAD_TOKEN_REFRESH_ONLY, "", "", "")
    # Username/Password fallback with a Customer ID.
    validate_authentication_params("", USER_NAME, USER_PASSWORD, CUSTOMER_ID)
