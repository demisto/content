...
class Client(BaseClient):
    ...
    def get_cves_by_product_request(self, vendor_name: str, product_name: str, params: dict | None = {}) -> dict:
        ...
        response = self._http_request("GET", f"vendors/{vendor_name}/products/{product_name}/cve", params=params)
        return response
...
def cve_to_indicator(cve: dict, ocve: OpenCVE, relationships: list) -> Common.CVE:
    ...
    publications = [
        Common.Publications(title=publication["title"], source=publication["source"], link=publication["link"])
        for publication in fields.get("publications", [])
    ]
...
def parse_tags(vendors: dict, cve_id: str, cwes: list) -> tuple[list[EntityRelationship], list]:
    ...
    for vendor in vendors.values():
        ...
        relationships.extend(
            [
                EntityRelationship(
                    name="targets", entity_a=cve_id, entity_a_type="cve", entity_b=product, entity_b_type="software"
                )
...
def parse_cve(ocve: OpenCVE, cve: dict):
    ...
    parsed_cve: dict = {
        ...
        "fields": {"description": f'{cve.get("summary", "No description available")}', "trafficlightprotocol": ocve.tlp},
    }
    ...
    parsed_cve["fields"]["publications"] = [
        {"title": reference.get("name", None), "source": reference.get("source", None), "link": reference.get("url", None)}
...
def cve_latest_command(client: Client, ocve: OpenCVE, args: dict) -> CommandResults:
    ...
    demisto.setLastRun(
        {  # Save the current run info as the lastRun variable
            "fetch_time": lastRun,
            "fetch_count": len(parsed_cves),
        }
    )

    return CommandResults(outputs_prefix="OpenCVE.CVE", outputs=parsed_cves)
...
def get_my_vendors_command(client: Client) -> CommandResults:
    ...
    return CommandResults(outputs_prefix="OpenCVE.myVendors", outputs=result)
...
def get_my_products_command(client: Client) -> CommandResults:
    ...
    return CommandResults(outputs_prefix="OpenCVE.myProducts", outputs=result)
...
def get_vendors_command(client: Client, args: dict) -> CommandResults:
    ...
    if vendor := args.get("vendor_name", ""):
        ...
        return CommandResults(outputs_prefix=f"OpenCVE.{vendor}", outputs=result)
        ...
        return CommandResults(outputs_prefix="OpenCVE.Vendors", outputs=vendors)
...
def get_products_command(client: Client, args: dict) -> CommandResults | None:
    ...
    if vendor := args.get("vendor_name", None):
        if product := args.get("product_name", None):
            ...
            return CommandResults(outputs_prefix=f"OpenCVE.{vendor}.{product}", outputs=results)
            ...
            return CommandResults(outputs_prefix=f"OpenCVE.{vendor}.Products", outputs=results)
    else:
...
def get_reports_command(client: Client, args: dict) -> CommandResults:
    ...
    if report_id := args.get("report_id", None):
        ...
        return CommandResults(outputs_prefix=f"OpenCVE.Reports.{report_id}", outputs=results)
        ...
        return CommandResults(outputs_prefix="OpenCVE.Reports", outputs=results)
...
def get_alerts_command(client: Client, args: dict) -> CommandResults | None:
    ...
    if report_id := args.get("report_id", None):
        if alert_id := args.get("alert_id", None):
            ...
            return CommandResults(outputs_prefix=f"OpenCVE.Reports.Alerts.{alert_id}", outputs=results)
        else:
            params = {}
            ...
            return CommandResults(outputs_prefix="OpenCVE.Reports.Alerts", outputs=results)
...
def main():  # pragma: no cover
    ...
    try:
        ...
        demisto.debug(f"Exception {err!s} ")
