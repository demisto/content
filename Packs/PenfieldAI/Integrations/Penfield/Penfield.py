...
class Client(BaseClient):
    def live_assign_get(self, analyst_ids, category, created, arg_id, name, severity) -> str:
        params = assign_params(
            analyst_ids=analyst_ids, category=category, created=created, id=arg_id, name=name, severity=severity
        )
        response = self._http_request(method="POST", url_suffix="/api/v1/xsoar_live_assign/", params=params)
        return response["analyst"]

    def test(self) -> str:
        response = self._http_request(method="GET", url_suffix="/api/v1/xsoar_live_assign/")
        return response
...
def get_assignee(client: Client, args) -> CommandResults:
    ...
    human_readable = tableToMarkdown(
        "Analyst Penfield Recommends", analyst, headers=["Recommendation"], headerTransform=pascalToSpace, removeNull=True
    )
    return CommandResults(
        readable_output=human_readable, outputs_prefix="Penfield.Recommended", outputs_key_field="Analyst", outputs=analyst
    )
