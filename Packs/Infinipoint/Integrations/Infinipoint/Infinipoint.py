...
COMMANDS_CONFIG = {
    ...
    "infinipoint-get-device": {
        "args": {"host": "contains", "osType": "=", "osName": "contains", "status": "=", "agentVersion": "="},
        "route": "/api/devices",
    ...
    "infinipoint-execute-action": {
        "args": {"id": "contains", "target": "contains"},
    ...
    "infinipoint-get-non-compliance": {
        "args": {"offset": "contains", "limit": "contains"},
...
class Client(BaseClient):
    def call_command(self, url_suffix: str, args: dict[str, Any], pagination=True, page_index=0, method="POST") -> dict[str, Any]:
        """
        ...
        return self._http_request(method=method, url_suffix=url_suffix, json_data=args)
    ...
    def call_api(self, route: str, rules, pagination=True, condition="AND", method="POST"):
        ...
        if not pagination:
            ...
            query = {"pageSize": 100, "page": 0, "ruleSet": {"condition": condition, "rules": rules}}
...
def get_auth_headers(access_key, private_key):
    ...
    """
    try:
        payload = {"iat": int(time.time()), "sub": access_key}
...
def fetch_incidents(client, last_run: dict[str, int], first_fetch_time: int | None):
    ...
    args = {"limit": max_results, "offset": last_fetch}
...
def test_module(route, base_url, insecure, headers):
    ...
    """
    res = requests.request("POST", base_url + route, headers=headers, verify=insecure)
...
def run_queries_command(client: Client, args: dict, optional_args=None):
    ...
    if res:
        command_results = CommandResults(
            outputs_prefix=optional_args["outputs_prefix"], outputs_key_field=optional_args["outputs_key_field"], outputs=res
        )
...
def main():
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            fetch_incidents(client=client, last_run=demisto.getLastRun(), first_fetch_time=first_fetch_time)

        elif demisto.command() == "infinipoint-execute-action":
            return_results(
                run_queries_command(
                    client=client, args=demisto.args(), optional_args=COMMANDS_CONFIG["infinipoint-execute-action"]
                )
            )

        elif demisto.command() in COMMANDS_CONFIG:
            return_results(
                infinipoint_command(client=client, args=demisto.args(), optional_args=COMMANDS_CONFIG[demisto.command()])
            )
