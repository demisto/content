...
SORT_DICTIONARY = {"most_recent": "desc", "least_recent": "asc"}
...
class Client(BaseClient):
    ...
    def get_token_and_set_headers(self, auth: tuple) -> dict:
        ...
        try:
            ...
            raise DemistoException(
                "Authentication failed. Verify the Qualys API Platform URL, "
    ...
    def get_incident_events(self, incident_id: str, data: dict):
        ...
        return:
            response (Dict): API response from Qualys FIM.
        """
        return self._http_request(method="POST", url_suffix=f"fim/v2/incidents/{incident_id}/events/search", json_data=data)
    ...
    def assets_list(self, params: dict):
        ...
        return:
            response (Dict): API response from Qualys FIM.
        """
        return self._http_request(method="POST", url_suffix="fim/v3/assets/search", json_data=params)
    ...
    def create_incident(self, data: dict):
        ...
        return self._http_request(method="POST", url_suffix="fim/v3/incidents/create", json_data=data)
    ...
    def approve_incident(self, incident_id: str, data: dict):
        ...
        return:
            response (Dict): API response from Qualys FIM.
        """
        return self._http_request(method="POST", url_suffix=f"fim/v3/incidents/{incident_id}/approve", json_data=data)


def create_event_or_incident_output(item: Dict, table_headers: List[str]) -> Dict[str, str]:
    """
...
def list_events_command(client: Client, args: dict):
    ...
    if raw_response:
        for item in raw_response:
            ...
            if date_time:
                item_dictionary["dateTime"] = datetime.strptime(date_time, DATETIME_FORMAT).strftime(TABLE_DATETIME_FORMAT)

            outputs.append(item_dictionary)

    readable_output = tableToMarkdown(name=f"Listed {len(outputs)} Events:", t=outputs, headers=table_headers, removeNull=True)

    return CommandResults(
        outputs_prefix="QualysFIM.Event",
...
def get_event_command(client: Client, args: dict):
    ...
    table_headers = ["name", "action", "id", "severity", "action", "incidentId", "profiles", "type", "dateTime", "fullPath"]

    object_data = create_event_or_incident_output(raw_response, table_headers)
    object_data["dateTime"] = datetime.strptime(str(object_data.get("dateTime")), DATETIME_FORMAT).strftime(TABLE_DATETIME_FORMAT)

    readable_output = tableToMarkdown(name="Found Event:", t=object_data, headers=table_headers, removeNull=True)
...
def list_incidents_command(client: Client, args: dict):
    ...
    readable_output = tableToMarkdown(name=f"Listed {len(outputs)} Incidents:", t=outputs, headers=table_headers, removeNull=True)

    return CommandResults(
        outputs_prefix="QualysFIM.Incident",
        outputs_key_field="id",
        raw_response=raw_response,
        outputs=raw_outputs,
        readable_output=readable_output,
    ...
    if raw_response:
        for item in raw_response:
            ...
            if data:
                ...
                date_time = item_dictionary.get("dateTime")
                if date_time:
                    date_time = datetime.strptime(date_time, DATETIME_FORMAT).strftime(TABLE_DATETIME_FORMAT)
                    item_dictionary["dateTime"] = date_time

                outputs.append(item_dictionary)

    readable_output = tableToMarkdown(
        name=f"Listed {len(outputs)} Events From Incident:", t=outputs, headers=table_headers, removeNull=True
    )
...
def create_incident_command(client: Client, args: dict):
    ...
    readable_output = tableToMarkdown(
        name=f'Created New Incident: {raw_response.get("name")}', t=output, headers=table_headers, removeNull=True
    )
...
def approve_incident_command(client: Client, args: dict):
    ...
    raw_response = client.approve_incident(str(args.get("incident_id")), remove_empty_elements(data))
    ...
    if raw_response:
        output = create_event_or_incident_output(raw_response, table_headers)
        output["approvalDate"] = datetime.strptime(str(output.get("approvalDate")), DATETIME_FORMAT).strftime(
            TABLE_DATETIME_FORMAT
        )

        output["filterFromDate"] = datetime.strptime(str(output.get("filterFromDate")), DATETIME_FORMAT).strftime(
            TABLE_DATETIME_FORMAT
        )

        output["filterToDate"] = datetime.strptime(str(output.get("filterToDate")), DATETIME_FORMAT).strftime(
            TABLE_DATETIME_FORMAT
        )
    else:
...
def list_assets_command(client: Client, args: dict):
    ...
    if raw_response:
        for item in raw_response:
            ...
            item_dictionary = remove_empty_elements(
                {
                    "Hostname": hostname,
    ...
    readable_output = tableToMarkdown(name=f"Listed {len(outputs)} Assets:", t=outputs, headers=table_headers, removeNull=True)

    return CommandResults(
        outputs_prefix="QualysFIM.Asset",
        outputs_key_field="id",
        raw_response=raw_response,
        outputs=raw_outputs,
        readable_output=readable_output,
...
def fetch_incidents(
    ...
    if fetch_filter:
        params["filter"] += f" and {fetch_filter}"

    demisto.debug(f"Sending API request with filter='{params['filter']}'.")
    raw_response = client.incidents_list(params)
    ...
    for idx, incident in enumerate(raw_response):
        ...
        if not incident:
            demisto.debug(f"Incident {idx} has no 'data' field, skipping")
            continue
...
def main():
    ...
    try:
        client = Client(base_url=base_url, verify=verify_certificate, auth=(username, password), proxy=proxy)
