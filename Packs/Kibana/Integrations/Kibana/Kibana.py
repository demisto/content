...
def kibana_find_cases(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/cases/_find", params=query_params, headers=headers)
...
def kibana_find_alerts_for_case(args, proxies):
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.CaseAlerts",
        outputs=response)
...
def kibana_update_alert_status(args, proxies):
    ...
    http_request(method="POST", url_suffix=f"/api/detection_engine/signals/status", data=data, headers=headers)
...
def kibana_update_case_status(args, proxies):
    ...
    response = http_request(method="PATCH", url_suffix=f"/api/cases", data=data, headers=headers)

    return f"Updated status to {status} for case ID {case_id}"
...
def kibana_find_user_spaces(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/spaces/space", headers=headers)
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.UserSpaces",
        outputs=response)
...
def kibana_find_case_comments(args, proxies):
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.CaseComments",
        outputs=response)
...
def kibana_delete_case(args, proxies):
    ...
    response = http_request(method="DELETE", url_suffix=f"/api/cases", params=params, headers=headers)
...
def kibana_search_rule_details(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/alerting/rules/_find", params=params, headers=headers)
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.RuleDetails",
        outputs=response)
...
def kibana_get_user_list(args, proxies):
    ...
    try:
        ...
        result = CommandResults(
            readable_output=md,
            outputs_prefix="Kibana.UserList",
            outputs=all_users)
...
def kibana_assign_alert_user(args, proxies):
    ...
    response = http_request(method="POST", url_suffix=f"/api/detection_engine/signals/assignees", data=json_data, headers=headers)
...
def kibana_list_detection_alerts(args, proxies):
    ...
    response = http_request(method="POST", url_suffix=f"/api/detection_engine/signals/search", data=json_data, headers=headers)
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.DetectionAlerts",
...
def kibana_add_alert_note(args, proxies):
    ...
    response = http_request(method="PATCH", url_suffix=f"/api/note", data=json_data, headers=headers)
...
def kibana_get_alerting_health(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/alerting/_health", headers=headers)
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.AlertingFrameworkHealth",
        outputs=response)
...
def kibana_get_exception_lists(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/exception_lists/_find", headers=headers)
    response = response["data"]
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.ExceptionLists",
        outputs=response)
...
def kibana_create_value_list(args, proxies):
    ...
    response = http_request(method="POST", url_suffix=f"/api/lists", data=json_data, headers=headers)
...
def kibana_get_value_lists(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/lists/_find", headers=headers)
    result_json = response["data"]

    # output results to markdown table
    md = tableToMarkdown("Kibana Value Lists", result_json, headers=[])

    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.ValueLists",
        outputs=result_json)
...
def kibana_import_value_list_items(args, proxies):
    ...
    response = http_request(method="POST", url_suffix=f"/api/lists/items/_import", params=json_data, files=files, headers=headers)
...
def kibana_create_value_list_item(args, proxies):
    ...
    response = http_request(method="POST", url_suffix=f"/api/lists/items", data=json_data, headers=headers)
...
def kibana_get_value_list_items(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/lists/items/_find", params=params, headers=headers)
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.ValueListItems",
...
def kibana_delete_value_list_item(args, proxies):
    ...
    response = http_request(method="DELETE", url_suffix=f"/api/lists/items", params=json_data, headers=headers)
...
def kibana_delete_value_list(args, proxies):
    ...
    response = http_request(method="DELETE", url_suffix=f"/api/lists", params=params, headers=headers)
...
def kibana_get_status(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/status", headers=headers)
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.OperationalStatus",
        outputs=response)
...
def kibana_get_task_manager_health(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/task_manager/_health", headers=headers)
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.TaskManagerHealth",
        outputs=response)
...
def kibana_get_upgrade_readiness_status(args, proxies):
    ...
    response = http_request(method="GET", url_suffix=f"/api/upgrade_assistant/status", headers=headers)
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.UpgradeReadinessStatus",
        outputs=response)
...
def kibana_get_user_by_email(args, proxies):
    ...
    try:
        ...
        result = CommandResults(
            readable_output=md,
            outputs_prefix="Kibana.UserData",
            outputs=user_data)
...
def kibana_get_case_information(args, proxies):
    ...
    result = CommandResults(
        readable_output=md,
        outputs_prefix="Kibana.CaseInfo",
        outputs=response)
