    ...
    def get_email_activity(
        ...
        if limit < 1 or limit > MAX_EVENTS_PER_FETCH:
            raise DemistoException(
                f"Limit must be between 1 and {MAX_EVENTS_PER_FETCH}. Got: {limit}"
            )
...
def build_query_filter(from_time: str, to_time: Optional[str] = None) -> str:
    """
...
def update_last_run(
    ...
    if latest_time:
        ...
        demisto.debug(
            f"Updated last_run: last_event_time={latest_time}, "
            f"previous_ids count={len(previous_ids)}"
        )
...
def get_events_command(
    ...
    command_results = CommandResults(
        readable_output=hr,
        outputs_prefix='TwilioSendGrid.EmailActivity',
        outputs_key_field='msg_id',
...
def fetch_events_producer_consumer(
    ...
    producer = threading.Thread(
        target=_event_producer,
        args=(client, event_queue, stop_event, metrics, last_run, max_fetch)
    )
    producer.start()
    ...
    for i in range(MAX_CONSUMER_THREADS):
        consumer = threading.Thread(
            ...
            args=(event_queue, stop_event, metrics, last_run),
            name=f"Consumer-{i + 1}"
        )
        consumer.start()
...
def main() -> None:  # pragma: no cover
    ...
    try:
        ...
        if max_fetch < 1 or max_fetch > MAX_EVENTS_PER_FETCH:
            raise DemistoException(
                f"Maximum Email Activity Messages per fetch must be between 1 and {MAX_EVENTS_PER_FETCH}"
            )
