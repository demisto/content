...
URL_SUFFIX: dict[str, str] = {"TEST_MODULE": "calendar/v3/users/me/calendarList", "CALENDAR_ACL": "calendar/v3/calendars/{}/acl"}
...
def prepare_acl_list_output(
    acl_records: dict[str, Any], calendar_id: str, user_id: str
    ...
    outputs = {OUTPUT_PREFIX["LIST_ACL"]: acl_context, OUTPUT_PREFIX["LIST_ACL_PAGE_TOKEN"]: page_context}
    outputs = GSuiteClient.remove_empty_entities(outputs)
    acl_hr = acl_context
    acl_hr_list = [
        {acl_key: acl_value for acl_key, acl_value in acl.items() if acl_key not in ["kind", "etag", "calendarId", "userId"]}
        for acl in acl_hr
    ]
...
def prepare_output_acl_add(
    ...
    acl_add_hr = {"id": acl_id, "scopeType": acl_scope_type, "scopeValue": acl_scope_value, "role": acl_role}
...
def prepare_body_gsuite_acl_add(args: dict[str, str]) -> dict[str, Any]:
    ...
    return GSuiteClient.remove_empty_entities(
        {"role": args.get("role"), "scope": {"type": args.get("scope_type"), "value": args.get("scope_value")}}
    )
...
def prepare_params_for_acl_list(args: dict[str, str]) -> dict[str, str | int]:
    ...
    GSuiteClient.validate_set_boolean_arg(
        args,
        "show_deleted",
    )
...
def acl_add_command(client, args: dict[str, Any]) -> CommandResults:
    ...
    response = client.http_request(
        url_suffix=URL_SUFFIX["CALENDAR_ACL"].format(calendar_id),
        body=body,
        method="POST",
...
def acl_list_command(client, args: dict[str, Any]) -> CommandResults:
    ...
    client.set_authorized_http(scopes=SCOPES["CALENDAR"], subject=user_id)
    response = client.http_request(url_suffix=URL_SUFFIX["CALENDAR_ACL"].format(calendar_id), method="GET", params=params)
    ...
    readable_hr += tableToMarkdown(
        HR_MESSAGES["LIST_COMMAND_SUCCESS"].format("ACL", len(acl_hr_list)),
        acl_hr_list,
        headerTransform=pascalToSpace,
        removeNull=True,
    )
    return CommandResults(outputs=outputs, readable_output=readable_hr, raw_response=response)
...
def main() -> None:
    ...
    try:
        ...
        service_account_dict = GSuiteClient.safe_load_non_strict_json(
            params.get("user_service_account_json_creds", {}).get("password") or params.get("user_service_account_json")
        )
        ...
        gsuite_client = GSuiteClient(
            service_account_dict, base_url="https://www.googleapis.com/", verify=verify_certificate, proxy=proxy, headers=headers
        )
