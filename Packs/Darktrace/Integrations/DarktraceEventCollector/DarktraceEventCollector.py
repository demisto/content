...
def test_module(client: Client, first_fetch_time: int, last_run: Dict[str, Any]) -> str:
    ...
    try:
        fetch_events(
            client, max_fetch=1, last_run=last_run, start_time=first_fetch_time, end_time=convert_to_timestamp(datetime.now())
        )
...
def fetch_events(
    client: Client, max_fetch: int, last_run: Dict[str, Any], start_time: int, end_time: int
    ...
    if retrieve_events:
        ...
        last_run = {"last_fetch_time": retrieve_events[-1].get("creationTime"), "last_fetch_pid": last_fetched_pbid}
    return retrieve_events, last_run


def get_events_command(
    client: Client, args: Dict[str, Any], first_fetch_time_timestamp: int
    ...
    start_time = (
        convert_to_timestamp(arg_to_datetime(arg=args.get("start_time"), arg_name="start_time")) or first_fetch_time_timestamp
    )
    ...
    if events:
        return add_time_field(events), CommandResults(
            readable_output=tableToMarkdown("Open Incidents", events), raw_response=events
        )
...
def main() -> None:  # pragma: no cover
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            events, results = get_events_command(client=client, args=args, first_fetch_time_timestamp=first_fetch_time_timestamp)
