...
class Client(BaseClient):
    ...
    def get_external_endpoint_details(self, endpoint_type, endpoint_value, additional_info, devices, score) -> dict[str, Any]:
        ...
        params = {endpoint_type: endpoint_value, "additionalinfo": additional_info, "devices": devices, "score": score}
    ...
    def get_similar_devices(self, did, max_results) -> list[dict[str, Any]]:
        ...
        params = {"did": did, "count": max_results}
        return self.get(query_uri, params)
...
def run_advanced_search_analysis_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    check_required_fields(
        args, "offset", "initialDate", "initialTime", "endDate", "endTime", "query", "metric", "operation", "offset"
    )
    query = {
    ...
    readable_output = tableToMarkdown(
        "Results for Advanced Search Query ", formatted_output, headers=("id", "count"), headerTransform=string_to_table_header
    )
    return CommandResults(readable_output=readable_output, outputs_prefix="Darktrace.AdvancedSearch", outputs=formatted_output)
...
def get_device_connection_info_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    device_info_response = client.get_device_connection_info(
        did=did,
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="Darktrace.DeviceConnectionInfo", outputs=formatted_output
    )
...
def get_external_endpoint_details_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    endpoint_details = client.get_external_endpoint_details(
        endpoint_type, endpoint_value, str(args["additionalInfo"]), str(args["devices"]), str(args["score"])
    )
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="Darktrace.ExternalEndpointDetails", outputs=formatted_output
    )
...
def get_similar_devices_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="Darktrace.Device", outputs_key_field="deviceId", outputs=formatted_output
    )
...
def post_to_watched_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(readable_output=readable_data, outputs_prefix="Darktrace.Endpoint", outputs=intel_response)
...
def get_tagged_devices_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="Darktrace.Device", outputs_key_field="deviceId", outputs=output
    )
...
def get_tags_for_device_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="Darktrace.Device", outputs_key_field="tid", outputs=output
    )
