...
class Client(BaseClient):
    ...
    def sendRequestInSession(self, method, uri="", body={}, params={}, full_url=None):
        if not full_url:
            response = self._http_request(
                method=method,
                url_suffix=f"{self.URI_PREFIX}{uri}",
            ...
            response = self._http_request(
                method=method, full_url=full_url, headers=self._headers, json_data=body, params=params, resp_type="response"
            )
            if response.status_code == 401:
                # get a new token and re-run the request
                self.SESSION_DATA = self.getNewToken()
    ...
    def postCaseComment(self, public, oid, caseNumber, text):
        ...
        data = {"CommentBody": text, "ParentId": oid, "IsPublished": public}
    ...
    def pushComment(self, oid, text, linkUrl):
        data = {
            "body": {
                "messageSegments": [{"type": "Text", "text": text}],
            },
        ...
        if linkUrl:
            data["body"]["messageSegments"].append({"type": "Link", "url": linkUrl})
...
def commentToEntry(raw_info, title, userMapping):
    ...
    if userMapping:
        for i in range(raw_info):
            ...
            raw_info[i].OwnerId = (
                userMapping[raw_info[i].get("OwnerId")] if userMapping[raw_info[i].get("OwnerId")] else raw_info[i].get("OwnerId")
            )
...
def post_case_comment_command(client: Client, args: dict[str, Any]) -> CommandResults:
    results = client.postCaseComment(args.get("public"), args.get("oid"), args.get("caseNumber"), args.get("text"))
...
def search_command(client: Client, args: dict[str, Any]) -> CommandResults:
    search_records = client.sendRequestInSession("GET", f"search/?q=FIND+%7B{args.get('pattern')}%7D", "").get("searchRecords")
...
def get_data(client, remote_incident_id, last_update, params):
    ...
    cases = client.queryObjects(
        list(set(properties)), "Case", f"LastModifiedDate >= {last_update} AND Id='{remote_incident_id}'"
    ).get("records")
    comments = []
    if len(cases) == 1:
        case = cases[0]
...
def get_remote_data_command(client, args, params):
    ...
    try:
        ...
        if new_incident_data:
            ...
            new_incident_data = {"id": parsed_args.remote_incident_id, "in_mirror_error": str(e)}
                        ...
                        incidents.append(
                            {
                                ...
                                "details": ms.get("text"),
                                "occurred": created_date_parsed.isoformat().split("+")[0].split(".")[0] + "Z"
