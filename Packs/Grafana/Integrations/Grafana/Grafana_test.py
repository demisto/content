...
from Grafana import (
    Client,
    alert_get_by_id_command,
    alert_pause_command,
    alert_to_incident,
    alert_unpause_command,
    alerts_list_command,
    annotation_create_command,
    calculate_fetch_start_time,
    change_key,
    dashboards_search_command,
    decapitalize,
    filter_alerts_by_id,
    filter_alerts_by_time,
    keys_to_lowercase,
    org_create_command,
    org_get_by_id_command,
    org_get_by_name_command,
    org_list_command,
    paging_heading,
    parse_alerts,
    reduce_incidents_to_limit,
    set_state,
    set_time_to_epoch_millisecond,
    team_add_command,
    team_delete_command,
    team_get_by_id_command,
    team_members_command,
    teams_search_command,
    user_add_to_team_command,
    user_get_by_id_command,
    user_remove_from_team_command,
    user_update_command,
    users_organization_command,
...
after_one_alert_was_fetched_high_limit = (
    parse_alerts_values_data["after_one_alert_was_fetched_high_limit"],
    10,
    dateparser.parse("2021-06-09 15:20:01 UTC").replace(tzinfo=utc, microsecond=0),
    1,
...
after_one_alert_was_fetched_low_limit = (
    parse_alerts_values_data["after_one_alert_was_fetched_low_limit"],
    1,
    dateparser.parse("2021-06-09 15:20:01 UTC").replace(tzinfo=utc, microsecond=0),
    1,
...
first_fetch_high_limit = (
    parse_alerts_values_data["first_fetch_high_limit"],
    100,
...
first_fetch_low_limit = (
    parse_alerts_values_data["first_fetch_low_limit"],
    2,
    dateparser.parse("2020-06-08 10:00:00 UTC").replace(tzinfo=utc, microsecond=0),
    -1,
    dateparser.parse("2021-07-08 12:08:40 UTC").replace(tzinfo=utc, microsecond=0),
...
limit_to_one_first_fetch = (
    parse_alerts_values_data["limit_to_one_first_fetch"],
    1,
    dateparser.parse("2020-06-08 10:00:00 UTC").replace(tzinfo=utc, microsecond=0),
    -1,
    dateparser.parse("2021-06-09 15:20:01 UTC").replace(tzinfo=utc, microsecond=0),
...
limit_to_one_second_fetch = (
    parse_alerts_values_data["limit_to_one_second_fetch"],
    1,
    dateparser.parse("2021-06-09 15:20:01 UTC").replace(tzinfo=utc, microsecond=0),
    1,
    dateparser.parse("2021-07-08 12:08:40 UTC").replace(tzinfo=utc, microsecond=0),
...
same_time_at_fetch_bigger_id = (
    parse_alerts_values_data["same_time_at_fetch_bigger_id"],
    100,
    dateparser.parse("2021-07-08 12:08:40 UTC").replace(tzinfo=utc, microsecond=0),
    2,
    dateparser.parse("2021-07-08 12:08:40 UTC").replace(tzinfo=utc, microsecond=0),
...
def test_alert_to_incident():
    ...
    expected_incident = {"name": "Adi's Alert", "occurred": "2021-07-29T14:03:20Z", "type": "Grafana Alert"}
...
first_fetch_high_limit = (
    alerts_to_filter_by_time_data["keep_all_alerts"][0],
    dateparser.parse("2020-06-08 10:00:00 UTC").replace(tzinfo=utc, microsecond=0),
    alerts_to_filter_by_time_data["keep_all_alerts"][1],
)
keep_two_alerts = (
    alerts_to_filter_by_time_data["keep_two_alerts"][0],
...
keep_one_alert_same_time = (
    alerts_to_filter_by_time_data["keep_one_alert_same_time"][0],
    dateparser.parse("2021-07-29 14:03:20 UTC").replace(tzinfo=utc, microsecond=0),
...
keep_all_alerts_first_fetch = (
    alerts_to_filter_by_date_data["keep_all_alerts_first_fetch"][0],
    dateparser.parse("2020-06-08 10:00:00 UTC").replace(tzinfo=utc, microsecond=0),
    0,
    alerts_to_filter_by_date_data["keep_all_alerts_first_fetch"][1],
)
keep_all_alerts_not_first = (
    alerts_to_filter_by_date_data["keep_all_alerts_not_first"][0],
...
keep_one_alert_greater_id_same_time = (
    alerts_to_filter_by_date_data["keep_one_alert_greater_id_same_time"][0],
    dateparser.parse("2021-07-08 12:08:40 UTC").replace(tzinfo=utc, microsecond=0),
    2,
...
keep_all_incidents = (
    incidents_to_reduce_data["keep_all_incidents"][0],
    100,
    dateparser.parse("2020-06-08 10:00:00 UTC").replace(tzinfo=utc, microsecond=0),
    0,
    dateparser.parse("2021-07-29 14:03:20 UTC").replace(tzinfo=utc, microsecond=0),
...
limit_to_two = (
    incidents_to_reduce_data["limit_to_two"][0],
    2,
    dateparser.parse("2020-06-08 10:00:00 UTC").replace(tzinfo=utc, microsecond=0),
    5,
    dateparser.parse("2021-07-08 12:08:40 UTC").replace(tzinfo=utc, microsecond=0),
...
@pytest.mark.parametrize(
    "alerts, limit, last_fetch, last_id_fetched, expected_last_fetch, expected_last_id, expected_incidents", INCIDENTS_TO_REDUCE
)
def test_reduce_incidents_to_limit(
    alerts, limit, last_fetch, last_id_fetched, expected_last_fetch, expected_last_id, expected_incidents
):
    """
...
def test_user_update_command(mocker, grafana_client):
    ...
    http_request.assert_called_with(
        "PUT", "api/users/2", json_data={"email": "e@mail", "login": "login", "name": "Name", "theme": "dark"}, headers=None
    )
...
def test_annotation_create_command(mocker, grafana_client):
    ...
    args = {
        "dashboard_id": "3",
        "panel_id": "2",
        "time": "2019-10-21T23:45:00",
        "time_end": "2019-10-21T23:45:01",
        "tags": "tag1",
        "text": "Text",
    }
...
def test_teams_search_command(mocker, grafana_client):
    ...
    http_request.assert_called_with(
        "GET", "api/teams/search", params={"perpage": "60", "page": "2", "query": "team", "name": "team_name"}, headers=None
    )
...
def test_team_add_command(mocker, grafana_client):
    ...
    http_request.assert_called_with(
        "POST", "api/teams", json_data={"email": "new@email", "name": "New Team", "orgId": "2"}, headers=None
    )
