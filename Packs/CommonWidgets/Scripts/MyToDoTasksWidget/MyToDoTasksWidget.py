...
def get_open_to_do_tasks_of_current_user() -> List[Dict]:
    ...
    todo_tasks_query_res = demisto.internalHttpRequest("POST", "/v2/statistics/widgets/query", json.dumps(body))
...
def main():  # pragma: no cover
    try:
        ...
        cmd_results = {"Type": entryTypes["note"], "ContentsFormat": formats["markdown"], "Contents": readable_output}
        return_results(cmd_results)

    except Exception as e:
        return_error(f"Failed to execute MyToDoTasksWidget Script. Error: {e!s}", e)
