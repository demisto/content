...
def get_feed_integration_errors() -> TableOrListWidget:
    ...
    if integration_search_res.get("statusCode") == 200:
        ...
        for instance in instances_health.values():
            if (
                "feed" in (brand := instance.get("brand", "")).lower()
                and (error := instance.get("lastError", ""))
                and (instance_name := instance.get("instance")) in enabled_instances
            ):
...
def main():
    try:
        ...
        return_error(f"Failed to execute FeedIntegrationErrorWidget Script. Error: {e!s}", e)
