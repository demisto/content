from Rundeck import filter_results, attribute_pairs_to_dict, convert_str_to_int, \
    project_list_command, Client, jobs_list_command, execute_job_command, job_retry_command, \
    job_execution_query_command, job_execution_output_command, job_execution_abort_command,  \
    adhoc_run_command, adhoc_script_run_command, adhoc_script_run_from_url_command, webhooks_list_command,  \
...
def test_jobs_list_command(mocker):
    ...
    Then
        - CommonResults object returns with the api response.
    """
    return_value = [{'href': '123', 'id': '123', 'scheduleEnabled': True, 'scheduled': False,
                     'enabled': True, 'permalink': '123', 'group': None, 'description': 'just a sample job',
                     'project': 'Demisto', 'name': "Arseny's Job"}]
    client = Client(
            base_url='base_url',
            verify=False,
            params={'authtoken': '123'},
    ...
    result = jobs_list_command(client, {})
    assert result.outputs == [{'id': '123', 'scheduleEnabled': True, 'scheduled': False, 'enabled': True, 'group': None,
                               'description': 'just a sample job', 'project': 'Demisto', 'name': "Arseny's Job"}]
    assert result.outputs_key_field == 'id'
    assert result.outputs_prefix == 'Rundeck.Jobs'
    assert result.readable_output == "### Jobs List:\n|Id|Schedule Enabled|Scheduled|Enabled|Group|Description" \
                                     "|Project|Name|\n|---|---|---|---|---|---|---|---|\n| 123 | true | false |" \
...
def test_execute_job_command(mocker):
    ...
    Then
        - CommonResults object returns with the api response.
    """
    return_value = {'id': 194, 'status': 'running', 'project': 'Demisto', 'executionType': 'user',
                    'user': 'Galb', 'datestarted': {'unixtime': 123, 'date': '123'},
                    'job': {'id': '123', 'averageDuration': 463, 'name': "Arseny's Job", 'group': '',
                            'project': 'Demisto', 'description': 'just a sample job', 'options':
                                {'foo': '0'}}, 'description': "123",
                    'argstring': '-foo 0'}
...
def test_job_execution_output_command(mocker):
    ...
    Then
        - CommonResults object returns with the api response.
    """
    return_value = {'id': '69', 'offset': '3732', 'completed': True, 'execCompleted': True, 'hasFailedNodes': True,
                    'execState': 'failed', 'lastModified': '123', 'execDuration': 237, 'percentLoaded': 12,
                    'totalSize': 3738, 'retryBackoff': 0, 'clusterExec': False, 'compacted': False, 'entries':
                        [{'node': 'localhost', 'step': '1', 'stepctx': '1', 'user': 'admin', 'time': '10:54:52',
                          'level': 'NORMAL', 'type': 'stepbegin', 'absolute_time': '123', 'log': ''}]}
    client = Client(
        base_url='base_url',
        verify=False,
        params={'authtoken': '123'},
...
def test_job_execution_abort_command(mocker):
    ...
    Then
        - CommonResults object returns with the api response.
    """
    return_value = {'abort': {'status': 'failed', 'reason': 'Job is not running'},
                    'execution': {'id': '69', 'status': 'failed', 'href': '123',
                                  'permalink': '123'}}
    output = {'abort': {'status': 'failed', 'reason': 'Job is not running'}, 'execution': {'id': '69',
                                                                                           'status': 'failed'}}
    client = Client(
        base_url='base_url',
        verify=False,
    ...
    assert result.readable_output == '### Job Execution Abort:\n|Abort|Execution|\n|---|---|\n|' \
...
def test_adhoc_command_run_command(mocker):
    ...
    Then:
        - CommonResults object returns with the api response.
    """
    return_value = {'message': 'Immediate execution scheduled (196)', 'execution':
                    {'id': 196, 'href': '123', 'permalink': '123'}}
    output = {'message': 'Immediate execution scheduled (196)', 'execution': {'id': 196}}
    client = Client(
        base_url='base_url',
        verify=False,
        params={'authtoken': '123'},
    ...
    assert result.readable_output == '### Adhoc Run:\n|Message|Execution|\n|---|---|\n| ' \
...
def test_adhoc_script_run_command(mocker):
    ...
    assert result.readable_output == '### Adhoc Run Script:\n|Message|Execution|\n|---|---|\n|' \
                                     ' Immediate execution scheduled (196) | id: 196 |\n'
...
def test_adhoc_script_run_from_url_command(mocker):
    ...
    assert result.readable_output == '### Adhoc Run Script From Url:\n|Message|Execution|\n|---|---|\n|' \
                                     ' Immediate execution scheduled (196) | id: 196 |\n'
...
def test_webhooks_list_command(mocker):
    ...
    Then:
        - CommonResults object returns with the api response.
    """
    return_value = [{'id': 1, 'uuid': '123', 'name': "Arseny's hook", 'project': 'Demisto',
                     'enabled': True, 'user': 'admin', 'creator': 'admin', 'roles': '123',
                     'authToken': '123', 'eventPlugin': 'webhook-run-job', 'config':
                         {'jobId': '123', 'argString': '123'}}]
    client = Client(
        base_url='base_url',
        verify=False,
        params={'authtoken': '123'},
