...
def test_domain(requests_mock):
    ...
    requests_mock.get(f"https://test.com/api/full/{domain_to_check}", json=mock_response)

    client = Client(base_url="https://test.com/api", verify=False, headers={"Authorization": "Bearer APIKEY"})

    args = {"domain": domain_to_check}
...
def test_search(requests_mock):
    ...
    requests_mock.get(f"https://test.com/api/domains/{field_to_check}/{value_to_check}", json=mock_response)

    client = Client(base_url="https://test.com/api", verify=False, headers={"Authorization": "Bearer APIKEY"})

    args = {"field": field_to_check, "value": value_to_check, "limit": 25}
...
ARGS_CASES = [
    ...
    (
        ...
        "get_search_data",
        {"domains": [{"domain": "test_domain1"}, {"domain": "test_domain2"}], "total": 2},
    ),
]
...
def test_main(mocker, function, client_function_to_mock, result_client):
    ...
    from HostIo import Client, main
