...
class Client(BaseClient):
    def get_domain_data(self, domain: str) -> Dict[str, Any]:
        return self._http_request(method="GET", url_suffix=f"full/{domain}", params={})

    def get_search_data(self, field: str, value: str, limit: int, page: int) -> Dict[str, Any]:
        return self._http_request(method="GET", url_suffix=f"domains/{field}/{value}", params={"limit": limit, "page": page})

    def test_module(self):
        return self._http_request(method="GET", url_suffix="domains/ip/8.8.8.8", params={})
...
def search_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    context = {"Field": field, "Value": value, "Domains": domains, "Total": total}
