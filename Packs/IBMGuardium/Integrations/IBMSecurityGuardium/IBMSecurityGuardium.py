...
def extract_field_mapping(response: dict[str, Any]) -> dict[str, str]:
    ...
    Returns:
        Dictionary mapping sequence number to header_name (e.g., {"1": "ClientIP", "2": "DBUserName"})
    """
    headers = response["result"]["report_layout"]["report_headers"]
    field_mapping = {str(header["sequence"]): header["header_name"] for header in headers}
...
def map_event(raw_event: dict[str, Any], field_mapping: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    Args:
        ...
        field_mapping: Optional mapping from field IDs to field names (defaults to DEFAULT_FIELD_MAPPING)
    
    Returns:
        Event with readable field names
    """
    mapping = field_mapping or DEFAULT_FIELD_MAPPING
    return {mapping.get(key, key): value for key, value in raw_event.items()}
...
def deduplicate_events(events: list[dict[str, Any]], last_run: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    demisto.debug(
        f"Deduplicating {len(events)} events. Last fetch time: {last_fetch_time}, "
...
def fetch_events(
    ...
    Returns:
        Tuple of (deduplicated events list, next run context)
    """
    ...
    if deduplicated_events:
        ...
        if "SessionStart" in last_event:
            next_run = {"last_fetch_time": last_event["SessionStart"], "fetched_event_hashes": list(new_ignore_set)}
            demisto.debug(
                f"Setting last_fetch_time to last event's SessionStart: {last_event['SessionStart']}, "
    ...
    try:
        ...
        if command == "test-module":
            ...
            if argToBoolean(args.get("should_push_events", False)):
                ...
                demisto.debug("should_push_events is False, skipping XSIAM push")
