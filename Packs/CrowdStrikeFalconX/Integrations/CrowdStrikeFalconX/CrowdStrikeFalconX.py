...
class Client:
    ...
    def _get_token_request(self):
        body = {"client_id": self._username, "client_secret": self._password}

        byte_creds = f"{self._username}:{self._password}".encode()

        headers = {"Authorization": f"Basic {base64.b64encode(byte_creds).decode()}"}
    ...
    def upload_file(self, file: str, file_name: Optional[str] = None, is_confidential: str = "true", comment: str = "") -> dict:
    ...
    def get_full_report(self, id: str) -> dict:
        """Creating the needed arguments for the http request
    ...
    def get_report_summary(self, id: str) -> dict:
        """Creating the needed arguments for the http request
        :param id: id of a submitted malware samples.
        :return: http response
        """
    ...
    def get_analysis_status(self, ids: list) -> dict:
        """Creating the needed arguments for the http request
    ...
    def download_ioc(self, id: str, name: str, accept_encoding: str) -> dict:
        """Creating the needed arguments for the http request
    ...
    for report_id, result in report_to_results.items():
        if result.output:
            if not (sha256 := result.output.get("sha256")):
                demisto.debug(
                    f"unexpected result for {report_id=}: "
            ...
            readable_output = (
                f"There are no results yet for {report_id=}, "
...
def find_suitable_hash_output(raw_results: tuple[RawCommandResults]) -> Dict[str, dict]:
    ...
    for result in filter(None, raw_results):  # filters out None results
        ...
        threat_score = max(temp_max.get("threat_score", -1), output.get("threat_score", -1))
        if threat_score != -1:
...
def parse_indicator(sandbox: dict, reliability_str: str) -> Optional[Common.File]:  # type: ignore[return]
    if sha256 := sandbox.get("sha256"):
        ...
        dbot = Common.DBotScore(indicator=sha256, indicator_type=DBotScoreType.FILE, score=score_field, reliability=reliability)
...
def upload_file_command(  # type: ignore[return]
    ...
    if submit_file == "no":
        ...
        return send_uploaded_file_to_sandbox_analysis_command(
            client,
            sha256,
            environment_id,
            action_script,
...
def send_uploaded_file_to_sandbox_analysis_command(
    ...
    result = parse_outputs(
        response, reliability=client.reliability, sandbox_fields=sandbox_fields, resources_fields=resource_fields
    )
...
def send_url_to_sandbox_analysis_command(
    ...
    sandbox_fields = ("environment_id", "sha256")
    result = parse_outputs(response, client.reliability, resources_fields=resources_fields, sandbox_fields=sandbox_fields)
    if result.output:  # the "if" is here to calm mypy down
        # in order identify the id source, upload or submit command, the id name changed
        result.output["submitted_id"] = result.output.pop("id")
        ...
        if result.output:
            if human_readable_values := filter_dictionary(result.output, hr_fields, sort_by_field_list=True):
                readable_output = tableToMarkdown(
                    "CrowdStrike Falcon Intelligence Sandbox response:", t=human_readable_values, headers=hr_fields
                )
            else:
                readable_output = tableToMarkdown("CrowdStrike Falcon Intelligence Sandbox response:", result.output)
        else:
    ...
    for resource in report_resources:
        ...
        for sandbox_entity in resource.get("sandbox", []):
            if error_message := sandbox_entity.get("error_message"):
                ...
                raise FileReportWarning(
                    f"Sandbox report for resource id {resource_id} returned an error of"
...
def get_report_summary_command(
    ...
    no_outputs_msg = (
        "There are no results yet, the sample might still be going through analysis."
        " Please wait to download the report.\n"
        "You can use cs-fx-get-analysis-status to check the status of a sandbox analysis."
    )
    results = []
...
def download_ioc_command(client: Client, id: str, name: str = "", accept_encoding: str = "") -> Union[CommandResults, dict]:
...
def check_quota_status_command(client: Client) -> List[CommandResults]:
...
def find_sandbox_reports_command(
    client: Client, limit: int = 50, filter: str = "", offset: str = "", sort: str = "", hashes: str = ""
) -> CommandResults:
    ...
    if hashes and not filter:
        ...
        readable_output = (
            tableToMarkdown("CrowdStrike Falcon Intelligence Sandbox response:", {"resource": all_report_ids})
            if all_report_ids
            else f"No reports found for hashes {hashes}."
        )
        return CommandResults(
            outputs_key_field="id",
            outputs_prefix=OUTPUTS_PREFIX,
        ...
        if filter:
            demisto.info(
                "Both the hashes and filter arguments were provided to the find-reports command. "
...
def run_polling_command(
    ...
    if is_new_polling_search(args):
        ...
        scheduled_command = ScheduledCommand(
            command=cmd, next_run_in_seconds=interval_in_secs, args=results_function_args, timeout_in_seconds=6000
        )
    ...
    if not status:
        ...
        scheduled_command = ScheduledCommand(
            command=cmd, next_run_in_seconds=interval_in_secs, args=polling_args, timeout_in_seconds=6000
        )
