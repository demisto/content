...
from CrowdStrikeFalconX import (
    ...
    find_submission_id_command,
    get_analysis_status_command,
...
GET_FULL_REPORT_ARGS_EXTENDED = {"ids": "ids", "extended_data": "true"}
...
def test_get_report_commands_error(mocker, command):
    ...
    assert (
        "Sandbox report for resource id resource_id returned an error of type FILE_TYPE_BAD_ERROR with content"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (get_full_report_command, GET_FULL_REPORT_ARGS_EXTENDED, GET_FULL_REPORT_HTTP_RESPONSE, GET_FULL_REPORT_CONTEXT_EXTENDED),
    ],
)
...
@pytest.mark.parametrize(
    "http_response, output", [(MULTI_ERRORS_HTTP_RESPONSE, MULTIPLE_ERRORS_RESULT), (NO_ERRORS_HTTP_RESPONSE, "")]
)
...
def test_running_polling_command_success_for_url(mocker):
    ...
    command_results = run_polling_command(
        client, args, "cs-fx-submit-url", send_url_to_sandbox_analysis_command, get_full_report_command, "URL"
    )
...
def test_running_polling_command_new_search_for_file(mocker):
    ...
    mocker.patch.object(
        Client, "send_uploaded_file_to_sandbox_analysis", return_value=SEND_UPLOADED_FILE_TO_SANDBOX_ANALYSIS_HTTP_RESPONSE
    )
    mocker.patch.object(Client, "get_full_report", return_value=GET_FULL_REPORT_HTTP_RESPONSE)
...
def test_remove_polling_related_args():
    args = {"interval_in_seconds": "interval_in_seconds", "polling": "polling", "ids": "ids", "extended_data": "extended_data"}
...
def test_parse_indicator():
    sandbox = {
        ...
        "contacted_hosts": [{"address": "example1.com"}, {"address": "example2.com"}],
    }
    ...
    expected_context = {
        ...
        "DBotScore(val.Indicator && val.Indicator == obj.Indicator && val.Vendor == obj.Vendor && val.Type == obj.Type)": {
            ...
            "Type": "file",
            "Vendor": "",
            "Score": 2,  # Vendor is auto-populated on XSOAR
...
@pytest.mark.parametrize(
    "file,mocked_address,mocked_response",
    (
        (
            "file",
...
@pytest.mark.parametrize(
    "mocked_address,ioc_id,mocked_response,command_results_output",
    (
        (
            "https://api.crowdstrike.com/falconx/entities/artifacts/v1",
            "123",
            {"headers": {"Content-Type": "image/png", "Content-Disposition": "attachment; filename=screen_0.png"}},
            "screen_0.png",
        ),
        (
            "https://api.crowdstrike.com/falconx/entities/artifacts/v1",
