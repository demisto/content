    ...
    if int(entityTypeId) == 1:
                                    ...
                                    if key1 == 'analyticalFeatureValues':
                                        ...
                                        if analyticalFeatures is not None:
                                            for feature in analyticalFeatures:
                                                displayData.append(
                                                    {'Anomaly Name': anomalyName, 'Analytical Feature': feature,
        ...
        if len(displayData) > 0:
            data = {
                ...
                'HumanReadable': tableToMarkdown(None, displayData,
...
def main():
    try:
        displayAnalyticalFeature()
    except Exception as ex:
        return_error(f'Failed to execute gra-analytical-features-entity-value. Error: {str(ex)}')
