        ...
        if res is not None:
            ...
            if anomaliesChangedCount != 0:
                ...
                if anomaliesChangedCount == 1:
                    return_results(
                        "There is 1 anomaly update identified for this case. "
                        "Refresh Analytical Features for updated attributes list."
                    )
                else:
                    return_results(
                        f"There are {anomaliesChangedCount} anomaly updates identified for this "
...
def main():
    try:
        ...
        return_error(f"Failed to execute gra-cases-anomaly. Error: {ex!s}")
