...
from CommonServerPython import DemistoException, timedelta,  set_integration_context
from MicrosoftCloudAppSecurity import Client, fetch_incidents, module_test, reset_auth
...
Objective:
The module_test function is used to test the connection to the Microsoft Cloud App Security API and verify that the integration 
is properly configured. It checks the authentication mode, lists alerts, and optionally lists incidents and 
...
class TestModuleTest:
    ...
    def test_module_test_device_code_flow(self, mocker):
        """
        Given:
...
Objective:
The objective of the "reset_auth" function is to reset the authorization context of the integration by calling the "set_integration_context" function with an empty dictionary as the context parameter. This function is decorated with the "logger" decorator, which logs the function call and its return value.

Inputs:
- client: an instance of the "Client" class, which is used to interact with the Microsoft Cloud App Security API.
- args: a dictionary containing the command arguments passed to the function.

Flow:
1. The function calls the "set_integration_context" function with an empty dictionary as the context parameter to reset the authorization context.
2. The function returns a CommandResults object with a readable output message indicating that the authorization was reset successfully.

Outputs:
- CommandResults object with a readable output message indicating that the authorization was reset successfully.

Additional aspects:
- The function is decorated with the "logger" decorator, which logs the function call and its return value.
- The "set_integration_context" function is called to reset the authorization context.
"""

class TestResetAuth:
    #  Tests that the function successfully resets authorization and returns a readable output.
    def test_reset_auth_happy_path(self, mocker):
        """
        Given:
        - A client object.
        ...
        When:
        - Calling the reset_auth function.

        Then:
        - Ensure the function successfully resets authorization and returns a readable output.
        """
        client = Client('app_id', True, False, 'base_url', 'auth_mode')
        args = {'key': 'value'}
        mocker.patch('CommonServerPython.set_integration_context')
        res = reset_auth(client, args)
        assert res.readable_output == 'Authorization was reset successfully. You can now run **!microsoft-cas-auth-start** and **!microsoft-cas-auth-complete**.'
        set_integration_context.assert_called_once_with({})

    #  Tests that the function sets the integration context to an empty dictionary.
    def test_reset_auth_integration_context(self, mocker):
        """
        Given:
        - A client object.
        - A dictionary of arguments.
        ...
        Then:
        - Ensure the function sets the integration context to an empty dictionary.
        """
        client = Client('app_id', True, False, 'base_url', 'auth_mode')
        args = {'key': 'value'}
        mocker.patch('MicrosoftApiModule.set_integration_context')
        reset_auth(client, args)
        MicrosoftApiModule.set_integration_context.assert_called_once_with({})

    #  Tests that the function successfully resets authorization when the args parameter is None.
    def test_reset_auth_with_args_none(self, mocker):
        """
        Given:
        - A client object.
        - A None arguments dictionary.

        When:
        - Calling the reset_auth function.

        Then:
        - Ensure the function successfully resets authorization when the args parameter is None.
        """
        client = Client('app_id', True, False, 'base_url', 'auth_mode')
        mocker.patch('MicrosoftApiModule.set_integration_context')
        res = reset_auth(client, None)
        assert res.readable_output == 'Authorization was reset successfully. You can now run **!microsoft-cas-auth-start** and **!microsoft-cas-auth-complete**.'
        MicrosoftApiModule.set_integration_context.assert_called_once_with({})

    #  Tests that the function successfully resets authorization when the args parameter is an empty dictionary.
    def test_reset_auth_with_empty_args(self, mocker):
        """
        Given:
        - A client object.
        - An empty dictionary of arguments.

        When:
        - Calling the reset_auth function.

        Then:
        - Ensure the function successfully resets authorization when the args parameter is an empty dictionary.
        """
        client = Client('app_id', True, False, 'base_url', 'auth_mode')
        args = {}
        mocker.patch('MicrosoftApiModule.set_integration_context')
