...
expected_filtered_alerts = {"filters": {"severity": {"eq": 0}, "resolutionStatus": {"eq": 0}}, "skip": 5, "limit": 10}
response_alerts_data = {
    "service": "111",
    "instance": "111",
    "severity": "Low",
...
expected_filtered_activities = {
    "filters": {
        "ip.address": {"eq": "8.8.8.8"},
        "ip.category": {"eq": 1},
        "activity.takenAction": {"eq": "block"},
        "source": {"eq": 0},
    },
    "skip": 5,
    "limit": 10,
}
response_activities_data = {
    "ip": "8.8.8.8",
    "ip_category": "Corporate",
    "taken_action": "block",
...
expected_filtered_files = {
    "filters": {"fileType": {"eq": 0}, "quarantined": {"eq": True}, "sharing": {"eq": 0}, "extension": {"eq": "png"}},
    "skip": 5,
    "limit": 10,
}
response_files_data = {
...
expected_filtered_users_accounts = {
    "filters": {
        "type": {"eq": "user"},
        "isExternal": {"eq": True},
        "status": {"eq": 0},
...
client_mocker = Client(
    base_url="https://demistodev.eu2.portal.cloudappsecurity.com/api/v1",
    app_id="1234",
    verify=True,
    proxy=True,
...
def test_list_alerts_command(requests_mock):
    ...
    requests_mock.get(
        "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/alerts/5f06d71dba4289d0602ba5ac",
...
def test_list_alerts_command_no_alerts(requests_mock):
    ...
    requests_mock.get(
        "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/alerts/5f06d71dba4289d0602ba5ac", json={"data": []}
    )
...
def test_list_activities_command(requests_mock):
    ...
    requests_mock.get(
        "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/activities/"
...
def test_list_files_command(requests_mock):
    ...
    requests_mock.get(
        "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/files/5f077e6fc3b664209dae1f6b", json=files["FILES_BY_ID_DATA"]
    )
...
class TestCloseBenign:
    ...
    def test_sanity_ids(self, requests_mock):
        ...
        requests_mock.post(
            "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/alerts/close_benign/", json=self.success_response
        )
    ...
    def test_failure_not_found(self, requests_mock):
        ...
        requests_mock.post(
            "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/alerts/close_benign/", json=self.failure_response
        )
...
class TestCloseTruePositive:
    ...
    def test_sanity_ids(self, requests_mock):
        ...
        requests_mock.post(
            "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/alerts/close_true_positive/", json=self.success_response
        )
    ...
    def test_failure_not_found(self, requests_mock):
        ...
        requests_mock.post(
            "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/alerts/close_true_positive/", json=self.failure_response
        )
...
class TestCloseFalsePositive:
    ...
    def test_sanity_ids(self, requests_mock):
        ...
        requests_mock.post(
            "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/alerts/close_false_positive/", json=self.success_response
        )
    ...
    def test_failure_not_found(self, requests_mock):
        ...
        requests_mock.post(
            "https://demistodev.eu2.portal.cloudappsecurity.com/api/v1/alerts/close_false_positive/", json=self.failure_response
        )
...
class TestFetchIncidents:
            ...
            (
                {"limit": 50, "first_fetch": "50 minutes", "look_back": 15},
                {
                    "data": [
                        ...
                        {
                            "title": "test5",
                            "timestamp": create_occur_timestamp(FREEZE_TIMESTAMP, timedelta(minutes=2)),
                            "_id": "5",
                            "entities": [],
                        },
                    ]
    ...
    def test_fetch_incidents_with_look_back_greater_than_zero(
        ...
        mocker.patch("MicrosoftCloudAppSecurity.format_fetch_start_time_to_timestamp", side_effect=create_occur_timestamp)
            ...
            (
                {"limit": 50, "first_fetch": "50 minutes", "look_back": 0},
                {
                    "data": [
                        ...
                        {
                            "title": "test3",
                            "timestamp": create_occur_timestamp(FREEZE_TIMESTAMP, timedelta(minutes=7)),
                            "_id": "3",
                            "entities": [],
                        },
                    ]
                ...
                },
                {
                    "data": [
                        {
                            "title": "test5",
                            "timestamp": create_occur_timestamp(FREEZE_TIMESTAMP, timedelta(minutes=4)),
                            "_id": "5",
                            "entities": [],
                        }
                    ]
            ...
            (
                {"limit": 50, "first_fetch": "3 days", "look_back": 0},
                {
                    "data": [
                        {
                            "title": "test1",
                            "timestamp": create_occur_timestamp(FREEZE_TIMESTAMP, timedelta(hours=8, minutes=51)),
                            "_id": "1",
                            "entities": [],
                        },
                        {
                            "title": "test2",
                            "timestamp": create_occur_timestamp(FREEZE_TIMESTAMP, timedelta(hours=7, minutes=45)),
                            "_id": "2",
                            "entities": [],
                        },
                        {
                            "title": "test3",
                            "timestamp": create_occur_timestamp(FREEZE_TIMESTAMP, timedelta(hours=7, minutes=44)),
                            "_id": "3",
                            "entities": [],
                        },
                    ]
                ...
                },
                {
                    "data": [
                        {
                            "title": "test5",
                            "timestamp": create_occur_timestamp(FREEZE_TIMESTAMP, timedelta(hours=1, minutes=34)),
                            "_id": "5",
                            "entities": [],
                        }
                    ]
    ...
    def test_fetch_incidents_with_look_back_equals_zero(self, mocker, params, incidents, phase2_incident, phase3_incident):
        """
        Given
...
class TestModuleTest:
    ...
    def test_test_module_device_code_flow(self):
        ...
        assert (
            test_module(client, False, None) == "To test the device code flow Please run !microsoft-cas-auth-start and "
