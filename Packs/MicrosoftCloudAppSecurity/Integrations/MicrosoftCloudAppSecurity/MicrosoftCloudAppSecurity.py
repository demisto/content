...
class Client:
    ...
    def __init__(
        ...
        if self.auth_mode == "legacy":
            self.ms_client = LegacyClient(base_url=base_url, verify=verify, headers=headers, proxy=proxy)
            ...
            if self.auth_mode == "device code flow":
                ...
                token_retrieval_url = (
                    f"{MICROSOFT_DEFENDER_FOR_APPLICATION_TOKEN_RETRIEVAL_ENDPOINTS[endpoint_type]}"
...
def build_filter_and_url_to_search_with(
    url_suffix: str, custom_filter: Optional[Any], arguments: dict, specific_id_to_search: Any = "", is_scan: bool = False
):
    """
...
def args_to_filter_close_alerts(
        ...
        if is_fetch:
            ...
            if custom_filter:
                try:
                    ...
                    raise DemistoException(
                        "Custom Filter Error: Your custom filter format is incorrect, please try again."
                    ) from e
...
def alerts_to_human_readable(alerts: List[dict]):
    ...
    for alert in alerts:
        readable_output = assign_params(
            ...
            is_open=alert.get("is_open"),
            status_value=[key for key, value in STATUS_OPTIONS.items() if alert.get("statusValue") == value],
            severity_value=[key for key, value in SEVERITY_OPTIONS.items() if alert.get("severityValue") == value],
            alert_date=datetime.fromtimestamp(alert.get("timestamp", 0) / 1000.0).isoformat(),
        )
...
def activity_to_human_readable(activity: dict):
    readable_output = assign_params(
        ...
        activity_date=datetime.fromtimestamp(activity.get("timestamp", 0) / 1000.0).isoformat(),
        app_name=activity.get("appName"),
        description=activity.get("description"),
    )
    headers = ["activity_id", "activity_date", "app_name", "description", "severity"]
    return tableToMarkdown("Microsoft CAS Activity", readable_output, headers, removeNull=True)
...
def list_activities_command(client: Client, args: dict):
    ...
    while has_next:
        ...
        list_activities.extend(activities_response_data.get("data") or [activities_response_data])
...
def files_to_human_readable(files: List[dict]):
    ...
    for file in files:
        readable_output = assign_params(
            owner_name=file.get("ownerName"),
            file_id=file.get("_id"),
            file_type=file.get("fileType"),
            file_name=file.get("name"),
            file_access_level=file.get("fileAccessLevel"),
            app_name=file.get("appName"),
            file_status=file.get("fileStatus"),
        )
        files_readable_outputs.append(readable_output)

    headers = ["owner_name", "file_id", "file_type", "file_name", "file_access_level", "file_status", "app_name"]
    return tableToMarkdown("Microsoft CAS Files", files_readable_outputs, headers, removeNull=True)
...
def list_files_command(client: Client, args: dict):
    ...
    return CommandResults(
        readable_output=human_readable, outputs_prefix="MicrosoftCloudAppSecurity.Files", outputs_key_field="_id", outputs=files
    )
...
def users_accounts_to_human_readable(users_accounts: List[dict]):
    ...
    for entity in users_accounts:
        readable_output = assign_params(
            ...
            is_external=entity.get("isExternal"),
...
def fetch_incidents(
    client: Client, max_results: Optional[str], last_run: dict, first_fetch: Optional[str], filters: dict, look_back: int
):
    if not first_fetch:
        first_fetch = "3 days"
    ...
    if not last_run.get("time") and last_run.get("last_fetch"):
        demisto.debug(f"last fetch from old version is: {last_run.get('last_fetch')!s}")
...
def params_to_filter(severity: List[str], resolution_status: List[str]):
    ...
    if len(severity) == 1:
        ...
        severities = [SEVERITY_OPTIONS[severity_option] for severity_option in severity]
    ...
    if len(resolution_status) == 1:
        ...
        resolution_statuses = [RESOLUTION_STATUS_OPTIONS[resolution] for resolution in resolution_status]
...
def main():  # pragma: no cover
    ...
    # Log exceptions
        ...
        return_error(f"Failed to execute {command} command. Error: {exc!s}", error=exc)
