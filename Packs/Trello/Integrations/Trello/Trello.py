...
class Client(BaseClient):
    ...
    def list_actions(self, board_id, actions_filter, since=None, before=None):
        ...
        data = self._http_request(method="GET", url_suffix=f"{BOARDS_SUFFIX}/{board_id}{ACTIONS}", params=params)
        return data
    ...
    def list_lists(self, board_id):
        ...
        """
        data = self._http_request(method="GET", url_suffix=f"{BOARDS_SUFFIX}/{board_id}{LISTS_SUFFIX}", params=self.params)
        return data
    ...
    def list_boards(self):
        ...
        """
        data = self._http_request(method="GET", url_suffix=f"{LIST_BOARDS_SUFFIX}", params=self.params)
        return data
    ...
    def create_card(self, demisto_args):
        ...
        data = self._http_request(method="POST", url_suffix=f"{CARDS_SUFFIX}", params=params)
        return data
    ...
    def list_cards(self, list_id):
        ...
        """
        data = self._http_request(method="GET", url_suffix=f"{LISTS_SUFFIX}/{list_id}{CARDS_SUFFIX}", params=self.params)
        return data
    ...
    def update_card(self, card_id, demisto_args):
        ...
        params = {**self.params, **demisto_args}
        data = self._http_request(method="PUT", url_suffix=f"{CARDS_SUFFIX}/{card_id}", params=params)
        return data
    ...
    def add_card_comment(self, card_id, demisto_args):
        ...
        params = {**self.params, **demisto_args}
        data = self._http_request(method="POST", url_suffix=f"{CARDS_SUFFIX}/{card_id}{COMMENTS}", params=params)
        return data
    ...
    def delete_card(self, card_id):
        ...
        data = self._http_request(method="DELETE", url_suffix=f"{CARDS_SUFFIX}/{card_id}", params=self.params)
        return data
    ...
    def get_card(self, card_id):
        ...
        data = self._http_request(method="GET", url_suffix=f"{CARDS_SUFFIX}/{card_id}", params=self.params)
        return data
    ...
    def list_labels(self, board_id):
        ...
        """
        data = self._http_request(method="GET", url_suffix=f"{BOARDS_SUFFIX}/{board_id}{LABELS_SUFFIX}", params=self.params)
        return data
    ...
    def create_label(self, board_id, demisto_args):
        ...
        """
        params = {**self.params, **demisto_args}
        data = self._http_request(method="POST", url_suffix=f"{BOARDS_SUFFIX}/{board_id}{LABELS_SUFFIX}", params=params)
        return data
...
def normalise_card_fields(cards):
    ...
    """
    fields = {"id": "ID", "name": "Name", "url": "URL", "due": "Due", "labels": "Labels"}
    for card in cards:
...
def update_card(client, card_id, demisto_args):
    ...
    r = CommandResults(
        ...
        outputs=outputs,
        readable_output=tableToMarkdown("Trello Updated card", result, headers=["id", "name"] + list(demisto.args().keys())),
        raw_response=result,
    )
    return r
...
def get_mapping_fields():
    fields = {"Trello": {"id": "str", "name": "str", "desc": "str"}}
...
def get_remote_data(client, args):
    ...
    if new_incident_data.get("closed"):
        ...
        entries.append(
            {
                "Type": EntryType.NOTE,
                "Contents": {"dbotIncidentClose": True, "closeReason": "Trello Card archived."},
...
def main():
    ...
    try:
        client = Client(key, token, base_url=base_url, verify=verify_certificate, proxy=proxy)
        ...
        if demisto.command() == "test-module":
            ...
            next_run, incidents = fetch_incidents(
                client=client, last_run=demisto.getLastRun(), board_id=board_id, list_id_filter=list_id
            )
