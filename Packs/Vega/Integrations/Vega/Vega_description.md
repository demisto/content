...
- **Target Parameters**: The action parameters returned by Vega (for example, `user_id` or `ip`).

The integration normalizes recommended action descriptions during ingestion by ensuring each description ends with a trailing newline. This prevents long descriptions from being truncated in the Cortex XSOAR grid UI and allows the full text to wrap and display correctly.
