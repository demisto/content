...
​
#### Validation that AD FS server role is enabled:
1. In "Server Manager", press "Manage" then "Add Roles and Features".
2. Navigate to "Server Roles" section in the left menu.
3. Validate that "Active Directory Federation Services" is selected and installed.
![Server Screenshot](https://i.postimg.cc/c41b2r0f/ADFSEnable.jpg)
...
![Server Screenshot](https://i.postimg.cc/6QHsbnG2/ADFSCommands.pngJ)
5. Additional validation of the logging can be located at the Windows "Event Viewer":
- Run "eventvwr.msc" in the search bar.
- In the left directory tree, select "Applications and Services Logs".
- Validate that "AD FS" exists and "Admin" logs are located within.
 ![Server Screenshot](https://i.postimg.cc/ZKhd5dCy/ADFSEvent-Viewer.pngJ)
...
​
When configuring the instance, you should use a yml file that configures the vendor and product, as shown in the below configuration for the Microsoft AD FS product.
​
