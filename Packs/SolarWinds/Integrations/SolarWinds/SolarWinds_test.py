...
@pytest.mark.parametrize(
    "args",
    [
        {"page": 1, "limit": 5, "sort_order": "ascending"},
        {"page": 1, "limit": 5, "sort_order": "descending"}
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"node": "temporary,dummy"},
         " WHERE ( Node = 'temporary' OR Node = 'dummy' ) ORDER BY EventID ASC WITH ROWS 1 TO 50"),
        ...
        ({"event_id": "1, 2"}, " WHERE ( EventID = 1 OR EventID = 2 ) ORDER BY "
                               "EventID ASC WITH ROWS 1 TO 50")
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"alert_id": "1, 2"}, " WHERE ( AlertActiveID = 1 OR AlertActiveID = 2 ) ORDER BY "
                               "AlertActiveID ASC WITH ROWS 1 TO 50")
    ]
)
...
def test_convert_query_output_to_hr_success():
    ...
    expected_response = util_load_json(os.path.join(os.path.dirname(__file__), "test_data", "test_swis_query_success"
                                                                                            ".json"))
...
def test_fetch_incidents_alerts_success(http_request, client):
    ...
    expected_response = util_load_json(os.path.join(os.path.dirname(__file__), "test_data", "test_fetch_incidents"
                                                                                            ".json"))["alerts"]
    params = {
        'fetch_type': "Alert",
        'max_fetch': "5",
    ...
    raw_response = util_load_json(
        os.path.join(os.path.dirname(__file__), "test_data", "test_fetch_incidents_raw_response"
                                                             ".json"))["alerts"]
