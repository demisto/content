...
def get_group_memberships_for_member(args: dict, client: Any, IdentityStoreId: str) -> list:
    ...
    kwargs = {"IdentityStoreId": IdentityStoreId, "MemberId": {"UserId": user_id}}
...
def create_user(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    kwargs = {
        ...
        "Emails": [{"Value": user_email, "Primary": primary_email}],
    ...
    result = CommandResults(outputs_prefix=PREFIX_USER, readable_output=human_readable, outputs=response)
...
def update_user(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    kwargs = {"IdentityStoreId": IdentityStoreId, "UserId": user_id, "Operations": operations}
    client.update_user(**kwargs)
    hr_data = f"User {user_name} has been successfully updated"
    result = CommandResults(readable_output=hr_data)
    return_results(result)
...
def delete_user(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    """
    user_id = get_userId_by_username(args, client, IdentityStoreId)
    client.delete_user(IdentityStoreId=IdentityStoreId, UserId=user_id)
...
def get_user(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    response = client.describe_user(IdentityStoreId=IdentityStoreId, UserId=response_id)
    response.pop("ResponseMetadata", None)
    hr_data = {
    ...
    result = CommandResults(
        outputs_prefix=PREFIX_USER, readable_output=human_readable, outputs_key_field="UserId", outputs=response
    )
    return_results(result)
...
def get_user_by_email(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    human_readable = tableToMarkdown("AWS IAM Identity Center Users ", hr_data, removeNull=True)
    result = CommandResults(
        outputs_prefix=PREFIX_USER, readable_output=human_readable, outputs_key_field="UserId", outputs=context_data
    )
    return_results(result)
...
def list_users(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    context_data = []
    hr_data = []
    kwargs = {"IdentityStoreId": IdentityStoreId, "MaxResults": get_limit(args), "NextToken": args.get("nextToken")}
    kwargs = remove_empty_elements(kwargs)
    ...
    outputs = {
        f"{PREFIX_USER}(val.UserId === obj.UserId)": context_data,
        f"{PREFIX}(true)": {"UserNextToken": response.get("NextToken")},
    }
    human_readable = tableToMarkdown("AWS IAM Identity Center Users", hr_data, removeNull=True)
    result = CommandResults(readable_output=human_readable, outputs_key_field="UserId", outputs=outputs)
    return_results(result)
...
def list_groups(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    outputs = {
        f"{PREFIX_GROUP}(val.GroupId === obj.GroupId)": context_data,
    ...
    result = CommandResults(readable_output=human_readable, outputs_key_field="GroupId", outputs=outputs)
    return_results(result)
...
def create_group(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    kwargs = {"IdentityStoreId": IdentityStoreId, "DisplayName": display_name, "Description": args.get("description")}
    kwargs = remove_empty_elements(kwargs)
    ...
    result = CommandResults(outputs_prefix=PREFIX_GROUP, readable_output=human_readable, outputs=response)
    return_results(result)
...
def delete_group(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    client.delete_group(IdentityStoreId=IdentityStoreId, GroupId=group_id)
...
def update_group(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    kwargs = {
        "IdentityStoreId": IdentityStoreId,
        "GroupId": group_id,
        "Operations": [{"AttributePath": "description", "AttributeValue": args.get("description")}],
    }
...
def get_group(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    response = client.describe_group(IdentityStoreId=IdentityStoreId, GroupId=response_id)
    hr_data = {"DisplayName": response.get("DisplayName"), "GroupId": response.get("GroupId")}
    response.pop("ResponseMetadata", None)
    human_readable = tableToMarkdown("AWS IAM Identity Center Groups", hr_data, removeNull=True)
    result = CommandResults(
        outputs_prefix=PREFIX_GROUP, readable_output=human_readable, outputs_key_field="GroupId", outputs=response
    )
    return_results(result)
...
def list_groups_for_user(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    for group in response.get("GroupMemberships", []):
        hr_data.append({"UserID": user_id, "GroupID": group.get("GroupId"), "MembershipID": group.get("MembershipId")})
        groups.append({"GroupId": group.get("GroupId"), "MembershipId": group.get("MembershipId")})
...
def add_user_to_group(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    user_id = get_userId_by_username(args, client, IdentityStoreId)
    Group_id = get_groupId_by_displayName(args, client, IdentityStoreId)
    response = client.create_group_membership(IdentityStoreId=IdentityStoreId, GroupId=Group_id, MemberId={"UserId": user_id})
...
def delete_group_membership(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    for member in memberships_to_delete:
        client.delete_group_membership(IdentityStoreId=IdentityStoreId, MembershipId=member)
...
def list_group_memberships(args: dict, client: Any, IdentityStoreId: str) -> None:
    ...
    for membership in response.get("GroupMemberships", []):
        ...
        memberships.append(
            {"MembershipId": membership.get("MembershipId"), "UserId": membership.get("MemberId", {}).get("UserId")}
        )
    ...
    human_readable = tableToMarkdown("AWS IAM Identity Center Groups", hr_data, removeNull=True)
    result = CommandResults(
        outputs_prefix=PREFIX_GROUP, readable_output=human_readable, outputs_key_field="GroupId", outputs=context_data
    )
    return_results(result)
...
def test_module(args: dict, client: Any, IdentityStoreId: str) -> None:  # pragma: no cover
    ...
    if not IdentityStoreId:
        return_error(
            "Identity Store ID was not specified - Test failure. The `Identity Store ID` parameter can be left empty "
