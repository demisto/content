...
def test_create_user(mocker):
    ...
    res = {"UserId": "USER_ID", "IdentityStoreId": IDENTITY_STORE_ID, "ResponseMetadata": {"HTTPStatusCode": 200}}
    ...
    assert {"UserId": "USER_ID", "IdentityStoreId": IDENTITY_STORE_ID} in contents.get("EntryContext").values()
...
def test_create_group(mocker):
    ...
    """
    args = {"displayName": "Test Group", "description": "Test Description"}
    res = {"GroupId": IDENTITY_STORE_ID, "ResponseMetadata": {"HTTPStatusCode": 200}}
    ...
    contents = demisto.results.call_args[0][0]
    assert {"GroupId": IDENTITY_STORE_ID} in contents.get("EntryContext").values()
...
def test_list_users(mocker):
    ...
    """
    args = {"limit": 1, "nextToken": "TOKEN"}

    res = {
        "IdentityStoreId": IDENTITY_STORE_ID,
        "Users": [
            {"UserId": "USER_ID", "UserName": "test_user", "DisplayName": "Test User", "Emails": [{"Value": "test@example.com"}]}
        ],
    ...
    assert [
        {"UserId": "USER_ID", "UserName": "test_user", "DisplayName": "Test User", "Emails": [{"Value": "test@example.com"}]}
    ] in contents.get("EntryContext").values()
    assert {"UserNextToken": "NEXT_TOKEN"} in contents.get("EntryContext").values()
...
def test_list_groups(mocker):
    ...
    res = {"Groups": [{"GroupId": "123", "DisplayName": "Test Group", "Description": "Test Description"}], "NextToken": None}
    ...
    assert {"GroupNextToken": None} in contents.get("EntryContext").values()
...
def test_get_user_by_email(mocker):
    ...
    args = {"emailAddress": "test@example.com"}
...
def test_get_user_by_email_not_exist(mocker):
    ...
    args = {"emailAddress": "notexist@example.com"}
...
def test_get_group(mocker):
    ...
    """
    args = {"displayName": "test_group"}

    response_id = {"GroupId": IDENTITY_STORE_ID}

    res = {"GroupId": "string", "DisplayName": "test_group", "Description": None, "ResponseMetadata": {"HTTPStatusCode": 200}}
    ...
    assert {"GroupId": "string", "DisplayName": "test_group", "Description": None} in contents.get("EntryContext").values()
    assert "AWS IAM Identity Center Groups" in contents.get("HumanReadable")
...
def test_add_user_to_group(mocker):
    ...
    """
    args = {"userName": "test_user", "displayName": "test_group"}

    res = {"MembershipId": "10203040", "ResponseMetadata": {"HTTPStatusCode": 200}}
...
def test_list_groups_for_user(mocker):
    ...
    res = {
        "GroupMemberships": [{"MemberId": {"UserId": "USER_ID"}, "GroupId": "GROUP_ID", "MembershipId": "MEMBERSHIP_ID"}],
        "NextToken": None,
    }
    ...
    assert {
        "GroupMemberships": [{"GroupId": "GROUP_ID", "MembershipId": "MEMBERSHIP_ID"}],
        "UserId": "USER_ID",
        "GroupsUserNextToken": None,
    } in contents.get("EntryContext").values()
    assert "AWS IAM Identity Center Groups" in contents.get("HumanReadable")
...
def test_delete_group_membership(mocker):
    ...
    res = {
        "GroupMemberships": [
            {"MemberId": {"UserId": "USER_ID"}, "GroupId": "GROUP_ID", "MembershipId": "MEMBERSHIP_ID"},
            {"MemberId": {"UserId": "USER_ID"}, "GroupId": "GROUP_ID2", "MembershipId": "MEMBERSHIP_ID123"},
        ],
        "NextToken": None,
    }
...
def test_delete_group_memberships_by_membershipId(mocker):
    ...
    Then:
        Verify that the correct group membership is deleted
    """
    args = {"membershipId": "MEMBERSHIP_ID, MEMBERSHIP_ID123"}
...
def test_list_group_memberships(mocker):
    ...
    response = {
        "GroupMemberships": [{"MembershipId": "MEMBERSHIP_ID", "MemberId": {"UserId": "USER_ID"}}],
        "NextToken": "NEXT_TOKEN",
    }
    ...
    assert {
        "GroupId": "GROUP_ID",
        "GroupMemberships": [{"MembershipId": "MEMBERSHIP_ID", "UserId": "USER_ID"}],
        "GroupMembershipNextToken": "NEXT_TOKEN",
    } in contents.get("EntryContext").values()
    assert "AWS IAM Identity Center Groups" in contents.get("HumanReadable")
...
@pytest.mark.parametrize(
    "last_data, current_data, expected_results",
    [
        (
            [
                {"id": 1, "groups": [{"GroupId": "GROUP_1", "MembershipId": "A"}, {"GroupId": "GROUP_2", "MembershipId": "B"}]},
                {"id": 2, "groups": [{"GroupId": "GROUP_1", "MembershipId": "C"}, {"GroupId": "GROUP_3", "MembershipId": "D"}]},
            ],
            [
                {"GroupId": "GROUP_1", "MembershipId": "C"},
                {"GroupId": "GROUP_3", "MembershipId": "D"},
                {"GroupId": "GROUP_4", "MembershipId": "F"},
            ],
            [
                {"GroupId": "GROUP_1", "MembershipId": "C"},
                {"GroupId": "GROUP_3", "MembershipId": "D"},
        ...
        (
            {"id": 2, "groups": [{"GroupId": "GROUP_1", "MembershipId": "C"}, {"GroupId": "GROUP_2", "MembershipId": "B"}]},
            [
                {"GroupId": "GROUP_1", "MembershipId": "C"},
                {"GroupId": "GROUP_3", "MembershipId": "D"},
                {"GroupId": "GROUP_4", "MembershipId": "F"},
            ...
            [
                {"GroupId": "GROUP_1", "MembershipId": "C"},
                {"GroupId": "GROUP_2", "MembershipId": "B"},
                {"GroupId": "GROUP_3", "MembershipId": "D"},
                {"GroupId": "GROUP_4", "MembershipId": "F"},
            ],
        ),
        ([], [{"GroupId": "GROUP_1", "MembershipId": "C"}], [{"GroupId": "GROUP_1", "MembershipId": "C"}]),
    ],
)
