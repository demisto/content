...
DEFAULT_SUSP_DISP = {"drug_spam", "gambling", "hacked_website", "streaming", "suspicious"}

DEFAULT_BAD_DISP = {"cryptojacking", "phish", "likely_phish", "scam"}
...
def test_module(**kwargs):
    query = {"apiKey": kwargs.get("api_key"), "urlInfo": {"url": "https://www.google.com"}}
...
def get_result_checkphish(jobID, api_key, base_url, use_ssl, reliability):
    ...
    if res and "errorMessage" not in res:
        ...
        if result["disposition"] != CLEAN_STATUS:
            url_dict["Malicious"] = {"Vendor": "CheckPhish", "Description": "Targets " + result["brand"]}
        ...
        context = {"CheckPhish." + outputPaths["url"]: result, outputPaths["url"]: url_dict, "DBotScore": dbot_score}

        human_readable = tableToMarkdown(
            "CheckPhish reputation for " + result["url"], result, ["url", "disposition", "brand", "status", "jobID"]
        )
    ...
    while len(job_ids):
        for job_id in job_ids[:]:
            if is_job_ready_checkphish(job_id, kwargs.get("api_key"), kwargs.get("base_url"), kwargs.get("use_ssl")):
                get_result_checkphish(
                    job_id, kwargs.get("api_key"), kwargs.get("base_url"), kwargs.get("use_ssl"), kwargs.get("reliability")
                )
