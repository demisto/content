...
INDICATOR_DATA = {"indicator": {"CustomFields": {"chronicleassethostname": "0.0.0.0", "chroniclepotentiallyblockedip": "No"}}}
...
def test_main_success(mocker):
    ...
    mocker.patch.object(ChroniclePotentiallyBlockedIPWidgetScript, "get_html_representation", return_value="")
...
def test_main_failure(mock_return_error, capfd, mocker):
    ...
    mocker.patch.object(demisto, "args", return_value=INDICATOR_DATA)
    mocker.patch.object(ChroniclePotentiallyBlockedIPWidgetScript, "get_html_representation", side_effect=Exception)
    with capfd.disabled():
        ChroniclePotentiallyBlockedIPWidgetScript.main()
...
def test_get_html_representation_when_ip_is_not_blocked():
    ...
    assert (
        "<div style='color:green; text-align:center;'><h1>0.0.0.0<br/>IP Address Not Blocked</h1>" "</div>" == html_representation
    )
...
def test_get_html_representation_when_ip_is_potentially_blocked():
    ...
    html_representation = ChroniclePotentiallyBlockedIPWidgetScript.get_html_representation("0.0.0.0", "Yes")
    assert (
        "<div style='color:orange; text-align:center;'><h1>0.0.0.0<br/>IP Address Potentially Blocked</h1>"
