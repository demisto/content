...
def get_html_representation(entity: str, is_isolated: str) -> str:
    if not entity:
        ...
        html = "<div style='color:green; text-align:center;'><h1>{0}<br/>Hostname Not Isolated</h1></div>".format(entity)
        if is_isolated == "Yes":
            html = "<div style='color:red; text-align:center;'><h1>{0}<br/>Hostname Isolated</h1></div>".format(entity)
    return html
