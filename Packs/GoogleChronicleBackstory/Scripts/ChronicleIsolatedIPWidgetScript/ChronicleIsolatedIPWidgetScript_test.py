...
INDICATOR_DATA = {"indicator": {"CustomFields": {"chronicleassetip": "0.0.0.0", "chronicleisolatedip": "No"}}}
...
def test_main_success(mocker):
    ...
    mocker.patch.object(ChronicleIsolatedIPWidgetScript, "get_html_representation", return_value="")
...
def test_main_failure(mock_return_error, capfd, mocker):
    ...
    mocker.patch.object(demisto, "args", return_value=INDICATOR_DATA)
    mocker.patch.object(ChronicleIsolatedIPWidgetScript, "get_html_representation", side_effect=Exception)
    with capfd.disabled():
        ChronicleIsolatedIPWidgetScript.main()
...
def test_get_html_representation_when_ip_is_not_isolated():
    ...
    assert (
        html_representation == "<div style='color:green; text-align:center;'><h1>0.0.0.0<br/>IP Address Not Isolated</h1>"
        "</div>"
    )
...
def test_get_html_representation_when_ip_is_potentially_isolated():
    ...
    html_representation = ChronicleIsolatedIPWidgetScript.get_html_representation("0.0.0.0", "Yes")
    assert html_representation == "<div style='color:red; text-align:center;'><h1>0.0.0.0<br/>IP Address Isolated</h1></div>"
