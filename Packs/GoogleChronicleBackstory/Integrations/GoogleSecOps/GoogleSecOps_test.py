...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"secops_project_instance_id": "dummy", "credentials": {"password": "{}"}, "secops_project_number": "invalid number"},
            "Google SecOps Project Number should be a positive number.",
        ),
        (
            {"secops_project_instance_id": "dummy", "credentials": {"password": "{}"}, "secops_project_number": "0"},
            "Google SecOps Project Number should be a positive number.",
        ),
        (
            {"secops_project_instance_id": "dummy", "credentials": {"password": "{}"}, "secops_project_number": "-123"},
            "Google SecOps Project Number should be a positive number.",
        ),
    ],
)
...
def test_fetch_incidents_exits_when_time_diff_exceeds_4_minutes(client, mocker):
    ...
    params = {"first_fetch": "2025-07-20T00:00:00Z", "max_fetch": 2}
    mocker.patch.object(demisto, "getLastRun", return_value={})
    mocker.patch("GoogleSecOps.MAX_IOCS_FETCH_SIZE", 2)

    # Patch datetime.now()
    mock_now = mocker.patch("GoogleSecOps.datetime")

    t1 = datetime(2025, 7, 21, 10, 0, 0)
...
def test_fetch_incidents_exits_when_time_diff_less_than_1_microsecond(client, mocker):
    """
