...
import pytest
from GoogleChronicleBackstory import (
...
ARGS = {"artifact_value": "0.0.0.0", "ip": "0.0.0.0", "domain": "test.com"}
...
def test_gcb_list_ioc_success(mocker):
    ...
    from GoogleChronicleBackstory import Client, auth_requests, gcb_list_iocs_command, service_account
    ...
    client = Client({"service_account_credential": json.dumps(credentials), "region": "General"}, proxy={}, disable_ssl=True)
...
def test_gcb_ioc_details_command_success(client):
    ...
    assert ec[IP_CONTEXT_PATH] == dummy_ec[IP_CONTEXT_PATH]
...
def test_validate_parameter_success(mocker):
    ...
    param = {"service_account_credential": DUMMY_DICT, "max_fetch": "20", "first_fetch": DUMMY_FETCH}
...
def test_validate_parameter_failure_wrong_json():
    ...
    wrong_credentials = {"service_account_credential": '{"key","value"}', "max_fetch": "20", "first_fetch": DUMMY_FETCH}
...
def test_validate_parameter_failure_page_size():
    ...
    wrong_page_sizes = {"service_account_credential": DUMMY_DICT, "max_fetch": "2a0", "first_fetch": DUMMY_FETCH}
...
def test_validate_parameter_failure_wrong_first_fetch_format():
    ...
    wrong_fetch_days_format = {"service_account_credential": DUMMY_DICT, "max_fetch": "20", "first_fetch": "29 feb 2021"}
    with pytest.raises(ValueError) as error:
...
def test_validate_parameter_failure_wrong_first_fetch_number():
    ...
    wrong_fetch_days_number = {"service_account_credential": DUMMY_DICT, "max_fetch": "20", "first_fetch": "120000 months"}
    with pytest.raises(ValueError) as error:
...
def test_validate_parameter_failure_wrong_first_fetch_unit():
    ...
    wrong_fetch_days_unit = {"service_account_credential": DUMMY_DICT, "max_fetch": "20", "first_fetch": "10 dais"}
    with pytest.raises(ValueError) as error:
...
def test_gcb_fetch_incident_success_with_alerts_with_demisto_last_run(mocker, client):
    ...
    param = {"max_fetch": 20, "incident_severity": None, "backstory_alert_type": ASSET_ALERT_TYPE}
    with open("test_data/gcb_alerts_response.txt") as f:
        gcb_alert_sample = f.read()
...
def test_asset_with_multiple_alerts_human_readable(client):
    ...
    from GoogleChronicleBackstory import get_gcb_alerts, group_infos_by_alert_asset_name
...
def test_list_detections_command(client):
    ...
    args = {
        "rule_id": "ru_e6abfcb5-1b85-41b0-b64c-695b3250436f",
        "detection_start_time": "2019-10-17T00:00:00Z",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"rule_id": "ru_e6abfcb5-1b85-41b0-b64c-695b3250436f", "start_time": "645.08"}, 'Invalid date: "start_time"="645.08"'),
        ({"rule_id": "ru_e6abfcb5-1b85-41b0-b64c-695b3250436f", "start_time": "-325.21"}, 'Invalid date: "start_time"="-325.21"'),
        ({"rule_id": "ru_e6abfcb5-1b85-41b0-b64c-695b3250436f", "end_time": "645.08"}, 'Invalid date: "end_time"="645.08"'),
        ({"rule_id": "ru_e6abfcb5-1b85-41b0-b64c-695b3250436f", "end_time": "-325.21"}, 'Invalid date: "end_time"="-325.21"'),
...
def test_list_curatedrule_detections_command(client):
    ...
    args = {
        "id": "ur_ttp_GCP__GlobalSSHKeys_Added",
        "detection_start_time": "2023-06-14T17:28:00Z",
        "detection_end_time": "2 days ago",
    }
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"id": "ur_ttp_GCP__GlobalSSHKeys_Added", "end_time": "-325.21"}, 'Invalid date: "end_time"="-325.21"'),
...
def test_gcb_fetch_incident_success_with_alerts_with_incident_identifiers(mocker, client):
    ...
    param = {"max_fetch": 20, "incident_severity": None, "backstory_alert_type": ASSET_ALERT_TYPE, "time_window": "45"}
    with open("test_data/gcb_alerts_response.txt") as f:
        gcb_alert_sample = f.read()
...
def test_validate_parameter_failure_invalid_time_window_values():
    ...
    invalid_time_window = {"service_account_credential": DUMMY_DICT, "max_fetch": "10"}
...
def test_no_duplicate_rule_id_on_detection_to_pull_exception(mock_error, mock_build, client):
    ...
    for _ in range(5):
        x, y, z, w = get_max_fetch_detections(
            client,
            "12",
            "23",
            5,
            [
                {"id": "123", "detection": [{"ruleVersion": "3423", "ruleName": "SampleRule"}]},
                {"id": "1234", "detection": [{"ruleVersion": "342", "ruleName": "SampleRule"}]},
                {"id": "12345", "detection": [{"ruleVersion": "34", "ruleName": "SampleRule"}]},
            ],
            {"rule_id": "456", "next_page_token": "foorbar"},
            z,
            "",
            {},
            "CREATED_TIME",
...
def test_detection_to_pull_is_empty_when_2nd_rule_returns_data_with_no_next_token(mock_build, client):
    ...
    """
    from GoogleChronicleBackstory import get_detections, get_max_fetch_detections
    ...
    x, y, z, w = get_max_fetch_detections(
        client, "st_dummy", "et_dummy", 3, [], {"rule_id": "123", "next_page_token": "foorbar"}, z, "", {}, "CREATED_TIME"
    )
...
def test_when_detection_to_pull_is_not_empty_and_return_empty_result(mock_validate_response, client):
    ...
    z = ["rule_2", "rule_3"]
    x, y, z, w = get_max_fetch_detections(
        client, "st_dummy", "et_dummy", 5, [], {"rule_id": "rule_1", "next_page_token": "foorbar"}, z, "", {}, "CREATED_TIME"
    )
...
def test_429_or_500_error_with_max_attempts_60(mock_error, client):
    ...
    client.http_client.request.side_effect = [MockResponse429] * 30 + [MockResponse3] + [MockResponse500] * 61 + [MockResponse3]
...
def test_400_and_404_error(mock_error, client):
    ...
    client.http_client.request.side_effect = [MockResponse3, MockResponse400, MockResponse3, MockResponse404, MockResponse3]
...
def test_fetch_incident_detection_case_4(client, mocker):
    ...
    client.http_client.request.side_effect = [MockResponse3, MockResponse5, MockResponse2, MockResponse3]
    ...
    mock_last_run = {
        ...
        "detection_to_pull": {"rule_id": "456", "next_page_token": "foorbar"},
...
def test_fetch_incident_detection_case_5(client, mocker):
    ...
    client.http_client.request.side_effect = [MockResponse3, MockResponse5, MockResponse2, MockResponse5]
...
def test_no_duplicate_curated_rule_id_on_detection_to_pull_exception(mock_error, mock_build, client):
    ...
    mock_error.return_value = {}
    for _ in range(5):
        x, y, z, w = get_max_fetch_curatedrule_detections(
            client,
            "12",
            "23",
            5,
            ...
            {"rule_id": "456", "next_page_token": "next_page_token"},
            z,
            "",
            {},
            "CREATED_TIME",
...
def test_curatedrule_detection_to_pull_is_empty_when_2nd_rule_returns_data_with_no_next_token(mock_build, client):
    ...
    for rule_1 and 2 records for rule_2 and complete the fetch-incident cycle since we don't have any rule to process.
    """
    from GoogleChronicleBackstory import get_curatedrule_detections, get_max_fetch_curatedrule_detections
    ...
    x, y, z, w = get_max_fetch_curatedrule_detections(
        client, "st_dummy", "et_dummy", 3, [], {"rule_id": "123", "next_page_token": "next_page_token"}, z, "", {}, "CREATED_TIME"
    )
...
def test_when_curatedrule_detection_to_pull_is_not_empty_and_return_empty_result(mock_validate_response, client):
    ...
    x, y, z, w = get_max_fetch_curatedrule_detections(
        ...
        5,
        [],
        {"rule_id": "rule_1", "next_page_token": "next_page_token"},
        z,
        "",
        {},
        "CREATED_TIME",
...
def test_fetch_user_alert_incident_success_with_param_and_alerts_when_executed_1st_time(mocker, client):
    ...
    param = {"first_fetch": "4 days", "max_fetch": 20, "time_window": "60", "backstory_alert_type": USER_ALERT}
...
def test_gcb_user_alert_fetch_incident_success_with_alerts_with_demisto_last_run(mocker, client):
    ...
    param = {"max_fetch": 20, "backstory_alert_type": USER_ALERT}
    with open("test_data/gcb_alerts_response.txt") as f:
        gcb_alert_sample = f.read()
...
def test_gcb_fetch_incident_user_alert_success_with_alerts_with_incident_identifiers(mocker, client):
    ...
    param = {"max_fetch": 20, "backstory_alert_type": USER_ALERT, "time_window": "45"}
    with open("test_data/gcb_alerts_response.txt") as f:
        gcb_alert_sample = f.read()
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"rule_id": "ru_ab4d76c1-20d2-4cde-9825-3fb1c09a9b62", "start_time": "dummy", "end_time": "today"},
            'Invalid date: "start_time"="dummy"',
        ),
        (
            {"rule_id": "ru_ab4d76c1-20d2-4cde-9825-3fb1c09a9b62", "start_time": "1 day", "end_time": "dummy"},
...
def test_gcb_start_retrohunt_command_when_invalid_rule_id_provided(client):
    ...
    with open("test_data/gcb_start_retrohunt_command_invalid_id_400.json") as f:
        raw_response = f.read()
    args = {"rule_id": "dummy", "start_time": "1 day", "end_time": "today"}
...
def test_gcb_list_retrohunts_command_when_retrohunts_for_all_versions_is_set_true(client):
    ...
    with open("test_data/gcb_list_retrohunts_all_versions_true_hr.md") as f:
        expected_hr = f.read()
    args = {"id": "dummy", "gcb_list_retrohunts_command": "true"}
...
def test__gcb_list_retrohunts_command_when_retrohunts_for_all_versions_is_set_false(client):
    ...
    with open("test_data/gcb_list_retrohunts_all_versions_false_hr.md") as f:
        expected_hr = f.read()
    args = {"id": "dummy", "gcb_list_retrohunts_command": "false"}
...
@pytest.mark.parametrize(
    "args, error_msg",
    [
        ({"id": "", "retrohunt_id": "dummy"}, "Missing argument id."),
...
def test_gcb_cancel_retrohunt_command_when_provided_rule_id_does_not_exist(client):
    ...
    with open("test_data/gcb_cancel_retrohunt_id_does_not_exist_404.json") as f:
        raw_response = f.read()
    args = {"id": "dummy", "retrohunt_id": "dummy"}
...
@pytest.mark.parametrize(
    "args,error_msg",
    [
        ({"rule_text": "meta events condition", "start_time": "dummy"}, 'Invalid date: "start_time"="dummy"'),
        (
            {"rule_text": "meta events condition", "start_time": "1 day ago", "end_time": "dummy"},
            'Invalid date: "end_time"="dummy"',
        ),
        (
            {"rule_text": "meta events condition", "start_time": "1 day ago", "end_time": "1 day ago", "max_results": 0},
            "Max Results should be in the range 1 to 10000.",
        ),
        (
            {"rule_text": "meta events condition", "start_time": "1 day ago", "end_time": "1 day ago", "max_results": "asd"},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            MESSAGES["VALIDATE_SINGLE_SELECT"].format("asset_identifier_type", ASSET_IDENTIFIER_NAME_DICT.keys()),
        ),
        (
            {"asset_identifier_type": "Host Name", "asset_identifier": "example.com", "start_time": "invalid_time"},
            'Invalid date: "start_time"="invalid_time"',
        ),
        (
            {"asset_identifier_type": "Host Name", "asset_identifier": "example.com", "end_time": "invalid_time"},
            'Invalid date: "end_time"="invalid_time"',
        ),
        (
            {"asset_identifier_type": "Host Name", "asset_identifier": "example.com", "page_size": "invalid_size"},
            "Page size must be a non-zero and positive numeric value",
        ),
        (
            {"asset_identifier_type": "Host Name", "asset_identifier": "example.com", "page_size": "-1"},
            "Page size must be a non-zero and positive numeric value",
        ),
    ],
)
...
def test_gcb_asset_aliases_list_command_when_valid_arguments_provided(client):
    ...
    with open("test_data/gcb_list_asset_aliases_hr.md") as f:
        expected_hr = f.read()
    args = {"asset_identifier_type": "Host Name", "asset_identifier": "example.com"}
...
@pytest.mark.parametrize(
    "args,error_msg",
    [
        ({"user_identifier_type": "Email", "user_identifier": ""}, MESSAGES["REQUIRED_ARGUMENT"].format("user_identifier")),
        (
            {"user_identifier_type": "invalid_type", "user_identifier": "xyz@example.com"},
            MESSAGES["VALIDATE_SINGLE_SELECT"].format("user_identifier_type", USER_IDENTIFIER_NAME_DICT.keys()),
        ),
        (
            {"user_identifier_type": "Email", "user_identifier": "xyz@example.com", "start_time": "invalid_time"},
            'Invalid date: "start_time"="invalid_time"',
        ),
        (
            {"user_identifier_type": "Email", "user_identifier": "xyz@example.com", "end_time": "invalid_time"},
            'Invalid date: "end_time"="invalid_time"',
        ),
        (
            {"user_identifier_type": "Email", "user_identifier": "xyz@example.com", "page_size": "invalid_size"},
            "Page size must be a non-zero and positive numeric value",
        ),
        (
            {"user_identifier_type": "Email", "user_identifier": "xyz@example.com", "page_size": "-1"},
            "Page size must be a non-zero and positive numeric value",
        ),
    ],
)
...
def test_gcb_user_aliases_list_command_when_valid_arguments_provided(client):
    ...
    with open("test_data/gcb_list_user_aliases_hr.md") as f:
        expected_hr = f.read()
    args = {"user_identifier_type": "Email", "user_identifier": "xyz@example.com"}
