...
HOST_CTX_KEY_DICT = {"hostname": "Hostname", "assetIpAddress": "IP", "productId": "ID", "MACAddress": "MACAddress"}

CONTEXT_KEY_DICT = {"hostname": "HostName", "assetIpAddress": "IpAddress", "productId": "ProductId", "MACAddress": "MACAddress"}
...
SEVERITY_MAP = {"unspecified": 0, "informational": 0.5, "low": 1, "medium": 2, "high": 3}
...
def get_params_for_reputation_command():
    ...
    suspicious_confidence_score_threshold_str = demisto.params().get("override_confidence_level_suspicious")
...
def validate_configuration_parameters(param: dict[str, Any]):
    ...
    if (
        param.get("backstory_alert_type", "ioc domain matches").lower() == "detection alerts"
        and not param.get("fetch_all_detections", False)
        and not get_unique_value_from_list(detection_by_id)
    ):
        raise ValueError(
            'Please enter one or more Rule ID(s) or Version ID(s) as value of "Detections to '
            'fetch by Rule ID or Version ID" or check the checkbox "Fetch all rules '
            'detections" to fetch detections.'
        )

    if (
        param.get("backstory_alert_type", "ioc domain matches").lower() == "curated rule detection alerts"
        and not detection_by_ids
    ):
...
def prepare_hr_for_ioc_details(addresses, hr_table_row):
    ...
    for address in addresses:
        if address.get("domain"):
            address_data.append({"Domain": address["domain"], "Port": address.get("port", [])})
            hr_table_row["Domain"] = address["domain"]
        if address.get("ipAddress"):
            address_data.append({"IpAddress": address["ipAddress"], "Port": address.get("port", [])})
...
def parse_alert_info(alert_infos, filter_severity):
    ...
    for alert_info in alert_infos:
        ...
        if filter_severity and filter_severity.lower() != alert_info.get("severity", "").lower() and filter_severity != "ALL":
            continue
...
def get_gcb_alerts(client_obj, start_time, end_time, max_fetch, filter_severity):
    ...
    for alert in json_response.get("alerts", []):
        ...
        asset_alert = {"AssetName": list(alert["asset"].values())[0], "AlertCounts": alert_count, "AlertInfo": alert_info}
...
def convert_alerts_into_hr(events):
    ...
    return tableToMarkdown(
        "Security Alert(s)",
        list(data),
        ["Alerts", "Asset", ALERT_NAMES, FIRST_SEEN, LAST_SEEN, "Severities", "Sources"],
        removeNull=True,
    )
...
def validate_list_asset_aliases_args(
    asset_identifier_type: Optional[str], asset_identifier: str, page_size: Optional[int]
) -> None:
    """
    ...
    if not asset_identifier_type:
        raise ValueError(MESSAGES["VALIDATE_SINGLE_SELECT"].format("asset_identifier_type", ASSET_IDENTIFIER_NAME_DICT.keys()))
...
def validate_list_user_aliases_args(user_identifier_type: Optional[str], user_identifier: str, page_size: Optional[int]) -> None:
    """
    ...
    if not user_identifier_type:
        raise ValueError(MESSAGES["VALIDATE_SINGLE_SELECT"].format("user_identifier_type", USER_IDENTIFIER_NAME_DICT.keys()))
...
def get_detections(
    ...
    ec: dict[str, Any] = {CHRONICLE_OUTPUT_PATHS["Detections"]: parsed_ec}
    if token_ec:
...
def deduplicate_events_and_create_incidents(contexts: list, event_identifiers: list[str], user_alert: bool = False):
    ...
    for event in contexts:
        try:
            ...
            demisto.error(
                "[CHRONICLE] Skipping insertion of current event since error occurred while calculating"
                f" Hash for the event {event}. Error: {e!s}"
            )
            continue
...
def convert_events_to_actionable_incidents(events: list) -> list:
    ...
    for event in events:
        ...
        incident = {"name": event["detection"][0]["ruleName"], "details": json.dumps(event), "rawJSON": json.dumps(event)}
...
def fetch_detections(
    ...
    detection_to_pull,
    pending_rule_or_version_id: list,
    alert_state,
...
def fetch_curatedrule_detections(
    client_obj,
    start_time,
    end_time,
    max_fetch,
    ...
    """
    if (
        not pending_curatedrule_id
        and not curatedrule_detection_to_process
        and not curatedrule_detection_to_pull
...
def get_max_fetch_detections(
    ...
    while len(detection_incidents) < max_fetch and (
        len(pending_rule_or_version_id) != 0 or detection_to_pull or simple_backoff_rules
    ):
        next_page_token = ""
        ...
        try:
            ...
            if str(e).startswith("API rate limit") or str(e).startswith("Internal server error"):
                ...
                if attempts < MAX_ATTEMPTS:
                    demisto.error(
                        f"[CHRONICLE DETECTIONS] Error while fetching incidents: {e!s} Attempt no : {attempts + 1} "
                        f"for the rule_id : {rule_id} and next_page_token : {next_page_token}"
                    )
                    simple_backoff_rules = {"rule_id": rule_id, "next_page_token": next_page_token, "attempts": attempts + 1}
                else:
                    demisto.error(
                        f"[CHRONICLE DETECTIONS] Skipping the rule_id : {rule_id} due to the maximum "
...
def get_max_fetch_curatedrule_detections(
    client_obj,
    start_time,
    end_time,
    max_fetch,
    ...
    while len(curatedrule_detection_to_process) < max_fetch and (
        ...
        try:
            _, raw_resp = get_curatedrule_detections(
                client_obj,
                rule_id,
                max_fetch,
                start_time,
            ...
            if str(e).startswith("API rate limit") or str(e).startswith("Internal server error"):
                ...
                if attempts < MAX_ATTEMPTS:
                    demisto.error(
                        f"[CHRONICLE CURATED RULE DETECTIONS] Error while fetching incidents: {e!s} Attempt no : "
                        f"{attempts + 1} for the curated rule_id : {rule_id} and next_page_token : {next_page_token}"
                    )
                    simple_backoff_rules = {"rule_id": rule_id, "next_page_token": next_page_token, "attempts": attempts + 1}
                else:
                    demisto.error(
                        f"[CHRONICLE CURATED RULE DETECTIONS] Skipping the rule_id : {rule_id} "
            ...
            if str(e).startswith("Status code: 404") or str(e).startswith("Status code: 400"):
                if str(e).startswith("Status code: 404"):
                    demisto.error(
                        "[CHRONICLE CURATED RULE DETECTIONS] Error while fetching incidents: Rule with ID"
                        f" {rule_id} not found."
                    )
                else:
                    demisto.error(
...
def fetch_incidents_asset_alerts(client_obj, params: dict[str, Any], start_time, end_time, time_window, max_fetch):
    ...
    new_event_hashes, incidents = deduplicate_events_and_create_incidents(list(contexts.values()), assets_alerts_identifiers)

    # Updates the event hashes in last run with the new event hashes
    if contexts:
        assets_alerts_identifiers = new_event_hashes

    demisto.setLastRun({"start_time": end_time, "assets_alerts_identifiers": assets_alerts_identifiers})
...
def fetch_incidents_user_alerts(client_obj, params: dict[str, Any], start_time, end_time, time_window, max_fetch):
    ...
    new_event_hashes, incidents = deduplicate_events_and_create_incidents(contexts, user_alerts_identifiers, user_alert=True)

    # Updates the event hashes in last run with the new event hashes
    if contexts:
        user_alerts_identifiers = new_event_hashes

    demisto.setLastRun({"start_time": end_time, "user_alerts_identifiers": user_alerts_identifiers})
...
def fetch_incidents_detection_alerts(client_obj, params: dict[str, Any], start_time, end_time, time_window, max_fetch):
    ...
    if last_run and "start_time" in last_run:
        ...
        pending_rule_or_version_id_with_alert_state = last_run.get(
    ...
    if (
        not pending_rule_or_version_id_with_alert_state.get("rule_id")
        and not detection_to_pull
        and not detection_to_process
        and not simple_backoff_rules
    ):
        ...
        fetch_detection_by_ids = get_unique_value_from_list([r_v_id.strip() for r_v_id in fetch_detection_by_ids.split(",")])

        fetch_detection_by_alert_state = params.get("fetch_detection_by_alert_state", fetch_detection_by_alert_state)
        ...
        pending_rule_or_version_id_with_alert_state.update(
            {
                "rule_id": fetch_detection_by_ids,
                "alert_state": fetch_detection_by_alert_state,
                "listBasis": fetch_detection_by_list_basis,
            }
        )

    events, detection_to_process, detection_to_pull, pending_rule_or_version_id, simple_backoff_rules = fetch_detections(
        client_obj,
        delayed_start_time,
        end_time,
        int(max_fetch),
        detection_to_process,
        detection_to_pull,
        pending_rule_or_version_id_with_alert_state.get("rule_id", ""),
    ...
    pending_rule_or_version_id_with_alert_state.update(
        {
            "rule_id": pending_rule_or_version_id,
            "alert_state": fetch_detection_by_alert_state,
            "listBasis": fetch_detection_by_list_basis,
        }
    )
...
def fetch_incidents_curatedrule_detection_alerts(
    client_obj, params: dict[str, Any], start_time, end_time, time_window, max_fetch
):
    ...
    if last_run and "start_time" in last_run:
        start_time = last_run.get("start_time") or start_time
        curatedrule_detection_identifiers = last_run.get("curatedrule_detection_identifiers", curatedrule_detection_identifiers)
        curatedrule_detection_to_process = last_run.get("curatedrule_detection_to_process", curatedrule_detection_to_process)
        ...
        pending_curatedrule_id_with_alert_state = last_run.get(
    ...
    if not last_run.get("curatedrule_first_fetched_time"):
        demisto.info(
            "[CHRONICLE CURATED RULE DETECTIONS] Starting new time window from START-TIME :"
    ...
    # giving priority to comma separated detection ids over check box of fetch all live detections
    if (
        not pending_curatedrule_id_with_alert_state.get("rule_id")
        and not curatedrule_detection_to_pull
        and not curatedrule_detection_to_process
        and not simple_backoff_curatedrules
    ):
        fetch_detection_by_ids = params.get("fetch_detection_by_ids") or ""
        ...
        pending_curatedrule_id_with_alert_state.update(
            {
                "rule_id": fetch_detection_by_ids,
                "alert_state": fetch_detection_by_alert_state,
                "listBasis": fetch_detection_by_list_basis,
    ...
    (
        ...
        curatedrule_detection_to_process,
        curatedrule_detection_to_pull,
        pending_curatedrule_id_with_alert_state.get("rule_id", ""),
        pending_curatedrule_id_with_alert_state.get("alert_state", ""),
        simple_backoff_curatedrules,
    ...
    if pending_curatedrule_id or curatedrule_detection_to_pull or simple_backoff_curatedrules:
        ...
        demisto.info(
            "[CHRONICLE CURATED RULE DETECTIONS] End of current time window from START-TIME :"
            f" {start_time} to END_TIME : {end_time}"
        )

    pending_curatedrule_id_with_alert_state.update(
        {
            "rule_id": pending_curatedrule_id,
            "alert_state": fetch_detection_by_alert_state,
            "listBasis": fetch_detection_by_list_basis,
        }
    )
...
def get_user_alerts(client_obj, start_time, end_time, max_fetch):
    ...
    for user_alert in json_response.get("userAlerts", []):
        ...
        asset_alert = {"User": user_identifier[0] if user_identifier else "", "AlertCounts": len(infos), "AlertInfo": infos}
        alerts.append(asset_alert)
    return alerts
...
def get_user_alert_hr_and_ec(client_obj: Client, start_time: str, end_time: str, page_size: str):
    ...
    hr = tableToMarkdown(
        "User Alert(s)", data, ["Alerts", "User", ALERT_NAMES, FIRST_SEEN, LAST_SEEN, "Sources"], removeNull=True
    )
    ...
    ec = {CHRONICLE_OUTPUT_PATHS["UserAlert"]: alerts}
...
def get_rules(client_obj, args: dict[str, str]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if live_rule:
        ...
        json_data = {"rules": list_live_rule}
    raw_resp = deepcopy(json_data)
...
def create_rule(client_obj, rule_text: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    """
    req_json_data = {"ruleText": rule_text}
    ...
    ec = {CHRONICLE_OUTPUT_PATHS["Rules"]: json_data}
...
def delete_rule(client_obj, rule_id: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    ec = {CHRONICLE_OUTPUT_PATHS["DeleteRule"]: json_data}
...
def gcb_change_rule_alerting_status(client_obj, rule_id, alerting_status):
    ...
    json_data = {"ruleId": rule_id, "actionStatus": "SUCCESS" if not json_data else "FAILURE", "alertingStatus": alerting_status}
    ec = {CHRONICLE_OUTPUT_PATHS["RuleAlertingChange"]: json_data}
    return ec, json_data
...
def gcb_change_live_rule_status(client_obj, rule_id, live_rule_status):
    ...
    json_data = {"ruleId": rule_id, "actionStatus": "SUCCESS" if not json_data else "FAILED", "liveRuleStatus": live_rule_status}

    ec = {CHRONICLE_OUTPUT_PATHS["LiveRuleStatusChange"]: json_data}
    return ec, json_data
...
def gcb_start_retrohunt(client_obj, rule_id, start_time, end_time):
    ...
    json_data = validate_response(client_obj, request_url, method="POST", body=json.dumps(body))
    ec = {CHRONICLE_OUTPUT_PATHS["RetroHunt"]: json_data}
    return ec, json_data
...
def prepare_hr_for_gcb_start_retrohunt_command(json_data):
    ...
    hr = tableToMarkdown(
        "Retrohunt Details",
        hr_output,
        headers=["Retrohunt ID", "Rule ID", "Version ID", "Event Start Time", "Event End Time", "Retrohunt Start Time", "State"],
        removeNull=True,
    )
    return hr
...
def gcb_list_retrohunts(client_obj, rule_id, retrohunts_for_all_versions, state, page_size, page_token):
    ...
    json_data = validate_response(client_obj, request_url)
    ec = {CHRONICLE_OUTPUT_PATHS["RetroHunt"]: json_data.get("retrohunts")}
    return ec, json_data
...
def gcb_create_reference_list(client_obj, name, description, lines, content_type):
    ...
    ec = {CHRONICLE_OUTPUT_PATHS["ReferenceList"]: json_data}
    return ec, json_data
...
def gcb_list_reference_list(client_obj, page_size, page_token, view):
    ...
    ec = {CHRONICLE_OUTPUT_PATHS["ListReferenceList"]: references_list}
    return ec, json_data
...
def gcb_verify_reference_list(client_obj, lines, content_type):
    ...
    content_type = "CONTENT_TYPE_DEFAULT_STRING" if content_type == DEFAULT_CONTENT_TYPE else content_type
    body = {"lines": lines, "content_type": content_type}
    json_data = validate_response(client_obj, request_url, method="POST", body=json.dumps(body))
    ...
    ec = {CHRONICLE_OUTPUT_PATHS["VerifyReferenceList"]: json_data}
    return ec, json_data
...
def gcb_test_rule_stream(client_obj, rule_text, start_time, end_time, max_results):
    ...
    ec = {CHRONICLE_OUTPUT_PATHS["StreamRules"]: {"list": json_data}}
    return ec, json_data


def gcb_list_asset_aliases(
    client_obj: Client,
    start_time: str,
    end_time: str,
    page_size: Optional[int],
    asset_identifier_type: Optional[str],
    ...
    """
    request_url = (
        f"{BACKSTORY_API_V1_URL}/alias/listassetaliases?asset.{asset_identifier_type}={asset_identifier}"
    ...
    ec = {
        CHRONICLE_OUTPUT_PATHS["AssetAliases"]: {
            "asset": {asset_identifier_type: urllib.parse.unquote(asset_identifier), "aliases": json_data.get("aliases")}
        }
    }
    return ec, json_data


def gcb_list_curated_rules(
    client_obj: Client, page_token: str, page_size: Optional[int]
) -> tuple[dict[str, Any], dict[str, Any]]:
    """
    ...
    # context data for the command
    ec = {CHRONICLE_OUTPUT_PATHS["CuratedRules"]: json_data.get("curatedRules", [])}
...
def gcb_list_user_aliases(
    client_obj: Client,
    start_time: str,
    end_time: str,
    page_size: Optional[int],
    user_identifier_type: Optional[str],
    user_identifier: str,
) -> tuple[dict[str, Any], dict[str, Any]]:
    """
    ...
    """
    request_url = (
        f"{BACKSTORY_API_V1_URL}/alias/listuseraliases?user.{user_identifier_type}={user_identifier}"
        f"&start_time={start_time}&end_time={end_time}&page_size={page_size}"
    )
    json_data = validate_response(client_obj, request_url, method="GET")
    ...
    ec = {
        CHRONICLE_OUTPUT_PATHS["UserAliases"]: {
            "user": {user_identifier_type: urllib.parse.unquote(user_identifier), "aliases": json_data.get("userAliases")}
        }
    }
    return ec, json_data
...
def gcb_assets_command(client_obj, args: dict[str, str]):
    ...
    request_url = (
        f"{BACKSTORY_API_V1_URL}/artifact/listassets?artifact.{artifact_type}={urllib.parse.quote(artifact_value)}"
        f"&start_time={start_time}&end_time={end_time}&page_size={page_size}"
    )
    ...
    if response and response.get("assets"):
        context_data, tabular_data, host_context = parse_assets_response(response, artifact_type, artifact_value)
        hr = tableToMarkdown(
            f"Artifact Accessed - {artifact_value}",
            tabular_data,
            ["Host Name", "Host IP", "Host MAC", FIRST_ACCESSED_TIME, LAST_ACCESSED_TIME],
        )
        hr += "[View assets in Chronicle]({})".format(response.get("uri", [""])[0])
        ec = {"Host": host_context, **context_data}
    else:
...
def gcb_ioc_details_command(client_obj, args: dict[str, str]):
    ...
    if response and response.get("sources"):
        ...
        context_dict = get_context_for_ioc_details(
            response.get("sources", []), artifact_value, normal_artifact_type, is_reputation_command=False
        )
        ec = {
...
def fetch_incidents(client_obj, params: dict[str, Any]):
    ...
    if backstory_alert_type == "assets with alerts":
        ...
        incidents = fetch_incidents_curatedrule_detection_alerts(client_obj, params, start_time, end_time, time_window, max_fetch)
    else:
        last_run = demisto.getLastRun()
        if last_run:
            start_time = last_run.get("start_time") or start_time
        ...
        for alert in alerts:
            for alert_info in alert.get("AlertInfo", []):
                if "Uri" in alert_info:
                    del alert_info["Uri"]
        ec = {CHRONICLE_OUTPUT_PATHS["Alert"]: alerts}
...
def gcb_list_events_command(client_obj, args: dict[str, str]):
    ...
    # Make a request URL
    request_url = (
        f"{BACKSTORY_API_V1_URL}/asset/listevents?asset.{asset_identifier_type}={asset_identifier}"
    ...
    ec = {CHRONICLE_OUTPUT_PATHS["Events"]: parsed_ec}
...
def gcb_udm_search_command(client_obj, args: dict[str, str]):
    ...
    # Make a request URL
    request_url = (
        f"{BACKSTORY_API_V1_URL}/events:udmSearch?time_range.start_time={start_time}&time_range.end_time={end_time}"
    ...
    ec = {CHRONICLE_OUTPUT_PATHS["UDMEvents"]: parsed_ec}
...
def gcb_list_curatedrule_detections_command(client_obj, args: dict[str, str]):
    ...
    hr += (
        ...
        "detection in Chronicle click on its respective Detection ID.".format(detections[0].get("detection")[0].get("ruleName"))
    )
...
def gcb_list_retrohunts_command(client_obj, args):
    ...
    ec, json_data = gcb_list_retrohunts(
        client_obj,
        valid_args.get("rule_id"),
        valid_args.get("retrohunts_for_all_versions"),
...
def gcb_create_reference_list_command(client_obj, args):
    ...
    ec, json_data = gcb_create_reference_list(
...
def gcb_verify_value_in_reference_list_command(client_obj, args):
    ...
    hr = tableToMarkdown(
        ...
        headerTransform=header_transform_to_title_case,
        removeNull=True,
    )
    ec = {CHRONICLE_OUTPUT_PATHS["VerifyValueInReferenceList"]: ec_data}
...
def gcb_list_asset_aliases_command(client_obj: Client, args: dict[str, Any]) -> tuple[str, dict[str, Any], dict[str, Any]]:
    """
    ...
    ec, json_data = gcb_list_asset_aliases(
        client_obj,
...
def gcb_list_curated_rules_command(client_obj: Client, args: dict[str, Any]) -> tuple[str, dict[str, Any], dict[str, Any]]:
    """
...
def gcb_list_user_aliases_command(client_obj: Client, args: dict[str, Any]) -> tuple[str, dict[str, Any], dict[str, Any]]:
    """
    ...
    ec, json_data = gcb_list_user_aliases(
        client_obj,
        start_time=start_time,
        end_time=end_time,
        page_size=page_size,
