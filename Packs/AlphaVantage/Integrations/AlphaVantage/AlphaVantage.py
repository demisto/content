...
class Client(BaseClient):
    ...
    def get_stock_history(self, symbol: str, interval: str, output_size: str = "compact") -> dict:
...
def remove_indexing_from_dictionary_keys(api_response: dict, regex: re.Pattern = re.compile(r"^[0-9][0-9]*\.\s*")) -> dict:
...
def get_stock_history(client: Client, args: Dict[str, Any]) -> List[CommandResults]:
    ...
    for symbol in symbols:
        ...
        results.append(
            CommandResults(
                readable_output=tableToMarkdown(f"Stock History (Interval: {interval})", outputs.get("Time Series", {})),
