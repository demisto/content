...
ALERT_TYPE_TO_INCIDENT_SEVERITY = {"Alert": 1, "Urgent": 2, "Flash": 3}
...
class DataminrPulseClient(BaseClient):
    ...
    def get_related_alerts(self, alert_id: str, include_root: bool) -> Optional[List]:
        ...
        :rtype: ``Optional[List]``
        """
        params = {"alertversion": ALERT_VERSION, "id": alert_id, "includeRoot": include_root}
        return self.http_request(method="GET", url_suffix=ENDPOINTS["RELATED_ALERTS_ENDPOINT"], params=params)  # type: ignore
...
def validate_params_for_alerts_get(
    ...
    if not watchlist_ids and not query:
        if use_configured_watchlist_names:
            ...
            raise ValueError(ERRORS["AT_LEAST_ONE_REQUIRED"].format("query", "watchlist_names configured in integration"))
        raise ValueError(ERRORS["AT_LEAST_ONE_REQUIRED"].format("query", "watchlist_ids"))
    if _from and to:
...
def fetch_incidents(
    ...
    validate_params_for_alerts_get(
        ...
        to=None,
        num=num,
        use_configured_watchlist_names=True,
        is_fetch=True,
    )  # type: ignore

    response = client.get_alerts(watchlist_ids=watchlist_ids, query=query, _from=_from, to=None, num=num)  # type: ignore
...
def main():
    ...
    try:
        ...
        client = DataminrPulseClient(client_id=client_id, client_secret=client_secret, proxy=proxy, verify=verify_certificate)
