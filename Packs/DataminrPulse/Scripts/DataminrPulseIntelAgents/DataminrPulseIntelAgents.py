...
def create_intel_agents_html(intel_agents_summary: list, event_corroboration_summary: list, alert_url: str) -> str:
    ...
    for summary in intel_agents_summary:
        ...
        elif "PHYSICAL" in summary.get("type", []):
            physical_intel_agent_summary.append(summary)

    if not cyber_intel_agent_summary and not physical_intel_agent_summary:
        return ""

    html_content = []

    if cyber_intel_agent_summary:
        html_content.extend(['<h4 style="margin:0; padding:0;">Cyber Context</h4>', '<div style="margin:10px 0;">'])
    ...
    if physical_intel_agent_summary:
        html_content.extend(['<h4 style="margin:0; padding:0;">Physical Context</h4>', '<div style="margin:10px 0;">'])
...
def main():
    try:
        ...
        intel_agents_data = create_intel_agents_html(intel_agents_summary, event_corroboration_summary, alert_url)

        if not intel_agents_data:
            return_results(
                {
                    "ContentsFormat": EntryFormat.HTML,
                    "Type": EntryType.NOTE,
