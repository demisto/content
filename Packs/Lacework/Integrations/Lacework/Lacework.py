...
try:
    if LACEWORK_SUBACCOUNT:
        lw_client = LaceworkClient(
            account=LACEWORK_ACCOUNT, subaccount=LACEWORK_SUBACCOUNT, api_key=LACEWORK_API_KEY, api_secret=LACEWORK_API_SECRET
        )
    else:
        lw_client = LaceworkClient(account=LACEWORK_ACCOUNT, api_key=LACEWORK_API_KEY, api_secret=LACEWORK_API_SECRET)
...
def create_search_json(start_time, end_time, filters, returns, time_delta=None):
    ...
    json_request["timeFilter"] = {"startTime": start_time, "endTime": end_time}
...
def format_compliance_data(compliance_data, rec_id):
    ...
    if len(compliance_data["data"]) > 0:
        ...
        readable_output = tableToMarkdown("Compliance Summary", compliance_data["summary"])
        ...
        return {"Type": entryTypes["error"], "ContentsFormat": formats["text"], "Contents": "No compliance data was returned."}
...
def get_gcp_projects_by_organization():
    ...
    create_entry("Google Cloud Platform Projects for Organization " + str(organization_id), response["data"], ec)
...
def get_container_vulnerabilities():
    ...
    json_request = create_search_json(start_time=start_time, end_time=end_time, filters=filters, returns=returns)

    try:
        response = lw_client.vulnerabilities.containers.search(json=json_request)
    ...
    return create_entry("Lacework Vulnerability Data for Containers", response_data, ec)
...
def get_host_vulnerabilities():
    ...
    try:
        response = lw_client.vulnerabilities.hosts.search(json=json_request)
    ...
    return create_entry("Lacework Vulnerability Data for Hosts", response_data, ec)
...
def get_alert_details():
    ...
    return create_entry("Lacework Alert " + str(alert_id), response["data"], ec)
...
def fetch_incidents():
    ...
    demisto.setLastRun({"max_alert_id": max_alert_id})
