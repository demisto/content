...
def parse_error_response(http_response):
    try:
        ...
        return (
            f"Error in API call. Recived status code: {http_response.status_code};"
...
def http(method, url_suffix, params=None, data=None, files=None):
    try:
        response = requests.request(
            method, BASE_URL + "/api/v1/" + API_KEY + url_suffix, verify=USE_SSL, params=params, data=data, files=files
        )
