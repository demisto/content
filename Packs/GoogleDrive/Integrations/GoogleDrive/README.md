...
***
Creates a new file or folder. By default the file is created metadata-only (0 bytes). Supply the content argument to write text into the file, for example a tombstone message left in place of a quarantined file.
...
| content | Text content to write into the new file. Cannot be used when mime_type is "application/vnd.google-apps.folder", since folders hold no content. When omitted the file is created empty. | Optional |
| supports_all_drives | Whether the requesting application supports both My Drives and shared drives. Possible values are: true, false. Default is false. | Optional |
