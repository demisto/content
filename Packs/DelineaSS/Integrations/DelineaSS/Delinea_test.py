...
from test_data.context import GET_PASSWORD_BY_ID_CONTEXT, GET_USERNAME_BY_ID_CONTENT, \
    ...
    USER_DELETE_CONTEXT, SECRET_CREATE_CONTEXT, USER_CREATE_CONTEXT,USER_UPDATE_CONTEXT, \
...
from test_data.http_responses import GET_PASSWORD_BY_ID_RAW_RESPONSE, GET_USERNAME_BY_ID_RAW_RESPONSE, \
    ...
    FOLDER_UPDATE_RAW_RESPONSE, USER_DELETE_RAW_RESPONSE, SECRET_CREATE_RAW_RESPONSE, \
    USER_CREATE_RAW_RESPONSE ,USER_UPDATE_RAW_RESPONSE,  SECRET_RPC_CHANGE_PASSWORD_RAW_RESPONSE
...
USER_CREATE_ARGS= {"displayName": "xsoarUserTest1", "password": "1234567890", "userName":"XSOAR", "emailAddress": "ff@ff.com"}
USER_DELETE_ARGS= {"id": "10"}
SECRET_CREATE_ARGS= {"name" :"xsoarSecret", "secrettemplateid": "6003", "siteid" :"1" ,"checkoutenabled" : "true", "folderid":"3", "machine_item": "my-machine", "username_item" :"my-username", "password_item":"XXXXXX@@@@@####"}
...
@pytest.mark.parametrize('command, args, http_response, context', [
    ...
    (secret_rpc_changepassword_command,SECRET_RPC_CHANGE_PASSWORD_ARGS, SECRET_RPC_CHANGE_PASSWORD_RAW_RESPONSE, SECRET_RPC_CHANGE_PASSWORD_CONTEXT )
])

def test_delinea_commands(command, args, http_response, context, mocker):

    mocker.patch.object(Client, '_generate_token')
    client = Client(server_url="https://test.secretservercloud.com", username="test", password="test123",
