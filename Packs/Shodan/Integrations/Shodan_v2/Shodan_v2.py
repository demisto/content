...
def http_request(method, uri, params=None, data=None, headers=None):
    ...
    url = f"{API_URL}{uri}"
    res = requests.request(method, url, params=params, data=data, headers=headers, verify=USE_SSL)
...
def alert_to_demisto_result(alert):
    ec = {"Shodan": {"Alert": {"ID": alert.get("id", ""), "Expires": alert.get("expires", 0)}}}

    human_readable = tableToMarkdown(
        f'Alert ID {ec["Shodan"]["Alert"]["ID"]}',
        {
            "Name": alert.get("name", ""),
...
def get_scan_status(scan_id):
    ...
    ec = {"Shodan": {"Scan": {"ID": res.get("id", ""), "Status": res.get("status", "")}}}

    human_readable = tableToMarkdown(
        f"Scanning results for scan {scan_id}", {"ID": ec["Shodan"]["Scan"]["ID"], "Status": ec["Shodan"]["Scan"]["Status"]}
    )
...
def search_command():
    ...
    return_json = argToBoolean(demisto.args().get("return_json", "No"))
        ...
        ec = {
            "Shodan": {
                "Banner": {
                    ...
                    "Ssl": {"versions": match.get("ssl", {"versions": []}).get("versions", [])},
...
def ip_command():
    ...
    for ip in ips:
        ...
        if not res:
            ...
            readable_output = tableToMarkdown(name=title, t=human_readable, removeNull=True)
...
def shodan_search_count_command():
    ...
    ec = {"Shodan": {"Search": {"ResultCount": res.get("total", 0)}}}
...
def shodan_scan_internet_command():
    ...
    res = http_request("POST", "/shodan/scan/internet", data={"port": port, "protocol": protocol})

    ec = {"Shodan": {"Scan": {"ID": res.get("id", "")}}}

    human_readable = tableToMarkdown(
        f"Intenet scanning results for port {port} and protocol {protocol}",
        {
...
def shodan_create_network_alert_command():
    ...
    res = http_request(
        "POST",
        "/shodan/alert",
        data=json.dumps({"name": alert_name, "filters": {"ip": ip}, "expires": expires}),
