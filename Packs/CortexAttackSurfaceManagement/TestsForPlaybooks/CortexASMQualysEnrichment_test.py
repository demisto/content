...
def playbook_tasks_data():
    ...
    playbook_data = helper.load_yaml_file('./Packs/CortexAttackSurfaceManagement/Playbooks/Cortex_ASM_-_Qualys_Enrichment.yml')
...
def test_playbook_name_and_id(full_playbook_data):
    assert full_playbook_data.get("name") == "Cortex ASM - Qualys Enrichment"
    assert full_playbook_data.get("id") == "Cortex ASM - Qualys Enrichment"


@pytest.mark.parametrize('asm_system_id_types', ["QUALYS-ASSET-ID"])
