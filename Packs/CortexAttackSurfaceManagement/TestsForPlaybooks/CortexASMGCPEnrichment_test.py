...
def playbook_tasks_data():
    ...
    playbook_data = helper.load_yaml_file('./Packs/CortexAttackSurfaceManagement/Playbooks/Cortex_ASM_-_GCP_Enrichment.yml')
...
@pytest.mark.parametrize('asm_system_id_types', ["ASSET-ID", "ASSET-NAME", "ASSET-SG", "ASSET-VIRTUAL-NET", "ASSET-SUBNET-NAME", "ASSET-NIC", "ASSET-ZONE"])
...
@pytest.mark.parametrize('grid_field_data', [({"gridfield": "asmsystemids", "val1": "ASSET-TYPE", "val2": "Google Compute Engine"})])
def test_asset_type_of_asmsytemid(playbook_tasks_data, grid_field_data):
    data_found = helper.check_multiple_grid_field_values(playbook_tasks_data, grid_field_data)
    assert data_found
