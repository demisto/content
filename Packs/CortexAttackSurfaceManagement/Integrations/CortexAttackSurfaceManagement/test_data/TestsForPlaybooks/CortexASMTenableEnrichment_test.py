...
def playbook_data_loader():
    ...
    playbook_path = base_path / "Playbooks" / "Cortex_ASM_-_Tenable.io_Enrichment.yml"
    print(playbook_path)
...
def test_expected_playbook_name_and_id(playbook_data_loader: PlaybookDataLoader):
    ...
    assert full_data.get("name") == "Cortex ASM - Tenable.io Enrichment"
    assert full_data.get("id") == "Cortex ASM - Tenable.io Enrichment"


@pytest.mark.parametrize("asm_system_id_type", ["TENABLE-ASSET-ID"])
