...
| AcceptedRiskDs | Comma-separated list of instance/VM IDs that are considered an acceptable risk and that should be closed. |  | Optional |
| AcceptedRiskProjects | Comma-separated list of projects numbers that are considered an acceptable risk and that should be closed.  For example, it could be a list of GCP projects and AWS accounts. |  | Optional |
| AcceptedRiskOther | Comma-separated list of other items that are considered an acceptable risk and that should be closed. For example, it could be a list of folder numbers in GCP and subscription IDs in Azure. |  | Optional |
