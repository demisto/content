...
@pytest.mark.parametrize(
    "service_account, expected_out, expected_raises",
    [
        ...
        (
            "person@example.com",
            None,
            pytest.raises(ValueError, match="Could not extract project name from service account person@example.com"),
        ),
    ],
)
...
@pytest.mark.parametrize(
    "results, expected_out, expected_raises",
    [
        (
            TEST_IAM_POLICY,
            [
        ...
        ([], None, pytest.raises(ValueError, match="Error getting project owners from IAM policy")),
    ],
)
...
@pytest.mark.parametrize(
    "owners, external_service, expected_out",
    [
        (
            [],
            "Amazon Web Services",
...
def test_main(mocker, owners, external_service, expected_out):
    ...
    mocker.patch("GetProjectOwners.get_iam_policy", return_value=TEST_IAM_POLICY)
