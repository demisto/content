...
@pytest.fixture()
def client():
    return MD_Sandbox.Client(base_url="https://test.com", api_key="mockkey", proxy=False, verify=False)
...
def test_scan_command_url_polling(mocker, client):
    ...
    assert response[0].indicator.dbot_score.integration_name == "MetaDefender Sandbox"
...
def test_scan_command_file_polling(mocker, client):
    ...
    assert response[1].indicator.dbot_score.integration_name == "MetaDefender Sandbox"
    ...
    assert response[2].indicator.dbot_score.integration_name == "MetaDefender Sandbox"
...
def test_scan_command_file_polling_no_threat(mocker, client):
    ...
    assert response[0].indicator.dbot_score.indicator == "b280719e9f2dd010260e6a023e0d69c64fbee8b6cbb8669c722a1da8142d3325"
    assert response[0].indicator.dbot_score.score == 1
