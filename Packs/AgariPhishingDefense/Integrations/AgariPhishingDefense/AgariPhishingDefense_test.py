...
def test_http_proxy_error(mocker_get_api_token, mock_base_http_request, demisto_version, client):
    ...
    # Assert
    assert (
        str(e.value) == "Proxy Error - cannot connect to proxy. Either try clearing the 'Use system proxy'"
...
def test_http_ssl_error(mocker_get_api_token, mock_base_http_request, demisto_version, client):
    ...
    # Assert
    assert (
        str(e.value) == "SSL Certificate Verification Failed - try selecting 'Trust any certificate' checkbox "
...
def test_validate_fetch_policy_action(mocker):
    ...
    assert (
        str(e.value) == 'The given value for Policy Actions is invalid. Expected "deliver", "mark-spam", "move", '
