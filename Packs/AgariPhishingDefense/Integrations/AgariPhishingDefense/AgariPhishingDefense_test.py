...
@patch('AgariPhishingDefense.Client.get_api_token')
def test_http_request_invalid_schema_error(
    mocker_api_token, mock_base_http_request, demisto_version, client
):
    """
...
@patch('AgariPhishingDefense.BaseClient._http_request')
@patch('AgariPhishingDefense.Client.get_api_token')
def test_http_proxy_error(
    mocker_get_api_token, mock_base_http_request, demisto_version, client
):
    """
...
@patch('AgariPhishingDefense.BaseClient._http_request')
@patch('AgariPhishingDefense.Client.get_api_token')
def test_http_request_read_timeout_error(
    mocker_get_api_token, mock_base_http_request, demisto_version, client
):
    """
    When http request return connection error with Demisto exception then appropriate error message should match.
...
@patch('AgariPhishingDefense.BaseClient._http_request')
@patch('AgariPhishingDefense.Client.get_api_token')
def test_http_request_missing_schema_error(
    mocker_get_api_token, mock_base_http_request, demisto_version, client
):
    """
...
@patch('AgariPhishingDefense.BaseClient._http_request')
@patch('AgariPhishingDefense.Client.get_api_token')
def test_http_request_invalid_url_error(
    mocker_get_api_token, mock_base_http_request, demisto_version, client
):
    """
...
def test_main_success(
    mocker_get_api_token,
    mocker_demisto_debug,
    mocker_demisto_info,
    mocker_demisto_results,
    mocker,
):
    """
    ...
    mocker.patch.object(
        AgariPhishingDefense, 'test_function', return_value='ok'
    )
...
def test_strip_blank():
    ...
    assert AgariPhishingDefense.strip_blank(args) == actual_output
...
@patch('demistomock.debug')
@patch('AgariPhishingDefense.Client.get_api_token')
def test_main_called_strip_function(
    mock_api_token,
    mocker_demisto_debug,
    mocker_demisto_info,
    mocker_demisto_results,
    ...
    mocker.patch.object(demisto, 'params', return_value=PARAMS)
    mocker.patch.object(
        demisto, 'command', return_value='agari-list-policy-events'
    )
    mocker.patch.object(
        AgariPhishingDefense, 'list_policy_events_command', return_value='ok'
    )
    args = {'policy_name': ' SystemAlert ', 'limit': 4, 'url': SAMPLE_URL}
    mocker.patch.object(demisto, 'args', return_value=args)
    strip_blank = mocker.patch.object(AgariPhishingDefense, 'strip_blank')
    AgariPhishingDefense.main()
...
@patch('AgariPhishingDefense.Client.http_request')
@patch('AgariPhishingDefense.Client.get_api_token')
def test_get_events_command_no_record_failure(
    mock_api_token, mock_request, client
):
    """
...
def test_validate_fetch_policy_action(mocker):
    ...
    assert (
        str(e.value)
        == 'The given value for Policy Actions is invalid. Expected "deliver", "mark-spam", "move", '
...
def test_validate_exclude_alert_type(mocker):
    ...
    assert (
        str(e.value)
        == 'The given value for Exclude Alerts is invalid. Expected "System Alert" or "Message Alert". '
    )
...
def test_event_params_invalid_start_date():
    ...
    assert (
        str(e.value) == 'The given value for start_date argument is invalid.'
    )
...
@patch('AgariPhishingDefense.Client.http_request')
@patch('AgariPhishingDefense.Client.get_api_token')
def test_remediate_message_valid_id_passed(
    mock_api_token, http_request, client
):
    """
...
def test_remediate_when_action_is_wrong(client):
    ...
    args = {'id': '53daff5a-2ac4-11eb-9375-0a8f2da72108', 'action': ' '}
    ...
    assert (
        str(e.value)
        == 'Invalid argument value. Requires both "id" and "operation" argument.'
    )
...
def test_incident_params_invalid_fetch_limit():
    ...
    assert (
        str(e.value)
        == 'Value of Fetch Limit must be a positive integer between 1 to 200.'
    )
...
def test_incident_params_valid():
    ...
    assert (
        params['filter']
        == 'created_at.gt(2020-11-22T12:59:59Z) and id.gt(+544635182)'
    )


@patch('AgariPhishingDefense.Client.http_request')
@patch('AgariPhishingDefense.Client.get_api_token')
def test_fetch_incidents_command_no_record_failure(
    mock_api_token, mock_request, client
):
    """
...
@patch('AgariPhishingDefense.Client.http_request')
@patch('AgariPhishingDefense.Client.get_api_token')
def test_list_message_data_command_success(
    mock_api_token, mock_request, client
):
    """
    When agari-list-policy-events command executes successfully then context output and
