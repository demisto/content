...
class Client(BaseClient):
    ...
    def http_request(
        ...
        try:
            ...
            headers["User-Agent"] = (
                "AgariDemisto APDIntegration/" + INTEGRATION_VERSION + " DemistoServer/" + demisto.demistoVersion()["version"]
...
def fetch_incidents(
    ...
    assert latest_created_time_date is not None, f"could not parse date {first_fetch}"
    ...
    assert latest_created_time_date is not None, f"failed parsing created_at date: {total_records[-1]['created_at']}"
    latest_created_time_date_str = latest_created_time_date.strftime(DATE_FORMAT)
    next_run = {
...
def main() -> None:
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            next_run, incidents = fetch_incidents(client, demisto.getLastRun(), demisto.params())
