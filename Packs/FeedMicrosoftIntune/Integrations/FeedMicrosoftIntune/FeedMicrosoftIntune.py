...
class Client(BaseClient):
    ...
    def build_iterator(self) -> list:
        ...
        for ipv4 in ipv4s:
            result.append({"value": ipv4, "type": FeedIndicatorType.IP, "FeedURL": self._base_url})
        for cidr in ipv4cidrs:
            result.append({"value": cidr, "type": FeedIndicatorType.CIDR, "FeedURL": self._base_url})
...
def fetch_indicators(client: Client, feed_tags: list = [], limit: int = -1) -> list[dict]:
    ...
    for item in iterator:
        ...
        indicator_obj = {"value": value, "type": type_, "rawJSON": raw_data, "fields": {}}
        if feed_tags:
...
def get_indicators_command(
    ...
    human_readable = tableToMarkdown(
        "Indicators from Microsoft Intune Feed:", indicators, headers=["value", "type"], removeNull=True
    )
...
def main():
    ...
    try:
        client = Client(base_url=base_url, verify=insecure, proxy=proxy, tlp_color=tlp_color)
