...
class Client(BaseClient):
    ...
    def all_raw_data(self) -> requests.Response:
        ...
        try:
            return self._http_request(method="GET", url_suffix="", resp_type="response")
...
def get_indicators_command(client: Client, **args) -> CommandResults:
    ...
    for value in indicators:
        ...
        indicators_and_type = {"value": value, "type": type_}
        final_indicators_lst.append(indicators_and_type)

    md = tableToMarkdown("Indicators from Webex:", final_indicators_lst, headers=["value", "type"], removeNull=True)
...
def fetch_indicators_command(
    client: Client, tags: tuple = None, tlp_color: str = None, enrichment_excluded: bool = False
) -> list:
