...
def main():
    ...
    if additional_populate_fields is not None and additional_populate_fields.strip() != "":
        get_incidents_by_query_args["populateFields"] = ",".join(
            [get_incidents_by_query_args["populateFields"], additional_populate_fields.strip()]
        )
    ...
    args = {
        "emailSubject": email_subject_list,
    ...
    file_columns = ["id", tag_field_name, "Prediction", "Probability", "Error"]
    if additional_populate_fields is not None and additional_populate_fields.strip() != "":
