...
switch (command) {
    ...
    case 'test-module':
        res = sendRequest(initialurl + '8.8.8.8' + '?apikey='+(params.apikey || params.apikey_creds.password));
        if (res.query_result === 'Success') {
            return 'ok';
        }
        return res;
    case 'packetmail-ip':
        var res =  sendRequest(initialurl + args.ip + '?apikey='+(params.apikey || params.apikey_creds.password));
        md = '';
        maliciousLists = '';
