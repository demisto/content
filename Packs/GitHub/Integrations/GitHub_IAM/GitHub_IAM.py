...
class Client(BaseClient):
    ...
    def disable_user(self, data):
        ...
        return self._http_request(method="DELETE", url_suffix=uri, resp_type="text")
...
def get_user_command(client, args, mapper_in, mapper_out):
    ...
    try:
        ...
        if res.get("totalResults", 0) == 0:
            ...
            email_result = iam_user_profile.get_attribute("emails", user_profile_data=iam_user_profile.mapped_user_profile)
        ...
        iam_user_profile.set_result(success=False, error_code=error_code, error_message=error_message, action=IAMActions.GET_USER)
        return iam_user_profile
...
def create_user_command(client, args, mapper_out, is_create_enabled, is_update_enabled):
    try:
        ...
        if not is_create_enabled:
            ...
            if get_user_response.get("totalResults", 0) > 0:
                ...
                iam_user_profile = update_user_command(
                    client, args, mapper_out, is_update_enabled, is_create_enabled, create_if_not_exists
                )

            else:
                github_user = iam_user_profile.map_object(
        ...
        error_code, error_message = github_handle_error(e)
        iam_user_profile.set_result(
            success=False, error_code=error_code, error_message=error_message, action=IAMActions.CREATE_USER
        )
        return iam_user_profile
...
def update_user_command(client, args, mapper_out, is_update_enabled, is_create_enabled, create_if_not_exists):
    try:
        ...
        error_code, error_message = github_handle_error(e)
        iam_user_profile.set_result(
            success=False, error_code=error_code, error_message=error_message, action=IAMActions.UPDATE_USER
        )
        return iam_user_profile
...
def disable_user_command(client, args, mapper_out, is_disable_enabled):
    try:
        ...
        error_code, error_message = github_handle_error(e)
        iam_user_profile.set_result(
            success=False, error_code=error_code, error_message=error_message, action=IAMActions.DISABLE_USER
        )
        return iam_user_profile
...
def main():  # pragma: no cover
    ...
    try:
        client = Client(
            base_url=base_url, org=org, verify=verify_certificate, proxy=proxy, headers=headers, ok_codes=(200, 201, 204)
        )
