...
def format_team_member_outputs(member: dict = {}) -> dict:
    """Take GitHub API team data and format to expected context outputs

    Args:
        member (dict): team member data returned from GitHub API

    Returns:
        (dict): team member object formatted to expected context outputs
    """
    ec_member = {
        'ID': member.get('id'),
        'NodeID': member.get('node_id'),
        'Name': member.get('login'),
        'Type': member.get('type'),
        'SiteAdmin': member.get('site_admin')
    }
    return ec_member
...
def list_team_members(organization: str, team_name: str) -> list:
    suffix = f'/orgs/{organization}/teams/{team_name}/members'
    response = http_request('GET', url_suffix=suffix)
    return response
...
def list_team_members_command():
    ...
    response = list_team_members(organization, team_name)

    ec_object = [format_team_member_outputs(member) for member in response]
    ec = {
        'GitHub.Team.Member(val.ID === obj.ID)': ec_object
    }

    # human_readable = tableToMarkdown(f'Teams for Organization "{organization}" and Slug "{team_name}"', response, removeNull=True)
    human_readable = tableToMarkdown(
        f'Teams for Organization "{organization}" and Slug "{team_name}"', ec_object, removeNull=True)
    # return_outputs(readable_output=human_readable, raw_response=response)
    return_outputs(readable_output=human_readable, outputs=ec, raw_response=response)
...
def list_all_projects_command():
    ...
    for proj in projects:
        if project_filter:
            ...
            projects_obj[proj["name"]] = get_project_details(project=proj, header=header)
    demisto.results(projects_obj)
    # return_results(Contents=projects_obj,ContentsFormat=formats["json"],EntryContext=projects_obj)
    ...
    demisto.results("results returned!")
