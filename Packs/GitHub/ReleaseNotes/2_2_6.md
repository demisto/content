...
- Added a new command **_github-get-workflow-run_** to retrieve details of a specific workflow run by its ID. Outputs are stored in `GitHub.WorkflowRun` with fields: `ID`, `Name`, `HeadBranch`, `HeadSha`, `DisplayTitle`, `RunNumber`, `Event`, `Status`, `Conclusion`, `WorkflowID`, `CreatedAt`, `UpdatedAt`, `Url`, and `HtmlUrl`.
