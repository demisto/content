...
def http_request(method, url_suffix, params_dict=None, data=None):
    ...
    try:
        res = requests.request(method, url, verify=USE_SSL, params=params_dict, headers=DEFAULT_HEADERS, data=data)
...
def create_new_incident_event(
    event_type="ALERT", summary="", details="No description", incident_key=None, priority=None, integrationKey=INTEGRATION_KEY
):
...
def submit_new_event_command(
    event_type="ALERT", summary="", details="No description", incident_key=None, priority=None, integrationKey=INTEGRATION_KEY
):
    """Create new incident."""

    create_new_incident_event(event_type, summary, details, incident_key, priority, integrationKey)
...
def submit_acknowledge_event_command(summary, incident_key=None, integrationKey=INTEGRATION_KEY):
    ...
    create_new_incident_event(event_type="ACCEPT", summary=summary, incident_key=incident_key, integrationKey=integrationKey)
...
def submit_resolve_event_command(summary, incident_key=None, integrationKey=INTEGRATION_KEY):
    ...
    create_new_incident_event(event_type="RESOLVE", summary=summary, incident_key=incident_key, integrationKey=integrationKey)
