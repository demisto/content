...
class Client(BaseClient):
    ...
    def remediate(self, report_id: str, email: str, requester_fullname: str):
        ...
        headers = {"Content-Type": "application/json", "Accept": "application/json", "Authorization": str(self.token)}

        payload = {"report_id": report_id, "requester_email": email, "requester_fullname": requester_fullname}
    ...
    def post_comment(self, comment: str, report_id: str, tenant_id: str, assigned: bool = True, parent_id=None):
        ...
        if parent_id:
            ...
            payload = {"content": comment, "discussion_id": f"{report_id}:{tenant_id}", "assigned": assigned}
    ...
    def get_report_pdf(self, report_id: str):
        ...
        headers = {"Accept": "application/pdf, application/json", "Authorization": str(self.token)}
        try:
...
def fetch_incidents(client: Client, first_fetch: bool, last_run: str | None, first_fetch_interval: int) -> list[dict[str, Any]]:
    if first_fetch:
...
def get_report_by_id_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    try:
        ...
        if not result:
            ...
            return CommandResults(readable_output="No result found")
        ...
        _set_context(client)
        return CommandResults(readable_output=f"Error: {str(e)}")
...
def get_report_attachment_command(client: Client, args: dict[str, Any]) -> CommandResults | dict[str, Any]:
    ...
    try:
        ...
        return CommandResults(readable_output=f"Error downloading attachment: {e}")
...
def remediate_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    try:
        response, status_code = client.remediate(str(report_id), email=str(email), requester_fullname=str(requester_name))
        _set_context(client)
        returnMessage = ""
        if status_code == 200:
        ...
        _set_context(client)
        return CommandResults(readable_output=f"Error posting comment {e}")
...
def post_comment_command(client: Client, tenant_id: str, args: dict[str, Any]) -> CommandResults:
    ...
    try:
        ...
        _set_context(client)
        return CommandResults(readable_output=f"Comment added to report {report_id}: {comment} : STATUS: {status_code}")
    except Exception as e:
        _set_context(client)
        return CommandResults(readable_output=f"Error posting comment {e}")
...
def update_status_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    try:
        ...
        _set_context(client)
        return CommandResults(readable_output=f"Error Updating status {e}")
...
def get_report_pdf_command(client: Client, args: dict[str, Any]) -> CommandResults | dict[str, Any]:
    ...
    try:
        ...
        if not report_pdf:
            return CommandResults(readable_output=f"No report found with ID: {report_id}.")
        ...
        return CommandResults(readable_output=f"Error downloading the report: {str(e)}")
...
def main() -> None:
    ...
    try:
        ...
        client = Client(client_id=client_id, client_secret=client_secret, auth_token=auth_token, token_time=expiry)
