...
RECO_API_TIMEOUT = 30  # Increase timeout for RECO API
...
class RecoClient:
    ...
    def get_incidents_assets(self, incident_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            demisto.info(f"Validate API key ReadTimeout error: {str(e)}")
        except DemistoException as e:
            demisto.info(f"Validate API key Error: {str(e)}")
...
def enrich_incident(reco_client: RecoClient, single_incident: Dict[str, Any]) -> Dict[str, Any]:
    ...
    incident_id = alert_as_dict.get(RECO_INCIDENT_ID_FIELD, None)
...
def main() -> None:
    ...
    try:
        ...
        reco_client = RecoClient(client=client)
        if command == "fetch-incidents":
            demisto_args = demisto.args()
            risk_level = demisto_args.get("risk_level")
            source = demisto_args.get("source")
            before = demisto_args.get("before")
            after = demisto_args.get("after")
