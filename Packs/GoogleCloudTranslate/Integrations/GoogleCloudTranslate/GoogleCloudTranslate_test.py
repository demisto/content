...
MOCK_GET_SUPPORTED_LANGUAGES = [
    {"language_code": "aa", "support_source": True, "support_target": False},
    {"language_code": "bb", "support_source": False, "support_target": True},
]


MOCK_TRANSLATE_TEXT = {"translated_text": "foo", "detected_language_code": "aa"}
...
class MockClient:
    ...
    def translate_text(self, text, target, source=None):
        ...
        result.update({"detected_language_code": None})
...
def test_translate_text_1(mocker):
    ...
    readable, outputs, result = translate_text(mclient, {"text": "bar"})
    ...
    assert isinstance(readable, str)
    assert outputs == {"GoogleCloudTranslate.TranslateText(val.ID && val.ID==obj.ID)": mock_result}
...
def test_translate_text_2(mocker):
    ...
    readable, outputs, result = translate_text(mclient, {"text": "bar", "target": "it"})
...
def test_translate_text_3(mocker):
    ...
    readable, outputs, result = translate_text(mclient, {"text": "bar", "target": "it", "source": "hr"})
