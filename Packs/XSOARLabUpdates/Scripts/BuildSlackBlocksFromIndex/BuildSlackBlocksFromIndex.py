...
class SlackBlocks:
    ...
    def __init__(self, packs: list[IndexPack]):
        self.packs: list[IndexPack] = sorted(packs, key=lambda p: SUPPORT.index(p.support) if p.support in SUPPORT else 3)
    ...
    def build(self) -> str:
        if self.packs:
            ...
            for pack in self._preview_packs:
                ...
                blocks.append(self.build_pack_section_block(pack.name, pack.pan_dev_mp_id, pack.author, pack.description))
    ...
    def get_list_packs_header_block():
        return {"type": "section", "text": {"type": "mrkdwn", "text": "*More new packs that have been released this week:*\n"}}
    ...
    def build_header_block(self) -> dict:
        ...
        return {
            "type": "section",
            "text": {"type": "mrkdwn", "text": f"We have released *{len(self.packs)} New {pack_str}* this week! :dbot-new:"},
        }
    ...
    def build_pack_context_block(price: int, support: str) -> dict:
        ...
        return {
            "type": "context",
            "elements": [{"type": "mrkdwn", "text": SlackBlocks.get_price_text(price)}, {"type": "mrkdwn", "text": support_text}],
        }
    ...
    def build_list_packs_block(self) -> dict:
        ...
        return {"type": "section", "text": {"type": "mrkdwn", "text": packs_str}}
    ...
    return_results(
        [
            ...
            CommandResults(
                ...
                readable_output=tableToMarkdown(name="Last Run", t=updated_last_run, headers=["LastRun"]),
            ),
            CommandResults(
                outputs=blocks,
                outputs_prefix="Blocks",
                readable_output=tableToMarkdown(name="Slack blocks json", t=blocks, headers=["Blocks"]),
            ),
        ]
    )
