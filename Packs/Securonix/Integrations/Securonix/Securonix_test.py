...
from Securonix import (
    reformat_resource_groups_outputs,
    reformat_outputs,
    parse_data_arr,
    Client,
    list_workflows,
    get_default_assignee_for_workflow,
    ...
    get_watchlist,
    create_watchlist,
    check_entity_in_watchlist,
    add_entity_to_watchlist,
    get_incident_name,
    fetch_securonix_incident,
    fetch_securonix_threat,
    list_threats,
    get_incident_activity_history,
    list_whitelists,
    get_whitelist_entry,
    create_whitelist,
    ...
    create_xsoar_to_securonix_state_mapping,
    delete_lookup_table_entries,
    main,
    validate_mirroring_parameters,
    filter_attachment_activity_entries,
    list_activity_data,
)
...
from test_data.response_constants import (
    ...
    RESPONSE_CREATE_WATCHLIST,
    RESPONSE_ENTITY_IN_WATCHLIST,
    RESPONSE_ADD_ENTITY_TO_WATCHLIST,
    ...
    RESPONSE_DELETE_LOOKUP_TABLE_CONFIG_AND_DATA,
    ...
    RESPONSE_LOOKUP_TABLE_ENTRY_ADD,
    ...
    MIRROR_ENTRIES,
    RESPONSE_DELETE_LOOKUP_ENTRIES_DELETE,
...
from test_data.result_constants import (
    ...
    EXPECTED_GET_INCIDENT_WORKFLOW,
    EXPECTED_GET_INCIDENT_STATUS,
    EXPECTED_GET_INCIDENT_AVAILABLE_ACTIONS,
    EXPECTED_ADD_COMMENT_TO_INCIDENT,
...
def add_comment_to_incident_request_side_effect(*args, **kwargs):
    ...
    assert (
        kwargs["comment"] == "[Mirrored From XSOAR] XSOAR Incident ID: 345\nClosed By: admin\nClose Reason: "
...
def test_parse_data_arr():
    outputs = {"employeeid": "123", "employeetype": "user"}

    parsed_readable, parsed_outputs = parse_data_arr(outputs)

    expected_readable = {"EmployeeID": "123", "Employeetype": "user"}
    expected_outputs = {"EmployeeID": "123", "Employeetype": "user"}
...
def test_get_incident_name():
    ...
    assert expected_multiple_reasons_incident_name == get_incident_name(
        RESPONSE_FETCH_INCIDENT_ITEM_MULTIPLE_REASONS, "10135", "12"
    )

    expected_multiple_reasons_incident_name = "Securonix Incident 10135, Violator ID: 12"
    assert expected_multiple_reasons_incident_name == get_incident_name(
        RESPONSE_FETCH_INCIDENT_ITEM_NO_THREAT_MODEL, "10135", "12"
    )

    expected_multiple_reasons_incident_name = "Data egress via network uploads: 10135"
    assert expected_multiple_reasons_incident_name == get_incident_name(RESPONSE_FETCH_INCIDENT_ITEM_VERSION_6_4, "10135", "12")
...
def test_fetch_securonix_incidents_is_already_fetched(mocker):
    ...
    incidents = fetch_securonix_incident(
        ...
        default_severity="",
        max_fetch="200",
        last_run={"already_fetched": ["100107"], "from": "1675900800000", "to": "1676367548000", "offset": 1},
        close_incident=False,
    )
...
def test_fetch_securonix_threats(mocker):
    ...
    incidents = fetch_securonix_threat(
        client, fetch_time="1 hour", tenant_name="Response-Automation", max_fetch="200", last_run={}
    )
    assert len(incidents) == 1
...
def test_fetch_securonix_threat_is_already_fetched(mocker):
    ...
    incidents = fetch_securonix_threat(
        client,
        fetch_time="1 hour",
        tenant_name="Response-Automation",
        max_fetch="200",
        last_run={
            "already_fetched": [
                (
                    "VIOLATOR5-1673852881421",
                    "RES10-RESOURCE-302184",
                    "Res-Playbook",
                    "RES-PLAYBOOK-DS-AUTOMATION",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (get_default_assignee_for_workflow, {"workflow": "SOCTeamReview"}, RESPONSE_DEFAULT_ASSIGNEE, EXPECTED_DEFAULT_ASSIGNEE),
        ...
        (
            create_incident,
            {
                ...
                "entity_name": "name",
                "entity_type": "Users",
                "resource_group": "BLUECOAT",
                "resource_name": "BLUECOAT",
                "violation_name": "Uploads to personal Websites",
        ...
        (
            check_entity_in_watchlist,
            {"watchlist_name": "test234", "entity_name": "1002"},
            RESPONSE_ENTITY_IN_WATCHLIST,
            EXPECTED_ENTITY_IN_WATCHLIST,
        ),
        (
            add_entity_to_watchlist,
            {"watchlist_name": "test234", "entity_name": "1004", "entity_type": "Users"},
            RESPONSE_ADD_ENTITY_TO_WATCHLIST,
            EXPECTED_ADD_ENTITY_TO_WATCHLIST,
        ),
        (
            list_threats,
            {"date_from": "1 day", "tenant_name": "Response-Automation"},
            RESPONSE_LIST_THREATS,
            EXPECTED_LIST_THREATS,
        ),
        (
        ...
        (get_whitelist_entry, {"whitelist_name": "test_whitelist"}, RESPONSE_GET_WHITELIST_ENTRY, EXPECTED_GET_WHITELIST_ENTRY),
        (
            create_whitelist,
            {"whitelist_name": "test_whitelist", "entity_type": "Users"},
            RESPONSE_CREATE_WHITELIST,
            EXPECTED_CREATE_WHITELIST,
        ),
        (
            add_whitelist_entry,
            {
                "whitelistname": "whitelistdemo1",
                "tenantname": "test_tenant",
                "whitelist_type": "Global",
                "entity_type": "Users",
                "entity_id": "f??abc",
                "exipry_date": "10/02/2023",
            },
            RESPONSE_ADD_WHITELIST_ENTRY_6_4,
            EXPECTED_ADD_WHITELIST_ENTRY_6_4,
        ),
        ...
        (
            delete_whitelist_entry,
            {"whitelist_name": "test_whitelist", "entity_id": "test_id", "tenant_name": "test_tenant"},
            RESPONSE_DELETE_WHITELIST_ENTRY,
            EXPECTED_DELETE_WHITELIST_ENTRY,
        ),
        (
            add_entry_to_lookup_table,
            {"name": "XSOAR_TEST", "json_data": json.dumps([{"key1": "value1"}])},
            RESPONSE_LOOKUP_TABLE_ENTRY_ADD,
            EXPECTED_LOOKUP_TABLE_ENTRY_ADD,
        ),
        (
            list_lookup_table_entries,
            {"name": "TEST_XSOAR", "max": "2"},
            RESPONSE_LOOKUP_TABLE_ENTRIES_LIST,
            EXPECTED_LOOKUP_TABLE_ENTRIES_LIST,
        ),
        (
            ...
            {
                "name": "test_table",
                "field_names": "accname,id",
                "key": "accname,id",
                "tenant_name": "test_tenant",
                "scope": "Global",
            },
            get_mock_create_lookup_table_response().text,
            EXPECTED_CREATE_LOOKUP_TABLE,
        ),
        (
            list_violation_data,
            {"from": "01/17/2022 00:00:00", "query": 'policy="Response-Account-\\?*AutoPlay"', "to": "01/17/2023 00:00:20"},
            RESPONSE_LIST_VIOLATION_6_4,
            EXPECTED_LIST_VIOLATION_DATA_6_4,
        ),
        (
            list_violation_data,
            {
                "query": ' index  = violation and policy="Response-Account-AutoPlay"',
                "from": "01/17/2022 00:00:00",
                "to": "01/17/2023 00:00:20",
            },
            RESPONSE_LIST_VIOLATION_6_4,
            EXPECTED_LIST_VIOLATION_DATA_6_4,
        ),
        (
            list_activity_data,
            {"from": "01/12/2024 10:00:00", "query": 'hostname = "HOST\\?*.com"', "to": "01/15/2024 12:01:00"},
            RESPONSE_LIST_ACTIVITY,
            EXPECTED_LIST_ACTIVITY,
        ),
        (
            list_activity_data,
            {"query": ' index  = activity and hostname = "HOST.com"', "from": "01/12/2024 10:00:00", "to": "01/15/2024 12:01:00"},
            RESPONSE_LIST_ACTIVITY,
            EXPECTED_LIST_ACTIVITY,
        ),
        (
        ...
        (
            add_comment_to_incident,
            {"incident_id": "123456", "comment": "testcomment"},
            RESPONSE_ADD_COMMENT_TO_INCIDENT,
            EXPECTED_ADD_COMMENT_TO_INCIDENT,
        ),
    ],
...
def test_commands(command, args, response, expected_result, mocker):
    ...
    elif command in (add_whitelist_entry, create_lookup_table, get_incident_workflow, get_incident_status,
                     get_incident_available_actions, add_comment_to_incident):
...
def test_get_modified_remote_data(mocker):
    ...
    assert result.modified_incident_ids == [
        record.get("incidentId") for record in RESPONSE_LIST_INCIDENT.get("result", {}).get("data", {}).get("incidentItems", [])
    ]
...
def test_get_remote_data(mocker):
    ...
    mocker.patch.object(client, "get_incident_request", return_value=RESPONSE_GET_INCIDENT["result"]["data"])
    mocker.patch.object(
        client,
        "get_incident_activity_history_request",
...
def test_list_lookup_table_entries_verify_default_values(mock_client, mocker):
    ...
    list_lookup_table_mock = mocker.patch.object(
        Client, "list_lookup_table_entries_request", return_value=RESPONSE_LOOKUP_TABLE_ENTRIES_LIST
    )
...
@pytest.mark.parametrize(
    "name,json_data,file_entry_id,err_msg",
    [
...
def test_add_entry_to_look_table_when_invalid_json_data_provided_via_file_entry_id(mock_client, mocker):
    ...
    file_obj = {"name": file_name, "path": os.path.join(tmp_dir, file_name)}
...
def test_update_remote_system_with_close_incidents(mock_client, mocker):
    ...
    mock_args = {
        "remoteId": "1234",
        "data": {"id": "345", "securonixcloseincident": False},
        "incidentChanged": True,
        "status": 2,
        "delta": {"closeNotes": "Closing XSOAR incident", "closeReason": "Resolved", "closingUserId": "admin"},
    }
    mocker.patch.object(demisto, "args", return_value=mock_args)

    mocker.patch.object(Client, "perform_action_on_incident_request", side_effect=perform_action_on_incident_request_side_effect)
    mocker.patch.object(Client, "add_comment_to_incident_request", side_effect=add_comment_to_incident_request_side_effect)
...
def test_get_remote_data_command_with_attachment(mock_client, mocker):
    ...
    mocker.patch.object(
        mock_client,
        "get_incident_activity_history_request",
        return_value=MIRROR_RESPONSE_GET_INCIDENT_ACTIVITY_HISTORY_ATTACHMENT["result"]["activityStreamData"],
    )
...
def test_validate_mirroring_parameters_for_incoming_mirroring_direction(close_states_of_securonix):
    ...
    expected_error_message = (
        'Following field is required for Incoming Mirroring: "Securonix workflow state(s) '
        'that can be considered as Close state in XSOAR for Incoming mirroring".'
    )

    with pytest.raises(ValueError) as error:
        validate_mirroring_parameters({"close_states_of_securonix": close_states_of_securonix, "mirror_direction": "Incoming"})
...
def test_validate_mirroring_parameters_for_outgoing_mirroring_direction(
    active_state_action, active_state_status, close_state_action, close_state_status, comment_entry_tag
):
...
def test_validate_mirroring_parameters_for_incoming_and_outgoing_mirroring_direction(
...
def test_validate_mirroring_parameters_for_close_incident_checkbox(
    active_state_action, active_state_status, close_state_action, close_state_status
):
...
def test_fetch_securonix_incident_when_no_new_incident_found(mock_client, mocker):
    ...
    mocker.patch.object(
        ...
        "getLastRun",
        return_value={
            "value": json.dumps({"already_fetched": ["100107"], "from": "1675900800000", "to": "1676367548000", "offset": 1})
        },
    )
    mocker.patch.object(Client, "list_incidents_request", return_value={"totalIncidents": 0.0, "incidentItems": []})
...
def test_fetch_securonix_incident_when_time_key_found_in_last_run_object(mock_client, mocker):
    ...
    mocker.patch.object(
        demisto,
        "getLastRun",
        return_value={"value": json.dumps({"already_fetched": ["100108"], "time": "2020-06-07T08:32:41.679579Z"})},
    )
    mocker.patch.object(Client, "list_incidents_request", return_value=RESPONSE_FETCH_INCIDENTS)
    mock_set_last_run = mocker.patch.object(demisto, "setLastRun")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "whitelist_name": "test_whitelist",
                "whitelist_type": "Attribute",
                "attribute_name": "test_attribute",
                "violation_type": "Policy",
            },
            "Provide valid attribute_name",
        ),
        (
            {
                "whitelist_name": "test_whitelist",
        ...
        (
            {
                ...
                "attribute_name": "resourcetype",
                "violation_type": "Policy",
                "expiry_date": "13-03-2023",
            },
            "exipry_date is not in MM/DD/YYYY format",
        ),
    ],
)
...
@pytest.mark.parametrize(
    "command_name,args,response,err_msg",
    [
        (
            create_lookup_table,
            {},
        ...
        (
            add_entry_to_lookup_table,
            {"name": "test_lookup_table", "json_data": '{"id": "1"}'},
            "Error in command.",
        ...
        (
            list_activity_data,
            {},
            {"error": True, "errorMessage": "Invalid Query"},
