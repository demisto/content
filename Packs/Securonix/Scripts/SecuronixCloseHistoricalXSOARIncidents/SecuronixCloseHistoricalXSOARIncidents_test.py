...
MOCK_EXECUTE_COMMAND_RESPONSE = [{"Contents": [], "ContentsFormat": "text", "EntryContext": None, "Type": 4}]
...
def test_get_securonix_incident_id():
    ...
    with open("test_data/mock_incident_contents.json") as f:
...
def test_is_securonix_incident_closed_when_incident_is_in_progress():
    ...
    with open("test_data/incident_activity_history_get_response.json") as f:
...
def test_close_xsoar_incident_script_success(mock_return, mock_execute_command, mock_args):
    ...
    with open("test_data/mock_incidents.json") as f:
        mock_incidents_1 = json.load(f)
    with open("test_data/incident_activity_history_get_response.json") as f:
        mock_activity_history = json.load(f)
    MOCK_EXECUTE_COMMAND_RESPONSE[0]["Contents"] = mock_activity_history
    mock_args.return_value = {"from": "1 months", "to": "now", "close_states": "Closed, Completed"}
