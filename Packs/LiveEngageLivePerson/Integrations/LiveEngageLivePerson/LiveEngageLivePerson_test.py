...
MOCK_TIME_1 = (datetime.now(timezone.utc) - timedelta(minutes=10)).isoformat().replace('+00:00', 'Z')
MOCK_TIME_2 = (datetime.now(timezone.utc) - timedelta(minutes=5)).isoformat().replace('+00:00', 'Z')
MOCK_TIME_3 = (datetime.now(timezone.utc) - timedelta(minutes=1)).isoformat().replace('+00:00', 'Z')
...
def test_get_event_domain_success(requests_mock: Mocker):
    ...
    requests_mock.get(
        LiveEngageLivePerson.DOMAIN_API_URL.format(account_id=PARAMS['account_id']),
        json=MOCK_DOMAIN_RESPONSE
    )
...
def test_get_event_domain_network_error(requests_mock: Mocker):
    ...
    requests_mock.get(
        LiveEngageLivePerson.DOMAIN_API_URL.format(account_id=PARAMS['account_id']),
        exc=RequestException("Connection timed out")
    )
...
def test_get_event_domain_json_error(requests_mock: Mocker):
    ...
    requests_mock.get(
        LiveEngageLivePerson.DOMAIN_API_URL.format(account_id=PARAMS['account_id']),
        text='<not_json>This is HTML</not_json>'
    )
...
def test_get_access_token_success(requests_mock: Mocker):
    ...
    token = LiveEngageLivePerson.Client._get_access_token(
        auth_url,
        PARAMS['account_id'],
        'id', 'secret', True, {}
    )
...
def test_http_request_non_401_error(client: LiveEngageLivePerson.Client, mocker):
    ...
    base_http_request = mocker.patch.object(
        LiveEngageLivePerson.BaseClient,
        '_http_request',
        side_effect=DemistoException("500 Server Error")
    )
...
def test_fetch_events_malformed_timestamp(client: LiveEngageLivePerson.Client, mocker):
    ...
    demisto.info.assert_any_call(
        "[LivePerson] [Warning] Could not parse timestamp: not-a-real-date. "
...
def test_fetch_events_command_first_run(client: LiveEngageLivePerson.Client, mocker):
    ...
    # Setup
    mocker.patch.object(demisto, 'getLastRun', return_value={})
    first_fetch_time = datetime.now(timezone.utc) - timedelta(days=1)

    new_max_time = datetime.fromisoformat(MOCK_TIME_2.replace('Z', '+00:00'))
    mocker.patch.object(
        client,
        'fetch_events',
        return_value=([MOCK_EVENT_1, MOCK_EVENT_2], new_max_time)
    )
...
def test_fetch_events_command_subsequent_run(client: LiveEngageLivePerson.Client, mocker):
    ...
    # Setup
    last_run_str = (datetime.now(timezone.utc) - timedelta(days=1)).isoformat()
    ...
    # Execute
    LiveEngageLivePerson.fetch_events_command(client, 1000, datetime.now(timezone.utc) - timedelta(days=30))
...
def test_fetch_events_command_infinite_loop_prevention(client: LiveEngageLivePerson.Client, mocker):
    ...
    mocker.patch.object(
        client,
        'fetch_events',
        return_value=([MOCK_EVENT_1], last_run_time)
    )
    ...
    demisto.setLastRun.assert_called_with({'last_fetch_time': expected_last_run_time})
    demisto.info.assert_any_call(
        f"[LivePerson] Setting new last run time to {expected_last_run_time} to avoid duplicates."
    )
...
def test_main_domain_lookup_fails(mocker):
    ...
    demisto.return_error.assert_called_once_with(
        'Failed to execute test-module command. Error: Failed to fetch event domain. Status: 404',
        error=mocker.ANY
    )
