...
class Client(BaseClient):
    ...
    def __init__(self, base_url: str, account_id: str, auth_server_url: str,
    ...
    @staticmethod
    def _get_access_token(auth_url: str, account_id: str, client_id: str,
                          client_secret: str, verify: bool, proxies: dict) -> str:
        """
    ...
    def _http_request(self, *args, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            if '401' in str(e) or '403' in str(e):
                demisto.info(f'{INTEGRATION_PREFIX} Received 401/403 error. Token may be expired. '
    ...
    def fetch_events(self, max_fetch: int, last_run_time: datetime) -> Tuple[List[Dict[str, Any]], datetime]:
        ...
        while len(all_events) < max_fetch:
            ...
            try:
                ...
                for event in events:
                    ...
                    if timestamp_str:
                        ...
                        try:
                            ...
                            demisto.info(f'{INTEGRATION_PREFIX} [Warning] Could not parse timestamp: {timestamp_str}. '
                ...
                demisto.error(f'{INTEGRATION_PREFIX} Error during event fetch loop: {str(e)}. '
...
def fetch_events_command(client: Client, max_fetch: int, first_fetch_time: datetime) -> List[Dict[str, Any]]:
    ...
    events, new_max_timestamp = client.fetch_events(
        max_fetch=max_fetch,
    ...
    if new_max_timestamp > last_run_time:
        ...
        demisto.info(f'{INTEGRATION_PREFIX} Setting new last run time to {new_last_run_time_str} '
        ...
        demisto.info(f'{INTEGRATION_PREFIX} [Warning] Fetched {len(events)} events, but latest event timestamp '
...
def main() -> None:
    ...
    try:
        ...
        if not (auth_url and account_id and client_id and client_secret):
            raise DemistoException('Missing required parameters: Authorization Server URL, Account ID, '
                                   'Client ID, or Client Secret.')

        first_fetch_time, _ = parse_date_range(first_fetch_str)
        if not first_fetch_time:
            raise DemistoException(f"Invalid 'first_fetch' format: {first_fetch_str}. "
