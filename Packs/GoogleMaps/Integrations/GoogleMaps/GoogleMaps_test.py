...
from GoogleMaps import MESSAGE_ZERO_RESULTS, STATUS_ZERO_RESULTS, Client, google_maps_geocode_command
...
def test_google_maps_geocode_command(requests_mock):
    ...
    client = Client(api_key="", base_url="https://maps.googleapis.com/maps/api", proxy=False, insecure=False)

    result_note, result_map = google_maps_geocode_command(client=client, search_address=search_address, error_on_no_results=False)
...
def test_google_maps_geocode_bad_api_key(requests_mock):
    ...
    client = Client(api_key="invalid-api-key", base_url="https://maps.googleapis.com/maps/api", proxy=False, insecure=False)
    ...
    with pytest.raises(DemistoException) as e:
        google_maps_geocode_command(client=client, search_address=search_address, error_on_no_results=False)
...
def test_google_maps_geocode_zero_results(requests_mock):
    ...
    actual_result = google_maps_geocode_command(
        client=client, search_address=zero_results_search_address, error_on_no_results=False
    )
    ...
    with pytest.raises(DemistoException) as e:
        google_maps_geocode_command(client=client, search_address=zero_results_search_address, error_on_no_results=True)
