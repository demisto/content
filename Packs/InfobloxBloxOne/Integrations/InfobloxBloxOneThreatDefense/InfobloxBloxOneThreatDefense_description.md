...
3. To fetch Infoblox IQ for TD Insights, DNS Security Events, or SOC Insights (deprecated), or DNS Security Events as an incidents in XSOAR, select the option `Fetches incidents` and follow the table to update configuration parameters.
...
| Max Fetch                               | The maximum number of IQ for TD Insight, SOC Insights, or DNS Security Events to fetch each time. If the value is greater than 200, it will be considered as 200. The maximum is 200. |
