...
def panos_get_previous_users():
    params = {
        'type': 'op',
        'cmd': '<show><global-protect-gateway><previous-user/></global-protect-gateway></show>',
        'key': demisto.params().get('apiKey')
    }
    r = http_request('GET', params)
...
def panos_update_infoblox():
    current_users_raw = panos_get_current_users()
    previous_users_raw = panos_get_previous_users()

    demisto.results(current_users_raw)
    demisto.results(previous_users_raw)

    # Current Users Table is Empty and Previous Users Table is Empty
    if current_users_raw is None and previous_users_raw is None:
        demisto.results("No Previous or Current Users")

    # Current Users Table has entries and Previous Users Table is Empty
    if current_users_raw and previous_users_raw is None:
        # Add User
        demisto.results("Add User")

    # Current Users Table is Empty and Previous Users Table has entries
    if current_users_raw is None and previous_users_raw:
        # Delete All Previous Users Up until timer
        demisto.results("Delete All Previous Users")

    # Current Users Table has entries and Previous Users Table has entries
    if current_users_raw and previous_users_raw:
        # Grab Current and Previous Users Tables
        current_client = current_users_raw["entry"]
        previous_client = previous_users_raw["entry"]

        # Create nested dictionaries with user + IP + login-time-utc as index
        current_users = get_current_users_dict(current_client)
        previous_users = get_previous_users_dict(previous_client)

        # Look for records where users + IP are the same in previous and current.  Remove any that match because no
        # DNS update is needed
        for key in current_users.keys():
            if key in previous_users:
                del previous_users[key]
                #del current_users[key]
            # If current user + IP does not match in previous Add/Update Infoblox
            else:
                demisto.results("Add/Update Infoblox Record " + current_users[key]['username'])

        for key in previous_users.keys():
            demisto.results("Delete Infoblox Record " + previous_users[key]['username'])

    return "No Condition"


def get_previous_users_dict(previous_client):
    username = ""
    computer = ""
    virtual_ip = ""
    public_ip = ""
    ...
    logout_time_utc = ""
    # Previous User List
    if isinstance(previous_client, list):
        for client in previous_client:
            for key, value in client.items():
                if value:
                    if key == "username":
                        # demisto.results("Current Username: " + value)
                        username = value
                    if key == "computer":
                        computer = value
                    if key == "client":
                    ...
                    if key == "public-ip":
                        public_ip = value
                    if key == "login-time-utc":
                        login_time_utc = value
                    if key == "logout-time-utc":
                        logout_time_utc = value
                    previous_users = {
                        username + public_ip + login_time_utc: {"username": username, "computer": computer, "client": client,
                                                                "virtual-ip": virtual_ip, "public-ip": public_ip,
                                                                "login-time-utc": login_time_utc, "logout-time-utc": logout_time_utc}}
    # If Previous Users is not a List
    else:
        for key, value in previous_client.items():
            if value:
                if key == "username":
                    # demisto.results("Current Username: " + value)
                    username = value
                ...
                if key == "logout-time-utc":
                    logout_time_utc = value
                previous_users = {username + public_ip + login_time_utc: {"username": username, "computer": computer, "client": client,
                                                                          "virtual-ip": virtual_ip, "public-ip": public_ip,
                                                                          "login-time-utc": login_time_utc,
                                                                          "logout-time-utc": logout_time_utc}}
    return previous_users


def get_current_users_dict(current_client):
    username = ""
    computer = ""
    virtual_ip = ""
    public_ip = ""
    ...
    login_time_utc = ""
    # Current User List
    if isinstance(current_client, list):
        for client in current_client:
            for key, value in client.items():
                if value:
                    if key == "username":
                        # demisto.results("Current Username: " + value)
                    ...
                    current_users = {
                        username + public_ip + login_time_utc: {"username": username, "computer": computer, "client": client,
                                                                "virtual-ip": virtual_ip, "public-ip": public_ip,
                                                                "login-time-utc": login_time_utc}}
    # If Current Users is not a list
    else:
        for key, value in current_client.items():
            ...
            if value:
                ...
                if key == "login-time-utc":
                    login_time_utc = value
                current_users = {username + public_ip + login_time_utc: {"username": username, "computer": computer, "client": client,
                                                                         "virtual-ip": virtual_ip, "public-ip": public_ip,
                                                                         "login-time-utc": login_time_utc}}
...
def main():
    ...
    try:
        ...
        if demisto.command() == 'panos-get-previous-users':
            response_from_api = panos_get_previous_users()
            raw_response = response_from_api
            command_results = CommandResults(
                outputs_prefix='GlobalProtect.PreviousUsers',
                outputs_key_field='public-ip',
                outputs=response_from_api,
                readable_output=raw_response,
                raw_response=raw_response
            ...
            return_results(command_results)
        if demisto.command() == 'panos-update-infoblox':
            response_from_api = panos_update_infoblox()
            raw_response = response_from_api
            """""
            command_results = CommandResults(
                outputs_prefix='',
