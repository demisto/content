...
from IgniteFeed import (
    ...
    flashpoint_ignite_get_indicators_command,
    main,
)
...
def test_test_module_with_invalid_apikey(requests_mock, mock_client):
    ...
    requests_mock.get(os.path.join(MOCK_URL, URL_SUFFIX["ATTRIBUTES"]), json=indicator_list_response_401, status_code=401)
...
def test_test_module_with_valid_apikey(requests_mock, mock_client):
    ...
    indicator_list_response_200: dict = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/indicator_list_200.json")
    )
...
def test_flashpoint_ignite_get_indicators_command_when_create_relationship_is_true(requests_mock, mock_client):
    ...
    indicators: dict = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/indicators_with_relationship.json")
    )
...
def test_fetch_indicators_command_when_valid_response_is_returned(requests_mock, mock_client):
    ...
    requests_mock.get(
        os.path.join(MOCK_URL, URL_SUFFIX["ATTRIBUTES"]),
        json=indicator_list_response_200,
        headers={"x-fp-total-hits": "5000"},
        status_code=200,
    )
...
def test_test_module_with_isfetch(requests_mock, mocker, mock_client):
    ...
    """
    requests_mock.get(
        os.path.join(MOCK_URL, URL_SUFFIX["ATTRIBUTES"]), json={}, headers={"x-fp-total-hits": "5000"}, status_code=200
    )
...
def test_test_module_with_isfetch_when_error(requests_mock, mocker, mock_client):
    ...
    requests_mock.get(
        os.path.join(MOCK_URL, URL_SUFFIX["ATTRIBUTES"]),
        json={},
        headers={"x-fp-total-hits": f"{MAX_INDICATORS + 1}"},
        status_code=200,
    )
    mocker.patch.object(demisto, "params", return_value={"feed": True, "url": MOCK_URL, "credentials": {"password": API_KEY}})
