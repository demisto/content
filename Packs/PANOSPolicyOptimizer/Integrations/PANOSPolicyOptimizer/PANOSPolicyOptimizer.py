...
class Client:
    ...
    def __init__(
        self, url: str, username: str, password: str, vsys: str, device_group: str, verify: bool, tid: int, version: str
    ):
    ...
    def session_post(self, url: str, json_cmd: dict) -> dict:
        response = self.session.post(url=url, json=json_cmd, verify=self.verify, headers=self.session_metadata.get("headers"))
        json_response = json.loads(response.text)
        if "type" in json_response and json_response["type"] == "exception":
            if "message" in json_response:
                raise Exception(f'Operation to PAN-OS failed. with: {json_response["message"]!s}')
            raise Exception(f"Operation to PAN-OS failed. with: {json_response!s}")
    ...
    def login(self) -> str:  # pragma: no cover
        ...
        try:
            ...
            if LooseVersion(self.version) >= LooseVersion("10.1.6"):
                ...
                response = self.session.get(url=f'{self.session_metadata["base_url"]}/php/login.php?', verify=self.verify)
            ...
            response = self.session.post(
                url=f'{self.session_metadata["base_url"]}/php/login.php?', data=login_data, verify=self.verify, headers=headers
            )
            self.session_metadata["cookie"] = f'PHPSESSID={response.cookies.get_dict().get("PHPSESSID")}'
            ...
            response = self.session.post(url=f'{self.session_metadata["base_url"]}/', verify=self.verify, headers=headers)
        except Exception as err:
            raise Exception(f"Failed to login. Please double-check the credentials and the server URL. {err!s}")
    ...
    def token_generator(self) -> str:
        ...
        :return: hash token
        """
        data_code = f'{self.session_metadata["cookie_key"]}{self.session_metadata["tid"]!s}'
    ...
    def policy_optimizer_no_apps(self, position: str) -> dict:
        ...
        return self.session_post(
            url=f'{self.session_metadata["base_url"]}/php/utils/router.php/PoliciesDirect.getPoliciesByUsage', json_cmd=json_cmd
        )
    ...
    def policy_optimizer_app_and_usage(self, rule_uuid: str) -> dict:
                ...
                [
                    {
                        ...
                        "summary": "no",
                        "resultfields": "<member>apps-seen</member>"
        ...
        return self.session_post(
            url=f'{self.session_metadata["base_url"]}/php/utils/router.php/PoliciesDirect.getAppDetails', json_cmd=json_cmd
        )
    ...
    def policy_optimizer_get_dag(self, dag: str) -> dict:
        ...
        json_cmd = {
            "action": "PanDirect",
            "method": "execute",
            "data": [self.token_generator(), "AddressGroup.showDynamicAddressGroup", {"id": dag, "vsysName": self.machine}],
            "type": "rpc",
            "tid": self.session_metadata["tid"],
        }
...
def get_unused_rules_by_position(
    client: Client,
    position: str,
    exclude: bool,
    rule_type: str,
...
def get_policy_optimizer_statistics_command(client: Client, args: dict) -> CommandResults:
    ...
    if "@status" in stats and stats["@status"] == "error":
        raise Exception(f"Operation Failed with: {stats!s}")
...
def policy_optimizer_no_apps_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(
            name="Policy Optimizer No App Specified:", t=rules_no_apps, headers=headers, removeNull=True
        ),
        raw_response=raw_response,
    )
...
def policy_optimizer_get_dag_command(client: Client, args: dict) -> CommandResults:
    ...
    if "@status" in result and result["@status"] == "error":
        raise Exception(f"Operation Failed with: {result!s}")
