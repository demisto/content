...
#### Integrations
##### SentinelOne v2
- Added 19 more new commands to the existing Integration:
    - ***sentinelone-expire-site***  - Expires the provided site Id
    ...
    - ***sentinelone-create-power-query*** - Start a Deep Visibility Power Query, get back status and potential results (ping afterwards using the queryId if query has not finished)
    - ***sentinelone-ping-power-query*** - Ping a Deep Visibility Power Query using the queryId if results have not returned from an initial Power Query or a previous ping
...
- Updated the *README.md* file with the new content.
