    ...
    for threat in threats:
        ...
        if IS_VERSION_2_1 or threat_rank >= rank:
            ...
            entry = {
                ...
                if IS_VERSION_2_1
                else threat_info.get("fileContentHash"),
                ...
                "FileMaliciousContent": threat_info.get(
...
class Client(BaseClient):
    ...
    def add_hash_to_blocklists_request(
        self, value, os_type, site_ids, description="", source=""
    ) -> dict:
        """
        ...
        for site_id in site_ids:
            ...
            response = self._http_request(
                method="POST", url_suffix="restrictions", json_data=body, ok_codes=[200]
            )
        return response.get("data") or {}
    ...
    def fetch_file_request(self, agent_id, file_path, password) -> dict:
        ...
        response = self._http_request(
            method="POST", url_suffix=f"agents/{agent_id}/actions/fetch-files", json_data=body
        )
        return response.get("data", {})

    def download_fetched_file_request(self, agent_id, activity_id) -> bytes:
        return self._http_request(
            method="GET", url_suffix=f"agents/{agent_id}/uploads/{activity_id}", resp_type="content"
        )
    ...
    def get_threats_request(
        ...
        response = self._http_request(
            method="GET", url_suffix="threats", params=params, ok_codes=[200]
        )
        return response.get("data", {})
    ...
    def get_exclusions_request(
        ...
        response = self._http_request(
            method="GET", url_suffix=endpoint_url, params=params, ok_codes=ok_codes
        )
        return response.get("data", {})
    ...
    def remove_exclusion_item_request(self, item_id) -> dict:
        ...
        response = self._http_request(
            method="DELETE", url_suffix="exclusions", json_data=body, ok_codes=[200]
        )
        return response.get("data") or {}
...
def get_groups_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One Groups", groups, headers, headerTransform=pascalToSpace, removeNull=True
        ),
...
def delete_group(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Delete Group", context, removeNull=True, headerTransform=pascalToSpace
        ),
...
def move_agent_to_group_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            ...
            f"agents were Moved successfully",
            context_entries,
            removeNull=True,
        ),
    ...
    demisto.results(
        {
            ...
            "HumanReadable": tableToMarkdown(
                "Sentinel One Agent Processes", contents, headers, removeNull=True
            ),
...
def mark_as_threat_command(client: Client, args: dict) -> CommandResults:
    ...
    if affected_threats.get("affected") and int(affected_threats.get("affected")) > 0:
        title = (
            f'Total of {affected_threats.get("affected")} provided threats were marked successfully'
        )
        affected = True
    else:
        affected = False
...
def mitigate_threat_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Mitigating threats", contents, metadata=meta, removeNull=True
        ),
        outputs_prefix="SentinelOne.Threat",
        outputs_key_field="ID",
        outputs=context_entries,
...
def update_alert_analyst_verdict(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Update alerts analyst verdict", contents, metadata=meta, removeNull=True
        ),
...
def create_star_rule(client: Client, args: dict) -> CommandResults:
    ...
    if expiration_mode == "Temporary" and expiration_date is None:
        raise DemistoException(
            "You must provide expiration_date argument when you selected the Temporary as expiration_mode"
        )
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Create star rule", context, removeNull=True
        ),
...
def update_star_rule(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Updated star rule", context, removeNull=True
        ),
        outputs_prefix="SentinelOne.StarRule",
        outputs_key_field="ID",
        outputs=context,
...
def create_ioc(client: Client, args: dict) -> CommandResults:
    ...
    ioc = client.create_ioc_request(
        name, source, ioc_type, method, validUntil, value, account_ids, externalId, description
    )[0]
...
def get_iocs(client: Client, args: dict) -> CommandResults:
    ...
    if pagination["nextCursor"] is not None:
        demisto.results(
            "Use the below cursor value to get the next page iocs \n {}".format(
...
def ping_power_query(client: Client, args: dict) -> CommandResults:
    ...
    if response.get("data"):
        ...
        return CommandResults(
            readable_output="There is no data returned by the id that you provided,"
...
def update_threat_status(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Update threats status", context_entries, metadata=meta, removeNull=True
        ),
        outputs_prefix="SentinelOne.Threat",
        outputs_key_field="ID",
        outputs=context_entries,
...
def update_alert_status(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Update alerts status", context_entries, metadata=meta, removeNull=True
        ),
        outputs_prefix="SentinelOne.Alert",
        outputs_key_field="ID",
        outputs=context_entries,
...
def expire_site(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Expire Site", context_entries, removeNull=True
        ),
    ...
    for threat_id in threat_ids:
        ...
        if threat_file_download_endpoint != "-1":
            ...
            zipped_file = fileResult(
                filename=f"{threat_id}.zip", data=zip_file_data, file_type=EntryType.ENTRY_INFO_FILE
            )
        context_entries.append(
            {"Downloadable": downloadable, "ID": threat_id, "ZippedFile": zipped_file}
        )
    return [
        CommandResults(
            readable_output=tableToMarkdown(
                "Sentinel One - Fetch threat file", context_entries, metadata=meta, removeNull=False
            ),
            outputs_prefix="SentinelOne.Threat",
            outputs_key_field="ID",
            outputs=context_entries,
...
def get_alerts(client: Client, args: dict) -> CommandResults:
    ...
    if pagination["nextCursor"] is not None:
        demisto.results(
            "Use the below cursor value to get the next page alerts \n {}".format(
                pagination["nextCursor"]
            )
        )
            ...
            context_entries.append(
                {
                    ...
                    "SrcParentProcSignerIdentity": source_parent_process_info.get(
                        "fileSignerIdentity"
                    ),
...
def get_item_ids_from_whitelist(
    client: Client, item: str, exclusion_type: str, os_type: str = None
) -> List[Optional[str]]:
    """
    ...
    if len(white_list) > limit:
        raise DemistoException(
            "Recieved More than 3 results when querying by hash. This condition should not occur"
        )
...
def create_white_item_command(client: Client, args: dict):
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            title, context_entries, removeNull=True, headerTransform=pascalToSpace
        ),
...
def reactivate_site_command(client: Client, args: dict) -> CommandResults:
    ...
    if unlimited is None and expiration is None:
        raise DemistoException(
            "You must provide unlimited argument or expiration argument as required."
        )
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            f"Sentinel One - Reactivated Site: {site_id}", context, removeNull=True
        ),
        outputs_prefix="SentinelOne.Site",
        outputs_key_field="ID",
        outputs=context,
...
def get_threat_summary_command(client: Client, args: dict) -> CommandResults:
    ...
    if threat_summary:
        context_entries = {
            ...
            "SuspiciousNotMitigatedNotResolved": threat_summary.get(
                "suspiciousNotMitigatedNotResolved"
            ),
...
def list_agents_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            ...
            headerTransform=pascalToSpace,
            removeNull=True,
            metadata="Provides summary information and details for all"
...
def connect_agent_to_network(client: Client, args: dict) -> Union[CommandResults, str]:
    ...
    if agents_affected > 0:
        ...
        contents = [
            {"NetworkStatus": agent.get("networkStatus"), "ID": agent.get("id")} for agent in agents
        ]
...
def broadcast_message(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Broadcast Message", context, metadata=meta, removeNull=True
        ),
...
def shutdown_agents(client: Client, args: dict) -> str:
    ...
    if not (agent_id or group_id):
        raise DemistoException(
            "Expecting at least one of the following arguments to filter by: agent_id, group_id."
        )
...
def uninstall_agent(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Sentinel One - Uninstall Agent", context, metadata=meta, removeNull=True
        ),
        ...
        contents.append(
            {
                ...
                "ProcessUID": event.get("srcProcUid")
                if IS_VERSION_2_1
                else event.get("processUniqueKey"),
        ...
        contents.append(
            {
                ...
                "ProcessUID": process.get("srcProcUid")
                if IS_VERSION_2_1
                else process.get("processUniqueKey"),
                "ProcessName": process.get("processName"),
...
def get_hash_ids_from_blocklist(
    client: Client, sha1: str, os_type: str = None, get_global: bool = True
) -> List[Optional[str]]:
    """
...
def extract_sentinelone_zip_file(zip_file_data: bytes, password: str) -> Tuple[str, bytes]:
    ...
    if len(file_names) < 1:
        raise DemistoException(
            "No file found in upload from agent. Perhaps the path submitted is wrong?"
        )
...
def fetch_incidents(
    client: Client, fetch_limit: int, first_fetch: str, fetch_threat_rank: int, fetch_site_ids: str
):
    ...
    threats = client.get_threats_request(
        limit=fetch_limit, created_after=last_fetch_date_string, site_ids=fetch_site_ids
    )
    for threat in threats:
        ...
        # for outcoming mirroring (XSOAR - S1)
            ...
            if command in commands["common"]:
                ...
                raise NotImplementedError(
                    f"The {command} command is not supported for API version {api_version}"
                )
