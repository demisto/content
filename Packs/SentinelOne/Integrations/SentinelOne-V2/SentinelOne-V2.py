...
class Client(BaseClient):
    ...
    def download_fetched_file_request(self, agent_id, activity_id) -> bytes:
        response = self._http_request(method='GET', url_suffix=f'agents/{agent_id}/uploads/{activity_id}', resp_type='content')
        return response
...
def extract_sentinelone_zip_file(zip_file_data: bytes, password: str) -> Tuple[str, bytes]:
    ...
    file_name = [name for name in zip_file.namelist() if name != "manifest.json"][0]
