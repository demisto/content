...
def test_set_rule_error_policy(mocker):
    ...
    assert 'Policy argument must be "allow" or "deny". Invalid value: wrong_policy' \
...
def test_set_rule_error_type(mocker):
    ...
    assert 'Type argument must be "dest_domain", "dest_ip", "dest_host" or "url_regex". Invalid value: wrong_type' \
           in results_mock.call_args[0][0]['Contents']
