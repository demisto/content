...
def test_get_alert_details_command(mocker):
    ...
    output = command_results.to_context().get('EntryContext', {}).get('CarbonBlackDefense.Alert(val.id == obj.id)')
...
def test_device_search_command(mocker):
    ...
    output = command_results.to_context().get('EntryContext', {}).get('CarbonBlackDefense.Device(val.id == '
                                                                      'obj.id)')
...
def test_find_events_command(mocker):
    ...
    output = command_results.to_context().get('EntryContext', {}).get('CarbonBlackDefense.Events.Search(val.job_id == '
                                                                      'obj.job_id)')
...
def test_find_processes_command(mocker):
    ...
    output = command_results.to_context().get('EntryContext', {}).get('CarbonBlackDefense.Process.Search(val.job_id == obj.job_id)')
