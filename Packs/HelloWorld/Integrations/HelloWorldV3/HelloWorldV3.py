...
"""Unified HelloWorldV3 Integration for Cortex XSOAR and XSIAM
...
Example Design document for this Integration (HelloWorldV3):
...
context_output_prefix = "HelloWorld"  # Context outputs from all commands will have this prefix
...
class HelloWorldSeverity(str, Enum):
    ...
    @classmethod
    def convertToIncidentSeverity(cls, raw_severity: str) -> int:
        """
...
class HelloWorldParams(BaseParams):
    ...
    firstFetch: str = "3 days"  # access via abstracted and normalized `first_fetch_time` property
...
class HelloWorldAuthHandler(APIKeyAuthHandler):
    """Custom authentication handler for API key-based authentication for HelloWorldV3.
...
def say_hello_command(client: HelloWorldClient, args: HelloworldSayHelloArgs) -> CommandResults:
    ...
    # INTEGRATION DEVELOPER TIP
    # In this case 'name' is an argument set in the HelloWorldV3.yml file as mandatory,
