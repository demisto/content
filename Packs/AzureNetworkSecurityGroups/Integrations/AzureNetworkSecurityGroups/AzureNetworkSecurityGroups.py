...
networkInterfaces"
    ...
    @logger
    def create_or_update_security_group_request(
...
networkSecurityGroups/{security_group_name}"
    ...
    @logger
    def create_or_update_network_interface_request(
        self, subscription_id: str, resource_group_name: str, nic_name: str, data: Dict
    ) -> Dict:
        full_url = f"{PREFIX_URL}{subscription_id}/resourceGroups/{resource_group_name}/providers/Microsoft.Network/\
...
def list_groups_command(client: AzureNSGClient, params: Dict, args: Dict) -> CommandResults:
    ...
    network_groups = client.list_network_security_groups(subscription_id=subscription_id, resource_group_name=resource_group_name)
    ...
    return CommandResults(
        outputs_prefix="AzureNSG.SecurityGroup", outputs_key_field="id", outputs=network_groups, readable_output=hr
    )
...
def list_rules_command(client: AzureNSGClient, params: Dict, args: Dict) -> CommandResults:
    ...
    for group in security_groups:
        rules_returned = client.list_rules(
...
def azure_nsg_virtual_networks_list_command(client: AzureNSGClient, params: Dict, args: Dict) -> CommandResults:
    ...
    for data in data_from_response:
        reformat_data(
            data,
            dict_to_extract=[("properties",), ("addressSpace",)],
            list_to_extract=[
                ("subnets", "name", "subnetName"),
...
def azure_nsg_security_group_create_command(client: AzureNSGClient, params: Dict, args: Dict) -> CommandResults:
    ...
    response = client.create_or_update_security_group_request(
        subscription_id=subscription_id,
        resource_group_name=resource_group_name,
        security_group_name=security_group_name,
        location=location,
...
def azure_nsg_network_interfaces_create_command(client: AzureNSGClient, params: Dict, args: Dict) -> CommandResults:
    ...
    data = {
        "location": args.get("location"),
        "properties": {"ipConfigurations": [{"name": args.get("ip_config_name"), "properties": {"subnet": {"id": subnet_id}}}]},
    }

    if nsg_name:
        data["properties"]["networkSecurityGroup"] = {  # type: ignore[union-attr, assignment, index]
    ...
    if private_ip:
        conf_properties["privateIPAddress"] = private_ip  # type: ignore[union-attr, index]
    if public_ip_address_name:
        conf_properties["publicIPAddress"] = {"id": public_ip_address_name}  # type: ignore[union-attr, index]

    response = client.create_or_update_network_interface_request(
        subscription_id=subscription_id, resource_group_name=resource_group_name, nic_name=nic_name, data=data
    )
    outputs = deepcopy(response)
    reformat_data(
        outputs,
        dict_to_extract=[("properties",)],
        list_to_extract=[
            ("ipConfigurations", "name", "ipConfigurationName"),
