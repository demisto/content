...
class Email:
    def __init__(self, message_bytes: bytes, include_raw_body: bool, save_file: bool, id_: int) -> None:
        ...
        self.date = email_object.date.replace(tzinfo=datetime.UTC) if email_object.date else None
...
def list_emails(client: IMAPClient,
    ...
    results = [{'Subject': email.subject,
                'Date': email.date.isoformat() if email.date else datetime.now(datetime.UTC).isoformat(),
                'To': email.to,
                'From': email.from_,
