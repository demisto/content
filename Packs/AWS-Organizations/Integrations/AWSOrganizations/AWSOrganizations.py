...
if TYPE_CHECKING:
    ...
    from mypy_boto3_organizations.literals import CreateAccountFailureReasonType, PolicyTypeType
...
CREATE_ACCOUNT_FAILURE_MAP: dict["CreateAccountFailureReasonType", str] = {
    ...
    "GOVCLOUD_ACCOUNT_ALREADY_EXISTS": "The account in the Amazon Web Services GovCloud (US) Region could not be created because this"
    " Region already includes an account with that email address.",
    "INVALID_IDENTITY_FOR_BUSINESS_VALIDATION": "The Amazon Web Services account that owns your organization can't complete business license validation"
...
def paginate(
    client_func: Callable, key_to_pages: str, limit=None, page_size=None, next_token=None, **kwargs
) -> tuple[list, str | None]:
...
def children_list_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    ...
    return CommandResults(
        outputs=next_token_output_dict(
            "Children",
            next_token,
            [child | {"ParentId": args["parent_id"]} for child in children],
            "Id",
        ),
        readable_output=tableToMarkdown(
...
def parent_list_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    ...
    return CommandResults(
        ...
        outputs=[parent | {"ChildId": args["child_id"]} for parent in parents],
        readable_output=tableToMarkdown(
...
def organization_unit_get_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    ou = aws_client.describe_organizational_unit(OrganizationalUnitId=args["organization_unit_id"])
...
def account_list_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    ...
    def account_get() -> CommandResults:
        description = aws_client.describe_account(AccountId=args["account_id"])

        account = dict_values_to_str(description.get("Account", {}), "JoinedTimestamp")
    ...
    def account_list() -> CommandResults:
        ...
        accounts = [dict_values_to_str(account, "JoinedTimestamp") for account in accounts]
...
def account_remove_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    aws_client.remove_account_from_organization(AccountId=args["account_id"])

    return CommandResults(readable_output=f'AWS account *{args["account_id"]}* removed successfully.')
...
def account_move_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    ...
    return CommandResults(readable_output=f'AWS account *{args["account_id"]}* moved successfully.')
...
def account_create_command(args: dict, aws_client: "OrganizationsClient") -> PollResult:
    ...
    def polling_call() -> dict:
        account = aws_client.describe_create_account_status(CreateAccountRequestId=args["request_id"])
        return account["CreateAccountStatus"]
    ...
    def create_response(account: dict) -> PollResult:
        match account["State"]:
            case "SUCCEEDED":
                return PollResult(
                    response=account_list_command({"account_id": account["AccountId"]}, aws_client), continue_to_poll=False
                )
            case "FAILED":
            ...
            case _:
                return PollResult(response=None, continue_to_poll=True)
...
def account_close_command(args: dict, aws_client: "OrganizationsClient") -> PollResult:
    if not argToBoolean(args["is_closed"]):
        aws_client.close_account(AccountId=args["account_id"])
        args["is_closed"] = True

    account = aws_client.describe_account(AccountId=args["account_id"])

    return PollResult(
        response=CommandResults(readable_output=f'AWS account *{args["account_id"]}* closed successfully.'),
...
def organization_unit_create_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    ou = aws_client.create_organizational_unit(
        ParentId=args["parent_id"], Name=args["name"], **assign_params(Tags=build_tags(args.get("tags")))
    )
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(
            "AWS Organization Unit", ou["OrganizationalUnit"], ["Id", "Name", "Arn"], removeNull=True
        ),
    )


def organization_unit_delete_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    aws_client.delete_organizational_unit(OrganizationalUnitId=args["organizational_unit_id"])

    return CommandResults(readable_output=f'AWS organizational unit *{args["organizational_unit_id"]}* deleted successfully.')


def organization_unit_rename_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    aws_client.update_organizational_unit(OrganizationalUnitId=args["organizational_unit_id"], Name=args["name"])
...
def target_policy_list_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    ...
    return CommandResults(
        outputs=next_token_output_dict(
            "TargetPolicy",
            next_token,
            [policy | {"TargetId": args["target_id"]} for policy in policies],
        ),
        readable_output=tableToMarkdown(
...
def policy_get_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    response = aws_client.describe_policy(PolicyId=args["policy_id"])
...
def policy_delete_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    aws_client.delete_policy(PolicyId=args["policy_id"])

    return CommandResults(readable_output=f'AWS Organizations policy *{args["policy_id"]}* successfully deleted.')


def policy_attach_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    aws_client.attach_policy(PolicyId=args["policy_id"], TargetId=args["target_id"])

    return CommandResults(readable_output=f'AWS Organizations policy *{args["policy_id"]}* successfully attached.')
...
def policy_target_list_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    ...
    return CommandResults(
        outputs=next_token_output_dict(
            "PolicyTarget", next_token, [target | {"PolicyId": args["policy_id"]} for target in targets], "TargetId"
        ),
        readable_output=tableToMarkdown(
            f'AWS Organization *{args["policy_id"]}* Targets', targets, ["TargetId", "Arn", "Name", "Type"], removeNull=True
        ),
    )


def resource_tag_add_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    aws_client.tag_resource(ResourceId=args["resource_id"], Tags=build_tags(args["tags"]))

    return CommandResults(readable_output=f'AWS Organizations resource *{args["resource_id"]}* successfully tagged.')


def resource_tag_list_command(args: dict, aws_client: "OrganizationsClient") -> CommandResults:
    tags = aws_client.list_tags_for_resource(ResourceId=args["resource_id"], **assign_params(NextToken=args.get("next_token")))

    return CommandResults(
        outputs=next_token_output_dict(
            "Tag", tags.get("NextToken"), [tag | {"ResourceId": args["resource_id"]} for tag in tags.get("Tags", [])], "Key"
        ),
        readable_output=tableToMarkdown(
