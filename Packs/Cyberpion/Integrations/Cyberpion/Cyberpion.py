...
class Client(BaseClient):
    ...
    def get_domain_state(self, domain: str):
        params = {"verbosity": "details", "domain": domain}
        ...
        if type(reverse_ips) is dict:
            formatted_reverse_ips = "\n".join([f"{k}: {v}" for k, v in reverse_ips.items()])
        else:
        ...
        for idx, domain_type in enumerate(domain_types, start=1):
            domain_info += f"{idx}.\n"
            domain_info += "\n".join([f"{k}: {v}" for k, v in domain_type.items()])
    ...
    def get_action_items(
        ...
        for idx, action_item in enumerate(results):
            ...
            if type(technical_det) is dict:
                formatted_technical_details = "\n".join([f"{k}: {v}" for k, v in technical_det.items()])
            else:
    ...
    def get_domain_action_items(
        self, domain: str, min_severity: int, alert_types: list = None, show_only_active=True
    ) -> dict[str, Any]:
        # call API
        return {
            "Domain": domain,
            "Vulnerabilities": self.get_action_items(
                domain=domain,
...
def fetch_incidents(
    client: Client, max_fetch: int, min_severity: int, alert_types: list, show_only_active: bool, first_fetch: str = None
    ...
    for action_item in action_items:
        ...
        iso_format_data = datetime.strptime(creation_date, DATE_FORMAT).replace(tzinfo=UTC).isoformat()
        incident = {
...
def get_domain_state_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=markdown, outputs_prefix="Cyberpion", outputs_key_field="id", outputs={"DomainState": domain_state}
    )


def get_domain_action_items_command(
    client: Client, args: dict[str, Any], min_severity: int, alert_types: list = None, show_only_active: bool = True
) -> CommandResults:
    domain = args.get("domain")
    if not domain:
        raise ValueError("no domain specified")
    demisto.debug(f"getting action items for domain {domain}")
    domain_data = client.get_domain_action_items(
        domain=domain,
        min_severity=min_severity,
        show_only_active=show_only_active,
        alert_types=alert_types,
    ...
    return CommandResults(
        readable_output=markdown, outputs_prefix="Cyberpion.DomainData", outputs_key_field="id", outputs=domain_data
    )
...
def main() -> None:  # pragma: no cover
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            return_results(
                get_domain_action_items_command(
                    client, demisto.args(), min_severity=min_severity, alert_types=alert_types, show_only_active=show_only_active
                )
            )
