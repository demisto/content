...
class IronDefense:
            ...
            for row in range(num_rows):
                ...
                if len(column.get("values")) - row > 0:
                    ...
                    val = [
                        ...
                        for d in column.get("values")[row].get("data")
                    ]
    ...
    def update_analyst_ratings(
        ...
        self.logger.debug(
            f"Submitting analyst rating: Alert ID={alert_id} Severity={severity} Expected={expectation} "
            f"Comments={comments} Share w/IronDome={share_irondome}"
        )
        ...
        if response.status_code != 200:
            ...
            self.logger.error(
                f"Failed to rate alert ({alert_id}). The response failed with status code {response.status_code}."
    ...
    def add_comment_to_alert(self, alert_id, comment="", share_irondome=False):
        ...
        if response.status_code != 200:
            err_msg = self._get_error_msg_from_response(response)
            self.logger.error(
                f"Failed to add comment to alert ({alert_id}). The response failed with status code {response.status_code}."
                f" The response was: {response.text}"
            )
    ...
    def set_alert_status(self, alert_id, status="STATUS_NONE", comments="", share_irondome=False):
        self.logger.debug(
            f"Submitting status: Alert ID={alert_id} Status={status} Comments={comments} Share w/IronDome={share_irondome}"
        )
        ...
        if response.status_code != 200:
            err_msg = self._get_error_msg_from_response(response)
            self.logger.error(
                f"Failed to set status for alert ({alert_id}). The response failed with status code {response.status_code}."
                f" The response was: {response.text}"
            )
    ...
    def report_observed_bad_activity(
        ...
        self.logger.debug(
            f"Submitting observed bad activity: Name={name} Description={description} IP={ip} Domain={domain} "
        ...
        if response.ok:
            ...
            err_msg = self._get_error_msg_from_response(response)
            self.logger.error(
                f"Failed to submit observed bad activity for IP={ip} and Domain={domain}. The response failed with"
                f" status code {response.status_code}. The response was: {response.text}"
            )
            raise HTTPException(
                f"Failed to submit observed bad activity for IP={ip} and Domain={domain} ({response.status_code}): {err_msg}"
            )
    ...
    def get_event(self, event_id):
        ...
        if response.status_code != 200:
            err_msg = self._get_error_msg_from_response(response)
            self.logger.error(
                f"Failed to retrieve event with ID ({event_id}). The response failed with status code {response.status_code}."
                f" The response was: {response.text}"
            )
    ...
    def get_events(self, alert_id, limit=None, offset=None):
        ...
        if response.status_code != 200:
            err_msg = self._get_error_msg_from_response(response)
            self.logger.error(
                f"Failed to retrieve events with alert ID ({alert_id}). The response failed with status code"
    ...
    def get_alerts(
        ...
        self.logger.debug(
            f"Getting alerts: AlertID={alert_id} Category={category} SubCategory={sub_category} Status={status}"
            f" AnalystSeverity={analyst_severity} "
            f"AnalystExpectation={analyst_expectation} MinSeverity={min_severity} MaxSeverity={max_severity} "
            f"MinCreated={min_created} MaxCreated= {max_created} MinUpdated={min_updated}"
            f"MaxUpdated={max_updated} MinFirstEventCreated={min_first_event_created} "
            f"MaxFirstEventCreated={max_first_event_created} MinLastEventCreated={min_last_event_created}"
            f"MaxLastEventCreated={max_last_event_created} MinFirstEventStartTime={min_first_event_start_time}"
            f" MaxFirstEventStartTime={max_first_event_start_time} MinLastEventEndTime={min_last_event_end_time}"
        ...
        if response.ok:
            ...
            err_msg = self._get_error_msg_from_response(response)
            self.logger.error(
                f"Failed to retrieve alerts. The response failed with status code {response.status_code}."
                f" The response was: {err_msg}"
            )
    ...
    def get_alert_irondome_information(self, alert_id):
        ...
        if response.status_code != 200:
            err_msg = self._get_error_msg_from_response(response)
            self.logger.error(
                f"Failed to retrieve IronDome information for alert with ID ({alert_id}). The response failed "
                f"with status code {response.status_code}. The response was: {response.text}"
            )
            raise HTTPException(
                f"Failed to retrieve IronDome information for alert with ID {alert_id} ({response.status_code}): {err_msg}"
            )
        else:
...
def update_analyst_ratings_command():
    ...
    share_irondome_arg = argToBoolean(demisto.getArg("share_comment_with_irondome"))
    results = IRON_DEFENSE.update_analyst_ratings(
        alert_id, severity=severity, expectation=expectation, comments=comments, share_irondome=share_irondome_arg
    )
    demisto.results(results)
...
def add_comment_to_alert_command():
    ...
    share_irondome_arg = argToBoolean(demisto.getArg("share_comment_with_irondome"))
    results = IRON_DEFENSE.add_comment_to_alert(alert_id, comment=comment, share_irondome=share_irondome_arg)
    demisto.results(results)
...
def set_alert_status_command():
    ...
    comments = demisto.getArg("comments")
    share_irondome_arg = argToBoolean(demisto.getArg("share_comment_with_irondome"))
    results = IRON_DEFENSE.set_alert_status(alert_id, status=status, comments=comments, share_irondome=share_irondome_arg)
    demisto.results(results)
...
def get_event_command():
    ...
    event_readable_output = tableToMarkdown(
        f'IronDefense Event: {event.get("category")} - {event.get("sub_category")}\n{vue_markdown_link}', event
    )
