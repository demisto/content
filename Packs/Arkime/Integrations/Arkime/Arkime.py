...
class Client(BaseClient):
    ...
    def connections_csv_request(
        ...
        response = self._http_request("POST", "api/connections/csv", params=params, headers=headers, resp_type="response")

        return response

    def get_connections_request(
        self,
        source_field: Optional[str],
        destination_field: Optional[str],
        date: int,
    ...
    def get_files_request(self, length: int, start: int):
    ...
    def sessions_pcap_request(
        self, ids: Optional[str], expression: Optional[str], start_time: Optional[str], stop_time: Optional[str]
    ):
        params = assign_params(
            ids=ids,
            expression=expression,
    ...
    def get_fields_request(self, array_response: bool):
    ...
    def unique_field_multi_request(
        self,
        counts: int,
        expression_field_names: str,
    ...
    def remove_session_tags_request(
        self,
        tags: str,
        ids: Optional[str],
        segments: List[str],
...
def arrange_output_for_session_list_command(response: Dict) -> List:
    ...
    for record in response.get("data", []):
        # temp_record = copy.deepcopy(record)
        temp_record = {
            "firstPacket": epochToTimestamp(record["firstPacket"]),
...
def unique_field_helper(response: Response, start: int, limit: int, pagination_dict: dict) -> CommandResults:
    ...
    command_results = CommandResults(
        ...
        readable_output=create_paging_header(
            len(unique_field_list),
            pagination_dict.get("page_number", DEFAULT_OFFSET),
            pagination_dict.get("length", DEFAULT_LIMIT),
            pagination_dict.get("pagination", False),
        )
...
def connection_csv_get_command(
    client: Client,
    ...
    response = client.connections_csv_request(
...
def connection_list_command(
    ...
    """
    response = client.get_connections_request(
        source_field=source_field,
        destination_field=destination_field,
        date=date,
        expression=expression,
    ...
    mapping = {"id": "Source IP", "cnt": "Count", "sessions": "Sessions", "node": "Node"}
...
def pcap_file_list_command(client: Client, limit: int = 500, page_number: int = None, page_size: int = None) -> CommandResults:
    """
    ...
    command_results = CommandResults(
        ...
        outputs=response,
        raw_response=response,
        readable_output=create_paging_header(len(output_for_hr), start, length, is_pagination)
        + tableToMarkdown(
            "Files List Result:", output_for_hr, headerTransform=lambda header: mapping.get(header, header), headers=headers
        ),
    )
...
def session_list_command(
    ...
    if not pagination_dict.get("pagination", False) and pagination_dict.get("length", DEFAULT_LIMIT) > MAX_BATCH_LIMIT:
        response = responses_by_batches(
            client.sessions_query_request,
            length=length,
            start=start,
            date=date,
            expression=expression,
        ...
        response = client.sessions_query_request(
            date=date,
            expression=expression,
            start_time=start_time,
            stop_time=stop_time,
    ...
    command_results = CommandResults(
        ...
        readable_output=create_paging_header(len(output_for_hr), start, length, is_pagination)
        + tableToMarkdown(
            "Session List Result:", output_for_hr, headerTransform=lambda header: mapping.get(header, header), headers=headers
        ),
    )
...
def sessions_csv_get_command(
    ...
    response = client.sessions_csv_request(
        date=date,
        expression=expression,
        start_time=start_time,
        stop_time=stop_time,
...
def sessions_pcap_get_command(
    client: Client, ids: str = None, expression: str = None, start_time: str = None, stop_time: str = None
) -> Dict:
    response = client.sessions_pcap_request(
        ids=ids,
        expression=expression,
        start_time=start_time,
        stop_time=stop_time,
...
def spigraph_get_command(
    ...
    response = client.spi_graph_request(
        ids=field,
        date=date,
        expression=expression,
        start_time=start_time,
...
def spiview_get_command(
    ...
    response = client.spi_view_request(
        spi=spi,
        date=date,
        expression=expression,
        start_time=start_time,
...
def fields_list_command(client: Client, array_response: bool = False) -> CommandResults:
...
def unique_field_list_command(
    ...
    response = client.unique_field_request(
        counts=counts,
    ...
    return unique_field_helper(
        response, pagination_dict.get("start", DEFAULT_OFFSET), pagination_dict.get("length", DEFAULT_LIMIT), pagination_dict
    )


def multi_unique_field_list_command(
    client: Client,
    expression_field_names: str,
    counts: int = DEFAULT_COUNTS,
...
def session_tag_add_command(
    ...
    mapping = {"success": "Success", "text": "Text"}
    command_results = CommandResults(
...
def session_tag_remove_command(
    client: Client,
    tags: str,
    session_ids: str = None,
    segments: List[str] = DEFAULT_SEGMENTS,
    ...
    response = client.remove_session_tags_request(
        tags=tags,
        ids=session_ids,
        segments=segments,
        date=date,
