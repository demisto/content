...
def test_connection_csv_get_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(
        arkime_client, "_http_request", return_value=util_load_csv("test_data/connection_csv_get.csv")
    )
...
def test_connection_list_command_with_default_start(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(
        arkime_client, "_http_request", return_value=util_load_json("test_data/connection_list.json")
    )
    args = {
        "baseline_date": "720",
        "start_time": "1648817940",
    ...
    assert (
        res.readable_output == "### Connection Results:\n"
...
def test_pcap_file_list_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(
        arkime_client, "_http_request", return_value=util_load_json("test_data/pcap_file_list.json")
    )
...
def test_session_list_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(arkime_client, "_http_request", return_value=util_load_json("test_data/session_list.json"))

    res = session_list_command(arkime_client, start_time="1650190238", stop_time="1650363038")

    params = {"date": 1, "length": 100, "start": 0, "startTime": "1650190238", "stopTime": "1650363038"}

    http_request.assert_called_with("POST", "api/sessions", params=params, headers=HEADERS)

    assert res.readable_output == (
        "Showing 1 results, limit=100\n"
        "### Session List Result:\n"
...
def test_session_csv_get_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(arkime_client, "_http_request", return_value=util_load_csv("test_data/sessions_list.csv"))

    args = {
        "start_time": "1650190238",
...
def test_session_pcap_get_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(
        arkime_client, "_http_request", return_value=util_load_txt_to_json("test_data/raw_session_data.pcap")
    )
...
def test_spigraph_get_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(arkime_client, "_http_request", return_value=util_load_json("test_data/spi_graph.json"))
    args = {
        "start_time": "1648817940",
        "stop_time": "1649595540",
...
def test_spiview_get_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(arkime_client, "_http_request", return_value=util_load_json("test_data/spi_view.json"))

    args = {
        "start_time": "1650868312",
...
def test_field_list_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(arkime_client, "_http_request", return_value=util_load_json("test_data/field_list.json"))
...
def test_unique_field_list_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(
        arkime_client, "_http_request", return_value=util_load_txt_to_json("test_data/unique_field_list.txt")
    )
...
def test_session_tag_add_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(
        arkime_client, "_http_request", return_value=util_load_json("test_data/session_tag_add.json")
    )
    ...
    params = {
        ...
        "date": 1,
        "strictly": False,
        "tags": {"ids": "220516-QHSdz21pJ_xCtJGoL8mbmyNv", "segments": ["no"], "tags": "deletethistag"},
    }
...
def test_session_tag_remove_command(mocker, arkime_client):
    ...
    http_request = mocker.patch.object(
        arkime_client, "_http_request", return_value=util_load_json("test_data/session_tag_remove.json")
    )
        ...
        items=[
            {
                ...
                "graph": {
                    ...
                    "source.packetsHisto": [
                        [1648911600000, 237763],
                        [1648915200000, 498464],
                    ],
                    "destination.packetsHisto": [
                        [1648911600000, 282884],
                        [1648915200000, 567752],
                    ],
                    "network.packetsTotal": 150692542,
                    "source.bytesHisto": [
                        [1648911600000, 42048340],
                        [1648915200000, 77687566],
                    ],
                    "destination.bytesHisto": [
                        [1648911600000, 116097823],
                        [1648915200000, 238375971],
                    ],
                    "network.bytesTotal": 45699563388,
                    "client.bytesHisto": [
                        [1648911600000, 4365402],
                        [1648915200000, 12113797],
                    ],
                    "server.bytesHisto": [
                        [1648911600000, 79323099],
                        [1648915200000, 163262642],
                    ],
...
@pytest.mark.parametrize(
    "page_number_input, page_size_input, start, length, page_number, page_size", calculate_offset_and_limit_input
)
def test_calculate_offset_and_limit(
    page_number_input: int, page_size_input: int, start: int, length: int, page_number: int, page_size: int
):
...
only_one_response = (request_method_helper, 500, 0, {"first_field": "a", "second_field": ["a"], "third_field": {"a": {1: None}}})
more_than_one_response = (
    request_method_helper,
    1500,
    0,
    {"first_field": "aaa", "second_field": ["a", "a", "a"], "third_field": {"a": {1: None}}},
)
