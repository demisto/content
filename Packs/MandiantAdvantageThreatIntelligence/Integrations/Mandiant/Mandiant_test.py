...
MOCK_CVE_RESPONSE = {
    ...
    "associated_actors": [
        {
            "aliases": [{"attribution_scope": "confirmed", "name": "Alias1"}],
            "country_code": "PK",
...
MOCK_THREATACTOR_RESPONSE = {
    ...
    "cve": [{"attribution_scope": "confirmed", "cve_id": "CVE-2024-1234", "id": "vulnerability--12345678"}],
    "observed": [
        {
...
MOCK_CAMPAIGN_RESPONSE = {
    ...
    "aliases": {"releasable": True, "malware": [], "campaign": [], "actor": []},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"reports": [{"title": "Report 1", "report_id": "12345", "published_date": "2024-07-16"}]}, 1),
...
def test_ip_reputation_command_success(client, mock_http_request):
    ...
    assert result.indicator.relationships[1]._name == "indicator-of"
    assert result.indicator.relationships[1]._reverse_name == "indicated-by"
    ...
    assert result.readable_output == (
        "### Mandiant Advantage Threat Intelligence information for 1.2.3.4\n[View on Man"
...
def test_domain_reputation_command_success(client, mock_http_request):
    ...
    assert result.indicator.dbot_score.reliability == "A - Completely reliable"
    assert result.readable_output == (
        "### Mandiant Advantage Threat Intelligence information for domain.test\n[View on"
...
def test_url_reputation_command_success(client, mock_http_request):
    ...
    assert result.readable_output == (
        "### Mandiant Advantage Threat Intelligence information for https://domain.test/t"
...
def test_file_reputation_command_success(client, mock_http_request):
    ...
    assert result.indicator.dbot_score.score == 2
    assert result.readable_output == (
        "### Mandiant Advantage Threat Intelligence information for ae1747c930e9e4f45fbc9"
...
def test_mati_threat_actor_build_attack_pattern_relationships(client, mocker):
    ...
    mock_get_mitre_attack_patterns.return_value = {
        "iocs": [{"id": "", "value": "", "fields": {"mitreId": "T1529", "name": "ttp name"}}]
    }
...
def test_mati_threat_actor_build_indicator(client, mocker):
    ...
    assert result.get("fields", {}).get("Publications") == [
        {"source": "Mandiant", "title": "REPORT TITLE", "link": f"{ADV_BASE_URL}/reports/23-00000722", "timestamp": 1673631860.0}
    ]
...
def test_mati_malware_build_indicator(client, mocker):
    ...
    assert result.get("fields", {}).get("Tags") == ["TARGET_INDUSTRY"]
    assert result.get("fields", {}).get("Publications") == [
        {"source": "Mandiant", "title": "REPORT_TITLE", "link": f"{ADV_BASE_URL}/reports/22-0000000", "timestamp": 1659371639.0}
    ]
    assert result.get("fields", {}).get("Industry sectors", []) == ["TARGET_INDUSTRY"]
