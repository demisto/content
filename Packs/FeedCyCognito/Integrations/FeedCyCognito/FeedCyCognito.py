...
class CyCognitoFeedClient(BaseClient):
    ...
    def get_indicators(
        ...
        filters: list[Dict[str, Union[str, list]]] = None,
    ) -> Any:
        ...
        return self._http_request(method="POST", url_suffix=f"assets/{asset_type}", params=query_params, json_data=filters)
...
def validate_get_indicators_arguments(
    asset_type: str = None,
    count: Optional[int] = None,
    sort_order: str = None,
    hosting_type: list[str] = None,
...
def prepare_body_filters_for_get_indicators(
    asset_type: str = None,
    ...
    for field, value in zip(fields_with_in_operator, values_with_in_operator):
        if value:
            req_body.append({"field": field, "op": "in", "values": value})
...
def build_iterators(
    response: list[Dict[str, Any]], feed_tags: list[str], tlp_color: str, default_mapping: bool
) -> list[Dict[str, Any]]:
    """
    ...
    for asset in response:
        indicator = {
            ...
            "fields": {
                ...
                "feedcycognitoassetdiscoverability": discoverability
                if (discoverability := asset.get("discoverability"))
                else "Unknown",
                "feedcycognitoexpiration": asset.get("expiration"),
                "firstseenbysource": asset.get("first_seen"),
                "feedcycognitodynamicallyresolved": dynamically_resolved
                if (dynamically_resolved := asset.get("dynamically_resolved"))
                else "",
...
def get_indicators_command(client: CyCognitoFeedClient, args: Dict[str, Any]) -> CommandResults:
    ...
    validate_get_indicators_arguments(
    ...
    filters = prepare_body_filters_for_get_indicators(
        asset_type=asset_type,
    ...
    response = client.get_indicators(
        asset_type=asset_type, count=count, offset=offset, sort_param=(sort_by, sort_order), search=search, filters=filters
    )
...
def fetch_indicators_command(
    client: CyCognitoFeedClient, params: Dict[str, Any], last_run: Dict[str, Any], is_test: bool = False
    ...
    validate_get_indicators_arguments(
        asset_type=asset_type, hosting_type=hosting_type, security_grade=security_grade, count=max_fetch, status=[]
    )
