...
def main():
    try:
        entity_id = demisto.incident().get("CustomFields", {}).get("vectraruxentityid")
        entity_type = demisto.incident().get("CustomFields", {}).get("vectraruxentitytype")
        ...
        result = map_and_update_entity_assignments(entity, "Vectra RUX - Incoming Mapper", "Vectra RUX Events Detection")
        return_results(result)
    except Exception as ex:
        demisto.error(traceback.format_exc())  # print the traceback
        return_error(f"Failed to execute VectraRUXSyncEntityAssignment. Error: {str(ex)}")
