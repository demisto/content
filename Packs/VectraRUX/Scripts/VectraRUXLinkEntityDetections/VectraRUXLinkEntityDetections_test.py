...
from VectraRUXLinkEntityDetections import main


def test_main_with_incidents_found_and_linked(mocker):
    """
    Given:
    - Arguments with incident_entity_id and incident_detection_id.
    - SearchIncidentsV2 returns incidents with matching entity and priority status.

    When:
    - Calling the 'main' function of the VectraRUXLinkEntityDetections script.
    ...
    mocker.patch.object(
        demisto,
        "args",
        return_value={"incident_entity_id": "123", "incident_detection_id": "456"},
    )
    ...
    return_results_mock = mocker.patch("VectraRUXLinkEntityDetections.return_results")
    ...
    expected_query = '-status:closed -category:job vectraruxentityid:"123" -vectraruxdetectionid:="456"'
...
def test_main_with_no_incidents_found(mocker):
    ...
    execute_command_mock.assert_called_once_with(
        "SearchIncidentsV2",
        {"query": '-status:closed -category:job vectraruxentityid:"123" -vectraruxdetectionid:="456"'},
    )
...
def test_main_with_empty_content(mocker):
    ...
    mocker.patch.object(
        demisto,
        "args",
        return_value={"incident_entity_id": "999", "incident_detection_id": "888"},
    )
...
def test_main_with_single_incident(mocker):
    ...
    mocker.patch.object(
        demisto,
        "args",
        return_value={"incident_entity_id": "100", "incident_detection_id": "200"},
    )
    ...
    mocker.patch("VectraRUXLinkEntityDetections.return_results")
...
def test_main_with_numeric_incident_ids(mocker):
    ...
    mocker.patch.object(
        demisto,
        "args",
        return_value={"incident_entity_id": "50", "incident_detection_id": "60"},
    )
...
def test_main_query_format(mocker):
    """
    Given:
    - Arguments with specific incident_entity_id and incident_detection_id values.
    ...
    mocker.patch.object(
        demisto,
        "args",
        return_value={"incident_entity_id": "entity_abc", "incident_detection_id": "detection_xyz"},
    )
    ...
    assert 'vectraruxentityid:"entity_abc"' in query
