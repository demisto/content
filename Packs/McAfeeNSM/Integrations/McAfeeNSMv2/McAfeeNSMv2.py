...
DEPLOY_ARGUMENT_MAPPER = {
    "push_ssl_key": "SSLPercentageComplete",
...
ADDRESS_LIST_MAP = {
    "IPv6AddressRange": "IPV6RangeList",
...
class Client(BaseClient):
    ...
    def update_alerts_request(
        self, time_period: str, start_time: str, end_time: str, state: str, search: str, filter_arg: str, body: dict
    ) -> dict:
    ...
    def assign_device_policy_request(
        self, domain_id: int, device_id: int, pre_firewall_policy: Optional[str], post_firewall_policy: Optional[str]
    ) -> dict:
        ...
        json_data = {"firewallPolicyLast": post_firewall_policy, "firewallPolicyFirst": pre_firewall_policy}
    ...
    def assign_interface_policy_request(
        self,
        domain_id: int,
        interface_id: int,
        firewall_policy: Optional[str],
        firewall_port_policy: Optional[str],
        ips_policy: Optional[str],
        custom_policy_json: Optional[dict],
    ) -> dict:
        ...
        json_data = {"firewallPolicy": firewall_policy, "firewallPortPolicy": firewall_port_policy, "ipsPolicy": ips_policy}
    ...
    def deploy_device_configuration_request(
        self,
        device_id: int,
        is_SSL_Push_Required: bool = False,
        is_GAM_Update_Required: bool = False,
        is_Sigset_Config_Push_Required: bool = False,
        is_Botnet_Push_Required: bool = False,
    ) -> dict:
        ...
        """
        json_data = {
            "isSSLPushRequired": is_SSL_Push_Required,
...
def alerts_list_pagination(
    ...
    if page and page_size:
        ...
        while (offset + page_size > 1000) and (len(records_list) == 1000):
            ...
            response = client.get_alerts_request(time_period, start_time, end_time, state, search, filter_arg, domain_id, "next")
...
def check_source_and_destination(
    source_rule_object_id: Optional[int],
    source_rule_object_type: Optional[str],
    destination_rule_object_id: Optional[int],
    destination_rule_object_type: Optional[str],
...
def create_body_firewall_policy(
    domain: int,
    name: str,
    visible_to_child: bool,
    description: str,
    is_editable: bool,
    policy_type: str,
            ...
            "MemberRuleList": [
                {
                    ...
                    "ServiceObjectList": [{"RuleObjectId": "-1", "Name": "Any", "RuleObjectType": None, "ApplicationType": None}],
                    "ApplicationObjectList": [],
                    "TimeObjectList": [{"RuleObjectId": "-1", "Name": "Always", "RuleObjectType": None}],
                }
            ]
        },
    }
...
def create_body_create_rule(rule_type: str, address: List, number: int, from_to_list: list[dict[str, Optional[Any]]]) -> tuple:
    ...
    """
    if HOST in rule_type:
        return f"HostIPv{number}", {f"hostIPv{number}AddressList": address}
    elif ADDRESS_RANGE in rule_type:
        return f"IPv{number}AddressRange", {f"IPV{number}RangeList": from_to_list}
    else:
        return f"Network_IPV_{number}", {f"networkIPV{number}List": address}


def create_body_create_rule_for_v10(
    rule_type: str, address: List, number: int, from_to_list: List[dict[str, Optional[Any]]], state: str = "Enabled"
) -> tuple:
    ...
    list_to_send: list[dict] = [{"value": single_address, "state": STATE_TO_NUMBER.get(state)} for single_address in address]
    ...
    if HOST in rule_type:
        return f"HostIPv{number}", {f"hostIPv{number}AddressList": list_to_send}
    elif ADDRESS_RANGE in rule_type:
        return f"IPv{number}AddressRange", {f"IPV{number}RangeList": from_to_list}
    else:
        return f"Network_IPV_{number}", {f"networkIPV{number}List": list_to_send}


def create_body_update_rule_for_v10(
    rule_type: str, address: List, number: int, from_to_list: List[dict[str, Optional[Any]]], state: str = "Enabled"
) -> tuple:
    ...
    for single_address in address:
        if type(single_address) is dict:  # if its a dict == its an existing address to overwrite, we saved from the 'get' call
            list_to_send.append({"value": single_address.get("value"), "state": STATE_TO_NUMBER.get(state), "changedState": 3})
        else:  # its a new address the user wants to add
            list_to_send.append({"value": single_address, "state": STATE_TO_NUMBER.get(state), "changedState": 1})
...
def modify_v10_results_to_v9_format(response: List[dict[Any, Any]]) -> List[dict[Any, Any]]:
    ...
    """
    key_list = ["IPv6AddressRange", "HostIPv6", "Network_IPV_6", "Network_IPV_4", "HostIPv4", "IPv4AddressRange"]
...
def capitalize_key_first_letter(input_lst: List[dict], check_lst: List = []) -> List[dict]:
    ...
    for my_dict in input_lst:
        my_dict = (
            {k[:1].upper() + k[1:]: v for k, v in my_dict.items() if k in check_lst}
            if check_lst
...
def check_args_create_rule(rule_type: str, address: List, from_address: str, to_address: str, number: int):
    ...
    if (HOST in rule_type or "NETWORK" in rule_type) and (not address or from_address or to_address):
        raise Exception(
            f'If the "rule_object_type" is “Endpoint IP V.{number}” or “Network IP V.{number}” than only'
    ...
    if ADDRESS_RANGE in rule_type and (not to_address or not from_address or address):
        raise Exception(
            f'If the "rule_object_type" is “Range IP V.{number}” than only the arguments '
...
def h_r_get_domains(children: List[dict], contents: List):
    ...
    for child in children:
        ...
        del child["name"]
        d = {"ID": child.get("ID"), "Name": child.get("Name")}
        contents.append(d)
        if child.get("childdomains", []):
            h_r_get_domains(child.get("childdomains", []), contents)


def update_source_destination_object(
    obj: List[dict], rule_object_id: Optional[int], rule_object_type: Optional[str]
) -> List[dict]:
...
def overwrite_source_destination_object(
    rule_object_id: Optional[int], rule_object_type: Optional[str], dest_or_src: str, member_rule_list: dict
) -> List:
    ...
    if rule_object_id:
        if rule_object_id == -1:
            return [{"RuleObjectId": -1, "RuleObjectType": None}]
        else:
            return [{"RuleObjectId": rule_object_id, "RuleObjectType": rule_object_type}]
    else:
...
def list_domain_firewall_policy_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name="Firewall Policies List", t=contents, removeNull=True, headers=headers)
...
def get_firewall_policy_command(client: Client, args: dict) -> CommandResults:
    ...
    contents = {
        "FirewallPolicyId": response.get("FirewallPolicyId"),
    ...
    readable_output = tableToMarkdown(name=f"Firewall Policy {policy_id}", t=contents, removeNull=True, headers=headers)
    return CommandResults(
        readable_output=readable_output,
        outputs_prefix="NSM.Policy",
        outputs=response,
...
def create_firewall_policy_command(client: Client, args: dict) -> CommandResults:
    ...
    check_source_and_destination(
        source_rule_object_id, source_rule_object_type, destination_rule_object_id, destination_rule_object_type, "create"
    )
    ...
    destination_rule_object_type = (
        rule_object_type_cases(destination_rule_object_type, "up") if destination_rule_object_type else None
    )
    ...
    return CommandResults(
        readable_output=f"The firewall policy no.{new_firewall_policy_id} was created successfully",
        outputs_prefix="NSM.Policy",
        outputs=response,
        raw_response=response,
        outputs_key_field="FirewallPolicyId",
...
def update_firewall_policy_command(client: Client, args: dict) -> CommandResults:
    ...
    check_source_and_destination(
        source_rule_object_id, source_rule_object_type, destination_rule_object_id, destination_rule_object_type, "update"
    )
    ...
    visible_to_child = policy_get_details.get("VisibleToChild") if not visible_to_child else argToBoolean(visible_to_child)
    ...
    if is_overwrite:
        source_object = overwrite_source_destination_object(
            source_rule_object_id, source_rule_object_type, "Source", member_rule_list
        )
        destination_object = overwrite_source_destination_object(
            destination_rule_object_id, destination_rule_object_type, "Destination", member_rule_list
        )
    else:
        ...
        destination_object = update_source_destination_object(
            destination_object, destination_rule_object_id, destination_rule_object_type
        )
...
def list_domain_rule_objects_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name="List of Rule Objects", t=contents, removeNull=True, headers=headers)
...
def get_rule_object_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name=f"Rule Objects {rule_id}", t=contents, removeNull=True, headers=headers)
    return CommandResults(
        readable_output=readable_output,
        outputs_prefix="NSM.Rule",
        outputs=response,
...
def create_rule_object_command(client: Client, args: dict) -> CommandResults:
    ...
    if (
        (address_ip_v_4 and address_ip_v_6)
        or (from_address_ip_v_4 and from_address_ip_v_6)
        or (to_address_ip_v_4 and to_address_ip_v_6)
    ):
        raise Exception(
            "Those pairs of arguments (address_ip_v_4 and address_ip_v_6) or "
    ...
    from_to_list = [{"FromAddress": from_address, "ToAddress": to_address}]
    # create the body according to the version of the NSM
    if VERSION == V10:
        d_name, extra_body = create_body_create_rule_for_v10(
            rule_type=rule_type, address=address, number=number, from_to_list=from_to_list, state=state
        )
    else:
...
def update_rule_object_command(client: Client, args: dict) -> CommandResults:
    ...
    if (rule_type == "HOST_IPV_4" or rule_type == "NETWORK_IPV_4") and (
        ...
        raise Exception(
            "If the rule object type is Endpoint IP V.4 or Network IP V.4 than only the argument "
        ...
        raise Exception(
            'If the rule object type is Range IP V.4 than both "from_address_ip_v_4" and '
            '"to_address_ip_v_4" must contain a value or be empty.'
        )
    elif (rule_type == "IPV_4_ADDRESS_RANGE") and (address_ip_v_4 or address_ip_v_6 or from_address_ip_v_6 or to_address_ip_v_6):
        raise Exception(
            'If the rule object type is Range IP V.4 than only the arguments "from_address_ip_v_4" and '
        ...
        raise Exception(
            "If the rule object type is Endpoint IP V.6 or Network IP V.6 than only the argument "
        ...
        raise Exception(
            'If the rule object type is Range IP V.6 than both "from_address_ip_v_6" and '
            '"to_address_ip_v_6" must contain a value or be empty.'
        )
    elif (rule_type == "IPV_6_ADDRESS_RANGE") and (address_ip_v_4 or address_ip_v_6 or from_address_ip_v_4 or to_address_ip_v_4):
        raise Exception(
            'If the rule object type is Range IP V.6 than only the arguments "from_address_ip_v_6" and '
    ...
    if (VERSION == V9 and is_overwrite) or (VERSION == V10 and not is_overwrite):
        if rule_type == "HOST_IPV_4":
            address_ip_v_4 = address_ip_v_4 if address_ip_v_4 else response_get.get("HostIPv4", {}).get("hostIPv4AddressList")
        if rule_type == "NETWORK_IPV_4":
            address_ip_v_4 = address_ip_v_4 if address_ip_v_4 else response_get.get("Network_IPV_4", {}).get("networkIPV4List")
        if from_address_ip_v_4:
            from_to_address_ip_v_4 = [{"FromAddress": from_address_ip_v_4, "ToAddress": to_address_ip_v_4}]
        ...
        if rule_type == "HOST_IPV_6":
            address_ip_v_6 = address_ip_v_6 if address_ip_v_6 else response_get.get("HostIPv6", {}).get("hostIPv6AddressList")
        if rule_type == "NETWORK_IPV_6":
            address_ip_v_6 = address_ip_v_6 if address_ip_v_6 else response_get.get("Network_IPV_6", {}).get("networkIPV6List")
        if from_address_ip_v_6:
            from_to_address_ip_v_6 = [{"FromAddress": from_address_ip_v_6, "ToAddress": to_address_ip_v_6}]
    ...
    # in v10 we do the same thing if the user wants to overwrite the addresses
        ...
        if rule_type == "HOST_IPV_4":
            ...
            if from_address_ip_v_4 and to_address_ip_v_4:
                from_to_address_ip_v_4.append({"FromAddress": from_address_ip_v_4, "ToAddress": to_address_ip_v_4})
            ...
            if from_address_ip_v_6 and to_address_ip_v_6:
                from_to_address_ip_v_6.append({"FromAddress": from_address_ip_v_6, "ToAddress": to_address_ip_v_6})
    ...
    # create the body according to the version of the NSM
    if VERSION == V10:
        d_name, extra_body = create_body_update_rule_for_v10(
            rule_type=rule_type, address=address, number=number, from_to_list=from_to_list, state=state
        )
...
def get_alerts_command(client: Client, args: dict) -> CommandResults:
    ...
    alerts_list = alerts_list_pagination(
        response.get("alertsList", []),
        limit,
        page,
        page_size,
        time_period,
    ...
    headers = ["ID", "Name", "Event Time", "Severity", "State", "Direction", "Result", "Attack Count", "Attacker IP", "Target IP"]
    if args.get("new_state"):
    ...
    readable_output = tableToMarkdown(name=title, t=contents, removeNull=True, headers=headers)
    return CommandResults(
        readable_output=readable_output,
...
def get_alert_details_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name=f"Alert no.{alert_id}", t=contents, removeNull=True, headers=headers)
    return CommandResults(
        readable_output=readable_output,
        outputs_prefix="NSM.Alerts",
        outputs=response,
...
def get_attacks_command(client: Client, args: dict) -> List:
    ...
    readable_outputs = tableToMarkdown(name=title, t=contents, removeNull=True, headers=headers)
    if not attack_id:
        file_ = fileResult(filename="get-attacks-file-result", data=readable_outputs, file_type=entryTypes["entryInfoFile"])
        return [file_]
    else:
        return [
            CommandResults(
...
def get_domains_command(client: Client, args: dict) -> CommandResults:
    ...
    if domain_id is not None:
        ...
        results = {"ID": results.get("id"), "Name": results.get("name"), "childdomains": results.get("childdomains")}
        contents = [{"ID": results.get("ID"), "Name": results.get("Name")}]
    else:
        title = "List of Domains"
        children = [results]
        h_r_get_domains(children, contents)
    contents = pagination(contents, limit, page, page_size)
    readable_outputs = tableToMarkdown(name=title, t=contents, removeNull=True)
    return CommandResults(
        readable_output=readable_outputs,
...
def get_ips_policies_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        name=f"IPS Policies List of Domain no.{domain_id}", t=contents, removeNull=True, headers=headers
    )
    return CommandResults(
        readable_output=readable_output,
...
def get_ips_policy_details_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name=f"IPS Policy no.{policy_id} Details", t=contents, removeNull=True, headers=headers)
    return CommandResults(
        readable_output=readable_output,
        outputs_prefix="NSM.IPSPolicies",
...
def update_alerts_command(client: Client, args: dict) -> CommandResults:
    ...
    body = {"alertState": new_state, "assignTo": new_assignee}
...
def list_pcap_file_command(client: Client, args: dict) -> CommandResults:
    ...
    for file_name in files_list:
        ...
        contents.append(d)
    readable_output = tableToMarkdown(name="PCAP files List", t=contents, removeNull=True)
    return CommandResults(
        readable_output=readable_output, outputs_prefix="NSM.PcapFile", outputs=files_list, raw_response=files_list
    )
...
def list_domain_device_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name="Domain devices List", t=capitalize_devices, removeNull=True)
...
def list_device_interface_command(client: Client, args: dict) -> CommandResults:
    ...
    capitalize_interfaces = (
        capitalize_key_first_letter(interfaces, key_list)
        if all_results
        else capitalize_key_first_letter(interfaces, key_list)[:limit]
    )

    readable_output = tableToMarkdown(name="Device interfaces List", t=capitalize_interfaces, removeNull=True)

    return CommandResults(
        readable_output=readable_output, outputs_prefix="NSM.Interface", outputs=capitalize_interfaces, raw_response=response
    )
...
def assign_device_policy_command(client: Client, args: dict) -> CommandResults:
    ...
    response = client.assign_device_policy_request(
        domain_id=domain_id,
        device_id=device_id,
...
def list_device_policy_command(client: Client, args: dict) -> CommandResults:
    ...
    capitalize_policies = (
        capitalize_key_first_letter(all_policies) if all_results else capitalize_key_first_letter(all_policies)[:limit]
    )

    readable_output = tableToMarkdown(name="Device policy List", t=capitalize_policies, removeNull=True)

    return CommandResults(
        readable_output=readable_output, outputs_prefix="NSM.DevicePolicy", outputs=capitalize_policies, raw_response=response
    )
...
def assign_interface_policy_command(client: Client, args: dict) -> CommandResults:
    ...
    response = client.assign_interface_policy_request(
        domain_id=domain_id,
...
def list_interface_policy_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(name="Interface policy List", t=capitalize_policies, removeNull=True),
...
def get_device_configuration_command(client: Client, args: dict) -> CommandResults:
    ...
    flattened_response = flatten_and_capitalize(main_dict=capitalize_response, inner_dict_key="PendingChanges")

    readable_output = tableToMarkdown(name="Device Configuration", t=flattened_response, removeNull=True)
...
def deploy_device_configuration_command(args: dict, client: Client) -> PollResult:
    ...
    status = client.check_deploy_device_configuration_request_status(device_id=device_id, request_id=request_id)

    fail_or_success_list, message_to_return = deploy_polling_message(args=args, status=status)
    message = CommandResults(readable_output=f"{message_to_return}\n\nChecking again in {INTERVAL} seconds...")
    ...
    if not all(fail_or_success_list):  # if one of the arguments was not fully deployed yet, the polling will continue
        return PollResult(
            ...
            args_for_next_run={"request_id": request_id, "device_id": device_id, **args},
        )

    message = CommandResults(readable_output="The device configuration has been deployed successfully.")
    return PollResult(response=message, continue_to_poll=False)
...
def main() -> None:  # pragma: no cover
    ...
    try:
        headers: dict = {"Accept": "application/vnd.nsm.v1.0+json", "Content-Type": "application/json"}
