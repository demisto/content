...
def http_request(method, request_suffix, params=None, data=None, should_convert_to_json=True):
    ...
    # the NetcraftTest API gets the arguments as params for GET requests, as data for POST
...
def generate_escalate_takedown_human_readable(response):
    if "status" in response:
        ...
        human_readable = "### Takedown escalation failed\n" \
                         ...
                         "Error code is: {0}\n" \
                         "Error message from NetcraftTest is: {1}".format(response["error_code"], response["error_message"])
    return human_readable
...
def generate_netcraft_context_with_notes(list_of_notes_contexts):
    all_takedowns_entry_context = demisto.context().get("NetcraftTest", {}).get("Takedown", {})
...
def get_takedown_notes_command():
    ...
    entry_context = {
        "NetcraftTest.Takedown(val.ID == obj.ID)": generate_netcraft_context_with_notes(list_of_notes_contexts)
    }
...
def get_takedown_info_command():
    ...
    entry_context = {
        'NetcraftTest.Takedown(val.ID == obj.ID)': list_of_takedowns_contexts
    }
    return_outputs(
        readable_output=human_readable,
...
def report_attack_command():
    ...
    if result_answer == MALICIOUS_REPORT_SUCCESS:
        ...
        entry_context = {
            'NetcraftTest.Takedown(val.ID == obj.ID)': list_of_new_takedown_contexts
        }
    else:
