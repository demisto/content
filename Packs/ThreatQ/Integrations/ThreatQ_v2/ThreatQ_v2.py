...
TABLE_HEADERS = {
    "indicator": ["ID", "Type", "Value", "Description", "Status", "TQScore", "CreatedAt", "UpdatedAt", "URL"],
...
OBJ_DIRECTORY = {"indicator": "indicators", "adversary": "adversaries", "event": "events", "attachment": "attachments"}

RELATED_KEY = {"indicator": "RelatedIndicator", "adversary": "RelatedAdversary", "event": "RelatedEvent"}
...
TABLE_TLP = {4: "WHITE", 3: "GREEN", 2: "AMBER", 1: "RED"}
...
def get_tlp_from_indicator(sources):
    ...
    for source in sources:
        try:
            tlp = max(tlp, int(source.get("TLP")))
...
def make_indicator_reputation_request(indicator_type, value, generic_context):
    ...
    if indicator_type == "ip":
        ...
        if is_httpx:
            body = {"criteria": {"+or": [{"value": value}, {"value": value_without_proto}]}, "filters": {"type_name": tq_type}}
        else:
            body = {"criteria": {"value": value}, "filters": {"type_name": tq_type}}
    ...
    res = tq_request(method="POST", url_suffix=url_suffix, params=body)
    ...
    entry_context = aggregate_search_results(
        indicators=indicators, default_indicator_type=indicator_type, generic_context=generic_context
    )

    readable = build_readable(
        readable_title=f"Search results for {indicator_type} {value}", obj_type="indicator", data=indicators
    )
...
def create_dbot_context(indicator, ind_type, ind_score, ind_status):
    ...
    """
    dbot_score_map = {-1: 0, 0: 1, 1: 1, 2: 1, 3: 1, 4: 2, 5: 2, 6: 2, 7: 2, 8: 2, 9: 2, 10: 2}
...
def attributes_to_demisto_format(lst):
    if lst is None:
        return None
    return [{"Name": elem.get("name"), "Value": elem.get("value"), "ID": elem.get("id")} for elem in lst]
...
def get_malicious_data(tq_score):
    malicious_data = {"Malicious": {"Vendor": "ThreatQ v2", "Description": f"Score from ThreatQ is {tq_score}"}}
...
def get_indicator_type_id(indicator_name: str) -> str:
    indicator_types_res = tq_request(method="GET", url_suffix="/indicator/types", retrieve_entire_response=True)
    try:
        indicator_types = indicator_types_res.json().get("data")
    except ValueError:
        raise ValueError(
            f"Could not parse data from ThreatQ [Status code: {indicator_types_res.status_code}]"
...
def aggregate_search_results(indicators, default_indicator_type, generic_context=None):
    ...
    for i in indicators:
        ...
        entry_context.append(
            set_indicator_entry_context(
                indicator_type=i.get("Type") or default_indicator_type, indicator=i, generic_context=generic_context
            )
        )
    ...
    search_body = {
        "indicators": [
            [
                ...
                {"field": "indicator_value", "operator": "like", "value": str(query)},
            ]
        ]
    }
    return search_body
...
def advance_search_command():
    ...
    res = tq_request(
        method="POST", url_suffix=f"/search/advanced?limit={limit}", params=search_body, retrieve_entire_response=True
    )
    try:
    ...
    readable = build_readable(readable_title=f'Search results for "{query}":', obj_type="indicator", data=indicators)
...
def edit_indicator_command():
    ...
    params = {"value": value, "type": indicator_type, "description": description}
...
def edit_event_command():
    ...
    params = {"title": title, "happened_at": parse_date(date) if date else None, "type": event_type, "description": description}
...
def get_related_objs_command(related_type):
    ...
    data = {RELATED_KEY[related_type]: createContext(info, removeNull=True), "ID": int(obj_id)}
...
def link_objects_command():
    ...
    demisto.results(f"Successfully linked {obj1_type} with ID {obj1_id} and {obj2_type} with ID {obj2_id}.")
...
def unlink_objects_command():
    ...
    demisto.results(f"Successfully unlinked {obj1_type} with ID {obj1_id} and {obj2_type} with ID {obj2_id}.")
...
def update_score_command():
    ...
    data = {"ID": int(indicator_id), "TQScore": get_tq_score_from_response(res["data"])}

    ec = {CONTEXT_PATH["indicator"]: data}

    readable = (
        "Successfully updated score of indicator with ID {} to {}. "
...
def add_attribute_command():
    ...
    params = {"name": attribute_name, "value": attribute_value}
...
def upload_file_command():
    ...
    try:
        ...
        return_error(f"Failed to prepare file for upload. Error message: {e!s}")
