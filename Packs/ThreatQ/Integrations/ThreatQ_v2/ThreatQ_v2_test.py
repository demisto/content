...
MOCK_PARAMS = {
    "credentials": {"identifier": "mock_email", "password": "mock_pass"},
    "insecure": True,
    "proxy": False,
    "serverUrl": MOCK_URL,
...
MOCK_ACCESS_TOKEN = {"expires_in": 3600, "access_token": "3220879210"}
...
MOCK_UPLOAD_FILE_ARGUMENTS = {"entry_id": "mock", "title": "TestTitle", "malware_safety_lock": "off", "file_category": "Cuckoo"}
...
MOCK_FILE_UPLOAD_RESPONSE = {
    "data": {"type_id": 1, "name": "testfile.txt", "title": "TestTitle", "malware_locked": 0, "content_type_id": 1}
}
    ...
    "data": [
        {
            ...
            "value": "foo@demisto.com",
            "status_id": 1,  # 'Active'
            "sources": [{"name": "test_source1", "pivot": {"id": 2017}}, {"name": "test_source2", "pivot": {"id": 2018}}],
            "attributes": [
    ...
    "data": [
        {
            ...
            "score": 0,
            "value": "foo@demisto.com",
            "touched_at": "2019-11-20 08:23:21",
            "id": 2019,
            "updated_at": "2019-11-20 08:22:51",
            "published_at": "2019-11-20 08:22:51",
            "created_at": "2019-11-20 08:22:51",
            "status_id": 5,
            "type_id": 4,
            "adversaries": [],
            "type": {"name": "Email Address", "id": 4, "class": "network"},
            "status": {"name": "Whitelisted", "id": 5, "description": "Poses NO risk and should never be deployed."},
            "sources": [
                {
                    "indicator_id": 20,
                    "indicator_status_id": 5,
                    "published_at": "2019-11-20 08:22:51",
                    "source_id": 8,
                    "id": 22,
                    "created_at": "2019-11-20 08:22:51",
...
MOCK_GET_EVENT_RESPONSE = {"data": {"id": 2019, "happened_at": "2019-03-01 00:00:00", "description": "test", "type_id": 1}}
...
MOCK_ERROR_RESPONSES = [
    {"data": {"errors": {"name": ["The name has already been taken."], "test": ["test_error1", "test_error2"]}}},
    {"errors": ["First Error", ["Second error - part 1", "Second error - part 2"]]},
]
...
def test_get_email_reputation(mocker, requests_mock):
    ...
    requests_mock.get(MOCK_API_URL + "/indicators/2019?with=attributes,sources,score,type", json=MOCK_GET_INDICATOR_RESPONSE)
    requests_mock.get(MOCK_API_URL + "/indicator/statuses/1", json=MOCK_GET_INDICATOR_STATUS_RESPONSE_1)
    requests_mock.get(MOCK_API_URL + "/indicator/types/4", json=MOCK_GET_INDICATOR_TYPE_RESPONSE_1)
...
def test_second_attempt_for_reputation_requests(mocker):
    ...
    def get_response(method, url, data=None, headers=None, verify=False, files=None, allow_redirects=True):
    ...
    results = tq_request("post", "", params={"criteria": {"value": "foo@demisto.com"}}, retrieve_entire_response=True)
