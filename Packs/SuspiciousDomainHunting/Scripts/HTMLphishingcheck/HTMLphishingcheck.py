...
def check_for_phishing_indicators(html_content):
    ...
    indicators = {
        ...
        "pop-up_forms": bool(
            re.search(r'<input[^>]*type=["\']?text["\']?.*?name=["\'](.*?(username|email).*?)["\']?', html_content, re.IGNORECASE)
        ),  # noqa: E501
        # Additional checks
        "meta_tags_phishing_keywords": bool(
            re.search(
        ...
        "javascript_phishing_code": bool(
            re.search(r"<script[^>]*>\s*function.*?(submit|login|password).*?</script>", html_content, re.DOTALL | re.IGNORECASE)
        ),
        ...
        "suspicious_js_functions": bool(
            re.search(r"<script[^>]*>\s*(submit|validate|sendData).*?</script>", html_content, re.DOTALL | re.IGNORECASE)
        ),
        "hidden_fields": bool(
            re.search(
        ...
        "payment_keywords": bool(
            re.search(r"(credit|card|debit|bank|account|payment|paypal|bitcoin|crypto)", html_content, re.IGNORECASE)
        ),
    }
...
def format_html_response(indicators):
    ...
    for key, value in indicators.items():
        html_output += f"<tr><td style='background-color: #ccc; padding: 8px; text-align: center; font-weight: bold; font-size: 16px;'>{key.replace('_', ' ').capitalize()}</td><td style='background-color: #eee; padding: 8px; text-align: center;'>{'✅'  if value else '❌' }</td></tr>"  # noqa: E501
