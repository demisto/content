...
def to_entry_type_code(name: str) -> int:
    etype = {"NOTE": EntryType.NOTE, "ERROR": EntryType.ERROR}.get(name)
...
class EntryFilter:
    def __init__(
        ...
        node_paths: list[str],
        filter_entry_formats: list[str],
        filter_entry_types: list[str],
    ...
    def match(self, entry: dict[str, Any]) -> Optional[tuple[re.Match, str]]:
        ...
        if self.__filter_entry_formats and entry.get("ContentsFormat") not in self.__filter_entry_formats:
            return None

        if self.__filter_user_type:
            user = demisto.get(entry, "Metadata.user") or ""
            if (user and ("user" not in self.__filter_user_type)) or ((not user) and ("dbot" not in self.__filter_user_type)):
                return None
...
def iterate_entries(
    ...
    while True:
        ...
        ents = demisto.executeCommand("getEntries", assign_params(id=incident_id, filter=query_filter))
        if not ents:
            break
...
def main():
    ...
    build_pattern_args = assign_params(
        pattern_algorithm=args.get("algorithm", "basic"), case_insensitive=argToBoolean(args.get("case_insensitive", False))
    )
    ...
    for entry in iterate_entries(
        ...
        query_filter=assign_params(categories=argToList(args.get("filter_categories")), tags=argToList(args.get("filter_tags"))),
        ...
        if action == "add":
            ...
            if not dry_run:
                for ent in ents:
                    if ent["Modified"]:
                        entry_id = ent["ID"]
                        res = demisto.executeCommand("setEntriesTags", {"entryIDs": entry_id, "entryTags": ent["Tags"]})
            ...
            if not dry_run:
                ...
                if ent_ids:
                    res = demisto.executeCommand("setEntriesTags", {"entryIDs": ent_ids, "entryTags": ",".join(tags)})
                    if not res or is_error(res[0]):
