...
from ProofpointThreatResponse import (
...
MOCK_INCIDENT_1 = {
    ...
    "incident_field_values": [
        ...
        {"name": "Abuse Disposition", "value": "Unknown"},
    ],
    "events": [
        {
            "id": 3,
...
INCIDENT_FIELD_INPUT = [(MOCK_INCIDENT_1, INCIDENT_FIELD_CONTEXT)]
...
EMAILS_CONTEXT_INPUT = [(MOCK_INCIDENT_1["events"][0], EMAIL_RESULT)]
...
ABUSE_DISPOSITION_INPUT = [(["Unknown"], True), ([], True), (["No such value"], False), (["No such value", "Unknown"], True)]
...
DEMISTO_PARAMS = [
    ({"event_sources": "No such source, Proofpoint TAP", "abuse_disposition": "No such value, Unknown"}, [MOCK_INCIDENT_1]),
    ({"event_sources": "", "abuse_disposition": ""}, [MOCK_INCIDENT_1]),
...
def test_fetch_incidents_limit_exceed(mocker):
    ...
    """
    params = {"fetch_delta": "6 hours", "fetch_limit": " 5", "created_after": "2021-03-30T11:44:24Z", "state": "closed"}
...
def test_fetch_incidents_with_same_created_time(mocker):
    ...
    params = {"fetch_delta": "2 hours", "fetch_limit": "3", "created_after": "2021-03-30T10:44:24Z", "state": "closed"}
...
def test_get_incident_command(mocker, requests_mock):
    ...
    mocker.patch.object(demisto, "args", return_value={"incident_id": "3064"})
    get_incident_command()
...
def test_close_incident_command(mocker, requests_mock):
    ...
    mocker.patch.object(demisto, "results")
    mocker.patch("ProofpointThreatResponse.BASE_URL", base_url)
    mocker.patch.object(demisto, "args", return_value={"incident_id": "3064", "summary": "summary", "details": "details"})
    close_incident_command()
    results = demisto.results.call_args[0][0]
...
def test_search_quarantine_command_with_str_messageDeliveryTime(mocker, requests_mock):
    ...
    mocker.patch.object(
        demisto,
        "args",
        return_value={"message_id": "<ABCD1234@cpus>", "recipient": "sabrina.test@test.com", "time": "2021-03-30T11:17:39Z"},
    )
    res = search_quarantine()

    assert (
        res.outputs_prefix == "<ABCD1234@cpus> Message ID found in TRAP alerts, "
...
@pytest.mark.parametrize(
    "list_id_to_search, filter_to_apply,  indicators_to_return, expected_result",
    [
...
def test_search_indicator_command(
    mocker, requests_mock, list_id_to_search, filter_to_apply, indicators_to_return, expected_result
):
    """
    Given:
...
def test_search_quarantine_command_mismatch_time(mocker, requests_mock):
    ...
    mocker.patch.object(
        demisto,
        "args",
        return_value={"message_id": "<ABCD1234@cpus>", "recipient": "sabrina.test@test.com", "time": "2021-04-30T11:17:39Z"},
    )
    res = search_quarantine()

    assert res.readable_output == (
        "<ABCD1234@cpus> Message ID found in TRAP alerts, but timestamp between email delivery time "
...
def test_search_quarantine_command_with_incident_far_from_alert_time_fail(mocker, requests_mock):
    ...
    assert res.readable_output == (
        "<ABCD1234@cpus> Message ID matches to 1 emails quarantined, but time between alert received "
