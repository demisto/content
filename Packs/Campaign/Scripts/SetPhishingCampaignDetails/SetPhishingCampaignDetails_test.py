...
from test_data.campaign_data import CAMPAIGN_INCIDENT_CONTEXT, INCIDENTS_BY_ID, NEW_INCIDENT_CONTEXT
...
INCIDENTS_CASES = [(CAMPAIGN_INCIDENT_CONTEXT[EMAIL_CAMPAIGN_KEY].get("incidents"), "3")]
...
def test_update_similarity_to_last_incident(mocker, incident_to_add, expected):
    ...
    mocker.patch.object(
        ...
        "get_campaign_context_from_incident",
...
COMPLETE_FLOW_CASES = [
    (
        "0",
        "3",  # case incident already in campaign (discovered through another incident already)
        [
            {
        ...
        ],  # expected same campaign data as before
    ),
    (
        "0",
        "4",  # case new incident
    ...
    (
        "1",
        "5",  # case new (empty campaign) with a new incident.
        [
            {
                "emailfrom": "examplesupport@example2.com",
                "emailfromdomain": "example.com",
...
def test_run_flow(mocker, campaign_id, incident_to_add_id, expected):
    ...
    mocker.patch.object(
        SetPhishingCampaignDetails,
        "get_current_incident_campaign_data",
