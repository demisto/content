...
def collect_campaign_recipients(args):
    ...
    try:
        ...
        raise DemistoException(f"Missing required arg: {e!s}") from e
...
def main():
    try:
        ...
        return_error(f"Failed to execute CollectCampaignRecipients. Error: {e!s}", error=traceback.format_exc())
