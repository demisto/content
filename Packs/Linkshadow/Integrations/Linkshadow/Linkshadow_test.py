...
def test_fetch_incidents(requests_mock):
    ...
    client = Client(base_url="https://LS_test_fetch_incidents.com/", verify=False, proxy=False)
    last_run = {"last_fetch": dateparser.parse(str(1621860339000), settings={"TIMEZONE": "UTC"}).strftime(DATE_FORMAT)}
...
def test_fetch_entity_anomalies(requests_mock):
    ...
    client = Client(base_url="https://LS_test_fetch_anomaly.com/", verify=False, proxy=False)
    params = {
        "apiKey": "",
        "username": "lsadmin",
        "plugin_id": "xsoar_integration_1604211382",
