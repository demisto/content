...
Note the following:
- The API Key can be generated but is shown only once. Once you get the API key, you will no longer be able to retrieve this key, only create a new key.
...
- API Key uses rules authorization (permissions) and can be used only per the rule it was assigned with. Any call that the rule does not permit will result with an access denied error code.
