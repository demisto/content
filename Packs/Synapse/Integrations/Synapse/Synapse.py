...
class Client(BaseClient):
    ...
    def login(self):
        ...
        resp = self._http_request(method="POST", url_suffix="/login", json_data={"user": self.username, "passwd": self.password})
    ...
    def add_user(self, new_user, new_passwd):
        ...
        resp = self._http_request(
            method="POST", url_suffix="/auth/adduser", json_data={"name": new_user, "passwd": new_passwd}, auth=self.auth
        )
    ...
    def add_role(self, new_role):
        ...
        resp = self._http_request(method="POST", url_suffix="/auth/addrole", json_data={"name": new_role}, auth=self.auth)
    ...
    def grant_role(self, user, role):
        ...
        resp = self._http_request(method="POST", url_suffix="/auth/grant", json_data={"user": user, "role": role}, auth=self.auth)
    ...
    def list_users(self):
        ...
        resp = self._http_request(method="GET", url_suffix="/auth/users", auth=self.auth)
    ...
    def list_roles(self):
        ...
        resp = self._http_request(method="GET", url_suffix="/auth/roles", auth=self.auth)
    ...
    def get_model(self):
        ...
        resp = self._http_request(method="GET", url_suffix="/model", auth=self.auth)
    ...
    async def synapse_get_nodes(self, data, limit=100):
        ...
        async with aiohttp.ClientSession(connector=TCPConnector(ssl=self._verify), trust_env=self.proxy) as sess:
...
def convert_raw_into_nodes_helper(results):
    ...
    for item in results:
        ...
        node = {"form": item[0][0], "created": t_stamp, "tags": get_full_tags_helper(item[1].get("tags"))}
...
def convert_epoch_timestamp_helper(timestamp):
    ...
    raw_date = datetime.fromtimestamp(float(timestamp) / 1000.0)
...
def file_context_builder_helper(file, data):
    ...
    """
    file_context = {"hash": file, "tags": get_full_tags_helper(data[0][1].get("tags"))}
...
def ip_reputation_command(client, args, good_tag, bad_tag) -> List[CommandResults]:
    ...
    for ip in ips:
        ...
        if not data:
            continue
        ip_data = {"ip": ip, "tags": get_full_tags_helper(data[0][1].get("tags"))}
...
def domain_reputation_command(client, args, good_tag, bad_tag) -> List[CommandResults]:
    ...
    for domain in domains:
        ...
        if not data:
            continue
        domain_data = {"domain": domain, "tags": get_full_tags_helper(data[0][1].get("tags"))}
...
def url_reputation_command(client, args, good_tag, bad_tag) -> List[CommandResults]:
    ...
    for url in urls:
        ...
        if not data:
            continue
        url_data = {"url": url, "tags": get_full_tags_helper(data[0][1].get("tags"))}
...
def list_roles_command(client):
    ...
    for role in data.get("result"):
        my_role = {"Name": role.get("name"), "Iden": role.get("iden"), "Rules": user_rules_helper(role.get("rules"))}
...
def query_model_command(client, args):
    ...
    if model_resp.get("form"):
        q_form = {"Form": model_resp.get("query"), "Properties": model_query_properties_helper(model_resp.get("form"))}
    full_resp = {"Valu": model_resp.get("query")}
...
def main() -> None:
    ...
    try:
        client = Client(base_url=base_url, username=username, password=password, verify=use_ssl, proxy=use_proxy)
