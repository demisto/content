...
class MockClient:
    ...
    def get_audit_logs(self, account_id, since, before, limit=100, cursor=None):
        ...
        idx = 0 if cursor is None else next((i + 1 for i, (_, c) in enumerate(self.pages) if c == cursor), len(self.pages))
...
def test_already_seen_ids_are_dropped():
    events = [
        collector.flatten_event(_record("a", "2026-07-01T00:00:00Z"), "x"),
...
def test_ids_sharing_the_newest_timestamp_are_all_carried():
    events = [
        collector.flatten_event(_record("a", "2026-07-02T00:00:00Z"), "x"),
        collector.flatten_event(_record("b", "2026-07-02T00:00:00Z"), "x"),
    ]
...
def test_login_events_survive_the_whole_pipeline():
    ...
    rec = _record(
        "login-1",
        "2026-07-27T21:44:57Z",
        description="LOGIN",
        actor={"email": "user@example.com", "ip_address": "192.0.2.10", "context": "api", "type": "user"},
    )
