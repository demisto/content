...
def test_right_location_format(mocker, requests_mock):
    ...
    mocker.patch.object(
        demisto,
        "params",
        return_value={"proxy": "proxy", "credentials": {"password": "password"}, "integrationReliability": "C - Fairly reliable"},
    )
    mocker.patch.object(demisto, "args", return_value={"ip": "1.2.3.4,8.8.8.8"})
    mocker_results = mocker.patch.object(demisto, "results")
    requests_mock.get("http://api.ipstack.com/1.2.3.4?access_key=password", json=RAW_RESPONSE_MOCK_1)
    requests_mock.get("http://api.ipstack.com/8.8.8.8?access_key=password", json=RAW_RESPONSE_MOCK_2)
...
def test_test_module(mocker, requests_mock):
    ...
    requests_mock.get("http://api.ipstack.com/1.2.3.4?access_key=password", json={"ip": "1.2.3.4"})
