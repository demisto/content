...
class Client(BaseClient):
    ...
    def patch_contact(self, contact_id: str = None, attributes: dict = None):
        ...
        res = self._http_request("PATCH", url_suffix=url_suffix, json_data=attributes)
        return res

    def delete_contact(self, contact_id: str = None):
        url_suffix = f"/contacts/{contact_id}"
        res = self._http_request("DELETE", url_suffix=url_suffix, resp_type="response", ok_codes=[200, 202])
        return res
...
def post_contact_command(client, args):
    email = {"attributes": {"email": args.get("email")}}
    res = client.post_contact(email=email)
    command_results = CommandResults(
        outputs_prefix="Drift.Contacts", outputs_key_field=["id"], outputs=res, readable_output=tableToMarkdown("Contact:", res)
    )
...
def patch_contact_command(client, args):
    contact_id = args.get("id")
    attributes = {"attributes": args.get("attributes")}
...
def delete_contact_command(client, args):
    ...
    command_results = CommandResults(readable_output=f"Contact ID {contact_id} was deleted.")
    return_results(command_results)
...
def main():
    ...
    try:
        ...
        client = Client(API_ENDPOINT, verify_certificate, proxy, headers=headers)
