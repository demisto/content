...
def ticket_to_incident(ticket):
    ...
    if attachments:
        ...
        for i in range(len(attachments)):
            incident_attachments.append({"path": attachments_content[i]["FileID"], "name": attachments[i]["Name"]})
...
def http_request(method, suffix_url, data=None, files=None, query=None):
    ...
    response = SESSION.request(method, url, data=data, params=params, files=files, headers=HEADERS, cert=cert_key_pair)
...
def create_ticket():
    ...
    ec = {"RTIR.Ticket(val.ID && val.ID === obj.ID)": ticket_context}
...
def search_ticket():
    ...
    for line in data:
        ...
        if current_ticket_search:
            search_context.append(current_ticket_search)
    if search_context:
        ec = {"RTIR.Ticket(val.ID && val.ID === obj.ID)": search_context}
...
def close_ticket():
    ...
    if "200 Ok" in closed_ticket.text:
        ec = {"RTIR.Ticket(val.ID && val.ID === obj.ID)": {"ID": int(ticket_id), "State": "resolved"}}
...
def get_ticket_attachments(ticket_id):
    ...
    for attachment_id, attachment_name, attachment_type, attachment_size in attachments_list:
        attachments.append({"ID": attachment_id, "Name": attachment_name, "Type": attachment_type, "Size": attachment_size})
...
def get_ticket_attachments_command():
    ...
    if attachments:
        ec = {"RTIR.Ticket(val.ID && val.ID === obj.ID)": {"ID": int(ticket_id), "Attachment": attachments}}
...
def get_ticket_history_command():
    ...
    if history_context:
        ec = {"RTIR.Ticket(val.ID && val.ID === obj.ID)": {"ID": int(ticket_id), "History": history_context}}
...
def get_ticket():
    ...
    ec = {"RTIR.Ticket(val.ID && val.ID === obj.ID)": ticket}
...
def main():
    ...
    try:
        ...
        global \
            ...
            CERTIFICATE, \
            PRIVATE_KEY
        SERVER = params.get("server", "")[:-1] if params.get("server", "").endswith("/") else params.get("server", "")
