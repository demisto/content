...
data_test_get_report_context = [None, "./data_test/get_report_file.json", "./data_test/get_report_url.json"]
...
def test_credentials_not_part_of_params(mocker):
    ...
    """
    mocker.patch.object(demisto, "params", return_value={"url": "testurl.com", "api_key": "apikey", "api_token": "apitoken"})
...
def test_connect_with_credentials(mocker):
    ...
    mocker.patch.object(demisto, "command", return_value="test-module")
    mocker.patch.object(Client, "get_task_list", return_value=("Human readable", {}, {}))
    client = Client(base_url="testurl.com", api_params={}, credentials={"username": "identifier", "password": "password"})
...
@pytest.mark.parametrize(
    "params",
    [
        {"url": "testurl.com", "credentials": {"identifier": "identifier", "password": "password"}},
        {"url": "testurl.com", "api_key": "apikey", "api_token": "apitoken"},
    ],
)
def test_test_module_wrong_credentials(mocker, params):
    ...
    api_params = {"key": params.get("api_key"), "api_token": params.get("api_token")}
...
def test_upload_file_with_csv_type(mocker):
    ...
    """
    params = {"url": "testurl.com", "credentials": {"identifier": "identifier", "password": "password"}}
