...
def test_azure_storage_create_table_command(requests_mock):
    ...
    When:
     - azure-storage-table-table-create called.
    Then:
    ...
    assert result.outputs.get('TableName') == 'xsoar'
...
def test_azure_storage_delete_table_command(requests_mock):
    ...
    Given:
     - User has provided valid credentials.
    When:
     - azure-storage-table-table-delete called.
    Then:
     - Ensure number of items is correct.
     - Ensure outputs prefix is correct.
     - Ensure a sample value from the API matches what is generated in the context.
...
def test_azure_storage_query_tables_command(requests_mock):
    ...
    Given:
     - User has provided valid credentials.
    When:
     - azure-storage-table-table-query called.
    Then:
     - Ensure number of items is correct.
     - Ensure outputs prefix is correct.
     - Ensure a sample value from the API matches what is generated in the context.
    ...
    assert result.outputs_prefix == 'AzureStorageTable.Table'
    assert result.outputs[0].get('TableName') == 'xsoar1'
...
def test_azure_storage_insert_entity_command(requests_mock):
    ...
    assert result.outputs_prefix == 'AzureStorageTable.Entity'
    assert result.outputs.get('PartitionKey') == 'xsoar-partition'
    assert result.outputs.get('RowKey') == 'xsoar-row'
    assert result.outputs.get('table_name') == table_name
...
def test_azure_storage_query_entity_command(requests_mock):
    ...
    assert len(result.outputs) == 1
    assert result.outputs_prefix == 'AzureStorageTable.Entity'
    assert result.outputs[0].get('PartitionKey') == 'xsoar-partition'
    assert result.outputs[0].get('RowKey') == 'xsoar-row'
    assert result.outputs[0].get('Address') == 'New York'
    assert result.outputs[0].get('table_name') == table_name
