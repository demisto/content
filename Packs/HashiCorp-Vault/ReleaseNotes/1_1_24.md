...
<~XSIAM> Added Parsing Rules (https://docs-cortex.paloaltonetworks.com/r/Cortex-XSIAM/Cortex-XSIAM-Administrator-Guide/Parsing-Rules) for ingesting [Hashicorp Vault Audit Logs](https://support.hashicorp.com/hc/en-us/articles/360001722947-Audit-Device-Notes) into the *`hashicorp_vault_raw`* dataset and populating the system *`_time`* field with the raw audit log *`time`* field (Available from Cortex XSIAM 2.3). </~XSIAM>
