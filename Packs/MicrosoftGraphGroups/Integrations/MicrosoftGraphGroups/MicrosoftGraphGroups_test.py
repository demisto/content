...
def test_parse_outputs():
    outputs = {"@odata.context": "a", "classification": "myclass", "securityEnabled": "true"}

    parsed_readable, parsed_outputs = parse_outputs(outputs)

    expected_readable = {"Classification": "myclass", "Security Enabled": "true"}
    expected_outputs = {"Classification": "myclass", "SecurityEnabled": "true"}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            create_group_command,
            {"group_id": "123", "mail_nickname": "nick", "security_enabled": True},
            RESPONSE_CREATE_GROUP,
...
def test_list_members_command(args, response, expected_result, mocker):
    ...
    assert (
        expected_result == result[1]["MSGraphGroups(val.ID === obj.ID)"]["Members"]
    )  # entry context is found in the 2nd place in the result of the command
...
@pytest.mark.parametrize("fun, mock_fun, mock_value, args, expected_result", TEST_SUPPRESS_ERRORS)
...
def test_test_module_command_with_managed_identities(mocker, requests_mock, client_id):
    ...
    params = {"managed_identities_client_id": {"password": client_id}, "use_managed_identities": "True", "url": Resources.graph}
    mocker.patch.object(demisto, "params", return_value=params)
...
def test_list_members(mocker):
    ...
    http_request.assert_called_with(
        method="GET",
        url_suffix="groups/123/members",
        params={"$top": None, "$count": "true"},
