...
class Client:
    ...
    def make_api_request(self, endpoint, method="GET", headers=None, params=None, data=None):
        ...
        response = requests.request(
            method, url, headers=headers, params=params, json=data, verify=self.verify, proxies=self.proxy
        )
        response.raise_for_status()
        data = {}
...
def get_ticket_data(client: Client):
    ...
    try:
        ...
        raise Exception(f"Could not find ticket with system id {sys_id}. Error: {e!s}") from e
...
def get_ticket(client: Client):
    ...
    try:
        ...
        table = tableToMarkdown(
            "Ticket Data",
            ticket_data,
            headers=["Date Created", "ServiceNow Ticket ID", "ServiceNow Ticket Number", "Status", "User"],
        )
...
def action_ticket(client, args):
    ...
    status_map = {"Denied": "2001", "Approved": "2000", "Pending": "1000"}

    message_map = {"Approved": "Authorization Request Was Approved", "Denied": "Authorization Request Was Denied"}
    ...
    try:
        ...
        raise Exception(f"Could not make POST API request. Error: {e!s}") from e
