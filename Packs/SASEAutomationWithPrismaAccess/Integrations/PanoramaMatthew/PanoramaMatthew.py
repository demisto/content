...
def get_show_get_action():
    show_uncommitted = demisto.args().get('show_uncommitted') == 'true'
    return 'get' if show_uncommitted else 'show'


@logger
def get_api_with_xpath(xpath, api_type="config"):
    params = {
        'action': get_show_get_action(),
        'type': api_type,
        'xpath': xpath
    }
    headers = {
    ...
    return dotdict(http_request(URL, 'GET', params=params, headers=headers))


def normalize_dict_with_xml_attr(d, filtered_fields=None):
    if isinstance(d, dict):
        result = {}
        for attr, value in d.items():
            if not filtered_fields or filtered_fields == [''] or attr.replace('@', '') in filtered_fields:
                if attr[0] == '@':
                    result[attr[1:]] = normalize_dict_with_xml_attr(value, filtered_fields)
                else:
                    result[attr] = normalize_dict_with_xml_attr(value, filtered_fields)
        return result
    return d


def get_list_with_xpath(xpath, entry_name, filtered_fields=None):
    res = get_api_with_xpath(xpath)

    entries = []
    try:
        entry = res.lookup(f'response.result.{entry_name}.entry')
        if isinstance(entry, list):
            for e in entry:
                entries.append(normalize_dict_with_xml_attr(e, filtered_fields))
        else:
            entries.append(normalize_dict_with_xml_attr(entry, filtered_fields))
...
def list_results_with_xpath(xpath, entry_name, context_path, filtered_fields=None):
    entries = get_list_with_xpath(xpath, entry_name, filtered_fields)
    demisto.results({
        'Type': entryTypes['note'],
        'ContentsFormat': formats['json'],
        'Contents': entries,
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': f'{entries}' if entries else f'no {entry_name} was configured',
        'EntryContext': {f"Panorama.{context_path}": entries}
    })


def panorama_list_access_domains_command():
    list_results_with_xpath(
        '/config/mgt-config/access-domain',
        'access-domain',
        'AccessDomains',
        demisto.args().get('filtered_fields', '').split(','))


def panorama_list_device_tenants_command():
    device = demisto.args().get('device', 'localhost.localdomain')
    list_results_with_xpath(
        f"/config/devices/entry[@name='{device}']/plugins/cloud_services/multi-tenant/tenants",
        'tenants',
        'DeviceTenants',
        demisto.args().get('filtered_fields', 'access-domain,name,bandwidth,users').split(','))


def panorama_list_device_templates_command():
    device = demisto.args().get('device', 'localhost.localdomain')
    list_results_with_xpath(
        f"/config/devices/entry[@name='{device}']/template",
        'template',
        'DeviceTemplates',
        demisto.args().get('filtered_fields', 'name').split(','))


def panorama_list_device_template_stacks_command():
    device = demisto.args().get('device', 'localhost.localdomain')
    list_results_with_xpath(
        f"/config/devices/entry[@name='{device}']/template-stack",
        'template-stack',
        'DeviceTemplateStacks',
        demisto.args().get('filtered_fields', '').split(','))


def panorama_list_device_groups_command():
    device = demisto.args().get('device', 'localhost.localdomain')
    list_results_with_xpath(
        f"/config/devices/entry[@name='{device}']/device-group",
        'device-group',
        'DeviceGroup',
        demisto.args().get('filtered_fields', 'name,description').split(','))


def panorama_get_unallocated_users_bandwidth():
    result = panorama_get_license()
    if 'response' not in result or '@status' not in result['response'] or result['response']['@status'] != 'success':
        demisto.debug(str(result))
        raise Exception('Failed to get the information about PAN-OS available licenses and their statuses.')
    ...
    entry = result.get('response', {}).get('result', {}).get('licenses', {}).get('entry', [])
    licensed_users = 0
    licensed_bandwidth = 0
    for item in entry:
        if 'custom' in item:
            if '_User_Limit' in item['custom']:
                licensed_users = int(item['custom']['_User_Limit'])
            if '_Total_Mbps' in item['custom']:
                licensed_bandwidth = int(item['custom']['_Total_Mbps'])

    device = demisto.args().get('device', 'localhost.localdomain')
    tenants = get_list_with_xpath(
        f"/config/devices/entry[@name='{device}']/plugins/cloud_services/multi-tenant/tenants",
        'tenants',
        demisto.args().get('filtered_fields', 'bandwidth,users').split(','))
    allocated_users = 0
    allocated_bandwidth = 0
    for tenant in tenants:
        if 'users' in tenant:
            allocated_users += int(tenant['users'])
        if 'bandwidth' in tenant:
            allocated_bandwidth += int(tenant['bandwidth'])

    unallocated = {
        'UnallocatedUsers': licensed_users - allocated_users,
        'UnallocatedBandwidth': licensed_bandwidth - allocated_bandwidth
    }
    ...
    demisto.results({
        ...
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': f'{unallocated}',
        'EntryContext': {
            "Panorama.UnallocatedUsers": unallocated['UnallocatedUsers'],
            "Panorama.UnallocatedBandwidth": unallocated['UnallocatedBandwidth'],
        }
    })
...
def main():
    ...
    try:
        ...
        elif demisto.command() == 'panorama-list-access-domains':
            panorama_list_access_domains_command()

        elif demisto.command() == 'panorama-list-device-tenants':
            panorama_list_device_tenants_command()

        elif demisto.command() == 'panorama-list-device-templates':
            panorama_list_device_templates_command()

        elif demisto.command() == 'panorama-list-device-template-stacks':
            panorama_list_device_template_stacks_command()

        elif demisto.command() == 'panorama-list-device-groups':
            panorama_list_device_groups_command()

        elif demisto.command() == 'panorama-get-unallocated-users-bandwidth':
            panorama_get_unallocated_users_bandwidth()
