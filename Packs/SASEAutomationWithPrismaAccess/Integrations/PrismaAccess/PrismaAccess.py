...
def prisma_access_get_network_details():
    tenant_name = demisto.args().get('tenant_name', 'DEFAULT')
    params = '{"serviceType":"fwaas","tenantName":"%s"}' % tenant_name
    params = b64encode(params.encode('utf-8')).decode('utf-8')
    cmd = 'debug plugins cloud_services gpcs query body b64{} action getNetworkDetailData'.format(params)
    ...
    if jsonStartPos < 0:
        ...
        if res['@status'] == 'success':
            data = res.get('result', {}).get('entry', {})
            for row in data:
                row['TenantName'] = tenant_name
            md = tableToMarkdown('Prisma Access Network Details', data)
            ec = {"PrismaAccess.NetworkDetails": data}
            contents = data
            return_outputs(md, ec, contents)
        else:
            resultsJsonAsText = json.dumps(res, indent=4)
...
@logger
def prisma_access_generate_api_key(tenant_name: str) -> Dict[str, str]:
    if apiConfigured:
        params = {
            'type': 'op',
            'key': API_KEY,
...
def prisma_access_generate_api_key_command():
    tenant_name = demisto.args().get('tenant_name', '')
    result = prisma_access_generate_api_key(tenant_name)

    if 'result' in result['response'] and result['response']['@status'] == 'success':
        res = result['response'].get('result', {}).get('result', {})
        if 'pass' == res.get('status', ''):
            try:
                msg = json.loads(res.get('msg', ''))
                # Add fields to response
                res['Code'] = msg.get('@code', '')
                res['Key'] = msg.get('result', '')

            except:
                demisto.error('Status is "pass" but failed to parse json from msg field in API response')
        hr = '### Prisma Access Generate API Key Results:\n' + json.dumps(res, indent=4)
        demisto.results({
            'Type': entryTypes['note'],
            'ContentsFormat': formats['json'],
            'Contents': res,
            ...
            'HumanReadable': hr,
            'EntryContext': {
                'PrismaAccess.GenerateAPIKey': res
            }
        })
    else:
        demisto.results(result)
...
def main():
    ...
    try:
        ...
        elif demisto.command() == 'prisma-access-generate-api-key':
            prisma_access_generate_api_key_command()

        elif demisto.command() == 'prisma-access-get-networks-details':
            prisma_access_get_network_details()
