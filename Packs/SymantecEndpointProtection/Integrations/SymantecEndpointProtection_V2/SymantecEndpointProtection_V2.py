...
def endpoint_ip_extract(raw_json):
    ...
    for content in raw_json:
        ip = {"Address": content.get("ipAddresses", [""])[0], "Mac": content.get("computerName")}
    ...
    demisto.results(
        {
            ...
            "Contents": {"cmdStatusDetail": cmd_status_details, "commandId": command_id},
            "HumanReadable": md,
            "EntryContext": {
                "SEPM.LastCommand": createContext(
                    {"CommandDetails": cmd_status_details, "CommandId": command_id}, removeNull=True
                )
            },
        }
    )
...
def update_content(token, computer_id):
    ...
    command_id = demisto.get(
        json.loads(res_json), "Envelope.Body.runClientCommandUpdateContentResponse.CommandClientResult.commandId"
    )
    if not command_id:
...
def quarantine(token, computer_id, action_type):
    ...
    res_json = do_post(token, True, "sepm/ws/v1/CommandService", xml)
    command_id = demisto.get(
        json.loads(res_json), "Envelope.Body.runClientCommandQuarantineResponse.CommandClientResult.commandId"
    )
    if not command_id:
        error_code = demisto.get(
            json.loads(res_json), "Envelope.Body.runClientCommandQuarantineResponse.CommandClientResult.inputErrors.errorCode"
        )
        error_message = demisto.get(
...
def get_endpoints_info(token, computer_name, last_update, os, page_size, columns, group_name=None):
    params = {"computerName": computer_name, "lastUpdate": last_update, "os": os, "pageSize": page_size, "columns": columns}
...
def filter_only_old_clients(filtered_json_response, desired_version):
    ...
    for content in filtered_json_response:
        ...
        if (desired_version and RunningVersion != desired_version) or (not desired_version and RunningVersion != TargetVersion):
...
def system_info_command(token):
    ...
    system_info_json = {"version": version_json, "avdef": avdef_json}
    ...
    demisto.results(
        {
            ...
            "HumanReadable": md,
            "EntryContext": {"SEPM.ServerAVDefVersion": context},
        }
    )
...
def old_clients_command(token):
    ...
    filtered_json_response, entry_context = get_endpoints_info(
        token, computer_name, last_update, os, page_size, columns, group_name
    )
    ...
    demisto.results(
        {"Type": entryTypes["note"], "ContentsFormat": formats["json"], "Contents": filtered_json_response, "HumanReadable": md}
    )
    ...
    demisto.results(
        {
            ...
            "HumanReadable": md,
            "EntryContext": {"SEPM.ClientContentVersions": client_version, "SEPM.LastUpdated": last_update_date},
        }
    )
    ...
    demisto.results(
        {
            "Type": entryTypes["note"],
            "ContentsFormat": formats["json"],
            "Contents": {"cmdStatusDetail": cmd_status_detail, "commandId": command_id},
            "HumanReadable": md,
            "IgnoreAutoExtract": True,
            "EntryContext": {
...
def list_policies_command(token):
    ...
    md = tableToMarkdown(
        "List of existing policies", md_list, ["Policy Name", "Type", "ID", "Enabled", "Assigned", "Description"]
    )
    demisto.results(
        {
            "Type": entryTypes["note"],
            ...
            "HumanReadable": md,
            "IgnoreAutoExtract": True,
            "EntryContext": {"SEPM.PoliciesList": createContext(fixed_policy_list, removeNull=True)},
        }
    )
    ...
    demisto.results(
        {
            ...
            "IgnoreAutoExtract": True,
            "EntryContext": {"SEPM.Quarantine": context},
        }
    )
    ...
    demisto.results(
        {
            ...
            "HumanReadable": message,
            "IgnoreAutoExtract": True,
            "EntryContext": {"SEPM.Scan": context},
        }
    )
    ...
    demisto.results(
        {
            ...
            "HumanReadable": message,
            "IgnoreAutoExtract": True,
            "EntryContext": {"SEPM.Update": context},
        }
    )


def move_client_to_group(token, group_id, hardware_key):
    body = [{"group": {"id": group_id}, "hardwareKey": hardware_key}]
    response = do_patch(token, "sepm/api/v1/computers", body)
    message = (
        "### Moved client to requested group successfully"
...
def main():
    ...
    try:
        ...
        resp = do_auth(
            server=demisto.getParam("server"),
            crads=demisto.getParam("authentication"),
            insecure=demisto.getParam("insecure"),
