...
| Listen Port                        | Runs the service on this port from within Cortex XSOAR. Requires a unique port for each long-running integration instance. Do not use the same port for multiple instances.       Note: If the test button is pressed twice, it may result in a failure and mistakenly indicate that the port is already in use.                                                              | True         |
