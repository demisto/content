...
from Workday_IAM import (
    AD_ACCOUNT_STATUS_FIELD,
    CONVERSION_HIRE_FIELD,
    EMAIL_ADDRESS_FIELD,
    EMPLOYEE_ID_FIELD,
    ...
    REHIRED_EMPLOYEE_FIELD,
    SOURCE_PRIORITY_FIELD,
    TERMINATION_TRIGGER_FIELD,
    Client,
    fetch_incidents,
)
...
def test_fetch_incidents_email_change(requests_mock, mocker):
    ...
    """
    from test_data.fetch_incidents_email_change_mock_data import (
...
def test_fetch_incidents_employee_id_change(requests_mock, mocker):
    ...
    - Make sure the IAM - Update User event is returned as expected.
    """
    from test_data.fetch_incidents_employee_id_change_mock_data import (
        email_to_user_profile,
        employee_id_to_user_profile,
        event_data,
        full_report,
...
def test_fetch_incidents_orphan_user(requests_mock, mocker):
    ...
    """
    from test_data.fetch_incidents_orphan_user_mock_data import email_to_user_profile, event_data, full_report
...
def test_fetch_incidents_source_priority(requests_mock, mocker):
    ...
    """
    from test_data.fetch_incidents_source_priority_mock_data import (
        email_to_user_profile,
        employee_id_to_user_profile,
        event_data,
        full_report,
    ...
    fetch_events = fetch_incidents(
        client, {}, "https://test.com", "%m/%d/%Y", LAST_DAY_OF_WORK_FIELD, None, None, source_priority=2
    )
    assert fetch_events == event_data
...
def test_fetch_incidents_partial_name_match(requests_mock, mocker):
    ...
    """
    from test_data.fetch_incidents_partial_name_match_mock_data import (
    ...
    mocker.patch.object(demisto, "mapObject", return_value=mapped_workday_user)
    mocker.patch(
        "Workday_IAM.get_all_user_profiles",
        return_value=(display_name_to_user_profile, employee_id_to_user_profile, email_to_user_profile),
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (None, {PREHIRE_FLAG_FIELD: "False", EMPLOYMENT_STATUS_FIELD: "Terminated", LAST_DAY_OF_WORK_FIELD: "12/12/2020"}, False),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"mocked_demisto_user": ""},
            {PREHIRE_FLAG_FIELD: "False", EMPLOYMENT_STATUS_FIELD: "Terminated", LAST_DAY_OF_WORK_FIELD: "12/12/2100"},
            True,
        ),
        ...
        (
            {"mocked_demisto_user": ""},
            {PREHIRE_FLAG_FIELD: "False", EMPLOYMENT_STATUS_FIELD: "Active", LAST_DAY_OF_WORK_FIELD: "12/12/2100"},
            False,
        ),
        # active, pre-hired user with future last day of work - should return False
        (
            {"mocked_demisto_user": ""},
            {PREHIRE_FLAG_FIELD: "True", EMPLOYMENT_STATUS_FIELD: "Active", LAST_DAY_OF_WORK_FIELD: "12/12/2100"},
            False,
        ),
        # pre-hired user with future last day of work - should return False
        (
            {"mocked_demisto_user": ""},
            {PREHIRE_FLAG_FIELD: "True", EMPLOYMENT_STATUS_FIELD: "Terminated", LAST_DAY_OF_WORK_FIELD: "12/12/2100"},
            False,
        ),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {PREHIRE_FLAG_FIELD: "True", REHIRED_EMPLOYEE_FIELD: "Yes"},
            "mocked_changed_fields",
            True,
        ),
        # a pre-hired + rehired employee, no changed_fields (already synced into XSOAR) - should return False
        ({"adaccountstatus": "Disabled"}, {PREHIRE_FLAG_FIELD: "True", EMPLOYMENT_STATUS_FIELD: ""}, None, False),
        # no demisto_user - should return False
        ...
        (
            {"adaccountstatus": "Disabled"},
            {PREHIRE_FLAG_FIELD: "False", REHIRED_EMPLOYEE_FIELD: "No"},
            "mocked_changed_fields",
            False,
        ),
    ],
...
@pytest.mark.parametrize(
    ...
    [(None, False), ({TERMINATION_TRIGGER_FIELD: "TUFE"}, True), ({TERMINATION_TRIGGER_FIELD: "Workday IAM"}, False)],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {EMAIL_ADDRESS_FIELD: "test@example.com", EMPLOYEE_ID_FIELD: "123", HIRE_DATE_FIELD: "02/02/2040"},
            {
