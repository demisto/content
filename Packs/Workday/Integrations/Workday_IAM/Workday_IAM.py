...
class Client(BaseClient):
    ...
    def get_full_report(self, report_url):
        res = self._http_request(method="GET", full_url=report_url, url_suffix="", timeout=READ_TIME_OUT_IN_SECONDS)
...
def report_to_indicators(
    ...
    for entry in report_entries:
        ...
        if (
            not has_reached_threshold_date(days_before_hire_to_sync, workday_user)
...
def has_reached_threshold_date(num_of_days_before_hire, workday_user):
    ...
    if delta > num_of_days_before_hire:
        demisto.debug(
            f"Skipped creating an incident for user "
...
def new_hire_email_already_taken(workday_user, demisto_user, email_to_user_profile):
    if demisto_user is None and email_to_user_profile.get(workday_user.get(EMAIL_ADDRESS_FIELD)) is not None:
        demisto.debug(
            f"Skipped creating a user profile for the following employee:\n{workday_user}\n\n"
...
def is_report_missing_required_user_data(workday_user):
    if (
        not workday_user.get(EMAIL_ADDRESS_FIELD)
        or not workday_user.get(EMPLOYEE_ID_FIELD)
...
def is_display_name_already_taken(demisto_user, workday_user, display_name_to_user_profile):
    ...
    if demisto_user is None:
        demisto.debug(
            f"Detected a potential new hire for user with email address "
    ...
    for user in demisto_users_by_display_name:
        ...
        if (
            user.get(EMPLOYEE_ID_FIELD) != demisto_user.get(EMPLOYEE_ID_FIELD)
    ...
    if display_name_is_taken_by_another_user and is_display_name_change:
        demisto.debug(
            f"Detected an event for user with email address "
...
def is_new_hire_event(demisto_user, workday_user, deactivation_date_field):
    ...
    if employment_status.lower() != "terminated" and (not deactivation_date or today <= deactivation_date):
        demisto.debug(
            f"A non-terminated user with an email address {workday_user.get(EMAIL_ADDRESS_FIELD)} "
...
def is_ad_deactivation_event(demisto_user, workday_user, days_before_hire_to_enable_ad, source_priority):
    ...
    if (demisto_user.get(AD_ACCOUNT_STATUS_FIELD, "") == "Enabled" and
        not has_reached_threshold_date(days_before_hire_to_enable_ad, workday_user)):
        demisto.debug(
...
def get_orphan_users(email_to_user_profile, user_emails, source_priority):
    ...
    orphan_users = [
        email
        for email, user in email_to_user_profile.items()
        if email not in user_emails
        and user.get(EMPLOYMENT_STATUS_FIELD) != "Terminated"
...
def is_valid_source_of_truth(demisto_user, source_priority):
    if demisto_user is not None and demisto_user.get(SOURCE_PRIORITY_FIELD, 1) < source_priority:
        demisto.debug(
            f"Skipped creating an incident for user profile {demisto_user.get(EMAIL_ADDRESS_FIELD)}: "
...
def get_event_details(
    ...
    if is_display_name_already_taken(demisto_user, workday_user, display_name_to_user_profile) and event_type in [
        NEW_HIRE_EVENT_TYPE,
        REHIRE_USER_EVENT_TYPE,
        UPDATE_USER_EVENT_TYPE,
    ]:
    ...
    return {"name": user_email, "rawJSON": json.dumps(entry), "type": event_type, "details": event_details}
...
def get_full_report_command(client, mapper_in, report_url, workday_date_format, source_priority):
    report_entries = client.get_full_report(report_url)
    outputs = [get_workday_user_from_entry(entry, mapper_in, workday_date_format, source_priority) for entry in report_entries]
    results = CommandResults(outputs_prefix="WorkdayIAM.ReportEntry", outputs_key_field=EMPLOYEE_ID_FIELD, outputs=outputs)
    return results
...
def fetch_incidents(
    ...
    try:
        ...
        for entry in report_entries:
            ...
            demisto.debug(
                f'Getting event details for user with email address {workday_user.get("email")}.\n'
...
def workday_first_run_command(
    ...
    report_entries = client.get_full_report(report_url)
    indicators = report_to_indicators(
        report_entries,
        mapper_in,
        workday_date_format,
        deactivation_date_field,
        ...
        if command == "fetch-incidents":
            ...
            if params.get("fetch_samples") and not last_run.get("fetched_samples"):
                sample_events = fetch_samples(
                    client=client, mapper_in=mapper_in, report_url=report_url, workday_date_format=workday_date_format
                )
