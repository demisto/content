...
from WorkdaySignOnEventCollector import get_from_time, fletcher16, generate_checksum, check_events_against_checksums, \
    filter_and_check_events, get_future_duplicates_within_timeframe, convert_to_json, process_events, Client, \
    fetch_sign_on_logs, get_sign_on_events_command, fetch_sign_on_events_command
...
def test_convert_to_json():
    ...
    assert raw_json_response["Envelope"]["Body"]["Get_Workday_Account_Signons_Response"]["Response_Data"][
               "Workday_Account_Signon"][0]["Signon_DateTime"] == "2021-09-01T12:00:00Z"
    assert account_signon_data["Workday_Account_Signon"][0]["Signon_DateTime"] == "2021-09-01T12:00:00Z"
...
def test_process_events():
    ...
    # Check if the '_time' key is added to the events
    assert events[0]["_time"] == "2021-09-01T12:00:00Z"
...
def test_generate_workday_account_signons_body():
    ...
    """
    client = Client(base_url="", verify_certificate=True, proxy=False, tenant_name="test_tenant", token="test_token",
                    username="test_user", password="test_pass")

    body = client.generate_workday_account_signons_body(page=1, count=10, to_time="2021-09-01T12:00:00Z",
    ...
    assert "<wsse:Password Type=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText\">test_pass</wsse:Password>" in body
...
def test_generate_test_payload():
    ...
    payload = client.generate_test_payload(from_time="2021-09-01T11:00:00Z", to_time="2021-09-01T12:00:00Z")
    ...
    assert "<wsse:Password Type=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText\">test_pass</wsse:Password>" in payload
...
def test_convert_to_json_valid_input():
    ...
    print("Workday Account Signons:", workday_account_signons)

    # Assertion
    assert isinstance(workday_account_signons, list), "workday_account_signons is not a list"
    ...
    assert workday_account_signons_data[0].get('Signon_DateTime') == "2021-09-01T11:00:00Z"
...
class TestFetchSignOnLogs(unittest.TestCase):
    ...
    def setUp(self):
        self.client = Client("mock_url", False, False, "mock_tenant", "mock_token", "mock_user", "mock_pass")
    ...
    def test_fetch_sign_on_logs_single_page(self, mock_retrieve_events):
        ...
        events = fetch_sign_on_logs(self.client, 10, '2021-09-01T00:00:00Z', '2021-09-02T00:00:00Z')
...
class TestGetSignOnEventsCommand(unittest.TestCase):
    ...
    def test_get_sign_on_events_command(self):
        ...
        with patch('WorkdaySignOnEventCollector.fetch_sign_on_logs', return_value=mock_events):
            client = Client("mock_url", False, False, "mock_tenant", "mock_token", "mock_user", "mock_pass")
            events, results = get_sign_on_events_command(client, '2021-09-01T00:00:00Z', '2021-09-02T00:00:00Z', 10)

            # Assertions
            self.assertEqual(len(events), 1)
            self.assertEqual(events[0]['User_Name'], 'John')
            self.assertEqual(events[0]['_time'], '2021-09-01T11:00:00Z')
            self.assertTrue(results.readable_output.startswith("### Sign On Events List:"))
...
def test_fetch_sign_on_events_command_single_page():
    ...
    mock_last_run = {'last_fetch_time': '2021-09-01T10:59:00Z', 'previous_run_checksums': set()}

    # Use patch to mock the client's retrieve_events method and demisto.getLastRun function
    with patch.object(Client, 'retrieve_events', return_value=mock_retrieve_response), \
