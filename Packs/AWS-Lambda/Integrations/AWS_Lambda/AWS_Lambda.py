...
def remove_permission(args, aws_client):
    kwargs = {"FunctionName": args.get("functionName"), "StatementId": args.get("StatementId")}
    if args.get("Qualifier") is not None:
        kwargs.update({"Qualifier": args.get("Qualifier")})
...
def get_policy_command(args: dict[str, str], aws_client) -> CommandResults:
    ...
    return CommandResults(outputs=response, readable_output=policy_table, outputs_prefix="AWS.Lambda", outputs_key_field="Sid")
...
def list_versions_by_function_command(args: dict[str, str], aws_client) -> CommandResults:
    ...
    return CommandResults(
        outputs=response, readable_output=table_for_markdown, outputs_prefix="AWS.Lambda", outputs_key_field="FunctionName"
    )
...
def delete_function_url_config_command(args: dict[str, str], aws_client) -> CommandResults:
    ...
    return CommandResults(readable_output=f"Deleted {args['functionName']} Successfully")
...
def create_function_command(args: dict[str, str], aws_client) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name="Create Function", t=readable_outputs, headerTransform=pascalToSpace)
...
def delete_layer_version_command(args: dict[str, str], aws_client) -> CommandResults:
    ...
    return CommandResults(readable_output=f"Deleted version number {version_number} of {layer_name} Successfully")
...
def list_layer_version_command(args, aws_client):
    ...
    outputs = {
        "AWS.Lambda.Layers(val.LayerVersionArn && val.LayerVersionArn == obj.LayerVersionArn)": res.get("LayerVersions"),
    ...
    return CommandResults(outputs=remove_empty_elements(outputs), raw_response=res, readable_output=readable_output)
