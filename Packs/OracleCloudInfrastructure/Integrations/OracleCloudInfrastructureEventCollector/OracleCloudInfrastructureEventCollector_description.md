...
| Private Key                                                                 | Private Key for authentication. <br> **Important:** The key pair must be in PEM format. For instructions on generating a key pair in PEM format, see [Required Keys and OCIDs](https://docs.oracle.com/en-us/iaas/Content/API/Concepts/apisigningkey.htm#Required_Keys_and_OCIDs). <br> **Note:** Enter only the raw key content, ***without*** the `-----BEGIN PRIVATE KEY-----` and `-----END PRIVATE KEY-----` lines. |
