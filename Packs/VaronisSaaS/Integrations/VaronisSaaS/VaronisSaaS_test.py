...
def test_update_remote_system_command(mocker: MockerFixture):
    ...
    args = {"incidentChanged": False, "remoteId": "12345"}
...
def test_get_mapping_fields_command():
    from VaronisSaaS import INCIDENT_FIELDS, get_mapping_fields_command, strEqual
...
def test_varonis_authenticate_content_type_header(requests_mock: MockerFixture):
    ...
    assert "content-type" in {k.lower() for k in last_request.headers.keys()}, \
...
def test_all_client_requests_have_content_type_header():
    ...
    assert "content-type" in client.headers, \
        "Client default headers must include content-type"
...
def test_convert_to_demisto_severity():
    from VaronisSaaS import IncidentSeverity, convert_to_demisto_severity
