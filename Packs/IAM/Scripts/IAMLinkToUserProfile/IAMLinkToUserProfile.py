...
try:
    indicator_id = (
        demisto.executeCommand("findIndicators", {"query": query})[0]
        .get("Contents")[0]
        .get("id")
    )
    ...
    if display_name:
        ...
        demisto.debug(
            f"IAMLinkToUserProfile failed: {employeeid=}, {query=}, {display_name=}, {employee_email=}, {indicator_id=}"
        )
    else:
...
demisto.results(
    {"ContentsFormat": formats["html"], "Type": entryTypes["note"], "Contents": html}
)
