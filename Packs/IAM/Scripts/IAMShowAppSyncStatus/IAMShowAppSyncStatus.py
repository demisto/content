...
if is_success is True:
    ...
    if (
        runstatus != "error" and runstatus != "idle"
    ):  # Did not sync because playbook is still running:
        html = (
            "<div style='text-align:center; font-size:32px; padding: 6px; "
            + PROGRESS_COLOR
        ...
        runstatus == "error"
    ):  # Did not sync because of error before the sync, incident still running:
        html = (
            "<div style='text-align:center; font-size:32px; padding: 6px; "
            + FAILURE_COLOR
...
demisto.results(
    {"ContentsFormat": EntryFormat.HTML, "Type": EntryType.NOTE, "Contents": html}
)
