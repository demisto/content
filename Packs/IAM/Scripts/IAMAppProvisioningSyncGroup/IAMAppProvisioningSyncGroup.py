...
USER_ADDED_TO_GROUP_MESSAGE = ['Successfully imported new member to an app group', 'Add user to group membership']
USER_DELETED_FROM_GROUP_MESSAGE = ['Successfully deleted member of an app group', 'Remove user from group membership']
...
''' GLOBAL VARIABLES '''
args = demisto.args()
okta_group_data = json.loads(args.get('group_data')) if type(args.get('group_data')) is not dict else args.get('group_data')
...
def main():
    ...
    try:
        ...
        app_group_data = get_group(app_instance, scim=get_group_scim, group_mappings=group_mappings)

        if app_group_data.get('id'):
            update_group(app_instance, app_group_data)
        elif app_group_data.get('errorCode') == 404:
            app_group_data = create_group(app_instance, okta_group_name)  # Creates an empty group
            update_group(app_instance, app_group_data)
        else:
        ...
        if group_profile_id:
            ...
            update_demisto_group_profile(indicator_id=group_profile_id, group_mappings=group_mappings,
            ...
            create_demisto_group_profile(okta_group_id=okta_group_id, group_mappings=group_mappings_updated)

        if war_room_warning_logs:
            return_results(f"{log_prefix}: Group synced with below warnings/errors:", {"Warn Logs": war_room_warning_logs})
        demisto.info(f"{log_prefix} Group sync completed. Response Time: {(datetime.now() - start).total_seconds()} seconds")
...
def get_group(app_instance, scim, group_mappings=None):
    try:
        ...
        if app_group_data and app_group_data.get(ID_FIELD) and app_group_data.get(LAST_SYNCED_FIELD):
            demisto.debug(f"{log_prefix} Group Mapping found in Indicator. App group id: {app_group_data.get('id')}")
        else:
            demisto.info(f"{log_prefix} Calling iam-get-group for {app_instance}. scim: {json.dumps(scim)}")
            get_group_response = demisto.executeCommand("iam-get-group",
                                                        {'scim': scim,
                                                         'includeMembers': 'true',
                                                         'using': app_instance})

            if isError(get_group_response[0]):
                demisto.error(f"{log_prefix} iam-get-group failed. Raw Response: {get_group_response}")
                raise Exception(f"{log_prefix} iam-get-group failed. Raw Response: {get_group_response}")
            app_group_data = read_result_from_command_response(get_group_response, app_instance)
            members = app_group_data.get('members') or list()

            if not app_group_data:
                raise Exception(f"{log_prefix} iam-get-group failed. Raw Response: {get_group_response}")
            elif app_group_data.get('success') and app_group_data.get('id'):
                demisto.info(f"{log_prefix} Completed iam-get-group for {app_instance}. group id: {app_group_data.get('id')}. "
                             f"Members: {len(members)}")
            elif not app_group_data.get('success') and app_group_data.get('errorCode') == 404:
        ...
        demisto.error(f'{okta_group_id}: Error Occurred while calling iam-get-group command for {app_instance}: {traceback.format_exc()}')
        return_error(f'Error Occurred while calling iam-get-group command for {app_instance}: {traceback.format_exc()}')
...
def create_group(app_instance, group_name):
    ...
    create_group_data = read_result_from_command_response(create_group_response, app_instance)

    if create_group_data and not create_group_data.get('success'):
        demisto.error(f"{log_prefix} Failed to create group. Response: {create_group_response}")
        raise Exception(f"{log_prefix} Failed to create group. Response: {create_group_response}")
    else:
        demisto.info(f"{log_prefix} Create group completed. Id: {create_group_data.get('id')}")
        demisto.log(f"{log_prefix} Create group completed. Id: {create_group_data.get('id')}")
...
def update_group(app_instance, app_group_data):
    ...
    if last_synced:
        group_member_ids_to_add, group_member_ids_to_remove = get_group_member_changes_since_last_sync(last_synced=last_synced,
                                                                                                     to_date=current_date)
    else:
        # Last sync time is not available. Do a full comparision and get the delta changes
        group_member_ids_to_add, group_member_ids_to_remove = get_group_member_changes_full_comparision(app_group_data)
    ...
    if group_member_ids_to_add and group_member_ids_to_remove:
        ...
        conflict_members = group_member_ids_to_add_set.intersection(group_member_ids_to_remove_set)
        if conflict_members:
            send_email(
            ...
            raise Exception(f"{log_prefix} Failed to update group. Conflict in members. "
                            f"Same members marked for add and remove. Member Ids: {conflict_members}")
    ...
    if group_member_ids_to_add or group_member_ids_to_remove:
        update_group_scim = {
            'id': app_group_data.get(ID_FIELD)
        }
        ...
        update_group_data = read_result_from_command_response(update_group_response, app_instance)

        if update_group_data and not update_group_data.get('success'):
            demisto.error(f"{log_prefix} Failed to update group. Response: {update_group_data}")
            raise Exception(f"{log_prefix} Failed to update group. Response: {update_group_data}. Update_group_args: {update_group_args}")
        else:
            demisto.info(f"{log_prefix} Update group completed for '{app_group_data.get(ID_FIELD)}'. "
        ...
        return_results(f'{log_prefix}: {app_group_data.get(ID_FIELD)} Members Updated', {"Members Added": group_member_ids_to_add,
...
def get_group_member_changes_full_comparision(app_group_data):
    ...
    okta_group_data = get_group(app_instance=okta_instance, scim=scim, group_mappings={})
    ...
    for member in okta_group_members:
        ...
        if app_instance_user_id:
            ...
            demisto.error(f"{log_prefix} {app_instance} id empty for user {username}. Skipping the user")
...
def execute_command(command_name, command_arguments, fail_on_error=True):
    ...
    if fail_on_error is True:
        if isError(command_response[0]):
            demisto.error(f"{log_prefix} Command {command_name} failed. Raw Response: {command_response}")
            raise Exception(f"{log_prefix} Command {command_name} failed. Raw Response: {command_response}")
...
def get_group_member_changes_since_last_sync(last_synced, to_date):
    ...
    okta_logs_events = get_okta_logs_for_group_changes(okta_group_id=okta_group_id, from_date=last_synced, to_date=to_date)
    ...
    for event in okta_logs_events:
        ...
        elif event_type == OKTA_LOCAL_GROUP_MEMBER_ADD_EVENT_TYPE or event_type == OKTA_LOCAL_GROUP_MEMBER_REMOVE_EVENT_TYPE:
            target_user_type = OKTA_LOCAL_USER_TYPE
        else:
            demisto.error(f"{log_prefix} Unsupported eventType for group change: {event_type}")
        ...
        if not app_instance_user_id:
            demisto.error(f"{log_prefix} {app_instance} id empty for user {username}. Failed to process okta event '{display_message}'")
        else:
            demisto.info(f"{log_prefix}: {display_message}: {username}: {app_instance_user_id}")
            ...
            if display_message in USER_ADDED_TO_GROUP_MESSAGE:
                ...
                demisto.error(f"{log_prefix} Unsupported displayMessage for group change: {display_message}")
                raise Exception(f"Unsupported displayMessage for group change: {display_message}")
...
def get_okta_logs_for_group_changes(okta_group_id, from_date, to_date):
    ...
    from_date_with_buffer = from_date - timedelta(minutes=int(int(OKTA_FETCH_LOGS_BUFFER_MINUTES)))
    ...
    okta_log_filter = f'(eventType eq "app.user_management" or eventType co "group.user_membership") ' \
                      f'and ' \
                      f'(target.id eq "{okta_group_id}" or target.displayName eq "{okta_group_name}")'

    demisto.info(f"{log_prefix} Fetching Okta logs from '{from_date_with_buffer}' to '{to_date}'. Filter: '{okta_log_filter}'")

    # Query Okta to get event logs for the group
    get_logs_response = execute_command("okta-get-logs",
...
def get_user_id_from_app_instance(app_instance, username):
    ...
    if not user_profile:
        ...
        demisto.info(f"{log_prefix} User '{username}' is inactive in XSOAR. Group sync skipped")
        return
    ...
    if user_data and user_data.get('id'):
        if user_data.get('active') is False:
            demisto.info(f"{log_prefix} User '{username}' is inactive in the app '{app_instance}'")
            user_id = None
            war_room_warning_logs.append(f"Warn: User '{username}' is inactive in app '{app_instance}'")
        else:
        ...
        get_user_response = execute_command("iam-get-user", {'user-profile': get_user_scim, 'using': app_instance})
        ...
        if not user_data:
            ...
            demisto.error(f"{log_prefix} User '{username}' not found in '{app_instance}'")
            war_room_warning_logs.append(f"Warn: User '{username}' not found in '{app_instance}'")
        else:
            demisto.error(f"{log_prefix} get-user failed for user '{username}'. Raw Response: '{get_user_response}'")
            raise Exception(f'Get User failed. Raw Response: {get_user_response}')
    return user_id

def get_app_provisioning_settings():
    app_provisioning_settings_response = demisto.executeCommand("getList", {'listName': APP_PROVISIONING_SETTINGS_LIST_NAME})
    ...
    if app_profiles:
        if type(app_profiles) != dict:
            try:
                ...
                demisto.error('appprofiles field in User Profile Indicator is not valid JSON. Returning empty data')
    ...
    if group_mappings:
        if type(group_mappings) != dict:
            try:
                ...
                demisto.error(f"{log_prefix} 'groupmappings' field in Group Profile Indicator is not valid JSON: {group_mappings}")
...
def get_demisto_user_profile(username):
    ...
    user_profiles = demisto.executeCommand("findIndicators", {"query": user_profile_query})[0][
        'Contents']

    if len(user_profiles)>1:
        demisto.error(f"{log_prefix} Multiple User Profile indicators found for: {username}")
        send_email(
    ...
    if user_profiles and len(user_profiles)!=0:
        ...
        war_room_warning_logs.append(f"Warn: User Profile indicator not found for: {username}")
...
def get_demisto_group_profile(group_id):
    ...
    group_profiles = demisto.executeCommand("findIndicators", {"query": group_profile_query})[0][
        'Contents']
...
def update_demisto_group_profile(indicator_id, group_mappings, group_mappings_updated):
    try:
        ...
        set_indicator_response = execute_command("setIndicator", {"id": indicator_id,
                                                                  GROUP_MAPPINGS_INDICATOR_FIELD: json.dumps(group_mappings)},
                                                 fail_on_error=False)

        demisto.debug("Group Mappings Data Updated in Group Profile Indicator: ", group_mappings)

    except Exception as e:
        return_error(f'{log_prefix} Error occurred while updating Group Profile Indicator {indicator_id} with the new group mappings: '
...
def create_demisto_group_profile(okta_group_id, group_mappings):
    try:
        ...
        create_indicator_response = demisto.executeCommand("createNewIndicator",
                                                           {"value": okta_group_id,
                                                            "displayname": okta_group_name,
                                                            GROUP_MAPPINGS_INDICATOR_FIELD: json.dumps(group_mappings),
                                                            "merge": True,
...
def send_email(send_to=None, subject=None, message=None, htmlbody=None):
    try:
        if not send_to:
            ...
            email_notification_list_response = demisto.executeCommand("getList", {'listName': EMAIL_NOTIFICATION_LIST})
            send_to = email_notification_list_response[0]['Contents']
        if send_to:
            demisto.executeCommand("send-mail", {"to":send_to, "subject":subject,"body":message, "htmlBody":htmlbody})
    except Exception as e:
...
def get_iam_html_body(app_instance_responses):
    ...
    try:
        for entry in app_instance_responses:
            htmlResults += '<table bgcolor="#ffffff" ' \
            ...
            for key, value in entry.items():
                if value is not None:
                    ...
                    htmlResults += '<td style="padding-top:24px">' + str(value) + '</td></tr>'
