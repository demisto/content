...
def get_app_from_okta(app_id, okta_iam_instance):
    okta_apps = demisto.executeCommand('okta-iam-list-applications', {'query': app_id, 'using': okta_iam_instance})[0]["Contents"]
...
def is_valid_instance(instance_name):
    ...
    for instance, details in all_instances.items():
      if instance == instance_name and details.get('category') == 'Identity and Access Management' and details.get('state') == 'active':
          return True
    return False
...
def check_instance_and_application_availability(configuration, instance_name, app_id):
    for conf in configuration:
        if conf.get('Instance') == instance_name:
            ...
            raise DemistoException(f'The selected instance is already connected to the application "{app_id}".')
        elif conf.get('ApplicationID') == app_id:
            instance = conf.get('Instance')
            raise DemistoException(f'The selected application is already connected to instance "{instance}".')
...
def add_configuration(configuration, custom_fields, okta_iam_instance):
    ...
    if instance_name != SINGLE_SELECT_PLACEHOLDER:
        check_instance_and_application_availability(configuration, instance_name, app_id)
...
def update_configuration_in_okta(configuration, okta_iam_instance):
    cmd_res = demisto.executeCommand('okta-iam-set-configuration', {'configuration': configuration, 'using': okta_iam_instance})
    if is_error(cmd_res):
        raise Exception('An unexpected error occurred - the configuration was not updated.')

def create_configuration_markdown(configuration):
    data = []
    # in the next lint, the empty string is the logo header name
    headers = ['Integration instance in XSOAR', 'App name in Okta', 'App label in Okta', 'App ID in Okta', '']
    for conf in configuration:
        data.append({
...
def main():
    ...
    if demisto.args().get('add_or_remove') == 'remove':
        ...
        if not config_removed:
            return
        custom_fields_to_clear = {CONNECTED_APP_ID_FIELD: '', CONNECTED_INSTANCE_NAME_FIELD: SINGLE_SELECT_PLACEHOLDER}
    else:
        config_added = add_configuration(configuration, custom_fields, okta_iam_instance)
        if not config_added:
            return
        custom_fields_to_clear = {INSTANCE_NAME_FIELD: SINGLE_SELECT_PLACEHOLDER, APP_ID_FIELD: '', APP_NAME_FIELD: '', SELECTED_APP_FIELD: SINGLE_SELECT_PLACEHOLDER}
    ...
    demisto.executeCommand('setIncident', {'id': incident.get('id'), 'customFields': custom_fields})
