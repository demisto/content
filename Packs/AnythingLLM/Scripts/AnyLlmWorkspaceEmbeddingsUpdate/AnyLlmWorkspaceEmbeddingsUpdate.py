            ...
            if old != new:
                if new["action"] in ["Pin", "Unpin"]:
                    ...
                    execute_command(
                        ...
                        {"workspace": workspace, "folder": folder, "document": new["title"], "status": status},
                    )
                elif new["action"] == "Remove":
                    newgrid.remove(new)
                    execute_command(
                        "anyllm-workspace-delete-embedding", {"workspace": workspace, "folder": folder, "document": new["title"]}
                    )
                updated = True
            index += 1
