...
class Client(BaseClient):
    ...
    def upload_entry_id(self, file_path: str, file_name: str) -> dict[str, Any]:
        ...
        response_get_form = self._http_request(method="POST", url_suffix="import/upload")
...
def raise_error_if_no_data(results: dict[str, Any]):
    ...
    if results.get("data") is None:
        if results.get("message"):
            ...
            raise ValueError(
                "No response from server, the server could be temporary unavailable or it is handling too "
...
def check_status_command(client: Client, arguments: dict[str, Any]):
    ...
    if argToBoolean(arguments.get("create_war_room_entry", False)) and results_data.get("operation") == "download/url":
    ...
    if (
        results_data.get("status") == "finished"
...
def format_operation_title(results: dict[str, Any]):
    ...
    """
    title_exchange_dict = {"import/url": "upload/url", "import/upload": "upload/entry", "export/url": "download/url"}
...
def main() -> None:
    try:
        ...
        err_msg = (
            ...
            if "No query results for model" in str(e)
            else ("No more conversion minutes for today for this user" if "Payment Required" in str(e) else str(e))
        )
