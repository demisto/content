...
def results_return(command, data):
    if command == 'GetVersion':
        results = CommandResults(
            ...
            outputs_key_field='',
            outputs={'version': data.get('version')}
        )
    elif data.get('data'):
        results = CommandResults(
            outputs_prefix='CiscoAMP.' + str(command),
...
def create_incident_from_log(log):
    occurred = log.get('date')
    keys = log.keys()
    labels = []
    for key in keys:
        labels.append({'type': key, 'value': str(log[key])})
        if log.get("detection"):
            formatted_description = f'Cisco AMP Incident - {log.get("detection")}'
        else:
...
def fetch_incidents(client, params):
    parameters = {}
    timefrom = dateparser.parse(params.get('fetch_time')).strftime('%Y-%m-%dT%H:%M:%SZ')
    ...
    if 'data' in results and results.get('metadata', {}).get('results', {}).get('total', 0) > 0:
        demisto.setLastRun({'start_time': end_time})
        demisto.incidents(form_incidents(results.get('data')))
    else:
        demisto.setLastRun({'start_time': end_time})
        demisto.incidents([])
...
def main():
    ...
    base_url = urljoin(params.get('server'), '/v1/')
