...
def objects_list_command(client, args):
    ...
    if raw_res.get("value"):
        human_readable = tableToMarkdown(
            f"{object_type} results", t=raw_res.get("value"), headers=OBJECT_HEADERS, removeNull=True
        )
...
def upload_attachment_command(client, args):
    ...
    if raw_res:
        ...
        entry_context = {"IvantiHeat.Attachment": {"RecId": rec_id, "AttachmentId": attachment_id, "FileName": file_name}}
...
def perform_action_command(client, args):
    ...
    raw_res = client.do_request(
        "POST", f"odata/businessobject/{object_type}('{object_id}')/{action}", json_data=request_data, resp_type="other"
    )
...
def create_object_command(client, args):
    ...
    human_readable = tableToMarkdown(
        f"{object_type} object created successfully", t=raw_res, headers=OBJECT_HEADERS, removeNull=True
    )
...
def main():
    ...
    try:
        client = Client(
            base_url=base_url, verify=verify_certificate, headers={"Authorization": f"rest_api_key={token}"}, proxy=proxy
        )
