...
def search_incidents(args: Dict):   # pragma: no cover
    ...
    while len(all_found_incidents) < limit and page < max_page:
        ...
        demisto.debug(
            f'Found the following incidents amount '
            f'{len(current_page_found_incidents)} before filtering with args {args} with page = {page}'
        )
    ...
    demisto.debug(f'amount of all the incidents that were found {all_found_incidents}')
