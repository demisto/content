...
def get_indicator_data(indicator):
    return {
        k: v
        for k, v in list(indicator.items())
        if k in ["indicator_type", "firstSeen", "lastSeen", "investigationIDs", "manualScore", "id", "score", "value"]
    }
...
def indicator_malicious_ratio_calculation(args):
    ...
    if incidents:
        ...
        indicators_result = [x for x in indicators_result if x["total_incidents_count"] >= appears_in_min_num_of_incidents]
        ...
        results = CommandResults(
            ...
            readable_output=tableToMarkdown(
                "Indicators Malicious Ratio",
                indicators_result[:max_results],
                headers=["value", "indicator_type", "malicious_ratio", "total_incidents_count", "score", "lastSeen"],
            ),
        )
