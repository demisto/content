...
def main():
    ...
    if parse_all:
        ...
        output = {"ParseCSV.ParsedCSV": all_csv}
        ...
        with open(file_path) as f:
            ...
            md = (
                ...
                + ("Domains |" if "domains" in d_args else "")
                + ("Hashes |" if "hashes" in d_args else "")
                + "\n"
            )
            md += (
                ("- |" if "ips" in d_args else "")
                + ("- |" if "domains" in d_args else "")
        ...
        if domain_list:
            old_domain_list = list(demisto.get(demisto.context(), "domains")) if demisto.get(demisto.context(), "domains") else []
        ...
        if hash_list:
            old_hash_list = list(demisto.get(demisto.context(), "hashes")) if demisto.get(demisto.context(), "hashes") else []
