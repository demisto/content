    ...
    def recursively_unpack_data(item_to_unpack: dict[Any, Any], path: str):
        for key, value in item_to_unpack.items():
            if isinstance(value, dict):
                ...
                unpacked_data.append({columns[0]: key, columns[1]: value if isinstance(value, str | int | float | bool) else ""})
...
def build_grid(
    context_path: str, keys: list[str], columns: list[str], unpack_nested_elements: bool, keys_from_nested: list[str]
) -> pd.DataFrame:
    ...
    if unpack_nested_elements:
        ...
        entry_context_data = [
            entry_dicts_to_string(dict_obj=filter_dict(item, keys, len(columns)), keys_to_choose=keys_from_nested)
        ...
        entry_context_data = entry_dicts_to_string(
            dict_obj=filter_dict(entry_context_data, keys), keys_to_choose=keys_from_nested
        )
        if keys == ["*"]:
...
def build_grid_command(
    ...
    overwrite: bool,
    sort_by: list[str],
    unpack_nested_elements: bool,
    keys_from_nested: list[str],
) -> list[dict[Any, Any]]:
    ...
    new_table: pd.DataFrame = build_grid(
