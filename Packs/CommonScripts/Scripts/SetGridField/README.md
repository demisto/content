...
| unpack_nested_elements | Columns names by which to sort the rows.                                                                                                                                                                                                                                                              |
| keys_from_nested | Keys to retrieve from nested dictionaries, can be used only when unpack_nested_elements set to False. Keys will not be columns correlated. Default is all keys. **Note**: when the number of values exceeds the number of columns, it truncates the last values that are outside the range for table. |
