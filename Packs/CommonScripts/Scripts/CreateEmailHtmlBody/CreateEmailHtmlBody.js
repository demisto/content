...
while (found = reg.exec(html)) {
    ...
    if (path.indexOf('incident.labels.') === 0 && platform !== 'x2') {
        logDebug("Field " + path + " is handled as label.")
        map[path] = getLabel(incidents[0], path);

    } else if (path.indexOf('incident.') === 0 && platform !== 'x2') {
    ...
    } else if (path.indexOf('alert.labels.') === 0 && platform === 'x2') {
        logDebug("Field " + path + " is handled as label.")
        map[path] = getLabel(incidents[0], path);

    } else if (path.indexOf('alert.') === 0 && platform === 'x2') {
