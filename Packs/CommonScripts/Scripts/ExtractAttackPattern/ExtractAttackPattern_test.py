...
def test_extract_existing_mitre_id(mocker):
    ...
    Validate that name extracted successfully from the ID.
    """
    mocker.patch.object(eap, "get_mitre_results", return_value=[{"id": "T1530", "value": "Data from Cloud Storage Object"}])
...
def test_extract_with_value_error(mocker):
    ...
    mocker.patch.object(
        eap, "get_mitre_results", side_effect=ValueError("verify you have proper integration enabled to support it")
    )
    mocker.patch.object(demisto, "info")
    ...
    assert not result
    assert (
        demisto.info.call_args[0][0] == "Unsupported Command : mitre-get-indicator-name, "
    ...
    mocker.patch.object(eap, "get_mitre_results", side_effect=ValueError("Something went wrong"))
    mocker.patch.object(demisto, "info")
...
def test_extract_with_unknown_error(mocker):
    ...
    validate the error massage.
    """
    mocker.patch.object(eap, "get_mitre_results", side_effect=TypeError("Something went wrong"))
    mocker.patch.object(demisto, "info")
