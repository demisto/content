...
class TestDownloadAndArchivePythonLibrary(unittest.TestCase):
    ...
    def test_install_library_success(self, mock_mkdtemp, mock_path, mock_open, mock_os_walk, mock_zipfile, mock_popen):
        ...
        # Set up mock for Path and mkdtemp
        mock_path.return_value = Path('/path/to/dir')
        mock_mkdtemp.return_value = '/path/to/dir'

        # Call the function to test
        result = installLibrary(Path('/path/to/dir'), 'library_name')
        ...
        mock_zipfile.assert_called_once_with(Path('/path/to/dir') / 'library_name.zip', 'w', compression=zipfile.ZIP_DEFLATED, compresslevel=9)
        mock_open.assert_called_once_with(Path('/path/to/dir') / 'library_name.zip', 'rb')
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('DownloadAndArchivePythonLibrary.Path')
    def test_install_library_no_files(self, mock_path, mock_open, mock_zipfile, mock_os_walk):
