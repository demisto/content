...
def test_main_malicious_ratio_reputation(mocker):
    ...
    mocker.patch.object(
        MaliciousRatioReputation, "get_indicator_from_value", return_value={"value": "value_a", "indicator_type": "IP"}
    )
    res_get_indicator_result = {
        "Type": 1,
        "EntryContext": {"DBotScore": {"Type": "ip", "Score": 2, "Vendor": "DBot-MaliciousRatio", "Indicator": "value_a"}},
        "Contents": 2,
        "ContentsFormat": "text",
        "HumanReadable": "Malicious ratio for value_a is -1.00",
        "ReadableContentsFormat": "markdown",
    }
    mocker.patch.object(MaliciousRatioReputation, "get_indicator_result", return_value=res_get_indicator_result)
...
def test_get_indicator_from_value(mocker):
    ...
    execute_command_res = [
        {
            "Contents": [{"id": "a", "investigationIDs": ["1", "2", "10"], "value": "value_a", "indicator_type": "File"}],
            "Type": "note",
        }
    ]
    execute_mock = mocker.patch.object(demisto, "executeCommand", return_value=execute_command_res)
