...
def indicator_set_validation_checks(ivalue: str, checks: list[CertificateValidationTag]) -> None:
    ...
    for c in checks:
        demisto.executeCommand("setIndicator", {"value": ivalue, "type": "Certificate", "certificatevalidationchecks": c.value})
...
def dbot_context(
    value: str, certificate_context: dict[str, Any]
    ...
    if parsed_validity_not_after is None:
        ...
        comments.append(
            f'{CertificateValidationTag.EXPIRED.value} '
    ...
    if parsed_validity_not_before is None:
        ...
        comments.append(
            f'{CertificateValidationTag.NOT_VALID_YET.value} '
    ...
    if parsed_validity_not_after is not None and parsed_validity_not_before is not None:
        if parsed_validity_not_before >= parsed_validity_not_after:
            comments.append(
                f"{CertificateValidationTag.INVALID_VALIDITY_WINDOW.value} "
    ...
    if extensions is None:
        ...
        subject_key_identifier = next(
            (e.get("Value") for e in extensions if e.get("OID") == "2.5.29.14"), None
        )  # disable-secrets-detection
        authority_key_identifier = next(
            (e.get("Value") for e in extensions if e.get("OID") == "2.5.29.35"), None
        )  # disable-secrets-detection
        ...
        if (
            authority_key_identifier is None
            or (authority_key_identifier_ki := authority_key_identifier.get("KeyIdentifier")) is None
        ):
            some_checks_not_performed = True
            comments.append("Valid AuthorityKeyIdentifier Extension not available, some checks not performed")

        if (
            subject_key_identifier_digest is not None
        ...
        if (
            CertificateValidationTag.SELF_SIGNED in tags
            and subject_dn is not None
            and issuer_dn is not None
            and subject_dn != issuer_dn
        ):
            comments.append(
                f"{CertificateValidationTag.INVALID_DISTINGUISHED_NAMES.value}"
    ...
    return (
        tags,
        comments,
        {"DBotScore": {"Score": current_score, "Vendor": "X509Certificate", "Type": "certificate", "Indicator": value}},
    )
