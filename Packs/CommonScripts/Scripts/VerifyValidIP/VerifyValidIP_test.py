...
def test_main_function(mocker):
    ...
    mock_args = mocker.patch('demisto.args')
    mock_info = mocker.patch('demisto.info')
    mock_return_results = mocker.patch('return_results')
    
    # Test with mixed valid and invalid IPs
    mock_args.return_value = {"input": "192.168.1.1,invalid_ip,2001:db8::1"}
    ...
    assert mock_info.call_count == 6  # 2 calls per IP (3 IPs total)
    mock_return_results.call_args[0][0]["HumanReadable"] = '| IP | Valid |\n|---|---|\n| 192.168.1.1 | True |\n| bad_ip | False |\n'