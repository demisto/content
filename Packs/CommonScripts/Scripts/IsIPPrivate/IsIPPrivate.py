...
def is_ip_internal(ip: str, ranges: List[str]) -> bool:
    try:
        ...
        return return_error(
            f"One or more IP ranges or IPs are invalid. Please make sure the list is in the correct structure." f"Error: {ve}"
        )
...
def main():
    ...
    ip_list = [
        {"Address": ip, "Private": is_ip_internal(ip, private_ranges), "Tag": get_ip_tag(ip, private_ranges)}
    ...
    for ip in ip_list:
        ...
        args_exists_check = {"indicator": address}
        ...
        args_set_existing_indicator = {"value": address, "internal": private}
        args_append_tag = {"field": "tags", "indicatorsValues": address, "fieldValue": ip.get("Tag", "")}
