...
def test_find_indicators_with_mal_ratio(mocker):
    ...
    widget_table, sorted_indicators = find_indicators_with_mal_ratio(
        max_indicators=1000, min_number_of_invs=4, max_results=1000, from_date="30 days ago"
    )
    assert (
        widget_table == '{"total": 2, "data": [{"ID": "7570", "Type": "URL", "Malicious Ratio": "0.11", "Value":'
...
def test_find_indicators_with_mal_ratio__no_indicators(mocker):
    ...
    widget_table, sorted_indicators = find_indicators_with_mal_ratio(
        max_indicators=1000, min_number_of_invs=5, max_results=1000, from_date="30 days ago"
    )
...
"""
    mocker.patch.object(
        demisto,
        "args",
        return_value={"maxNumberOfIndicators": "1000", "minimumNumberOfInvs": "4", "maximumNumberOfResults": "1000"},
    )
    mocker.patch.object(
        demisto,
        "executeCommand",
