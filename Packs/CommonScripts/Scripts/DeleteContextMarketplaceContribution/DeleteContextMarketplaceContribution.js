...
} else if (args.keyList !== undefined) {
    var keys = args.keyList.split(",");
    for (let i = 0; i < keys.length; i++) {
        if (isSubPlaybookKey) {
          key = 'subplaybook-${currentPlaybookID}.' + keys[i];
          executeCommand('delContext', {key: key});
        } else {
            executeCommand('delContext', {key: keys[i]});
        }

    }
    return "Successfully deleted keys [" + keys.join(", ") + "]";
