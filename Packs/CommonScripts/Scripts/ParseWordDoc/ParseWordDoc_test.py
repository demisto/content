...
class TestParseWordDoc:
    ...
    def test_parse_word_doc(self, mocker):
        ...
        self.mock_demisto(mocker, file_obj=self.create_file_object(str(Path(__file__).parent / "test_data/file-sample.docx")))
        main()
...
def test_extract_urls_xml_with_hyperlink():
    ...
    urls = extract_urls_xml(str(Path(__file__).parent / 'test_data/file-sample2.docx'))
...
def test_extract_urls_xml_without_hyperlink():
    ...
    """
    from ParseWordDoc import extract_urls_xml
    urls = extract_urls_xml(str(Path(__file__).parent / 'test_data/file-sample.docx'))
    assert urls == []
...
def test_extract_urls_docx_without_hyperlink():
    ...
    document = Document(str(Path(__file__).parent / 'test_data/file-sample2.docx'))
...
def test_extract_urls_docx_with_hyperlinks():
    ...
    from docx import Document
    from ParseWordDoc import extract_urls_docx
    document = Document(str(Path(__file__).parent / 'test_data/MS-DOCX-190319.docx'))
    urls = extract_urls_docx(document)
