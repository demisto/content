...
def docker_auth(image_name, verify_ssl=True, registry=DEFAULT_REGISTRY, gateway_creds=""):
    ...
    """
    res = requests.get("https://{}/v2/".format(registry), headers=ACCEPT_HEADER, timeout=TIMEOUT, verify=verify_ssl)
    if res.status_code == 401:  # need to authenticate
        ...
        if www_auth:
            ...
            demisto.info("Failed extracting www-authenticate header from registry: {}, final url: {}".format(registry, res.url))
...
def main():
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            res = requests.get(
                "https://{}/v2/{}/tags/list".format(registry, image_name), headers=headers, timeout=TIMEOUT, verify=verify_ssl
            )
            res.raise_for_status()
