...
def test_extract_info_from_qr_code(mocker: MockerFixture):
    ...
    mocker.patch.object(
        demisto,
        "getFilePath",
        return_value={"path": "test_data/qr_code.png"},
    )
    mocker.patch.object(
        demisto,
        "executeCommand",
        return_value=[{"Contents": '{"Domain": "xsoar.pan.dev"}', "Type": EntryType.NOTE}],
    )
...
def test_with_non_qr_code_file(mocker: MockerFixture):
    ...
    mocker.patch.object(
        demisto,
        "getFilePath",
        return_value={"path": "test_data/not_a_qr_code.png"},
    )
...
def test_extract_indicators_from_text(mocker: MockerFixture):
    ...
    mocker.patch.object(
        demisto,
        "executeCommand",
        return_value=[{"Contents": "Error message", "Type": EntryType.ERROR}],
    )
