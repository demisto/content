...
def main():
    try:
        ...
        tagsAndOperator = args.get('tagsAndOperator')
        if tagsAndOperator:
            tagsAndOperator = argToBoolean(tagsAndOperator)

        ents = demisto.executeCommand('getEntries',
                                      assign_params(id=args.get('id'),
                                                    filter=assign_params(pageSize=arg_to_number(args.get('pageSize')),
                                                                         lastId=args.get('lastId'),
                                                                         firstID=args.get('firstID'),
                                                                         selectedEntryID=args.get('selectedEntryID'),
                                                                         categories=argToList(args.get('categories')),
                                                                         tags=argToList(args.get('tags')),
                                                                         users=argToList(args.get('users')),
                                                                         tagsAndOperator=tagsAndOperator,
