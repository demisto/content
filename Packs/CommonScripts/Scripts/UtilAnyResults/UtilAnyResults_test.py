...
def test_ustil_any_results_list(mocker):
    mocker.patch.object(demisto, 'get', return_value=['a', 'b', 'c'])
    assert util_any_results({}) == ['yes']


def test_ustil_any_results_str(mocker):
    mocker.patch.object(demisto, 'get', return_value='abc')
    assert util_any_results({}) == ['yes']


def test_ustil_any_results_str_lst(mocker):
    mocker.patch.object(demisto, 'get', return_value='[a, b, c]')
    assert util_any_results({}) == ['yes']


def test_ustil_any_results_str_empty_lst(mocker):
