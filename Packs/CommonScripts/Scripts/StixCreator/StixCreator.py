...
def create_stix_sco_indicator(stix_id: Optional[str], stix_type: Optional[str], value: str, xsoar_indicator: Dict) -> Dict:
    ...
    """
    stix_indicator: Dict[str, Any] = {"type": stix_type, "spec_version": "2.1", "id": stix_id}
    if stix_type == "file":
...
def main():
    ...
    for indicator_fields in all_args:
        ...
        if demisto_indicator_type in XSOAR_TYPES_TO_STIX_SCO and is_sco:
            ...
            try:
                ...
                indicator = Indicator(pattern=f"[{SCOs[indicator_type]} = '{value}']", pattern_type="stix", **kwargs)
                ...
                try:
                    ...
                    demisto.info(f"Indicator type: {demisto_indicator_type}, with the value: {value} is not STIX compatible")
    ...
    if len(indicators) > 1:
        ...
        context = {"StixExportedIndicators(val.pattern && val.pattern == obj.pattern)": json.loads(str(bundle))}
        res = CommandResults(readable_output="", outputs=context, raw_response=str(bundle))

    elif len(indicators) == 1:
        bundle = Bundle(indicators, allow_custom=True)
        bundle_obj = bundle.get("objects", [])[0]
        context = {"StixExportedIndicators(val.pattern && val.pattern == obj.pattern)": json.loads(str(bundle_obj))}
        res = CommandResults(readable_output="", outputs=context, raw_response=str(bundle_obj))
    else:
        context = {"StixExportedIndicators": {}}
        res = CommandResults(readable_output="", outputs=context, raw_response={})
