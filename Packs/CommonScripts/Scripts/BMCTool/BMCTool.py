...
class BMCContainer(Generic[AnyStr]):
    ...
    def b_log(self, ltype, verbose, lmsg):
        log_type = {"info": demisto.info, "debug": demisto.debug, "error": demisto.error}
                ...
                if t_params & 0x08:  # This bit is always ONE when relevant data is smaller than expected data,
                    ...
                    if bl == 0:
                        if "22.bmc" in self.fname:
                            ...
                            if bl == 0:
                                self.b_log(
                                    "error", False, "Unable to determine data pattern size; exiting before throwing any error!"
                                )
                                return False
                    o_bmp = b""
                    ...
                    if len(t_bmp) > 0:
                        if len(t_bmp) != t_width * t_height * bl // (64 * 64):
                            self.b_log(
                                ...
                                False,
                                (
                                    f"Uncompressed tile data seems bogus (uncompressed {len(t_bmp)} bytes "
        ...
        while len(data) > 0:
            ...
            if cmd == -1:
                if rl == 1:
                    ...
                    self.b_log("error", False, f"Unexpected decompression command encountered (0x{sz:02x}). Skipping tile.")
                else:
            ...
            if cmd in [0x00, 0xF0]:
                ...
                if cmd in [0xC0, 0xF6]:
                    if len(data) < bbp:
                        self.b_log("error", False, f"Unexpected end of compressed stream. Skipping tile. ({cmd}, {rl})")
                        return b""
                    fgc = data[:bbp]
                    data = data[bbp:]
                if len(d_out) < 64 * bbp:
    ...
    def b_export_bmp(self, width, height, data):
        if not self.pal:
            return (
                ...
                + pack("<L", len(data))
                + b"\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\xff\x00\x00\xff\x00\x00\xff\x00\x00\x00\x00\x00\x00\xff niW"  # noqa: E501
                + (  # noqa: E501
                    b"\x00" * 36
                )
            ...
            return (
                b"BM"
                + pack("<L", len(data) + 0x36)
                + b"\x00\x00\x00\x00\x36\x04\x00\x00\x28\x00\x00\x00"
                + pack("<L", width)
                + pack("<L", height)
                + b"\x01\x00\x08\x00\x00\x00\x00\x00"
                + pack("<L", len(data) - 0x400)
...
def main():
    ...
    try:
        ...
        results = CommandResults(outputs_prefix="Collages", outputs=sources)
