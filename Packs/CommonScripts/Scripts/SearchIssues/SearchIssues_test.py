...
def test_filter_events(mocker, args, filtered_args, expected_result):
    ...
    """
    import Packs.CommonScripts.Scripts.SearchIssues.SearchIssues as SearchIssues

    execute_mock = mocker.patch.object(SearchIssues, "execute_command", side_effect=get_incidents_mock)
    ...
    _, res, _ = SearchIssues.search_incidents(args)
...
def test_includeinformational_logic(mocker, args, expected_filtered_args, expected_result):
    ...
    mocker.patch.object(SearchIssues, "is_xsiam", return_value=True)
    ...
    execute_mock = mocker.patch.object(SearchIssues, "execute_command", side_effect=get_incidents_mock_include_informational)

    if expected_result is ValueError:
        with pytest.raises(ValueError):
            SearchIssues.search_incidents(args)
    else:
        _, res, _ = SearchIssues.search_incidents(args)
