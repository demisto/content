...
def test_eliminate_urls_extensions():
    url = (
        "https://urldefense.proofpoint.com/v2/url?u=http-3A__hellothere.com_&d=DwIGaQ&c=XRWvQHnpdBDRh-yzrHjqLpXuH"
...
def test_empty_query(mocker):
    ...
    """
    mocker.patch.object(
        demisto, "executeCommand", side_effect=lambda function_name, args: [{"Contents": json.dumps(args), "Type": -1}]
    )
