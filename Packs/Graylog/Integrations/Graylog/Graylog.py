...
def results_return(command, thingtoreturn):
    results = CommandResults(outputs_prefix="Graylog." + str(command), outputs_key_field="", outputs=thingtoreturn)
...
def create_incident_from_log(log):
    ...
    return {"name": formatted_description, "labels": labels, "rawJSON": json.dumps(log), "occurred": occurred}
...
def fetch_incidents(client):
    ...
    end_time += "Z"
    parameters = {"query": incidentquery, "from": start_time, "to": end_time}
...
def main():
    ...
    headers = {"X-Requested-By": "xsoar", "Accept": "application/json"}

    demisto.info(f"Command being called is {demisto.command()}")
    try:
        client = BaseClient(base_url=base_url, verify=verify_certificate, auth=(username, password), headers=headers, proxy=proxy)
