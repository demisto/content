...
EXPECTED_TABLE_PARSE_URL_DOMAIN_ALL_RESULTS = tableToMarkdown(f'results for DOMAIN from {MOCK_URL}',
                                                              ["a1.com", "b2.com"],
                                                              headers='DOMAIN')

EXPECTED_TABLE_PARSE_URL_DOMAIN_LIMIT1_RESULTS = tableToMarkdown(f'results for DOMAIN from {MOCK_URL}',
                                                                 ["a1.com"],
                                                                 headers='DOMAIN')

EXPECTED_TABLE_PARSE_URL_URL_ALL_RESULTS = tableToMarkdown(f'results for URL from {MOCK_URL}',
                                                           ["https://a1.com"],
                                                           headers='URL')

EXPECTED_OUTPUTS_PARSE_URL_NO_KEYS_NO_LIMIT = {'url': MOCK_URL,
                                               'Results': [{'type': 'DOMAIN', 'value': "a1.com"},
                                                           {'type': 'DOMAIN', 'value': "b2.com"},
                                                           {'type': 'URL', 'value': "https://a1.com"}]}

EXPECTED_OUTPUTS_PARSE_URL_DOMAIN_KEYS_1_LIMIT = {'url': MOCK_URL,
                                                  'Results': [{'type': 'DOMAIN', 'value': "a1.com"}]}

EXPECTED_OUTPUTS_PARSE_URL_URL_KEYS_NO_LIMIT = {'url': MOCK_URL,
...
EXPECTED_TABLE_PARSE_JSON_AND_RAW_TEXT_DOMAIN_ALL_RESULTS = tableToMarkdown(f'results for DOMAIN',
                                                                            ["a1.com"],
                                                                            headers='DOMAIN')

EXPECTED_TABLE_PARSE_JSON_AND_RAW_TEXT_DOMAIN_LIMIT1_RESULTS = tableToMarkdown(f'results for DOMAIN',
                                                                               ["a1.com"],
                                                                               headers='DOMAIN')

EXPECTED_TABLE_PARSE_JSON_AND_RAW_TEXT_FILE_HASH_SHA256_ALL_RESULTS = tableToMarkdown(f'results for FILE_HASH_SHA256',
                                                                                      ["dummysha256"],
                                                                                      headers='FILE_HASH_SHA256')

EXPECTED_OUTPUTS_PARSE_JSON_TEXT_NO_KEYS_NO_LIMIT = {'data': MOCK_JSON_TEXT,
                                                     'Results': [{'type': 'DOMAIN', 'value': "a1.com"},
                                                                 {'type': 'FILE_HASH_SHA256', 'value': "dummysha256"}]}

EXPECTED_OUTPUTS_PARSE_JSON_TEXT_DOMAIN_KEYS_1_LIMIT = {'data': MOCK_JSON_TEXT,
                                                        'Results': [{'type': 'DOMAIN', 'value': "a1.com"}]}

EXPECTED_OUTPUTS_PARSE_JSON_TEXT_FILE_HASH_SHA256_KEYS_NO_LIMIT = {'data': MOCK_JSON_TEXT,
                                                                   'Results':
                                                                       [{'type': 'FILE_HASH_SHA256',
                                                                         'value': "dummysha256"}]}

EXPECTED_OUTPUTS_PARSE_RAW_TEXT_NO_KEYS_NO_LIMIT = {'data': MOCK_RAW_TEXT,
                                                    'Results': [{'type': 'DOMAIN', 'value': "a1.com"},
                                                                {'type': 'FILE_HASH_SHA256', 'value': "dummysha256"}]}

EXPECTED_OUTPUTS_PARSE_RAW_TEXT_DOMAIN_KEYS_1_LIMIT = {'data': MOCK_RAW_TEXT,
                                                       'Results': [{'type': 'DOMAIN', 'value': "a1.com"}]}

EXPECTED_OUTPUTS_PARSE_RAW_TEXT_FILE_HASH_SHA256_KEYS_NO_LIMIT = {'data': MOCK_RAW_TEXT,
                                                                  'Results':
                                                                      [{'type': 'FILE_HASH_SHA256',
                                                                        'value': "dummysha256"}]}
...
IOC_FROM_RAW_TEXT_LIST = [
    ...
    ({'data': MOCK_RAW_TEXT, 'keys': ['DOMAIN'], 'limit':1},
     "./test_data/ioc_from_json_and_raw_text_response.json",
     [EXPECTED_TABLE_PARSE_JSON_AND_RAW_TEXT_DOMAIN_LIMIT1_RESULTS],
     EXPECTED_OUTPUTS_PARSE_RAW_TEXT_DOMAIN_KEYS_1_LIMIT),
    ({'data': MOCK_RAW_TEXT, 'keys': ['FILE_HASH_SHA256']},
     "./test_data/ioc_from_json_and_raw_text_response.json",
     [EXPECTED_TABLE_PARSE_JSON_AND_RAW_TEXT_FILE_HASH_SHA256_ALL_RESULTS],
...
EXPECTED_TABLE_PARSE_TWITTER_DOMAIN_ALL_RESULTS = tableToMarkdown(f'results for DOMAIN from {MOCK_USERNAME}',
                                                                  ["a1.com"],
                                                                  headers='DOMAIN')

EXPECTED_TABLE_PARSE_TWITTER_FILE_HASH_SHA256_ALL_RESULTS = tableToMarkdown(f'results for FILE_HASH_SHA256 from {MOCK_USERNAME}',
                                                                            ["dummysha256_0", "dummysha256_1"],
                                                                            headers='FILE_HASH_SHA256')

EXPECTED_TABLE_PARSE_TWITTER_URL_ALL_RESULTS = tableToMarkdown(f'results for URL from {MOCK_USERNAME}',
                                                               ["https://a1.com"],
                                                               headers='URL')

EXPECTED_OUTPUTS_PARSE_TWITTER_NO_KEYS_NO_LIMIT = {'data': MOCK_USERNAME,
                                                   'Results': [{'type': 'FILE_HASH_SHA256', 'value': "dummysha256_0"},
                                                               {'type': 'FILE_HASH_SHA256', 'value': "dummysha256_1"},
                                                               {'type': 'DOMAIN', 'value': "a1.com"},
                                                               {'type': 'URL', 'value': "https://a1.com"}]}

EXPECTED_OUTPUTS_PARSE_TWITTER_DOMAIN_KEYS_1_LIMIT = {'data': MOCK_USERNAME,
                                                      'Results': [{'type': 'DOMAIN', 'value': "a1.com"}]}

EXPECTED_OUTPUTS_PARSE_TWITTER_FILE_HASH_SHA256_KEYS_NO_LIMIT = {'data': MOCK_USERNAME,
                                                                 'Results':
                                                                     [{'type': 'FILE_HASH_SHA256', 'value': "dummysha256_0"},
                                                                      {'type': 'FILE_HASH_SHA256', 'value': "dummysha256_1"}]}

EXPECTED_OUTPUTS_PARSE_TWITTER_URL_DOMAIN_KEYS_NO_LIMIT = {'data': MOCK_USERNAME,
                                                           'Results': [{'type': 'DOMAIN', 'value': "a1.com"},
                                                                       {'type': 'URL', 'value': "https://a1.com"}]}
...
IOC_FROM_TWITTER_LIST = [
    ...
    ({'data': MOCK_USERNAME, 'keys': ['DOMAIN'], 'limit':1},
     "./test_data/ioc_from_twitter_response.json",
     [EXPECTED_TABLE_PARSE_TWITTER_DOMAIN_ALL_RESULTS],
     EXPECTED_OUTPUTS_PARSE_TWITTER_DOMAIN_KEYS_1_LIMIT),
    ({'data': MOCK_USERNAME, 'keys': ['URL', 'DOMAIN']},
     "./test_data/ioc_from_twitter_response.json",
     [EXPECTED_TABLE_PARSE_TWITTER_DOMAIN_ALL_RESULTS,
      EXPECTED_TABLE_PARSE_TWITTER_URL_ALL_RESULTS],
...
# @pytest.mark.parametrize('args, response_file_name, expected_tables, expected_outputs', IOC_FROM_RAW_TEXT_LIST_ENTRY_ID)
# def test_ioc_from_raw_text_command_valid_response_for_entry_id_field(mocker,
#                                                                      args,
#                                                                      response_file_name,
#                                                                      expected_tables,
#                                                                      expected_outputs):
