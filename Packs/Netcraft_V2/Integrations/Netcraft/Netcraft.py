...
RES_CODE_TO_MESSAGE = {
    ...
    "TD_VERIFY": "The submitted content is undergoing additional verification to ensure that it is a valid takedown target."
...
class Client(BaseClient):
    def takedown_http_request(
        ...
        **kwargs,
        ...
        return self._http_request(
            method,
            full_url=urljoin(self._base_url["takedown_url"], url_suffix),
            params=params,
            data=data,
            files=files,
            resp_type=resp_type,
    ...
    def submission_http_request(
        ...
        **kwargs,
        ...
        return self._http_request(
            method,
            full_url=urljoin(self._base_url["submission_url"], url_suffix),
            params=params,
            json_data=json_data,
            files=files,
            resp_type=resp_type,
    ...
    def get_submission(self, uuid: str, ignore_404: bool) -> dict:
        return self.submission_http_request("GET", f"api/v3/submission/{uuid}", ok_codes=(200, 404 * ignore_404))
    ...
    def submission_file_list(self, submission_uuid, params: dict) -> dict:
        return self.submission_http_request(
            "GET",
            f"api/v3/submission/{submission_uuid}/files",
            params=params,
        )

    def file_screenshot_get(self, submission_uuid: str, file_hash: str) -> requests.Response:
        return self.submission_http_request(
            "GET", f"api/v3/submission/{submission_uuid}/files/{file_hash}/screenshot", resp_type="response", ok_codes=(200, 404)
        )
    ...
    def submission_mail_get(self, submission_uuid: str) -> dict:
        return self.submission_http_request(
            "GET",
            f"api/v3/submission/{submission_uuid}/mail",
        )

    def mail_screenshot_get(self, submission_uuid: str) -> requests.Response:
        return self.submission_http_request(
            "GET", f"api/v3/submission/{submission_uuid}/mail/screenshot", resp_type="response", ok_codes=(200, 404)
        )
    ...
    def submission_url_list(self, submission_uuid, params: dict) -> dict:
        return self.submission_http_request(
            "GET",
            f"api/v3/submission/{submission_uuid}/urls",
            params=params,
        )
    ...
    def url_screenshot_get(self, submission_uuid: str, url_uuid: str, screenshot_hash: str) -> requests.Response:
        return self.submission_http_request(
            "GET",
            f"api/v3/submission/{submission_uuid}/urls/{url_uuid}/screenshots/{screenshot_hash}",
            resp_type="response",
        )
...
def convert_binary_keys_to_bool(d: dict, *keys):
    ...
    Examples:
        ...
        {'a': True, 'b': False}
    """
    d.update({key: int_to_bool(d.get(key)) for key in keys})
...
def attack_report_command(args: dict, client: Client) -> CommandResults:
    ...
    def response_to_outputs(response: str) -> dict[str, Any]:
        ...
        if code not in RES_CODE_TO_MESSAGE:
            raise DemistoException(f"Error in Netcraft API call:\n{response}")
        table = {
        ...
        return {
            ...
            "readable_output": tableToMarkdown("Netcraft attack reported", table, list(table), removeNull=True),
        }

    response = client.attack_report(*args_to_api_body_and_file(args))
    return CommandResults(
        outputs_prefix="Netcraft.Takedown", outputs_key_field="id", raw_response=response, **response_to_outputs(response)
    )
...
def takedown_list_command(args: dict, client: Client) -> CommandResults:
    def args_to_params(args: dict) -> dict:
        return args | {
            ...
            "max_results": arg_to_number(args.pop("limit")) if not argToBoolean(args.pop("all_results")) else TAKEDOWN_API_LIMIT,
            "region": args.get("region") or PARAMS["region"],
        }

    def response_to_context(response: list[dict]) -> list[dict]:
        for takedown in response:
            convert_binary_keys_to_bool(takedown, "authgiven", "escalated", "has_phishing_kit", "false_positive", "managed")
    ...
    def response_to_readable(response: list[dict]) -> str:
        ...
        return tableToMarkdown("Netcraft Takedowns", response, list(header_map), headerTransform=header_map.get, removeNull=True)

    response = client.get_takedowns(args_to_params(args))
    return CommandResults(
...
def takedown_update_command(args: dict, client: Client) -> CommandResults:
    ...
    response = client.takedown_update(args_to_params(args))
...
def takedown_note_create_command(args: dict, client: Client) -> CommandResults:
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(
            ...
            {"Note ID": response["note_id"], "Takedown ID": args["takedown_id"]},
            ["Note ID", "Takedown ID"],
        ),
    )
...
def attack_type_list_command(args: dict, client: Client) -> CommandResults:
    ...
    response = client.attack_type_list(
        ...
        | {"auto_authorise": args.get("auto_authorize"), "region": args.get("region") or PARAMS["region"]}
    )
    response = response if argToBoolean(args["all_results"]) else response[:50]
    return CommandResults(
...
def submission_list(args: dict, client: Client) -> CommandResults:
    ...
    def args_to_params(args: dict) -> dict:
        return sub_dict(args, "source_name", "submission_reason", "submitter_email") | {
...
def submission_report_results(args: dict, response: dict, client: Client) -> CommandResults:
    ...
    if not argToBoolean(args["polling"]):
        return CommandResults(
            ...
            readable_output=tableToMarkdown("Submission successfully reported.", {"Submission UUID": response["uuid"]}),
        )
...
def file_report_submit_command(args: dict, client: Client) -> CommandResults:
    ...
    response = client.file_report_submit(args_to_body(args))
...
def file_screenshot_get_command(args: dict, client: Client) -> dict | CommandResults:
    ...
    if response.status_code == 404:
        return CommandResults(readable_output="No screenshot for file.")
    return fileResult(
...
def mail_screenshot_get_command(args: dict, client: Client) -> dict | CommandResults:
    ...
    if response.status_code == 404:
        return CommandResults(readable_output="No screenshot for mail.")
    return fileResult(
    ...
    response = client.url_report_submit(
        {
            "email": args.pop("reporter_email"),
            "reason": args.pop("reason", None),
            "urls": [{"url": url} for url in argToList(args.pop("urls"))],
        }
    )
    return submission_report_results(args, response, client)
...
def url_screenshot_get_command(args: dict, client: Client) -> dict:
    ...
    return fileResult(f'url_screenshot_{args["screenshot_hash"]}.{file_type}', response.content, EntryType.ENTRY_INFO_FILE)
