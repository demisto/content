...
def get_access_token(cloud_type: str, scopes: list = None) -> str:
    ...
    result = demisto._platformAPICall("/cts/accounts/token", "POST", {"request_data": request_data})

    if result.get("status_code") != 200:
        raise DemistoException(
            f'Failed to get token from CTS for {cloud_type}. Error code: {result.get("status_code")}'
        )  # TODO - Add error details

    try:
        return json.loads(result.get("data")).get("access_token")
    except json.JSONDecodeError as e:
        raise DemistoException(f"Failed to parse token response for {cloud_type}") from e
...
def main():
    ...
    try:
        ...
        if not creds:
            token = get_access_token("GCP", scopes=SCOPES)
