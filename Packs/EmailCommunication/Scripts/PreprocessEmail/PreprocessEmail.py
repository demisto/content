...
def get_query_window():
    ...
    try:
        ...
        demisto.error(
            "Invalid input for number of days to query in the `XSOAR - Email Communication Days To Query` "
...
def create_email_html(email_html="", entry_id_list=[]):
    ...
    for image_name, image_entry_id in entry_id_list:
        ...
        if re.search(rf'(src="cid:{content_id}")', email_html):
            ...
            email_html = re.sub(f'src="[^>]+"(?=[^>]+alt="{image_name}")', f"src={saas_xsoar_xsiam_prefix}entry/download/{image_entry_id}", email_html)
...
def add_entries(email_reply, email_related_incident, reputation_calc_async=False):
    ...
    res = demisto.executeCommand(
        "addEntries", {"entries": entries_str, "id": email_related_incident, "reputationCalcAsync": reputation_calc_async}
    )
    if is_error(res):
...
def get_incident_by_query(query):
    ...
    res = demisto.executeCommand("getIncidents", {"query": query, "populateFields": "id,status,type,emailsubject"})[0]
...
def get_attachments_using_instance(email_related_incident, labels, email_to, identifier_ids=""):
    ...
    if integration_name in ["EWS v2", "EWSO365"]:
        demisto.executeCommand(
            "executeCommandAt",
            {
                "command": "ews-get-attachment",
                "incidents": email_related_incident,
        ...
        if email_subject and email_original_subject in email_subject:
            ...
            try:
                ...
                if incident_email_threads:
                    ...
                    search_result = next(
                        (i for i, item in enumerate(incident_email_threads) if email_original_subject in item["EmailSubject"]),
                        None,
                    )
        ...
        try:
            demisto.executeCommand(
                "executeCommandAt",
                {
                    ...
                    "arguments": {"key": "EmailThreads", "value": email_message, "append": "true"},
                },
            )
        except Exception as e:
