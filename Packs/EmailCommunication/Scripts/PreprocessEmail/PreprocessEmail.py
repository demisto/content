...
def add_entries(single_reply, email_related_incident):
    ...
    if is_error(res):
        ...
        # Fail safe to not drop emails if we couldn't add entries to existing incident
...
def check_incident_status(incident_details, email_related_incident):
    ...
    if status == 2:  # closed
        ...
        if is_error(res):
            ...
            # Fail safe to not drop emails if we couldn't reopen incident
            demisto.results(True)
            sys.exit(0)
