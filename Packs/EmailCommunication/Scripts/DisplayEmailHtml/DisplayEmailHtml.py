...
def create_email_html(email_html="", entry_id_list=None):
    ...
    for entry_id in entry_id_list:
        ...
        if re.search(f'src="[^>]+"(?=[^>]+alt="{entry_id[0]}")', email_html):
            email_html = re.sub(
                f'src="[^>]+"(?=[^>]+alt="{entry_id[0]}")', f"src={account_name}/{saas_xsoar8_xsiam_prefix}entry/download/{entry_id[1]}", email_html
            )
        ...
        # special validation that the right src was being replaced.
            ...
            email_html = re.sub(
                '(src="cid(.*?"))',
                f"src={account_name}/{saas_xsoar8_xsiam_prefix}entry/download/{entry_id[1]}",
                email_html,
                count=1,
            )
    return email_html
...
def main(args):
    ...
    if not email_html_image or 'src="cid' in email_html_image:
        if 'src="cid' in email_html:
            ...
            return_results({"ContentsFormat": formats["html"], "Type": entryTypes["note"], "Contents": email_reply})

    else:
        return_results({"ContentsFormat": formats["html"], "Type": entryTypes["note"], "Contents": email_html_image})
