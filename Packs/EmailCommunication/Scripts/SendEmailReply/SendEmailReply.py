...
def validate_email_sent(incident_id, email_subject, email_to, reply_body, service_mail, email_cc, reply_html_body,
    ...
    """
    Validate that the email was actually sent, raises an error if it wasn't sent successfully.
    ...
    Raises:
        DemistoException: in case the execution of the reply-mail command has failed.

    Returns:
        str: a message which indicates that the mail was sent successfully.
    """
    ...
    if is_error(email_reply):
        error = get_error(email_reply)
        raise DemistoException(f'Error:\n {error}')
