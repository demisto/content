...
def append_email_signature(html_body):
    ...
    email_signature = execute_command("getList", {"listName": "XSOAR - Email Communication Signature"}, extract_contents=False)
...
def execute_reply_mail(
    ...
    return execute_command("reply-mail", mail_content, extract_contents=False)
...
def get_email_threads(incident_id):
    ...
    incident_context = execute_command("getContext", {"id": incident_id}, extract_contents=False)
...
def send_new_mail_request(
    ...
    email_result = execute_command("send-mail", mail_content, extract_contents=False)
...
def get_reply_body(notes, incident_id, attachments, reputation_calc_async=False):
    ...
    if notes:
        for note in notes:
            ...
            note_userdata = execute_command("getUserByUsername", {"username": note_user}, extract_contents=False)
...
def resend_first_contact(
    ...
    if str(email_selected_thread) == str(email_thread["EmailCommsThreadNumber"]):
        ...
        return_error(
            f"The selected Thread Number to respond to ({email_selected_thread}) "
...
def convert_internal_url_to_base64(match):
    ...
    result = execute_command("core-api-download", {"uri": original_src}, extract_contents=False)
...
def collect_thread_details(incident_email_threads, email_selected_thread):
    ...
    return (
        ...
        thread_cc,
        thread_bcc,
        last_thread_processed,
    )


def multi_thread_reply(
    new_email_body,
    body_type,
    incident_id,
    email_selected_thread,
    ...
    try:
        ...
        if isinstance(incident_email_threads, dict):
            ...
            (
                ...
                thread_bcc,
                last_thread_processed,
            ) = collect_thread_details(incident_email_threads, email_selected_thread)
            ...
            if outbound_only is True:
                ...
                result = resend_first_contact(
                    email_selected_thread,
                    incident_email_threads[last_thread_processed],
                    incident_id,
                    new_email_attachments,
                    files,
                    new_email_body,
...
def main():
    try:
        ...
        notes = execute_command("getEntries", {"filter": {"categories": ["notes"]}}, extract_contents=False)
        ...
        return_error(f"Failed to execute SendEmailReply. Error: {ex!s}")
