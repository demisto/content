...
@pytest.mark.parametrize("email_to, email_from, service_mail, mailbox, excepted", GET_EMAIL_RECIPIENTS)
...
def test_get_query_window(list_response, expected_result, mocker):
    ...
    if expected_result == "success":
        ...
        assert (
            debug_mocker_call_args.args[0] == "Error occurred while trying to load the `XSOAR - Email "
...
def test_create_thread_context(email_code, email_threads, scenario, mocker):
    ...
    if scenario == "thread_found":
        ...
        expected = {
            "EmailCommsThreadId": "123",
            "EmailCommsThreadNumber": "2",
            "EmailCC": "cc_user@company.com",
            "EmailBCC": "bcc_user@company.com",
            "EmailBody": "Email body.",
            "EmailFrom": "soc_sender@company.com",
...
def test_send_new_email(test_args, expected_result, expected_message, mocker):
    ...
    send_new_mail_request_mocker = mocker.patch.object(
        SendEmailReply, "send_new_mail_request", return_value=[{"Contents": "Success", "Type": 1}]
    )
...
def test_resend_first_contact(email_selected_thread, email_thread, expected_result, mocker):
    ...
    mocker.patch.object(
        SendEmailReply,
        "format_body",
        return_value=("<html><body>Resending email.</body></html>", "<html><body>Resending email.</body></html>"),
    )
    ...
    result = resend_first_contact(
        email_selected_thread,
        email_thread,
        1,
        "",
        "",
    ...
    if expected_result == "success":
        ...
        assert send_new_email_args.args == (
            1,
            "<69433507> Test Email 2",
            False,
            "end_user@company.com",
            "Resending email.",
            "soc_sender@company.com",
...
@pytest.mark.parametrize("email_code", ["12345678", ""])
def test_single_thread_reply(email_code, mocker):
    ...
    execute_command_call_args = execute_command_mocker.call_args
    assert validate_mail_sent_call_args.args == (
        1,
        "Email Subject",
        False,
        "end_user@company.com",
...
def test_multi_thread_new(scenario, mocker):
    ...
    if scenario == "required_fields_missing":
        # Test Scenario 1
        expected = (
            "The following required fields have not been set. Please set them and try again. "
...
def test_collect_thread_details():
    ...
    expected = (
        True,
        "AAMkAGRcOGZlZTEzLTkyZGDtNGJkNy1iOWMxLYM0NTAwODZhZjlxNABGAAAAAAAP2ksrJ8icRL4Zhadm7iVXBwAkkBJXBb0"
        "sRJWC0zdXEMqsAAAAAAEMAAAkkBJFBb0fRJWC0zdXEMqsABApcWVYAAA=",
        False,
        "87692312",
        "Re: <87692312> Test Email 4",
        "end_user@company.com, soc_sender@company.com",
        "soc_sender@company.com",
        "",
        "",
        3,
...
@pytest.mark.parametrize("scenario", ["single_outbound", "multiple_outbound", "replies_present"])
def test_multi_thread_reply(scenario, mocker):
    ...
    mocker.patch.object(
        SendEmailReply,
        "format_body",
        return_value=("<html><body>Email body</body></html>", "<html><body>Email body</body></html>"),
    )
    mocker.patch.object(SendEmailReply, "append_email_signature", return_value="<html><body>Email body+signature</body></html>")
    reset_fields_mocker = mocker.patch.object(SendEmailReply, "reset_fields", return_value=True)
    validate_email_sent_mocker = mocker.patch.object(SendEmailReply, "validate_email_sent", return_value=True)
    ...
    if scenario == "single_outbound":
        ...
        # Return all email thread entries
        mocker.patch.object(SendEmailReply, "get_email_threads", return_value=email_threads)
        validate_email_sent_expected = (
            1,
            "<87692312> Test Email 4",
            False,
            "end_user@company.com",
            "Email body",
            "html",
...
@pytest.mark.parametrize("new_thread", ["n/a", "true", "false"])
...
@pytest.mark.parametrize(
    "input_md, expected_html, test_id",
    [
        ...
        ("_italic_ **bold**", "<p><em>italic</em> <strong>bold</strong></p>", "emphasis_conversion"),
        # Test ID: #5 - List conversion
        ("- Item 1\n- Item 2", "<ul>\n<li>Item 1</li>\n<li>Item 2</li>\n</ul>", "list_conversion"),
        # Test ID: #6 - New lines to <br> conversion
        ("Line 1\nLine 2", "<p>Line 1<br />\nLine 2</p>", "newline_to_br_conversion"),
    ],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("# $%^&*()", ("<h1>$%^&amp;*()</h1>", "<h1>$%^&amp;*()</h1>"), "special_characters_only"),
    ],
    ids=lambda test_id: test_id,
)
...
def test_format_body(mocker):
    ...
    expected_result = (
        '<p><img alt="image" src="/markdown/image/aljhgfghdjakldvygi" /></p>',
