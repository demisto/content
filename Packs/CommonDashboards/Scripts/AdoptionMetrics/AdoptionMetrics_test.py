...
def test_check_phishing_incidents(mocker, data, expected_result):
    ...
    mocker.patch.object(demisto, "executeCommand", return_value=[{"Contents": {"data": data}}])
...
def test_get_use_cases(mocker):
    ...
    res = get_use_cases()
    assert res == {
        "use_cases_in_production": {"Case Management", "Network Security"},
        "at_risk": {
            "Ransomware & Malware Coverage": f"{link}?useCase=Malware",
