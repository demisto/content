...
def test_ipinfo_ip_command(mocker):
    ...
    client = Client(
        api_key="", base_url="https://ipinfo.io", verify_certificate=False, proxy=False, reliability=DBotScoreReliability.C
    )
...
def test_ipinfo_nultiple_ips_command(mocker, requests_mock):
    ...
    assert command_results.call_args_list[3][1].get("outputs").get("Address") == "8.8.8.1"
