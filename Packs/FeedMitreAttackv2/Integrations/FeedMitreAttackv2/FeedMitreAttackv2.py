...
class Client:
    ...
    def build_iterator(
        self, create_relationships=False, is_up_to_6_2=True, limit: int = -1
...
def add_obj_to_mitre_id_to_mitre_name(mitre_id_to_mitre_name, mitre_item_json) -> None:
    if mitre_item_json["type"] in ("attack-pattern", "x-mitre-tactic"):
        mitre_id = [
            ...
            for external in mitre_item_json.get("external_references", [])
...
def add_technique_prefix_to_sub_technique(indicators, id_to_name, mitre_id_to_mitre_name) -> None:
    for indicator in indicators:
        if (
            indicator["type"] in ["Attack Pattern", "STIX Attack Pattern"] and len(indicator["fields"]["mitreid"]) > 5
        ):  # Txxxx.xxx is sub technique
...
def map_fields_by_type(indicator_type: str, indicator_json: dict) -> dict[str, Any]:
    ...
    mitre_id = [
        external.get("external_id")
        for external in indicator_json.get("external_references", [])
        if external.get("source_name", "") == "mitre-attack"
    ]
    ...
    mapping_by_type = {
        "Attack Pattern": {"killchainphases": kill_chain_phases, "operatingsystemrefs": indicator_json.get("x_mitre_platforms")},
        "Intrusion Set": {"aliases": indicator_json.get("aliases")},
        "Threat Actor": {"aliases": indicator_json.get("aliases")},
        "Malware": {
        ...
        "Campaign": {"aliases": indicator_json.get("aliases")},
    }
...
def remove_citations(description: str) -> str:
    ...
    """
    return "".join(substring for substring in re.findall(r"\([^)]*\)|[^()]+", description) if "Citation" not in substring)
...
def create_relationship(item_json, id_to_name):
    ...
    if entity_b and entity_a:
        return EntityRelationship(
            name=item_json.get("relationship_type"),
            entity_a=entity_a,
...
def fetch_indicators(client, create_relationships):
    ...
    if create_relationships and mitre_relationships_list:
        dummy_indicator_for_relations = {"value": "$$DummyIndicator$$", "relationships": relationships}
...
def build_command_result(value, score, md, attack_obj):
    dbot_score = Common.DBotScore(
        indicator=value, indicator_type=DBotScoreType.ATTACKPATTERN, score=score, integration_name="MITRE ATT&CK v2"
    )
...
def attack_pattern_reputation_command(client, args):
    ...
    for name in mitre_names:
        ...
        if ":" not in name:  # not sub-technique
            ...
            attack_pattern = list(
                filter(
                    lambda attack_pattern_obj: filter_attack_pattern_object_by_attack_id(mitre_id, attack_pattern_obj), mitre_data
                )
            )
...
def get_attack_pattern_by_name(mitre_data, name):
    ...
    """
    attack_pattern = [attack_pattern_obj for attack_pattern_obj in mitre_data if (attack_pattern_obj.get("name") == name)]
...
def get_mitre_value_from_id(client, args):
    ...
    if attack_pattern_objects:
        for attack_id in attack_ids:
            attack_pattern = list(
                filter(
                    lambda attack_pattern_obj: filter_attack_pattern_object_by_attack_id(attack_id, attack_pattern_obj),
            ...
            if attack_pattern_name and "." in attack_id:  # sub-technique (e.g. T1059.001)
                ...
                parent_object = list(
                    filter(
                        lambda attack_pattern_obj: filter_attack_pattern_object_by_attack_id(
                            sub_technique_attack_id, attack_pattern_obj
                        ),
                        attack_pattern_objects,
                    )
                )
                if parent_object:
                    ...
                    demisto.debug(f"MA: Did not found parent attack pattern for sub-technique ID {attack_id}")
