register_module_line('MITRE ATT&CK v2', 'start', __line__())
import logging
import json
import urllib3
...
def attack_pattern_reputation_command(client, args, collection_name):
    ...
    if not command_results:
        return CommandResults(readable_output=f'MITRE ATTACK Attack Patterns values: '
                                              f'No Attack Patterns found for {mitre_names} in the {collection_name} collection.')
...
def get_mitre_value_from_id(client, args, collection_name):
    ...
    return CommandResults(readable_output=f'MITRE ATTACK Attack Patterns values: '
                                          f'No Attack Patterns found for {attack_ids} in the {collection_name} collection.')
...
def main():
    ...
    try:
        ...
        if demisto.command() == 'mitre-get-indicators':
            ...
            return_results(get_mitre_value_from_id(client, args, collection_name))

        elif demisto.command() == 'attack-pattern':
            return_results(attack_pattern_reputation_command(client, args, collection_name))
...
register_module_line('MITRE ATT&CK v2', 'end', __line__())
