...
from test_data.mitre_test_data import (
    ATTACK_PATTERN,
    ATTACK_PATTERNS,
    CAMPAIGN,
    COURSE_OF_ACTION,
...
@pytest.mark.parametrize(
    "indicator, expected_result", [({"x_mitre_deprecated": True}, True), ({"revoked": True}, True), ({}, False)]
)
...
@pytest.mark.parametrize(
    "indicator_type, indicator_json, expected_result",
    [
...
def test_publication_link_not_none():
    ...
    indicator = {"created": "2022-01-05T14:27:46.612705Z", "modified": "2022-01-05T14:27:46.612705Z", "external_references": [{}]}
...
def test_create_relationships_invalid():
    ...
    item_json = {"source_ref": "", "target_ref": ""}
...
def test_create_relationship_with_unknown_relationship_name():
    ...
    item_json = {"source_ref--source_ref": "source_ref", "target_ref--target_ref": "target_ref"}
    output = create_relationship(RELATIONSHIP_ENTITY, item_json)
    assert output is not None


@pytest.mark.parametrize(
    "attack_id, attack_pattern_obj, expected_result",
    [
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("Insikt Group (Recorded Future). (2017, May 17).", "2017-05-17T00:00:00"),
        ("Insikt Group (Recorded Future). (2017, May17).", "2017-05-17T00:00:00"),
        ("Insikt Group (Recorded Future). (2017,May17).", "2017-05-17T00:00:00"),
        ("Insikt Group (Recorded Future). (2017,March17).", "2017-03-17T00:00:00"),
        ("Insikt Group (Recorded Future). (2017, March 17).", "2017-03-17T00:00:00"),
    ],
)
...
def test_attack_pattern_reputation_without_answer_command(mocker):
    ...
    assert (
        command_results.readable_output == "MITRE ATTACK Attack Patterns values: No Attack "
...
def test_get_mitre_value_from_id_without_answer_command(mocker):
    ...
    assert command_results
    assert (
        command_results.readable_output == "MITRE ATTACK Attack Patterns values: "
...
def test_show_feeds_command(mocker):
    ...
    """
    from FeedMitreAttackv2 import Client, show_feeds_command
    ...
    assert demisto.results.call_args[0][0] == {
        "Type": 1,
        "Contents": [{"Name": "default", "ID": "101010101010101010101010101010101"}, {"Name": "not_default", "ID": 2}],
        "ContentsFormat": "json",
