...
def test_search_events_retrieves_all_available_events_when_limit_is_higher_than_number_of_available_events(mocker):
    ...
    mocker.patch.object(client, "get_events", side_effect=[response, {"AuditEvents": []}, {}])
...
def test_search_events_retrieves_when_limit_is_lower_than_results(mocker):
    ...
    mocker.patch.object(
        ...
        return_value={
            "AuditEvents": [
                {"event_id": 1, "Timestamp": "2022-01-01T00:00:00Z"},
...
def test_search_events_retrieves_no_events_when_time_parameters_have_no_events(mocker):
    ...
    mock_http_request = mocker.patch.object(
        TeamViewerEventCollector.Client, "get_events", return_value={"AuditEvents": [], "ContinuationToken": None}
    )
    client = TeamViewerEventCollector.Client(base_url="https://test.com", verify=False, proxy=False, headers={})
    results = TeamViewerEventCollector.search_events(
        client=client, limit=10, body={"StartTimeUtc": "2022-01-01T00:00:00Z", "EndTimeUtc": "2022-01-02T00:00:00Z"}
    )
...
def test_fetch_events_command(mocker):
    ...
    http_request_mock = mocker.patch.object(
        TeamViewerEventCollector.Client, "get_events", return_value={"AuditEvents": expected_events}
    )
...
def test_main_function(mocker):
    ...
    from TeamViewerEventCollector import Client, main
    ...
    demisto_results_mocker = mocker.patch.object(
        TeamViewerEventCollector,
        "fetch_events_command",
        return_value=(
            {"last_fetch": "2023-04-16T10:46:49Z"},
            [{"id": 1, "Timestamp": "2023-01-01T01:00:00Z"}, {"id": 2, "Timestamp": "2023-04-16T10:46:49Z"}],
        ),
    )
    main()
