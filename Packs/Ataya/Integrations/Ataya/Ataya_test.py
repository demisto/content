...
MOCK_ASSIGN_USER_RESPONSE = {"status": "assigned", "resources": [MOCK_IMSI]}

MOCK_UNASSIGN_USER_RESPONSE = {"status": "unassigned", "resources": [MOCK_IMSI]}

MOCK_GET_NODE_RESPONSE = {"count": 1}


def test_assign_command(requests_mock):
    requests_mock.put(f"{MOCK_URL}/api/v1/mgmt/5gc/clientAction/setstatus", json=MOCK_ASSIGN_USER_RESPONSE)
    client = Client(api_key="123456789", base_url=MOCK_URL, proxy=False, verify=False)
...
def test_assign_command_assign_fail(requests_mock):
    requests_mock.put(f"{MOCK_URL}/api/v1/mgmt/5gc/clientAction/setstatus", json=MOCK_UNASSIGN_USER_RESPONSE)
    client = Client(api_key="123456789", base_url=MOCK_URL, proxy=False, verify=False)
...
def test_get_node_api(requests_mock):
    requests_mock.get(f"{MOCK_URL}/api/v1/mgmt/5gc/networks/default/nodes", json=MOCK_GET_NODE_RESPONSE)
    client = Client(api_key="123456789", base_url=MOCK_URL, proxy=False, verify=False)
