...
def get_value_from_list(org_type, classification):
    if org_type == 'pt_org':
        if classification:
            result = [enisa_value['Type'] for enisa_value in pt_enisa if enisa_value['Classification'] == classification]

        else:
            result = [enisa_value['Classification'] for enisa_value in pt_enisa]
            result = list(set(result))

    elif org_type == 'non_pt_org':
        if classification:
            result = [enisa_value['ENISA'].split(":")[1] for enisa_value in pt_enisa if classification in enisa_value['ENISA']]
        else:
            result = [enisa_value['ENISA'].split(":")[0] for enisa_value in pt_enisa]
            result = list(set(result))
...
def main():
    ...
    try:
        ...
        # Check which action should be the fetch: First Part of Classification vs Second Part Based On Input

        classification = demisto.args().get('classification', None)
        result = get_value_from_list(org_type=org_type, classification=classification)

        markdown = tableToMarkdown('Incident Classification', result, headers=['Classification'])

        command_result = CommandResults(
            outputs_prefix='IncidentClassification',
