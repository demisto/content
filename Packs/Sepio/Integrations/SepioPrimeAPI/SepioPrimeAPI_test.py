...
def test_sepio_query_agents(requests_mock):
    ...
    requests_mock.post(f"{BASE_URL}/auth/signin", json=mock_signin_http_response)

    mock_agents_http_response = util_load_json("test_data/agents_http_response.json")
    requests_mock.get(f"{BASE_URL}/agents", json=mock_agents_http_response)

    mock_agents_query_test = util_load_json("test_data/agents_query_test.json")

    client = Client(base_url=f"{BASE_URL}", verify=False, headers={"Authentication": TOKEN}, auth=(USERNAME, PASSWORD))
...
def test_sepio_query_global_peripherals(requests_mock):
    ...
    requests_mock.get(f"{BASE_URL}/peripherals", json=mock_global_peripherals_http_response)
...
def test_sepio_query_switches(requests_mock):
    ...
    requests_mock.get(f"{BASE_URL}/switches/switches", json=mock_switches_http_response)

    mock_switches_query_test = util_load_json("test_data/switches_query_test.json")

    client = Client(base_url=f"{BASE_URL}", verify=False, headers={"Authentication": TOKEN}, auth=(USERNAME, PASSWORD))

    args = {"model": "WS-C2960", "is_alarmed": False, "limit": 10}
...
def test_sepio_query_switch_ports(requests_mock):
    ...
    requests_mock.get(f"{BASE_URL}/switches/ports", json=mock_ports_http_response)

    mock_ports_query_test = util_load_json("test_data/ports_query_test.json")

    client = Client(base_url=f"{BASE_URL}", verify=False, headers={"Authentication": TOKEN}, auth=(USERNAME, PASSWORD))

    args = {"switch_ip_address": "192.168.100.25", "switch_name": "sepio2960g", "is_alarmed": False, "limit": 30}
...
def test_sepio_query_system_events(requests_mock):
    ...
    requests_mock.get(f"{BASE_URL}/events/getevents", json=mock_events_http_response)
...
def test_sepio_set_agent_mode(requests_mock):
    ...
    requests_mock.post(f"{BASE_URL}/agents/configuration", status_code=200, json=mock_set_agent_mode_http_reponse)
...
def test_sepio_set_agent_peripherals_mode(requests_mock):
    ...
    requests_mock.post(f"{BASE_URL}/peripherals/command", status_code=200, json=mock_set_peripherals_mode_http_reponse)
...
def test_fetch_incidents(requests_mock):
    ...
    last_run = {"last_fetch": "2020-06-22T12:00:00Z"}

    categories = ["USB", "Network"]

    new_last_run, new_incidents = fetch_incidents(
        client=client, last_run=last_run, first_fetch_time="1 Day", min_serverity="Warning", categories=categories, max_results=20
    )

    mock_new_last_run = {"last_fetch": "2020-06-22T17:27:06Z", "last_fetch_eventid": 406}
