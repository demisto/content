...
class Client(BaseClient):
    ...
    def create_indicators_from_response(self, feed_type, response, feedTags):
        ...
        for indicator in response:
            if indicator:
                ...
                parsed_indicators.append({
                    ...
                    'fields': {'service': feed_type, 'tags': feedTags}
                })
    ...
    def build_iterator(self, feedTags: List, limit=None, offset=None):
        ...
        for service in ["Daily Threat Feed", "Custom Feed"]:
            if service in self.indicator_feeds:
                ...
                parsed_indicators.extend(self.create_indicators_from_response(service, response, feedTags))
...
def module_test_command(client: Client, args: dict, feedTags: list):
    ...
    if 'Custom Feed' in indicator_feeds:
        ...
        for url in url_list:
            ...
            try:
                client.build_iterator(feedTags, 1, 0)
            except Exception:
