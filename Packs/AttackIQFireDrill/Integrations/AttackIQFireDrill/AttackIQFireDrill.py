...
SERVER = (
    demisto.params().get("url")[:-1]
    if ("url" in demisto.params() and demisto.params()["url"].endswith("/"))
...
def http_request(method, url_suffix, params=None, data=None):
    ...
    LOG(
        f"AttackIQ is attempting {method} request sent to {url} with params:\n{json.dumps(params, indent=4)} \n "
        f'data:\n"{json.dumps(data)}'
    )
    try:
        ...
        try:
            ...
            return_error(
                "Response contained no valid body. See logs for more information.",
...
def get_assessment_execution_status_command():
    ...
    try:
        ...
        ec = {"AttackIQ.Assessment(val.Id === obj.Id)": {"Running": ex_status, "Id": ass_id}}
...
def get_test_results(page, page_size, test_id, show_last_res):
    ...
    """
    params = {"page": page, "page_size": page_size, "test_id": test_id, "show_last_result": show_last_res}
...
def list_assessments_command():
    ...
    hr = tableToMarkdown(
        f"AttackIQ Assessments Page {page}/{total_pages}",
        assessments_res,
...
def get_assessment_by_id_command():
    ...
    try:
        ...
        hr = tableToMarkdown(
            f"AttackIQ Assessment {assessment_id}",
            assessments_res,
            headers=["Id", "Name", "Description", "User", "Created", "Modified"],
        )
...
def list_tests_by_assessment_command():
    ...
    page, page_size = get_page_number_and_page_size(demisto.args())
    ass_id = demisto.getArg("assessment_id")
    params = {"project": ass_id, "page_size": page_size, "page": page}
...
def list_templates_command():
    ...
    ec = {"AttackIQ.Template(val.ID && val.ID === obj.ID)": templates}
...
def list_assets_command():
    ...
    for asset in res.get("results", []):
        ...
        for group in asset.get("asset_groups", []):
            temp_group = {"ID": group.get("id"), "Name": group.get("name")}
            groups.append(temp_group)
        asset_dict["Groups"] = groups
        assets.append(asset_dict)
    ec = {"AttackIQ.Asset(val.ID && val.ID === obj.ID)": assets}
    hr = tableToMarkdown("Assets:", assets, ["ID", "Hostname", "IPv4", "MacAddress", "Domain", "Description", "User", "Status"])
...
def create_assessment_command():
    ...
    """
    body = {"project_name": demisto.args().get("name"), "template": demisto.args().get("template_id")}
    ...
    assessments_res = build_transformed_dict(raw_assessments, ASSESSMENTS_TRANS)
    hr = tableToMarkdown(
        f"Created Assessment: {assessment_id} successfully.",
        assessments_res,
        headers=["Id", "Name", "Description", "User", "Created", "Modified"],
    )
    return_outputs(hr, {"AttackIQ.Assessment(val.Id === obj.Id)": assessments_res}, raw_assessments)
