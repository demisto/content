...
2. For using OAuth 2.0 see the above instructions. The OTP code should be appended to the password parameter in the `!servicenow-oauth-login` command.
    ...
    | Mirrored XSOAR Ticket custom close state code | Define how to close the mirrored tickets in XSOAR with a custom state. Enter here a comma-separated list of custom closure state codes and their labels (Acceptable Format Example: “10=Design,11=Development,12=Testing”) to override the default closure method. Note that a matching user-defined list of custom close reasons must be configured as a "Server configuration" in XSOAR. Not following this format will result in closing the incident with default "Other" close reason. | False |
