...
2. For using OAuth 2.0 see the above instructions. The OTP code should be appended to the password parameter in the `!servicenow-oauth-login` command.
    ...
    | Mirror Existing Notes For New Fetched Incidents | Select this checkbox to retrieve existing comments and work notes for new fetched incidents. Enabling this involves adding API calls on the machine to execute, which may cause overload in case of many incidents. | False |
