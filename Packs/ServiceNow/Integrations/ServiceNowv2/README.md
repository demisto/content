...
4. To ensure that mirroring works:
   ...
   5. To enable mirroring to close a ticket in Cortex XSOAR, under the **Mirrored XSOAR Ticket closure method** dropdown, select the ticket closing method,
      ...
      In order to use **Mirrored XSOAR Ticket custom close state code** parameter, it must follow this format example: “10=Design,11=Development,12=Testing”.
      Also a matching user-defined list of customized incident close reasons must be configured as a "Server configuration" in XSOAR. Not following this format will result in a server error.
...
2. For using OAuth 2.0 see the above instructions. The OTP code should be appended to the password parameter in the `!servicenow-oauth-login` command.
    ...
    | Mirrored XSOAR Ticket closure method | Define how to close the mirrored tickets in XSOAR, choose 'resolved' to enable reopening from the UI. Otherwise, choose 'closed'. | False |
    | Mirrored XSOAR Ticket custom close state code | Define how to close the mirrored tickets in XSOAR with a custom state. Enter here the custom closure state codes and their labels (Format: “10=Design,11=Development,12=Testing”) to override the default closure method. Note that a matching user-defined list of close reasons must be configured as a "Server configuration" in XSOAR. Not following this format will result in a server error.| False |
