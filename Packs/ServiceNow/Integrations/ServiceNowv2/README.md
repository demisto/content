...
5. If you did **not** provide `Username` and `Password` in step 3, run the command `!servicenow-oauth-login` from the XSOAR CLI and fill in the ServiceNow user's User ID (admin or a dedicated integration username) and corresponding password of the ServiceNow instance. This step generates an access token to the ServiceNow instance and is required only the first time after configuring a new instance in the XSOAR platform. If you provided `Username` and `Password`, this step is handled automatically.
