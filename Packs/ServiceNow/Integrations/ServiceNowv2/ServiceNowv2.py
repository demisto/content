    ...
    for arg in SNOW_ARGS:
        ...
        if arg in fields_to_clear:
            ...
            if arg in ['impact', 'urgency', 'severity']:
                ...
                demisto.debug(f"before: input_arg: {input_arg}")
                ticket_fields[arg] = inv_states.get(input_arg, input_arg)
                demisto.debug(f" after: {ticket_fields[arg]}")
            elif arg == 'approval':
...
def update_remote_system_command(client: Client, args: dict[str, Any], params: dict[str, Any]) -> str:
    ...
    demisto.debug(f"state will change to= {parsed_args.data['state']}")
    if parsed_args.incident_changed:
        ...
        if closure_case:
            ...
            if parsed_args.data['state'] == '7 - Closed' and not is_custom_close:
