...
def create_ticket_context(data: dict, additional_fields: list = None) -> Any:
    ...
    if priority:
        if isinstance(priority, dict):
            context["Priority"] = (
                TICKET_PRIORITY.get(str(int(priority.get("value", ""))), str(int(priority.get("value", "")))),
            )
        else:
...
def get_ticket_fields(args: dict, template_name: dict = {}, ticket_type: str = "") -> dict:
    ...
    fields_to_clear = argToList(
        args.get("clear_fields", [])
    )  # This argument will contain fields to allow their value empty
...
class Client(BaseClient):
    ...
    def get_ticket_attachment_entries(self, ticket_id: str, sys_created_on: Optional[str] = None) -> list:
        ...
        if "result" in attachments_res and len(attachments_res["result"]) > 0:
            ...
            links = [
                (attachment.get("download_link", ""), attachment.get("file_name", "")) for attachment in attachments
            ]
...
def create_ticket_command(client: Client, args: dict) -> Tuple[str, Dict, Dict, bool]:
    ...
    human_readable = tableToMarkdown(
        "ServiceNow ticket was created successfully.", t=hr_, headers=headers, removeNull=True
    )
...
def add_link_command(client: Client, args: dict) -> Tuple[str, Dict, Dict, bool]:
    ...
    human_readable = tableToMarkdown(
        "Link successfully added to ServiceNow ticket", t=hr_, headers=headers, removeNull=True
    )
...
def add_comment_command(client: Client, args: dict) -> Tuple[str, Dict, Dict, bool]:
    ...
    hr_ = get_ticket_human_readable(result["result"], ticket_type)
    human_readable = tableToMarkdown(
        "Comment successfully added to ServiceNow ticket", t=hr_, headers=headers, removeNull=True
    )
...
def query_table_command(client: Client, args: dict) -> Tuple[str, Dict, Dict, bool]:
    ...
    result = client.query(
        table_name, sys_param_limit, sys_param_offset, sys_param_query, system_params, sysparm_fields=fields
    )
    if not result or "result" not in result or len(result["result"]) == 0:
    ...
    if fields:
        ...
        mapped_records = [
            {DEFAULT_RECORD_FIELDS[k]: r[k] for k in DEFAULT_RECORD_FIELDS if k in r} for r in table_entries
        ]
    ...
    mapped_groups = [
        {
            ...
            if isinstance(group.get("manager"), dict)
            else group.get("manager"),
...
def create_order_item_command(client: Client, args: dict) -> Tuple[Any, Dict[Any, Any], Dict[Any, Any], bool]:
    ...
    human_readable = tableToMarkdown(
        "ServiceNow Order Request", mapped_item, removeNull=True, headerTransform=pascalToSpace
    )
...
def get_remote_data_command(client: Client, args: Dict[str, Any], params: Dict) -> Union[List[Dict[str, Any]], str]:
    ...
    sys_param_query = (
        f"element_id={ticket_id}^sys_created_on>"
...
def update_remote_system_command(client: Client, args: Dict[str, Any], params: Dict[str, Any]) -> str:
    ...
    if parsed_args.incident_changed:
        ...
        if (
            (ticket_type == "sc_task" or ticket_type == "sc_req_item")
            and parsed_args.inc_status == IncidentStatus.DONE
            and params.get("close_ticket")
        ):
...
def get_tasks_for_co_command(client: Client, args: dict) -> CommandResults:
    ...
    if not result or "result" not in result:
        return CommandResults(
            outputs_prefix="ServiceNow.Tasks", readable_output="Item was not found.", raw_response=result
        )
    items = result.get("result", {})
    if not isinstance(items, list):
        items_list = [items]
    ...
    if len(items_list) == 0:
        return CommandResults(
            outputs_prefix="ServiceNow.Tasks", readable_output="No items were found.", raw_response=result
        )
...
def generic_api_call_command(client: Client, args: Dict) -> Tuple[Any, Dict[Any, Any], Dict[Any, Any]]:
    ...
    response = client.generic_request(
        method=method, path=path, body=body, headers=headers, sc_api=sc_api, cr_api=cr_api
    )
...
class ServiceNowClient(BaseClient):
    def __init__(
        ...
        if (
            self.use_oauth
        ):  # if user selected the `Use OAuth` box use OAuth authorization, else use basic authorization
        ...
        try:
            ...
            if res.status_code in [200, 201]:
                try:
                    ...
                    raise DemistoException(
                        "Failed to parse json object from response: {}".format(res.content), exception
                    )
            ...
            if res.status_code in [401]:
                if self.use_oauth:
                    ...
                    raise Exception(
                        f"Authorization failed. Please verify that the username and password are correct." f"\n{res}"
                    )
    ...
    def login(self, username: str, password: str):
        ...
        try:
            ...
            res = super()._http_request(
                method="POST", url_suffix=OAUTH_URL, resp_type="response", headers=headers, data=data
            )
            try:
                res = res.json()
            except ValueError as exception:
    ...
    def get_access_token(self):
        ...
        if previous_token.get("access_token") and previous_token.get("expiry_time") > date_to_timestamp(
            datetime.now()
        ):
            ...
            if previous_token.get("refresh_token"):
                ...
                raise Exception(
                    "Could not create an access token. User might be not logged in. Try running the"
