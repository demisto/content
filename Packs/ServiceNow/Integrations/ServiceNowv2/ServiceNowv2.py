...
def fetch_incidents(client: Client) -> list:
    ...
    for result in res.get("result", []):
        ...
        labels = [{"type": k, "value": v if isinstance(v, str) else json.dumps(v)} for k, v in result.items()]
...
def get_remote_data_command(client: Client, args: Dict[str, Any], params: Dict) -> Union[List[Dict[str, Any]], str]:
    ...
    sys_param_query = (
        f"element_id={ticket_id}^sys_created_on>" f"{datetime.fromtimestamp(last_update)}^element=comments^ORelement=work_notes"
    )
    ...
    if ticket.get("resolved_by") or ticket.get("closed_at"):
        if params.get("close_incident"):
            ...
            entries.append(
                {
                    "Type": EntryType.NOTE,
                    "Contents": {"dbotIncidentClose": True, "closeReason": f'From ServiceNow: {ticket.get("close_notes")}'},
        ...
        for entry in entries:
            ...
            if entry.get("type") == 3:
                ...
                if not file_extension:
                    file_extension = ""
                client.upload_file(ticket_id, entry.get("id"), file_name + "_mirrored_from_xsoar" + file_extension, ticket_type)
            else:
...
def generic_api_call_command(client: Client, args: Dict):
