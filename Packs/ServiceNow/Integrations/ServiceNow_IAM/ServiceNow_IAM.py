...
class IAMCommand:
    ...
    def disable_user(self, client, args):
        ...
        if not self.is_disable_enabled:
            ...
            try:
                # username --> OktaUserName  mapper in
                # OktaUserName --> username mapper out
    ...
    def create_user(self, client, args):
        ...
        if not self.is_create_enabled:
            ...
            try:
                iam_attr, iam_attr_value = get_first_available_iam_user_attr(user_profile, self.get_user_iam_attrs)
                # identifier = user_profile.get_attribute(self.attr)
                user_app_data = client.get_user(iam_attr_value)
                if user_app_data:
                    # if user exists, update it
...
def get_first_available_iam_user_attr(user_profile: IAMUserProfile,
                                      iam_attrs: List[IAMAttribute]) -> Tuple[Optional[IAMAttribute], Optional[str]]:
    """
    ...
    Returns:
        (Tuple[Optional[IAMAttribute], Optional[str]]): First value of given attribute if exists with the given
                                                        attribute. None if no attribute data exists in user profile.
    """
