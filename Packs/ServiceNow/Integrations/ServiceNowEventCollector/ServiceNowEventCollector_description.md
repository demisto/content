...
Note: If you're using OAuth 2.0 and encounter access_denied with access token related errors, you can use the !service-now-reset-auth command to manually clear the expired refresh token from the integration context. Then the integration will generate new refresh token through the API automatically. 