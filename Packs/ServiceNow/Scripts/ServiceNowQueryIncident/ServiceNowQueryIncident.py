...
"""
INCIDENT_PRIORITY = {"1": "1 - Critical", "2": "2 - High", "3": "3 - Moderate", "4": "4 - Low", "5": "5 - Planning"}
...
def get_user(query):
    user_args = {"table_name": "sys_user", "query": query}
...
def main():
    ...
    """
    command_args = {"table_name": "incident", "fields": list(fields_to_map.keys())}
            ...
            if not isinstance(record_data, dict):
                ...
                mapped_records = [
                    {
                        ...
                        for (key, value) in [k_v for k_v in list(r.items()) if k_v[0] in list(fields_to_map.keys())]
                    }
                    for r in records
                ]
                ...
                result = {
                    ...
                    "HumanReadable": tableToMarkdown(
                        "ServiceNow Incidents", mapped_records, headers=display_headers, removeNull=True
                    ),
                    "EntryContext": {"ServiceNow.Incident(val.ID===obj.ID)": createContext(mapped_records)},
                }
