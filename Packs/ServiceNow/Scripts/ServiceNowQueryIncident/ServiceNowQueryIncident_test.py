...
def test_get_username(mocker):
    ...
    mocker.patch.object(demisto, "executeCommand")
    mocker.patch.object(demisto, "get", return_value={"result": [{"first_name": "first-name", "last_name": "last-name"}]})
...
def test_main_flow_success(mocker):
    ...
    assert result.call_args.args[0] == {
        ...
        "EntryContext": {
            "ServiceNow.Incident(val.ID===obj.ID)": [{"Number": "INC0021211", "ID": "123", "Priority": "1 - Critical"}]
        },
    }
