...
class crossproc_complete(ProcessEventDetail):
    FIELDS = [
        "cross-process_access_type",
        "event_time",
        "targeted_process_unique_id",
        "targeted_process_md5",
        "targeted_process_path",
        "ProcessOpen_sub-type",
        "requested_access_priviledges",
        "flagged_as_potential_tamper_attempt",
    ]
...
class Client(BaseClient):
    ...
    def get_sensors(
        ...
        id: str = None,
        hostname: str = None,
        ipaddr: str = None,  # noqa: F841
        groupid: str = None,
        inactive_filter_days: str = None,  # noqa: F841
        limit: int | str | None = None,
    ) -> tuple[int, list[dict]]:
        url = f"/v1/sensor/{id}" if id else "/v1/sensor"
        query_params = assign_params(ip=ipaddr, hostname=hostname, groupid=groupid, inactive_filter_days=inactive_filter_days)
    ...
    def get_alerts(
        ...
        query_params = assign_params(
            status=status, username=username, feedname=feedname, hostname=hostname, report=report, query=query
        )
    ...
    def get_binaries(
        ...
        signed: str = None,  # noqa: F841
        group: str = None,
        hostname: str = None,
        digsig_publisher: str = None,  # noqa: F841
    ...
    def get_formatted_ProcessEventDetail(self, process_json: dict):
        complex_fields = {
            "filemod_complete": filemod_complete,
            "modload_complete": modload_complete,
            "regmod_complete": regmod_complete,
            "crossproc_complete": crossproc_complete,
...
def _create_query_string(params: dict, allow_empty_params: bool = False) -> str:
    ...
    if "query" in params and len(params) > 1:
        raise DemistoException(
            f"{INTEGRATION_NAME} - Searching with both query and other filters is not allowed. "
...
def _get_sensor_isolation_change_body(client: Client, sensor_id: str, new_isolation: bool) -> dict:
    ...
    new_sensor_data = {"network_isolation_enabled": new_isolation, "group_id": sensor_data.get("group_id")}
...
def unquarantine_device_command(client: Client, sensor_id: str) -> CommandResults:
    ...
    res = client.http_request(url=url, method="PUT", json_data=_get_sensor_isolation_change_body(client, sensor_id, False))
    if not res:
...
def quarantine_device_command(client: Client, sensor_id: str) -> CommandResults:
    url = f"/v1/sensor/{sensor_id}"
    res = client.http_request(url=url, method="PUT", json_data=_get_sensor_isolation_change_body(client, sensor_id, True))
    if not res:
...
def sensors_list_command(
    client: Client,
    id: str = None,
    hostname: str = None,
    ip: str = None,
    group_id: str = None,
    inactive_filter_days: str = None,
    limit: int = None,
) -> CommandResults:
    try:
            ...
            human_readable_data.append(
                {
                    ...
                    "IP Address/MAC Info": _parse_field(
                        sensor_data.get("network_adapters", ""), index_after_split=1, chars_to_remove="|"
                    ),
        ...
        return CommandResults(
            outputs=res, outputs_prefix="CarbonBlackEDR.Sensor", outputs_key_field="id", readable_output=md, raw_response=res
        )
...
def watchlist_update_command(
    client: Client, id: str, search_query: str, description: str, enabled: bool = None
) -> CommandResults:
...
def watchlist_update_action_command(client: Client, id: str, action_type: str, enabled: str) -> CommandResults:
...
def watchlist_create_command(
    client: Client, name: str, search_query: str, index_type: str = "events", description: str = ""
) -> CommandResults:
    ...
    if watchlist_id:
        ...
        return CommandResults(
            outputs=output,
            outputs_prefix="CarbonBlackEDR.Watchlist",
...
def get_watchlist_list_command(client: Client, id: str = None, limit: str = None) -> CommandResults:
    ...
    return CommandResults(outputs=res, outputs_prefix="CarbonBlackEDR.Watchlist", outputs_key_field="name", readable_output=md)


def binary_ban_command(
    client: Client, md5: str, text: str, last_ban_time: str = None, ban_count: str = None, last_ban_host: str = None
) -> CommandResults:
    body = assign_params(md5hash=md5, text=text, last_ban_time=last_ban_time, ban_count=ban_count, last_ban_host=last_ban_host)
    try:
...
def binary_bans_list_command(client: Client, limit: str = None) -> CommandResults:
    ...
    return CommandResults(
        outputs=res,
        outputs_prefix="CarbonBlackEDR.BinaryBan",
...
def alert_update_command(
    client: Client, alert_ids: str, status: str = None, set_ignored: bool = None, query: str = None
) -> CommandResults:
    url = "/v1/alerts"
    body = assign_params(alert_ids=argToList(alert_ids), requested_status=status, set_ignored=set_ignored, query=query)
...
def alert_search_command(
    ...
    outputs = assign_params(
        Results=alerts, Facets=res.get("facets"), Terms=res.get("terms"), total_results=res.get("total_results")
    )
...
def binary_summary_command(client: Client, md5: str) -> CommandResults:
    ...
    try:
        ...
        if not res:
            return CommandResults(readable_output=f"Could not find data for file {md5}.")
        ...
        return CommandResults(
            outputs=res,
            outputs_prefix="CarbonBlackEDR.BinaryMetadata",
            outputs_key_field="md5",
            readable_output=tableToMarkdown(f"{INTEGRATION_NAME} -Summary For File {md5}", human_readable_data, removeNull=True),
        )
    except DemistoException as e:
        if "404" in e.message:
...
def binary_search_command(
    ...
    outputs = assign_params(
        Results=res.get("results"), Facets=res.get("facets"), Terms=res.get("terms"), total_results=res.get("total_results")
    )
    human_readable_data = []
    ...
    return CommandResults(
        outputs=outputs, outputs_prefix="CarbonBlackEDR.BinarySearch", outputs_key_field="md5", readable_output=md
    )
...
def process_events_list_command(client: Client, process_id: str, segment_id: str, start: str = None, count: str = None):
    ...
    return CommandResults(
        outputs=process, outputs_prefix="CarbonBlackEDR.Events", outputs_key_field="id", readable_output=process, raw_response=res
    )
...
def process_segments_get_command(client: Client, process_id: str, limit: str = "50") -> CommandResults:
    ...
    res = client.http_request(url=url, method="GET")
    if not res:
        return CommandResults(readable_output=f"Could not find segment data for process id {process_id}.")
    res = res.get("process", {}).get("segments")
    res = res[: arg_to_number(limit, "limit")] if limit else res
    # Human readable is depending on request therefore is not prettified.
    return CommandResults(
        outputs=res, outputs_prefix="CarbonBlackEDR.ProcessSegments", outputs_key_field="unique_id", readable_output=res
    )
...
def processes_search_command(
    ...
    res = client.get_processes(
        process_name, group, hostname, parent_name, process_path, md5, query, group_by, sort, facet, facet_field, limit, start
    )
    ...
    return CommandResults(
        outputs=outputs, outputs_prefix="CarbonBlackEDR.ProcessSearch", outputs_key_field="Terms", readable_output=md
    )
...
def endpoint_command(client: Client, id: str = None, ip: str = None, hostname: str = None):
    ...
    command_results = []
    for sensor in res:
        is_isolated = _get_isolation_status_field(sensor["network_isolation_enabled"], sensor["is_isolating"])
        ...
        command_results.append(CommandResults(readable_output=md, raw_response=res, indicator=endpoint))
    if exceptions:
...
def fetch_incidents(
    ...
    for alert in alerts:
        ...
        if last_fetch and (incident_created_time_ms <= last_fetch.timestamp()):
            demisto.debug(f"{INTEGRATION_NAME} - alert {str(alert)} was created at {incident_created_time_ms}." f" Skipping.")
            continue
...
def main() -> None:
    try:
        ...
        client = Client(base_url=base_url, use_ssl=verify_certificate, use_proxy=proxy, apitoken=api_token)
