...
QUERY_STRING_CASES = [
    (
        {"query": "chrome.exe"},
        False,  # case both query and params
        "chrome.exe",  # expected
    ),
    (
        {"hostname": "ec2amaz-l4c2okc"},
        False,  # case only params
...
QUERY_STRING_CASES_FAILS = [
    (
        {"hostname": "ec2amaz-l4c2okc", "query": "chrome.exe"},
        False,  # case both query and params
...
@pytest.mark.parametrize(
    "isolation_activated, is_isolated, expected",
        ...
        [
            {
                ...
                "operation_type": "Created the file",
            }
        ],  # expected
    )
]
        ...
        [
            {
                ...
                "loaded_module_md5": "f404e59db6a0f122ab26bf4f3e2fd0fa",
            }
        ],  # expected
    )
]
...
def test_fetch_incidents_first_fetch(mocker):
    ...
    """
    from CarbonBlackResponseV2 import Client, fetch_incidents
...
def test_quarantine_device_command_not_have_id(mocker):
    ...
    """
    from CarbonBlackResponseV2 import Client, _get_sensor_isolation_change_body
...
def test_get_sensor_isolation_change_body_compatible(mocker):
    ...
    client = Client(base_url="url", apitoken="api_key", use_ssl=True, use_proxy=False)
    mocker.patch.object(
        Client, "get_sensors", return_value=(1, [{"id": "some_id", "group_id": "some_group_id", "some_other_stuff": "some"}])
    )
    sensor_data = _get_sensor_isolation_change_body(client, 5, False)
...
def test_endpoint_command(mocker):
    ...
    """
    from CarbonBlackResponseV2 import Client, endpoint_command
...
def test_watchlist_update_action_command(mocker, requests_mock):
    from CarbonBlackResponseV2 import Client, watchlist_update_action_command
...
def test_remove_PREPREPRE_POSTPOSTPOST_tags(mocker):
    ...
    mocker.patch.object(Client, "get_alerts", return_value=alerts)
    first_fetch_time = "7 days"
    last_fetch, incidents = fetch_incidents(client, last_run=last_run, first_fetch_time=first_fetch_time, max_results="10")
