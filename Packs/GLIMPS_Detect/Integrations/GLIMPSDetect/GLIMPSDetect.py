...
from gdetect.api import Client as gClient
from gdetect.api import HTTPExceptions
from gdetect.api import logger as gLogger
...
def gdetect_send_command(client, args):  # TO TEST
    ...
    outputs = {"entryID": entry_id, "uuid": uuid}
...
def gdetect_get_all_command(client, args):  # TO TEST
    ...
    if "error" in response:
        ...
        results = CommandResults(
            outputs_prefix="GLIMPS.GDetect.All", outputs=response, raw_response=response, readable_output=readable_buffer
        )
        return results
    ...
    if "threats" in response:
        ...
        for sha256, threat in threats.items():
            ...
            readable_buffer += tableToMarkdown(
                f"Threat {sha256}",
                threat,
...
def gdetect_get_threats_command(client, args):  # TO TEST
    ...
    for sha256, threat in response.get("threats").items():
        tags = threat.get("tags")
        readable_output += tableToMarkdown(
            f"Threat {sha256}", threat, ["filenames", "score", "magic", "sha256", "sha1", "md5", "ssdeep", "file_size", "mime"]
        )
