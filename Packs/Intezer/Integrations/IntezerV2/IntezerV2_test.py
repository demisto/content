...
from IntezerV2 import (
    analyze_by_hash_command,
    analyze_by_uploaded_file_command,
    analyze_url_command,
    check_analysis_status_and_get_results_command,
    check_is_available,
    enrich_dbot_and_display_alert_results,
    get_alert_result_command,
    get_analysis_code_reuse_command,
    get_analysis_iocs_command,
    get_analysis_metadata_command,
    get_analysis_sub_analyses_command,
    get_endpoint_analysis_result_command,
    get_family_info_command,
    get_file_analysis_result_command,
    get_latest_result_command,
    get_url_analysis_result_command,
    submit_alert_command,
    submit_suspected_phishing_email_command,
)
...
def test_analyze_by_hash_command_success(requests_mock):
    ...
    requests_mock.post(
        f"{full_url}/analyze-by-hash", status_code=HTTPStatus.CREATED, json={"result_url": f"/analyses/{analysis_id}"}
    )
...
def test_analyze_by_hash_command_missing_hash(requests_mock):
    ...
    _setup_access_token(requests_mock)
    requests_mock.post(f"{full_url}/analyze-by-hash", status_code=HTTPStatus.NOT_FOUND)
...
def test_analyze_by_hash_command_already_running(requests_mock):
    # Arrange
    _setup_access_token(requests_mock)
    requests_mock.post(f"{full_url}/analyze-by-hash", json={}, status_code=HTTPStatus.CONFLICT)
...
def test_get_latest_result_command_success(requests_mock):
    ...
    requests_mock.get(
        ...
        json={"sub_analyses": [{"sha256": sha256, "source": "root", "sub_analysis_id": root_sub_analysis}]},
    )
...
def test_get_latest_result_command_file_missing(requests_mock):
    ...
    sha256 = "sha256"
    _setup_access_token(requests_mock)
    requests_mock.get(f"{full_url}/files/{sha256}", status_code=HTTPStatus.NOT_FOUND)
...
def test_analyze_by_uploaded_file_command_success(requests_mock, mocker):
    ...
    _setup_access_token(requests_mock)
    requests_mock.post(f"{full_url}/analyze", status_code=HTTPStatus.CREATED, json={"result_url": f"/analyses/{analysis_id}"})
...
def test_analyze_by_uploaded_file_command_analysis_already_running(requests_mock, mocker):
    ...
    _setup_access_token(requests_mock)
    requests_mock.post(f"{full_url}/analyze", json={}, status_code=HTTPStatus.CONFLICT)
...
def test_check_analysis_status_and_get_results_url_command_single_success(requests_mock):
    ...
    requests_mock.get(
        ...
        json={
            "result": {
                "analysis_id": analysis_id,
                "summary": {"title": "malicious", "verdict_name": "malicious", "verdict_type": "malicious"},
                "indicators": [
                    {"classification": "informative", "text": "URL is accessible"},
                    {"classification": "informative", "text": "Assigned IPv4 domain"},
                    {"classification": "informative", "text": "Vaild IPv4 domain"},
                    {"classification": "suspicious", "text": "suspicious"},
                    {"classification": "malicious", "text": "malicious"},
                ],
                "redirect_chain": [
                    {"response_status": 301, "url": "https://foo.com/"},
                    {"response_status": 200, "url": "http://www.foo.com/"},
                ],
                ...
                "downloaded_file": {
                    ...
                    "analysis_summary": {
                        "verdict_description": "This file contains code from malicious s"
    ...
    requests_mock.get(
        ...
        json={"sub_analyses": [{"sha256": sha256, "source": "root", "sub_analysis_id": file_root_analysis_id}]},
    )
...
def test_get_endpoint_analysis_missing(requests_mock):
    ...
    analysis_id = "analysis_id"
    _setup_access_token(requests_mock)
    requests_mock.get(f"{full_url}/endpoint-analyses/{analysis_id}", status_code=HTTPStatus.NOT_FOUND)
...
def test_check_analysis_status_and_get_results_command_multiple_analyses(requests_mock):
    ...
    requests_mock.get(
        ...
        json={"sub_analyses": [{"sha256": sha256_2, "source": "root", "sub_analysis_id": root_analysis_id_2}]},
    )
...
def test_get_analysis_sub_analyses_command_analysis_doesnt_exist(requests_mock):
    ...
    analysis_id = "analysis_id"
    _setup_access_token(requests_mock)
    requests_mock.get(f"{full_url}/analyses/{analysis_id}", status_code=HTTPStatus.NOT_FOUND)
...
def test_get_endpoint_analysis_polling_false(requests_mock, mocker):
    ...
    requests_mock.get(f"{full_url}/endpoint-analyses/{analysis_id}", status_code=HTTPStatus.CONFLICT)
...
def test_get_endpoint_analysis_result_http_error(requests_mock):
    ...
    analysis_id = "analysis_id"
    _setup_access_token(requests_mock)
    requests_mock.get(f"{full_url}/endpoint-analyses/{analysis_id}", status_code=HTTPStatus.BAD_REQUEST)
...
def test_get_file_analysis_polling_false(requests_mock, mocker):
    ...
    requests_mock.get(f"{full_url}/analyses/{analysis_id}", status_code=HTTPStatus.CONFLICT)
...
def test_get_file_analysis_result_http_error(requests_mock):
    ...
    analysis_id = "analysis_id"
    _setup_access_token(requests_mock)
    requests_mock.get(f"{full_url}/analyses/{analysis_id}", status_code=HTTPStatus.BAD_REQUEST)
...
def test_get_analysis_code_reuse_command_success_root(requests_mock):
    ...
    requests_mock.get(
        ...
        json={"families": [{"family_id": "123", "family_name": "Name", "reused_gene_count": 123}], "unique_gene_count": 0},
    )
...
def test_get_analysis_code_reuse_command_analysis_doesnt_exist(requests_mock):
    ...
    _setup_access_token(requests_mock)
    requests_mock.get(
        f"{full_url}/analyses/{analysis_id}/sub-analyses/{sub_analysis_id}/code-reuse", status_code=HTTPStatus.NOT_FOUND
    )
...
def test_get_analysis_code_reuse_command_no_code_reuse(requests_mock):
    ...
    _setup_access_token(requests_mock)
    requests_mock.get(
        f"{full_url}/analyses/{analysis_id}/sub-analyses/{sub_analysis_id}/code-reuse", status_code=HTTPStatus.CONFLICT
    )
...
def test_get_analysis_metadata_command_success_root(requests_mock):
    ...
    _setup_access_token(requests_mock)
    requests_mock.get(
        f"{full_url}/analyses/{analysis_id}/sub-analyses/root/metadata", json={"sha256": "sha256", "product_name": "something"}
    )
...
def test_get_analysis_metadata_command_success(requests_mock):
    ...
    requests_mock.get(
        ...
        json={"sha256": "sha256", "product_name": "something"},
    )
...
def test_get_analysis_metadata_command_analysis_doesnt_exist(requests_mock):
    ...
    _setup_access_token(requests_mock)
    requests_mock.get(
        f"{full_url}/analyses/{analysis_id}/sub-analyses/{sub_analysis_id}/metadata", status_code=HTTPStatus.NOT_FOUND
    )
    ...
    requests_mock.get(
        ...
        json={
            "result": {
                ...
                "network": [
                    {"ioc": "1.1.1.1", "source": ["Network communication"], "type": "ip"},
                    {"ioc": "raw.exampledomain.com", "source": ["Network communication"], "type": "domain"},
                ],
            }
        },
    )
...
def test_get_analysis_iocs_command_no_iocs(requests_mock):
    ...
    requests_mock.get(f"{full_url}/analyses/{analysis_id}/iocs", status_code=HTTPStatus.NOT_FOUND)
...
def test_get_family_info_command_success(requests_mock):
    ...
    _setup_access_token(requests_mock)
    requests_mock.get(
        f"{full_url}/families/{family_id}/info", json={"result": {"family_name": family_name, "family_type": family_type}}
    )
...
def test_get_family_info_command_analysis_doesnt_exist(requests_mock):
    ...
    family_id = "family_id"
    _setup_access_token(requests_mock)
    requests_mock.get(f"{full_url}/families/{family_id}/info", status_code=HTTPStatus.NOT_FOUND)
...
def test_check_is_available_http_error(requests_mock):
    requests_mock.get(f"{full_url}/is-available", status_code=HTTPStatus.BAD_REQUEST)
...
def test_analyze_url_command_success(requests_mock):
    ...
    _setup_access_token(requests_mock)
    requests_mock.post(f"{full_url}/url", status_code=HTTPStatus.CREATED, json={"result_url": f"/url/{analysis_id}"})
...
def test_analyze_url_command_missing_url(requests_mock):
    ...
    _setup_access_token(requests_mock)
    requests_mock.post(f"{full_url}/url", status_code=HTTPStatus.BAD_REQUEST, json={"error": "Bad url"})
    ...
    # Assert
    assert (
        "The Url 123test was not found on Intezer. Error Server returned bad request error: Bad url"
...
def test_submit_alert_command_success(requests_mock):
    ...
    _setup_access_token(requests_mock)
    alert_id = "112233"
    requests_mock.post(f"{full_url}/alerts/ingest", status_code=HTTPStatus.OK, json={"alert_id": alert_id, "result": True})
...
def test_submit_alert_command_invalid_mapping_file(requests_mock):
    ...
    alert_id = "112233"
    requests_mock.post(
        f"{full_url}/alerts/ingest", status_code=HTTPStatus.BAD_REQUEST, json={"alert_id": alert_id, "result": True}
    )
...
def test_submit_suspected_phishing_email_command_success(requests_mock, mocker):
    ...
    _setup_access_token(requests_mock)
    alert_id = "112233"
    requests_mock.post(f"{full_url}/alerts/ingest/binary", status_code=HTTPStatus.OK, json={"alert_id": alert_id, "result": True})
...
def test_get_alert_response_command_alert_not_found(requests_mock, mocker):
    ...
    requests_mock.get(f"{full_url}/alerts/get-by-id", status_code=HTTPStatus.NOT_FOUND, json={"alert_id": "123"})
...
def test_get_alert_response_command_alert_in_progress(requests_mock, mocker):
    ...
    requests_mock.get(
        f"{full_url}/alerts/get-by-id", status_code=HTTPStatus.OK, json={"result": {"123": "123"}, "status": "in_progress"}
    )
...
def test_get_alert_response_command_alert_success(requests_mock, mocker):
    ...
    requests_mock.get(
        f"{full_url}/alerts/get-by-id", status_code=HTTPStatus.OK, json={"result": {"123": "123"}, "status": "succeeded"}
    )
...
def test_enrich_dbot_and_display_alert_results_no_scans(requests_mock, mocker):
    ...
    mocker.patch.object(
        Alert,
        "result",
        return_value={"scans": [], "triage_result": {"alert_verdict_display": "dangerous", "risk_category_display": "malicious"}},
    )
...
def test_enrich_dbot_and_display_alert_results_artifact_analyses(requests_mock, mocker):
    ...
    mocker.patch.object(
        ...
        return_value={
            ...
            "triage_result": {"alert_verdict_display": "dangerous", "risk_category_display": "malicious"},
        },
    )
...
def test_enrich_dbot_and_display_alert_results_file_analysis(requests_mock, mocker):
    ...
    analysis._report = {
        "analysis_id": analysis_id,
        "analysis_time": "Mon, 24 Jul 2023 15:45:58 GMT",
        "analysis_url": f"https://analyze.intezer.com/analyses/{analysis_id}",
        "file_name": "body-html.html",
        "is_private": True,
        "sha256": sha256,
    ...
    mocker.patch.object(
        ...
        "result",
        return_value={
            "scans": [{"scan_type": "file", "file_analysis": analysis.result()}],
            "triage_result": {"alert_verdict_display": "dangerous", "risk_category_display": "malicious"},
        },
    )
