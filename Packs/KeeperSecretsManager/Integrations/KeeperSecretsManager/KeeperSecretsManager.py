...
metadata_collector = YMLMetadataCollector(
    integration_name="KeeperSecretsManager",
...
class Client:
    ...
    def __init__(self, credentials: str, insecure: bool):
        ...
        try:
            self.secrets_manager = SecretsManager(config=config, verify_ssl_certs=self.verify_ssl_certs)
    ...
    def ksm_list_files(self, record_uids: list[str]) -> list[dict[str, str]]:
        ...
        for r in records:
            ...
            for f in files:
                result.append(
                    {"record_uid": r.uid, "file_uid": f.f.get("fileUid", ""), "file_name": f.name or f.title, "file_size": f.size}
                )
        return result
        ...
        for r in records:
            ...
            for f in files:
                ...
                if fname == file_name or (partial_match and file_name and fname.__contains__(file_name)):
                    result.append(
                        {"record_uid": r.uid, "file_uid": f.f.get("fileUid", ""), "file_name": f.name, "file_size": f.size}
                    )
        return result
...
LIST_CREDENTIALS_OUTPUTS = [
    OutputArgument(name="uid", output_type=str, description="Record UID."),
...
@metadata_collector.command(
    command_name="ksm-list-credentials",
...
@metadata_collector.command(
    command_name="ksm-get-field",
    outputs_prefix="KeeperSecretsManager.Field",
    inputs_list=[InputArgument(name="notation", required=True, description="Keeper KSM notation URI.")],
...
def get_field_command(client: Client, args: dict[str, Any], **kwargs) -> CommandResults:
    ...
    results = CommandResults(
        readable_output=readable_output, outputs_prefix="KeeperSecretsManager.Field", outputs_key_field="", outputs=result
    )
    return results
...
def list_records_command(client: Client, args: dict[str, Any], **kwargs) -> CommandResults:
    ...
    results = CommandResults(
        readable_output=markdown, outputs_prefix="KeeperSecretsManager.Records", outputs_key_field="uid", outputs=records
    )
    return results


@metadata_collector.command(
    command_name="ksm-find-records",
    outputs_prefix="KeeperSecretsManager.Records",
    outputs_list=GET_RECORDS_OUTPUTS,
    restore=True,
    inputs_list=[
        InputArgument(name="title", required=True, description="Title text to search for."),
        InputArgument(name="partial_match", required=False, default=False, description="Search for partial title match."),
    ],
...
GET_FILES_OUTPUTS = [
    OutputArgument(name="record_uid", output_type=str, description="Record UID."),
...
@metadata_collector.command(
    command_name="ksm-list-files",
    outputs_prefix="KeeperSecretsManager.Files",
    outputs_list=GET_FILES_OUTPUTS,
    restore=True,
    inputs_list=[
        InputArgument(
            name="record_uids",
...
def list_files_command(client: Client, args: dict[str, Any], **kwargs) -> CommandResults:
    ...
    results = CommandResults(
        readable_output=markdown, outputs_prefix="KeeperSecretsManager.Files", outputs_key_field="file_uid", outputs=records
    )
    return results


@metadata_collector.command(
    command_name="ksm-find-files",
    outputs_prefix="KeeperSecretsManager.Files",
    outputs_list=GET_FILES_OUTPUTS,
    restore=True,
    inputs_list=[
        InputArgument(name="file_name", required=True, description="File name text to search for."),
        InputArgument(name="partial_match", required=False, default=False, description="Search for partial file name match."),
    ],
...
@metadata_collector.command(
    command_name="ksm-get-file",
    inputs_list=[
        InputArgument(name="file_uid", required=True, description="File UID to search for."),
        InputArgument(
            name="record_uid",
            required=False,
            default="",
