...
class Client(BaseClient):
    ...
    def _generate_token(self) -> str:
        ...
        try:
            response = self._http_request(
                "POST", "/aioc-rest-web/rest/login", json_data=body, headers=headers, resp_type="response"
            )
    ...
    def search_sapm_with_secret_name(self, secret_name: str):
        body = {"secretName": secret_name}
        return self._http_request("POST", "/aioc-rest-web/rest/sc/sapm/searchSapmAccounts", json_data=body)

    def show_password(self, password_expiration_in_minute=int, sapm_db_id=int, comment=str):
        body = {"passwordExpirationInMinute": password_expiration_in_minute, "sapmDbId": sapm_db_id, "comment": comment}
    ...
    def get_sapm_user_info(self, device_ip: str) -> dict[str, str]:
        body = {"deviceIp4Search": device_ip}
...
def list_all_sapm_accounts_command(client: Client) -> CommandResults:
    ...
    if result is not None:
        return CommandResults(outputs_prefix="SingleConnect.SapmAccount", outputs_key_field="dbId", outputs=result)
    else:
        raise Exception("Unexpected response format")
...
def show_password_command(client: Client, password_expiration_in_minute=int, sapm_db_id=int, comment=str) -> CommandResults:
    ...
    if isinstance(response, dict) and len(response) > 0:
        return CommandResults(outputs_prefix="SingleConnect.SapmAccount", outputs=response)
    else:
        raise Exception("Unexpected response format")
...
def get_sapm_user_info_command(client: Client, device_ip: str) -> CommandResults:
    ...
    if isinstance(response, list):
        return CommandResults(outputs_prefix="SingleConnect.SapmAccount", outputs_key_field="dbId", outputs=response)
    else:
        raise Exception("Unexpected response format")
...
def main() -> None:  # pragma: no cover
    ...
    try:
        client = Client(base_url=base_url, use_ssl=verify_certificate, username=username, password=password, proxy=proxy)
        ...
        try:
            ...
            demisto.info(f"Single Connect error: {err!s}")
