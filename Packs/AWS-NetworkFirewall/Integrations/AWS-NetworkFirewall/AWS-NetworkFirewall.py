...
config = Config(connect_timeout=1, retries=dict(max_attempts=5), proxies=proxies)
...
def parse_tag_field(tags_str):
    ...
    if demisto.args().get("tag_key") and demisto.args().get("tag_value"):
        if demisto.args().get("tags"):
            return_error("Please select either the arguments 'tag_key' and 'tag_value' or only 'tags'.")
        tags.append({"Key": demisto.args().get("tag_key"), "Value": demisto.args().get("tag_value")})
    else:
...
def aws_session(
    service="network-firewall", region=None, roleArn=None, roleSessionName=None, roleSessionDuration=None, rolePolicy=None
):
    client = None
    kwargs = {}
...
def associate_firewall_policy_command(args):
    ...
    outputs = {"AWS-NetworkFirewall.AssociationResults.FirewallPolicy(val.FirewallArn === obj.FirewallArn)": response}
...
def associate_subnets_command(args):
    ...
    for subnet_id in subnet_ids:
        kwargs["SubnetMappings"].append({"SubnetId": subnet_id})
    ...
    response = json.dumps(response, default=myconverter)
    response = json.loads(response)
    outputs = {"AWS-NetworkFirewall.AssociationResults.Subnets(val.FirewallArn === obj.FirewallArn)": response}
    del response["ResponseMetadata"]
...
def create_firewall_command(args):
    ...
    response = json.dumps(response, default=myconverter)
    response = json.loads(response)
    outputs = {"AWS-NetworkFirewall.Firewall(val.Firewall.FirewallArn === obj.Firewall.FirewallArn)": response}
    del response["ResponseMetadata"]
...
def delete_firewall_command(args):
    ...
    kwargs = {"FirewallName": args.get("firewall_name", None), "FirewallArn": args.get("firewall_arn", None)}
    kwargs = remove_empty_elements(kwargs)
    if args.get("raw_json") is not None and not kwargs:
        del kwargs
        kwargs = safe_load_json(args.get("raw_json", "{ }"))
...
def delete_resource_policy_command(args):
    ...
    kwargs = {"ResourceArn": args.get("resource_arn", None)}
    kwargs = remove_empty_elements(kwargs)
    if args.get("raw_json") is not None and not kwargs:
        del kwargs
        kwargs = safe_load_json(args.get("raw_json", "{ }"))
...
def describe_logging_configuration_command(args):
    ...
    kwargs = {"FirewallArn": args.get("firewall_arn", None), "FirewallName": args.get("firewall_name", None)}
    kwargs = remove_empty_elements(kwargs)
    if args.get("raw_json") is not None and not kwargs:
        del kwargs
        kwargs = safe_load_json(args.get("raw_json", "{ }"))
    ...
    response = json.dumps(response, default=myconverter)
    response = json.loads(response)
    outputs = {"AWS-NetworkFirewall.Logging(val.FirewallArn === obj.FirewallArn)": response}
    del response["ResponseMetadata"]
...
def list_firewall_policies_command(args):
    ...
    response = json.dumps(response, default=myconverter)
    response = json.loads(response)
    outputs = {"AWS-NetworkFirewall.FirewallPolicies(val.Arn === obj.Arn)": response.get("FirewallPolicies")}
    del response["ResponseMetadata"]
...
def list_firewalls_command(args):
    ...
    response = json.dumps(response, default=myconverter)
    response = json.loads(response)
    outputs = {"AWS-NetworkFirewall.Firewalls(val.FirewallArn === obj.FirewallArn)": response.get("Firewalls")}
    del response["ResponseMetadata"]
...
def list_rule_groups_command(args):
    ...
    kwargs = {"NextToken": args.get("next_token", None), "MaxResults": args.get("max_results", None)}
    kwargs = remove_empty_elements(kwargs)
    if args.get("raw_json") is not None and not kwargs:
        del kwargs
        kwargs = safe_load_json(args.get("raw_json", "{ }"))
    ...
    response = json.dumps(response, default=myconverter)
    response = json.loads(response)
    outputs = {"AWS-NetworkFirewall.RuleGroups(val.Arn === obj.Arn)": response.get("RuleGroups")}
    del response["ResponseMetadata"]
...
def list_tags_for_resource_command(args):
    ...
    kwargs = {"NextToken": args.get("next_token", None), "ResourceArn": args.get("resource_arn", None)}
    kwargs = remove_empty_elements(kwargs)
    if args.get("raw_json") is not None and not kwargs:
        del kwargs
        kwargs = safe_load_json(args.get("raw_json", "{ }"))
...
def put_resource_policy_command(args):
    ...
    kwargs = {"ResourceArn": args.get("resource_arn", None), "Policy": args.get("policy", None)}
    kwargs = remove_empty_elements(kwargs)
    if args.get("raw_json") is not None and not kwargs:
        del kwargs
        kwargs = safe_load_json(args.get("raw_json", "{ }"))
...
def untag_resource_command(args):
    ...
    kwargs = {"ResourceArn": args.get("resource_arn", None), "TagKeys": parse_resource_ids(args.get("tag_keys", ""))}
    kwargs = remove_empty_elements(kwargs)
    if args.get("raw_json") is not None and not kwargs:
        del kwargs
        kwargs = safe_load_json(args.get("raw_json", "{ }"))
...
def update_firewall_delete_protection_command(args):
    ...
    response = json.dumps(response, default=myconverter)
    response = json.loads(response)
    outputs = {"AWS-NetworkFirewall.FirewallAttributes(val.FirewallArn === obj.FirewallArn)": response}
    del response["ResponseMetadata"]
...
def main():  # pragma: no cover
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            human_readable, outputs, response = associate_firewall_policy_command(args)
            ...
            human_readable, outputs, response = create_firewall_policy_command(args)
            ...
            human_readable, outputs, response = delete_firewall_policy_command(args)
        elif demisto.command() == "aws-network-firewall-delete-resource-policy":
            human_readable, outputs, response = delete_resource_policy_command(args)
            ...
            human_readable, outputs, response = describe_firewall_policy_command(args)
        elif demisto.command() == "aws-network-firewall-describe-logging-configuration":
            human_readable, outputs, response = describe_logging_configuration_command(args)
        elif demisto.command() == "aws-network-firewall-describe-resource-policy":
            human_readable, outputs, response = describe_resource_policy_command(args)
        elif demisto.command() == "aws-network-firewall-describe-rule-group":
            human_readable, outputs, response = describe_rule_group_command(args)
        elif demisto.command() == "aws-network-firewall-disassociate-subnets":
            human_readable, outputs, response = disassociate_subnets_command(args)
        elif demisto.command() == "aws-network-firewall-list-firewall-policies":
            human_readable, outputs, response = list_firewall_policies_command(args)
            ...
            human_readable, outputs, response = list_tags_for_resource_command(args)
        elif demisto.command() == "aws-network-firewall-put-resource-policy":
            human_readable, outputs, response = put_resource_policy_command(args)
            ...
            human_readable, outputs, response = update_firewall_delete_protection_command(args)
        elif demisto.command() == "aws-network-firewall-update-firewall-description":
            human_readable, outputs, response = update_firewall_description_command(args)
        elif demisto.command() == "aws-network-firewall-update-firewall-policy":
            human_readable, outputs, response = update_firewall_policy_command(args)
        elif demisto.command() == "aws-network-firewall-update-firewall-policy-change-protection":
            human_readable, outputs, response = update_firewall_policy_change_protection_command(args)
        elif demisto.command() == "aws-network-firewall-update-logging-configuration":
            human_readable, outputs, response = update_logging_configuration_command(args)
            ...
            human_readable, outputs, response = update_subnet_change_protection_command(args)
        return_outputs(human_readable, outputs, response)

    except ResponseParserError as e:
        return_error(
            "Could not connect to the AWS endpoint. Please check that the region is valid. {error}".format(error=type(e))
        )
        LOG(e)
    except Exception as e:
        LOG(e)
        return_error(
            "Error has occurred in the AWS network-firewall Integration: {code} {message}".format(code=type(e), message=e)
        )
