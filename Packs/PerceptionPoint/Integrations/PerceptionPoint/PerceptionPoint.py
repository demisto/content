...
RELEASE = "release"
LIST = "list"
API_ACTIONS_DICT = {RELEASE: RELEASE_ENDPOINT, LIST: INCIDENTS_ENDPOINT}
SPAM = "SPM"
BLOCKED = "BLK"
MALICIOUS = "MAL"
...
FETCH_INCIDENTS_TYPE = [
    {"demisto_param": "fetch_malicious", "req_pname": VERBOSE_VERDICT_PARAM, "req_pval": MALICIOUS},
    {"demisto_param": "fetch_blocked", "req_pname": VERBOSE_VERDICT_PARAM, "req_pval": BLOCKED},
    {"demisto_param": "fetch_spam", "req_pname": VERBOSE_VERDICT_PARAM, "req_pval": SPAM},
]
...
def build_fetch_incident_types(fetch_blocked, fetch_malicious, fetch_spam):
    ...
    fetch_select = {"fetch_blocked": fetch_blocked, "fetch_malicious": fetch_malicious, "fetch_spam": fetch_spam}
...
def get_pp_api_result(url, params):
    try:
        res = requests.get(url=url, params=params, headers=HEADER, verify=SECURED)
...
def command_fetch_incidents():
    try:
        ...
        return_error(
            f"An error occurred while trying to fetch new incidents. "
...
def command_release_email():
    try:
        ...
        entry = {"Type": entryTypes["note"], "ReadableContentsFormat": formats["markdown"]}
        email_release_response = release_email_and_get_message(scan_id_to_release)
        entry.update(
            {
                "Contents": email_release_response,
