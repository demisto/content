...
def get_events_command(client: Client, args: Dict[str, Any]) -> tuple:
    ...
    if events:
        events = events[:limit]
        return events, CommandResults(readable_output=tableToMarkdown("Bitwarden Events", events), raw_response=events)

    return [], CommandResults(readable_output="No events found")


def fetch_events(
    client: Client, max_fetch: int, dates: Dict[str, Any] = {"start": DEFAULT_FIRST_FETCH, "end": DEFAULT_END_DATE}
    ...
    if continuation_token:
        ...
        new_last_fetch_date = (
            max(
                [dt for dt in (arg_to_datetime(event.get("date"), DATE_FORMAT) for event in unique_events) if dt is not None]
...
def get_events_with_pagination(
    client: Client, max_fetch: int, dates: Dict[str, Any], last_run: Dict[str, Any]
