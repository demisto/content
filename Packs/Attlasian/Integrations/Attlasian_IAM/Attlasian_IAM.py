...
class Client(BaseClient):
    ...
    def update_user(self, user_id, user_data):
        ...
        res = self._http_request(method="PUT", url_suffix=uri, json_data=user_data)
        user_app_data = res
        user_id = user_app_data.get("id")
        is_active = user_app_data.get("active")
        username = user_app_data.get("userName")
    ...
    def disable_user(self, user_id):
        uri = f"/scim/directory/{self.directory_id}/Users/{user_id}"
        res = self._http_request(method="DELETE", url_suffix=uri)
        user_app_data = res
        user_id = user_app_data.get("id")
        is_active = user_app_data.get("active")
        username = user_app_data.get("userName")
    ...
    def handle_exception(user_profile, e, action):
        ...
        if error_code == 204:
            user_profile.set_result(action=action, success=True, details="The user was successfully disabled.")

        user_profile.set_result(
            action=action, success=False, error_code=error_code, error_message=f"{error_message}\n{traceback.format_exc()}"
        )
...
def main():
    ...
    client = Client(
        base_url=base_url, directory_id=directory_id, verify=verify_certificate, proxy=proxy, headers=headers, ok_codes=(200, 201)
    )
