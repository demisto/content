...
INCIDENT_MAPPING_FIELDS = {
    "state": {"description": "incident can be either open or resolved", "xsoar_field_name": "hoxstate"},
    "socClassification": {
    ...
    "shouldSendFeedback": {"description": "should feedback be sent to the reporter(s)", "xsoar_field_name": "hoxsendfeedback"},
    "feedbackReportedAtLimit": {"description": "time limit for feedback", "xsoar_field_name": "hoxreportedatlimit"},
    "socMessage": {"description": "custom message to send with feedback", "xsoar_field_name": "hoxmessagetousers"},
}
...
MIRROR_DIRECTION = {"None": None, "Incoming": "In", "Outgoing": "Out", "Incoming And Outgoing": "Both"}.get(
    demisto.params().get("mirror_direction")
)  # type: ignore
...
class Client(BaseClient):
    def query(self, data: str, variables=None) -> GqlResult:
        ...
        response = self._http_request(
            method="POST", json_data={"query": data, "variables": variables}, error_handler=http_error_handler
        )

        gql_result = GqlResult(
            data=response.get("data"), errors=[error.get("message", "") for error in response.get("errors", [])]
        )

        return gql_result

    def add_incident_note(self, incident_id: str | None, note_text: str | None):
        return self.query(MUTATIONS["CreateIncidentNote"], {"incidentId": incident_id, "note": {"text": note_text}})
    ...
    def send_incident_soc_feedback(
        self, incident_id: str | None, custom_message: str | None = None, threat_feedback_reported_at_limit: str | None = None
    ):
        return self.query(
            MUTATIONS["SendIncidentSocFeedback"],
            variables={
    ...
    def set_incident_sensitive(self, incident_id: str | None, is_sensitive: bool) -> dict[str, Any]:
        response = self.query(MUTATIONS["SetIncidentSensitive"], {"incidentId": incident_id, "isSensitive": is_sensitive})
    ...
    def set_incident_soc_classification(self, incident_id: str | None, classification: str | None):
        return self.query(
            MUTATIONS["SetIncidentSocClassification"], {"incidentId": incident_id, "classification": classification}
        )

    def update_incident_state(self, incident_id: str | None, state: str | None):
        return self.query(MUTATIONS["UpdateIncidentState"], {"incidentId": incident_id, "state": state})
...
""" TYPES """
DemistoCommandReturnType = Union[
    CommandResults,
    PollResult,
    str,
    dict,
...
def create_output(results: dict[str, str], endpoint: str, key_field: str = "") -> CommandResults:
    ...
    return CommandResults(
        outputs_prefix=f"Hoxhunt.{endpoint}", outputs_key_field=key_field, outputs=results, readable_output=human_readable
    )
...
def create_incident_from_log(incident: dict[str, Any], now: float) -> dict[str, Any]:
    ...
    return {"name": human_readable_id, "dbotMirrorId": dbot_mirror_id, "rawJSON": json.dumps(incident), "occurred": occurred}
...
def get_mapping_fields_command(client: Client, args: dict, params: dict):
    ...
    for mapping_field in INCIDENT_MAPPING_FIELDS:
        campaign_incident_type_scheme.add_field(
            name=mapping_field, description=INCIDENT_MAPPING_FIELDS[mapping_field].get("description")
        )  # type: ignore
...
def hoxhunt_send_incident_soc_feedback_command(client: Client, args: dict, params: dict):
    ...
    date_limit_as_iso_string = (
        threat_feedback_reported_at_limit_parsed.strftime(  # type: ignore
            "%Y-%m-%dT%H:%M:%S.%f"
        )[:-3]
        + "Z"
    )
    ...
    result_data: dict = {"_id": incident_id, "custom_message": custom_message, "limit date": threat_feedback_reported_at_limit}
...
def hoxhunt_set_incident_sensitive_command(client: Client, args: dict, params: dict):
    ...
    sensitive_data = {
        "incident_id": str(response.get("_id", "")),
