...
def incident_1():
    return {
        ...
        "firstThreat": {"_id": "threat-1", "createdAt": FROZEN_DATE, "classification": "malicious"},
    }
...
def incident_2():
    return {
        ...
        "threatCount": 5,
        "globalThreatCount": 10,
        "firstThreat": {"_id": "threat-2", "createdAt": FROZEN_DATE, "classification": "malicious"},
    }
...
@pytest.mark.parametrize(
    ...
    [
        (FROZEN_DATE, "50", None, None, {}, {"start_time": FROZEN_DATE_AFTER}, "createdAt_gt", False),
        (FROZEN_DATE, "50", False, True, {}, {"start_time": FROZEN_DATE_AFTER}, "escalation__escalatedAt_exists: true", False),
        (
            FROZEN_DATE,
            "50",
            True,
            ...
            "escalation__escalatedAt_exists: true, state_eq: OPEN",
            False,
        ),
        (FROZEN_DATE, "50", True, False, {}, {"start_time": FROZEN_DATE_AFTER}, "state_eq: OPEN", False),
        (FROZEN_DATE, "50", False, False, {}, {"start_time": FROZEN_DATE_AFTER}, "createdAt_gt", False),
...
def test_fetch_incidents(
    ...
    if raises_exception:
        ...
        incidents, next_run = fetch_incidents(
            mock_client, first_fetch, max_fetch, only_open_incidents, only_escalated_incidents, now, last_run
        )
...
def test_get_mapping_fields_command(mock_client, mocker):
    ...
    expected_type_names = [CAMPAIGN_INCIDENT_TYPE_NAME, USER_ACTED_TYPE_NAME, BEC_TYPE_NAME]
...
def test_update_remote_system_command_incident_opened(mock_client, mock_demisto, mocker):
    ...
    """
    setup_mock_params(
        mock_demisto, delta={"field1": "value1"}, incidentChanged=True, status=1, data={"some": "data"}, remoteId="1234"
    )
    ...
    [
        ...
        (
            ...
            {"incident_id": "inc123", "note": "Test note"},
            {
                "data": {
                    "addIncidentNote": {
                        "notes": [{"text": "Previous note", "_id": "note122"}, {"text": "Test note", "_id": "note123"}]
                    }
                }
            },
...
def test_commands(
    ...
    if command_function == hoxhunt_set_incident_sensitive_command:
        ...
        mocker.patch.object(mock_client, command_function_to_client_method[command_function], return_value=query_return_value)
    else:
        mock_result = GqlResult(data=query_return_value.get("data", {}), errors=query_return_value.get("errors", []))
        mocker.patch.object(mock_result, "has_errors", return_value=bool(query_return_value.get("errors", [])))
        ...
        if command_function in command_function_to_client_method:
            ...
            mocker.patch.object(mock_client, client_method_name, return_value=mock_result)
        else:
            pass
