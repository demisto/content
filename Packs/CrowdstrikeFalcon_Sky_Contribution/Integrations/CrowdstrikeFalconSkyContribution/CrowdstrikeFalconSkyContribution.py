...
def add_remove_tag_command():
    ...
    if isinstance(device_ids, str):
        ...
        raise ValueError('Invalid device ids, please provide comma separated device ids')

    if isinstance(tags, str):
        tags = [t.strip() for t in tags.split(',')]
    else:
        raise ValueError('Invalid tags, please provide comma separated tags')

    api_data = {"action": action, "device_ids": device_ids, "tags": tags}

    raw_res = http_request('patch', '/devices/entities/devices/tags/v1', json=api_data)

    result = raw_res['resources'][0]

    return CommandResults(
        readable_output='The {0} tag action has been applied with status: {1} and http code: {2}'.format(action, result['updated'], result['code']),
        outputs_prefix='CrowdStrike.Tag',
        outputs_key_field='updated',
        outputs=result
    )
...
def main():
    ...
    try:
        ...
        elif command == 'cs-falcon-add-remove-tag':
            return_results(add_remove_tag_command())
        else:
