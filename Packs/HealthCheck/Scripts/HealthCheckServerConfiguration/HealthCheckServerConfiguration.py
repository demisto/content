...
RESOLUTION = [
    ...
    "Index War Room Entries: https://docs.paloaltonetworks.com/cortex/cortex-xsoar/5-5/cortex-xsoar-admin/incidents/incidents-management/war-room-overview/index-war-room-entries",
    "Make sure you are aware of the purpose of this key: Docker containers overloaded: https://docs.paloaltonetworks.com/cortex/cortex-xsoar/6-0/cortex-xsoar-admin/cortex-xsoar-overview/performance-tuning-of-cortex-xsoar-server",
    ...
    "You should consider disabling it: https://docs.paloaltonetworks.com/cortex/cortex-xsoar/5-5/cortex-xsoar-admin/manage-indicators/auto-extract-indicators",
    ...
    for (key, value) in config.items():
        ...
        if key == "log.http.traffic" and value == "true":
            res.append({"category": "Configuration", "severity": "Medium",
                        "description": f"{DESCRIPTION[0]}".format(key), "resolution": f"{RESOLUTION[0]}"})
            continue

        if key == "python.executable.no.docker" and value == "true":
            res.append({"category": "Configuration", "severity": "Medium",
                        "description": f"{DESCRIPTION[1]}".format(key), "resolution": f"{RESOLUTION[1]}"})
            continue

        if key == "security.timeout" and int(value) > 60:
            res.append({"category": "Configuration", "severity": "Medium",
                        "description": f"{DESCRIPTION[2]}".format(key, value), "resolution": f"{RESOLUTION[2]}"})
            continue

        if key == "db.index.entry.disable" and value == "false":
            res.append({"category": "Configuration", "severity": "High",
                        "description": f"{DESCRIPTION[3]}", "resolution": f"{RESOLUTION[3]}"})
            continue
        if key == "containers.high.water.mark" and int(value) > 20:
            res.append({"category": "Configuration", "severity": "Medium",
                        "description": f"{DESCRIPTION[2]}".format(key, str(value)), "resolution": f"{RESOLUTION[4]}"})
            continue

        if key == "containers.low.water.mark" and int(value) < 2:
            res.append({"category": "Configuration", "severity": "Medium",
                        "description": f"DESCRIPTION[2]".format(key, str(value)), "resolution": f"{RESOLUTION[5]}"})
            continue

        if key == "ingestion.samples.save-mapped" and value == "true":
            res.append({"category": "Configuration", "severity": "Medium",
                        "description": f"{DESCRIPTION[0]}".format(key), "resolution": f"{RESOLUTION[6]}"})
            continue
...
try:
    ...
    if AE:
        res.append({"category": "Configuration", "severity": "Medium",
                    "description": f"{DESCRIPTION[4]}", "resolution": f"{RESOLUTION[7]}"})

    if EVAL:
        res.append({"category": "Configuration", "severity": "Medium",
                    "description": "{DESCRIPTION[5]}", "resolution": f"{RESOLUTION[8]}"})
