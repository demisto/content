...
IncidentID = demisto.args().get('IncidentID')

restartcount = 0
suggestions = []
knownerrors = [
    {"Got permission denied while trying to connect to the Docker daemon socket at unix:///var/run/docker.sock: Get":
        "*Found Error:* `Got permission denied while trying to connect to the Docker daemon socket at unix` - Please refer to https://knowledgebase.paloaltonetworks.com/KCSArticleDetail?id=kA14u000000HB4oCAG"},
    ...
    {'proxyconnect tcp: tls: oversized record received with length 20527':
        '*Found Error:* `proxyconnect tcp: tls: oversized record received with length 20527` - Please refer to https://knowledgebase.paloaltonetworks.com/KCSArticleDetail?id=kA10g000000PNhpCAG'},
    {"error: websocket: not a websocket handshake: 'upgrade' token not found in 'Connection' header":
        '*Found Error:* `websocket: not a websocket handshake: upgrade token not found in Connection header` - Please refer to https://knowledgebase.paloaltonetworks.com/KCSArticleDetail?id=kA10g000000PNiOCAW'},
    {"Create more free space in thin pool or use dm.min_free_space":
        '*Found Error:* `Create more free space in thin pool or use dm.min_free_space` - Please refer to https://knowledgebase.paloaltonetworks.com/KCSArticleDetail?id=kA10g000000PNhQCAW'},
    ...
    {"(error: websocket: close 1006 (abnormal closure): unexpected EOF)":
     " *Found Error:* `error: websocket: close 1006 (abnormal closure): unexpected EOF` - https://support.demisto.com/hc/en-us/articles/115004463233-Websocket-configuration"},
    ...
    try:
        ...
        if suggestions:
            for entry in suggestions:
                res.append({"category": "Log Analysis", "severity": "High", "description": entry})
