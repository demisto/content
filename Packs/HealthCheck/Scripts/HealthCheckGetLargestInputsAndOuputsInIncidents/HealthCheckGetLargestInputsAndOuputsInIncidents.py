...
def main():
    try:
        args = demisto.args()
        daysAgo = datetime.today() - timedelta(days=180)
        ...
        if not is_table_result:
            ...
            for entry in data:
                if entry['size'] > 2:
                    ...
                    entry['size'] = str(entry['size']) + " MB"
                    incidentsList.append(entry)
            numIncidetsList = len(incidentsList)
            numIncidentsListBiggerThan10 = len(incidentsListBiggerThan10)
            analyzeFields = {
                "incidentsinputoutputbiggerthan1mb": incidentsList,
                "incidentsinputoutputbiggerthan10mb": incidentsListBiggerThan10,
                "numberofincidentsinputoutputbiggerthan1mb": numIncidetsList,
                "numberofincidentsinputoutputbiggerthan10mb": numIncidentsListBiggerThan10
            }

            demisto.executeCommand('setIncident', analyzeFields)
            if numIncidetsList >= 10:
                actionableItems.append({'category': 'DB analysis', 'severity': 'Medium',
                                        'description': f"{numIncidetsList} incidents were found with large and input and output, improve your task configuration"})
            if numIncidentsListBiggerThan10 >= 1:
                actionableItems.append({'category': 'DB analysis', 'severity': 'High',
                                        'description': f"{numIncidentsListBiggerThan10} incidents were found with large and input and output bigger than 10 MB, improve your task configuration"})
