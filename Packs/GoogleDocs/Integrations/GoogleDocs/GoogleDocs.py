...
def replace_all_text(action_name, source, target, match_case):
    return {action_name: {"replaceText": target, "containsText": {"text": source, "matchCase": match_case}}}


def insert_text(action_name, index, text, segment_id=None):
    return {action_name: {"location": get_location_object(index, segment_id), "text": text}}
...
def delete_named_range_by_id(action_name, named_range_id):
    return {action_name: {"namedRangeId": named_range_id}}
...
def insert_inline_image(action_name, index, uri, width, height, segment_id=None):
    return {
        action_name: {
            "uri": uri,
            "objectSize": {"height": {"magnitude": height, "unit": "PT"}, "width": {"magnitude": width, "unit": "PT"}},
...
def insert_table(action_name, index, rows, columns, segment_id=None):
    return {action_name: {"rows": rows, "columns": columns, "location": get_location_object(index, segment_id)}}
...
def insert_page_break(action_name, index, segment_id=None):
    return {action_name: {"location": get_location_object(index, segment_id)}}
...
def generate_results(document, human_readable_text):
    ...
    res = {"RevisionId": document["revisionId"], "DocumentId": document["documentId"], "Title": document["title"]}
    ec = {"GoogleDocs(val.DocumentId && val.DocumentId == obj.DocumentId)": res}
    return {
...
def batch_update_document_command(service):
    ...
    human_readable_text = "The document with the title {title} and actions {actions} was updated. the results are:".format(
        title=document["title"], actions=args.get("actions")
    )
...
def get_document_command(service):
    ...
    human_readable_text = "The document with the title {title} was returned. The results are:".format(title=document["title"])
    return generate_results(document, human_readable_text)
...
def main():  # pragma: no cover
    ...
    service_account_credentials = params.get("credentials_service_account", {}).get("password") or params.get(
