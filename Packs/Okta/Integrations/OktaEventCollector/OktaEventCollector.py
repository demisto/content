...
class Client(BaseClient):
    ...
    def make_api_call(self, limit: int, since: int):
        response = self.call(limit=limit, since=since)
        events: list = response.json()
        self.client.request.params.limit = str(limit_tmp - len(events))  # type: ignore
        return events

    def call(self, limit, since) -> requests.Response:  # pragma: no cover
        params = {
            "since": since,
            "sortOrder": "ASCENDING",
            "limit": limit
        ...
        try:
            response = self._http_request(url_suffix='/api/v1/logs', method='GET', params=params, headers=self._headers)
            return response
    ...
    def set_next_run_filter(self, after: str):
        next_run_filter = after  # type: ignore



def iter_events(self, client, limit, since, last_object_ids: list) -> None:  # type: ignore  # pragma: no cover
        """
        Function that responsible for the iteration over the events returned from the Okta api
        """

        events: list = client.make_api_call(limit, since)  # type: ignore
        if last_object_ids:
        ...
        while True:
            ...
            events: list = client.make_api_call()  # type: ignore
            try:
                assert events
...
def aggregated_results(client, limit, since, last_object_ids: List[str] = None) -> List[dict]:  # pragma: no cover
    """
    Function to group the events returned from the api
    """
    stored_events = []
    for events in iter_events(client, limit, since, last_object_ids):  # type: ignore
        stored_events.extend(events)
        if int(limit) == 0 or len(events) == 0:  # type: ignore
            break
...
def main():  # pragma: no cover
    try:
        ...
        if command == 'test-module':
            aggregated_results(client, events_limit, since=last_run_after, last_object_ids=last_object_ids)
            demisto.results('ok')
