    ...
    def get_events(self, since: int, limit: int = FETCH_LIMIT, next_link_url: str = ""):
        if next_link_url:
            ...
            return self._http_request(
                url_suffix="/api/v1/logs", method="GET", headers=self._headers, params=params, resp_type="response"
            )


def get_events_command(
    client: Client, total_events_to_fetch, since, last_object_ids: list[str] = [], next_link: str = ""
    ...
    """
    stored_events: list = []
    num_of_events_to_fetch = min(total_events_to_fetch, FETCH_LIMIT)
...
def fetch_events(
    client: Client, start_time_epoch: int, events_limit: int, last_run_after, last_object_ids: list[str] = [], next_link: str = ""
) -> tuple[list[dict], str]:
    while True:
        events, epoch_time_to_continue_fetch, next_link = get_events_command(
            client=client,
