...
def create_zone_command(client, args):
    ...
    IPs = argToList(args.get('gateway_ips'))
    proxies = argToList(args.get('proxies'))
    if not IPs and proxies:
        return 'Please provide either the gateway IPs or Proxy IPs. Both cannot be empty'
    ...
    zoneObject = apply_zone_updates(zoneObject, zone_name, IPs, proxies)
    # demisto.results(zoneObject)
