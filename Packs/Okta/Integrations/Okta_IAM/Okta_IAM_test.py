...
from Okta_IAM import (
    ...
    fetch_incidents,
    get_app_user_assignment_command,
...
def test_create_user_command__success(mocker):
    ...
    user_profile = create_user_command(
        client, args, "mapper_out", is_command_enabled=True, is_update_user_enabled=True, is_enable_enabled=True
    )
    outputs = get_outputs_from_user_profile(user_profile)
...
def test_get_mapping_fields_command(mocker):
    ...
    assert mapping.get(IAMUserProfile.DEFAULT_INCIDENT_TYPE, {}).get("field1") == "description1"
    assert mapping.get(IAMUserProfile.DEFAULT_INCIDENT_TYPE, {}).get("field2") == "description2"
...
def test_get_app_user_assignment_command(mocker):
    ...
    args = {"user_id": "mock_user_id", "application_id": "mock_app_id"}

    get_assignment_response = Response()
    get_assignment_response.status_code = 200
    get_assignment_response._content = (
        b'{"id": "mock_user_id", '
        b'"profile": {}, '
...
def test_fetch_incidents__two_logs_batches(mocker):
    ...
    events, _ = fetch_incidents(
        client=mock_client(), last_run={}, query_filter="mock_query_filter", first_fetch_str="7 days", fetch_limit=5
    )
...
def test_fetch_incidents__fetch_limit(mocker):
    ...
    mocker.patch("Okta_IAM.get_all_user_profiles", return_value={})
    events, _ = fetch_incidents(
        client=mock_client(), last_run={}, query_filter="mock_query_filter", first_fetch_str="7 days", fetch_limit=2
    )
...
def test_fetch_incidents__last_run(mocker):
    ...
    last_run = {"incidents": [{"mock_log1": "mock_value1"}, {"mock_log2": "mock_value2"}, {"mock_log3": "mock_value3"}]}
    mocker.patch("Okta_IAM.get_all_user_profiles", return_value={})

    events, next_run = fetch_incidents(
        client=mock_client(), last_run=last_run, query_filter="mock_query_filter", first_fetch_str="7 days", fetch_limit=2
    )
