...
def getPoStats(s, session_meta):
    # Increment TID
    session_meta['tid'] += 1
    # Create the file handle for the plugin to upload using the filename provided
    multipart_form_data = {
        # The current TID value in string format
        '___tid': (None, str(session_meta['tid'])),
        # The SecurityToken/Data String value generated by the token_generator function
        '___token': (None, token_generator(session_meta)),
    }

    url_str = session_meta['base_url'] + '/php/utils/router.php/PoliciesDirect.getRuleCountInRuleUsage'
    # Generate the JSON
    # {"action":"PanDirect","method":"run","data":["d8a233df2c60299b907463b1f05553f7","PoliciesDirect.getRuleCountInRuleUsage",[{"type":"security","position":"main","vsysName":"vsys1"}]],"type":"rpc","tid":35}
    json_cmd = {"action": "PanDirect", "method": "run", "data": [token_generator(session_meta),
                                                                 "PoliciesDirect.getRuleCountInRuleUsage", [{"type": "security", "position": "main", "vsysName": VSYS}]], "type": "rpc", "tid": session_meta['tid']}
...
def getNoAppSpecified(s, session_meta):
    ...
    # Generate the JSON
    # {"action":"PanDirect","method":"run","data":["1b14f4e1bc6370f64e1dfdaa6a6009a1","PoliciesDirect.getPoliciesByUsage",[{"type":"security","position":"main","vsysName":"vsys1","serialNumber":"007254000099856","isCmsSelected":false,"isMultiVsys":false,"showGrouped":false,"usageAttributes":{"timeframeTag":"30","application/member":"any","apps-seen-count":"geq '1'","action":"allow"},"pageContext":"app_usage","field":"$.bytes","direction":"DESC"}]],"type":"rpc","tid":329}
    json_cmd = {"action": "PanDirect", "method": "run", "data": [token_generator(session_meta),
                                                                 "PoliciesDirect.getPoliciesByUsage", [{"type": "security", "position": "main", "vsysName": VSYS, "serialNumber": SN, "isCmsSelected": false, "isMultiVsys": false, "showGrouped": false, "usageAttributes": {"timeframeTag": "30", "application/member": "any", "apps-seen-count": "geq \'1\'", "action": "allow"}, "pageContext": "app_usage", "field": "$.bytes", "direction": "DESC"}]], "type": "rpc", "tid": session_meta['tid']}
    # Send the JSON command to the firewall
    r = s.post(url=url_str, json=json_cmd)
    # Verbose output
    return json.loads(r.text)
...
def getUnusedApps(s, session_meta):
    ...
    url_str = session_meta['base_url'] + '/php/utils/router.php/PoliciesDirect.getPoliciesByUsage'
    # Generate the JSON
    # "PoliciesDirect.getPoliciesByUsage",[{"type":"security","position":"main","vsysName":"vsys1","serialNumber":"007254000099856","isCmsSelected":false,"isMultiVsys":false,"showGrouped":false,"usageAttributes":{"timeframeTag":"30","application/member":"unused","action":"allow"},"pageContext":"app_usage","field":"$.bytes","direction":"DESC"}]]
    # {"action":"PanDirect","method":"run","data":["4d1fbf63b209650d2fb56f8688afdd3e","PoliciesDirect.getPoliciesByUsage",[{"type":"security","position":"main","vsysName":"vsys1","serialNumber":"007254000099856","isCmsSelected":false,"isMultiVsys":false,"showGrouped":false,"usageAttributes":{"timeframeTag":"30","application/member":"unused","action":"allow"},"pageContext":"app_usage","field":"$.bytes","direction":"DESC"}]],"type":"rpc","tid":340}
    json_cmd = {"action": "PanDirect", "method": "run", "data": [token_generator(session_meta),
                                                                 "PoliciesDirect.getPoliciesByUsage", [{"type": "security", "position": "main", "vsysName": VSYS, "serialNumber": SN, "isCmsSelected": false, "isMultiVsys": false, "showGrouped": false, "usageAttributes": {"timeframeTag": "30", "application/member": "unused", "action": "allow"}, "pageContext": "app_usage", "field": "$.bytes", "direction": "DESC"}]], "type": "rpc", "tid": session_meta['tid']}
    # Send the JSON command to the firewall
    r = s.post(url=url_str, json=json_cmd)
    # Verbose output
    return json.loads(r.text)
...
def getRules(s, session_meta, timeframe, usage, exclude):
    ...
    url_str = session_meta['base_url'] + '/php/utils/router.php/PoliciesDirect.getPoliciesByUsage'
    # Generate the JSON
    # {"action":"PanDirect","method":"run","data":["c4c8f7e0b8be505fcdb8a0ab380e0ee5","PoliciesDirect.getPoliciesByUsage",[{"type":"security","position":"main","vsysName":"vsys1","serialNumber":"007254000099856","isCmsSelected":false,"isMultiVsys":false,"showGrouped":false,"usageAttributes":{"timeframe":"all","usage":"Unused","exclude":false,"exclude-reset-text":"90"},"pageContext":"rule_usage"}]],"type":"rpc","tid":410}
    json_cmd = {"action": "PanDirect", "method": "run", "data": [token_generator(session_meta),
                                                                 "PoliciesDirect.getPoliciesByUsage", [{"type": "security", "position": "main", "vsysName": VSYS, "serialNumber": SN, "isCmsSelected": false, "isMultiVsys": false, "showGrouped": false, "usageAttributes": {"timeframe": timeframe, "usage": usage, "exclude": false, "exclude-reset-text": "90"}, "pageContext": "rule_usage"}]], "type": "rpc", "tid": session_meta['tid']}
    # Send the JSON command to the firewall
    r = s.post(url=url_str, json=json_cmd)
    # Verbose output
    return json.loads(r.text)
...
def getUnusedIn30daysRules(s, session_meta):
    ...
    url_str = session_meta['base_url'] + '/php/utils/router.php/PoliciesDirect.getPoliciesByUsage'
    # Generate the JSON
    # {"action":"PanDirect","method":"run","data":["18d981caca1cb254403c8177baeaadb3","PoliciesDirect.getPoliciesByUsage",[{"type":"security","position":"main","vsysName":"vsys1","serialNumber":"007254000099856","isCmsSelected":false,"isMultiVsys":false,"showGrouped":false,"usageAttributes":{"timeframe":"30","usage":"Unused","exclude":false,"exclude-reset-text":"30"},"pageContext":"rule_usage"}]],"type":"rpc","tid":421}
    json_cmd = {"action": "PanDirect", "method": "run", "data": [token_generator(session_meta),
                                                                 "PoliciesDirect.getPoliciesByUsage", [{"type": "security", "position": "main", "vsysName": VSYS, "serialNumber": SN, "isCmsSelected": false, "isMultiVsys": false, "showGrouped": false, "usageAttributes": {"timeframe": "30", "usage": "Unused", "exclude": false, "exclude-reset-text": "30"}, "pageContext": "rule_usage"}]], "type": "rpc", "tid": session_meta['tid']}
    # Send the JSON command to the firewall
    r = s.post(url=url_str, json=json_cmd)
    # Verbose output
    return json.loads(r.text)
...
def getAppAndUsage(s, session_meta, rule_uuid):
    ...
    # Generate the JSON
    # {"action":"PanDirect","method":"run","data":["22e39b809e7558bc3b6fb1beae2e75a5","PoliciesDirect.getAppDetails",[{"type":"security","vsysName":"vsys1","position":"main","ruleUuidList":["8f2b7bab-2b7d-4d08-a262-27578bf5e555"],"summary":"no","resultfields":"<member>apps-seen</member><member>last-app-seen-since-count</member><member>days-no-new-app-count</member>","appsSeenTimeframe":"any","trafficTimeframe":30}]],"type":"rpc","tid":442}
    json_cmd = {"action": "PanDirect", "method": "run", "data": [token_generator(session_meta),
                                                                 "PoliciesDirect.getAppDetails", [{"type": "security", "vsysName": VSYS, "position": "main", "ruleUuidList": [rule_uuid], "summary":"no", "resultfields":"<member>apps-seen</member><member>last-app-seen-since-count</member><member>days-no-new-app-count</member>", "appsSeenTimeframe":"any", "trafficTimeframe":30}]], "type": "rpc", "tid": session_meta['tid']}
    # Send the JSON command to the firewall
    r = s.post(url=url_str, json=json_cmd)
    # Verbose output
    return json.loads(r.text)
...
def getDag(s, session_meta, dag):
    ...
    # Generate the JSON
    # {"action":"PanDirect","method":"run","data":["22e39b809e7558bc3b6fb1beae2e75a5","PoliciesDirect.getAppDetails",[{"type":"security","vsysName":"vsys1","position":"main","ruleUuidList":["8f2b7bab-2b7d-4d08-a262-27578bf5e555"],"summary":"no","resultfields":"<member>apps-seen</member><member>last-app-seen-since-count</member><member>days-no-new-app-count</member>","appsSeenTimeframe":"any","trafficTimeframe":30}]],"type":"rpc","tid":442}
    json_cmd = {"action": "PanDirect", "method": "execute", "data": [token_generator(session_meta), "AddressGroup.showDynamicAddressGroup", {
