...
import pytest
from OneLoginEventCollector import Client, check_response, prepare_query_params
...
@pytest.mark.parametrize(
    "params, last_run, expected_params",
    [
        ({"limit": "1"}, {}, {"limit": 1}),
        ({"since": "2022-08-01T09:00:00Z"}, {"since": "2022-09-01T09:00:00Z"}, {"limit": 1000, "since": "2022-09-01T09:00:00Z"}),
        (
            {"since": "2022-08-01T09:00:00Z", "after_cursor": "param_cursor"},
            {"after_cursor": "last_run_cursor"},
...
def test_onelogin_events_params_good(params, last_run, expected_params):
    ...
    if query_params["since"]:
        assert (
            query_params["since"] == "2022-09-01T09:00:00Z"
            if "since" in last_run
            else query_params["since"] == "2022-08-01T09:00:00Z"
        )

    if query_params["after_cursor"]:
        assert (
            query_params["after_cursor"] == "last_run_cursor"
            if "after_cursor" in last_run
            else query_params["after_cursor"] == "param_cursor"
        )

    assert expected_params.items() <= prepare_query_params(params, last_run).items()


@pytest.mark.parametrize("params", [{"limit": "hello"}, {"since": "hello"}, {"until": "hello"}])
...
@pytest.mark.parametrize(
    ...
    [({"status": {"error": False}}, False), ({"status": {"error": True, "code": 400}}, True), (MOCK_EVENTS, False)],
)
...
def test_fetch_events(requests_mock):
    ...
    events, _ = fetch_events_command(Client(base_url=CORE_URL, headers={}), params={"limit": 3}, last_run=last_run)
...
def test_fetch_events_with_iterations(requests_mock):
    ...
    events, _ = fetch_events_command(Client(base_url=CORE_URL, headers={}), params={"limit": 10}, last_run={})
...
def test_fetch_events_with_last_event_ids(requests_mock):
    ...
    events, _ = fetch_events_command(Client(base_url=CORE_URL, headers={}), params={"limit": 2}, last_run=last_run)
...
@pytest.mark.parametrize("last_run, call_count", [({"event_types": {"1": "TYPE"}}, 0), ({}, 1)])
...
@pytest.mark.parametrize(
    "event, call_count, expected_name",
    [
