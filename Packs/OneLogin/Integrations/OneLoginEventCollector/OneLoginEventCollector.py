...
class Client(BaseClient):
    def get_access_token_request(self):
        ...
        raw_response = self._http_request(
            method="POST", url_suffix="/auth/oauth2/v2/token", params={"grant_type": "client_credentials"}
        )
    ...
    def convert_type_id_to_name(self, event: dict, event_types: dict, last_run: dict) -> str:
        ...
        demisto.info(
            f"Could not find the event type id '{event['event_type_id']!s}' "
    ...
    def fetch_events(self, access_token: str, query_params: dict, last_run: dict) -> List[dict]:
        ...
        last_run.update({"after_cursor": cursor, "since": since, "last_event_ids": event_ids})
