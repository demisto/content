...
class Client(BaseClient):
    ...
    def __init__(
        self, regions_list: list, services_list: list, polling_timeout: int = 20, insecure: bool = False, proxy: bool = False
    ):
    ...
    def extract_metadata_of_indicators_group(indicators_group_data: dict) -> dict:
        ...
        # The first part of the ID is the service name by default. For example {ID: "a.b", serviceName: "a"}.
        indicator_metadata["system_service"] = (
            indicator_properties.get("systemService") or str(indicator_metadata.get("id", ".")).split(".")[0]
        )
    ...
    def extract_indicators_from_values_dict(self, values_from_file: dict) -> list:
        ...
        for indicators_group in values_from_file:
            ...
            is_region_not_in_filter = "All" not in self.regions_list and indicator_metadata["region"] not in self.regions_list
            is_service_not_in_filter = (
                "All" not in self.services_list and indicator_metadata["system_service"] not in self.services_list
            )
...
def get_indicators_command(
    client: Client, feedTags: list, tlp_color: str | None, enrichment_excluded: bool = False
) -> tuple[str, dict, dict]:
    ...
    human_readable = tableToMarkdown("Indicators from Azure Feed:", indicators, headers=["value", "type"], removeNull=True)

    return human_readable, {}, {"raw_response": raw_response}


def fetch_indicators_command(
    client: Client, feedTags: list, tlp_color: str | None, limit: int = -1, enrichment_excluded: bool = False
