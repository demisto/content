...
class Client(BaseClient):
    ...
    def _login(self) -> None:
        auth_data = {"selectedRealm": "ssl_vpn_Username_Password", "userName": self.client_id, "password": self.access_key}
        ...
        if res.status_code == 200:
            ...
            raise DemistoException(f"Log-in failed: {res.status_code!s}: {res.text}")
    ...
    def _call_api(
        ...
        res_json = self._http_request(
            method, url_suffix=f"/incidents/v1/{url_suffix}", headers={"domain": self.domain}, params=params, json_data=json_data
        )
...
def fetch_incidents(
    client: Client, last_run: dict[str, str], first_fetch: datetime, domain: str, max_fetch: int, min_probability: int
):
