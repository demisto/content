...
class TestsActiveDirectory:
    ...
    def test_parse_ldap_group_entries_and_referrals(self):
        ...
        ldap_group_entries = [
            ...
            {
                "raw_dn": b"CN=test,CN=Users,DC=demisto,DC=test",
                "dn": "CN=test,CN=Users,DC=demisto,DC=test",
                "raw_attributes": {"primaryGroupToken": [b"11111"]},
                "attributes": {"primaryGroupToken": 11111},
                "type": "searchResEntry",
            },
            {
                "raw_dn": b"CN=DisabledUsers,DC=demisto,DC=test",
                "dn": "CN=DisabledUsers,DC=demisto,DC=test",
                "raw_attributes": {"primaryGroupToken": [b"22222"]},
                "attributes": {"primaryGroupToken": 22222},
                "type": "searchResEntry",
            },
        ]
        ...
        expected_entries = [
            {"DN": "CN=test,CN=Users,DC=demisto,DC=test", "Attributes": [{"Name": "primaryGroupToken", "Values": ["11111"]}]},
            {"DN": "CN=DisabledUsers,DC=demisto,DC=test", "Attributes": [{"Name": "primaryGroupToken", "Values": ["22222"]}]},
        ]
    ...
    def test_parse_and_authenticate_ldap_group_entries_and_referrals(self, mocker):
        ...
        ldap_group_entries = [
            ...
            {"uri": ["ldap://domain2/DC=test,DC=demisto,DC=test"], "type": "searchResRef"},
            {
                "raw_dn": b"CN=username,CN=Users,DC=demisto,DC=test",
                "dn": "CN=username,CN=Users,DC=demisto,DC=test",
                "raw_attributes": {
                    "memberOf": [b"CN=test-group,CN=Users,DC=demisto,DC=test"],
                    "name": [b"username"],
                    "primaryGroupID": [b"111"],
                    "mail": [b"user@mail.com"],
                    "mobile": [b"050-1111111"],
                },
                "attributes": {
                    "memberOf": ["CN=test-group,CN=Users,DC=demisto,DC=test"],
                    "name": "username",
                    "primaryGroupID": 111,
                    "mail": "user@mail.com",
                    "mobile": "050-1111111",
        ...
        referrals, entries = client._parse_and_authenticate_ldap_group_entries_and_referrals(ldap_group_entries, password)

        assert referrals == expected_referrals
        assert entries == expected_entries

    @pytest.mark.parametrize(
        "user_logon_name, expected_ad_username",
        [
    ...
    @pytest.mark.parametrize(
        "connection_type, expected_auto_bind_value", [("Start TLS", "TLS_BEFORE_BIND"), ("SSL", "NO_TLS"), ("None", "NO_TLS")]
    )
    def test_get_auto_bind_value(self, connection_type, expected_auto_bind_value):
        ...
        """
        client = LdapClient({"ldap_server_vendor": "Active Directory", "host": "server_ip", "connection_type": connection_type})
...
class TestsOpenLDAP:
    ...
    @pytest.mark.parametrize(
        "dn, user_identifier_attribute, expected_result",
        [
...
class TestLDAPAuthentication:
    ...
    def test_get_ssl_version(self, ssl_version, expected_ssl_version):
        ...
        """
        client = LdapClient(
            {"ldap_server_vendor": "OpenLDAP", "host": "server_ip", "connection_type": "SSL", "ssl_version": ssl_version}
        )
    ...
    @pytest.mark.parametrize(
        "custom_attributes, expected_formatted_attributes",
        ...
        client = LdapClient(
            {
                ...
                "host": "server_ip",
                "connection_type": "SSL",
                "custom_attributes": "attr1val1,attr2=val2,attr3=val3",
            }
        )
...
class TestEntriesPagedSearch(unittest.TestCase):
    ...
    def test_subsequent_page(self):
        ...
        self.connection.result = {"controls": {"1.2.840.113556.1.4.319": {"value": {"cookie": b"cookie_value"}}}}
...
class TestEntriesSearchCommand(unittest.TestCase):
    ...
    def test_entries_search_command_first_page(
        self, mock_get_search_attributes, mock_create_entries_search_filter, mock_connection
    ):
        """
        ...
        args = {"search_base": "dc=example,dc=com", "search_scope": "SUBTREE", "page": "1", "page_size": "50"}
    ...
    @patch("OpenLDAP.create_entries_search_filter", return_value="(objectClass=*)")
    @patch("OpenLDAP.get_search_attributes", return_value=["cn", "mail"])
    def test_entries_search_command_subsequent_page(
        self, mock_get_search_attributes, mock_create_entries_search_filter, mock_connection
    ):
        """
        ...
        args = {"search_base": "dc=example,dc=com", "search_scope": "SUBTREE", "page": "2", "page_size": "50"}
