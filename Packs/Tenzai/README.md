...
**Active testing and credits.** Tenzai validation runs **active penetration testing against the live production asset** and **consumes Tenzai credits**. Because ASM attribution can occasionally be wrong, the playbook gates on analyst approval by default: the `RequireAnalystApproval` input (default `true`) pauses for an analyst to confirm the target and exposure before testing starts. It is also self-guarding — it first checks that an enabled Tenzai integration instance exists and ends without changing the issue if none is configured, so enabling the trigger on a tenant without a Tenzai instance is a no-op. Enabling the trigger means every matching High/Critical ASM exposure runs this active testing automatically; keep `RequireAnalystApproval` set to `true`, or leave the trigger disabled and validate ad-hoc from the Validate button, unless you intend fully automatic testing.
