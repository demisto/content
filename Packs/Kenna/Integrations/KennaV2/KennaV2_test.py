...
from KennaV2 import (
    ...
    get_connectors,
    parse_response,
    search_assets_by_external_id_command,
...
def test_parse_response():
    raw = [{"id": "12", "cve-id": "CVE-AS12", "list": [{"list-id": "123"}]}]
    expected = [{"ID": "12", "CVE-ID": "CVE-AS12", "List": [{"List-ID": "123"}]}]
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param({"status": "success"}, "Asset with ID 123 was successfully updated.", id="successfully updated."),
        pytest.param({"status": "failure"}, "Could not update asset with ID 123.", id="failure."),
    ],
)
