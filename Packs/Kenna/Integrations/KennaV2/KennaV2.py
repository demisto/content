...
def parse_response(raw_data: List[Dict[str, Any]], wanted_keys: List[Any], actual_keys: List[Any]) -> List[Dict[str, Any]]:
...
class Client(BaseClient):
    def __init__(self, base_url: str, api_key: str, verify: bool, proxy: bool):
        header = {"X-Risk-Token": api_key, "Content-Type": "application/json", "Accept": "application/json"}
        super().__init__(base_url=base_url, verify=verify, proxy=proxy, headers=header)

    def http_request(self, message: str, suffix: str, params: Optional[dict] = None, data: Optional[dict] = None):
            ...
            if not response.ok:
                if response_dict.get("error") == "unauthorized":
                    raise DemistoException(
                        f'Connection error in the API call to Kenna.\n'
...
def search_vulnerabilities(client: Client, args: dict) -> tuple[str, Dict[str, Any], List[Dict[str, Any]]]:
    ...
    response = client.http_request(message="GET", suffix=url_suffix, params=params).get("vulnerabilities")

    if response:
        vulnerability_list = response[:limit]
        wanted_keys = [
            "AssetID",
            ["Connectors", "DefinitionName", "ID", "Name", "Vendor"],
            "CveID",
            "FixID",
            "ID",
            "Patch",
            "Score",
            ["ScannerVulnerabilities", "ExternalID", "Open", "Port"],
            "Severity",
            "Status",
            "Threat",
            "TopPriority",
        ...
        actual_keys = [
            "asset_id",
            ["connectors", "connector_definition_name", "id", "name", "vendor"],
            "cve_id",
            "fix_id",
            "id",
            "patch",
        ...
        for lst in vulnerability_list:
            human_readable.append({"id": lst.get("id"), "Name": lst.get("cve_id"), "Score": lst.get("risk_meter_score")})
        context = {"Kenna.Vulnerabilities(val.ID === obj.ID)": context_list}
...
def get_connectors(client: Client, *_) -> tuple[str, Dict[str, Any], List[Dict[str, Any]]]:
    ...
    if connectors:
        ...
        context = {"Kenna.ConnectorsList(val.ID === obj.ID)": context_list}
...
def inactivate_asset(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    asset = {"asset": {"inactive": argToBoolean(args["inactive"]), "notes": args["notes"]}}
...
def get_connector_runs(client: Client, args: dict) -> tuple[str, Dict[str, Any], List[Dict[str, Any]]]:
    ...
    if connectors:
        ...
        for connector in connectors:
            ...
            human_readable.append(curr_dict)
        context = {"Kenna.ConnectorRunsList(val.ID === obj.ID)": context_list}
...
def search_fixes(client: Client, args: dict) -> tuple[str, Dict[str, Any], List[Dict[str, Any]]]:
    ...
    if response:
        wanted_keys = [
            "ID",
            "Title",
            ["Assets", "ID", "Locator", "PrimaryLocator", "DisplayLocator"],
            ["Vulnerabilities", "ID", "ServiceTicketStatus", "ScannerIDs"],
            "CveID",
            "LastUpdatedAt",
            "Category",
            "VulnerabilityCount",
        ...
        actual_keys = [
            "id",
            "title",
            ["assets", "id", "locator", "primary_locator", "display_locator"],
            ["vulnerabilities", "id", "service_ticket_status", "scanner_ids"],
            "cves",
            "updated_at",
            "category",
            "vuln_count",
        ...
        context = {"Kenna.Fixes(val.ID === obj.ID)": context_list}
    else:
...
def update_vulnerability(client: Client, args: dict) -> tuple[str, Dict[str, Any], List[Dict[str, Any]]]:
    ...
    Returns:
        Success/ Failure , according to the response
    """
    params_to_update: dict = {"vulnerability": {}}
...
def get_asset_vulnerabilities(client: Client, args: dict) -> tuple[str, Dict[str, Any], List[Dict[str, Any]]]:
    ...
    response = client.http_request(message="GET", suffix=url_suffix).get("vulnerabilities")
    if response:
        ...
        for lst in vulnerabilities_list:
            human_readable.append({"id": lst.get("id"), "Name": lst.get("cve_id"), "Score": lst.get("risk_meter_score")})
        context = {"Kenna.VulnerabilitiesOfAsset(val.ID === obj.ID)": context_list}
        human_readable_markdown = tableToMarkdown("Kenna Vulnerabilities", human_readable, removeNull=True)
    else:
