...
def get_managed_accounts():
    ...
    return_outputs(
        tableToMarkdown("BeyondTrust Managed Accounts", data, headers, removeNull=True), entry_context, managed_accounts
    )
...
def get_managed_systems():
    ...
    return_outputs(tableToMarkdown("BeyondTrust Managed Systems", data, removeNull=True), entry_context, managed_systems)
...
def list_release_requests():
    ...
    return_outputs(
        tableToMarkdown("Current requests are as follows", data, headers, removeNull=True), entry_context, release_list
    )
...
def create_release():
    ...
    response = {"RequestID": request_id, "Password": credentials}
...
def fetch_credentials():
    ...
    for account in account_info:
        ...
        credentials.append({"user": account_name, "password": password, "name": system_name + "_" + account_name})
    if identifier:
