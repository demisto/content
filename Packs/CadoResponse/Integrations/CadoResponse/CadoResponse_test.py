...
def test_module_command_success(requests_mock: Any) -> None:
    ...
    mock_get_response: dict[str, str] = {"status": "Running"}

    expected_mocked_command_result: str = "ok"

    requests_mock.get("https://test.com/api/v2/system/status", json=mock_get_response)

    client: Client = Client(base_url="https://test.com/api/v2/", verify=False, headers={"Authentication": "Bearer some_api_key"})
...
def test_module_command_fail(requests_mock: Any) -> None:
    ...
    mock_get_response: dict[str, str] = {"status": "Down"}
...
def test_create_project_command_success(requests_mock: Any) -> None:
    ...
    mock_post_response: dict[str, int] = {"id": 1}
    ...
    args: dict[str, str] = {"project_name": "testing-project", "project_description": "This is a testing project"}
    ...
    mock_get_response: list[dict[str, Any]] = [
        {
            ...
            "id": 1,
            "status": "Pending",
            "users": [{"display_name": "admin", "id": 1, "is_admin": True, "login_type": 0, "username": "admin"}],
        }
    ]
...
def test_get_project_single_command_success(requests_mock: Any) -> Any:
    ...
    args: dict[str, int] = {"project_id": 1}
        ...
        "pipelines": [
            {
                ...
                "summary": {"cancelled": 0, "failure": 0, "pending": 0, "running": 0, "success": 15, "total": 15},
                "terminated": True,
                "user_id": 1,
                "user_name": "admin",
            }
...
def test_get_single_pipeline_command_success(requests_mock: Any) -> None:
    ...
    args: dict[str, int] = {"project_id": 1, "pipeline_id": 1}
...
def test_get_pipeline_command_limit_success(requests_mock: Any) -> None:
    ...
    args: dict[str, int] = {"project_id": 1, "limit": 1}
...
def test_list_ec2_command_success(requests_mock: Any) -> None:
    ...
    args: dict[str, int | str] = {"project_id": 1, "region": "us-west-2"}
...
def test_list_s3_command_success(requests_mock: Any) -> None:
    ...
    mock_get_response: dict[str, list[str]] = {"buckets": ["Bucket_name"]}
...
def test_trigger_s3_command_success(requests_mock: Any) -> None:
    ...
    args: dict[str, int | str] = {"project_id": 1, "file_name": "test", "bucket": "test-bucket"}
