...
def hour_in_shift(day, hour, shift):
    ...
    return (shift_from_time <= check_time_bottom < shift_to_time) or (shift_from_time <= check_time_top < shift_to_time)


def main():
    get_roles_response: list = demisto.executeCommand("getRoles", {})
    if is_error(get_roles_response):
        demisto.error(f"Failed to get roles: {get_error(get_roles_response)!s}")
    else:
        ...
        widget = TextWidget(
            ...
            + tableToMarkdown(
                ...
                headers=[
                    ...
                    SUNDAY_HEADER,
                    MONDAY_HEADER,
                    TUESDAY_HEADER,  # disable-secrets-detection
                    WEDNESDAY_HEADER,
                    THURSDAY_HEADER,
                    FRIDAY_HEADER,
                    SATURDAY_HEADER,
