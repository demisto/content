...
result = demisto.executeCommand("demisto-api-post", {"uri": "/playbook/search",
                                "body": {"query": "system:F"}})[0]["Contents"]["response"]
playbooks = [{"Name": x.get("name", ""), "Description": x.get("comment", "")}
             for x in result["playbooks"] if "XSOAR Engineer Training" in x.get("name")]
