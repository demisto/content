...
class TestClient:
    ...
    def test_api_request(self, client, mocker):
        ...
        result = client.api_request('/v2/test', {'param': 'value'})
...
class TestCalculateDbotScore:
    ...
    def test_score_good_zero_detections(self):
        ...
        """
        score = calculate_dbot_score(engines_count=10, detections=0, good_threshold=10, suspicious_threshold=30, bad_threshold=60)
    ...
    def test_score_good_below_threshold(self):
        ...
        Then: Score should be GOOD (1)
        """
        score = calculate_dbot_score(engines_count=100, detections=5, good_threshold=10, suspicious_threshold=30, bad_threshold=60)
        assert score == Common.DBotScore.GOOD
    ...
    def test_score_suspicious(self):
        ...
        """
        score = calculate_dbot_score(engines_count=100, detections=40, good_threshold=10, suspicious_threshold=30, bad_threshold=60)
    ...
    def test_score_bad(self):
        ...
        """
        score = calculate_dbot_score(engines_count=100, detections=70, good_threshold=10, suspicious_threshold=30, bad_threshold=60)
    ...
    def test_score_none_zero_engines(self):
        ...
        """
        score = calculate_dbot_score(engines_count=0, detections=0, good_threshold=10, suspicious_threshold=30, bad_threshold=60)
    ...
    def test_score_edge_case_at_threshold(self):
        ...
        Then: Score should be SUSPICIOUS (2) - at threshold
        """
        score = calculate_dbot_score(engines_count=100, detections=30, good_threshold=10, suspicious_threshold=30, bad_threshold=60)
        assert score == Common.DBotScore.SUSPICIOUS
...
class TestIpReputationCommand:
    ...
    def test_ip_reputation_api_error(self, client, mocker):
        ...
        assert 'Error checking IP' in result.readable_output
...
class TestDnsLookupCommand:
    ...
    def test_dns_lookup_success(self, client, mocker):
        ...
        assert 'APIVoid.DNS(val.host && val.type && val.host == obj.host && val.type == obj.type)' in results[0]['EntryContext']
    ...
    def test_dns_lookup_default_type(self, client, mocker):
        ...
        # Verify API was called with dns-a action
        client.api_request.assert_called_with(
            '/v2/dns-lookup',
            {'host': 'example.com', 'action': 'dns-a'}
        )
    ...
    def test_dns_lookup_error(self, client, mocker):
        ...
        assert isinstance(results, list)
        assert 'Error looking up DNS' in results[0].readable_output
...
class TestSiteTrustworthinessCommand:
    ...
    def test_site_trust_api_error(self, client, mocker):
        ...
        assert isinstance(result, CommandResults)
        assert 'Error getting site trustworthiness' in result.readable_output
...
class TestMain:
    ...
    def test_main_params_called_once(self, mock_demisto, mocker):
        ...
        """
        mocker.patch.object(demisto, 'command', return_value='test-module')
        mock_params = mocker.patch.object(demisto, 'params', return_value={
