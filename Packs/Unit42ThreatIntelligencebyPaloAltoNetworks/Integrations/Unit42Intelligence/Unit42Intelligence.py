...
def parse_url_list(url_input: str | list | None) -> list[str]:
    ...
    for line in url_input.split('\n'):
        if line := line.strip():
            all_urls.extend(
                url.strip()
                for url in re.split(r',\s*(?=[a-zA-Z][a-zA-Z0-9+.-]*://)', line)
                if url.strip()
            )
...
def unit42_error_handler(res: requests.Response):
    ...
    demisto.debug(
        f'{INTEGRATION_NAME} API Error - X-Request-ID: {request_id}, Status: {res.status_code}, URL: {res.url}'
    )
...
class Client(BaseClient):
    ...
    def __init__(
        ...
        headers = {
            'Authorization': f'Bearer {demisto.getLicenseID()}',
    ...
    def lookup_indicator(self, indicator_type: str, indicator_value: str) -> requests.Response:
        ...
        endpoint = LOOKUP_ENDPOINT.format(
...
def create_dbot_score(
    ...
    if verdict.lower() == 'malicious':
        malicious_description = (
            f'Unit 42 Intelligence classified this {indicator_type.lower()} as malicious'
        )
...
def create_relationships(
    ...
    if not create_relationships or not threat_objects:
        demisto.debug(
            f'Skipping create_relationships as {create_relationships} and {threat_objects=}'
        )
    ...
    for threat_obj in threat_objects:
        ...
        if not threat_name or threat_class not in INDICATOR_TYPE_MAPPING:
            demisto.debug(
                f'Skipping create_relationships for threat_name {threat_name} and threat_class {threat_class}'
            )
            continue
...
def extract_response_data(response: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        'verdict_categories': [
            item.get('value') for item in response.get('verdict_categories', [])
        ],
...
def extract_malware_families_from_threat_objects(
    threat_objects: list[dict[str, Any]],
) -> str | None:
    """
...
def build_threat_object_description(threat_obj: dict[str, Any]) -> str:
    ...
    if (
        highlights and highlights != 'Highlights / Key Takeaways (external)'
    ):  # Do not add if it is only the default title
        description += '\n\n##'
        description += highlights

    # Add methods section if available (for threat actors)
    methods = demisto.get(
        threat_obj, 'battlecard_details.threat_actor_details.methods', ''
    ).replace('\\n', '\n')
    if methods:
        description += '\n\n##'
        description += methods
    ...
    targets = demisto.get(
        threat_obj, 'battlecard_details.threat_actor_details.targets', ''
    ).replace('\\n', '\n')
    if targets:
        description += '\n\n##'
        description += targets
...
def create_malware_relationships(
    ...
    """
    relationships = []
    malware_associations = demisto.get(
        threat_obj, 'battlecard_details.threat_actor_details.malware_associations', []
    )
...
def create_tools_relationships(
    ...
    """
    relationships = []
    tools_associations = demisto.get(
        threat_obj, 'battlecard_details.threat_actor_details.tools', []
    )
...
def create_vulnerabilities_relationships(
    ...
    """
    relationships = []
    vulnerabilities = demisto.get(
        threat_obj, 'battlecard_details.threat_actor_details.vulnerability_associations', []
    )
...
def create_actor_relationships(
    ...
    """
    relationships = []
    actor_associations = demisto.get(
        threat_obj, 'battlecard_details.malware_family_details.actor_associations', []
    )
...
def create_location_indicators_and_relationships(
    ...
    affected_regions = demisto.get(
        threat_obj, 'battlecard_details.threat_actor_details.affected_regions', []
    )
...
def create_threat_object_indicators(
    ...
    for threat_obj in threat_objects:
        ...
        relationships = []
        relationships += create_threat_object_relationships(
            threat_obj, name, threat_class, reliability
        )
        relationships += create_campaigns_relationships(threat_obj, name, threat_class, reliability)
        relationships += create_attack_patterns_relationships(
            threat_obj, name, threat_class, reliability
        )
        ...
        relationships += create_vulnerabilities_relationships(
            threat_obj, name, threat_class, reliability
        )
        ...
        fields = {
            ...
            'industrysectors': [
                string_to_table_header(industry)
                for industry in demisto.get(threat_obj, 'battlecard_details.industries', [])
            ],
            'primarymotivation': string_to_table_header(
                demisto.get(
                    threat_obj, 'battlecard_details.threat_actor_details.primary_motivation', ''
                )
            ),
            'publications': create_publications(threat_obj.get('publications', [])),
            'geocountry': demisto.get(
                threat_obj, 'battlecard_details.threat_actor_details.origin', ''
            ).upper(),
        ...
        location_indicators = create_location_indicators_and_relationships(
            threat_obj, name, reliability
        )
...
def create_context_data(
    response_data: dict[str, Any], indicator_value: str | None = None
) -> dict[str, Any]:
    """
    ...
    return {
        ...
        'VerdictCategories': list(
            {string_to_table_header(item) for item in response_data['verdict_categories']}
        ),
...
def construct_404_response(indicator_value: str, indicator_type: str) -> dict[str, Any]:
    ...
    return {
        ...
        'verdict_categories': [],
        'counts': [
            {'count_type': 'wf_sample', 'count_values': {'benign': 0, 'grayware': 0, 'malware': 0}}
        ],
        'first_seen': '',
        'last_seen': '',
        'seen_by': [],
...
def ip_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    create_threat_object_indicators_flag = argToBoolean(
        args.get('create_threat_object_indicators', False)
    )
    ...
    dbot_score = create_dbot_score(
        ip, DBotScoreType.IP, response_data['verdict'], client.reliability
    )
    ...
    ip_indicator = Common.IP(
        ip=ip, dbot_score=dbot_score, tags=tags, malware_family=malware_families
    )

    # Create relationships
    relationships = create_relationships(
        ip, FeedIndicatorType.IP, threat_objects, create_relationships_flag, client.reliability
    )
...
def domain_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    # Create DBotScore
    dbot_score = create_dbot_score(
        domain, DBotScoreType.DOMAIN, response_data['verdict'], client.reliability
    )
    ...
    domain_indicator = Common.Domain(
        domain=domain, dbot_score=dbot_score, tags=tags, malware_family=malware_families
    )
...
def url_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    # Create DBotScore
    dbot_score = create_dbot_score(
        url, DBotScoreType.URL, response_data['verdict'], client.reliability
    )
    ...
    url_indicator = Common.URL(
        url=url, dbot_score=dbot_score, tags=tags, malware_family=malware_families
    )
...
def file_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    # Create DBotScore
    dbot_score = create_dbot_score(
        file_hash, DBotScoreType.FILE, response_data['verdict'], client.reliability
    )
...
def main() -> None:
    ...
    create_threat_object_indicators = argToBoolean(
        params.get('create_threat_object_indicators', False)
    )
