...
def test_unit42_error_handler_with_request_id(mocker):
    ...
    mock_debug = mocker.patch("Unit42Intelligence.demisto.debug")
...
def test_client_retry_configuration(mocker):
    """
    Given:
    ...
    """
    mocker.patch("Unit42Intelligence.demisto.getLicenseID", return_value="test_license")

    client = Client(verify=False, proxy=False, reliability="A - Completely reliable")
...
def test_lookup_indicator_with_error_handler(client, mocker):
    """
    Given:
        - A Unit42Intelligence client
        - Mock API that returns 500 error with X-Request-ID
    When:
        - Calling lookup_indicator
    Then:
    ...
    mock_response.url = "https://api.example.com/indicators/1.2.3.4"
    mock_response.text = "Internal Server Error"
    ...
    with pytest.raises(Exception) as exc_info:
        client.lookup_indicator("1.2.3.4", "ip")

    assert "500" in str(exc_info.value) or "Error" in str(exc_info.value)


def test_ip_command_with_api_error(client, mocker):
    """
    Given:
        - A Unit42Intelligence client
    ...
    When:
        - Running ip_command
    Then:
        - Error is propagated with X-Request-ID information
    """
    mock_response = mocker.Mock()
    ...
    mocker.patch.object(client, "lookup_indicator", side_effect=Exception("HTTP 503 Error"))
...
def test_error_handler_rate_limit(mocker):
    """
    Given:
        - A mock response with 429 (Too Many Requests) status code
    When:
        - Calling unit42_error_handler function
    Then:
        - Returns error message indicating rate limiting
        - Includes X-Request-ID for tracking
    """
    mock_response = mocker.Mock()
    mock_response.status_code = 429
    mock_response.headers = {"X-Request-ID": "rate-limit-id", "Retry-After": "60"}
    mock_response.url = "https://api.example.com/test"
    ...
    assert "Error in API request [Status: 429]" in error_msg
    assert "[X-Request-ID: rate-limit-id]" in error_msg
