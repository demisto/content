...
def test_fetch_indicators_with_global_limit(client, mocker):
    """
    Given:
        - Client with limit parameter set
        - Multiple indicator types configured
    When:
        - Calling fetch_indicators with global limit
    Then:
        - Enforces global limit across all types
        - Fetches in priority order (Threat Objects → IPs → Domains → URLs → Files)
    ...
    params = {
        ...
        "feed_types": ["Indicators", "Threat Objects"],
        "indicator_types": ["IP", "Domain", "URL", "File"],
        "feedTags": [],
        "tlp_color": None,
    }
...
def test_fetch_indicators_remaining_limit_tracking(client, mocker):
    """
    Given:
        - Client with global limit
        - Multiple feed types enabled
    When:
        - Calling fetch_indicators
    Then:
        - Tracks remaining_limit correctly across types
