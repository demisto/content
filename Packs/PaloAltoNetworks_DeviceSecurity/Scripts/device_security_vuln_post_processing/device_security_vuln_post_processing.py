...
def device_security_resolve_vuln():
    ...
    for label in incident.get("labels", []):
        if label.get("type") == "zb_ticketid":
            zb_ticketid = label.get("value", "")
        if label.get("type") == "vulnerability_name":
            vulnerability_name = label.get("value", "")
    ...
    if is_error(result):
        return_error(f"Failed to resolve the vulnerability. Error: {get_error(result)}")
