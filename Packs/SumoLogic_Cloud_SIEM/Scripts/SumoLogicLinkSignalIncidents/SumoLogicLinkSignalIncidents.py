    ...
    if cur_incident is not None and cur_incident["rawType"] != "Sumo Logic Insight":
        ...
        if len(linked_signal_ids) > 0:
            ...
            call_result = demisto.executeCommand(
                "linkIncidents", {"incidentId": cur_incident.get("id"), "linkedIncidentIDs": ",".join(linked_signal_ids)}
            )
...
def main():
    try:
        ...
        return_error(f"Failed to execute SumoLogicLinkSignalIncidents. Error: {ex!s}")
