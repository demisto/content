    ...
    if cur_incident is not None and cur_incident["rawType"] != "Sumo Logic Insight":
        ...
        if linked_incidents:
            ...
            for signal_incident_id in linked_incidents:
                ...
                call_result = demisto.executeCommand(
                    "closeInvestigation",
                    {
                        "id": signal_incident_id,
                        "closeNotes": f"Close becaused Insight Incident {cur_incident.get('id')} is closed",
...
def main():
    try:
        ...
        return_error(f"Failed to execute SumoLogicCloseLinkSignalIncidents. Error: {ex!s}")
