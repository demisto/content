...
def test_insight_get_details(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, insight_get_details, insight_signal_to_readable
    ...
    args = {"insight_id": insight_id, "record_summary_fields": RECORD_SUMMARY_FIELDS_DEFAULT}
...
def test_insight_get_comments(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, insight_get_comments
...
def test_insight_add_comment(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, insight_add_comment
...
def test_signal_get_details(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, insight_signal_to_readable, signal_get_details
...
def test_entity_get_details(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, entity_get_details, entity_to_readable
...
def test_insight_search(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, insight_search, insight_signal_to_readable
...
def test_entity_search(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, entity_search, entity_to_readable
    ...
    args = {"query": 'hostname:matchesWildcard("*test*")', "limit": "2"}
...
def test_signal_search(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, insight_signal_to_readable, signal_search
    ...
    args = {"created": "All time", "contentType": "ANOMALY", "limit": "2"}
...
def test_insight_set_status(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, insight_set_status, insight_signal_to_readable
    ...
    args = {"insight_id": insight_id, "status": "closed", "resolution": "Resolved"}
...
def test_match_list_get(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, match_list_get
...
def test_match_list_update(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, get_update_result, match_list_update
...
def test_threat_intel_search_indicators(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, threat_intel_search_indicators
...
def test_threat_intel_get_sources(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, threat_intel_get_sources
...
def test_threat_intel_update_source(requests_mock):
    ...
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, get_update_result, threat_intel_update_source
...
def test_fetch_incidents_with_signals(requests_mock):
    ...
    url_combo = (
        f'{MOCK_URL}/sec/v1/signals?q=id:in("b4709a3c-c886-5cf0-9c98-88d976161edd","38c4bfea-23b5-5e4f-998c-5433dd093834'
    ...
    requests_mock.get(
        f'{MOCK_URL}/sec/v1/signals?q=id:in("2e4f64c2-e8c5-53ae-b0aa-01e5005155a7",' + '"ead54992-94f6-5aa3-b902-ec0fb5318dbc")',
        json=mock_response4,
    )
...
DEMISTO_ARGS = {"api_endpoint": MOCK_URL, "access_id": "SUMO_ACCESS_ID", "access_key": "SUMO_ACCESS_KEY"}


def get_demisto_arg(name):
    if name in DEMISTO_ARGS:
        return DEMISTO_ARGS[name]
    raise Exception(f"Test setup did not specify a Demisto argument named {name}.")


def test_get_remote_data_command(requests_mock, mocker):
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, get_remote_data_command
    ...
    assert response.entries[0]["Contents"]["closeNotes"].startswith(
        "Insight INSIGHT-220 was closed on Sumo Logic SIEM with resolution"
    )
    assert demisto.getParam("api_endpoint") == DEMISTO_ARGS["api_endpoint"]


def test_update_remote_system_command(requests_mock, mocker):
    from SumoLogicCloudSIEM import DEFAULT_HEADERS, Client, update_remote_system_command
    ...
    args = {
        ...
        "delta": {"closeNotes": "Closing incident", "closeReason": "For UT", "closingUserId": "admin", "runStatus": "completed"},
    }
