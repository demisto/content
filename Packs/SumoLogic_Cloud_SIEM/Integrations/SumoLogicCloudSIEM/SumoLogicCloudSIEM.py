...
def insight_search_with_lookback(
    ...
    """
    demisto.info("Step 1 - Initializing lookback parameters.")
    # Setup
    ...
    demisto.info(f"Step 2 - Pruning deduplication cache. Current cache size: {len(recent_ids)} IDs.")
    ...
    demisto.info(f"Step 2 - Pruning complete. New cache size: {len(pruned_ids)} IDs.")
    ...
    demisto.info(f"Step 3 - Calculated fetch start time: {created_time_str}")

    # Compose query
    user_query = args.get("query", "")
    lookback_query = f'created:>={created_time_str}'
    full_query = f'{lookback_query} {user_query}'.strip()
    demisto.info(f"Step 4 - Composed final query for API: {full_query}")
    ...
    demisto.info("Step 5 - Making API call to fetch insights.")
    resp_json = client.req("GET", "sec/v1/insights", query)
    fetched_objects = resp_json.get("objects", [])
    demisto.info(f"Step 6 - API call complete. Received {len(fetched_objects)} potential insights.")
    ...
    for i, insight in enumerate(fetched_objects):
        ...
        demisto.info(f"Step 7.{i+1} - Processing insight ID: {insight_id} with timestamp: {insight_created_str}")

        if not insight_id or not insight_created_str:
            demisto.info(f"Step 7.{i+1} - Skipping insight due to missing ID or creation time.")
            continue
        ...
        if insight_id in pruned_ids or insight_id in current_fetch_ids:
            demisto.info(f"Step 7.{i+1} - Skipping insight {insight_id} as it is a duplicate.")
            continue
        if insight_created_epoch < fetch_start_time:
            demisto.info(f"Step 7.{i+1} - Skipping insight {insight_id} as it is older than the fetch window.")
            continue
    ...
    demisto.info(f"Step 8 - Finished processing. Found {len(insights)} new insights.")
    ...
    demisto.info(f"Step 9 - Preparing next_run data. last_fetch will be set to: {latest_created_time}")
    ...
    demisto.info("Step 10 - Returning command results.")
    return CommandResults(
...
def main() -> None:  # pragma: no cover
    ...
    try:
        ...
        elif command == "sumologic-sec-insight-search-with-lookback":
            return_results(insight_search_with_lookback(client, demisto.args()))
