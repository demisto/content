...
def delegates_to_entry(title, mailbox, response):
    context = []
    for delegates_data in response:
        context.append({
            'DelegateEmail': delegates_data.get('delegateEmail'),
            'Mailbox': mailbox,
            'VerificationStatus': delegates_data.get('verificationStatus'),
        })

    headers = ['Mailbox', 'DelegateEmail', 'VerificationStatus']
    ...
    return {
        ...
        'EntryContext': {'Gmail.Delegates(val.delegateEmail && val.delegateEmail == obj.delegateEmail)': context}
    }
...
def list_delegates_command():
    args = demisto.args()
    user_id = args.get('userid')

    delegates = list_delegates(
        user_id)

    return delegates_to_entry('delegates:', user_id, delegates)


def list_delegates(user_id):
    command_args = {
        'userId': user_id,
    }
    service = get_service('gmail', 'v1', additional_scopes=['https://www.googleapis.com/auth/gmail.settings.basic'],
                          delegated_user=user_id)
    result = service.users().settings().delegates().list(**command_args).execute()
...
def main():
    ...
    COMMANDS = {
        ...
        'gmail-list-delegates': list_delegates_command,
    }
