...
def test_parse_privileges():
    ...
    privileges = [
        {"serviceId": "", "privilegeName": "name_no_id"},
        {"serviceId": "", "privilegeName": ""},
...
def test_dict_keys_snake_to_camelcase():
    ...
    dictionary = {"user_name": "user1", "user_id": "2"}
...
def test_labels_to_entry():
    ...
    labels = [
        {"id": "CHAT", "labelListVisibility": "labelHide", "messageListVisibility": "hide", "name": "CHAT", "type": "system"},
        {"id": "SENT", "labelListVisibility": "labelHide", "messageListVisibility": "hide", "name": "SENT", "type": "system"},
    ]
...
@pytest.mark.parametrize(
    "params_str, expected_result", [('{"varname" :{"value": "some value", "key": "context key"}}', {"varname": "some value"})]
)
...
@pytest.mark.parametrize(
    "title, response, expected_result",
    [("User 000000000000000000000:", input_data.response_test_users_to_entry, input_data.expected_result_test_users_to_entry)],
)
...
@pytest.mark.parametrize(
    "title, response, user_id, expected_result",
    [
        (
            "User johndoe@test.com:",
            [input_data.get_auto_replay_result],
            "johndoe@test.com",
            input_data.expected_result_test_autoreply_to_entry,
        )
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            "User Roles of 222222222222222222222:",
            input_data.get_user_role_mock_result,
...
@pytest.mark.parametrize(
    "title, response, expected_result",
    [("Tokens:", input_data.get_user_tokens_mock_result, input_data.expected_result_test_tokens_to_entry)],
)
...
@pytest.mark.parametrize(
    "title, response, to, emailfrom, cc, bcc, body, subject, expected_result",
    [
        (
            "Email sent:",
            [input_data.send_mail_mock_result],
            ...
            None,
            "ls",
            input_data.expected_result_test_sent_mail_to_entry,
        )
    ],
)
...
@pytest.mark.parametrize(
    "title, mailbox, response, expected_result",
    [("filters:", "1111111", input_data.list_filters_mock_result, input_data.expected_result_test_filters_to_entry)],
)
...
@pytest.mark.parametrize(
    "mailboxes, expected_result", [(input_data.list_mailboxes, input_data.expected_result_test_mailboxes_to_entry)]
)
...
@pytest.mark.parametrize(
    "title, raw_emails, format_data, mailbox, expected_result",
    [
        (
            ...
            "full",
            "11111",
            input_data.expected_result_test_emails_to_entry,
        )
    ],
)
...
def test_forwarding_address_list_command(mocker):
    ...
    mocker.patch.object(
        ...
        return_value={
            "forwardingAddresses": [
                {"forwardingEmail": "test1@gmail.com", "verificationStatus": "accepted"},
    ...
    assert result.raw_response == {
        "forwardingAddresses": [
            {"forwardingEmail": "test1@gmail.com", "userId": "111", "verificationStatus": "accepted"},
    ...
    assert (
        ...
        == '### Forwarding addresses list for: "111"\n|forwardingEmail|\
...
@pytest.mark.parametrize(
    "return_value_from_mocker_args, expected_result",
    [
        ({"user_id": "111", "forwarding_email": "test@gmail.com"}, input_data.expected_result_forwarding_address_get_command_1),
        ({"user_id": "111"}, input_data.expected_result_forwarding_address_get_command_2),
    ],
)
def test_forwarding_address_get_command(mocker, return_value_from_mocker_args, expected_result):
    ...
    mocker.patch.object(
        Gmail, "forwarding_address_get", return_value={"forwardingEmail": "test@gmail.com", "verificationStatus": "accepted"}
    )
    mocker.patch.object(
        Gmail,
        "forwarding_address_list",
        return_value={
            "forwardingAddresses": [
                {"forwardingEmail": "test1@gmail.com", "verificationStatus": "accepted"},
                {"forwardingEmail": "test2@gmail.com", "verificationStatus": "accepted"},
                {"forwardingEmail": "test3@gmail.com", "verificationStatus": "accepted"},
            ]
        },
    )
...
def test_forwarding_address_add(mocker):
    ...
    mocker.patch.object(
        Gmail,
        "forwarding_address_add",
        return_value=(
            {"forwardingEmail": "test@gmail.com", "verificationStatus": "accepted", "userId": "me"},
            False,
            {"forwardingEmail": "test@gmail.com", "errorMessage": "", "userId": "me"},
        ),
    )
...
def test_forwarding_address_update(mocker):
    ...
    mocker.patch.object(
        Gmail,
        "forwarding_address_update",
        return_value=(
            {"enabled": True, "emailAddress": "test@gmail.com", "disposition": "markRead", "userId": "me"},
            None,
            {"emailAddress": "test@gmail.com", "errorMessage": None, "userId": "me"},
        ),
    )
    mocker.patch.object(
        demisto, "args", return_value={"user_id": "me", "forwarding_email": "test@gmail.com", "disposition": "markRead"}
    )
...
enabled|\n|---|---|---|---|\n| test@gmail.com | me | markRead | true |\n'
    ...
    assert result.raw_response == [
        {"enabled": True, "forwardingEmail": "test@gmail.com", "disposition": "markRead", "userId": "me"}
    ]
...
@pytest.mark.parametrize(
    "return_value_get_last_run, expected_result",
    [
...
@pytest.mark.parametrize(
    "email_data, expected_occurred, expected_occurred_is_valid",
    [
        # no internalDate
...
def test_get_date_from_email_header():
    ...
    result = get_date_from_email_header(
        "by 2002:a9d:4b03:: with SMTP id q3mr13206164otf.88.1608581517297;\
            Mon, 21 Dec 2020 12:11:57 -0800 (PST)"
    )
    assert str(result) == "2020-12-21 12:11:57-08:00"
    assert result == datetime.datetime(
        2020, 12, 21, 12, 11, 57, tzinfo=datetime.timezone(datetime.timedelta(days=-1, seconds=57600))
    )
...
def test_get_date_isoformat_server():
    ...
    date = get_date_isoformat_server(
        datetime.datetime(2022, 11, 9, 22, 45, 44, tzinfo=datetime.timezone(datetime.timedelta(days=-1, seconds=68400)))
    )
...
def test_filter_by_fields(full_mail: dict[str, str], filter_fields: list[str], expected_result: dict[str, str]):
    """
    Given:
...
def test_search_command_with_next_page_token(mocker):
    ...
    assert "GmailMailsNextPageToken" in result["EntryContext"]
    assert result["EntryContext"]["GmailMailsNextPageToken"] == "next_page_token_123"
