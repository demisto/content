...
class Client(BaseClient):
    ...
    def get_project_list_request(self, page: int, page_size: int, project_key: str = None) -> dict:
        ...
        """
        params = {"page": page, "pagelen": page_size}
        if not project_key:
    ...
    def branch_create_request(self, name: str, target_branch: str, repo: str) -> dict:
        ...
        """
        url_suffix = f"/repositories/{self.workspace}/{repo}/refs/branches"
        body = {"target": {"hash": target_branch}, "name": name}
    ...
    def commit_create_request(self, body: dict, repo: str) -> Response:
        ...
        return self._http_request(method="POST", url_suffix=url_suffix, data=body, resp_type="response")

    def commit_list_request(
        self, repo: str, file_path: str, page: int, page_size: int, included_list: list, excluded_list: list = []
    ) -> dict:
        ...
        Returns:
            A response object in a form of a dictionary.
        """
        params = {"path": file_path, "page": page, "pagelen": page_size}
    ...
    def pull_request_list_request(self, repo: str, pr_id: str, page: int, page_size: int, state: str) -> dict:
        ...
        Returns:
            A response object in a form of a dictionary.
        """
        params: dict = {"page": page, "pagelen": page_size}
        if state:
            params["state"] = state
        url_suffix = f"/repositories/{self.workspace}/{repo}/pullrequests"
        if pr_id:
...
def create_pull_request_body(
    title: str, source_branch: str, destination_branch: str, reviewer_id: str, description: str, close_source_branch: str
) -> dict:
...
def issue_partial_result(response: dict) -> dict:
    ...
    return {
        ...
        "content": {"raw": response.get("content", {}).get("raw")},
        "reporter": {
...
def pull_request_partial_result(response: dict) -> dict:
    ...
    return {
        ...
        "created_on": response.get("created_on"),
        "updated_on": response.get("updated_on"),
        "destination": {"branch": {"name": response.get("destination", {}).get("branch", {}).get("name")}},
        "source": {"branch": {"name": response.get("source", {}).get("branch", {}).get("name")}},
    }
...
def project_list_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name=readable_name, t=human_readable, removeNull=True, headers=headers)
    outputs = results
    if partial_response:
        outputs = []
        for result in results:
            temp = {
                ...
                "workspace": {"name": result.get("workspace", {}).get("name"), "slug": result.get("workspace", {}).get("slug")},
            }
            outputs.append(temp)
...
def open_branch_list_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name="Open Branches", t=human_readable, removeNull=True, headers=headers)
    outputs = results
    if partial_response:
        outputs = []
        for result in results:
...
def branch_get_command(client: Client, args: dict) -> CommandResults:
    ...
    human_readable = {
        ...
        "LastCommitCreatedBy": response.get("target", {}).get("author", {}).get("user", {}).get("display_name"),
        "LastCommitCreatedAt": response.get("target", {}).get("date"),
    }
    headers = ["Name", "LastCommitCreatedBy", "LastCommitCreatedAt", "LastCommitHash"]
    readable_output = tableToMarkdown(
        name=f"Information about the branch: {branch_name}", t=human_readable, removeNull=True, headers=headers
    )
    outputs = response
    if partial_response:
...
def commit_create_command(client: Client, args: dict) -> CommandResults:
    ...
    if file_name and entry_id:
        ...
        with open(file_path, "rb") as f:
            file_content = f.read()
    body = {"message": message, "branch": branch, file_name: file_content}
...
def commit_list_command(client: Client, args: dict) -> CommandResults:
    ...
    for value in results:
        d = {
            "Author": value.get("author").get("raw"),
    ...
    readable_output = tableToMarkdown(name="The list of commits", t=human_readable, removeNull=True, headers=headers)
    outputs = results
    if partial_response:
        outputs = []
        for res in results:
...
def file_delete_command(client: Client, args: dict) -> CommandResults:
    ...
    author_name = args.get("author_name", None)
    author_email = args.get("author_email", None)
    body = {"message": message, "branch": branch, "files": file_name}
    if author_name and author_email:
        body["author"] = f"{author_name} <{author_email}>"
    elif (author_name and not author_email) or (not author_name and author_email):
        raise Exception("Both author email and author name should be given")
...
def raw_file_get_command(client: Client, args: dict) -> List[CommandResults]:
    ...
    try:
        commit_list = client.commit_list_request(
            repo=repo, file_path=file_path, page=1, page_size=100, included_list=including_list
        )
    except DemistoException as e:
        m_json = get_exception_message(e)
    ...
    output = {"file_path": file_path, "file_content": response.text}
    file_ = fileResult(filename=file_path, data=response.text, file_type=entryTypes["entryInfoFile"])
    return [
        CommandResults(
            readable_output=f'The content of the file "{file_path}" is: {response.text}',
...
def issue_create_command(client: Client, args: dict) -> CommandResults:
    ...
    partial_response = argToBoolean(args.get("partial_response", True))
    body = {"title": title, "state": state, "kind": issue_type, "priority": priority}
    if content:
        body["content"] = {"raw": content}
    if assignee_id:
        body["assignee"] = {"account_id": assignee_id, "username": assignee_user_name}
    response = client.issue_create_request(repo, body)
    outputs = response
    if partial_response:
        outputs = issue_partial_result(response)
    return CommandResults(
        readable_output=f'The issue "{title}" was created successfully',
...
def issue_list_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name=hr_title, t=human_readable, removeNull=True, headers=headers)
    outputs = results
    if partial_response:
        outputs = []
        for res in results:
...
def issue_update_command(client: Client, args: dict) -> CommandResults:
    ...
    if partial_response:
        outputs = issue_partial_result(response)
    return CommandResults(
        readable_output=f'The issue with id "{issue_id}" was updated successfully',
        outputs_prefix="Bitbucket.Issue",
        outputs=outputs,
        raw_response=response,
        outputs_key_field="id",
...
def pull_request_create_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output="The pull request was created successfully",
...
def pull_request_update_command(client: Client, args: dict) -> CommandResults:
    ...
    if partial_response:
        outputs = pull_request_partial_result(response)
    return CommandResults(
        readable_output=f"The pull request {pull_request_id} was updated successfully",
        outputs_prefix="Bitbucket.PullRequest",
        outputs=outputs,
        raw_response=response,
        outputs_key_field="id",
...
def pull_request_list_command(client: Client, args: dict) -> CommandResults:
    ...
    headers = ["Id", "Title", "Description", "SourceBranch", "DestinationBranch", "State", "CreatedBy", "CreatedAt", "UpdatedAt"]
    readable_output = tableToMarkdown(name=hr_title, t=human_readable, removeNull=True, headers=headers)
    outputs = results
    if partial_response:
        outputs = []
...
def issue_comment_create_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output="The comment was created successfully",
...
def workspace_member_list_command(client: Client, args: dict) -> CommandResults:
    ...
    human_readable = []
    for value in results:
        d = {"Name": value.get("user").get("display_name"), "AccountId": value.get("user").get("account_id")}
        human_readable.append(d)
    headers = ["Name", "AccountId"]
    readable_output = tableToMarkdown(
        name="The list of all the workspace members", t=human_readable, removeNull=True, headers=headers
    )
    outputs = results
    if partial_response:
        outputs = []
        for res in results:
            temp = {
                ...
                "workspace": {"name": res.get("workspace", {}).get("name"), "slug": res.get("workspace", {}).get("slug")},
            }
            outputs.append(temp)
    return CommandResults(
        readable_output=readable_output,
