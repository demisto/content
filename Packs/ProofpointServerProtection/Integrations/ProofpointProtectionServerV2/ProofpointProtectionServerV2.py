...
class Client(BaseClient):
    ...
    @logger
    def list_quarantined_messages_request(
        self,
        from_: Optional[str] = None,
    ...
    def get_user(self, email_or_uid: str) -> Response:
        return self._http_request(method="GET", url_suffix=f"/enduser/{email_or_uid}")

    @logger
    def create_user(self, email: str, fields: dict, attributes: dict) -> Response:
        json_data = {"attributes": attributes}
        json_data.update(fields)
        return self._http_request(method="POST", url_suffix=f"/enduser/{email}", json_data=json_data, ok_codes=(200, 400))

    @logger
    def modify_user(self, email_or_uid: str, fields: dict, attributes: dict) -> Response:
        json_data = {"attributes": attributes}
        json_data.update(fields)
        return self._http_request(method="PUT", url_suffix=f"/enduser/{email_or_uid}", json_data=json_data)

    @logger
    def delete_user(self, email_or_uid: str) -> Response:
        return self._http_request(method="DELETE", url_suffix=f"/enduser/{email_or_uid}", ok_codes=(200, 404))
...
def smart_search(client: Client, args: dict[str, Any]) -> CommandResults:
    assert (
        start_time := parse(args.get("start_time", "24 hours"), settings={"RETURN_AS_TIMEZONE_AWARE": True})
    ), f"Failed parsing start time: {args.get('start_time')}"
    if end_time := args.get("end_time"):
        assert (
            end_time := parse(end_time, settings={"RETURN_AS_TIMEZONE_AWARE": True})
...
def release_message(client: Client, args: dict[str, Any]) -> CommandResults:
    result = str(
        client.quarantine_action_request(
            action="release",
            folder=args.get("folder_name"),
...
def get_user(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    if email or uid:
        ...
        command_results_args = {"readable_output": "Please specify an email or uid"}
    return CommandResults(**command_results_args)
    ...
    if isinstance(result, dict):
        if result.get("status") == 400:
            if result.get("errors", {}).get("invalidarguments", [])[0].get("error") == "User already exists":
                command_results_args: dict[str, Any] = {"readable_output": "User already exists"}
            else:
        ...
        if isinstance(result, dict):
            if result.get("status") == 404:
                if result.get("errors", {}).get("invalidarguments", [])[0].get("error") == "User not found":
                    command_results_args: dict[str, Any] = {"readable_output": "User not found"}
                else:
