...
import pytest
from CofenseVision import (
    API_ENDPOINTS,
    ERROR_MESSAGE,
    IOC_OUTPUT_PREFIX,
    QUARANTINE_JOB_OUTPUT_PREFIX,
    SEARCH_OUTPUT_PREFIX,
    STATUS,
    SUPPORTED_CRITERIA,
    SUPPORTED_HASH,
    SUPPORTED_HASH_VALUE_FORMAT,
    SUPPORTED_HEADERS_FORMAT,
    SUPPORTED_QUARANTINE_EMAILS_FORMAT,
    SUPPORTED_SORT,
    SUPPORTED_SORT_FORMAT,
...
def test_cofense_message_metadata_get_command_when_object_not_found(mock_client, requests_mock):
    ...
    params = {"internet_message_id": "test_id", "recipient_address": "test_mail_address"}
...
def test_cofense_message_metadata_command_success(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_metadata_response_success.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_metadata_hr_success.md")) as file:
        hr_output = file.read()
...
@pytest.mark.parametrize(
    ...
    [
        (
            ERROR_MESSAGE["MISSING_REQUIRED_PARAM"].format("internet_message_id"),
            {"internet_message_id": ""},
        ),
        (
...
def test_message_token_get_command_when_valid_arguments_provided(mock_client, requests_mock):
    ...
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_token_hr_success.md")) as file:
        expected_hr = file.read()

    expected_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_token_response_success.json")
    )
...
@pytest.mark.parametrize(
    "args,error_msg",
    [
        ({"page": "-5"}, ERROR_MESSAGE["INVALID_PAGE_VALUE"]),
        ({"page": "$"}, '"$" is not a valid number'),
        ({"size": "0"}, ERROR_MESSAGE["INVALID_PAGE_SIZE_RANGE"]),
        ({"size": "2525"}, ERROR_MESSAGE["INVALID_PAGE_SIZE_RANGE"]),
        ({"include_status": "dummy"}, ERROR_MESSAGE["UNSUPPORTED_FIELD"].format("dummy", "status", "status", ", ".join(STATUS))),
        ({"exclude_status": "dummy"}, ERROR_MESSAGE["UNSUPPORTED_FIELD"].format("dummy", "status", "status", ", ".join(STATUS))),
        (
        ...
        (
            ...
            ERROR_MESSAGE["UNSUPPORTED_FIELD"].format("ascq", "sort order", "sort order", ", ".join(SUPPORTED_SORT["order_by"])),
        ),
        (
            {"sort": "idq:asc"},
            ERROR_MESSAGE["UNSUPPORTED_FIELD"].format(
                "idq", "property name", "property name", ", ".join(SUPPORTED_SORT["quarantine_jobs_list"])
            ),
        ),
...
def test_cofense_quarantine_jobs_list_command_valid_params_provided(mock_client, requests_mock):
    ...
    expected_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_quarantine_jobs_response_success.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_quarantine_jobs_hr_success.md")) as file:
        expected_hr = file.read()
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            ERROR_MESSAGE["INVALID_FORMAT"].format("<test_id>", "quarantine_emails", SUPPORTED_QUARANTINE_EMAILS_FORMAT),
        ),
        (
            {"quarantine_emails": "<test_id>:dummy@xyz.com:1"},
            ERROR_MESSAGE["INVALID_FORMAT"].format(
                "<test_id>:dummy@xyz.com:1", "quarantine_emails", SUPPORTED_QUARANTINE_EMAILS_FORMAT
            ),
        ),
        (
            {"quarantine_emails": "<test_id1>:dummy@xyz.com,<test_id2>"},
            ERROR_MESSAGE["INVALID_FORMAT"].format("<test_id2>", "quarantine_emails", SUPPORTED_QUARANTINE_EMAILS_FORMAT),
        ),
...
def test_cofense_quarantine_job_create_command_success(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/create_quarantine_job_response_success.json")
    )

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "./test_data/create_quarantine_job_hr_success.md")
    ) as file:
        hr_output = file.read()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ERROR_MESSAGE["UNSUPPORTED_FIELD"].format(
                "invalidProperty", "property name", "property name", ", ".join(SUPPORTED_SORT["message_searches_list"])
            ),
        ),
        ({"sort": "id:desc:test"}, ERROR_MESSAGE["INVALID_FORMAT"].format("id:desc:test", "sort", SUPPORTED_SORT_FORMAT)),
        (
            {"sort": "id:invalidOrder"},
            ERROR_MESSAGE["UNSUPPORTED_FIELD"].format(
                "invalidOrder", "sort order", "sort order", ", ".join(SUPPORTED_SORT["order_by"])
            ),
        ),
    ],
)
...
def test_cofense_message_searches_list_command_success(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_message_searches_response_success.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_message_searches_hr_success.md")) as file:
        hr_output = file.read()
...
def test_cofense_last_ioc_get_command_success(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_last_ioc_response_success.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_last_ioc_hr_success.md")) as file:
        hr_output = file.read()
...
def test_cofense_message_search_get_command_success(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_search_response_success.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_search_hr_success.md")) as file:
        hr_output = file.read()
...
def test_cofense_quarantine_job_get_command_when_valid_arguments_provided(mock_client, requests_mock):
    ...
    mock_arguments = {"id": "265"}
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_quarantine_job_response_success.json")
    )
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_quarantine_job_hr_success.md")) as file:
        expected_hr = file.read()
    mock_get_quarantine_job = API_ENDPOINTS["QUARANTINE_JOB"] + "/" + mock_arguments["id"]
...
def test_quarantine_job_approve_command_when_valid_arguments_provided(mock_client, requests_mock):
    ...
    mock_quarantine_job_approve_endpoint = MOCK_BASE_URL + API_ENDPOINTS["APPROVE_QUARANTINE_JOB"].format(1) + "?messageCount=10"
...
def test_cofense_quarantine_job_delete_command_when_valid_arguments_provided(mock_client, requests_mock):
    ...
    mock_arguments = {"id": "265"}
    expected_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/delete_quarantine_job_response_success.json")
    )
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/delete_quarantine_job_hr_success.md")) as file:
        expected_hr = file.read()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ERROR_MESSAGE["UNSUPPORTED_FIELD"].format(
                "invalidProperty", "property name", "property name", ", ".join(SUPPORTED_SORT["message_search_result_get"])
            ),
        ),
        (
        ...
        ({"id": 1, "sort": ","}, ERROR_MESSAGE["INVALID_FORMAT"].format("None", "sort", SUPPORTED_SORT_FORMAT)),
        (
...
def test_cofense_message_search_results_get_command_success(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_search_results_response_success.json")
    )

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_search_results_hr_success.md")
    ) as file:
        hr_output = file.read()
...
def test_cofense_message_search_results_get_command_when_empty_results(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_search_results_empty_message_search_result.json"
        )
    )
    ...
    with open(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/get_message_search_results_empty_message_search_result_hr.md"
        )
    ) as file:
        hr_output = file.read()
...
@pytest.mark.parametrize(
    "args, err_msg",
    [
        ({"source": "", "id": "oxffpd"}, ERROR_MESSAGE["MISSING_REQUIRED_PARAM"].format("source")),
        ({"source": "Vision-UI", "id": ""}, "id is a required parameter. Please provide correct value."),
    ],
)
...
def test_cofense_ioc_delete_command_when_valid_args_provided(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/delete_ioc_response_success.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/delete_ioc_hr_success.md")) as file:
        hr_output = file.read()
...
def test_cofense_quarantine_job_stop_command_when_valid_arguments_provided(mock_client, requests_mock):
    ...
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/stop_quarantine_job_hr_success.md")) as file:
        expected_hr = file.read()
    mock_get_quarantine_job = MOCK_BASE_URL + API_ENDPOINTS["STOP_QUARANTINE_JOB"].format("265")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"attachment_names": "test1, test2, test3, test4"}, ERROR_MESSAGE["INVALID_SEARCH_LENGTH"].format("attachment_names")),
        ({"attachment_hashes": "test1, test2, test3, test4"}, ERROR_MESSAGE["INVALID_SEARCH_LENGTH"].format("attachment_hashes")),
        (
        ...
        ({"whitelist_urls": "test1, test2, test3, test4"}, ERROR_MESSAGE["INVALID_SEARCH_LENGTH"].format("whitelist_urls")),
        ({"headers": "test1, test2, test3, test4"}, ERROR_MESSAGE["INVALID_SEARCH_LENGTH"].format("headers")),
        (
            {"attachment_hash_match_criteria": "test"},
            ERROR_MESSAGE["UNSUPPORTED_FIELD"].format(
                "test", "attachment_hash_match_criteria", "attachment_hash_match_criteria", SUPPORTED_CRITERIA
            ),
        ),
        (
            {"domain_match_criteria": "test"},
            ERROR_MESSAGE["UNSUPPORTED_FIELD"].format(
                "test", "domain_match_criteria", "domain_match_criteria", SUPPORTED_CRITERIA
            ),
        ),
        ...
        ({"headers": "test"}, ERROR_MESSAGE["INVALID_FORMAT"].format("test", "headers", SUPPORTED_HEADERS_FORMAT)),
        (
            {"attachment_hashes": "test_type:test_value"},
            ERROR_MESSAGE["UNSUPPORTED_FIELD"].format("test_type", "hash", "hash", SUPPORTED_HASH),
        ),
        ({"attachment_hashes": "MD5:test_value"}, ERROR_MESSAGE["INVALID_ARGUMENT"].format("test_value", "MD5")),
    ],
)
...
def test_cofense_message_search_create_command_when_422_error_for_header_key(mock_client, requests_mock):
    ...
    mock_response = {
        ...
        "message": "Validation failed for request data",
        "details": [
            "Field error in object 'search' on field 'headers[0]': header 'test' "
...
def test_cofense_message_search_create_command_success(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/create_message_search_response_success.json")
    )

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "./test_data/create_message_search_hr_success.md")
    ) as file:
        hr_output = file.read()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"source": "Vision-UI"}, ERROR_MESSAGE["MISSING_REQUIRED_PARAM"].format("threat_type")),
        (
            {
                "source": "Vision-UI",
                "iocs_json": '{"threat_type": "Domain", \
        ...
        (
            {
                "source": "Vision-UI",
                "iocs_json": '{"threat_type": "dummy", \
                          "threat_value": "abc.com", "threat_level": "Low", "source_id": "oxcff", \
                          "created_at": "2022-07-07", "updated_at": "2022-07-08", \
                          "requested_expiration": "2022-09-09"}',
            },
            ERROR_MESSAGE["UNSUPPORTED_FIELD"].format("dummy", "threat type", "threat type", THREAT_TYPES),
        ),
        (
            {
                "source": "Vision-UI",
...
def test_update_iocs_command_when_valid_args_provided(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/update_iocs_response_success.json")
    )
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/update_iocs_hr_success.md")) as file:
        expected_hr = file.read()
...
@pytest.mark.parametrize(
    "args, err_msg",
    [
        ({"id": "", "expires_at": "1 day"}, ERROR_MESSAGE["MISSING_REQUIRED_PARAM"].format("id")),
        (
            {"id": "11111111111111111111111111111111", "expires_at": ""},
            ERROR_MESSAGE["MISSING_REQUIRED_PARAM"].format("expires_at"),
        ),
        ({"id": "11111111111111111111111111111111", "expires_at": "test"}, 'Invalid date: "expires_at"="test"'),
    ],
)
...
def test_update_ioc_command_when_valid_args_provided(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/update_ioc_response_success.json")
    )
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/update_ioc_hr_success.md")) as file:
        expected_hr = file.read()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ERROR_MESSAGE["UNSUPPORTED_FIELD_FOR_IOCS_LIST"].format(
                "invalidProperty", "property name", "property name", ", ".join(SUPPORTED_SORT["iocs_list"])
            ),
...
def test_iocs_list_command_when_valid_response_returned(mock_client, requests_mock):
    ...
    mock_arguments = {"source": "test-source-1", "page": "5", "size": "5", "since": TEST_DAY_DATA, "include_expired": "True"}
    mock_request_url = MOCK_BASE_URL + API_ENDPOINTS["GET_IOCS"]
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_iocs_response_success.json")
    )
    requests_mock.get(mock_request_url, json=mock_response.get("rawResponse"), status_code=200)

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_iocs_hr_success.md")) as file:
...
def test_cofense_searchable_headers_list_command_success(mock_client, requests_mock):
    ...
    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_searchable_header_hr_success.md")
    ) as file:
        hr_output = file.read()
...
def test_cofense_ioc_get_command_success(mock_client, requests_mock):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_ioc_response_success.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_ioc_hr_success.md")) as file:
        hr_output = file.read()
