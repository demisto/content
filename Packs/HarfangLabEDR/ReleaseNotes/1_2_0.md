...
- **Hurukai alert**, no change.
- **Hurukai threat**, correspond to a set of security events since they belong to the same attack context.
...
- Added support for the mirroring of security events and threats between XSOAR and HarfangLab EDR. For now, are supported:
  - Incident status for security event and threat.
...
Layout for Hurukai threats. The new layout allows to display both security events or threats.
...
Mapping for mirroring XSOAR incidents between an XSOAR instance and the HarfangLab EDR.
