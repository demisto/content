...
TC_INDICATOR_TO_XSOAR_INDICATOR = {
    ...
    "Campaign": {"dateAdded": "firstseenbysource", "lastModified": "updateddate", "name": "name"},
    "Course of Action": {"dateAdded": "firstseenbysource", "lastModified": "updateddate", "name": "name"},
    "Intrusion Set": {"dateAdded": "firstseenbysource", "lastModified": "updateddate", "name": "name"},
    "Malware": {"dateAdded": "firstseenbysource", "lastModified": "updateddate", "name": "name"},
    "Report": {"dateAdded": "firstseenbysource", "lastModified": "updateddate", "name": "name", "publishDate": "published"},
    "Tool": {"dateAdded": "firstseenbysource", "lastModified": "updateddate", "name": "name"},
    "CVE": {"dateAdded": "firstseenbysource", "lastModified": "updateddate", "name": "name", "publishDate": "published"},
}
...
def create_rk_grid_field(indicator: dict):
    ...
    """
    key_value = [{"name": indicator.get("Key Name"), "type": indicator.get("Value Name"), "data": indicator.get("Key Type")}]
...
class Client(BaseClient):
    ...
    def create_header(self, url_suffix: str, full_url: str, method: Method) -> dict:
        ...
        return {"Authorization": f"TC {self.api_id}:{api_hash}", "Timestamp": str(timestamp), "Content-Type": "application/json"}
...
def fetch_indicators_command(client: Client, params: dict, last_run: dict) -> tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:  # noqa  # pragma: no cover
    ...
    try:
        while True:
            if indicators_next_link or groups_next_link:
                if indicators_next_link:
                    ...
                    response, _, indicators_next_link = client.make_request(
        ...
        demisto.error(
            f"Got an error in the fetch loop. Returning {len(groups)} groups + {len(indicators)} indicators. error: {e!s}"
        )
...
def get_indicators_command(client: Client, args: dict) -> dict:  # type: ignore # pragma: no cover
    ...
    if not tql:
        ...
        active_only = (
            f'AND indicatorActive EQ {args.get("active_indicators")} ' if argToBoolean(args.get("active_indicators")) else ""
        )
    ...
    if status == "Success":
        ...
        readable_output: str = tableToMarkdown(name=f"{INTEGRATION_NAME} - Indicators", t=t, removeNull=True)  # type: ignore # noqa
...
def get_owners_command(client: Client, args: dict) -> COMMAND_OUTPUT:  # pragma: no cover
    ...
    readable_output: str = tableToMarkdown(name=f"{INTEGRATION_NAME} - Owners", t=list(response))
...
def main():  # pragma: no cover # noqa
    ...
    try:
        ...
        return_error(f"Integration {INTEGRATION_NAME} Failed to execute {command} command. Error: {e!s}")
