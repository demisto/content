...
def convert_position(_id, _type, xy_arr):
    return {"id": _id, "type": _type, "position": {"x": xy_arr[0], "y": xy_arr[1]}}
    ...
    for indicator in indicators:
        for investigation_id in indicator.get("investigationIDs", []):
            if investigation_id in incident_ids + [current_incident_id]:
                incident_to_indicator_connections.append(get_incident_to_indicator_connection(investigation_id, indicator["id"]))
...
def main():
    ...
    res = demisto.executeCommand(
        "drawCanvas", {"canvas": canvas, "canvasLinks": connections, "id": current_incident_id, "overrideUserCanvas": override}
    )
    if res is None:
