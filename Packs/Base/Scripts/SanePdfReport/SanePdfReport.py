...
def find_zombie_processes():
    ...
    ps_out = subprocess.check_output(
        ["ps", "-e", "-o", "pid,ppid,state,stime,cmd"], stderr=subprocess.STDOUT, universal_newlines=True
    )
...
def main():  # pragma: no cover
    ...
    try:
        sane_json_b64 = demisto.args().get("sane_pdf_report_base64", "").encode("utf-8")
        ...
        extra_cmd = (
            f"{orientation} {resourceTimeout} {reportType} "
        ...
        with tempfile.TemporaryDirectory(suffix="sane-pdf", ignore_cleanup_errors=True) as tmpdir:  # type: ignore[call-overload]
            ...
            cmd = ["./reportsServer", input_file, output_file, dist_dir] + shlex.split(extra_cmd)
            ...
            out = subprocess.check_output(cmd, cwd=WORKING_DIR, stderr=subprocess.STDOUT)
            demisto.debug(f"Sane-pdf output: {out!s}")

            with open(output_file, "rb") as f:
                encoded = base64.b64encode(f.read()).decode("utf-8", "ignore")

            return_outputs(readable_output="Successfully generated pdf", outputs={}, raw_response={"data": encoded})
