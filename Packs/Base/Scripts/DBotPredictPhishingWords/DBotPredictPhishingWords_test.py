...
from DBotPredictPhishingWords import get_model_data, main, predict_phishing_words
...
def executeCommand(command, args=None):
    ...
    if command == "getList":
        ...
        return [{"Contents": {"modelData": "ModelDataML", "model": {"type": {"type": ""}}}, "Type": "note"}]
        ...
        return [{"Contents": TOKENIZATION_RESULT, "Type": "note"}]
...
def test_predict_phishing_words(mocker):
    global TOKENIZATION_RESULT
    d = {"Label": "Valid", "Probability": 0.7, "PositiveWords": ["word1"], "NegativeWords": ["word2"]}
    ...
    mocker.patch.object(phishing_mock, "explain_model_words", return_value=d, create=True)

    TOKENIZATION_RESULT = {
        "originalText": "word1 word2 word3",
    ...
    correct_res = {
        "OriginalText": concatenate_subject_body(email_subject, email_body),
        "Probability": 0.7,
        "NegativeWords": ["word2"],
...
def test_predict_phishing_words_hashed(mocker):
    global TOKENIZATION_RESULT
    d = {"Label": "Valid", "Probability": 0.7, "PositiveWords": ["23423"], "NegativeWords": ["432432"]}
    phishing_mock = PhishingModelMock()
    mocker.patch.object(demisto, "incidents", return_value=[{"isPlayground": True}])
...
def test_predict_phishing_words_tokenization_by_character(mocker):
    ...
    positive_tokens = ["t", "i"]
    negative_tokens = []
    d = {"Label": "Valid", "Probability": 0.7, "PositiveWords": positive_tokens, "NegativeWords": negative_tokens}
    mocker.patch.object(phishing_mock, "explain_model_words", return_value=d, create=True)
    ...
    assert res["Contents"] == {
        ...
        "PositiveWords": [w for w in original_text.split() if any(pos_token in w for pos_token in positive_tokens)],
        "Label": "Valid",
    }
...
def test_predict_phishing_words_tokenization_by_character_hashed(mocker):
    ...
    TOKENIZATION_RESULT = {
        ...
        "wordsToHashedTokens": {w: [hash_token(t) for t in token_list] for w, token_list in original_words_to_tokes.items()},
    }
    ...
    assert res["Contents"] == {
        ...
        "NegativeWords": negative_tokens,
        "TextTokensHighlighted": correct_highlighted,
        "PositiveWords": [w for w in original_text.split() if any(unhash_token(pos_token) in w for pos_token in positive_tokens)],
        "Label": "Valid",
    }
...
def test_main(mocker):
    ...
    args = {
        ...
        "modelStoreType": "list",
        "emailSubject": "word1",
        "emailBody": "word2 word3",
        "minTextLength": "0",
        "labelProbabilityThreshold": "0",
        "wordThreshold": "0",
        "topWordsLimit": "10",
        "returnError": "true",
    }
    ...
    TOKENIZATION_RESULT = {
        "originalText": "{} {}".format(args["emailSubject"], args["emailBody"]),
    ...
    res = main()
    correct_res = {
        "OriginalText": concatenate_subject_body(args["emailSubject"], args["emailBody"]),
        "Probability": 0.7,
        "NegativeWords": ["word2"],
    ...
    TOKENIZATION_RESULT = {
        "originalText": concatenate_subject_body(args["emailSubject"], args["emailBodyHTML"]),
...
def test_no_positive_words(mocker):
    ...
    global TOKENIZATION_RESULT
    phishing_mock = PhishingModelMock()
    d = {"Label": "Valid", "Probability": 0.7, "PositiveWords": [], "NegativeWords": ["word2"]}
    args = {
        "modelName": "modelName",
        "modelStoreType": "list",
        "emailSubject": "word1",
