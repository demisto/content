...
def test_threshold_found_0(mocker):
    ...
    entry = find_threshold(y_pred_all_classes=y_pred, y_true=y_true, customer_target_precision=0, target_recall=0)
    assert abs(entry["Contents"]["threshold"] - 0.7) < 10**-2


def test_threshold_found_1(mocker):
    global y_true, y_pred
    entry = find_threshold(y_pred_all_classes=y_pred, y_true=y_true, customer_target_precision=0.63, target_recall=0)
    assert abs(entry["Contents"]["threshold"] - 0.7) < 10**-2


def test_threshold_found_2(mocker):
    global y_true, y_pred
    entry = find_threshold(y_pred_all_classes=y_pred, y_true=y_true, customer_target_precision=0.7, target_recall=0)
    assert abs(entry["Contents"]["threshold"] - 0.8) < 10**-2


def test_threshold_found_3(mocker):
    global y_true, y_pred
    entry = find_threshold(y_pred_all_classes=y_pred, y_true=y_true, customer_target_precision=0.875, target_recall=0)
    assert abs(entry["Contents"]["threshold"] - 0.8) < 10**-2


def test_no_existing_threshold(mocker):
    entry = find_threshold(y_pred_all_classes=y_pred, y_true=y_true, customer_target_precision=0.9, target_recall=0)
    assert abs(entry["Contents"]["threshold"] - 0.8) < 10**-2
...
def test_predictions_are_correct_and_all_equals_one_prob(mocker):
    ...
    entry = find_threshold(y_pred_all_classes=y_pred, y_true=y_true, customer_target_precision=0.6, target_recall=0)
