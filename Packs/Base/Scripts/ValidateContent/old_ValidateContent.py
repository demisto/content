...
def get_file_name_and_contents(
    ...
    #     demisto.debug(f'get_file_name_and_contents {os.listdir(file_path)=}')
    # else:
        # demisto.debug(f'get_file_name_and_contents {os.listdir(os.getcwd())=}')

    demisto.debug(f'$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$')
    # demisto.debug(f'{demisto.getFilePath("181@61851")=}')
    if filename and data:
        return filename, b64decode(data)
    elif entry_id:
        demisto.debug(f'Entry ID: {entry_id}')
        file_object = demisto.getFilePath(entry_id)
        demisto.debug(f'{file_object=}')
        demisto.debug(f'{os.listdir()=}')
        # file_contents = open(file_object['path'])
...
def main():
    try:
        ...
        # Assuming file_object['path'] contains the file path
        ...
        try:
            ...
            demisto.debug("Files in directory:", files_in_directory)
        except FileNotFoundError:
            demisto.debug(f"Error: The directory {directory} does not exist.")
        except NotADirectoryError:
            demisto.debug(f"Error: {directory} is not a directory.")

        # Check if the file exists and is a file (not a directory)
        if os.path.isfile(file_path):
            with open(file_path, 'rb') as f:
                file_contents = f.read()
                demisto.debug(f'success')
        else:
            demisto.debug(f"Error: File not found at {file_path}")
