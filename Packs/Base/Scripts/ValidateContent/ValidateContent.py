...
HOOK_ID_TO_PATTERN = {
    ...
    "check-ast": {"regex": re.compile(r'File\s"(Packs/.*?)",\sline\s(\d+)'), "groups": ["file", "line"]},
    "mypy": {
...
def content_item_to_package_format(
    ...
    source_mapping: dict | None = None,  # noqa: F841
) -> None:
...
def convert_contribution_to_pack(contrib_converter):
    ...
    if contrib_converter.create_new:
        if contrib_converter.contribution:
            ...
            with (
                zipfile.ZipFile(contrib_converter.contribution) as zipped_contrib,
...
def prepare_single_content_item_for_validation(file_name: str, data: bytes, packs_path: str) -> str:
    ...
    contrib_converter = ContributionConverter(name=pack_name, pack_dir_name=pack_name, contribution=pack_name)
...
def get_content_modules(content_path: str, verify_ssl: bool = True) -> None:
    ...
    for module in modules:
        ...
        try:
            ...
            modified_time = (
                datetime.fromtimestamp(fname.stat().st_mtime) if os.path.isfile(cached_module_path) else datetime(1970, 1, 1)
            )
...
def read_pre_commit_results(pre_commit_dir: Path):
    ...
    for output_file in pre_commit_dir.iterdir():
        ...
        for output in raw_outputs:
            ...
            parsed_results: list[dict] = parse_pre_commit_output(stdout, pattern_obj)
...
def setup_content_dir(file_name: str, file_contents: bytes | str, entry_id: str, verify_ssl=False) -> str:
    ...
    if file_name.endswith(".zip"):
        path_to_validate = prepare_content_pack_for_validation(file_name, file_contents, CONTENT_DIR_PATH)
    else:
        path_to_validate = prepare_single_content_item_for_validation(file_name, file_contents, packs_path)
