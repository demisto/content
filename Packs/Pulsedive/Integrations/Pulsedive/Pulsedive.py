...
class Client(BaseClient):
    def test_connect(self, api_key):
        return self._http_request(method="GET", url_suffix="/info.php?", params={"indicator": "pulsedive.com", "key": api_key})

    def get_ip_reputation(self, ip: str, api_key) -> dict[str, Any]:
        return self._http_request(method="GET", url_suffix="/info.php?", params={"indicator": ip, "pretty": "1", "key": api_key})

    def get_domain_reputation(self, domain: str, api_key) -> dict[str, Any]:
        return self._http_request(
            method="GET", url_suffix="/info.php?", params={"indicator": domain, "pretty": "1", "key": api_key}
        )

    def get_url_reputation(self, url: str, api_key) -> dict[str, Any]:
        return self._http_request(method="GET", url_suffix="/info.php?", params={"indicator": url, "pretty": "1", "key": api_key})

    def post_value_scan(self, value: str, probe: str, api_key) -> dict[str, Any]:
        return self._http_request(
            method="POST", url_suffix="/analyze.php", params={"value": value, "probe": probe, "pretty": "1", "key": api_key}
        )

    def get_value_scan(self, qid: str, api_key) -> dict[str, Any]:
        return self._http_request(method="GET", url_suffix="/analyze.php?", params={"qid": qid, "pretty": "1", "key": api_key})
...
def ip_reputation_command(client: Client, args: dict[str, Any], api_key) -> list[CommandResults]:
    ...
    for ip in ips:
        try:
            ...
            # dbot_score to it.
            ip_standard_context = Common.IP(ip=indicator_ip, dbot_score=dbot_score)
...
def domain_reputation_command(client: Client, args: dict[str, Any], api_key) -> list[CommandResults]:
    ...
    for domain in domains:
        try:
            ...
            domain_standard_context = Common.Domain(domain=indicator_domain, dbot_score=dbot_score)
...
def url_reputation_command(client: Client, args: dict[str, Any], api_key) -> list[CommandResults]:
    ...
    for url in urls:
        try:
            ...
            url_standard_context = Common.URL(url=indicator_url, dbot_score=dbot_score)
            ...
            url_standard_context = Common.URL(url=str(url), dbot_score=dbot_score)
        ...
        try:
            ...
            if qid_data.get("data"):
                ...
                if qid_data["data"]["type"] == "url":
                    ...
                    url_indicator = Common.URL(url=qid_data["data"]["indicator"], dbot_score=dbot_score)

                    command_results.append(
                        CommandResults(
                            readable_output=tableToMarkdown(
                                "Value Details:", qid_data, headers=("indicator", "qid", "status", "success")
                            ),
                ...
                if qid_data["data"]["type"] == "ip":
                    ...
                    if "geo" in qid_data["data"]["properties"]:
                        ip_indicator = Common.IP(
                            ...
                            if qid_data["data"]["attributes"] != []
                            else None,
                            dbot_score=dbot_score,
                        )
                    else:
                        ip_indicator = Common.IP(ip=qid_data["data"]["indicator"], dbot_score=dbot_score)
                ...
                if qid_data["data"]["type"] == "domain":
                    ...
                    if "whois" in qid_data["data"]["properties"]:
                        ...
                        domain_indicator = Common.Domain(domain=qid_data["data"]["indicator"], dbot_score=dbot_score)
