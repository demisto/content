...
from SysdigResponseActions import Client, execute_response_action_command, create_system_capture_command, get_capture_file_command, get_action_execution_command
...
GET_ACTION_EXECUTION_RESPONSE = util_load_json('Packs/Sysdig/Integrations/Sysdig-Response-Actions/test_data/response_api/get_execution_response.json')
...
def test_execute_response_action(mock_response):
    ...
    result = execute_response_action_command(client, {'method': 'POST', 'url_suffix': '/secure/response-actions/v1alpha1/action-executions', 'actionType': 'KILL_CONTAINER', 'container_id': '123456789012', "callerId": "test_kill_container_07"})
...
def test_get_action_execution(mock_response):
    ...
    result = result.to_context().get('Contents')
    assert result.get('actionType') == 'KILL_CONTAINER' and result.get('status') == 'COMPLETED' and result.get('callerId') == 'test_kill_container_07' and result.get('executionContext').get('container.id') == '123456789012'
