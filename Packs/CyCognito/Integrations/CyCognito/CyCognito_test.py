...
def test_get_issue_command_status_404(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_issue_status_404_response.json")
    )
...
def test_get_issue_command_success(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_issue_success_response.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "./test_data/get_issue_success_hr.md")) as file:
        hr_output = file.read()
...
def test_cycognito_asset_get_status_404(requests_mock, mock_client):
    ...
    json_data = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_asset_status_404_response.json")
    )
...
def test_cycognito_asset_get_success_ip(requests_mock, mock_client):
    ...
    json_data = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_asset_success_response_ip.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "./test_data/get_asset_success_hr_ip.md")) as file:
        hr_output = file.read()
...
def test_cycognito_asset_get_success_iprange(requests_mock, mock_client):
    ...
    json_data = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_asset_success_response_iprange.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "./test_data/get_asset_success_hr_iprange.md")) as file:
        hr_output = file.read()

    requests_mock.get(f"{BASE_URL}/assets/iprange/0.0.0.1-1.0.0.0", json=json_data, status_code=200)
    response = cycognito_asset_get_command(mock_client, args={"asset_type": "iprange", "asset_id": "0.0.0.1-1.0.0.0"})
...
def test_cycognito_asset_get_success_cert(requests_mock, mock_client):
    ...
    json_data = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/get_asset_success_response_cert.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "./test_data/get_asset_success_hr_cert.md")) as file:
        hr_output = file.read()

    requests_mock.get(f"{BASE_URL}/assets/cert/test_cert", json=json_data, status_code=200)
    response = cycognito_asset_get_command(mock_client, args={"asset_type": "cert", "asset_id": "test_cert"})
...
def test_issue_investigation_status_change_with_invalid_args(mock_client):
    ...
    assert str(err.value) == ERRORS["INVALID_SINGLE_SELECT_PARAM"].format(
        "incorrect-status", "investigation_status", VALID_INVESTIGATION_STATUS
    )
...
def test_issue_investigation_status_change_command_success(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/change_issue_investigation_status_success_response.json"
        )
    )
    context_output = util_load_json(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/change_issue_investigation_status_success_context.json"
        )
    )

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/change_issue_investigation_status_success_hr.md")
    ) as file:
        hr_output = file.read()

    requests_mock.put(f"{BASE_URL}/issues/issue/127.0.0.1-test/investigation-status", json=mock_response, status_code=200)
...
def test_issue_investigation_status_change_command_failure(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/change_issue_investigation_status_failure_response.json"
        )
    )
    context_output = util_load_json(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/change_issue_investigation_status_failure_context.json"
        )
    )

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/change_issue_investigation_status_failure_hr.md")
    ) as file:
        hr_output = file.read()
...
def test_asset_investigation_status_change_with_invalid_args(mock_client):
    ...
    assert str(e.value) == ERRORS["INVALID_SINGLE_SELECT_PARAM"].format(
        "dummy", "investigation_status", VALID_INVESTIGATION_STATUS
    )
...
def test_asset_investigation_status_change_command_success(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/change_asset_investigation_status_success_response.json"
        )
    )
    context_output = util_load_json(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/change_asset_investigation_status_success_context.json"
        )
    )

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/change_asset_investigation_status_success_hr.md")
    ) as file:
        hr_output = file.read()
...
def test_asset_investigation_status_change_command_failure(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/change_asset_investigation_status_failure_response.json"
        )
    )
    context_output = util_load_json(
        os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "test_data/change_asset_investigation_status_failure_context.json"
        )
    )

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/change_asset_investigation_status_failure_hr.md")
    ) as file:
        hr_output = file.read()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"asset_type": "dummy"},
            ERRORS["INVALID_SINGLE_SELECT_PARAM"].format("dummy", "asset_type", ["ip", "domain", "cert", "iprange", "webapp"]),
        ),
        (
            {"investigation_status": "dummy"},
            ERRORS["INVALID_SINGLE_SELECT_PARAM"].format(
                "dummy", "investigation_status", ["investigated", "investigating", "uninvestigated"]
            ),
        ),
...
def test_list_issues_command_when_empty_body_params(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_issues_empty_request_body_response.json")
    )

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_issues_empty_request_body_hr.md")
    ) as file:
        hr_output = file.read()
...
def test_list_issues_command_with_body_params(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_issues_response.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "./test_data/list_issues_hr.md")) as file:
        hr_output = file.read()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"advanced_filter": "incorrect_json", "asset_type": "ip"}, ERRORS["INVALID_ADVANCED_FILTER"].format("incorrect_json")),
    ],
)
...
def test_list_assets_command_when_empty_body_params(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_assets_empty_request_body_response.json")
    )

    with open(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_assets_empty_request_body_hr.md")
    ) as file:
        hr_output = file.read()
...
def test_list_assets_command_with_body_params(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/list_assets_response.json")
    )

    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "./test_data/list_assets_hr.md")) as file:
        hr_output = file.read()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            ERRORS["INVALID_SINGLE_SELECT_PARAM"].format("incorrect_status", "Investigation Status", VALID_INVESTIGATION_STATUS),
        ),
...
def test_fetch_incident_success_with_last_run(requests_mock, mock_client):
    ...
    mock_response = util_load_json(
        os.path.join(os.path.dirname(os.path.realpath(__file__)), "test_data/fetch_incident_response.json")
    )
...
def test_update_remote_data_command(requests_mock, mock_client):
    ...
    args = {
        "remoteId": DUMMY_ISSUE_INSTANCE_ID,
        "data": data,
        "entries": [],
