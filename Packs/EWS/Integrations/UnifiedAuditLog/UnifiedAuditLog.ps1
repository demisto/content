...
function SearchAuditLogCommand{
    ...
    try
    {
        ...
        foreach ($item in $raw_response){
            $arr.add($item.AuditData)
        }
        $arr = $arr.ToArray()
        $human_readable = TableToMarkdown $arr "Audit log"
        $context = @{
            "$script:INTEGRATION_ENTRY_CONTEXT.AuditLog" = $raw_response
        }
        return Write-Output $human_readable, $context, $raw_response
    } finally {
        $client.CloseSession()
    }
}
