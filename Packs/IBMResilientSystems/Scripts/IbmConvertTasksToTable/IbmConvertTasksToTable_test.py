...
def test_multiple_tasks(mocker):
    ...
    mocker.patch.object(
        demisto, "incident", return_value={"CustomFields": {"ibmsecurityqradarsoartasks": [task_data1, task_data2]}}
    )
    result = convert_to_table()
    assert "| Initial | 1 | Analyze Log | Open | Review logs | 2023-05-01 | No Body | Yes |" in result.readable_output
    assert (
        "| Custom | 2 | Isolate System | Completed | Isolate affected system | 2023-05-02 | No Body | No |"
