...
def _mocker(mocker):
    mocker.patch.object(
        demisto, "params", return_value={"server": "example.com:80", "org": "example", "proxy": True, "close_ibm_incident": True}
    )
    return mocker
...
def test_update_incident_command_with_invalid_json(_mocker):
    ...
    """
    args = {"incident-id": "1234", "other-fields": "Invalid json"}
...
def test_add_incident_artifact(_mocker):
    ...
    mock_result = _mocker.patch.object(MockClient, "post")
    expected_result = (
        "/incidents/1234/artifacts",
        {
            "type": "IP Address",
            "value": "1.1.1.1",
            "description": {"format": "text", "content": "This is the artifact description"},
        },
    )
...
def test_test_module(_mocker):
    ...
    from IBMResilientSystems import SimpleClient, test_module
...
def test_test_module_fetch_time(fetch_time, expected_result, _mocker):
    ...
    from IBMResilientSystems import SimpleClient, test_module, validate_iso_time_format
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ),  # Test without any filters or pagination params
        (
            {"severity": "Low"},
            {
                "filters": [{"conditions": [{"field_name": "severity_code", "method": "in", "value": [50]}]}],
                "sorts": [{"field_name": "create_date", "type": "asc"}],
                "length": DEFAULT_MAX_FETCH,
            },
        ),
        (
            ...
            {
                "filters": [{"conditions": [{"field_name": "create_date", "method": "lte", "value": 1641031200000}]}],
                "sorts": [{"field_name": "create_date", "type": "asc"}],
                "length": DEFAULT_MAX_FETCH,
            },
        ),
        (
            ...
            {
                "filters": [{"conditions": [{"field_name": "inc_last_modified_date", "method": "gte", "value": 1641031200000}]}],
                "sorts": [{"field_name": "create_date", "type": "asc"}],
                "start": 0,
                "length": 10,
            },
...
@pytest.mark.parametrize(
    ...
    [
            ...
            [
                {
                    ...
                    "mentioned_users": [],
                    "is_deleted": False,
                    "modify_user": {"id": 0, "first_name": "Demisto", "last_name": "Resilient"},
                    "actions": [],
                    "inc_id": 2222,
                    "inc_name": "inci-11",
                    "task_id": None,
                    ...
                    "modify_principal": {"id": 0, "type": "user", "name": "demist", "display_name": "Demisto Resilient"},
                    "comment_perms": {"update": True, "delete": True},
                }
            ],
            [
                {
        ...
        (
            ...
            [{"create_date": "2024-07-31T11:10:53Z", "created_by": " ", "id": 2, "modify_date": None, "text": " "}],
        ),
    ],
)
...
def test_search_incidents(_mocker, args):
    from IBMResilientSystems import DEFAULT_RETURN_LEVEL, SimpleClient, search_incidents
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {
                "changes": [
                    {"field": "severity_code", "old_value": {"id": 6}, "new_value": {"id": 4}},
                    {
                        "field": "incident_type_ids",
                        "old_value": {"ids": [21, 19, 17, 6]},
                    ...
                    {"field": "resolution_id", "old_value": {"id": 9}, "new_value": {"id": 9}},
                    {
                        ...
                        "old_value": {"textarea": {"format": "html", "content": "This is a test incident."}},
                        "new_value": {"textarea": {"format": "html", "content": "This is a test incident."}},
                    },
                    {
                        "field": "description",
                        "old_value": {"textarea": {"format": "html", "content": "1111 2222 3333"}},
...
def test_update_incident_command(_mocker, args, processed_payload):
    ...
    client = SimpleClient()
    client.org_id = 0
    _mocker.patch.object(
        Session, "get", return_value=dict_to_response(load_test_data("./test_data/test_get_incident_response.json"))
    )

    request = _mocker.patch.object(
        Session, "patch", return_value=dict_to_response({"success": True, "title": None, "message": None, "hints": []})
    )
...
@pytest.mark.parametrize(
    "script_id, expected_outputs, expected_readable_output",
    [
        (
            "1",
            [
                {
                    ...
                    "creator_id": 0,
                    "last_modified_by": 0,
                    "last_modified_time": 1600698818789,
                }
            ],
            """### example Scripts
            ...
            [
                {
                    "error_code": "generic",
                    "hints": [],
                    "message": "Unable to find object with ID 2",
                    "success": False,
                    "title": None,
                }
            ],
...
def test_upload_incident_attachment(_mocker, file_entry_id: str):
    ...
    post_attachment_request.assert_called_once_with(
        uri=f"/incidents/{args['incident_id']}/attachments", filepath="/path/to/file", filename=f"filename_{TAG_TO_IBM}.txt"
    )
...
def test_delete_incidents_command(_mocker):
    ...
    delete_incident_request = _mocker.patch.object(
        SimpleClient, "put", return_value={"success": True, "title": None, "message": None, "hints": []}
    )
...
def test_list_incident_notes_command(_mocker):
    ...
    get_incident_notes_request = _mocker.patch.object(
        SimpleClient, "get", return_value=load_test_data("./test_data/test_get_incident_notes_reponse.json")
    )
    list_incident_notes_command(client, {"incident_id": "2000"})

    get_incident_notes_request.assert_called_once_with("/incidents/2000/comments?text_content_output_format=objects_convert_text")
...
def test_update_incident_note(_mocker):
    ...
    update_incident_note_request = _mocker.patch.object(SimpleClient, "put", return_value={})
    update_incident_note_command(client, args={"incident_id": 2000, "note_id": 1, "note": "NOTE_BODY"})

    update_incident_note_request.assert_called_once_with(
        "/incidents/2000/comments/1", payload={"text": {"format": "text", "content": "NOTE_BODY"}}
    )


@pytest.mark.parametrize(
    "args, expected_task_dto",
    [
        (
            {
                "incident_id": "2000",
        ...
        (  # Task without Instructions
            ...
            {"name": "TASK-2", "phase_id": {"name": "Custom"}, "description": "TASK 2", "due_date": 1682942400000, "owner_id": 1},
        ),
        (  # Invalid Owner ID
            {
                "incident_id": "2003",
...
def test_add_custom_task_command(_mocker, args, expected_task_dto):
    ...
    add_custom_task_request = _mocker.patch.object(SimpleClient, "post", side_effect=post_side_effect)
    ...
    if isinstance(expected_task_dto, Exception):
        ...
        assert (
            result.readable_output == f"Successfully created new task for incident with ID {args['incident_id']}. Task ID: 1234"
        )
...
def test_list_tasks_command(_mocker):
    ...
    get_tasks_request = _mocker.patch.object(SimpleClient, "get", return_value={})
...
def test_get_task_members_command(_mocker):
    ...
    client.org_id = 0
    task_id = "1234"
    get_task_members_request = _mocker.patch.object(SimpleClient, "get", return_value={})
    get_task_members_command(client, args={"task_id": task_id})

    get_task_members_request.assert_called_with(f"/tasks/{task_id}/members")


@pytest.mark.parametrize("task_ids, should_raise_exception", [("1000", False), ("2000,3000", False), ("", True)])
def test_delete_tasks_command(_mocker, task_ids, should_raise_exception):
    ...
    delete_tasks_request = _mocker.patch.object(
        SimpleClient, "put", return_value={"success": True, "title": None, "message": None, "hints": []}
    )
...
def test_delete_task_members_command(_mocker):
    ...
    delete_task_members_request = _mocker.patch.object(SimpleClient, "delete", return_value=mock_response)
...
def test_list_task_instructions_command(_mocker):
    ...
    task_id = "5678"
    mock_response = {"text": {"content": "These are the instructions for the task.", "format": "text"}}

    get_task_instructions_request = _mocker.patch.object(SimpleClient, "get", return_value=mock_response)
...
def test_get_attachment_command(_mocker):
    ...
    get_attachment_request.assert_has_calls(
        [_mocker.call(get_attachment_endpoint), _mocker.call(get_attachment_contents_endpoint, get_response_object=True)]
    )
...
def test_get_modified_remote_data_command(_mocker):
    ...
    mock_search_incidents = _mocker.patch(
        "IBMResilientSystems.search_incidents",
        return_value=[
...
def test_get_remote_data_command(_mocker):
    ...
    args = {"id": incident_id, "lastUpdate": last_update}

    # Mock the get_incident and process_raw_incident function behavior
    mock_incident_data = {
        "plan_status": "A",  # 'A' stands for Active
        "end_date": None,
        "notes": [{"modify_date": 1725880565507, "text": {"content": "Note content"}, "created_by": "User 1"}],
        "attachments": [{"ID": "1", "Create Time": 1725880565507, "Name": "Attachment1"}],
    }
...
def test_update_remote_system_command_no_changes(_mocker):
    ...
    client = SimpleClient()
    args = {"remoteId": "1000", "incidentChanged": False, "entries": [], "delta": None, "data": {}, "incStatus": "Active"}

    debug_mock = _mocker.patch.object(demisto, "debug")
    result = update_remote_system_command(client, args, tag_to_ibm="FROM XSOAR")

    assert result == "1000"
    debug_mock.assert_called_with("Skipping updating remote incident fields [1000] as it is not new nor changed")
...
def test_update_remote_system_command_with_changes(_mocker):
    ...
    prepare_mock = _mocker.patch(
        "IBMResilientSystems.prepare_incident_update_dto_for_mirror", return_value={"name": "Updated Incident Name"}
    )
...
def test_update_remote_system_command_with_multiple_entries(_mocker):
    ...
    prepare_mock = _mocker.patch(
        "IBMResilientSystems.prepare_incident_update_dto_for_mirror", return_value={"description": "Updated description"}
    )
    update_mock = _mocker.patch("IBMResilientSystems.update_incident")
    add_note_mock = _mocker.patch("IBMResilientSystems.add_note")
    upload_mock = _mocker.patch("IBMResilientSystems.upload_incident_attachment")
...
def test_get_mapping_fields_command(_mocker):
    ...
    from IBMResilientSystems import IBM_QRADAR_INCIDENT_FIELDS, IBM_QRADAR_SOAR_INCIDENT_SCHEMA_NAME, get_mapping_fields_command
...
@pytest.mark.parametrize(
    "last_run, first_fetch_time, expected_args, expected_last_run",
    [
...
def test_process_raw_incident(_mocker):
    _mocker.patch.object(
        demisto,
        "params",
        return_value={"server": "example.com:80", "org": "example", "proxy": True, "fetch_tasks": True, "fetch_notes": True},
    )
...
@pytest.mark.parametrize(
    "incident_id, delta, expected_dto",
    [
        (
            "1000",
            ...
            {
                "changes": [
                    {"field": "name", "new_value": {"text": "Updated Incident Name"}, "old_value": {"text": "incident_name"}},
                    {
                        "field": "description",
                        "new_value": {"textarea": {"content": "New description", "format": "html"}},
                        "old_value": {"textarea": {"content": "1111 2222 3333", "format": "html"}},
                    },
                ]
            },
        ),
        (
            ...
            {
                "changes": [
                    {"field": "plan_status", "new_value": {"text": "C"}, "old_value": {"text": "A"}},
                    {"field": "resolution_id", "new_value": {"textarea": None}, "old_value": {"id": 9}},
                ]
            },
        ),
    ],
...
def test_prepare_incident_update_dto_for_mirror(_mocker, incident_id, delta, expected_dto):
    ...
    mock_get_incident = _mocker.patch(
        "IBMResilientSystems.get_incident", return_value=load_test_data("./test_data/test_get_incident_response.json")
    )
...
def test_prettify_incident_tasks_missing_fields(_mocker):
    ...
    client = SimpleClient()
    tasks = [
        {"id": 1, "name": "Minimal Task", "description": "", "due_date": None, "status": "O", "required": False, "phase_id": 1}
    ]
...
def test_get_phase_name(_mocker):
    ...
    _mocker.patch.object(
        ...
        return_value={
            ...
            "name": "Engage",
            "enabled": True,
            "perms": {"deleteable": True, "reorderable": True},
            ...
            "entities": [
                {
                    ...
                    "name": "Initial",
                    "enabled": True,
                    "perms": {"deleteable": False, "reorderable": False},
                    "uuid": "0000-0000-0000-0000",
                    "order": 0,
                    "tags": [],
                }
...
@pytest.mark.parametrize(
    "resolution_id, resolution_summary, expected_close_reason, expected_close_notes",
    [
...
def test_handle_incoming_incident_resolution(
    _mocker, resolution_id, resolution_summary, expected_close_reason, expected_close_notes
):
    from IBMResilientSystems import EntryFormat, EntryType, handle_incoming_incident_resolution
