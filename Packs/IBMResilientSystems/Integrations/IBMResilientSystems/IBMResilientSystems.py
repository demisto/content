...
SEVERITY_CODE_DICT = {"Low": 4, "Medium": 5, "High": 6}

RESOLUTION_DICT = {7: "Unresolved", 8: "Duplicate", 9: "Not an Issue", 10: "Resolved"}

RESOLUTION_TO_ID_DICT = {"Unresolved": 7, "Duplicate": 8, "NotAnIssue": 9, "Resolved": 10}
...
XSOAR_CLOSE_REASON_MAPPING = {"Other": 7, "Duplicate": 8, "False Positive": 9, "Resolved": 10}

EXP_TYPE_ID_DICT = {1: "Unknown", 2: "ExternalParty", 3: "Individual"}
...
IBM_QRADAR_INCIDENT_FIELDS = {
    ...
    "incident_type_ids": {"xsoar_name": "alerttypeid", "description": "The IDs of the incident types."},
    "resolution_id": {"xsoar_name": "ibmsecurityqradarsoarresolution", "description": ""},
    "resolution_summary": {"xsoar_name": "ibmsecurityqradarsoarresolutionsummary", "description": ""},
    "owner_id": {
        "xsoar_name": "",
...
def prepare_search_query_data(args: dict) -> dict:
    ...
    if "severity" in args:
        ...
        conditions.append({"field_name": "severity_code", "method": "in", "value": value})
    if "date-created-before" in args:
        value = to_timestamp(args["date-created-before"])
        conditions.append({"field_name": "create_date", "method": "lte", "value": value})
    elif "date-created-after" in args:
        value = to_timestamp(args["date-created-after"])
        conditions.append({"field_name": "create_date", "method": "gte", "value": value})
        ...
        conditions.extend(
            (
                {"field_name": "create_date", "method": "lte", "value": now * 1000},
                {"field_name": "create_date", "method": "gte", "value": from_time * 1000},
            )
        )
    if "date-occurred-before" in args:
        value = to_timestamp(args["date-occurred-before"])
        conditions.append({"field_name": "start_date", "method": "lte", "value": value})
    elif "date-occurred-after" in args:
        value = to_timestamp(args["date-occurred-after"])
        conditions.append({"field_name": "start_date", "method": "gte", "value": value})
        ...
        conditions.extend(
            (
                {"field_name": "start_date", "method": "lte", "value": now * 1000},
                {"field_name": "start_date", "method": "gte", "value": from_time * 1000},
            )
        )
    if "incident-type" in args:
        type_id = INCIDENT_TYPE_DICT[args["incident-type"]]
        conditions.append({"field_name": "incident_type_ids", "method": "contains", "value": [type_id]})
    if "nist" in args:
        nist = NIST_DICT[args["nist"]]
        conditions.append({"field_name": "nist_attack_vectors", "method": "contains", "value": [nist]})
    if "status" in args:
        status = "A" if args["status"] == "Active" else "C"
        conditions.append({"field_name": "plan_status", "method": "in", "value": [status]})
    if "due-in" in args:
        if "timeframe" not in args:
            raise Exception("Timeframe was not given.")
        ...
        conditions.extend(
            (
                {"field_name": "due_date", "method": "lte", "value": to_time * 1000},
                {"field_name": "due_date", "method": "gte", "value": now * 1000},
            )
        )
    if "last-modified-after" in args:
        value = to_timestamp(args["last-modified-after"])
        conditions.append({"field_name": "inc_last_modified_date", "method": "gte", "value": value})

    data: Dict[str, Any] = {"filters": [{"conditions": conditions}], "sorts": [{"field_name": "create_date", "type": "asc"}]}
...
def get_mirroring_data() -> dict:
    ...
    mirror_tags = [params.get("tag_from_ibm"), params.get("tag_to_ibm")]
    return {
...
def prepare_incident_update_dto_for_mirror(client: SimpleClient, incident_id: str, delta: dict) -> dict:
    ...
    for field, new_value in delta.items():
        ...
        changes.append(get_field_changes_entry(field=field, old_value=incident[field], new_value=new_value))
...
def get_attachment(client: SimpleClient, incident_id: str, attachment_id: str) -> tuple[str, str]:
    ...
    response: requests.Response = client.get(
        f"/incidents/{incident_id}/attachments/{attachment_id}/contents", get_response_object=True
    )
...
def get_incident_command(client, incident_id):
    ...
    ec = {"Resilient.Incidents(val.Id && val.Id === obj.Id)": result_incident}
...
def search_incidents_command(client, args):
    ...
    if incidents:
        ...
        result_incidents = createContext(
            pretty_incidents, id=None, keyTransform=underscoreToCamelCase, removeNull=True
        )  # pragma: no cover
        ec = {"Resilient.Incidents(val.Id && val.Id === obj.Id)": result_incidents}
...
def update_incident_command(client, args):
    ...
    demisto.debug(f"update_incident_command {response=!s}")
...
def get_members_command(client, incident_id):
    ...
    for user in users:
        if user["id"] in response:
            members.append({"FirstName": user["fname"], "LastName": user["lname"], "ID": user["id"], "Email": user["email"]})

    ec = {"Resilient.Incidents(val.Id && val.Id === obj.Id)": {"Id": incident_id, "Members": members}}
...
def get_users_command(client):
    ...
    for user in response:
        users.append({"FirstName": user["fname"], "LastName": user["lname"], "ID": user["id"], "Email": user["email"]})
...
def get_tasks_command(client, incident_id):
    ...
    for task in tasks:
        ...
        ec = {"Resilient.Incidents(val.Id && val.Id === obj.Id)": {"Id": incident_id, "Name": incident_name, "Tasks": tasks}}
...
def set_member_command(client, incident_id, members):
    ...
    data = {"vers": incident_version, "members": members}
        ...
        for user in users:
            if user["id"] in members:
                if isinstance(response, dict):
                    response.update(
                        {"FirstName": user["fname"], "LastName": user["lname"], "ID": user["id"], "Email": user["email"]}
                    )
                else:
                    response.append(
                        {"FirstName": user["fname"], "LastName": user["lname"], "ID": user["id"], "Email": user["email"]}
                    )
        ec = {"Resilient.Incidents(val.Id && val.Id === obj.Id)": {"Id": incident_id, "Members": response}}
        title = "Members of incident " + incident_id
        entry = {
            "Type": entryTypes["note"],
            "Contents": response,
...
def close_incident(client, incident_id, incident):
    ...
    data = {"changes": [get_field_changes_entry("plan_status", old_status, "C")]}
    return update_incident(client, incident_id, data)


def create_incident_command(client, args):
    incident_name = args["name"]
    data = {"name": incident_name, "discovered_date": 0}
    response = create_incident(client, data)
    hr = {"ID": response["id"], "Name": incident_name}
    ec = {"Resilient.Incidents(val.Id && val.Id === obj.Id)": {"Id": str(response["id"]), "Name": incident_name}}
...
def incident_artifacts_command(client, incident_id):
    ...
    if response:
        ...
        entry = {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": tableToMarkdown(
                title, hr_artifacts, headers=["ID", "Value", "Description", "CreatedDate", "Creator"]
            ),
            "EntryContext": ec,
        }
        return entry
...
def related_incidents_command(client, incident_id):
    ...
    if response:
        ...
        ec = {"Resilient.Incidents(val.Id && val.Id === obj.Id)": {"Id": incident_id, "Related": ec_incidents}}
...
def fetch_incidents(client, first_fetch_time: str, fetch_closed: bool):
    ...
    if resilient_incidents:
        demisto.info(
            f'fetch_incidents retrieved {len(resilient_incidents)=} | '
        ...
        for incident in resilient_incidents:
            ...
            if fetch_closed or (not incident.get("end_date") and incident.get("plan_status") == "A"):  # 'A' stands for 'Active'
                ...
                demisto_incident = {}
                demisto_incident["name"] = f'IBM QRadar SOAR incident ID {incident["id"]!s}'
...
def add_note(client: SimpleClient, incident_id: str, note_content: str) -> dict:
    ...
    """
    body = {"text": {"format": "text", "content": note_content}}
    return client.post(f"/incidents/{incident_id!s}/comments", body)
...
def list_scripts_command(client: SimpleClient, args: dict) -> CommandResults:
    ...
    demisto.info(f"list_scripts_command received script ids: {script_ids!s}")
    return CommandResults(
        outputs_prefix="Resilient.Scripts",
        outputs=scripts_to_process,  # Already processed and enriched with additional data.
        readable_output=tableToMarkdown(
            f'{DEMISTO_PARAMS.get("org")} Scripts', scripts_to_process, headers=["id", "name", "description", "language"]
        ),
    )
...
def upload_incident_attachment_command(client: SimpleClient, args: dict, tag_to_ibm: str) -> CommandResults:
    """
    ...
    return CommandResults(readable_output=f"File was uploaded successfully to {incident_id}.")
...
def delete_incidents_command(client: SimpleClient, args: dict) -> CommandResults:
    ...
    human_readable: str = (
        f"Incidents {incident_ids} were deleted successfully." if response["success"] else f"{response['message']}"
    )
...
def update_incident_note_command(client: SimpleClient, args: dict) -> CommandResults:
    ...
    demisto.debug(f"{response=}")
    return CommandResults(readable_output=f"Successfully updated note ID {note_id} for incident ID {incident_id}")
...
def delete_tasks_command(client: SimpleClient, args: dict) -> CommandResults:
    ...
    human_readable = f"Tasks with IDs {task_ids} were deleted successfully." if response["success"] else f"{response['message']}"
    demisto.debug(f"{response=}")
    return CommandResults(readable_output=human_readable)
...
def get_remote_data_command(client: SimpleClient, args: dict, tag_to_ibm: str, tag_from_ibm: str) -> GetRemoteDataResponse:
    """
    Args:
    ...
    for note_entry in note_entries:
        ...
        if (
            tag_to_ibm not in str(note_entry["text"])
    ...
    for attachment_entry in attachment_entries:
        ...
        if (
            tag_to_ibm not in attachment_entry.get("Name", "")
...
def get_mapping_fields_command() -> GetMappingFieldsResponse:
    ...
    for field in fields_copy:
        ibm_qradar_incident_type_scheme.add_field(name=field, description=IBM_QRADAR_INCIDENT_FIELDS[field].get("description"))
...
def test_module(client: SimpleClient, fetch_time: str, tag_to_ibm=DEFAULT_TAG_TO_IBM, tag_from_ibm=DEFAULT_TAG_FROM_IBM) -> str:
    ...
    if tag_from_ibm == tag_to_ibm:
        raise DemistoException(f"Tag *to* IBM (`{tag_to_ibm}`) and Tag *from* IBM (`{tag_from_ibm}`) cannot have the same value.")
...
def main():  # pragma: no cover
    ...
    try:
        ...
        if command == "test-module":
            ...
            return_results(set_member_command(client, args["incident-id"], args["members"]))
