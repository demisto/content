...
class TestHelperFunctions:
    ...
    @pytest.mark.parametrize(
        "hash_value, hash_type, exception",
        [
    ...
    @pytest.mark.parametrize(
        "ind_val, ind_type, dbot_score, output",
        [
            (
                "8.8.8.8",
                "ip",
    ...
    def test_get_indicator_object(self, ind_val, ind_type, dbot_score, output):
        ...
        if not output:
            ...
            assert get_indicator_object(ind_val, ind_type, dbot_score).to_context() == output.to_context()

    @pytest.mark.parametrize(
        "items_list, ret_type, keys, output",
        [
