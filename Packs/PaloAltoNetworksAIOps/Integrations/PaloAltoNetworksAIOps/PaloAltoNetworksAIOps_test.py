...
def test_generate_report_command(mocker, AIOps_client):
    ...
    get_info_about_device_request_mock.return_value = (
        "<system ><family>test1</family><model>test2</model><serial>test3</serial>"
    ...
    with (
        patch("PaloAltoNetworksAIOps.convert_config_to_bytes", return_value=(b'<?xml version="1.0"?><config>test</config>')),
        ...
        http_request_mock.assert_called_once_with(
            ...
            headers={"Content-Type": "application/json", "Accept": "application/json", "Authorization": "Bearer None"},
            json_data={
    ...
    assert result[0].outputs == [
        {
            ...
            "data": [
                {
                    "check_id": 1,
                    "check_message": "Warning message 1",
                    "check_type": "warning",
                ...
                {
                    "check_id": 3,
                    "check_message": "Warning message 2",
                    "check_type": "warning",
...
def test_get_info_about_device_request_called_with(mocker, AIOps_client):
    ...
    http_request_mock = mocker.patch.object(AIOps_client, "_http_request")
    response_text = (
        '<response status="success"><result><system><hostname>test</hostname><ip-address>1.1.1.1'
...
def test_get_info_about_device_request_fails(mocker: MockerFixture, AIOps_client):
    ...
    response_text = (
        '<response status="success"><result><systems><hostname>test</hostname><ip-address>1.1.1.1'
...
def test_get_config_file_request_called_with(mocker, AIOps_client):
    ...
    http_request_mock = mocker.patch.object(AIOps_client, "_http_request")
    response_text = (
        '<response status="success"><result><config version="10.2.0" urldb="paloaltonetworks" '
...
def test_get_config_file_request_fails(mocker: MockerFixture, AIOps_client):
    ...
    response_text = (
        '<response status="success"><result><configs version="10.2.0" urldb="paloaltonetworks" '
...
def test_config_file_to_report_request_called_with(mocker, AIOps_client):
    ...
    http_request_mock.assert_called_once_with(
        method="PUT",
        full_url="cloud.test",
        headers={"Content-Type": "application/octet-stream", "Accept": "*/*", "Authorization": "Bearer None"},
...
def test_adjust_xml_format():
    ...
    xml_data = (
        '<response status="success"><result><system><hostname>test</hostname><ip-address>1.2.3.4</ip-address></system>'
        "</result></response>"
    )
    new_root_tag = "system"
...
def test_get_values_from_xml():
    ...
    xml_string = (
        "<system ><family>test1</family><model>test2</model><serial>test3</serial><hostname>test</hostname><sw-version>"
        "test4</sw-version><ip-address>1.1.1.1</ip-address></system>"
    )
...
def test_convert_config_to_bytes_with_user_flag(mocker, AIOps_client):
    ...
    with (
        patch("builtins.open", mock_open(read_data=expected_bytes)) as mock_open_func,
...
def test_create_markdown():
    ...
    assert create_markdown(response_array) == (
        "### BPA results:\n|Check Id|Check Category|Check Feature|Check Message|Check Type"
        "|\n|---|---|---|---|---|\n| 1 | device | feature1 | Warning message 1 | warning |"
