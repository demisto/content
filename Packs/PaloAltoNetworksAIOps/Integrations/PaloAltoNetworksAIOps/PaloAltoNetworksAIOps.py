...
class Client(BaseClient):
    ...
    def generate_access_token_request(self):
        ...
        if previous_token and previous_token_expiry_time and previous_token_expiry_time > date_to_timestamp(datetime.now()):
            ...
            data = {"grant_type": "client_credentials", "scope": f"tsg_id:{self._tsg_id}"}
            headers = {
    ...
    def config_file_to_report_request(self, upload_url, config_in_binary):
        headers = {"Content-Type": "application/octet-stream", "Accept": "*/*", "Authorization": f"Bearer {self._access_token}"}
        res = self._http_request(
            method="PUT",
            full_url=upload_url,
            headers=headers,
    ...
    def check_upload_status_request(self, report_id):
        headers = {"Accept": "*/*", "Authorization": f"Bearer {self._access_token}"}
        res = self._http_request(
            method="GET", full_url=f"https://api.stratacloud.paloaltonetworks.com/aiops/bpa/v1/jobs/{report_id}", headers=headers
        )
    ...
    def data_of_download_bpa_request(self, downloaded_BPA_url):
        ...
        res = self._http_request(method="GET", full_url=downloaded_BPA_url, headers=headers)
        return res
...
def create_response(client, report_id, show_in_context, export_as_file, upload_status):
    ...
    if show_in_context:
        context_json = [{"report_id": report_id, "report_status": upload_status, "data": converted_json}]
        response.append(
            CommandResults(
        ...
        response.append(CommandResults(raw_response=downloaded_BPA_json, readable_output=human_readable_markdown))
...
def create_markdown(original_dict):
    ...
    return tableToMarkdown(
        "BPA results:", original_dict, headers=headers, removeNull=True, headerTransform=string_to_table_header
    )
...
def generate_report_command(client: Client, args: dict[str, Any]):
    ...
    return_results(
        polling_until_upload_report_command(
            {
                "report_id": report_id,
                "export_as_file": export_as_file,
