...
class MockIntegrationContext:
    ...
    def encode_values(ctx: dict[str, Any]) -> dict[str, str]:
        return {k: json.dumps(v) if k.startswith(os.sep) and not isinstance(v, str) else v for k, v in ctx.items()}

    @staticmethod
    def decode_values(ctx: dict[str, Any]) -> dict[str, Any]:
        return {k: json.loads(v) if k.startswith(os.sep) and isinstance(v, str) else v for k, v in ctx.items()}

    def __init__(self, ctx: dict[str, Any], mocker: Optional[pytest_mock.plugin.MockerFixture] = None):
        self.__ctx = MockIntegrationContext.encode_values(ctx)
        if mocker:
            mocker.patch("WebFileRepository.get_integration_context", side_effect=self.get_integration_context)
            mocker.patch("WebFileRepository.set_integration_context", side_effect=self.set_integration_context)
...
class MockUUID:
    def __init__(self, mocker: pytest_mock.plugin.MockerFixture):
        self.__count = 0
...
class MockBaseClient:
    def __init__(
        self, mocker: pytest_mock.plugin.MockerFixture, headers: dict[str, str], content: bytes = None, json_data: Any = None
    ):
    ...
    def _http_request(
        ...
        **kwargs,
        ...
        if resp_type == "json":
            ...
            return MockRequestsResponse(headers=self.__headers, content=self.__content)


def MockFileResult(filename, data, file_type=None):
    if file_type is None:
        file_type = entryTypes["file"]
    return {"Contents": "", "ContentsFormat": formats["text"], "Type": file_type, "File": filename, "FileID": "fileid"}


@pytest.mark.parametrize(
    argnames="filename, content_type",
    argvalues=[
...
@pytest.mark.parametrize(
    argnames="integration_context_filename, "
    "storage_protection, "
    "max_storage_size, "
    "max_sandbox_size, "
    "sandbox_usage, "
    "storage_usage",
    argvalues=[("./test_data/integration_ctx_empty.json", "read/write", "10000", "100000", 0, 0)],
)
def test_process_root_get_status(
    mocker, integration_context_filename, storage_protection, max_storage_size, max_sandbox_size, sandbox_usage, storage_usage
):
    """
    Given:
...
@pytest.mark.parametrize(
    argnames="integration_context_filename, dir_name, recursive, output_filename",
    argvalues=[
...
@pytest.mark.parametrize(
    argnames="integration_context_filename, path",
    argvalues=[
...
@pytest.mark.parametrize(
    argnames="integration_context_filename, filenames",
    argvalues=[
        (
            "./test_data/integration_ctx_common.json",
            [
...
def test_process_root_post_health(
    mocker, integration_context_filename, storage_protection, user_permission, request_permission, ok
):
    """
    Given:
    ...
    post_data = json.dumps({"q": "health", "permission": request_permission}).encode()
...
def test_process_root_post_cleanup(mocker, integration_context_filename, storage_protection):
    """
    Given:
    ...
    post_data = json.dumps({"q": "cleanup"}).encode()
...
def test_process_root_post_reset(
    mocker, integration_context_filename_before, integration_context_filename_after, storage_protection
):
    """
    Given:
    ...
    post_data = json.dumps({"q": "delete", "path": ["/a.dat"]}).encode()
    ...
    post_data = json.dumps({"q": "reset"}).encode()
...
def test_process_root_post_delete(mocker, integration_context_filename_before, path_list, integration_context_filename_after):
    """
    Given:
    ...
    post_data = json.dumps({"q": "delete", "path": path_list}).encode()
...
@pytest.mark.parametrize(
    argnames="integration_context_filename_before, "
    "file_list, "
    "upload_dir, "
    "extract_archive, "
...
def test_process_root_post_upload(
    mocker, integration_context_filename_before, file_list, upload_dir, extract_archive, integration_context_filename_after
):
    """
    Given:
    ...
    post_data += "\r\n".join(["", f"--{boundary}", 'Content-Disposition: form-data; name="dir"', "", upload_dir]).encode()

    post_data += "\r\n".join(
        ["", f"--{boundary}", 'Content-Disposition: form-data; name="extract"', "", "true" if extract_archive else "false"]
    ).encode()

    post_data += "\r\n".join(["", f"--{boundary}", 'Content-Disposition: form-data; name="q"', "", "upload"]).encode()
        ...
        with open(file_path, "rb") as f:
            ...
            for chunk_index in range(chunk_num):
                ...
                for k, v in form_params.items():
                    post_data += "\r\n".join(
                        ["", f"--{boundary}", f'Content-Disposition: form-data; name="{k}"', "", str(v)]
                    ).encode()
...
def test_process_root_post_upload_limit(mocker, storage_limit, sandbox_limit, storage_protection):
    """
    Given:
    ...
    post_data += "\r\n".join(["", f"--{boundary}", 'Content-Disposition: form-data; name="dir"', "", "/"]).encode()

    post_data += "\r\n".join(["", f"--{boundary}", 'Content-Disposition: form-data; name="extract"', "", "false"]).encode()
...
def test_command_cleanup(mocker):
    ...
    client = MockBaseClient(mocker, headers={}, json_data={"success": True, "message": ""})
    mocker.patch.object(WebFileRepository, "new_client", return_value=client)
...
def test_command_upload_as_file(mocker, file_name, input_data, encoding, file_data):
    ...
    client = _MockBaseClient(
        mocker, headers={}, file_name=file_name, file_data=file_data, json_data={"success": True, "message": ""}
    )
    mocker.patch.object(WebFileRepository, "new_client", return_value=client)

    importlib.reload(WebFileRepository)

    args = assign_params(file_name=file_name, data=input_data, encoding=encoding)
    settings = WebFileRepository.Settings(params)
...
def test_command_upload_file(mocker, entry_id, name):
    ...
    mocker.patch.object(demisto, "params", return_value=params)
    mocker.patch.object(demisto, "getFilePath", return_value={"name": "upload_file.dat", "path": "test_data/upload_file.dat"})
    ...
    args = assign_params(entry_id=entry_id, name=name)
    settings = WebFileRepository.Settings(params)
...
def test_command_list_files(mocker, directory, recursive, response_filename, results_filename):
    """
    Given:
        Patterns of parameters for command_list_files
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        ("/あいうえお.dat", "アイウエオ.txt", "test_data/download_file.dat"),
    ],
)
def test_command_download_file(mocker, path, save_as, content_filename):
    ...
    args = {"path": path, "save_as": save_as}
    settings = WebFileRepository.Settings(params)
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        ("/test.dat", "base64", "Hello!", "test_data/download_as_text_02.json"),
        ("test.dat", None, "Hello!", "test_data/download_as_text_01.json"),
        ("test.dat", "utf-8", "Hello!", "test_data/download_as_text_01.json"),
        ("test.dat", "base64", "Hello!", "test_data/download_as_text_02.json"),
    ],
...
def test_command_download_as_text(mocker, path, encoding, content, results_filename):
    ...
    args = assign_params(path=path, encoding=encoding)
    settings = WebFileRepository.Settings(params)
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        ("アイウエオ.txt", "test_data/download_file.dat"),
    ],
)
...
def test_parse_mime_types(mocker, mimetypes_input_filename, mimetypes_output_filename, merge_mime_types):
    """
    Given:
        The MIME types file
...
def test_parse_attachment_exts(mocker, attachment_exts_input, attachment_exts_output):
    """
    Given:
...
@pytest.mark.parametrize(
    ...
    argvalues=[
        ...
        # Then:
        #   - Auth OK
        ("test", "password", None, None, True, False, "POST", "Basic", "Basic dGVzdDpwYXNzd29yZA==", True),
        # Given:
        ...
        # Then:
        #   - Auth OK
        ("", "", None, None, True, False, "POST", "Basic", "Basic Og==", True),
        # Given:
        ...
        # Then:
        #   - Auth OK
        (None, None, "test", "password", False, False, "POST", "Basic", "Basic dGVzdDpwYXNzd29yZA==", True),
        # Given:
        #   - read-only credentials
        #   - right credentials
        #
        ...
        # Then:
        #   - Auth NG
        (None, None, "test", "password", True, False, "POST", "Basic", "Basic dGVzdDpwYXNzd29yZA==", False),
        # Given:
        #   - read/write credentials
        #   - wrong credentials
        #
        ...
        # Then:
        #   - Auth NG
        ("test", "password1", None, None, True, False, "POST", "Basic", "Basic dGVzdDpwYXNzd29yZA==", False),
        # Given:
        #   - read/write credentials
        #   - read-only credentials
        #   - right credentials
        ...
        # Then:
        #   - Auth OK
        ("test", "password", "test", "password", True, False, "POST", "Basic", "Basic dGVzdDpwYXNzd29yZA==", True),
        # Given:
        #   - read/write credentials
        #   - right credentials
        #
        ...
        # Then:
        #   - Auth NG
        ("test", "password", None, None, False, False, "POST", "Basic", "", False),
    ],
)
def test_handle_auth(
    ...
    mocker.patch.object(
        ...
        "params",
        return_value={
            "rwCredentials": {"identifier": rw_identifier, "password": rw_password},
            "roCredentials": {"identifier": ro_identifier, "password": ro_password},
    ...
    if auth_method == "Digest":
        ...
        mocker.patch.object(
            WebFileRepository.NONCE_MANAGER, "_NonceManager__new_nonce", side_effect=lambda: __new_nonce(auth_params["nonce"])
        )

        WebFileRepository.NONCE_MANAGER._NonceManager__expires = 60 * 60 * 24 * 3650
        WebFileRepository.NONCE_MANAGER.gen_nonce()

    environ = {"REQUEST_METHOD": request_method, "PATH_INFO": "/", "HTTP_AUTHORIZATION": auth_header}
    request = bottle.LocalRequest(environ)
    handler = WebFileRepository.ServiceHandler(WebFileRepository.SETTINGS, WebFileRepository.MASTER_REPOSITORY)
    if require_write:
