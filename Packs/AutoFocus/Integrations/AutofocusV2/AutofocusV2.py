...
RELATIONSHIP_TYPE_BY_TAG_CLASS_ID = {
    1: {"entity_b_type": "STIX Threat Actor", "name": "indicator-of"},
    2: {"entity_b_type": "Campaign", "name": "indicator-of"},
    3: {"entity_b_type": "STIX Malware", "name": "indicator-of"},
    5: {"entity_b_type": "STIX Attack Pattern", "name": "indicator-of"},
}
API_PARAM_DICT = {
    "scope": {"Private": "private", "Public": "public", "Global": "global"},
    ...
    "search_arguments": {
        "file_hash": {"api_name": "alias.hash_lookup", "operator": "is"},
        "domain": {"api_name": "alias.domain", "operator": "contains"},
        "ip": {"api_name": "alias.ip_address", "operator": "contains"},
        "url": {"api_name": "alias.url", "operator": "contains"},
        "wildfire_verdict": {
        ...
        "first_seen": {"api_name": "sample.create_date", "operator": "is in the range"},
        "last_updated": {"api_name": "sample.update_date", "operator": "is in the range"},
        "time_range": {"api_name": "session.tstamp", "operator": "is in the range"},
        "time_after": {"api_name": "session.tstamp", "operator": "is after"},
        "time_before": {"api_name": "session.tstamp", "operator": "is before"},
    },
    "file_indicators": {
        "Size": "Size",
        "SHA1": "SHA1",
...
SAMPLE_ANALYSIS_LINE_KEYS = {
    "behavior": {"display_name": "behavior", "indexes": {"risk": 0, "behavior": -1}},
    "process": {"display_name": "processes", "indexes": {"parent_process": 0, "action": 1}},
    "file": {"display_name": "files", "indexes": {"parent_process": 0, "action": 1}},
    "registry": {"display_name": "registry", "indexes": {"action": 1, "parameters": 2}},
    "dns": {"display_name": "DNS", "indexes": {"query": 0, "response": 1}},
    "http": {"display_name": "HTTP", "indexes": {"host": 0, "method": 1, "url": 2}},
    "connection": {"display_name": "connections", "indexes": {"destination": 2}},
    "mutex": {"display_name": "mutex", "indexes": {"process": 0, "action": 1, "parameters": 2}},
}
SAMPLE_ANALYSIS_COVERAGE_KEYS = {
    "wf_av_sig": {"display_name": "wildfire_signatures", "fields": ["name", "create_date"]},
    "fileurl_sig": {"display_name": "fileurl_signatures", "fields": ["name", "create_date"]},
    "dns_sig": {"display_name": "dns_signatures", "fields": ["name", "create_date"]},
    "url_cat": {"display_name": "url_categories", "fields": ["url", "cat"]},
}

VERDICTS_TO_DBOTSCORE = {"benign": 1, "malware": 3, "grayware": 2, "phishing": 3, "c2": 3}
...
API_POINTS_TABLE = CommandResults(outputs_prefix="AutoFocus.Quota", replace_existing=True)
...
class Client(BaseClient):
    ...
    def http_request(
        self, url_suffix, method="POST", data={}, err_operation=None, ok_codes=(200, 409, 503), headers=HEADERS, params={}
    ):
...
def run_polling_command(client: Client, args: dict, cmd: str, search_function: Callable, results_function: Callable):
    ...
    if "af_cookie" not in args:
        ...
        if outputs.get("Status") != "complete":
            polling_args = {"af_cookie": af_cookie, "interval_in_seconds": interval_in_secs, "polling": True, **args}
...
def validate_sort_and_order_and_artifact(
    sort: Optional[str] = None, order: Optional[str] = None, artifact_source: Optional[str] = None
) -> bool:
    """
...
def do_search(
    ...
    err_operation: Optional[str] = None,
...
def get_data_from_line(line, category_name):
    ...
    if not category_indexes:
        ...
        for sub_category in category_indexes:  # type: ignore
            ...
            sub_categories.update({sub_category: values[sub_category_index]})
        return sub_categories
...
def autofocus_top_tags_search(client: Client, scope, tag_class_display, private, public, commodity, unit42):
    ...
    query = {"operator": "all", "children": [{"field": "sample.tag_class", "operator": "is", "value": tag_class}]}
    tag_scopes = []
    if private:
    ...
    data = {"query": query, "scope": scope, "tagScopes": tag_scopes}
    ...
    status = "in progress" if in_progress else "complete"
    search_info = {"AFCookie": resp.get("af_cookie"), "Status": status}
    return search_info
...
def print_hr_by_category(category_name, category_data):
    ...
    if category_name == "coverage":
        ...
        if category_data:
            hr = tableToMarkdown(
                f"{string_to_table_header(category_name)}:", category_data, headerTransform=string_to_table_header
            )
        else:
            hr = f"### {string_to_table_header(category_name)}:\nNo entries"
        demisto.results({"Type": entryTypes["note"], "ContentsFormat": formats["text"], "Contents": content, "HumanReadable": hr})
    else:
        for os_name, os_data in category_data.items():
            content = os_data
            table_header = f"{category_name}_{os_name}"
            if os_data:
                hr = tableToMarkdown(f"{string_to_table_header(table_header)}:", os_data, headerTransform=string_to_table_header)
            else:
                hr = f"### {string_to_table_header(table_header)}:\nNo entries"
            demisto.results(
                {"Type": entryTypes["note"], "ContentsFormat": formats["text"], "Contents": content, "HumanReadable": hr}
            )
...
def build_sample_search_query(used_indicator, indicators_values, wildfire_verdict, first_seen, last_updated):
    ...
    if wildfire_verdict:
        filtering_args_for_search["wildfire_verdict"] = demisto.get(
            API_PARAM_DICT, f"search_arguments.wildfire_verdict.translate.{wildfire_verdict}"
        )
    if first_seen:
...
def build_logic_query(logic_operator, condition_list):
    return {"operator": {"AND": "all", "OR": "any"}.get(logic_operator), "children": condition_list}
...
def children_list_generator(field_name, operator, val_list):
    ...
    for value in val_list:
        query_list.append({"field": field_name, "operator": operator, "value": value})
    return query_list
...
def search_samples_command(client: Client, args):
    ...
    info = search_samples(
        ...
        ip=ip,
        url=url,
        wildfire_verdict=wildfire_verdict,
    ...
    return CommandResults(
        outputs=info, readable_output=md, outputs_key_field="AFCookie", outputs_prefix="AutoFocus.SamplesSearch"
    )
...
def search_sessions_with_polling_command(client: Client, args):
    return run_polling_command(
        client, args, "autofocus-search-sessions", search_sessions_command, sessions_search_results_command
    )
...
def search_sessions_command(client: Client, args):
    ...
    cmd_results = CommandResults(
        outputs_prefix="AutoFocus.SessionsSearch", outputs_key_field="AFCookie", outputs=info, readable_output=md
    )
    return cmd_results
...
def samples_search_result_hr(result: dict, status: str) -> str:
    ...
    hr = "\n\n".join(
        (
            ...
            tableToMarkdown(
                "Artifacts for Sample: ", updated_artifact, headers=["b", "g", "m", "indicator_type", "confidence", "indicator"]
            ),
        )
    )
    return hr
...
def get_session_details_command(client: Client):
    ...
    context = {"AutoFocus.Sessions(val.ID === obj.ID)": result, outputPaths["file"]: files}
    demisto.results(
        {
...
def top_tags_search_command(client: Client, args):
    ...
    return CommandResults(
        outputs_prefix="AutoFocus.TopTagsSearch", outputs_key_field="AFCookie", outputs=info, readable_output=md
    )
        ...
        if indicator:
            ...
            if create_relationships:
                relationships = create_relationships_list(
                    entity_a=ip_address, entity_a_type=indicator_type, tags=raw_tags, reliability=reliability
                )
            ip = Common.IP(
                ip=ip_address,
...
def search_domain_command(client, domain, reliability, create_relationships):
    ...
    for domain_name in domain_name_list:
        ...
        if indicator:
            ...
            if create_relationships:
                relationships = create_relationships_list(
                    entity_a=domain_name, entity_a_type=indicator_type, tags=raw_tags, reliability=reliability
                )
            ...
            domain = Common.Domain(domain=domain_name, dbot_score=dbot_score)
...
def search_url_command(client, url, reliability, create_relationships, separator=","):
    ...
    for url_name in url_list:
        ...
        if indicator:
            ...
            if create_relationships:
                relationships = create_relationships_list(
                    entity_a=url_name, entity_a_type=indicator_type, tags=raw_tags, reliability=reliability
                )
            url = Common.URL(
                url=url_name,
                dbot_score=dbot_score,
            ...
            url = Common.URL(url=url_name, dbot_score=dbot_score)
...
def search_file_command(client, file, reliability, create_relationships):
    ...
    for file_hash in file_list:
        ...
        if indicator:
            ...
            if create_relationships:
                relationships = create_relationships_list(
                    entity_a=file_hash, entity_a_type=indicator_type, tags=raw_tags, reliability=reliability
                )
            ...
            file = Common.File(dbot_score=dbot_score, **hash_val_arg)
    ...
    for tag in tags:
        ...
        if entity_b and relation_by_type:
            relationships.append(
                EntityRelationship(
                    relation_by_type.get("name"),
                    entity_a=entity_a,
                    entity_a_type=entity_a_type,
                    entity_b=entity_b,
                    entity_b_type=FeedIndicatorType.indicator_type_by_server_version(relation_by_type.get("entity_b_type")),
...
def get_export_list_command(client: Client, args):
    ...
    data = {"label": args.get("label"), "panosFormatted": True, "apiKey": ""}

    results = client.http_request(
        url_suffix="/export", method="POST", data=data, err_operation=f"Failed to fetch export list: {args.get('label')}"
    )
    ...
    for indicator_value in results.get("export_list"):
        ...
        if indicator_type in [FeedIndicatorType.IP, FeedIndicatorType.IPv6, FeedIndicatorType.IPv6CIDR, FeedIndicatorType.CIDR]:
            if "-" in indicator_value:
                context_ip.append({"Address": indicator_value.split("-")[0]})
                context_ip.append({"Address": indicator_value.split("-")[1]})

            elif ":" in indicator_value:
                context_ip.append({"Address": indicator_value.split(":", 1)[0]})

            else:
                context_ip.append({"Address": indicator_value})

        elif indicator_type == FeedIndicatorType.Domain:
            context_domain.append({"Name": indicator_value})

        elif indicator_type == FeedIndicatorType.File:
            context_file.append({"SHA256": indicator_value})
...
def main():
    if is_demisto_version_ge("8.12.0"):
        ...
        return_warning("AutoFocus v2 is deprecated and will stop working after November 1, 2025. Please migrate to Unit 42 Intelligence integration.")
    command = demisto.command()
