...
FILE_RES_JSON = {"indicator": {"latestPanVerdicts": {"WF_SAMPLE": "MALWARE"}}}
...
IP_RES_JSON = {
    "indicator": {
        ...
        "latestPanVerdicts": {"PAN_DB": "MALWARE"},
        "seenByDataSourceIds": ["WF_SAMPLE"],
        "wildfireRelatedSampleVerdictCounts": {"MALWARE": 95, "GRAYWARE": 5},
    },
    "tags": [
        {
            "support_id": 1,
            ...
            "tagGroups": [
                {
                    ...
                    "description": "This type of malware secretly downloads malicious files from a remote server, "
...
def test_connection_error(mocker, autofocusv2_client):
    ...
    def raise_connection_error(url_suffix, method, ok_codes, headers, params, data={}, err_operation=None):
        ...
        assert method == "GET"
        assert headers == {"Content-Type": "application/json", "apiKey": "1234"}
        assert params == {
            "indicatorType": "ip",
    ...
    with pytest.raises(
        AutofocusV2.DemistoException, match="Error connecting to server. Check your URL/Proxy/Certificate settings"
    ):
...
TAGS_FROM_RESPONSE = [
    {
        ...
        "source": "Unit 42",
        "support_id": 1,
        "tagGroups": [{"description": "Remote", "tag_group_name": "RemoteAccessTrojan"}],
        "tag_class_id": 3,
...
def test_create_relationships_list():
    ...
    relationships = create_relationships_list(
        entity_a="Test", entity_a_type="IP", tags=RAW_TAGS, reliability="B - Usually reliable"
    )
...
def test_search_url_command(mocker, autofocusv2_client):
    ...
    mock_response = {
        "indicator": {"indicatorValue": "www.こんにちは.com", "indicatorType": "URL", "latestPanVerdicts": {"PAN_DB": "BENIGN"}},
        "tags": [],
    }
...
@pytest.mark.parametrize(
    "separator, expected_value", [("|", ["https:firstpart,connectedpart"]), (None, ["https:firstpart", "connectedpart"])]
)
def test_search_url_custom_separator(mocker, autofocusv2_client, separator, expected_value):
    ...
    # Mock args
    args = {
        "url": "https:firstpart,connectedpart",
...
def test_search_url_command_args(mocker, autofocusv2_client):
    ...
    mock_response = {
        "indicator": {"indicatorValue": "www.test.com", "indicatorType": "URL", "latestPanVerdicts": {"PAN_DB": "BENIGN"}},
        "tags": [],
    }
...
TEST_DATA = [
    ("autofocus_md5_response", "123456789012345678901234567890ab", ["123456789012345678901234567890ab", None, None]),
    (
...
@pytest.mark.parametrize(argnames="mock_response, domain", argvalues=[("autofocus_domain_response", "mail16.amadeus.net")])
...
def test_metrics(mocker: MockerFixture, autofocusv2_client):
    ...
    assert (
        return_results_mock.call_args_list[2][0][0].readable_output
