...
- **Palo Alto Networks AutoFocus v2**: Distinguish the most important threats from everyday commodity attacks.
- **AutoFocus Tags Feed**: Use the AutoFocus Tags Feed integration to fetch indicators from AutoFocus Tags.
- **Palo Alto AutoFocus**: (Deprecated) Use the Palo Alto Networks AutoFocus v2 integration instead.
- **AutoFocus Daily Feed**: (Deprecated) Fetch new, daily additions to the list created by the AutoFocus feed.

### Playbooks
- **Autofocus Query Samples, Sessions and Tags**: Query the PANW threat intelligence Autofocus system. The playbook accepts indicators such as IPs, hashes, domains to run basic queries or mode advanced queries that can leverage several query parameters. 
