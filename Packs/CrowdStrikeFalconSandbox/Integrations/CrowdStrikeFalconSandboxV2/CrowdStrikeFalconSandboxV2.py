...
class Client(BaseClient):
    ...
    def get_report(self, key: str, filetype: str) -> Response:
        return self._http_request(
            method="GET", url_suffix=f"/report/{key}/report/{filetype}", resp_type="response", ok_codes=(200, 404)
        )
    ...
    def submit_url(self, url: str, params: dict[str, Any]) -> dict[str, Any]:
        return self._http_request(method="POST", data={"url": url, **params}, url_suffix="/submit/url")

    def submit_file(self, file_contents: dict, params: dict[str, Any]) -> dict:
        return self._http_request(
            method="POST",
            data=params,
            url_suffix="/submit/file",
...
def translate_verdict(param: str) -> int:
    return {"Whitelisted": 1, "NoVerdict": 2, "NoSpecificThreat": 3, "Suspicious": 4, "Malicious": 5}[param]
...
def create_scan_results_readable_output(scan_response) -> Any:
    ...
    return tableToMarkdown(
        "Scan Results:",
        scan_response,
        headers=list(table_field_dict.keys()),
...
def get_dbot_score(filehash, threat_score: int) -> Common.DBotScore:
    def calc_score() -> int:
        return {3: 0, 2: 3, 1: 2, 0: 1}.get(threat_score, 0)

    return Common.DBotScore(
        indicator=filehash,
        integration_name="CrowdStrike Falcon Sandbox V2",
...
def submission_response(client, response, polling) -> List[CommandResults]:
    context = {
        "CrowdStrike.Submit(val.submission_id && val.submission_id === obj.submission_id)": response,
        "CrowdStrike.JobID": response.get("job_id"),
        "CrowdStrike.EnvironmentID": response.get("environment_id"),
    }
    submission_res = CommandResults(
        outputs_prefix="",
        outputs_key_field="",
        raw_response=response,
        outputs=context,
        readable_output=tableToMarkdown("Submission Data:", response, headerTransform=underscore_to_space),
    )
...
def crowdstrike_analysis_overview_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    file = Common.File(
        ...
        name=result.get("last_file_name"),
    )
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(
            "Analysis Overview:", result, headers=table_cols, headerTransform=underscore_to_space, removeNull=True
        ),
    )
...
def crowdstrike_result_command(args: dict[str, Any], client: Client):
    ...
    if successful_response:
        ret_list = [
            fileResult(get_default_file_name(args["file-type"]), report_response.content, file_type=EntryType.ENTRY_INFO_FILE)
        ]
        if args.get("file"):
...
def crowdstrike_report_state_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    state = client.get_state(key)
    return CommandResults(
        outputs_prefix="CrowdStrike.State",
        raw_response=state,
        outputs=state,
...
def crowdstrike_get_environments_command(client: Client, _) -> CommandResults:
    ...
    environments = [
        map_dict_keys(
            ...
            {
                "environment_id": "ID",
                "total_virtual_machines": "VMs_total",
                "analysis_mode": "analysisMode",
                "group_icon": "groupicon",
                "busy_virtual_machines": "VMs_busy",
            },
        )
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(
            ...
            environments,
            list(readable_output_column_conversion.keys()),
            removeNull=True,
...
def main() -> None:
    ...
    try:
        headers: dict = {"api-key": demisto.params().get("credentials", {}).get("password"), "User-Agent": "Falcon Sandbox"}
