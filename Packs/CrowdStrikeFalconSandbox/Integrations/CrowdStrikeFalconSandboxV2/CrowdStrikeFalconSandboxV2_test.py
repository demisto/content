...
import pytest
from CommonServerPython import Common, DBotScoreReliability, ScheduledCommand
from CrowdStrikeFalconSandboxV2 import (
    BWCFile,
    Client,
    crowdstrike_analysis_overview_command,
    crowdstrike_analysis_overview_refresh_command,
    crowdstrike_analysis_overview_summary_command,
    crowdstrike_result_command,
    crowdstrike_scan_command,
    crowdstrike_submit_sample_command,
    crowdstrike_submit_url_command,
    get_api_id,
    get_default_file_name,
    get_search_term_args,
    get_submission_arguments,
    main,
    map_dict_keys,
    split_query_to_term_args,
...
client = Client(base_url=BASE_URL, verify=False, proxy=False, headers={})
...
@pytest.mark.parametrize(
    ...
    [
        ("hello:world,three:split:fine,heelo:, another: arg ", {"hello": "world", "three": "split:fine", "another": "arg"}),
...
def test_bwc_file_context():
    ...
    sha256 = "filehash"
    context = BWCFile(
        {"environment_description": ed_val, "type": type_val},
        {"type": "type1"},
        False,
        sha256=sha256,
...
def test_crowdstrike_submit_url_command_no_poll(requests_mock):
    ...
    assert (
        result[0].outputs["CrowdStrike.Submit(val.submission_id && val.submission_id === obj.submission_id)"] == submit_response
    )
...
def test_crowdstrike_submit_sample_command(mocker, requests_mock):
    ...
    mocker.patch.object(
        demisto, "getFilePath", return_value={"id": id, "path": "./test_data/scan_response.json", "name": "scan_response.json"}
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("cs-falcon-sandbox-search", "crowdstrike_search_command"),
        ("crowdstrike-get-environments", "crowdstrike_get_environments_command"),
        ("cs-falcon-sandbox-report-state", "crowdstrike_report_state_command"),
    ],
)
