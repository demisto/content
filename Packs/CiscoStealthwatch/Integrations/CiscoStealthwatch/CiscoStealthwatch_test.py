...
def test_cisco_stealthwatch_query_flows_initialize_command(requests_mock):
    ...
    req_mock = requests_mock.post('https://token/v2/authenticate', cookies={'cookies': 'jar', 'XSRF-TOKEN': 'some token'})
    # req_mock = requests_mock.post('https://token/v2/authenticate', cookies={'cookies': 'jar'})
    ...
    assert req_mock.last_request.headers['XSRF-TOKEN']
