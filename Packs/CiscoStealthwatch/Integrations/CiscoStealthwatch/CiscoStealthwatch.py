...
class Client(BaseClient):
    ...
    def prepare_request(
        self, url_suffix: str, method: str = "GET", data: dict = {}, json_data: dict = {}, resp_type: str = "json"
    ):
    ...
    def _get_cookies(self) -> requests.cookies.RequestsCookieJar:
        data = {"username": self._auth[0], "password": self._auth[1]}

        response = self._http_request(method="POST", url_suffix="/token/v2/authenticate", data=data, resp_type="response")
...
def cisco_stealthwatch_query_flows_initialize_command(
    ...
    data = remove_empty_elements(
        {
            ...
            "recordLimit": limit,
            "subject": {"ipAddresses": {"includes": ip_addresses if isinstance(ip_addresses, list) else [ip_addresses]}},
        }
    )
    ...
    table = tableToMarkdown(
        ...
        outputs,
        headers=["id", "status", "percentComplete"],
        removeNull=True,
...
def cisco_stealthwatch_query_flows_status_command(client: Client, tenant_id: str, search_id: str) -> CommandResults:
...
def cisco_stealthwatch_query_flows_results_command(client: Client, tenant_id: str, search_id: str) -> CommandResults:
    ...
    for data in dict_safe_get(response, ["data", "flows"]):
        outputs.append(data)
    headers = ["id", "tenantId", "flowCollectorId", "protocol", "serviceId", "statistics", "peer", "subject"]
    table = tableToMarkdown(
        "Query Flows Results Information:", outputs, headers=headers, removeNull=True, headerTransform=pascalToSpace
    )
    return CommandResults(
...
def cisco_stealthwatch_list_tags_command(client: Client, tenant_id: str) -> CommandResults:
    ...
    table = tableToMarkdown(
        f"Tags for tenant_id: {tenant_id}:",
        outputs,
...
def cisco_stealthwatch_get_tag_command(client: Client, tenant_id: str, tag_id: str) -> CommandResults:
    ...
    table = tableToMarkdown(
        f"Tag {tag_id} with tenant id {tenant_id} results:",
        outputs,
...
def cisco_stealthwatch_list_tenants_command(client: Client, tenant_id: str = None) -> CommandResults:
    ...
    if tenant_id:
        ...
        table = tableToMarkdown(
            f"Tenant {tenant_id}:", outputs, headers=["id", "displayName"], removeNull=True, headerTransform=pascalToSpace
        )
        ...
        table = tableToMarkdown(
            "Tenants:", outputs, headers=["id", "displayName"], removeNull=True, headerTransform=pascalToSpace
        )
...
def cisco_stealthwatch_get_tag_hourly_traffic_report_command(client: Client, tenant_id: str, tag_id: str) -> CommandResults:
    ...
    table = tableToMarkdown(title, outputs, headers=headers, removeNull=True, headerTransform=pascalToSpace)
...
def cisco_stealthwatch_get_top_alarming_tags_command(client: Client, tenant_id: str) -> CommandResults:
...
def cisco_stealthwatch_list_security_events_initialize_command(
    client: Client, tenant_id: str, start_time: str = None, end_time: str = None, time_range: str = None
) -> CommandResults:
    ...
    if not start_time:
        raise Exception("Invalid time format. Check: start_time, time_range, and end_time")
    data = {"timeRange": {"from": start_time, "to": end_time}}
    ...
    table = tableToMarkdown(
        ...
        outputs,
        headers=["id", "searchJobStatus", "percentComplete"],
        removeNull=True,
        headerTransform=pascalToSpace,
    )
    return CommandResults(
...
def cisco_stealthwatch_list_security_events_status_command(client: Client, tenant_id: str, search_id: str) -> CommandResults:
...
def cisco_stealthwatch_list_security_events_results_command(
    client: Client, tenant_id: str, search_id: str, limit: int
) -> CommandResults:
...
def times_handler(start_time: str = None, end_time: str = None, time_range: str = None):
    ...
    """
    start_time_obj = dateparser.parse(time_range) if time_range else dateparser.parse(start_time)  # type: ignore
...
def main():
    ...
    try:
        ...
        if command == "test-module":
            ...
            return_results(cisco_stealthwatch_query_flows_initialize_command(client, **demisto.args()))
            ...
            return_results(cisco_stealthwatch_get_tag_hourly_traffic_report_command(client, **demisto.args()))

        elif command == "cisco-stealthwatch-get-top-alarming-tags":
            return_results(cisco_stealthwatch_get_top_alarming_tags_command(client, **demisto.args()))

        elif command == "cisco-stealthwatch-list-security-events-initialize":
            return_results(cisco_stealthwatch_list_security_events_initialize_command(client, **demisto.args()))

        elif command == "cisco-stealthwatch-list-security-events-status":
            return_results(cisco_stealthwatch_list_security_events_status_command(client, **demisto.args()))

        elif command == "cisco-stealthwatch-list-security-events-results":
            return_results(cisco_stealthwatch_list_security_events_results_command(client, **demisto.args()))
