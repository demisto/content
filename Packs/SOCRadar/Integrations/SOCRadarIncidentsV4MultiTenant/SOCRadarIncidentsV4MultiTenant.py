...
def fetch_incidents(
    ...
    demisto.debug(f"[SOCRadar-MT] Previously fetched alarm IDs: {len(cached_alarms)}")
        ...
        while True:
            ...
            if total_incidents_created >= max_results:
                ...
                if current_page == latest_page:
                    ...
                    next_run_incidents_to_skip = len(page_incidents)
                demisto.debug(f"[SOCRadar-MT] Reached max_results ({max_results}), fetch pages is stopped. Next run will start from the date of the latest run with skipping {next_run_incidents_to_skip} incidents")
                break
            else:
