...
class Client(BaseClient):
    ...
    def change_alarm_status(
        self, alarm_ids: List[int], status_reason: str, comments: Optional[str] = None, company_id: Optional[str] = None
    ) -> Dict[str, Any]:
        ...
        # Use provided company_id or fall back to default
        company_id_to_use = company_id or self.company_id
        url_suffix = f"/company/{company_id_to_use}/alarms/status/change"
        json_data = {
            "alarm_ids": [str(aid) for aid in alarm_ids],
            "status": STATUS_REASON_MAP[status_reason_upper],
    ...
    def add_alarm_comment(self, alarm_id: int, user_email: str, comment: str, company_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    def change_alarm_assignee(
        self, alarm_id: int, user_ids: Optional[List[int]] = None, user_emails: Optional[List[str]] = None, company_id: Optional[str] = None
    ) -> Dict[str, Any]:
    ...
    def add_remove_tag(self, alarm_id: int, tag: str, company_id: Optional[str] = None) -> Dict[str, Any]:
        """Add or remove a tag from an alarm"""
        # Use provided company_id or fall back to default
        company_id_to_use = company_id or self.company_id
        url_suffix = f"/company/{company_id_to_use}/alarm/tag"
...
def fetch_incidents(
    ...
    if last_fetch:
        ...
        demisto.debug(f"[SOCRadar] Subsequent fetch: Using last_fetch timestamp")
    else:
    ...
    try:
        ...
        # Update next run
        next_run = {
            "last_fetch": latest_timestamp.isoformat() + "Z",
