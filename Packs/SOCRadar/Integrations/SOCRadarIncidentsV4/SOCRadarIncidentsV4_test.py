...
def test_alarm_to_incident_include_related_assets():
    ...
    alarm = _make_alarm()
    alarm["alarm_related_assets"] = [
        {"key": "domain", "value": ["example.com", "test.com"]}
    ]

    incident = alarm_to_incident(alarm, include_related_assets=True)
    assert (
        "domain: example.com || test.com"
        in incident["CustomFields"]["socradarrelatedassets"]
    )
...
def test_alarm_to_incident_include_company_id():
    ...
    incident = alarm_to_incident(
        alarm, include_company_id=True, configured_company_id="789"
    )
...
def test_fetch_incidents_passes_include_params(mock_client, mocker):
    ...
    mocker.patch.object(
        mock_client, "search_incidents", return_value=_mock_response([alarm])
    )
...
def test_test_module_unexpected_exception(mock_client, mocker, capfd):
    ...
    mocker.patch.object(
        mock_client, "search_incidents", side_effect=RuntimeError("unexpected")
    )
    with capfd.disabled():
        result = test_module(mock_client)
