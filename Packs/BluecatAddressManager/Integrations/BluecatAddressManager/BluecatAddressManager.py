...
class Client:
    ...
    def get_token_request(self):
        url_args = {"username": self._username, "password": self._password}
        start_idx = 16
        end_delim = " <-"
...
def query_ip_command(client: Client):
    ...
    try:
        ...
        if base_ip_raw_res.get("id") in (None, 0, client.conf):
            ...
            ec = {"BlueCat.AddressManager.IP(obj.ID === val.ID)": ip_object, "IP(val.Address === obj.Address)": {"Address": ip}}
...
def query_ipv4(client: Client, ip: str):
    params = {"containerId": client.conf, "address": ip}
...
def get_range_by_ip(client: Client, ip: str):
    params = {"containerId": client.conf, "type": "", "address": ip}
...
def get_response_policies(client: Client, start: str, count: str):
    params = {"parentId": client.conf, "type": "ResponsePolicy", "start": start, "count": count}
...
def add_domain_response_policy(client: Client, policy_id: str, domain: str):
    params = {"policyId": policy_id, "itemName": domain}
...
def remove_domain_response_policy_command(client: Client):
    ...
    error_msg = (
        f"Failed to remove {domain} from response policy {policy_id}, "
...
def search_response_policy_by_domain(client: Client, domain: str):
    params = {"configurationId": client.conf, "itemName": domain}
