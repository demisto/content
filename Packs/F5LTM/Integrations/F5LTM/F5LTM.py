...
class Client(BaseClient):
    def __init__(self, base_url: str, token: object, partition: str, use_ssl: bool, use_proxy: bool, **kwargs):
        ...
        self.headers = {"Content-Type": "application/json", "X-F5-Auth-Token": token}
    ...
    def get_pools(self, expand_collection=False, partition=None):
        url_suffix = (
            f"ltm/pool?expandSubcollections={str(expand_collection).lower()}&"
    ...
    def get_pool_members(self, pool, partition=None):
        ...
        response = self._http_request(method="GET", url_suffix=url_suffix, headers=self.headers, params={})
        return {"name": pool, "members": response.get("items")}

    def get_nodes(self, partition=None):
        url_suffix = (
            f"ltm/node?$select=name,partition,address,ration,session,state&$filter=partition eq "
            f"{partition if partition else self.partition}"
        )
        response = self._http_request(method="GET", url_suffix=url_suffix, headers=self.headers, params={})
    ...
    def disable_node(self, node, partition=None):
        ...
        response = self._http_request(
            method="PATCH", url_suffix=url_suffix, headers=self.headers, json_data={"session": "user-disabled"}
        )
        return response
    ...
    def enable_node(self, node, partition=None):
        url_suffix = f"ltm/node/~{partition if partition else self.partition}~{node}"
        response = self._http_request(
            method="PATCH", url_suffix=url_suffix, headers=self.headers, json_data={"session": "user-enabled"}
        )
        return response
    ...
    def get_pool_member_stats(self, pool, member, partition=None):
        ...
        return {"pool": pool, "stats": pool_stats, "members": [{"name": member, "stats": member_stats}]}
...
def login(server: str, port: str, username: str, password: str, verify_certificate: bool):
    response = requests.post(
        f"https://{server}:{port}/mgmt/shared/authn/login",
...
def ltm_get_pool_members_command(client, args) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        "Pool Members:", {"name": results.get("name"), "members": [member["name"] for member in results.get("members")]}
    )

    return CommandResults(
        outputs_prefix="F5.LTM.Pools", outputs_key_field="name", outputs=results, readable_output=readable_output
    )
...
def ltm_get_pool_member_stats_command(client, args) -> CommandResults:
    ...
    return CommandResults(
        outputs_prefix="F5.LTM.Stats", outputs_key_field="name", outputs=results, readable_output=readable_output
    )
...
def ltm_get_node_stats_command(client, args) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        "Node Stats:", {"node": results.get("name"), "curConns": results.get("stats")["serverside.curConns"]["value"]}
    )
    return CommandResults(
        outputs_prefix="F5.LTM.Nodes", outputs_key_field="name", outputs=results, readable_output=readable_output
    )
    ...
    for item in results:
        for subitem in item.get("members"):
            if subitem.get("name").split(":")[0] == node:
                ...
                break
    node_mapping = {"name": node, "pools": pools}
    return CommandResults(
        outputs_prefix="F5.LTM.Nodes",
        outputs_key_field="name",
...
def main() -> None:
    ...
    try:
        client = Client(base_url=base_url, token=token, partition=partition, use_ssl=verify_certificate, use_proxy=proxy)
