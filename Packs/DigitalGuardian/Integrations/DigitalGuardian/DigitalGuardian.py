...
def request_api_token():
    ...
    """
    payload = {"client_id": CLIENT_ID, "client_secret": CLIENT_SECRET, "grant_type": "client_credentials", "scope": "client"}
...
def get_watchlist_entry_id(watchlist_name: str, watchlist_entry: str) -> str:
    ...
    if watchlist_id:
        full_url = ARC_URL + "/watchlists/"
        r = requests.get(url=full_url + watchlist_id + "/values?limit=100000", headers=CLIENT_HEADERS, verify=VERIFY_CERT)
        json_text = json.loads(r.text)
        ...
        if list_id:
            ...
            if 200 <= r.status_code <= 299:
                ...
                return_error(
                    ...
                    f" The response failed with status "
...
def check_componentlist_entry():
    ...
    if componentlist:
        ...
        return_outputs(
            ...
            outputs={"DigitalGuardian.Componentlist.Found": False},
            raw_response="Componentlist entry not found",
        )
...
def rm_entry_from_componentlist():
    ...
    if 200 <= r.status_code <= 299:
        ...
        return_error(
            ...
            f" ({componentlist_name}). The response failed with "
...
def add_entry_to_watchlist():
    ...
    full_url = ARC_URL + "/watchlists/"
    r = requests.post(
        url=full_url + watchlist_id + "/values/", data=watchlist_entry_json, headers=CLIENT_HEADERS, verify=VERIFY_CERT
    )
    if 200 <= r.status_code <= 299:
        demisto.results(f"added watchlist entry ({watchlist_entry}) to watchlist name ({watchlist_name})")
    else:
        return_error(
            f"Failed to add watchlist entry({watchlist_entry}) to watchlist name ({watchlist_name})."
            f" The response failed with status code {r.status_code}. "
...
def check_watchlist_entry():
    ...
    if watchlist_entry_id:
        return_outputs(
            readable_output="Watchlist found", outputs={"DigitalGuardian.Watchlist.Found": True}, raw_response="Watchlist found"
        )
    else:
        return_outputs(
            readable_output="Watchlist not found",
            outputs={"DigitalGuardian.Watchlist.Found": False},
...
def rm_entry_from_watchlist():
    ...
    full_url = ARC_URL + "/watchlists/"
    r = requests.delete(url=full_url + watchlist_id + "/values/" + watchlist_entry_id, headers=CLIENT_HEADERS, verify=VERIFY_CERT)
    if 200 <= r.status_code <= 299:
        demisto.results(f"removed watchlist entry ({watchlist_entry}) from watchlist name ({watchlist_name})")
    else:
        return_error(
            f"Failed to remove watchlist entry({watchlist_entry}) from watchlist name ({watchlist_name})."
            f" The response failed with status code {r.status_code}. "
            f"The response was: {r.text}"
        )
...
def get_items_request():
    ...
    if r.status_code == 200:
        ...
        return_error(
            "DigitalGuardian ARC Export Failed "
            "Please check authentication related parameters. " + json.dumps(r.json(), indent=4, sort_keys=True)
        )
        return None
