...
def setup_params(mocker):
    demisto_params = {"auth_url": auth_url, "arc_url": arc_url, "client_id": "client_id", "client_secret": "client_secret"}
...
def test_test_module(mocker, capfd):
    ...
    with requests_mock.Mocker() as request_mocker:
        ...
        request_mocker.register_uri("POST", re.compile(auth_url), json={"access_token": "access_token"}, status_code=200)
...
@pytest.mark.parametrize(
    "dg_severity, demisto_severity", [("Low", 1), ("Medium", 2), ("High", 3), ("Critical", 4), (None, 1), ("Other", 1)]
)
...
@pytest.mark.parametrize(
    "was_classified, demisto_sensitivity",
    [
        ("Something", "none"),
        (None, "none"),
        ("LOW", "Low"),
