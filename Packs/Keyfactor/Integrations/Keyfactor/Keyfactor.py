...
class Client(BaseClient):
    ...
    def get_enrollment_csr_context_template(self) -> dict:
        ...
        headers = {"x-keyfactor-api-version": "1", "x-keyfactor-requested-with": "APIClient", "Accept": "application/json"}

        return self._http_request(method="GET", url_suffix="Enrollment/CSR/Context/My", headers=headers)

    def post_enrollment_csr(
        self,
        csr_base64: str,
        cert_authority: str,
        include_chain: str,
        ...
        return self._http_request(method="POST", url_suffix="Enrollment/CSR", headers=headers, json_data=body)
...
def get_enrollment_csr_context_template_ec(raw_response: dict) -> tuple[list, list]:
    ...
    if raw_response:
        for template in raw_response["Templates"]:
            entry_context.append(assign_params(Name=template.get("Name"), CAs=template.get("CAs")[0].get("Name")))
            human_readable.append(assign_params(Name=template.get("Name"), CAs=template.get("CAs")[0].get("Name")))
...
@logger
def post_enrollment_csr_command(
    client: Client,
    csr_base64: dict,
    cert_authority: str,
    include_chain: str,
    ...
    raw_response: dict = client.post_enrollment_csr(
...
def main():
    ...
    client = Client(base_url=params.get("host"), verify=verify_ssl, proxy=proxy, auth=(username, password))
