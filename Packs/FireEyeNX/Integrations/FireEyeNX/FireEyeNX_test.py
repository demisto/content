...
def test_module_success_with_fetch_incident(
    ...
    mock_last_run = {'alerts': {'start_time': datetime.now().replace(tzinfo=datetime.UTC).timestamp(), 'alert_ids': ['1']}}
...
def test_fetch_incidents_for_alert_success(
    ...
    # Configure
    start_time = datetime.strftime(datetime.now().replace(tzinfo=datetime.UTC), API_SUPPORT_DATE_FORMAT)
