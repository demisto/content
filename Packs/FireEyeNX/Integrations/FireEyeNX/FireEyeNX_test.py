...
def test_get_api_token_when_not_found_in_integration_context(mocker_set_context, mocker_get_context, mock_request, client):
    """
...
def test_is_supported_context_type_failure(client):
    ...
    """
    assert not (client.is_supported_context_type("image/png"))


@pytest.mark.parametrize("context_type", [CONTENT_TYPE_ZIP, "text/csv", "application/pdf", "application/octet-stream"])
...
def test_add_time_suffix_into_arguments(client):
    ...
    args = {"start_time": "2020-05-20", "end_time": "2020-05-20"}
    add_time_suffix_into_arguments(args)
    actual_output = {
        "end_time": "2020-05-20T00:00:00.000-00:00",
...
def test_fetch_incidents_for_alert_success(mock_api_token, mock_request, set_attachment, replace_url, client):
    ...
    """
    from FireEyeNX import API_SUPPORT_DATE_FORMAT, fetch_incidents
...
def test_update_start_time_within_last_forty_eight_hours():
    ...
    from FireEyeNX import DATE_FORMAT, update_start_time
...
def test_update_start_time_older_than_last_forty_eight_hours():
    ...
    """
    from FireEyeNX import DATE_FORMAT, FORTY_EIGHT_HOURS_IN_SECOND, update_start_time
