...
class Client:
    def __init__(self, feed_url: str, tags: Optional[list] = None, tlp_color: Optional[str] = None, insecure: bool = False):
...
def get_indicators_command(client: Client) -> tuple[str, dict[Any, Any], dict[Any, Any]]:
    ...
    human_readable = tableToMarkdown(f"{INTEGRATION_NAME}:", indicators, headers=["value", "type"], removeNull=True)
...
def main():  # pragma: no cover
    ...
    try:
        ...
        commands: dict = {"test-module": test_module, "public-dns-get-indicators": get_indicators_command}
