...
API_TOKEN = "api_token"
FILE_ENTRY = {"name": "freshservice_ticket_create_update.json", "path": "test_data/freshservice_ticket_create_update.json"}
...
@pytest.mark.parametrize(
    "url, args, entity_name, output_prefix",
    [
        (
...
def test_get_freshservice_entities_command(
    ...
    mock_response_freshservice_entities_list = util_load_json(f"freshservice_{entity_name}.json")
...
def test_list_freshservice_ticket_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_ticket_list.json")
...
def test_get_freshservice_ticket_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_ticket_get.json")

    requests_mock.get(f"{SERVER_URL}api/v2/tickets/1", json=mock_response_freshservice_entities_list)
    results = list_freshservice_ticket_command(client=client, args={"ticket_id": 1, "command_name": "freshservice-ticket-list"})
...
def test_list_freshservice_problem_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_problem_list.json")
...
def test_get_freshservice_problem_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_problem_get.json")

    requests_mock.get(f"{SERVER_URL}api/v2/problems/1", json=mock_response_freshservice_entities_list)
    results = list_freshservice_ticket_command(client=client, args={"problem_id": 1, "command_name": "freshservice-problem-list"})
...
def test_list_freshservice_change_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_change_list.json")
...
def test_get_freshservice_change_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_change_get.json")

    requests_mock.get(f"{SERVER_URL}api/v2/changes/1", json=mock_response_freshservice_entities_list)
    results = list_freshservice_ticket_command(client=client, args={"change_id": 1, "command_name": "freshservice-change-list"})
...
def test_list_freshservice_release_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_release_list.json")
...
def test_get_freshservice_release_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_release_get.json")

    requests_mock.get(f"{SERVER_URL}api/v2/releases/1", json=mock_response_freshservice_entities_list)
    results = list_freshservice_ticket_command(client=client, args={"release_id": 1, "command_name": "freshservice-release-list"})
...
def test_create_update_freshservice_ticket_command(
    ...
    results = create_update_freshservice_ticket_command(client=client, args=args)
...
def test_get_freshservice_ticket_task_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_ticket_task_get.json")
...
def test_list_freshservice_ticket_task_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_ticket_task_list.json")
...
def test_create_update_freshservice_ticket_task_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_ticket_task_create_update.json")

    help_method = requests_mock.post if method == "POST" else requests_mock.put
    help_method(url, json=mock_response_freshservice_entities_list)

    results = create_update_freshservice_ticket_task_command(client=client, args=args)
...
def test_list_freshservice_ticket_conversation_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_ticket_conversation_list.json")

    requests_mock.get(url, json=mock_response_freshservice_entities_list)
    results = list_freshservice_ticket_conversation_command(client=client, args=args)
...
def test_create_freshservice_ticket_conversation_reply_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_ticket_conversation_reply_create.json")

    requests_mock.post(url, json=mock_response_freshservice_entities_list)

    results = create_freshservice_ticket_conversation_reply_command(client=client, args=args)
...
def test_create_freshservice_ticket_conversation_note_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_ticket_conversation_note_create.json")

    requests_mock.post(url, json=mock_response_freshservice_entities_list)

    results = create_freshservice_ticket_conversation_note_command(client=client, args=args)
...
def test_update_freshservice_ticket_conversation_command(
    ...
    mock_response_freshservice_entities_list = util_load_json("freshservice_ticket_conversation_update.json")

    requests_mock.put(url, json=mock_response_freshservice_entities_list)

    results = update_freshservice_ticket_conversation_command(client=client, args=args)
...
def test_delete_freshservice_ticket_conversation_command(
    ...
    results = delete_freshservice_ticket_conversation_command(client=client, args=args)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "ticket",
            1,
            {"page": None, "page_size": None, "updated_query": None, "command_response_key": "ticket"},
        ),
        (
            {
                "first_name": "Alice",
            ...
            None,
            {
                "page": 1,
                "page_size": 50,
                "updated_query": "\"first_name:'Alice' AND job_title:'Developer'\"",
        ...
        (
            ...
            "software",
            None,
            {"page": 1, "page_size": 50, "updated_query": "\"first_name:'Bob'\"", "command_response_key": "applications"},
        ),
    ],
)
def test_get_command_list_args(args, command_response_key, command_arg_id, output):
    """
    ...
    command_args = get_command_list_args(args, command_response_key, command_arg_id)
...
@pytest.mark.parametrize(
    "command_entity, command_request",
    [
...
def test_convert_command_properties(args, ticket_type, output):
    ...
    urgency, status, source, priority, impact, risk, change_type, release_type = convert_command_properties(args, ticket_type)
...
def test_convert_response_properties():
    ...
    from FreshworksFreshservice import TICKET_PROPERTIES_BY_TYPE, convert_response_properties
    ...
    assert converted_response[0]["ticket"].get("status") == updated_response["ticket"].get("status")
    assert converted_response[0]["ticket"].get("priority") == updated_response["ticket"].get("priority")
    assert converted_response[0]["ticket"].get("source") == updated_response["ticket"].get("source")
...
def test_get_request_arguments_per_ticket_type():
    ...
    from FreshworksFreshservice import TICKET_PROPERTIES_BY_TYPE, get_request_arguments_per_ticket_type

    response = get_request_arguments_per_ticket_type("ticket", {"status": "Open", "priority": "Low", "source": "Email"}, 11)

    assert response.get("status") == TICKET_PROPERTIES_BY_TYPE["ticket"]["status"]["Open"]
    assert response.get("priority") == TICKET_PROPERTIES_BY_TYPE["ticket"]["priority"]["Low"]
    assert response.get("source") == TICKET_PROPERTIES_BY_TYPE["ticket"]["source"]["Email"]
...
def test_validate_mandatory_ticket_requester_fields():
    ...
    with pytest.raises(ValueError) as error_info:
        ...
        assert str(error_info.value) == "One of the following is mandatory: requester_id, phone, email"
...
def test_get_alert_properties():
    ...
    assert response == (
        ["Incident/Service Request", "Problem Request"],
        [("impact", ["Low", "Medium"]), ("status", ["Open"]), ("risk", []), ("urgency", []), ("priority", ["Low", "High"])],
    )
