...
def test_format_list_policies():
    command_results = format_list_policy_functions(MOCK_EMPTY_RESPONSE, 'Hostname')
    outputs = command_results.outputs
    assert isinstance(outputs, list) and outputs == []

    command_results = format_list_policies(MOCK_POLICIES_RESPONSE)
    outputs = command_results.outputs
    assert len(outputs) == 2


def test_format_delete_policy():
    command_results = format_delete_policy(MOCK_POLICY_RESPONSE)
    outputs = command_results.outputs
    outputs = outputs
...
def test_format_list_policy():
    command_results = format_list_policy_functions(MOCK_EMPTY_RESPONSE, 'PolicyMethods')
    outputs = command_results.outputs
    assert isinstance(outputs, list) and outputs == []

    command_results = format_list_policy_functions(MOCK_METHODS_RESPONSE, 'PolicyMethods')
    outputs = command_results.outputs
    assert len(outputs) == 3


def test_format_policy_methods_command():
    command_results = format_policy_object(MOCK_METHOD_RESPONSE, 'PolicyMethods')
    outputs = command_results.outputs
    outputs = outputs
...
def test_format_list_policy_file_type():
    command_results = format_list_policy_functions(MOCK_EMPTY_RESPONSE, 'FileType')
    outputs = command_results.outputs
    assert isinstance(outputs, list) and outputs == []

    command_results = format_list_policy_functions(MOCK_FILETYPES_RESPONSE, 'FileType')
    outputs = command_results.outputs
    outputs = outputs
...
def test_format_file_type_command():
    command_results = format_policy_object(MOCK_FILETYPE_RESPONSE, 'FileType')
    outputs = command_results.outputs
...
def test_format_list_policy_cookies():
    command_results = format_list_policy_functions(MOCK_EMPTY_RESPONSE, 'Cookies')
    outputs = command_results.outputs
    assert isinstance(outputs, list) and outputs == []

    command_results = format_list_policy_functions(MOCK_COOKIES_RESPONSE, 'Cookies')
    outputs = command_results.outputs
    assert len(outputs) == 4


def test_format_cookies_command():
    command_results = format_policy_object(MOCK_COOKIE_RESPONSE, 'Cookies')
    outputs = command_results.outputs
    outputs = outputs
    outputs = {key: value for key, value in outputs.items() if value is not None}
...
def test_format_policy_hostnames_command():
    ...
    outputs = command_results.outputs
    assert isinstance(outputs, list) and outputs == []
    command_results = format_list_policy_functions(MOCK_HOSTNAMES_RESPONSE, 'Hostname')
    outputs = command_results.outputs
    assert len(outputs) == 4


def test_format_policy_hostname_command():
    command_results = format_policy_object(MOCK_HOSTNAME_RESPONSE, 'Hostname')
    outputs = command_results.outputs
    outputs = outputs
    outputs = {key: value for key, value in outputs.items() if value is not None}
...
def test_format_policy_blocking_settings_list_command():
    command_results = format_policy_blocking_settings_list_command(
...
def test_format_policy_blocking_settings_single_command():
    command_results = format_policy_blocking_settings_update_command(
...
def test_format_policy_urls_command():
    command_results = format_list_policy_functions(MOCK_EMPTY_RESPONSE, 'Url')
    outputs = command_results.outputs
    assert isinstance(outputs, list) and outputs == []
    command_results = format_list_policy_functions(MOCK_URLS_RESPONSE, 'Url')
    outputs = command_results.outputs
    assert len(outputs) == 7


def test_format_policy_url_command():
    command_results = format_policy_object(MOCK_URL_RESPONSE, 'Url')
    outputs = command_results.outputs
    outputs = outputs
    outputs = {key: value for key, value in outputs.items() if value is not None}
