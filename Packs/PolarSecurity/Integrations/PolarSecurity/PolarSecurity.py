...
def polar_list_data_stores_command(polar_client):
    """
    ...
    try:
        ...
        if res.json()['results'] is not None:
            found_items = res.json()
            while res.json()['nextToken'] is not None:
                next_token = res.json()['nextToken']
                res = polar_client.send_request(method='GET',
                                                path=f'/dataStores/?nextToken={next_token}',
                                                params={},
                                                body={})
                if res.json()['results'] is not None:
                    for item in res.json()['results']:
                        found_items['results'].append(item)
        if found_items['results'] is not None:
            outputs = found_items['results']
...
def polar_list_vendors_data_stores_command(polar_client, vendor_id):
    """
    ...
    try:
        ...
        if res.json()['results'] is not None:
            ...
            found_items['data_stores'] = res.json()['results']
            while res.json()['nextToken'] is not None:
                next_token = res.json()['nextToken']
                res = polar_client.send_request(
                    method='GET',
                    path=f'/linkedVendor/{vendor_id}/dataStore/?nextToken={next_token}',
                    params={},
                    body={})
                for item in res.json()['results']:
                    found_items['data_stores'].append(item)
        if found_items['data_stores'] is not None:
            return found_items
    except Exception as e:
        return_error(f'Operation failed. Please check the instance configuration \
                     and the given username and password.\n'
...
def polar_list_vendor_accessible_data_stores(polar_client):
    ...
    """
    vendors = polar_list_linked_vendors_command(polar_client)
    pairs: dict = {}
    big_list: list = []
    stores_list: list = []
    for vendor in vendors:
        ...
        vendor_stores = polar_list_vendors_data_stores_command(
            polar_client, vendor_id)
        if len(vendor_stores['data_stores']):  # anything greater than 0 results
            for store in vendor_stores['data_stores']:
...
def main():
    ...
    try:
        ...
        if command == 'test-module':
            ...
            outputs = polar_list_linked_vendors_command(polar_client)
            readable_output = tableToMarkdown(
            ...
            results = polar_list_data_stores_command(polar_client)
            ...
            outputs = polar_list_vendors_data_stores_command(
                polar_client, vendor_id)
            stores = outputs['data_stores']
            readable_output = tableToMarkdown(
