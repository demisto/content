...
class Client(BaseClient):
    ...
    def __init__(self, server_url: str, auth_token: str, proxy: bool, verify: bool):
        """
        Args:
        ...
        self._headers = {"Authorization": self._auth_token, "Content-Type": "application/json", "Accept": "application/json"}
    ...
    if previous_token.get("access_token") and previous_token.get("expiry_time") > date_to_timestamp(datetime.now()):
        ...
        try:
            try:
                ...
                raise DemistoException("Failed to parse json object from response", exception)
            if "error" in res:
                return_error(
...
def polar_login(server_url, username, password):
    ...
    try:
        ...
        return_error(
            f"Login failed. Please check the instance configuration \
...
def polar_list_data_stores_command(polar_client, limit: int, page_size: int, next_token):
    ...
    try:
        while has_next_page and limit > 0:
            ...
            try:
                res = polar_client.send_request(method="GET", path="/dataStores", params=query_params, body={})
            except ValueError as exception:
                raise DemistoException("Unable to retrieve data store list", exception)
        ...
        if found_items is not None:
            ...
            readable_output = tableToMarkdown(
                ...
                outputs,
                headers=["dataStoreId", "dataStoreType", "dataStoreName", "vpcId", "country", "cloudRegion"],
            )
            command_results = []
            ...
            if next_token is not None:
                ...
                command_results.append(
                    CommandResults(
                        outputs={"PolarSecurity.DataStores(true)": {"NextToken": next_token}}, readable_output=human_readable
                    )
                )
        ...
        return_error(
            f"Operation failed. Please check the instance configuration \
                     and the given username and password.\n"
            f"{e.args[0]}"
        )
...
def polar_get_data_store_command(polar_client, store_id):
    ...
    try:
        try:
            res = polar_client.send_request(method="GET", path=f"/dataStores/{safe_store_id}", params={}, body={})
        except ValueError as exception:
            raise DemistoException("Unable to retrieve data store list", exception)
        if res.json() is not None:
            ...
            readable_output = tableToMarkdown(
                ...
                outputs,
                headers=["dataStoreType", "dataStoreName", "dataStoreUrl", "vpcId", "country", "cloudRegion"],
            )
...
def polar_data_stores_summary_command(polar_client):
    ...
    try:
        ...
        try:
            res = polar_client.send_request(method="GET", path="/dataStores/summary?", params=parameters, body={})
        except ValueError as exception:
            raise DemistoException("Unable to retrieve d list:", exception)
        if res.json() is not None:
            outputs = res.json()
            readable_output = tableToMarkdown(
                ...
                headers=[
                    ...
                    "totalPotentialFlows",
                    "totalActualFlows",
                    "totalSensitiveStores",
                    "serviceProviders",
                ],
            )
            results = CommandResults(
                outputs=outputs, readable_output=readable_output, outputs_prefix="PolarSecurity.DataStores.Summary"
            )
            return results
    except Exception as e:
        return_error(
...
def polar_list_linked_vendors_command(polar_client):
    ...
    try:
        try:
            res = polar_client.send_request(method="GET", path="/linkedVendor", params={}, body={})
        except ValueError as exception:
            raise DemistoException("Unable to retrieve vendor list", exception)
        if res:
            outputs: list = []
            ...
            readable_output = tableToMarkdown(
                ...
                outputs,
                headers=["vendorId", "vendorName", "vendorUrl", "description", "accounts"],
            )
    ...
    try:
        while has_next_page and limit > 0:
            ...
            try:
                res = polar_client.send_request(
                    method="GET", path=f"/linkedVendor/{vendor_id}/dataStore", params=query_params, body={}
                )
            except ValueError as exception:
                raise DemistoException("Unable to retrieve vendor data store list", exception)
            if res.json()["results"] is not None:
        ...
        if found_items["dataStores"] is not None:
            ...
            readable_output = tableToMarkdown(
                ...
                stores,
                headers=["cloudProvider", "cloudRegion", "dataStoreId", "dataStoreName", "dataStoreType", "sensitivitiesSummary"],
            )
            command_results = []
            command_results.append(
                CommandResults(
            ...
            command_results.append(
                CommandResults(outputs={"PolarSecurity.Vendors(true)": {"vendorId": vendor_id}}, readable_output=human_readable)
            )
            if next_token is not None:
                human_readable = f"\nNext page token is {next_token}"
                command_results.append(
                    CommandResults(outputs={"PolarSecurity(true)": {"NextToken": next_token}}, readable_output=human_readable)
                )
...
def polar_list_vendor_accessible_data_stores(polar_client):
    ...
    for vendor in vendors:
        ...
        vendor_stores = polar_list_vendors_data_stores_command(
            polar_client, vendor_id, limit=9999999, page_size=50, next_token=None
        )[1]
...
def polar_apply_label_command(polar_client, store_id, mylabel):
    ...
    safe_store_id = quote(store_id, safe="")
    try:
        polar_client.send_request(method="PUT", path=f"/dataStores/{safe_store_id}/labels", params={}, body={"label": mylabel})
    except ValueError as exception:
...
def main():
    ...
    try:
        ...
        polar_client = Client(server_url=server_url, auth_token=auth_token, proxy=proxy, verify=verify)
        if command == "test-module":
            ...
            limit = arg_to_number(args.get("limit", "50")) or 50
            page_size = arg_to_number(args.get("page_size", "50")) or 50
            results = polar_list_vendors_data_stores_command(polar_client, vendor_id, limit, page_size, next_token)[0]
