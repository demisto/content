...
SEVERITY_MAPPING = {"no_rating": "Unknown", "informational": "Good", "suspicious": "Suspicious", "malicious": "Bad"}
...
OUTPUT_PREFIX = {"IP": "TeamCymruScout.IP", "QUERY_USAGE": "TeamCymruScout.QueryUsage"}
...
class Client(BaseClient):
    ...
    def ip_request(
        self, ip: str, start_date: Optional[str], end_date: Optional[str], days: Optional[int], size: Optional[int]
    ) -> Dict:
        """
    ...
    def scout_indicator_search_request(
        self, query: str, start_date: Optional[str], end_date: Optional[str], days: Optional[int], size: Optional[int]
    ) -> Dict:
        """
...
def validate_common_search_command_args(
    args: Dict[str, str], required_arg: str, maximum_page_size: int = MAXIMUM_DETAIL_IP_SIZE
) -> Tuple:
    """
    ...
    if isinstance(size, int) and not (MINIMUM_PAGE_SIZE <= size <= maximum_page_size):
        raise DemistoException(ERROR_MESSAGES["INVALID_PAGE_SIZE"].format(size, MINIMUM_PAGE_SIZE, maximum_page_size))
...
def validate_ip_list_args(args: Dict) -> Dict:
    ...
    if invalid_ips:
        return_warning(
            ERROR_MESSAGES["INVALID_IP_ADDRESSES"].format(", ".join(invalid_ips)), exit=len(invalid_ips) == len(ip_addresses)
        )
...
def prepare_hr_for_ip(response: dict) -> str:
    ...
    human_readable = (
        tableToMarkdown(
            f"Summary Information For The Given {severity} IP: {ip_md_link}",
            summary_hr,
            json_transform_mapping={"Whois": JsonTransformer(), "Insights": JsonTransformer(), "Tags": JsonTransformer()},
            removeNull=True,
            headers=["Country Code", "Whois", "Tags", "Insights"],
        )
        + "\n"
    )
    human_readable += (
        tableToMarkdown("Top PDNS", pdns_hr, headerTransform=header_transformer_for_ip, removeNull=True) + "\n" if pdns_hr else ""
    )
    human_readable += (
        tableToMarkdown(
            "Top Peers",
            top_peers_hr,
            removeNull=True,
            json_transform_mapping={"Client Services": JsonTransformer(), "Server Services": JsonTransformer()},
            headers=[
                "Proto",
                "Client IP",
                "Client Country Code(s)",
                "Client Tag(s)",
                "Client Services",
                "Server IP",
                "Server Country Code(s)",
                "Server Tag(s)",
                "Server Services",
                "Event Count",
                "First Seen",
    ...
    human_readable += (
        tableToMarkdown(
            ...
            open_port_hr,
            removeNull=True,
            headers=["Event Count", "Port", "Protocol", "Protocol Text", "Service", "First Seen", "Last Seen"],
        )
        + "\n"
        if open_port_hr
        else ""
    ...
    human_readable += (
        tableToMarkdown("Top Fingerprints", fingerprints_hr, removeNull=True, headerTransform=header_transformer_for_ip) + "\n"
        if fingerprints_hr
        else ""
    )
    human_readable += (
        tableToMarkdown("Top Certificates", certs_hr, removeNull=True, headerTransform=header_transformer_for_ip) + "\n"
        if certs_hr
        else ""
    )
...
def prepare_hr_and_context_for_indicator_search(response: Dict) -> Tuple[str, Dict, Common.IP, List]:
    ...
    ip_indicator = Common.IP(
        ...
        port=",".join([str(open_port.get("port", "")) for open_port in response.get("summary", {}).get("open_ports", [])]),
    ...
    human_readable = (
        tableToMarkdown(
            f"Summary Information for the given indicator: {ip_md_link}",
            scout_ip_hr,
    ...
    human_readable += (
        tableToMarkdown("PDNS Information", pdns_hr, headerTransform=header_transformer_for_ip) + "\n" if pdns_hr else ""
    )
    human_readable += (
        tableToMarkdown("Open Ports Information", open_ports_hr, headerTransform=header_transformer_for_ip) + "\n"
        if open_ports_hr
        else ""
    )
    human_readable += (
        tableToMarkdown("Top Peers Information", top_peers_hr, headerTransform=header_transformer_for_ip) + "\n"
        if top_peers_hr
        else ""
    )
    human_readable += (
        tableToMarkdown("Service Counts Information", service_counts_hr, headerTransform=header_transformer_for_ip) + "\n"
        if service_counts_hr
        else ""
    )
    human_readable += (
        tableToMarkdown("Fingerprints Information", fingerprints_hr, headerTransform=header_transformer_for_ip) + "\n"
        if fingerprints_hr
        else ""
    )
    human_readable += (
        tableToMarkdown("Certs Information", certs_hr, headerTransform=header_transformer_for_ip) + "\n" if certs_hr else ""
    )
...
def prepare_hr_and_context_for_ip_list(response: dict) -> Tuple[str, Dict, Common.IP]:
    ...
    hr = tableToMarkdown(
        ...
        json_transform_mapping={"AS Info": JsonTransformer(), "Insights": JsonTransformer(), "Tags": JsonTransformer()},
    )
...
def prepare_hr_and_context_for_api_usage(response: dict, command_name: str) -> Tuple[str, Dict]:
    ...
    table_headers = [
        ...
        "Remaining Queries",
        "Query Limit",
        "Foundation Used Queries",
        "Foundation Remaining Queries",
        "Foundation Query Limit",
    ]
...
def scout_indicator_search_command(client: Client, args: Dict[str, Any]) -> List[CommandResults]:
    ...
    usage_hr, usage_context = prepare_hr_and_context_for_api_usage(usage_response.get("usage", {}), "scout-indicator-search")
