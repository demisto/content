...
from CommonServerPython import DBotScoreReliability, DemistoException, arg_to_datetime
from TeamCymruScout import (
    API_KEY,
    BASE_URL,
    BASIC_AUTH,
    DATE_FORMAT,
    ENDPOINTS,
    ERROR_MESSAGES,
    MAXIMUM_DETAIL_IP_SIZE,
    MAXIMUM_INDICATOR_SEARCH_SIZE,
    MAXIMUM_IP_LIST_SIZE,
    OUTPUT_KEY_FIELD,
    OUTPUT_PREFIX,
    Client,
    main,
    scout_indicator_search_command,
    scout_ip_list_command,
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"query": "0.0.0.1", "start_date": "1 day", "end_date": "2 day"}, ERROR_MESSAGES["START_DATE_GREATER_THAN_END_DATE"]),
        (
            {"query": "0.0.0.1", "start_date": "2 day", "end_date": (datetime.now() + timedelta(days=2)).strftime(DATE_FORMAT)},
            ERROR_MESSAGES["END_DATE_GREATER_THAN_CURRENT_TIME"],
        ),
        ({"query": "0.0.0.1", "start_date": "60 day", "end_date": "29 day"}, ERROR_MESSAGES["REACHED_MAXIMUM_DIFF_DAYS"]),
        ({"query": "0.0.0.1", "start_date": "91 day", "end_date": "70 day"}, ERROR_MESSAGES["REACHED_MAXIMUM_START_DAYS"]),
        ({"query": "0.0.0.1", "days": "31"}, ERROR_MESSAGES["INVALID_DAY"].format("31")),
        ({"query": "0.0.0.1", "days": "0"}, ERROR_MESSAGES["INVALID_DAY"].format("0")),
        ({"query": "0.0.0.1", "size": "0"}, ERROR_MESSAGES["INVALID_PAGE_SIZE"].format("0", 1, MAXIMUM_INDICATOR_SEARCH_SIZE)),
        (
            {"query": "0.0.0.1", "size": MAXIMUM_INDICATOR_SEARCH_SIZE + 1},
            ERROR_MESSAGES["INVALID_PAGE_SIZE"].format(MAXIMUM_INDICATOR_SEARCH_SIZE + 1, 1, MAXIMUM_INDICATOR_SEARCH_SIZE),
        ),
    ],
)
