"""HelloAhikam Feed Integration for Cortex XSOAR - Unit Tests file

This file contains the Unit Tests for the HelloAhikam Integration based
...
demisto-sdk lint -i Packs/HelloAhikam/Integrations/FeedHelloAhikam
...
before each test to simulate the API calls to the FeedHelloAhikam API (which is
...
from FeedHelloAhikam import Client, get_indicators_command, fetch_indicators_command
...
def test_build_iterator(requests_mock):
    ...
    """
    with open('test_data/FeedHelloAhikam_mock.txt', 'r') as file:
        response = file.read()
...
def test_get_indicators_command(mocker):
    ...
    results = get_indicators_command(client, params={'tlp_color': 'RED', 'create_relationships': 'false'},
                                     args={'limit': '10'})
    human_readable = tableToMarkdown('Indicators from HelloAhikam Feed:', indicators_list,
