"""HelloAhikam Integration for Cortex XSOAR (aka Demisto)
...
Example Design document for the this Integration (HelloAhikam):
...
The HelloAhikam API is a simple API that shows a realistic use case for an XSOAR
...
This function is used in conjunction of the HelloAhikam Scan playbook that uses
...
Please check the HelloAhikam Design Document referenced above for details about
...
and the prefix (i.e. ``HelloAhikam.Alert``) is passed via the ``outputs_prefix``
argument.
...
the list of entities (i.e. in HelloAhikam you want to update the status of an
existing ``HelloAhikam.Alert`` in the context when you retrieve it, rather than
...
HELLOAhikam_SEVERITIES = ['Low', 'Medium', 'High', 'Critical']
...
class Client(BaseClient):
    ...
    For this HelloAhikam implementation, no special attributes defined
    """
    ...
    def search_alerts(self, alert_status: Optional[str], severity: Optional[str],
        ...
        """
        Searches for HelloAhikam alerts using the '/get_alerts' API endpoint.
        ...
        Returns:
            list: list of HelloAhikam alerts as dicts.
        """
    ...
    def get_alert(self, alert_id: str) -> Dict[str, Any]:
        """
        Gets a specific HelloAhikam alert by id.
    ...
    def update_alert_status(self, alert_id: str, alert_status: str) -> Dict[str, Any]:
        """
        Changes the status of a specific HelloAhikam alert.
    ...
    def scan_start(self, hostname: str) -> Dict[str, Any]:
        """
        Starts a HelloAhikam scan on a specific hostname.
    ...
    def scan_status(self, scan_id: str) -> Dict[str, Any]:
        """
        Gets the status of a HelloAhikam scan.
    ...
    def scan_results(self, scan_id: str) -> Dict[str, Any]:
        """Gets the results of a HelloAhikam scan
...
def parse_domain_date(domain_date: Union[List[str], str], date_format: str = '%Y-%m-%dT%H:%M:%S.000Z') -> Optional[str]:
    ...
    Converts the HelloAhikam domain WHOIS date (YYYY-mm-dd HH:MM:SS) format
...
def convert_to_demisto_severity(severity: str) -> int:
    """
    Maps HelloAhikam severity to Cortex XSOAR severity.
    Converts the HelloAhikam alert severity level ('Low', 'Medium', 'High', 'Critical') to Cortex XSOAR incident
    severity (1 to 4).

    Args:
        severity (str): severity as returned from the HelloAhikam API.
...
def test_module(client: Client, first_fetch_time: int) -> str:
    ...
    Args:
        client (Client): HelloAhikam client to use.
        first_fetch_time (int): The first fetch time as configured in the integration params.
...
def say_hello_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """
    helloAhikam-say-hello command: Returns Hello {somename}
    ...
    Returns:
        CommandResults: A ``CommandResults`` object that is then passed to ``return_results``, that hello Ahikam message.
    """

    # INTEGRATION DEVELOPER TIP
    # In this case 'name' is an argument set in the HelloAhikam.yml file as mandatory,
...
def fetch_incidents(client: Client, max_results: int, last_run: Dict[str, int],
    ...
    severity = ','.join(HELLOAhikam_SEVERITIES[HELLOAhikam_SEVERITIES.index(min_severity):])
    ...
    for alert in alerts:
        ...
        # convert it from the HelloAhikam API response
        ...
        # "type") or custom fields (like "helloAhikamid") directly here in the
        ...
        incident = {
            ...
            # 'type': 'Hello Ahikam Alert',  # Map to a specific XSOAR incident Type
            'severity': convert_to_demisto_severity(alert.get('severity', 'Low')),
            # 'CustomFields': {  # Map specific XSOAR Custom Fields
            #     'helloAhikamid': alert.get('alert_id'),
            #     'helloAhikamstatus': alert.get('alert_status'),
            #     'helloAhikamtype': alert.get('alert_type')
            # }
        }
...
def ip_reputation_command(client: Client, args: Dict[str, Any], default_threshold: int,
    ...
    for ip in ips:
        ...
        # HelloAhikam score to XSOAR reputation mapping
        ...
        # parts: the vendor-specific context (HelloAhikam), the standard-context
        # (IP) and the DBotScore.
        # More information:
        # https://xsoar.pan.dev/docs/integrations/context-and-outputs
        # https://xsoar.pan.dev/docs/integrations/context-standards
        # https://xsoar.pan.dev/docs/integrations/dbot
        # Also check the HelloAhikam Design Document

        # Create the DBotScore structure first using the Common.DBotScore class.
        dbot_score = Common.DBotScore(
            indicator=ip,
            indicator_type=DBotScoreType.IP,
            integration_name='HelloAhikam',
            score=score,
            malicious_description=f'Hello Ahikam returned reputation {reputation}',
        ...
        # INTEGRATION DEVELOPER TIP
        # In the integration specific Context output (HelloAhikam.IP) in this
        ...
        # INTEGRATION DEVELOPER TIP
        # The output key will be ``HelloAhikam.IP``, using ``ip`` as the key field.
        # ``indicator`` is used to provide the context standard (IP)
        command_results.append(CommandResults(
            readable_output=readable_output,
            outputs_prefix='HelloAhikam.IP',
...
def domain_reputation_command(client: Client, args: Dict[str, Any], default_threshold: int,
    ...
    for domain in domains:
        ...
        # INTEGRATION DEVELOPER TIP
        # The output key will be ``HelloAhikam.Domain``, using ``domain`` as the key
        # field.
        # ``indicator`` is used to provide the context standard (Domain)
        command_results.append(CommandResults(
            readable_output=readable_output,
            outputs_prefix='HelloAhikam.Domain',
...
def search_alerts_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """
    helloAhikam-search-alerts command: Search alerts in HelloAhikam
    ...
    severities: List[str] = HELLOAhikam_SEVERITIES
    severity = args.get('severity', None)
    if severity:
        severities = severity.split(',')
        if not all(s in HELLOAhikam_SEVERITIES for s in severities):
            raise ValueError(
                f'severity must be a comma-separated value '
                f'with the following options: {",".join(HELLOAhikam_SEVERITIES)}')
    ...
    return CommandResults(
        outputs_prefix='HelloAhikam.Alert',
...
def get_alert_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """
    helloAhikam-get-alert command: Returns a HelloAhikam alert.
    ...
    readable_output = tableToMarkdown(f'HelloAhikam Alert {alert_id}', alert)
...
def update_alert_status_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """
    helloAhikam-update-alert-status command: Changes the status of an alert.
    Changes the status of a HelloAhikam alert and returns the updated alert info
...
def scan_start_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """
    helloAhikam-start-scan command: Starts a HelloAhikam scan.
    ...
    return CommandResults(
        readable_output=readable_output,
        outputs_prefix='HelloAhikam.Scan',
...
def scan_status_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """
    helloAhikam-scan-status command: Returns status for HelloAhikam scans.
...
def scan_results_command(client: Client, args: Dict[str, Any]) ->\
    ...
    """
    helloAhikam-scan-results command: Returns results for a HelloAhikam scan.
