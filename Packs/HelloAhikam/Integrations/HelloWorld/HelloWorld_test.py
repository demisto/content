"""HelloAhikam Integration for Cortex XSOAR - Unit Tests file
...
demisto-sdk lint -i Packs/HelloAhikam/Integrations/HelloAhikam
...
before each test to simulate the API calls to the HelloAhikam API. This way we
...
def test_say_hello():
    """Tests helloAhikam-say-hello command function.
    ...
    any external API.
    """
    from HelloAhikam import Client, say_hello_command
...
def test_start_scan(requests_mock):
    """Tests helloAhikam-scan-start command function.
    ...
    """
    from HelloAhikam import Client, scan_start_command
    ...
    assert response.outputs_prefix == 'HelloAhikam.Scan'
...
def test_status_scan(requests_mock):
    """Tests helloAhikam-scan-status command function.
    ...
    """
    from HelloAhikam import Client, scan_status_command
...
def test_scan_results(mocker, requests_mock):
    """Tests helloAhikam-scan-results command function.

    Configures requests_mock instance to generate the appropriate
    get_scan_results API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloAhikam import Client, scan_results_command
    ...
    assert response[0].outputs_prefix == 'HelloAhikam.Scan'
...
def test_search_alerts(requests_mock):
    """Tests helloAhikam-search-alerts command function.

    Configures requests_mock instance to generate the appropriate
    get_alerts API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloAhikam import Client, search_alerts_command
    ...
    assert response.outputs_prefix == 'HelloAhikam.Alert'
    assert response.outputs_key_field == 'alert_id'
    assert response.outputs == mock_response['alerts']


def test_get_alert(requests_mock):
    """Tests helloAhikam-get-alert command function.

    Configures requests_mock instance to generate the appropriate
    get_alerts API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloAhikam import Client, get_alert_command
...
def test_update_alert_status(requests_mock):
    """Tests helloAhikam-update-alert-status command function.

    Configures requests_mock instance to generate the appropriate
    get_alerts API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloAhikam import Client, update_alert_status_command
...
def test_ip(requests_mock):
    ...
    the output of the command function with the expected output.
    """
    from HelloAhikam import Client, ip_reputation_command
    ...
    assert response[0].outputs == mock_response
    assert response[0].outputs_prefix == 'HelloAhikam.IP'
...
def test_domain(requests_mock):
    ...
    the output of the command function with the expected output.
    """
    from HelloAhikam import Client, domain_reputation_command
    from CommonServerPython import Common, DBotScoreReliability
    ...
    assert response[0].outputs == mock_response
    assert response[0].outputs_prefix == 'HelloAhikam.Domain'
...
def test_fetch_incidents(requests_mock):
    ...
    the output of the command function with the expected output.
    """
    from HelloAhikam import Client, fetch_incidents
