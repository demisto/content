...
@pytest.mark.parametrize(
    "creds_name_to_fetch, expected_res",
    [
        (
            "name1",
            [
...
@pytest.mark.parametrize("creds_name_to_fetch, get_call_num, expected_res", [("name1, name2", 2, "ok"), ("", 1, "ok")])
def test_test_module_passes(creds_name_to_fetch, get_call_num, expected_res, mocker):
    ...
    client = Client(
        ...
        folder="Root",
        safe="safe1",
        credentials_object=creds_name_to_fetch,
        username="",
        password="",
        cert_text="",
        key_text="",
