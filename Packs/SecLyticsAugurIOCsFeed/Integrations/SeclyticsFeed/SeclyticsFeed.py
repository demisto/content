...
class Client(BaseClient):
    """Client for Seclytics Feed - gets indicator lists from Daily threat feeds

    Attributes:
        api_key(str): The API key for Seclytics.
    ...
    def daily_http_request(self) -> list:
        ...
        csv_file = 'palo_alto_xsoar_iocs.csv'
        token_param = f"?access_token={self.api_key}"
        res = requests.request(
            method="GET",
            url=f"https://api.seclytics.com/bulk/private/{csv_file}{token_param}",
            verify=self.verify,
            headers=headers
        )
...
def module_test_command(client: Client, args: dict, feed_tags: list, tlp_color: Optional[str]):
    ...
    Args:
        client(Client): Seclytics Feed client
    ...
    """
    try:
        client.build_iterator(argToList(demisto.params().get('feedTags')), tlp_color, 1000, 0)
    except Exception:
        raise Exception("Could not fetch Daily Threat Feed\n"
                        "\nCheck your API key and your connection to Seclytics.")
    return 'ok', {}, {}
...
def get_indicators_command(client: Client, args: dict, feed_tags: list, tlp_color: Optional[str]) \
    ...
    Args:
        client(Client): The Seclytics Client.
...
def fetch_indicators_command(client: Client, feed_tags: List, tlp_color: Optional[str], limit=None, offset=None) \
        -> list:
    """Fetch-indicators command from Seclytics Feeds
...
def main():
    ...
    commands = {
        ...
        'seclytics-get-indicators': get_indicators_command
    }
    try:
        seclytics_key = params.get('api_key').strip()
        client = Client(api_key=seclytics_key,
        ...
        raise Exception(f'Error in SeclyticsFeed Daily Integration [{e}]')
