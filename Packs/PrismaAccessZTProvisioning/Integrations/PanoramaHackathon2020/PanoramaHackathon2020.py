...
def panorama_advanced_command():
    params = {
        'action': demisto.args().get('action'),
        'type': demisto.args().get('type'),
        'xpath': demisto.args().get('xpath'),
        'element': demisto.args().get('element'),
        'key': API_KEY
    }
    ...
    demisto.results(result['response'])
...
def main():
    ...
    try:
        ...
        elif demisto.command() == 'panorama-advanced-command':
            panorama_advanced_command()
