...
def dsv_secret_get_command(client, name: str = ""):
    ...
    return CommandResults(
        readable_output=markdown, outputs_prefix="DSV.Secret", outputs_key_field="secret", raw_response=secret, outputs=secret
    )
...
def main():  # pragma: no cover
    ...
    delinea_commands = {"dsv-secret-get": dsv_secret_get_command}

    try:
        client = Client(
            server_url=url, client_id=client_id, client_secret=client_secret, proxy=proxy, verify=verify, provider=provider
        )
        ...
        return_error(f"Failed to execute {demisto.command()} command.", f"Error: {str(e)}")
