...
DEFAULT_HEADERS = {"Content-Type": "application/json", "Accept": "application/json", "Authorization": API_KEY}
...
def http_request(method, url_suffix, params_dict=None, headers=DEFAULT_HEADERS, data=None):
    ...
    try:
        res = requests.request(method, url, verify=USE_SSL, params=req_params, headers=headers, data=data)
...
def show_alert_command():
    ...
    return {
        ...
        "HumanReadable": md,
        "EntryContext": {"Stealthwatch.Alert(val.id==obj.id)": alert_data},
    }
...
def update_alert_command():
    ...
    if username is not None:
        update_params["resolved_user"] = {"username": username}
...
def list_alerts_command():
    ...
    return {
        ...
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": md,
        "EntryContext": {"Stealthwatch.Alert(val.id==obj.id)": alerts_data},
    }
...
def block_domain_command():
    ...
    if not (domain or ip):
        return {"Type": entryTypes["error"], "ContentsFormat": formats["text"], "Contents": "Please enter either domain or ip"}
    ...
    domain_params = {"identifier": identifier, "category": "domain", "list_on": "blacklist"}
    ...
    if domain:
        ec = {"Stealthwatch.Domain(val.identifier==obj.identifier)": domain_result}
    else:
        ec = {"Stealthwatch.IP(val.identifier==obj.identifier)": domain_result}
...
def list_blocked_domains_command():
    ...
    return {
        ...
        "EntryContext": {"Stealthwatch.Domain(val.identifier==obj.identifier)": domains_result},
    }
...
def list_observations_command():
    ...
    """
    args = demisto.args()
    list_params = {"ordering": "creation_time"}
    # adding the possible params for update
    ...
    return {
        ...
        "EntryContext": {"Stealthwatch.Observation(val.id==obj.id)": observations_data},
    }
...
def list_sessions_command():
    ...
    return {
        ...
        "EntryContext": {"Stealthwatch.Session(val.id==obj.id)": final_sessions_data},
    }
...
def fetch_incidents():
    ...
    list_params = {"ordering": "created", "limit": 100}
    final_alerts = []
