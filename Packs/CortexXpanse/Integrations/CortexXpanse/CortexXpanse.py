...
class Client(BaseClient):
    ...
    def list_asset_internet_exposure_request(
        self, search_params: list[dict], search_from: int = 0, search_to: int = DEFAULT_SEARCH_LIMIT
    ) -> dict[str, Any]:
    ...
    def apply_tags_to_assets_request(self, search_params: list[dict], tags: str, operation: str) -> dict[str, Any]:
        ...
        response = self._http_request(
            "POST", f"{V1_URL_SUFFIX}/assets/tags/assets_internet_exposure/{operation}/", json_data=data
        )
    ...
    def apply_tags_to_ranges_request(self, search_params: list[dict], tags: str, operation: str) -> dict[str, Any]:
        ...
        response = self._http_request(
            "POST", f"{V1_URL_SUFFIX}/assets/tags/external_ip_address_ranges/{operation}/", json_data=data
        )
    ...
    def add_note_to_asset(self, asm_asset_id: str, entity_type: str, annotation_note: str, should_append: bool) -> dict[str, Any]:
        ...
        data = {
            "request_data": {
                "assets": [{"entity_id": asm_asset_id, "entity_type": entity_type, "annotation": annotation_note}],
...
def list_alerts_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    if sort_by_creation_time:
        ...
        request_data = {
            "request_data": {
                ...
                "search_from": search_from,
                "search_to": search_to,
                "sort": {"field": "severity", "keyword": sort_by_severity},
            }
        }
    else:
    ...
    command_results = CommandResults(
        outputs_prefix="ASM.Alert", outputs_key_field="alert_id", outputs=parsed, raw_response=response, readable_output=markdown
    )
...
def list_attack_surface_rules_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    parsed = format_asm_id(formatted_response)
    markdown = tableToMarkdown("Attack Surface Rules", parsed, removeNull=True, headerTransform=string_to_table_header)
    command_results = CommandResults(
...
def list_external_websites_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    human_readable = (
        f"Total results: {len(hosts)}\n \
        {tableToMarkdown('External Websites', hosts, ['Host', 'Authentication type'])}"
        if hosts
        else "No Results"
    )
    command_results = CommandResults(
...
def add_note_to_asset_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    response = client.add_note_to_asset(
        asm_asset_id=asset_id, entity_type=entity_type, annotation_note=note_to_add, should_append=should_append
    )
    response_message = {"status": response.get("reply", {})}
    response_message["asset"] = asset_id
    markdown = tableToMarkdown(
        "Add Note to Asset Command Results:", response_message.get("status"), headers=["Status"], removeNull=True
    )
    command_results = CommandResults(
...
def fetch_incidents(
    ...
    optional_filters = {"severity": severity, "status": status, "tags": tags.split(",") if tags else None}
    ...
    if next_page_token:
        alerts = sorted(
            alerts, key=lambda alert: alert["local_insert_ts"]
        )  # Sort is not supported when using the use_page_token / next_page_token fields.  # noqa: E501
    ...
    for alert in filtered_alerts:
        alert_created_time = datetime.fromtimestamp(alert.get("local_insert_ts") / 1000.0).strftime(
            TIME_FORMAT_Z
        )  # local_insert_ts is the closest time to alert creation time in Xpanse.  # noqa: E501
    ...
    if len(xsoar_incidents) > 0:
        demisto.debug(f"CortexXpanse - Logger - Last fetched alert timestamp: {last_run.get('last_fetch', None)!s}")
    ...
    demisto.debug(f"CortexXpanse - Logger - last_run: {last_run!s}")
...
def main() -> None:
    ...
    try:
        first_fetch_time = arg_to_datetime(
            arg=params.get("first_fetch", "3 days"), arg_name="First fetch timestamp", required=True
        )
