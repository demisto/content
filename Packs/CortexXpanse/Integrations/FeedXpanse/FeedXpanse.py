    ...
    def list_asset_internet_exposure_request(
        ...
        while True:
            result = self._http_request(
                method="POST", url_suffix=f"{V1_URL_SUFFIX}/assets/get_assets_internet_exposure/", json_data=body
            )
...
def build_asset_indicators(client: Client, raw_indicators: list[dict[str, Any]]) -> list:
    ...
    for raw_indicator in raw_indicators:
        ...
        indicator = {"value": name, "type": indicator_type, "fields": fields, "rawJSON": raw_indicator}
...
def main() -> None:  # pragma: no cover
    ...
    headers = {"Authorization": f"{api}", "x-xdr-auth-id": f"{auth_id}", "Content-Type": "application/json"}
    command = demisto.command()

    demisto.info(f"Command being called is {command}")
    try:
        client = Client(
            base_url=base_url, verify=verify_certificate, proxy=proxy, feed_tags=feed_tags, tlp_color=tlp_color, headers=headers
        )
