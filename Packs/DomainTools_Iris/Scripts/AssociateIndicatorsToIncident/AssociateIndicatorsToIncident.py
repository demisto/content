...
def get_related_indicators(addintional_indicators: str = "") -> list[str]:
    related_indicators = addintional_indicators.strip("[]").replace('"', "").split(",")
...
def associate_indicator_to_incident() -> CommandResults:
    ...
    if is_associated:
        ...
        related_indicators = get_related_indicators(incident_obj.get("CustomFields", {}).get("additionalindicators") or "")
        related_indicators_count = len(related_indicators)
        if related_indicators_count > 0:
            demisto.info(f"Associating {related_indicators_count} Indicators to Incident {incident_id}: {related_indicators}")
            associated_indicators = {"incidentId": incident_id, "indicatorsValues": ",".join(related_indicators)}

            demisto.executeCommand("associateIndicatorsToIncident", associated_indicators)
...
def main():
    try:
        ...
        return_error(f"Failed to execute AssociateIndicatorsToIncident. Error: {str(ex)}")
