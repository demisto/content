...
""" GLOBALS """
USERNAME = demisto.params().get("credentials", {}).get(
    "identifier"
) or demisto.params().get("username")
...
YOUNG_DOMAIN_TIMEFRAME = (
    arg_to_number(demisto.params().get("young_domain_timeframe")) or 7
)
...
DOMAINTOOLS_MONITOR_DOMAINS_INCIDENT_NAME_BY_IRIS_SEARCH_HASH = (
    "DomainTools Iris Monitor Domain Search Hash"
)
DOMAINTOOLS_MONITOR_DOMAINS_INCIDENT_NAME_BY_IRIS_TAGS = (
    "DomainTools Iris Monitor Domain Tags"
)
...
def get_dbot_score(proximity_score, age, threat_profile_score):
    ...
    if proximity_score >= RISK_THRESHOLD or threat_profile_score >= RISK_THRESHOLD:
        return 3
    elif age < YOUNG_DOMAIN_TIMEFRAME and (
        proximity_score < RISK_THRESHOLD or threat_profile_score < RISK_THRESHOLD
    ):
        return 2
    else:
        return 1
...
def format_contact_grid(title, contact_dict):
    ...
    email = ",".join(
        [email["value"] for email in contact_dict.get("Email", []) if "value" in email]
    )
    phone = contact_dict.get("Phone", {}).get("value")
    fax = contact_dict.get("Fax", {}).get("value")
    address = (
        f"Street: {contact_dict.get('Street', {}).get('value')}, "
    ...
    return [
        {
            "type": type,
            "ip": ",".join(
                [ip["value"] for ip in item.get("ip", []) if ip.get("value")]
            ),
...
def format_risk_grid(domain_risk):
    result = [
        Common.ThreatTypes(
            ...
            threat_category_confidence=domain_risk.get("risk_score"),
        )
    ]
    ...
    for component in domain_risk.get("components", []):
        result.append(
            Common.ThreatTypes(
                threat_category=component.get("name"),
...
def convert_and_format_date(
    string_date: str,
    string_date_format: str = "%Y-%m-%dT%H:%M:%SZ",
...
def create_results(domain_result):
    ...
    domain_risk_score_details = get_domain_risk_score_details(
        domain_result.get("domain_risk") or {}
    )
    ...
    email_domains = [
        email_domain.get("value") for email_domain in domain_result.get("email_domain")
    ]
    ...
    ip_country_code = (
        ip_addresses[0].get("country_code", {}).get("value")
        if len(ip_addresses)
        else ""
    )
    ...
    website_title = (
        domain_result.get("website_title", {}).get("value")
        if domain_result.get("website_title")
        else ""
    )
    server_type = (
        domain_result.get("server_type", {}).get("value")
        if domain_result.get("server_type")
        else ""
    )
    first_seen = (
        domain_result.get("first_seen", {}).get("value")
        if domain_result.get("first_seen")
        else ""
    )
    ...
    domain_tools_context = {
        ...
        "Analytics": {
            ...
            "MalwareRiskScore": domain_risk_score_details[
                "threat_profile_malware_risk_score"
            ],
            "PhishingRiskScore": domain_risk_score_details[
                "threat_profile_phishing_risk_score"
            ],
            "SpamRiskScore": domain_risk_score_details[
                "threat_profile_spam_risk_score"
            ],
            "ThreatProfileRiskScore": {
                "RiskScore": domain_risk_score_details["threat_profile_risk_score"],
    ...
    dns = (
        [
            {"type": "DNS", "ip": ip.get("address", {}).get("value")}
    ...
    registrant_country = (
        domain_result.get("registrant_contact", {}).get("country", {}).get("value")
    )
    ...
    admin_country = (
        domain_result.get("admin_contact", {}).get("country", {}).get("value")
    )
    ...
    tech_country = (
        domain_result.get("technical_contact", {}).get("country", {}).get("value")
    )
    rank = [
        Common.Rank(
    ...
    if dbot_score_value == 3:
        ...
        malicious_description = (
            ...
            if threat_profile_evidence is not None and len(threat_profile_evidence)
            else "This domain has been profiled as a threat."
        )
...
def format_enrich_output(result):
    ...
    domaintools_registration_data = indicators.get("domaintools", {}).get(
        "Registration", {}
    )
    ...
    human_readable_data = {
        ...
        "Proximity Risk Score": domaintools_analytics_data.get(
            "ProximityRiskScore", ""
        ),
        "Threat Profile Risk Score": domaintools_analytics_data.get(
            "ThreatProfileRiskScore", {}
        ).get("RiskScore", ""),
        "Threat Profile Threats": domaintools_analytics_data.get(
            "ThreatProfileRiskScore", {}
        ).get("Threats", ""),
        "Threat Profile Evidence": domaintools_analytics_data.get(
            "ThreatProfileRiskScore", {}
        ).get("Evidence", ""),
        "Google Adsense Tracking Code": domaintools_analytics_data.get(
            "GoogleAdsenseTrackingCode", ""
        ),
        "Google Analytic Tracking Code": domaintools_analytics_data.get(
            "GoogleAnalyticTrackingCode", ""
        ),
        "Website Response Code": domaintools_analytics_data.get(
            "WebsiteResponseCode", ""
        ),
        ...
        "SSL Certificate Email": domaintools_identity_data.get(
            "SSLCertificateEmail", ""
        ),
        ...
        "Additional Whois Emails": domaintools_identity_data.get(
    ...
    iris_title = "Investigate [{0}](https://research.domaintools.com/iris/search/?q={0}) in Iris.".format(
        domain
    )
...
def format_investigate_output(result):
    ...
    human_readable_data = {
        ...
        "Google Adsense Tracking Code": format_single_value(
            "ad", result.get("adsense", {})
        ),
        "Google Analytic Tracking Code": format_single_value(
            "ga", result.get("google_analytics", {})
        ),
        "Website Response Code": domaintools_analytics_data.get(
            "WebsiteResponseCode", ""
        ),
        "Tags": domaintools_analytics_data.get("Tags", "not here"),
        "Registrant Name": format_single_value(
            "r_n", result.get("registrant_name", {})
        ),
        "Registrant Org": format_single_value("r_o", result.get("registrant_org", {})),
        "Registrant Contact": format_contact(
            result.get("registrant_contact", {}), domain, "empr"
        ),
        "Registrar": format_single_value("reg", result.get("registrar", {})),
        "SOA Email": format_list_value("ema", result.get("soa_email", [])),
        "SSL Certificate Email": format_list_value(
            "ssl.em", result.get("ssl_email", [])
        ),
        "Admin Contact": format_contact(
            result.get("admin_contact", {}), domain, "empa"
        ),
        "Technical Contact": format_contact(
            result.get("technical_contact", {}), domain, "empt"
        ),
        "Billing Contact": format_contact(
            result.get("billing_contact", {}), domain, "empb"
        ),
        "Email Domains": format_list_value("emd", result.get("email_domain", [])),
        "Additional Whois Emails": format_list_value(
            "em", result.get("additional_whois_email", [])
        ),
        "Registrar Status": domaintools_registration_data.get("RegistrarStatus", ""),
        "Domain Status": domaintools_registration_data.get("DomainStatus", ""),
        "Create Date": format_single_value("cre", result.get("create_date", {})),
        "Expiration Date": format_single_value(
        ...
        "Redirect Domain": format_single_value(
            "rdd", result.get("redirect_domain", {})
        ),
        "Website Title": format_single_value(
            None, result.get("website_title", {}), domain
        ),
        "First Seen": format_single_value(None, result.get("first_seen", {}), domain),
        "Server Type": format_single_value(
...
def get_domain_risk_score_details(domain_risk: dict[str, Any]) -> dict[str, Any]:
    ...
    if risk_components:
        ...
        threat_profile_data = utils.get_threat_component(
            risk_components, "threat_profile"
        )
        if threat_profile_data:
            risk_scores["threat_profile_risk_score"] = (
                threat_profile_data.get("risk_score") or ""
            )
        ...
        if threat_profile_spam_data:
            risk_scores["threat_profile_spam_risk_score"] = (
                threat_profile_spam_data.get("risk_score", 0)
            )
...
def create_indicator_from_dt_domain(
    domain: dict[str, Any], source: str
) -> dict[str, Any]:
    ...
    return {
        ...
        "fields": {
            ...
            "domaintoolsiristags": format_attribute(
                domain.get("tags", []), key="label"
            ),
            "domaintoolsirisadditionalwhoisemails": format_attribute(
                domain.get("additional_whois_email", []), key="value"
            ),
            "domaintoolsirisemaildomains": format_attribute(
                domain.get("email_domain", []), key="value"
            ),
            "nameservers": format_attribute(
                domain.get("name_server", []), key="host.value"
            ),
            "domaintoolsirisipaddresses": format_attribute(
                domain.get("ip", []), key="address.value"
            ),
            "domaintoolsirismailservers": format_attribute(
                domain.get("mx", []), key="domain.value"
            ),
            "domaintoolsirisipcountrycode": ip_country_code,
            "domaintoolsirisregistrantorg": domain.get("registrant_org", {}).get(
                "value"
            )
            or "",
            "registrantname": domain.get("registrant_name", {}).get("value") or "",
            "domaintoolsirissoaemail": format_attribute(
                domain.get("soa_email", []), key="value"
            ),
...
def fetch_domains_from_dt_api(
    search_type: str, search_value: str
) -> list[dict[str, Any]]:
...
def fetch_and_process_domains(
    iris_search_hash: dict[str, Any], iris_tags: dict[str, Any]
) -> None:
    """
    ...
    def process_domains(
        ...
        demisto.info(
            f"Added {len(indicators)} indicators to demisto (source: {source})"
        )
...
def domain_command():
    ...
    for chunk in domain_chunks:
        ...
        for result in response.get("results", []):
            ...
            domaintools_context = (
                indicators.get("domaintools") if include_context else None
            )
...
def domain_analytics_command():
    ...
    human_readable_data = {
        ...
        "Google Adsense": domaintools_analytics_data.get(
            "GoogleAdsenseTrackingCode", ""
        ),
        "Google Analytics": domaintools_analytics_data.get(
...
def threat_profile_command():
    ...
    if risk_components:
        ...
        if threat_profile_malware_data:
            threat_profile_malware_risk_score = threat_profile_malware_data.get(
                "risk_score", 0
            )
        threat_profile_phshing_data = utils.get_threat_component(
            risk_components, "threat_profile_phishing"
        ...
        if threat_profile_phshing_data:
            threat_profile_phishing_risk_score = threat_profile_phshing_data.get(
                "risk_score", 0
            )
        threat_profile_spam_data = utils.get_threat_component(
            risk_components, "threat_profile_spam"
        ...
        if threat_profile_spam_data:
            threat_profile_spam_risk_score = threat_profile_spam_data.get(
                "risk_score", 0
            )
...
def domain_pivot_command():
    ...
    for pivot_type in available_pivots:
        if demisto.args().get(pivot_type):
            ...
            search_type, search_value = available_pivots[
                pivot_type
            ], demisto.args().get(pivot_type)
            break

    if not search_type or not search_value:
        raise Exception(
            f"Invalid pivot type or value. pivot type: {search_type} search value: {search_value}"
        )
    ...
    human_readable = tableToMarkdown(
        f"Domains for {search_type}: {search_value} "
...
def whois_history_command():
    ...
    response = whois_history(
        domain=domain, sort=sort, limit=limit, mode=mode, offset=offset
    )
    ...
    for entry in history:
        ...
        human_readable += tableToMarkdown(
            f"{domain}: {entry.get('date')}", table, headers=headers
        )
        all_context.append(entry_context)

    history_result = {"Value": domain, "WhoisHistory": all_context}
...
def hosting_history_command():
    ...
    human_readable_all = (
        human_readable_registrar + human_readable_ns + human_readable_ip
    )
    history_result = {
        "Value": domain,
        "IPHistory": ip_table,
        "NameserverHistory": ns_table,
        "RegistrarHistory": registrar_table,
    }
...
def reverse_whois_command():
    ...
    scope = (
        "current" if demisto.args().get("onlyHistoricScope") == "false" else "historic"
    )
...
def parsed_whois_command():
    ...
    human_readable = tableToMarkdown(
        f"DomainTools whois result for {domain}", table, headers=headers
    )
    ...
    domain_indicator = Common.Domain(
        ...
        registrant_country=parsed.get("contacts", {})
        .get("registrant", {})
        .get("country"),
...
def fetch_domains():
    # iris search hash
    monitor_domain_by_search_hash = (
        demisto.params().get("monitor_iris_search_hash") or "Import Indicators Only"
    )
    iris_search_hash = demisto.params().get("domaintools_iris_search_hash") or False
    # iris tags
    monitor_domain_by_iris_tags = (
        demisto.params().get("monitor_iris_tags") or "Import Indicators Only"
    )
    ...
    fetch_and_process_domains(
        iris_search_hash=iris_search_hash_params, iris_tags=iris_tags_params
    )
