...
def format_risk_grid(domain_risk):
    ...
    for component in domain_risk.get("components", []):
        result.append(
            Common.ThreatTypes(threat_category=component.get("name"), threat_category_confidence=component.get("risk_score"))
        )
...
def create_results(domain_result):
    ...
    dns = (
        [{"type": "DNS", "ip": ip.get("address", {}).get("value")} for ip in ip_addresses]
...
def create_indicator_from_dt_domain(domain: dict[str, Any], source: str) -> dict[str, Any]:
    ...
    try:
        dbot_score = get_dbot_score(
            domain_risk_score_details["proximity_risk_score"], domain_age, domain_risk_score_details["threat_profile_risk_score"]
        )
    except Exception as e:
    ...
    raw_json_indicator = {"type": DOMAINTOOLS_IRIS_INDICATOR_TYPE, "value": domain.get("domain")}
...
def fetch_and_process_domains(iris_search_hash: dict[str, Any], iris_tags: dict[str, Any]) -> None:
    ...
    def process_domains(
        domains_list: list[dict[str, Any]] = [], import_only: bool = True, incident_name: str = "", source: str = ""
    ) -> str:
        ...
        indicators = [create_indicator_from_dt_domain(domain, source=source) for domain in domains_list]
    ...
    for domains_list, import_only, incident_name, source, timestamp_key in domains_to_process:
        last_runs[timestamp_key] = process_domains(
            domains_list=domains_list, import_only=import_only, incident_name=incident_name, source=source
        )
...
def hosting_history_command():
    ...
    human_readable_ip = tableToMarkdown("IP Address History", ip_table, headers=ip_headers)
    ...
    human_readable_ns = tableToMarkdown("Name Server History", ns_table, headers=ns_headers)
    ...
    human_readable_registrar = tableToMarkdown("Registrar History", registrar_table, headers=registrar_headers)
...
def reverse_whois_command():
    ...
    scope = "current" if demisto.args().get("onlyHistoricScope") == "false" else "historic"
...
def fetch_domains():
    ...
    iris_tags_params = {"import_only": monitor_domain_by_iris_tags == "Import Indicators Only", "tags": iris_tags}

    fetch_and_process_domains(iris_search_hash=iris_search_hash_params, iris_tags=iris_tags_params)
...
def main():
    ...
    try:
        ...
        return_error(f"Unable to perform command : {demisto.command()}, Reason: {str(e)}")
