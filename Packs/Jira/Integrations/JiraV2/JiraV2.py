        ...
        if not result.ok:
            ...
            try:
                ...
                if rj.get('errorMessages'):
                    DemistoException(f'Status code: {result.status_code}\nMessage: {",".join(rj["errorMessages"])}')
                elif rj.get('errors'):
                ...
                if result.status_code == 401:
                    DemistoException('Unauthorized request, please check authentication related parameters.'
                                 f'{BASIC_AUTH_ERROR_MSG}')
                elif result.status_code == 404:
                    DemistoException("Could not connect to the Jira server. Verify that the server URL is correct.")
                elif result.status_code == 500 and files:
                    DemistoException(f"Failed to execute request, status code: 500\nBody: {result.text}"
...
def generate_md_context_get_issue(client, data, customfields=None, nofields=None, extra_fields=None):
...
def get_issue(client: Client, issue_id, headers=None, expand_links=False, is_update=False, get_attachments=False, extra_fields=None):
    ...
    md_and_context = generate_md_context_get_issue(client, j_res, extra_fields=extra_fields)
...
def edit_issue_command(client: Client, issue_id, mirroring=False, headers=None, status=None, transition=None, **kwargs):
    ...
    if status and transition:
        ...
        edit_status(client, issue_id, status, issue)
    elif transition:
        edit_transition(client, issue_id, transition, issue)
    else:
        url = f'rest/api/latest/issue/{issue_id}/'
        client.send_request('PUT', url, json.dumps(issue))
...
def append_to_field_command(client, issue_id, field_json, headers=None):
    issue = client.send_request('GET', f'rest/api/latest/issue/{issue_id}', resp_type='json')
    fields = json.loads(field_json, strict=False)
    new_data = {}
    for field in fields:
        field_type = __get_field_type(client, field)
        if not field_type:
    ...
    _update_fields(client, issue_id, new_data)

    return get_issue(client, issue_id, headers, is_update=True)


def _update_fields(client, issue_id, new_data):
    url = f'rest/api/latest/issue/{issue_id}/'
    if new_data:
        client.send_request('PUT', url, json.dumps({'fields': new_data}))
...
def __get_field_type(client, field_id):
    fields = client.send_request('GET', 'rest/api/2/field')
...
def edit_status(client: Client, issue_id, status, issue):
    ...
    for i, transition in enumerate(transitions):
        if transition.lower() == status.lower():
            ...
            return client.send_request('POST', url, json.dumps(issue))
...
def edit_transition(client: Client, issue_id, transition_name, issue):
    """
...
def update_remote_system_command(client, args):
    ...
    try:
        if remote_args.delta and remote_args.incident_changed:
            ...
            edit_issue_command(client, remote_id, mirroring=True, **delta)
