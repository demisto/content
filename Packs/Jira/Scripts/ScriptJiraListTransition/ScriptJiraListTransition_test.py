...
def test_list_transition_for_v3_brand():
    ...
    demisto.incidents = MagicMock(return_value=[{"dbotMirrorId": "1234", "sourceBrand": "Jira V3"}])
    ...
    main()
    assert execute_command_mocker.call_args_list[0][0] == (
...
def test_list_transition_for_v2_brand():
    ...
    demisto.incidents = MagicMock(return_value=[{"dbotMirrorId": "1234", "sourceBrand": "jira-v2"}])
    execute_command_mocker = demisto.executeCommand = MagicMock(
    ...
    expected_statuses_names = ["Transition 1", "Transition 2"]
    main()
    assert execute_command_mocker.call_args_list[0][0] == ("jira-list-transitions", {"issueId": "1234", "using-brand": "jira-v2"})
    assert demisto_results_mocker.call_args_list[0][0][0] == ({"hidden": False, "options": expected_statuses_names})
