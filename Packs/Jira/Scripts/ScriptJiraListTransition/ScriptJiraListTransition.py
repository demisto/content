...
def get_transition_names_by_source_brand(incident_id: Dict[str, Any], source_brand: str) -> Dict[str, Any]:
    ...
    if not transitions_names:
        raise DemistoException((f'Error occurred while running script-JiraListTransition. Could not find retrieve the list of'
                               f' transitions from the response. The response is: {res} '))
    if isError(res):
        raise DemistoException(f'Error occurred while running jira-list-transitions. The response is: {transitions_names}')
...
def main():
    ...
    try:
        ...
        if incident_id := incident.get("dbotMirrorId"):
            ...
            # res = demisto.executeCommand(
            #     "jira-list-transitions", {"issueId": incident.get("dbotMirrorId"), 'issue_id': incident.get("dbotMirrorId")}
            # )
            # if res and isinstance(res, list):
            #     transitions_names = res[0].get("Contents")
