        ...
        if incident_id:
            ...
            if res and isinstance(res, list):
                ...
                if transitions_names:
                    ...
                    output = create_output(
                        f"Error occurred while running script-JiraListTransition. Could not find:\"Content\" as key. The response is: {res}")
            else:
                output = create_output(
                    f"Error occurred while running script-JiraListTransition. expected a list as response but got: {type(res)}. The response is: {res}")
        else:
            output = create_output(
                f"Error occurred while running script-JiraListTransition because could not get \"dbotMirrorId\" from incident.")
