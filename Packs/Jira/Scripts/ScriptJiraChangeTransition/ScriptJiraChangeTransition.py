...
def main():
    ...
    try:
        ...
        if incident_id:
                                        ...
                                        if incident_fields:
                                            ...
                                            if jira_status:
                                                ...
                                                output = create_output(
                                                    f"Error occurred while running script-JiraChangeTransition. Could not find: the issue's status name. The issue returned is: {incident_fields}")
                                        else:
                                            output = create_output(
                                                f"Error occurred while running script-JiraChangeTransition. Could not find:\"fields\" as key. The issue returned is: {incident_content}")
                                    else:
                                        output = create_output(
                                            f"Error occurred while running jira-get-issue. The response is: {incident_content}")
                                else:
                                    output = create_output(
                                        f"Error occurred while running script-JiraChangeTransition. Could not find:\"Content\" as key. The response is: {res}")
                    else:
                        output = create_output(
                            f"Error occurred while running script-JiraChangeTransition. expected a list as response but got: {type(res)}. The response is: {res}")
                else:
                    output = create_output(
            ...
            output = create_output(f"Failed to get a list of transaction because could not get \"dbotMirrorId\" from incident.")

    except Exception as ex:
        output = create_output("Error occurred while running script-JiraChangeTransition. got the next error:\n" + str(ex))
    finally:
        if output:
