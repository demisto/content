...
async def add_or_delete_safe_list_entry(
    ...
    def return_api_error(fieldname, fieldvalue, valid_values):
        return {"errors": ["Bad request. ({}) not " "in list of valid {} values {}".format(fieldvalue, fieldname, valid_values)]}
    ...
    if api_list_entry.action not in VALID_API_LIST_ACTIONS:
        response.status_code = 400
        return return_api_error("action", api_list_entry.action, VALID_API_LIST_ACTIONS)

    if api_list_entry.attribute not in VALID_API_LIST_ATTRIBUTES:
        response.status_code = 400
        return return_api_error("attribute", api_list_entry.attribute, VALID_API_LIST_ATTRIBUTES)

    if api_list_entry.operator not in VALID_SAFE_LIST_OPERATORS:
        response.status_code = 400
        return return_api_error("operator", api_list_entry.operator, VALID_SAFE_LIST_OPERATORS)

    if not isinstance(api_list_entry.value, str):
        response.status_code = 400
        return return_api_error("value", api_list_entry.value, "valid object of type str()")
