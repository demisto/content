...
class Client(BaseClient):
        ...
        if integration_context.get("access_token") and integration_context.get("expiry_time") > date_to_timestamp(datetime.now()):
            ...
            try:
                ...
                if access_token is not None:
                    ...
                    context = {"access_token": access_token, "expiry_time": expiry_time}
    ...
    def get_safelist(self, cluster_id):
        return self._http_request("GET", url_suffix=URL_SUFFIX_SAFELIST, headers=self._headers, params={"clusterId": cluster_id})
    ...
    def get_blocklist(self, cluster_id):
        return self._http_request("GET", url_suffix=URL_SUFFIX_BLOCKLIST, headers=self._headers, params={"clusterId": cluster_id})
...
def safelist_list_command(client: Client, cluster_id: str) -> CommandResults:
    ...
    res_rc = make_return_command_results(
        OSL_HEADER, "Safelist", res.get("entries") if all_results else res.get("entries")[-limit:]
    )
    return res_rc
...
def blocklist_list_command(client: Client, cluster_id: str) -> CommandResults:
    ...
    res_rc = make_return_command_results(
        OBL_HEADER, "Blocklist", res.get("entries") if all_results else res.get("entries")[-limit:]
    )
    return res_rc
