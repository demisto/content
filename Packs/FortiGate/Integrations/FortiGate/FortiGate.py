...
def get_address_type(args: dict[str, Any], include_ipv6: bool = False) -> str:
    ...
    # If arguments are from more than one group, raise an erro
...
def handle_list_command(
    ...
    outputs = remove_empty_elements([map_keys(result, mappings) for result in response["results"]])
...
def list_firewall_policies_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    for result in response["results"]:
        if name == result["name"]:
            outputs = map_keys(result, mappings) | build_security(result)
            break
    else:
        for result in response["results"]:
            outputs.append(map_keys(result, mappings) | build_security(result))
