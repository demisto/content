...
def test_investigate_url_authentication_error(requests_mock):
    ...
    requests_mock.post(
        f"{BASE_URL}/sherlock/investigate?apikey={MOCK_APIKEY}", json=util_load_json("test_data/authentication_error.json")
    )
    client = Client(base_url=BASE_URL, verify=False)
    args = {"url": ["https://www.paloaltonetworkscom/"]}
...
def test_success_investigate_url_command(requests_mock):
    ...
    requests_mock.post(
        f"{BASE_URL}/sherlock/investigate?apikey={MOCK_APIKEY}",
        json=util_load_json("test_data/investigate_api_successful_response.json"),
    )
    client = Client(base_url=BASE_URL, verify=False)
    args = {"url": "https://www.paloaltonetworks.com/"}
...
def test_get_chosen_nothing_phishup_action_command():
    ...
    params = {"phishup-playbook-action": "Nothing"}
...
def test_auth_success_test_module(requests_mock):
    ...
    requests_mock.post(
        f"{BASE_URL}/auth-service/ValidateApiKey?apikey={MOCK_APIKEY}", json={"Status": {"Result": "Success", "Message": ""}}
    )
    client = Client(base_url=BASE_URL, verify=False)
...
def test_auth_error_test_module(requests_mock, mocker):
    ...
    patcher.start()
    requests_mock.post(
        f"{BASE_URL}/auth-service/ValidateApiKey?apikey={MOCK_APIKEY}",
