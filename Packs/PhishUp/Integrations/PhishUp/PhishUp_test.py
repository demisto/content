...
def test_investigate_url_authentication_error(requests_mock):
    ...
    requests_mock.post(f'{BASE_URL}/sherlock/investigate?apikey={MOCK_APIKEY}',
                       json=util_load_json(os.path.dirname(__file__) + "/test_data/authentication_error.json"))
    client = Client(
        base_url=BASE_URL,
        verify=False)
    args = {
...
def test_success_investigate_url_command(requests_mock):
    ...
    requests_mock.post(f'{BASE_URL}/sherlock/investigate?apikey={MOCK_APIKEY}',
                       json=util_load_json(os.path.dirname(__file__) + "/test_data/investigate_api_successful_response.json"))
    client = Client(
        base_url=BASE_URL,
        verify=False)
    args = {
