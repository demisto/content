    ...
    (
        {
            ...
            "contacts": {
                "admin": {"country": "US", "state": "CA", "name": "Google LLC"},
                "tech": {"organization": "Google LLC", "state": "CA", "country": "US"},
                "registrant": {"organization": "Google LLC", "state": "CA", "country": "US"},
                "billing": None,
            },
...
def test_ip_command(mocker: MockerFixture):
    ...
    result = ip_command(reliability=DBotScoreReliability.B, should_error=False)
    assert len(result) == 3
...
@pytest.mark.parametrize(
    ...
    [
        (["2024-05-09T00:00:00Z"], datetime.datetime(2024, 5, 9, 0, 0, 0)),
...
def test_whois_with_verbose(args, expected_res, mocker: MockerFixture):
    ...
    result = Whois.whois_command(reliability="B - Usually reliable")
    ...
    expected = [
        {
            ...
            "street1": "Google Ireland Holdings Unlimited Company",
            "street2": "70 Sir John Rogerson's Quay",
            "street3": "2 Dublin",
            "country": "IE",
            "phone": "+353.14361000",
            "fax": None,
            "email": "email@google.com",
            "changedate": "2022-10-15T05:41:14.918179Z",
        ...
        },
        {
            ...
            "street1": "2150 S. Bonito Way, Suite 150",
            "street2": "83642 Meridian",
            "street3": None,
            "street4": None,
            "phone": "+1.2083895740",
            "fax": "+1.2083895771",
            "email": "email@markmonitor.com",
            "changedate": "2023-09-07T07:32:23.899353Z",
            "country": "US",
            "registrar": "MARKMONITOR Inc.",
        },
        {
            "handle": "GIHU101-FRNIC",
            "type": "ORGANIZATION",
            "name": "Google Ireland Holdings Unlimited Company",
            "street1": "70 Sir John Rogerson's Quay",
            "street2": "2 Dublin",
            "street3": None,
            "street4": None,
            "phone": "+353.14361000",
...
@pytest.mark.parametrize(
    "param_key, param_value, arg_key, arg_value, expected_res",
    [
...
@pytest.mark.parametrize(
    "args,execution_metrics_supported,expected_entries",
    [
...
def test_execution_metrics_appended(
    args: dict[str, str], execution_metrics_supported: bool, expected_entries: int, mocker: MockerFixture, capfd
):
    """
...
def test_error_entry_type(args: dict[str, str], entry_type: EntryType, mocker: MockerFixture, capfd: pytest.CaptureFixture):
    mocker.patch.object(demisto, "command", "whois")
    mocker.patch.object(demisto, "args", return_value=args)
    mocker.patch.object(Whois, "get_whois_raw", return_value=load_test_data("./test_data/whois_raw_response.json")["result"])
...
@pytest.mark.parametrize(
    ...
    [
        (
            ExecutionMetrics(success=100, general_error=6),
            ipwhois_exception_mapping,
            ipwhois.exceptions.WhoisLookupError,
            (ErrorTypes.GENERAL_ERROR, 7),
        ),
        (
            ExecutionMetrics(service_error=12),
            ipwhois_exception_mapping,
            ipwhois.exceptions.BlacklistError,
            (ErrorTypes.SERVICE_ERROR, 13),
        ),
        (
            ExecutionMetrics(success=100, general_error=6),
            ipwhois_exception_mapping,
            ipwhois.exceptions.NetError,
            (ErrorTypes.CONNECTION_ERROR, 1),
        ),
        (ExecutionMetrics(), ipwhois_exception_mapping, BaseException, (ErrorTypes.GENERAL_ERROR, 1)),
        (ExecutionMetrics(), whois_exception_mapping, BaseException, (ErrorTypes.GENERAL_ERROR, 1)),
        (ExecutionMetrics(success=100, general_error=6), whois_exception_mapping, socket.error, (ErrorTypes.CONNECTION_ERROR, 1)),
        (ExecutionMetrics(), whois_exception_mapping, TypeError, (ErrorTypes.GENERAL_ERROR, 1)),
        (ExecutionMetrics(), whois_exception_mapping, WhoisInvalidDomain, (ErrorTypes.GENERAL_ERROR, 1)),
    ],
)
def test_exception_type_to_metrics(
    em: ExecutionMetrics, mapping: dict[type, str], exception_caught: type, expected: tuple[str, int]
):
    """
    ...
    """
    actual: ExecutionMetrics = increment_metric(execution_metrics=em, mapping=mapping, caught_exception=exception_caught)
...
@pytest.mark.parametrize("domain,expected", [("google.com", "whois.verisign-grs.com")])
...
def test_domain_command(args: dict[str, Any], expected_res, mocker: MockerFixture):
    ...
    result = domain_command(reliability="B - Usually reliable")
    assert len(result) == expected_res
...
def test_new_test_command(mocker: MockerFixture):
    ...
    mocker.patch.object(
        ...
        return_value=load_test_data("test_data/test-arrange-data.json")["res"]["google"],
    )
    Whois.main()
    assert_results_ok()
...
@pytest.mark.parametrize(
    ...
    [
        ({"contact": "abuse@domain.com", "support": "support@domain.com"}, "abuse@domain.com", {"support": "support@domain.com"}),
        ({"emails": ["abuse@domain.com", "infoabuse@domain.com"]}, ["abuse@domain.com", "infoabuse@domain.com"], {"emails": []}),
        (
