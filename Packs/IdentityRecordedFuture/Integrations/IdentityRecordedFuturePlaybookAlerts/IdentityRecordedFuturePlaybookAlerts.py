...
class Client(BaseClient):
    ...
    def details_playbook_alerts(self) -> Dict[str, Any]:
        ...
        return self._call(url_suffix="/playbook_alert/lookup", demisto_args=parsed_args)

    def update_playbook_alerts(self) -> Dict[str, Any]:
        parsed_args = demisto.args()
        if ids := parsed_args.get("alert_ids"):
            parsed_args["alert_ids"] = ids.split(",")
        return self._call(url_suffix="/playbook_alert/update", demisto_args=parsed_args)
    ...
    def search_playbook_alerts(self) -> Dict[str, Any]:
        ...
        return self._call(url_suffix="/playbook_alert/search", demisto_args=parsed_args)
...
class Actions:
    ...
    def _transform_incidents_attachments(self, incidents: list) -> None:
...
def main() -> None:
    ...
    try:
        ...
        headers = {
            ...
            "X-RF-User-Agent": (
                f"IdentityRecordedFuturePlaybookAlerts.py/{__version__} ({platform.platform()}) "
                f"XSOAR/{__version__} "
