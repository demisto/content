...
| custom_fields | - The custom field to update.<br/>- "The format is `{\"customFieldID1\": [\"value1\",\"value2\"], \"customFieldID2\": \"value3\"}`."<br/>- Use an array if the field is of multiselect type.<br/>- Instruction for each custom field type:<br/>  - Boolean: use "0" for False and "1" for True.<br/>  - Date: the format is "yyyy-mm-dd".<br/>  - Float: A number with two digits after the decimal point.<br/>  - Integer: A number.<br/>  - Key/Value list: Use the Key (ID) of the value. | Optional | 
