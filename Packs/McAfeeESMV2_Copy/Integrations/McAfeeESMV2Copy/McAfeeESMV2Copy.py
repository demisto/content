...
class McAfeeESMClient(BaseClient):
    ...
    def __requestyot(self, mcafee_command, data=None, params=None):
        if data:
            data = json.dumps(data)
        try:
            result = self._http_request('POST', mcafee_command, data=data,
    ...
    def fetch_alarms(self, since: str = None, start_time: str = None, end_time: str = None, raw: bool = False) \
        ...
        raw_response = self.__request(path, params=params)
        result = raw_response
    ...
    def __alarms_to_incidents(self, start_time: str, start_id: int = 0, limit: int = 1) -> Tuple[List, Dict]:
        ...
        if all_alarms:
            for i in all_alarms:
                current_run['time'] = i.get('triggeredDate', current_time)
                current_run['id'] = i['id']
                path = 'notifyGetTriggeredNotificationDetail'
                xxx = self.__request(path, data={'id': i['id']})
                xxx = dict_times_set(xxx, self.difference)
                i['xxx'] = xxx
                #i['evt_detail'] = []
                #i['evt'] = []
                # for m in xxx['events']:
                #     path = 'ipsGetAlertData'
                #     evt_detail = self.__request(path, data={'id': m['eventId']})
                #     evt_detail = dict_times_set(evt_detail, self.difference)
                #     #i['evt_detail'].append(evt_detail)
                #     #for a in evt_detail:
                #     ips = evt_detail.get('ipsId')
                #     #ipsId = ips['id']
                #     alertId = evt_detail.get('alertId')
                #     ips = str(ips)
                #     alertId = str(alertId)
                #     corId = f'{ips}|{alertId}'
                #     corId = ips + '|' + alertId
                #     #corId = '144134980090462208|1562363234'
                #     u = 'NGCP'
                #     usr = u.encode()
                ...
                #     pswd = 'Cyf3nc3789@'
                #     pswd = pswd.encode()
                #     pswd = base64.b64encode(pswd)
                #     pswd = pswd.decode('UTF-8')
                #     #usr = self.base64encoder('NGCP')
                ...
                #     aut = requests.post("https://192.168.50.102/rs/esm/v2/login",data=payload,headers=headers,verify=False)
                #     header_response = aut.headers
                #     jwttoken = header_response['Set-Cookie'].split(';')[0]
                #     xsrf_token = header_response['Xsrf-Token']
                #     authenticated_header = {'content-type': "application/json",
                #                                 'cache-control': "no-cache",
                #                                 'Cookie': "",
                #                                 'X-Xsrf-Token': ""}
                #     authenticated_header["Cookie"] = jwttoken.strip()
                #     authenticated_header["X-Xsrf-Token"] = xsrf_token.strip()
                #     corId = '144134980593778688|1562722858'
                #     fields = []
                ...
                #     else:
                #         list_of_fields = []
                #     data = {'eventId': {'value': corId}, 'fields': list_of_fields}
                #     pj = json.dumps(data)
                #     data=requests.post("https://192.168.50.102/rs/esm/qryGetCorrEventDataForID?queryType=EVENT",data=pj,headers=authenticated_header,verify=False)
                #     d = data.json()
                # for e in evt['return']:
                #    for v in e['values']:
                # i['evt'].append(d)
                ...
                #i['xxx'] = xxx
                all_alarms = create_incident(i, alarms=True)
        else:
            current_run['time'] = current_time
            current_run['id'] = start_id
        #_, _, all_events = self.list_alarm_events(self)
...
def convert_time_format(current_time: str,
    ...
    # mcafee_format: str = '%d-%m-%Y %H:%M:%S') -> str:
    """
...
def create_incident(raw_incidents: List[Dict], alarms: bool) -> List[Dict[str, Dict]]:
    ...
    alarm_id = str(raw_incidents['id'])
    incidents = []
    incident = raw_incidents
    # incident1 = raw_incidents
    # incident2 = incident1['xxx'][0].pop('events')
    # incident1['xxx'] = ''
    #events = incident['events']
    for i in incident['xxx']['events']:
        eventsName = i['ruleMessage']
        # eventId = i['eventId']
    # for incident in raw_incidents:
        #alarm_id = str(incident.get('id'))
        #inc = list_alarm_events2(alarm_id)
        #summary = str(incident.get('summary'))
        #alarmName = str(incident.get('alarmName'))
        incident_type = 'alarm' if alarms else 'case'
        incidents.append({
            'name': f'McAfee ESM {incident_type}. id: {alarm_id}. {eventsName}',
            # 'eventId': eventId,
            # 'name': f'McAfee ESM {incident_type}. id: {alarm_id}. {incident}',
