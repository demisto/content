...
def http_request(url_suffix, commands=None):
    ...
    if url_suffix != "/login":
        ...
        data = {"token": session_token, "customer_name": CUSTOMER_NAME}
    else:
    ...
    res = requests.post(BASE_URL + url_suffix, verify=USE_SSL, json=data)
...
def initialize():
    endpoint = "/login"
    commands = {"command": "login", "api_token": API_KEY}
    ...
    if session_token is None or (token_expires is not None and token_expires < int(time.time())):
        ...
        demisto.setIntegrationContext({"session_token": r["token"], "token_expires": int(time.time() + (10 * 60))})
        return
...
def block_ip(ip, description):
    commands = {"command": "new_blocklist", "entry": {"ip": ip, "description": description, "created": int(time.time())}}
...
def block_ip_command(args):
    ...
    if results:
        readable_outputs = tableToMarkdown("Block IP", results, ["Result"], removeNull=True)

        return_results(
            CommandResults(
                outputs=results, readable_output=readable_outputs, outputs_prefix="IP(val.Address === obj.Address).Address"
            )
        )
    if errors:
        return_error("\n".join(errors))
...
@logger
def unblock_ip(ip):
    commands = {"command": "delete_blocklist", "ip": ip}
...
def unblock_ip_command(args):
    ...
    md = tableToMarkdown("Unblock IP", results, ["Result"], removeNull=True)

    ec = {"IP(val.Address === obj.Address)": {"Address": ip}}
...
def blacklist_ip(ip, description):
    commands = {"command": "new_blacklist", "entry": {"ip": ip, "description": description, "created": int(time.time())}}
...
def blacklist_ip_command(args):
    ...
    if results:
        readable_outputs = tableToMarkdown("Blacklist IP", results, ["Result"], removeNull=True)
...
def unblacklist_ip(ip):
    commands = {"command": "delete_blacklist", "ip": ip}
...
def unblacklist_ip_command(args):
    ...
    md = tableToMarkdown("Unblacklist IP", results, ["Result"], removeNull=True)
...
def whitelist_ip(ip, description):
    commands = {"command": "new_whitelist", "entry": {"ip": ip, "description": description, "created": int(time.time())}}
...
def whitelist_ip_command(args):
    ...
    md = tableToMarkdown("Whitelist IP", results, ["Result"], removeNull=True)
...
def unwhitelist_ip(ip):
    commands = {"command": "delete_whitelist", "ip": ip}
...
def unwhitelist_ip_command(args):
    ...
    md = tableToMarkdown("Unwhitelist IP", results, ["Result"], removeNull=True)
...
def get_entity_risk(entity_id):
    commands = {"command": "risk_changes", "id": entity_id}
...
def get_entities_command(args):
    ...
    for entity in results:
        ...
        for actor in entity.get("actors", []):
            ...
            dbot_scores.append({"Vendor": "ThreatX", "Indicator": ipdot, "Type": "ip", "Score": dbscore})

            if dbscore == 3:
                ip_enrich.append(
                    {
                        "Address": ipdot,
                        "Malicious": {"Vendor": "ThreatX", "Description": "ThreatX risk score is " + str(risk_score)},
                    }
                )
            else:
                ip_enrich.append({"Address": ipdot})
        entities_context.append({"ID": e_id, "Name": entity["codename"], "IP": iplist, "Risk": risk_score})

        human_readable.append(
            {"Name": entity["codename"], "ID": e_id, "IP Addresses": ", ".join(iplist), "ThreatX Risk Score": risk_score}
        )
...
def get_entity_notes(entity_id):
    commands = {"command": "notes", "id": entity_id}
...
def get_entity_notes_command(args):
    ...
    md = tableToMarkdown("Entity Notes", sorted_results, headerTransform=string_to_table_header)

    ec = {"Threatx.Entity(val.ID && val.ID === obj.ID)": {"ID": entity_id, "Note": sorted_results}}
...
def add_entity_note(entity_id, message):
    commands = {"command": "new_note", "note": {"entity_id": entity_id, "content": message}}
...
def add_entity_note_command(args):
    ...
    md = tableToMarkdown("New Entity Note", results, ["Result"], removeNull=True)
