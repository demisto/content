...
def test_get_user_command__existing_user(mocker):
    ...
    """
    client = mock_client()
    args = {"user-profile": {"emails": "testdemisto2@paloaltonetworks.com"}}
    ...
    user_profile = IAMCommand(get_user_iam_attrs=["emails"]).get_user(client, args)
...
def test_create_user_command__success(mocker):
    ...
    user_profile = IAMCommand(get_user_iam_attrs=["emails"]).create_user(client, args)
    outputs = get_outputs_from_user_profile(user_profile)
...
def test_create_user_command__user_already_exists(mocker):
    ...
    """
    client = mock_client()
    args = {"user-profile": {"emails": "testdemisto2@paloaltonetworks.com"}, "allow-enable": "false"}
...
def test_update_user_command__allow_enable(mocker):
    ...
    client = mock_client()
    args = {
        "user-profile": {"emails": "testdemisto2@paloaltonetworks.com", "givenname": "mock_first_name"},
        "allow-enable": "true",
    }
...
def test_disable_user_command__non_existing_user(mocker):
    ...
    user_profile = IAMCommand(get_user_iam_attrs=["emails"]).disable_user(client, args)
    outputs = get_outputs_from_user_profile(user_profile)
