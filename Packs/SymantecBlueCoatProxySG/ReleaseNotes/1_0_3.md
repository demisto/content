...
- Updated the mapping for the following fields: 
  - `cs-uri-port` &rarr; `xdm.target.port` (~*xdm.source.port*~).
  - `cs-host` &rarr; `xdm.target.host.hostname` & `xdm.network.http.domain` (~*xdm.source.host.hostname*~).
  - `s-ip` &rarr; `xdm.intermediate.host.ipv4_addresses` & `xdm.intermediate.host.ipv6_addresses` (~*xdm.source.host.ipv4_addresses* & *xdm.source.host.ipv6_addresses*~).
  - `x-virus-id` &rarr; `xdm.alert.original_threat_id` (~*xdm.alert.original_alert_id*~).
