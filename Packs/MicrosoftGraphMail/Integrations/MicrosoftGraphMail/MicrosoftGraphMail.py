...
class MsGraphMailClient(MsGraphMailBaseClient):
    ...
    def fetch_incidents(self, last_run):
        ...
        demisto.debug("[test]MicrosoftGraphMail - Start fetching")
        demisto.debug(f"[test]MicrosoftGraphMail - Last run: {json.dumps(last_run)}")
        ...
        demisto.debug(f"[test]MicrosoftGraphMail - {start_fetch_time=}, {end_fetch_time=}")

        exclude_ids = list(set(last_run.get("LAST_RUN_IDS", [])))  # remove any possible duplicates
        demisto.debug(f"[test]MicrosoftGraphMail - Exclude ids: {exclude_ids}")

        last_run_folder_path = last_run.get("LAST_RUN_FOLDER_PATH")
        demisto.debug(f"[test]MicrosoftGraphMail - {last_run_folder_path=}")
        folder_path_changed = last_run_folder_path != self._folder_to_fetch

        last_run_account = last_run.get("LAST_RUN_ACCOUNT")
        demisto.debug(f"[test]MicrosoftGraphMail - {last_run_account=}")
        ...
        if folder_path_changed or mailbox_to_fetch_changed:
            ...
            demisto.info("[test]MicrosoftGraphMail - detected file path change, ignored LAST_RUN_FOLDER_ID from last run.")
            demisto.debug(f"[test]MicrosoftGraphMail - {folder_id=}")
        else:
            ...
            demisto.debug(f"[test]MicrosoftGraphMail - folder id from last run: {folder_id=}")
        ...
        demisto.debug(f"[test]MicrosoftGraphMail - fetched {len(fetched_emails)} emails")
        demisto.debug(f'[test]MicrosoftGraphMail - fetched email IDs before removing duplications - {[email.get("id") for email in fetched_emails]}')
        ...
        demisto.debug(f'[test]MicrosoftGraphMail - fetched email IDs after removing duplications - {[email.get("ID") for email in incidents]}')
        ...
        demisto.debug(f"[test]MicrosoftGraphMail - Next run after incidents fetching: {json.dumps(last_run)}")

        demisto.debug(f"[test]MicrosoftGraphMail - Number of incidents before filtering: {len(fetched_emails)}")
        demisto.debug(f"[test]MicrosoftGraphMail - Number of incidents after filtering: {len(incidents)}")
        demisto.debug(f"[test]MicrosoftGraphMail - Number of incidents skipped: {len(fetched_emails)-len(incidents)}")
        for incident in incidents:  # remove the ID from the incidents, they are used only for look-back.
            incident.pop("ID", None)

        demisto.info(f"[test]fetched {len(incidents)} incidents")
        demisto.debug(f"[test]{next_run=}")
...
def main():  # pragma: no cover
    try:
        ...
        demisto.debug("[test]MicrosoftGraphMail - Command being called is {command}")

        if command == "test-module":
            client.test_connection()
            return_results("ok")
        if command == "fetch-incidents":
            demisto.debug("[test]MicrosoftGraphMail - Fetching incidents")
            next_run, incidents = client.fetch_incidents(demisto.getLastRun())
            demisto.debug("[test]MicrosoftGraphMail - Completed fetching incidents")
            demisto.debug(f"[test]MicrosoftGraphMail - Fetched {len(incidents)} incidents")
            demisto.debug("[test]MicrosoftGraphMail - Setting last run")
            demisto.debug(f"[test]MicrosoftGraphMail - Next run: {next_run}")
            demisto.setLastRun(next_run)
            demisto.debug("[test]MicrosoftGraphMail - Completed setting last run, setting incidents")
            demisto.incidents(incidents)
            demisto.debug("[test]MicrosoftGraphMail - Completed setting incidents")
