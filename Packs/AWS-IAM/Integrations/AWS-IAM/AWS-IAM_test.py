...
ATTACHED_RESPONSE = {"AttachedPolicies": ATTACHED_POLICIES, "IsTruncated": True, "Marker": "111"}

PAGINATION_CHECK = [
    ({"userName": "test"}, ["AllAccessPolicy", "KeyPolicy"]),
    ({"userName": "test", "page": 2, "page_size": 1, "marker": "111"}, ["KeyPolicy"]),
]
...
def test_list_user_policies(mocker, args, res):
    ...
    response = {"PolicyNames": ["AllAccessPolicy", "KeyPolicy"], "IsTruncated": True, "Marker": "111"}
...
def test_list_attached_user_polices(mocker):
    ...
    assert [{"UserName": "test", "PolicyName": policy_name, "PolicyArn": policy_arn}] in contents.get("EntryContext").values()
...
def test_list_attached_group_polices(mocker):
    ...
    assert [{"GroupName": "test", "PolicyName": policy_name, "PolicyArn": policy_arn}] in contents.get("EntryContext").values()
    assert "111" in contents.get("EntryContext").values()
...
def test_get_user_login_profile(mocker):
    ...
    data = {"UserName": "test", "LoginProfile": {"CreateDate": "2021-11-07 15:55:03", "PasswordResetRequired": False}}
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"ResponseMetadata": {"HTTPStatusCode": 200}},
            "Policy policy_name was added to user user_name",
        )
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            {"ResponseMetadata": {"HTTPStatusCode": 200}},
            "Policy policy_name was added to group group_name",
        )
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"roleName": "role_name", "tags": "Key:Value"},
            {"ResponseMetadata": {"HTTPStatusCode": 200}},
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"userName": "user_name", "tags": "Key:Value, Key1:Value1"},
            {"ResponseMetadata": {"HTTPStatusCode": 200}},
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"roleName": "role_name", "tagKeys": "Key1"},
            {"ResponseMetadata": {"HTTPStatusCode": 200}},
...
@pytest.mark.parametrize(
    "args, mocked_res, expected_hr, expected_ec",
    [
        (
            {"accessKeyId": "access_Key_Id"},
            {
                "UserName": "user_name",
                "AccessKeyLastUsed": {
                    "LastUsedDate": datetime.datetime(2023, 6, 6, 14, 32),
                    "ServiceName": "test",
                    "Region": "Here",
                },
...
@pytest.mark.parametrize(
    "is_truncated,expeted_second_output,expected_marker",
    (
        pytest.param(
            True,
            (
...
def test_list_attached_role_policies(mocker, is_truncated: bool, expeted_second_output: str, expected_marker: str | None) -> None:
    ...
    args = {"roleName": "test-role"}
    response = {
        "AttachedPolicies": [{"PolicyName": "policy1", "PolicyArn": "Arn1"}, {"PolicyName": "policy2", "PolicyArn": "Arn2"}],
...
def test_deactivate_mfa_device(mocker: MockerFixture):
    ...
    """
    mock_res = {"ResponseMetadata": {"HTTPStatusCode": 200}}

    args = {"userName": "test", "serialNumber": "test1"}
...
def test_list_mfa_devices(mocker: MockerFixture, page_size: int):
    ...
    mock_res = {
        "MFADevices": [
            {"UserName": "test", "SerialNumber": "test", "EnableDate": datetime.datetime(2021, 11, 7, 15, 55, 3)},
            {"UserName": "test2", "SerialNumber": "test2", "EnableDate": datetime.datetime(2022, 11, 7, 15, 55, 3)},
        ],
        "Marker": "test",
    }
