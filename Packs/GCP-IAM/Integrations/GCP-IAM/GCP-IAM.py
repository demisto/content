...
class Client(BaseClient):
    ...
    def gcp_iam_project_list_request(self, parent: str, limit: int, page_token=None) -> dict:
        """
...
def update_time_format(data: dict, keys: list) -> None:
    """
    Update dictionary time values to appropriate XSOAR system time format.
    Args:
        data (dict): Information to update.
...
def generate_iam_policy_command_output(response: dict, resource_name: str) -> CommandResults:
    """
    Generate command output for iam-policy commands.
    Args:
        response (dict): API response from XSOAR.
    ...
    readable_output = tableToMarkdown(
        f'{resource_name} IAM policy information:',
...
def gcp_iam_project_list_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """
    List projects under the specified parent.
    Args:
    ...
    if page > 1:
        page_token = get_next_page_token(limit, page, client.gcp_iam_project_list_request, args={"parent": parent})
    ...
    response = client.gcp_iam_project_list_request(parent, limit, page_token)
...
def gcp_iam_project_get_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    """
    Retrieve project information.
    Args:
        client (Client): GCP API client.
    ...
    readable_output = tableToMarkdown(
        f'Project {project_name} information:',
        outputs,
        headers=['name', 'parent', 'projectId', 'displayName', 'createTime', 'updateTime'],
        headerTransform=pascalToSpace
    )
...
def gcp_iam_project_iam_member_add_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    updated_policies = add_members_to_policy(role=role, iam_policy=iam_policy, members=members,
                                             command_name='gcp-iam-project-iam-policy-add')

    response = client.gcp_iam_project_iam_policy_set_request(project_name, updated_policies)
...
def gcp_iam_project_iam_policy_add_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    response = client.gcp_iam_project_iam_policy_set_request(project_name, iam_policy)
    return generate_iam_policy_command_output(response, project_name)
...
def main() -> None:
    ...
    service_account_key = json.loads(params.get('service', "{}"))
    ...
    try:
        ...
        commands = {
            'gcp-iam-project-list': gcp_iam_project_list_command,
            'gcp-iam-project-get': gcp_iam_project_get_command,
            'gcp-iam-project-iam-policy-get': gcp_iam_project_iam_policy_get_command,
            'gcp-iam-project-iam-test-permission': gcp_iam_project_iam_test_permission_command,
            ...
            'gcp-iam-project-iam-policy-add': gcp_iam_project_iam_policy_add_command,
            'gcp-iam-folder-list': gcp_iam_folder_list_command,
            'gcp-iam-folder-get': gcp_iam_folder_get_command,
            'gcp-iam-folder-iam-policy-get': gcp_iam_folder_iam_policy_get_command,
            'gcp-iam-folder-iam-test-permission': gcp_iam_folder_iam_test_permission_command,
            ...
            'gcp-iam-folder-iam-policy-add': gcp_iam_folder_iam_policy_add_command,
            'gcp-iam-organization-list': gcp_iam_organization_list_command,
            'gcp-iam-organization-get': gcp_iam_organization_get_command,
            'gcp-iam-organization-iam-policy-get': gcp_iam_organization_iam_policy_get_command,
            'gcp-iam-organization-iam-test-permission': gcp_iam_organization_iam_test_permission_command,
            ...
            'gcp-iam-organization-iam-policy-add': gcp_iam_organization_iam_policy_add_command,
            ...
            'gcp-iam-service-account-list': gcp_iam_service_account_list_command,
            'gcp-iam-service-account-get': gcp_iam_service_account_get_command,
            ...
            'gcp-iam-service-account-key-list': gcp_iam_service_account_key_list_command,
            'gcp-iam-service-account-key-get': gcp_iam_service_account_key_get_command,
