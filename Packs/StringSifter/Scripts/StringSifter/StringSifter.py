...
def stringsifter(args: dict):
    ...
    for line in output_data.splitlines():
        ...
        words_rating_list.append(
            {"Rating": matches.group("rating") if matches else "", "Word": matches.group("word") if matches else ""}
        )
    ...
    return CommandResults(readable_output=readable, outputs=outputs, outputs_prefix="Stringsifter", outputs_key_field="FileName")
