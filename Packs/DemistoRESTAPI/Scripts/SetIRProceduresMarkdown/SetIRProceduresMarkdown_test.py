...
from SetIRProceduresMarkdown import SECTIONS_TO_KEEP, get_tasks_and_readable, set_incident_with_count
...
def test_set_ir_md(mocker):
    ...
    tasks_filtered = list(filter(lambda task: any(section in task.get("section") for section in SECTIONS_TO_KEEP), all_tasks))
