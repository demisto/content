...
def panos_keygen(url: str, credentials: dict) -> str:
    result = http_request(uri=url, method="POST", params={
        'type': 'keygen'
    },
        headers={
        ...
        body={
        'user': str(credentials.get('identifier')).lower(),
        'password': credentials.get('password')
    })

    try:
        key = str(result['response']['result']['key'])
    except KeyError:
        raise Exception('keygen result did not contain a key')
...
def initialize_instance(args: Dict[str, str], params: Dict[str, str]):
    ...
    if not params.get('key') in ('', None):
        API_KEY = params.get('key')  # type: ignore

    if params.get('credentials').get('identifier') and params.get('credentials').get('password'):
        API_KEY = panos_keygen(URL, params.get('credentials'))  # type: ignore
