...
def panorama_commit_command(args: dict):
    ...
    """
    use_polling = argToBoolean(args.get('polling', 'false'))
    ...
    if use_polling:
        ...
        if not job_id:
            api_result = ResponseObject(panorama_commit(args))

            if api_result.response.result.job.id:
                # commit has been given a jobid
                job_id = api_result.response.result.job.id
        ...
        # Check existing job
            ...
            api_response = ResponseObject(panorama_commit_status(args))

            if api_response.response.result.job.type != 'Commit':
                raise Exception('JobID given is not of a commit.')

            # Reschedule job if it's not complete
            if api_response.response.result.job.status != 'FIN':
                polling_args = {
                    'job_id': job_id,
                    'polling': 'true',
                    **args
            ...
            # Check the status of a completed job
                ...
                commit_status_output = {'JobID': api_response.response.result.job.id}
                if api_response.response.result.job.result == 'OK':
...
def panorama_query_logs_command(args: dict):
    ...
    if use_polling:
        ...
        if not job_id:
            ...
            # create new search
            api_result = ResponseObject(panorama_query_logs(log_type, number_of_logs, query, address_src, address_dst, ip_,
                                         zone_src, zone_dst, time_generated, action,
                                         port_dst, rule, url, filedigest)
            )
            #response = dict_safe_get(api_result, ['response'])
            #response_msg = dict_safe_get(response, ['msg'])
            #response_result = dict_safe_get(response, ['result'])
            ...
            if not api_result.response or not api_result.response.result or not api_result.response.result.job.id:
                raise Exception("Missing JobID in response.")
            ...
            if api_result.response.result.job.status != "FIN":
                ...
                if not api_result.response or not api_result.response.result or not api_result.response.result.job \
                        or not api_result.response.result.job.status:
                    raise Exception("Missing JobID status in response.")

                if not api_result.response or not api_result.response.result or not api_result.response.result.log \
        ...
        # Check the existing job
            ...
            # Output the result if the job has finished
                ...
                content_download_status = {
                    'JobID': api_result.response.result.job.id
                }
                if api_result.response.result.job.result == 'OK':
