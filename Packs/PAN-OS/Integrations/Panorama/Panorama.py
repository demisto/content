...
def prettify_data_filtering_rule(rule: Dict) -> Dict:
    ...
    if 'application' in rule and isinstance(rule['application'], dict) and 'member' in rule['application']:
        pretty_rule['Application'] = rule['application']['member']
    if 'file-type' in rule and isinstance(rule['file-type'], dict) and 'member' in rule['file-type']:
...
def prettify_profile_rule(rule: Dict) -> Dict:
    ...
    if 'severity' in rule and isinstance(rule['severity'], dict) and 'member' in rule['severity']:
        pretty_rule['Severity'] = rule['severity']['member']
    if 'threat-name' in rule and isinstance(rule['threat-name'], dict) and 'member' in rule['threat-name']:
    ...
    if 'sinkhole' in rule:
        if isinstance(rule['sinkhole'], dict):
    ...
    if 'cve' in rule and isinstance(rule['cve'], dict) and 'member' in rule['cve']:
        pretty_rule['CVE'] = rule['cve']['member']
    if 'vendor-id' in rule and isinstance(rule['vendor-id'], dict) and 'member' in rule['vendor-id']:
...
def prettify_wildfire_rule(rule: Dict) -> Dict:
    ...
    if 'application' in rule and isinstance(rule['application'], dict) and 'member' in rule['application']:
        pretty_rule['Application'] = rule['application']['member']
    if 'file-type' in rule and isinstance(rule['application'], dict) and 'member' in rule['file-type']:
        pretty_rule['File-type'] = rule['file-type']['member']
    if 'analysis' in rule:
        pretty_rule['Analysis'] = rule['analysis']
