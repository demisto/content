...
def panorama_query_logs_command(args: dict):
    ...
    if not job_id:
        ...
        result: SimpleNamespace = panorama_query_logs(
        ...
        if result.response.status == 'error':
            if result.response.result.msg.line:
                raise Exception(f"Query logs failed. Reason is: {result.response.result.msg.line}")
            else:
                raise Exception('Query logs failed.')
        if not result.response.result.job:
            raise Exception('Missing JobID in response.')
        query_logs_output = {
            'JobID': result.response.result.job,
            'Status': 'Pending',
        ...
        poll_result = PollResult(
            ...
            args_for_next_run={
                'query_log_job_id': result.response.result.job,
                'log-type': log_type,
            ...
            partial_result=CommandResults(
                readable_output=f"Fetching logs for job ID {result.response.result.job}..."
            )
        )
        ...
        if parsed.response.result.job.status == 'FIN':
            ...
            try:
                pretty_traffic_logs = prettify_traffic_logs(
                    res.get('response', {}).get('result', {}).get('log', {}).get('logs', {}).get('entry', []))
            except:
                pretty_traffic_logs = []
            query_traffic_logs_output['Logs'] = pretty_traffic_logs
