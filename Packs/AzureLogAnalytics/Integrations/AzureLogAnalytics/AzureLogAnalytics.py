...
class Client:
    ...
    def http_request(self, method, url_suffix=None, full_url=None, params=None, data=None, resource=None, timeout=10):
        if not params:
            params = {}
        if not full_url:
...
def validate_params(
    refresh_token, managed_identities_client_id, client_credentials, enc_key, self_deployed, certificate_thumbprint, private_key
):
    ...
    if not managed_identities_client_id:
        if not self_deployed and not enc_key:
            raise DemistoException(
                "Key must be provided when not using self deployed flow. For further information see "
...
def tags_arg_to_request_format(tags) -> list[dict[str, str]] | None:
    ...
    try:
        ...
        return [{"name": tag[0], "value": tag[1]} for tag in tags]
...
def execute_query_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    data = {"timespan": args.get("timespan"), "query": query, "workspaces": argToList(args.get("workspaces"))}

    remove_nulls_from_dictionary(data)

    response = client.http_request(
        "POST", full_url=full_url, data=data, resource=client.azure_cloud.endpoints.log_analytics_resource_id, timeout=timeout
    )
    ...
    for table in response.get("tables", []):
        ...
        readable_output += tableToMarkdown(name=name, t=data, headers=columns, headerTransform=pascalToSpace, removeNull=True)
        output.append({"TableName": name, "Data": data, "Query": query})
...
def list_saved_searches_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    output = [flatten_saved_search_object(saved_search) for saved_search in response[from_index:to_index]]

    readable_output = tableToMarkdown(
        "Saved searches", output, headers=SAVED_SEARCH_HEADERS, headerTransform=pascalToSpace, removeNull=True
    )
...
def get_saved_search_by_id_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(title, output, headers=SAVED_SEARCH_HEADERS, headerTransform=pascalToSpace, removeNull=True)
...
def workspace_list_command(client: Client) -> CommandResults:
    ...
    response = client.http_request(
        "GET", full_url=full_url, params={"api-version": API_VERSION}, resource=client.azure_cloud.endpoints.resource_manager
    )
    value = response.get("value", [])
...
def run_search_job_command(args: dict[str, Any], client: Client) -> PollResult:
    ...
    if argToBoolean(args["first_run"]):
        ...
        try:
            client.http_request(
                "PUT", url_suffix, resource=client.azure_cloud.endpoints.resource_manager, data=data
