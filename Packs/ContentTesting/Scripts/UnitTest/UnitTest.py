    ...
    try:
        ...
        if attempts < 5:
            for _key, val in tasks.items():
                if (
                    "scriptId" in val["task"]
                    and val["task"]["scriptId"] == scriptid
                    and val["task"]["name"] == "DisplayUnitTestResults"
                ):
                    new_id = val["id"]
            ...
            while attempts < 5:
                start_response = demisto.executeCommand(
                    "core-api-post",
                    {
                        ...
                        "body": {
                            "taskinfo": {"invId": incid, "inTaskID": task_id, "version": -1, "args": newargs, "loopArgs": {}},
                            "playbooksdebuginfo": {},
                            "pageSize": 50,
                        },
                    },
...
def RunAdhocPlaybook(playbookname, taskname, addafter, incid):
    try:
            ...
            if "name" in task["task"] and task["task"]["name"] == taskname:
                demisto.executeCommand(
                    "core-api-post",
                    {
                        "uri": "/inv-playbook/task/execute",
                        "body": {
                            "taskinfo": {"invId": incid, "inTaskID": key, "version": -1, "args": {}, "loopArgs": {}},
                            "playbooksdebuginfo": {},
                            "pageSize": 50,
                        },
                    },
        ...
        demisto.error(traceback.format_exc())
        return_error(f"RunAdhocPlaybook: Exception failed to execute. Error: {ex!s}")
...
def main():
    try:
        ...
        RunUTResults(newargs)
        return_error(f"UnitTest: Exception failed to execute. Error: {ex!s}")
