...
class Client(BaseClient):
    ...
    def test_apiModule(self):
        return self._http_request(
            method="GET", url_suffix="/realize/ransomwarerecovery/v1/quarantineranges", resp_type="response"
        )
    ...
    def get_findFsNasDevice(self, search_string):
        ...
        return self._http_request(
            method="GET", url_suffix="/realize/ransomwarerecovery/v1/search/backupset", params=params, resp_type="response"
        )
    ...
    def get_findUser(self, user_string):
        ...
        return self._http_request(
            method="GET", url_suffix="realize/ransomwarerecovery/v1/users", params=params, resp_type="response"
        )

    def get_findUserDevice(self, userId):
        params = {"users[]": userId, "resourceTypes[]": ["Endpoint", "OneDrive", "Google Drive"]}
        return self._http_request(
            method="GET", url_suffix="realize/ransomwarerecovery/v1/search/device", params=params, resp_type="response"
        )

    def get_findSharePointSites(self, search_string):
        params = {"siteTitlePrefix": search_string}
        return self._http_request(
            method="GET", url_suffix="/realize/ransomwarerecovery/v1/search/sharepoint-sites", params=params, resp_type="response"
        )
    ...
    def get_findSharedDrives(self, search_string):
        ...
        return self._http_request(
            method="GET",
            url_suffix="/realize/ransomwarerecovery/v1/search/shareddrive-accounts",
            params=params,
            resp_type="response",
        )
    ...
    def post_restoreToEndpoint(self, source_resourceid, target_resourceid, restore_location):
        ...
        json_data = {
            "deviceID": int(source_resourceid),
            "targetDeviceID": int(target_resourceid),
            "restoreLocation": restore_location,
        }
        return self._http_request(method="POST", url_suffix=url_suffix, json_data=json_data, resp_type="response")

    def get_restoreStatus(self, restore_id):
        url_suffix = f"/insync/endpoints/v1/restores/{restore_id!s}"
        return self._http_request(method="GET", url_suffix=url_suffix, resp_type="response")

    def post_decommission(self, resource_id):
        url_suffix = f"https://apis.druva.com/insync/endpoints/v1/devices/{resource_id!s}/decommission"
...
def Druva_QuarantineResource_Command(clientObj, org_id, resource_id, resource_type, from_date, to_date):
    ...
    if statusCode == 200:
        ...
        headers = ["RangeID"]
        results = CommandResults(
            readable_output=tableToMarkdown("Resource quarantined successfully", str(responseJson["rangeID"]), headers=headers),
...
def main():
    ...
    try:
        clientObj = Client(base_url=druvaBaseURL, verify=verify_certificate, headers=None, proxy=proxy)
        ...
        if command == "druva-update-quarantine-range":
            ...
            to_date = demisto.args().get("to_date")
            return_results(
                Druva_UpdateQuarantineRange_Command(clientObj, resource_id, resource_type, range_id, from_date, to_date)
            )
            return_results(Druva_ListQuarantineRanges_Command(clientObj))
