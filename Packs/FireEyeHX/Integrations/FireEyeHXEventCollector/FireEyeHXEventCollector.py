...
class Client(BaseClient):
    ...
    def get_access_token(self) -> str:
        ...
        token_response = self._http_request(
            "GET", url_suffix="/hx/api/v3/token", auth=(self.username, self.password), resp_type="response", ok_codes=[204]
        )
    ...
    def get_events_request(self, limit: str = "100", min_id: str = None, filter_query: str = None) -> dict:
        ...
        params = assign_params(sort="_id+ascending", limit=limit, min_id=min_id, filterQuery=filter_query)

        return self.http_request("GET", url_suffix="/hx/api/v3/alerts", ok_codes=[200], params=params)
...
def get_events_command(client: Client, max_fetch: str, first_fetch: str, should_push_events: bool) -> Union[str, CommandResults]:
    """
    Fetches events from the fireeye and return them to the war-room.
    """

    events = fetch_events(client=client, max_fetch=max_fetch, first_fetch=first_fetch, should_push_events=should_push_events)

    if not events:
        return "No events were found."

    return CommandResults(
        readable_output=tableToMarkdown("FireEye HX events", events, headerTransform=underscoreToCamelCase, removeNull=True),
        raw_response=events,
        outputs=events,
...
def fetch_events(client: Client, max_fetch: str, first_fetch: str, min_id: str = None, should_push_events: bool = True) -> list:
    """
...
def main() -> None:  # pragma: no cover
    ...
    try:
        ...
        if command == "test-module":
            ...
            fetch_events(client=client, max_fetch=max_fetch, first_fetch=last_fetch, min_id=last_run.get("last_alert_id"))
            ...
            return_results(
                get_events_command(client, max_fetch=max_fetch, first_fetch=first_fetch, should_push_events=should_push_events)
            )
        else:
            raise NotImplementedError(f"Command {command} is not implemented.")
    except Exception as e:
        return_error(f"Failed to execute {command} command. Error in FireEye HX Event Collector Integration [{e}].")
