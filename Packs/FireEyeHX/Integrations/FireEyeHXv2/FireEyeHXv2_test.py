...
UPSERT_COMMAND_DATA_BAD_CASES = [({"policyName": "test", "policyId": "test"}, "Enter a name or ID but not both")]
...
UPSERT_COMMAND_DATA_BAD_CASES = [({"hostSetId": "test", "policyId": "test"}, "Enter a Policy Id or Host Set Id but not both")]
...
UPSERT_COMMAND_DATA_CASES_LIST_POLICY = [
    ({"policyId": "test", "hostSetId": "test"}, Exception("400"), "This hostset may already be included in this policy"),
    ({"policyId": "test", "hostSetId": "test"}, "test", "Success"),
]
...
def test_assign_host_set_policy_command(mocker, demisto_args, return_mocker, expected_results):
    from FireEyeHXv2 import Client, assign_host_set_policy_command
...
UPSERT_COMMAND_DATA_CASES_LIST_POLICY = [
    ...
    ({"policyName": "test"}, {"policy_id": None, "enabled": None, "limit": 50, "offset": 0, "name": "test"}),
]
...
def test_list_policy_command(mocker, demisto_args, call_args):
    from FireEyeHXv2 import Client, list_policy_command

    return_value = {
        "data": {"entries": [{"_id": "test", "name": "test", "description": "test", "priority": "test", "enabled": "test"}]}
    }
...
UPSERT_COMMAND_DATA_CASES_LIST_HOST_SET_POLICY = [
    ({"hostSetId": 11}, {"hostSetId": 1, "hostSetPolicy": 0}, {}),
    ({}, {"hostSetId": 0, "hostSetPolicy": 1}, {"limit": 50, "offset": 0}),
    ({"limit": 5, "offset": 2}, {"hostSetId": 0, "hostSetPolicy": 1}, {"limit": 5, "offset": 2}),
]
...
def test_list_host_set_policy_command(mocker, demisto_args, call_count, call_args):
    from FireEyeHXv2 import Client, list_host_set_policy_command
...
def test_delete_host_set_policy_command(mocker, demisto_args, exception, expected_results):
    from FireEyeHXv2 import Client, delete_host_set_policy_command
...
UPSERT_COMMAND_DATA_BAD_CASES = [
    ...
    ({"agentId": "test"}, "agentId test is not correct"),
    ({"hostName": "test"}, "test is not found"),
]


@pytest.mark.parametrize("demisto_args, expected_results", UPSERT_COMMAND_DATA_BAD_CASES)
def test_get_host_information_command_failed(mocker, demisto_args, expected_results):
    from FireEyeHXv2 import Client, get_host_information_command
...
UPSERT_COMMAND_DATA_CASES_GET_ALL_HOSTS = [
    ({}, {"hosts_partial": 2}, [{"data": {"entries": [{}]}}, {"data": {"entries": []}}], [list, dict]),
    ({}, {"hosts_partial": 1}, [{"data": {"entries": []}}], [list, 0]),
]
...
def test_get_all_hosts_information(mocker, demisto_args, call_count, return_value_mock, expected_results):
    from FireEyeHXv2 import Client, get_all_hosts_information_command
...
def test_get_host_set_information(mocker, demisto_args, return_value_mock, expected_results):
    from FireEyeHXv2 import Client, get_host_set_information_command
...
UPSERT_COMMAND_DATA_BAD_CASES = [({}, "Please provide either agentId or hostName")]
...
UPSERT_COMMAND_DATA_BAD_CASES = [({}, "Agent ID is required")]
...
UPSERT_COMMAND_DATA_CASES_HOST_CONTAINMENT = [
    ({"hostName": "test"}, 1, None, dict),
    ({"agentId": "test"}, 0, None, dict),
    (
        {"agentId": "test"},
        0,
        Exception("422"),
    ...
    ({"agentId": "test"}, 0, Exception("409"), "This host may already in containment"),
]
...
def test_host_containment(mocker, demisto_args, call_count, return_mocker, expected_results):
    from FireEyeHXv2 import Client, host_containment_command
...
UPSERT_COMMAND_DATA_CASES_APPROVE_CONTAINMENT = [({"agentId": "test"}, 1)]
...
def test_approve_containment_command(mocker, demisto_args, call_count):
    from FireEyeHXv2 import Client, approve_containment_command
...
UPSERT_COMMAND_DATA_CASES_CANCEL_CONTAINMENT = [({"hostName": "test"}, 1), ({"agentId": "test"}, 0)]
...
def test_cancel_containment_command(mocker, demisto_args, call_count):
    from FireEyeHXv2 import Client, cancel_containment_command
...
def test_get_list_containment_command(mocker, demisto_args, args_call):
    from FireEyeHXv2 import Client, get_list_containment_command
...
UPSERT_COMMAND_DATA_BAD_CASES = [
    ({"name": "test", "condition": "test"}, "All of the following arguments are required -> ['name','category','condition']"),
    ({"category": "test", "name": "test"}, "All of the following arguments are required -> ['name','category','condition']"),
    ({"category": "test", "condition": "test"}, "All of the following arguments are required -> ['name','category','condition']"),
]
...
UPSERT_COMMAND_DATA_CASES_ENABLED_CONDITIONS = [
    ("test", "test", [{"data": {"entries": ["test", "test"]}}, {"data": {"entries": []}}], 2),
    ("test", "test", [{"data": {"entries": []}}], 0),
]
...
def test_get_all_enabled_conditions(mocker, category, name, results_mock, expected_results):
    from FireEyeHXv2 import Client, get_all_enabled_conditions
...
UPSERT_COMMAND_DATA_CASES_GET_INDICATOR = [({"category": "test", "name": "test"})]
...
def test_get_indicator_command(mocker, demisto_args):
    from FireEyeHXv2 import Client, get_indicator_command
...
UPSERT_COMMAND_DATA_CASES_GET_ALL_INDICATORS = [
    (2, [{"data": {"entries": ["test", "test"]}}, {"data": {"entries": []}}], 2),
    (2, [{"data": {"entries": []}}], 0),
    (1, [{"data": {"entries": ["test", "test"]}}, {"data": {"entries": []}}], 1),
]
...
def test_get_all_indicators(mocker, limit, results_mocker, expected_results):
    from FireEyeHXv2 import Client, get_all_indicators
...
UPSERT_COMMAND_DATA_CASES_GET_INDICATORS = [({"limit": 5, "alerted": "yes", "sort": "createdBy"})]
...
def test_get_indicators_command(mocker, demisto_args):
    from FireEyeHXv2 import Client, get_indicators_command
...
UPSERT_COMMAND_DATA_CASES_GET_INDICATOR_RESULT = [
    ({"event_type": "fileWriteEvent"}, "File"),
    ({"event_type": "ipv4NetworkEvent"}, "Ip"),
    ({"event_type": "Unknown"}, None),
]
...
UPSERT_COMMAND_DATA_CASES_START_SEARCH = [
    ({"searchId": 12}, {"data": {"state": "RUNNING", "stats": {"search_state": {"MATCHED": 0, "PENDING": 1}}}}, [False, "12"]),
    ({"searchId": 12}, {"data": {"state": "STOPPED", "stats": {"search_state": {"MATCHED": 0, "PENDING": 1}}}}, [True, "12"]),
    (
...
def test_start_search_command(mocker, args, searchInfo, expected_results):
    from FireEyeHXv2 import Client, start_search_command
...
UPSERT_COMMAND_DATA_CASES_SEARCH_RESULT = [
    ({"searchId": "11,12"}, {"search_result_request": 2, "tableToMarkdown": 2, "search_stop": 0, "search_delete": 0}, list),
    ({"searchId": "11"}, {"search_result_request": 1, "tableToMarkdown": 1, "search_stop": 0, "search_delete": 0}, list),
    (
...
def test_search_result_get_command(mocker, demisto_args, call_count, expected_results):
    from FireEyeHXv2 import Client, search_result_get_command
...
UPSERT_COMMAND_DATA_CASES_SEARCH_LIST = [
    ({"searchId": "11,12"}, {"search_by_id": 2, "search_request": 0}),
...
def test_get_search_list_command(mocker, demisto_args, call_count):
    from FireEyeHXv2 import Client, get_search_list_command
...
def test_search_delete_command(mocker, demisto_args, return_mocker, expected_results):
    from FireEyeHXv2 import Client, search_delete_command
...
def test_search_stop_command(mocker, demisto_args, return_mocker, expected_results):
    from FireEyeHXv2 import Client, search_stop_command
...
UPSERT_COMMAND_DATA_BAD_CASES = [
    ...
    ({"agentId": "test"}, "If the script is not provided, defaultSystemScript must be specified"),
    ({"agentId": "test", "script": "test"}, "If the script is provided, script name must be specified as well"),
]
...
UPSERT_COMMAND_DATA_CASES_DATA_ACQUISITION = [
    ({"hostName": "test", "defaultSystemScript": "win"}, 1),
    ({"agentId": "test", "defaultSystemScript": "win"}, 0),
]
...
def test_data_acquisition(mocker, demisto_args, call_count):
    from FireEyeHXv2 import Client, get_data_acquisition
...
UPSERT_COMMAND_DATA_CASES_DATA_ACQUISITION2 = [
    ({}, "RUNNING", [False, "12", 1, None]),
    ({"acquisition_id": "12"}, "COMPLETE", [True, "12", 0, {"state": "COMPLETE"}]),
]
...
def test_data_acquisition_command(mocker, demisto_args, state, expected_results):
    from FireEyeHXv2 import Client, data_acquisition_command
...
UPSERT_COMMAND_DATA_CASES_FILE_ACQUISITION = [
    ({"hostName": "test"}, "RUNNING", [False, "12", 1]),
    ({"agentId": "test"}, "RUNNING", [False, "12", 0]),
    ({"acquisition_id": "12"}, "COMPLETE", [True, "12", 0]),
]
...
def test_file_acquisition_command(mocker, demisto_args, state, expected_results):
    from FireEyeHXv2 import Client, file_acquisition_command
...
UPSERT_COMMAND_DATA_CASES_GET_ACQUISITION = [
    ...
    ({"acquisitionId": "test"}, "RUNNING", {"get_data": 0}),
]
...
def test_get_data_acquisition_command(mocker, demisto_args, state, call_count):
    from FireEyeHXv2 import Client, get_data_acquisition_command
...
def test_get_triage_acquisition_information_success(mocker, demisto_args, call_count, results):
    ...
    mocker.patch.object(Client, "get_token_request", return_value="test")
    client = Client(base_url="base_url", verify=False, proxy=True, auth=("userName", "password"))
    triage_info_call = mocker.patch.object(client, "triage_acquisition_information_request", return_value={"host":{"_id":"testID"}})
...
UPSERT_COMMAND_DATA_CASES_GET_ALERTS = [
    ({"limit": 2}, [{"data": {"entries": ["test", "test"]}}, {"data": {"entries": []}}], 2),
    ({"limit": 2}, [{"data": {"entries": []}}], 0),
    ({"limit": 1}, [{"data": {"entries": ["test", "test"]}}, {"data": {"entries": []}}], 1),
]
...
def test_get_alerts(mocker, demisto_args, results_mocker, expected_results):
    from FireEyeHXv2 import Client, get_alerts
        ...
        [
            {
                ...
                "event_values": {"regKeyEvent/path": "test", "regKeyEvent/valueName": "test", "regKeyEvent/value": "test"},
            },
            {"event_type": "test"},
        ],
...
def test_get_all_alerts_command(mocker, demisto_args, return_mocker, call_args, call_count):
    from FireEyeHXv2 import Client, get_all_alerts_command
...
UPSERT_COMMAND_DATA_BAD_CASES = [
    (("fieldSearchName", "test"), {}, "fieldSearchOperator and fieldSearchValue are required arguments"),
    (("dnsHostname", "test"), {}, "dnsHostnameOperator is required argument"),
]
...
UPSERT_COMMAND_DATA_CASES_ONE_FROM_LIST = [
    (["test1", "test2"], {"test1": 1}, ("test1", 1)),
    (["test1", "test2"], {"test1": 1, "test2": 2}, False),
    (["test1", "test2"], {}, False),
]
...
UPSERT_COMMAND_DATA_CASES_REPORTED_AT = [
    ("1990-06-24T22:16:26.865Z", "1990-06-24T22:16:26.866Z"),
    ("1990-06-24T22:16:26.999Z", "1990-06-24T22:16:27.000Z"),
    ("1990-06-24T22:16:26.004Z", "1990-06-24T22:16:26.005Z"),
    ("1990-06-24T22:16:26.065Z", "1990-06-24T22:16:26.066Z"),
]
...
UPSERT_COMMAND_DATA_CASES_SEARCH_BODY = [
    (("hostsNames", "localhost"), {"get_agent_id": 1, "get_host_set": 0}),
    (("agentsIds", "GfLI00Q4zpidezw9I11rV6"), {"get_agent_id": 0, "get_host_set": 0}),
    (("hostSetName", "Demisto"), {"get_agent_id": 0, "get_host_set": 1}),
    (("hostSet", 1001), {"get_agent_id": 0, "get_host_set": 0}),
]
...
def test_organize_search_body_host(mocker, args, call_count):
    from FireEyeHXv2 import Client, organize_search_body_host

    get_agent_id = mocker.patch("FireEyeHXv2.get_agent_id_by_host_name", return_value=None)
    get_host_set = mocker.patch.object(
        Client, "get_host_set_information_request", return_value={"data": {"entries": [{"_id": "test"}]}}
    )
...
UPSERT_COMMAND_DATA_CASES_GENERAL_CONTEXT_EVENT = [
    ...
    ({"event_type": "test", "event_values": {}}, None),
]
...
UPSERT_COMMAND_DATA_CASES_QUERY_FETCH = [
    ({"reported_at": "1990-06-24T22:16:26.865Z", "first_fetch": "1990-06-23T22:16:26.865Z"}, "1990-06-24T22:16:26.865Z"),
    ({"reported_at": None, "first_fetch": "1990-06-23T22:16:26.865Z"}, "1990-06-23T22:16:26.865Z"),
]
...
def test_fetch_incidents(mocker, demisto_args, alerts_return, expected_call):
    from FireEyeHXv2 import Client, fetch_incidents
...
UPSERT_COMMAND_DATA_CASES_RUN_COMMANDS_WITHOUT_POLLING = [
    ({"cmd": "fireeye-hx-search"}, {"search": 1, "data_acquisition": 0, "file_acquisition": 0}),
    ({"cmd": "fireeye-hx-data-acquisition"}, {"search": 0, "data_acquisition": 1, "file_acquisition": 0}),
    ({"cmd": "fireeye-hx-file-acquisition"}, {"search": 0, "data_acquisition": 0, "file_acquisition": 1}),
]
...
def test_run_commands_without_polling(mocker, demisto_args, call_count):
    from FireEyeHXv2 import Client, run_commands_without_polling
...
def test_delete_indicator_command(mocker, requests_mock, status_code: int, expected_output: str):
    ...
    category = "category"
    from FireEyeHXv2 import Client, delete_indicator_command

    mocker.patch.object(Client, "get_token_request", return_value="")
    request = requests_mock.delete(f"{base_url}/indicators/{category}/{indicator_name}", status_code=status_code, json={})
...
@pytest.mark.parametrize(
    "status_code,expected_output_prefix", ((204, "Successfully deleted"), (404, "Failed deleting"), (418, "Failed deleting"))
)
def test_delete_indicator_condition_command(mocker, requests_mock, status_code: int, expected_output_prefix: str):
    ...
    client = Client(base_url)
    command_result = delete_condition_command(
        client, {"indicator_name": indicator_name, "category": category, "type": indicator_type, "condition_id": condition_id}
    )
...
@pytest.mark.parametrize(
    "file_name,status_code", (("list_indicators_success.json", 200), ("list_indicators_unprocessable.json", 402))
)
def test_list_indicator_categories_command(mocker, requests_mock, file_name: str, status_code: int):
    ...
    from FireEyeHXv2 import Client, list_indicator_categories_command
...
def test_delete_host_set_command(mocker):
    ...
    """
    from FireEyeHXv2 import Client, delete_host_set_command
...
def test_create_static_host_set_command(mocker):
    ...
    Then:
        - ensure the command ran successfully
    """
    from FireEyeHXv2 import Client, create_static_host_set_command

    base_url = "https://example.com"
    args = {"host_set_name": "host_set_name", "hosts_ids": "hosts_ids"}

    mocker.patch.object(Client, "get_token_request", return_value="")
    mocker.patch.object(
        Client,
        "create_static_host_set_request",
        return_value={"data": {"_id": "host_set_id", "_revision": "20220719071022107807465576"}},
    )
...
def test_create_dynamic_host_set_command(mocker):
    ...
    Then:
        - ensure the command ran successfully
    """
    from FireEyeHXv2 import Client, create_dynamic_host_set_command

    base_url = "https://example.com"
    args = {"host_set_name": "host_set_name", "query": "query"}

    mocker.patch.object(Client, "get_token_request", return_value="")
    mocker.patch.object(
        Client,
        "create_dynamic_host_set_request",
        return_value={"data": {"_id": "host_set_id", "_revision": "20220719071022107807465576"}},
    )
...
def test_update_static_host_set_command(mocker):
    ...
    Then:
        - ensure the command ran successfully
    """
    from FireEyeHXv2 import Client, update_static_host_set_command
    ...
    mocker.patch.object(
        Client,
        "update_static_host_set_request",
        return_value={"data": {"_id": "host_set_id", "_revision": "20220719071022107807465576"}},
    )
...
def test_update_dynamic_host_set_command(mocker):
    ...
    Then:
        - ensure the command ran successfully
    """
    from FireEyeHXv2 import Client, update_dynamic_host_set_command

    base_url = "https://example.com"
    args = {"host_set_name": "host_set_name", "host_set_id": "host_set_id", "query": "query"}

    mocker.patch.object(Client, "get_token_request", return_value="")
    mocker.patch.object(
        Client,
        "update_dynamic_host_set_request",
        return_value={"data": {"_id": "host_set_id", "_revision": "20220719071022107807465576"}},
    )
...
def test_create_dynamic_host_request_body(mocker):
    ...
    assert result == {
        "name": "host_set_name",
        "query": {"key": "query_key", "operator": "query_operator", "value": "query_value"},
    }
...
def test_create_static_host_request_body(mocker):
    ...
    assert result == {
        "changes": [{"add": "host_ids_to_add", "command": "change", "remove": "host_ids_to_remove"}],
        "name": "host_set_name",
    }
...
@pytest.mark.parametrize(
    ...
    [
        ("<dummy_url>/hx", "The base URL is invalid please set the base URL without including /hx"),
        ("<dummy_url>/hx/", "The base URL is invalid please set the base URL without including /hx"),
        ("<dummy_url>/hx/api", "The base URL is invalid please set the base URL without including /hx/api"),
        ("<dummy_url>/hx/api/", "The base URL is invalid please set the base URL without including /hx/api"),
        ("<dummy_url>/hx/api/v3", "The base URL is invalid please set the base URL without including /hx/api/v3"),
        ("<dummy_url>/hx/api/v3/", "The base URL is invalid please set the base URL without including /hx/api/v3"),
    ],
)
