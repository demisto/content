...
DEFAULT_HEADERS_POST_REQUEST = {"accept": "application/json;charset=UTF-8", "Content-Type": "application/json;charset=UTF-8"}
...
def get_deceptive_users_command(client: Client, args: dict) -> Tuple:
    ...
    outputs = {"Illusive.DeceptiveUser(val.userName == obj.userName)": result}
    return (
        readable_output,
        outputs,
...
def get_deceptive_servers_command(client: Client, args: dict) -> Tuple:
    ...
    outputs = {"Illusive.DeceptiveServer(val.host == obj.host)": result}
    return (
        readable_output,
        outputs,
        result,  # raw response - the original response
...
def add_deceptive_users_command(client: Client, args: dict) -> Tuple:
    ...
    request_body = [{"domainName": domain_name, "password": password, "policyNames": policy_names, "username": user_name}]
    try:
...
def assign_host_to_policy_command(client: Client, args: dict) -> Tuple:
    ...
    for host in host_names:
        result.append({"isAssigned": True, "hosts": host, "policy_name": policy_name})

    readable_output = tableToMarkdown("Illusive Assign Machines to Policy Succeeded", result)
    outputs = {"Illusive.DeceptionPolicy.isAssigned(val.hosts == obj.hosts)": result}
    return (
        readable_output,
        outputs,
        result,  # raw response - the original response
...
def remove_host_from_policy_command(client: Client, args: dict) -> Tuple:
    ...
    result = []
    for host in host_names:
        result.append({"isAssigned": False, "hosts": host, "policy_name": ""})
...
def get_asm_host_insight_command(client: Client, args: dict) -> Tuple:
    ...
    outputs = {"Illusive.AttackSurfaceInsightsHost(val.ipAddresses == obj.ipAddresses)": result}
...
def get_asm_cj_insight_command(client: Client, args: dict) -> Tuple:
    ...
    outputs = {"Illusive.AttackSurfaceInsightsCrownJewel(val.hostname == obj.hostname)": result}
...
def run_forensics_on_demand_command(client: Client, args: dict) -> Tuple:
    ...
    outputs = {"Illusive.Event(val.eventId == obj.eventId)": result}
    return (
        readable_output,
        outputs,
        result,  # raw response - the original response
...
def is_deceptive_user_command(client: Client, args: dict) -> Tuple:
    ...
    result = {"Username": username, "IsDeceptiveUser": is_deceptive_user}
    readable_output = tableToMarkdown("Illusive Is Deceptive", result)
    outputs = {"Illusive.IsDeceptive(val.Username == obj.Username)": result}
    return (
        readable_output,
        outputs,
...
def is_deceptive_server_command(client: Client, args: dict) -> Tuple:
    ...
    result = {"Hostname": hostname, "IsDeceptiveServer": is_deceptive_server}
    readable_output = tableToMarkdown("Illusive Is Deceptive", result)
    outputs = {"Illusive.IsDeceptive(val.Hostname == obj.Hostname)": result}
    return (
        readable_output,
        outputs,
        None,  # raw response - the original response
...
def get_incidents_command(client: Client, args: dict) -> Tuple:
    ...
    outputs = {"Illusive.Incident(val.incidentId == obj.incidentId)": incident}
    return (
        readable_output,
        outputs,
...
def get_event_incident_id_command(client: Client, args: dict) -> Tuple:
    ...
    try:
        ...
        if "404" in e.args[0]:
            ...
            raise DemistoException("{}".format(e.args[0]))
    result = [{"eventId": event_id, "incidentId": incident, "status": status}]
    outputs = {"Illusive.Event(val.eventId == obj.eventId)": result}
...
def get_incident_events_command(client: Client, args: dict) -> Tuple:
    ...
    readable_output = tableToMarkdown(
        "Illusive get incident's events", events, metadata="Number of events {}".format(len(events))
    )
...
def link_forensics_artifacts_name_command(file_names, client: Client, args: dict) -> CommandResults:
    ...
    if len(file_names) > 0:
        outputs = {"eventId": int(event_id), "Artifacts": file_names}

        return CommandResults(outputs_prefix="Illusive.Event", outputs_key_field="eventId", outputs=outputs)
    else:
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            next_run, incidents = fetch_incidents(
                client=client, last_run=demisto.getLastRun(), first_fetch_time=first_fetch_time, has_forensics=has_forensics
            )
