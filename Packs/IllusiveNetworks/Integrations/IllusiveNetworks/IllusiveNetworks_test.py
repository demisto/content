from IllusiveNetworks import (
    ...
    get_forensics_artifacts_command,
)
...
def test_delete_deceptive_users_command(requests_mock):
    ...
    requests_mock.delete(
        "https://server/api/v1/deceptive-entities/users?deceptive_users=user1&deceptive_users=user2", json=mock_response
    )

    client = Client(base_url="https://server", verify=False)

    args = {"deceptive_users": ["user1", "user2"]}
...
def test_get_event_incident_id_command(requests_mock):
    ...
    assert outputs == {
        "Illusive.Event(val.eventId == obj.eventId)": [
            {"eventId": 1234, "incidentId": {"EventId": "1234", "IncidentId": "1"}, "status": "Done"}
        ]
    }
...
def test_get_asm_host_insight_command(requests_mock):
    ...
    requests_mock.get(
        "https://server/api/v1/attack-surface/machine-insights?hostNameOrIp=myIp", json=mock_response, status_code=202
    )

    client = Client(base_url="https://server", verify=False)

    args = {"hostnameOrIp": "myIp"}
    _, outputs, _ = get_asm_host_insight_command(client, args)

    assert outputs == {
        "Illusive.AttackSurfaceInsightsHost(val.ipAddresses == obj.ipAddresses)": {
            "hostname": "aaa",
            "domainName": "bbb",
            "ipAddresses": "1.1.1.1",
        }
...
def test_get_asm_cj_insight_command(requests_mock):
    ...
    assert outputs == {
        "Illusive.AttackSurfaceInsightsCrownJewel(val.hostname == obj.hostname)": {
            "data": [],
            "hostname": "bbb",
            "machineTagAndSubTags": {"tag": "tag", "subTag": "sub"},
        }
    }
...
def test_get_deceptive_users_command(requests_mock):
    ...
    assert outputs == {
        "Illusive.DeceptiveUser(val.userName == obj.userName)": {
            "data": [],
            "hostname": "bbb",
            "machineTagAndSubTags": {"tag": "tag", "subTag": "sub"},
        }
...
def test_get_deceptive_servers_command(requests_mock):
    mock_response = {"data": [], "hostname": "bbb", "machineTagAndSubTags": {"tag": "tag", "subTag": "sub"}}
    requests_mock.get("https://server/api/v1/deceptive-entities/servers?deceptive_server_type=SUGGESTED", json=mock_response)
    ...
    assert outputs == {
        "Illusive.DeceptiveServer(val.host == obj.host)": {
            "data": [],
            "hostname": "bbb",
            "machineTagAndSubTags": {"tag": "tag", "subTag": "sub"},
        }
...
def test_get_forensics_timeline_command(requests_mock):
    ...
    start_date = "1 month"
    end_date = "3 days"
    args = {"incident_id": "3", "start_date": start_date, "end_date": end_date}
    ...
    assert outputs == {
        "Illusive.Forensics(val.IncidentId == obj.IncidentId)": {
            "IncidentId": "3",
            "Status": "Done",
            "Evidence": [{"IncidentId": "aaa", "Status": "Done", "details": {"date": "aaa"}, "date": "aaa"}],
        }
    }
...
def test_remove_host_from_policy_command(requests_mock):
    ...
    client = Client(base_url="https://server", verify=False)
    args = {"hosts": ["aaa@domain.com"]}
    _, outputs, _ = remove_host_from_policy_command(client, args)

    assert outputs == {
        "Illusive.DeceptionPolicy.isAssigned(val.hosts == obj.hosts)": [
...
def test_assign_host_to_policy_command(requests_mock):
    ...
    client = Client(base_url="https://server", verify=False)
    args = {"policy_name": "myPolicy", "hosts": ["aaa@domain.com"]}
...
def test_add_deceptive_users_command(requests_mock):
    ...
    args = {"password": "aaa", "username": user_name, "domain_name": domain_name, "policy_names": ["myPolicy"]}
...
def test_add_deceptive_servers_command(requests_mock):
    ...
    args = {"service_types": ["FTP", "SSH"], "host": user_name}

    _, outputs, _ = add_deceptive_servers_command(client, args)

    assert outputs == {
        "Illusive.DeceptiveServer(val.host == obj.host)": {
            "host": "aaa.illusive.com",
            "serviceTypes": ["FTP", "SSH"],
...
def test_get_incident_command(requests_mock):
    ...
    client = Client(base_url="https://server", verify=False)
    args = {"incident_id": "13", "start_date": "3 days"}
    ...
    mock_response = [
        {
            ...
            "hasForensics": True,
            "incidentTypes": "MACHINE",
            "incidentTimeUTC": "2020-04-21T15:39:32.954Z",
        },
        {
            "deceptionFamilies": [],
            "incidentId": "4321",
            "hasForensics": False,
            "incidentTypes": "MACHINE",
            "incidentTimeUTC": "2020-04-21T14:53:54.234Z",
        },
    ]
    first_fetch_time = "7 days"
    requests_mock.get(
        "https://server/api/v1/incidents?limit=10&offset=0&start_date=2018-10-24T14:13:20+00:000Z", json=mock_response
    )
...
def test_fetch_incidents_first_fetch(requests_mock):
    ...
    requests_mock.get(f"https://server/api/v1/incidents?limit=10&offset=0&start_date={last_fetch}", json=mock_response)
...
def test_get_forensics_analyzers_command(requests_mock):
    mock_response1 = [{"analyzerName": "bbb", "analyzerValue": "1234"}, {"analyzerName": "aaa", "analyzerValue": "4321"}]
    mock_response2 = 3
...
def test_get_forensics_artifacts_command(requests_mock):
    ...
    requests_mock.get("https://server/api/v1/incidents/id?event_id=3", json=mock_response2)
    requests_mock.get(
        "https://server/api/v1/forensics/artifacts?event_id=3&artifacts_type=DESKTOP_SCREENSHOT", content=mock_response
    )

    client = Client(base_url="https://server", verify=False)
    args = {"event_id": "3", "artifact_type": "DESKTOP_SCREENSHOT"}
