...
class Client(BaseClient):
    ...
    def do_request(self, method, url_suffix, json_data=None, params=None, data=None, resp_type="json", headers=None):
        ...
        if res.status_code == 403:
            self.update_session()
            res = self._http_request(
                method, url_suffix, headers=headers, json_data=json_data, params=params, data=data, ok_codes=(200, 400, 404)
            )
            return res
...
def get_event_header(event_type):
    if event_type == "combined":
        ...
        headers = ["ID", "Type", "File", "Timestamp", "Domain", "ProcessTableID", "ProcessID", "ProcessName", "Username"]
...
def get_event_item(raw_event, event_type):
    ...
    if event_type == "security":
        event["Property"] = [{k.title(): v for k, v in prop.items()} for prop in raw_event.get("properties")]
...
def get_local_snapshot_items(raw_snapshots, limit, offset, conn_name):
    ...
    for key in snapshot_keys[from_idx:to_idx]:
        snapshots.append({"ConnectionName": conn_name, "FileName": key, "Deleted": False})
...
def alarm_to_incident(client, alarm):
    ...
    return {"name": f"{host} found {intel_doc}", "occurred": alarm.get("alertedAt"), "rawJSON": json.dumps(alarm)}
...
def get_intel_doc(client, data_args):
    ...
    headers = ["ID", "Name", "Description", "Type", "AlertCount", "UnresolvedAlertCount", "CreatedAt", "UpdatedAt", "LabelIds"]
...
def alert_update_state(client, data_args):
    ...
    human_readable = tableToMarkdown(
        f"Alert state updated to {state}", alert, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def get_snapshots(client, data_args):
    ...
    human_readable = tableToMarkdown(
        f"Snapshots for connection {conn_name}", snapshots, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def delete_snapshot(client, data_args):
    ...
    context = {"ConnectionName": con_name, "ID": snapshot_id, "Deleted": True}
    outputs = {"Tanium.Snapshot(val.ID === obj.ID && val.ConnectionName === obj.ConnectionName)": context}
...
def get_local_snapshots(client, data_args):
    ...
    context = createContext(snapshots, removeNull=True)
    outputs = {"Tanium.LocalSnapshot(val.FileName === obj.FileName && val.ConnectionName === obj.ConnectionName)": context}
    headers = ["FileName", "ConnectionName"]
    human_readable = tableToMarkdown(
        f"Local snapshots for connection {conn_name}", snapshots, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def delete_local_snapshot(client, data_args):
    ...
    context = {"FileName": file_name, "Deleted": True}
...
def get_connections(client, data_args):
    ...
    human_readable = tableToMarkdown("Connections", connections, headers=headers, headerTransform=pascalToSpace, removeNull=True)
    return human_readable, outputs, raw_response
...
def get_connection(client, data_args):
    ...
    headers = ["Name", "State", "Remote", "CreateTime", "DST", "DestinationType", "OSName"]
    human_readable = tableToMarkdown(
        "Connection information", connection, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
...
def create_connection(client, data_args):
    ...
    body = {"remote": remote, "dst": dst, "dstType": dst_type, "connTimeout": conn_timeout}
...
def delete_connection(client, data_args):
    ...
    context = {"Name": conn_name, "Deleted": True}
    outputs = {"Tanium.Connection(val.Name && val.Name === obj.Name)": context}
...
def get_label(client, data_args):
    ...
    outputs = {"Tanium.Label(val.ID && val.ID === obj.ID)": context}
    headers = ["Name", "Description", "ID", "IndicatorCount", "SignalCount", "CreatedAt", "UpdatedAt"]
    human_readable = tableToMarkdown("Label information", label, headers=headers, headerTransform=pascalToSpace, removeNull=True)
    return human_readable, outputs, raw_response
...
def get_file_downloads(client, data_args):
    ...
    human_readable = tableToMarkdown("File downloads", files, headers=headers, headerTransform=pascalToSpace, removeNull=True)
    return human_readable, outputs, raw_response


def get_downloaded_file(client, data_args):
    file_id = data_args.get("file-id")
    file_content, content_desc = client.do_request("GET", f"/plugin/products/trace/filedownloads/{file_id}", resp_type="content")
...
def get_events_by_connection(client, data_args):
    ...
    params = {"limit": limit, "offset": offset, "sort": sort, "fields": fields, "match": match}
    ...
    raw_response = client.do_request("GET", f"/plugin/products/trace/conns/{connection}/{event_type}/events/", params=params)
    ...
    human_readable = tableToMarkdown(
        f"Events for {connection}", events, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def get_file_download_info(client, data_args):
    ...
    human_readable = tableToMarkdown(
        f'File download metadata for file `{file["Path"]}`', file, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def get_process_info(client, data_args):
    ...
    human_readable = tableToMarkdown(
        f"{PROCESS_TEXT} {ptid}", process, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def get_events_by_process(client, data_args):
    ...
    raw_response = client.do_request(
        "GET", f"/plugin/products/trace/conns/{conn_name}/processevents/{ptid}", params={"limit": limit, "offset": offset}
    )
    ...
    human_readable = tableToMarkdown(
        f"Events for process {ptid}", events, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def get_parent_process_tree(client, data_args):
    ...
    if children_item:
        ...
        human_readable = tableToMarkdown(
            f"{PARENT_PROCESS_TEXT} {ptid}", process_tree, headers=headers, headerTransform=pascalToSpace, removeNull=True
        )
        human_readable += tableToMarkdown(
            "Processes with the same parent", children_item, headers=headers, headerTransform=pascalToSpace, removeNull=True
        )
    else:
        human_readable = tableToMarkdown(
            f"{PARENT_PROCESS_TEXT} {ptid}", readable_output, headers=headers, headerTransform=pascalToSpace, removeNull=True
        )
...
def get_process_tree(client, data_args):
    ...
    if children_item:
        ...
        del process_tree["Children"]
        human_readable = tableToMarkdown(
            f"Process information for process with PTID {ptid}",
            process_tree,
            headers=headers,
            headerTransform=pascalToSpace,
            removeNull=True,
        ...
        human_readable += tableToMarkdown(
            f"{PROCESS_CHILDREN_TEXT} {ptid}", children_item, headers=headers, headerTransform=pascalToSpace, removeNull=True
        )
    else:
        human_readable = tableToMarkdown(
            f"{PROCESS_TEXT} {ptid}", readable_output, headers=headers, headerTransform=pascalToSpace, removeNull=True
        )
...
def list_evidence(client, data_args):
    ...
    human_readable = tableToMarkdown("Evidence list", evidences, headers=headers, headerTransform=pascalToSpace, removeNull=True)
    return human_readable, outputs, raw_response
...
def get_evidence(client, data_args):
    ...
    human_readable = tableToMarkdown(
        "Label information", evidence, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def delete_evidence(client, data_args):
    ...
    context = {"ID": int(evidence_id), "Deleted": True}
...
def request_file_download(client, data_args):
    ...
    context = {"ConnectionName": con_name, "Path": path, "Downloaded": datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S.%f")}
    outputs = {"Tanium.FileDownload(val.Path === obj.Path && val.ConnectionName === obj.ConnectionName)": context}

    data = {"path": path, "connId": con_name}
...
def get_file_download_request_status(client, data_args):
    ...
    file_download_request = {"ID": file_id, "ConnectionName": host, "Path": path, "Status": status, "Downloaded": downloaded}

    context = createContext(file_download_request, removeNull=True)
    outputs = {"Tanium.FileDownload(val.Path === obj.Path && val.ConnectionName === obj.ConnectionName)": context}
    headers = ["ID", "ConnectionName", "Status", "Path", "Downloaded"]
    human_readable = tableToMarkdown(
        "File download request status", file_download_request, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def delete_file_download(client, data_args):
    ...
    context = {"ID": int(file_id), "Deleted": True}
    outputs = {"Tanium.FileDownload(val.ID && val.ID === obj.ID)": context}
...
def get_file_info(client, data_args):
    ...
    human_readable = tableToMarkdown(
        f"Information for file `{path_name}`", file_info, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def delete_file_from_endpoint(client, data_args):
    ...
    context = {"Path": data_args.get("path").replace("\\", "/"), "ConnectionName": con_name, "Deleted": True}
    outputs = {"Tanium.File(val.Path === obj.Path && val.ConnectionName === obj.ConnectionName)": context}
...
def get_process_timeline(client, data_args):
    ...
    human_readable = tableToMarkdown(
        f"Timeline data for process with PTID `{ptid}`", timeline, headers=headers, headerTransform=pascalToSpace, removeNull=True
    )
    return human_readable, outputs, raw_response
...
def intel_doc_create(client, data_args):
    ...
    raw_response = client.do_request(
        ...
        headers={"Content-Disposition": f"filename=file.{file_extension}", "Content-Type": "application/xml"},
        data=file_content,
    )
    intel_doc = get_intel_doc_item(raw_response)
