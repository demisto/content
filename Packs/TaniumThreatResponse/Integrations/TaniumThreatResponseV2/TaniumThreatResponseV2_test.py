...
def test_get_intel_docs_labels_list(requests_mock):
    ...
    human_readable, outputs, raw_response = TaniumThreatResponseV2.get_intel_docs_labels_list(
...
def test_add_intel_docs_label(requests_mock):
    ...
    human_readable, outputs, raw_response = TaniumThreatResponseV2.add_intel_docs_label(
...
def test_remove_intel_docs_label(requests_mock):
    ...
    human_readable, outputs, raw_response = TaniumThreatResponseV2.remove_intel_docs_label(
        MOCK_CLIENT, {"intel_doc_id": intel_doc_id, "label_id": label_id}
    )
...
def test_create_intel_doc(mocker, requests_mock):
    ...
    mocker.patch("TaniumThreatResponseV2.get_file_data", return_value=("test_name", "test_data/test.ioc", file_content))
    requests_mock.post(BASE_URL + "/api/v2/session/login", json={"data": {"session": "session-id"}})
    ...
    human_readable, outputs, raw_response = TaniumThreatResponseV2.create_intel_doc(
...
def test_update_intel_doc_ioc(mocker, requests_mock):
    ...
    human_readable, outputs, raw_response = TaniumThreatResponseV2.update_intel_doc(
...
def test_update_intel_doc_yara(mocker, requests_mock):
    ...
    api_update_expected_response = util_load_json("test_data/update_intel_docs_raw_response.json")
    api_get_expected_response = util_load_json("test_data/get_intel_doc_raw_response.json")
    mocker.patch("TaniumThreatResponseV2.get_file_data", return_value=("test_name", "test_data/test.yara", file_content))
    requests_mock.post(BASE_URL + "/api/v2/session/login", json={"data": {"session": "session-id"}})
    requests_mock.get(BASE_URL + "/plugin/products/detect3/api/v1/intels/423", json=api_get_expected_response)
    requests_mock.get(BASE_URL + "/plugin/products/threat-response/api/v1/intels/423", json={"data": api_get_expected_response})
    requests_mock.put(
...
def test_start_quick_scan(mocker, requests_mock):
    ...
    human_readable, outputs, raw_response = TaniumThreatResponseV2.start_quick_scan(
...
def test_filter_get_connections(requests_mock, command_args, expected_output_len):
    ...
    _, outputs, _ = TaniumThreatResponseV2.get_connections(client=MOCK_CLIENT, command_args=command_args)
...
def test_filter_get_system_status(requests_mock, command_args, expected_output_len):
    ...
    _, outputs, _ = TaniumThreatResponseV2.get_system_status(MOCK_CLIENT, command_args)
...
def test_fetch_new_incidents(requests_mock):
    ...
    assert next_run.get("time") == datetime.strftime(parse("2021-09-26T14:04:59.000Z"), TaniumThreatResponseV2.DATE_FORMAT)
