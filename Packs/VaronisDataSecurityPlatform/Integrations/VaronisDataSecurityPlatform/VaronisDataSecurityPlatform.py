...
class Client(BaseClient):
    ...
    def varonis_get_users(self, search_string: str) -> list[Any]:
        ...
        response = self._http_request("GET", "api/userdata/users", params=request_params)
    ...
    def varonis_update_alert_status(self, query: dict[str, Any]) -> bool:
        ...
        """
        return self._http_request("POST", "/api/alert/alert/SetStatusToAlerts", json_data=query)

    def varonis_get_alerted_events(
        self, alerts: list[str], count: int, page: int, descending_order: bool
    ) -> list[dict[str, Any]]:
        """Get alerted events
        ...
        return self._http_request("GET", "/api/alert/alert/GetAlertedEvents", params=request_params)

    def varonis_get_alerts(
        self,
        threat_models: Optional[list[str]],
        start_time: Optional[datetime],
        end_time: Optional[datetime],
        device_names: Optional[list[str]],
        last_days: Optional[int],
        sid_ids: Optional[list[int]],
        from_alert_id: Optional[int],
        ...
        return self._http_request("GET", "/api/alert/alert/GetAlerts", params=request_params)
...
def get_sids(client: Client, values: list[str], user_domain_name: Optional[str], key: str) -> list[int]:
    ...
    for value in values:
        ...
        for user in users:
            if strEqual(user[key], value) and (not user_domain_name or strEqual(user["DomainName"], user_domain_name)):
...
def varonis_update_alert(client: Client, close_reason_id: int, status_id: int, alert_ids: list) -> bool:
    ...
    query: dict[str, Any] = {"AlertGuids": alert_ids, "closeReasonId": close_reason_id, "statusId": status_id}
...
def fetch_incidents(
    ...
    alerts = client.varonis_get_alerts(
        threat_models, first_fetch_time, None, None, None, None, last_fetched_id, statuses, severities, True, max_results, 1, True
    )
...
def varonis_get_alerts_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    max_results = try_convert(
        max_results, lambda x: int(x), ValueError(f"max_results should be integer, but it is {max_results}.")
    )
    ...
    page = try_convert(page, lambda x: int(x), ValueError(f"page should be integer, but it is {page}."))

    sid_ids = (
        get_sids_by_email(client, emails)
        + get_sids_by_sam(client, sam_account_names)
    ...
    readable_output = tableToMarkdown(
        "Varonis Alerts", alerts, headers=["Name", "Severity", "Time", "Category", "UserName", "Status"]
    )

    return CommandResults(
        readable_output=readable_output, outputs_prefix="Varonis", outputs_key_field="Varonis.Alert.ID", outputs=outputs
    )
...
def varonis_get_alerted_events_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="Varonis", outputs_key_field="Varonis.Event.ID", outputs=outputs
    )
