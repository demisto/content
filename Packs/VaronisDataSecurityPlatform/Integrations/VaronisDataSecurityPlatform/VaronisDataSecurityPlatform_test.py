...
def test_varonis_get_alerts_command(mocker: MockerFixture):
    ...
    mocker.patch.object(
        client, "varonis_get_alerts", return_value=util_load_json("test_data/varonis_get_alerts_api_response.json")
    )
    mocker.patch.object(client, "varonis_get_enum", return_value=util_load_json("test_data/varonis_get_enum_response.json"))
    mocker.patch.object(client, "varonis_get_users", return_value=util_load_json("test_data/varonis_get_users_api_response.json"))
...
def test_varonis_get_alerted_events_command(mocker: MockerFixture):
    ...
    client = Client(base_url="https://test.com", verify=False, proxy=False)
    mocker.patch.object(
        client, "varonis_get_alerted_events", return_value=util_load_json("test_data/varonis_get_alerted_events_response.json")
    )
...
def test_get_sids_user(mocker: MockerFixture):
    from VaronisDataSecurityPlatform import DISPLAY_NAME_KEY, NON_EXISTENT_SID, get_sids
...
def test_convert_to_demisto_severity():
    from VaronisDataSecurityPlatform import IncidentSeverity, convert_to_demisto_severity
...
def test_varonis_get_auth_url(requests_mock: MockerFixture):
    ...
    requests_mock.get("https://test.com/auth/configuration", json=fetch_output)
