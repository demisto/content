...
class Client(BaseClient):
    ...
    def create_login(self):
        ...
        res = self._http_request(method="POST", url_suffix=uri, params=params, headers=self.headers)
    ...
    def get_user_by_id(self, user_id: str) -> Optional["IAMUserAppData"]:
        ...
        res = self._http_request(method="GET", url_suffix=uri, headers=self.headers)
        if res:
    ...
    def get_user(self, filter_name: str, filter_value: str) -> Optional["IAMUserAppData"]:
        ...
        res = self._http_request(method="GET", url_suffix=uri, params=params, headers=self.headers)
    ...
    def create_user(self, user_data: Dict[str, Any]) -> "IAMUserAppData":
        ...
        res = self._http_request(method="POST", url_suffix=uri, json_data=user_data, headers=self.headers)
        user_id = res.get("id")
        email = res.get("Work_Email__c")
        username = res.get("Name")
        is_active = True
...
def main():
    ...
    secret_token = params.get("secret_token_creds", {}).get("password") or params.get("secret_token")
