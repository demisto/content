...
class Client(BaseClient):
    ...
    def security_events_list(self, service: str, event_type: str, start=None, end=None, limit=None) -> dict:
        ...
        result = self._http_request(method="GET", url_suffix="siem/security_events", params=params)
        return result
    ...
    def email_sweep(
        ...
        result = self._http_request(method="GET", url_suffix="sweeping/mails", params=params)
        return result
    ...
    def user_take_action(self, action_type: str, account_list: list) -> dict:
        ...
        result = self._http_request(method="POST", url_suffix="mitigation/accounts", json_data=data)
        return result

    def email_take_action(
        self, action_type: str, mailbox: str, mail_message_id: str, mail_unique_id: str, mail_message_delivery_time: str
    ) -> dict:
        """
        ...
        result = self._http_request(method="POST", url_suffix="mitigation/mails", json_data=data)
        return result
    ...
    def action_result_query(self, batch_id: str, start: str, end: str, limit: str, action_type: str) -> dict:
        ...
        data = self._http_request(method="GET", url_suffix=f"mitigation/{action_type}", params=params)
        return data
    ...
    def blocked_lists_get(self):
        ...
        """
        result = self._http_request(method="GET", url_suffix="remediation/mails")
        return result
    ...
    def blocked_lists_update(
        ...
        data = {"action_type": action_type, "rules": rules}
        result = self._http_request(method="POST", url_suffix="remediation/mails", json_data=data)
        return result
    ...
    def next_link(self, link: str) -> dict:
        ...
        """
        data = self._http_request(method="GET", full_url=link, url_suffix="")
        return data
...
def parse_date_to_isoformat(arg: str, arg_name: str):
    ...
    if not date:
        return_error(
            f"invalid date value for: {arg_name}\n{arg} should be in the format of:"
        ...
        for event_type in list_event_type:
            ...
            for event in security_events:
                if event.get("log_item_id") not in last_fetch_ids:
                    ...
                    incident_name = (
                        f"{event.get('event')} on {message.get('affected_user')} at"
                        f" {message.get('location')} - {event.get('log_item_id')}"
                    )
                    incident = {"name": incident_name, "occurred": message.get("detection_time"), "rawJSON": json.dumps(event)}
...
def security_events_list_command(client, args):
    ...
    if not security_events:
        ...
        if result.get("next_link"):
            meta_data = {"next_link": result.get("next_link"), "traceId": result.get("traceId")}
            entries.append(
                CommandResults(
...
def main() -> None:
    ...
    URLS = {
        ...
        "Singapore": "https://api.tmcas.trendmicro.com.sg/v1/",
        "Middle East (UAE)": "https://api-mea.tmcas.trendmicro.com/v1/",
    }
    ...
    try:
        client = Client(base_url=base_url, verify=verify_certificate, headers={"Authorization": f"Bearer {token}"}, proxy=proxy)
