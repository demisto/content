...
class Client(BaseClient):
    ...
    def list_hosts_request(self, wstr_filter: Optional[str] = None, fields_to_return: Optional[List[str]] = None) -> Dict:
        response = self._http_request(
            method="POST",
    ...
    def list_groups_request(self, wstr_filter: Optional[str] = None, fields_to_return: Optional[List[str]] = None) -> Dict:
        response = self._http_request(
            method="POST",
            url_suffix="/HostGroup.FindGroups",
            json_data={"wstrFilter": wstr_filter, "lMaxLifeTime": 600, "vecFieldsToReturn": fields_to_return},
        )
        self._raise_for_error(response)
        return response
    ...
    def list_host_software_patches_request(self, hostname: str) -> Dict:
        response = self._http_request(
            method="POST", url_suffix="/InventoryApi.GetHostInvPatches", json_data={"szwHostId": hostname}
        )
        self._raise_for_error(response)
        return response
    ...
    if not outputs:
        ...
        command_results_args = {
            ...
            "readable_output": tableToMarkdown(
                "Hosts List", outputs, ["KLHST_WKS_HOSTNAME", "KLHST_WKS_DN", "KLHST_WKS_OS_NAME", "KLHST_WKS_FQDN"]
            ),
