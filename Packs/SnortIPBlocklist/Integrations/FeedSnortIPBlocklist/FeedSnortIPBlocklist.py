...
class Client(BaseClient):
    ...
    def build_iterator(self) -> list:
        ...
        res = self._http_request("GET", url_suffix="", full_url=self._base_url, resp_type="text", allow_redirects=True)
        ...
        try:
            ...
            for indicator in indicators:
                ...
                if indicator_type := auto_detect_indicator_type(indicator):
                    result.append({"value": indicator, "type": indicator_type, "FeedURL": self._base_url})
...
def get_indicators_command(client: Client, params: dict[str, str], args: dict[str, str]) -> CommandResults:
    ...
    human_readable = tableToMarkdown(
        f"Indicators from Snort IP Blocklist Feed: (first {limit} indicators)",
        indicators,
        headers=["value", "type"],
