...
class Client(BaseClient):
    ...
    def generate_new_access_token(self):
        ...
        return self._http_request(method="POST", url_suffix="oauth", json_data=body)
    ...
    def get_expiration_in_seconds(self, auth_response: dict[str, str]):
        ...
        error_msg = (
            f"HPEArubaClearPass error: Got an invalid access token expiration time from the API: "
    ...
    def prepare_request(self, method: str, params: dict, url_suffix: str, body: dict = {}, resp_type: str = "json"):
        return self._http_request(
            method=method, params=params, url_suffix=url_suffix, headers=self.headers, json_data=body, resp_type=resp_type
        )
...
def update_endpoint_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    request_body = get_endpoint_request_body(status, mac_address, description, device_insight_tags, attributes_values, attributes)
...
def create_new_attribute_body(args: dict[str, Any]):
    ...
    return get_attribute_request_body(
        name,
        entity_name,
        data_type,
        mandatory,
        attribute_default_value,
        allow_multiple_data_type_string,
        allowed_list_data_types_value,
    )


def get_attribute_request_body(
    name,
    entity_name,
    data_type,
    mandatory,
    ...
    new_attribute_body.update({"allow_multiple": allow_multiple_data_type_string}) if allow_multiple_data_type_string else None
    new_attribute_body.update({"allowed_value": allowed_list_data_types_value}) if allowed_list_data_types_value else None
...
def update_attribute_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    new_attribute_body = create_new_attribute_body(args)
    outputs = client.prepare_request(method="PATCH", params={}, url_suffix=f"attribute/{attribute_id}", body=new_attribute_body)
    delete_redundant_data(outputs)
...
def main() -> None:
    ...
    client = Client(proxy=proxy, verify=verify_certificate, base_url=base_url, client_id=client_id, client_secret=client_secret)
