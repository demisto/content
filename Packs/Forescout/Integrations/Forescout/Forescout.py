...
DEX_HEADERS = {"Content-Type": "application/xml", "Accept": "application/xml"}
...
def check_web_api_credentials():
    ...
    if not (WEB_API_USERNAME and WEB_API_PASSWORD):
        err_msg = (
            "Error in Forescout Integration - Web API credentials must"
...
def check_dex_credentials():
    ...
    if not (DEX_USERNAME and DEX_PASSWORD and DEX_ACCOUNT):
        err_msg = (
            "Error in Forescout Integration - Data Exchange (DEX) credentials must"
            " be entered in the Forescout integration configuration in order to execute this command."
        )
        return_error(err_msg)
...
def create_update_hostfields_request_body(
    host_ip: str, update_type: str, field: str, value: str, fields_json: str
) -> ET_PHONE_HOME.Element:
    """
    Create XML request body formatted to DEX expectations
    ...
    if composite_fields:
        for table_prop_name, values in composite_fields.items():
            table_property_xml = ET_PHONE_HOME.SubElement(props_xml, "TABLE_PROPERTY", attrib={"NAME": table_prop_name})
            if update_type == "update":
...
def format_policies_data(data: dict) -> list:
    ...
    for policy in policies:
        ...
        for rule in rules:
            formatted_rule = {"ID": str(rule.get("ruleId")), "Name": rule.get("name"), "Description": rule.get("description")}
...
def create_web_api_headers() -> dict:
    ...
    """
    web_api_login()
    headers = {"Authorization": WEB_AUTH, "Accept": "application/hal+json"}
    return headers
...
def http_request(
    ...
    try:
        ...
        err_msg = (
            "Connection Timeout Error - potential reasons may be that the Server URL parameter"
...
def get_host_command():
    ...
    endpoint = {"IPAddress": host.get("ip", ""), "MACAddress": host.get("mac", "")}
    ...
    context = {"Forescout.Host(val.ID && val.ID === obj.ID)": content, "Endpoint(val.ID && val.ID === obj.ID)": endpoint}
...
def get_hosts_command():
    ...
    content = [
        {"ID": str(x.get("hostId")), "IPAddress": x.get("ip", ""), "MACAddress": x.get("mac", "")}
        for x in response_data.get("hosts", [])
    ]
    endpoints = [{"IPAddress": x.get("ip", ""), "MACAddress": x.get("mac", "")} for x in response_data.get("hosts", [])]
    context = {"Forescout.Host(val.ID && val.ID === obj.ID)": content, "Endpoint(val.ID && val.ID === obj.ID)": endpoints}
...
def update_lists_command():
    ...
    if len(msg_list) >= 1 and msg_list[0] is not None:
        ...
        err_msg = (
            "The response from Forescout could not be parsed correctly. It is uncertain if the list updates "
...
def update_host_fields_command():
    ...
    if len(msg_list) >= 1 and msg_list[0] is not None:
        ...
        err_msg = (
            "The response from Forescout could not be parsed correctly. It is uncertain if the host fields "
...
def main():
    ...
    try:
        ...
        if not ((WEB_API_USERNAME and WEB_API_PASSWORD) or (DEX_USERNAME and DEX_PASSWORD)):
            err_msg = (
                "The username and password for at least one of the 'Data Exchange (DEX)' or the 'Web API' "
        ...
        if (DEX_USERNAME and DEX_PASSWORD) and not DEX_ACCOUNT:
            err_msg = (
                "When entering your 'Data Exchange (DEX)' credentials, the 'Data Exchange (DEX) Account' "
