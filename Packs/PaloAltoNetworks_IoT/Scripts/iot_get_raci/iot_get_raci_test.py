...
_CONFIG = {
    ...
    "alerts": [
        ...
        },
        {
            ...
            "raci": {"r": "INFOSEC", "i": ["IOT_OWNER", "SOC"]},
        },
        {"iot_raw_type": "IoT Vulnerability", "raci": {"r": "IOT_OWNER", "i": ["INFOSEC", "SOC"]}},
    ],
    "groups": {
...
_CONFIG_WITHOUT_DEFAULT = {
    ...
    "alerts": [{"iot_raw_type": "IoT Vulnerability", "raci": {"r": "IOT_OWNER", "i": ["INFOSEC", "SOC"]}}],
    "groups": {},
}
...
def test_iot_get_raci_no_default_email(monkeypatch):
    ...
    outputs = get_raci(
        {"alert_name": "", "raw_type": "IoT Vulnerability", "category": "Audio Streaming", "profile": "Profusion Media Player"}
    ).outputs
    assert outputs == {
        "owner": "IT_AUDIO_VIDEO",
        "r": "IT_AUDIO_VIDEO",
...
def test_iot_get_raci_no_name_regex(monkeypatch):
    ...
    outputs = get_raci({"alert_name": "FooBar", "raw_type": "IoT Vulnerability", "category": "Foo"}).outputs
    assert outputs == {
        "owner": None,
        "r": None,
        "r_email": None,
    ...
    outputs = get_raci(
        {"alert_name": "FooBar", "raw_type": "IoT Vulnerability", "category": "Camera", "profile": "Avigilon Camera"}
    ).outputs
    assert outputs == {
        "owner": "WPR_SECURITY",
        "r": "WPR_SECURITY",
...
def test_iot_get_raci_no_raci(monkeypatch):
    ...
    monkeypatch.setattr(iot_get_raci, "get_iot_config", lambda x: _CONFIG)
    outputs = get_raci(
        {"alert_name": "FooBar", "raw_type": "IoT Alert", "category": "Camera", "profile": "Avigilon Camera"}
    ).outputs
