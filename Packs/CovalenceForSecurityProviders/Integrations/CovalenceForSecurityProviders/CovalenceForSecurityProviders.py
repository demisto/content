...
def list_alerts(
    ...
    keys = ["acknowledgedStatus", "analystDescription", "analystTitle", "destIp", "sourceIp", "subType", "title", "type"]
...
def get_health():
    if BROKER:
        ...
        orgs = list_org()
        for org in orgs:
            health_check_resp.append(send_request("GET", "/rest/v1/health", target_org=org["org_name"]))
...
def list_sensors():
    ...
    details = argToBoolean(demisto.args().get("details", "false"))
    keys = ["isAuthorized", "isNetflowGenerator", "name"]
...
def list_dns_resolutions():
    ...
    details = argToBoolean(demisto.args().get("details", "false"))
    keys = ["domainName", "requestOriginIp", "requestTime", "resolvedIp"]
...
def set_internal_networks():
    ...
    networks = []
    networks.append({"cidr": cidr, "notes": notes})
...
def list_endpoint_agents():
    ...
    details = argToBoolean(demisto.args().get("details", "false"))
    keys = ["hardwareVendor", "hostName", "ipAddress", "isConnected", "lastSessionUser", "operatingSystem", "serialNumber"]
...
def search_endpoint_process():
    ...
    details = argToBoolean(demisto.args().get("details", "false"))
    keys = ["commandLine", "firstSeenTime", "lastSeenTime", "processPath", "username"]
...
def search_endpoint_installed_software():
    ...
    details = argToBoolean(demisto.args().get("details", "false"))
    keys = ["installTimestamp", "name", "uninstallTimestamp", "vendor", "version"]
    ...
    try:
        if demisto.command() == "test-module":
            ...
            results = CommandResults(
                outputs_prefix="Covalence.Alert", outputs_key_field="id", outputs=r, readable_output=readable_output
            )
            ...
            results = CommandResults(
                outputs_prefix="Covalence.Sensors", outputs_key_field="id", outputs=r, readable_output=readable_output
            )
            return_results(results)
            ...
            results = CommandResults(
                outputs_prefix="Covalence.Sensor", outputs_key_field="id", outputs=r, readable_output=readable_output
            )
            return_results(results)
            ...
            results = CommandResults(
                outputs_prefix="Covalence.Connections", outputs_key_field="id", outputs=r, readable_output=readable_output
            )
            return_results(results)
            ...
            results = CommandResults(
                outputs_prefix="Covalence.DNSResolutions", outputs_key_field="id", outputs=r, readable_output=readable_output
            )
            return_results(results)
            ...
            results = CommandResults(
                outputs_prefix="Covalence.InternalNetworks", outputs_key_field="cidr", outputs=r, readable_output=readable_output
            )
            return_results(results)
            ...
            results = CommandResults(
                outputs_prefix="Covalence.EndpointProcess", outputs_key_field="id", outputs=r, readable_output=readable_output
            )
            return_results(results)
            ...
            results = CommandResults(
                outputs_prefix="Covalence.EndpointSoftware", outputs_key_field="id", outputs=r, readable_output=readable_output
            )
            return_results(results)
