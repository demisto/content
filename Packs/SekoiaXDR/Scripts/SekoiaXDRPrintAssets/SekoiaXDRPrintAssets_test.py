...
def test_assets_info(mocker):
    ...
    mocker.patch.object(demisto, "executeCommand", return_value=[{"Type": 3, "Contents": asset}])
...
def test_assets_ids(mocker):
    ...
    mocker.patch.object(demisto, "executeCommand", return_value=[{"Type": 3, "Contents": alert_infos}])
    mocker.patch.object(SekoiaXDRPrintAssets, "get_assets_info", return_value=assets_infos)
