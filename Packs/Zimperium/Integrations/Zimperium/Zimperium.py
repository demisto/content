...
class Client(BaseClient):
    ...
    def devices_search_request(self, query: str, size: str, page: str) -> dict:
        ...
        return self._http_request(method="GET", url_suffix="/devices/public/search", headers=self._headers, params=params)
    ...
    def devices_get_last_updated_request(self, last_updated: str, exclude_deleted: bool, size: str, page: str) -> dict:
        ...
        return self._http_request(method="GET", url_suffix="/devices/public/device_updates", headers=self._headers, params=params)
    ...
    def report_get_request(self, bundle_id: str, itunes_id: str, app_hash: str, platform: str) -> dict:
        ...
        return self._http_request(method="GET", url_suffix=url_suffix, headers=self._headers, params=params)
    ...
    def app_upload_for_analysis_request(self, entry_id: str) -> dict:
        ...
        file_name = os.path.basename(demisto.getFilePath(entry_id).get("name"))
        ...
        try:
            with open(file_path, "rb") as file:
                ...
                result = self._http_request(
                    method="POST",
                    url_suffix="/malware/public/upload/app",
    ...
    def events_search_request(self, query: str, size: str, page: str, verbose: bool):
        ...
        return self._http_request(method="GET", url_suffix="/events/public/search", headers=self._headers, params=params)
...
def users_search(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        name=f"Number of users found: {total_elements}. {table_name}", t=users_data, headers=headers, removeNull=True
    )
...
def devices_search(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        name=f"Number of devices found: {total_elements}. {table_name}", t=devices_data, headers=headers, removeNull=True
    )
...
def file_reputation(client: Client, args: dict) -> List[CommandResults]:
    ...
    for app_hash in hash_list:
        ...
        if hash_type == "md5":
            file = Common.File(md5=app_hash, dbot_score=dbot_score)
        elif hash_type == "sha1":
            file = Common.File(sha1=app_hash, dbot_score=dbot_score)
        else:
            file = Common.File(sha256=app_hash, dbot_score=dbot_score)

        if not score:
            readable_output = tableToMarkdown(
                name=f"Hash {app_hash} reputation is unknown to Zimperium.", t=application_data, headers=headers, removeNull=True
            )
        else:
            readable_output = tableToMarkdown(
                name=f"Hash {app_hash} reputation:", t=application_data, headers=headers, removeNull=True
            )
...
def report_get(client: Client, args: dict) -> CommandResults:
    ...
    if not report_data:
        command_results = CommandResults(readable_output="A report was not found.", raw_response=report)
    else:
...
def events_search(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        name=f"Number of events found: {total_elements}. {table_name}", t=events_data, headers=headers, removeNull=True
    )
...
def fetch_incidents(
    client: Client, last_run: dict, fetch_query: str, first_fetch_time: str, max_fetch: str = "50"
) -> tuple[dict, list]:
    """
    ...
    if not last_run:  # if first time fetching
        next_run = {"time": parse_date_range(first_fetch_time, date_format=timestamp_format)[0], "last_event_ids": []}
    else:
        next_run = last_run
