...
@pytest.mark.parametrize(
    ...
    [
        (
            events_search,
            {"query": "eventId==*", "size": "10", "page": "0", "verbose": "true"},
            RESPONSE_SEARCH_EVENTS,
            EXPECTED_SEARCH_EVENTS,
        ),
        (users_search, {"query": "objectId==*", "size": "10", "page": "0"}, RESPONSE_SEARCH_USERS, EXPECTED_SEARCH_USERS),
        (user_get_by_id, {"object_id": "1B9182C7-8C12-4499-ADF0-A338DEFDFC33"}, RESPONSE_USER_GET_BY_ID, EXPECTED_USER_GET_BY_ID),
        (devices_search, {"query": "deviceId==*", "size": "10", "page": "0"}, RESPONSE_SEARCH_DEVICES, EXPECTED_SEARCH_DEVICES),
        (
            device_get_by_id,
        ...
        (
            devices_get_last_updated,
            {"from_last_update": "5 days"},
            RESPONSE_GET_LAST_UPDATED_DEVICES,
            EXPECTED_GET_LAST_UPDATED_DEVICES,
        ),
        (
            app_classification_get,
            {"app_hash": "aad9b2fd4606467f06931d72048ee1dff137cbc9b601860a88ad6a2c092"},
            RESPONSE_APP_CLASSIFICATION_GET,
            EXPECTED_APP_CLASSIFICATION_GET,
        ),
        (
...
def test_file_reputation(mocker):
    ...
    command_results_list = file_reputation(client, args={"file": "aad9b2fd4606467f06931d72048ee1dff137cbc9b601860a88ad6a2c092"})
...
def test_fetch_incidents(mocker):
    ...
    assert (
        incidents[0].get("name") == "Detected network scan after connecting to Free Wi-Fi. No active attacks were"
