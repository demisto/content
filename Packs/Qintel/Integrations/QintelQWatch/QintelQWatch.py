...
def _set_fetch_params(params, now):
    ...
    limit = arg_to_number(arg=demisto.params().get("max_fetch"), arg_name="max_fetch", required=False)
    ...
    for r in data:
        ...
        for ts in timestamps:
            if ts["context"] == "loaded":
                entry["loaded"] = _make_timestamp(ts["value"]).strftime(DATE_FORMAT)

        timestamps = demisto.get(r, "meta.stats.credential.timestamps") or []
        for ts in timestamps:
            if ts["context"] == "first_seen":
                entry["firstseen"] = _make_timestamp(ts["value"]).strftime(DATE_FORMAT)

            if ts["context"] == "last_seen":
                entry["lastseen"] = _make_timestamp(ts["value"]).strftime(DATE_FORMAT)
...
def search_exposures(client, args, params):
    ...
    for search in searches:
        ...
        if response and response.get("data"):
            ...
            hr = tableToMarkdown(header, hr_data, headers=list(EXPOSURE_HR_MAP.keys()))

            command_results.append(
                CommandResults(outputs_prefix="Qintel", outputs=data, outputs_key_field="", readable_output=hr)
            )
