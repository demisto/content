...
def _process_cve_attributes(data, rv):
    rv["AffectedSystem"] = _map_fields(data, ("attributes", "affected_system", "name"))

    rv["AffectedVersions"] = _map_fields(data, ("attributes", "affected_system", "versions"))

    if isinstance(rv["AffectedVersions"], list):
        rv["AffectedVersions"] = ", ".join(rv["AffectedVersions"])

    rv["LastObserved"] = _make_timestamp(_map_fields(data, ("attributes", "last_observed"))).strftime(DATE_FORMAT)  # noqa
        ...
        if timestamps:
            for t in timestamps:
                if t["context"] == "observed":
                    ts_observ = _make_timestamp(t["value"]).strftime(DATE_FORMAT)
...
def cve_command(client, **args):
    ...
    for cve in cves:
        ...
        if response and response.get("data"):
            ...
            cve_return = Common.CVE(id=cve, cvss="None", description="None", published="None", modified="None")

            command_results.append(
                CommandResults(
                    outputs_prefix="Qintel.CVE", outputs_key_field="id", outputs=data, indicator=cve_return, readable_output=hr
                )
            )
            ...
            command_results.append(CommandResults(readable_output=hr))
