...
import pytest
from CommonServerPython import date_to_timestamp, set_integration_context
from dateutil import tz
from freezegun import freeze_time
from IBMMaaS360SecurityEventCollector import DATE_FORMAT, AuditEventType, Client
...
def admin_changes_request(requests_mock):
    def mock_admin_changes_resp(request, context):
        if request.headers.get("Authorization") != f'MaaS token="{AUTH_TOKEN}"':
            ...
            return {"adminChanges": {"errorCode": 1009, "errorDesc": "Token invalid"}}
    ...
    return requests_mock.get(f"{BASE_URL}{AUDIT_CHANGES_SUFFIX}", json=mock_admin_changes_resp)
...
def login_reports_request(requests_mock):
    ...
    return requests_mock.get(f"{BASE_URL}{AUDIT_LOGIN_REPORTS_SUFFIX}", json=mock_login_reports_resp)
...
def test_get_auth_token_long_wait(mocker, client):
    ...
    with freeze_time(TEST_TIME) as frozen_time:
        ...
        mocked_authenticate = mocker.patch.object(
            client, "authenticate", return_value=(f"new_{AUTH_TOKEN}", f"new_{REFRESH_TOKEN}")
        )
        mocked_refresh = mocker.patch.object(
            client, "refresh_auth_token", return_value=(f"refreshed_{AUTH_TOKEN}", f"refreshed_{REFRESH_TOKEN}")
        )
