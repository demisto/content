...
from MicrosoftGraphApplications import MANAGED_IDENTITIES_TOKEN_URL, Resources, main
...
def test_reset_auth_command(mocker, requests_mock):
    ...
    assert (
        "Authorization was reset successfully. Please regenerate the credentials, and "
...
def test_auth_complete_with_managed_identities(mocker, requests_mock):
    ...
    params = {"managed_identities_client_id": {"password": "test_client_id"}, "authentication_type": "Azure Managed Identities"}
...
def test_test_module_command_with_managed_identities(mocker, requests_mock, client_id):
    ...
    params = {"managed_identities_client_id": {"password": client_id}, "authentication_type": "Azure Managed Identities"}
    mocker.patch.object(demisto, "params", return_value=params)
...
@pytest.mark.parametrize("limit, list_url_count, next_url_count, top_param_in_url", [(10, 1, 0, True), (0, 1, 1, False)])
def test_service_principal_list_command(mocker, requests_mock, limit, list_url_count, next_url_count, top_param_in_url):
    """
    Given:
        -
    When:
...
def test_remove_service_principals_command_validation(mocked_client):
    ...
    with pytest.raises(
        DemistoException, match=re.escape("Either the (object's) `id` or the `application_id` arguments must be provided.")
    ):
        remove_service_principals_command(mocked_client, args={})


GET_SERVICE_PRINCIPAL_RESPONSE = {
    ...
    "addIns": [],
    "appRoles": [],
    "info": {"logoUrl": None, "marketingUrl": None, "privacyStatementUrl": None, "supportUrl": None, "termsOfServiceUrl": None},
    "keyCredentials": [],
...
def test_get_service_principal_command(requests_mock, mocked_client):
    ...
    assert (
        results.readable_output == "### Service Principal (application):\n|id|appId|appDisplayName|accountEnabled|\n"
        "|---|---|---|---|\n| XXXX | XXXX | Test | true |\n"
    )
...
@pytest.mark.parametrize(
    "args, expected_id, expected_args",
    [
        ({"id": "TEST", "app_id": None, "account_enabled": "true"}, "/TEST", {"data": {"accountEnabled": True}}),
        (
            {"id": None, "app_id": "TEST", "app_role_assignment_required": "true"},
            "(appId='TEST')",
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"id": "TEST", "app_id": None, "display_name": "Password friendly name"},
            "/TEST",
            {"data": {"displayName": "Password friendly name", "endDateTime": "2022-02-28 12:00:00"}},
        ),
        (
            {"id": None, "app_id": "TEST", "display_name": "NAME", "end_date_time": "end", "start_date_time": "start"},
            "(appId='TEST')",
