...
class Client(BaseClient):
    def get_api_indicators(self, query_filter: str = None, limit: int = 10):
    ...
    def fetch_indicators(self, limit: int = 10, filter_tag: Optional[str] = None, fetch_command: bool = False) -> List:
        ...
        try:
            for indicator in response.get("data", []):
                result.append({"data": indicator, "type": "file", "FeedURL": self._base_url})
...
def fetch_indicators_command(
    ...
    raw_indicators = client.fetch_indicators(limit, filter_tag, fetch_command=fetch_command)
...
def get_indicators_command(client: Client, params: Dict[str, str], args: Dict[str, str]) -> CommandResults:
    ...
    indicators = fetch_indicators_command(client, tlp_color, feed_tags, limit, filter_tag)

    human_readable = tableToMarkdown(
        "Indicators from VirusTotal Livehunt Feed:",
        indicators,
        headers=["sha256", "detections", "fileType", "rulesetName", "ruleName"],
...
def main():
    ...
    try:
        ...
        if command == "test-module":
            ...
            # interval.
            indicators = fetch_indicators_command(client, tlp_color, feed_tags, limit, filter_tag, fetch_command=True)
