...
class TestScoreCalculator:
    ...
    @pytest.mark.parametrize("malicious, suspicious, threshold, result", [(0, 5, 5, True), (10, 0, 5, True), (0, 0, 2, False)])
    def test_is_suspicious_by_threshold(self, malicious: int, suspicious: int, threshold: int, result: bool):
        analysis_results = {"malicious": malicious, "suspicious": suspicious}
        assert self.score_calculator.is_suspicious_by_threshold(analysis_results, threshold) is result

    @pytest.mark.parametrize("malicious, threshold, result", [(5, 5, True), (10, 5, True), (0, 2, False)])
    def test_is_malicious_by_threshold(self, malicious: int, threshold: int, result: bool):
        analysis_results = {"malicious": malicious}
    ...
    @pytest.mark.parametrize("yara_rules_found, result", [(1, False), (3, True), (2, True)])
    def test_is_suspicious_by_rules_yara(self, yara_rules_found: int, result: bool):
        ...
        # process
        response = {"data": {"crowdsourced_yara_results": [1] * yara_rules_found}}
...
class TestHelpers:
    ...
    @pytest.mark.parametrize(
        "epoch_time, output", [(0, "1970-01-01 00:00:00Z"), (999113584, "2001-08-29 19:33:04Z"), ("a string", None)]
    )
    ...
    def test_encode_to_base64(self):
        assert (
            encode_to_base64("c59bffd0571b8c341c7b4be63bf0e3cd", 1613568775)
            == "YzU5YmZmZDA1NzFiOGMzNDFjN2I0YmU2M2JmMGUzY2Q6MTYxMzU2ODc3NQ=="
        )
...
def test_create_relationships():
    ...
    with open("./test_data/relationships.json") as f:
        relationships = create_relationships(
            entity_a="Test", entity_a_type="IP", relationships_response=json.load(f), reliability="B - Usually reliable"
        )
...
def test_domain_command(mocker, requests_mock):
    ...
    from VirusTotalV3 import Client, ScoreCalculator, domain_command
    ...
    requests_mock.get(
        f"https://www.virustotal.com/api/v3/domains/testing.com?relationships={domain_relationships}", json=mock_response
    )

    # Run command and collect result array
    results = domain_command(
        client=client, score_calculator=mocked_score_calculator, args=demisto.args(), relationships=domain_relationships
    )
...
def test_ip_command(mocker, requests_mock):
    ...
    """
    import CommonServerPython
    from VirusTotalV3 import Client, ScoreCalculator, ip_command

    # Setup Mocks
    mocker.patch.object(demisto, "args", return_value={"ip": "192.168.0.1", "extended_data": "false"})
    mocker.patch.object(demisto, "params", return_value=DEFAULT_PARAMS)
    mocker.patch.object(CommonServerPython, "is_demisto_version_ge", return_value=True)
    mocker.patch("VirusTotalV3.is_ip_internal", return_value=False)
    ...
    requests_mock.get(
        f"https://www.virustotal.com/api/v3/ip_addresses/192.168.0.1?relationships={ip_relationships}", json=mock_response
    )
...
def test_ip_command_override_private_lookup(mocker, requests_mock):
    ...
    # Setup Mocks
    mocker.patch.object(
        demisto, "args", return_value={"ip": "192.168.0.1", "extended_data": "false", "override_private_lookup": "true"}
    )
    mocker.patch.object(demisto, "params", return_value=DEFAULT_PARAMS)
    mocker.patch.object(CommonServerPython, "is_demisto_version_ge", return_value=True)
    mocker.patch("VirusTotalV3.is_ip_internal", return_value=False)
...
def test_url_command(mocker, requests_mock):
    ...
    """
    import CommonServerPython
    from VirusTotalV3 import Client, ScoreCalculator, url_command
    ...
    # Run command and collect result array
    results = url_command(
        client=client, score_calculator=mocked_score_calculator, args=demisto.args(), relationships=url_relationships
    )
...
def test_private_file_command(mocker, requests_mock):
    ...
    requests_mock.get(f"https://www.virustotal.com/api/v3/private/files/{sha256}", json=mock_response)
...
def test_private_url_command(mocker, requests_mock):
    ...
    """
    import CommonServerPython
    from VirusTotalV3 import Client, private_url_command
    ...
    requests_mock.get(f"https://www.virustotal.com/api/v3/private/urls/{encode_url_to_base64(url)}", json=mock_response)
...
def test_private_url_scan_command(mocker, requests_mock):
    ...
    """
    import CommonServerPython
    from VirusTotalV3 import Client, private_scan_url_command
    ...
    mocker.patch.object(demisto, "args", return_value={"url": url})
    requests_mock.post("https://www.virustotal.com/api/v3/private/urls", json=mock_response)
...
def test_not_found_file_sandbox_report_command(mocker, requests_mock):
    ...
    """
    import CommonServerPython
    from VirusTotalV3 import Client, file_sandbox_report_command
    ...
    mock_response = {"error": {"code": "NotFoundError"}}
    requests_mock.get(f"https://www.virustotal.com/api/v3/files/{sha256}/behaviours", json=mock_response)
