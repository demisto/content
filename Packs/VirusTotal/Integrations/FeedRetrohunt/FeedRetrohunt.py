...
class Client(BaseClient):
    ...
    def build_iterator(self, limit: int = 40, job_id: str = "") -> List:
        ...
        try:
            for job in jobs:
                result.append({"data": job, "type": "file", "FeedURL": self._base_url})
    ...
    def fetch_jobs(self):
        ...
        return self._http_request("GET", "intelligence/retrohunt_jobs?filter=status:finished")
    ...
    def list_job_matches(self, limit: int = 40, job_id: str = "") -> list:
...
def fetch_indicators_command(
    client: Client, tlp_color: Optional[str] = None, feed_tags: List = [], limit: int = 40, job_id: str = ""
) -> List[Dict]:
...
def get_indicators_command(client: Client, params: Dict[str, str], args: Dict[str, str]) -> CommandResults:
    ...
    human_readable = tableToMarkdown(
        ...
        indicators,
        headers=["sha256", "detections", "fileType"],
