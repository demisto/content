...
class Client(BaseClient):
    ...
    def get_info(self) -> Dict[str, Any]:
        return self._http_request(method="GET", url_suffix="/info")

    def get_top_stats(self) -> Dict[str, Any]:
        return self._http_request(method="GET", url_suffix="/stats/top")

    def bulk_search(self, hashtype: str, data: Dict[str, List[str]]) -> Dict[str, Any]:
        return self._http_request(method="POST", url_suffix=f"/bulk/{hashtype}", json_data=data)

    def file_search(self, hashtype: str, hashvalue: str) -> Dict[str, Any]:
        return self._http_request(method="GET", url_suffix=f"/lookup/{hashtype}/{hashvalue}")
...
def relationship_creator(entity_a, entity_b_list, reliability) -> List[EntityRelationship]:
    ...
    for item in entity_b_list:
        ...
        list_of_relationships.append(
            EntityRelationship(
                name=relation_by_type,
                entity_a=entity_a,
                entity_a_type="File",
                entity_b=entity_b,
                entity_b_type="File",
...
def create_file_output(results: Dict[str, str], hashtype: str, reliability: str, create_relationships: bool) -> CommandResults:
    ...
    if "KnownMalicious" in results:
        dbot_score_object = Common.DBotScore(
            indicator=file_hash, indicator_type=DBotScoreType.FILE, integration_name="Circl", score=3, reliability=reliability
        )
    else:
        dbot_score_object = Common.DBotScore(
            indicator=file_hash, indicator_type=DBotScoreType.FILE, integration_name="Circl", score=0, reliability=reliability
        )

    file = Common.File(
        dbot_score=dbot_score_object,
        name=results.get("FileName"),
        file_type=results.get("mimetype"),
        md5=results.get("MD5"),
        sha1=results.get("SHA-1"),
        sha256=results.get("SHA-256"),
...
def create_output(results: Dict[str, str], endpoint: str, keyfield: str = "") -> CommandResults:
    ...
    output = CommandResults(
        outputs_prefix=f"CIRCL.{endpoint}", outputs_key_field=keyfield, outputs=results, readable_output=human_readable
    )
    return output
