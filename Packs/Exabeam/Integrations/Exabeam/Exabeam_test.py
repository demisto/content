...
from Exabeam import (
    Client,
    contents_append_notable_user_info,
    contents_user_info,
    get_peer_groups,
    get_user_labels,
    get_watchlist,
    ...
    get_rules_model_definition,
    parse_context_table_records_list,
    get_notable_assets,
    get_notable_session_details,
    get_notable_sequence_details,
    get_notable_sequence_event_types,
    delete_context_table_records,
    list_incidents,
    convert_all_unix_keys_to_date,
    fetch_incidents,
    build_incident_response_query_params,
    fetch_notable_users,
)
...
from test_data.response_incidents import (
    INCIDENTS,
    EXPECTED_INCIDENTS,
    EXPECTED_LAST_RUN,
    EXPECTED_CALL_ARGS,
...
def test_contents_append_notable_user_info():
    contents = []
    user_ = {"username": "my_username", "lastActivityType": "malicious", "labels": "bad"}
    user = {"userFullName": "my_fullname", "notableSessionIds": "123", "highestRiskSession": 5}
    user_info = {
        "riskScore": 3,
...
def test_commands(command, args, response, expected_result, mocker):
    ...
    client = Client("http://exabeam.com/api/auth/login", verify=False, username="user", password="1234", proxy=False, headers={})
...
@pytest.mark.parametrize(
    "records_input, fmt, is_delete, expected_result",
    [
        (
            "k1:v1,v2:,v3:v31;v32",
            "key:value",
            False,
            [{"key": "k1", "value": ["v1"]}, {"key": "v2", "value": []}, {"key": "v3", "value": ["v31", "v32"]}],
        ),
        ("k1,k2", "id", True, [{"id": "k1", "key": ""}, {"id": "k2", "key": ""}]),
        (
            "id1:k1:v1,id2:k2:,id3:k3:v31;v32",
            "id:key:value",
            False,
            [
                {"id": "id1", "key": "k1", "value": ["v1"]},
                {"id": "id2", "key": "k2", "value": []},
...
def test_fetch_incidents_with_look_back(mocker, params, expected_incidents, expected_last_run):
    ...
    mocker.patch.object(Client, "_login", return_value=None)
    client = Client(base_url="https://example.com", username="test_user", password="1234", verify=False, proxy=False, headers={})
    request_get_incidents = mocker.patch.object(client, "get_incidents", return_value=INCIDENTS_FOR_LOOK_BACK_FIRST_TIME)
    mocker.patch(
        "Exabeam.demisto.getLastRun",
        return_value={
            "found_incident_ids": {"SOC-402": 1671703085},  # Unix Epoch Time
            "limit": 3,
            "time": "2022-12-22T09:58:05.000000",
        },
    )
    try:
        results, last_run = fetch_incidents(client, params)
