...
EXPECTED_USER_LABELS = {
    "Exabeam.UserLabel(val.Label && val.Label === obj.Label)": [{"Label": "privileged_user"}, {"Label": "service_account"}]
}
EXPECTED_WATCHLISTS = {
...
EXPECTED_NOTABLE_SESSION_DETAILS = {
    "Exabeam.NotableSession(val.SessionID && val.SessionID === obj.SessionID)": {
        "sessions": [
            {"SessionID": "session1", "InitialRiskScore": 0, "LoginHost": "host1", "Accounts": ["account1", "account2"]},
            {"SessionID": "session2", "InitialRiskScore": 26, "LoginHost": "host2", "Accounts": ["account1", "account2"]},
        ],
        "users": [
            {
                "UserName": "username2",
...
EXPECTED_RESULT_AFTER_RECORD_DELETION = {
    "Exabeam.ContextTableUpdate(val.changeId && val.changeId === obj.changeId)": [
        {
...
EXPECTED_INCIDENT_LIST = {
    "Exabeam.Incident(val.incidentId && val.incidentId === obj.incidentId)": [
        {
            "incidentId": "SOC-19",
            "name": "phil: Notable AA Session",
            "fields": {
                "startedDate": "2020-12-15T02:31:10.130000",
                "closedDate": None,
                "createdAt": "2020-12-15T08:19:19.194000",
                "owner": "unassigned",
                "status": "new",
                "incidentType": ["generic", "ueba"],
                "source": "Exabeam AA",
                "priority": "medium",
                "queue": "1",
                "description": None,
