...
def ensure_new_last_run_schema(last_run: dict) -> dict[str, Any]:
    ...
    if DEPRECATED_AUDIT_START_DATE_KEY in last_run:
        ...
        log_prefix = audit_type.log_prefix
        demisto.info(f"{log_prefix} Detected old last run schema.")
        try:
            old_audit_start_date = cast(datetime, arg_to_datetime(last_run.get(DEPRECATED_AUDIT_START_DATE_KEY)))
            new_last_run[audit_type.value] = {
            ...
            demisto.info(f"{log_prefix} Successfully migrated previous run schema to {new_last_run['audit']}.")
        except (ValueError, AttributeError):
            # `arg_to_datetime` throws an exception if value is not a valid datetime string / timestamp
            demisto.error(f"{log_prefix} Could not migrate previous run schema. Failed to parse audit start date.")
    ...
    if DEPRECATED_SIEM_NEXT_PAGE_KEY in last_run:  # API V1.0 next page token (incompatible with API 2.0 next page token)
        ...
        log_prefix = siem_type.log_prefix
        demisto.info(f"{log_prefix} Detected old last run schema.")
        demisto.error(f"{log_prefix} last run cannot be migrated to new schema due to SIEM next page token incompatibility.")
        # Only migrate last fetched SIEM IDs
        new_last_run[siem_type.value] = {LAST_FETCHED_IDS_KEY: last_run.get(DEPRECATED_SIEM_LAST_FETCHED_IDS_KEY, [])}
