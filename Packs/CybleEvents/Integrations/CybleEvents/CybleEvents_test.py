...
def test_fetch_incidents(requests_mock):
    ...
    assert (
        response[0]["rawJSON"] == '{"name": "Cyble Intel Alert on some_alias_2", '
...
@pytest.mark.parametrize(
    "eID,eType", [("type1", "id1"), ("type2", "id2"), ("some_event_type", "some_event_id"), ("new_event_type", "new_event_id")]
)
...
@pytest.mark.parametrize("offset,limit", [("0", "-2"), ("0", "1289")])
def test_limit_cyble_vision_fetch_detail(requests_mock, capfd, offset, limit):
    ...
    args = {"token": "some_random_token", "event_type": "eType", "event_id": "eID", "from": offset, "limit": limit}

    with capfd.disabled(), pytest.raises(ValueError, match=f"Limit should a positive number up to 1000, limit: {limit}"):
...
def test_offset_cyble_vision_fetch_detail(requests_mock, capfd):
    ...
    args = {"token": "some_random_token", "event_type": "eType", "event_id": "eID", "from": "-1", "limit": 1}

    with capfd.disabled(), pytest.raises(ValueError, match="Parameter having negative value, from: -1'"):
        fetch_alert_details(client=client, args=args)
...
def test_etype_cyble_vision_fetch_detail(requests_mock, capfd):
    ...
    args = {"token": "some_random_token", "event_id": "eID"}

    with capfd.disabled(), pytest.raises(ValueError, match="Event Type not specified"):
        fetch_alert_details(client=client, args=args)
...
def test_eid_cyble_vision_fetch_detail(requests_mock, capfd):
    ...
    args = {"token": "some_random_token", "event_type": "eType"}

    with capfd.disabled(), pytest.raises(ValueError, match="Event ID not specified"):
        fetch_alert_details(client=client, args=args)
...
def test_limit_validate_input(capfd):
    ...
    with capfd.disabled(), pytest.raises(ValueError, match=f"Limit should a positive number upto 50, limit: {args.get('limit', '50')}"):
        validate_input(args=args)
