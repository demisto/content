...
class GoogleClient:
    ...
    def __init__(
        self, service_name: str, service_version: str, client_secret: str, scopes: list, proxy: bool, insecure: bool, **kwargs
    ):
        """
...
def functions_list_command(client: GoogleClient, args: dict):
    ...
    hr = tableToMarkdown(f'Functions in project "{client.project}" and region "{disp_region}"', functions, ["name"] + keys)
...
def main():
    ...
    credentials_json = json.loads(params.get("project_id_creds", {}).get("password") or params.get("credentials_json", {}))
    ...
    client = GoogleClient("cloudfunctions", "v1", credentials_json, scopes, proxy, insecure, project=project, region=region)
