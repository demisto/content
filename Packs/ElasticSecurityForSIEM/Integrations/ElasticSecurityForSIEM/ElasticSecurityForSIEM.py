...
ELASTIC_SERVER = PARAMS.get("url", "").rstrip("/") + ":" + PARAMS.get("elastic_port")
...
def get_mapping_fields_command():
    ...
    for index in indexes:
        if index == "":
            res = requests.get(ELASTIC_SERVER + "/_mapping", auth=(USERNAME, PASSWORD), verify=INSECURE)
        else:
            res = requests.get(ELASTIC_SERVER + "/" + index + "/_mapping", auth=(USERNAME, PASSWORD), verify=INSECURE)
        res_json = res.json()
