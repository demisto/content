Search for and analyze data in real time. Supports Elastic Security SIEM functionality for security operations. 
## Configure Elastic Security for SIEM in Cortex
...
| document | The document object (JSON format) to be indexed. See Elasticsearch documentation (https://www.elastic.co/guide/en/elasticsearch/client/python-api/current/examples.html#ex-index) for further information about indexing documents. | Required | 
