...
from Envoy_IT_Admin import Client, get_user_command, create_user_command,\
...
def test_get_user_command_fail(mocker):
    ...
    client = Client(base_url='https://test.com', token='123456', verify=False, headers={})
...
def test_disable_command_success(mocker):
    ...
    mocker.patch.object(client, 'get_user', return_value=res1)
    mocker.patch.object(client, 'update_user', return_value=res1)
...
def test_enable_command_user_not_found(mocker):
    ...
    mocker.patch.object(requests, 'request', return_value=err_res)
    demisto.callingContext = {'context': {'IntegrationInstance': 'Test', 'IntegrationBrand': 'Test'}}
    client = Client(base_url='https://test.com', token='1234567', headers=headers)

    readable_output, outputs, data = enable_user_command(client, args)
    assert not data['success']
    assert data['errorCode'] == 404
    assert data['errorMessage'] == "404 Not Found"
