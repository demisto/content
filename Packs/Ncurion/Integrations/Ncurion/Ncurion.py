...
def login(base_url, username, password):
    ...
    headers1 = {"Authorization": f"Bearer {access_token}", "Content-Type": "application/json"}
...
def get_log_list(base_url, username, password):
    ...
    results = CommandResults(outputs_prefix=f"{INTEGRATION_CONTEXT_NAME}.logsever", outputs_key_field="id", outputs=context_entry)
    return_results(results)


def fetch_incidents(
    base_url, username, password, last_run: dict[str, int], first_fetch_time: int
    ...
    logout = json.dumps({"access_token": access_token, "refresh_token": refresh_token})
    ...
    try:
        if command == "test-module":
            ...
            next_run, incidents = fetch_incidents(
                base_url, username, password, last_run=demisto.getLastRun(), first_fetch_time=first_fetch_timestamp
            )
