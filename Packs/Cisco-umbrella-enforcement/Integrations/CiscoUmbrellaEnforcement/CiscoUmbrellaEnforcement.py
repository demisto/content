...
class Client(BaseClient):
    ...
    def delete_domains(self, domain_name: str, domain_id: str):
        url_suffix = (
            f"domains/{domain_id}?customerKey={self.api_key}"
            if domain_id
            else f"domains?customerKey={self.api_key}&where[name]={domain_name}"
        )
        return self._http_request("DELETE", url_suffix, return_empty_response=True)

    def add_event_to_domain(self, event: dict):
        return self._http_request("POST", f"events?customerKey={self.api_key}", json_data=event, return_empty_response=True)
...
def domains_list_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="UmbrellaEnforcement.Domains", outputs_key_field="id", outputs=response
    )
...
def domain_delete_command(client: Client, args: dict) -> CommandResults:
    ...
    return CommandResults(
        readable_output=message, outputs_prefix="UmbrellaEnforcement.Domains", outputs_key_field="id", outputs=curr_context
    )
