...
def http_request(method, url_suffix, params=None, data=None, files=None, is_json=True):
    ...
    if is_json:
        try:
            ...
            return_error(f"failed to parse json object from response: {res.content!s}")
...
def login():
    ...
    data = {"user": CREDENTIALS.get("identifier"), "password": CREDENTIALS.get("password")}
...
def update_alertstatus_command():
    status_to_explicit_score = {"False Positive": 1, "Not Interesting": 2, "Interesting": 3, "Actionable": 4}
    args = demisto.args()
    alert_id = args["alert_id"]
    status = args["status"]

    data = {"alertIds": [alert_id], "explicitScore": status_to_explicit_score[status]}
...
def get_alert_dpath_command():
    ...
    output = {"ID": alert_id, "DecodingPath": context_result}
    ...
    demisto.results(
        {
            ...
            "HumanReadable": tableToMarkdown(f"Alert {alert_id}", context_result, headerTransform=pascalToSpace, removeNull=True),
            "EntryContext": {
...
def alert_ef_submission_command():
    ...
    output = {"ID": alert_id, "ExecutionForensics": context_result}
...
def add_alert_comment_command():
    ...
    data = {"type": "byAlertID", "alertIds": [alert_id], "comment": comment}
...
def manage_alert_label_command():
    ...
    label_action = {"Add": "LABEL_ADD", "Remove": "LABEL_REMOVE"}
...
def manage_alert_assignuser_command():
    ...
    entry_context = {"AssignedUser": assign_user, "ConclusionID": conclusion_id}

    return_outputs(
        f"Assigned User: {assign_user} to alert with conclusion ID {conclusion_id}",
        {"Fidelis.Alert(val.ConclusionID && val.ConclusionID == obj.ConclusionID)": entry_context},
        raw_response,
    )
...
def get_alert_sessiondata_command():
    ...
    output = {"ID": alert_id, "SessionData": context_result}
...
def get_alert_forensictext_command():
    ...
    output = {"ID": alert_id, "ForensicText": result}
    ...
    demisto.results(
        {
            ...
            "Contents": alert,
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": tableToMarkdown(f"Alert {alert_id}", output, headerTransform=pascalToSpace, removeNull=True),
            "EntryContext": {
                "Fidelis.Alert(val.ID && val.ID == obj.ID)": output,
            },
        }
...
def get_alert_report_command():
    ...
    demisto.results(fileResult(f"Alert_Details_{alert_id}.pdf", pdf_content, file_type=entryTypes["entryInfoFile"]))


@logger
def get_alert_report(alert_id):
    result = http_request("GET", "/j/rest/v1/alert/export/alertdetails/pdf", params={"alertIds": alert_id}, is_json=False)
...
def list_alerts_by_ip_request(time_frame=None, start_time=None, end_time=None, src_ip=None, dest_ip=None):
    ...
    data = {
        "commandPosts": [],
        "filter": {"composite": {"logic": "or", "filters": filters}},
        "order": [{"column": "ALERT_TIME", "direction": "DESC"}],
...
def list_alerts_by_ip():
    ...
    results = list_alerts_by_ip_request(
        time_frame=time_frame, start_time=start_time, end_time=end_time, src_ip=src_ip, dest_ip=dest_ip
    )
    output = [
        {
    ...
    context = {"Fidelis.Alert(val.ID && val.ID == obj.ID)": output}
...
def upload_pcap(component_ip, entry_id):
    ...
    try:
        with open(file_info["name"], "rb") as f:
            http_request("POST", f"/j/rest/policy/pcap/upload/{component_ip}/", files={"uploadFile": f}, is_json=False)
    finally:
...
def run_pcap(component_ip, file_names):
    data = {"component": component_ip, "files": file_names}
...
def list_metadata_request(
    time_frame=None, start_time=None, end_time=None, client_ip=None, server_ip=None, request_direction=None
):
    filters = []
    ...
    data = {
        ...
        "displaySettings": {
            ...
            "currentPage": 1,
            "pageNavigation": "",
            "sorting": {"column": "Timestamp", "sortingOrder": "D"},
        },
        "dataSettings": {"composite": {"logic": "and", "filters": filters}},
        "searchId": search_id,
    }
...
def list_metadata():
    ...
    context = {"Fidelis.Metadata(val.ID && val.ID == obj.ID)": event_context}
...
def download_malware_file_request(alert_id):
    ...
    query_params = {"uid": SESSION_ID, "alert_id": alert_id, "type": "1", "params": dpath}
    res = http_request("GET", "/query/tcpses_getfile.cgi", params=query_params, is_json=False)
...
def download_malware_file():
    ...
    if not file_name:
        ...
        demisto.results(fileResult(decoded_file_name + ".zip", results, file_type=entryTypes["file"]))
...
def download_pcap_request(alert_id):
    ...
    results = http_request("GET", "/e.cgi", params=query_params, is_json=False)
...
def download_pcap_file():
    ...
    demisto.results(fileResult("Alert ID_" + alert_id + ".pcap", results, file_type=entryTypes["file"]))
