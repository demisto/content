...
def test_get_last_run(last_run, first_fetch):
    ...
    if last_run:
        assert last_run_dt.replace(microsecond=0, second=0, minute=0) == \
            arg_to_datetime(arg_name="last_run", arg=last_run).replace(microsecond=0, second=0, minute=0, tzinfo=None)
    else:
        ...
        assert last_run_dt.replace(microsecond=0, second=0, minute=0, tzinfo=None) == \
            arg_to_datetime(arg_name="first_fetch", arg=first_fetch).replace(microsecond=0, second=0, minute=0, tzinfo=None)
