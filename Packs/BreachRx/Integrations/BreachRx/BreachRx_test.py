...
def set_up_mocker(m, found_incident=True):
    m.post(requests_mock.ANY, additional_matcher=create_incident_matcher, json=util_load_json("test_data/create_incident.json"))
    m.post(
        requests_mock.ANY, additional_matcher=get_severities_matcher, json=util_load_json("test_data/incident_severities.json")
    )
    m.post(requests_mock.ANY, additional_matcher=get_types_matcher, json=util_load_json("test_data/incident_types.json"))
    if found_incident:
        m.post(requests_mock.ANY, additional_matcher=get_incident_matcher, json=util_load_json("test_data/get_incident.json"))
    else:
        m.post(
            requests_mock.ANY, additional_matcher=get_incident_matcher, json=util_load_json("test_data/get_incident_empty.json")
        )
    m.post(
        requests_mock.ANY, additional_matcher=get_actions_matcher, json=util_load_json("test_data/get_actions_for_incident.json")
    )
...
def test_create_incident_command():
    ...
    with requests_mock.Mocker() as m:
        ...
        results = create_incident_command(client, incident_name=incident_name, description=incident_description)
...
def test_create_incident_command_no_description():
    ...
    with requests_mock.Mocker() as m:
        client = set_up_mocker(m)
        results = create_incident_command(client, incident_name=incident_name)

        create_incident_request = m.request_history[-1]
        assert incident_name == create_incident_request.json()["variables"]["name"]
        assert (
            create_incident_request.json()["variables"]["description"]
...
def test_get_incident_actions_command_incident_name(mocker):
    ...
    with requests_mock.Mocker() as m:
        client = set_up_mocker(m)
        results = get_incident_actions_command(client, incident_name=incident_name)
...
def test_get_incident_actions_command_incident_identifier():
    ...
    with requests_mock.Mocker() as m:
        client = set_up_mocker(m)
        results = get_incident_actions_command(client, incident_identifier=incident_identifier)
        get_incident_request = m.request_history[-2]
...
def test_import_incident_command():
    ...
    with requests_mock.Mocker() as m:
        client = set_up_mocker(m)
        results = import_incident_command(client, incident_name=incident_name, incident_identifier=incident_identifier)
...
def test_import_incident_command_no_incident():
    ...
    with requests_mock.Mocker() as m:
        ...
        with pytest.raises(Exception) as error:
            import_incident_command(client, incident_name=incident_name, incident_identifier=incident_identifier)
...
def test_get_incident_command():
    ...
    with requests_mock.Mocker() as m:
        client = set_up_mocker(m)
        results = get_incident_command(client, incident_name=incident_name, incident_identifier=incident_identifier)
