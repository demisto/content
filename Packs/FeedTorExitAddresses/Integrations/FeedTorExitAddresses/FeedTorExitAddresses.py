...
class Client(BaseClient):
    ...
    def http_request_indicators(self):
        res = requests.get(url=self.url, verify=self._verify)
    ...
    def build_iterator(self, feedTags, limit):
        ...
        for line in raw_indicator_list:
            if line.startswith("ExitNode"):
                indicator = {"type": FeedIndicatorType.IP, "name": line.split()[1]}
...
def get_indicators_command(client: Client, args: dict):
    ...
    human_readable = tableToMarkdown(
        "Indicators from Tor Exit Addresses:",
        indicator_list,
...
def main():
    ...
    client = Client(params.get("insecure"), params.get("proxy"), tlp_color)

    command = demisto.command()
    demisto.info(f"Command being called is {command}")
    # Switch case
    commands = {"test-module": module_test_command, "tor-get-indicators": get_indicators_command}
    try:
