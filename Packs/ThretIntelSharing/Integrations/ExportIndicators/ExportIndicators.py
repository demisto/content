...
TEXT_FILTER_FIELDS: Optional[str] = "name,type"
...
class RequestArguments:
    def __init__(self, query: str, out_format: str = FORMAT_TEXT, limit: int = 10000, offset: int = 0,
                 ...
                 sort_order: str = '', filter_fields: str = ''):
    ...
    def is_request_change(self, last_update_data: Dict):
        ...
        elif self.filter_fields != last_update_data.get('filter_fields'):
            return True
...
def refresh_outbound_context(request_args: RequestArguments, on_demand: bool = False) -> str:
    ...
    print(request_args.query)
    indicator_searcher = IndicatorsSearcher(
        query=request_args.query,
        size=PAGE_SIZE,
        filter_fields=request_args.filter_fields
    )
    while True:
        ...
        if request_args.filter_fields:
            iocs = arrange_iocs_by_fields(iocs, request_args.filter_fields)
...
def arrange_iocs_by_fields(iocs_list, fields_to_filter):
    fields_to_filter = argToList(fields_to_filter)
    new_iocs_list = []

    for ioc in iocs_list:
        print(ioc)
        new_ioc = {}
        for field, field_value in ioc.items():
            if field in fields_to_filter:
                new_ioc[field] = field_value
            if field == 'CustomFields':
                for custom, custom_value in field_value.items():
                    if custom in fields_to_filter:
                        new_ioc[custom] = custom_value
        new_iocs_list.append(new_ioc)

    return new_iocs_list
...
def get_request_args(params):
    ...
    filter_fields = request.args.get('filter', params.get('filter_fields'))
    ...
    if out_format == FORMAT_TEXT:
        filter_fields = TEXT_FILTER_FIELDS

    return RequestArguments(query, out_format, limit, offset, mwg_type, strip_port, drop_invalids, category_default,
                            category_attribute, collapse_ips, csv_text, sort_field, sort_order, filter_fields)
...
def update_outbound_command(args, params):
    ...
    filter_fields = args.get('filter_fields')
