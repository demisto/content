...
ID_TO_MISP_DISTRIBUTION = {
    "0": 'Your_organization_only',
    "1": 'This_community_only',
    "2": 'Connected_communities',
    "3": 'All_communities',
    "5": 'Inherit_event',
}
...
def create_tir_from_event_command(demisto_args: dict) -> CommandResults:
    event_id = demisto_args.get('event_id')
    response = PYMISP.search(controller='events', eventid=event_id)
    if not response:
        raise DemistoException(f"event {event_id} does not exist in MISP")
    event = response[0].get('Event', {})
    tir_object = {}
    tir_object['creator'] = event['Orgc']['name']
    tir_object['name'] = event['info']
    tir_object['created'] = event['date']
    tir_object['mispthreatlevelid'] = ID_TO_THREAT_LEVELS[event['threat_level_id']]
    tir_object['mispdistributionid'] = ID_TO_MISP_DISTRIBUTION[event['distribution']]
    tir_object['mispeventuuid'] = event['uuid']
    tir_object['mispeventid'] = event['id']
    tir_object['type'] = "MISP"
    event_tags = event.get('Tag', [])
    if event_tags:
        tir_object['tags'] = []
        for tag in event_tags:
            name = tag['name']
            if "tlp" in name:
                tir_object['trafficlightprotocol'] = name.split(':')[-1]
            if "galaxy" in name:
                gal_type = get_galaxy_indicator_type(name)
                name = name.split('=')[-1].replace('\"', '')
                tir_object['tags'].append(f"{gal_type}-{name}")
            else:
                tir_object['tags'].append(tag['name'])
    attributes = event.get('Attribute', [])
    if attributes:
        tir_object['relationships'] = []
        for attribute in attributes:
            att_type = get_attribute_indicator_type(attribute)
            if not att_type:
                continue
            relation = {
            ...
            tir_object['relationships'].append(relation)

    return CommandResults(
        outputs_prefix='MISP.TIR',
...
def main():
    ...
    try:
        ...
        elif command == 'misp-create-tir-from-event':
            return_results(create_tir_from_event_command(args))
    except PyMISPError as e:
