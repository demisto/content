...
def _extract_incidents_from_query_result(search_incident) -> list[dict]:
    ...
    if isinstance(contents, str):
        try:
            ...
            _log(f"[GIBIncidentUpdate] Failed to parse Contents JSON string: {e!s}")
            return []
...
def prevent_duplication(current_incident):
    ...
    """
    _log("[GIBIncidentUpdate] prevent_duplication: started")
    result = True
    custom_fields = current_incident.get("CustomFields", {})
    _log(
        f"[GIBIncidentUpdate] Incoming incident keys={list(current_incident.keys())}, "
    ...
    if "CustomFields" in current_incident:
        _log("[GIBIncidentUpdate] Removing 'CustomFields' from incident before update payload")
        del current_incident["CustomFields"]
    if "labels" in current_incident:
        _log("[GIBIncidentUpdate] Removing 'labels' from incident before update payload")
        del current_incident["labels"]
    if "occurred" in current_incident:
        _log("[GIBIncidentUpdate] Removing 'occurred' from incident before update payload")
        del current_incident["occurred"]
    if "sla" in current_incident:
        _log("[GIBIncidentUpdate] Removing 'sla' from incident before update payload")
        del current_incident["sla"]

    current_incident.update(custom_fields)
    _log(f"[GIBIncidentUpdate] Payload keys after custom fields merge: {list(current_incident.keys())}")

    gibid = custom_fields.get("gibid") if isinstance(custom_fields, dict) else None
    if not gibid:
        _log("[GIBIncidentUpdate] gibid is empty or missing in CustomFields. Incident will be created.")
        return True
    query = f"gibid: {gibid} and -status:Closed"
    _log(f"[GIBIncidentUpdate] Searching existing incidents with query: {query}")
    search_incident = demisto.executeCommand("GetIncidentsByQuery", {"query": query, "limit": 200, "outputFormat": "json"})
    _log(
        f"[GIBIncidentUpdate] Search command returned type={type(search_incident)}, "
    ...
    if search_incident:
        ...
        _log(f"[GIBIncidentUpdate] Search first entry Type={first_type}, " f"Brand={first_brand}")
        incidents_data = _extract_incidents_from_query_result(search_incident)
        total = len(incidents_data)
        _log(f"[GIBIncidentUpdate] Search total incidents found: {total}")
        if total > 0:
            result = False
            incident_id = incidents_data[total - 1].get("id")
            _log(f"[GIBIncidentUpdate] Existing incident found, id={incident_id}. Updating incident in place.")
            update_args = {"id": incident_id, **current_incident}
            _log(f"[GIBIncidentUpdate] setIncident payload keys={list(update_args.keys())}, " f"fields_count={len(update_args)}")
            update_res = demisto.executeCommand("setIncident", update_args)
            _log(f"[GIBIncidentUpdate] setIncident response: {update_res}")
        else:
            _log("[GIBIncidentUpdate] No matching incidents found. New incident will be created.")
    else:
        _log("[GIBIncidentUpdate] Search returned empty response. New incident will be created.")

    _log(f"[GIBIncidentUpdate] prevent_duplication: finished with result={result}")
    return result


def main():
    try:
        _log("[GIBIncidentUpdate] main: script started")
        return_results(prevent_duplication(demisto.incident()))
        _log("[GIBIncidentUpdate] main: script finished successfully")
    except Exception as e:
        _log(f"[GIBIncidentUpdate] main: script failed with error={e!s}")
