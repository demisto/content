...
import pytest
from GroupIBTIA import Client, fetch_incidents_command, get_available_collections_command, main, transform_function
...
def test_fetch_incidents(mocker, session_fixture):
    ...
    next_run, incidents = fetch_incidents_command(
        client=client, last_run={}, first_fetch_time="3 days", incident_collections=[], requests_count=3
    )
        ...
        "files": [
            {
                ...
                "revisions": {"info": {"authorEmail": "author@example.com", "authorName": "John Doe", "timestamp": 1234567890}},
            },
            # ...
        ]
    }
...
def test_transform_some_fields_into_markdown_phishing_kit():
    ...
    feed = {
        "downloadedFrom": [
            {"date": "2023-10-16", "url": "https://example.com", "domain": "example.com", "fileName": "phish.zip"},
            # ...
        ]
    }

    expected_output = {
        "downloadedFrom": "| URL | File Name | Domain | Date |\n"
        "| --- | --------- | ------ | ---- |\n"
...
def test_transform_some_fields_into_markdown_public_leak():
    ...
    feed = {
        ...
        "matches": {"Type1": {"SubType1": ["Value1", "Value2"], "SubType2": ["Value3"]}, "Type2": {"SubType3": ["Value4"]}},
    }
...
def test_get_human_readable_feed():
    ...
    feed = {"id": 123, "field1": "value1", "field2": "value2", "field3": "value3"}

    expected_output = (
        "### Feed from TestCollection with ID 123\n"
