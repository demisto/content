...
class TestSpotlightSeverityBasedFetch:
    ...
    async def test_fetch_vulnerabilities_by_severity_single_page(self, mocker):
        ...
        assert task_kwargs["data"] == vulnerabilities
    ...
    async def test_fetch_spotlight_by_severity_parallel_success(self, mocker):
        ...
        async def mock_fetch_by_severity(client, severity, **kwargs):
            severity_data = {
                "CRITICAL": (10, {"aid1", "aid2"}, set()),
                "HIGH": (20, {"aid3", "aid4"}, set()),
                "MEDIUM": (15, {"aid5"}, set()),
                "LOW": (5, {"aid6"}, set()),
                "NONE": (0, set(), set()),
                "UNKNOWN": (0, set(), set()),
    ...
    async def test_fetch_vulnerabilities_by_severity_empty_response(self, mocker):
        ...
        assert total == 0
        assert aids == set()
        assert len(tasks) == 1  # Task still created
    ...
    async def test_fetch_vulnerabilities_by_severity_backpressure(self, mocker):
        """
        Tests that backpressure limits concurrent pending send tasks using asyncio.wait.

        Given:
            - MAX_PENDING_TASKS_PER_SEVERITY is set to 2 (for testing).
            - API returns 4 pages of vulnerabilities (4 batches).
            - Send tasks are slow (simulated with asyncio.sleep).
        When:
            - fetch_vulnerabilities_by_severity is called.
        Then:
            - The backpressure loop waits when 2 tasks are pending.
            - All 4 batches are eventually processed.
        """
        from CrowdStrikeFalcon import fetch_vulnerabilities_by_severity

        # Override the pending tasks limit for testing
        mocker.patch("CrowdStrikeFalcon.MAX_PENDING_TASKS_PER_SEVERITY", 2)
