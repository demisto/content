...
def http_request(
    ...
    try:
        ...
        return_error(
            f"Error in connection to the server. Please make sure you entered the URL correctly." f" Exception is {str(e)}."
        )
    try:
        if get_token_flag:
...
def change_host_group_members(action_name: str, host_group_id: str, host_ids: list[str]) -> dict:
    ...
    data = {"action_parameters": [{"name": "filter", "value": f"(device_id:{str(host_ids)})"}], "ids": [host_group_id]}
