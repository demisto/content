...
| Long running instance for Spotlight vulnerabilities | Whether to run the Spotlight vulnerabilities fetch in a long-running container instead of on the assets fetch schedule. Use this when a full Spotlight fetch takes longer than the Assets Fetch Interval and is repeatedly interrupted. When enabled, do not enable the assets fetch as well; only one of the two should be active per instance. This mode fetches Spotlight vulnerabilities ONLY, regardless of the Asset types to fetch selection. CNAPP Alerts are not collected in this mode; to collect them, configure a separate integration instance using the regular assets fetch. A fetch cycle starts every 24 hours. If a cycle runs longer than 24 hours, the next one starts as soon as it finishes. | False |
