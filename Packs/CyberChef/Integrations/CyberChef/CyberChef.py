...
def test_module(client: BaseClient | None, local_execution: bool):
    data = {"input": "One, two, three, four.", "recipe": "to decimal"}
...
def create_output(results, endpoint: str):
    output = CommandResults(outputs_prefix=f"CyberChef.{endpoint}", outputs_key_field="", outputs=results)
    return output
...
def main():
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            data = {"input": demisto.args().get("input"), "args": demisto.args().get("args")}
            data = remove_empty_elements(data)
