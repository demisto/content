...
valid_ip_response = {
    ...
    "raw_data": {
        ...
        "ja3": [
            {
                "fingerprint": "30017f6f809155387cbcf95be6e7225d",
                "port": 443
            },
            {
                "fingerprint": "330ed8deb9b34592442c3bb392ee0926",
                "port": 444
            }
        ],
        "hassh": [
...
test_module_data = [
    ("true_key", "Success: Access and API Key Valid", 200, "ok"),
...
ip_reputation_command_data = [
    ...
    ({"ip": "71.6.135.131"}, "negative", "invalid ip response", 200,  # NOSONAR
...
ip_quick_check_command_data = [
    ...
    ({"ip": "71.6.135.131,8.8.8.8"}, "positive", valid_multiple_qc_resp, 200,  # NOSONAR
     valid_multiple_qc_resp_expected),  # NOSONAR
    ({"ip": "71.6.135.131"}, "custom", "invalid ip response", 200,  # NOSONAR
     "Invalid response from GreyNoise. Response: invalid ip response"),  # NOSONAR
    ({"ip": "71.6.135.131"}, "negative", "forbidden", 401,  # NOSONAR
     "Unauthenticated. Check the configured API Key."),  # NOSONAR
    ({"ip": "71.6.135.131"}, "negative", [], 429, "API Rate limit hit. Try after sometime."),  # NOSONAR
    ({"ip": "71.6.135.131"}, "negative", "Dummy message", 405,  # NOSONAR
     "Failed to execute  command.\n Error: Dummy message"),  # NOSONAR
    ({"ip": "71.6.135.131"}, "negative", [], 505,  # NOSONAR
...
query_command_data: list = [
    ...
    ({}, "negative", {"message": "dummy_message"}, 200,  # NOSONAR
     "GreyNoise request failed. Reason: dummy_message"),  # NOSONAR
    ...
    ({}, "negative", "Dummy message", 405,  # NOSONAR
     "Failed to execute  command.\n Error: Dummy message"),  # NOSONAR
    ({}, "negative", {}, 505,  # NOSONAR
     "The server encountered an internal error for GreyNoise and was unable to complete your request.")  # NOSONAR
]
...
stats_command_data: list = [
    ...
    ({}, "negative", "dummy message", 200, "Invalid response from GreyNoise. Response: dummy message"),  # NOSONAR
    ({}, "negative", invalid_stats_response, 200, "GreyNoise request failed. Reason: no results"),  # NOSONAR
    ({}, "negative", "forbidden", 401, "Unauthenticated. Check the configured API Key."),  # NOSONAR
    ({}, "negative", {}, 429, "API Rate limit hit. Try after sometime."),  # NOSONAR
    ({}, "negative", "Dummy message", 405,  # NOSONAR
     "Failed to execute  command.\n Error: Dummy message"),  # NOSONAR
