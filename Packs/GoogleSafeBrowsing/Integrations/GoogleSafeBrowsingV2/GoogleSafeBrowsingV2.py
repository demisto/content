...
class Client(BaseClient):
    def __init__(self, proxy: bool, verify: bool, reliability: str, base_url: str, params: dict):
        ...
        if DBotScoreReliability.is_valid_type(reliability):
            ...
            raise Exception("Google Safe Browsing v2 error: Please provide a valid value for the Source Reliability parameter.")
    ...
    def url_request(self, client_body, list_url) -> dict:
        ...
        result = self._http_request(method="POST", json_data=body, full_url=self.base_url)
        return result
...
def url_command(client: Client, args: dict[str, Any]) -> Union[List[CommandResults], CommandResults]:
    ...
    for url_key, url_data in urls_data.items():
        if url_data:
            ...
            url_standard_context = Common.URL(url=url_key, dbot_score=dbot_score)
...
def main() -> None:
    ...
    try:
        client = Client(params=params, base_url=base_url, verify=verify_certificate, proxy=proxy, reliability=reliability)
