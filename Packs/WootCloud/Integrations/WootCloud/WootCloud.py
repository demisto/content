...
HEADERS = {"Content-Type": "application/json", "Accept": "application/json", "Host": "api.wootuno.wootcloud.com"}
...
class Client(BaseClient):
    ...
    def get_woot_alerts(self, type, start, end, severity=None, skip=None, limit=None, site_id=None, getAll=False):
        ...
        payload = {
            "starttime": str(start),
            "endtime": str(end),
            "filter": {"severity": [str(severity)] if severity else None},
        ...
        if getAll:
            ...
            return CommandResults(
                outputs=result_data, outputs_prefix=prefix, outputs_key_field="id", readable_output=readable_output
            )
