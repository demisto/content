...
- New: Added a new triggers recommendation- silent-A mail forwarding rule was configured in Google Workspace Test that This trigger runs the A mail forwarding rule was configured in Google Workspace playbook, which handles the A mail forwarding rule was configured in Google Workspace and A mail forwarding rule was configured in Google Workspace to an uncommon domain alerts.
<~XSIAM> (Available from Cortex XSIAM %%XSIAM_VERSION%%).</~XSIAM>
##### New: silent-User attempted to connect from a suspicious country
