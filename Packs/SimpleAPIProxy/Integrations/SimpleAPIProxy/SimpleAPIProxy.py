...
def make_api_request(
    ...
    if API_CRED_TYPE:
        if API_CRED_TYPE == "Basic":
            ...
            headers_to_pass = {"Authorization": f"Bearer {API_CREDENTIALS.get('password')}"}

        elif API_CRED_TYPE == "Custom Header":
            if not API_CUSTOM_HEADER:
                return Response(
                    status_code=200,
                    content="Custom Header is not set in integration configuration "
                ...
                return Response(
                    status_code=200,
                    content="Custom Header value is not set in integration configuration "
                    "and Credential Type is selected as 'Custom Header'",
                )

            headers_to_pass = {API_CUSTOM_HEADER: API_CUSTOM_HEADER_VALUE}
        ...
        if method in allowed_methods:
            ...
            if isinstance(result, Response):
                ...
                if is_json(result.text):
                    return Response(
                        status_code=result.status_code, content=json.dumps(result.json()), media_type="application/json"
                    )
                else:
