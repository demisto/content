...
class Client(BaseClient):
    ...
    async def _http_request(  # type: ignore[override]
        ...
        headers=None,
        auth=None,
        json_data=None,  # noqa: F841
        params=None,
        data=None,
        files=None,
        timeout=10,
        resp_type="json",
        ok_codes=None,  # noqa: F841
        return_empty_response=False,
        retries=0,
        status_list_to_retry=None,  # noqa: F841
        backoff_factor=5,
        raise_on_redirect=False,
        raise_on_status=False,  # noqa: F841
        error_handler=None,
        **kwargs,  # noqa: F841
    ):
        while True:
            try:
                ...
                if res.ok:
                    try:
                        ...
                        demisto.debug(f"Failed deserializing the json-encoded content of the response: {res.content} - {e!s}")
                elif res.status_code == UNAUTHORIZED_STATUS_CODE:
                    sleep_time = uniform(1, 10)
                    demisto.debug(f"Got status code 401 on stream discovery, going to sleep for {sleep_time} - {res.content!s}")
                    ...
                    demisto.debug(
                        f"Rate limit exceeded, going to sleep for {time_to_wait} seconds and then retry. "
                        f"Response headers: {res.headers!s} "
                        f"Response body: {res.content!s}"
                    )
                    demisto.updateModuleHealth(f"Rate limit exceeded, going to sleep for {time_to_wait} and then retry.")
    ...
    async def fetch_event(
        ...
        while True:
            try:
                ...
                self.client = Client(base_url=self.base_url, app_id=self.app_id, verify_ssl=self.verify_ssl, proxy=self.proxy)
                ...
                async with ClientSession(
                    ...
                    headers={"Authorization": f"Token {self.session_token}", "Connection": "keep-alive"},
                            ...
                            for line in lines[:-1]:
                                ...
                                if stripped_line:
                                    ...
                                    try:
                                        ...
                                        demisto.debug(f"Failed decoding event (skipping it) - {stripped_line!s}")
                                else:
                                    new_lines_fetched += 1
                ...
                demisto.debug(
                    f"An error occurred in the fetch event loop: {e} - {traceback.format_exc()}. "
...
class RefreshToken:
    ...
    async def get_access_token(self) -> str:
        ...
        max_retries = 3
        async with ClientSession(
            connector=TCPConnector(ssl=self.verify_ssl), headers=TOKEN_RETRIEVAL_HEADERS, trust_env=self.proxy
        ) as session:
                ...
                async with session.post(f"{self.base_url}/oauth2/token", data=data) as res:
                    if res.status == TOO_MANY_REQUESTS_STATUS_CODE:
                        ...
                        try:
                            ...
                            raise RuntimeError(f"Failed to decode successful token retrieval response: {res.content!s}")
                    else:
                        try:
                            body = await res.json()
                            error = body.get("errors", [{}])
                            error_message = error[0].get("message", "")
                            raise RuntimeError(f"Failed to retrieve token, verify client details are correct: {error_message}")
                        except json.decoder.JSONDecodeError:
                            raise RuntimeError(f"Failed to decode token retrieval failure response: {res.content!s}")
            if not body:
                raise RuntimeError(f"Failed to retrieve token - got empty response: {res.content!s}")
...
async def init_refresh_token(
    base_url: str, client_id: str, client_secret: str, verify_ssl: bool, proxy: bool
...
def get_sample_events(store_samples: bool = False) -> None:
    ...
    if sample_events:
        ...
        if not store_samples:
            output += (
                ' The "Store sample events for mapping" integration parameter '
...
def main():
    ...
    try:
        ...
        error_msg = f"Error in CrowdStrike Falcon Streaming v2: {e!s}"
