...
Rules that parse and model raw syslog and CEF Cisco Firepower logs ingested via the Broker VM in Cortex XSIAM.

* The ingested Cisco Firepower logs can be queried in XQL Search using the *`cisco_firepower_raw`* dataset.

## Centralized Management Capabilities

Cisco Firepower integration for unified management of firewalls, application control, intrusion prevention, URL filtering, and advanced malware protection.
Additional setup information and supported actions are available on the integration page.

## Supported event IDs (raw syslog)
...
Configure the Broker VM to receive Cisco Firepower logs. For more information on creating or configuring the Broker VM, see [Set up and configure Broker VM](https://cortex-docs.paloaltonetworks.com/cortex-xdr-5.x/configure-cortex-xdr/data-management/broker-vm/set-up-and-configure-broker-vm).
...
4. When configuring the Syslog Collector, set the following parameters:
    ...
    | Parameter    | Value                                                                                                                       |
    |:-------------|:----------------------------------------------------------------------------------------------------------------------------|
    | `Protocol`   | Select **UDP** for the default forwarding, **TCP** or **Secure TCP** (depends on the protocol you configured in Cisco Firepower). |
    | `Port`       | Enter the syslog service port that Cortex XSIAM Broker VM should listen on for receiving forwarded events from Cisco Firepower.   |
    | `Format`     | Enter CEF  (needed only for CEF). |
    | `Vendor`     | Enter Cisco (not needed for CEF). |
    | `Product`    | Enter Firepower (not needed for CEF).|
