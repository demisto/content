...
def http_request(query):
    ...
    if r.status_code != 200:
        return_error(f"Error in API call to MaxMind, got status code - {r.status_code} and a reason: {r.reason}")
    return r


def create_map_entry(lat, lng):
    demisto.results({"Type": entryTypes["map"], "ContentsFormat": formats["json"], "Contents": {"lat": lat, "lng": lng}})
