...
def test_filter_creator():
    ...
    check = {"FieldPath": [3881], "FilterType": "3", "Value": "Blue"}
...
def test_update_field_integration_context(mocker, requests_mock):
    ...
    from Lockpath_KeyLight_v2 import INTEGRATION_CONTEXT_SIZE, Client

    client = Client("http://example.com", False, False, headers={"Accept": "application/json"})
    mocker.patch.object(demisto, "setIntegrationContext")
    requests_mock.get(
        "http://example.com/ComponentService/GetFieldList",
        json=[
            {
                "Id": 2260,
    ...
    assert demisto.getIntegrationContext().get("1").get("fields") == {
        "1501": "Auto-Apply Status",
...
def test_field_output_to_hr_fields(mocker, requests_mock):
    ...
    assert client.field_output_to_hr_fields(
        [{"Key": "2260", "Value": "check authentication"}, {"Key": "2261", "Value": "check new field"}], "1"
...
def test_string_to_key_value(mocker, requests_mock):
    ...
    mocker.patch.object(
        ...
        return_value={"10359": {"fields": {"1": "Audit Project", "2": "Task ID", "3": "Updated At"}}},
    )

    json = [
        {"fieldName": "Task ID", "value": "Updated by Demisto Test Playbook", "isLookup": False},
        {"fieldName": "Audit Project", "value": 3, "isLookup": True},
    ]
    field_value = client.string_to_FieldValues(json, "10359")
    assert field_value == [{"Key": "2", "Value": "Updated by Demisto Test Playbook"}, {"Key": "1", "Value": {"Id": 3}}]
