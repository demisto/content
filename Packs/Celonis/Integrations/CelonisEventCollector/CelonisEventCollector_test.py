...
def create_client():
    return Client(base_url=MOCK_BASEURL, verify=False, client_id=MOCK_CLIENT_ID, client_secret=MOCK_CLIENT_SECRET)
...
def test_sort_events_by_timestamp(mocker):
    ...
    assert all(sorted_events[i]["timestamp"] <= sorted_events[i + 1]["timestamp"] for i in range(len(sorted_events) - 1))
