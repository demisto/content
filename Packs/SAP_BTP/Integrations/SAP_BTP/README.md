...
- **mTLS (Mutual TLS)** - Recommended. Uses client certificates for secure authentication.
- **Non-mTLS** - Uses client credentials (Client ID and Client Secret) for authentication.
...
1. **Use mTLS Authentication**: For enhanced security, we recommend using mTLS authentication with client certificates.
2. **Configure Appropriate Limits**: Set the maximum number of events per fetch based on your organization's event volume.
3. **Regular Monitoring**: Periodically review the collected events to ensure proper integration operation.
