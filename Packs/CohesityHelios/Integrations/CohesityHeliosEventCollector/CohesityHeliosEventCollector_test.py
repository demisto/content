...
def test_dedup_elements():
    from CohesityHeliosEventCollector import ALERT_TIME_FIELD, AUDIT_LOGS_TIME_FIELD, adjust_and_dedup_elements
    ...
    """
    new_elements = [
        {"id": "1", "latestTimestampUsecs": 1704096000000000},
        {"id": "2", "latestTimestampUsecs": 1704182400000000},
        {"id": "3", "latestTimestampUsecs": 1704268800000000},
    ]
    existing_element_ids = ["1", "2", "3"]
    deduped_elements = adjust_and_dedup_elements(
        new_elements=new_elements, existing_element_ids=existing_element_ids, time_field_name=""
    )
    ...
    We expect the result list to have no elements at all and that the final list length was not changed.
    """
    new_elements = [{"id": "2", "latestTimestampUsecs": 1704182400000000}, {"id": "3", "latestTimestampUsecs": 1704268800000000}]
    existing_element_ids = ["1", "2", "3"]
    deduped_elements = adjust_and_dedup_elements(
        new_elements=new_elements, existing_element_ids=existing_element_ids, time_field_name=""
    )
    ...
    deduped_elements = adjust_and_dedup_elements(
        new_elements=new_elements, existing_element_ids=existing_element_ids, time_field_name=ALERT_TIME_FIELD
    )
    assert deduped_elements == [
        {"id": "2", "latestTimestampUsecs": 1704182400000000, "_time": "2024-01-02T08:00:00.000Z"},
    ...
    """
    new_elements = [
        {"id": "1", "timestampUsecs": 1704096000000000},
        {"id": "2", "timestampUsecs": 1704182400000000},
        {"id": "3", "timestampUsecs": 1704268800000000},
    ]
    existing_element_ids = []
    deduped_elements = adjust_and_dedup_elements(
        new_elements=new_elements, existing_element_ids=existing_element_ids, time_field_name=AUDIT_LOGS_TIME_FIELD
    )
    assert deduped_elements == [
        {"id": "1", "timestampUsecs": 1704096000000000, "_time": "2024-01-01T08:00:00.000Z"},
...
def test_get_earliest_event_ids_with_the_same_time():
    from CohesityHeliosEventCollector import ALERT_TIME_FIELD, AUDIT_LOGS_TIME_FIELD, get_earliest_event_ids_with_the_same_time
    ...
    """
    events = [{"timestampUsecs": "3", "id": "c"}, {"timestampUsecs": "2", "id": "b"}, {"timestampUsecs": "1", "id": "a"}]
    earliest_event_fetched_ids = get_earliest_event_ids_with_the_same_time(events=events, time_field=time_field)
    assert earliest_event_fetched_ids == ["a"]
...
class TestFetchEventsCommand:
    ...
    def load_response(event_type) -> dict:
        ...
        filename = (
            "test_data/CohesityHeliosEventCollector-AuditLogList.json"
            if event_type == EventType.audit_log
            else "test_data/CohesityHeliosEventCollector-AlertList.json"
        )
    ...
    def test_fetch_events_command_case_2(self, requests_mock, mocker, audit_logs_mock_res, alerts_mock_res):
        ...
        mocker.patch("CohesityHeliosEventCollector.arg_to_datetime", return_value=arg_to_datetime(self.mock_time))
        audit_logs_call = requests_mock.get(
            f"{self.base_url}/{self.audit_logs_endpoint}", [{"json": audit_logs_mock_res[0]}, {"json": audit_logs_mock_res[2]}]
        )
        alerts_call = requests_mock.get(
            f"{self.base_url}/{self.alerts_endpoint}", [{"json": alerts_mock_res[0]}, {"json": alerts_mock_res[2]}]
        )
    ...
    def test_fetch_events_command_case_3(self, requests_mock, mocker, audit_logs_mock_res, alerts_mock_res):
        ...
        mocker.patch("CohesityHeliosEventCollector.arg_to_datetime", return_value=arg_to_datetime(self.mock_time))
        audit_logs_call = requests_mock.get(
            f"{self.base_url}/{self.audit_logs_endpoint}", [{"json": audit_logs_mock_res[0]}, {"json": audit_logs_mock_res[1]}]
        )
        alerts_call = requests_mock.get(
            f"{self.base_url}/{self.alerts_endpoint}", [{"json": alerts_mock_res[0]}, {"json": alerts_mock_res[1]}]
        )
