...
class Client(BaseClient):
    ...
    def table_list_with_id_request(
        self,
        list_id: str,
        entity: str = None,
        entity_id: str = None,
    ...
    def service_record_list_request(
        self,
        record_type: str,
        fields: str = None,
        offset: int = None,
    ...
    def service_record_search_request(
        self,
        record_type: str,
        query: str,
        fields: str = None,
    ...
    def service_record_close_request(self, id_: str, solution: str = None):
        ...
        response = self._http_request(
            "PUT", f"sr/{id_}/close", json_data=data, cookies=self._cookies, resp_type="response", ok_codes=(200, 400)
        )
    ...
    def service_record_create_request(
        self, record_type: str, info: List[dict[str, str]], fields: str = None, template_id: str = None
    ):
        params = assign_params(fields=fields, type=record_type, template=template_id)
        data = {"info": info}
    ...
    def service_record_delete_request(self, ids: str, solution: str = None):
        ...
        response = self._http_request(
            "DELETE", "sr", params=params, json_data=data, cookies=self._cookies, resp_type="response", ok_codes=(200, 400)
        )
        return response
    ...
    def service_record_delete_file_request(self, sr_id: str, file_id: str):
        ...
        response = self._http_request(
            "DELETE", f"sr/{sr_id}/attachment", json_data=data, cookies=self._cookies, resp_type="response"
        )
    ...
    def service_record_add_note_request(self, sr_id: str, note: str, username: str):
        ...
        data = {
            "id": f"{sr_id}",
            "info": [{"key": "notes", "value": [{"userName": f"{username}", "createDate": f"{now_ms}", "text": f"{note}"}]}],
        }
...
def create_readable_response(
    responses: Union[dict, List[dict], str], handle_one_response: Callable, remove_if_null: str = None
    ...
    for response in responses:
        if "info" in response:
            for response_info in response["info"]:
                response_entry = {}  # type: ignore
                for key in TEMPLATE_OUTPUTS:
                    response_entry[key] = response_info.get(key, None)
...
def get_service_record_update_time(service_record: dict[str, Any], use_classic_date_format: bool = False) -> Optional[datetime]:
    ...
    The SysAid Classic version sends dates in non-ISO format, instead of the declared ISO format,
    which can be ambiguous when day <= 12.
...
def service_record_to_incident_context(service_record: dict[str, Any], use_classic_date_format: bool = False):
    ...
    demisto.debug(
        f'New service record {record_type} is: name: {incident_context["name"]}, occurred: '
...
def table_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    command_results = CommandResults(
        ...
        readable_output=tableToMarkdown(
            f'List {f"ID {list_id}" if list_id else entity or "sr"} Results:',
            response,
            headers=headers,
            removeNull=True,
...
def asset_search_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    command_results = CommandResults(
        ...
        outputs=response,
        raw_response=response,
        readable_output=create_paging_header(limit, page_number)
        + tableToMarkdown("Asset Results:", readable_response, headers=headers, removeNull=True, headerTransform=pascalToSpace),
    )
...
def filter_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    command_results = CommandResults(
        ...
        raw_response=response,
        readable_output=tableToMarkdown(
            "Filter Results:", readable_response, headers=headers, removeNull=True, headerTransform=pascalToSpace
        ),
    )
...
def user_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    command_results = CommandResults(
        ...
        raw_response=response,
        readable_output=create_paging_header(limit, page_number)
        + tableToMarkdown("Filter Results:", response, headers=headers, removeNull=True),
    )
...
def user_search_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    command_results = CommandResults(
        ...
        raw_response=response,
        readable_output=create_paging_header(limit, page_number)
        + tableToMarkdown("User Results:", response, headers=headers, removeNull=True),
    )
...
def service_record_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    command_results = CommandResults(
        ...
        readable_output=create_paging_header(limit, page_number)
        + tableToMarkdown(
            "Service Record Results:", readable_response, headers=headers, removeNull=True, headerTransform=pascalToSpace
        ),
    )
...
def service_record_update_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    command_results = CommandResults(outputs_prefix="SysAid.ServiceRecord", readable_output=msg)
...
def service_record_template_get_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    command_results = CommandResults(
        ...
        raw_response=response,
        readable_output=tableToMarkdown(
            "Service Record Results:", readable_response, headers=TEMPLATE_OUTPUTS, removeNull=True, headerTransform=pascalToSpace
        ),
    )
    ...
    if params["isFetch"]:
        ...
        if max_fetch is not None and (max_fetch > MAX_INCIDENTS_TO_FETCH or max_fetch <= 0):
            raise DemistoException(
                f"Maximum number of service records to fetch exceeds the limit "
        ...
        if statuses_set - STATUSES:
            raise DemistoException(
                f"Statuses {statuses_set - STATUSES} were given and are not legal statuses. "
        ...
        client.service_record_list_request(
            record_type=fetch_types, limit=max_fetch, archive=int(include_archived), filters=filters
        )
