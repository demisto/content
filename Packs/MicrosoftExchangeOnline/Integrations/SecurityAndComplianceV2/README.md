...
3. Authentication / Authorization methods:
   ...
   3. Run the ***!o365-sc-auth-start*** command and follow the instructions. Expected output is:
   ...
   4. Test - OAuth2.0 authorization, Run the ***!o365-sc-auth-test*** command. 
...
| --- | --- | --- |
| search_name | The name of the compliance search. | Required | 
| action | Search action to perform. Possible values are: "Preview" and "Purge". Default is "Preview". Possible values are: Preview, Purge, Export. Default is Preview. | Optional | 
| purge_type | Purge type. Possible values are: "Soft Delete" and "HardDelete". Default is "SoftDelete". Possible values are: SoftDelete, HardDelete. Default is SoftDelete. | Optional | 
| share_point_archive_format | Specifies how to export SharePoint and OneDrive search results. IndividualMessage: Export the files uncompressed. This is the default value. PerUserZip: One ZIP file for each user. Each ZIP file contains the exported files for the user. SingleZip: One ZIP file for all users. The ZIP file contains all exported files from all users. This output setting is available only in PowerShell. To specify the format for Exchange search results, use the exchange_archive_format parameter. Possible values are: IndividualMessage, PerUserZip, SingleZip. | Optional | 
...
| include_sharepoint_document_versions | Specifies whether to export previous versions of the document when you use the Export switch. Possible values are: true, false. | Optional | 
| notify_email | Specifies the email address target for the search results when you use the export switch. | Optional | 
| notify_email_cc | Specifies the cc email address target for the search results when you use the export switch. | Optional | 
| scenario | Specifies the scenario type when you use the Export switch. Possible values are: AnalyzeWithZoom, General, GenerateReportsOnly, Inventory, RetentionReports, TriagePreview. | Optional | 
