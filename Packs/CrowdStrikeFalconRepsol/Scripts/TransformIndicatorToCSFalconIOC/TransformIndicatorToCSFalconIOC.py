...
def convert_unique_fields(ioc: dict, action: str, host_groups: list, platforms: list, applied_globally: bool):
    ...
    ioc["severity"] = DBOT_SCORE_TO_CS_FALCON_SEVERITY[ioc.get("severity")]  # type: ignore
...
def get_indicators_by_query():
    ...
    if indicators:
        for indicator in indicators:
            ...
            cs_falcon_ioc = {
                INDICATOR_FIELDS_TO_CS_FALCON_IOC[indicator_field]: indicator_value
...
def main():
    try:
        ...
        return_error(f"Failed to execute TransformIndicatorToCSFalconIOC. Error: {ex!s}")
