...
def get_indicators(
    ...
    filters: dict[str, Any] = {
        "mode": "and",
        "filters": [{"key": "entity_type", "values": indicator_type, "operator": "eq", "mode": "or"}],
        "filterGroups": [],
    }
    if score:
    ...
    observables = client.stix_cyber_observable.list(filters=filters, after=last_run_id, first=limit, withPagination=True)
...
def fetch_indicators_command(
    client: OpenCTIApiClient, indicator_types: list, max_fetch: int, score: list = None, tlp_color=None, tags=None, is_test=False
) -> list:
    ...
    new_last_run, indicators_list = get_indicators(
        client, indicator_types, limit=max_fetch, last_run_id=last_run_id, tlp_color=tlp_color, score=score, tags=tags
    )
...
def get_indicators_command(client: OpenCTIApiClient, args: dict) -> CommandResults:
    ...
    last_run_id, indicators_list = get_indicators(
        client=client, indicator_types=indicator_types, limit=limit, last_run_id=last_run_id, score=score
    )
    ...
    if indicators_list:
        ...
        readable_output = tableToMarkdown("Indicators", indicators, headers=["type", "value", "id"], removeNull=True)
        return CommandResults(readable_output=readable_output, raw_response=indicators_list)
    else:
...
def main():
    ...
    if start or end:
        score = [str(i) for i in range(start, end)]  # type:ignore
    try:
        client = OpenCTIApiClient(base_url, api_key, ssl_verify=params.get("insecure"), log_level="error", proxies=handle_proxy())
