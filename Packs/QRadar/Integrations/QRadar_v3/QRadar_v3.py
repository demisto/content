...
def get_modified_remote_data_command(client: Client, params: dict[str, str],
    ...
    # if this call fails, raise an error and stop command execution
    filter_query = f'id > {highest_fetched_id} AND ((status!=closed AND last_persisted_time >= {last_update}) OR (status=closed AND close_time > {last_update}))'  # noqa: E501
    print_debug_msg(f'Query to get modified offenses: {filter_query}')
    offenses = client.offenses_list(range_=range_,
                                    filter_=filter_query,  # noqa: E501
