...
3. Click **Add instance** to create and configure a new integration instance.
    ...
    | Query to fetch offenses - configuring "status=OPEN" won't mirror closed offenses to XSOAR platform.| Define a query to determine which offenses to fetch. E.g., "severity >= 4 AND id > 5". filtering by status in the query may result in unexpected behavior when changing an incident's status.| False |
