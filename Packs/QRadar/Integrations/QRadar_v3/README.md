...
In case of setting a query with the intent to fetch specific offenses or older offenses, make sure that the *First fetch timestamp* parameter is set to a value that covers the desired time range for the offenses to be fetched, if using an already existing instance that has performed fetch, run the ***qradar-reset-last-run*** command to before starting the fetch with the new query.
