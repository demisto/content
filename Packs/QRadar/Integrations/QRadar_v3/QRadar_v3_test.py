...
class TestGetDomainNamesEnhanced:
    """
    Test suite for the enhanced get_domain_names function with retry logic and improved error handling.
    """

    @pytest.fixture
    def mock_client(self, mocker):
        """Create a mock QRadar client for testing."""
        client = mocker.Mock()
        return client

    @pytest.fixture
    def sample_outputs(self):
        """Sample offense outputs with domain IDs."""
        return [
            {"domain_id": 5, "offense_name": "offense1"},
            {"domain_id": 6, "offense_name": "offense2"},
            {"domain_id": 7, "offense_name": "offense3"},
        ]

    @pytest.fixture
    def sample_domains_response(self):
        """Sample successful domains API response."""
        return [{"id": 5, "name": "PRV"}, {"id": 6, "name": "DEV"}, {"id": 7, "name": "TEST"}]

    def test_get_domain_names_success(self, mock_client, sample_outputs, sample_domains_response, mocker):
        """
        Given:
         - List of outputs with domain IDs
         - Successful API response from domains_list

        When:
         - get_domain_names is called

        Then:
         - Ensure domain mapping is returned correctly
         - Ensure debug logs are generated
         - Ensure no retry attempts are made
        """
        # Mock the domains_list API call to return successful response
        mock_client.domains_list.return_value = sample_domains_response

        # Mock demisto debug logging
        mock_debug = mocker.patch("QRadar_v3.demisto.debug")

        # Execute function
        result = QRadar_v3.get_domain_names(mock_client, sample_outputs)

        # Assertions
        expected_mapping = {5: "PRV", 6: "DEV", 7: "TEST"}
        assert result == expected_mapping

        # Verify API call was made correctly
        mock_client.domains_list.assert_called_once_with(filter_="id in (5,6,7)", fields="id,name")

        # Verify debug logging
        assert mock_debug.call_count >= 2
        mock_debug.assert_any_call("Attempting to resolve domain names for domain IDs: 5,6,7")
        mock_debug.assert_any_call("Successfully resolved 3 domain names: {5: 'PRV', 6: 'DEV', 7: 'TEST'}")

    def test_get_domain_names_empty_input(self, mock_client, mocker):
        """
        Given:
         - Empty list of outputs or outputs without domain_id

        When:
         - get_domain_names is called

        Then:
         - Ensure empty dict is returned
         - Ensure debug log indicates no domain IDs found
        ...
        result = QRadar_v3.get_domain_names(mock_client, [])
        assert result == {}

        # Test with outputs without domain_id
        outputs_without_domain = [{"offense_name": "test"}]
        result2 = QRadar_v3.get_domain_names(mock_client, outputs_without_domain)
        assert result2 == {}

        # Verify no API calls were made
        mock_client.domains_list.assert_not_called()

        # Verify debug logging
        mock_debug.assert_called_with("No domain IDs found in outputs for domain name enrichment")

    def test_get_domain_names_empty_api_response(self, mock_client, sample_outputs, mocker):
        """
        Given:
         - List of outputs with domain IDs
         - Empty API response from domains_list
        ...
        Then:
         - Ensure empty dict is returned
         - Ensure warning log is generated
         - Ensure no retries are attempted
        """
        # Mock the domains_list API call to return empty response
        mock_client.domains_list.return_value = []

        # Mock demisto logging
        mock_debug = mocker.patch("QRadar_v3.demisto.debug")
        ...
        mock_client.domains_list.assert_called_once()

        # Verify debug logging
        mock_debug.assert_called_with("Domain list API returned empty response for domain IDs: 5,6,7")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (DemistoException, "QRadar API error"),
            (Exception, "Unexpected error"),
        ],
    )
    def test_get_domain_names_retry_with_eventual_success(
        self, mock_client, sample_outputs, sample_domains_response, mocker, exception_type, exception_message
    ):
        """
        Given:
         - List of outputs with domain IDs
         - API call fails twice then succeeds on third attempt

        When:
         - get_domain_names is called

        Then:
         - Ensure domain mapping is returned after retries
         - Ensure appropriate warning logs for retries
         - Ensure sleep is called between retries
         - Ensure final success debug log
        """
        # Mock the domains_list to fail twice, then succeed
        mock_client.domains_list.side_effect = [
            exception_type(exception_message),  # First failure
            exception_type(exception_message),  # Second failure
            sample_domains_response,  # Success on third try
        ]

        # Mock demisto logging and sleep
        mock_debug = mocker.patch("QRadar_v3.demisto.debug")
        ...
        assert mock_client.domains_list.call_count == 3

        # Verify retry debug logs
        assert mock_debug.call_count == 7
        mock_debug.assert_any_call(f"Domain name resolution attempt 1/5 failed: {exception_message}. Retrying in 1 seconds...")
        mock_debug.assert_any_call(f"Domain name resolution attempt 2/5 failed: {exception_message}. Retrying in 2 seconds...")

        # Verify sleep was called with exponential backoff
        mock_sleep.assert_any_call(1)  # First retry delay
        mock_sleep.assert_any_call(2)  # Second retry delay

        # Verify final success debug log
        mock_debug.assert_any_call("Successfully resolved 3 domain names: {5: 'PRV', 6: 'DEV', 7: 'TEST'}")

    def test_get_domain_names_all_retries_exhausted(self, mock_client, sample_outputs, mocker):
        """
        Given:
         - List of outputs with domain IDs
         - API call fails for all retry attempts
        ...
        Then:
         - Ensure empty dict is returned
         - Ensure all retry attempts are made
         - Ensure appropriate error logs
         - Ensure sleep is called with exponential backoff
        """
        exception_message = "Persistent API failure"

        # Mock the domains_list to always fail
        mock_client.domains_list.side_effect = requests.ConnectionError(exception_message)

        # Mock demisto logging and sleep
        mock_debug = mocker.patch("QRadar_v3.demisto.debug")
        mock_error = mocker.patch("QRadar_v3.demisto.error")
        mock_info = mocker.patch("QRadar_v3.demisto.info")
        mock_sleep = mocker.patch("time.sleep")
        ...
        # Verify API was called 5 times (max retries)
        assert mock_client.domains_list.call_count == 5

        # Verify debug logs for retries (4 retries, last attempt doesn't log retry warning)
        assert mock_debug.call_count == 10
        expected_retry_warnings = [
            f"Domain name resolution attempt 1/5 failed: {exception_message}. Retrying in 1 seconds...",
            f"Domain name resolution attempt 2/5 failed: {exception_message}. Retrying in 2 seconds...",
            f"Domain name resolution attempt 3/5 failed: {exception_message}. Retrying in 4 seconds...",
            f"Domain name resolution attempt 4/5 failed: {exception_message}. Retrying in 8 seconds...",
        ]

        for warning_msg in expected_retry_warnings:
            mock_debug.assert_any_call(warning_msg)

        # Verify sleep was called with correct exponential backoff
        expected_sleep_calls = [1, 2, 4, 8]  # 2^0, 2^1, 2^2, 2^3
        for delay in expected_sleep_calls:
            mock_sleep.assert_any_call(delay)

        # Verify final error logs
        mock_error.assert_any_call(
            "Domain name resolution attempt 5/5 failed: Persistent API failure. All retry attempts exhausted."
        )
        mock_error.assert_any_call(
            "Failed to resolve domain names after 5 attempts for domain IDs: 5,6,7. " "Last error: Persistent API failure"
        )
        mock_info.assert_called_once_with("Falling back to using domain IDs instead of domain names for affected offenses")

    def test_get_domain_names_mixed_domain_ids(self, mock_client, mocker):
        """
        Given:
         - List of outputs with some valid domain IDs and some None values

        When:
         - get_domain_names is called

        Then:
         - Ensure only valid domain IDs are included in API call
         - Ensure correct domain mapping is returned
        """
        outputs_mixed = [
            {"domain_id": 5, "offense_name": "offense1"},
            {"domain_id": None, "offense_name": "offense2"},  # None domain_id
            {"offense_name": "offense3"},  # Missing domain_id
            {"domain_id": 6, "offense_name": "offense4"},
        ]

        domains_response = [{"id": 5, "name": "PRV"}, {"id": 6, "name": "DEV"}]

        mock_client.domains_list.return_value = domains_response

        # Execute function
        result = QRadar_v3.get_domain_names(mock_client, outputs_mixed)

        # Assertions
        expected_mapping = {5: "PRV", 6: "DEV"}
        assert result == expected_mapping

        # Verify API call excludes None/missing domain IDs
        mock_client.domains_list.assert_called_once_with(
            filter_="id in (5,6)",  # Only valid domain IDs
            fields="id,name",
        )

    def test_get_domain_names_partial_api_response(self, mock_client, sample_outputs, mocker):
        """
        Given:
         - List of outputs with domain IDs 5, 6, 7
         - API response only returns domains for IDs 5, 6 (missing 7)

        When:
         - get_domain_names is called

        Then:
         - Ensure mapping only includes returned domains
         - Ensure success debug log reflects actual count
        """
        partial_response = [
            {"id": 5, "name": "PRV"},
            {"id": 6, "name": "DEV"},
        ...
        mock_client.domains_list.return_value = partial_response
        mock_debug = mocker.patch("QRadar_v3.demisto.debug")
        ...
        # Assertions
        expected_mapping = {5: "PRV", 6: "DEV"}  # Only available domains
        assert result == expected_mapping

        # Verify debug log shows actual count
        mock_debug.assert_any_call("Successfully resolved 2 domain names: {5: 'PRV', 6: 'DEV'}")
