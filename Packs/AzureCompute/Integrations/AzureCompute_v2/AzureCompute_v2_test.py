...
import pytest
from AzureCompute_v2 import (
    MsGraphClient,
    assign_image_attributes,
    create_nic_command,
    create_vm_parameters,
    get_all_public_ip_details_command,
    get_network_interface_command,
    get_public_ip_details_command,
    get_single_ip_details_from_list_of_ip_details,
    get_vm_command,
    list_vms_command,
    screen_errors,
)
...
Expected_VM_PARAMS = {
    ...
    "properties": {
        ...
        "storageProfile": {
            "imageReference": {"sku": "18.04-LTS", "publisher": "Canonical", "version": "latest", "offer": "UbuntuServer"},
            "osDisk": {
                "caching": "ReadWrite",
        ...
        "osProfile": {"adminUsername": "Admin", "computerName": "TestVM", "adminPassword": "password"},
        "networkProfile": {
            "networkInterfaces": [
                {
...
SINGLE_VM_EC = {
    "Azure.Compute(val.Name && val.Name == obj.Name)": {
        ...
        "Location": "westeurope",
        "ProvisioningState": "Succeeded",
        "ResourceGroup": "fake-rg-1",
        "Tags": {"owner": "fake@email.com", "env": "dev"},
    }
}
...
INTERFACE_EC = {
    "Azure.Network.Interfaces(val.ID && val.ID == obj.ID)": {
        ...
        "DNSSuffix": None,
        "IPConfigurations": [
            {"ConfigName": "ipconfig1", "ConfigID": "nic_id", "PrivateIPAddress": "10.0.0.4", "PublicIPAddressID": None}
        ],
    }
}
...
PUBLIC_IP_DETAILS_LIST_ENTRY_EC = {
    ...
    "properties": {
        ...
        "dnsSettings": {"domainNameLabel": "tesdomain", "fqdn": "webserver.eastus.cloudapp.azure.com"},
        "ipTags": [],
        "ipConfiguration": {
...
@pytest.mark.parametrize(
    "error_message, tenant, expected_error_message", [("Error Message tenant_id", "tenant_id", "Error Message <xxxxxxxxx>")]
)
...
@pytest.mark.parametrize("args, expected_parameters", [(CREATE_VM_PARAMS_ARGS, Expected_VM_PARAMS)])
...
def test_create_nic_command(mocker):
    ...
    command_results = create_nic_command(
        client,
        {
            "resource_group": "resource_group",
            "nic_name": "test-nic100",
            "nic_location": "eastus",
            "vnet_name": "subnet_id",
