...
def get_authentication_token(scope=None):
    ...
    # Token claims
    claims = {"exp": epoch_timeout, "iat": epoch_time, "iss": "http://cylance.com", "sub": APP_ID, "tid": TID, "jti": token_id}
...
def threat_to_incident(threat):
    ...
    labels = [
        {"type": "Classification", "value": threat["classification"]},
        {"type": "MD5", "value": threat["md5"]},
        {"type": "SHA256", "value": threat["sha256"]},
        {"type": "ThreatLastFound", "value": threat["last_found"]},
...
def get_devices():
    ...
    for device in devices:
        ...
        endpoint_context.append(
            {"IPAddress": device["ip_addresses"], "MACAdress": device["mac_addresses"], "Hostname": device["name"]}
        )
...
def get_policies():
    ...
    ec = {"CylanceProtect.Policies(val.id && val.id === obj.id)": context_policies}
...
def create_zone_request(name, policy_id, criticality):  # pragma: no cover
    ...
    body = {"name": name, "policy_id": policy_id, "criticality": criticality}
...
def get_zones():
    ...
    ec = {"CylanceProtect.Zones(val.Id && val.Id === obj.Id)": context_zones}
...
def get_zone():
    ...
    ec = {"CylanceProtect.Zones(val.Id && val.Id === obj.Id)": context_zone}
...
def create_dbot_score_entry(threat, dbot_score):
    dbot_score_entry = Common.DBotScore(
        threat.get("sha256"), DBotScoreType.FILE, integration_name="Cylance Protect", score=dbot_score
    )
...
def get_threat_devices():
    ...
    if threats:
        ...
        for threat in threats_context:
            endpoint_context.append(
                {"Hostname": threat["name"], "IPAddress": threat["ip_addresses"], "MACAddress": threat["mac_addresses"]}
            )
            current_device = {
        ...
        file_context = {"SHA256": threat_hash, "Path": file_paths}

        ec = {
            "File": file_context,
            "Endpoint(val.Hostname && val.Hostname === obj.Hostname)": endpoint_context,
            "CylanceProtect.Threat(val.SHA256 && val.SHA256 === obj.SHA256)": {"SHA256": threat_hash, "Devices": devices_context},
        }
...
def get_list_entry_by_hash(sha256=None, list_type_id=None):
    ...
    if demisto.command() == "cylance-protect-get-list-entry":
        if found_hash:
            ...
            ec = {"CylanceListSearch": context_list}
...
def update_device_threats_request(device_id, threat_id, event):  # pragma: no cover
    ...
    body = {"threat_id": threat_id, "event": event}
    ...
    demisto.results(
        {
            ...
            "HumanReadable": tableToMarkdown(
                "Cylance Protect - Downloading threat attached to the following hash: " + sha256, contents
            ),
...
def add_hash_to_list():
    ...
    contents = {"Threat File SHA256": sha256, "List Type": list_type, "Category": category, "Reason": reason}

    context[outputPaths["file"]] = {"SHA256": sha256, "Cylance": {"ListType": list_type, "Category": category}}
...
def add_hash_to_list_request(sha256, list_type, reason, category=None):  # pragma: no cover
    ...
    body = {"sha256": sha256, "list_type": list_type, "reason": reason}
    if category:
...
def delete_hash_from_lists():
    ...
    contents = {"Threat File SHA256": sha256, "Threat List Type": list_type}

    context[outputPaths["file"]] = {"SHA256": sha256, "Cylance": {"ListType": list_type}}
...
def delete_hash_from_lists_request(sha256, list_type):  # pragma: no cover
    ...
    body = {"sha256": sha256, "list_type": list_type}
...
def delete_devices():
    ...
    for device_id in device_ids_list:
        ...
        context_list.append({"Id": device_id, "Name": device_name, "Deleted": True})
        contents.append({"Device Removed": device_id, "Device Name": device_name, "Deletion status": True})
...
def get_policy_details():
    ...
    title_memory_exclusion = (
        "Cylance Policy Details - Memory Violation Actions \n"
    ...
    title_additional_settings = (
        "Cylance Policy Details - Policy Settings. \n" + "Various policy settings are contained within this section."
    )
    ...
    if policy_details:
        ...
        context = {"ID": policy_details.get("policy_id"), "Name": policy_details.get("policy_name"), "Timestamp": date_time}

        contents = {"Policy Name": policy_details.get("policy_name"), "Policy Created At": date_time}

        suspicious_files = policy_details.get("filetype_actions").get("suspicious_files")
        if suspicious_files:
            suspicious_files_list = []
            for file in suspicious_files:
                suspicious_files_list.append({"Actions": file.get("actions"), "File Type": file.get("file_type")})

        threat_files = policy_details.get("filetype_actions").get("threat_files")
        if threat_files:
            threat_files_list = []
            for file in threat_files:
                threat_files_list.append({"Actions": file.get("actions"), "File Type": file.get("file_type")})
        ...
        for memory_violation in memory_violations:
            memory_violations_content.append(
                {"Action": memory_violation.get("action"), "Violation Type": memory_violation.get("violation_type")}
            )

        additional_settings = policy_details.get("policy")
        additional_settings_content = []
        for additional_setting in additional_settings:
            additional_settings_content.append({"Name": additional_setting.get("name"), "Value": additional_setting.get("value")})
...
def create_instaquery():
    ...
    if res:
        ...
        results = CommandResults(outputs=res, outputs_prefix="InstaQuery.New", outputs_key_field="id")
        return_results(results)
...
def get_instaquery_result():
    ...
    if res["result"]:
        ...
        result_title = (
            str(results_count) + " results found, find more details in context. Here is the 1st result:"
            if results_count > 1
            else "1 result found:"
        )
        readable_results = tableToMarkdown(result_title, json.loads(res["result"][0]["Result"]).get("Properties"))
    else:
        readable_results = "### No result found"

    # Return results to context and war room
    results = CommandResults(
        outputs=res, outputs_prefix="InstaQuery.Results", outputs_key_field="id", readable_output=readable_results
    )
    return_results(results)
...
def main():  # pragma: no cover
    ...
    try:
        ...
        demisto.results({"Type": 11, "Contents": str(w), "ContentsFormat": formats["text"]})
