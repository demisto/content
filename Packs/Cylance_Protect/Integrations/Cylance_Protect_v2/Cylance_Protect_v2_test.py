...
from Cylance_Protect_v2 import (
    create_dbot_score_entry,
    translate_score,
    get_device,
...
EXPECTED_POLICIES = {
    "DateAdded": "2020-04-13T10:32:43.5072251",
    "Name": "fff",
    "ZoneCount": 1,
...
ZONE_OUTPUT = {
    "date_created": "2022-02-03T15:52:30.4108727Z",
...
POLICY_OUTPUT = {
    "memoryviolation_actions": {"memory_violations": [], "memory_exclusion_list": [], "memory_violations_ext": []},
    "logpolicy": {"log_upload": None, "maxlogsize": "100", "retentiondays": "30"},
    "file_exclusions": [],
    "checksum": "987978644c220a71f6fa67685b06571d",
    "filetype_actions": {
        "suspicious_files": [{"file_type": "executable", "actions": "0"}],
        "threat_files": [{"file_type": "executable", "actions": "0"}],
    },
    "policy_name": "fff",
    "policy_utctimestamp": "/Date(1586773964507+0000)/",
    "policy": [
        {"name": "auto_blocking", "value": "0"},
        {"name": "auto_uploading", "value": "0"},
        {"name": "threat_report_limit", "value": "500"},
        {"name": "low_confidence_threshold", "value": "-600"},
...
EXPECTED_POLICY = {"Timestamp": "2020-04-13T10:32:44.507000+00:00", "ID": "980fad21-b119-4cc4-ac97-2b2c035b4666", "Name": "fff"}
...
def test_get_device_threats(mocker):
    ...
    mocker.patch.object(Cylance_Protect_v2, "get_device_threats_request", return_value={"page_items": [DEVICE_THREAT_OUTPUT]})
...
def test_create_zone(mocker):
    ...
    Then
        - checks if the output is as expected
    """
    args = {"policy_id": "980fad21-b119-4cc4-ac97-2b2c035b4666", "name": "name", "criticality": "Low"}
...
def test_update_zone(mocker):
    ...
    Then
        - checks if the output is as expected
    """
    args = {"id": "1998235b-a6ab-4043-86b5-81b0dc63887b", "name": "name"}
...
def test_get_threat_devices(mocker):
    ...
    mocker.patch.object(Cylance_Protect_v2, "get_threat_devices_request", return_value={"page_items": [THREAT_DEVICES_OUTPUT]})
    mocker.patch.object(Cylance_Protect_v2, "translate_score", return_value=1)
...
def test_get_list(mocker):
    ...
    mocker.patch.object(Cylance_Protect_v2, "get_list_request", return_value={"page_items": [LIST_OUTPUT]})
    mocker.patch.object(Cylance_Protect_v2, "translate_score", return_value=1)
...
def test_get_list_entry_by_hash(mocker):
    ...
    args = {"listTypeId": "GlobalSafe", "sha256": "234E5014C239FD89F2F3D56091B87763DCD90F6E3DB42FD2FA1E0ABE05AF0487"}
    mocker.patch.object(
        Cylance_Protect_v2, "get_list_request", return_value={"page_items": [LIST_OUTPUT], "total_pages": "total_pages"}
    )
    mocker.patch.object(Cylance_Protect_v2, "translate_score", return_value=1)
...
def test_delete_hash_from_lists(mocker):
    ...
    contents = demisto_results.call_args[0][0]
    assert "The requested threat has been successfully removed from GlobalSafe hashlist." in contents.get("HumanReadable")
...
def test_delete_devices(mocker):
    ...
    args = {
        "deviceIds": "8e836c98-102e-4332-b00d-81dcb7a9b6f7",
        "batch_size": 1,
        "category": "Admin Tool",
        "reason": "Added by Demisto",
    }
    ...
    contents = demisto_results.call_args[0][0]
    assert "The requested devices have been successfully removed from your organization list." in contents.get("HumanReadable")
...
def test_get_policy_details(mocker):
    ...
    assert EXPECTED_POLICY.get("ID") == contents.get("EntryContext").get(
        "Cylance.Policy(val.policy_id && val.policy_id == obj.policy_id)"
    ).get("policy_id")
...
def test_create_instaquery(mocker):
    ...
    contents = demisto_results.call_args[0][0]
    assert INSTAQUERY_OUTPUT.get("id") == contents.get("EntryContext").get("InstaQuery.New(val.id && val.id == obj.id)").get("id")
...
def test_get_instaquery_result(mocker):
    ...
    contents = demisto_results.call_args[0][0]
    assert json.loads(INSTAQUERY_RESULT_OUTPUT["result"][0]["Result"]).get("Properties").get("Sha256") in contents.get(
        "HumanReadable"
    )
...
def test_list_instaquery(mocker):
    ...
    contents = demisto_results.call_args[0][0]
    assert len(LIST_INSTAQUERY_OUTPUT.get("page_items")) == len(
        contents.get("EntryContext").get("InstaQuery.List").get("page_items")
    )
