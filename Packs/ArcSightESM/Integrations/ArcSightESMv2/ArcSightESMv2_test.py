...
PARAMS = {"server": "https://server.local", "credentials": {}, "proxy": True}

ARGS = {"ids": "lastDateRange", "lastDateRange": "2 hours"}
...
test_data = [
    (True, "as-get-entries", "https://server/www/manager-service/rest/ActiveListService/getEntries?alt=json"),
    (False, "as-get-entries", "https://server/www/manager-service/services/ActiveListService/"),
...
def test_use_rest(mocker, use_rest, cmd_name, expected_rest_endpoint):
    ...
    """
    params = {"server": "https://server", "credentials": {}, "use_rest": use_rest, "proxy": True}
    mocker.patch.object(demisto, "params", return_value=params)
    ...
    with requests_mock.Mocker() as m:
        ...
        fake_xml = (
            '<?xml version="1.0"?><Envelope><Body><getEntriesResponse><return>'
...
def test_decoding_incidents():
    ...
    incident = {
        "name": "Test XSOAR",
        "occurred": "2023-03-22T12:44:51.000Z",
        "labels": [
            {"type": b"Event ID", "value": b"1234"},
            {"type": b"Start Time", "value": b"1234"},
            {"type": b"Name", "value": b"Test XSOAR"},
...
def test_filtered():
    ...
    entries = [
        {"userAccount": "abba", "internalAddress": "127.0.0.2", "externalLocation": "Russia", "externalAddress": "1.2.3.4"},
        {"userAccount": "abba", "internalAddress": "127.0.0.1", "externalLocation": "USA", "externalAddress": "1.2.3.4"},
        {"userAccount": "abba", "internalAddress": "127.0.0.1", "externalLocation": "ISS", "externalAddress": "1.2.3.4"},
    ]
...
def test_add_entries_using_detect_api(mocker, requests_mock):
    ...
    params = {"server": base_url, "credentials": {}, "productVersion": "7.4 and above", "proxy": True}
    entries = [{"Name": "foo", "IP": "8.8.8.8"}, {"Name": "roo", "IP": "1.1.1.1"}]
    resource_id = "100"
    ...
    expected_request_body = {
        "fields": ["Name", "IP"],
        "entries": [{"fields": ["foo", "8.8.8.8"]}, {"fields": ["roo", "1.1.1.1"]}],
    }
                    ...
                    "rows": [
                        {
                            ...
                            "value": [{"@xsi.type": "xs:string", "$": "test_one"}, {"@xsi.type": "xs:string", "$": "event_one"}],
                        },
                        {
                            "@xsi.type": "listWrapper",
                            "value": [{"@xsi.type": "xs:string", "$": "test_two"}, {"@xsi.type": "xs:string", "$": "event_two"}],
                        },
                    ],
                }
            }
...
def test_update_case_command(mocker):
    ...
    mocker.patch.object(demisto, "results")
    mocker.patch.object(demisto, "params", return_value=PARAMS)
    mocker.patch.object(demisto, "args", return_value={"caseId": "test id", "stage": "QUEUED", "severity": "INSIGNIFICANT"})
    ...
    class ReqMock:
        ...
        def json():
            return {
                "cas.updateResponse": {
                    "cas.return": {"createdTimestamp": 1629097454417, "events": [{"event one": {"case_id": "test id"}}]}
                }
            }
    ...
    mocker.patch.object(
        ArcSightESMv2,
        "get_case",
        return_value={"Name": "test case", "EventIDs": [], "CaseID": "test id", "createdTimestamp": 1629097454417},
    )
...
def test_get_case_event_ids_command(mocker, requests_mock):
    ...
    requests_mock.get(
        ...
        json={"cas.getCaseEventIDsResponse": {"cas.return": [12396713, 45695741, 78996719]}},
    )
    import ArcSightESMv2
...
def test_invalid_json_requiring_fixing_json_string_with_multi_json_object():
    ...
    assert parse_json_response(broken_json_response) == {
        "test": [{"test1": "test_val", "$": 'value with "quotes" inside'}, {"$": 'value with "quotes" inside'}]
    }
