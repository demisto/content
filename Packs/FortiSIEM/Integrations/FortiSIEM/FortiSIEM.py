...
def getEventsByIncident(incident_id, max_results, extended_data, max_wait_time):
    ...
    queryData = {
        ...
        "eventFilters": [
            {"name": "Filter_OVERALL_STATUS", "singleConstraint": f"(phEventCategory = 1) AND incidentId = {incident_id}"}
        ],
        "hints": "IgnoreTime",
    }
    ...
    return getEventsByQuery(
        ...
        extended_data,
        max_wait_time,
        "FortiSIEM events for Incident " + incident_id,
...
def getEventsByQuery(session, queryData, max_results, extended_data, max_wait_time, tableTitle, incident_id=None):
    ...
    response = session.post(REST_ADDRESS + "/report/resultByReport", params=params, headers=headers, data=data, verify=VERIFY_SSL)
...
def create_query_xml(
    include_value, interval="", single_evt_value="phEventCategory=1", interval_type="Minute", attr_list=None, limit="All"
):
    doc = Document()
...
def getEventsByFilter(maxResults, extendedData, maxWaitTime, reportWindow, reportWindowUnit):
    ...
    query_data = {
        ...
        "eventFilters": [{"groupBy": "", "singleConstraint": query_string}],
        "custId": 1,
    }
    return getEventsByQuery(session, query_data, maxResults, extendedData, maxWaitTime, "FortiSIEM Event Results")
...
def get_cmdb_devices_command():
    ...
    return_outputs(tableToMarkdown("Devices", list_of_devices), {"FortiSIEM.CmdbDevices": list_of_devices}, raw_response)
...
@logger
def get_events_by_query(query, report_window="60", interval_type="Minute", limit="20", extended_data="false", max_wait_time=60):
    session = login()
    ...
    query_data = {
        ...
        "timeRangeRelative": True,
        "eventFilters": [{"groupBy": "", "singleConstraint": query}],
        "custId": 1,
    }
    return getEventsByQuery(session, query_data, limit, extended_data, max_wait_time, "FortiSIEM Event Results")
...
def flatten_resources(raw_resources):
    for r in raw_resources:
        ...
        yield from flatten_resources(r["children"])
...
def get_resource_list_command():
    ...
    return_outputs(tableToMarkdown("Resource list:", ec, headerTransform=pascalToSpace, removeNull=True), outputs, raw_response)
...
def fetch_incidents():
    ...
    demisto.setLastRun({"ids": known_ids, "extended_keys": EXTENDED_KEYS})
...
def main():
    try:
        ...
        if demisto.command() == "test-module":
            ...
            args = demisto.args()
            getEventsByFilter(
                args["maxResults"], args["extendedData"], args["maxWaitTime"], args["reportWindow"], args["reportWindowUnit"]
            )
