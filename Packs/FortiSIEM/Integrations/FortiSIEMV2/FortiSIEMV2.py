...
INCIDENT_STATUS_INT_VERBAL_MAPPING = {0: "ACTIVE", 1: "AUTOMATICALLY CLEARED", 2: "MANUALLY CLEARED", 3: "SYSTEM CLEARED"}

INCIDENT_RESOLUTION_INT_VERBAL_MAPPING = {0: "None", 1: "Open", 2: "True Positive", 3: "False Positive", 4: "In Progress"}

INCIDENT_CATEGORY_INT_VERBAL_MAPPING = {1: "AVAILABILITY", 2: "PERFORMANCE", 3: "CHANGE", 4: "SECURITY", 5: "OTHER"}

INCIDENT_EVENT_CATEGORY_MAPPING = {"LOW": 1, "MEDIUM": 2, "HIGH": 3}

EVENT_GENERIC_ATTRIB = {"eventId", "eventName", "reptDevIpAddr", "reptDevName"}

REFORMAT_INCIDENT_FIELDS = {"incidentSrc", "incidentTarget", "incidentDetail"}
...
class FortiSIEMClient(BaseClient):
    ...
    def cmdb_devices_list_request(
        self, include_ip_list: list, exclude_ip_list: list, include_ip_range: str, exclude_ip_rage: str
    ) -> dict[str, Any]:
        """
        ...
        params = assign_params(includeIps=include_ip_set, excludeIps=exclude_ip_set)
    ...
    def fetch_incidents_request(
        self, status: List[int], time_from: int, time_to: int, size: int, start: int = 0
    ) -> dict[str, Any]:
        """
    ...
    def incident_update_request(
        ...
        comments: str,
        incident_status: str,
        external_ticket_type: str,
        external_ticket_id: str,
        external_ticket_state: str,
        external_assigned_user: str,
    ) -> str:
        """
        ...
        data = assign_params(
            ...
            incidentStatus=incident_numeric_status,
            comments=comments,
            externalAssignedUser=external_assigned_user,
            externalTicketId=external_ticket_id,
            externalTicketState=external_ticket_state,
    ...
    def watchlist_entry_add_request(
        self,
        watchlist_id: str,
        value: str,
        inclusive: bool,
...
def search_events_with_polling_command(
    client: FortiSIEMClient,
    args: dict[str, Any],
    cmd: str,
    search_command: Callable,
    status_command: Callable,
    results_command: Callable,
) -> CommandResults:
    """
    ...
    if "search_id" not in args:
        ...
        polling_args = {"search_id": search_id, "interval_in_seconds": interval_in_secs, "polling": True, **args}
        # schedule first poll
...
def events_search_init_command(client: FortiSIEMClient, args: dict[str, Any]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        f"Successfully Initiated search query {response}", outputs, headers=["search_id"], headerTransform=string_to_table_header
    )
...
def events_search_status_command(client: FortiSIEMClient, args: dict[str, Any]) -> CommandResults:
    ...
    outputs = {"percentage_status": response, "search_id": search_id}
    readable_outputs = tableToMarkdown(
        f"Search query:{search_id} status", outputs, headers=["percentage_status"], headerTransform=string_to_table_header
    )
    command_results = CommandResults(
...
def cmdb_devices_list_command(client: FortiSIEMClient, args: dict[str, Any]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        header, outputs, headers=["name", "accessIp", "approved", "unmanaged", "deviceType"], headerTransform=pascalToSpace
    )
...
def cmdb_device_get_command(client: FortiSIEMClient, args: dict[str, Any]) -> List[CommandResults]:
    ...
    for ip_address in ip_address_list:
        try:
            ...
            readable_output = tableToMarkdown(
                f"CMDB device {ip_address}",
                outputs,
                headers=["name", "accessIp", "approved", "unmanaged", "deviceType", "discoverTime", "discoverMethod"],
                headerTransform=pascalToSpace,
            )
            command_results = CommandResults(
                outputs_prefix="FortiSIEM.Device",
            ...
            error_results = CommandResults(readable_output=f"**{error}**")
...
def watchlist_get_command(client: FortiSIEMClient, args: dict[str, Any]) -> List[CommandResults]:
    ...
    for watchlist_id in watchlist_id_list:
        try:
            ...
            outputs, _ = format_watchlist_output(response, f"**Watchlist ID {watchlist_id} doesn't exist.**")
            watchlist_readable_output = tableToMarkdown(
                f"Watchlist {watchlist_id}",
                outputs,
                headers=["id", "name", "displayName", "description", "valueType"],
                headerTransform=pascalToSpace,
            )
            entry_readable_output = tableToMarkdown(
                "Watchlist Entries",
                outputs[0]["entries"],
                headers=["id", "state", "entryValue", "triggeringRules", "count", "firstSeen", "lastSeen"],
                headerTransform=pascalToSpace,
            )
    ...
    for entry_id in entry_id_list:
        try:
            ...
            outputs, _ = format_watchlist_output(response, f"Watchlist with entry ID of {entry_id} does not exist.")
            watchlist_readable_output = tableToMarkdown(
                f"Watchlist with entry ID {entry_id}",
                outputs,
                headers=["id", "name", "displayName", "description", "valueType"],
                headerTransform=pascalToSpace,
            )
...
def watchlist_add_command(client: FortiSIEMClient, args: dict[str, Any]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        f"Added new Watchlist group: {display_name}", output, headers=["id", "name", "displayName", "description", "valueType"]
    )
    command_results = CommandResults(
        outputs_prefix="FortiSIEM.Watchlist",
        outputs_key_field="id",
...
def watchlist_entry_update_command(client: FortiSIEMClient, args: dict[str, Any]) -> CommandResults:
    ...
    response = client.watchlist_entry_update_request(
        ...
        firstSeen=first_seen,
        lastSeen=last_seen,
        expiredTime=expiry_time,
        dataCreationType=data_creation_type,
    )

    outputs, _ = format_watchlist_output(response)
    readable_output = tableToMarkdown(
        format_update_watchlist_entry_header(entry_id, response),
        outputs,
        headers=["id", "state", "entryValue", "triggeringRules", "count", "firstSeen", "lastSeen"],
        headerTransform=pascalToSpace,
    )
...
def watchlist_delete_command(client: FortiSIEMClient, args: dict[str, Any]) -> List[CommandResults]:
    ...
    for watchlist_id in watchlist_id_list:
        try:
            ...
            command_results = CommandResults(readable_output=format_message_delete_watchlist(watchlist_id, response))
            command_results_list.append(command_results)
        except Exception as error:
            error_results = CommandResults(readable_output=f"**{error}**")
            command_results_list.append(error_results)
...
def watchlist_entry_delete_command(client: FortiSIEMClient, args: dict[str, Any]) -> List[CommandResults]:
    ...
    for entry_id in entry_id_list:
        try:
            ...
            command_results = CommandResults(readable_output=format_message_delete_entry(entry_id, response))
            command_results_list.append(command_results)
        except Exception as error:
            error_results = CommandResults(readable_output=f"**{error}**")
            command_results_list.append(error_results)
...
def fetch_incidents(
    ...
    relevant_incidents = fetch_relevant_incidents(
        client,
        numeric_status_list,
        time_from,  # type: ignore[arg-type]
...
def test_module(client: FortiSIEMClient, max_fetch, fetch_with_events, max_events_fetch, first_fetch, status_filter_list) -> None:
        ...
        if ip_list:
            for ip_address in argToList(ip_list):  # ip_list os an comma separated list argument of a command.
                if not (re.match(ipv4Regex, ip_address)) or (re.match(ipv6Regex, ip_address)):
                    raise ValueError("IP address should be in the format of: X.X.X.X where x is a number between 0 to 255.")
...
def format_list_commands_output(
    response: dict[str, Any], default_dict_keys: List[str], page_number: int, limit: int
) -> tuple[list, int]:
    """
...
def format_organizations_output(response: dict[str, Any], page_number: int, limit: int) -> tuple[list, int]:
    ...
    relevant_output_entities, total_page_number = format_list_commands_output(
        response, ["response", "result", "domains", "domain"], page_number, limit
    )
...
def format_outputs_time_attributes_to_iso(outputs: List[dict]) -> List[dict]:
    ...
    if not isinstance(outputs, list):
        outputs = [outputs]
    time_keys = ["lastModified", "creationTime", "receiveTime", "discoverTime", "firstSeen", "lastSeen", "expiredTime", "Time"]
...
def fetch_relevant_incidents(
    client: FortiSIEMClient, status: List[int], time_from: int, time_to: int, last_run: dict[str, Any], max_fetch: int
) -> List[dict]:
    """
    ...
    """
    demisto.debug(f"Fetch incident from: {time_from!s} to {time_to!s}")
    ...
    while len(filtered_incidents) < max_fetch and start_index < total:  # type: ignore[operator]
        for incident in incidents:  # type: ignore[union-attr]
            if (
                ...
                and len(filtered_incidents) < max_fetch
                and incident.get("incidentFirstSeen") >= last_incident_create_time
            ):
...
def validate_fetch_params(
    max_fetch: int, max_events_fetch: int, fetch_events: bool, first_fetch: str, status_filter_list: list
) -> None:
    """
...
def update_last_run_obj(last_run: dict[str, Any], formatted_incidents: List[dict]):
    ...
    if cur_last_incident_create_time == prev_last_incident_create_time and prev_last_incident_create_time:
        ...
        last_run = {"create_time": cur_last_incident_create_time, "last_incidents": cur_incidents_id, "start_index": 0}
    return last_run
...
def main() -> None:
    ...
    try:
        ...
        client: FortiSIEMClient = FortiSIEMClient(
            urljoin(url, ""), verify_certificate, proxy, headers=headers, auth=(f"super/{username}", password)
        )
        ...
        if command == "test-module":
            test_module(client, max_fetch, fetch_with_events, max_events_fetch, params.get("first_fetch"), status_filter_list)
            ...
            if argToBoolean(args.get("polling")):
                return_results(
                    search_events_with_polling_command(
                        client,
                        args,
