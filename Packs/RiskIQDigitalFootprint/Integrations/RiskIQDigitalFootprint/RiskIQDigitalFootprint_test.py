...
def test_http_request_bad_request_error(mock_base_http_request, client):
    ...
    # Assert
    assert "An error occurred while fetching the data." " Reason: Id:dummy could not be normalized" == str(e.value)
...
def test_http_request_internal_server_error(mock_base_http_request, client):
    ...
    # Assert
    assert (
        "The server encountered an internal error for RiskIQ Digital Footprint and"
...
def test_http_request_proxy_error_based_on_status(mock_base_http_request, client):
    ...
    # Assert
    assert (
        "Proxy Error - cannot connect to proxy. Either try clearing the 'Use system proxy' check-box or check "
...
def test_http_request_connect_timeout_error(mock_base_http_request, client):
    ...
    # Assert
    assert (
        "Connection timed out. Check your internet connection or try decreasing the"
...
def test_asset_connections_success(mocker_http_request, client):
    ...
    result = asset_connections_command(client, args={"name": "dummy", "type": "ASN", "global": "true", "size": 2})
...
def test_asset_connections_no_record_found(mocker_http_request, client):
    ...
    assert (
        result == "No connected assets were found for the given argument(s). If the page argument is specified, try"
...
def test_asset_connections_invalid_type(client):
    ...
    assert (
        ...
        " IP Address, IP Block, ASN, Page, SSL Cert, Contact. This argument supports a single value only." == str(e.value)
    )
...
def test_asset_connections_invalid_global(client):
    ...
    assert (
        "The given value for global argument is invalid. Valid values: true, false."
...
def test_asset_connections_exceeding_page_lower_limit(client):
    ...
    assert "Page argument must be 0 or a positive integer. The index is zero based so the first page is page 0." == str(e.value)
...
def test_asset_changes_summary_success(mocker_http_request, client):
    ...
    result = asset_changes_summary_command(
        client, args={"date": "2020-05-12", "range": "7", "tag": "Dummy", "organization": "Dummy", "brand": "Dummy"}
    )
...
def test_asset_changes_summary_invalid_date(client):
    ...
    args = {"date": "2002-2-05", "range": ""}
    with pytest.raises(ValueError) as e:
        validate_asset_changes_summary_args(args["date"], args["range"])

    assert (
        "The given value for date is invalid. The accepted format for date is YYYY-MM-DD."
        " This argument supports a single value only." == str(e.value)
    )
...
def test_asset_changes_summary_invalid_range(client):
    ...
    args = {"date": "2020-05-12", "range": "5"}

    with pytest.raises(ValueError) as e:
        validate_asset_changes_summary_args(args["date"], args["range"])

    assert "The given value for range is invalid. Valid values: 1, 7, 30." " This argument supports a single value only." == str(
        e.value
    )
...
def test_asset_changes_success_asset_type(mocker_http_request, client):
    ...
    result = asset_changes_command(
        client, args={"range": "30", "type": "DOMAIN", "organization": "dummy", "measure": "Added", "size": 1}
    )
...
def test_asset_changes_success_asset_detail_type(mocker_http_request, client):
    ...
    result = asset_changes_command(client, args={"type": "SELF_HOSTED_RESOURCE", "range": "1", "measure": "Added", "size": 200})
...
def test_asset_changes_deep_link_with_only_date(client):
    ...
    args = {"type": "DOMAIN", "measure": "Removed", "date": "2020-06-05", "range": ""}
    ...
    deep_link = prepare_deep_link_for_asset_changes(
        resp.get("content")[0].get("runDate"), args["type"], args["measure"], args["date"], args["range"]
    )

    assert (
        "https://app.riskiq.net/a/main/index#/dashboard/inventorychanges/details/date=2020-06-05"
...
def test_asset_changes_deep_link_with_date_and_range(client):
    ...
    args = {"type": "IP_ADDRESS", "measure": "Added", "date": "2020-06-13", "range": "30"}
    ...
    assert (
        "https://app.riskiq.net/a/main/index#/dashboard/inventorychanges/details/date=2020-06-13"
...
def test_asset_changes_invalid_range_for_asset_detail_type(client):
    ...
    with pytest.raises(ValueError) as e:
        get_asset_changes_params(
            args={"range": "30", "tag": "dummy", "brand": "dummy", "organization": "dummy", "type": "SELF_HOSTED_RESOURCE"}
        )

    assert (
        "The given value for range is invalid. Only single day changes can be shown for Self Hosted Resource type."
...
def test_asset_changes_invalid_measure_for_asset_type(client):
    ...
    assert (
        "The given value for measure(type of change) is invalid. Valid options are Added or Removed."
        " This argument supports a single value only." == str(e.value)
    )
...
def test_asset_changes_invalid_measure_for_asset_detail_type(client):
    ...
    assert (
        "The given value for measure(type of change) is invalid. Valid options are Added or Changed."
        " This argument supports a single value only." == str(e.value)
    )
...
def test_get_asset_success_for_domain(mocker_http_request, client):
    ...
    result = get_asset_command(client, args={"uuid": "42696470-7b2a-617b-2f5e-ab674438e4f5", "global": "true", "recent": "true"})
...
def test_get_asset_by_name_and_type_success_for_host(mocker_http_request, client):
    ...
    result = get_asset_command(client, args={"name": "www.dummy.com", "type": "HOST", "global": "true", "recent": "true"})
...
def test_get_asset_by_name_and_type_success_for_ip_address(mocker_http_request, client):
    ...
    result = get_asset_command(client, args={"name": "dummy.ip", "type": "IP_ADDRESS", "recent": "true"})
...
def test_get_asset_by_name_and_type_success_for_as(mocker_http_request, client):
    ...
    result = get_asset_command(client, args={"name": "63245", "type": "ASN", "recent": "true"})
...
def test_get_asset_invalid_recent(client):
    ...
    args = {"uuid": "dummy", "recent": "dummy"}

    with pytest.raises(ValueError) as e:
        get_asset_params(args)

    assert (
        "The given value for recent argument is invalid. Valid values: true, false."
        " This argument supports a single value only." == str(e.value)
    )
...
def test_get_asset_invalid_asset_type(client):
    ...
    args = {"name": "dummy", "type": "dummy"}

    with pytest.raises(ValueError) as e:
        validate_and_fetch_get_asset_arguments(args)

    assert (
        "The given value for type is invalid. Valid Types: Domain, Host, IP Address, IP Block, ASN, Page,"
...
def test_get_asset_when_type_is_not_passed_with_name_argument(client):
    ...
    assert "Required argument(s) uuid or [name, type] to get asset details. One or more of them are not present." == str(e.value)
...
def test_add_asset_incomplete_for_asset_json(mocker_http_request, client):
    ...
    assert result.outputs == expected_custom_ec
    assert result.readable_output == "### The request for adding asset(s) is incomplete." " Reason: An unexpected error occurred."
    assert result.outputs_key_field == "uuid"
    assert result.outputs_prefix == "RiskIQDigitalFootprint.Task"
...
def test_add_asset_warning_for_asset_json(mocker_http_request, client):
    ...
    assert result.outputs == expected_custom_ec
    assert (
        result.readable_output == "### The request for adding asset(s) is completed with a warning."
...
def test_update_asset_success_for_single_asset(mocker_http_request, client):
    ...
    result = update_assets_command(
        client,
        args={
            "name": "test",
            "type": "ASN",
            "removed_state": "Dismissed",
            "target_asset_types": "IP_ADDRESS",
            "fail_on_error": "true",
            "action": "Remove",
        },
...
def test_add_and_update_asset_with_no_properties(client):
    ...
    assert (
        "At least one property argument should have a value in order to update the asset. "
...
def test_add_and_update_asset_invalid_state(client):
    ...
    args = {"name": "xyz", "type": "IP_ADDRESS", "state": "dummy"}
    with pytest.raises(ValueError) as e:
        prepare_single_asset_payload(args, operation="update")
    assert (
        "The given value for state is invalid. Valid States: Candidate, Approved Inventory, "
        "Requires Investigation, Dependencies, Monitor Only. This argument supports a single value only." == str(e.value)
    )
...
def test_add_and_update_asset_invalid_priority(client):
    ...
    args = {"name": "xyz", "type": "IP_ADDRESS", "priority": "dummy"}
    with pytest.raises(ValueError) as e:
        prepare_single_asset_payload(args, operation="add")
    assert (
        "The given value for priority is invalid. Valid Priority levels: High, Medium, Low, None. This argument"
...
def test_add_and_update_asset_invalid_enterprise(client):
    ...
    args = {"name": "xyz", "type": "IP_ADDRESS", "enterprise": "dummy"}
    with pytest.raises(ValueError) as e:
        prepare_single_asset_payload(args, operation="update")
    assert (
        "The given value for enterprise argument is invalid. Valid values: true, false. "
        "This argument supports a single value only." == str(e.value)
    )
...
def test_add_and_update_asset_invalid_confirm(client):
    ...
    args = {"name": "xyz", "type": "IP_ADDRESS", "confirm": "dummy"}
    with pytest.raises(ValueError) as e:
        prepare_single_asset_payload(args, operation="add")
    assert (
        "The given value for confirm argument is invalid. Valid values: true, false. "
        "This argument supports a single value only." == str(e.value)
    )
...
def test_add_and_update_asset_invalid_removed_state(client):
    ...
    args = {"name": "xyz", "type": "IP_ADDRESS", "removed_state": "dummy"}
    with pytest.raises(ValueError) as e:
        prepare_single_asset_payload(args, operation="update")
    assert (
        "The given value for removed state is invalid. Valid value: Dismissed. This argument supports a"
...
def test_update_asset_invalid_action(client):
    ...
    args = {"name": "xyz", "type": "IP_ADDRESS", "action": "dummy", "brand": "dummy"}
    with pytest.raises(ValueError) as e:
        prepare_single_asset_payload(args, operation="update")
    assert (
        "The given value for action is invalid. Valid values: Add, Remove, Update. This argument supports"
...
def test_add_and_update_asset_invalid_arguments(client):
    ...
    args = {"name": "xyz", "type": "IP_ADDRESS", "asset_json": '{"assets":[{"name": "xyz"}]}'}
    with pytest.raises(ValueError) as e:
...
def test_add_and_update_asset_required_keys(client):
    ...
    args = {"asset_json": {"assets": [{"name": "xyz"}]}}
    with pytest.raises(ValueError) as e:
        validate_asset_payload(args, operation="add")
    assert (
        "Required keys for add asset(s) are ['assets', 'properties']. One or more of them are not present in the"
...
def test_add_and_update_asset_required_keys_in_asset(client):
    ...
    args = {"asset_json": {"assets": [{"name": "xyz"}], "properties": []}}
    with pytest.raises(ValueError) as e:
        validate_asset_payload(args, operation="update")
    assert (
        "Required keys for update asset(s) are ['name', 'type'] in assets key."
...
def test_add_and_update_asset_invalid_value_fail_on_error(client):
    ...
    args = {"name": "dummy", "type": "IP_ADDRESS", "fail_on_error": "dummy"}
    with pytest.raises(ValueError) as e:
        get_add_and_update_assets_params(args)
    assert (
        "The given value for fail_on_error argument is invalid. Valid values: true, false. "
        "This argument supports a single value only." == str(e.value)
    )
...
def test_update_asset_required_keys_in_asset_json(client):
    ...
    with pytest.raises(ValueError) as e:
        validate_asset_payload(args, operation="update")
    assert (
        "Required keys for update asset(s) are ['assets', 'properties'] or ['query', 'properties']. One or more of"
...
def test_add_asset_success_for_asset_json_retries(mocker_http_request, client, mocker):
    ...
    assert result.outputs == expected_custom_ec
    assert (
        result.readable_output == "### The request for adding asset(s) has been successfully submitted. "
...
def test_command_with_strip_args_from_main_success(mocker, client):
    ...
    mocker.patch.object(demisto, "args", return_value={"name": "dummy", "type": "      ASN          ", "global": "   true"})
...
def test_command_with_strip_args_from_main_failure(mock_return_error, mocker, client, capfd):
    ...
    mock_return_error.assert_called_once_with(
        "Error: The given value for type is invalid. Valid Types: Domain, Host,"
        " IP Address, IP Block, ASN, Page, SSL Cert, Contact. This argument"
        " supports a single value only."
    )
...
def test_request_timeout_and_size_failure():
    ...
    assert "Size argument must be a positive integer. The value of this argument should be between 1 and 1000." == str(e.value)
...
def test_command_with_size_arg_from_main_success(mocker, client):
    ...
    mocker.patch.object(demisto, "command", return_value="df-get-asset")
    mocker.patch.object(
        demisto, "args", return_value={"name": "dummy", "type": "      ASN          ", "global": "   true", "size": "400"}
    )
