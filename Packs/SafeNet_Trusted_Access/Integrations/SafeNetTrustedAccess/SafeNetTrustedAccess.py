...
class Client(BaseClient):
    ...
    def http_request(
        self, method, url_suffix, data=None, headers=None, json_data=None, params=None, full_url=None, resp_type="response"
    ):
    ...
    def get_paged_results_sta(self, uri, query_params=None, limit=None):
        response = self.http_request(method="GET", url_suffix=uri, params=query_params)
        request_count = 1
        quotient = 0
        if limit:
        ...
        while "next" in response.json()["links"] and len(response.json()["page"]["items"]) > 0:
            if request_count >= 10:
                raise Exception(
                    "You have reached the maximum number of request attempts."
            ...
            response = self.http_request(method="GET", full_url=next_page, url_suffix="", params=query_params)
    ...
    def create_user_sta(self, args):
        ...
        return self.http_request(method="POST", url_suffix="/users", data=json.dumps(data)).json()
    ...
    def update_user_sta(self, args):
        ...
        return self.http_request(
            method="PATCH", url_suffix=urljoin("/users/", quote_plus(args.get("userName"))), data=json.dumps(data)
        ).json()
    ...
    def create_group_sta(self, args):
        ...
        return self.http_request(method="POST", url_suffix="/groups", data=json.dumps(data)).json()
    ...
    def update_group_sta(self, args):
        ...
        return self.http_request(
            method="PATCH", url_suffix=urljoin("/groups/", quote_plus(group_id)), data=json.dumps(data)
        ).json()
    ...
    def add_user_group_sta(self, userName, groupName):
        if self.user_exist_group_sta(userName=userName, groupName=groupName) is False:
            ...
            return {"user_id": user_id, "userName": userName, "group_id": group_id, "groupName": groupName, "status": True}
        else:
    ...
    def remove_user_group_sta(self, userName, groupName):
        if self.user_exist_group_sta(userName=userName, groupName=groupName) is True:
            ...
            self.http_request(
                method="DELETE",
                url_suffix=urljoin(urljoin(urljoin("/groups/", quote_plus(group_id)), "/members/"), quote_plus(user_id)),
            )
            return {"user_id": user_id, "userName": userName, "group_id": group_id, "groupName": groupName, "status": False}
        else:
    ...
    def logs_attributes_sta(self, response):
        logs_attributes = {
            ...
            "userName": response["context"]["principalId"],
            "logType": response["details"]["type"],
            "ip": response["context"]["originatingAddress"],
            "credentialType": "",
            "resultText": "",
            "actionText": "",
            "applicationName": "",
    ...
    def get_logs_sta(self, userName=None, since=None, until=None, limit=None):
        ...
        if limit:
            limit = self.validate_limit_sta(limit)
            request_count = 1
            paged_results, count = self.logs_data_filter_sta(response.json()["page"]["items"], userName=userName, limit=limit)
            while "next" in response.json()["links"] and len(response.json()["page"]["items"]) > 0 and limit >= count:
                ...
                request_count = request_count + 1
                paged_results, count = self.logs_data_filter_sta(
                    response.json()["page"]["items"], userName=userName, limit=limit, count=count, logs_items=paged_results
                )
        else:
    ...
    def validate_tenant_sta(self):
        return self.http_request(method="GET", url_suffix="/authorized")
    ...
    def get_application_info_sta(self, applicationName):
        ...
        response = self.http_request(method="GET", url_suffix=urljoin("/applications/", quote_plus(application_id))).json()
    ...
    def delete_sessions_sta(self, userName):
        ...
        data = {"id": user_id, "userName": userName, "sessions": {"Deleted": True}}
...
def get_userlist_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "List of users in the tenant :", response, headers=header_sequence, headerTransform=pascalToSpace, removeNull=True
        ),
...
def get_user_info_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            f"Information for user - {args.get('userName')} :",
            response,
            headers=header_sequence,
...
def create_user_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "STA user successfully created :", response, headers=header_sequence, headerTransform=pascalToSpace, removeNull=True
        ),
        outputs_prefix="STA.USER",
        outputs_key_field=["id"],
        outputs=response,
...
def update_user_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "STA user successfully updated:", response, headers=header_sequence, headerTransform=pascalToSpace, removeNull=True
        ),
        outputs_prefix="STA.USER",
        outputs_key_field=["id"],
        outputs=response,
...
def get_user_groups_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            f"Groups associated with user - {args.get('userName')} : ",
            response,
            headers=header_sequence,
            headerTransform=pascalToSpace,
            removeNull=True,
...
def get_group_list_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "STA groups in the tenant : ", response, headers=header_sequence, headerTransform=pascalToSpace, removeNull=True
        ),
...
def get_group_members_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            f"Members of group - {args.get('groupName')} : ",
            response,
            headers=header_sequence,
            headerTransform=pascalToSpace,
            removeNull=True,
...
def create_group_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            f"STA group - {args.get('groupName')} successfully created:",
            response,
            headers=header_sequence,
            headerTransform=pascalToSpace,
            removeNull=True,
...
def get_logs_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    response = client.get_logs_sta(
        userName=args.get("userName"), since=args.get("since"), until=args.get("until"), limit=args.get("limit")
    )
    if not response:
        return CommandResults(
            readable_output=NO_RESULT_MSG,
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            "Logs : ", response, headers=header_sequence, headerTransform=pascalToSpace, removeNull=True
        ),
...
def validate_tenant_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output="## The requested tenant is accessible.", outputs_prefix="STA.VALIDATE.TENANT", outputs=True
    )
...
def get_application_info_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            f"Information of application - {args.get('applicationName')} :",
            readable_output,
            headers=header_sequence,
            headerTransform=pascalToSpace,
            removeNull=True,
...
def get_user_applications_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=tableToMarkdown(
            f"Applications associated with user - {args.get('userName')} : ",
            response,
            headers=header_sequence,
            headerTransform=pascalToSpace,
            removeNull=True,
...
def get_user_sessions_sta_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    session_data = tableToMarkdown(
        f"Sessions associated with user - {args.get('userName')} : ",
        response,
    ...
    return CommandResults(readable_output=session_data, outputs_prefix="STA.USER", outputs_key_field=["id"], outputs=output_data)
