...
class Client(BaseClient):
    def __init__(self, params):
        ...
        super().__init__(
            base_url=self._server_url, verify=self._verify_certificate, ok_codes=(), proxy=params.get("proxy", False)
        )
        ...
        for actor in response["resources"]:
            if actor:
                ...
                indicator = {
                    ...
                    "rawJSON": {"type": "STIX Threat Actor", "value": actor.get("name"), "service": "List Actors Feed"},
                    "fields": fields,
                }
    ...
    def get_indicators(
        ...
        return self.create_indicators_from_response(response, feed_tags, tlp_color)
...
def get_indicators_command(client: Client, args: dict, feed_tags: list, tlp_color: str | None) -> tuple[str, dict, list]:
    ...
    target_countries = args.get("target_countries") if args.get("target_countries") else demisto.params().get("target_countries")
    target_industries = (
        args.get("target_industries") if args.get("target_industries") else demisto.params().get("target_industries")
    )
    custom_filter = args.get("custom_filter") if args.get("custom_filter") else demisto.params().get("custom_filter")

    indicators = client.get_indicators(feed_tags, tlp_color, limit, offset, target_countries, target_industries, custom_filter)
    ...
    human_readable = tableToMarkdown(
        "Indicators from CrowdStrike:", hr_indicators, headers=["Value", "Type", "rawJSON", "fields"], removeNull=True
    )
...
def fetch_indicators(
    ...
    if len(indicators) >= limit:
        ...
        last_run = {"last_modified_time": last_modified_time, "offset": offset + limit}
...
def main():
    ...
    # Switch case
    commands = {"test-module": test_module, "crowdstrike-falcon-intel-get-indicators": get_indicators_command}
    try:
