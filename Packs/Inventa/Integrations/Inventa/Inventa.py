...
empty_file = {"id": "", "timestamp": "", "name": "", "size": "", "path": "", "url": "", "entityTypes": ""}
empty_transaction: dict = {}
empty_dataasset: dict = {"id": "", "name": "", "piis": "", "reasonsOfProcessing": ""}
empty_database: dict = {"id": "", "name": "", "database": "", "entityTypes": ""}
empty_pii: dict = {"entities": []}
...
class Client(BaseClient):
    ...
    def get_entities(self):
        pii_entities = self._http_request(
            method="GET", url_suffix="/pii/api/configuration/categories", return_empty_response=True
        )
        return pii_entities
    ...
    def get_ticket(self, ticket_id: Any) -> dict:
        result = self._http_request(
            method="GET", url_suffix=f"/dsr/api/dsar-management/tickets/{ticket_id}", return_empty_response=True, retries=5
        )
        return result
    ...
    def get_sources(self, datasubject_id: Any) -> dict:
        return self._http_request(
            method="GET", url_suffix=f"/pii/api/piis/{datasubject_id}/sources/details", return_empty_response=True, retries=5
        )
...
def get_entities_command(client: Client) -> CommandResults:
    ...
    return CommandResults(outputs_prefix="Inventa.Entities", outputs_key_field="entity", outputs=result)
...
def get_datasubjects_command(client: Client, **kwargs) -> CommandResults:
    ...
    return CommandResults(outputs=result, outputs_prefix="Inventa.DataSubjects", outputs_key_field="id")
...
def get_datasubject_id_command(client: Client, **kwargs) -> CommandResults:
    ...
    if datasubs:
        ...
        return CommandResults(
            outputs={"datasubject_id": datasubject_id}, outputs_prefix="Inventa.DataSubjects", outputs_key_field="datasubject_id"
        )
    else:
        return CommandResults(
            outputs={"datasubject_id": 0}, outputs_prefix="Inventa.DataSubjects", outputs_key_field="datasubject_id"
        )
        ...
        result.append(
            {
                ...
                "timestamp": datetime.utcfromtimestamp(float(f"{str(source_ts)[:10]}.{str(source_ts)[10:]}")).strftime(
                    "%d %b %Y %H:%M:%S"
                ),
...
def create_ticket_command(client: Client, reason: str, datasubject_id: int) -> CommandResults:
    ...
    return CommandResults(
        outputs={"ticket_id": f"{ticket_id}"}, outputs_prefix="Inventa.DataSubjects.Ticket", outputs_key_field="ticket_id"
    )
...
def get_datasubject_details_command(client: Client, ticket_id: int) -> CommandResults:
    ...
    return CommandResults(
        outputs={"name": datasubject_name, "email": datasubject_email},
...
def get_dsar_piis_command(client: Client, ticket_id: int) -> CommandResults:
    ...
    return CommandResults(outputs={"piis": list(set(pii_list))}, outputs_prefix="Inventa.Dsar.Piis")
...
def get_dsar_transactions_command(client: Client, ticket_id: int) -> CommandResults:
    ...
    if "entityTypes" in transactions:
        transactions["entityTypes"] = [
            transactions["entityTypes"]["type"] for item in transactions["entityTypes"] if item == "type"
        ]
    ...
    if transactions:
        return CommandResults(
            outputs={"transactions": transactions}, outputs_prefix="Inventa.Dsar.Transactions", outputs_key_field="id"
        )
    else:
        return CommandResults(
            outputs={"transactions": [empty_transaction]}, outputs_prefix="Inventa.Dsar.Transactions", outputs_key_field="id"
        )
...
def get_dsar_files_command(client: Client, ticket_id: int) -> CommandResults:
    ...
    for file in files:
        ...
        if "timestamp" in file:
            ...
            if type(ts) is int:
                file["timestamp"] = datetime.utcfromtimestamp(float(f"{str(ts)[:10]}.{str(ts)[10:]}")).strftime(
                    "%d %b %Y %H:%M:%S"
                )
            # file["entityTypes"] = stripped
    if files:
        return CommandResults(outputs={"files": files}, outputs_prefix="Inventa.Dsar.Files", outputs_key_field="id")

    else:
        return CommandResults(outputs={"files": [empty_file]}, outputs_prefix="Inventa.Dsar.Files", outputs_key_field="id")
...
def get_dsar_databases_command(client: Client, ticket_id: int) -> CommandResults:
    ...
    if databases:
        return CommandResults(outputs={"databases": tables}, outputs_prefix="Inventa.Dsar.Databases", outputs_key_field="id")
    else:
        return CommandResults(
            outputs={"databases": [empty_database]}, outputs_prefix="Inventa.Dsar.Databases", outputs_key_field="id"
        )
...
def get_dsar_dataassets_command(client: Client, ticket_id: int) -> CommandResults:
    ...
    if dataAssets:
        ...
        return CommandResults(
            outputs={"dataAssets": dataAssets}, outputs_prefix="Inventa.Dsar.DataAssets", outputs_key_field="id"
        )
    else:
        return CommandResults(
            outputs={"dataAssets": [empty_dataasset]}, outputs_prefix="Inventa.Dsar.DataAssets", outputs_key_field="id"
        )
...
def validate_incident_inputs_command(**kwargs):
    ...
    if constraints_validated:
        return CommandResults(outputs={"validated": True}, outputs_prefix="Inventa.Incident", outputs_key_field="validated")
