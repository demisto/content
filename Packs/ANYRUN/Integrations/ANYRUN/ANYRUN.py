...
def travel_object(obj, key_functions=[underscoreToCamelCase, make_capital, make_singular, make_upper], val_functions=[]):
...
def get_history_command(client: Client, args: Dict):
...
def get_report_command(client: Client, args: Dict):
    ...
    if images:
        returned_results.append(images)
...
def run_analysis_command(client: Client, args: Dict):
    ...
    if not task_id:
        ...
        images = client.get_images_from_report(response)
        if images:
            submit_output.append([image for image in images])
