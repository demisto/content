...
ALWAYS_UPPER_CASE = {"md5", "sha1", "sha256", "sha512", "pcap", "ip", "url", "id", "pid", "ppid", "uuid", "asn", "mime"}
THREAT_TEXT_TO_DBOTSCORE = {"no threats detected": 1, "suspicious activity": 2, "malicious activity": 3}
...
def add_malicious_key(entity, verdict):
    ...
    if threat_level_text.casefold() == "malicious activity":
        entity["Malicious"] = {"Vendor": "ANYRUN", "Description": threat_level_text}
    return entity
...
def ec_file(main_object):
    ...
    file_ec = {"File": {"Name": name, "MD5": md5, "SHA1": sha1, "SHA256": sha256, "SSDeep": ssdeep, "Extension": ext}}
    return file_ec
...
def get_history_command():
    ...
    if contents:
        entry_context: dict | None = {"ANYRUN.Task(val.ID && val.ID === obj.ID)": formatted_contents}
...
def get_report_command():
    ...
    entry_context = {"ANYRUN.Task(val.ID && val.ID === obj.ID)": {"ID": task_id, **formatted_contents}, **dbot_score, **entity}
