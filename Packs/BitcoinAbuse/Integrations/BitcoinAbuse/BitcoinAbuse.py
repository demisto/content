...
class Client(BaseClient):
    ...
    def report_address(self, report_address_params: ReportAddressParams) -> str:
...
def fetch_indicators() -> str:
    params = {k: v for k, v in demisto.params().items() if v is not None}

    fetch_interval = params.get('fetchInterval')  # TODO TOM CHECK NAME And Validate?

    feed_url_to_config = {
        f'{SERVER_URL}{FEED_ENDPOINT_PREFIX}?api_token={fetch_interval}{API_KEY}': {
    ...
    params['url'] = SERVER_URL + FEED_ENDPOINT_PREFIX
...
def report_address_command(client: Client) -> str:
    ...
    report_address_params = Client.ReportAddressParams(
        api_token=API_KEY,
...
def main() -> None:
    ...
    try:
        ...
        if command == 'test-module':
            ...
            return_results(fetch_indicators())
