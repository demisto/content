...
IP_COMMUNITY_HEADERS = ["IP", "Noise", "Riot", "Classification", "Name", "Link", "Last Seen"]
EXCEPTION_MESSAGES = {
...
def get_community_ip_data(responses: list) -> list:
    ...
    for response in responses:
        ...
        ip_responses.append(tmp_response)
...
@exception_handler
@logger
def ip_community_command(client: Client, args: dict) -> List[CommandResults]:
    """Get information about a given IP address from GN Community API. Returns classification (benign, malicious or
        unknown) and basic information from GreyNoise about the IP.
    ...
    for ip in ips:
        ...
        tmp_response = get_community_ip_data([response])
        ...
        human_readable += tableToMarkdown(
            name="GreyNoise Community IP Response",
            t=tmp_response,
            headers=IP_COMMUNITY_HEADERS,
            removeNull=False,
        )
        ...
        command_results.append(
            CommandResults(
                ...
                outputs_prefix="GreyNoise_Community.IP",
...
def main() -> None:
    ...
    api_server = demisto.params().get("api_server", "https://api.greynoise.io")
    ...
    try:
        ...
        elif demisto.command() == "greynoise-community-lookup":
            result = ip_community_command(client, demisto.args())
            return_results(result)
