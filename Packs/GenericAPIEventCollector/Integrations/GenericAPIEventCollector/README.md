...
| Request data | If required to pass DATA when calling the API to collect data, add it here in dictionary format \{unique_field : 286\}. If there's a need to add more then one, use it in this format: \{'field-1': value_example, 'field-2': value_2, 'field-3': value_3\} | False |
...
| Request JSON parameters | If required to pass JSON data when calling the API to collect data, add it here in dictionary format \{unique_field : 286\}. If there's a need to add more then one, use it in this format: \{'field-1': value_example, 'field-2': value_2, 'field-3': value_3\} | False |
...
| Allowed HTTP status codes for successful response from the API | OK codes is a comma-separated list \(e.g., "200,201,202"\). Default is "200". | False |
