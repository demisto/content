"""HelloWorldNew Integration for Cortex XSOAR - Unit Tests file

This file contains the Unit Tests for the HelloWorldNew Integration based
...
demisto-sdk lint -i Packs/HelloWorldNew/Integrations/HelloWorldNew
...
before each test to simulate the API calls to the HelloWorldNew API. This way we
...
def test_say_hello():
    """Tests helloworldNew-say-hello command function.
    ...
    any external API.
    """
    from HelloWorldNew import Client, say_hello_command
...
def test_start_scan(requests_mock):
    """Tests helloworldNew-scan-start command function.
    ...
    """
    from HelloWorldNew import Client, scan_start_command
    ...
    assert response.outputs_prefix == 'HelloWorldNew.Scan'
...
def test_status_scan(requests_mock):
    """Tests helloworldNew-scan-status command function.
    ...
    """
    from HelloWorldNew import Client, scan_status_command
...
def test_scan_results(mocker, requests_mock):
    """Tests helloworldNew-scan-results command function.

    Configures requests_mock instance to generate the appropriate
    get_scan_results API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloWorldNew import Client, scan_results_command
    ...
    assert response[0].outputs_prefix == 'HelloWorldNew.Scan'
...
def test_search_alerts(requests_mock):
    """Tests helloworldNew-search-alerts command function.

    Configures requests_mock instance to generate the appropriate
    get_alerts API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloWorldNew import Client, search_alerts_command
    ...
    assert response.outputs_prefix == 'HelloWorldNew.Alert'
    assert response.outputs_key_field == 'alert_id'
    assert response.outputs == mock_response['alerts']


def test_get_alert(requests_mock):
    """Tests helloworldNew-get-alert command function.

    Configures requests_mock instance to generate the appropriate
    get_alerts API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloWorldNew import Client, get_alert_command
...
def test_update_alert_status(requests_mock):
    """Tests helloworldNew-update-alert-status command function.

    Configures requests_mock instance to generate the appropriate
    get_alerts API response, loaded from a local JSON file. Checks
    the output of the command function with the expected output.
    """
    from HelloWorldNew import Client, update_alert_status_command
...
def test_ip(requests_mock):
    ...
    the output of the command function with the expected output.
    """
    from HelloWorldNew import Client, ip_reputation_command
    from CommonServerPython import Common
    ...
    assert response[0].outputs == mock_response
    assert response[0].outputs_prefix == 'HelloWorldNew.IP'
...
def test_domain(requests_mock):
    ...
    the output of the command function with the expected output.
    """
    from HelloWorldNew import Client, domain_reputation_command
    from CommonServerPython import Common
    ...
    assert response[0].outputs == mock_response
    assert response[0].outputs_prefix == 'HelloWorldNew.Domain'
...
def test_fetch_incidents(requests_mock):
    ...
    the output of the command function with the expected output.
    """
    from HelloWorldNew import Client, fetch_incidents
