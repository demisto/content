...
def results_return(command, thingtoreturn):
    if command == "RecentBlocked":
        results = CommandResults(outputs_prefix="PiHole." + str(command), outputs_key_field="", outputs={"Data": thingtoreturn})
    else:
        results = CommandResults(outputs_prefix="PiHole." + str(command), outputs_key_field="", outputs=thingtoreturn)
...
def main():
    ...
    try:
        client = BaseClient(base_url=base_url, verify=verify_certificate, proxy=proxy)
        ...
        if demisto.command() == "test-module":
            ...
            results_return(
                "List",
                client._http_request("GET", "?list=" + str(usedlist) + "&" + str(action) + "=" + str(domain) + str(authtoken)),
            )
