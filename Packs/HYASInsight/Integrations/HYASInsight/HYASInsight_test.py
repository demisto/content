...
from HYASInsight import Client, get_passive_dns_records_by_indicator, \
    get_dynamic_dns_records_by_indicator, \
    get_whois_records_by_indicator, get_whois_current_records_by_domain, \
    get_malware_samples_records_by_indicator, \
    get_associated_ips_by_hash, get_associated_domains_by_hash, \
    get_c2_attribution_record_by_indicator
...
WHOIS_CURRENT_RAW_RESPONSE1 = util_load_json(
    'test_data/whoiscurrent_input.json')
WHOIS_CURRENT_RAW_RESPONSE2 = util_load_json(
    'test_data/whoiscurrent_response.json')
MALWARE_SAMPLE_RESPONSE = util_load_json(
    'test_data/malwaresample_response.json')
...
ASSOCIATED_DOMAINS = {'md5': '1d0a97c41afe5540edd0a8c1fb9a0f2d',
                      'domains': ['butterfly.bigmoney.biz']}


@pytest.mark.parametrize('raw_response, expected',
                         [(PASSIVE_DNS_RESPONSE, PASSIVE_DNS_RESPONSE)])
def test_get_passive_dns_records_by_indicator(mocker, raw_response, expected):
    ...
    command_results_ipv4 = get_passive_dns_records_by_indicator(client, {
        'indicator_type': 'ipv4',
        'indicator_value': '189.70.45.212',
        'limit': 1})
    command_results_domain = get_passive_dns_records_by_indicator(client, {
    ...
    with pytest.raises(ValueError):
        get_passive_dns_records_by_indicator(client,
                                             {'indicator_type': 'domain',
                                              'indicator_value': '344444'})


@pytest.mark.parametrize('raw_response, expected',
                         [(DYNAMIC_DNS_RESPONSE, DYNAMIC_DNS_RESPONSE)])
def test_get_dynamic_dns_records_by_indicator(mocker, raw_response, expected):
    ...
    command_results_ip = get_dynamic_dns_records_by_indicator(client, {
        'indicator_type': 'ip',
        'indicator_value': '4.4.4.4',
        'limit': 1})
    command_results_domain = get_dynamic_dns_records_by_indicator(client, {
        'indicator_type': 'domain',
        'indicator_value': 'fluber12.duckdns.org',
        'limit': 1})

    command_results_email = get_dynamic_dns_records_by_indicator(client, {
    ...
    with pytest.raises(ValueError):
        get_dynamic_dns_records_by_indicator(client, {'indicator_type': 'tttt',
                                                      'indicator_value': '4.4.4.4'})
    with pytest.raises(ValueError):
        get_dynamic_dns_records_by_indicator(client, {'indicator_type': 'ip',
                                                      'indicator_value': 'gggg'})
    with pytest.raises(ValueError):
        get_dynamic_dns_records_by_indicator(client,
                                             {'indicator_type': 'domain',
                                              'indicator_value': '3333'})
    with pytest.raises(ValueError):
        get_dynamic_dns_records_by_indicator(client, {'indicator_type': 'email',
...
def test_get_whois_records_by_indicator(mocker, raw_response, expected):
    mocker.patch.object(client, 'query', side_effect=[raw_response] * 7)
    command_results_phone = get_whois_records_by_indicator(client, {
        'indicator_type': 'phone',
        'indicator_value': '+84909095309',
        'limit': 1})

    command_results_domain = get_whois_records_by_indicator(client, {
        'indicator_type': 'domain',
        'indicator_value': 'domain.net',
        'limit': 1})

    command_results_email = get_whois_records_by_indicator(client, {
        'indicator_type': 'email',
...
@pytest.mark.parametrize('raw_response, expected', [
    (WHOIS_CURRENT_RAW_RESPONSE1, WHOIS_CURRENT_RAW_RESPONSE2)])
def test_get_whois_current_records_by_domain(mocker, raw_response, expected):
    ...
    command_results_domain = get_whois_current_records_by_domain(client, {
        'domain': 'www.hyas.com'})
...
@pytest.mark.parametrize('raw_response, expected',
                         [(MALWARE_SAMPLE_RESPONSE, MALWARE_SAMPLE_RESPONSE)])
def test_get_malware_samples_records_by_indicator(mocker, raw_response,
                                                  expected):
    mocker.patch.object(client, 'query', side_effect=[raw_response] * 7)
    command_results_ipv4 = get_malware_samples_records_by_indicator(client, {
        'indicator_type': 'ipv4',
        'indicator_value': '106.187.43.98',
        'limit': 1})

    command_results_domain = get_malware_samples_records_by_indicator(client, {
        'indicator_type': 'domain',
        'indicator_value': 'butterfly.bigmoney.biz',
        'limit': 1})

    command_results_md5 = get_malware_samples_records_by_indicator(client, {
        'indicator_type': 'md5',
        'indicator_value':
...
@pytest.mark.parametrize('raw_response, expected',
                         [(ASSOCIATED_IPS_INPUT, ASSOCIATED_IPS)])
def test_get_associated_ips_by_hash(mocker, raw_response, expected):
    mocker.patch.object(client, 'query', side_effect=[raw_response] * 2)

    command_results = get_associated_ips_by_hash(client, {
...
@pytest.mark.parametrize('raw_response, expected',
                         [(ASSOCIATED_DOMAINS_INPUT, ASSOCIATED_DOMAINS)])
def test_get_associated_domains_by_hash(mocker, raw_response, expected):
    mocker.patch.object(client, 'query', side_effect=[raw_response] * 2)

    command_results = get_associated_domains_by_hash(client, {
        'md5': '1d0a97c41afe5540edd0a8c1fb9a0f2d'})
