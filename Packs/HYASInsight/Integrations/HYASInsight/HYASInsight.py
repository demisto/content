...
class Client(BaseClient):
    ...
    def fetch_data_from_hyas_api(
        self, end_point: str, ind_type: str, ind_value: str, current: bool, req_method: str, limit=0
    ) -> list[Dict]:
        """
    ...
    def query(self, end_point: str, ind_type: str, ind_value: str, current: bool, method: str, limit: int) -> list[Dict]:
        """
        ...
        if method == "GET":
            ...
            response = self._http_request("POST", url_suffix=url_path, json_data=req_body, timeout=TIMEOUT)
        if limit != 0:
    ...
    def request_body(query_param: str, query_input: str, current: bool) -> Dict[str, Any]:
        ...
        if current:
            return {"applied_filters": {query_param: query_input, "current": True}}
        else:
            return {"applied_filters": {query_param: query_input}}
...
def check_valid_indicator_type(indicator_type: str, api_query_params: list) -> bool:
    ...
    if indicator_type not in api_query_params:
        raise ValueError(f"Invalid indicator_type: {indicator_type}, Valid indicator_type are {api_query_params}")
    return True
    ...
    if not check_all:
        if MD5_PARAM in hash_type:
            if not re.match(md5Regex, hash_value):
                raise ValueError(f"Invalid indicator_value: {hash_value} for indicator_type {MD5_PARAM}")
        elif SHA1_PARAM in hash_type:
            if not re.match(sha1Regex, hash_value):
                raise ValueError(f"Invalid indicator_value: {hash_value} for indicator_type {SHA1_PARAM}")
        elif SHA256_PARAM in hash_type:
            if not re.match(sha256Regex, hash_value):
                raise ValueError(f"Invalid indicator_value: {hash_value} for indicator_type {SHA256_PARAM}")
        elif SHA512_PARAM in hash_type and not re.match(sha512Regex, hash_value):
            raise ValueError(f"Invalid indicator_value: {hash_value} for indicator_type {SHA512_PARAM}")
    else:
        if (
            re.match(md5Regex, hash_value)
            or re.match(sha1Regex, hash_value)
            or re.match(sha256Regex, hash_value)
            or re.match(sha512Regex, hash_value)
        ):
            pass
        else:
            raise ValueError(f"Invalid indicator_value: {hash_value} for indicator_type {HASH_PARAM}")
...
def get_flatten_json_response(raw_api_response: list[Dict], endpoint: str) -> list[Dict]:
    """
...
def ssl_certificate_lookup_to_markdown(results: list[Dict], title: str) -> str:
    ...
    keys = [  # type: ignore[name-defined]
        ...
        ("ISP Autonomous System Number", "geo_isp_autonomous_system_number", str),
        ("ISP Autonomous System Organization", "geo_isp_autonomous_system_organization", str),
        ...
        ("Issuer Organization Name", "ssl_cert_issuer_organizationName", str),
        ("Issuer Organizational UnitName", "ssl_cert_issuer_organizationalUnitName", str),
        ("Issuer State/Province Name", "ssl_cert_issuer_stateOrProvinceName", str),
        ...
        ("Certificate Subject Common Name", "ssl_cert_subject_commonName", str),
        ("Certificate Subject Country Name", "ssl_cert_subject_countryName", str),
        ("Certificate Subject Locality Name", "ssl_cert_subject_localityName", str),
        ("Certificate Subject Organization Name", "ssl_cert_subject_organizationName", str),
        ("Certificate Subject Organizational Unit Name", "ssl_cert_subject_organizationalUnitName", str),
        ("Certificate Subject State/Province Name", "ssl_cert_subject_stateOrProvinceName", str),
    ...
    if scan_results:
        for res in scan_results:
            malware_info_data = {
                ...
                "MD5": results.get("md5", ""),
                "AV Name": res.get("av_name", ""),
                "AV DateTime": res.get("def_time", ""),
                "Source": res.get("threat_found", ""),
...
def get_malware_sample_information_by_hash(client, args):
    ...
    title = get_command_title_string(MALWARE_INFO_SUB_CONTEXT, HASH_PARAM, hash_value)
    end_point = MALWARE_INFO_ENDPOINT
    raw_api_response = client.fetch_data_from_hyas_api(end_point, HASH_PARAM, hash_value, False, "POST")
    lookup_result: str = ""
    if raw_api_response:
...
def get_sinkhole_records_by_ipv4_address(client, args):
    ...
    title = get_command_title_string(SINKHOLE_SUB_CONTEXT, IPV4_PARAM, ipv4_value)
    end_point = SINKHOLE_ENDPOINT
    raw_api_response = client.fetch_data_from_hyas_api(end_point, IPV4_PARAM, ipv4_value, False, "POST", limit)
    if raw_api_response:
        flatten_json_response = get_flatten_json_response(raw_api_response, end_point)

    return CommandResults(
        readable_output=sinkhole_indicators_lookup_to_markdown(flatten_json_response, title),
...
def get_device_geo_records_by_ip_address(client, args):
    ...
    title = get_command_title_string(DEVICE_GEO_SUB_CONTEXT, indicator_type, indicator_value)
    ...
    return CommandResults(
        readable_output=device_geo_indicators_lookup_to_markdown(flatten_json_response, title),
...
def get_opensource_indicator_records_by_indicator(client, args):
    ...
    check_valid_indicator_value(indicator_type, indicator_value)
    title = get_command_title_string(OS_INDICATORS_SUB_CONTEXT, indicator_type, indicator_value)
    ...
    return CommandResults(
        readable_output=open_source_indicators_lookup_to_markdown(flatten_json_response, title),
...
def get_ssl_certificate_records_by_indicator(client, args):
    ...
    check_valid_indicator_value(indicator_type, indicator_value)
    title = get_command_title_string(SSL_CERTIFICATE_SUB_CONTEXT, indicator_type, indicator_value)
    end_point = SSL_CERTIFICATE_ENDPOINT
    raw_api_response = client.fetch_data_from_hyas_api(end_point, indicator_type, indicator_value, False, "POST", 0)
    if raw_api_response:
    ...
    return CommandResults(
        readable_output=ssl_certificate_lookup_to_markdown(flatten_json_response, title),
...
def get_passive_hash_records_by_indicator(client, args):
    ...
    check_valid_indicator_value(indicator_type, indicator_value)
    title = get_command_title_string(PASSIVE_HASH_SUB_CONTEXT, indicator_type, indicator_value)
    ...
    return CommandResults(
        readable_output=passive_hash_lookup_to_markdown(flatten_json_response, title),
...
def get_c2attribution_records_by_indicator(client, args):
    ...
    check_valid_indicator_value(indicator_type, indicator_value)
    title = get_command_title_string(C2_ATTRIBUTION_SUB_CONTEXT, indicator_type, indicator_value)
    end_point = C2_ATTRIBUTION_ENDPOINT
    raw_api_response = client.fetch_data_from_hyas_api(end_point, indicator_type, indicator_value, False, "POST", limit)
    if raw_api_response:
        flatten_json_response = get_flatten_json_response(raw_api_response, end_point)
    outputs_key_field = {"ip": "actor_ipv4", "domain": "c2_domain", "email": "email", "sha256": "sha256"}

    return CommandResults(
        readable_output=c2_attribution_lookup_to_markdown(flatten_json_response, title),
...
def main():
    ...
    try:
        ...
        if command == "test-module":
            ...
            return_results(get_c2attribution_records_by_indicator(client, demisto.args()))
        elif command == f"{INTEGRATION_COMMAND_NAME}-get-passive-hash-records-by-indicator":
            return_results(get_passive_hash_records_by_indicator(client, demisto.args()))
        elif command == f"{INTEGRATION_COMMAND_NAME}-get-ssl-certificate-records-by-indicator":
            return_results(get_ssl_certificate_records_by_indicator(client, demisto.args()))
        elif command == f"{INTEGRATION_COMMAND_NAME}-get-opensource-indicator-records-by-indicator":
            return_results(get_opensource_indicator_records_by_indicator(client, demisto.args()))
        elif command == f"{INTEGRATION_COMMAND_NAME}-get-device-geo-records-by-ip-address":
            return_results(get_device_geo_records_by_ip_address(client, demisto.args()))
        elif command == f"{INTEGRATION_COMMAND_NAME}-get-sinkhole-records-by-ipv4-address":
            return_results(get_sinkhole_records_by_ipv4_address(client, demisto.args()))
        elif command == f"{INTEGRATION_COMMAND_NAME}-get-malware-sample-information-by-hash":
            return_results(get_malware_sample_information_by_hash(client, demisto.args()))
            # Log exceptions
