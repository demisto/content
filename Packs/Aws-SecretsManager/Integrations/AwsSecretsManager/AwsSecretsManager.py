...
def aws_secrets_manager_secret_list_command(client: AWSClient, args: dict[str, Any]):
    ...
    return_results(
        CommandResults(
            outputs_prefix="AWS.SecretsManager.Secret", outputs=response, outputs_key_field="Name", readable_output=human_readable
        )
    )
...
def aws_secrets_manager_secret_policy_get_command(client: AWSClient, args: dict[str, Any]):
    ...
    response = client.get_resource_policy(SecretId=args.get("secret_id"))
    ...
    return_results(
        CommandResults(
            outputs_prefix="AWS.SecretsManager.Policy", outputs=response, outputs_key_field="Name", readable_output=human_readable
        )
    )
...
def main():  # pragma: no cover:
    try:
        ...
        if argToBoolean(params.get("disable_sensitive_commands") or False) and demisto.command() in SENSITIVE_COMMANDS:
        ...
        verify_certificate = not argToBoolean(params.get("insecure") or False)
        timeout = params.get("timeout")
        retries = int(params.get("retries")) if params.get("retries") else 5  # noqa: ucp-param-default  (guarded by `if params.get("retries")`; int() never sees None)
