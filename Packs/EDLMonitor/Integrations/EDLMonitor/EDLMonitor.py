...
def sendemail(reason, create_ts, email, email_server, email_user, email_pwd, txtfile=None):
    ...
    if txtfile is not None:
        ...
        msg.add_header("Content-Disposition", "attachment", filename="EDL Monitor Results.zip")
        themsg.attach(msg)
    else:
...
def check_edl(
    cmd, start_time, EDL, edl_user, edl_pwd, verify_certificate, email, email_server, email_user, email_pwd, timeout, mon_contents
):
    try:
