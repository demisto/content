...
SERVER_URL = (
    demisto.params().get("server")[:-1] if demisto.params().get("server").endswith("/") else demisto.params().get("server")
)
...
def http_request(method, suffix_url, headers=DEFAULT_HEADERS, body=None):
    ...
    response = requests.request(method, url, auth=(USERNAME, PASSWORD), headers=headers, verify=USE_SSL, data=body)
...
def get_endpoints():
    ...
    context = {
        "CounterTack.Endpoint(val.Id && val.Id === obj.Id)": createContext(endpoints, keyTransform=underscoreToCamelCase),
    ...
    entry = {
        ...
        "HumanReadable": tableToMarkdown("CounterTack Endpoints", data, headers, removeNull=True),
...
def get_endpoint():
    ...
    entry = {
        ...
        "ContentsFormat": formats["json"],
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown("CounterTack Endpoint information:", content, headers, removeNull=True),
        "EntryContext": context,
    }
    demisto.results(entry)
...
def get_endpoint_tags():
    ...
    tags_context = {"Id": endpoint_id, "tags": response}
...
def add_tags():
    ...
    response = {"tags": response, "Id": endpoint_id}
...
def delete_tags():
    ...
    entry = {
        ...
        "ContentsFormat": formats["json"],
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown("Endpoint tags were deleted successfully", response),
        "EntryContext": context,
    }
    demisto.results(entry)
...
def endpoint_quarantine():
    ...
    quarantine_context = {"Id": endpoint_id, "is_quarantine": quarantine_response.get("is_quarantined")}
...
def extract_file():
    ...
    body = {"type": "extract_files", "paths": paths}
    ...
    entry = {
        ...
        "ContentsFormat": formats["json"],
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown("The file has been extracted successfully:", data, headers, removeNull=True),
        "EntryContext": context,
    }
    demisto.results(entry)
...
def delete_file():
    ...
    body = {"type": "delete_file", "path": path}
...
def kill_process():
    ...
    body = {"type": "kill_process", "pid": pid, "name": name}
    ...
    context = {
        "CounterTack.Endpoint(val.Id && val.Id === obj.Id)": createContext(
            response, keyTransform=underscoreToCamelCase, removeNull=True
        )
    }
    ...
    entry = {
        ...
        "ContentsFormat": formats["json"],
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown("The process has been terminated", data, removeNull=True),
        "EntryContext": context,
    }
    demisto.results(entry)
...
def get_all_files():
    ...
    entry = {
        ...
        "ContentsFormat": formats["json"],
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown("CounterTack Endpoints Files", data, headers, removeNull=True),
        "EntryContext": context,
    }
    demisto.results(entry)
...
def get_endpoint_files():
    ...
    entry = {
        ...
        "ContentsFormat": formats["json"],
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown("The extracted files from the endpoint:", data, headers, removeNull=True),
        "EntryContext": context,
    }
    demisto.results(entry)
...
def get_behaviors():
    ...
    context = {"CounterTack.Behavior(val.Id && val.Id === obj.Id)": createContext(behaviors, keyTransform=underscoreToCamelCase)}
...
def get_behavior():
    ...
    context = {"CounterTack.Behavior(val.Id && val.Id === obj.Id)": createContext(response, keyTransform=underscoreToCamelCase)}
...
def add_behavior_tags():
    ...
    response = {"tags": behavior_tags.get("tags"), "Id": behaviour_id}
...
def delete_behavior_tags():
    ...
    response = {"tags": response.get("tags"), "Id": behaviour_id}
...
def search_events():
    ...
    if events.get("results"):
        ...
        context = {
            "CounterTack.Event(val.Id && val.Id === obj.Id)": createContext(
                results_lst, keyTransform=underscoreToCamelCase, removeNull=True
            )
        }
        headers = [
            "ID",
...
def search_behaviors():
    ...
    if behaviors.get("results"):
        ...
        context = {
            "CounterTack.Behavior(val.Id && val.Id === obj.Id)": createContext(
                results_lst, keyTransform=underscoreToCamelCase, removeNull=True
            )
        }
...
def search_hashes():
    ...
    if hashes.get("results"):
        ...
        for hash_type in hashes.get("results"):
            ...
            if file_hash_type:
                file_standards.append({file_hash_type: hash_id})
...
def fetch_incidents():
    ...
    demisto.setLastRun({"time_stamp": max_timestamp})
