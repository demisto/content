...
Manage Prisma Cloud Compute alerts in XSOAR.
...
Configure Prisma Cloud Compute to send alerts to XSOAR by creating an alert profile.
...
1. On the right, select the alert triggers. Alert triggers specify which alerts are sent to XSOAR.
...
3 of the above playbooks (all except _Audit Alert v3_) contain a single script. The script in each playbook encode the raw JSON alerts into XSOAR objects that can then be used in the playbooks. The scripts are:
...
</br>The playbook has the following sections:
Enrichment:
- Image details
...
If any alerts are missing in XSOAR, check the status of the integration:
