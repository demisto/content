...
class PrismaCloudComputeClient(BaseClient):
    ...
    def add_custom_md5_malware(self, feeds: List[str]) -> None:
        ...
        """
        self._http_request(method="PUT", url_suffix="/feeds/custom/malware", json_data={"feed": feeds}, resp_type="text")
        ...
        while current_count < total_count:
            try:
                ...
                if de.res.status_code == 429:
                    ...
                    demisto.info(
                        f"Rate limit exceeded, waiting 2 seconds before continuing.\n"
    ...
    def get_impacted_resources(self, cve: str, resource_type: str) -> dict:
        ...
        return self._http_request(method="GET", url_suffix="/stats/vulnerabilities/impacted-resources", params=params)
    ...
    def get_waas_policies(self) -> dict:
        ...
        """
        return self._http_request(method="GET", url_suffix="policies/firewall/app/container")
    ...
    def update_waas_policies(self, policy: dict) -> dict:
        ...
        return self._http_request(
            method="PUT",
            url_suffix="policies/firewall/app/container",
            json_data=policy,
            resp_type="response",
            ok_codes=(200),
    ...
    def get_firewall_audit_container_alerts(self, image_name: str, from_time: str, to_time: str, limit: int, audit_type: str):
        ...
        """
        params = {"type": audit_type, "imageName": image_name, "from": from_time, "to": to_time, "limit": limit}
        return self._http_request(method="GET", url_suffix="audits/firewall/app/container", params=params)
    ...
    def get_file_integrity_events(
        self, limit, sort, hostname=None, event_id=None, from_date=None, to_date=None, search_term=None
    ):
        """
            ...
            for a in alerts:
                ...
                for key, value in a.items():
                    ...
                    if isinstance(value, list) and value and isinstance(value[0], dict):
                        tables[key + "MarkdownTable"] = tableToMarkdown(
                            camel_case_transformer(key + " table"),
                            value,
                ...
                incidents.append({"name": name, "occurred": a.get("time"), "severity": severity, "rawJSON": json.dumps(a)})
...
def get_hostname_description_info(host_info: dict) -> dict:
    ...
    return {"Hostname": host_info.get("_id"), "Distribution": dist, "Collections": host_info.get("collections")}
...
def get_profile_host_list(client: PrismaCloudComputeClient, args: dict) -> CommandResults:
    ...
    args["limit"], args["offset"] = parse_limit_and_offset_values(limit=args.get("limit", "15"), offset=args.get("offset", "0"))
...
def get_container_hosts_list(client: PrismaCloudComputeClient, args: dict) -> CommandResults:
    ...
    if hosts := filter_api_response(
        api_response=client.get_containers_hosts(container_id=container_id), limit=limit, offset=offset
    ):
        context_output = {"containerID": container_id, "hostsIDs": hosts}
        table = tableToMarkdown(
            name="Hosts", t=context_output, headers=["hostsIDs"], headerTransform=lambda word: word[0].upper() + word[1:]
        )
    else:
    ...
    if container_forensics := filter_api_response(
        ...
        for forensic in container_forensics:
            ...
            if "listeningStartTime" in forensic:
                forensic["listeningStartTime"] = parse_date_string_format(date_string=forensic.get("listeningStartTime", ""))

        context_output = {"containerID": container_id, "hostname": args.get("hostname"), "Forensics": container_forensics}
...
def get_profile_host_forensic_list(client: PrismaCloudComputeClient, args: dict) -> CommandResults:
    ...
    if host_forensics := filter_api_response(
        api_response=client.get_host_forensics(host_id=host_id, params=assign_params(**args)), limit=limit, offset=offset
    ):
        ...
        context_output = {"hostID": host_id, "Forensics": host_forensics}
    ...
    if filtered_cves_information := filter_api_response(api_response=all_cves_information, limit=MAX_API_LIMIT):
        for cve_info in filtered_cves_information:
            ...
            if cve_id not in unique_cve_ids:
                ...
                cve_data = {"ID": cve_id, "CVSS": cvss, "Modified": modified, "Description": description}
                ...
                results.append(
                    CommandResults(
                        ...
                        indicator=Common.CVE(
                            id=cve_id, cvss=cvss, published="", modified=modified, description=description, dbot_score=dbot_score
                        ),
...
def get_defenders(client: PrismaCloudComputeClient, args: dict) -> CommandResults:
    ...
    Returns:
        CommandResults: command-results object.
    """
    limit, offset = parse_limit_and_offset_values(limit=args.get("limit", "20"), offset=args.get("offset", "0"))
    cluster, connected, hostname, type = (args.get("cluster"), args.get("connected"), args.get("hostname"), args.get("type"))
    params = assign_params(cluster=cluster, connected=connected, hostname=hostname, type=type, limit=limit, offset=offset)
            ...
            t=[
                {
                    ...
                    if defender.get("connected")
                    else f"Disconnected since {defender.get('lastModified')}",
...
def get_namespaces(client: PrismaCloudComputeClient, args: dict) -> CommandResults:
    ...
    if namespaces := filter_api_response(api_response=client.get_namespaces(params=params), limit=limit):
        ...
        if len(namespaces) == 1 and namespaces[0] == "":
            ...
            table = tableToMarkdown(
                name="Namespaces", t=[{"Namespace": namespace} for namespace in namespaces], headers=["Namespace"]
            )
    else:
        namespaces, table = [], "No results found."
...
def get_images_scan_list(client: PrismaCloudComputeClient, args: dict) -> CommandResults:
    ...
    Returns:
        CommandResults: command-results object.
    """
    limit, offset = parse_limit_and_offset_values(limit=args.pop("limit_record", "10"), offset=args.get("offset", "0"))
        ...
        for scan in images_scans:
            if "vulnerabilities" in scan:
                ...
                scan["vulnerabilities"] = (
                    filter_api_response(api_response=scan.get("vulnerabilities"), limit=stats_limit)
                    if not all_results
                    else scan.get("vulnerabilities")
                )
            ...
            if "complianceIssues" in scan:
                ...
                scan["complianceIssues"] = (
                    filter_api_response(api_response=scan.get("complianceIssues"), limit=stats_limit)
                    if not all_results
                    else scan.get("complianceIssues")
                )
...
def get_hosts_scan_list(client: PrismaCloudComputeClient, args: dict) -> CommandResults:
    ...
    if hosts_scans := client.get_hosts_scan_info(all_results, params=params):
        ...
        host_description_table = tableToMarkdown(
            ...
            headers=["Hostname", "Docker Version", "OS Distribution", "Vulnerabilities Count", "Compliance Issues Count"],
            removeNull=True,
        )
...
def get_impacted_resources(client: PrismaCloudComputeClient, args: dict) -> CommandResults:
    ...
    if context_output:
        ...
        mapping_resources_to_names = {
            "images": "Impacted Images",
        ...
        for resources in resources_list:
            if final_impacted_resources.get(resources):
                impacted_resources_tables.append(
                    tableToMarkdown(
...
def get_waas_policies(client: PrismaCloudComputeClient, args: dict) -> List[CommandResults]:
    ...
    for rule in policies.get("rules") or {}:
        for spec in rule.get("applicationsSpec") or {}:
            ...
            data = {"Name": rule.get("name"), "WaasPolicy": formatted_waas_policy}
...
def update_waas_policies(client: PrismaCloudComputeClient, args: dict) -> CommandResults:
    ...
    return CommandResults(readable_output=txt)
...
def get_settings_defender_command(client: PrismaCloudComputeClient, args: dict):
    ...
    return CommandResults(
        outputs_prefix="PrismaCloudCompute.DefenderSettings", outputs=format_context(response), raw_response=response
    )
...
def get_logs_defender_command(client: PrismaCloudComputeClient, args: dict):
    ...
    entry = {"Hostname": hostname, "Logs": response}
    return CommandResults(
...
def get_file_integrity_events_command(client: PrismaCloudComputeClient, args: dict):
    ...
    response = client.get_file_integrity_events(
        limit, sort, hostname=hostname, event_id=event_id, from_date=from_date, to_date=to_date, search_term=search_term
    )
    if not response:
...
def unstuck_fetch_stream_command():
    ...
    return CommandResults(readable_output="The fetch stream was released successfully.")
...
async def preform_fetch_assets_main_loop_logic(client: PrismaCloudComputeAsyncClient):
    ...
    tasks = [collect_assets_and_sent_to_xsiam(client, tas_droplets_related_data),
...
async def collect_assets_and_sent_to_xsiam(client: PrismaCloudComputeAsyncClient, asset_type_related_data: AssetTypeRelatedData):
    while True:
        ...
        res = await client._http_request(
            "GET",
            url_suffix=asset_type_related_data.endpoint,
            headers=None,
            params=params,
            timeout=300
            )
...
def main():
    ...
    # Log exceptions
        ...
        return_error(f"Failed to execute {requested_command} command. Error: {e!s}")
