...
def paginate(args: Dict[str, Any], results: Generator) -> List:
    ...
    if page and page_size:
        try:
            all_pages = [next(results) for _ in range(number_of_entries)]
...
def build_analysis_hr(analysis: Dict[str, Any]) -> Dict[str, Any]:
    ...
    hr_analysis = {
        ...
        "Result": list({run.get("detection") for run in analysis.get("runs", [])}),
        "Tags": tags,
...
def build_quota_hr(res: Dict[str, Any]) -> tuple[Dict[str, Any], List[str]]:
    ...
    headers = [
        ...
        "Daily Quota Current",
        "Daily Quota Limit",
        "Daily Quote Remaining",
        "Monthly Quota Current",
        "Monthly Quota Limit",
        "Monthly Quota Remaining",
    ]
    hr = {
        "Quota Type": res.get("type"),
        "Daily Quota Current": res.get("quota", {}).get("daily", {}).get("current", 0),
...
def build_submission_hr(res: Dict[str, Any], analyses: List[Dict[str, Any]]) -> tuple[Dict[str, Any], List[str]]:
    ...
    hr = {
        "Submission Id": res.get("submission_id"),
        "Sample Name": res.get("name"),
        "Time": res.get("time"),
        "Status": res.get("status"),
        "Web Id": analyses[0].get("webid"),
        "Encrypted": analyses[0].get("encrypted"),
        "Analysis Id": analyses[0].get("analysisid"),
        "Classification": analyses[0].get("classification"),
        "Threat Name": analyses[0].get("threatname"),
    ...
    if res.get("analyses", [{}])[0].get("sha256"):
        ...
        hr.update(
            {
                "SHA256": analyses[0].get("sha256"),
                "MD5": analyses[0].get("md5"),
                "SHA1": analyses[0].get("sha1"),
...
def build_indicator_object(
    client: Client, analysis: Dict[str, Any], analyses: List[Dict[str, Any]], is_reputation: bool = False
) -> CommandResults:
    """
...
def build_file_object(
    client: Client, analysis: Dict[str, Any], analyses: List[Dict[str, Any]], is_reputation: bool
) -> CommandResults:
    """
    ...
    )  # Find the max dbot score between all the analysis results.
    ...
    indicator = Common.File(
        name=file_name, sha1=sha1, sha256=sha256, dbot_score=dbot_score, md5=md5, tags=tags, relationships=relationships
    )
    if is_reputation:
        return CommandResults(
            ...
            outputs_prefix="Joe.File",
            readable_output=tableToMarkdown("File Result:", hr, headers),
        )
    return CommandResults(
        indicator=indicator, relationships=relationships, readable_output=tableToMarkdown("File Result:", hr, headers)
    )
...
def build_url_object(
    ...
    if is_reputation:
        return CommandResults(
            ...
            indicator=indicator,
            relationships=relationships,
            outputs_prefix="Joe.URL",
            readable_output=tableToMarkdown("Url Result:", {"Url": url}),
        )
    return CommandResults(
        indicator=indicator, relationships=relationships, readable_output=tableToMarkdown("Url Result:", {"Url": url})
    )
...
def build_search_command_result(analyses: List[Dict[str, Any]]) -> CommandResults:
    ...
    return CommandResults(
        ...
        outputs_prefix="Joe.Analysis",
        outputs_key_field="Id",
    )


def build_analysis_command_result(client: Client, analyses: List[Dict[str, Any]], full_display: bool) -> List[CommandResults]:
    """
...
def build_submission_command_result(
    client: Client, res: Dict[str, Any], args: Dict[str, Any], exe_metrics: ExecutionMetrics, download_report: bool
    ...
    command_results.append(
        CommandResults(
            ...
            execution_metrics=exe_metrics.get_metric_list(),
            raw_response=res_copy,
        )
    )
...
def build_submission_params(args: Dict[str, Any], on_premise: bool) -> Dict[str, Any]:
    ...
    params = {
        ...
        "systems": argToList(args.get("systems")),
        "tags": argToList(args.get("tags")),
        "internet-access": argToBoolean(args.get("internet_access", True)),
        ...
        "document-password": args.get("document_password"),
        "archive-password": args.get("archive_password"),
...
def file_submission(
    client: Client, args: Dict[str, Any], params: Dict[str, Any], file: str, exe_metrics: ExecutionMetrics
) -> PollResult:
    """
    ...
    with open(file_path["path"], "rb") as f:
        ...
        return PollResult(
            ...
            args_for_next_run={"submission_id": res.get("submission_id"), **args},
...
def url_submission(
    client: Client, args: Dict[str, Any], params: Dict[str, Any], url: str, exe_metrics: ExecutionMetrics
) -> PollResult:
    """
    ...
    if submission_id := args.get("submission_id"):
        ...
        return PollResult(
            ...
            partial_result=CommandResults(
                outputs=res,  # this is what the response will be in case job has finished
                outputs_prefix="Joe.Submission",
                outputs_key_field="submission_id",
...
def is_online_command(client: Client) -> CommandResults:
    ...
    return CommandResults(
        outputs_prefix="Joe.ServerStatus.Online", outputs=result.get("online"), readable_output=f"Joe server is {status}"
    )
...
def list_lia_countries_command(client: Client) -> CommandResults:
    ...
    if res:
        ...
        return CommandResults(
            outputs_prefix="Joe.LIACountry", outputs=data, readable_output=tableToMarkdown("Results:", {"Name": data})
        )
    return CommandResults(readable_output="No Results were found.")
...
def list_lang_locales_command(client: Client) -> CommandResults:
    ...
    if res:
        ...
        return CommandResults(
            outputs_prefix="Joe.LangLocale", outputs=data, readable_output=tableToMarkdown("Results:", {"Name": data})
        )
    return CommandResults(readable_output="No Results were found.")
...
def get_account_quota_command(client: Client) -> CommandResults:
    ...
    if res:
        ...
        return CommandResults(
            outputs_prefix="Joe.AccountQuota", outputs=res, readable_output=tableToMarkdown("Results:", hr, headers=headers)
        )
    return CommandResults(readable_output="No Results were found.")


def submission_info_command(
    client: Client, args: Dict[str, Any], exe_metrics: ExecutionMetrics
) -> List[Union[CommandResults, Dict[str, Any]]]:
    """
    ...
    for submission_id in submission_ids:
        ...
        if res:
            ...
            command_results.append(CommandResults(readable_output=f"No Results were found for submission {submission_id}."))
    return command_results
