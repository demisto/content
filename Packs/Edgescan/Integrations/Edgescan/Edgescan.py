...
class Client(BaseClient):
    ...
    def asset_create_request(self, id_, name, priority, type_, authenticatied, tags, location_specifiers):
        data = {
            "asset": {
                ...
                "id": id_,
                "location_specifiers": location_specifiers,
                "name": name,
                "priority": priority,
                "tags": tags,
                "type": type_,
    ...
    def vulnerabilities_get_export_request(self, export_format):
        headers = self._headers
        response = self._http_request("GET", "api/v1/vulnerabilities/export." + export_format, headers=headers, resp_type="Raw")
        return response
    ...
    def vulnerabilities_get_query_request(self, request, limit, o):
        data = {"c": request, "l": limit, "o": o, "s": {"date_opened": "asc"}}
        headers = self._headers
    ...
    def vulnerabilities_risk_accept_request(self, id, value):
        ...
        response = self._http_request(
            "POST", "api/v1/vulnerabilities/" + id + "/risk_accept.json", json_data=data, headers=headers
        )
        return response
    ...
    def vulnerabilities_add_annotation_request(self, id, text):
        ...
        headers = self._headers
        response = self._http_request(
            "POST", "api/v1/vulnerabilities/" + id + "/annotations.json", json_data=data, headers=headers
        )
        return response
...
def fetch_incidents(
    ...
    next_run = {"last_fetch": latest_created_time, "offset": offset, "total": total}
...
def host_update_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="Edgescan.HostUpdate", outputs_key_field="", outputs=response, raw_response=response
    )
...
def asset_get_assets_command(client, args):
    ...
    readable_output = tableToMarkdown("Assets", response, ["id", "name", "asset_status", "blocked_status", "hostname"])
    command_results = CommandResults(
        readable_output=readable_output,
...
def asset_get_query_command(client, args):
    ...
    readable_output = tableToMarkdown("Assets query", response, ["id", "name", "asset_status", "blocked_status", "hostname"])
    command_results = CommandResults(
        readable_output=readable_output,
...
def asset_create_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="Edgescan.AssetCreate", outputs_key_field="", outputs=response, raw_response=response
    )
...
def asset_update_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="Edgescan.AssetUpdate", outputs_key_field="", outputs=response, raw_response=response
    )
...
def asset_delete_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="Edgescan.AssetDelete", outputs_key_field="", outputs=response, raw_response=response
    )
...
def user_get_users_command(client, args):
    ...
    readable_output = tableToMarkdown("Users", response, ["id", "username", "email", "phone_number", "mfa_enabled"])
    command_results = CommandResults(
        readable_output=readable_output,
...
def user_get_command(client, args):
    ...
    readable_output = tableToMarkdown("User", response, ["id", "username", "email", "phone_number", "mfa_enabled"])
    command_results = CommandResults(
        readable_output=readable_output,
...
def user_get_query_command(client, args):
    ...
    readable_output = tableToMarkdown("User query", response, ["id", "username", "email", "phone_number", "mfa_enabled"])
    command_results = CommandResults(
        readable_output=readable_output,
...
def user_create_command(client, args):
    ...
    response = client.user_create_request(
        username, email, first_name, last_name, phone_number, mfa_enabled, mfa_method, is_super
    )["user"]
...
def user_reset_password_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="Edgescan.UserResetPassword", outputs_key_field="", outputs=response, raw_response=response
    )
...
def user_reset_email_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="Edgescan.UserResetEmail", outputs_key_field="", outputs=response, raw_response=response
    )
