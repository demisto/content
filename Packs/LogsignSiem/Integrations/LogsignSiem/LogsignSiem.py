...
class Client(BaseClient):
    ...
    def get_incidents(self, method: str, last_run: Any, query: str) -> Any:
        ...
        return self._http_request(
            ...
            params={"api_key": self._api_key, "last_run": last_run, "query": query if query is not None else ""},
        )

    def get_query(self, method: str, query: str, url_suffix: str, grouped_column: str, criteria: str, time_frame: str):
        """
        Get-Query Service
...
def get_generic_data(data: dict[str, Any], key: str, output_prefix: str) -> CommandResults:
    ...
    return CommandResults(outputs_prefix=output_prefix, outputs=result, raw_response=json.dumps(data))
...
def fetch_incidents(client: Client, first_fetch: str, max_fetch: int, query: str) -> tuple[dict[str, str], list[dict]]:
    ...
    for incident in data["incidents"]:
        ...
        inc = {"name": f"Logsign-{created_at_str}", "occured": created_at_str, "rawJSON": json.dumps(incident)}
