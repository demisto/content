...
RELATIONSHIP_TYPE = {
    "page": {
        ...
        "ip": {"indicator_type": FeedIndicatorType.IP, "name": EntityRelationship.Relationships.HOSTED_ON, "detect_type": False},
    }
}
...
def http_request(client, method, url_suffix, json=None, retries=0):
    headers = {"API-Key": client.api_key, "Accept": "application/json"}
    if client.user_agent:
        headers["User-Agent"] = client.user_agent
    if method == "POST":
        headers.update({"Content-Type": "application/json"})
    demisto.debug(f"requesting https request with method: {method}, url: {client.base_api_url + url_suffix}, data: {json}")
    r = requests.request(method, client.base_api_url + url_suffix, data=json, headers=headers, verify=client.use_ssl)
    ...
    **k,
    ...
    while True:
        ...
        try:
            ...
            last_item = e
            demisto.debug(f"Polling request failed with exception {e!s}")
        else:
...
def create_relationship(scan_type, field, entity_a, entity_a_type, entity_b, entity_b_type, reliability):
    ...
    return EntityRelationship(
        name=RELATIONSHIP_TYPE.get(scan_type, {}).get(field, {}).get("name", ""),
        entity_a=entity_a,
...
def format_results(client, uuid, use_url_as_name, scan_lists_attempts=True):
    ...
    if feed_related_indicators:
        ...
        for related_indicator in feed_related_indicators:
            related_indicators.append(
                Common.FeedRelatedIndicators(value=related_indicator["value"], indicator_type=related_indicator["type"])
            )
        url_cont["FeedRelatedIndicators"] = related_indicators
    if demisto.params().get("create_relationships") is True:
        relationships = create_list_relationships({"page": scan_page}, url_query, client.reliability)
    outputs = {"URLScan(val.URL && val.URL == obj.URL)": cont, outputPaths["file"]: file_context}
    ...
    dbot_score = Common.DBotScore(
        ...
        indicator_type=dbot_score.get("Type"),
        integration_name=BRAND,
        score=dbot_score.get("Score"),
        reliability=dbot_score.get("Reliability"),
    )

    url = Common.URL(
        url=url_cont.get("Data"),
...
def urlscan_search_only(client: Client, url: str, command_results: list, execution_metrics: ExecutionMetrics):
    ...
    if not found_result:
        ...
        command_results.append(CommandResults(readable_output=no_results_message, raw_response=no_results_message))
    return
...
def cert_format(x):
    ...
    info = {"Subject Name": x["subjectName"], "Issuer": x["issuer"], "Validity": f"{valid_to} - {valid_from}"}
    ec_info = {"SubjectName": x["subjectName"], "Issuer": x["issuer"], "ValidFrom": valid_from, "ValidTo": valid_to}
    return info, ec_info
