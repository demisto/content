...
def test_assets_search(requests_mock):
    ...
    requests_mock.get(f"{BASE_URL}/org/assets", json=mock_response)

    client = get_client()

    actual_commandResult = asset_search_command(client=client, args={})
    ...
    expected_commandResult = CommandResults(
...
def test_assets_search_exits_if_invalid_args(requests_mock):
    ...
    with pytest.raises(SystemExit) as excinfo:
        asset_search_command(client=client, args={"ips": "192.168.1.1", "hostnames": "localhost"})
...
def test_service_search_exits_if_invalid_args(requests_mock):
    ...
    requests_mock.get(f"{BASE_URL}/org/services", json=mock_response)

    client = get_client()
    with pytest.raises(SystemExit) as excinfo:
        service_search_command(
            client=client, args={"service_id": "04d60ddf-8d28-494c-8186-8cd514e5b9cb", "search": "ips:192.168.1.1"}
        )
    assert excinfo.value.code == 0
...
def test_wireless_search_exits_if_invalid_args(requests_mock):
    ...
    requests_mock.get(f"{BASE_URL}/org/wireless", json=mock_response)

    client = get_client()
    with pytest.raises(SystemExit) as excinfo:
        wireless_lan_search_command(
            client=client, args={"wireless_id": "04d60ddf-8d28-494c-8186-8cd514e5b9cb", "search": "interface:wlan0"}
        )
    assert excinfo.value.code == 0
...
def test_asset_search(requests_mock):
    ...
    mock_response = util_load_json("test_data/assets.json")
    requests_mock.get(f"{BASE_URL}/org/assets?search=address:192.168.1.91", json=mock_response)

    client = client = get_client()

    actual_commandResult = asset_search_command(
        client=client, args={"ips": "192.168.1.91", "display_attributes": "True", "display_services": "True"}
    )
    readable_output = util_load_file("test_data/asset_hr.txt")
    expected_commandResult = CommandResults(
        outputs_prefix="RunZero.Asset",
...
def test_comment_add(requests_mock):
    ...
    mock_response = util_load_json("test_data/assets.json")
    requests_mock.patch(f"{BASE_URL}/org/assets/{ASSET_ID}/comments", json=mock_response)
...
def test_tag_add(requests_mock):
    ...
    mock_response = util_load_json("test_data/assets.json")
    requests_mock.patch(f"{BASE_URL}/org/assets/{ASSET_ID}/tags", json=mock_response)

    client = get_client()

    actual_commandResult = tags_add_command(client=client, args={"asset_id": ASSET_ID, "tags": "tag1 tag2"})
...
def test_service_search(requests_mock):
    ...
    actual_commandResult = service_search_command(client=client, args={"display_attributes": "True"})
...
def test_service_search_using_search_string(requests_mock):
    ...
    actual_commandResult = service_search_command(
        client=client, args={"search": "service_address:192.168.1.91", "display_attributes": "False"}
    )
...
def test_quota_get(requests_mock):
    ...
    requests_mock.get(f"{BASE_URL}/org/key", json=mock_response)
...
def test_asset_delete(requests_mock):
    ...
    requests_mock.delete(f"{BASE_URL}/org/assets/bulk/delete?asset_ids=[{ASSET_ID}]", json={})
...
def test_service_delete(requests_mock):
    ...
    requests_mock.delete(f"{BASE_URL}/org/services/{ASSET_ID}", json={})
...
def test_wireless_lan_delete(requests_mock):
    ...
    requests_mock.delete(f"{BASE_URL}/org/wireless/{ASSET_ID}", json={})
...
def test_tag_delete(requests_mock):
    ...
    requests_mock.patch(f"{BASE_URL}/org/assets/{ASSET_ID}/tags", json={})
    client = get_client()
    tagsList = ["tag1", "tag2"]
