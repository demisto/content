...
def ip_enrichment_script(
    ...
    """
    Enrich IP indicators using batch validation.
    ...
    valid_inputs = create_and_extract_indicators_batch(ip_list, "IP")

    if not valid_inputs:
        demisto.debug("No valid IP indicators found in the provided input.")
        return_results(CommandResults(readable_output="No valid IP indicators were found in the provided input."))
        return None

    demisto.debug(f"Found {len(valid_inputs)} valid IP indicator(s): {valid_inputs}")

    # Build IndicatorInstance objects from the validated inputs
    ip_instances = [IndicatorInstance(raw_input=ip, extracted_value=ip) for ip in valid_inputs]
...
def main():
    ...
    try:
        result = ip_enrichment_script(ip_list, external_enrichment, verbose, brands, additional_fields, args)
