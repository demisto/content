...
Google Cloud Security Command Center is a platform that offers deep visibility into cloud infrastructure, identifies security threats, and provides proactive measures to mitigate risks. 
...
>To add inclusion filter for *Google Cloud Security Command Center Pub/ Sub Service* go to *Logging -> Log Router -> Create Sink* and add the filter `***protoPayload.serviceName="securitycenter.googleapis.com"***` this filter will include only logs related to Security Command Center.
