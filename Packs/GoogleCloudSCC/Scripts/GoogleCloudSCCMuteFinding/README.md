Mutes a Google Cloud Security Command Center finding by its finding name, using the `google-cloud-scc-finding-mute` command of the Google Cloud SCC integration.
...
After the finding is muted, the mute state returned by the command (`MUTED` when the response does not contain it) is written to the **GoogleCloudSCC Finding Mute Status** incident field using the `setIncident` command.
...
| GoogleCloudSCC.FindingV2.muteInitiator | String | Records additional information about the mute operation, for example, the mute configuration that muted the finding and the user who muted the finding. |
| GoogleCloudSCC.FindingV2.muteUpdateTime | Date | The time at which the finding was muted or unmuted. |
| GoogleCloudSCC.FindingV2.eventTime | Date | The time the finding was first detected. |
| GoogleCloudSCC.FindingV2.createTime | Date | The time at which the finding was created in Security Command Center. |
...
The script is already wired to the **Mute Finding** button of the **Google Cloud SCC Finding** incident layout. To add the button to a different layout:
...
3. Set **Label** to `Mute Finding`.
4. Set **Script** to `GoogleCloudSCCMuteFinding`.
5. Leave the script arguments empty to mute the finding of the incident, or set `finding_name` to `${incident.googlecloudsccfindingname}` explicitly.
6. Save the layout.

Clicking the button mutes the finding of the incident and posts the result to the incident War Room.

## Script Example

```!GoogleCloudSCCMuteFinding finding_name="organizations/1094826489209/sources/5629340921983475201/locations/global/findings/6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a"```
...
```json
{
    "GoogleCloudSCC": {
        "FindingV2": {
            ...
            "severity": "CRITICAL",
            "mute": "MUTED",
            "muteInitiator": "user@example.com",
            "muteUpdateTime": "2026-07-29T09:15:20.123Z",
