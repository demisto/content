...
mock_demisto_args = {"ip": "172.29.62.3"}
...
def test_get_ip_nodeid(mocker):
    ...
    run_command_test(
        command_func=get_ip_nodeid, args=mock_demisto_args["ip"], response=response_data_1, result=result_data_1, mocker=mocker
    )
...
def test_get_tag_list(mocker):
    ...
    run_command_test(command_func=get_tag_list, args="", response=response_data_2, result=result_data_2, mocker=mocker)
