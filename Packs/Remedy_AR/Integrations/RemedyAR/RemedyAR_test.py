...
def test_get_server_details(mocker, requests_mock):
    ...
    mocker.patch.object(
        demisto,
        "params",
        return_value={"server": base_url, "proxy": True, "credentials": {"identifier": "identifier", "password": "password"}},
    )

    from RemedyAR import get_server_details

    mock_response = {"entries": [{"values": {"NC_IOPs": "1.1.1.1", "Name": "test_name"}}]}
    args = {"fields": "Name"}

    requests_mock.get(f"{base_url}/api/arsys/v1/entry/AST:ComputerSystem/?q=&fields=values(Name)", json=mock_response)
