...
def verify_url(url):
    ...
    try:
        ...
        raise ValueError(
            "Incorrect URL format. Use the following format: https://example.looker.com:19999\n"
...
def http_request(method, url_suffix, params=None, data=None, response_type="json"):
    ...
    if isinstance(res_obj, list) and len(res_obj) == 1 and isinstance(res_obj[0], dict) and "looker_error" in res_obj[0]:
...
def get_new_token(client_id, client_secret):
    ...
    try:
        ...
        return {"token": response_json["access_token"], "expires": datetime.utcnow().timestamp() + response_json["expires_in"]}
...
def get_session_token(client_id, client_secret):
    ...
    if (
        client_id not in ic
        or "expires" not in ic[client_id]
...
def get_look_id_from_name(name):
    ...
    if len(looks) > 1:
        raise Exception(
            f'There is more than one Look with the name "{name}".'
...
def get_entries_for_search_results(contents, look_id=None, result_format="json", look_name=""):
    ...
    if result_format == "json":
        ...
        if look_id:
            context = {
                "LookerResults(val.LookID && val.LookID === obj.LookID)": {"LookID": int(look_id), "Results": formatted_contents}
            }
        ...
        entries.append(
            fileResult("look_result.csv" if look_id else "inline_query_result.csv", contents, entryTypes["entryInfoFile"])
        )
...
def run_inline_query(result_format, args_dict):
    return http_request(
        method="POST", url_suffix=f"/queries/run/{result_format}", data=json.dumps(args_dict), response_type=result_format
    )
...
def create_look(query_id, space_id, look_title, look_description=""):
    data = {"title": look_title, "query_id": query_id, "space_id": space_id}
    if look_description:
