...
def test_get_attachment_id():
    from EmailTopicsSelect import get_attachment_id
...
def test_get_attachment_url(mocker):
    from EmailTopicsSelect import get_attachment_url
...
def test_get_files_list():
    from EmailTopicsSelect import get_files_list
    file_entries = util_load_json('test_data/file_entries.json')
...
def test_count_new_img():
    from EmailTopicsSelect import count_new_img
...
def test_convert_attachment_name_to_url(mocker):
    import EmailTopicsSelect
    from EmailTopicsSelect import convert_attachment_name_to_url
    mocker.patch.object(demisto, 'executeCommand')
    mocker.patch.object(EmailTopicsSelect, 'count_new_img', return_value=0)
    mocker.patch.object(EmailTopicsSelect, 'get_files_list',
...
def test_build_email_thread(mocker):
    from EmailTopicsSelect import build_email_thread
    import EmailTopicsSelect
    emails = util_load_json('test_data/email_thread.json')
    mocker.patch.object(EmailTopicsSelect, 'convert_attachment_name_to_url',
...
def test_generate_email_topic():
    from EmailTopicsSelect import generate_email_topic
...
def test_main(mocker):
    import EmailTopicsSelect
    from EmailTopicsSelect import main
    ...
    mocker.patch.object(EmailTopicsSelect, 'generate_email_topic', return_value="This_is_test_1")
    ...
    mocker.patch.object(demisto, 'args', return_value={"new": "Malware incident 1"})
    mocker.patch.object(EmailTopicsSelect, 'build_email_thread',
