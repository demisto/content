...
def build_email_thread(emails):
    ...
    """
    email_thread = new_email = convert_attachment_name_to_url(emails[-1]['Contents'])
    if len(emails) > 1:
...
def main():
    # New field is set by Preprocessing script that contains
    # email_to#&email_cc#&email_from#&message_id#&email_from
    new_field_value = demisto.args().get('new')
    email_metadata = re.findall("(?<=\d#).+",new_field_value)[0].split("#&")
    selected_topic = generate_email_topic(email_metadata[0])
    ...
    title_topic = re.findall("(?<=\d#)[\w\s]+",new_field_value)[0]
    title_html = "<h2>" + title_topic + "</h2><br>"

    if emails:
        if ctx:
            email_thread, new_email = build_email_thread(emails)
            email_display = {
            ...
            if "with_attachment" not in emails[-1]['Tags'] and "</p>Attachments:" in email_thread[0:400]:
                ...
                entries_str = json.dumps([{"Type": 1, "ContentsFormat": 'html', "Contents": new_email, "tags": [selected_topic, "with_attachment"]}])
