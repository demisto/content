...
class Client(BaseClient):
    ...
    def fetch_indicators(self, limit: int = 1000, offset: int = 0, order_by: str = "", order: str = "desc"):
        ...
        return self._http_request(method="GET", url_suffix="/library/entries", params=params)
        ...
        if "data" in res:
            ...
            return CommandResults(
                readable_output=tableToMarkdown(
                    "ORKL Reports", table, headers=["Created At", "Report Name", "Source", "References", "Threat Actors"]
                )
            )
            ...
            for report in data:
                if int(report.get("ts_updated_at")) > last_fetch:
                    indicator = {
                        ...
                        "service": FEED_NAME,
                        "rawJSON": report,
                        "fields": {"description": report.get("plain_text")},
                    }
                    ...
                    if len(report.get("references")) > 0:
                        ...
                        for pub in report.get("references"):
                            pub_obj = {"title": "Reference Report", "source": FEED_NAME, "link": pub}
                    ...
                    if len(report.get("threat_actors")) > 0:
                        for actor in report.get("threat_actors"):
                            ...
                            ind_actor = {
                                ...
                                "source": FEED_NAME,
                                "fields": {"aliases": actor.get("tools"), "trafficlightprotocol": tlp_color},
                            }
                            ...
                            if argToBoolean(cRel):
                                ...
                                relationships.append(
                                    create_relationships(
                                        feedRel, indicator["value"], "Report", actor.get("main_name"), "Threat Actor"
                                    )
                                )
