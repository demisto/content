...
# HELPER FUNCTIONS
def get_pan_device(device_type: str, hostname: str, api_key: str, vsys: str | None, insecure: bool) -> PanDevice:
    ...
    if device_type == DEVICE_TYPE_FIREWALL:
        ...
        raise ValueError(
            f"Invalid device type specified: {device_type}. Must be '{DEVICE_TYPE_FIREWALL}' or '{DEVICE_TYPE_PANORAMA}'."
        )
...
def get_ha_state_command(client: PanDevice, args: dict) -> CommandResults:
    ...
    if isinstance(ha_state_response, tuple):
        if len(ha_state_response) > 1 and isinstance(ha_state_response[1], ET.Element):
            ...
            reason = ha_state_response[1] if len(ha_state_response) > 1 else "Device is not configured for HA."
...
def get_ha_config_command(client: PanDevice) -> CommandResults:
    ...
    def find_text(el: ET.Element | None, path: str, default: str = "N/A") -> str:
        if el is None:
            return default
        node = el.find(path)
...
def request_ha_failover_command(client: PanDevice, suspend: bool) -> CommandResults:
    ...
    try:
        ...
        if "already in suspend state" in error_message and suspend:
            ...
            return CommandResults(readable_output=(f"Device {client.hostname} cannot be made active. It may already be active."))
...
def panorama_reconfigure_ha_command(
    ...
    try:
        ...
        message = f"Successfully sent HA reconfiguration command to Panorama at {client.hostname}."
    except PanDeviceError as e:
...
def validate_interfaces_command(client: PanDevice, args: dict) -> CommandResults:
    ...
    table_data = []
    for iface in interfaces_to_check:
        status = "Not Found" if iface in missing_interfaces else "Exists"
        table_data.append(
            {
                "Interface": iface,
                "Status": status,
    ...
    if all_valid:
        title = f"Interface Validation - {client.hostname} - All {len(interfaces_to_check)} valid"
    else:
        title = f"Interface Validation - {client.hostname} - {len(missing_interfaces)} missing"
...
def configure_ha_command(client: PanDevice, args: dict) -> CommandResults:
    ...
    if interfaces_to_validate:
        demisto.debug(f"Validating interfaces: {interfaces_to_validate}")
...
def set_ha_enabled_state(client: PanDevice, args: dict, enabled: bool) -> CommandResults:
    ...
    demisto.debug(f"{action} HA (setting enabled to '{enabled_str}').")
