...
def test_module(client: Client, args) -> str:
    ...
    if demisto.params().get('isFetch'):
        version_filter = filter_version(demisto.params())
        if not version_filter:
            raise DemistoException(
                'Missing both rule ids and node ids. \n At least one of the above is needed.')
        fetch_incidents(client, max_results=1, first_fetch=demisto.params().get('first_fetch'),
                        fetch_filter=version_filter,last_run={})
    client.get_nodes("")
    return 'ok'
...
def fetch_incidents(client: Client, max_results: int, last_run: Dict[str, int], first_fetch: Optional[str] = None,
                    fetch_filter: Optional[str] = ''
...
def main() -> None:
    ...
    try:
        ...
        if command in commands:
            ...
            next_run, incidents = fetch_incidents(client=client, first_fetch=params.get('first_fetch'),
