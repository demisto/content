...
def register():
    ...
    http_request("web_service/sample_upload/register", "get", headers_register)
...
def unregister():
    ...
    http_request("web_service/sample_upload/unregister", "get", headers_unregister)
...
def test():
    ...
    http_request("web_service/sample_upload/test_connection", "get", headers_test)
    demisto.results("ok")


def prettify_simple_upload_sample_file(sha1):
    pretty_sample = {"SHA1": sha1.upper()}
...
def simple_upload_sample_file(sample_file):
    ...
    with open(demisto.getFilePath(sample_file)["path"], "rb") as f:
        ...
        res = http_request(cmd_url, "post", headers_simple_upload_sample_file, files={"uploadsample": f})
    ...
    demisto.results(
        {
            ...
            "EntryContext": {"TrendMicroDDA.Submission(val.SHA1 && val.SHA1==obj.SHA1)": pretty_res},
        }
    )


def prettify_simple_upload_sample_url(url, sha1):
    pretty_sample = {"SHA1": sha1.upper(), "URL": url}
    return pretty_sample
...
def simple_upload_sample_url(sample_url):
    ...
    cmd_url = "web_service/sample_upload/simple_upload_sample"
    res = http_request(cmd_url, "post", headers_simple_upload_sample_url, files={"uploadsample": sample_url})
...
def get_sample(sha1, archive_type, archive_encrypted, archive_name):
    ...
    res = http_request(cmd_url, "get", headers_get_sample)
...
def get_sample_list(interval_start, interval_end, interval_type):
    try:
        ...
        return_error(
            "Given interval times are not in the required format, which is: YYYY-MM-DD HH:MM:SS, " + "e.g. 2008-11-22 19:53:42"
        )
    ...
    res = http_request(cmd_url, "get", headers_get_sample_list)
...
def build_report(res, threshold, status, verbose):
    ...
    if not reports_type_is_list:  # if the submission doesn't have sub-files
        ...
        if file_analyze_report["TrueFileType"] == "URL":
            # add the URL address
            context[outputPaths["url"]] = {"Data": file_analyze_report["OrigFileName"]}
        else:
        ...
        if reports["OVERALL_RISK_LEVEL"] >= threshold:
            if file_analyze_report["TrueFileType"] == "URL":
                context[outputPaths["url"]].update(
                    {"Malicious": {"Vendor": "Trend Micro DDA", "Description": "RiskLevel: " + reports["OVERALL_RISK_LEVEL"]}}
                )
            else:
        ...
        if verbose == "true":
            ...
            if "FileItem" in dropped_files and "DownloadURL" in dropped_files["FileItem"]:
                ...
                context["TrendMicroDDA.Submission"].update({"DownloadURL": dropped_files["FileItem"]["DownloadURL"]})
        ...
        for file_analyzed in reports["FILE_ANALYZE_REPORT"][1:]:  # iterate over all the subfiles excluding the main file
            ...
            if file_analyzed["TrueFileType"] == "URL":
                # add the URL address
                context[outputPaths["url"]] = {"Data": file_analyzed["OrigFileName"]}
            else:
                context[outputPaths["file"]] = {
                    "MD5": file_analyzed["FileMD5"],
                    "SHA1": file_analyzed["FileSHA1"],
...
def get_report(sha1):
    ...
    res = http_request(cmd_url, "get", headers_get_report)
...
def get_report_command():
    ...
    if res.status_code == 102:
        ...
        demisto.results(
            {
                ...
                "EntryContext": {"TrendMicroDDA.Submission(val.SHA1 && val.SHA1==obj.SHA1)": ec},
            }
        )
    else:
        status = "Completed"
        ...
        demisto.results(
            {  # add context and the Report File to the war room
        ...
        demisto.results(
            {  # add table to the war room
                "Type": entryTypes["note"],
                "Contents": res.text,
                "ContentsFormat": formats["text"],
                "HumanReadableFormat": formats["markdown"],
                "HumanReadable": "## Submission Report from TrendMicroDDA\n"
...
def get_brief_report(sha1):
    ...
    res = http_request(cmd_url, "put", headers=headers_get_brief_report, body=data)
    return res
...
def check_status(sha1_list):
    ...
    for sha1 in sha1_list:
        ...
        if res.status_code == 102:
            manyEC.append({"Status": "Analyzing", "SHA1": sha1})
        else:
            manyEC.append({"Status": "Completed", "SHA1": sha1})
    ...
    demisto.results(
        {
            ...
            "EntryContext": {"TrendMicroDDA.Submission(val.SHA1 && val.SHA1==obj.SHA1)": manyEC},
        }
    )
