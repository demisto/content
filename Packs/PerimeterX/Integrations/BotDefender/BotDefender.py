...
class Client(BaseClient):
    ...
    def test_api_connection(self, api_key: str) -> dict[str, Any]:
        ...
        return self._http_request(method="GET", full_url=f"{self._base_url}?search=ip:1.1.1.1&tops=path", headers=headers)
    ...
    def post_investigate_by_name(self, name_type: str, name: str) -> dict[str, Any]:
        ...
        return self._http_request(method="POST", full_url=self._base_url, json_data=request_params)
...
def perimeterx_get_investigate_details(
    client: Client, args: dict[str, Any], thresholds: dict[str, Any], api_key: str
) -> CommandResults:
    """
    ...
    if search_type in supported_ip_search_types:
        ...
        indicator = pXScoring.get_ip_score(ip_address=search_term, risk_score=result["max_risk_score"], thresholds=thresholds)

        return CommandResults(outputs_prefix="PerimeterX", outputs_key_field="", outputs=result, indicator=indicator)
    ...
    return CommandResults(outputs_prefix="PerimeterX", outputs_key_field="", outputs=result)
...
def ip(client: Client, args, thresholds: dict[str, Any], api_key):
    ...
    for ip_address in ip_list:
        ...
        indicator = pXScoring.get_ip_score(ip_address=ip_address, risk_score=result["max_risk_score"], thresholds=thresholds)
...
def main():
    ...
    try:
        ...
        if command == "test-module":
            ...
            return_results(
                perimeterx_get_investigate_details(client=client, args=demisto.args(), thresholds=thresholds, api_key=api_key)
            )
