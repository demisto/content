...
def create_api_call():
    if USE_SSL:
        ...
        client = duo_client.Admin(ikey=INTEGRATION_KEY, skey=SECRET_KEY, host=HOST, ca_certs="DISABLE")
    try:
...
def get_entry_for_object(title, obj, contents, context, headers=None):
    ...
    readable = tableToMarkdown(title, obj, headers, lambda h: h.title().replace("_", " ").replace(".", ":"))
...
def get_all_users(admin_api):
    ...
    entry = get_entry_for_object(
        ...
        headers=["username", "user_id", "is_enrolled", "last_login", "realname", "email", "phones", "status"],
    )
...
def get_authentication_logs_by_user(admin_api, args):
    ...
    entry = get_entry_for_object(
        "Authentication logs for " + user_name,
        raw_logs,
        raw_logs,
        {"DuoAdmin.UserDetails(val.username && val.username == obj.username)": {"username": user_name, "auth_logs": raw_logs}},
...
def get_devices_by_user(admin_api, args):
    ...
    entry = get_entry_for_object(
        f"Devices for {user_name}",
        res,
        res,
        {"DuoAdmin.UserDetails(val.username && val.username == obj.username)": {"username": user_name, "phones": res}},
    )
...
def get_all_devices(admin_api):
    ...
    entry = get_entry_for_object("Devices", res, res, {"DuoAdmin.Phones(val.phone_id==obj.phone_id)": res})
...
def get_u2f_tokens_by_user(admin_api, args):
    ...
    entry = get_entry_for_object(
        "U2F Tokens for " + user_name,
        res,
        res,
        {"DuoAdmin.UserDetails(val.username && val.username == obj.username)": {"username": user_name, "u2ftokens": res}},
    )
...
def get_all_admins(admin_api):
    ...
    entry = get_entry_for_object(
        ...
        headers=["admin_id", "admin_units", "created", "email", "last_login", "name", "phone", "role", "status"],
    )
    demisto.results(entry)


def modify_admin_user(admin_api, admin_id=None, name=None, phone=None, password=None, password_change_required=None):
