...
def test_get_queries(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-get-queries", args={})
...
def test_get_companies(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-get-companies", args={})
    first_data = result.outputs
    assert first_data["count"] == 1
...
def test_get_domains(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-get-domains", args={})
    first_data = result.outputs
    assert first_data["count"] == 1
...
def test_get_users(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-get-users", args={})
    first_data = result.outputs
    assert first_data["count"] == 1
...
def test_get_user_by_id(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-get-user-by-id", args={"user_id": 1})
    first_data = result.outputs
...
def test_get_groups(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-get-groups", args={})
    first_data = result.outputs
...
def test_get_permissions(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-get-permissions", args={})
    first_data = result.outputs
    assert first_data["count"] == 1
...
def test_get_scroll_devices(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-search-scroll-devices", args={})
    first_data = result.outputs
...
def test_get_scroll_users(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-search-scroll-users", args={})
    first_data = result.outputs
...
def test_get_scroll_applications(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-search-scroll-applications", args={})
    first_data = result.outputs
...
def test_get_scroll_avm(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-search-scroll-avm", args={})
    first_data = result.outputs
...
def test_search_devices(requests_mock, octox_client):
    devices_data = util_load_json(path="test_data/get_devices.json")
    requests_mock.post("/devices/devices", json=devices_data)
    result = run_command(octox=octox_client, command_name="octoxlabs-search-devices", args={})
    first_data = result.outputs
...
def test_search_users_inventory(requests_mock, octox_client):
    users_data = util_load_json(path="test_data/get_users_inv.json")
    requests_mock.post("/userinventory/users", json=users_data)
    result = run_command(octox=octox_client, command_name="octoxlabs-search-users-inventory", args={})
    first_data = result.outputs
...
def test_search_applications(requests_mock, octox_client):
    app_data = util_load_json(path="test_data/get_applications.json")
    requests_mock.post("/appinventory/applications", json=app_data)
    result = run_command(octox=octox_client, command_name="octoxlabs-search-applications", args={})
    first_data = result.outputs
...
def test_search_avm(requests_mock, octox_client):
    avm_data = util_load_json(path="test_data/get_avm.json")
    requests_mock.post("/avm/vulnerabilities", json=avm_data)
    result = run_command(octox=octox_client, command_name="octoxlabs-search-avm", args={})
    first_data = result.outputs
...
def test_get_user_detail(requests_mock, octox_client):
    ...
    result = run_command(octox=octox_client, command_name="octoxlabs-get-user-inventory-detail", args={})
    first_data = result.outputs
...
def test_get_application_detail(requests_mock, octox_client):
    ...
    requests_mock.get("/discoveries/last", json={"id": 1})
    result = run_command(octox=octox_client, command_name="octoxlabs-get-application-detail", args={})
    first_data = result.outputs
...
def test_get_device_detail(requests_mock, octox_client):
    ...
    requests_mock.get("/discoveries/last", json={"id": 1})
    result = run_command(octox=octox_client, command_name="octoxlabs-get-device", args={})
    first_data = result.outputs
