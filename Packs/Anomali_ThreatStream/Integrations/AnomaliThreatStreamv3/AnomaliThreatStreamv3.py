...
INTELLIGENCE_TYPE_TO_ENTITY_TYPE = {
...
class Client(BaseClient):
        ...
        if res.status_code == 401:
            if command == "threatstream-add-threat-model-association":
                raise DemistoException(
                    f"{THREAT_STREAM} - Got unauthorized from the server."
    ...
    def create_whitelist_entry_with_file_request(self, file_data: dict) -> dict:
        ...
        Returns:
            A response object in a form of a dictionary.
        """
        return self.http_request("POST", "v1/orgwhitelist/upload/", params={"remove_existing": "false"}, files=file_data)
    ...
    def add_threat_model_association_request(
        self,
        entity_type_url: Optional[str],
        entity_id: Optional[str],
        associated_entity_type_url: Optional[str],
        associated_entity_ids_list: Optional[list],
    ) -> dict:
        """
        ...
        return self.http_request("POST", url_suffix, json={"ids": associated_entity_ids_list})
    ...
    def add_indicator_tag(self, indicator_ids: list[str], tags: list[str]):
        ...
        self.http_request(method="POST", url_suffix="v2/intelligence/bulk_tagging/", data=json.dumps(data_request))
    ...
    def remove_indicator_tag(self, indicator_ids: list[str], tags: list[str]):
        ...
        self.http_request(method="PATCH", url_suffix="v2/intelligence/bulk_remove_tags/", data=json.dumps(data_request))
...
def get_generic_threat_context(indicator, indicator_mapping=DEFAULT_INDICATOR_MAPPING):
    ...
    context = {indicator_mapping[k]: v for (k, v) in indicator.items() if k in indicator_mapping}
...
def parse_indicators_list(iocs_list):
    ...
    for indicator in iocs_list:
        ...
        iocs_context.append({key: indicator.get(ioc_key) for (key, ioc_key) in INDICATOR_EXTENDED_MAPPING.items()})
...
def build_model_data(model, name, is_public, tlp, tags, intelligence, description):
    ...
    data = {
        k: v for (k, v) in (("name", name), ("is_public", is_public), ("tlp", tlp), (description_field_name, description)) if v
    }
    if tags:
        data["tags"] = tags if isinstance(tags, list) else [t.strip() for t in tags.split(",")]
    if intelligence:
        data["intelligence"] = intelligence if isinstance(intelligence, list) else [i.strip() for i in intelligence.split(",")]
    return data
...
def ips_reputation_command(
    client: Client, score_calc: DBotScoreCalculator, ip, status, threshold=None, threat_model_association=False
):
    results = []  # type: ignore
    ips = argToList(ip, ",")
...
def get_ip_reputation(
    ...
    if not indicator:
        return create_indicator_result_with_dbotscore_unknown(
            indicator=ip, indicator_type=DBotScoreType.IP, reliability=client.reliability
        )
    ...
    if threat_model_association:
        intelligence_relationships, outputs = get_intelligence(client, indicator, FeedIndicatorType.IP)
...
def list_rule_command(
    client: Client, rule_id: str = None, limit: str = "50", page: str = None, page_size: str = None
) -> CommandResults:
    """
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(
            ...
            headers=["name", "id", "matches", "intelligence_initiatives", "created_ts", "modified_ts", "notify_me", "is_enabled"],
        ),
        raw_response=res,
    )


def create_request_body_rule(
    rule_id: str = None,
    rule_name: str = None,
    keywords: str = None,
    match_include: str = None,
    actor_ids: str = None,
    campaign_ids: str = None,
    investigation_action: str = None,
    new_investigation_name: str = None,
    existing_investigation_id: str = None,
    exclude_indicator: str = None,
    include_indicator: str = None,
    exclude_notify_org_whitelisted: str = None,
    exclude_notify_owner_org: str = None,
    incident_ids: str = None,
    malware_ids: str = None,
    signature_ids: str = None,
    threat_bulletin_ids: str = None,
    ttp_ids: str = None,
    vulnerability_ids: str = None,
    tags: str = None,
) -> dict:
    """
...
def validate_investigation_action(
    investigation_action: Optional[str], new_investigation_name: Optional[str], existing_investigation_id: Optional[str]
):
    """
    ...
    if investigation_action == "Create New" and (not new_investigation_name):
        raise DemistoException(
            "Please ensure to provide the 'new_investigation_name'"
    ...
    if investigation_action == "Add To Existing" and (not existing_investigation_id):
        raise DemistoException(
            "Please ensure to provide the 'existing_investigation_id'"
...
def list_user_command(
    client: Client, user_id: str = None, limit: str = "50", page: str = None, page_size: str = None
) -> CommandResults:
    """
...
def list_investigation_command(
    client: Client, investigation_id: str = None, limit: str = "50", page: str = None, page_size: str = None
) -> CommandResults:
    """
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown(
            ...
            json_transform_mapping={
                "assignee": JsonTransformer(keys=["email"], func=lambda hdr: hdr.get("email", "")),
                "reporter": JsonTransformer(keys=["email"], func=lambda hdr: hdr.get("email", "")),
            },
        ),
        raw_response=res,
    )
...
def create_investigation_command(
    ...
    connect_related_indicators: str = None,
    associated_signature_ids: str = None,
    associated_threat_bulletin_ids: str = None,
    associated_ttp_ids: str = None,
    associated_vulnerability_ids: str = None,
    associated_actor_ids: str = None,
    associated_campaign_ids: str = None,
    associated_incident_ids: str = None,
    associated_observable_ids: str = None,
) -> CommandResults:
    """
    ...
    if res.get("all_added") is False:
        ...
        successful_ids = [str(entity.get("r_id")) for entity in data if str(entity.get("r_id")) in associated_list]
        readable_output = (
            f'Investigation was created successfully with ID: {res.get("id")}.\n'
...
def update_investigation_command(
    client: Client,
...
def add_investigation_element_command(
    client: Client,
    investigation_id: str = None,
    connect_related_indicators: str = None,
    associated_signature_ids: str = None,
    ...
    if not res.get("objects"):
        raise DemistoException(
            "The addition of elements to the investigation has failed. "
            "Please verify the accuracy of the investigation_id argument."
        )
    if res.get("all_added") is False:
        exists_elements = res.get("already_exists_elements_count")
        if isinstance(exists_elements, int) and exists_elements >= len(associated_list) and res.get("added_elements_count") == 0:
            ...
            if unsuccessful_ids:
                readable_output = (
                    f'The following elements with IDs were successfully added: '
...
def list_whitelist_entry_command(
    client: Client, format: str = "json", limit: str = "50", page: str = None, page_size: str = None
) -> CommandResults:
    """
    ...
    if format and format.lower() == "json":
        ...
        return CommandResults(
            ...
            readable_output=tableToMarkdown(
                ...
                removeNull=True,
                headerTransform=header_transformer,
                headers=["id", "value", "resource_uri", "created_ts", "modified_ts", "value_type", "notes"],
            ),
            raw_response=res,
        )
    else:
        return fileResult(
            filename=f'whitelist-entries-{date.today().strftime("%b-%d-%Y")}.csv', data=res, file_type=EntryType.ENTRY_INFO_FILE
        )
...
def create_indicators_list(names_and_indicators_list: list[tuple[str, list]], notes: Optional[str]) -> list:
    ...
    for name_and_indicators_list in names_and_indicators_list:
        if indicators_list := name_and_indicators_list[0] and name_and_indicators_list[1]:
            if notes:
                indicators_list_to_return.extend(
                    [
                        {"value_type": name_and_indicators_list[0], "value": indicator, "notes": notes}
                        for indicator in indicators_list
                    ]
                )
            else:
                indicators_list_to_return.extend(
                    [{"value_type": name_and_indicators_list[0], "value": indicator} for indicator in indicators_list]
                )
    return indicators_list_to_return


def create_whitelist_entry_command(
    client: Client,
    entry_id: str = None,
    cidr: str = None,
    domains: str = None,
    ...
    if entry_id:
        ...
        if res.get("success"):
            return CommandResults(readable_output=res.get("message"), raw_response=res)
        else:
            return CommandResults(readable_output=f'ERROR: {res.get("message")}', raw_response=res)
    else:
        whitelist = create_indicators_list(
            [
                ("domain", argToList(domains)),
...
def update_whitelist_entry_note_command(client: Client, entry_id: str = None, note: str = None) -> CommandResults:
    """
...
def list_import_job_command(
    client: Client, import_id: str = None, status_in: str = None, limit: str = "50", page: str = None, page_size: str = None
) -> CommandResults:
    """
    ...
    readable_output = tableToMarkdown(
        ...
        json_transform_mapping={
            ...
            "intelligence_initiatives": JsonTransformer(func=lambda hdr: ", ".join([item.get("type") for item in hdr])),
        },
    )
    if not import_id:
        for item in outputs:
...
def validate_values_search_threat_model(model_type: str = None, publication_status: str = None, signature_type: str = None):
    """
    ...
    if raise_exception_publication_status:
        raise DemistoException(
            f'The publication_status argument contains the following invalid values: '
...
def search_threat_model_command(
    client: Client,
    model_type: str = None,
    name: str = None,
...
def add_threat_model_association_command(
    client: Client,
    entity_type: str = None,
    entity_id: str = None,
    associated_entity_type: str = None,
    associated_entity_ids: str = None,
) -> CommandResults:
    """
    ...
    res = client.add_threat_model_association_request(
        entity_type_url, entity_id, associated_entity_type_url, associated_entity_ids_list
    )
    res_ids = res.get("ids")
    ...
    for type, ids_list in arguments_dict.items():
        if isinstance(ids_list, list):
            ...
            if is_update:
                ...
                element_list.extend(
                    [
                        {"r_type": type, "r_id": arg_to_number(id), "add_related_indicators": add_related_indicators}
                        for id in ids_list
                    ]
                )
...
def get_intelligence_information(client: Client, indicator, ioc_type, intelligence_type):
    ...
    for intelligence in intelligences:
        ...
        if entity_b_name:
            relationship = create_intelligence_relationship(client, indicator, ioc_type, entity_b_name, entity_b_type)
...
def domains_reputation_command(
    client: Client, score_calc: DBotScoreCalculator, domain, status, threshold=None, threat_model_association=False
):
    """
...
def get_domain_reputation(
    ...
    if threat_model_association:
        intelligence_relationships, outputs = get_intelligence(client, indicator, FeedIndicatorType.Domain)
        if intelligence_relationships:
            relationships.extend(intelligence_relationships)
        threat_context.update(outputs)
        human_readable += create_human_readable(outputs)
...
def files_reputation_command(
    client: Client, score_calc: DBotScoreCalculator, file, status, threshold=None, threat_model_association=False
):
    """
...
def get_file_reputation(
    ...
    if not indicator:
        return create_indicator_result_with_dbotscore_unknown(
            indicator=file, indicator_type=DBotScoreType.FILE, reliability=client.reliability
        )
    ...
    if threat_model_association:
        intelligence_relationships, outputs = get_intelligence(client, indicator, FeedIndicatorType.File)
        if intelligence_relationships:
            relationships.extend(intelligence_relationships)
        threat_context.update(outputs)
...
def urls_reputation_command(
    client: Client, score_calc: DBotScoreCalculator, url, status, threshold=None, threat_model_association=False
):
    """
...
def get_url_reputation(
    ...
    if threat_model_association:
        intelligence_relationships, outputs = get_intelligence(client, indicator, FeedIndicatorType.URL)
        if intelligence_relationships:
            relationships.extend(intelligence_relationships)
        threat_context.update(outputs)
        human_readable += create_human_readable(outputs)
...
def import_ioc_with_approval(
    ...
    data = assign_params(
        ...
        severity=severity,
        tags=(
            json.dumps([({"name": tag, "tlp": tags_tlp.lower()} if tags_tlp else {"name": tag}) for tag in argToList(tags)])
            if tags
            else None
        ),
    )
...
def import_ioc_without_approval(
    client: Client,
    classification,
    file_id=None,
    confidence=None,
    ...
    meta |= assign_params(
        ...
        severity=severity,
        tags=(
            [({"name": tag, "tlp": tags_tlp.lower()} if tags_tlp else {"name": tag}) for tag in argToList(tags)] if tags else None
        ),
...
def get_iocs_by_model(client: Client, model, id, limit="20", page=None, page_size=None):
    ...
    readable_output = tableToMarkdown(f"Indicators list for Threat Model {model_type} with id {id}", iocs_context)
    return CommandResults(
...
def update_model(
    client: Client, model, model_id, name=None, is_public="false", tlp=None, tags=None, intelligence=None, description=None
):
    """
...
def add_tag_to_model(client: Client, model_id, tags, model="intelligence"):
    ...
    data = {"tags": [{"name": t, "tlp": "red"} for t in tags]}
...
def search_intelligence(client: Client, **kwargs):
    ...
    intelligence_table = tableToMarkdown(
        "The intelligence results", intelligence_list, removeNull=True, headerTransform=string_to_table_header
    )
...
def clone_ioc_command(client: Client, indicator_id: str = None) -> CommandResults:
    ...
    clone_table = tableToMarkdown(
        f"Clone operation results for indicator {indicator_id}", res_json, removeNull=True, headerTransform=string_to_table_header
    )

    return CommandResults(
        outputs_prefix=f"{THREAT_STREAM}.Clone", outputs=res_json, readable_output=clone_table, raw_response=res_json
    )
...
def main():
    ...
    try:
        ...
        return_error(f"{err!s}, traceback {traceback.format_exc()}")
