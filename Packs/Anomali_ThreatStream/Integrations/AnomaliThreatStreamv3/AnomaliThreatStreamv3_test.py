...
import emoji
import pytest
from AnomaliThreatStreamv3 import (
    DEFAULT_INDICATOR_MAPPING,
    FILE_INDICATOR_MAPPING,
    INDICATOR_EXTENDED_MAPPING,
    REPUTATION_COMMANDS,
    Client,
    add_indicator_tag_command,
    add_investigation_element_command,
    add_tag_to_model,
    add_threat_model_association_command,
    approve_import_job_command,
    clone_ioc_command,
    create_element_list,
    create_indicators_list,
    create_investigation_command,
    create_model,
    create_rule_command,
    create_whitelist_entry_command,
    delete_investigation_command,
    delete_rule_command,
    delete_whitelist_entry_command,
    edit_classification_job_command,
    file_name_to_valid_string,
    get_indicators,
    get_intelligence,
    get_model_description,
    import_ioc_with_approval,
    import_ioc_without_approval,
    list_import_job_command,
    list_investigation_command,
    list_rule_command,
    list_user_command,
    list_whitelist_entry_command,
    main,
    remove_indicator_tag_command,
    return_params_of_pagination_or_limit,
    search_intelligence,
    search_threat_model_command,
    submit_report,
    update_investigation_command,
    update_model,
    update_rule_command,
    update_whitelist_entry_note_command,
    validate_investigation_action,
    validate_values_search_threat_model,
)
...
MOCK_OBJECTS = {"objects": [{"srcip": "8.8.8.8", "itype": "mal_ip", "confidence": 50}, {"srcip": "1.1.1.1", "itype": "apt_ip"}]}
MOCK_OBJECTS_2 = {
    "objects": [
        {"email": "email_test@domain.com", "itype": "compromised_email", "confidence": 50},
        {"srcip": "78.78.78.67", "classification": "private", "itype": "bot_ip", "confidence": 50, "severity": "low"},
        {
    ...
    "meta": {"confidence": 50, "classification": "Private", "allow_unresolved": True, "tags": ["test1", "test2"]},
}
...
class TestReputationCommands:
    ...
    def mocked_http_request_ioc(
        self, method, url_suffix, params=None, data=None, headers=None, files=None, json=None, resp_type="json"
    ):
    ...
    def test_reputation_commands__happy_path(self, mocker, ioc_type, ioc_value, ioc_context_key):
        ...
        mocker.patch.object(
            demisto, "args", return_value={value_key: ioc_value, "status": "active", "threat_model_association": "True"}
        )
    ...
    @pytest.mark.parametrize(
        argnames="threshold_in_command, threshold_in_params, exp_dbot_score",
        argvalues=[
            (None, None, Common.DBotScore.SUSPICIOUS),
    ...
    def test_ioc_reputation_with_thresholds_in_instance_param(
        self, mocker, threshold_in_command, threshold_in_params, exp_dbot_score
    ):
        """
        Given
    ...
    @pytest.mark.parametrize(
        argnames="include_inactive, exp_status_param", argvalues=[("true", "active,inactive"), ("false", "active")]
    )
...
class TestImportCommands:
    ...
    def test_import_indicator_with_approval__happy_path(self, mocker, import_type):
        ...
        mocker.patch.object(
            Client, "http_request", return_value={"success": True, "import_session_id": "test_session_id", "job_id": "id"}
        )

        # run
        result = import_ioc_with_approval(
            mock_client(), import_type, "test_value", tags="tag1,tag2", expiration_ts="2023-12-25T00:00:00"
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                {"classification": "Private", "confidence": 50, "severity": "low", "allow_unresolved": True},
            ),
            (
                MOCK_OBJECTS_2,
                "test_file",
    ...
    def test_import_indicator_without_approval__happy_path(
        self,
        mocker,
        mock_object: dict,
        file_name: str,
...
class TestGetCommands:
    ...
    @pytest.mark.parametrize(
        argnames="command, command_args, expected_context_keys", argvalues=commands_with_expected_context_key
    )
    def test_get_commands__happy_path(self, mocker, command, command_args, expected_context_keys):
        ...
        # prepare
        mocker.patch.object(
            Client, "http_request", side_effect=lambda *args, **kwargs: self.mocked_http_get_response(command, **kwargs)
        )
    ...
    @pytest.mark.parametrize(argnames="command, command_args, expected_output", argvalues=commands_with_expected_output)
    ...
    @pytest.mark.parametrize(
        ...
        argvalues=[
            ...
            (
                "threatstream-get-indicators-by-model",
                {"model": "Actor", "id": 1, "page": 2, "page_size": 2},
                {"limit": 2, "offset": 2},
            ),
            ...
            (
                ...
                {"page": 2, "page_size": 4, "status_in": "Errors"},
                {"limit": 4, "offset": 4, "status": "errors"},
            ),
            (
                "threatstream-list-import-job",
                {"page": 2, "page_size": 4, "status_in": "Approved"},
                {"limit": 4, "offset": 4, "status": "approved"},
            ),
            (
                "threatstream-list-import-job",
                {"page": 2, "page_size": 4, "status_in": "Ready To Review"},
                {"limit": 4, "offset": 4, "status": "done"},
            ),
            (
                "threatstream-list-import-job",
                {"page": 2, "page_size": 4, "status_in": "Rejected"},
                {"limit": 4, "offset": 4, "status": "deleted"},
            ),
            (
                "threatstream-list-import-job",
                {"page": 2, "page_size": 4, "status_in": "Processing"},
...
class TestUpdateCommands:
    ...
    def test_create_model__happy_path(self, mocker):
        ...
        # run
        created_model_id = create_model(
            mock_client(), model="Actor", name="test_actor", tags="tag_1,tag_2", description="test_desc"
        )
    ...
    def test_submit_url_report__happy_path(self, mocker):
        ...
        # validate
        expected_data_keys = ["report_radio-classification", "report_radio-platform", "use_premium_sandbox", "report_radio-url"]
...
def test_search_intelligence(mocker):
    ...
    args = {"uuid": "9807794e-3de0-4340-91ca-cd82dd7b6d24", "itype": "apt_ip"}
    client = mock_client()
...
def test_search_intelligence_with_confidence(mocker):
    ...
    args = {"uuid": "9807794e-3de0-4340-91ca-cd82dd7b6d24", "confidence": "lt 80"}
    client = mock_client()
...
def test_update_whitelist_entry_note_command(mocker):
    ...
    args = {"entry_id": "66", "note": "some_note"}
    client = mock_client()
...
def test_create_whitelist_entry_command(mocker, args):
    ...
    mocker_file_get = mocker.patch.object(
        demisto, "getFilePath", return_value={"id": "xxx", "path": "test/test.txt", "name": "test.txt"}
    )
...
def test_list_whitelist_entry_command(mocker):
    ...
    assert (
        command_result.readable_output == "### Whitelist entries\n"
    ...
    assert command_result.outputs == [
        {
            "created_ts": "2023-02-21T19:59:02.091404",
            "id": 13,
            "modified_ts": "2023-02-21T19:59:02.091404",
        ...
        {
            "created_ts": "2023-03-14T14:28:20.110021",
            "id": 12,
        ...
        {
            "created_ts": "2023-03-14T14:26:18.765256",
            "id": 11,
            "modified_ts": "2023-03-14T14:26:18.765256",
            "notes": None,
            "resource_uri": "resource_uri",
            "value": "u",
            "value_type": "user-agent",
        },
    ]
...
def test_list_user_command(mocker):
    ...
    assert (
        command_result.readable_output == "### Users\n|User Id|Email|Is Active|Last Login|\n|"
...
@pytest.mark.parametrize(
    "args, get_from_jason, result_of_readable_output",
    [
        (
            {"investigation_id": "222", "associated_actor_ids": "55555,11111"},
            "add_investigation_elements_all",
            "All The elements was added successfully to investigation ID: 222",
        ),
        (
            {"investigation_id": "222", "associated_actor_ids": "55555,22222"},
            "add_investigation_elements",
...
def test_search_threat_model_command(mocker):
    ...
    assert (
        command_result.readable_output == "### Threat model entities\n|Id|Type|Name|Publication Status|Modified At|\n|"
...
@pytest.mark.parametrize(
    "mocker_return_value, expected_readable_output",
    [
        (
...
def test_add_threat_model_association_command(mocker, mocker_return_value, expected_readable_output):
    ...
    args: dict = {
        "entity_type": "Actor",
        "entity_id": "11111",
        "associated_entity_ids": "2222,3333",
...
@pytest.mark.parametrize(
    "model_type, publication_status, signature_type, message",
    [
...
@pytest.mark.parametrize(
    "arguments_dict, expected_result",
    [
        (
            {
                "vulnerability": [1],
...
@pytest.mark.parametrize(
    "investigation_action, new_investigation_name, existing_investigation_id, message",
    [
        (
            "Create New",
            None,
            None,
            "Please ensure to provide the 'new_investigation_name' argument when selecting the"
        ...
        (
            ...
            None,
            None,
            "Please ensure to provide the 'existing_investigation_id' argument when selecting"
...
@pytest.mark.parametrize("page, page_size, limit", [(2, 2, 0), (2, None, 2), (None, 2, 2)])
...
@pytest.mark.parametrize(
    "names_and_indicators_list, notes, expected_result",
    [
        (
            [
                ("domain", ["some_domain"]),
...
def test_add_indicator_tag_success(mocker, args: dict[str, str], expected_data: dict[str, str], expected_output: str):
    """
    Given:
    ...
    mock_http_request.assert_called_with(method="POST", url_suffix="v2/intelligence/bulk_tagging/", data=expected_data)
...
def test_remove_indicator_tag_command_success(mocker, args: dict[str, str], expected_output: str):
    """
    Given:
        - A list of indicator IDs and tags
    When:
...
def test_clone_ioc_command(mocker):
    ...
    # Mock API response
    mocked_response = {"import_session_id": "139", "job_id": "1b0ad011-e595-4f7f-8eb6"}

    readable_output = (
        "### Clone operation results for indicator 123\n"
...
def test_edit_classification_job_command(mocker):
    ...
    # Call function
    command_result = edit_classification_job_command(client=client, import_id="139", data='{"is_public":false,"circles":[11111]}')
