...
def test_get_alert_group(requests_mock):
    ...
    input_params = {"order_by": [{"created_at": "desc"}], "skip": 0, "take": 1, "include": {"tags": True}}
...
def test_get_alert(requests_mock):
    ...
    args = {"from": 0, "limit": 1, "start_date": "2023-04-18T00:00:00+00:00", "end_date": "2023-04-19T00:00:00+00:00"}
    ...
    response, next = cyble_events(
        client, "POST", "some_random_token", url, args, {}, False, collections, incident_severity, False
    )
...
@pytest.mark.parametrize("offset,limit", [("0", "1789"), ("0", "-2")])
...
def test_offset_cyble_vision_fetch_detail(requests_mock, capfd):
    ...
    with capfd.disabled(), pytest.raises(ValueError, match=f"The parameter from has a negative value, from: {args.get('from')}'"):
        cyble_events(client, "POST", "some_random_token", url, args, {}, True, [], [], True)
...
def test_get_alert_fetch(requests_mock):
    ...
    response, next = cyble_events(client, "POST", "some_random_token", url, args, {}, False, [], [], False)
...
def test_get_alert_fetch2(requests_mock):
    ...
    response, next = cyble_events(client, "POST", "some_random_token", url, args, {}, False, [], [], True)
...
def test_update_incident(requests_mock):
    ...
    args = {"from": 0, "limit": 1, "start_date": "2023-09-18T00:00:00+00:00", "end_date": "2023-09-19T00:00:00+00:00"}

    url = "https://test.com/apollo/api/v1/y/alerts"

    response, next = cyble_events(client, "PUT", "some_random_token", url, args, {}, False, [], [], False)
