...
def cyble_alert_group(client, method, token, url, input_params):
    """
    ...
    alert_groups = set_request(client, method, token, input_params, url)
    lst_alert_group = []
...
def cyble_fetch_iocs(client, method, token, input_params, url):
    """
    ...
    """
    iocs = set_request(client, method, token, input_params, url)
...
def cyble_events(client, method, token, url, input_params, args, skip=True):
    """
    ...
    alerts = set_request(client, method, token, input_params, url)
    ...
    try:
        if token:
            ...
            if demisto.command() == "test-module":
                fetch_subscribed = test_response(client, "GET", base_url, token)
                # request was successful
                return_results(fetch_subscribed)
                ...
                max_results = arg_to_number(params.get('max_fetch', 0))
                ...
                data = cyble_events(client, 'POST', token, url, final_input_structure, args, False)
                ...
                subscribed_service_list = fetch_subscribed_services(client, "GET", base_url, token)

                markdown = tableToMarkdown('Alerts Group Details:', subscribed_service_list, )
                return_results(CommandResults(
                    readable_output=markdown,
                    outputs_prefix='CybleEventsV2.ServiceList',
                    raw_response=subscribed_service_list,
                    outputs=subscribed_service_list
                ))
                ...
                return_results(cyble_alert_group(client, 'POST', token, url, input_params_alerts_group))
                ...
                url = base_url + str(ROUTES[COMMAND[demisto.command()]])
                command_results = cyble_fetch_iocs(client, 'GET', token, input_params_alerts_iocs, url)
                ...
                # print(final_input_structure)
                url = base_url + str(ROUTES[COMMAND[demisto.command()]])
                lst_alerts = cyble_events(client, 'POST', token, url, final_input_structure, args, True)
