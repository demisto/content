...
def create_threat_intel_set(args):
    try:
        ...
        ec = {
            "AWS.GuardDuty.Detectors(val.DetectorId === obj.DetectorId).ThreatIntelSet(val.ThreatIntelSetId === obj.ThreatIntelSetId)": data}
    ...
    try:
        ...
        for member in response['Members']:
            data.append({
                ...
                'MemberDetectorId': member['DetectorId'],
                'MemberMasterId': member['MasterId'],
                'MemberEmail': member['Email'],
                'MemberRelationshipStatus': member['RelationshipStatus'],
                'MemberInvitedAt': member['InvitedAt'],
                'MemberUpdatedAt': member['UpdatedAt'],
            })

        output = json.dumps(response['Members'], cls=DatetimeEncoder)
        raw = json.loads(output)
        ec = {"AWS.GuardDuty.Members": data}
        return create_entry('AWS GuardDuty Members', data, ec)
...
def get_members(args):
    try:
        ...
        ec = {"AWS.GuardDuty.Members(val.MemberAccountId === obj.MemberAccountId)": data}
        return create_entry('AWS GuardDuty Members', data, ec)
