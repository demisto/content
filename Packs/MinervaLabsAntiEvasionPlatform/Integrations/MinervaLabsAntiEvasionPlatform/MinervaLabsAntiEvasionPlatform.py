...
def get_from_url(url):
    ...
    return {
        ...
        "HumanReadable": tableToMarkdown(
            pascalToSpace(url_text), response.json(), headerTransform=pascalToSpace, removeNull=True
        ),
...
@logger
def login():
    return session.post(f"{BASE_URL}/api/login", json={"username": USERNAME, "password": PASSWORD})
...
def fetch_incidents():
    try:
        r_events = session.post(f"{BASE_URL}/api/events", json={"archive": False})
        ...
        for event in r_events.json():
            incident = {"name": f'Minerva Labs Event - {event["type"]}', "rawJSON": json.dumps(event)}
            incidents.append(incident)
            session.put(f"{BASE_URL}/api/events/archive", json={"events": event["id"]})
...
def add_exclusion(exclusion_type, exclusion_data, exclusion_description, groups_id):
    ...
    results = {
        ...
        "Contents": response.json(),
        "HumanReadable": tableToMarkdown(
            f"Exclusion '{exclusion_description}' was added", response.json(), headerTransform=pascalToSpace, removeNull=True
        ),
...
def delete_exclusion(exclusion_id, exclusion_type):
    json_params = {"id": exclusion_id, "type": exclusion_type}
...
def add_vaccine(vaccine_name, vaccine_desc, monitor_only):
    json_params = {"name": vaccine_name, "description": vaccine_desc, "isMonitorOnly": monitor_only, "type": "mutex"}
    ...
    results = {
        ...
        "Contents": response.json(),
        "HumanReadable": tableToMarkdown(
            f"Vaccination '{vaccine_desc}' was added", response.json(), headerTransform=pascalToSpace, removeNull=True
        ),
    }
...
def search(search_url, search_param, search_condition, search_value):
    ...
    results = {
        ...
        "Contents": response.json(),
        "HumanReadable": tableToMarkdown(
            f'Search results for \'{search_url.split("/")[-1]}\'', response.json(), headerTransform=pascalToSpace, removeNull=True
        ),
