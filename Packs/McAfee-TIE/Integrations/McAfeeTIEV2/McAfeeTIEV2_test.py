...
def test_set_files_reputation_invalid():
    ...
    with patch("dxltieclient.TieClient") as mock_tie_client:
        ...
        with pytest.raises(Exception) as e:
            set_files_reputation_command(
                hashes=["hash1", "hash2"], tie_client=tie_client, trust_level="invalid_trust_level", filename="", comment=""
            )
...
def test_set_files_reputation_valid(mocker):
    ...
    with patch("dxltieclient.TieClient") as mock_tie_client:
        ...
        result = set_files_reputation_command(
            hashes=["hash1", "hash2"], tie_client=tie_client, trust_level="valid_trust_level", filename="", comment=""
        )
...
def test_files_references_invalid_query_limit(params):
    ...
    with patch("dxltieclient.TieClient") as mock_tie_client:
        tie_client = mock_tie_client.return_value
        with pytest.raises(Exception) as e:
            files_references_command(hashes=["hash1", "hash2"], tie_client=tie_client, query_limit=params["query_limit"])
...
REFERENCES_HUMAN_READABLE_CASES = [
    (
        [
            {"agentGuid": "0c906be0-224c-45d4-8e6f-bc89da69d268", "date": 1508081600445},
            {"agentGuid": "{3a6f574a-3e6f-436d-acd4-bcde336b054d}", "date": 1475873692},
        ],
        [
            {"Date": "2017-10-15 15:33:20", "AgentGuid": "0c906be0-224c-45d4-8e6f-bc89da69d268"},
            {"Date": "2016-10-07 20:54:52", "AgentGuid": "3a6f574a-3e6f-436d-acd4-bcde336b054d"},
        ],
    ),
]
...
FILE_REFERENCES_CASES = [
    (
        ...
        [
            [
                {"agentGuid": "0c906be0-224c-45d4-8e6f-bc89da69d268", "date": 1508081600445},
                {"agentGuid": "{68125cd6-a5d8-11e6-348e-000c29663178}", "date": 1478626172},
            ],
            [
                {"agentGuid": "{3a6f574a-3e6f-436d-acd4-bcde336b054d}", "date": 1475873692},
                {"agentGuid": "70be2ee9-7166-413b-b03e-64a48f6ab6c8", "date": 1508081651295},
            ],
        ],
        [
            {
                ...
                "References": [
                    {"Date": "2017-10-15 15:33:20", "AgentGuid": "0c906be0-224c-45d4-8e6f-bc89da69d268"},
                    {"Date": "2016-11-08 17:29:32", "AgentGuid": "68125cd6-a5d8-11e6-348e-000c29663178"},
                ],
            },
            {
                "Hash": "hash2",
                "References": [
                    {"Date": "2016-10-07 20:54:52", "AgentGuid": "3a6f574a-3e6f-436d-acd4-bcde336b054d"},
                    {"Date": "2017-10-15 15:34:11", "AgentGuid": "70be2ee9-7166-413b-b03e-64a48f6ab6c8"},
                ],
            },
        ],
    ),
...
def test_files_references(mocker, params, raw_result, expected_parsed_results):
    ...
    from McAfeeTIEV2 import MAX_QUERY_LIMIT, files_references_command
    ...
    with patch("dxltieclient.TieClient") as mock_tie_client:
        ...
        results = files_references_command(hashes=params["hashes"], tie_client=tie_client, query_limit=query_limit)
...
def test_files_reputations(mocker, raw_result_file, parsed_results_file):
    ...
    with patch("dxltieclient.TieClient") as mock_tie_client:
        tie_client = mock_tie_client.return_value
        results = files_reputations_command(
            hashes=raw_responses["hashes"], tie_client=tie_client, reliability="C - Fairly reliable"
        )
