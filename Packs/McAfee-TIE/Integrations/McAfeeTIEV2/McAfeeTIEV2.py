    ...
    def parse_data(self, reputation_data: Dict[str, Any]):
        ...
        for key, val in reputation_data.items():
            if key == FileReputationProp.ATTRIBUTES:
                ...
                parsed_res |= self.parse_reputation_key(reputation_key=key, val=val)
        return parsed_res
...
def safe_get_file_reputation(tie_client: TieClient, api_input: Dict[str, str]):
    try:
        ...
        demisto.info(f"McAfee failed to get file reputation with error: {e!s}")
        return None
    return res
...
def parse_file_reputation_raw_result(raw_result: Dict, file_hash: str, reliability: str, hash_type_key: str):
    ...
    for reputation in reputations:
        ...
        parsed_reputation_data[get_provider_abbr(provider_id=provider_id)] = parse_reputation(
            provider_id=provider_id, reputation=reputation
        )
    ...
    if lowest_trust_level == max_num:
        lowest_tl_score = {LOWEST_TRUST_LEVEL_KEY: TrustLevel.NOT_SET, LOWEST_SCORE_KEY: Common.DBotScore.NONE}
    else:
        lowest_tl_score = {LOWEST_TRUST_LEVEL_KEY: lowest_trust_level, LOWEST_SCORE_KEY: trust_level_to_score(lowest_trust_level)}
    ...
    file_instance = get_file_instance(dbot_score=dbot_score, file_hash=file_hash, hash_type_key=hash_type_key)
    if lowest_tl_score[LOWEST_SCORE_KEY] >= Common.DBotScore.SUSPICIOUS:
        dbot_score.malicious_description = f"Trust level is {lowest_tl_score[LOWEST_TRUST_LEVEL_KEY]!s}"
    ...
    return CommandResults(
        readable_output=tableToMarkdown(f"McAfee TIE Hash Reputations For {file_hash}:", table),
    ...
    for file_hash in hashes:
        ...
        if not raw_result:
            ...
            command_result = CommandResults(
                readable_output=f"McAfee TIE Hash Reputation For {file_hash} was not found",
                indicator=file_instance,
            )
        else:
            command_result = parse_file_reputation_raw_result(
...
def files_references_command(hashes: List[str], tie_client: TieClient, query_limit: int) -> List[CommandResults]:
    ...
    for file_hash in hashes:
        ...
        api_input = {hash_type_key: file_hash}
        raw_result = tie_client.get_file_first_references(api_input, query_limit=query_limit)

        table = references_to_human_readable(raw_result)

        file_instance = get_file_instance(dbot_score=None, file_hash=file_hash, hash_type_key=hash_type_key)
        if not raw_result:
            command_result = CommandResults(
                readable_output=f"McAfee TIE Hash Reference For {file_hash} was not found",
                indicator=file_instance,
            )
        else:
            ...
            command_result = CommandResults(
                readable_output=tableToMarkdown(f"References For Hash {file_hash}:", table),
                raw_response=raw_result,
...
def set_files_reputation_command(hashes: List[str], tie_client: TieClient, trust_level: str, filename: str, comment: str):
    ...
    for file_hash in hashes:
        ...
        tie_client.set_file_reputation(trust_level=trust_level_key, hashes=api_input, filename=filename, comment=comment)
...
def create_dxl_config(instance_cert: InstanceCertificates) -> DxlClientConfig:
    with (
        ...
        dxl_config_files = DXLConfigFiles(
            broker_ca_bundle_file=broker_certs_file.name,
...
def get_instance_certificates(params: Dict[str, str]) -> InstanceCertificates:
    return InstanceCertificates(
        broker_ca_bundle=params["broker_ca_bundle"],
...
def files_reputations(dxl_client: DxlClient, hashes: List[str], reliability: str) -> List[CommandResults]:
    ...
    return files_reputations_command(hashes=hashes, tie_client=tie_client, reliability=reliability)
...
def files_references(dxl_client: DxlClient, hashes: List[str], args_query_limit: Union[int, None]) -> List[CommandResults]:
    ...
    return files_references_command(
        hashes=hashes,
...
def set_files_reputation(
    dxl_client: DxlClient, hashes: List[str], trust_level: str, filename: str, comment: str
) -> CommandResults:
    tie_client = get_tie_client(dxl_client)
    ...
    try:
        ...
        with create_dxl_client(instance_cert=instance_cert) as dxl_client:
            ...
            if command == "test-module":
                ...
                return_results(files_reputations(dxl_client=dxl_client, hashes=hashes, reliability=reliability))

            elif command == "tie-file-references":
                return_results(
                    files_references(
                        dxl_client=dxl_client, hashes=hashes, args_query_limit=arg_to_number(args.get("query_limit", None))
                    )
                )

            elif command == "tie-set-file-reputation":
                return_results(
                    set_files_reputation(
                        dxl_client=dxl_client,
                        hashes=hashes,
