...
To use the `tie-set-file-reputation` command, you need to authorize the client (Demisto) to run the command. Follow the [instructions](https://opendxl.github.io/opendxl-client-python/pydoc/marsendauth.html) to do so. In step #4, instead of selecting **Active Response Server API**, select **TIE Server Set Enterprise Reputation**.
