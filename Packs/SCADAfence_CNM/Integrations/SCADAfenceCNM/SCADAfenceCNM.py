...
SCADAFENCE_ALERT_SEVERITY_LEVEL = {"Information": 0, "Warning": 1, "Threat": 2, "Severe": 3, "Critical": 4}

""" HELPER FUNCTIONS """

INCIDENT_TYPES = {"IP conflict detected": "SCADAfence IP conlict"}
...
def http_request(method, url_suffix, params_dict, headers):
    ...
    try:
        if method in ["PATCH", "POST"]:
            ...
            params = req_params
            data = None
        res = requests.request(method, url, verify=USE_SSL, data=data, params=params, headers=headers)
        if res.text:
            res_msg = res.text
...
def fetch_incidents():
    ...
    if tmp_time[0] > last_updated[0]:
        demisto.setLastRun({"createdOn": tmp_time[1]})
...
if demisto.command() == "test-module":
    ...
    api_suffix = "/alert"
    alert_data = {"ip": ip, "severity": severity, "details": description, "active": active, "remediation": remediation}
    ...
    demisto.results(
        {
            ...
            "EntryContext": {"SCADAfence.Alert": data},
        }
    )
    ...
    demisto.results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": md,
            "EntryContext": {"SCADAfence.Alert(val.id==obj.id)": output},
        }
    )
    ...
    demisto.results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": md,
            "EntryContext": {"SCADAfence.Alert.status": alert_status},
        }
    )
    ...
    demisto.results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": md,
            "EntryContext": {"SCADAfence.Asset(val.ip==obj.ip)": data},
        }
    )
    ...
    demisto.results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": md,
            "EntryContext": {"SCADAfence.Asset.Conn(val.ip==obj.ip)": result},
        }
    )
    ...
    demisto.results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": md,
            "EntryContext": {"SCADAfence.Connection": data},
        }
    )
    ...
    demisto.results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": md,
            "EntryContext": {"SCADAfence.AssetTraffic": data_x},
        }
    )
