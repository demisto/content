...
def create_search_request_body(query_string: str, entity_type: str):
    return {"requests": [{"entityTypes": [entity_type], "query": {"queryString": query_string}}]}
    ...
    def search_content_command(self, args):
        ...
        if response["value"][0]["hitsContainers"][0]["total"] > 0:
            for hit in response["value"][0]["hitsContainers"][0]["hits"]:
                row = {"Summary": hit["summary"], "WebURL": hit["resource"]["webUrl"]}
        ...
        return CommandResults(
            readable_output=readable_output, outputs_prefix="SearchContent", outputs=response, raw_response=response
        )
