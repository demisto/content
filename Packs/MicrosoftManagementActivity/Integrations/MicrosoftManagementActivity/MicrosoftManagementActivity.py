...
class Client(BaseClient):
    ...
    def list_content_request(self, content_type, start_time, end_time):
        ...
        params = {"PublisherIdentifier": PUBLISHER_IDENTIFIER, "contentType": content_type}
    ...
    def list_subscriptions_request(self):
        ...
        params = {"PublisherIdentifier": PUBLISHER_IDENTIFIER}
        response = self.http_request(
            method="GET", url_suffix=self.suffix_template.format(self.tenant_id, "list"), headers=headers, params=params
        )
        return response
...
def test_module(client: Client):
    ...
    raise DemistoException(
        "The basic parameters are ok, authentication cannot be checked using the *Test* button.\n "
...
def get_start_or_stop_subscription_context(content_type, start_or_stop):
    ...
    subscription_context = {"ContentType": content_type, "Enabled": is_subscription_enabled}
    entry_context = {
...
def start_or_stop_subscription_command(client, args, start_or_stop):
    ...
    try:
        ...
        return_outputs(readable_output=human_readable, outputs=entry_context, raw_response={})
...
def get_enabled_subscriptions_content_types(enabled_subscriptions):
    enabled_subscriptions_content_types = [
        subscription.get("contentType") for subscription in enabled_subscriptions if subscription.get("status") == "enabled"
    ]
...
def get_subscriptions_context(enabled_subscriptions):
    ...
    for subscription_content_type in enabled_subscriptions:
        subscription_context = {"ContentType": subscription_content_type, "Enabled": True}
...
def list_subscriptions_command(client):
    ...
    enabled_subscriptions_content_types = get_enabled_subscriptions_content_types(
        subscriptions
    )  # Subscriptions are defined by their content type
    enabled_subscriptions_context = get_subscriptions_context(enabled_subscriptions_content_types)
    human_readable = tableToMarkdown(
        "Current Subscriptions", enabled_subscriptions_content_types, headers="Current Subscriptions"
    )
    entry_context = {
        "MicrosoftManagement.Subscription(val.ContentType && val.ContentType === obj.ContentType)": enabled_subscriptions_context
    }
    return_outputs(readable_output=human_readable, raw_response=enabled_subscriptions_context, outputs=entry_context)
...
def does_record_match_filters(record, filter_accepted_record_type_ids, filter_accepted_workloads, filter_accepted_operations):
    ...
    record_matches_record_type_filter = (
        not should_filter_by_record_types or record.get("RecordType") in filter_accepted_record_type_ids
    )
...
def filter_records(content_records, filter_data):
    ...
    if filter_accepted_operations:
        filter_accepted_operations = [op.lower() for op in filter_accepted_operations]
    ...
    filter_accepted_record_type_ids = (
        record_types_to_type_ids(filter_accepted_record_types) if filter_accepted_record_types else None
    )

    filtered_records = []
    for record in content_records:
        if does_record_match_filters(
            record, filter_accepted_record_type_ids, filter_accepted_workloads, filter_accepted_operations
        ):
...
def list_content_command(client, args):
    ...
    return_outputs(
        ...
        outputs={"MicrosoftManagement.ContentRecord(val.ID && val.ID === obj.ID)": content_records_context},
...
def get_content_types_to_fetch(client):
    ...
    if not content_types_to_fetch:
        ...
        subscriptions = client.list_subscriptions_request()
        content_types_to_fetch = get_enabled_subscriptions_content_types(subscriptions)
    ...
    try:
        ...
        if not managed_identities_client_id:
            if not self_deployed and not enc_key:
                raise DemistoException(
                    "Key must be provided. For further information see https://xsoar.pan.dev/docs"
        ...
        if command == "fetch-incidents":
            next_run, incidents = fetch_incidents(
                client=client, last_run=demisto.getLastRun(), first_fetch_datetime=first_fetch_datetime
            )
