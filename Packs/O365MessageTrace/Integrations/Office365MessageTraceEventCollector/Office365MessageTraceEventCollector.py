...
# TODO: Implement check for start_date to ensure it is not more than 10 days in the past
...
class Client(BaseClient):
    ...
    def _make_authenticated_request(self, method: str, url_suffix: str, params: Optional[Dict] = None) -> Dict:
    ...
    def get_message_trace(self, start_date: Optional[str] = None, end_date: Optional[str] = None,
                         sender_address: Optional[str] = None, recipient_address: Optional[str] = None,
                         message_trace_id: Optional[str] = None, status: Optional[str] = None,
                         top: int = 0, skip_token: int = 0) -> Dict[str, Any]:
    ...
    def get_message_trace_detail(self, message_trace_id: str, recipient_address: str,
                                sender_address: str, start_date: str, end_date: str) -> Dict[str, Any]:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            demisto.debug(f"Connection test failed: {str(e)}")
            return False


def office365_message_trace_list_paging(client: Client, start_date: Optional[str] = None,
    ...
    while True:
        ...
        if "odata.nextLink" not in result.keys():
            break
...
def parse_message_trace_date_range(date_range: Optional[str] = None,
                                   processing_delay: int = 0) -> tuple[str, str]:
    ...
    if date_range:
        if 'hour' in date_range.lower():
            hours = int(date_range.split()[0])
            start_date = end_date - timedelta(hours=hours)
        elif 'day' in date_range.lower():
            days = int(date_range.split()[0])
            start_date = end_date - timedelta(days=days)
        elif 'minute' in date_range.lower():
            minutes = int(date_range.split()[0])
            start_date = end_date - timedelta(minutes=minutes)
        else:
            # Default to fallback if format not recognized
...
def format_message_trace_results(data: List[Dict[str, Any]],
                                 for_dataset: bool = False) -> List[Dict[str, Any]]:
...
def get_events(client: Client,
               last_timestamp: str,
               processing_delay: int) -> tuple[List[Dict[str, Any]], str]:
    ...
    result = office365_message_trace_list_paging(
        client=client,
...
def office365_message_trace_fetch_events_command(client: Client):
    ...
    demisto.debug(f"Context: {json.dumps(ctx)}")
    ...
    events, new_last_timestamp = get_events(
        client=client,
