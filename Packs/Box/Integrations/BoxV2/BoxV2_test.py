...
class ClientTestBox:
    ...
    def __init__(self, mocker):
        ...
        self.client = Client(base_url="https://api.box.com", verify=False, proxy=False, auth_params=test_params)
...
def test_find_file_folder_by_share_link(requests_mock, mocker):
    ...
    mock_response = util_load_json("test_data/find_file_folder_by_share_link.json")
    requests_mock.get("https://api.box.com/2.0/shared_items/", json=mock_response)

    args = {"shared_link": "https://app.box.com/s/testing", "password": "some_pass", "as_user": "1234567"}
    ...
    assert (
        requests_mock.request_history[0].headers.get("BoxApi")
        == "shared_link=https://app.box.com/s/testing&shared_link_password=some_pass"
    )
...
def test_file_share_link_object():
    ...
    expected_request_object = {"access": "some_access", "password": "some_pass", "permissions": {"can_download": False}}
...
def test_create_update_file_share_link(requests_mock, mocker):
    ...
    requests_mock.put("https://api.box.com/2.0/files/742246263170/?fields=shared_link", json=mock_response)

    client = ClientTestBox(mocker).client

    args = {"file_id": "742246263170", "password": "some_pass", "access": "open", "as_user": "1234567"}
...
def test_get_shared_link_for_file_command(requests_mock, mocker):
    ...
    mock_response = util_load_json("test_data/create_update_file_share_link.json")
    requests_mock.get("https://api.box.com/2.0/files/742246263170/", json=mock_response)

    client = ClientTestBox(mocker).client

    args = {"file_id": "742246263170", "as_user": "1234567"}
...
def test_create_update_folder_share_link_command(requests_mock, mocker):
    ...
    mock_response = util_load_json("test_data/create_update_file_share_link.json")
    requests_mock.put("https://api.box.com/2.0/folders/742246263170/", json=mock_response)
...
def test_remove_folder_share_link_command(requests_mock, mocker):
    ...
    args = {"folder_id": "742246263170", "as_user": "1234567"}
...
def test_get_folder_command(requests_mock, mocker):
    ...
    requests_mock.get("https://api.box.com/2.0/folders/0/", json=mock_response)

    client = ClientTestBox(mocker).client

    args = {"folder_id": "0", "as_user": "1234567"}
...
def test_list_folder_items_command(requests_mock, mocker):
    ...
    args = {"folder_id": "0", "as_user": "1234567", "limit": "100", "offset": "0", "sort": "asc"}
...
def test_folder_create_command(requests_mock, mocker):
    ...
    mock_response = util_load_json("test_data/get_folder.json")
    requests_mock.post("https://api.box.com/2.0/folders/", json=mock_response)

    client = ClientTestBox(mocker).client

    args = {"parent_id": "0", "as_user": "1234567", "name": "Testing Folder"}
...
def test_file_delete_command(requests_mock, mocker):
    ...
    requests_mock.delete("https://api.box.com/2.0/files/12345", status_code=successful_file_deletion_status_code)

    client = ClientTestBox(mocker).client

    args = {"as_user": "1234567", "file_id": "12345"}
...
def test_list_users_command(requests_mock, mocker):
    ...
    mock_response = util_load_json("test_data/get_folder.json")
    requests_mock.get("https://api.box.com/2.0/users/", json=mock_response)

    client = ClientTestBox(mocker).client

    args = {"fields": "name", "filter_term": "test_user", "limit": "100", "offset": "0"}
...
def test_upload_file_command_with_chunks(requests_mock, mocker):
    ...
    session_request_response = {"id": "test_session_id", "part_size": 70000}
    ...
    part_data = {"part": {"some_info": "some-result"}}

    # The response given when a session is committed.
    session_commit_response = {"entities": {"name": "some_file", "file_type": "jpg", "id": 123}}

    # Mock for the request to open an upload session.
    requests_mock.post("https://upload.box.com/api/2.0/files/upload_sessions", json=session_request_response)
    requests_mock.put("https://upload.box.com/api/2.0/files/upload_sessions/test_session_id", json=part_data)
    requests_mock.post(
        "https://upload.box.com/api/2.0/files/upload_sessions/test_session_id/commit", json=session_commit_response
    )

    client = ClientTestBox(mocker).client

    args = {"entry_id": "123@123", "file_name": "test_user.png", "folder_id": "100", "as_user": "0"}
...
def test_get_current_user_command(requests_mock, mocker):
    ...
    args = {"as_user": "sample_current_user"}

    requests_mock.get("https://api.box.com/2.0/users/me/", json=mock_response)
...
def test_url_as_param(requests_mock, mocker):
    ...
    client = Client(base_url=other_url, verify=False, proxy=False, auth_params=test_params)

    args = {"as_user": "sample_current_user"}

    mocked_request = requests_mock.get(f"{other_url}/2.0/users/me/", json=mock_response)
...
def test_create_user_command(requests_mock, mocker):
    ...
    requests_mock.post("https://api.box.com/2.0/users/", json=mock_response)
...
def test_update_user_command(requests_mock, mocker):
    ...
    requests_mock.put("https://api.box.com/2.0/users/12345/", json=mock_response)
...
def test_delete_user_command(requests_mock, mocker):
    ...
    args = {"as_user": "sample_current_user", "user_id": "12345", "force": "true"}

    requests_mock.delete("https://api.box.com/2.0/users/12345/", status_code=204)
...
def test_list_user_events_command(requests_mock, mocker):
    ...
    args = {"as_user": "sample_current_user", "stream_type": "all"}
    mock_response = util_load_json("test_data/events.json")
    requests_mock.get("https://api.box.com/2.0/events/", json=mock_response)
...
def test_list_enterprise_events_command(requests_mock, mocker):
    ...
    args = {"as_user": "sample_current_user", "stream_type": "all", "created_after": "3 days"}
    mock_response = util_load_json("test_data/events.json")
    requests_mock.get("https://api.box.com/2.0/events/", json=mock_response)
...
def test_move_folder_command(requests_mock, mocker):
    ...
    args = {"from_user_id": "123456", "to_user_id": "7890", "notify": "true"}
    mock_response = util_load_json("test_data/get_folder.json")
    requests_mock.put("https://api.box.com/2.0/users/123456/folders/0/", json=mock_response)
...
def test_main(mocker):
    ...
    client = ClientTestBox(mocker).client
    demisto_args = {"max_fetch": 50, "fields": "name", "filter_term": "test_user", "limit": "100", "offset": "0"}
...
def test_trashed_items_list_command(mocker):
    ...
    assert (
        results.readable_output == "### Trashed items were found.\n|Id|Name|\n|---|---|\n"
        "| 123 | test_file |\n| 456 | test_folder |\n"
    )
