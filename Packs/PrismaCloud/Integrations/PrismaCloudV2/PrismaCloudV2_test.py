...
def prisma_cloud_v2_client(mocker):
    ...
    return Client(
        server_url="https://api.prismacloud.io/",
        verify=True,
        proxy=False,
        headers=headers,
        username="username",
...
def test_alert_search_command_no_next_token(mocker, prisma_cloud_v2_client):
    ...
    http_request = mocker.patch.object(prisma_cloud_v2_client, "_http_request")
    args = {
        "filters": "alert.status=open,policy.remediable=true,cloud.type=gcp,policy.type=config",
        "limit": "10",
...
def test_alert_search_command_with_next_token(mocker, prisma_cloud_v2_client):
    ...
    http_request = mocker.patch.object(prisma_cloud_v2_client, "_http_request")
    args = {"limit": "10", "time_range_unit": "week", "time_range_value": "3", "next_token": "TOKEN"}
    alert_search_command(prisma_cloud_v2_client, args)
    http_request.assert_called_with(
        "POST",
        "v2/alert",
        params={"detailed": "true"},
        json_data={"limit": 10, "timeRange": {"type": "relative", "value": {"amount": 3, "unit": "week"}}, "pageToken": "TOKEN"},
        retries=2,
    )
...
def test_alert_dismiss_command(mocker, prisma_cloud_v2_client):
    ...
    args = {
        "alert_ids": "P-123456",
        "policy_ids": "a11b2cc3-1111-2222-33aa-a1b23ccc4dd5",
        "dismissal_note": "from XSOAR",
...
def test_alert_snooze_command(mocker, prisma_cloud_v2_client):
    ...
    http_request.assert_called_with(
        ...
        "alert/dismiss",
        json_data={
            "alerts": ["P-123456", "P-111111"],
            "dismissalNote": "from XSOAR",
            "dismissalTimeRange": {"type": "relative", "value": {"amount": 1, "unit": "hour"}},
            "filter": {"timeRange": {"type": "relative", "value": {"amount": 1, "unit": "hour"}}},
        },
        resp_type="response",
    )
...
def test_alert_reopen_command(mocker, prisma_cloud_v2_client):
    ...
    args = {
        "policy_ids": "a11b2cc3-1111-2222-33aa-a1b23ccc4dd5",
        "filters": "alert.status=dismissed",
        "time_range_date_from": "01/31/2023",
        "time_range_date_to": "02/01/2023",
    }
    ...
    http_request.assert_called_with(
        ...
        json_data={
            "policies": ["a11b2cc3-1111-2222-33aa-a1b23ccc4dd5"],
            "dismissalTimeRange": {"type": "absolute", "value": {"startTime": 1675123200000, "endTime": 1675209600000}},
            "filter": {
                "timeRange": {"type": "absolute", "value": {"startTime": 1675123200000, "endTime": 1675209600000}},
                "filters": [{"name": "alert.status", "operator": "=", "value": "dismissed"}],
            },
        },
        resp_type="response",
    )
...
def test_remediation_command_list_command(mocker, prisma_cloud_v2_client):
    ...
    http_request.assert_called_with(
        ...
        "alert/remediation",
        json_data={
            "filter": {"timeRange": {"type": "to_now", "value": "epoch"}},
            "policies": ["a11b2cc3-1111-2222-33aa-a1b23ccc4dd5"],
        },
    )
...
def test_alert_remediate_command_fail(mocker, prisma_cloud_v2_client):
    ...
    http_request = mocker.patch.object(
        ...
        side_effect=DemistoException(
            message="Error in API call [405] - Method Not Allowed", res=MockRes({"x-redlock-status": error_header}, 405)
        ),
    )
    args = {"alert_id": "P-123456"}
    command_results = alert_remediate_command(prisma_cloud_v2_client, args)
    ...
    assert command_results.outputs == {
        "alertId": "P-123456",
        "successful": False,
        "failureReason": "remediation unavailable",
        "errorValue": None,
    }
...
def test_config_search_command(mocker, prisma_cloud_v2_client):
    ...
    http_request.assert_called_with(
        ...
        json_data={
            "limit": 1,
            "query": "config from cloud.resource where cloud.region = 'AWS Ohio' ",
...
def test_event_search_command(mocker, prisma_cloud_v2_client):
    ...
    http_request.assert_called_with(
        ...
        json_data={
            "limit": 5,
            "query": "event from cloud.audit_logs where cloud.type = 'aws'",
            "timeRange": {"type": "to_now", "value": "epoch"},
        },
    )
...
def test_network_search_command(mocker, prisma_cloud_v2_client):
    ...
    http_request = mocker.patch.object(prisma_cloud_v2_client, "_http_request")
    args = {
        "query": "network from vpc.flow_record where cloud.account = 'AWS Prod' AND "
...
def test_trigger_scan_command(mocker, prisma_cloud_v2_client):
    ...
    Then:
        - The http request is called with the right arguments
    """
    from PrismaCloudV2 import HEADERS, REQUEST_CCS_AUTH_HEADER, trigger_scan_command
...
def test_error_file_list_command(mocker, prisma_cloud_v2_client):
    ...
    Then:
        - The http request is called with the right arguments
    """
    from PrismaCloudV2 import HEADERS, REQUEST_CCS_AUTH_HEADER, error_file_list_command
    ...
    http_request.assert_called_with(
        "POST", "code/api/v1/errors/files", json_data={"repository": "name/Name", "sourceTypes": ["Github"]}, headers=headers
    )
    ...
    return_value = [
        {
            ...
            "enabled": True,
            "roles": [
                {"id": "a4b4", "name": "Read Only", "type": "Account Group Read Only"},
        ...
        {
            "displayName": "User Not Listed",
            "email": "mail",
            "enabled": True,
            "roles": [{"id": "a4b4", "name": "Read Only", "type": "Account Group Read Only"}],
            "type": "USER_ACCOUNT",
            "username": "not_to_appear",
        },
    ]
    ...
    assert command_results.outputs == [
        {
            ...
            "roles names": ["Read Only", "Other Role"],
            "type": "USER_ACCOUNT",
            "username": "test@paloaltonetworks.com",
        },
        {
...
def test_host_finding_list_command(mocker, prisma_cloud_v2_client):
    ...
    http_request = mocker.patch.object(prisma_cloud_v2_client, "_http_request")
    args = {"rrn": "rrn::name:place:111:a1b2:a%3Ajj55-2023-01-29-09-25", "finding_types": "guard_duty_host,guard_duty_iam"}
...
def test_permission_list_command_no_next_token(mocker, prisma_cloud_v2_client):
    ...
    http_request.assert_called_with(
        "POST", "api/v1/permission", json_data={"limit": 2, "query": "config from iam where source.cloud.service.name = 'EC2'"}
    )
...
@pytest.mark.parametrize(
    "dict_input, url_field, expected_result",
    (
        input_data.nested_url_field,
...
@pytest.mark.parametrize(
    "url_to_format, formatted_url",
    (
        ("https://api.prismacloud.io", "https://api.prismacloud.io/"),
...
def test_extract_nested_values():
    ...
    readable_response = {
        ...
        "status": "open",
        "reason": "NEW_ALERT",
        "firstSeen": 1660654610830,
        "lastSeen": 1660654610830,
        "alertTime": 1660654610830,
        "eventOccurred": 1660654610256,
        "resource": {
            "id": "-123456712345679737",
            "name": "AssumeRole",
            "account": "MyAccount",
    ...
    nested_headers = {
        ...
        "resource.data.country": "Country",
        "policy.remediable": "Is Remediable",
        "id": "Alert ID",
    }
...
@pytest.mark.parametrize(
    "base_case, unit_value, amount_value, time_from, time_to, expected_output",
    (
...
@pytest.mark.parametrize("input_filters,expected_parsed_filters", (input_data.with_filters, input_data.empty_filters))
...
def test_handle_filters_error(filter_name):
    ...
    assert (
        de.value.message == f'Filters should be in the format of "filtername1=filtervalue1,filtername2=filtervalue2". '
...
def test_handle_tags_error(tag_name):
    ...
    with pytest.raises(DemistoException) as de:
        handle_tags(filters)
    assert (
        de.value.message == f'Tags should be in the format of "tagkey1=tagvalue1,tagkey2=tagvalue2". '
...
def test_remove_empty_values():
    ...
    dict_input = {
        ...
        "with_value5": {
            "timeRange": {"type": "to_now", "value": "epoch"},
            "filters": [
                {"name": "string1", "operator": "=", "value": "string1"},
    ...
    res = [
        {
            ...
            "lastModifiedBy": "name@company.com",
            "lastModifiedTs": 1611682405313,
            "members": [{"env": "env"}, {"projec": "project"}, {"securit": "security"}],
        },
        {
            "id": "2",
            "name": "Members is strings",
            "resourceListType": "RESOURCE_GROUP",
            "description": "",
            "lastModifiedBy": "name@company.com",
            "lastModifiedTs": 1648181381197,
            "members": ["common"],
        },
        {
            "id": "3",
            "name": "Have namespaces",
            "resourceListType": "GROUP",
            "description": "Have namespaces",
            "lastModifiedBy": "name@company.com",
            "lastModifiedTs": 1648507192479,
            "members": [
                {
                    "hosts": ["*"],
                    "appIDs": ["*"],
...
@pytest.mark.parametrize(
    "given_alert, expected_severity",
    (
        ({"policy": {"severity": "high"}}, IncidentSeverity.HIGH),
...
def test_expire_stored_ids():
    ...
    """
    from PrismaCloudV2 import FETCH_LOOK_BACK_TIME, expire_stored_ids

    updated_last_run_time = 1000000000000
    fetched_ids = {
        "N-111111": 1000000000000,  # same time
...
@pytest.mark.parametrize(
    "now, first_fetch, look_back, last_run_time, expected_fetch_time_range",
    (
...
@pytest.mark.parametrize(
    "last_run_epoch_time, look_back_minutes, expected_epoch_time",
    (
...
def test_fetch_request(
    ...
    fetched_ids = {"P-111111": 1000000110000, "P-222222": 999996400000}
    now = 1000000140000
    assert fetch_request(
        client=prisma_cloud_v2_client,
...
@pytest.mark.parametrize(
    "limit, expected_incidents, expected_updated_fetched_ids",
    (
...
def test_filter_alerts(prisma_cloud_v2_client, limit, expected_incidents, expected_updated_fetched_ids):
    ...
    fetched_ids = {"N-111111": 1000000000000, "P-222222": 999996400000}
    response_items = [
        {"id": "N-111111", "alertTime": 1000000000000, "policy": {"name": "Policy One", "severity": "high"}},
...
def test_filter_alerts_with_updated_alert_time(prisma_cloud_v2_client, limit, expected_incidents, expected_updated_fetched_ids):
    ...
    fetched_ids = {"N-111111": 1000000000000, "P-222222": 999996400000}
    response_items = [
        {"id": "N-111111", "alertTime": 1000000000001, "policy": {"name": "Policy One", "severity": "high"}},
        input_data.truncated_alert6,
        input_data.truncated_alert7,
    ]
...
@pytest.mark.parametrize(
    "alert, expected_incident_context",
    (
...
def test_alert_to_incident_context(prisma_cloud_v2_client, alert, expected_incident_context):
    ...
    """
    from PrismaCloudV2 import add_mirroring_fields, alert_to_incident_context
...
def test_get_modified_remote_data_command(mocker, prisma_cloud_v2_mirroring_client):
    ...
    result = get_modified_remote_data_command(client=prisma_cloud_v2_mirroring_client, args=args, params=params)

    assert result.modified_incident_ids == ["P-1111111", "P-1111112", "P-1111113"]


@pytest.mark.parametrize(
    "raw_response, expected_updated_object",
    [
        (
...
def test_get_remote_alert_data(mocker, prisma_cloud_v2_mirroring_client, raw_response, expected_updated_object):
    ...
    mocker.patch("PrismaCloudV2.Client.alert_get_details_request", return_value=raw_response)
...
@pytest.mark.parametrize(
    "updated_mirrored_object, function_calls",
    [
...
@pytest.mark.parametrize(
    "mirrored_status, mirrored_dismissal_note",
    [
...
@pytest.mark.parametrize(
    "mirrored_data, updated_object, expected_entry",
    [
        (
            input_data.alert_get_details_request_dismissed_alert_raw_response,
            input_data.get_remote_alert_data_dismissed_alert_updated_object,
...
def test_update_remote_alert(
    mocker, prisma_cloud_v2_mirroring_client, incident_status, whether_to_close, whether_to_reopen, expected_result
):
    """
    Given
...
@pytest.mark.parametrize(
    "user_selection, delta, expected_result",
    [
...
labels_data = [
    ...
    (
        [
            {"label": "Breaking Change Fix"},
            {"metadata": {"imageName": "weaveworksdemos/front-end:0.3.12"}, "label": "Image Referencer"},
        ],  # case two dicts
        ["Breaking Change Fix", "Image Referencer"],
    ),  # expected
    (
...
invalid_args = [
    (
        {"license_type": "invalid_type", "some_filter": "value1"},
        DemistoException,
    ...
    (
        {"search_scopes": "scope1", "search_term": "term1", "limit": 10},
        DemistoException,
    ...
    (
        {"search_scopes": "scope1", "some_filter": "value1"},
        DemistoException,
        "The `search_term` argument is required when specifying `search_scopes`.",
    ),
    ({"page": 40, "some_filter": "value1"}, DemistoException, "Please provide both `page` and `page_size` arguments."),
    ({"page_size": 50, "some_filter": "value1"}, DemistoException, "Please provide both `page` and `page_size` arguments."),
    ({"page_size": 1001, "page": 40, "some_filter": "value1"}, DemistoException, "`Page_size` argument can't be more than 1000."),
]
...
def test_code_issues_list_command__has_next(mocker, prisma_cloud_v2_client):
    ...
    m = mocker.patch.object(
        ...
        side_effect=[
            {
                "data": [{"firstDetected": "some_date", "policy": "policy1", "severity": "severity1", "labels": ["label1"]}],
                "hasNext": True,
            },
            {
                "data": [{"firstDetected": "some_date", "policy": "policy1", "severity": "severity1", "labels": ["label1"]}],
...
limit_reached_data = [
        ...
        "data": [
            {
                ...
                "severity": "high",
                "labels": [],
                "repositorySource": "source1",
            },
            {
                "repository": "repo2",
                "firstDetected": "2024-01-02",
                "policy": "policy2",
                "severity": "medium",
                "labels": [],
    ...
    },
    {
        "data": [
            {
                "repository": "repo3",
                "firstDetected": "2024-01-03",
                "policy": "policy3",
                "severity": "low",
                "labels": [],
                "repositorySource": "source3",
            },
            {
                "repository": "repo4",
                "firstDetected": "2024-01-04",
                "policy": "policy4",
                "severity": "critical",
                "labels": [],
                "repositorySource": "source4",
            },
        ],
        "hasNext": False,
    },
...
code_issues_list_request_data = [
    ...
    (
        ...
        {
            "filters": {"branch": "main", "checkStatus": "open", "fixableOnly": True, "severities": ["high", "critical"]},
            "limit": 50,
            "offset": 0,
        },
    ),
    ...
    (
        {"fixable_only": True, "search_scopes": ["scope1", "scope2"], "search_term": "vulnerability", "limit": 10, "offset": 5},
        {
            "filters": {"fixableOnly": True},
            "search": {"scopes": ["scope1", "scope2"], "term": "vulnerability"},
            "limit": 10,
            "offset": 5,
        },
    ),
...
def test_code_issues_list_request(mocker, given_params, expected_body, prisma_cloud_v2_client):
    ...
    m.assert_called_once_with("POST", "/code/api/v2/code-issues/branch_scan", json_data=expected_body)
...
def test_code_issues_list_command__user_pagination(mocker, args, expected_call_count, expected_offset, prisma_cloud_v2_client):
    ...
    m = mocker.patch.object(
        ...
        side_effect=[
            {
                "data": [
                    {"firstDetected": "some_date1", "policy": "policy1", "severity": "severity1", "labels": ["label1"]},
                    {"firstDetected": "some_date2", "policy": "policy2", "severity": "severity2", "labels": ["label2"]},
                ],
                "hasNext": True,
            },
            {
