...
def users_list_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    # reduce response to hold only items that are associated with this username
    if username and response_items:
        response_items = [item for item in response_items if item.get('username') == username]
