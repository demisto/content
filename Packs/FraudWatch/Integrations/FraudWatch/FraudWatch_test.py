...
import pytest
import pytz
from CommonServerPython import CommandResults, DemistoException, datetime
from FraudWatch import (
    ...
    get_and_validate_positive_int_argument,
    get_page_and_limit_args,
    get_time_parameter,
)

BASE_URL = f"{DEFAULT_URL}v1/"
client = Client(api_key="api_key", base_url=BASE_URL, verify=False, proxy=False)
...
@pytest.mark.parametrize(
    "args, argument_name,minimum, maximum, expected",
    [
...
@pytest.mark.parametrize(
    "args, arg_name, maximum, expected_err_msg",
    [
        ({"limit": -3}, "limit", None, f"limit should be equal or higher than {MINIMUM_POSITIVE_VALUE}"),
...
def test_commands_get_methods(
    ...
    """
    requests_mock.get(f"{BASE_URL}{url_suffix}", json=response)
...
def test_commands_put_methods(
    ...
    Then:
     - Ensure that the expected CommandResults object is returned by the command function.
    """
    requests_mock.put(f"{BASE_URL}{url_suffix}", json=response)
    expected_command_results = CommandResults(
        outputs_prefix=expected.get("outputs_prefix"),
        outputs_key_field=expected.get("outputs_key_field"),
        outputs=expected.get("outputs"),
...
def test_commands_post_methods(
    ...
    Then:
     - Ensure that the expected CommandResults object is returned by the command function.
    """
    requests_mock.post(f"{BASE_URL}{url_suffix}", json=response)
    expected_command_results = CommandResults(
        outputs_prefix=expected.get("outputs_prefix"),
        outputs_key_field=expected.get("outputs_key_field"),
        outputs=expected.get("outputs"),
...
@pytest.mark.parametrize(
    "args", [({"page": 2, "page_size": 4, "limit": 3}), ({"page": 2, "limit": 3}), ({"page_size": 4, "limit": 3})]
)
def test_get_page_and_limit_args_invalid(args):
    ...
    """
    with pytest.raises(DemistoException, match="""Limit argument cannot be given with 'page_size' or 'page' argument."""):
