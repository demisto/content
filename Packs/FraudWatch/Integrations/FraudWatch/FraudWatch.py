...
INCIDENT_LIST_MARKDOWN_HEADERS = [
    "identifier",
    "reference_id",
    "url",
    "status",
...
class Client(BaseClient):
    ...
    def __init__(self, api_key: str, base_url: str, verify: bool, proxy: bool):
        ...
        self.base_headers = {"Authorization": f"Bearer {api_key}", "Accept": "application/json"}
    ...
    def incidents_list(
        ...
        params = assign_params(brand=brand, status=status, page=page, limit=limit, to=to_date)
    ...
    def incident_update(
        ...
        return self.http_request(
            ...
            data=assign_params(brand=brand, reference_id=reference_id, evidence=evidence, instructions=instructions),
            headers={**self.base_headers, **self.URL_ENCODED_HEADER},
        )

    def incident_list_by_id(self, incident_id: str):
        return self.http_request(method="GET", url_suffix=f"incident/{incident_id}")

    def incident_get_by_reference(self, reference_id: str):
        return self.http_request(method="GET", url_suffix=f"incident/reference/{reference_id}")

    def incident_forensic_get(self, incident_id: str):
        return self.http_request(method="GET", url_suffix=f"incident/{incident_id}/forensic")

    def incident_contact_emails_list(self, incident_id: str, page: Optional[int] = None, limit: Optional[int] = None):
        return self.http_request(
            method="GET", url_suffix=f"incident/{incident_id}/message", params=assign_params(page=page, limit=limit)
        )
    ...
    def attachment_upload_command(self, incident_id: str, file: Any):
        return self.http_request(method="POST", url_suffix=f"incident/{incident_id}/upload", files=file)

    def brands_list(self, page: Optional[int] = None, limit: Optional[int] = None):
        return self.http_request(method="GET", url_suffix="account/brands", params=assign_params(page=page, limit=limit))
...
def get_and_validate_positive_int_argument(
    args: Dict, argument_name: str, lower_bound: int = MINIMUM_POSITIVE_VALUE, upper_bound: Optional[int] = None
) -> Optional[int]:
    """
...
def get_page_and_limit_args(
    args: Dict, minimum_page_size: int = MINIMUM_POSITIVE_VALUE, maximum_page_size: int = MAX_PAGE_SIZE_VALUE
) -> tuple[int, int]:
    """
    ...
    page_size = get_and_validate_positive_int_argument(
        args, "page_size", lower_bound=minimum_page_size, upper_bound=maximum_page_size
    )
...
def fetch_incidents_command(client: Client, params: Dict, last_run: Dict):
    ...
    current_page = 1
    while True:
        raw_response = client.incidents_list(
            brand=brand, status=status, page=current_page, limit=MAX_PAGE_SIZE_VALUE, from_date=from_date
        )
        if raw_response.get("error"):
            raise DemistoException(f"""Error occurred while pulling incidents from FraudWatch: {raw_response.get('error')}""")
    ...
    incidents = [incident for incident in incidents if get_time_parameter(incident.get("date_opened")) > minimum_date_opened]
    ...
    next_run = {"last_fetch_time": incidents[-1].get("date_opened") if incidents else minimum_date_opened_str}
...
def fraud_watch_incidents_list_command(client: Client, args: Dict) -> CommandResults:
    ...
    return CommandResults(
        ...
        readable_output=tableToMarkdown("FraudWatch Incidents", final_outputs, INCIDENT_LIST_MARKDOWN_HEADERS, removeNull=True),
    )
...
def fraud_watch_incident_update_command(client: Client, args: Dict) -> CommandResults:
    ...
    return CommandResults(
        raw_response=raw_response, readable_output=f"### Incident with ID {incident_id} was updated successfully"
    )
...
def fraud_watch_incident_messages_add_command(client: Client, args: Dict):
    ...
    return CommandResults(raw_response=raw_response, readable_output=human_readable)
...
def fraud_watch_incident_urls_add_command(client: Client, args: Dict) -> CommandResults:
    ...
    """
    incident_id: str = args.get("incident_id")  # type: ignore
    urls: Dict[str, List[str]] = {"urls[]": argToList(args.get("urls"))}
...
def fraud_watch_attachment_upload_command(client: Client, args: Dict):
    ...
    files = [("incident_attachment", (file_info["name"], file))]
...
def fraud_watch_brands_list_command(client: Client, args: Dict) -> CommandResults:
    ...
    Returns:
        CommandResults.
    """
    page, limit = get_page_and_limit_args(
        args, minimum_page_size=MINIMUM_PAGE_SIZE_BRANDS, maximum_page_size=MAX_PAGE_SIZE_BRANDS
    )
    raw_responses = []
    outputs = []
    count = limit
...
def main() -> None:
    ...
    if not api_key:
        raise DemistoException("API token was not given. Please insert your API token. See documentation for further info")
