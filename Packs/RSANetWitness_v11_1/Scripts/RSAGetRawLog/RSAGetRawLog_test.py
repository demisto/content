...
import pytest
import RSAGetRawLog
from RSAGetRawLog import create_id_set, get_metas_log, get_raw_log, is_json, main, merge_dict
...
def test_get_raw_log(mocker):
    ...
    mocker.patch.object(demisto, "executeCommand", return_value=[{"Contents": {"logs": ["log1", "log2"]}}])
...
def test_get_metas_log(mocker):
    mocker.patch.object(RSAGetRawLog, "isCommandAvailable", return_value=True)
    mocker.patch.object(
        demisto, "executeCommand", return_value=[{"Contents": {}, "EntryContext": {"NetWitness.Events": "Test Value"}}]
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"CustomFields": {"rsaalerts": [], "rsarawlogslist": [], "rsametasevents": []}},
...
def test_main(mocker, alerts_incident, expected_results):
