...
from RSANetWitnessv115 import (
    ...
    remove_incident_command,
    incident_add_journal_entry_command,
    incident_list_alerts_command,
    services_list_command,
    hosts_list_command,
    snapshots_list_for_host_command,
    snapshot_details_get_command,
    files_list_command,
    scan_request_command,
    host_alerts_list_command,
    file_alerts_list_command,
    file_download_command,
    mft_download_request_command,
    system_dump_download_request_command,
    process_dump_download_request_command,
    endpoint_isolate_from_network_command,
    endpoint_update_exclusions_command,
    endpoint_isolation_remove_command,
    endpoint_command,
    create_time,
    create_filter,
    create_exclusions_list,
    remove_duplicates_in_items,
    remove_duplicates_for_fetch,
    fetch_incidents,
    paging_command,
    fetch_alerts_related_incident,
    get_mapping_fields_command,
    xsoar_status_to_rsa_status,
    update_remote_system_command,
    get_remote_data_command,
    get_modified_remote_data_command,
    struct_inc_context,
    clean_secret_integration_context,
...
client = Client(
    ...
    fetch_time="1 year",
    fetch_limit="1",
    cred={"identifier": "test", "password": "test"},
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            update_incident_command,
            {"id": "INC-1", "status": "InProgress"},
            "single_incident",
            "update_incident",
        ),
        ("remove_incident_request", remove_incident_command, {"id": "INC-1"}, "empty_response", "empty_response"),
        (
            "incident_add_journal_entry_request",
            incident_add_journal_entry_command,
            {"id": "INC-1", "notes": "test", "author": "Admin"},
            "empty_response",
            "empty_response",
        ),
        (
        ...
        (
            "snapshot_details_get_request",
            snapshot_details_get_command,
            {"agent_id": "1", "snapshot_timestamp": '"2021-10-19T08:51:50.913Z"', "limit": "1"},
            "snapshot_details",
            "snapshot_details",
        ),
        ...
        ("mft_download_request_request", mft_download_request_command, {"agent_id": "1"}, "empty_response", "empty_response"),
        (
            "system_dump_download_request_request",
            system_dump_download_request_command,
            {"agent_id": "1"},
            "empty_response",
            "empty_response",
        ...
        (
            "process_dump_download_request_request",
            process_dump_download_request_command,
            {"agent_id": "1"},
            "empty_response",
            "empty_response",
        ...
        (
            "endpoint_isolate_from_network_request",
            endpoint_isolate_from_network_command,
            {"agent_id": "1"},
            "empty_response",
            "empty_response",
        ...
        (
            "endpoint_update_exclusions_request",
            endpoint_update_exclusions_command,
            {"agent_id": "1"},
            "empty_response",
            "empty_response",
        ...
        (
            "endpoint_isolation_remove_request",
            endpoint_isolation_remove_command,
            {"agent_id": "1"},
            "empty_response",
            "empty_response",
...
def test_endpoint_command_service_id_error(mocker):
    ...
    with pytest.raises(
        DemistoException,
        match="No Service Id provided - To use endpoint command via RSA NetWitness"
...
@pytest.mark.parametrize("alerts_limit", [1, 2])
...
def test_fetch_incidents(mocker, import_alerts: bool, test_data_key: str):
    ...
    expected = fetch_responses[test_data_key][0]
    actual = incidents

    # Normalize rawJSON for comparison
    for a, e in zip(actual, expected):
        if isinstance(a.get("rawJSON"), str):
            a["rawJSON"] = json.loads(a["rawJSON"])
        if isinstance(e.get("rawJSON"), str):
            e["rawJSON"] = json.loads(e["rawJSON"])
...
def test_get_modified_remote_data_command_from_alerts(mocker):
    ...
    mocker.patch.object(RSANetWitnessv115, "clean_old_inc_context", return_value=False)
    mocker.patch.object(
        demisto, "getIntegrationContext", return_value={"IncidentsDataCount": {"INC-1": {"alertCount": 1, "eventCount": 1}}}
    )
    res = get_modified_remote_data_command(client, {}, {"max_fetch": 2, "max_alerts": 2, "max_mirror_time": 0})
...
def test_clean_old_inc_context_with_non_expired_incident(mocker):
    ...
    mocker_setint.assert_called_with(
        {
            "IncidentsDataCount": {"INC-1": {"Created": created_date.strftime(DATE_FORMAT)}},
...
def test_clean_old_inc_context_with_expired_incident(mocker):
    ...
    clean_old_inc_context(max_time_mirror_inc)
    mocker_set_integration.assert_called_with(
        {"IncidentsDataCount": {}, "refresh_token": "SECRET REPLACED", "token": "SECRET REPLACED"}
    )
