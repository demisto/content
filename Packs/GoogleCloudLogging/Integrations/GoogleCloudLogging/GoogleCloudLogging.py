...
def get_entries_request(client: GSuiteClient, request_body: dict) -> dict:
    ...
    return client.http_request(url_suffix="v2/entries:list", method="POST", body=request_body)
...
def create_readable_output(response: list[dict]) -> str:
    ...
    hr = [
        {
            ...
            "Principal Email": entry.get("protoPayload", {}).get("authenticationInfo", {}).get("principalEmail"),
            "Type": entry.get("resource", {}).get("type", {}),
            "Project ID": entry.get("resource", {}).get("labels", {}).get("project_id"),
            "Cluster Name": entry.get("resource", {}).get("labels", {}).get("cluster_name"),
    ...
    return tableToMarkdown(
        "Lists log entries",
        t=hr,
        headers=["TimeStamp", "Log Name", "Insert ID", "Principal Email", "Type", "Project ID", "Cluster Name", "Service Name"],
        removeNull=True,
    )
...
def log_entries_list_command(client: GSuiteClient, args: dict[str, Any]) -> CommandResults:
    ...
    request_body = {"resourceNames": resources, "filter": args.get("filter"), "orderBy": args.get("order_by")}
    if limit:
    ...
    return CommandResults(
        ...
        outputs={
            "GoogleCloudLogging(true)": {"nextPageToken": response.get("nextPageToken")},
        ...
        + tableToMarkdown(
            "Next page token",
            t={"nextPageToken": response.get("nextPageToken", "").replace("--", "\--")},
