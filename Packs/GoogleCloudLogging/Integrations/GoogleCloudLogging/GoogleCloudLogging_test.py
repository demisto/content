...
test_log_entries_command_data = [
    (
        ...
        {
            "resourceNames": ["projects/some_resource", "organizations/some_resource", "billingAccounts/some_resource"],
            "filter": None,
            "orderBy": None,
            "pageSize": 2,
        },
    ...
    (1, {"pageToken": "xx", "filter": None, "order_by": None, "limit": None, "page_size": 3}, {}),
]
...
def test_get_all_results(mocker, client, limit, request_page_size, expected_result):
    ...
    mocker_for_request = mocker.patch.object(
        GoogleCloudLogging, "get_entries_request", return_value={"entries": [], "nextPageToken": "xxx"}
    )
    get_all_results(client, limit, {"resourceNames": [""], "filter": None, "orderBy": "", "pageSize": None, "pageToken": None})
    assert mocker_for_request.call_args[0][1] == {
        "filter": None,
        "orderBy": "",
...
@pytest.mark.parametrize("expected_call_count, args ,expected_request_body", test_log_entries_command_data)
def test_log_entries_list_command(mocker, client, args, expected_call_count, expected_request_body):
    ...
    mocker_for_request = mocker.patch.object(GoogleCloudLogging, "get_entries_request", return_value=DATA.get("api_response"))
    if not all(resource in args for resource in ["organization_name", "billing_account_name", "folders_names", "project_name"]):
        ...
        assert (
            str(ve.value) == "At least one of the following resources must be provided: project_name, organization_name, "
        ...
        assert command_result.outputs == {
            "GoogleCloudLogging(true)": {"nextPageToken": "xxxxxx-xxxxxx"},
            "GoogleCloudLogging.LogsEntry(val.insertId === obj.insertId)": DATA.get("api_response").get("entries"),
        }
        assert (
            command_result.readable_output == "### Lists log entries\n"
            "|TimeStamp|Log Name|Insert ID|Principal Email|Type|Project ID|Cluster Name|\n"
            "|---|---|---|---|---|---|---|\n|"
...
@pytest.mark.parametrize(
    "params, args, command, expected_result",
    [
        (
            {"credentials": {"password": "{334}"}},
            {},
            "test-module",
            "Unable to parse JSON string. Please verify the JSON is valid.",
        ),
        ({"credentials": {"password": json.dumps(TEST_JSON)}}, {}, "test-module", "ok"),
    ],
)
