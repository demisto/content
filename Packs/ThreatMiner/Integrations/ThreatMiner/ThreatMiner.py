...
def http_request(method, url, verify_certificates, headers):
    try:
        res = requests.request(method, url, verify=verify_certificates, headers=headers)
...
def get_domain_whois_rawdata(domain_name, threat_miner_url, verify_certificates):
    ...
    response = http_request("GET", threat_miner_url + threat_miner_domain_url_postfix, verify_certificates, DEFAULT_HEADERS)
...
def get_domain_URI(uris_raw_data, max_returned_array_size):
    ...
    for uri_info in uris_raw_data["raw_domain_uris"]:
        if max_returned_array_size == -1 or uri_counter < max_returned_array_size:
            uris.append({"Address": uri_info["uri"], "LastSeen": uri_info["last_seen"]})
            uri_counter += 1
        else:
            break
...
def create_domain_command_markdown(domain, context):
    ...
    if len(context.get("Whois", "")) != 0:
        md += tableToMarkdown(
            f"Whois for {domain} domain",
            context["Whois"],
            ["Domain", "Server", "CreateDate", "UpdateDate", "Expiration", "NameServers"],
        )
        threat_miner_found_results = True
    if len(context.get("PassiveDNS", "")) != 0:
        md += tableToMarkdown(f"PassiveDNS for {domain} domain", context["PassiveDNS"], ["IP", "FirstSeen", "LastSeen"])
        threat_miner_found_results = True
    if len(context.get("Subdomains", "")) != 0:
        md += tableToMarkdown(f"{domain} Subdomains", context["Subdomains"], ["Subdomains"])
        threat_miner_found_results = True
    if len(context.get("URI", "")) != 0:
        md += tableToMarkdown(f"{domain} URIs", context["URI"], ["Address", "LastSeen"])
        threat_miner_found_results = True
    if len(context.get("MD5", "")) != 0:
        md += tableToMarkdown(f"{domain} Related Samples(hash only)", context["MD5"], ["hashes"])
        threat_miner_found_results = True
...
def domain_command(**kwargs):
    ...
    for domain_name in domains_names_list:
        threat_miner_raw_results = get_domain_report_from_threat_miner(
            domain_name, kwargs.get("threat_miner_url"), kwargs.get("verify_certificates")
        )
        ...
        for passive_dns in passive_dnses:
            context_passive_dnses.append(
                {"IP": passive_dns["ip"], "FirstSeen": passive_dns["first_seen"], "LastSeen": passive_dns["last_seen"]}
            )
...
def create_ip_command_markdown(ip_address, context):
    ...
    if len(context["Whois"]) != 0:
        md += tableToMarkdown(f"Whois for {ip_address}", context["Whois"], ["Address", "Country", "Org", "Bgp", "Reverse", "ASN"])
        threat_miner_found_results = True
    if len(context["PassiveDNS"]) != 0:
        md += tableToMarkdown(f"PassiveDNS for {ip_address}", context["PassiveDNS"], ["Domain", "FirstSeen", "LastSeen"])
        threat_miner_found_results = True
    if len(context["URI"]) != 0:
        md += tableToMarkdown(f"{ip_address} URIs", context["URI"], ["Address", "LastSeen"])
        threat_miner_found_results = True
...
def get_passive_dns(threat_miner_raw_results):
    ...
    for passive_dns in threat_miner_raw_results["raw_passive_dns"]:
        passive_dnses.append(
            {"Domain": passive_dns["domain"], "FirstSeen": passive_dns["first_seen"], "LastSeen": passive_dns["last_seen"]}
        )
...
def ip_command(**kwargs):
    ...
    for ip_address in ips_address_list:
        threat_miner_raw_results = get_ip_report_from_threat_miner(
            ip_address, kwargs.get("threat_miner_url"), kwargs.get("verify_certificates")
        )
...
def get_file_domains_and_ip(domain_and_ip_raw_data, max_returned_array_size):
    ...
    for domain_and_ip in domain_and_ip_raw_data["raw_file_domains"][0]["domains"]:
        if max_returned_array_size == -1 or counter < max_returned_array_size:
            domains_and_ips.append({"Domain": domain_and_ip["domain"], "IP": domain_and_ip["ip"]})
        else:
            break
...
def get_file_AV_detection(raw_file_av_dectection):
    ...
    for av_detection in raw_file_av_dectection["raw_file_av"][0]["av_detections"]:
        av_detections.append({"Name": av_detection["av"], "Detection": av_detection["detection"]})
...
def file_command(**kwargs):
    ...
    for hashed_file in hashed_files_list:
        threat_miner_raw_results = get_file_report_from_threat_miner(
            hashed_file, kwargs.get("threat_miner_url"), kwargs.get("verify_certificates")
        )
...
def create_file_command_markdown(hashed_file, File_context):
    ...
    if len(File_context.get("HTTP", "")) != 0:
        md += tableToMarkdown(f"HTTP for hashed file {hashed_file}", File_context["HTTP"], ["Domain", "URL", "Useragent"])
        threat_miner_found_results = True
    if len(File_context.get("Domains", "")) != 0:
        md += tableToMarkdown(f"Hashed file: {hashed_file} Domains", File_context["Domains"], ["Domain", "IP"])
        threat_miner_found_results = True
    if len(File_context.get("Mutants", "")) != 0:
        md += tableToMarkdown(f"Hashed file: {hashed_file} Mutants", File_context["Mutants"], ["Mutants"])
        threat_miner_found_results = True
    if len(File_context.get("Registry", "")) != 0:
        md += tableToMarkdown(f"Hashed file: {hashed_file} Registry keys", File_context["Registry"], ["Registry"])
        threat_miner_found_results = True
    if len(File_context.get("AV", "")) != 0:
        md += tableToMarkdown(f"Hashed file: {hashed_file} Anti Virus detections", File_context["AV"], ["Name", "Detection"])
        threat_miner_found_results = True
