...
| Absolute.CustomDeviceField.CDFValues.Type | String | The data type of field value. Possible values are: Text, Date, Dropdown. | 
...
| --- | --- | --- |
| device_id | The system unique identifier of the device. | Required | 
| cdf_uid | The unique identifier of the custom device field. Please note, in order to get this value, use the *absolute-custom-device-field-list* command. | Required | 
...
***
Creates a new Freeze request for the devices specified in the device_ids arg.
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| device_ids | A comma sparated list of the unique identifiers of devices included in the request. The Recommendation is using up to 10,000 devices per request. | Required | 
...
| html_message | The user-defined, HTML coded message shown on devices when the Freeze is applied. The message should be a Non-unique HTML format and has 1-4000 characters. | Required | 
| message_name | The user-defined name for the Freeze message. | Required | 
| device_freeze_type | The type of Freeze. Please note: 1. You can not freeze a device that has been reported Stolen in the Absolute console.<br/>2. OnDemand: freezes a device on its next connection to the Absolute Monitoring Center, which is typically within 15 minutes. 2.1. Applies to all supported operating systems.<br/>3. Scheduled: freezes a device on its next connection to the Absolute Monitoring Center on or after a specified date and time. 3.1. Applies to Windows and Mac devices. 3.2. Scheduled freeze date is specified in the scheduled_freeze_date arg. 3.3. Scheduled Freeze requests are only supported on Windows and Mac devices with an active Absolute agent that is regularly connecting to the Absolute Monitoring Center.<br/>4. Offline: freezes a device if it has been offline for a specified period of time. 4.1. Applies to Windows and Mac devices. 4.2. Offline period is specified in the offline_time_seconds args. 4.3. Offline freeze is not available if your Absolute account has been migrated to Offline Freeze Rules. For more information, see the console Help. Possible values are: OnDemand, Scheduled, Offline. | Required | 
...
| passcode_type | The type of passcode to unfreeze a device. Please note: 1. UserDefined: manually set the passcode in passcode. You must specify the passcode arg. 2. RandomForEach: a unique passcode is randomly generated for each device. You must specify the passcode_length arg. 3. RandomForAll: : a passcode is randomly generated and is the same for all devices. ou must specify the passcode_length arg. Possible values are: UserDefined, RandomForEach, RandomForAll. | Required | 
...
| passcode_length | The length of the passcode when it is randomly generated. Required if passcode_type is RandomForEach or RandomForAl. A valid passcode is a number that has 4-8 characters. For example, 8. | Optional | 
...
| --- | --- | --- |
| device_ids | A comma sparated list of the unique identifiers of devices included in the request. The Recommendation is using up to 10,000 devices per request. | Required | 
| remove_scheduled | Whether to remove only a Scheduled Freeze request. Please note: When setting to true: if the Freeze request is a Scheduled Freeze request, the Freeze request is removed. Else, when setting to false, if the Freeze request is not a Scheduled Freeze request, the Freeze request is not removed. Possible values are: true, false. | Optional | 
| remove_offline | Whether to remove only an Offline Freeze request. Please note: When setting to true: if the Freeze request is a Offline Freeze request, the Freeze request is removed. Else, when setting to false, if the Freeze request is not a Offline Freeze request, the Freeze request is not removed. Possible values are: true, false. | Optional | 
...
| Absolute.FreezeRequestDetail.Statuses.messageKey | String | The reference key for the error message or the info message. Errors message statts with 'dds'. Info message starts with 'ddsui'. | 
| Absolute.FreezeRequestDetail.Statuses.messageParams | Unknown | A list of strings describing the error message when status is LaunchFailed. If status isn't LaunchFailed, messageParams is empty. | 
| Absolute.FreezeRequestDetail.Statuses.message | String | The message for the status change that occurred. | 
| Absolute.FreezeRequestDetail.Statuses.udpatedBy | String | The last entity to update the Freeze request. | 
| Absolute.FreezeRequestDetail.Statuses.updatedUtc | Date | The time \(in Unix Epoch\) that the Freeze request was last updated. | 
...
| Absolute.FreezeRequestDetail.Statuses.instruction | String | All action instructions which are sent from device DFZ agent component. | 
| Absolute.FreezeRequestDetail.Statuses.scheduledFreezeDateUTC | Number | The date and time \(in UNIX Epoch\) that a Scheduled Freeze request was scheduled to be performed. | 
...
| Absolute.FreezeRequestDetail.Configuration.freezeId | String | The user-friendly identifier of the request that is displayed in event history in the Absolute console \(same as EventHistoryId\). | 
...
| Absolute.FreezeRequestDetail.Configuration.preLoginEnabled | Boolean | Whether prelogin is enabled on the device. | 
...
| Absolute.FreezeRequestDetail.RequesterUid | String | The system-defined unique identifier of the user that created the Freeze request. | 
| Absolute.FreezeRequestDetail.CreatedUTC | Date | The date and time \(in UNIX Epoch\) that the Freeze request was created. | 
| Absolute.FreezeRequestDetail.ChangedUTC | Date | The date and time \(in UNIX Epoch\) that the Freeze request was last modified. | 
...
| Absolute.FreezeRequestDetail.EventHistoryId | String | The user-friendly identifier of the request that is displayed in event history in the Absolute console \(same as freezeId\). | 
...
| Absolute.FreezeRequestDetail.PolicyConfigurationVersion | Number | The version of configuration for the policy. | 
...
| Absolute.FreezeMessage.Content | String | The user-defined, HTML coded message that shows on a devices when a Freeze is applied. | 
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| html_message | The user-defined, HTML coded message that shows on a devices when a Device Freeze is applied. Should be in an HTML format with 1-4000 characters. | Required | 
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| device_ids | A comma separateed list of device UIDs should be unenrolled. | Required | 
...
| Absolute.DeviceUnenroll.SystemName | String | The The name assigned to the device. | 
...
| return_fields | A comma sparated list of all specific values to return. If not set, all possible values will be returned. | Optional | 
| account_uids | A comma sparated list of the unique ID associated with this Absolute accounts. | Optional | 
| device_ids | A comma sparated list of the system-defined unique identifier of the devices. | Optional | 
| device_names | A comma sparated list of the devices names. | Optional | 
| app_names | A comma sparated list of the applications namnes. | Optional | 
| app_publishers | A comma sparated list of the name of the software publishers of the application. | Optional | 
| user_names | A comma sparated list of the user name of the users logged in to the device. | Optional | 
| os | A comma sparated list of the operating systems that are installed on the device. | Optional | 
| esn | A comma sparated list of the system-defined unique Electronic Serial Numbers (ESN) assigned to the Absolute agent installed on the device. | Optional | 
...
| Absolute.DeviceApplication.AppName | String | TThe name of the application. | 
...
| Absolute.DeviceApplication.InstallDate | Date | The date \(in UNIX Epoch time\) that the application was installed. | 
...
| user_names | A comma sparated list of the usernames of the users that were logged in to the device at the time of the most recent agent call. If no user was logged in during the last agent call, last detected username is used. | Optional | 
| limit | Maximum number of results to return. The default is 50. Default is 50. | Optional | 
| page | The page number of the results to retrieve. Minimum value is 0. Default is 0. | Optional | 
...
| Absolute.Device.SystemModel | String | The manufacturer of the device. | 
| Absolute.Device.SystemType | String | The product name from the manufacturer. | 
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| fields | A comma sparated list of all specific values to return. | Optional | 
| device_ids | A comma sparated list of the system-defined unique identifier of the devices. | Optional | 
| device_names | A comma sparated list of the devices names. | Optional | 
| local_ips | A comma sparated list of the last known local IP addresses of a device. | Optional | 
| public_ips | A comma sparated list of the last known public IP addresses of a device. | Optional | 
...
| Absolute.Device.Bios.smBiosVersion | String | The major version number of the BIOS. | 
...
| Absolute.Device.PolicyGroupName | String | The name of the policy group to that the device belongs to. | 
...
| Absolute.Device.DeviceStatus.reported | Number | The data and time \(in UNIX Epoch\) the device was reported missing or stolen. | 
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| device_ids | A comma sparated list of the system-defined unique identifier of the devices. | Required | 
...
| **Path** | **Type** | **Description** |
| --- | --- | --- |
| Absolute.LocationReport.Coordinates | Unknown | An array containing a comma separated list, the first number is the estimated latitude and the second number is the estimated longitude \(in degrees\) where the device is located. | 
