...
def absolute_client_v3():
    return ClientV3(
        proxy=False, verify=False, base_url="https://api.absolute.com", token_id="token", secret_key="secret", headers={}
    )
...
def test_get_custom_device_field_list_command(mocker, absolute_client_v3):
    ...
    command_result = get_custom_device_field_list_command(
        client=absolute_client_v3, args={"device_id": "02b9daa4-8e60-4640-8b15-76d41ecf6a94"}
    )
    assert command_result.outputs == {
        "DeviceUID": "02b9daa4-8e60-4640-8b15-76d41ecf6a94",
        "CDFValues": [
            {
                "CDFUID": "njazpLrEQwqeFDqk4yQCfg",
                "FieldName": "Asset Number",
                "FieldKey": 1,
                "CategoryCode": "ESNCOLUMN",
                "FieldValue": "No Asset Tag",
                "Type": "Text",
            },
            {
                "CDFUID": "7PwIrjEXTAqvpb5WdV2w",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"passcode_type": "UserDefined"}, "when setting passcode_type to be UserDefined, you must specify the passcode arg."),
        ...
        (
            {
                ...
                "message_name": "name",
                "device_ids": ["1", "2"],
                "scheduled_freeze_date": "2017-09-26T17:22:13Z",
                "device_freeze_type": "Scheduled",
            ...
            {
                "deviceUids": ["1", "2"],
                "freezeDefinition": {"deviceFreezeType": "Scheduled", "scheduledFreezeDateTimeUtc": "2017-09-26T17:22:13Z"},
                "message": "test",
                "messageName": "name",
                "requestTitle": "name",
        ...
        (
            {
                ...
                "message_name": "name",
                "device_ids": ["1", "2"],
                "offline_time_seconds": "1201",
                "device_freeze_type": "OffLine",
                "passcode_type": "RandomForEach",
                "passcode_length": "5",
            },
            {
                "deviceUids": ["1", "2"],
                "freezeDefinition": {"deviceFreezeType": "OffLine", "offlineTimeSeconds": 1201},
                "message": "test",
                "messageName": "name",
                "requestTitle": "name",
...
@pytest.mark.parametrize(
    "field_name, list_of_values, query, expected_query",
    [
        ("id", [], "query", "query"),
        ("accountUid", ["1", "2"], "", "substringof('1',accountUid) or substringof('2',accountUid)"),
        (
            "accountUid",
            ["1", "2"],
            "deviceUID eq '1'",
...
@pytest.mark.parametrize(
    "field_name, value, query, expected_query",
    [
        ("id", "", "query", "query"),
...
@pytest.mark.parametrize(
    "args, change_device_name_to_system, expected_filter",
    [
        ({"filter": "accountUid eq '1'"}, False, "$filter=accountUid eq '1'"),
        ({"filter": "accountUid eq '1'", "account_uids": "1"}, False, "$filter=accountUid eq '1'"),
        ({"account_uids": "1,2"}, False, "$filter=substringof('1',accountUid) or substringof('2',accountUid)"),
        (
            {"account_uids": "1,2", "device_names": "name1, name2"},
            False,
...
@pytest.mark.parametrize(
    "return_fields, query, expected_query",
    [
        ("", "", ""),
