...
INCIDENT_COLUMNS = ["id", "incident_type", "severity", "start_time", "end_time", "ended", "affected_assets"]
ASSET_COLUMNS = ["asset_id", "name", "status", "last_seen", "ip_addresses", "tenant_name"]
""" CLIENT CLASS """
...
class Client(BaseClient):
    ...
    def __init__(self, proxy: bool, verify: bool, base_url: str, username: str, password: str):
    ...
    def login(self):
        ...
        if self._is_access_token_valid(integration_context):
            ...
            demisto.debug(f"{INTEGRATION_NAME} - Generating a new token (old one isn't valid anymore).")
    ...
    def authenticate(self):
        ...
        new_token = self._http_request(method="POST", url_suffix="/authenticate", json_data=body)

        if not new_token or not new_token.get("access_token"):
            raise DemistoException(f"{INTEGRATION_NAME} error: The client credentials are invalid.")

        new_token = new_token.get("access_token")
        return new_token

    def get_assets(self, url_params: dict):
        data = self._http_request(method="GET", url_suffix="/assets", params=url_params, timeout=GLOBAL_TIMEOUT)
        return data

    def get_incident(self, url_params: str):
        data = self._http_request(method="GET", url_suffix=f"/incidents/{url_params}", timeout=GLOBAL_TIMEOUT)
        return data

    def get_incidents(self, url_params: dict):
        data = self._http_request(method="GET", url_suffix="/incidents", params=url_params, timeout=GLOBAL_TIMEOUT)
        return data
...
def test_module(client: Client, is_fetch: bool = False) -> str:
    ...
    try:
        ...
        from_time = int(one_day.replace(tzinfo=utc).timestamp()) * 1000
        now = parse("now")
        assert now is not None
        to_time = int(now.replace(tzinfo=utc).timestamp()) * 1000
...
def get_incidents(client: Client, args: dict[str, Any]):
    ...
    if not from_time or not to_time:
        raise DemistoException(f"{INTEGRATION_NAME} - get incidents needs from_time and to_time.")
    ...
    md = tableToMarkdown(f"{INTEGRATION_NAME} - Incidents: {len(results)}", results, headers=INCIDENT_COLUMNS)
...
def fetch_incidents(client: Client, args: dict[str, Any]) -> tuple[List[dict], int, List[str]]:
    last_run = demisto.getLastRun()
    last_fetch = last_run.get("last_fetch")
    last_ids = last_run.get("last_ids", [])
    first_fetch = args.get("first_fetch", None)
    demisto.debug(f"{INTEGRATION_NAME} - Fetch incidents last fetch: {last_fetch}, first fetch: {first_fetch}")
...
def get_indicent(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    if not incident_id:
        raise DemistoException(f"{INTEGRATION_NAME} - get incident needs an id parameter.")
    ...
    md = tableToMarkdown(f"{INTEGRATION_NAME} - Incident: {incident_id}", hr, headers=INCIDENT_COLUMNS)
...
def get_assets(client: Client, args: dict[str, Any]) -> List[CommandResults]:
    ...
    if not ip_address and not name and not asset_id:
        raise DemistoException(f"{INTEGRATION_NAME} - Endpoint search must have ip, name or asset_id defined.")

    params = {"asset_id": asset_id, "limit": limit, "offset": offset}
    ...
    for res in results.get("objects"):
        ...
        md = tableToMarkdown(f"{INTEGRATION_NAME} - Asset: {hostname}", res, headers=ASSET_COLUMNS)
...
def endpoint_command(client: Client, args: dict[str, Any]) -> List[CommandResults]:
    ...
    for res in results.get("objects"):
        ...
        endpoint = Common.Endpoint(
            hostname=hostname,
            id=res.get("_id"),
            os_version=res.get("guest_agent_details", {}).get("os_details", {}).get("os_display_name"),
        ...
        md = tableToMarkdown(f"{INTEGRATION_NAME} - Endpoint: {hostname}", endpoint_context)

        endpoints.append(
            CommandResults(readable_output=md, outputs_prefix="", raw_response=res, outputs_key_field="_id", indicator=endpoint)
        )
...
def main() -> None:
    ...
    client = Client(username=username, password=password, base_url=base_url, proxy=proxy, verify=(not insecure))
    ...
    try:
        ...
        if command == "test-module":
            ...
            incidents, last_fetch, last_ids = fetch_incidents(
                client,
                {
                    "severity": severity,
                    "incident_type": incident_type,
