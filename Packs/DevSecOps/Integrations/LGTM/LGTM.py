...
class Client(BaseClient):
    ...
    def add_project_request(self, repository, language, mode, commit, date, worker_label):
        params = assign_params(
            repository=repository, language=language, mode=mode, commit=commit, date=date, worker_label=worker_label
        )
    ...
    def request_review_request(self, project_id, base, external_id, callback_url, callback_secret, patch_file):
        params = {"base": base, "external-id": external_id, "callback-url": callback_url, "callback-secret": callback_secret}
        headers = self._headers
        headers["Content-Type"] = "application/octet-stream"
        response = self._http_request("post", f"codereviews/{project_id}", params=params, headers=headers, files=patch_file)

        return response

    def create_query_job_request(self, language, project_id, query):
        parameters = {"project-id": project_id, "language": language}
...
def add_project_command(client, args):
    ...
    command_results = CommandResults(outputs_prefix="LGTM", outputs_key_field="", outputs=response, raw_response=response)
...
def get_alerts_command(client, args):
    ...
    command_results = CommandResults(outputs_prefix="LGTM.alerts", outputs_key_field="", outputs=response, raw_response=response)
...
def get_analysis_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.analysis_summary", outputs_key_field="id", outputs=response, raw_response=response
    )
...
def get_code_review_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.code_review_result", outputs_key_field="id", outputs=response, raw_response=response
    )
...
def get_project_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.project_details", outputs_key_field="id", outputs=response, raw_response=response
    )
...
def get_projects_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.project_list", outputs_key_field="id", outputs=response, raw_response=response
    )
...
def get_version_command(client, args):
    ...
    command_results = CommandResults(outputs_prefix="LGTM.version", outputs_key_field="", outputs=response, raw_response=response)
...
def request_analysis_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.analysis_request", outputs_key_field="id", outputs=response, raw_response=response
    )
...
def request_review_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.code_review_request", outputs_key_field="id", outputs=response, raw_response=response
    )
...
def create_query_job_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.queryjob", outputs_key_field="LGTM.queryjob.task-result.id", outputs=response, raw_response=response
    )
...
def get_query_job_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.queryjob", outputs_key_field="LGTM.queryjob.id", outputs=response, raw_response=response
    )
...
def get_query_job_results_for_project_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.queryjob_project_results", outputs_key_field="", outputs=response, raw_response=response
    )
...
def get_query_job_results_overview_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="LGTM.queryjob_results_overview", outputs_key_field="", outputs=response, raw_response=response
    )
