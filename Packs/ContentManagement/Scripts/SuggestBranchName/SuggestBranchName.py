...
def find_available_branch_azure_devops(pack_name: str):
    ...
    for i in range(1, ATTEMPTS + 1):
        ...
        branch_exists = any(branch.get("name", "").endswith(branch_name) for branch in existing_branches)
...
def main():  # pragma: no cover
    try:
        ...
        return_results(CommandResults(readable_output=branch_name, outputs_prefix="AvailableBranch", outputs=branch_name))
