...
def test_commit_content_item_azure_devops_creating_file(mocker):
    ...
    request = mocker.patch.object(
        demisto,
        "executeCommand",
        return_value=[{"Type": 1, "Contents": {"value": [{"name": "demisto", "objectId": "XXXX", "path": "Test"}]}}],
    )
    commit_content_item_azure_devops(branch_name, content_file, [], [])
    request.assert_called_with(
        "azure-devops-file-create",
        args={
            "commit_comment": "hello.py was added.",
...
def test_commit_content_item_azure_devops_updating_file(mocker):
    ...
    request.assert_called_with(
        ...
        args={
            "commit_comment": "hello.py was updated.",
            "file_path": "Packs/Hi/Integrations/Folder/hello.py",
            "branch_name": "demisto",
            "file_content": "hello world!",
            "branch_id": "XXXX",
...
list_files = [
    {
        ...
        "name": "hello.py",
        "path": "Packs/Hi/Integrations/Folder/hello.py",
...
def test_main(mocker):
    ...
    expected_pr_body = (
        f'### Pull Request created in Cortex XSOAR\n**Created by:** {user.get("username")} '
        f'({user.get("email")})\n\n**Pack:** {pack_name}\n\n**Branch:** {branch_name}\n\n'
    ...
    mocker.patch.object(
        demisto,
        "args",
        return_value={"files": files, "branch": branch_name, "pack": pack_name, "user": user, "git_integration": gitIntegration},
    )
    mock_file = {
        "id": "7@47",
