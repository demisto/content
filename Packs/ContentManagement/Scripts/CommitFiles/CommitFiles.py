...
def get_file_sha(branch_name: str, content_file: ContentFile, get_files_command: str):
    ...
    status, list_files_res = execute_command(
        get_files_command, {"branch": branch_name, "path": content_file.path_to_file}, fail_on_error=False
    )
    if status:
...
def commit_content_item(
    branch_name: str, content_file: ContentFile, new_files: List, modified_files: List, keep_placeholders_in_files: bool = False
):
    commit_args = {
...
def commit_content_item_gitlab(branch_name: str, content_file: ContentFile, new_files: List, modified_files: List):
    commit_args = {
        ...
        "branch": branch_name,
        "file_content": content_file.file_text,
    }
...
def does_file_exist(branch_name: str, content_file: ContentFile) -> bool:
    ...
    if full_path in files_path:
        return True
    # try to get the file from branch
    status, list_commit_res = execute_command(
        "bitbucket-commit-list", {"included_branches": branch_name, "file_path": full_path}, fail_on_error=False
    )
...
def searched_file_path(branch_name: str, content_file: ContentFile) -> bool:
    ...
    response = execute_command(
        "azure-devops-file-list", args={"branch_name": branch_name.split("/")[-1], "recursion_level": "Full"}
    )
...
def commit_content_item_azure_devops(branch_name: str, content_file: ContentFile, new_files: List, modified_files: List):
    file_args = {
        "commit_comment": f"{content_file.file_name} was added.",
        "file_path": f"{content_file.path_to_file}/{content_file.file_name}",
...
def commit_git(
    ...
    new_files: List,
    modified_files: List,
    keep_placeholders_in_files: bool,
):
...
def main():  # pragma: no cover
    try:
        ...
        for file in files:
            ...
            if content_file.content_type in ("script", "integration"):
                ...
                for file_to_commit in content_files:
                    commit_git(
                        git_integration, branch_name, file_to_commit, new_files, modified_files, keep_placeholders_in_files
                    )
        ...
        return_results(CommandResults(readable_output=pr_body, outputs_prefix="PR_text", outputs=pr_body))
