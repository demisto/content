...
def format_alert(alert: dict) -> dict:
    ...
    """
    return {"name": alert.get("name"), "kind": alert.get("kind"), **alert.get("properties", {})}
...
def convert_to_table(context_results: str) -> CommandResults:
    ...
    context_formatted = [format_alert(alert) for alert in context_results]
...
def main():  # pragma: no cover
    context = dict_safe_get(demisto.callingContext, ["context", "Incidents", 0, "CustomFields", "microsoftsentinelalerts"], {})
