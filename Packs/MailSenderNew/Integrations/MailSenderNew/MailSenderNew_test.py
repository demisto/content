...
def test_debug_smtp(mocker):
    ...
    """
    mocker.patch.object(demisto, "params", return_value={"from": "test@test.com", "host": "localhost", "port": "2025"})
...
@pytest.mark.parametrize(
    "template_params_arg",
    ['{"name": {"value": "hello3"}}', ({"name": {"value": "hello3"}}), ({"name": {"key": "key_from_context"}})],
)
...
def test_attachments(mocker):
    ...
    mocker.patch.object(demisto, "getFilePath", return_value={"path": "test_data/attachment.txt", "name": "attachment.txt"})
