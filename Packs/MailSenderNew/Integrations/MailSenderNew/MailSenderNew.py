    ...
    for attachment in manual_attach_obj:
        ...
        if maintype == "text":
            ...
            with open(path, "rb") as fp:
                data = fp.read()
        attachments.append({"name": attachment["FileName"], "maintype": maintype, "subtype": subtype, "data": data, "cid": ""})
...
def collect_attachments():
    ...
    for name, data, cid in zip_longest(f_names, f_contents, f_cids):
        ...
        attachments.append({"name": name, "maintype": maintype, "subtype": subtype, "data": data, "cid": cid})
...
def get_user_pass():
    ...
    if credentials:
        return (str(credentials.get("identifier", "")), str(credentials.get("password", "")))
    return None, None
...
def main():
    ...
    try:
        ...
        if tls == "SSL/TLS":
            ...
            SERVER = SMTP(
                demisto.getParam("host"),  # type: ignore[assignment]
