...
JSON_RESP = {"message": "test message"}
...
def test_is_html_response(mocker):
    mocker.patch.object(
        demisto,
        "params",
        return_value={"server": "mock_server", "insecure": False, "proxy": "", "credentials": {"identifier": "", "password": ""}},
    )
...
def test_parse_error_response(mocker):
    ...
    assert (
        parse_error_response(ResponseMock(JSON_RESP))
