...
MESSAGES: dict[str, str] = {
    ...
    "TEST_CONFIGURE_ERROR": (
        "In order for the test_module to run, an admin_email is required, "
    ...
    "INVALID_PAGINATION_ARGS_SUPPLIED": (
        "In order to use pagination, please supply either the argument limit,"
...
COMMAND_SCOPES: dict[str, list[str]] = {
    ...
    "ROLE_ASSIGNMENT": ["https://www.googleapis.com/auth/admin.directory.rolemanagement.readonly", *SCOPES["ROLE_MANAGEMENT"]],
...
MobileDevicesConfig = DevicesCommandConfig(
...
ChromeOSDevicesConfig = DevicesCommandConfig(
...
class Client(GSuiteClient):
    ...
    def __init__(
        self,
        service_account_dict: dict[str, Any],
        proxy: bool,
        verify: bool,
        ...
        super().__init__(
            service_account_dict=service_account_dict, base_url=base_url, verify=verify, proxy=proxy, headers=headers
        )
...
def prepare_output_user_alias_add(alias: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return GSuiteClient.remove_empty_entities(
        {"kind": alias.get("kind", ""), "id": alias.get("id", ""), "etag": alias.get("etag", ""), "alias": alias.get("alias", "")}
    )
...
def prepare_args_for_user(args: dict[str, str]) -> dict[str, Any]:
    ...
    return GSuiteClient.remove_empty_entities(
        {
            "name": {"familyName": args.get("last_name"), "givenName": args.get("first_name")},
            ...
            "emails": [{"address": args.get("secondary_email_address"), "type": args.get("secondary_email_type")}],
            "gender": {"type": args.get("gender")},
...
def prepare_readable_output_for_user_command(outputs):
    ...
    readable_outputs = {
        ...
        "phoneDetails": prepare_markdown_from_dictionary(outputs.pop("phones", [])[0] if outputs.get("phones", []) else {}),
    }
    ...
    return GSuiteClient.remove_empty_entities(
        {
            ...
            "maxResults": GSuiteClient.validate_get_int(args.get("max_results"), MESSAGES["INTEGER_ERROR"].format("max_results")),
        }
    )
    ...
    return GSuiteClient.remove_empty_entities(
        {
            ...
            OUTPUT_PREFIX["ROLE_ASSIGNMENT_LIST_PAGE_TOKEN"]: {"nextPageToken": response["nextPageToken"]}
...
def prepare_readable_output_for_datatransfer_list(response: dict[str, Any]) -> str:
    ...
    for datatransfer in response.get("dataTransfers", []):
        ...
        for app_transfer in datatransfer.get("applicationDataTransfers", []):
            app_transfer_markdown += prepare_markdown_from_dictionary(app_transfer, ["applicationTransferParams"]) + "\n\n"
        datatransfer["applicationDataTransfers"] = app_transfer_markdown

    readable_output += tableToMarkdown(
        HR_MESSAGES["LIST_COMMAND_SUCCESS"].format("Data Transfers", len(response.get("dataTransfers", []))),
        response.get("dataTransfers", []),
        ["id", "oldOwnerUserId", "newOwnerUserId", "overallTransferStatusCode", "requestTime", "applicationDataTransfers"],
        ...
        try:
            ...
            for transfer_param in transfer_params_str.split(";"):
                transfer_params.append(
                    {"key": transfer_param.split(":")[0].strip(), "value": transfer_param.split(":")[1].strip().split(",")}
                )
        except Exception:
    ...
    return GSuiteClient.remove_empty_entities(
        {
            ...
            "applicationDataTransfers": [
                {"applicationId": args.get("application_id"), "applicationTransferParams": transfer_params}
            ],
        }
    )
...
def mobile_update_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    try:
        client.http_request(
            url_suffix=URL_SUFFIX["MOBILE_UPDATE"].format(urllib.parse.quote(customer_id), resource_id), method="POST", body=args
        )
...
def mobile_delete_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    resource_id = urllib.parse.quote(args.pop("resource_id", ""))
    client.http_request(
        url_suffix=URL_SUFFIX["MOBILE_DELETE"].format(urllib.parse.quote(customer_id), resource_id), method="DELETE"
    )
...
def user_create_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        HR_MESSAGES["USER_CREATE"],
        readable_output_dict,
        [
            "id",
            "customerId",
            "primaryEmail",
    ...
    return CommandResults(
        outputs_prefix=OUTPUT_PREFIX["CREATE_USER"],
...
def role_assignment_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    response = client.http_request(
        url_suffix=URL_SUFFIX["ROLE_ASSIGNMENT"].format(urllib.parse.quote(customer_id)), params=arguments
    )
    ...
    readable_output += (
        ...
        else HR_MESSAGES["NO_RECORDS"].format("role assignment details")
    )
...
def role_assignment_create_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    response = client.http_request(
        url_suffix=URL_SUFFIX["ROLE_ASSIGNMENT"].format(urllib.parse.quote(customer_id)), body=arguments, method="POST"
    )
...
def group_create_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        HR_MESSAGES["GROUP_CREATE_SUCCESS"].format(response["name"]),
        response,
        headerTransform=pascalToSpace,
        removeNull=True,
...
def group_get_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    response = client.http_request(url_suffix=group_key_suffix, method="GET")
    ...
    readable_output = tableToMarkdown(
        HR_MESSAGES["GROUP_GET_SUCCESS"].format(response["name"]),
        response,
        headerTransform=pascalToSpace,
        removeNull=True,
        headers=hr_output_fields,
...
def role_create_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    response = client.http_request(
        url_suffix=URL_SUFFIX["ROLE_CREATE"].format(urllib.parse.quote(customer_id)), body=params, method="POST"
    )
    ...
    readable_output = tableToMarkdown(
        HR_MESSAGES["ROLE_CREATE_SUCCESS"], hr_output, headerTransform=pascalToSpace, removeNull=True, headers=hr_output_fields
    )
...
def datatransfer_list_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    # Readable Output
    readable_output = (
        prepare_readable_output_for_datatransfer_list(copy.deepcopy(response))
        if response.get("dataTransfers")
        else HR_MESSAGES["NO_RECORDS"].format("data transfer details")
    )
...
def custom_user_schema_create_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    customer_id = return_customer_id(args)
    response = client.http_request(
        method="POST", url_suffix=URL_SUFFIX["CUSTOM_USER_SCHEMA"].format(urllib.parse.quote(customer_id)), body=body
    )
    ...
    readable_output += tableToMarkdown(
        ...
        ["fieldId", "fieldName", "displayName", "fieldType", "readAccessType", "multiValued", "indexed", "numericIndexingSpec"],
        headerTransform=pascalToSpace,
        removeNull=True,
    )
...
def custom_user_schema_update_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    readable_output += f"Schema Id: {readable_output_dict['schemaId']}\n" if readable_output_dict.get("schemaId", "") else ""
    readable_output += (
        f"Schema Name: {readable_output_dict['schemaName']}\n" if readable_output_dict.get("schemaName", "") else ""
    )
    readable_output += (
        f"Schema Display Name: {readable_output_dict['displayName']}\n" if readable_output_dict.get("displayName", "") else ""
    )
    readable_output += tableToMarkdown(
        HR_MESSAGES["CUSTOM_USER_SCHEMA_FIELD_DETAILS"],
        readable_output_dict.get("fields", []),
...
def datatransfer_request_create_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    :return: CommandResults.
    """
    if not any([args.get("applications_raw_json"), args.get("applications_raw_json_entry_id"), args.get("application_id")]):
    ...
    application_transfer_hr_output = "\n\n".join(
        [
            "Application Id: {}\nApplication Transfer Status: {}".format(
                transfer.get("applicationId", ""), transfer.get("applicationTransferStatus", "")
            )
            for transfer in response.get("applicationDataTransfers", [])
        ]
    )
...
def user_get_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    readable_output_dict = prepare_readable_output_for_user_command(copy.deepcopy(outputs))
    readable_output = tableToMarkdown(
        HR_MESSAGES["USER_GET"].format(user_key),
        readable_output_dict,
        [
            "id",
            "customerId",
...
def mobile_device_list_request(client: Client, customer_id: str, query_params: dict = {}):
    response = client.http_request(
        url_suffix=URL_SUFFIX.get("MOBILE_DEVICES_LIST", "").format(urllib.parse.quote(customer_id)), params=query_params
    )
    return response
...
def chromeos_device_list_request(client: Client, customer_id: str, query_params: dict = {}):
    response = client.http_request(
        url_suffix=URL_SUFFIX.get("CHROMEOS_DEVICES_LIST", "").format(urllib.parse.quote(customer_id)), params=query_params
    )
    return response


def chromeos_device_action_request(client: Client, customer_id: str, resource_id: str, action: str, deprovision_reason: str = ""):
    json_body = {"action": action}
    if action == "deprovision":
        json_body["deprovisionReason"] = deprovision_reason
    response = client.http_request(
        method="POST",
        url_suffix=URL_SUFFIX.get("CHROMEOS_DEVICE_ACTION", "").format(
            urllib.parse.quote(customer_id), urllib.parse.quote(resource_id)
        ),
        body=json_body,
    )
    return response
...
def device_list_automatic_pagination(
    request_by_device_type: Callable, client, customer_id: str, query_params: dict, limit: int, response_devices_list_key: str
) -> dict:
...
def device_list_manual_pagination(
    request_by_device_type: Callable,
    client,
    customer_id: str,
    query_params: dict,
    page_token: str,
...
def mobile_device_list_create_query_parameters(projection: str, query: str, order_by: str, sort_order: str) -> dict:
...
def gsuite_mobile_device_list_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    customer_id = return_customer_id(args)
    query_params = mobile_device_list_create_query_parameters(
        projection=args.get("projection", "full"),
        query=args.get("query", ""),
        order_by=args.get("order_by", "status"),
        sort_order=args.get("sort_order", "ascending"),
    )
    pagination_args = prepare_pagination_arguments(
    ...
    if not context_data:
        ...
        markdown = tableToMarkdown(
            MobileDevicesConfig.table_title,
            human_readable,
...
def chromeos_device_list_create_query_parameters(
    projection: str, query: str, include_child_org_units: bool, order_by: str, sort_order: str, org_unit_path: str
...
def gsuite_chromeos_device_list_command(client: Client, args: dict[str, str]) -> CommandResults:  # pragma: no cover
    ...
    customer_id = return_customer_id(args)
    query_params = chromeos_device_list_create_query_parameters(
        projection=args.get("projection", "full"),
        query=args.get("query", ""),
    ...
    try:
        ...
        if not context_data:
            ...
            num_of_devices = len(context_data)
            markdown = tableToMarkdown(
                ChromeOSDevicesConfig.table_title,
                human_readable,
                metadata=f'{num_of_devices} {"results" if num_of_devices != 1 else "result"} found',
            )
        outputs: dict[str, Any] = {}
        if context_data:
            outputs[
                (
                    f"{ChromeOSDevicesConfig.outputs_prefix}."
                    "ChromeOSListObjects(val.resourceId && val.resourceId == obj.resourceId)"
                )
            ] = context_data
...
def gsuite_chromeos_device_action_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    try:
        ...
        customer_id = return_customer_id(args)
        chromeos_device_action_request(
            client=client,
            customer_id=customer_id,
...
def user_reset_password_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    # Output
    readable_output = tableToMarkdown(
        HR_MESSAGES["USER_UPDATE"].format(user_key),
        outputs,
...
def chromebrowser_move_ou_command(client: Client, args: dict[str, str]) -> str:
    ...
    org_unit_path = args.get("org_unit_path", "")
    full_url = (
        f"https://www.googleapis.com/admin/directory/v1.1beta1/customer/{customer_id}"
...
def chromebrowser_list_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    if device_id:
        ...
        params_for_command = GSuiteClient.remove_empty_entities({"projection": projection})
        ...
        if page_size or page_token:
            ...
            params_for_command = assign_params_chromebrowser_list(
                projection, query, order_by, sort_order, org_unit_path, page_token, page_size
            )
            response = client.http_request(full_url=full_url, method="GET", params=params_for_command)
    ...
    readable_output = tableToMarkdown(
        HR_MESSAGES["CHROME_BROWSER_LIST"].format(device_id),
        cb_list_resp,
    ...
    outputs = {
        "GSuite.ChromeBrowserDevices(val.deviceId && val.deviceId == obj.deviceId)": cb_list_resp,
...
def modify_policy_command(client: Client, args: dict[str, str]) -> str:
    ...
    if policy_raw_json:
        ...
        app_payload = {
            "requests": [
                {
                    "policyTargetKey": {"targetResource": target_resource_customized, "additionalTargetKeys": atk_dict},
                    "policyValue": {"policySchema": policy_schema, "value": {"appInstallType": policy_value}},
...
def policy_resolve_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    for res in policy_resolved_resp:
        customized_resp = {
            "targetResource": res["targetKey"]["targetResource"],  # type: ignore
...
def assign_params_policy_schemas(filter, page_size, page_token):
    return GSuiteClient.remove_empty_entities({"filter": filter, "pageSize": page_size, "pageToken": page_token})
...
def policy_schemas_list_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    outputs = {
        "GSuite.Policy(val.name && val.name == obj.name)": policy_schemas_resp,
...
def main() -> None:
    ...
    try:
        ...
        admin_email = (
            args.get("admin_email") or params.get("admin_email_creds", {}).get("identifier") or params.get("admin_email")
        )
