...
class FireEyeClient(BaseClient):
    def __init__(self, base_url: str, username: str, password: str, verify: bool, proxy: bool, ok_codes: tuple = OK_CODES):
    ...
    def http_request(
        ...
        try:
            ...
            res = self._session.request(
                method, address, headers=self._headers, verify=self._verify, params=params, json=json_data, timeout=timeout
            )
    ...
    def _generate_token(self) -> str:
        try:
            ...
            raise DemistoException(f"Token request failed. message: {er!s}")
        if "X-FeApi-Token" not in resp.headers:
            raise DemistoException(f"Token request failed. API token is missing. message: {resp!s}")
    ...
    def get_alert_details_request(self, alert_id: str, timeout: int) -> Dict[str, str]:
        return self.http_request(method="GET", url_suffix=f"alerts/alert/{alert_id}", resp_type="json", timeout=timeout)
    ...
    def alert_acknowledge_request(self, uuid: str) -> Dict[str, str]:
        ...
        return self.http_request(
            method="POST",
            url_suffix=f"alerts/alert/{uuid}",
            params={"schema_compatibility": True},
            json_data={"annotation": "<test>"},
            resp_type="resp",
        )
    ...
    def get_artifacts_by_uuid_request(self, uuid: str, timeout: int) -> Dict[str, str]:
        ...
        return self.http_request(method="GET", url_suffix=f"artifacts/{uuid}", resp_type="content", timeout=timeout)
    ...
    @logger
    def get_quarantined_emails_request(
        self, start_time: str, end_time: str, from_: str, subject: str, appliance_id: str, limit: int
    ) -> Dict[str, str]:
    ...
    def get_reports_request(
        ...
        params = {"report_type": report_type, "start_time": start_time, "end_time": end_time}
        if limit:
            params["limit"] = limit
        if interface:
        ...
        return self.http_request(method="GET", url_suffix="reports/report", params=params, resp_type="content", timeout=timeout)

    @logger
    def list_allowedlist_request(self, type_: str) -> Dict[str, str]:
        return self.http_request(method="GET", url_suffix=f"devicemgmt/emlconfig/policy/allowed_lists/{type_}", resp_type="json")
    ...
    def list_blockedlist_request(self, type_: str) -> Dict[str, str]:
        return self.http_request(method="GET", url_suffix=f"devicemgmt/emlconfig/policy/blocked_lists/{type_}", resp_type="json")
