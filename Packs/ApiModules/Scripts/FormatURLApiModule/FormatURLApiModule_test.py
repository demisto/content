...
ATP_REDIRECTS = [
    ...
    (
        ...
        "&email_address=paulameixner@curo.com",
    ),  # disable-secrets-detection
    ("hxxps://nam10.safelinks.protection.outlook.com/ap/w-59523e83/?url=hxxps://test.com/test&data=", "https://test.com/test"),
    ("hxxps://nam10.safelinks.protection.office365.us/ap/w-59523e83/?url=hxxps://test.com/test&data=", "https://test.com/test"),
]
...
FORMAT_QUERY = [
    (
        ...
        "www.test.test.com/test.html?paramaters=testagain",
    ),  # disable-secrets-detection
    (
    ...
    (
        ...
        "https://test.test.com/v2/test?test&test=[test]test",
    ),  # disable-secrets-detection
    (
...
FORMAT_NON_ASCII = [
    ...
    (
        ...
        "https://www.test.se/Auth/?&rUrl=https://test.com/wp-images/amclimore@test.com",
    ),  # disable-secrets-detection
...
FAILS = [
    ...
    ),  # IPv6 must have square brackets
    (
    ...
    (
        ...
        pytest.raises(URLError),
    ),  # Invalid hex code in path
    (
        "https://www.%gg.com/",  # disable-secrets-detection
        pytest.raises(URLError),
    ),  # Non valid hexadecimal value in host
    (
        "",  # disable-secrets-detection
        pytest.raises(URLError),
    ),  # Empty string
    ...
    (
        ...
        pytest.raises(URLError),
    ),  # No host data, only scheme and user info
    (
    ...
    (
        ...
        pytest.raises(URLError),
    ),  # Square brackets closing without opening
    (
        "https://[192.168.1.1]",  # disable-secrets-detection
        pytest.raises(URLError),
    ),  # Only IPv6 allowed in square brackets
    (
        "https://[www.test.com]",  # disable-secrets-detection
        pytest.raises(URLError),
    ),  # Only IPv6 allowed in square brackets
    (
        ...
        pytest.raises(URLError),
    ),  # invalid domain in host section (no tld)
    (
        "https://www.t/",  # disable-secrets-detection
        pytest.raises(URLError),
    ),  # invalid domain in host section (single letter tld)
    (
...
class TestFormatURL:
    ...
    # XSUP-76731: a mailto: link whose query contains percent-encoded CRLF (%0D%0A) must not be
    # decoded into literal control characters, which would split one indicator into two lines.
    CRLF_INJECTION = [
        (
            "mailto:maria.garcia@visitorslistdata.com?subject=Unsubscribe%205B7327A6%2D638C%2D4591%2D"
            "B781%2DBA05E77E32EA&body=Please%20don%27t%20change%20the%20email%20content%20and%20send"
            "%20this%20email%20to%20unsubscribe.%0D%0A%0D%0Avb.info@mahle.com"
        ),
        # The "/" after the host is the formatter's pre-existing normalization, unrelated to this fix.
        "mailto:maria.garcia@visitorslistdata.com/?subject=Unsubscribe%205B7327A6-638C-4591-B781-BA05E77E32EA"
        "&body=Please%20don't%20change%20the%20email%20content%20and%20send%20this%20email%20to%20unsubscribe."
        "%0D%0A%0D%0Avb.info@mahle.com",
    ]
