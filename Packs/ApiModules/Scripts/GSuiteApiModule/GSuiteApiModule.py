...
class GSuiteClient:
    ...
    def _maybe_invalidate_ucp_credentials(self, response: tuple) -> None:
        ...
        if status in (401, 403):
            invalidate_ucp_credentials(self._ucp_method_id)
    ...
    @staticmethod
    def validate_set_boolean_arg(args: dict[str, Any], arg: str, arg_name: str | None = None) -> None:
        """
    ...
    def remove_empty_entities(d):
        ...
        if not isinstance(d, dict | list):
            ...
            return {
                key: value
                for key, value in ((key, GSuiteClient.remove_empty_entities(value)) for key, value in d.items())
                if not empty(value)
            }

    @staticmethod
    def validate_get_int(max_results: str | None, message: str, limit: int = 0) -> int | None:
        """
