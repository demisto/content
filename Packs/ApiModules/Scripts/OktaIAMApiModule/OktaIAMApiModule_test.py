...
def test_fetch_incidents__two_logs_batches(mocker):
    ...
    mocker.patch("OktaIAMApiModule.get_all_user_profiles", return_value={})
...
def test_should_drop_event(log_entry, email_to_user_profile, expected):
    from OktaIAMApiModule import should_drop_event
...
def test_get_logs_command(mocker, requests_mock, limit, logs_amount):
    ...
    Then:
        - Ensure three events are returned in incident the correct format.
    """
    from OktaIAMApiModule import get_logs_command
...
def test_main_no_legacy_auth_header_under_ucp(mocker):
    ...
    """
    import OktaIAMApiModule

    mocker.patch.object(OktaIAMApiModule, "should_use_ucp_auth", return_value=True)
    mocker.patch.object(demisto, "params", return_value={"url": "https://test.com", "credentials": {}})
    mocker.patch.object(demisto, "command", return_value="test-module")
    mocker.patch.object(demisto, "getIntegrationContext", return_value={})
    test_module_mock = mocker.patch.object(OktaIAMApiModule, "test_module")
    client_init_spy = mocker.spy(OktaIAMApiModule, "Client")
...
def test_main_legacy_auth_header_when_ucp_off(mocker):
    ...
    mocker.patch.object(OktaIAMApiModule, "should_use_ucp_auth", return_value=False)
    mocker.patch.object(demisto, "params", return_value={"url": "https://test.com", "credentials": {"password": "my-token"}})
    mocker.patch.object(demisto, "command", return_value="test-module")
    mocker.patch.object(demisto, "getIntegrationContext", return_value={})
    mocker.patch.object(OktaIAMApiModule, "test_module")
    client_init_spy = mocker.spy(OktaIAMApiModule, "Client")
