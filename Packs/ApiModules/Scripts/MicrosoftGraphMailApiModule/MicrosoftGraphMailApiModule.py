...
class MsGraphMailBaseClient(MicrosoftClient):
    ...
    def _get_email_attachments(self, message_id, user_id=None, overwrite_rate_limit_retry=False) -> list:
        ...
        :rtype: ``list``
        """
        demisto.debug(f"[test]MicrosoftGraphMailApiModule._get_email_attachments: {message_id=}")
        ...
        demisto.debug(f"[test]MicrosoftGraphMailApiModule._get_email_attachments: retrieved {len(attachments)} attachments. handling")
        for attachment in attachments:
            ...
            demisto.debug(f"[test]MicrosoftGraphMailApiModule._get_email_attachments: handling attachment {attachment_name=}")
            ...
            if not attachment_name.isascii():
                try:
                    demisto.debug(f"[test]MicrosoftGraphMailApiModule._get_email_attachments: Trying to decode the attachment file name: {attachment_name}")
                    attachment_name = b64_decode(attachment_name)  # type: ignore
                except Exception as e:
                    demisto.debug(f"[test]MicrosoftGraphMailApiModule._get_email_attachments: Could not decode the {attachment_name=}: error: {e}")

            if attachment_type == self.FILE_ATTACHMENT:
                try:
                    demisto.debug(f"[test]MicrosoftGraphMailApiModule._get_email_attachments: Decoding base64 file attachment")
                    attachment_content = b64_decode(attachment.get("contentBytes", ""))
                except Exception as e:  # skip the uploading file step
                    demisto.info(f"[test]MicrosoftGraphMailApiModule._get_email_attachments: failed in decoding base64 file attachment with error {e!s}")
                    continue
            elif attachment_type == self.ITEM_ATTACHMENT:
                demisto.debug(f"[test]MicrosoftGraphMailApiModule._get_email_attachments: Handling item attachment")
            ...
            demisto.debug(f"[test]MicrosoftGraphMailApiModule._get_email_attachments: Uploading attachment file: {attachment_name=}, {attachment_content=}")
            demisto.debug("[test]MicrosoftGraphMailApiModule._get_email_attachments uploading file")
    ...
    def update_email_read_status(self, user_id: str, message_id: str, read: bool, folder_id: str | None = None) -> dict:
        ...
        Returns:
            dict: API response
        """
        demisto.debug(f"[test]MicrosoftGraphMailApiModule.update_email_read_status: {read=}, {user_id=}, {message_id=}, {folder_id=}")
        user_id = user_id or self._mailbox_to_fetch
        folder_path = f"/{GraphMailUtils.build_folders_path(folder_id)}" if folder_id else ""
    ...
    def _fetch_last_emails(self, folder_id, last_fetch, exclude_ids):
        ...
        :rtype: ``list`` and ``list``
        """
        demisto.debug(f"[test]MicrosoftGraphMailApiModule._fetch_last_emails: Fetching emails since {last_fetch}")
        ...
        demisto.debug(f"[test]MicrosoftGraphMailApiModule._fetch_last_emails: Fetched {len(fetched_emails)} emails since {last_fetch}")

        fetched_emails_ids = {email.get("id") for email in fetched_emails}
        demisto.debug(f"[test]MicrosoftGraphMailApiModule._fetch_last_emails: Fetched emails ids: {fetched_emails_ids=}")
        exclude_ids_set = set(exclude_ids)
        if not fetched_emails or not (filtered_new_email_ids := fetched_emails_ids - exclude_ids_set):
            # no new emails
            demisto.debug(f"[test]MicrosoftGraphMailApiModule._fetch_last_emails: No new emails: {fetched_emails_ids=}. {exclude_ids_set=}")
            return [], exclude_ids

        new_emails = [mail for mail in fetched_emails if mail.get("id") in filtered_new_email_ids][: self._emails_fetch_limit]
        demisto.debug(f"[test]MicrosoftGraphMailApiModule._fetch_last_emails: {new_emails=}")
        last_email_time = new_emails[-1].get("receivedDateTime")
        demisto.debug(f"[test]MicrosoftGraphMailApiModule._fetch_last_emails: {last_email_time=}")
    ...
    def get_emails_as_text_and_html(emails_as_html, emails_as_text):
        ...
        demisto.debug(f"[test]MicrosoftGraphMailApiModule.get_emails_as_text_and_html: {len(emails_as_html)} emails as html, {len(emails_as_text)} emails as text")
        emails_as_html_and_text = []

        demisto.debug(f"[test]MicrosoftGraphMailApiModule.get_emails_as_text_and_html iterating over {len(emails_as_html)} emails as html")
        for email_as_html in emails_as_html:
            html_email_id = email_as_html.get("id")
            demisto.debug(f"[test]MicrosoftGraphMailApiModule.get_emails_as_text_and_html handling email {html_email_id=}")
            ...
            if not text_email_data:
                ...
                demisto.info(f"[test]MicrosoftGraphMailApiModule.get_emails_as_text_and_html There is no matching text email to html email-ID {html_email_id}")
    ...
    def get_email_content_as_text_and_html(email):
        ...
        demisto.debug(f"[test]MicrosoftGraphMailApiModule.get_email_content_as_text_and_html: {email_body=}")
        demisto.debug(f"[test]MicrosoftGraphMailApiModule.get_email_content_as_text_and_html: {email_unique_body=}")
        ...
        try:
            ...
            demisto.info(f"[test]MicrosoftGraphMailApiModule.get_email_content_as_text_and_html: email body content is missing from email {email}")
            return "", ""
    ...
    def _parse_email_as_incident(self, email, overwrite_rate_limit_retry=False):
        ...
        def body_extractor(email, parsed_email):
            demisto.debug(f"[test]MicrosoftGraphMailApiModule._parse_email_as_incident.body_extractor: {email=}")
            ...
            demisto.debug(f"[test]MicrosoftGraphMailApiModule._parse_email_as_incident.body_extractor: {parsed_email=}")

        demisto.debug(f"[test]MicrosoftGraphMailApiModule._parse_email_as_incident: {email=}")
        parsed_email = GraphMailUtils.parse_item_as_dict(email, body_extractor)
        demisto.debug(f"[test]MicrosoftGraphMailApiModule._parse_email_as_incident: {parsed_email=}")


        demisto.debug(f"[test]MicrosoftGraphMailApiModule._parse_email_as_incident: handling attachments of fetched email")
        ...
        if attachments:
            demisto.debug("[test]MicrosoftGraphMailApiModule._parse_email_as_incident: adding Attachments to parsed_email")
        ...
        if not body or self._display_full_email_body:
            demisto.debug("[test]MicrosoftGraphMailApiModule._parse_email_as_incident: fetching full email body")
        ...
        demisto.debug(f"[test]MicrosoftGraphMailApiModule._parse_email_as_incident: incident created: {incident=}")
...
class GraphMailUtils:
    ...
    def parse_item_as_dict(email, body_extractor=None):
        ...
        :rtype: ``dict``
        """
        demisto.debug(f"[test]MicrosoftGraphMailApiModule.parse_item_as_dict")
        ...
        if body_extractor:
            demisto.debug(f"[test]MicrosoftGraphMailApiModule.parse_item_as_dict: executing body extractor")
        ...
        demisto.debug(f"[test]MicrosoftGraphMailApiModule.parse_item_as_dict completed with {parsed_email=}")
        return parsed_email
