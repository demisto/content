...
class MsGraphClient(MicrosoftClient):
    """
    ...
    # def __init__(self, self_deployed, tenant_id, auth_id, enc_key, app_name, base_url, use_ssl, proxy,
    #              ok_codes, mailbox_to_fetch, folder_to_fetch, first_fetch_interval, emails_fetch_limit,
    #              auth_code: Optional[str] = None,
    #              redirect_uri: Optional[str] = None,
    #              refresh_token: Optional[str] = None,
    #              timeout=10, endpoint='com', certificate_thumbprint=None, private_key=None,
    #              look_back=0,
    ...
    #     self.ms_client = MicrosoftClient(self_deployed=self_deployed, tenant_id=tenant_id, auth_id=auth_id,
    ...
    def _get_root_folder_children(self, user_id, overwrite_rate_limit_retry=False):
        ...
        root_folder_children = self.http_request('GET',
                                                 suffix_endpoint,
                                                 overwrite_rate_limit_retry=overwrite_rate_limit_retry).get('value', None)
    ...
    def get_emails_from_api(self, limit: int, received_date_time_filter: str, folder_id: str,
                            overwrite_rate_limit_retry: bool = False, body_as_text: bool = False) -> list:
        ...
        response = self.http_request(
            ...
            params={
                '$filter': f'receivedDateTime ge {received_date_time_filter}',
    ...
    def list_mails(self, user_id: str, folder_id: str = '', search: str = None, odata: str = None) -> dict | list:
        ...
        folder_path = f'/{build_folders_path(folder_id)}' if folder_id else ''
...
def list_mails_command(client: MsGraphClient, args):
    ...
    user_id = args.get('user_id', self._mailbox_to_fetch)

    raw_response = client.list_mails(user_id=user_id, folder_id=folder_id, search=search, odata=odata)
    last_page_response = raw_response[-1]
    metadata = ''
    ...
    if mail_context := build_mail_object(raw_response):
    ...
    result_context_entry = CommandResults(
    ...
    if next_page:
        result_context_entry['EntryContext'].update({'MSGraphMail(val.NextPage.indexOf(\'http\')>=0)':{'NextPage': next_page}})
    return result_context_entry
