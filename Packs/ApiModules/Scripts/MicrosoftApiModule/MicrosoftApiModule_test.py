...
def test_raise_authentication_error(mocker):
    ...
    err.status_code = 401
    error_content_str = (
        "Error: failed to get access token with err: "
    ...
    expected_msg = (
        "Error in Microsoft authorization. Status: 401, body: invalid_grant. \nDevice object was not found in "
...
def test_epoch_seconds(mocker):
    ...
    mocker.patch.object(MicrosoftClient, "_get_utcnow", return_value=date_time(2019, 12, 24, 14, 12, 0, 586636))
    mocker.patch.object(MicrosoftClient, "_get_utc_from_timestamp", return_value=date_time(1970, 1, 1, 0, 0))
    integer = MicrosoftClient.epoch_seconds()
    assert integer == 1577196720


@pytest.mark.parametrize(
    "client, tokens, context",
    [
        (
            oproxy_client_refresh(),
            (TOKEN, 3600, REFRESH_TOKEN),
            {"access_token": TOKEN, "valid_until": 3605, "current_refresh_token": REFRESH_TOKEN},
        ),
        (oproxy_client_tenant(), (TOKEN, 3600, ""), {"access_token": TOKEN, "valid_until": 3605, "current_refresh_token": ""}),
        (
...
@pytest.mark.parametrize(
    "client, tokens, context_invalid, context_valid",
    [
        (
            oproxy_client_refresh(),
            (TOKEN, 3600, REFRESH_TOKEN),
            {"access_token": TOKEN, "valid_until": 3605, "current_refresh_token": REFRESH_TOKEN},
            {"access_token": TOKEN, "valid_until": 8595, "current_refresh_token": REFRESH_TOKEN},
        ),
        (
            oproxy_client_tenant(),
            (TOKEN, 3600, ""),
            {"access_token": TOKEN, "valid_until": 3605, "current_refresh_token": REFRESH_TOKEN},
            {"access_token": TOKEN, "valid_until": 8595, "current_refresh_token": ""},
        ),
        (
            self_deployed_client(),
            (TOKEN, 3600, ""),
            {"access_token": TOKEN, "valid_until": 3605, "current_refresh_token": ""},
            {"access_token": TOKEN, "valid_until": 8595, "current_refresh_token": ""},
        ),
    ],
)
...
def test_retry_on_rate_limit(requests_mock, mocker):
    ...
    requests_mock.get("https://graph.microsoft.com/v1.0/test_id", status_code=429, json={"content": "Rate limit reached!"})
...
def test_general_error_metrics(requests_mock, mocker):
    ...
    requests_mock.get("https://graph.microsoft.com/v1.0/test_id", status_code=500, json={"content": "General Error!"})
...
def test_reset_auth(mocker):
    ...
    expected_output = (
        "Authorization was reset successfully. Please regenerate the credentials, "
...
def test_generate_login_url():
    ...
    expected_url = (
        f"[login URL](https://login.microsoftonline.com/{TENANT}/oauth2/v2.0/authorize?"
...
@pytest.mark.parametrize(
    "params, expected_resource_manager, expected_active_directory, expected_microsoft_graph_resource_id",
    [
        (
            {"azure_cloud": "Germany"},
        ...
        (
            {"azure_cloud": "Custom", "server_url": "mock_url"},
            "mock_url",
            "https://login.microsoftonline.com",
            "https://graph.microsoft.com/",
        ),
        (
            {"azure_ad_endpoint": "mock_endpoint"},
...
def test_get_azure_cloud(params, expected_resource_manager, expected_active_directory, expected_microsoft_graph_resource_id):
    ...
    assert (
        get_azure_cloud(params=params, integration_name="test").endpoints.microsoft_graph_resource_id
        == expected_microsoft_graph_resource_id
    )
...
@freezegun.freeze_time(FREEZE_STR_DATE)
@pytest.mark.parametrize(
    "mocked_delay_request_counter,excepted",
    [
