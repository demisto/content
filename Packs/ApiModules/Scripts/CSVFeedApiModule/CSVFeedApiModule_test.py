...
def test_get_indicators_1():
    ...
    with requests_mock.Mocker() as m:
        itype = "IP"
        args = {"indicator_type": itype, "limit": 35}
...
def test_get_indicators_with_mapping():
    """Test with 1 fieldname"""
    feed_url_to_config = {"https://ipstack.com": {"fieldnames": ["value", "a"], "indicator_type": "IP", "mapping": {"AAA": "a"}}}
    ...
    with requests_mock.Mocker() as m:
        ...
        client = Client(url="https://ipstack.com", feed_url_to_config=feed_url_to_config)
        hr, indicators_ec, raw_json = get_indicators_command(client, args)
        assert not indicators_ec
        for ind_json in raw_json:
            ind_val = ind_json.get("value")
...
def test_get_indicators_2():
    ...
    feed_url_to_config = {"https://ipstack.com": {"fieldnames": ["special_ind"], "indicator_type": "IP"}}
...
def test_get_feed_content():
    ...
    feed_url_to_config = {
        "https://ipstack1.com": {"content": ip_ranges_unzipped},
        "https://ipstack2.com": {
            "content": ip_ranges_unzipped,
            "is_zipped_file": False,
        },
        "https://ipstack3.com": {"content": ip_ranges_zipped, "is_zipped_file": True},
    }
...
@pytest.mark.parametrize(
    ...
    [
        ("2020-02-10 13:39:14", "2020-02-10T13:39:14Z"),
        ("2020-02-10T13:39:14", "2020-02-10T13:39:14Z"),
        ("2020-02-10 13:39:14.123", "2020-02-10T13:39:14Z"),
        ("2020-02-10T13:39:14.123", "2020-02-10T13:39:14Z"),
        ("2020-02-10T13:39:14Z", "2020-02-10T13:39:14Z"),
        ("2020-11-01T04:16:13-04:00", "2020-11-01T08:16:13Z"),
    ],
)
...
class TestTagsParam:
    def test_tags_exists(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            args = {"indicator_type": itype, "limit": 35}
            m.get("https://ipstack.com", content=ip_ranges)
            client = Client(url="https://ipstack.com", feed_url_to_config=feed_url_to_config, feedTags=tags)
    ...
    def test_tags_not_exists(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            args = {"indicator_type": itype, "limit": 35}
            m.get("https://ipstack.com", content=ip_ranges)
            client = Client(url="https://ipstack.com", feed_url_to_config=feed_url_to_config, feedTags=[])
...
def test_create_fields_mapping():
    ...
    assert result == {"Value": "John", "Country": "United States", "Count": "Low"}
...
def test_get_indicators_with_relations():
        ...
        [
            {
                ...
                "rawJSON": {
                    "value": "test.com",
                    "a": "Domain used by Test c&c",
                    None: ["2021-04-22 06:03", "https://test.com/manual/test-iplist.txt"],
                    "type": "IP",
                },
                "fields": {"AAA": "Domain used by Test c&c", "relationship_entity_b": "Test", "tags": []},
                "relationships": [
                    {
                        "name": "resolved-from",
    ...
    with requests_mock.Mocker() as m:
        ...
        indicators = fetch_indicators_command(
            client, default_indicator_type=itype, auto_detect=False, limit=35, create_relationships=True
        )
...
def test_fetch_indicators_with_enrichment_excluded(requests_mock):
    ...
    client = Client(url="https://ipstack.com", feed_url_to_config=feed_url_to_config)
    indicators = fetch_indicators_command(
        client, default_indicator_type=itype, auto_detect=False, limit=35, create_relationships=False, enrichment_excluded=True
    )
    assert indicators == expected_res
...
def test_get_indicators_without_relations():
    ...
    with requests_mock.Mocker() as m:
        ...
        client = Client(url="https://ipstack.com", feed_url_to_config=feed_url_to_config)
        indicators = fetch_indicators_command(
            client, default_indicator_type=itype, auto_detect=False, limit=35, create_relationships=False
        )
        assert indicators == expected_res
...
def test_get_no_update_value(mocker):
    ...
    class MockResponse:
        headers = {
            "Last-Modified": "Fri, 30 Jul 2021 00:24:13 GMT",  # guardrails-disable-line
    ...
    assert (
        demisto.debug.call_args[0][0] == "New indicators fetched - the Last-Modified value has been updated,"
...
def test_build_iterator_modified_headers(mocker):
    ...
    mocker.patch(
        ...
        return_value={"https://api.github.com/meta": {"etag": "etag", "last_modified": "2023-05-29T12:34:56Z"}},
    )
...
@pytest.mark.parametrize(
    ...
    [(True, {}), (False, {"If-None-Match": "etag", "If-Modified-Since": "2023-05-29T12:34:56Z"})],
)
def test_build_iterator__with_and_without_passed_time_threshold(mocker, has_passed_time_threshold_response, expected_result):
    ...
    client = Client(url="https://api.github.com/meta", credentials={"identifier": "user", "password": "password"})
...
def test_get_indicators_command(mocker):
    ...
    args = {"indicator_type": "IP", "limit": "50"}
    tags = ["tag1", "tag2"]
    tlp_color_red_params = {"tlp_color": "RED", "enrichmentExcluded": False}
