...
def test_update_teams_message_policy_violation_command_chat(mocker: MockerFixture):
    """
    Given:
    ...
    """
    http_mock = mocker.patch.object(CLIENT_MOCKER.ms_client, "http_request", return_value=MagicMock())
    args = {
        "chat_id": "chat-1",
        "message_id": "msg-1",
    ...
    result = update_teams_message_policy_violation_command(CLIENT_MOCKER, args)
    ...
    assert result.outputs_prefix == "MsGraphFiles.TeamsMessagePolicyViolation"
    assert result.outputs["messageId"] == "msg-1"
    assert result.outputs["chatId"] == "chat-1"
    assert "verdictDetails" not in result.outputs


def test_update_teams_message_policy_violation_command_channel_reply(mocker: MockerFixture):
    """
    Given:
...
def test_update_teams_message_policy_violation_command_invalid_target(mocker: MockerFixture):
    """
    Given:
    ...
    with pytest.raises(DemistoException):
        update_teams_message_policy_violation_command(CLIENT_MOCKER, args)
