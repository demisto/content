...
class TestInitCollectionsToFetch:
    ...
    default_id = 1
    nondefault_id = 2
    mock_client.collections = [MockCollection(nondefault_id, "not_default"), MockCollection(default_id, "default")]
...
class TestInitRoots:
    ...
    def test_given_default_api_root_v20(self):
        ...
        mock_client = Taxii2FeedClient(
            ...
            collection_to_fetch="default",
            proxies=[],
            verify=False,
            objects_to_fetch=[],
            default_api_root="federal",
        )
        mock_client.init_server()
        self._title = ""
    ...
    has_version_error = (
        "Unexpected Response. Got Content-Type: 'application/taxii+json; charset=utf-8; version=2.1' "
    ...
    has_both_errors = (
        "Unexpected Response. 406 Client Error. Got Content-Type: 'application/taxii+json; charset=utf-8; "
...
class TestFetchingStixObjects:
    ...
    @pytest.mark.parametrize(
        "last_modifies_client, last_modifies_param, expected_modified_result",
        [
            (None, None, None),
            (None, "2021-09-29T15:55:04.815Z", "2021-09-29T15:55:04.815Z"),
    ...
    @pytest.mark.parametrize("objects_to_fetch_param", ([], ["example_type"], ["example_type1", "example_type2"]))
    def test_objects_to_fetch_parameter(self, mocker, objects_to_fetch_param):
        ...
        mock_client = Taxii2FeedClient(
            url="",
            collection_to_fetch=mock_collection_to_fetch,
            proxies=[],
            verify=False,
            objects_to_fetch=objects_to_fetch_param,
        )
...
class TestParsingIndicators:
    ...
    @pytest.fixture()
    def taxii_2_client():
        return Taxii2FeedClient(url="", collection_to_fetch="", proxies=[], verify=False, tlp_color="GREEN", objects_to_fetch=[])
    ...
    def test_parse_autonomous_system_indicator(self, taxii_2_client):
        ...
        assert (
            taxii_2_client.parse_sco_autonomous_system_indicator(autonomous_system_obj)
            == xsoar_expected_response_with_update_custom_fields
        )
            ...
            (
                {
                    ...
                    "type": "ipv4-addr",
                    "value": "1.1.1.1",
                    "extensions": {"extension-definition--1234": {"tags": ["test"], "description": "test"}},
                },
                [
                    {
                        "value": "1.1.1.1",
    ...
    def test_parse_general_sco_indicator(
        self,
        taxii_2_client,
        _object: dict,
        xsoar_expected_response: List[dict],
    ...
    def test_parse_file_sco_indicator(self, taxii_2_client):
        ...
        file_obj = {
            ...
            "hashes": {"SHA-256": "841a8921140aba50671ebb0770fecc4ee308c4952cfeff8de154ab14eeef4649"},
            "name": "quêry.dll",
        ...
        xsoar_expected_response = [
            {
                "fields": {
                    ...
                    "keyvalue": [
                        {"data": "qwerty", "type": "REG_SZ", "name": "Foo"},
                        {"data": "42", "type": "REG_DWORD", "name": "Bar"},
                    ],
    ...
    def test_parse_vulnerability(self, taxii_2_client):
        ...
        vulnerability_object = {
            ...
            "extensions": {"extension-definition--1234": {"CustomFields": {"tags": ["test", "elevated"], "description": "test"}}},
            "created_by_ref": "identity--ce222222-2a22-222b-2222-222222222222",
            "external_references": [
                {"external_id": "CVE-1234-5", "source_name": "cve"},
    ...
    def test_parse_identity(self, taxii_2_client):
        ...
        """
        identity_object = {
            "contact_information": "test@org.com",
            "created": "2021-06-01T00:00:00.000Z",
            ...
            "spec_version": "2.1",
            "extensions": {"extension-definition--1234": {"CustomFields": {"tags": ["consent-everyone"], "description": "test"}}},
            "type": "identity",
        }
    ...
    upper_case_country_object = {
        "administrative_area": "US-MI",
        "country": "US",
    ...
    @pytest.mark.parametrize(
        "location_object, xsoar_expected_response",
        [
    ...
    @pytest.mark.parametrize(
        "x509_certificate_object, xsoar_expected_response",
        [(X509_CERTIFICATE, EXPECTED_RESULT_X509_CERTIFICATE), (X509_CERTIFICATE_WITHOUT_SERIAL_NUMBER, [])],
    )
...
def test_reports_objects_with_relationships():
    ...
    cilent = XSOAR2STIXParser(
        server_version="2.0", fields_to_present=set(), types_for_indicator_sdo=[], namespace_uuid=uuid_for_cilent
    )
    objects = [
        {
...
def test_create_entity_b_stix_objects_with_file_object(mocker):
    ...
    uuid_for_cilent = uuid.uuid5(PAWN_UUID, "test")
    cilent = XSOAR2STIXParser(
        server_version="2.1", fields_to_present=set(), types_for_indicator_sdo=[], namespace_uuid=uuid_for_cilent
    )
...
TEST_CREATE_SCO_STIX_UUID_PARAMS = [
    (XSOAR_INDICATOR_1, STIX_TYPE_1, VALUE_1, EXPECTED_STIX_ID_1),
...
TEST_CREATE_SDO_STIX_UUID_PARAMS = [
    (SDO_XSOAR_INDICATOR_1, SDO_STIX_TYPE_1, SDO_VALUE_1, SDO_EXPECTED_STIX_ID_1),
...
def test_create_manifest_entry(xsoar_indicator, xsoar_type, expected_manifest_entry):
    ...
    """
    cilent = XSOAR2STIXParser(server_version="2.1", fields_to_present=set(), types_for_indicator_sdo=[], namespace_uuid=PAWN_UUID)
...
def test_init_client_with_wrong_version():
    ...
    with pytest.raises(Exception) as e:
        XSOAR2STIXParser(
            server_version="2.3", fields_to_present={"name", "type"}, types_for_indicator_sdo=[], namespace_uuid=PAWN_UUID
        )

    # Assert
    assert str(e.value) == "Wrong TAXII 2 Server version: 2.3. Possible values: 2.0, 2.1."


@pytest.mark.parametrize(
    "indicator_json, expected_result",
    [
        ({}, ""),
...
def test_add_sdo_required_field_2_0(stix_object, xsoar_indicator, expected_stix_object):
    ...
    """
    cilent = XSOAR2STIXParser(
        server_version="2.0", fields_to_present={"name", "type"}, types_for_indicator_sdo=[], namespace_uuid=PAWN_UUID
    )
...
def test_get_indicator_publication():
    ...
    assert STIX2XSOARParser.get_indicator_publication(data.get("attack_pattern_data")[0], ignore_external_id=True) == data.get(
        "publications"
    )
...
def test_create_indicators(mocker):
    ...
    iocs, extensions, total = cilent.create_indicators(
        IndicatorsSearcher(
...
@pytest.mark.parametrize(
    "indicator_name, expected_result",
    [
        (
            {
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            id="case: file hashed with SHA-256",
        ),
        pytest.param(
            "[file:hashes.'MD5' = '00000000000000000000000000000000']",
        ...
        pytest.param(
            ...
            "1.1.1.1/32",
            id="Complex pattern with multiple values",
        ),
    ],
)
