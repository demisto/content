...
def test_initial_setup_role_already_assigned(mocker):
    ...
    mocker.patch.object(
        OktaClient, "_http_request", side_effect=DemistoException("Error in API call [409] - Conflict", res=mock_response)
    )
...
def test_get_token_use_existing(mocker):
    ...
    mocker.patch.object(
        OktaApiModule, "get_integration_context", return_value={"token": "X", "token_expiration": "2021-01-01T01:00:00"}
    )
...
def test_get_token_regenerate_existing(mocker):
    ...
    mocker.patch.object(
        OktaApiModule, "get_integration_context", return_value={"token": "YYY", "token_expiration": "2021-01-01T01:00:00"}
    )
...
"""  OKTA V2 INTEGRATION TESTS  """

from unittest.mock import MagicMock, patch  # noqa: E402

import requests_mock  # noqa: E402,F401
...
def test_get_logs_command_with_limit(mocker, requests_mock, limit, logs_amount):
    ...
    """
    from OktaApiModule import get_logs_command
...
def test_expire_password_with_revoke_session():
    ...
    """
    from OktaApiModule import expire_password_command
...
def test_get_group_id(mocker, group_name, mock_response, expected_id):
    ...
    mocker.patch("OktaApiModule.encode_string_results", side_effect=lambda x: x)  # Mock encode_string_results
...
def test_reset_auth_command_ucp(mocker):
    ...
    mocker.patch.object(OktaApiModule, "should_use_ucp_auth", return_value=True)
    mocker.patch.object(OktaApiModule, "get_ucp_method_unique_id", return_value="mid-123")
    mock_invalidate = mocker.patch.object(OktaApiModule, "invalidate_ucp_credentials")
    mock_reset_context = mocker.patch.object(OktaApiModule, "reset_integration_context")
