...
def run_microsoft_graph_teams_integration():
    """COMMANDS MANAGER / SWITCH PANEL

    Shared entrypoint consumed by every pack that fronts the Microsoft Graph Teams
    integration (community `MicrosoftGraphTeams` pack + xsoar-supported
    `MicrosoftGraphTeamsStandardConnector` satellite pack). Each pack's YML declares its
    own command surface, and the platform routes only YML-declared commands here — so
    a satellite pack listing a subset of commands cannot invoke the others even though
    they are dispatched below.
    """
    ...
    # Under UCP the credential envelope is injected per-request by the platform,
    # so config-time validation of `enc_key` / certificate is skipped.
    ...
    try:
        ...
        if command == "msgraph-teams-generate-login-url":
            ...
            update_teams_message_policy_violation_command(client, demisto.args())
        else:
