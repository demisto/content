...
COMMAND_SCOPES: dict[str, list[str]] = {
    ...
    "DRIVES": ["https://www.googleapis.com/auth/drive", "https://www.googleapis.com/auth/drive.readonly"],
    "FILES": [
        "https://www.googleapis.com/auth/drive",
        "https://www.googleapis.com/auth/drive.file",
        "https://www.googleapis.com/auth/drive.readonly",
...
def prepare_markdown_from_dictionary(data: dict[str, Any], ignore_fields: list[str] = []) -> str:
    ...
    for key, value in data.items():
        if key not in ignore_fields:
            hr_cell_info.append("{}: {}".format(pascalToSpace(key), ", ".join(value) if isinstance(value, list) else value))
    return "\n".join(hr_cell_info)
...
def prepare_drive_changes_output(
    response: dict[str, Any], drive_id: str, user_id: str
...
def prepare_body_for_drive_activity(args: dict[str, str]) -> dict[str, str | int]:
    ...
    if action_detail_case_include:
        filter_activity += (
            " AND " + DRIVE_ACTIVITY_DETAIL_ACTION.format(action_detail_case_include)
            if time_range
            else DRIVE_ACTIVITY_DETAIL_ACTION.format(action_detail_case_include)
        )

    if action_detail_case_remove:
        filter_activity += (
            " AND -" + DRIVE_ACTIVITY_DETAIL_ACTION.format(action_detail_case_remove)
...
def set_true_for_empty_dict(d):
    ...
    if not isinstance(d, dict | list):
        ...
        if d == {}:
            return True
        return {
            key: True if value == {} else value
            for key, value in ((key, set_true_for_empty_dict(value)) for key, value in d.items())
        }
...
def prepare_hr_setting_changes_entity(context: dict[str, Any]) -> str:
    ...
    restriction_changes = ""
    settings_change = [
        {"feature": setting.get("feature", ""), "newRestriction": setting.get("newRestriction", "")}
...
def prepare_drive_activity_human_readable(outputs_context: list[dict[str, Any]]) -> str:
    ...
    for context in outputs_context:
        ...
        if name == "edit":
            drive_hr.append({ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "Edit", OBJECT_HEADER: object_target})

        elif name == "delete":
            delete_type: str = "Delete Type: " + primary_action_detail.get("delete", {}).get("type", "") + "\n"
            drive_hr.append({ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "Delete", OBJECT_HEADER: delete_type + object_target})

        elif name == "restore":
            restore_type: str = "Restore Type: " + primary_action_detail.get("restore", {}).get("type", "") + "\n"
            drive_hr.append({ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "Restore", OBJECT_HEADER: restore_type + object_target})

        elif name == "dlpChange":
            dlp_type: str = "DataLeakPreventionChange Type: " + primary_action_detail.get("dlpChange", {}).get("type", "") + "\n"
            drive_hr.append(
                {ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "DataLeakPreventionChange", OBJECT_HEADER: dlp_type + object_target}
            )
            ...
            drive_hr.append(
                {ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "Reference", OBJECT_HEADER: reference_type + object_target}
            )
            ...
            rename_titles: str = (
                "Old Title: '"
                + rename_object.get("oldTitle", "")
                + "'\n"
                + "New Title: '"
                + rename_object.get("newTitle", "")
                + "'\n"
            )
            drive_hr.append({ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "Rename", OBJECT_HEADER: rename_titles})

        elif name == "move":
            drive_hr.append({ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "Move", OBJECT_HEADER: object_target})

        elif name == "permissionChange":
            drive_hr.append({ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "PermissionChange", OBJECT_HEADER: object_target})

        elif name == "settingsChange":
            settings_changes: str = prepare_hr_setting_changes_entity(context)
            drive_hr.append(
                {ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "SettingsChange", OBJECT_HEADER: settings_changes + object_target}
            )

        elif name == "comment":
            drive_hr.append({ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "Comment", OBJECT_HEADER: object_target})

        elif name == "create":
            created: list = list(primary_action_detail.get("create", {}).keys())
            drive_hr.append(
                {ACTIVITY_TIME: time_stamp, PRIMARY_ACTION: "Create " + created[0].capitalize(), OBJECT_HEADER: object_target}
            )

    drive_hr = GSuiteClient.remove_empty_entities(drive_hr)
    drive_activity_hr = tableToMarkdown(
        HR_MESSAGES["LIST_COMMAND_SUCCESS"].format("Drive Activity(s)", len(drive_hr)),
        drive_hr,
...
def flatten_permission_change_keys_for_fetch_incident(permission_change: dict[str, Any]) -> None:
    ...
    if "addedPermissions" in permission_change:
        permission_change["addedPermissions"] = [
            update_permission(permission) for permission in permission_change["addedPermissions"]
        ]
    if "removedPermissions" in permission_change:
        permission_change["removedPermissions"] = [
            update_permission(permission) for permission in permission_change["removedPermissions"]
        ]
...
def flatten_move_keys_for_fetch_incident(move: dict[str, Any]) -> None:
    ...
    if "addedParents" in move:
        move["addedParents"] = [update_move_parents(parents) for parents in move["addedParents"]]
    if "removedParents" in move:
        move["removedParents"] = [update_move_parents(parents) for parents in move["removedParents"]]
...
def flatten_comment_mentioned_user_keys_for_fetch_incident(comment: dict[str, Any]) -> None:
    ...
    if "mentionedUsers" in comment:
        comment["mentionedUsers"] = [flatten_user_dict(user) for user in comment["mentionedUsers"]]
...
def validate_params_for_fetch_incidents(params: dict[str, Any]) -> None:
    ...
    params["max_fetch"] = GSuiteClient.validate_get_int(
        params.get("max_fetch", 10), limit=100, message=MESSAGES["MAX_INCIDENT_ERROR"]
    )
...
def drive_create_command(client: "GSuiteClient", args: dict[str, str]) -> CommandResults:
    ...
    response = client.http_request(
        url_suffix=URL_SUFFIX["DRIVE_CREATE"], params={"requestId": str(uuid.uuid4())}, body=args, method="POST"
    )
    ...
    readable_output = tableToMarkdown(
        HR_MESSAGES["DRIVE_CREATE_SUCCESS"], hr_output, headerTransform=pascalToSpace, removeNull=True, headers=hr_output_fields
    )
...
def drive_changes_list_command(client: "GSuiteClient", args: dict[str, Any]) -> CommandResults:
    ...
    response = client.http_request(url_suffix=URL_SUFFIX["DRIVE_CHANGES"], method="GET", params=params)

    outputs, drive_changes_hr_files_list, drive_changes_hr_drives_list = prepare_drive_changes_output(
        response, args.get("drive_id", ""), user_id
    )
    readable_output = ""
...
def handle_response_drive_list(response: dict[str, Any]) -> CommandResults:
    ...
    readable_output += drives_hr if response.get("drives", "") else HR_MESSAGES["NOT_FOUND"].format("Drives")
...
def handle_response_single_drive(response: dict[str, Any], args: dict[str, str]):
    ...
    readable_output += drive_hr if response.get("id", "") else HR_MESSAGES["NOT_FOUND"].format("Drive")
...
def prepare_drives_human_readable(outputs_context: list[dict[str, Any]]) -> str:
    ...
    return tableToMarkdown(
        HR_MESSAGES["LIST_COMMAND_SUCCESS"].format("Drive(s)", len(outputs_context)),
...
def prepare_single_drive_human_readable(outputs_context: dict[str, Any], args: dict[str, str]) -> str:
    ...
    return tableToMarkdown(
        HR_MESSAGES["LIST_COMMAND_SUCCESS"].format("Drive(s)", 1),
        GSuiteClient.remove_empty_entities(outputs_context),
...
def prepare_file_read_request(client: "GSuiteClient", args: dict[str, str]) -> dict[str, Any]:
    corpora_values = {"User": "user", "Domain": "domain", "Drive": "drive", "All Drives": "allDrives"}
...
def file_get_parents(client: "GSuiteClient", args: dict[str, str]) -> CommandResults:
    ...
    while not is_root:
        modify_label_request_res = prepare_file_modify_labels_request(client, args, scopes=COMMAND_SCOPES["FILES"])
    ...
    outputs: dict = {OUTPUT_PREFIX["PARENTS"]: parents}
...
def handle_response_files_list(response: dict[str, Any]) -> CommandResults:
    ...
    readable_output += files_hr if response.get("files", "") else HR_MESSAGES["NOT_FOUND"].format("Files")
...
def handle_response_single_file(response: dict[str, Any], args: dict[str, str]):
    ...
    outputs: dict = {OUTPUT_PREFIX["GOOGLE_DRIVE_FILE_HEADER"]: {OUTPUT_PREFIX["FILE"]: outputs_context}}
    outputs = GSuiteClient.remove_empty_entities(outputs)

    readable_output = file_hr if response.get("id", "") else HR_MESSAGES["NOT_FOUND"].format("File")
...
def prepare_files_human_readable(outputs_context: list[dict[str, Any]]) -> str:
    ...
    return tableToMarkdown(
        HR_MESSAGES["LIST_COMMAND_SUCCESS"].format("File(s)", len(outputs_context)),
        GSuiteClient.remove_empty_entities(outputs_context),
        [
            "id",
            "name",
...
def modify_label_command(client: "GSuiteClient", args: dict[str, str]) -> CommandResults:
                ...
                "fieldModifications": [
                    {
                        ...
                        "setSelectionValues": [args.get("selection_label_id")],
                    }
                ],
    ...
    table_hr_md = tableToMarkdown(
        HR_MESSAGES["MODIFY_LABEL_SUCCESS"].format(args.get("file_id")), response, headerTransform=pascalToSpace, removeNull=False
    )
    outputs_context = {OUTPUT_PREFIX["LABELS"]: response}
...
def get_file_labels_command(client: "GSuiteClient", args: dict[str, str]) -> CommandResults:
    ...
    table_hr_md = tableToMarkdown(
        HR_MESSAGES["GET_LABEL_SUCCESS"].format(args.get("file_id")),
        response["labels"],
        headerTransform=pascalToSpace,
        removeNull=False,
    )
...
def get_labels_command(client: "GSuiteClient", args: dict[str, str]) -> CommandResults:
    modify_label_request_res = prepare_get_labels_request(client, args, scopes=COMMAND_SCOPES["MODIFY_LABELS_PERMISSIONS_CRUD"])
    http_request_params = modify_label_request_res["http_request_params"]
    ...
    table_hr_md = tableToMarkdown(
        HR_MESSAGES["GET_LABELS_SUCCESS"], response["labels"], headerTransform=pascalToSpace, removeNull=False
    )
    return CommandResults(readable_output=table_hr_md, outputs=outputs_context)
...
def prepare_file_modify_labels_request(client: "GSuiteClient", args: dict[str, str], scopes: list[str]) -> dict[str, Any]:
    ...
    client.set_authorized_http(scopes=scopes, subject=user_id)
    # Prepare generic HTTP request params
    http_request_params: dict[str, str] = assign_params(fileId=args.get("file_id"))
...
def prepare_get_labels_request(client: "GSuiteClient", args: dict[str, str], scopes: list[str]) -> dict[str, Any]:
    ...
    user_id = args.get("user_id") or client.user_id
    client.set_authorized_http(scopes=scopes, subject=user_id)
    http_request_params: dict[str, str] = assign_params(view="LABEL_VIEW_FULL")
...
def file_permission_list_command(client: "GSuiteClient", args: dict[str, str]) -> CommandResults:
    ...
    http_request_params.update(assign_params(pageSize=args.get("page_size"), pageToken=args.get("page_token"), fields="*"))
...
def handle_response_permissions_list(response: dict[str, Any], args: dict[str, str]) -> CommandResults:
    ...
    readable_output += files_hr if response.get("permissions", "") else HR_MESSAGES["NOT_FOUND"].format("Permissions")
...
def prepare_permissions_human_readable(outputs_context: list[dict[str, Any]], args: dict[str, str]) -> str:
    ...
    return tableToMarkdown(
        HR_MESSAGES["LIST_COMMAND_SUCCESS"].format("Permission(s)", len(outputs_context)),
        GSuiteClient.remove_empty_entities(outputs_context),
...
def handle_response_permission_single(response: dict[str, Any], args: dict[str, str]) -> CommandResults:
    ...
    outputs: dict = {OUTPUT_PREFIX["GOOGLE_DRIVE_FILE_PERMISSION_HEADER"]: {OUTPUT_PREFIX["FILE_PERMISSION"]: outputs_context}}
    outputs = GSuiteClient.remove_empty_entities(outputs)
    readable_output += files_hr
...
def drive_activity_list_command(client: "GSuiteClient", args: dict[str, str]) -> CommandResults:
    ...
    readable_output = ""
    outputs_context = [prepare_drive_activity_output(activity) for activity in response.get("activities", [])]
    drive_activity_hr = prepare_drive_activity_human_readable(outputs_context)

    outputs: dict = {OUTPUT_PREFIX["DRIVE_ACTIVITY_LIST"]: outputs_context}
    if response.get("nextPageToken", ""):
        outputs[OUTPUT_PREFIX["DRIVE_ACTIVITY_LIST_PAGE_TOKEN"]] = {"nextPageToken": response["nextPageToken"]}
        readable_output += NEXT_PAGE_TOKEN.format(response.get("nextPageToken"))
    outputs = GSuiteClient.remove_empty_entities(outputs)
    readable_output += drive_activity_hr if response.get("activities", "") else HR_MESSAGES["NOT_FOUND"].format("Drive Activity")
...
def file_copy_command(client: "GSuiteClient", args: dict[str, str]) -> CommandResults:
    ...
    return CommandResults(
        ...
        outputs_key_field="id",
        outputs=file,
        readable_output=tableToMarkdown("File copied successfully.", file, headerTransform=string_to_table_header),
    )


def fetch_incidents(
    client: "GSuiteClient", last_run: dict, params: dict, is_test: bool = False
    ...
    last_fetch = last_run["last_fetch"] if last_run.get("last_fetch") else date_to_timestamp(params.get("first_fetch_interval"))
    ...
    response = client.http_request(body=body, full_url=URLS["DRIVE_ACTIVITY"], method="POST")
    ...
    for activity in activities:
        ...
        flatten_permission_change_keys_for_fetch_incident(activity.get("primaryActionDetail", {}).get("permissionChange", {}))
        flatten_move_keys_for_fetch_incident(activity.get("primaryActionDetail", {}).get("move", {}))
        flatten_comment_mentioned_user_keys_for_fetch_incident(activity.get("primaryActionDetail", {}).get("comment", {}))
        ...
        timestamp = int(
            dateparser.parse(  # type: ignore
                activity.get("timestamp", activity.get("timeRange", {}).get("endTime", ""))
            ).timestamp()
            * 1000
        )
...
def _extract_file_id_from_url(url: str) -> str:
    ...
    - any URL containing an ?id=<fileId> / &id=<fileId> query parameter
    ...
    raise ValueError(
        ...
        "https://docs.google.com/{document|spreadsheets|presentation|forms}/d/<fileId>/edit, "
        "https://drive.google.com/file/d/<fileId>/view, "
        "https://drive.google.com/open?id=<fileId>, "
        "or any URL containing an ?id=<fileId> / &id=<fileId> query parameter."
    )
...
def _download_drive_file_content(client: "GSuiteClient", file_id: str, mime_type: str) -> tuple[str, str]:
    ...
    if export_mime:
        raw_bytes = drive_service.files().export(fileId=file_id, mimeType=export_mime).execute()  # pylint: disable=no-member
    else:
        raw_bytes = drive_service.files().get_media(fileId=file_id, supportsAllDrives=True).execute()  # pylint: disable=no-member
...
def run_google_drive_integration() -> None:  # pragma: no cover
    """
