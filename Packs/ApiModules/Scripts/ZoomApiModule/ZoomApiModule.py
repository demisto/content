...
class Zoom_Client(BaseClient):
    ...
    def generate_oauth_token(self):
        ...
        token_res = self._http_request(
            method="POST",
            full_url=full_url,
            params={"account_id": self.account_id, "grant_type": "account_credentials"},
    ...
    def generate_oauth_client_token(self):
        ...
        token_res = self._http_request(
            method="POST",
            full_url=full_url,
            params={"account_id": self.account_id, "grant_type": "client_credentials"},
    ...
    def get_oauth_token(self, force_gen_new_token=False):
        ...
        if not ctx or not ctx.get("token_info").get("generation_time", force_gen_new_token):
            ...
            if generation_time := dateparser.parse(ctx.get("token_info").get("generation_time")):
        ...
        ctx.update(
            {
                "token_info": {
                    ...
                    "client_oauth_token": client_oauth_token,
    ...
    def error_handled_http_request(
        self,
        method,
        url_suffix="",
        full_url=None,
