...
def mock_client_ouath(mocker):
    ...
    client = Zoom_Client(
        base_url="https://test.com", account_id="mockaccount", client_id="mockclient", client_secret="mocksecret"
    )
    return client
...
def test_generate_oauth_token(mocker):
    ...
    assert m.call_args[1]["params"] == {"account_id": "mockaccount", "grant_type": "account_credentials"}
...
def test_get_oauth_token__if_not_ctx(mocker, result):
    ...
    mocker.patch.object(
        ...
        return_value={"token_info": {"generation_time": result, "oauth_token": "old token"}},
    )
    generate_token_mock = mocker.patch.object(Zoom_Client, "generate_oauth_token")
    Zoom_Client(base_url="https://test.com", account_id="mockaccount", client_id="mockclient", client_secret="mocksecret")
...
def test_get_oauth_token__while_old_token_still_valid(mocker):
    ...
    mocker.patch.object(
        ZoomApiModule,
        "get_integration_context",
        return_value={"token_info": {"generation_time": "1988-03-03T10:50:00", "oauth_token": "old token"}},
    )
    generate_token_mock = mocker.patch.object(Zoom_Client, "generate_oauth_token")
    client = Zoom_Client(
        base_url="https://test.com", account_id="mockaccount", client_id="mockclient", client_secret="mocksecret"
...
def test_get_oauth_token___old_token_expired(mocker):
    ...
    mocker.patch.object(
        ZoomApiModule,
        "get_integration_context",
        return_value={"token_info": {"generation_time": "1988-03-03T10:00:00", "oauth_token": "old token"}},
    )
    generate_token_mock = mocker.patch.object(Zoom_Client, "generate_oauth_token")
    client = Zoom_Client(
        base_url="https://test.com", account_id="mockaccount", client_id="mockclient", client_secret="mocksecret"
...
def test_get_oauth_token___old_token_is_unreachable(mocker, return_val):
    ...
    mocker.patch.object(ZoomApiModule, "get_integration_context", return_value=return_val)
    generate_token_mock = mocker.patch.object(Zoom_Client, "generate_oauth_token")
    client = Zoom_Client(
        base_url="https://test.com", account_id="mockaccount", client_id="mockclient", client_secret="mocksecret"
    )
...
def test_http_request___when_raising_invalid_token_message(mocker):
    ...
    m = mocker.patch.object(ZoomApiModule.BaseClient, "_http_request", side_effect=DemistoException("Invalid access token"))
