    ...
    with requests_mock.Mocker() as m:
        ...
        feed_type = {
            "https://www.spamhaus.org/drop/asndrop.txt": {
                ...
                "fields": [
                    {"asndrop_country": {"regex": r"^.*;\W([a-zA-Z]+)\W+", "transform": r"\1"}},
                    {"asndrop_org": {"regex": r"^.*\|\W+(.*)", "transform": r"\1"}},
                ],
            }
        }
...
def test_get_indicators_with_relations():
    ...
    feed_url_to_config = {
        "https://www.spamhaus.org/drop/asndrop.txt": {
            ...
            "fields": [
                {
                    "firstseenbysource": {"regex": r"^(\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})", "transform": "\\1"},
                    "port": {"regex": r"^.+,.+,(\d{1,5}),", "transform": "\\1"},
                    "updatedate": {"regex": r"^.+,.+,.+,(\d{4}-\d{2}-\d{2})", "transform": "\\1"},
                    "malwarefamily": {"regex": r"^.+,.+,.+,.+,(.+)", "transform": "\\1"},
                    "relationship_entity_b": {"regex": r"^.+,.+,.+,.+,\"(.+)\"", "transform": "\\1"},
                }
            ],
        }
    }
    ...
    with requests_mock.Mocker() as m:
        ...
        indicators = fetch_indicators_command(
            client, feed_tags=[], tlp_color=[], itype="IP", auto_detect=False, create_relationships=True
        )
...
def test_get_indicators_without_relations():
    ...
    with requests_mock.Mocker() as m:
        ...
        indicators = fetch_indicators_command(
            client, feed_tags=[], tlp_color=[], itype="IP", auto_detect=False, create_relationships=False
        )
...
@pytest.mark.parametrize(
    ...
    [(True, None), (False, {"If-None-Match": "etag", "If-Modified-Since": "2023-05-29T12:34:56Z"})],
)
...
def test_feed_main_enrichment_excluded(mocker):
    ...
    params = {"tlp_color": "RED", "enrichmentExcluded": False}
    feed_name = "test_feed"
    prefix = "test_prefix"
