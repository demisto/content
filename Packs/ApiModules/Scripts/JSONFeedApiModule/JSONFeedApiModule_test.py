...
from JSONFeedApiModule import Client, fetch_indicators_command, get_no_update_value, jmespath
...
def test_json_feed_no_config():
    ...
    with requests_mock.Mocker() as m:
        ...
        indicators, _ = fetch_indicators_command(client=client, indicator_type="CIDR", feedTags=["test"], auto_detect=False)
...
def test_list_of_indicators_with_no_json_object():
    feed_name_to_config = {
        "Github": {"url": "https://api.github.com/meta", "extractor": "hooks", "indicator": None, "remove_ports": "true"}
    }

    with requests_mock.Mocker() as m:
        m.get("https://api.github.com/meta", json=json.loads(FLAT_LIST_OF_INDICATORS))

        client = Client(url="https://api.github.com/meta", feed_name_to_config=feed_name_to_config, insecure=True)

        indicators, _ = fetch_indicators_command(
            client=client, indicator_type=None, feedTags=["test"], auto_detect=True, remove_ports=True
        )
...
def test_fetch_indicators_with_exclude_enrichment():
    ...
    with requests_mock.Mocker() as m:
        ...
        indicators, _ = fetch_indicators_command(
            client=client, indicator_type=None, feedTags=["test"], auto_detect=True, remove_ports=True, enrichment_excluded=True
        )
...
def test_post_of_indicators_with_no_json_object():
    feed_name_to_config = {
        "Github": {"url": "https://api.github.com/meta", "extractor": "hooks", "indicator": None, "remove_ports": "false"}
    }
    ...
    with requests_mock.Mocker() as m:
        ...
        client = Client(url="https://api.github.com/meta", feed_name_to_config=feed_name_to_config, insecure=True, data="test=1")
...
def test_fetch_indicators_command_google_ip_ranges(mocker):
    ...
    mocker.patch.object(
        client,
        "build_iterator",
        return_value=([{"ipv4Prefix": "1.1.1.1"}, {"ipv4Prefix": "1.2.3.4"}, {"ipv6Prefix": "1111:1111::/28"}], True),
    )
...
@pytest.mark.parametrize("remove_ports, expected_result", [(True, "192.168.1.1"), (False, "192.168.1.1:443")])
def test_remove_ports_threatfox(mocker, remove_ports, expected_result):
    ...
    with requests_mock.Mocker() as m:
        ...
        client = Client(
            url="https://threatfox.abuse.ch/export/json/recent/", feed_name_to_config=feed_name_to_config, insecure=True
        )

        indicators = fetch_indicators_command(
            client=client, indicator_type="IP", auto_detect=True, remove_ports=remove_ports, feedTags=["ThreatFox"]
        )
