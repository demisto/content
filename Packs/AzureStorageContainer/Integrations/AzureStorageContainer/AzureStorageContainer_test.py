...
def test_azure_storage_get_blob_command(requests_mock):
    ...
    result = get_blob_command(client, {"container_name": container_name, "blob_name": blob_name})
...
def test_azure_storage_get_blob_tags_command(requests_mock):
    ...
    result = get_blob_tags_command(client, {"container_name": container_name, "blob_name": blob_name})
...
def test_azure_storage_block_public_access_command(mocker, requests_mock):
    ...
    params = {"shared_key": {"password": SHARED_KEY}, "credentials": {"identifier": ACCOUNT_NAME}}
    ...
    result = block_public_access_command(client, {"container_name": container_name})
    ...
    with pytest.raises(ValueError, match="Incorrect shared key provided"):
        ...
        invalid_shared_key_params = {"shared_key": {"password": "invalid-key"}, "credentials": {"identifier": ACCOUNT_NAME}}
    ...
    with pytest.raises(KeyError, match="The 'shared_key' parameter must be provided."):
        missing_shared_key_params = {"shared_key": {"password": ""}, "credentials": {"identifier": ACCOUNT_NAME}}
...
def test_azure_storage_set_blob_tags_command(requests_mock):
    ...
    result = set_blob_tags_command(client, {"container_name": container_name, "blob_name": blob_name, "tags": "{}"})
...
def test_azure_storage_delete_blob_command(requests_mock):
    ...
    result = delete_blob_command(client, {"container_name": container_name, "blob_name": blob_name})
...
def test_azure_storage_get_blob_properties_command(requests_mock):
    ...
    result = get_blob_properties_command(client, {"container_name": container_name, "blob_name": blob_name})
...
def test_azure_storage_set_blob_properties_command(requests_mock):
    ...
    result = set_blob_properties_command(client, {"container_name": container_name, "blob_name": blob_name})
...
def test_create_set_tags_request_body():
    ...
    expected = (
        "<Tags><TagSet><Tag><Key>tag-name-1</Key><Value>tag-value-1</Value></Tag><Tag><Key>tag-name-2</Key>"
...
def test_generate_sas_signature():
    ...
    assert (
        ...
        == "sp=test&st=test&se=test&sip=test&spr=https&sv=test&sr=test&sig=pyUQ25%2BIijJ2TstI5Q6Sre3jJWI0b4qwvRg2LtD9uhc%3D"
    )  # noqa
...
def test_generate_sas_signature_no_key(mocker):
    ...
    """
    from AzureStorageContainer import Client, generate_sas_token_command
...
def test_test_module_command_with_managed_identities(mocker, requests_mock, client_id):
    ...
    from AzureStorageContainer import MANAGED_IDENTITIES_TOKEN_URL, main
...
def test_put_blob_request_uses_basename(mocker):
    ...
    import os
    import demistomock as demisto
    from AzureStorageContainer import Client
