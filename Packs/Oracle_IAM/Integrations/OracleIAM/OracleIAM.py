...
class Client(BaseClient):
    ...
    def __init__(self, base_url, verify=True, proxy=False, ok_codes=(), headers=None, client_id=None, client_secret=None):
    ...
    def get_app_fields(self) -> Dict[str, Any]:
        ...
        res = self._http_request(method="GET", url_suffix="admin/v1/Schemas/urn:ietf:params:scim:schemas:core:2.0:User")
    ...
    if group_id:
        try:
            ...
            if res.status_code == 200:
                generic_iam_context = OutputContext(
                    success=True, id=group_id, displayName=res_json.get("displayName"), members=res_json.get("members")
                )
        ...
        try:
            ...
            if res.status_code == 200 and res_json.get("totalResults") > 0:
                ...
                generic_iam_context = OutputContext(
                    success=True, id=res_json.get("id"), displayName=group_name, members=res_json.get("members")
                )
        except DemistoException as exc:
            if exc.res.status_code == 404:
                generic_iam_context = OutputContext(
