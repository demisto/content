...
REPUTATION_MALICIOUS_FIELDS = [
    "googleSafeBrowsingIsSafeUrl",
    "malwareEngineIsSafeUrl",
    "reputationDatabaseIsSafeUrl",
...
REPUTATION_HEADERS_MALWARE_ENGINE_ANALYSIS_HEADERS = ["malwareEngineAnalysisSuccess", "malwareEngineAnalysisDescription"]
...
metadata_collector = YMLMetadataCollector(
    ...
    fromversion="6.2.0",
    conf=[
        ConfKey(name="auth", display="Username", required=True, key_type=ParameterTypes.AUTH),
        ConfKey(name="account_settings_id", display="Account Settings ID", required=True, key_type=ParameterTypes.STRING),
        ConfKey(
            name="insecure",
...
class Client(BaseClient):
    ...
    def _get_cloud_token(self):
        ...
        result = self._http_request(
            full_url="https://accounts.iboss.com/ibossauth/web/tokens", method="get", timeout=60, auth=auth, params=params
        )
    ...
    def _get_cloud_settings_tokens(self, auth_token):
        ...
        cookie_values = self._http_request(
            full_url=CLOUD_ACCOUNT_SETTINGS_URL, method="get", headers=headers, resp_type="response"
    ...
    def _get_swg_xsrf_token(self, primary_swg_dns, auth_token):
        ...
        result = self._http_request(method="post", full_url=url, json_data=data)
    ...
    def get_url_reputation(self, url):
        ...
        """
        data = {"url": url}
        result = self._http_request(method="post", url_suffix="/json/controls/urlLookup", json_data=data)
        return result

    @set_auth
    def get_policy_layers(self):
        result = self._http_request(method="get", url_suffix="/json/controls/policyLayers/all")
        return result
    ...
    def get_policy_layer_list_entries(self, custom_category_id):
        ...
        result = self._http_request(
            method="get", url_suffix="/json/controls/policyLayers/urls", params=params, ok_codes=(200, 201, 202, 204)
        )
        return result
...
@metadata_collector.command(
    ...
    outputs_list=[
        ...
        OutputArgument(
            name="Description", description="The indicator context description", prefix="IP.Malicious", output_type=str
        ),
        OutputArgument(
            name="Vendor",
        ...
        OutputArgument(name="isSafeUrl", description="Whether entity is deemed safe", prefix="iboss", output_type=int),
        OutputArgument(name="categories", description="The entity categories.", prefix="iboss", output_type=[str]),
        OutputArgument(
            name="activeMalwareSubscription",
            description="Whether active malware subscription is active",
            prefix="DBotScore",
            output_type=int,
        ),
        OutputArgument(name="categorized", description="Whether entity is categorized.", prefix="iboss", output_type=int),
        OutputArgument(
            name="googleSafeBrowsingDescription", description="Google safe browsing description", prefix="iboss", output_type=str
        ),
        OutputArgument(name="message", description="Entity lookup message.", prefix="iboss", output_type=str),
        OutputArgument(name="url", description="The entity to perforum URL check on.", prefix="iboss", output_type=str),
        OutputArgument(
            name="googleSafeBrowsingEnabled",
            description="Whether Google safe browsing is enabled.",
            prefix="iboss",
            output_type=int,
        ),
        OutputArgument(
            name="googleSafeBrowsingIsSafeUrl",
            description="Whether entity deemed safe by Google safe browsing.",
            prefix="iboss",
            output_type=int,
        ),
        OutputArgument(
            name="googleSafeBrowsingSuccess",
        ...
        OutputArgument(
            name="malwareEngineAnalysisDescription",
            description="Malware engine analysis description",
            prefix="iboss",
            output_type=str,
        ),
        OutputArgument(
            name="malwareEngineAnalysisEnabled",
        ...
        OutputArgument(
            name="malwareEngineIsSafeUrl",
            description="Whether the entity was deemed safe by the malware engine.",
            prefix="iboss",
            output_type=int,
        ),
        OutputArgument(
            name="malwareEngineResultCode", description="The result code from the malware engine analysis", output_type=str
        ),
        OutputArgument(
            name="realtimeCloudLookupDomainIsGrey",
            description="Whether realtime cloud lookup is grey.",
            prefix="iboss",
            output_type=int,
        ),
        OutputArgument(
            name="realtimeCloudLookupDomainEnabled",
            description="Whether realtime cloud lookup is enabled.",
            prefix="iboss",
            output_type=int,
        ),
        OutputArgument(
            name="realtimeCloudLookupIsSafeUrl",
            description="Whether realtime cloud lookup determined entity is safe.",
            prefix="iboss",
            output_type=int,
        ),
        OutputArgument(
            name="realtimeCloudLookupRiskDescription",
            description="Realtime cloud lookup risk description.",
            prefix="iboss",
            output_type=str,
        ),
        OutputArgument(
            name="realtimeCloudLookupSuccess", description="Whether realtime cloud lookup chec was successful.", output_type=int
        ),
        OutputArgument(
            name="reputationDatabaseBotnetDetection",
            description="Whether reputation database detected a botnet.",
            prefix="iboss",
            output_type=int,
        ),
        OutputArgument(
            name="reputationDatabaseMalwareDetection",
            description="Whether reputation database detected malware.",
            prefix="iboss",
            output_type=int,
        ),
        OutputArgument(
            name="reputationDatabaseEnabled",
    ...
    for ip in ips:
        ...
        if data := raw_response:
            ...
            ip_indicator = Common.IP(
                ip=ip, dbot_score=dbot_score, detection_engines=num_engines, positive_engines=num_positive_engines
            )
        else:  # no data
            dbot_score = Common.DBotScore(
                indicator=ip,
...
@metadata_collector.command(
    ...
    outputs_list=[
        ...
        OutputArgument(name="Score", description="The DBot score.", prefix="DBotScore", output_type=int),
        OutputArgument(
            name="Description", description="The indicator context description", prefix="Domain.Malicious", output_type=str
        ),
        OutputArgument(
            name="Vendor",
            description="The vendor that indicator context originated from.",
            prefix="Domain.Malicious",
            output_type=str,
        ),
    ...
    for domain in domains:
        ...
        if data := raw_response:
            ...
            domain_indicator = Common.Domain(
                domain=domain, dbot_score=dbot_score, detection_engines=num_engines, positive_detections=num_positive_engines
            )
        else:  # no data
            dbot_score = Common.DBotScore(
                indicator=domain,
...
@metadata_collector.command(
    ...
    outputs_list=[
        ...
        OutputArgument(name="Score", description="The DBot score.", prefix="DBotScore", output_type=int),
        OutputArgument(
            name="Description", description="The indicator context description", prefix="URL.Malicious", output_type=str
        ),
        OutputArgument(
            name="Vendor",
            description="The vendor that indicator context originated from.",
            prefix="URL.Malicious",
            output_type=str,
        ),
    ...
    for url in urls:
        ...
        if data := raw_response:
            ...
            url_indicator = Common.URL(
                url=url, dbot_score=dbot_score, detection_engines=num_engines, positive_detections=num_positive_engines
            )
        else:  # no data
            dbot_score = Common.DBotScore(
                indicator=url,
...
@metadata_collector.command(
    command_name="iboss-add-entity-to-block-list",
    outputs_prefix="iboss.AddEntityToBlockList",
    description="Adds domains, IPs, and/or URLs to a block list.",
    inputs_list=[
        InputArgument(
            name="entity", description="Domains, IPs, and/or URLs to add to the block list.", required=True, is_array=True
        ),
        InputArgument(name="current_policy_being_edited", description="The group/policy number to update.", default="1"),
        InputArgument(name="allow_keyword", description="Whether to enforce blocked keywords.", options=["0", "1"], default="0"),
        InputArgument(name="direction", description="Which direction(s) to match.", options=["0", "1", "2"], default="2"),
        InputArgument(name="start_port", description="Which start port(s) to match; 0 indicates all ports.", default="0"),
        InputArgument(name="end_port", description="Which end port(s) to match; 0 indicates all ports.", default="0"),
        InputArgument(name="global", description="Whether to apply to all groups.", options=["0", "1"], default="0"),
        InputArgument(
            name="is_regex", description="Whether entity consists of a regex pattern.", options=["0", "1"], default="0"
        ),
        InputArgument(
            name="priority",
...
def add_entity_to_block_list_command(client: Client, args: dict[str, Any]) -> list[CommandResults]:
    ...
    entities = _get_validate_argument("entity", args, validator=lambda x: x and len(x) > 0, message="value is not specified")

    current_policy_being_edited = _get_validate_argument(
        "current_policy_being_edited",
        args,
        validator=lambda x: (x or x == 0) and x > 0,
        message="value must be >= 0",
        return_type=int,
    )
    allow_keyword = _get_validate_argument(
        "allow_keyword", args, validator=lambda x: x in [0, 1], message="value must be 0 or 1", return_type=int
    )
    ...
    for entry in argToList(entities):
        result = client.add_entity_to_block_list(
            ...
            is_regex=is_regex,
            is_timed_url=is_timed_url,
            note=note,
            priority=priority,
            start_port=start_port,
            time_url_expires_in_minutes=time_url_expires_in_minutes,
        )
        ...
        if "No changes made to list" in message:
            result["message"] = (
                f"`{entry}` is already in policy {current_policy_being_edited} block list and covers "
...
@metadata_collector.command(
    ...
    inputs_list=[
        InputArgument(
            name="entity", description="Domains, IPs, and/or URLs to remove from a block list.", required=True, is_array=True
        ),
        InputArgument(name="current_policy_being_edited", description="The group/policy number to update.", default="1"),
    ],
    outputs_list=[OutputArgument(name="message", description="Operation result.", output_type=str)],
)
...
@metadata_collector.command(
    command_name="iboss-add-entity-to-allow-list",
    outputs_prefix="iboss.AddEntityToAllowList",
    description="Adds domains, IPs, and/or URLs to an allow list.",
    inputs_list=[
        InputArgument(
            name="entity", description="Domains, IPs, and/or URLs to add to an allow list.", required=True, is_array=True
        ),
        InputArgument(name="current_policy_being_edited", description="The group/policy number to update.", default="1"),
        InputArgument(name="allow_keyword", description="Whether to enforce blocked keywords.", options=["0", "1"], default="0"),
        InputArgument(name="direction", description="Which direction(s) to match.", options=["0", "1", "2"], default="2"),
...
def add_entity_to_allow_list_command(client: Client, args: dict[str, Any]) -> list[CommandResults]:
    ...
    for entry in argToList(entities):
        ...
        if "No changes made to list" in message:
            result["message"] = (
                f"`{entry}` is already in policy {current_policy_being_edited} allow list and covers "
                f"the domain entry being added. No changes made to list. "
            )
        else:
...
@metadata_collector.command(
    ...
    inputs_list=[
        InputArgument(
            name="entity", description="Domains, IPs, and/or URLs to remove from an allow list.", required=True, is_array=True
        ),
        InputArgument(name="current_policy_being_edited", description="The group/policy number to update.", default="1"),
        InputArgument(name="direction", description="Which direction(s) to match. Options=[0, 1, 2].", default="2"),
        InputArgument(name="start_port", description="Which start port(s) to match. 0 indicates all ports.", default="0"),
...
def add_entity_to_policy_layer_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    urls = _get_validate_argument("entity", args, validator=lambda x: x and len(x) > 0, message="value is not specified")
    ...
    for url in argToList(urls):
        result = client.add_entity_to_policy_layer_list(
            ...
            do_malware_scan=do_malware_scan,
            note=note,
            priority=priority,
        ...
        if "No changes made to list" in message:
            result["message"] = (
                f"{url} is already in policy layer `{policy_layer_name}` and covers "
                f"the domain entry being added. No changes made to list. "
            )
        ...
        if "No changes made to list" in message and upsert == 1:
            ...
            result = client.update_entity_in_policy_layer_list(
                original_entity=original_entry,
                url=url,
                custom_category_id=custom_category_id,
                custom_category_number=custom_category_number,
                start_port=start_port,
...
def main() -> None:
    ...
    try:
        client = Client(
            username=username, password=password, account_settings_id=account_settings_id, verify=verify_certificate, proxy=proxy
        )
