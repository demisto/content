...
- Data normalization capabilities:
  - Rules for parsing and modeling Slack's Audit logs that are ingested via the event collector into Cortex XSIAM.
...
4. **Retrieve Token:**
    Once the OAuth flow is complete, you will recevie an OAuth (starting with `xoxp-`) that authorize access to the Audit Logs API.
