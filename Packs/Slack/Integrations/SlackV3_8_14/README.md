...
This integration was integrated and tested with version xx of SlackV3.

Some changes have been made that might affect your existing content. 
If you are upgrading from a previous version of this integration, see [Breaking Changes](#breaking-changes-from-the-previous-version-of-this-integration-slack-v3).

## Configure Slack v3 in CortexSend messages and notifications to your Slack team.
...
| Types of Notifications to Send. | Types of notifications to send \(to individual users and to the dedicated Slack channel, if specified\).To enable use User \(User Name\) -&gt; User Preferences -&gt; Notifications | False |
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| user | The Slack user (username, email or user ID). | Required | 
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| channel_id | Deprecated. Use *conversation_id* instead. | Optional | 
| conversation_name | The name of the conversation. For channels, provide the channel name. For direct messages, use the username or display name. Either this or *conversation_id* is required. If both are provided, *conversation_id* takes precedence. | Optional | 
| conversation_id | The ID of the conversation. Either this or *conversation_name* is required. If both are provided, *conversation_id* takes precedence. | Optional | 
| limit | Set this argument to specify how many results to return. Default is 100. | Optional | 
| from_time | Lower bound for conversation history (sent to Slack 'oldest' query parameter). Returns messages with time stamp ≥ this value. Accepts Unix timestamp or ISO strings (e.g., "1727448000.000200", "2025-10-12T09:00:00+03:00"). Results are returned in descending order.<br/>. | Optional | 
| page_token | Token to retrieve the next page of results. | Optional | 
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| message | The message to send to the channel. Use issue fields as variables to enrich the text. Example: “${issue.details}”. | Required | 
| channel | The name of the Slack channel to which to send the message. | Required | 
...
## Breaking changes from the previous version of this integration - Slack v3
%%FILL HERE%%
