...
def test_fetch_events_(mocker):
    ...
    mocker.patch.object(Client, "_http_request", side_effect=[mock_response1.data, mock_response2.data])
    ...
    assert last_run == {
        "cursor": "mock_response2",  # The cursor was returned because we still haven't finished
    ...
    assert len(events) == 4  # 4 events according to the limit
    assert last_run == {
        "cursor": "mock_response4",  # The cursor was returned because we still haven't finished
        ...
        "newest_event_fetched": {"last_event_id": "11", "last_event_time": 11},
    }  # The last event we collected
    # (in this search)
