...
class Client(BaseClient):
    ...
    def do_request(self, method, url_suffix, json_data=None):
        ...
        res = self._http_request(
            method,
            f"SecureSphere/api/v1/{url_suffix}",
            json_data=json_data,
            headers={"Cookie": self.session_id},
            ok_codes=(200, 401, 406),
            resp_type="response",
        )
    ...
    def get_ip_group_entities(self, group_name, table_name):
        ...
        human_readable = tableToMarkdown(
            table_name, entries, removeNull=True, headers=["Type", "IpAddressFrom", "IpAddressTo", "NetworkAddress", "CidrMask"]
        )
        entry_context = {f"{INTEGRATION_CONTEXT_NAME}.IpGroup(val.Name===obj.Name)": {"Name": group_name, "Entries": entries}}
...
def generate_match_criteria(body, args):
    ...
    if ip_addresses_criteria_operation:
        ...
        ip_addresses_match_item = {"type": "sourceIpAddresses", "operation": ip_addresses_criteria_operation}
        if ip_groups:
...
def ip_group_list_entries_command(client, args):
    ...
    human_readable, entry_context, raw_res = client.get_ip_group_entities(group_name, f"IP group entries for {group_name}")
    return_outputs(human_readable, entry_context, raw_res)
...
def get_custom_policy_command(client, args):
    ...
    human_readable, entry_context, raw_res = client.get_custom_policy_outputs(policy_name, f"Policy data for {policy_name}")
    return_outputs(human_readable, entry_context, raw_res)
...
def create_ip_group_command(client, args):
    ...
    human_readable, entry_context, raw_res = client.get_ip_group_entities(group_name, f"Group {group_name} created successfully")
    return_outputs(human_readable, entry_context, raw_res)
...
def update_ip_group_command(client, args):
    ...
    human_readable, entry_context, raw_res = client.get_ip_group_entities(group_name, f"Group {group_name} updated successfully")
    return_outputs(human_readable, entry_context, raw_res)
...
def create_custom_policy_command(client, args):
    ...
    human_readable, entry_context, raw_res = client.get_custom_policy_outputs(
        policy_name, f"Policy {policy_name} created successfully"
    )
    return_outputs(human_readable, entry_context, raw_res)
...
def update_custom_policy_command(client, args):
    ...
    if apply_operation:
        body["applyTo"] = [
            {"operation": apply_operation, "siteName": site, "serverGroupName": server_group, "webServiceName": web_service}
        ]

    client.do_request("PUT", f"conf/policies/security/webServiceCustomPolicies/{policy_name}", json_data=body)
    human_readable, entry_context, raw_res = client.get_custom_policy_outputs(
        policy_name, f"Policy {policy_name} updated successfully"
    )
    return_outputs(human_readable, entry_context, raw_res)
