...
def get_devices_from_panw_iot_cloud(offset):
    ...
    resp = demisto.executeCommand(
        ...
        {"asset_type": "device", "offset": offset, "page_length": PAGE_SIZE, "using": PANW_IOT_INSTANCE},
    )
    if isError(resp[0]):
...
def query_servicenow_table(query):
    ...
    """
    sn_query_resp = demisto.executeCommand(
...
def get_servicenow_upsert_device_list(sn_query_result, device_list):
    ...
    """
    upsert_devices_resp = demisto.executeCommand(
...
def create_servicenow_record(insert_list):
    ...
    for x in range(len(insert_list)):
        ...
        demisto.executeCommand(
            ...
            {"table_name": SERVICENOW_TABLE_NAME, "fields": fields, "custom_fields": custom_fields, "using": SERVICENOW_INSTANCE},
        )
    ...
    while True:
        ...
        if len(sn_query_result) > 0:
            ...
            if insert_count == 0 and update_count == 0:
                err_msg = (
                    f"Error, Bulk Servicenow sync failed to get upsert " f"devices from PANW IoT cloud {upsert_devices_result}"
                )
