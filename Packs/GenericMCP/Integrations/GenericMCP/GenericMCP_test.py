...
def test_update_integration_context_oauth_flow_basic(mocker: MockerFixture):
    ...
    """
    mock_get_context = mocker.patch("GenericMCP.get_integration_context")
    mock_set_context = mocker.patch("GenericMCP.set_integration_context")
    mock_remove_empty = mocker.patch("GenericMCP.remove_empty_elements")
...
def test_get_client_metadata_basic_functionality(mocker: MockerFixture):
    ...
    """
    mock_demisto_urls = mocker.patch("GenericMCP.demisto.demistoUrls")
    ...
    assert result["client_name"] == "Cortex AgentiX"
...
class TestOAuthHandler:
    ...
    async def test_discover_oauth_protected_resource_metadata_success(
        ...
        mocker.patch("GenericMCP.httpx.AsyncClient", return_value=mock_client)
    ...
    async def test_discover_oauth_protected_resource_metadata_fallback_on_http_error(
        ...
        mocker.patch("GenericMCP.httpx.AsyncClient", return_value=mock_client)
        mocker.patch("GenericMCP.demisto.debug")

        # The test function is structured to fail with UnboundLocalError
        # due to a flaw in the original GenericMCP.py source (variable not initialized
    ...
    async def test_oauth_register_client_success(self, mocker: MockerFixture, mock_oauth_handler: OAuthHandler):
        ...
        mocker.patch("GenericMCP.httpx.AsyncClient", return_value=mock_client)
        mocker.patch("GenericMCP.get_client_metadata", return_value={"client_name": "Test Client"})
...
class TestClient:
    ...
    async def test_resolve_headers_basic_auth(self, mocker: MockerFixture, mock_client_instance: Client):
        ...
        """
        mocker.patch("GenericMCP.demisto")
    ...
    async def test_generate_oauth_headers_with_valid_existing_token(self, mocker: MockerFixture):
        ...
        """
        mocker.patch(
            "GenericMCP.get_integration_context",
            return_value={
    ...
    async def test_generate_oauth_headers_client_credentials_flow(self, mocker: MockerFixture):
        ...
        """
        mocker.patch("GenericMCP.get_integration_context", return_value={"access_token": "", "expires_in": 0})
        mocker.patch("GenericMCP.update_integration_context_oauth_flow")
        mocker.patch("GenericMCP.demisto.debug")
    ...
    async def test_list_tools_success(self, mocker: MockerFixture, mock_client_instance: Client):
        ...
        mock_client_session = mocker.patch("GenericMCP.ClientSession")
    ...
    async def test_call_tool_success(self, mocker: MockerFixture, mock_client_instance: Client):
        ...
        mocker.patch("GenericMCP.tableToMarkdown", return_value="Tool executed successfully")
