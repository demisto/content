...
class Client(BaseClient):
    ...
    def http_request_indicators(self):
        res = requests.get(url=self.base_url, verify=self._verify)
        try:
    ...
    def http_request(self, url_endpoint, params: dict = None):
        ...
        res = requests.request(
            method="GET", url=urljoin(self.base_url, url_endpoint), verify=self._verify, headers=self.headers, params=params
        )
        return res
...
def get_indicators_command(client: Client, params: dict, args: dict):
    ...
    human_readable = tableToMarkdown(
        "Indicators from Github Maltrail:",
        entry_result,
...
def main():
    ...
    # Switch case
    commands = {"test-module": test_module_command, "gh-maltrail-get-indicators": get_indicators_command}
