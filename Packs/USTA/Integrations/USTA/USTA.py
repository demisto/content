...
def test_module() -> str:
    ...
    try:
        url = demisto.params().get("url")[:-1] if str(demisto.params().get("url")).endswith("/") else demisto.params().get("url")
        CREDENTIALS = demisto.params().get("apikey")
        # AUTH_HEADERS = {'Content-Type': 'application/json'}

        params = "TOKEN " + CREDENTIALS
        headers = {"Authorization": params, "Content-Type": "application/json"}
...
class Client(BaseClient):
    ...
    def get_malicious_url(self, param: dict) -> Dict[str, Any]:
        ...
        :rtype: ``Dict[str, Any]``
        """
        return self._http_request(method="GET", url_suffix="/threat-stream/malicious-urls", params=param)
    ...
    def get_malware_hash(self, param: dict) -> Dict[str, Any]:
        ...
        :rtype: ``Dict[str, Any]``
        """
        return self._http_request(method="GET", url_suffix="/threat-stream/malware-hashs", params=param)
    ...
    def get_phishing_sites(self, param: dict) -> Dict[str, Any]:
        ...
        :rtype: ``Dict[str, Any]``
        """
        return self._http_request(method="GET", url_suffix="/threat-stream/phishing-sites", params=param)
    ...
    def get_identity_leaks(self, param: dict) -> Dict[str, Any]:
        ...
        :rtype: ``Dict[str, Any]``
        """
        return self._http_request(method="GET", url_suffix="/threat-stream/identity-leaks", params=param)
    ...
    def get_stolen_client_accounts(self, param: dict) -> Dict[str, Any]:
        ...
        :rtype: ``Dict[str, Any]``
        """
        return self._http_request(method="GET", url_suffix="/threat-stream/stolen-client-accounts", params=param)
    ...
    def get_domain(self, param: dict) -> Dict[str, Any]:
        ...
        return self._http_request(method="POST", url_suffix="/threat-stream/domain", json_data=param)
    ...
    def get_ip_address(self, param: dict) -> Dict[str, Any]:
        ...
        return self._http_request(method="POST", url_suffix="/threat-stream/ip-address", json_data=param)
...
def search_malicious_urls(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="USTA.Malicious_URL", outputs_key_field="url", outputs=maliciousURL
    )
...
def search_malware_hashs(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    param = {"md5": md5, "sha1": sha1, "tag": tag, "start": startDate, "end": endDate, "format": formatType}
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="USTA.Malware_Hash", outputs_key_field="md5", outputs=malwareHash
    )
...
def search_phishing_sites(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    param = {"status": status, "source": source, "page": page}
    ...
    return CommandResults(
        readable_output=readable_output, outputs_prefix="USTA.Phishing_Sites", outputs_key_field="results", outputs=phishingSites
    )
...
def search_stolen_client_accounts(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    param = {"username": username, "password": password, "source": source, "start": startDate, "end": endDate}
...
def send_referrer_url() -> CommandResults:
    ...
    return CommandResults(readable_output=readable_output, outputs=referredResult)
...
def search_specific_identity_leaks() -> CommandResults:
    ...
    for i in range(len(identityList)):
        mappingDict = {"Identity": identityList[i], "Identity Hash": hashedIdentities[i]}
        mappingHashIdentity.append(mappingDict)

    data = {"signatures": hashedIdentities}
    ...
    return CommandResults(readable_output=readable_output, outputs=identityLeaks)
...
def close_incident() -> CommandResults:
    ...
    return CommandResults(readable_output=readable_output, outputs=closedIncident)
...
def timeToEpoch(time: str):
    ...
    if len(timeParameterList) != 5:
        ...
        epochTime = datetime(
            ...
            int(timeParameterList[1]),
            int(timeParameterList[2]),
            int(timeParameterList[3]),
            int(timeParameterList[4]),
        ).strftime("%s")
        return epochTime
