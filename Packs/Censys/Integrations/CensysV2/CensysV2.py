...
def censys_view_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    if index == "ipv4":
        content = {
            ...
            "Protocols": ", ".join(
                [f"{service.get('port')}/{service.get('service_name')}" for service in result.get("services", [])]
            ),
        ...
        content = {
            ...
            "Browser Trust": [
                f"{name}: {'Valid' if val.get('is_valid') else 'Invalid'}" for name, val in result.get("validation", {}).items()
            ],
...
def censys_search_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    if index == "ipv4":
            ...
            contents.append(
                {
                    "IP": hit.get("ip"),
                    "Services": ", ".join(
                        [f"{service.get('port')}/{service.get('service_name')}" for service in hit.get("services", [])]
                    ),
        ...
        return CommandResults(
            readable_output=human_readable, outputs_prefix="Censys.Search", outputs_key_field="ip", outputs=hits, raw_response=res
        )
    else:
...
def search_certs_command(client: Client, args: dict[str, Any], query: str, limit: Optional[int]):
    ...
    contents = []
    data = {"query": query, "page": int(args.get("page", 1)), "fields": fields, "flatten": False}
...
def main() -> None:
    ...
    try:
        client = Client(base_url=base_url, auth=(username, password), verify=verify_certificate, proxy=proxy)
