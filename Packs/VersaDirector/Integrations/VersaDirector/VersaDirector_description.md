...
This integration supports both Basic and Advanced Authentication methods.
...
This authentication method is based on the **Username** and **Password** parameters only.

1. Enter the required values for the **Username** and **Password** parameters in the Versa Director instance configuration window.
...
1. Enter valid **Username** and **Password** parameters and, optionally, **Client ID** and **Client Secret**, in the Versa Director instance configuration window.
...
1. Enter valid **Username** and **Password** parameters and check the **Use Basic Authentication** checkbox in the Versa Director instance configuration window.
2. In the Playground, run the `!vd-auth-start` command, optionally passing the **auth_client_name** and **description** arguments.
...
3. Uncheck the **Use Basic Authentication** checkbox and, optionally, clear the previously specified **Username** and **Password** parameter fields.
...
1. Directly specify the **Auth Token** parameter in the Versa Director instance configuration window.
...
1. If an Auth Token is created using the `!vd-auth-start` command, a process is initiated each time a command is run to check if the Auth Token is valid. If not, an attempt will be made to send a request for a new Auth Token using the Refresh Token. If successful, the Integration Context will be updated.
