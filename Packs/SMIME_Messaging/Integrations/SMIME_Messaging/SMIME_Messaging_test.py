...
from SMIME_Messaging import Client, decode_str, decrypt_email_body, encrypt_email_body, sign_and_encrypt, sign_email, verify
...
test_decode_data = [
    (b"Za\xbf\xf3\xb3\xe6 g\xea\xb6l\xb1 ja\xbc\xf1", "Zażółć gęślą jaźń", "", "iso-8859-2"),
    (b"\xe3\x81\x8c\xe3\x81\x84\xe3\x83\xa2", "がいモ", "", ""),
    (b"\xd7\xa9\xd7\x9c\xd7\x95\xd7\x9d", "שלום", "", ""),
]
...
def test_sign():
    ...
    assert (
        'MIME-Version: 1.0\nContent-Type: multipart/signed; protocol="application/x-pkcs7-signature"; micalg="sha-256";' in sign
    )
...
def test_verify_der(mocker):
    ...
    """
    mocker.patch(
        "SMIME_Messaging.fileResult",
        return_value={"Contents": "", "ContentsFormat": "text", "Type": "", "File": "", "FileID": ""},
    )
    mocker.patch.object(
        demisto, "getFilePath", return_value={"name": "signed-binary-format.p7m", "path": "./test_data/signed-binary-format.p7m"}
    )
...
def test_decrypt_der(mocker):
    ...
    mocker.patch.object(
        demisto,
        "getFilePath",
        return_value={"name": "encrypted-binary-format.p7m", "path": "./test_data/encrypted-binary-format.p7m"},
    )
...
def test_sign_and_encrypt(mocker, sign, encrypt, recipients, cc, bcc, create_file, attachments):
    ...
    """
    mocker.patch.object(
        demisto, "getFilePath", side_effect=lambda file_name: {"name": file_name, "path": f"./test_data/{file_name}"}
    )
...
def test_sign_encrypt_decrypt_verify(mocker):
    ...
    try:
        mocker.patch.object(
            demisto, "getFilePath", return_value={"name": "temp_encrypted_signed.p7", "path": "./temp_encrypted_signed.p7"}
        )
        decrypted_out = decrypt_email_body(client, {})[0].to_context()
    ...
    try:
        mocker.patch.object(
            demisto, "getFilePath", return_value={"name": "temp_decrypted_signed.p7", "path": "./temp_decrypted_signed.p7"}
        )
