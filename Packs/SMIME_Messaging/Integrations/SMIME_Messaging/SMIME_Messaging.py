...
def handle_attachment(attachment_part: Message) -> None:
    ...
    if file_name.lower().endswith(".p7") or file_name.lower().endswith(".p7", 0, -1):
...
def create_message_with_attachments(message: str, attach_ids: list[str]) -> str:
    ...
    for attach_id in attach_ids:
        try:
            ...
            raise Exception(f"Error while opening attachment id {attach_id}: {ex!s}")
...
def decode_str(decrypted_text: bytes, encoding: str) -> tuple[str, str]:
    ...
    if not encoding:
        with warnings.catch_warnings(record=True) as e:
            ...
            if e:
                msg = (
                    f"Note: encoding detection ended with warning: {e[0].message} Characters may be missing."
