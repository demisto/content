        ...
        try:
            ...
            if hasattr(e.res, "status_code"):
                ...
                if res_status == 404:
                    ...
                    demisto.debug("The status code is 500")
                    if self.should_error:
                        raise e
                    result = {}
                elif res_status == 504:
...
def create_pulse_by_ec(entry: dict) -> dict:
    pulse_by_ec = {
        "ID": entry.get("id"),
        "Author": {"ID": entry.get("author", {}).get("id"), "Username": entry.get("author", {}).get("username")},
...
def relationships_manager(
    ...
    if client.max_indicator_relationships > 0:
        ...
        _, _, passive_dns_raw_response = alienvault_get_passive_dns_data_by_indicator_command(
...
def create_relationships(
    client: Client, relevant_field: list, entity_a: str, entity_a_type: str, relevant_id: str, entity_b_type: str
):
    """
        ...
        if raw_response and raw_response != 404:
            ...
            command_results.append(
                create_indicator_result_with_dbotscore_unknown(
                    indicator=ip_, indicator_type=DBotScoreType.IP, reliability=client.reliability
                )
            )
...
def file_command(client: Client, file: str) -> List[CommandResults]:
    ...
    for hash_ in hashes_list:
        ...
        if (
            ...
            and (shortcut := dict_safe_get(raw_response_analysis, ["analysis", "info", "results"], {}))
            ...
            context = {
                ...
                "Malicious": {"PulseIDs": raw_response_general.get("pulse_info", {}).get("pulses")},
            }
            ...
            command_results.append(
                create_indicator_result_with_dbotscore_unknown(
                    indicator=hash_, indicator_type=DBotScoreType.FILE, reliability=client.reliability
                )
            )
    if not command_results:
        return [CommandResults(f"{INTEGRATION_NAME} - Could not find any results for given query")]
        ...
        if raw_response:
            if raw_response == 404:
                ...
                if client.create_relationships:
                    ...
                    relationships = create_relationships(
                        client, extract_attack_ids(raw_response), url, FeedIndicatorType.URL, "display_name", indicator
                    )
...
def alienvault_search_cve_command(client: Client, cve_id: str) -> tuple[str, dict, dict]:
    ...
    if raw_response and raw_response != 404:
        ...
        human_readable = tableToMarkdown(t=context_entry.get(outputPaths.get("cve")), name=title)
...
def alienvault_get_related_urls_by_indicator_command(
    ...
    if limit:
        params["limit"] = limit
    raw_response = client.query(section=indicator_type, argument=indicator, sub_section="url_list", params=params)
    if raw_response and raw_response != 404:
        ...
        context: dict = {"AlienVaultOTX.URL(val.URL.Data && val.URL.Data == obj.URL.Data)": context_entry}
...
def alienvault_get_related_hashes_by_indicator_command(
    ...
    if limit:
        params["limit"] = limit
    raw_response = client.query(section=indicator_type, argument=indicator, sub_section="malware", params=params)
    if raw_response and raw_response != 404:
...
def alienvault_get_passive_dns_data_by_indicator_command(
    ...
    if limit:
        params["limit"] = limit
    raw_response = client.query(section=indicator_type, argument=indicator, sub_section="passive_dns", params=params)
    if raw_response and raw_response != 404:
        ...
        context_entry: dict = {
            ...
            "val.PassiveDNS.IP && val.PassiveDNS.IP == obj.PassiveDNS.IP)": create_list_by_ec(
                list_entries=raw_response.get("passive_dns", {}), list_type="passive_dns"
            )
        }
        human_readable = tableToMarkdown(
            t=context_entry.get(
...
def alienvault_search_pulses_command(client: Client, page: str) -> tuple[str, dict, dict]:
    ...
    Returns:
        Outputs
    """
    raw_response = client.query(section="search", sub_section="pulses", params={"page": page})
    if raw_response and raw_response != 404:
...
def alienvault_get_pulse_details_command(client: Client, pulse_id: str) -> tuple[str, dict, dict]:
    ...
    if raw_response and raw_response != 404:
        ...
        human_readable = tableToMarkdown(t=context_entry.get("AlienVaultOTX.Pulses(val.ID && val.ID == obj.ID)"), name=title)
...
def main():
    ...
    try:
        if command == f"{INTEGRATION_COMMAND_NAME}-search-ipv6":
            return_results(ip_command(client=client, ip_address=demisto.args().get("ip"), ip_version="IPv6"))
        elif command == "ip":
            return_results(ip_command(client=client, ip_address=demisto.args().get("ip"), ip_version="IPv4"))
