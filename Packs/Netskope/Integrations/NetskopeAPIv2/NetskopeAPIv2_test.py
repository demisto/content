...
def test_list_client(client, requests_mock):
    ...
    result = list_client_command(client, {"filter": "userName eq someUserName", "limit": 5})
...
def test_list_dlp_incident(client, requests_mock):
    ...
    requests_mock.get(f"{SERVER_URL}api/v2/events/dataexport/events/incident", json=mock_response)
...
def test_fetch_incidents(client, requests_mock):
    ...
    requests_mock.get(f"{SERVER_URL}api/v2/events/data/application", json=event_mock_response)
    requests_mock.get(f"{SERVER_URL}api/v2/events/data/page", json=event_mock_response)
    requests_mock.get(f"{SERVER_URL}api/v2/events/dataexport/events/incident", json=dlp_incidents_mock_response)
    ...
    assert incidents == [
        ...
        {
            "name": "dlp_incident ID: 01",
            "occurred": "2024-03-20T10:01:00Z",
...
def test_set_and_get_demisto_integration_context():
    ...
    """
    from NetskopeAPIv2 import get_demisto_integration_context, set_demisto_integration_context
...
def test_remove_duplicates():
    ...
    dicts_list = [
        {"object_id": "1", "value": "A"},
        {"object_id": "2", "value": "B"},
        {"object_id": "1", "value": "C"},  # Duplicate object_id
        {"object_id": "3", "value": "D"},
    ]

    expected_output = [
        {"object_id": "1", "value": "C"},  # Last occurrence of object_id "1"
        {"object_id": "2", "value": "B"},
        {"object_id": "3", "value": "D"},
    ]
...
def test_get_modified_remote_data(client):
    ...
    with patch("NetskopeAPIv2.Client") as MockClient:
        ...
        with patch("NetskopeAPIv2.get_demisto_integration_context") as mock_get_context:
            mock_get_context.return_value = 123456789

            with patch("NetskopeAPIv2.get_hourly_timestamps") as mock_get_hourly_timestamps:
...
def test_get_remote_data_command():
    ...
    with patch("NetskopeAPIv2.get_demisto_integration_context") as mock_get_context:
        mock_get_context.return_value = [{"incident_type": "dlp_incident", "object_id": "incident123", "status": "closed"}]

        result = get_remote_data_command(args, {"close_incident": True})

        expected_response = GetRemoteDataResponse(
            {"incident_type": "dlp_incident", "object_id": "incident123", "status": "closed"},
            [
                {
                    "Type": 1,
                    "Contents": {"dbotIncidentClose": True, "closeReason": "Closed from Netskope."},
                    "ContentsFormat": "json",
                }
            ],
        )
...
def test_update_remote_system():
    ...
    with patch("NetskopeAPIv2.Client") as MockClient:
        ...
        with patch("NetskopeAPIv2.UpdateRemoteSystemArgs") as mock_UpdateRemoteSystemArgs:
