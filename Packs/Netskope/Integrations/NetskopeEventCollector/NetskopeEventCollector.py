...
class Client(BaseClient):
    ...
    def get_alerts_request_v1(self, last_run: dict, limit: Optional[int] = None) -> list[Any] | Any:
        """
...
def test_module(client: Client, api_version: str, last_run: dict) -> str:
    ...
    events = fetch_events_command(client, api_version, last_run, max_fetch=1)
    if events:
        return 'ok'
    return 'Test failed - Make sure the URL and the API Token are correctly set.'


def get_events_v1(client: Client, last_run: dict, limit: Optional[int] = None) -> List[Any] | Any:
    """
...
def get_events_v2(client, last_run: dict, limit: Optional[int] = None, is_test: bool = False) -> List[Any] | Any:
    """
    Get all events extracted from Saas traffic and or logs.
    Args:
        client (Client): Netskope Client.
...
def main() -> None:  # pragma: no cover
    ...
    try:
        ...
        if not last_run:
            first_fetch = int(arg_to_datetime(first_fetch).timestamp() // 100)
            last_run = {
                'alert': first_fetch,
