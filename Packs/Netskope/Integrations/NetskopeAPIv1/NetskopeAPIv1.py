...
class Client(BaseClient):
    ...
    def list_quarantined_files_request(
        self, start_time: int | None = None, end_time: int | None = None, limit: int | None = None, skip: int | None = None
    ) -> dict[str, Any]:
        """
        ...
        body = remove_empty_elements(
            {"starttime": start_time, "endtime": end_time, "limit": limit, "skip": skip, "op": "get-files"}
        )
    ...
    def get_quarantined_file_request(self, quarantine_profile_id: str, file_id: str) -> bytes:
        ...
        body = {"quarantine_profile_id": quarantine_profile_id, "file_id": file_id, "op": "download-url"}

        return self._http_request(method="POST", url_suffix="quarantine", json_data=body, resp_type="content")

    def update_quarantined_file_request(self, quarantine_profile_id: str, file_id: str, action: str) -> None:
        """
        ...
        body = {"quarantine_profile_id": quarantine_profile_id, "file_id": file_id, "action": action, "op": "take-action"}
    ...
    def list_clients_request(self, query: str | None = None, limit: int | None = None, skip: int | None = None) -> dict[str, Any]:
        """
...
def validate_time_arguments(
    ...
    combination = (all((start_time, end_time)), all((insertion_start_time, insertion_end_time)), bool(timeperiod))

    if not any(combination):
        raise DemistoException(
            "Missing time arguments. Please provide start_time and end_time, "
...
def validate_fetch_params(
    max_fetch: int, max_events_fetch: int, fetch_events: bool, first_fetch: str, event_types: list[str]
) -> None:
    """
    ...
    if fetch_events and max_events_fetch > MAX_EVENTS_FETCH:
        return_error(f"The Maximum number of events for each incident per fetch should not exceed {MAX_EVENTS_FETCH}.")
...
def list_events_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    response = client.list_events_request(
        query=query,
...
def list_alerts_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    response = client.list_alerts_request(
        query=query,
        alert_type=alert_type,
        acked=acked,
        timeperiod=timeperiod,
...
def list_quarantined_files_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    response = client.list_quarantined_files_request(start_time=start_time, end_time=end_time, limit=limit, skip=skip)
    ...
    readable_header = get_pagination_readable_message("Quarantined Files List:", page=page, limit=limit)
    readable_output = tableToMarkdown(
        readable_header,
        outputs,
        removeNull=True,
        headers=["quarantine_profile_id", "quarantine_profile_name", "file_id", "original_file_name", "policy"],
        headerTransform=string_to_table_header,
    )

    return CommandResults(
        outputs_prefix="Netskope.Quarantine",
...
def get_quarantined_file_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    response = client.get_quarantined_file_request(quarantine_profile_id=quarantine_profile_id, file_id=file_id)
...
def update_quarantined_file_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    client.update_quarantined_file_request(quarantine_profile_id=quarantine_profile_id, file_id=file_id, action=action)
...
def update_url_list_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    return CommandResults(
        outputs_prefix="Netskope.URLList", outputs_key_field="name", outputs=outputs, readable_output=readable_output
    )
...
def update_file_hash_list_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    return CommandResults(
        outputs_prefix="Netskope.FileHashList", outputs_key_field="name", outputs=outputs, readable_output=readable_output
    )
...
def list_host_associated_user_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    response = client.list_clients_request(query=f"host_info.hostname eq {hostname}", limit=limit, skip=skip)

    outputs: list = sum((client["attributes"].get("users") for client in response["data"]), [])
    for output in outputs:
        output["user_id"] = output["_id"]

    readable_header = get_pagination_readable_message(f"Users Associated With {hostname}:", page=page, limit=limit)
    readable_output = tableToMarkdown(
        readable_header,
        outputs,
        removeNull=True,
...
def list_user_associated_host_command(client: Client, args: dict[str, str]) -> CommandResults:
    ...
    readable_header = get_pagination_readable_message(f"Hosts Associated With {username}:", page=page, limit=limit)
    readable_output = tableToMarkdown(
        readable_header,
        outputs,
        removeNull=True,
...
def test_module(
    client: Client, max_fetch: int, first_fetch: str, fetch_events: bool, max_events_fetch: int, event_types: list[str]
) -> str:
    """
    ...
    client.list_alerts_request(
        limit=1, skip=0, start_time=date_to_seconds_timestamp(datetime.now()), end_time=date_to_seconds_timestamp(datetime.now())
    )
    return "ok"


def fetch_multiple_type_events(
    client: Client, max_fetch: int, start_time: int, event_types: list[str], query: str | None
) -> list[dict[str, Any]]:
    """
    ...
    for event_type in event_types:
        new_events = client.list_events_request(
            start_time=start_time,
...
def fetch_incidents(
    ...
    alerts = client.list_alerts_request(
        start_time=last_alert_time,
        end_time=date_to_seconds_timestamp(datetime.now()),
        limit=max_fetch,
        query=alerts_query,
        unsorted=False,
    ...
    if fetch_events:
        events = fetch_multiple_type_events(
            client, max_fetch=max_events_fetch, start_time=last_event_time, event_types=event_types, query=events_query
        )
    else:
        events = []
    ...
    for alert in alerts:
        ...
        incidents.append(
            {"name": alert["alert_name"], "occurred": timestamp_to_datestring(alert["timestamp"]), "rawJSON": json.dumps(alert)}
        )

    for event in events:
        incidents.append(
            {"name": event["event_id"], "occurred": timestamp_to_datestring(event["timestamp"]), "rawJSON": json.dumps(event)}
        )
