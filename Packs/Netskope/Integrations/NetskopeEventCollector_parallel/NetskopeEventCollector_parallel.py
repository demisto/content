...
class Client(BaseClient):
    ...
    def perform_data_export(self, endpoint, _type, index_name, operation):
        ...
        demisto.debug(response.content)
...
def honor_rate_limiting(headers, endpoint):
    ...
    try:
        if remaining := headers.get(RATELIMIT_REMAINING):
            ...
            demisto.debug(f'Sleeping for 1 seconds between calls')
            time.sleep(1)
...
def test_module(client: Client, api_version: str, last_run: dict, max_fetch: int) -> str:
    fetch_events_command(client, api_version, last_run, max_fetch=1000, is_command=True)
    return 'ok'


def get_all_events_v1(client: Client, last_run: dict, limit: int, is_command: bool) -> Tuple[list, dict]:
    """
...
def get_specific_event_type(client, event_type, operation, limit, start_time):
    wait_time = 0
    events = []
    instance_name = demisto.callingContext.get('context', {}).get('IntegrationInstance')
    thread_prefix = f'Thread #{threading.get_native_id()} -'

    while len(events) < limit:
        end_time = datetime.utcnow()
        secs_from_beginning = (end_time - start_time).seconds
        demisto.debug(f'{thread_prefix} Execution length so far is {secs_from_beginning} secs')

        if secs_from_beginning > EXECUTION_TIMEOUT_SECONDS:
            demisto.debug(f'{thread_prefix} Timeout reached, stopped pulling events')
            break

        # Wait time between queries
        if wait_time:
            demisto.debug(f'{thread_prefix} Going to sleep between queries, wait_time is {wait_time} seconds')
        ...
        try:
            ...
            demisto.error(f'{thread_prefix} received error: {e}')
            return []

        results = response.get('result', [])
        demisto.debug(f'{thread_prefix} The number of received events - {len(results)}')
        operation = 'next'
        ...
        demisto.debug(f'{thread_prefix} Wait time is {wait_time} seconds')
    ...
    demisto.debug(f'{thread_prefix} __[{event_type}]__ - Total events fetched this round: {len(events)}')
    if events:
        event_times = f'{thread_prefix} __[{event_type}]__ - First event: {events[0].get("timestamp")} __[{event_type}]__ - Last event: {events[-1].get("timestamp")}'
        demisto.debug(event_times)

    return events


def get_all_events_v2(client: Client, last_run: dict, limit: int, is_command: bool) -> Tuple[list, dict]:
    if limit is None:
        limit = MAX_EVENTS_PAGE_SIZE
    ...
    with ThreadPoolExecutor(max_workers=len(ALL_SUPPORTED_EVENT_TYPES_v2 + ALL_SUPPORTED_ALERT_TYPES_v2)) as executor:
        future_event_types_results = []
        for event_type in ALL_SUPPORTED_EVENT_TYPES_v2 + ALL_SUPPORTED_ALERT_TYPES_v2:
            future_event_types_results.append(executor.submit(
                get_specific_event_type,
                client=client,
        ...
        for type_results in as_completed(future_event_types_results):
            demisto.debug('Got into final loop')
            if thread_exception := type_results.exception():
                raise thread_exception

            events = type_results.result()
...
def fetch_events_command(client, api_version, last_run, max_fetch, is_command):  # pragma: no cover
    if api_version == 'v1':
        events, new_last_run = get_all_events_v1(client, last_run=last_run, limit=max_fetch,
                                                 is_command=is_command)
    else:
        events, new_last_run = get_all_events_v2(client, last_run=last_run, limit=max_fetch,
                                                 is_command=is_command)
