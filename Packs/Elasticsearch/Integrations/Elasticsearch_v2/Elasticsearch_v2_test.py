...
def test_get_indices_statistics_command(mocker):
    """
    Given
      - index name, document in JSON format, id of document
    ...
    mocker.patch.object(Elasticsearch_v2.Elasticsearch, '__init__', return_value=None)
    command_result = Elasticsearch_v2.get_indices_statistics_command({'limit': 50, 'all_results': False}, '')
    expected_index_context = [{'Name': 'index_name1', 'Status': 'open', 'Health': 'yellow', 'UUID': '1111', 'Documents Count': 2, 'Documents Deleted': 0}, {
        'Name': 'index_name2', 'Status': 'closed', 'Health': 'green', 'UUID': '2222', 'Documents Count': 40, 'Documents Deleted': 2}]
