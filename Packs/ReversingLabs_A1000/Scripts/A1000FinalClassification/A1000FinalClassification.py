...
def overall_classification_command(args: Dict[str, Any]) -> CommandResults:
    ...
    markdown = (
        f"### ReversingLabs A1000 overall classification for sha1: {a1000_classification_report.get('sha1')}:"
        f"\n **Threat status:** {overall_classification}\n"
    )

    if overall_classification == "UNKNOWN":
        markdown += (
            "**NOTE:** The file is not yet classified because this is the first time TiCloud has seen the"
    ...
    command_results = CommandResults(outputs_prefix="ReversingLabs", indicator=common_file, readable_output=markdown)
...
def main():
    try:
        ...
        return_error(f"Failed to execute A1000FinalClassification. Error: {ex!s}")
