...
class TestClient:
    ...
    def test_rate_limit(self, requests_mock):
        ...
        requests_mock.get(
            f"{DNSDB.DEFAULT_DNSDB_SERVER}/dnsdb/v2/rate_limit?swclient={DNSDB.SWCLIENT}&version={DNSDB.VERSION}", json={}
        )
...
class TestRDataCommand:
    ...
    @staticmethod
    def _run_test(
        requests_mock, args: dict, input: list, expected_readable: str, expected_output_prefix: str, expected_outputs: list
    ):
        client = DNSDB.Client(DNSDB.DEFAULT_DNSDB_SERVER, "")
        requests_mock.get(
            f'{DNSDB.DEFAULT_DNSDB_SERVER}/dnsdb/v2/lookup/rdata/{args["type"]}/{args["value"]}'
            f'?limit={args["limit"]}'
...
class TestSummarizeRDataCommand:
    ...
    @staticmethod
    def _run_test(
        requests_mock, args: dict, input: dict, expected_readable: str, expected_output_prefix: str, expected_outputs: dict
    ):
        client = DNSDB.Client(DNSDB.DEFAULT_DNSDB_SERVER, "")
        requests_mock.get(
            f'{DNSDB.DEFAULT_DNSDB_SERVER}/dnsdb/v2/summarize/rdata/{args["type"]}/{args["value"]}'
            f'?limit={args["limit"]}'
...
class TestRRSetCommand:
    ...
    def _run_test(
        ...
        client = DNSDB.Client(DNSDB.DEFAULT_DNSDB_SERVER, "")
        requests_mock.get(
            f'{DNSDB.DEFAULT_DNSDB_SERVER}/dnsdb/v2/lookup/rrset/name/{DNSDB.quote(args["owner_name"])}'
            f'?limit={args["limit"]}'
            f'&swclient={DNSDB.SWCLIENT}&version={DNSDB.VERSION}',
            text=_saf_wrap(input),
        )
...
class TestSummarizeRRSetCommand:
    ...
    @staticmethod
    def _run_test(
        requests_mock, args: dict, input: list, expected_readable: str, expected_output_prefix: str, expected_outputs: dict
    ):
        client = DNSDB.Client(DNSDB.DEFAULT_DNSDB_SERVER, "")
        requests_mock.get(
            f'{DNSDB.DEFAULT_DNSDB_SERVER}/dnsdb/v2/summarize/rrset/name/{args["owner_name"]}'
            f'?limit={args["limit"]}'
            f'&max_count={args["max_count"]}'
            f'&swclient={DNSDB.SWCLIENT}&version={DNSDB.VERSION}',
            text=_saf_wrap(input),
...
def _saf_wrap(records):
    return "\n".join(['{"cond":"begin"}'] + [f'{{"obj":{r}}}' for r in records] + ['{"cond":"succeeded"}'])
