...
| --- | --- | --- | --- |
| search_name | The name of the compliance search. |  | Optional |
...
| exchange_location | Comma-separated list of mailboxes/distribution groups to include, or use the value "All" to include all. |  | Optional |
...
| delete_type | Specify the delete type to perform on the search results. Possible values are Hard and Soft or leave empty to select manually. \(Hard = Unrecoverable, Soft=Recoverable\) |  | Optional |
...
| search_this_week | Limit the search to the current week \(true/false\). |  | Optional |
...
![O365 - Security And Compliance - Search And Delete_searchlastweek](../doc_files/O365_-_Security_And_Compliance_-_Search_And_Delete_searchlastweek.png)
