...
def detect_qrcode_image(path):
    ...
    return CommandResults(outputs_prefix="QR.Data", outputs=result)


def main():
    try:
        entry_id = demisto.args().get("entry_id")
        file_path = demisto.executeCommand("getFilePath", {"id": entry_id})[0].get("Contents").get("path")
        return_results(detect_qrcode_image(file_path))
    except Exception as ex:
        demisto.error(traceback.format_exc())  # print the traceback
        return_error(f"Failed to execute qrcodereader. Error: {ex!s}")
