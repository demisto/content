...
Palo Alto Networks Cortex XSOAR Data Lake provides cloud-based, centralized log storage and aggregation for your on premise, virtual (private cloud and public cloud) firewalls, for Prisma Access, and for cloud-delivered services such as Cortex XDR
This integration was integrated and tested with version 2 of Cortex XSOAR Data Lake

---

## Configure Cortex XSOAR Data Lake on Cortex XSOAR
---
...
6. Search for Cortex XSOAR Data Lake.
...
| CDL.Logging.Traffic.LogTime | date | Time the log was received in Cortex XSOAR Data Lake. | 
...
| CDL.Logging.URL.LogTime | String | Time the log was received in Cortex XSOAR Data Lake. | 
...
its standard port. | 
| CDL.Logging.File.IsDupLog | Boolean | Indicates whether this log data is available in multiple locations, such as from Cortex XSOAR Data Lake as well as from an on-premise log collector. | 
| CDL.Logging.File.LogTime | Date | Time the log was received in Cortex XSOAR Data Lake. | 
...
| CDL.Logging.File.Protocol | String | IP protocol associated with the session. | 
| CDL.Logging.File.CustomerID | Number | The ID that uniquely identifies the Cortex XSOAR Data Lake instance which received this log record. | 
...
* The provided authentication items ([configuration step 4](#configure-cortex-data-lake-on-cortex-xsoar)) can only be used once for each Cortex XSOAR Data Lake tenant (but can be shared for different Cortex XSOAR instances). Trying to re-generate those items will revoke any previously generated set of authentication items.