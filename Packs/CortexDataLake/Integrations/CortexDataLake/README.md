...
* The provided authentication items ([configuration step 4](#configure-cortex-data-lake-on-cortex-xsoar)) are limited for each cortex data lake tenant (but can be shared for different XSOAR instances). Trying to re-generate those items will revoke the first generated set of authentication items.