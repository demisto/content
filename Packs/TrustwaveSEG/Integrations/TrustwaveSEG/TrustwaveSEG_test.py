...
def test_trustwave_seg_get_version_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_get_version_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
...
def test_trustwave_seg_statistics_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_statistics_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    ...
    requests_mock.get(
        f"https://1.1.1.1:2/seg/api/console/array/stats?fromtime={start_time}&totime={end_time}", json=mock_response
    )
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
    response = trustwave_seg_statistics_command(client, start_time="2021-04-04 07:35:58 PM", end_time="2021-05-05 07:35:58 PM")
...
def test_trustwave_seg_list_classifications_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_list_classifications_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.get("https://1.1.1.1:2/seg/api/quarantine/classifications", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
...
def test_trustwave_seg_automatic_config_backup_list_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_automatic_config_backup_list_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.get("https://1.1.1.1:2/seg/api/services/config/autobackups", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
...
def test_trustwave_seg_automatic_config_backup_restore_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_automatic_config_backup_restore_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.put("https://1.1.1.1:2/seg/api/services/config/autobackups/restore", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
    response = trustwave_seg_automatic_config_backup_restore_command(client, timeout=30, name="xxx", include_dkim=False)
...
def test_trustwave_seg_automatic_config_backup_run_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_automatic_config_backup_run_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.put("https://1.1.1.1:2/seg/api/services/config/autobackups/backup", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
    response = trustwave_seg_automatic_config_backup_run_command(client, timeout=30, include_dkim=False)
...
def test_trustwave_seg_list_servers_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_list_servers_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.get("https://1.1.1.1:2/seg/api/services/servers/", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
...
def test_trustwave_seg_get_server_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_get_server_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.get("https://1.1.1.1:2/seg/api/services/servers/1", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
...
def test_trustwave_seg_list_alerts_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_list_alerts_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.get("https://1.1.1.1:2/seg/api/console/alerts?activeonly=False", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
...
def test_trustwave_seg_list_quarantine_folders_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_list_quarantine_folders_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.get("https://1.1.1.1:2/seg/api/quarantine/folders/", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
...
def test_trustwave_seg_list_quarantine_folders_with_day_info_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_list_quarantine_folders_with_day_info_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.get("https://1.1.1.1:2/seg/api/quarantine/folderswithdayinfo/", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
    response = trustwave_seg_list_quarantine_folders_with_day_info_command(client)
...
def test_trustwave_seg_list_day_info_by_quarantine_folder_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_list_day_info_by_quarantine_folder_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.get("https://1.1.1.1:2/seg/api/quarantine/folders/1007/dayinfo", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
    response = trustwave_seg_list_day_info_by_quarantine_folder_command(client, "1007")
...
def test_trustwave_seg_find_quarantine_message_command(requests_mock):
    ...
    mock_response = util_load_json("test_data/trustwave_seg_find_quarantine_message_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.post("https://1.1.1.1:2/seg/api/quarantine/findmessage/?maxRows=5", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
    response = trustwave_seg_find_quarantine_message_command(client, max_rows="5", time_range="1 day ago")
...
def test_trustwave_seg_spiderlabs_forward_quarantine_message_as_spam_command(requests_mock):
    ...
    Checks the output of the command function with the expected output.
    """
    from TrustwaveSEG import Client, trustwave_seg_spiderlabs_forward_quarantine_message_as_spam_command

    mock_response = util_load_json("test_data/trustwave_seg_list_alerts_command.json")
    requests_mock.post("https://1.1.1.1:1/token", json={"access_token": "token", "expires_in": 0})
    requests_mock.post("https://1.1.1.1:2/seg/api/quarantine/forwardspam/", json=mock_response)
    client = Client("1.1.1.1", "1", "2", "username", "password", False, False)
    response = trustwave_seg_spiderlabs_forward_quarantine_message_as_spam_command(
        client, "1", "1", "1", "xxx", "xxx", "1", "1", "xxx", "true"
    )
