...
from FeedCyjax import (
    DATE_FORMAT,
    INDICATORS_LAST_FETCH_KEY,
    INDICATORS_LIMIT,
    Client,
    UnauthorizedException,
    convert_cyjax_indicator,
    ...
    main,
    map_indicator_type,
    map_reputation_to_score,
    set_indicators_last_fetch_date,
)
from FeedCyjax import test_module as module_test
...
def test_get_incidents_last_fetch_date(mocker):
    ...
    mocker.patch.object(demisto, "getIntegrationContext", return_value={INDICATORS_LAST_FETCH_KEY: str(timestamp)})
...
def test_get_indicators_command_arguments_specified(mocker):
    ...
    list_call_spy.assert_called_with(
...
def test_get_indicators_command_without_arguments_specified(mocker):
    ...
    result = get_indicators_command(client_for_testing, demisto.args())
    list_call_spy.assert_called_with(since=None, until=None, indicator_type=None, source_type=None, source_id=None, limit=50)
    assert isinstance(result, dict)
...
def test_fetch_indicators_main_command_call(mocker):
    mocker.patch.object(
        demisto,
        "params",
        return_value={"apikey": "test-api-key", "url": "https://cyjax-api-for-testing.com", "use_cyjax_tlp": True},
    )

    last_fetch = datetime(2020, 12, 27, 15, 45)
    last_fetch_timestamp = int(last_fetch.timestamp())

    mocker.patch.object(demisto, "getIntegrationContext", return_value={INDICATORS_LAST_FETCH_KEY: last_fetch_timestamp})
...
def test_fetch_indicators_main_command_call_no_new_indicators(mocker):
    mocker.patch.object(demisto, "params", return_value={"apikey": "test-api-key", "url": "https://cyjax-api-for-testing.com"})
...
def test_fetch_indicators_main_command_call_use_set_tlp(mocker):
    ...
    cyjax_indicator = mocked_indicators
    expected_indicators = [convert_cyjax_indicator(cyjax_indicator[1], None, "AMBER")]
...
def test_fetch_indicators_main_command_call_use_tags(mocker):
    ...
    cyjax_indicator = mocked_indicators
    expected_indicators = [convert_cyjax_indicator(cyjax_indicator[1], None, None, "TestTag, YellowTag")]
...
def test_get_indicators_main_command_call_with_one_new_indicator(mocker):
    ...
    mocker.patch.object(
        ...
        return_value={INDICATORS_LAST_FETCH_KEY: int(datetime(2020, 12, 27, 15, 45).timestamp())},
    )

    cyjax_indicator = mocked_indicators
    expected_indicators = [convert_cyjax_indicator(cyjax_indicator[0])]
...
def test_unset_indicators_last_fetch_date_main_command_call(mocker):
    mocker.patch.object(
        demisto, "getIntegrationContext", return_value={INDICATORS_LAST_FETCH_KEY: 1640988032, "Something": "Else"}
    )

    assert demisto.getIntegrationContext() == {INDICATORS_LAST_FETCH_KEY: 1640988032, "Something": "Else"}
