    ...
    if close_incident:
        for incident in modified_incidents:
            if incident.get("status") == "Resolved":
                ...
                close_reason = MICROSOFT_RESOLVED_CLASSIFICATION_TO_XSOAR_CLOSE_REASON.get(
                    incident.get("classification", "Unknown"), "Other")
                if argToBoolean(demisto.params().get("custom_defender_to_xsoar_close_reason", False)):
                    ...
                    custom_defender_to_xsoar_close_reason_mapping = comma_separated_mapping_to_dict(
                        params.get("custom_defender_to_xsoar_close_reason_mapping")
                    )
                    ...
                    if incident_close_reason in custom_defender_to_xsoar_close_reason_mapping:
                        ...
                        close_reason = custom_defender_to_xsoar_close_reason_mapping.get(
...
def handle_incident_close_out_or_reactivation(delta: dict, incident_status: IncidentStatus):
    ...
    if incident_status == IncidentStatus.DONE:
        if any(delta.get(key) for key in ["closeReason", "closeNotes", "closingUserId"]):
            ...
            if argToBoolean(demisto.params().get("custom_xsoar_to_defender_close_reason", False)):
