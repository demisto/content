add in rule hit count and disable command
This integration was integrated and tested with version xx of Palo Alto Sam Version
## Configure Palo Alto Sam Version on Cortex XSOAR

1. Navigate to **Settings** > **Integrations** > **Servers & Services**.
2. Search for Palo Alto Sam Version.
3. Click **Add instance** to create and configure a new integration instance.
    ...
    | Use URL Filtering for auto enrichment | False |
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| drop_mode | Type of session reject. Possible values are: drop, alert, block-ip, reset-both, reset-client, reset-server. | Optional | 
...
| element_to_change | Parameter in the security rule to change. Can be 'source', 'destination', 'application', 'action', 'category', 'description', 'disabled', 'target', 'log-forwarding' or 'tag'. Possible values are: source, destination, application, action, category, description, disabled, target, log-forwarding, tag. | Required | 
...
| show_uncommitted | Whether to show an uncommitted configuration. Possible values are: true, false. Default is false. | Optional | 
...
| nexthop_type | The type for the nexthop. Can be: "ip-address", "next-vr", "fqdn" or "discard". Possible values are: ip-address,  next-vr,  fqdn,  discard. | Required | 
...
***
custom rule count command by Sam Soon for demo purpose
...
| **Path** | **Type** | **Description** |
| --- | --- | --- |
| Panorama.SecurityRule.HitCount | number | security rule hit count | 
| Panorama.SecurityRule.LastHitTimestamp | number | security rule last hit timestamp | 
| Panorama.SecurityRule.FirstHitTimestamp | string | security rule first hit timestamp | 
| Panorama.SecurityRule.RuleCreationTimestamp | string | security rule create timestamp | 
| Panorama.SecurityRule.Name | string | security rule name | 
...
| **Argument Name** | **Description** | **Required** |
| --- | --- | --- |
| rulename | rule name to disable. | Required | 
| pre_post | Pre rule or Post rule (Panorama instances). Possible values are: pre-rulebase, post-rulebase. | Optional | 
| device-group | The device group for which to return addresses for the rule (Panorama instances). | Optional | 
