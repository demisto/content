...
@logger
def panorama_disable_rule_command():
    """
    ...
    demisto.results({
        ...
        'Contents': result,
        'ReadableContentsFormat': formats['text'],
        'HumanReadable': 'Rule disable successfully.',
    })
...
def panorama_show_rule_hit_count(target: str = None):
    params = {
        'type': 'op',
    ...
    return result['response']['result']['rule-hit-count']['vsys']['entry']['rule-base']['entry']['rules']['entry']


def prettify_rule1(rule):
    pretty_rule1 = {
        'Name': rule['@name'],
        'HitCount': rule['hit-count'],
        'LastHitTimestamp': rule['last-hit-timestamp'],
        'FirstHitTimestamp': rule['first-hit-timestamp'],
        'RuleCreationTimestamp': rule['rule-creation-timestamp']

    }
    return pretty_rule1


def prettify_rules1(rules):
    if not isinstance(rules, list):
        return prettify_rule1(rules)
    pretty_rules_arr = []
    for rule in rules:
        pretty_rule1 = prettify_rule1(rule)
        pretty_rules_arr.append(pretty_rule1)

    return pretty_rules_arr


def panorama_show_rule_hit_count_command():
    """
    Get device details and show message in war room
    """
    target = str(demisto.args()['target']) if 'target' in demisto.args() else None

    rules = panorama_show_rule_hit_count(target)
    pretty_rules1 = prettify_rules1(rules)
    ...
    demisto.results({
        ...
        'Contents': rules,
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('Security Rules:', pretty_rules1,
                                         ['Name', 'RuleCreationTimestamp', 'HitCount', 'FirstHitTimestamp', 'LastHitTimestamp'],
                                         removeNull=True),
        'EntryContext': {
            "Panorama.SecurityRule(val.Name == obj.Name)": pretty_rules1
        }
    })
...
def main():
    ...
    try:
        ...
        # Sam_custom_rule_hit_count_command
        elif demisto.command() == 'panorama-show-rule-hit-count':
            panorama_show_rule_hit_count_command()
