...
def main():
    ...
    if isError(resp[0]):
        ...
        if upload_res and isinstance(upload_res, list):
            ...
            demisto.results(
                {"Type": entryTypes["error"], "ContentsFormat": formats["text"], "Contents": "Coudn't extract TaskID from upload"}
            )
            sys.exit(0)
        ...
        demisto.results(
            {
                ...
                "Contents": f"Could not retrieve results from ATD (may be due to timeout). last status = {status}",
            }
        )
        sys.exit(0)
    if istate and int(istate) in [1, 2]:
