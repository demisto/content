...
HEARTBEAT_HEADERS = {"Accept": "application/vnd.ve.v1.0+json", "Content-Type": "application/json"}
API_HEADERS = None
...
def prettify_list_users_res(users):
    ...
    for user in users:
        pretty_users.append(
            {"FullName": user["fullName"], "UserId": user["idx"], "LoginId": user["loginId"], "UserType": user["userType"]}
        )
...
def get_session_command():
    ...
    human_readable = tableToMarkdown(
        "ATD Current User", prettify_current_user_res(result), ["APIVersion", "IsAdmin", "SessionId", "UserId"]
    )
    demisto.results(
        {
            ...
            "EntryContext": {"ATD.Session(val.SessionId == obj.SessionId)": prettify_current_user_res(result)},
        }
    )
...
def list_users_command():
    ...
    human_readable = tableToMarkdown("ATD User List", pretty_users, ["FullName", "UserId", "LoginId", "UserType"])
...
def list_profiles_command():
    ...
    human_readable = tableToMarkdown(
        "ATD Analyzers Profile List",
        prettify_list_profiles_res(result),
        ["Name", "AnalyzerProfileId", "Description", "Sandbox", "Internet", "LocalBlackList"],
    )
...
def check_task_status_by_task_id(task_ids):
    ...
    return {"status": status, "tasks": tasks, "multipleResults": multiple_results}
...
def check_task_status_command():
    ...
    human_readable = tableToMarkdown("ATD Sandbox Task Status", result["tasks"], list((result["tasks"][0]).keys()))
...
def get_task_ids_command():
    ...
    for i, result in enumerate(results):
        multiple_human_readable.append({"taskId": result["result"]["taskIdList"], "jobId": job_ids[i]})
        entry_context.append({"taskId": result["result"]["taskIdList"], "jobId": job_ids[i]})

    human_readable = tableToMarkdown("ATD TaskIds and JobIds List", multiple_human_readable, ["taskId", "jobId"])
    ...
    demisto.results(
        {
            ...
            "ReadableContentsFormat": formats["markdown"],
            "HumanReadable": human_readable,
            "EntryContext": {"ATD.Task(val.jobId == obj.jobId)": entry_context},
        }
    )
...
def file_upload_raw(body, file_entry_id, filename_to_upload):
    ...
    if not filename_to_upload:  # first priority for the file name is user's argument
        ...
        filename_dq = demisto.dt(demisto.context(), 'File(val=val.EntryID=="' + file_entry_id + '")=val.Name')
...
def url_upload_raw(body):
    uri = "php/fileupload.php"
    res = http_request(uri, "post", API_HEADERS, body)
...
def file_upload(
    ...
    return {"taskId": result_obj["results"][0]["taskId"], "resultObj": result_obj}
...
def handling_errors_with_file_upload_command(args):
    ...
    if ("entryID" in args and "url" in args and args["submitType"] != "2") or ("entryID" not in args and "url" not in args):
...
def file_upload_command():
    ...
    human_readable = tableToMarkdown(
        "ATD sandbox sample submission",
        prettify_file_upload_res(result["resultObj"]),
...
def build_report_context(report_summary, upload_data, status, threshold, task_id):
    ...
    if report_summary and report_summary["Subject"]:
        ...
        if "FileType" in subject:
            ...
            submission_dt = demisto.dt(demisto.context(), f'ATD.Task(val.taskId === "{task_id}")')
...
def get_report(uri_suffix, task_id, report_type, upload_data, status, threshold):
    ...
    if report_type == "json":
        human_readable = tableToMarkdown("McAfee ATD Sandbox Report", summary, list(summary.keys()), None, removeNull=True)
        return {"content": json_res_string, "md": human_readable, "ec": entry_context}

    result = http_request("php/showreport.php?" + uri_suffix + "&iType=" + report_type, "get", API_HEADERS)

    if report_type == "pdf" or report_type == "zip":
        filename = str(task_id) + "." + report_type
        return {"content": result, "filename": filename, "ec": entry_context}

    if report_type == "sample":
        return {"content": result, "filename": task_id + ".zip", "ec": entry_context}
    return result
...
def detonate(submit_type, sample, timeout, report_type, threshold, file_name):
    ...
    return_error(
        "Timeout due to no answer after "
        + demisto.args()["timeout"]
...
def main():  # pragma: no cover
    ...
    try:
        ...
        # deprecated, please use 'ATD - Detonate File' playbook
            ...
            detonate(
                0,
                demisto.args().get("upload"),
                demisto.args().get("timeout"),
                demisto.args().get("format"),
                demisto.args().get("threshold"),
