...
integration_params = {"baseUrl": " http://demi.demi.com", "username": "bark", "password": "my_password"}
...
def test_prettify_current_user_res():
    ...
    prettify_user_res = prettify_current_user_res({"apiVersion": "1.0", "isAdmin": "1", "session": "42", "userId": 101})
...
def test_get_report_command(mocker, requests_mock):
    ...
    mocker.patch.object(demisto, "args", return_value={"taskId": 1, "threshold": 1, "type": "json"})
    requests_mock.get(
    ...
    requests_mock.get(
        ...
        json={
            "Summary": {
                "Verdict": {"Description": "desc", "Severity": 5},
                "Subject": {
...
def test_detonate_file_command(mocker, requests_mock):
    ...
    requests_mock.get(
        ...
        json={"Summary": {"Verdict": {"Description": "desc", "Severity": 5}}},
    )
    results_mock = mocker.patch.object(demisto, "results")
...
def test_list_users_command(mocker, requests_mock):
    ...
    requests_mock.get(
        ...
        json={"results": [{"fullName": "fullName", "idx": "idx", "loginId": "loginId", "userType": "userType"}]},
    )
    results_mock = mocker.patch.object(demisto, "results")
    list_users_command()
...
def test_list_profiles_command(mocker, requests_mock):
    ...
    requests_mock.get(
        ...
        json={
            "results": [
                {"name": "name", "vmProfileid": "vmProfileid", "vmDesc": "vmDesc", "sandbox": 0, "internet": 0, "locBlackList": 0}
            ]
        },
    )
    results_mock = mocker.patch.object(demisto, "results")
...
def test_get_headers(requests_mock):
    ...
    requests_mock.get("http://demi.demi.com/php/session.php", json={"results": {"session": "session", "userId": "userId"}})
    res = get_headers()
