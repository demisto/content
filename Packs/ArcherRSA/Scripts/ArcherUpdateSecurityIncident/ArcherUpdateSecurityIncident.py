...
"""
This script is used to simplify the process of updating a new record in Archer.
...
createRecordArgs = {"applicationId": 75, "fieldsToValues": {}}
args = demisto.args()
if content_id := args.get("contentId"):
    createRecordArgs["contentId"] = content_id
elif incident_id := args.get("incidentId"):
    createRecordArgs["incidentId"] = incident_id
else:
    raise DemistoException("Please enter either contentId or incidentId")
...
createRecordArgs["fieldsToValues"].update(
    {(keysToChange[k] if k in keysToChange else k): v for k, v in args.items()}
)
createRecordResult = demisto.executeCommand("archer-update-record", createRecordArgs)
return_results(createRecordResult)
