...
def http_request(method, path, params=None, data=None):
    ...
    try:
        ...
        return return_error(f"Could not connect to Symantec MC: {e!s}")
...
def verify_policy_content(content_type, ips, categories, urls):
    if (
        (content_type == IP_LIST_TYPE and not ips)
    ...
    if (
        (content_type == IP_LIST_TYPE and (urls or categories))
...
def list_devices_command():
    ...
    if devices:
        ...
        human_readable = tableToMarkdown(
            "Symantec Management Center Devices", contents, removeNull=True, headers=headers, headerTransform=pascalToSpace
        )
...
def get_device_command():
    ...
    return_outputs(
        tableToMarkdown(
            "Symantec Management Center Device", content, removeNull=True, headers=headers, headerTransform=pascalToSpace
        ),
        context,
        device,
    )
...
def get_device_health_command():
    ...
    if device_health:
        device_content = {"UUID": device_health.get("uuid"), "Name": device_health.get("name")}
        ...
        if health_content:
            ...
            human_readable += tableToMarkdown(
                "Device Health", health_content, removeNull=True, headers=health_headers, headerTransform=pascalToSpace
            )
...
def get_device_license_command():
    ...
    if device_license:
        ...
        if license_content:
            ...
            human_readable += tableToMarkdown(
                "License Components", license_content, removeNull=True, headers=license_headers, headerTransform=pascalToSpace
            )
...
def get_device_status_command():
    ...
    return_outputs(
        tableToMarkdown(
            "Symantec Management Center Device Status", content, removeNull=True, headers=headers, headerTransform=pascalToSpace
        ),
        context,
        device,
    )
...
def list_policies_command():
    ...
    if policies:
        ...
        human_readable = tableToMarkdown(
            "Symantec Management Center Policies", contents, removeNull=True, headers=headers, headerTransform=pascalToSpace
        )
...
def get_policy_command():
    ...
    if policy:
        ...
        if policy.get("contentType") == URL_LIST_TYPE:
            ...
            for url in urls:
                policy_content_content.append(
                    {"Address": url.get("url"), "Description": url.get("description"), "Enabled": url.get("enabled")}
                )
            ...
            for ip in ips:
                policy_content_content.append(
                    {"Address": ip.get("ipAddress"), "Description": ip.get("description"), "Enabled": ip.get("enabled")}
                )
            ...
            for category in categories:
                policy_content_content.append({"Name": category.get("categoryName")})
        ...
        if revision_content:
            ...
            human_readable += tableToMarkdown(
                "Revision Information", revision_content, removeNull=True, headers=revision_headers, headerTransform=pascalToSpace
            )

        if policy_content_content:
            content[content_key] = policy_content_content
            human_readable += tableToMarkdown(
                content_title, policy_content_content, removeNull=True, headers=content_headers, headerTransform=pascalToSpace
            )
...
def create_policy_command():
    ...
    return_outputs(
        tableToMarkdown("Policy created successfully", content, removeNull=True, headers=headers, headerTransform=pascalToSpace),
        context,
        policy,
    )
...
def create_policy_request(name, content_type, description, reference_id, tenant, shared, replace_variables):
    ...
    body = {"name": name, "contentType": content_type}
...
def update_policy_command():
    ...
    return_outputs(
        tableToMarkdown("Policy updated successfully", content, removeNull=True, headers=headers, headerTransform=pascalToSpace),
        context,
        policy,
    )
...
def add_policy_content_command():
    ...
    if content_type == IP_LIST_TYPE:
        add_policy_content_request(
            uuid, content_type, change_description, schema_version, ips=ips, enabled=enabled, description=description
        )
    elif content_type == URL_LIST_TYPE:
        add_policy_content_request(
            uuid, content_type, change_description, schema_version, urls=urls, enabled=enabled, description=description
        )
    elif content_type == CATEGORY_LIST_TYPE:
        add_policy_content_request(uuid, content_type, change_description, schema_version, categories=categories)
...
def add_policy_content_request(
    uuid, content_type, change_description, schema_version, ips=None, urls=None, categories=None, enabled=None, description=""
):
    """
    ...
    body = {"contentType": content_type, "changeDescription": change_description}
    ...
    if content_type == LOCAL_CATEGORY_DB_TYPE:
        ...
        for category in categories:
            ...
            for entity in content_entities:
                entries.append({"type": "url", "url": entity, "comment": description})
            ...
            if not found_existing_category:
                content["content"]["categories"].append({"type": "inline", "name": category, "entries": entries})
    elif ips:
...
def delete_policy_content_command():
    ...
    if content_type == IP_LIST_TYPE:
        ...
        content_deleted = delete_policy_content_request(
            uuid, content_type, change_description, schema_version, ips=ips, urls=urls, categories=categories
        )
...
def delete_policy_content_request(uuid, content_type, change_description, schema_version, ips=None, urls=None, categories=None):
    """
    Add content to a specified policy using the provided arguments
    :param uuid: Policy UUID
    :param content_type: Policy content type
    ...
    if content_type == LOCAL_CATEGORY_DB_TYPE:
                ...
                if category.get("name") in categories:
                    ...
                    for entry in category.get("entries"):
                        if entry.get("url") in content_entities:
                            content_deleted.append({"CategoryName": category.get("name"), "ObjectDeleted": entry.get("url")})
                            found_object_to_delete = True
                        else:
                            entries_to_keep.append(entry)
                    if entries_to_keep:
                        categories_to_keep.append({"name": category.get("name"), "type": "inline", "entries": entries_to_keep})
                    else:
        ...
        categories_to_keep = [
            category for category in content["content"]["categories"] if category["categoryName"] not in categories
        ]
        content["content"]["categories"] = categories_to_keep
...
def update_policy_content_request(
    uuid,
    content_type,
    change_description,
    schema_version,
...
def list_tenants_command():
    ...
    if tenants:
        ...
        human_readable = tableToMarkdown(
            "Symantec Management Center Tenants", contents, removeNull=True, headers=headers, headerTransform=pascalToSpace
        )
