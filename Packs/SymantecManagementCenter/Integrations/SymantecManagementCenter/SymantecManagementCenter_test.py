...
from SymantecManagementCenter import LOCAL_CATEGORY_DB_TYPE, main
...
def test_add_policy_content_local_category_db(mocker, requests_mock):
    ...
    content = {
        "content": {
            "categories": [
                {"type": "inline", "name": "category1", "entries": [{"type": "url", "url": "www.demisto.com", "comment": None}]},
                {
                    "type": "inline",
                    "name": "category2",
                    "entries": [
                ...
                {"type": "inline", "name": "category5", "entries": [{"type": "url", "url": "8.8.8.8", "comment": "comment"}]},
                {
                    "type": "inline",
                    "name": "category6",
                    "entries": [
    ...
    mocker.patch.object(
        demisto, "params", return_value={"url": "https://server", "credentials": {"identifier": "user", "password": "password"}}
    )
    mocker.patch.object(
        demisto,
        "args",
    ...
    expected_request_body["content"]["categories"][1]["entries"].append({"type": "url", "url": url_to_add, "comment": comment})
...
def test_update_policy_content_local_category_db(mocker, requests_mock):
    ...
    main()
    expected_request_body = dict(content)
    expected_request_body["content"]["categories"][0]["entries"][0] = {"type": "url", "url": url_to_update, "comment": comment}
    assert requests_mock.request_history[1].json()["content"] == expected_request_body["content"]
    assert demisto.results.call_count == 1
    results = demisto.results.call_args[0][0]
    assert (
