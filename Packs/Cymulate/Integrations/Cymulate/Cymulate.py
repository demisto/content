...
class Client(BaseClient):
    ...
    def general_api_query(self, route):
        ...
        """
        data = self._http_request(method="GET", verify=False, url_suffix=route)
    ...
    def test_api(self):
        ...
        results = self._http_request(method="GET", url_suffix="user/modules")
    ...
    def get_attack_info(self, module_type, from_date):
        ...
        results = self._http_request(method="GET", url_suffix=get_ids_route, params={"from": from_date})
...
def fetch_incidents(
    client, module_type, last_run, first_fetch_time, only_penatrated, limit=DEFAULT_LIMIT, integration_context=None
):
    """
...
def main():
    ...
    try:
        client = Client(base_url=BASE_URL, headers=header, verify=insecure, proxy=use_proxy)
        ...
        if demisto.command() == "test-module":
            ...
            technical_data = cymulate_get_incident_info(client=client, attack_id=attack_id)
            ...
            if technical_data and len(technical_data) > 0:
                ...
                cymulate_output = tableToMarkdown(
                    "Cymulate Results",
                    parsed_technical_data,
                    [
                        "ID",
                ...
                outputs = {"Cymulate.Incident(val.ID == obj.ID)": parsed_technical_data}
