...
class Client(BaseClient):
    ...
    def validate(self):
        ...
        return self._http_request(method="GET", url_suffix="/user/modules", headers=self.headers, resp_type="response")
    ...
    def start_assessment(self, endpoint: str | None, data: dict):
        ...
        """
        return self._http_request(method="POST", url_suffix=f"{endpoint}/start", headers=self.headers, data=json.dumps(data))
    ...
    def stop_assessment(self, endpoint: str | None):
        ...
        """
        return self._http_request(method="POST", url_suffix=f"/{endpoint}/stop", headers=self.headers)
    ...
    def get_assessment_status(self, endpoint: str | None, assessment_id: str | None):
        ...
        return self._http_request(
            method="GET", url_suffix=f"/{endpoint}/status", headers=self.headers, params={"id": assessment_id}
        )
    ...
    def list_templates(self, endpoint: str | None):
        ...
        """
        return self._http_request(method="GET", url_suffix=f"/{endpoint}/templates", headers=self.headers)

    def list_phishing_contacts(self):
        """Retrieve a list af all phishing contacts."""
        return self._http_request(method="GET", url_suffix="/phishing/contacts/groups", headers=self.headers)

    def get_phishing_contacts(self, group_id: str | None):
        """Retrieve a list of phishing contacts by group ID."""
        return self._http_request(
            method="GET", url_suffix="/phishing/contacts", headers=self.headers, params={"groupId": group_id}
        )

    def create_phishing_contacts(self, group_name: str | None):
        """Create phishing contacts group."""
        return self._http_request(
            method="POST", url_suffix="/phishing/contacts/group", headers=self.headers, params={"groupName": group_name}
        )

    def get_agents(self):
        """Retrieve a list af all agents."""
        return self._http_request(method="GET", url_suffix="/agents/get/", headers=self.headers, resp_type="json")
    ...
    def list_attacks(self, endpoint: str | None):
        ...
        """
        response = self._http_request(method="GET", url_suffix=f"/{endpoint}/attacks/technical", headers=self.headers)
        return response.get("data")

    def list_attack_ids_by_date(self, endpoint: str | None, from_date: str | None, to_date: str | None = None):
    ...
    def list_attack_ids(self, endpoint: str | None):
        ...
        """
        response = self._http_request(method="GET", url_suffix=f"/{endpoint}/ids", headers=self.headers)
        return response.get("data")
    ...
    def get_attack_by_id(self, endpoint: str | None, attack_id: str | None):
        ...
        """
        response = self._http_request(method="GET", url_suffix=f"/{endpoint}/attack/technical/{attack_id}", headers=self.headers)
        return response.get("data")
    ...
    def get_immediate_threat_assessment(self, attack_id: str | None):
        ...
        """
        response = self._http_request(
            method="GET", url_suffix=f"/immediate-threats/attack/technical/{attack_id}", headers=self.headers
        ).get("data")
    ...
    def get_simulations_by_id(self, endpoint: str | None, attack_id: str | None):
        ...
        return self._http_request(
            method="GET", url_suffix=f"{endpoint}/history/technical/{attack_id}", headers=self.headers, params={"id": attack_id}
        )
...
def get_alerts_by_module(client: Client, module_name: str, last_fetch: int) -> tuple[List[Any], int, int, int]:
    ...
    if module_name in ("web-gateway", "exfiltration", "endpoint-security"):
        ...
        for cur_id in id_data:
            raw_data.extend(client.get_attack_by_id(ENDPOINT_DICT.get(module_name), cur_id.get("Id")))
        ...
        relevant_id_list = []
        for cur_id in id_data:
            alert_created_time = int(date_to_timestamp(cur_id.get("Timestamp"), date_format=CY_GENERAL_DATE_FORMAT))
        ...
        relevant_id_list = []
        for cur_id in id_data:
            alert_created_time = int(date_to_timestamp(cur_id.get("Timestamp"), date_format=CY_UNIQUE_DATE_FORMAT))
            if alert_created_time >= last_fetch:
                relevant_id_list.append(cur_id.get("Id"))
...
def format_incidents(
    events: list, event_offset: int, last_fetch: int, module_name: str, timestamp_endpoint: str = None
    ...
    for event in events[offset:]:
        ...
        if (module_name in ("endpoint-security", "kill-chain")) and (
            ...
            if module_name == "endpoint-security":
                alert_created_time = int(date_to_timestamp(alert_created_time, date_format=DATE_FORMAT))
            elif module_name == "kill-chain":
                alert_created_time = int(date_to_timestamp(alert_created_time, date_format=CY_GENERAL_DATE_FORMAT))

            step_num = event.get("Scenario_Counter") if module_name == "endpoint-security" else event.get("Stage_Phase")

            data = json.loads(last_incident.get("rawJSON"))
            current_description = data.get("description")
            new_description = f"\nStep {step_num}:\n{extract_event_description(event)}"

            data["description"] = f"{current_description}{new_description}" if current_description else new_description
        ...
        # The current event is new (has new name), then we need to build a new incident.
            ...
            if event_counter >= min(MAX_INCIDENTS_TO_FETCH, int(demisto.params().get("max_fetch", MAX_INCIDENTS_TO_FETCH))):
                break
            ...
            if timestamp_endpoint is None:
                t_stamp = event.get("Timestamp") if event.get("Timestamp") else event.get("Attack_Timestamp")
            else:
            ...
            if validate_timestamp(t_stamp):
                try:
                    alert_created_time = date_to_timestamp(t_stamp, date_format=CY_GENERAL_DATE_FORMAT)
                except Exception:
                    alert_created_time = date_to_timestamp(t_stamp, date_format=CY_UNIQUE_DATE_FORMAT)
...
def event_status_changed(event: dict) -> bool:
    ...
    prev = event.get("PrevStatus") if event.get("PrevStatus") else event.get("Previous_Scenario_Status")
...
def build_incident_dict(event: dict, module_name: str, event_timestamp=None) -> dict:
    ...
    mitigation = event.get("Mitigation_Details") if event.get("Mitigation_Details") else event.get("Mitigation")
    if mitigation:
    ...
    if not event_timestamp:
        event_timestamp = event.get("Timestamp") if event.get("Timestamp") else event.get("Attack_Timestamp")
...
def test_module(client: Client) -> str:
    ...
    try:
        ...
        if not response.ok:
            raise DemistoException("Authorization Error: make sure API Key is correctly set.", res=response)
...
def start_exfiltration_assessment_command(
    client: Client, template_id: str, agent_name: str, schedule: bool, schedule_loop: str, agent_profile_name: str = None
) -> CommandResults:
...
def get_exfiltration_assessment_status_command(client: Client, assessment_id: str) -> CommandResults:
...
def list_email_gateway_template_command(client: Client) -> CommandResults:
    ...
    readable_output = tableToMarkdown("Email gateway templates list:", outputs, removeNull=True)
    command_results = CommandResults(
...
def start_email_gateway_assessment_command(
    client: Client, template_id: str, agent_email: str, schedule: bool, schedule_loop: str
) -> CommandResults:
    ...
    params = {"templateID": template_id, "agentEmail": agent_email, "schedule": schedule, "scheduleLoop": schedule_loop}

    raw_response = client.start_assessment(ENDPOINT_DICT.get("email-gateway"), params)
    output = {"id": raw_response.get("data"), "success": raw_response.get("success")}
...
def get_email_gateway_assessment_status_command(client: Client, assessment_id: str) -> CommandResults:
...
def list_endpoint_security_template_command(client: Client) -> CommandResults:
    ...
    readable_output = tableToMarkdown("Endpoint security templates list:", outputs, removeNull=True)
...
def start_endpoint_security_assessment_command(
    client: Client, template_id: str, agent_name: str, schedule: bool, schedule_loop: str, agent_profile_name: str = None
) -> CommandResults:
...
def get_endpoint_security_assessment_status_command(client: Client, assessment_id: str) -> CommandResults:
    ...
    readable_output = tableToMarkdown("Endpoint security assessment status:", output, removeNull=True)
...
def start_waf_assessment_command(client: Client, template_id: str, sites, schedule: bool, schedule_loop: str) -> CommandResults:
    ...
    params = {"templateID": template_id, "sites": sites, "schedule": schedule, "scheduleLoop": schedule_loop}
...
def start_immediate_threat_assessment_command(
    client: Client,
    template_id: str,
...
def get_immediate_threat_assessment_status_command(client: Client, assessment_id: str) -> CommandResults:
    ...
    output = extract_status_commands_output(raw_response)
    readable_output = tableToMarkdown("Immediate-threats assessment status:", output, removeNull=True)
...
def start_lateral_movement_assessment_command(
    client: Client,
    agent_name: str,
    template_id: str,
    upload_to_cymulate: bool,
    schedule: bool,
    schedule_loop: str,
...
def get_lateral_movement_assessment_status_command(client: Client, assessment_id: str) -> CommandResults:
    ...
    readable_output = tableToMarkdown("Lateral movement assessment status:", output, removeNull=True)
...
def list_phishing_awareness_contact_groups_command(client: Client) -> CommandResults:
    ...
    readable_output = tableToMarkdown("Phishing awareness contact groups:", outputs, removeNull=True)
    command_results = CommandResults(
...
def add_phishing_awareness_contact_groups_command(client: Client, group_name: str) -> CommandResults:
    ...
    readable_output = tableToMarkdown("Phishing awareness contact group created:", raw_response, removeNull=True)
    command_results = CommandResults(
        outputs_prefix="Cymulate.Phishing.Groups",
        outputs_key_field="id",
        readable_output=readable_output,
...
def list_agents_command(client: Client) -> CommandResults:
    ...
    readable_output = tableToMarkdown("Agents list:", raw_response, headers=headers, removeNull=True)
...
def list_attack_simulations_command(client: Client, module: str, from_date: str, to_date: str = None) -> CommandResults:
...
def fetch_incidents(
    client: Client, last_run: dict[str, int], first_fetch_time: int, fetch_categories: list
    ...
    if current_module:
        incidents, offset, creation_time, total_simulated_events = get_alerts_by_module(client, current_module, last_fetch)
    ...
    # Finished fetching current module, checking if there are more modules to fetch.
        ...
        # Finished fetching all modules, re-initializing integration context.
            ...
            integration_context = {"offset": 0, "current_module": None, "modules": fetch_categories, "current_time": current_time}
...
def main() -> None:
    ...
    try:
        ...
        # endpoint-security
            ...
            return_results(get_endpoint_security_assessment_status_command(client, **demisto.args()))
