...
def create_error_entry(contents):
    return {"ContentsFormat": formats["text"], "Type": entryTypes["error"], "Contents": "Error - " + contents}
...
def get_projection(collector, outputs):
    return {ProjectionConstants.NAME: collector, ProjectionConstants.OUTPUTS: outputs or MAR_COLLECTORS.get(collector)}
    ...
    with DxlClient(config) as client:
        ...
        if not filter_by and not filter_operator and not filter_value:
            result_context = mar_client.search(
                projections=[{ProjectionConstants.NAME: collector, ProjectionConstants.OUTPUTS: outputs}]
            )
        else:
            if not filter_by or not filter_operator or not filter_value:
                ...
                result_context = mar_client.search(
                    projections=[{ProjectionConstants.NAME: projection_collector, ProjectionConstants.OUTPUTS: outputs}],
                    conditions={
...
def search_multiple(collectors, filter_collector, filter_by, filter_operator, filter_value):
    ...
    with DxlClient(config) as client:
        ...
        if not filter_collector:
            result_context = mar_client.search(projections=[get_projection(c, None) for c in collectors])
        else:
            if not filter_by or not filter_operator or not filter_value:
                raise Exception(
...
def search_multiple_wrapper(collectors, filter_collector, filter_by, filter_operator, filter_value):
    ...
    context = {"MAR.SearchMultiple": table}
...
def validate_certificates_format():
    ...
    if not demisto.params()["broker_ca_bundle"].rstrip(" ").endswith("-----END CERTIFICATE-----"):
        return_error("The broker certificate seem to be incorrect as they don't end with '-----END CERTIFICATE-----'")
...
def main():
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            results = search_wrapper(
                "Processes", "Processes", "", args.get("filter-by"), args.get("filter-operator"), args.get("filter-value"), True
            )
            demisto.results(results)
            sys.exit(0)
        elif demisto.command() == "mar-search-services":
            results = search_wrapper(
                "Services", "Services", "", args.get("filter-by"), args.get("filter-operator"), args.get("filter-value"), True
            )
            demisto.results(results)
            sys.exit(0)
            ...
            results = search_wrapper(
                "Files", "Files", "", args.get("filter-by"), args.get("filter-operator"), args.get("filter-value"), True
            )
            demisto.results(results)
            sys.exit(0)
            ...
            results = search_wrapper(
                "HostInfo", "HostInfo", "", args.get("filter-by"), args.get("filter-operator"), args.get("filter-value"), True
            )
            demisto.results(results)
            sys.exit(0)
