...
def test_get_client_config(mocker):
    ...
    # Invalid private Key
    valid_params = {"private_key": invalid_private_key, "cert_file": valid_certificate, "broker_ca_bundle": valid_certificate}
    mocker.patch.object(demisto, "params", return_value=valid_params)
    with pytest.raises(SystemExit):
        mcafee_mar.validate_certificates_format()

    # Invalid cert file
    valid_params = {"private_key": valid_private_key, "cert_file": invalid_certificate, "broker_ca_bundle": valid_certificate}
    mocker.patch.object(demisto, "params", return_value=valid_params)
    with pytest.raises(SystemExit):
        mcafee_mar.validate_certificates_format()
    ...
    valid_params = {"private_key": valid_private_key, "cert_file": valid_certificate, "broker_ca_bundle": spaces_in_certificate}
    mocker.patch.object(demisto, "params", return_value=valid_params)
    mcafee_mar.validate_certificates_format()
