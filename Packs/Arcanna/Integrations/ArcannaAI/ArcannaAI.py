...
def export_event(client: Client, default_job_id: int, args: Dict[str, Any]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name="Arcanna Raw Event", headers=headers, t=result, removeNull=True)
    outputs = {
        'Arcanna.RawEvent(val.job_id && val.job_id === obj.job_id)': createContext(result)
    }
...
def get_decision_set(client: Client, default_job_id: int, args: Dict[str, Any]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name="Arcanna Get Decision Set", headers=headers, t=result, removeNull=True)
    outcome = {
        "decision_set": result
    }
    outputs = {
        'Arcanna.GetDecisionSet(val.job_id && val.job_id === obj.job_id)': createContext(outcome)
    }
    return CommandResults(
        readable_output=readable_output,
        outputs=outputs,
...
def post_event(client: Client, default_job_id: int, args: Dict[str, Any]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name="Arcanna Send Event", headers=headers, t=result, removeNull=True)

    outputs = {
        'Arcanna.SendEventDetails(val.job_id && val.job_id === obj.job_id)': createContext(result)
    }
...
def get_event_status(args: Dict[str, Any], client: Client, default_job_id) -> CommandResults | PollResult:
    ...
    outputs = {
        'Arcanna.GetEventDetails': createContext(result)
    }
    headers = ["event_id", "ingest_timestamp", "status", "error_message",
               "bucket_state", "confidence_score", "outlier", "result_label"]
    return_data = CommandResults(
        outputs=outputs,
        readable_output=tableToMarkdown(name="Arcanna Get Event Status", headers=headers, t=result),
        raw_response=result
    )
...
def send_event_feedback(client: Client, default_job_id: int, args: Dict[str, Any]) -> CommandResults:
    ...
    readable_output = tableToMarkdown(name="Arcanna Send Event Feedback", headers=headers, t=result, removeNull=True)
    outputs = {
        'Arcanna.SendEventFeedback(val.job_id && val.job_id === obj.job_id)': createContext(result)
    }
