...
from ArcannaAI import Client, get_decision_set, get_event_status, get_jobs, post_event, send_event_feedback, trigger_training
...
arcanna_trigger_train_response = {"status": "OK", "error_message": ""}
...
arcanna_jobs_response = [
    {"job_id": 1, "data_type": "a", "title": "cortex", "status": "STARTED"},
    {"job_id": 2, "data_type": "palo", "title": "cortex5", "status": "IDLE"},
]

arcanna_event_feedback_response = {"status": "updated"}
...
def test_arcanna_trigger_train(mocker):
    ...
    command_args = {"job_id": 10, "username": "myusername"}
    mocker.patch.object(demisto, "args", return_value=command_args)
...
def test_arcanna_get_event_status_command(mocker):
    ...
    command_args = {"job_id": 10, "event_id": "11013867751676", "polling": "false", "interval": 10, "timeout": 60}
    mocker.patch.object(demisto, "args", return_value=command_args)
