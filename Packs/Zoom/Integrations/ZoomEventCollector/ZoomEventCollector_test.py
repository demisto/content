...
@pytest.mark.parametrize(
    "first_fetch_time, expected_result, expect_error",
    [
...
def test_main(first_fetch_time, expected_result, expect_error, mocker):
    ...
    mocker.patch(
        "ZoomEventCollector.Client.search_events",
        return_value={"from": "2023-03-31", "to": "2023-04-01", "page_size": 300, "next_page_token": "", "operation_logs": []},
    )
...
def test_fetch_events(mocker):
    ...
    """
    from ZoomEventCollector import Client, fetch_events

    first_fetch_time = datetime(2023, 3, 1).replace(tzinfo=UTC)

    http_request_mocker = mocker.patch.object(
        Client,
        "error_handled_http_request",
        side_effect=[
    ...
    next_run, events = fetch_events(client, last_run={}, first_fetch_time=datetime(2023, 2, 1).replace(tzinfo=UTC))

    mock_events = util_load_json("test_data/zoom_fetch_events.json")
    assert http_request_mocker.call_args_list[0][1].get("params") == {"page_size": 300, "from": "2023-02-01", "to": "2023-03-01"}
    assert http_request_mocker.call_args_list[1][1].get("params") == {"page_size": 300, "from": "2023-03-02", "to": "2023-03-30"}
    assert http_request_mocker.call_args_list[2][1].get("params") == {"page_size": 300, "from": "2023-02-01", "to": "2023-03-01"}
    assert http_request_mocker.call_args_list[3][1].get("params") == {"page_size": 300, "from": "2023-03-02", "to": "2023-03-30"}
    ...
    next_run, events = fetch_events(
        client,
        last_run={"activities": "2023-03-29T11:38:50Z", "operationlogs": "2023-03-21T08:22:09Z"},
...
def test_fetch_events_with_last_run(mocker):
    ...
    next_run, events = fetch_events(
        client,
        last_run={"activities": "2023-03-21T08:14:27Z", "operationlogs": "2023-03-20T16:37:58Z"},
        first_fetch_time=first_fetch_time,
    )

    mock_events = util_load_json("test_data/zoom_fetch_events_with_token.json")
    assert http_request_mocker.call_args_list[0][1].get("params") == {"page_size": 300, "from": "2023-03-20", "to": "2023-03-30"}
    assert http_request_mocker.call_args_list[1][1].get("params") == {
        "page_size": 300,
        "from": "2023-03-20",
        "to": "2023-03-30",
        "next_page_token": "next_token_operationlogs",
    }
    assert http_request_mocker.call_args_list[2][1].get("params") == {"page_size": 300, "from": "2023-03-21", "to": "2023-03-30"}
    assert http_request_mocker.call_args_list[3][1].get("params") == {
        "page_size": 300,
        "from": "2023-03-21",
        "to": "2023-03-30",
...
def test_get_events_command(mocker):
    ...
    """
    from ZoomEventCollector import Client, get_events
    ...
    client = Client(base_url=BASE_URL)
    events, results = get_events(client, limit=2, first_fetch_time=datetime(2023, 3, 1).replace(tzinfo=UTC))

    mock_events = util_load_json("test_data/zoom_get_events.json")
    assert http_request_mocker.call_args_list[0][1].get("params") == {"page_size": 2, "from": "2023-03-01", "to": "2023-03-30"}
    assert http_request_mocker.call_args_list[1][1].get("params") == {"page_size": 2, "from": "2023-03-01", "to": "2023-03-30"}
    assert http_request_mocker.call_args_list[0][1].get("url_suffix") == "report/operationlogs"
    ...
    with pytest.raises(DemistoException) as e:
        get_events(client, limit=MAX_RECORDS_PER_PAGE + 1, first_fetch_time=datetime(2023, 3, 1).replace(tzinfo=UTC))
    assert (
        e.value.message == f"The requested limit ({MAX_RECORDS_PER_PAGE + 1}) exceeds the maximum number of "
