...
class Client(BaseClient):
    ...
    def get_user(self, username):
        ...
        if res and len(res.get('users', [])) == 1:  # TODO: make sure you verify a single result was retrieved
            user_app_data = res.get('users')[0]  # TODO: get the user_id, username, is_active and user_app_data

            user_id = user_app_data.get('id')
            is_active = True if user_app_data.get('status') == 'active' else False
            username = user_app_data.get('email')
...
def test_module(client: Client, base_url):
