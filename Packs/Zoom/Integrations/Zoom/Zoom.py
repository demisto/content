...
MEETING_TYPE_NUM_MAPPING = {"Instant": 1, "Scheduled": 2, "Recurring meeting with fixed time": 8}
FILE_TYPE_MAPPING = {"MP4": "Video", "M4A": "Audio"}
...
MEMBER_PERMISSIONS_MAPPING = {"All channel members can add": 1, "Only channel owner and admins can add": 2}
...
AT_TYPE = {"Mention a contact": 1, 'Mention "all" to notify everyone in the channel.': 2}
...
class Client(Zoom_Client):
    ...
    def zoom_create_user(self, user_type_num: int, email: str, first_name: str, last_name: str):
        return self.error_handled_http_request(
            ...
            json_data={
                "action": "create",
                "user_info": {"email": email, "type": user_type_num, "first_name": first_name, "last_name": last_name},
            },
        )
    ...
    def zoom_create_meeting(self, url_suffix: str, json_data: dict):
        return self.error_handled_http_request(
            method="POST", url_suffix=url_suffix, headers={"authorization": f"Bearer {self.access_token}"}, json_data=json_data
        )

    def zoom_meeting_get(self, meeting_id: str, occurrence_id: str | None = None, show_previous_occurrences: bool | str = False):
        return self.error_handled_http_request(
            method="GET",
            url_suffix=f"/meetings/{meeting_id}",
            headers={"authorization": f"Bearer {self.access_token}"},
            params={"occurrence_id": occurrence_id, "show_previous_occurrences": show_previous_occurrences},
        )

    def zoom_meeting_list(
        self,
        user_id: str,
        next_page_token: str | None = None,
        page_size: int | str = 30,
        ...
        return self.error_handled_http_request(
            ...
            headers={"authorization": f"Bearer {self.access_token}"},
            params={"type": type, "next_page_token": next_page_token, "page_size": page_size, "page_number": page_number},
        )

    def zoom_fetch_recording(
        self, method: str, url_suffix: str = "", full_url: str = "", stream: bool = False, resp_type: str = "json"
    ):
        return self.error_handled_http_request(
            method=method,
            full_url=full_url,
    ...
    def zoom_list_channels(self, page_size: int, next_page_token: str = None, url_suffix: str = "", page_number: int = 1):
        return self.error_handled_http_request(
            method="GET",
    ...
    def zoom_list_user_channels(
        self, user_id: str, page_size: int, next_page_token: str = None, url_suffix: str = "", page_number: int = None
    ):
        return self.error_handled_http_request(
            method="GET",
            url_suffix=f"chat/{url_suffix}",
            headers={"authorization": f"Bearer {self.access_token}"},
            params={"user_id": user_id, "page_size": page_size, "page_number": page_number, "next_page_token": next_page_token},
        )
    ...
    def zoom_invite_to_channel(self, members_json: str, url: str = None):
        return self.error_handled_http_request(
            method="POST", url_suffix=url, headers={"authorization": f"Bearer {self.access_token}"}, json_data=members_json
        )
    ...
    def zoom_upload_file(self, url_suffix: str, file_info: dict):
        ...
        return self._http_request(
            "POST", headers={"Authorization": f"Bearer {self.access_token}"}, files=files, full_url=url_suffix
        )

    def zoom_send_message(self, url_suffix: str, json_data: dict):
        return self.error_handled_http_request(
            method="POST", url_suffix=url_suffix, json_data=json_data, headers={"authorization": f"Bearer {self.access_token}"}
        )
    ...
    def zoom_get_admin_user_id_from_token(self):
        return self.error_handled_http_request(
            method="get", url_suffix="users/me", headers={"authorization": f"Bearer {self.access_token}"}
        )

    def zoom_delete_user_token(self, url_suffix: str):
        return self.error_handled_http_request(
            method="DELETE", url_suffix=url_suffix, resp_type="response", headers={"authorization": f"Bearer {self.access_token}"}
        )
...
async def process_entitlement_reply(
    ...
    content_json = {
        "content": {"body": [{"type": "message", "text": entitlement_reply}]},
        "to_jid": to_jid,
        "robot_jid": robot_jid,
...
async def event_url_validation(payload):
    ...
    json_res = {"plainToken": plaintoken, "encryptedToken": expected_signature}
    return json_res


@app.post("/")
async def handle_zoom_response(
...
def manual_list_user_pagination(client: Client, next_page_token: str | None, limit: int, status: str, role_id: str | None):
    res = []
    page_size = min(limit, MAX_RECORDS_PER_PAGE)
    while limit > 0 and next_page_token != "":
        response = client.zoom_list_users(
            page_size=page_size, status=status, next_page_token=next_page_token, role_id=role_id, url_suffix="users"
        )
...
def manual_list_channel_pagination(client: Client, next_page_token: str | None, limit: int, url_suffix: str):
    res = {}
    page_size = min(limit, MAX_RECORDS_PER_PAGE)
    while limit > 0:
        response = client.zoom_list_channels(page_size=page_size, next_page_token=next_page_token, url_suffix=url_suffix)
        next_page_token = response.get("next_page_token")
...
def manual_list_user_channel_pagination(client: Client, user_id: str, next_page_token: str | None, limit: int, url_suffix: str):
    res = {}
    page_size = min(limit, MAX_RECORDS_PER_PAGE)
    while limit > 0:
        response = client.zoom_list_user_channels(
            user_id=user_id, page_size=page_size, next_page_token=next_page_token, url_suffix=url_suffix
        )
        next_page_token = response.get("next_page_token")
...
def manual_meeting_list_pagination(client: Client, user_id: str, next_page_token: str | None, limit: int, type: str | int | None):
    res = []
    page_size = min(limit, MAX_RECORDS_PER_PAGE)
    while limit > 0 and next_page_token != "":
        response = client.zoom_meeting_list(user_id=user_id, next_page_token=next_page_token, page_size=page_size, type=type)
        next_page_token = response.get("next_page_token")
        res.append(response)
...
def zoom_list_users_command(client, **args) -> CommandResults:
    ...
    if limit:
        if "page_size" in args or "page_number" in args or next_page_token or user_id:
            ...
            raw_data = manual_list_user_pagination(
                client=client, next_page_token=next_page_token, limit=limit, status=status, role_id=role_id
            )

            minimal_needed_info = remove_extra_info_list_users(limit, raw_data)

            md = tableToMarkdown(
                "Users", minimal_needed_info, ["id", "email", "type", "pmi", "verified", "created_at", "status", "role_id"]
            )
            md += "\n" + tableToMarkdown("Metadata", [raw_data][0][0], ["total_records"])
            raw_data = raw_data[0]
    else:
        # only one request is needed
        raw_data = client.zoom_list_users(
            page_size=page_size,
        ...
        if user_id:
            md = tableToMarkdown(
                "User", [raw_data], ["id", "email", "type", "pmi", "verified", "created_at", "status", "role_id"]
            )
        else:
            md = tableToMarkdown(
                "Users", raw_data.get("users"), ["id", "email", "type", "pmi", "verified", "created_at", "status", "role_id"]
            )
            md += "\n" + tableToMarkdown(
                "Metadata", [raw_data], ["page_count", "page_number", "page_size", "total_records", "next_page_token"]
            )
...
def zoom_create_meeting_command(client, **args) -> CommandResults:
    ...
    if type != RECURRING_WITH_TIME and any(
        (
            end_date_time,
            args.get("end_times"),
            args.get("monthly_day"),
            monthly_week,
            monthly_week_day,
            repeat_interval,
            args.get("weekly_days"),
        )
    ):
    ...
    if (
        ...
        and any((args.get("monthly_day"), monthly_week, monthly_week_day))
    ):
    ...
    if (
        type == RECURRING_WITH_TIME
        and recurrence_type == "Monthly"
        and not (monthly_week and monthly_week_day)
        and not args.get("monthly_day")
    ):
    ...
    return CommandResults(outputs_prefix="Zoom.Meeting", readable_output=md, outputs=safe_raw_data, raw_response=raw_data)
...
def zoom_fetch_recording_command(client: Client, **args):
    ...
    data = client.zoom_fetch_recording(method="GET", url_suffix=f"meetings/{meeting_id}/recordings")
    ...
    for file in recording_files:
        ...
        try:
            ...
            record = client.zoom_fetch_recording(method="GET", full_url=download_url, resp_type="response", stream=True)
            ...
            results.append(
                CommandResults(readable_output=f"The {file_type_as_literal} file {filename} was downloaded successfully")
            )
            ...
            if delete_after:
                try:
                    ...
                    client.zoom_fetch_recording(
                        method="DELETE", url_suffix=f'meetings/{meeting_id}/recordings/{file["id"]}', resp_type="response"
                    )
                    results.append(
                        CommandResults(
                    ...
                    results.append(CommandResults(readable_output=f"Failed to delete file {filename}. {exp}"))

        except DemistoException as exp:
            raise DemistoException(f"Unable to download recording for meeting {meeting_id}: {exp}")
...
def zoom_meeting_get_command(client, **args) -> CommandResults:
    ...
    for sensitive_info in ["password", "pstn_password", "encrypted_password", "h323_password"]:
        safe_raw_data.pop(sensitive_info, None)
    return CommandResults(
        outputs_prefix="Zoom.Meeting", readable_output=md, outputs_key_field="id", outputs=safe_raw_data, raw_response=raw_data
    )
...
def zoom_meeting_list_command(client, **args) -> CommandResults:
    ...
    if limit:
        if "page_size" in args or next_page_token or "page_number" in args:
            ...
            raw_data = manual_meeting_list_pagination(
                client=client, user_id=user_id, next_page_token=next_page_token, limit=limit, type=type
            )
        ...
        raw_data = client.zoom_meeting_list(
            user_id=user_id, next_page_token=next_page_token, page_size=page_size, type=type, page_number=page_number
        )
        # parsing the data
        md = tableToMarkdown(
            "Meeting list",
            raw_data.get("meetings"),
            ["uuid", "id", "host_id", "topic", "type", "start_time", "duration", "timezone", "created_at", "join_url"],
        )
    ...
    return CommandResults(
        ...
        outputs={"Meeting": raw_data, "Metadata": {"Size": raw_data.get("page_size"), "Total": raw_data.get("total_records")}},
        raw_response=raw_data,
    )


def check_authentication_type_parameters(
    api_key: str,
    api_secret: str,
...
def zoom_list_account_public_channels_command(client, **args) -> CommandResults:
    ...
    if limit:
        ...
        # only one request is needed
        raw_data = client.zoom_list_channels(
            page_size=page_size, next_page_token=next_page_token, url_suffix=url_suffix, page_number=page_number
        )
        # parsing the data according to the different given arguments
    ...
    return CommandResults(
        outputs_prefix="Zoom.Channel", readable_output=md, outputs={**raw_data, "ChannelsNextToken": token}, raw_response=raw_data
    )
...
def zoom_list_user_channels_command(client, **args) -> CommandResults:
    ...
    if limit:
        if "page_size" in args or "page_number" in args or next_page_token:
            ...
            # multiple requests are needed
            raw_data = manual_list_user_channel_pagination(
                client=client, user_id=user_id, next_page_token=next_page_token, limit=limit, url_suffix=url_suffix
            )
            data = remove_extra_info_list("channels", limit, raw_data)
            token = raw_data.get("next_page_token", None)
    else:
        # only one request is needed
        raw_data = client.zoom_list_user_channels(
            user_id=user_id, page_size=page_size, next_page_token=next_page_token, url_suffix=url_suffix, page_number=page_number
        )
        # parsing the data according to the different given arguments
        data = [raw_data] if channel_id else raw_data.get("channels")
        token = raw_data.get("next_page_token", None)
    ...
    return CommandResults(
        ...
        readable_output=md,
        outputs={**raw_data, "UserChannelsNextToken": token},
        raw_response=raw_data,
    )
...
def zoom_invite_to_channel_command(client, **args) -> CommandResults:
    ...
    outputs = [{"User id": raw_data.get("ids"), "Channel ID": channel_id, "Added at date and time": raw_data.get("added_at")}]
...
def zoom_send_file_command(client, **args) -> CommandResults:
    ...
    json_data = remove_None_values_from_dict({"to_contact": to_contact, "to_channel": to_channel})
    upload_response = client.zoom_send_file(upload_url, file_info, json_data)

    message_id = upload_response.get("id")
    return CommandResults(readable_output=f"Message with id {message_id} was successfully sent")
...
def process_links(formatted_message, formats):
    ...
    for match in matches:
        ...
        add_link_range = {"text": link_text, "format_type": "AddLink", "format_attr": link_url}
...
def process_font_sizes(formatted_message, formats):
    ...
    for match in matches:
        size, text = match
        font_size_range = {"text": text, "format_type": "FontSize", "format_attr": size}
...
def parse_markdown_message(markdown_message: str, at_contact: str = None):
    ...
    return formatted_message, {"message": formatted_message, "rich_text": rich_text, "at_items": at_items}
...
def zoom_send_message_command(client, **args) -> CommandResults:
    ...
    data = {"Mentioned user": at_contact, "Channel ID ": to_channel, "Message ID": raw_data.get("id"), "Contact": to_contact}

    md = tableToMarkdown("Message", data, removeNull=True)
    return CommandResults(
        outputs_prefix="Zoom.ChatMessage", readable_output=md, outputs_key_field="id", outputs=raw_data, raw_response=raw_data
    )
...
def zoom_update_message_command(client, **args) -> CommandResults:
    ...
    json_data = remove_None_values_from_dict(
        {"message": message, "file_ids": zoom_file_id, "to_channel": to_channel, "to_contact": to_contact}
    )
...
def zoom_list_messages_command(client, **args) -> CommandResults:
    ...
    while True:
        try:
            raw_data = client.zoom_list_user_messages(
                url_suffix=url_suffix,
                user_id=user_id,
                to_contact=to_contact,
                to_channel=to_channel,
...
To find the appropriate values, refer to the ChatMessageNextToken located in the context. \n {error_message}")
    ...
    return CommandResults(
        outputs_prefix="Zoom",
        readable_output=md,
        outputs={"ChatMessage": {"messages": all_messages}, "ChatMessageNextToken": request_data},
        raw_response=raw_data,
    )
...
def send_notification(client, **args):
    ...
    return CommandResults(readable_output=f'Message sent to Zoom successfully. Message ID is: {raw_data.get("message_id")}')
...
def mirror_investigation(client, **args) -> CommandResults:
    ...
    if not current_mirror:
        ...
        if not channel_filter:
            # create new channel
            result = zoom_create_channel_command(
                client=client,
                user_id=admin_user_id,
                channel_type="Public channel",
                channel_name=channel_name,
                member_emails=admin_user_id,
            )
        ...
        if channel_name and not channel_filter:
            # update channel name
            result = zoom_update_channel_command(
                client=client, user_id=admin_user_id, channel_name=channel_name, channel_id=channel_id
            )
    ...
    if send_first_message:
        ...
        message = (
            f"This channel was created to mirror incident {investigation_id}."
            f" \n View it on: {server_link}#/WarRoom/{investigation_id}"
        )
        url_suffix = "/im/chat/messages"
        json_data_all = {
            "content": {"body": [{"type": "message", "text": message}]},
            "to_jid": channel_jid,
            "robot_jid": client.bot_jid,
            "account_id": client.account_id,
        }
    ...
    if mirror:
        ...
        for mirror in channel_mirrors:
            ...
            demisto.mirrorInvestigation(
                mirror["investigation_id"], f'none:{mirror["mirror_direction"]}', bool(mirror["auto_close"])
            )
        set_to_integration_context_with_retries({"mirrors": mirrors}, OBJECTS_TO_KEYS, SYNC_CONTEXT)
    if channel_id:
