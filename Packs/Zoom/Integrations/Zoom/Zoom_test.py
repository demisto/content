...
def test_zoom_list_users_command__no_limit(mocker):
    ...
    zoom_list_users_command(
        client=client, page_size=30, user_id=None, status="active", next_page_token=None, role_id=None, limit=None
    )
...
def test_zoom_list_users_command__limit_and_page_size(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        zoom_list_users_command(
            client=client, page_size=30, user_id="fdghdf", status="active", next_page_token=None, role_id=None, limit=50
        )

    assert (
        e.value.message
        == """Too many arguments. If you choose a limit,
...
def test_zoom_list_users_command__user_id(mocker):
    ...
    zoom_list_users_command(
        client=client, page_size=30, user_id="bla@bla.com", status="active", next_page_token=None, role_id=None, limit=None
    )
...
def test_manual_list_user_pagination__small_limit(mocker):
    ...
    manual_list_user_pagination(client=client, next_page_token="None", limit=limit, status="None", role_id="None")
...
def test_zoom_create_user__basic_user_type(mocker):
    ...
    client.zoom_create_user(user_type_num=1, email="mock@moker.com", first_name="John", last_name="Smith")
...
def test_zoom_user_create__pro_user_type(mocker):
    ...
    client.zoom_create_user(user_type_num=2, email="mock@moker.com", first_name="John", last_name="Smith")
...
def test_zoom_user_create__Corporate_user_type(mocker):
    ...
    client.zoom_create_user(user_type_num=3, email="mock@moker.com", first_name="John", last_name="Smith")
...
def test_zoom_meeting_create_command__scheduled_meeting(mocker):
    ...
    zoom_create_meeting_command(client=client, user_id="mock@moker.com", topic="nonsense", type="Scheduled")
...
def test_zoom_create_meeting_command_too_many_arguments(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        zoom_create_meeting_command(
            client=client, user_id="mock@moker.com", topic="nonsense", type="Scheduled", waiting_room=True, join_before_host=True
        )
...
def test_zoom_create_meeting_command__too_meny_arguments1(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        zoom_create_meeting_command(
            client=client, type="Instant", topic="nonsense", user_id="mock@moker.com", start_time="2022-10-04T15:59:00Z"
        )
...
def test_zoom_create_meeting_command__too_many_arguments2(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        zoom_create_meeting_command(
            ...
            topic="nonsense",
            user_id="mock@moker.com",
            end_date_time="2022-10-04T15:59:00Z",
            monthly_week=2,
            monthly_week_day=3,
            end_times=7,
        )
...
def test_zoom_create_meeting_command__too_meny_arguments3(mocker):
    ...
    assert (
        e.value.message
        == """Missing arguments. A recurring meeting with a fixed time and monthly recurrence_type
...
def test_zoom_create_meeting_command__too_meny_arguments4(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        zoom_create_meeting_command(
            client=client, type="Recurring meeting with fixed time", topic="nonsense", user_id="mock@moker.com"
        )
    assert (
        e.value.message
        == """Missing arguments. A recurring meeting with a fixed
...
def test_zoom_create_meeting_command__too_many_arguments5(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        zoom_create_meeting_command(client=client, type="Instant", end_times=7)
...
def test_manual_meeting_list_pagination__small_limit(mocker):
    ...
    manual_meeting_list_pagination(client=client, user_id="bla", next_page_token=None, limit=limit, type="all")
...
def test_check_authentication_type_parameters_with_extra_jwt_member(mocker):
    ...
    """
    with pytest.raises(DemistoException) as e:
        Zoom.check_authentication_type_parameters(
...
def test_zoom_user_list_command__when_user_id(mocker):
    ...
    to_md = {
        ...
        "verified": 1,
        "dept": "",
        "created_at": "2022-12-01T07:40:02Z",
...
def test_zoom_fetch_recording__download_success(mocker):
    ...
    res = zoom_fetch_recording_command(client=client, meeting_id="000000", delete_after="false")
...
def test_zoom_fetch_recording_command__delete_success(mocker):
    ...
    res = zoom_fetch_recording_command(client=client, meeting_id="000000", delete_after="true")
...
def test_zoom_fetch_recording_command__recording_dose_not_exist(mocker):
    ...
    mocker.patch.object(demisto, "debug")
    mocker.patch.object(shutil, "copyfileobj")
    mocker.patch.object(Client, "zoom_fetch_recording", side_effect=[DemistoException("mockerror")])
    mocker.patch.object(Client, "generate_oauth_token")
    client = Client(base_url="https://test.com", account_id="mockaccount", client_id="mockclient", client_secret="mocksecret")
...
def test_zoom_list_user_channels_command(mocker):
    ...
    result = zoom_list_user_channels_command(client, channel_id=channel_id, user_id=user_id)

    zoom_list_user_channels_mock.assert_called_with(
        user_id=user_id, page_size=50, next_page_token=None, url_suffix=expected_url_suffix, page_number=1
    )
...
def test_zoom_create_channel_command(mocker):
    ...
    response = {"id": "channel_id", "name": "TestChannel3", "type": "1", "channel_url": "https://zoom.us/channel/channel_id"}
    ...
    expected_json_data = {
        ...
        "members": [{"email": "user2@example.com"}, {"email": "user1@example.com"}],
        "name": "TestChannel",
        "type": 1,
    }
...
def test_zoom_invite_to_channel_command(mocker):
    ...
    expected_json_data = {"members": [{"email": "user1@example.com"}, {"email": "user2@example.com"}]}
    ...
    zoom_invite_to_channel_command(client, channel_id=channel_id, user_id=user_id, members=members)
...
def test_zoom_remove_from_channel_command(mocker):
    ...
    zoom_remove_from_channel_command(client, channel_id=channel_id, member_id=member_id, user_id=user_id)
...
def test_zoom_send_file_command(mocker):
    ...
    expected_json_data = {"to_channel": to_channel}
    expected_upload_response = {"id": "file_id"}
    ...
    results = zoom_send_file_command(client, user_id=user_id, to_channel=to_channel, entry_id=entry_id)
...
def test_zoom_list_account_public_channels_command(mocker):
    ...
    result = zoom_list_account_public_channels_command(
        client, page_size=page_size, channel_id=channel_id, next_page_token=next_page_token, page_number=page_number
    )

    zoom_list_channels_mock.assert_called_with(
        page_size=page_size, next_page_token=next_page_token, url_suffix=expected_url_suffix, page_number=page_number
    )
...
def test_zoom_send_message_command_with_file(mocker):
    ...
    expected_response = {"id": "message_id", "contact": "user2@example.com", "channel_name": "channel_name"}
    ...
    zoom_send_message_command(client, user_id=user_id, message="Hello from @dima!", to_channel="channel1", entry_ids="entry_id")
    # Assert function calls
    Zoom.demisto.getFilePath.assert_called_with(entry_id)
...
def test_zoom_send_message_command(mocker):
    ...
    expected_request_payload = {"message": "Hello from @dima!", "to_channel": "channel1", "file_ids": []}
    ...
    result = zoom_send_message_command(
        client,
        user_id="user1",
...
def test_zoom_send_message_markdown_command(mocker):
    ...
    expected_request_payload = {
        ...
        "to_channel": "channel1",
        "at_items": [{"text": "@John", "at_contact": "user2@example.com", "at_type": 1, "start_position": 4, "end_position": 8}],
        "rich_text": [
            {
                "text": "report",
                "format_type": "AddLink",
                "format_attr": "https://example.com",
                "start_position": 39,
                "end_position": 44,
            },
            {"text": "HI ", "format_type": "paragraph", "format_attr": "h1", "start_position": 0, "end_position": 2},
            {
                "text": "@John, please review the following report",
...
def test_zoom_list_messages_command(mocker):
        ...
        "messages": [
            {
                ...
                "date_time": "2023-03-07T10:30:00Z",
            },
            {
                "id": "message_id_2",
                "message": "Message 2",
                ...
                "date_time": "2023-03-08T09:15:00Z",
            },
        ]
    }
    expacted_result = {
    ...
    assert (
        result.outputs["ChatMessage"]["messages"][0]["sender_display_name"]
        == expacted_result["ChatMessage"]["messages"][0]["sender_display_name"]
    )  # noqa: E501
...
def test_zoom_update_message_command(mocker):
    ...
    expected_payload = {"message": message, "file_ids": [], "to_channel": to_channel, "to_contact": to_contact}
...
@pytest.mark.parametrize(
    "channel_name, investigation_id, expected_result",
    [
...
@pytest.mark.parametrize(
    "bot_Jid, client_id, client_secret, expected_exception",
    [
...
def test_mirror_investigation_create_new_channel(mocker):
    ...
    mocker.patch.object(
        Zoom, "zoom_create_channel_command", return_value=CommandResults(outputs={"jid": "mock_jid", "id": "mock_id"})
    )
            ...
            [
                {
                    ...
                    "reply": "thanks",
                    "expiry": "2023-08-29 12:32:40",
                    "sent": "2023-08-29 12:27:42",
...
async def test_answer_question(mocker):
    ...
    mock_question = {"entitlement": "guid123@incident456|task789", "to_jid": "ToJID123"}
    Zoom.CLIENT = Client(
...
async def test_process_entitlement_reply(mocker):
    ...
    mock_zoom_send_notification.assert_called_with(
        ...
        "/im/chat/messages",
        {
            "content": {"body": [{"type": "message", "text": mock_entitlement_reply}]},
            "to_jid": mock_to_jid,
...
@pytest.mark.asyncio
async def test_handle_zoom_response(event_type, expected_status, mocker):
    """
    ...
    json_res = {"plainToken": 123, "encryptedToken": 123}
    ...
    mock_credentials = HTTPBasicCredentials(username="test", password="testpass")
    ...
    mock_request.json.return_value = {
        ...
        "payload": {
            ...
            "actionItem": {"text": "no", "value": "no", "action": "command"},
...
async def test_event_url_validation():
    ...
    assert response == {"plainToken": payload["plainToken"], "encryptedToken": expected_signature}
