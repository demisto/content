...
1. Configure an Google Threat Intelligence - DTM Alerts integration instance with valid API Key.
2. Click **Test** to validate the connection.
3. To fetch DTM Alerts as an incidents in XSOAR, select the option `Fetches incidents` and follow the table to update configuration parameters.
...
| Alert mscore | Fetch Alerts with mscore greater than or equal to the given value.<br/>Note: The value can range in between from 0 to 100. |
