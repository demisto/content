...
class Client(BaseClient):
    ...
    def get_threat_list(self, feed_type: str, package: str, filter_query: str = None, limit: int = 10) -> dict:
    ...
    def fetch_indicators(
        self, feed_type: str, package: str = None, filter_query: str = None, limit: int = 10, fetch_command: bool = False
    ) -> list:
...
def _add_gti_attributes(indicator_obj: dict, item: dict):
    ...
    malware_families: list[str] = [
        x.get("attributes", {}).get("name") for x in relationships.get("malware_families", {}).get("data", [])
    ]
    malware_families = list(set(malware_families))
    threat_actors: list[str] = [
        x.get("attributes", {}).get("name") for x in relationships.get("threat_actors", {}).get("data", [])
    ]
...
def fetch_indicators_command(
    ...
    raw_indicators = client.fetch_indicators(
        feed_type, package=package, filter_query=filter_query, limit=limit, fetch_command=fetch_command
    )
