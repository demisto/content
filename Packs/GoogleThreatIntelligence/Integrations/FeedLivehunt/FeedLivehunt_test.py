...
MOCK_VT_RESPONSE = {
    'data': [
        {
            'type': 'file',
            'id': FILE_ATTRIBUTES_MOCK['sha256'],
            'attributes': FILE_ATTRIBUTES_MOCK.copy()
        },
    ],
}


def _mock_file(sha256=None, gti_score=None):
    attributes_mock = FILE_ATTRIBUTES_MOCK.copy()

    if sha256 is not None:
        attributes_mock['sha256'] = sha256

    if gti_score is not None:
        attributes_mock['gti_assessment'] = {
            'threat_score': {
                'value': gti_score,
            }
        }
...
def test_get_indicators_command(mocker):
    ...
    for mock_gti_score, params_gti_score, len_response in [
        ...
        mocker.patch.object(
            ...
            return_value={
                'data': [
                    _mock_file(sha256='a' * 64, gti_score=mock_gti_score),
                ],
            },
        )
        params = {
        ...
        if params_gti_score is not None:
            params['feedMinimumGTIScore'] = params_gti_score
...
def test_main_manual_command(mocker):
    ...
    mocker.patch.object(demisto, 'command', return_value='gti-livehunt-get-indicators')
