...
| Mirroring Direction | The mirroring direction in which to mirror the details. You can mirror "Outgoing" \(from XSOAR to GTI\) direction for ASM Issues. Cortex XSOAR only parameter. | False |
...
| Search String | Search String to filter out the ASM Issues.<br/><br/>For Example: collection:google severity:5 status_new:open scoped:true entity_type:domain<br/><br/>Note: The fields last_seen_after, last_seen_before, and first_seen_after will be ignored from the provided search string. |  |
