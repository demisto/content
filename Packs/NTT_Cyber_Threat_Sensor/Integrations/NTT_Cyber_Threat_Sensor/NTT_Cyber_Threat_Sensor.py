...
def http_request(method, url_suffix, json_dict=None, params=None, headers=None, **kwargs):
    ...
    if json_dict:
        res = requests.request(
            method, BASEURL + url_suffix, verify=USE_SSL, params=params, headers=headers, json=json_dict, **kwargs
        )
    else:
        res = requests.request(method, BASEURL + url_suffix, verify=USE_SSL, params=params, headers=headers, **kwargs)
...
def item_to_incident(item):
    ...
    incident = {
        ...
        "CTS": {
            "DeviceName": item.get("devicephysical"),
...
def test_module():
    ...
    if len(errors) > 0:
        return_results(
            {"Type": entryTypes["error"], "ContentsFormat": formats["text"], "Contents": "Errors:\n{}".format("\n".join(errors))}
        )
