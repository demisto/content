...
class Client(BaseClient):
    ...
    def udso_list(self, list_type="", contentfilter=""):
        ...
        full_url = urljoin(self.base_url, UDSOAPIPATH + querystring)

        response = (
            self._http_request("GET", UDSOAPIPATH, full_url=full_url, headers=headers))
        return response
    ...
    def udso_delete(self, list_type="", content=""):
        ...
        headers = {'Authorization': 'Bearer ' + self.create_jwt_token(http_method='DELETE',
                                                                      ...
                                                                      request_body='')}
        full_url = urljoin(self.base_url, UDSOAPIPATH + querystring)
        response = self._http_request("DELETE", UDSOAPIPATH, full_url=full_url,
                                      headers=headers)
        return response
    ...
    def udso_add(self, add_type=None, content=None, scan_action=None, notes='', expiration=''):
        if add_type and content and scan_action:
            ...
            full_url = urljoin(self.base_url, UDSOAPIPATH + '/')
            response = self._http_request("PUT", UDSOAPIPATH + '/', full_url=full_url,
