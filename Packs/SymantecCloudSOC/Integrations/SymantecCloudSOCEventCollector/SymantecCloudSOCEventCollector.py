...
PRODUCT = "cloud_soc"
LOG_TYPES = {
    "Investigate_logs": {"app": "Investigate", "subtype": "all"},
...
class Client(BaseClient):
    ...
    def get_events_request(
        ...
        if next_url:
            ...
            params = assign_params(app=app, subtype=subtype, limit=max_fetch, created_timestamp=created_timestamp)
...
def test_module(client: Client) -> str:
    ...
    try:
        ...
        demisto.debug(f"SymantecEventCollector: {e!s}")
...
def get_events_command(
    ...
    investigate_logs_events, incident_logs_events, _ = get_all_events(
        client, first_fetch_time, first_fetch_time_investigate, last_run, limit
    )
...
def dedup_by_id(
    last_run: dict, events: list, log_type: str, limit: int, number_of_events: int, last_fetch: str
) -> tuple[list, dict]:
    """
    Dedup mechanism for the fetch to check both log_id and created_timestamp/incident_start_time
    (since timestamp can be duplicate)
    ...
    if events:
        for event in events:
            if len(new_events) + number_of_events < limit:
                event_timestamp = (
                    event.get("incident_start_time") if log_type == "Incident_logs" else event.get("created_timestamp")
                )  # log type is Investigate_logs
                ...
                # The event has a timestamp we have not yet fetched meaning it is a new event
                    ...
                    if new_last_run_time_date < event_timestamp_date:
                        ...
                        demisto.debug(
                            f"SymantecEventCollector: new_last_run_time is {new_last_run_time}"
...
def get_all_events_for_log_type(
    client: Client,
    max_fetch: int,
    log_type: str,
    last_run: dict[str, dict],
    ...
    while next_url is not None and len(all_events_list) < max_fetch:
        ...
        log_events, next_url = client.get_events_request(
            max_fetch=MAX_LIMIT_PER_CALL, subtype=subtype, app=app, created_timestamp=last_fetch, next_url=next_url
        )
        list_of_events, last_run_for_log_type = dedup_by_id(
            last_run_for_log_type, log_events, log_type, max_fetch, number_of_events, last_fetch
        )
...
def fetch_events_command(
    client: Client, max_fetch: int, last_run: dict[str, dict], first_fetch_time: str, first_fetch_time_investigate: str
    ...
    # Initialize an empty next_run object to return
    new_last_run: dict = {}
    investigate_logs_events, incident_logs_events, new_last_run = get_all_events(
        client, first_fetch_time, first_fetch_time_investigate, last_run, max_fetch
    )
...
def create_client_with_authorization(
    base_url: str, verify_certificate: bool, proxy: bool, key_id: str, key_secret: str
) -> Client:
    """
    ...
    headers = {"Authorization": f"Basic {encoded_credentials}", "X-Elastica-Dbname-Resolved": "True"}
