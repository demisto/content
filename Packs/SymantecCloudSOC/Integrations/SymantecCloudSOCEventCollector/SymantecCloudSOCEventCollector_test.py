...
def get_mocked_url(app: str, subtype: str, created_timestamp: str = None, limit: int = None):
    if limit:
...
def test_get_events_command():
    ...
    with requests_mock.Mocker() as request_mocker:
        investigate_mocker = request_mocker.get(
            url=get_mocked_url(app="investigate", subtype="all", created_timestamp="2023-01-01T23%3A23%3A59", limit=1000),
            status_code=200,
            json=response_data["Investigate_logs"],
        )
        detect_incidents_mocker = request_mocker.get(
            url=get_mocked_url(app="Detect", subtype="incidents", created_timestamp="2023-01-01T23%3A23%3A59", limit=1000),
            status_code=200,
            json=response_data["Incident_logs"],
        )
...
def test_test_module():
    ...
    with requests_mock.Mocker() as request_mocker:
        investigate_mocker = request_mocker.get(
            url=get_mocked_url(app="Investigate", subtype="all", created_timestamp="2022-01-01T00%3A00%3A00Z", limit=1),
            status_code=200,
            json={},
        )
        detect_incidents_mocker = request_mocker.get(
            url=get_mocked_url(app="detect", subtype="incidents", limit=1), status_code=200, json={}
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "Investigate_logs",
            {"_id": "_id1", "_time": "2023-01-02T23:23:59", "created_timestamp": "2023-01-02T23:23:59", "type": "Investigate"},
        ),
    ],
)
...
dedup_by_id_test_case: list = [
    ...
    (
        ...
        100,
        "2022-01-02T23:23:59",
        {"last_run": "2023-01-02T23:23:59", "Incident_logs-ids": []},
        [],
    ),
    (
        {"last_run": "2023-01-02T23:23:59"},
        ...
        99,
        "2022-01-02T23:23:59",
        {"last_run": "2023-01-02T23:23:59", "Incident_logs-ids": ["_id1"]},
        [
            {
                "_id": "_id1",
                "_time": "2023-01-02T23:23:59",
    ...
    (
        ...
        98,
        "2022-01-02T23:23:59",
        {"last_run": "2023-01-02T23:23:59", "Incident_logs-ids": ["_id2"]},
        [
            {
                "_id": "_id1",
                "_time": "2023-01-02T22:22:59",
                "incident_start_time": "2023-01-02T22:22:59",
                "type": "Detect incident",
            },
            {
                "_id": "_id2",
    ...
    (
        ...
        [
            {
                "_id": "_id2",
                "_time": "2023-01-02T23:22:59",
                "incident_start_time": "2023-01-02T23:22:59",
                "type": "Detect incident",
            }
        ],
    ),
...
@pytest.mark.parametrize(
    "last_run_for_log_type, log_events, log_type, max_fetch, "
    "number_of_events, last_fetch, expected_last_run,"
    "expected_event_list",
    dedup_by_id_test_case,
)
def test_dedup_by_id_with_last_run(
    last_run_for_log_type, log_events, log_type, max_fetch, number_of_events, last_fetch, expected_last_run, expected_event_list
):
    """
    Given:
...
dedup_by_id_test_case_without_last_run: list = [
    ...
    (
        ...
        [
            {
                "_id": "_id1",
                "_time": "2023-01-03T23:23:59",
                "incident_start_time": "2023-01-03T23:23:59",
                "type": "Detect incident",
            },
            {
                "_id": "_id2",
                "_time": "2023-01-04T23:23:59",
                "incident_start_time": "2023-01-04T23:23:59",
                "type": "Detect incident",
            },
        ],
    ),
...
@pytest.mark.parametrize(
    ...
    "number_of_events, last_fetch, expected_last_run,"
    "expected_event_list",
    dedup_by_id_test_case_without_last_run,
)
def test_dedup_by_id_without_last_run(
    last_run_for_log_type, log_events, log_type, max_fetch, number_of_events, last_fetch, expected_last_run, expected_event_list
):
    """
    Given:
...
get_first_fetch_time_params = [
    ({"first_fetch": "7 months"}, "2022-06-01T23:23:59", "2022-07-05T23:23:59"),
...
def test_create_client_with_authorization():
    ...
    client_results = create_client_with_authorization(
        "http://some_mock_url.com", True, False, key_id="key_id", key_secret="key_secret"
    )

    assert client_results._base_url == "http://some_mock_url.com"
    assert client_results._headers == {"Authorization": "Basic a2V5X2lkOmtleV9zZWNyZXQ=", "X-Elastica-Dbname-Resolved": "True"}
...
def test_get_all_events_for_log_type_with_pagination(
    max_fetch, log_type, last_run, first_fetch_time, first_fetch_time_investigate, expected_all_events_list, expected_last_run
):
    """
    Given:
    ...
    with requests_mock.Mocker() as request_mocker:
        investigate_mocker = request_mocker.get(
            url=get_mocked_url(app="Investigate", subtype="all", created_timestamp="2022-06-21T00%3A00%3A00Z", limit=1000),
            status_code=200,
            json=response_pagination_investigate["page_1_response"],
        )
        detect_incidents_mocker = request_mocker.get(
            url=get_mocked_url(app="detect", subtype="incidents", limit=1000),
            status_code=200,
            json=response_pagination_incident["page_1_response"],
        )
        investigate_mocker = request_mocker.get(
        ...
        detect_incidents_mocker_page = request_mocker.get(
            url="http://some_mock_url.com/incident", status_code=200, json=response_pagination_incident["page_2_response"]
        )
        detect_investigate_mocker_page = request_mocker.get(
            url="http://some_mock_url.com/investigate", status_code=200, json=response_pagination_investigate["page_2_response"]
        )

        all_events_list, last_run_for_log_type = get_all_events_for_log_type(
            mocked_client, max_fetch, log_type, last_run, first_fetch_time, first_fetch_time_investigate
        )
...
def test_get_all_events_for_log_type_without_pagination(
    max_fetch, log_type, last_run, first_fetch_time, first_fetch_time_investigate, expected_all_events_list, expected_last_run
):
    """
    Given:
    ...
    with requests_mock.Mocker() as request_mocker:
        investigate_mocker = request_mocker.get(
            url=get_mocked_url(app="Investigate", subtype="all", created_timestamp="2022-01-01T00%3A00%3A00Z", limit=1000),
            status_code=200,
            json=response_pagination_investigate["page_1_response"],
        )
        investigate_mocker = request_mocker.get(
...
test_main_params: list = [
    (
        ...
        {
            "Investigate_logs": {
                "Investigate_logs-ids": response_pagination_investigate["expected_ids_list"][0:3],
                "last_run": "2021-06-01T00:00:00",
            },
            "Incident_logs": {
                "Incident_logs-ids": response_pagination_incident["expected_ids_list"][0:3],
                "last_run": "2021-06-01T00:00:00",
            },
        },
        [
            ...
            {
                "_id": "id0",
                "incident_start_time": "2021-06-01T00:00:00",
                "_time": "2021-06-01T00:00:00",
                "type": "Detect incident",
            },
            {
                "_id": "id1",
...
def test_fetch_events_command(
    ...
    with requests_mock.Mocker() as request_mocker:
        ...
        detect_incidents_mocker = request_mocker.get(
            url=get_mocked_url(app="detect", subtype="incidents", created_timestamp="2021-06-01T00%3A00%3A00", limit=1000),
            status_code=200,
            json=response_pagination_incident["page_1_response"],
        )
        investigate_mocker = request_mocker.get(
        ...
        next_run, events = fetch_events_command(
            mocked_client, max_fetch, last_run, first_fetch_time, first_fetch_time_investigate
        )
