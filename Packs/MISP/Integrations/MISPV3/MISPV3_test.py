...
TEST_TAG_SCORES = [
    CASE_OF_MALICIOUS_ATTRIBUTE,
    CASE_OF_SUSPICIOUS_ATTRIBUTE,
    CASE_OF_BENIGN_ATTRIBUTE,
    CASE_OF_MALICIOUS_EVENT,
    CASE_OF_SUSPICIOUS_EVENT,
    CASE_OF_BENIGN_EVENT,
    CASE_OF_UNKNOWN,
    CASE_OF_BAD_THREAT_LEVEL_ID,
]
...
TEST_EVENTS_INCLUDE_DETECTED_TAG = [
    ("2", ["149", "145", "144"]),  # 3 events include the detected tag
    ("278", ["145"]),  # 1 event includes the detected attribute's tag
    (None, []),  # no tag was detected, no event returns
    ("104", ["149"]),
]  # 1 event includes the detected event's tag
...
@pytest.mark.parametrize(
    "malicious_tag_ids, suspicious_tag_ids, benign_tag_ids, return_malicious_tag_ids, "
    "return_suspicious_tag_ids, return_benign_tag_ids",
    TAG_IDS_LISTS,
)
def test_handle_tag_duplication_ids(
    mocker,
    malicious_tag_ids,
    suspicious_tag_ids,
    benign_tag_ids,
    return_malicious_tag_ids,
    return_suspicious_tag_ids,
    return_benign_tag_ids,
):
    """
...
@pytest.mark.parametrize(
    "dbot_type, dbot_score_type, value, expected_type_object",
    [
...
def test_get_dbot_indicator(dbot_type: str, dbot_score_type: DBotScoreType, value: Any, expected_type_object: Any) -> None:
...
@pytest.mark.parametrize(
    "is_event_level, expected_output, expected_tag_list_ids",
    [(False, ATTRIBUTE_TAG_LIMIT, {"1", "3"}), (True, ATTRIBUTE_TAG_LIMIT, {"1", "3", "2"})],
)
...
@pytest.mark.parametrize(
    "attribute_tags_ids, event_tags_ids, malicious_tag_ids, suspicious_tag_ids, benign_tag_ids, "
...
def test_get_events_related_to_scored_tag(mocker, detected_tag, expected_related_events):
    ...
    events_to_human_readable = get_events_related_to_scored_tag(reputation_command_outputs.get("Attribute"), detected_tag)
...
def test_parse_response_reputation_command(mocker):
    ...
    benign_tag_ids = []
    attribute_limit = 3
    outputs, _, _, _ = parse_response_reputation_command(
        reputation_response, malicious_tag_ids, suspicious_tag_ids, benign_tag_ids, attribute_limit
    )
...
def get_response(status_code, mocker, mock_response_key):
    ...
    the_response.request = mocker.patch.object(
        demisto,
        "internalHttpRequest",
        return_value={"body": json.dumps({"key": "value"}), "headers": json.dumps({"key": "value"})},
    )
...
@pytest.mark.parametrize(
    "file_path, expected_output_key, mock_response_key",
    [
...
def test_fail_to_add_email_object(mocker):
    ...
    event_id = 1231
    demisto_args: dict = {"entry_id": "", "event_id": event_id}
    mocker.patch.object(demisto, "getFilePath", return_value={"path": "test_data/test_add_email_object_case_1.eml"})
...
def test_add_msg_email_object(mocker):
    ...
    mocker.patch.object(
        demisto,
        "getFilePath",
        return_value={"name": "test_add_email_object_case_1.msg", "path": "test_data/test_add_email_object_case_1.msg"},
    )
    with pytest.raises(DemistoException) as exception_info:
        add_email_object(demisto_args)
...
def test_add_custom_object(mocker):
    ...
    expected_output = {"readable_output": f"Object has been added to MISP event ID {event_id}", "outputs": response_add_obj}
...
def test_add_user_to_misp(mocker):
    ...
    demisto_args = {"email": "test@example.com", "org_id": "123", "role_id": "456", "password": "password123"}
...
def test_get_organizations_info(mocker):
    ...
    mock_organizations = [{"Organisation": {"id": 1, "name": "Org1"}}, {"Organisation": {"id": 2, "name": "Org2"}}]
    mocker.patch("MISPV3.PYMISP.organisations", return_value=mock_organizations)
    result = get_organizations_info()
    expected_output = {"MISP.Organization": [{"id": 1, "name": "Org1"}, {"id": 2, "name": "Org2"}]}
...
def test_get_role_info(mocker):
    ...
    mock_roles = [{"Role": {"id": 1, "name": "Role1"}}, {"Role": {"id": 2, "name": "Role2"}}]
    mocker.patch("MISPV3.PYMISP.roles", return_value=mock_roles)
    result = get_role_info()
    expected_output = {"MISP.Role": [{"id": 1, "name": "Role1"}, {"id": 2, "name": "Role2"}]}
