...
class Client(BaseClient):
    ...
    def _call_api(
        self, method: str, url_suffix: str, params: dict = None, json_data: dict = None, resp_type: str = "json"
    ) -> Dict[str, Any]:
        if self.is_infinity:
    ...
    def restore_requests(
        ...
        request_data: dict[str, Any] = {
            ...
            "entityExtendedFilter": [
                ...
                {"saasAttrName": "entityPayload.restoreRequestTime", "saasAttrOp": "greaterThan", "saasAttrValue": start_date},
                {"saasAttrName": "entityPayload.isRestoreDeclined", "saasAttrOp": denied_attr_op, "saasAttrValue": "true"},
    ...
    def query_events(
        ...
        payload = {"requestData": request_data}
        return self._call_api("POST", url_suffix="event/query", json_data=payload)

    def get_entity(self, entity: str) -> dict[str, Any]:
        return self._call_api("GET", url_suffix=f"search/entity/{entity}")

    def get_email(self, entity: str) -> dict[str, Any]:
        return self._call_api("GET", url_suffix=f"soar/entity/{entity}")
    ...
    def send_action(self, entities: list, entity_type: str, action: str, restore_decline_reason: str = None):
        request_data = assign_params(
            entityIds=entities, entityType=entity_type, entityActionName=action, restoreDeclineReason=restore_decline_reason
        )
        payload = {"requestData": request_data}
    ...
    def send_notification(self, entity: str, emails: List[str]):
        payload = {"requestData": {"entityId": entity, "emails": emails}}
        return self._call_api("POST", "soar/notify", json_data=payload)

    def report_mis_classification(self, entities: List[str], classification: str, confident: str):
        request_data = {"entityIds": entities, "classification": classification, "confident": confident}
        payload = {"requestData": request_data}
    ...
    def create_ap_exception(
        ...
        nickname: str = None,
        recipient: str = None,
        sender_client_ip: str = None,
        from_domain_ends_with: str = None,
        sender_ip: str = None,
        email_link: List[str] = None,
        subject: str = None,
        comment: str = None,
        action_needed: str = None,
        ignoring_spf_check: bool = None,
        subject_matching: str = None,
        email_link_matching: str = None,
        from_name_matching: str = None,
        from_domain_matching: str = None,
        from_email_matching: str = None,
        recipient_matching: str = None,
    ):
        request_data = assign_params(
            entityId=entity_id,
    ...
    def create_sectool_exception(
        ...
        if exc_payload_condition:
            request_data["exceptionPayload"] = {"condition": exc_payload_condition}
        payload = {"requestData": request_data}
        return self._call_api("POST", f"sectool-exceptions/{sectool}", json_data=payload)

    def update_sectool_exception(
        self, sectool: str, exc_type: str, exc_str: str, comment: str = None, exc_payload_condition: str = None
    ):
        request_data = assign_params(exceptionType=exc_type, exceptionStr=exc_str, comment=comment)
        if exc_payload_condition:
            request_data["exceptionPayload"] = {"condition": exc_payload_condition}
        payload = {"requestData": request_data}
    ...
    def delete_sectool_exception(self, sectool: str, exc_type: str, exc_str: str, entity_type: str = None, entity_id: str = None):
        request_data = assign_params(exceptionType=exc_type, exceptionStr=exc_str, entityType=entity_type, entityId=entity_id)
        payload = {"requestData": request_data}
        return self._call_api("DELETE", f"sectool-exceptions/{sectool}", json_data=payload)

    def get_sectool_exceptions(
        self,
        sectool: str,
        exc_type: str,
        filter_str: str = None,
        ...
        payload = {"requestData": request_data}
        return self._call_api("GET", f"sectool-exceptions/{sectool}/exceptions/{exc_type}", json_data=payload)

    def delete_sectool_exceptions(
        self, sectool: str, exc_type: str, exc_str_list: List[str], entity_type: str = None, entity_id: str = None
    ):
        request_data = assign_params(
            exceptionType=exc_type, exceptionStrList=exc_str_list, entityType=entity_type, entityId=entity_id
        )
        payload = {"requestData": request_data}
        return self._call_api("DELETE", f"sectool-exceptions/{sectool}/exceptions", json_data=payload)
    ...
    def create_anomaly_exceptions(self, request_json: dict, added_by: str = None):
        request_data = assign_params(requestJson=request_json, addedBy=added_by)
        payload = {"requestData": request_data}
    ...
    def create_ctp_list_item(self, list_id: str, list_item_name: str, created_by: str):
        request_data = {"listId": list_id, "listItemName": list_item_name, "createdBy": created_by}
        payload = {"requestData": request_data}
    ...
    def delete_ctp_list_items(self, list_item_ids: List[str]):
        request_data = {"listItemIds": list_item_ids}
        payload = {"requestData": request_data}
...
def checkpointhec_get_entity(client: Client, args: dict) -> CommandResults:
    ...
    if entities := result.get("responseData"):
        ...
        return CommandResults(readable_output=f"Entity with id {entity} not found")
...
def checkpointhec_get_events(client: Client, args: dict) -> CommandResults:
    ...
    if events := result.get("responseData"):
        ...
        return CommandResults(readable_output="Events not found with the given criteria")
...
def checkpointhec_get_scan_info(client: Client, args: dict) -> CommandResults:
    ...
    if entities := result.get("responseData"):
        ...
        return CommandResults(outputs_prefix="CheckPointHEC.ScanResult", outputs=outputs)
    else:
        return CommandResults(readable_output=f"Entity with id {entity} not found")
...
def checkpointhec_search_emails(client: Client, args: dict, params: dict) -> CommandResults:
    ...
    if date_last:
        if date_from or date_to:
            return CommandResults(readable_output=f"Argument {date_last=} cannot be used with {date_from=} or {date_to=}")
        else:
            if _start_date := dateparser.parse(date_last, date_formats=[DATE_FORMAT]):
                start_date = _start_date.isoformat()
            else:
                return CommandResults(readable_output=f"Could not establish start date with {date_last=}")
    elif date_from:
        start_date = date_from
        if date_to:
            end_date = date_to
    else:
        return CommandResults(readable_output="Argument date_last and date_from cannot be both empty")

    if subject_contains and subject_match:
        return CommandResults(readable_output=f"Argument {subject_contains=} and {subject_match=} cannot be both set")

    if sender_contains and sender_match:
        return CommandResults(readable_output=f"Argument {sender_contains=} and {sender_match=} cannot be both set")

    if recipients_contains and recipients_match:
        return CommandResults(readable_output=f"Argument {recipients_contains=} and {recipients_match=} cannot be both set")

    if name_contains and name_match:
        return CommandResults(readable_output=f"Argument {name_contains=} and {name_match=} cannot be both set")
    ...
    if entities := result.get("responseData"):
        ...
        return CommandResults(outputs_prefix="CheckPointHEC.Entity", readable_output=human_readable, outputs=emails)
    else:
        return CommandResults(readable_output="Emails with the specified parameters were not found")
...
def checkpointhec_send_action(client: Client, args: dict) -> CommandResults:
    ...
    if resp := result.get("responseData"):
        return CommandResults(outputs_prefix="CheckPointHEC.Task", outputs={"task": resp[0]["taskId"]})
    else:
...
def checkpointhec_get_action_result(client: Client, args: dict) -> CommandResults:
    ...
    if resp := result.get("responseData"):
        return CommandResults(outputs_prefix="CheckPointHEC.ActionResult", outputs=resp)
    else:
        return CommandResults(readable_output=f"Cannot get results about task with id {task}")
...
def checkpointhec_send_notification(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("ok"):
        return CommandResults(outputs_prefix="CheckPointHEC.Notification", outputs=result)
    else:
...
def checkpointhec_report_mis_classification(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 200:
        return CommandResults(readable_output="Mis-classification reported successfully")
    else:
...
def checkpointhec_get_ap_exceptions(client: Client, args: dict) -> CommandResults:
    ...
    if exceptions := result.get("responseData"):
        ...
        return CommandResults(readable_output="No Anti-Phishing exceptions found")
...
def checkpointhec_create_ap_exception(client: Client, args: dict) -> CommandResults:
    ...
    result = client.create_ap_exception(
        exc_type,
        entity_id,
        attachment_md5,
        from_email,
        ...
        email_link_matching,
        from_name_matching,
        from_domain_matching,
        from_email_matching,
        recipient_matching,
    )
    if result.get("responseEnvelope", {}).get("responseCode") == 200:
        return CommandResults(readable_output="Anti-Phishing exception created successfully")
    else:
...
def checkpointhec_update_ap_exception(client: Client, args: dict) -> CommandResults:
    ...
    result = client.update_ap_exception(
        exc_type,
        exc_id,
        entity_id,
        attachment_md5,
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 200:
        return CommandResults(readable_output="Anti-Phishing exception updated successfully")
    else:
...
def checkpointhec_delete_ap_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="Anti-Phishing exception deleted successfully")
    else:
...
def checkpointhec_get_cp2_exception(client: Client, args: dict) -> CommandResults:
    ...
    if exception := result.get("responseData"):
        ...
        return CommandResults(readable_output="No Anti-Malware exception found")
...
def checkpointhec_create_cp2_exception(client: Client, args: dict) -> CommandResults:
    ...
    result = client.create_sectool_exception(
        ANTI_MALWARE_SAAS_NAME,
        exc_type,
        exc_str,
        entity_type,
        entity_id,
        comment,
        exc_payload_condition,
        file_name,
        created_by_email,
        is_exclusive,
    )
    if result.get("responseEnvelope", {}).get("responseCode") == 201:
        return CommandResults(readable_output="Anti-Malware exception created successfully")
    else:
...
def checkpointhec_update_cp2_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 200:
        return CommandResults(readable_output="Anti-Malware exception updated successfully")
    else:
...
def checkpointhec_delete_cp2_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="Anti-Malware exception deleted successfully")
    else:
...
def checkpointhec_get_cp2_exceptions(client: Client, args: dict) -> CommandResults:
    ...
    result = client.get_sectool_exceptions(
        ANTI_MALWARE_SAAS_NAME, exc_type, filter_str, filter_index, sort_dir, last_evaluated_key, insert_time_gte, limit
    )
    if exceptions := result.get("responseData"):
        human_readable = tableToMarkdown("exceptions", exceptions, removeNull=True)
        return CommandResults(
        ...
        return CommandResults(readable_output="No Anti-Malware exceptions found")
...
def checkpointhec_delete_cp2_exceptions(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="Anti-Malware exceptions deleted successfully")
    else:
...
def checkpointhec_get_anomaly_exceptions(client: Client) -> CommandResults:
    ...
    if exceptions := result.get("responseData"):
        ...
        return CommandResults(readable_output="No Anomaly exceptions found")
...
def checkpointhec_create_anomaly_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 201:
        return CommandResults(readable_output="Anomaly exception created successfully")
    else:
...
def checkpointhec_delete_anomaly_exceptions(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="Anomaly exceptions deleted successfully")
    else:
...
def checkpointhec_get_ctp_lists(client: Client) -> CommandResults:
    ...
    if lists := result.get("responseData"):
        ...
        return CommandResults(readable_output="No CTP lists found")
...
def checkpointhec_get_ctp_list(client: Client, args: dict) -> CommandResults:
    ...
    if lists := result.get("responseData"):
        ...
        return CommandResults(readable_output="No CTP list found")
...
def checkpointhec_get_ctp_list_items(client: Client) -> CommandResults:
    ...
    if items := result.get("responseData"):
        ...
        return CommandResults(readable_output="No CTP list items found")
...
def checkpointhec_create_ctp_list_item(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 201:
        return CommandResults(readable_output="CTP list item created successfully")
    else:
...
def checkpointhec_update_ctp_list_item(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 200:
        return CommandResults(readable_output="CTP list item updated successfully")
    else:
...
def checkpointhec_delete_ctp_list_item(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="CTP list item deleted successfully")
    else:
...
def checkpointhec_delete_ctp_list_items(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="CTP list items deleted successfully")
    else:
...
def checkpointhec_delete_ctp_lists(client: Client) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="CTP lists deleted successfully")
    else:
...
def checkpointhec_get_avurl_exception(client: Client, args: dict) -> CommandResults:
    ...
    if exception := result.get("responseData"):
        ...
        return CommandResults(readable_output="No Avanan URL exception found")
...
def checkpointhec_create_avurl_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 201:
        return CommandResults(readable_output="Avanan URL exception created successfully")
    else:
...
def checkpointhec_update_avurl_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 200:
        return CommandResults(readable_output="Avanan URL exception updated successfully")
    else:
...
def checkpointhec_delete_avurl_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="Avanan URL exception deleted successfully")
    else:
...
def checkpointhec_get_avurl_exceptions(client: Client, args: dict) -> CommandResults:
    ...
    result = client.get_sectool_exceptions(
        AVANAN_URL_SAAS_NAME, exc_type, filter_str, filter_index, sort_dir, last_evaluated_key, insert_time_gte, limit
    )
    if exceptions := result.get("responseData"):
        human_readable = tableToMarkdown("exceptions", exceptions, removeNull=True)
        return CommandResults(
        ...
        return CommandResults(readable_output="No Avanan URL exceptions found")
...
def checkpointhec_delete_avurl_exceptions(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="Avanan URL exceptions deleted successfully")
    else:
...
def checkpointhec_get_avdlp_exception(client: Client, args: dict) -> CommandResults:
    ...
    if exception := result.get("responseData"):
        ...
        return CommandResults(readable_output="No Avanan DLP exception found")
...
def checkpointhec_create_avdlp_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 201:
        return CommandResults(readable_output="Avanan DLP exception created successfully")
    else:
...
def checkpointhec_update_avdlp_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 200:
        return CommandResults(readable_output="Avanan DLP exception updated successfully")
    else:
...
def checkpointhec_delete_avdlp_exception(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="Avanan DLP exception deleted successfully")
    else:
...
def checkpointhec_get_avdlp_exceptions(client: Client, args: dict) -> CommandResults:
    ...
    result = client.get_sectool_exceptions(
        AVANAN_DLP_SAAS_NAME, exc_type, filter_str, filter_index, sort_dir, last_evaluated_key, insert_time_gte, limit
    )
    if exceptions := result.get("responseData"):
        human_readable = tableToMarkdown("exceptions", exceptions, removeNull=True)
        return CommandResults(
        ...
        return CommandResults(readable_output="No Avanan DLP exceptions found")
...
def checkpointhec_delete_avdlp_exceptions(client: Client, args: dict) -> CommandResults:
    ...
    if result.get("responseEnvelope", {}).get("responseCode") == 204:
        return CommandResults(readable_output="Avanan DLP exceptions deleted successfully")
    else:
...
def main() -> None:  # pragma: no cover
    ...
    client = Client(base_url, client_id, client_secret, verify, proxy)
