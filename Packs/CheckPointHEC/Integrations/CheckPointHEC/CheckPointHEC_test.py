...
from CheckPointHEC import (
    ...
    checkpointhec_create_avdlp_exception,
    checkpointhec_create_avurl_exception,
    checkpointhec_create_cp2_exception,
    checkpointhec_create_ctp_list_item,
    checkpointhec_delete_anomaly_exceptions,
    checkpointhec_delete_ap_exception,
    checkpointhec_delete_avdlp_exception,
    checkpointhec_delete_avdlp_exceptions,
    checkpointhec_delete_avurl_exception,
    checkpointhec_delete_avurl_exceptions,
    checkpointhec_delete_cp2_exception,
    checkpointhec_delete_cp2_exceptions,
    checkpointhec_delete_ctp_list_item,
    checkpointhec_delete_ctp_list_items,
    checkpointhec_delete_ctp_lists,
    checkpointhec_download_email,
    checkpointhec_get_action_result,
    checkpointhec_get_anomaly_exceptions,
    checkpointhec_get_ap_exceptions,
    checkpointhec_get_cp2_exception,
    checkpointhec_get_cp2_exceptions,
    checkpointhec_get_ctp_list,
    checkpointhec_get_ctp_list_item,
    checkpointhec_get_ctp_list_items,
    checkpointhec_get_ctp_lists,
    checkpointhec_get_entity,
    checkpointhec_get_events,
    checkpointhec_get_scan_info,
    checkpointhec_report_mis_classification,
    checkpointhec_search_emails,
    checkpointhec_send_action,
    checkpointhec_send_notification,
    checkpointhec_update_ap_exception,
    checkpointhec_update_avdlp_exception,
    checkpointhec_update_avurl_exception,
    checkpointhec_update_cp2_exception,
...
def test_generate_signature_with_request_string():
    ...
    assert (
        client._generate_signature(
            f"{'0' * 8}-{'0' * 4}-{'0' * 4}-{'0' * 4}-{'0' * 12}", "2023-08-13T19:08:35.263817", "/v1.0/soar/test"
        )
...
def test_generate_signature_with_no_request_string():
    ...
    assert (
        client._generate_signature(f"{'0' * 8}-{'0' * 4}-{'0' * 4}-{'0' * 4}-{'0' * 12}", "2023-08-13T19:08:35.263817")
...
def test_token_header(mocker):
    ...
    get_token = mocker.patch.object(Client, "_get_token")
...
def test_infinity_token_header(mocker):
    ...
    get_token = mocker.patch.object(Client, "_generate_infinity_token")
...
def test_get_token_empty(mocker):
    ...
    _token = "super token"
    mocker.patch.object(Client, "_http_request", return_value=_token)
...
def test_call_smart_api(mocker):
    ...
    get_headers = mocker.patch.object(Client, "_get_headers", return_value={})
...
def test_checkpointhec_get_entity_fail(mocker):
    ...
    call_api = mocker.patch.object(Client, "_call_api", return_value={"responseData": []})
...
def test_checkpointhec_get_events_success(mocker):
    ...
    result = checkpointhec_get_events(
        client,
        {
...
def test_checkpointhec_search_emails_fail(mocker):
    ...
    call_api = mocker.patch.object(Client, "_call_api")
    ...
    result = checkpointhec_search_emails(
        client, {"date_last": "1 day", "subject_contains": "Any subject, ...", "subject_match": "This subject"}, {}
    )
    call_api.assert_not_called()
    assert result.readable_output == (
    ...
    result = checkpointhec_search_emails(
        client, {"date_last": "1 day", "recipients_contains": "a@b.c", "recipients_match": "d@e.f"}, {}
    )
    call_api.assert_not_called()
    assert result.readable_output == "Argument recipients_contains='a@b.c' and recipients_match='d@e.f' cannot be both set"

    result = checkpointhec_search_emails(client, {"date_last": "1 day", "name_contains": "My Nam", "name_match": "My Name"}, {})
    call_api.assert_not_called()
...
def test_checkpointhec_send_action(mocker):
    ...
    result = checkpointhec_send_action(client, {"entity": "0" * 32, "saas": "Microsoft Exchange", "action": "restore"})
    call_api.assert_called_once()
...
def test_report_mis_classification_success(mocker):
    ...
    result = checkpointhec_report_mis_classification(
        client, {"entities": "0" * 32, "classification": "Clean Email", "confident": "Not so sure"}
    )
    call_api.assert_called_once()
...
def test_create_cp2_exception_success(mocker):
    ...
    result = checkpointhec_create_cp2_exception(
        client, {"exc_type": "file_type", "exc_str": ".pdf", "comment": "From Unit Tests"}
    )
    call_api.assert_called()
...
def test_create_cp2_exception_fail(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        checkpointhec_create_cp2_exception(client, {"exc_type": "not_file_type", "exc_str": ".pdf", "comment": "From Unit Tests"})
...
def test_create_anomaly_exception_success(mocker):
    ...
    result = checkpointhec_create_anomaly_exception(
        client,
        {
            "request_json": {
...
def test_create_ctp_list_item_success(mocker):
    ...
    result = checkpointhec_create_ctp_list_item(
        client, {"list_id": "0", "list_item_name": "example.com", "created_by": "a@b.test"}
    )
    call_api.assert_called()
...
def test_create_ctp_list_item_fail(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        checkpointhec_create_ctp_list_item(client, {"list_id": "-1", "list_item_name": "example.com", "created_by": "a@b.test"})
...
def test_update_ctp_list_item_success(mocker):
    ...
    result = checkpointhec_update_ctp_list_item(
        client, {"item_id": "00000000000", "list_id": "0", "list_item_name": "new.example.com", "created_by": "a@b.test"}
    )
    call_api.assert_called()
...
def test_update_ctp_list_item_fail(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        checkpointhec_update_ctp_list_item(
            client, {"item_id": "00000000000", "list_id": "-1", "list_item_name": "example.com", "created_by": "a@b.test"}
        )
...
def test_create_avurl_exception_success(mocker):
    ...
    result = checkpointhec_create_avurl_exception(
        client, {"exc_type": "allow-url", "exc_str": "example.com", "comment": "From Unit Tests"}
    )
    call_api.assert_called()
...
def test_create_avurl_exception_fail(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        checkpointhec_create_avurl_exception(
            client, {"exc_type": "not-allow-url", "exc_str": "example.com", "comment": "From Unit Tests"}
        )
...
def test_update_avurl_exception_success(mocker):
    ...
    result = checkpointhec_update_avurl_exception(
        client, {"exc_type": "allow-url", "exc_str": "example.com", "comment": "New comment"}
    )
    call_api.assert_called()
...
def test_update_avurl_exception_fail(mocker):
    ...
    with pytest.raises(DemistoException) as e:
        checkpointhec_update_avurl_exception(
            client, {"exc_type": "not-allow-url", "exc_str": "example.com", "comment": "New comment"}
        )
