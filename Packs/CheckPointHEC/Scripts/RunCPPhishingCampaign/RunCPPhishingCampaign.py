...
def get_sender_and_subject(entity: str) -> tuple[str, str]:
    email_info = demisto.executeCommand("checkpointhec-get-entity", {"entity": entity})[0]["Contents"]
...
def search_and_quarantine(date_range: str, sender: str, subject: str):
    result = demisto.executeCommand(
        "checkpointhec-search-emails", {"date_last": date_range, "sender_match": sender, "subject_contains": subject}
    )

    entity_ids = [x.get("entityId") for x in result[0].get("Contents")]
    if entity_ids:
        result = demisto.executeCommand("checkpointhec-send-action", {"entity": entity_ids, "action": "quarantine"})
        task = result[0]["Contents"]["task"]
        demisto.executeCommand("setIncident", {"customFields": json.dumps({"checkpointheccampaigntask": task})})
    return result
