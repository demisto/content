...
3. When running the `proofpoint-es-get-events` command, HTTP 400 (Bad Request) errors may be raised if the time range is older than 30 days. Ensure both the `since_time` and `to_time` arguments fall within the last 30 days.
