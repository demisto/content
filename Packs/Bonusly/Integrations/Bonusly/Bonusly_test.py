...
def test_bonusly_module(requests_mock):
    ...
    mock_response = {"success": True, "result": [{"id": "111111111111111111"}]}

    requests_mock.get("https://test.com/bonuses", json=mock_response)
    test_api_key = "TESTAPIKEY"
    bearer_token = "Bearer " + test_api_key
    client = Client(
        base_url="https://test.com/", verify=False, headers={"Authorization": bearer_token, "Content-Type": "application/json"}
    )
...
def test_bonusly_delete_bonus_command(requests_mock):
    ...
    mock_response = {"success": True, "message": "Delete example"}
