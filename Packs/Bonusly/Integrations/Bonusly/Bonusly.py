...
class Client(BaseClient):
    ...
    def bonusly_get_bonus_http_request(self, url_params) -> dict:
        ...
        """
        id = url_params["id"]
        result = self._http_request(method="GET", url_suffix="/bonuses/" + id)
        return result
    ...
    def bonusly_update_bonus_http_request(self, url_params) -> dict:
        ...
        result = self._http_request(method="PUT", url_suffix="/bonuses/" + id, data=valstring)
        return result
    ...
    def bonusly_delete_bonus_http_request(self, url_params) -> dict:
        ...
        """
        id = url_params["id"]
        result = self._http_request(method="DELETE", url_suffix="/bonuses/" + id)
        return result
...
def bonusly_delete_bonus_command(client, args):
    ...
    outputs = {"Bonusly.Bonus.message": result_text}
    ...
    try:
        ...
        if demisto.command() == "test-module":
            ...
            next_run, incidents = fetch_incidents(
                client=client, last_run=demisto.getLastRun(), first_fetch_time=first_fetch_time, fetch_params=params
            )
