...
def ua_parse_command():
    ...
    if "success" in r["result"]["code"]:
        ...
        ec = {"UA.Parse(val.UserAgent && val.UserAgent == obj.UserAgent)": ua_ec, "DBotScore": dbot_score}
        demisto.results(
            {
    ...
    if r["result"]["code"] == "error":
        ...
        demisto.results(
            {"Type": entryTypes["note"], "ContentsFormat": formats["markdown"], "Contents": r, "HumanReadable": f"{error_msg}"}
        )
