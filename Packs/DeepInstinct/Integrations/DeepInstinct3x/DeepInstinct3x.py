...
def http_request(method, url_suffix, json=None):
    ...
    try:
        ...
        headers = {"Content-Type": "application/json", "Accept": "application/json", "Authorization": str(api_key)}
...
def get_events():
    ...
    return_results(CommandResults(readable_output=tableToMarkdown("Events", events), outputs=ec, raw_response=events))
...
def get_all_groups():
    ...
    return_results(CommandResults(readable_output=tableToMarkdown("Groups", result), outputs=ec, raw_response=result))
...
def get_all_policies():
    ...
    return_results(CommandResults(readable_output=tableToMarkdown("Policies", result), outputs=ec, raw_response=result))
