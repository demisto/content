...
def http_request(method, url_suffix, json=None):
    ...
    try:
        ...
        return_error(message=f"Error occurred on API call: {base_url + api_suffix + url_suffix}. Error is: {e!s}")
...
def get_events():
    ...
    return_outputs(readable_output=tableToMarkdown("Events", events), outputs=ec, raw_response=events)
...
def get_all_groups():
    ...
    return_outputs(readable_output=tableToMarkdown("Groups", result), outputs=ec, raw_response=result)
...
def get_all_policies():
    ...
    return_outputs(readable_output=tableToMarkdown("Policies", result), outputs=ec, raw_response=result)
...
def add_hash_to_blacklist():
    ...
    http_request("POST", f"/policies/{policy_id!s}/blacklist/hashes/{file_hash}", json={"comment": comment})
    demisto.results("ok")
...
def add_hash_to_whitelist():
    ...
    file_hash = demisto.args().get("file_hash")
    comment = demisto.args().get("comment") or ""
    http_request("POST", f"/policies/{policy_id!s}/whitelist/hashes/{file_hash}", json={"comment": comment})
    demisto.results("ok")
...
def remove_hash_from_blacklist():
    ...
    policy_id = demisto.args().get("policy_id")
    file_hash = demisto.args().get("file_hash")
    http_request("DELETE", f"/policies/{policy_id!s}/blacklist/hashes/{file_hash}")
    demisto.results("ok")
...
def remove_hash_from_whitelist():
    ...
    policy_id = demisto.args().get("policy_id")
    file_hash = demisto.args().get("file_hash")
    http_request("DELETE", f"/policies/{policy_id!s}/whitelist/hashes/{file_hash}")
    demisto.results("ok")
