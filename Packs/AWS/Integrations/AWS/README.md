...
***
Waits until the specified EC2 instances reach the 'stopped' state. Checks every `waiter_delay` seconds until successful or until the maximum number of attempts (`waiter_max_attempts`) is reached.```
...
***
Waits until EC2 instances are in the 'terminated' state. Checks every waiter_delay arg value seconds until a successful state is reached (default max waiter_max_attempts arg value  attempts).
