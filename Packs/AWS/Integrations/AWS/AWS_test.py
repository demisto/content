...
def test_ec2_create_launch_template_command_with_all_parameters(mocker):
    ...
    mocker.patch("AWS.serialize_response_with_datetime_encoding", return_value=mock_client.create_launch_template.return_value)
    mocker.patch("AWS.parse_tag_specifications", return_value=[{"ResourceType": "instance", "Tags": [{"Key": "Name", "Value": "Test"}]}])
...
def test_parse_tag_specifications_with_valid_input():
    """
    Given: A valid tag specifications string with resource type and tags.
    When: parse_tag_specifications processes the input.
    Then: It should return a list with properly formatted tag specification dictionaries.
    """
    from AWS import parse_tag_specifications

    result = parse_tag_specifications("instance:key=Name,value=TestInstance;key=Environment,value=Production")
    assert len(result) == 1
    assert result[0]["ResourceType"] == "instance"
    assert len(result[0]["Tags"]) == 2
    assert result[0]["Tags"][0] == {"Key": "Name", "Value": "TestInstance"}
    assert result[0]["Tags"][1] == {"Key": "Environment", "Value": "Production"}


def test_parse_tag_specifications_with_multiple_resource_types():
    """
    Given: A tag specifications string with multiple resource types.
    When: parse_tag_specifications processes the input.
    Then: It should return a list with multiple tag specification dictionaries.
    """
    from AWS import parse_tag_specifications

    result = parse_tag_specifications("instance:key=Name,value=Instance1#volume:key=Name,value=Volume1")
    assert len(result) == 2
    assert result[0]["ResourceType"] == "instance"
    assert result[0]["Tags"][0] == {"Key": "Name", "Value": "Instance1"}
    assert result[1]["ResourceType"] == "volume"
    assert result[1]["Tags"][0] == {"Key": "Name", "Value": "Volume1"}


def test_parse_tag_specifications_with_none_input():
    """
    Given: A None value is passed to parse_tag_specifications function.
    When: The function attempts to process the None input.
    Then: It should return an empty list.
    """
    from AWS import parse_tag_specifications

    result = parse_tag_specifications(None)
    assert result == []


def test_parse_tag_specifications_with_empty_string():
    """
    Given: An empty string is passed to parse_tag_specifications function.
    When: The function attempts to process the empty string.
    Then: It should return an empty list.
    """
    from AWS import parse_tag_specifications

    result = parse_tag_specifications("")
    assert result == []
