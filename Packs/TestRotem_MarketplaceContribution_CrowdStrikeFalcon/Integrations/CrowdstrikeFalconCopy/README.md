...
## Configure CrowdstrikeFalcon_copy on Cortex XSOAR

1. Navigate to **Settings** > **Integrations** > **Servers & Services**.
2. Search for CrowdstrikeFalcon_copy.
...
You can enable incident mirroring between Cortex XSOAR incidents and CrowdstrikeFalcon_copy corresponding events (available from Cortex XSOAR version 6.0.0).
...
2. In the *Mirroring Direction* integration parameter, select in which direction the incidents should be mirrored:
    ...
    | Incoming | Any changes in CrowdstrikeFalcon_copy events (mirroring incoming fields) will be reflected in Cortex XSOAR incidents. |
    | Outgoing | Any changes in Cortex XSOAR incidents will be reflected in CrowdstrikeFalcon_copy events (outgoing mirrored fields). |
    | Incoming And Outgoing | Changes in Cortex XSOAR incidents and CrowdstrikeFalcon_copy events will be reflected in both directions. |

3. Optional: Check the *Close Mirrored XSOAR Incident* integration parameter to close the Cortex XSOAR incident when the corresponding event is closed in CrowdstrikeFalcon_copy.

Newly fetched incidents will be mirrored in the chosen direction. However, this selection does not affect existing incidents.
**Important Note:** To ensure the mirroring works as expected, mappers are required, both for incoming and outgoing, to map the expected fields in Cortex XSOAR and CrowdstrikeFalcon_copy.
