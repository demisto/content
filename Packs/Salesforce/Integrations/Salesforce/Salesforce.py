...
def create_entry(raw_info, options):
    ...
    entry = {
        ...
        "EntryContext": {context_path: mapped_list if len(mapped_list) > 1 else mapped_list[0]},
    }
    return entry
...
def get_new_token():
    ...
    if response.status_code < 200 or response.status_code >= 300:
        raise Exception(
            "Failed to get new token, request status code: " + str(response.status_code) + " and Body: " + response.text + "."
        )
...
def send_request(method, url, body, token=None):
    ...
    response = requests.request(method=method, url=url, headers=headers, data=body, verify=verify, proxies=proxies)
...
def org_to_entry(raw_info, title, user_mapping):
    ...
    return create_entry(
        raw_info,
        {
            ...
            "title": title,
            "data": [{"to": "ID", "from": "Id", "humanReadable": False}, {"to": "Name", "from": "Name"}],
        },
    )
...
def tasks_to_entry(raw_info, title, lead_dict):
    ...
    if lead_dict:
        for i in range(len(raw_info)):
            ...
            raw_info[i]["WhoId"] = lead_dict.get(raw_info[i].get("WhoId")) or raw_info[i].get("WhoId")
...
def users_to_entry(raw_info, title, *args):
    return create_entry(
        raw_info,
        {
    ...
    if response_data.get("success") is not True:
        return {
            ...
            "HumanReadable": tableToMarkdown(
                "Request failed with errors", response_data.get("errors"), headerTransform=dot_to_space
            ),
        }
...
def get_user(oid, case_number):
    ...
    if oid is not None:
        ...
        # Replicating JS behavior where accessing property of function returns undefined (None in Python via getattr)
        return users_to_entry(getattr(users_to_entry, "records", None), "User #" + str(users_oid) + ":", get_user_names())
...
def get_org_name(case_number):
    ...
    return {"Type": EntryType.ERROR, "Contents": "You must specify a Case Number", "ReadableContentsFormat": EntryFormat.MARKDOWN}


# Add the capability to post comment in specific case
def post_case_comment(oid, case_number, text):
    data = {"CommentBody": text, "ParentId": case_number}
...
def push_comment(oid, text, link_url):
    ...
    if link_url is not None:
        data["body"]["messageSegments"].append({"type": "Link", "url": link_url})
...
def push_comment_thread(id, text):
    ...
    message = response.json()
    output = {"Body": message.get("body", {}).get("text"), "CreatedDate": message.get("createdDate"), "URL": message.get("url")}

    ec = {
        "SalesForce.Comment(val.URL && val.URL == obj.URL)": {"URL": URI_PREFIX + "chatter/feed-elements/" + id, "Reply": output}
    }
                ...
                if parent_text is not None and "DemistoID" in parent_text:
                    ...
                    for j in range(len(message_segments)):
                        if message_segments[j].get("type") == "Text":
                            ...
                            return [{"name": parent_text, "details": message_segments[j].get("text")}]
