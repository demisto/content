...
def get_splunk_license_status():
    ...
    mod_license = requests.get(baseurl + 'services/licenser/licenses', data=args, headers=auth_header,
                               verify=VERIFY_CERTIFICATE)

    data = json.loads(mod_license.content)
    for entry in data['entry']:
        if entry['content']['group_id'] == 'Enterprise':
            status = entry['content']['status']
            return status
        # elif entry['content']['group_id'] == 'Free':
        #    status = 'Free Version'
        else:
            status = 'EXPIRED'
            return status
...
def main():
    ...
    if demisto.command() == 'get-license-status':
        return_results(get_splunk_license_status())
