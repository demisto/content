...
FE_CONFIDENCE_TO_REPUTATION = {Common.DBotScore.BAD: 70, Common.DBotScore.SUSPICIOUS: 30, Common.DBotScore.NONE: 0}
        ...
        for value in values:
            ...
            if indicator["type"]:
                ...
                indicator["score"] = self.calculate_indicator_reputation(
                    raw_data.get("confidence", 0), raw_data.get("created", "2000-01-01T00:00:00.000Z"), self.reputation_interval
                )
...
class Client(BaseClient):
    ...
    def fetch_new_access_token(self) -> str:
        ...
        integration_context = demisto.getIntegrationContext()
        integration_context.update({"auth_token": auth_token, "expiration_time": epoch_expiration_time})
    ...
    def fetch_all_indicators_from_api(self, limit: int) -> tuple[list, dict, dict, str]:
        ...
        while round_count != 50:
            ...
            response = self._http_request(
                method="GET", url_suffix=query_url, headers=headers, timeout=self._polling_timeout, resp_type="response"
            )
            round_count += 1

            if response.status_code == 204:
                demisto.info(
                    f"{INTEGRATION_NAME} info - "
            ...
            if response.status_code != 200:
                return_error(
                    f"{INTEGRATION_NAME} indicators fetching - "
    ...
    def fetch_all_reports_from_api(self, limit: int) -> tuple[list, str]:
        ...
        while round_count != 50:
            ...
            if response.status_code != 200:
                return_error(
                    f"{INTEGRATION_NAME} reports fetching - "
                    f"API Status Code: {response.status_code} Error Reason: {response.text}"
                )

            raw_reports += [report for report in response.json().get("objects") if report.get("type") == "report"]
    ...
    def build_iterator(self, collections_to_fetch: list, limit: int) -> list:
        if "Indicators" in collections_to_fetch:
            raw_indicators, relationships, stix_entities, last_indicators_fetch_time = self.fetch_all_indicators_from_api(limit)
        else:
            raw_indicators, relationships, stix_entities, last_indicators_fetch_time = [], {}, {}, get_last_fetch_times()[0]
        ...
        stix_processor = STIX21Processor(
            raw_indicators, relationships, stix_entities, raw_reports, self.malicious_threshold, self.reputation_interval
        )
        ...
        integration_context.update(
            {"last_indicators_fetch_time": last_indicators_fetch_time, "last_reports_fetch_time": last_reports_fetch_time}
        )
        demisto.setIntegrationContext(integration_context)
...
def get_indicators_command(client: Client, collections_to_fetch: list):
    ...
    human_readable = tableToMarkdown(
        "Indicators from FireEye Feed:", indicators, headers=["value", "type", "rawJSON"], removeNull=True
    )
...
def add_fields_if_exists(client: Client, fields_dict: dict) -> dict:
    ...
    """
    if client.tags:
        fields_dict.update({"tags": client.tags})
...
def fetch_indicators_command(client: Client, collections_to_fetch: list, limit: int = -1) -> tuple[list, list]:
    ...
    for indicator in iterator:
        ...
        indicators.append({"value": indicator["value"], "type": indicator["type"], "fields": fields, "rawJSON": indicator})
...
def reset_fetch_command():
    ...
    return (
        "Fetch was reset successfully. Your next indicator fetch will collect indicators "
