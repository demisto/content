...
def humio_query(client, args, headers):
    ...
    if response.status_code == 200:
        ...
        return_outputs(markdown, outputs, result)
        return markdown, outputs, result
    else:
        raise ValueError("Error in command: " + command + " response from server was: " + str(response.text))
...
def humio_get_alert_by_id(client, args, headers):
    ...
    if response.status_code == 200:
        ...
        markdown = tableToMarkdown("Humio Alerts", result, removeNull=True)
        outputs = {
            "Humio.Alerts(val.id == obj.id)": result
        }
        return_outputs(markdown, outputs, result)
        return markdown, outputs, result
    else:
...
def humio_create_alert(client, args, headers):
    ...
    fulldata['silenced'] = args.get('silenced', 'false').lower() in ['true', '1', 't', 'y', 'yes']
    ...
    if response.status_code == 201:
        ...
        markdown = tableToMarkdown("Humio Alerts", result, removeNull=True)
        outputs = {
            "Humio.CreatedAlerts(val.id == obj.id)": result
        }
        return_outputs(markdown, outputs, result)
        return markdown, outputs, result
    else:
...
def humio_update_alert(client, args, headers):
    ...
    if response.status_code == 200:
        ...
        markdown = tableToMarkdown("Humio Alerts", result, removeNull=True)
        outputs = {
            "Humio.ModifiedAlerts(val.id == obj.id)": result
        }
        return_outputs(markdown, outputs, result)
        return markdown, outputs, result
    else:
...
def humio_get_notifier_by_id(client, args, headers):
    ...
    if response.status_code == 200:
        ...
        markdown = tableToMarkdown("Humio Alerts", result, removeNull=True)
        outputs = {
            "Humio.Notifiers(val.id == obj.id)": result
        }
        return_outputs(markdown, outputs, result)
        return markdown, outputs, result
    else:
