...
def save_configuration(arguments: dict) -> List[str]:
    res = demisto.executeCommand("core-api-put", {"uri": "settings/integration", "body": arguments})
    return get_errors(res)


def execute_test_module(arguments: dict) -> List[str]:
    res = demisto.executeCommand("core-api-post", {"uri": "/settings/integration/test", "body": arguments})
...
def main():
    ...
    try:
        ...
        if errors:
            err_str = "\n".join(errors)
            human_readable = (
                f'Found errors in instance {instance_name} after changing the next keys: '
        ...
        if keys_in_instance:
            ...
            if save_configuration(instance):
                return_error(f'Could not revert instance to original configuration. Changed keys: {", ".join(keys_in_instance)}')
