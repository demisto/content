...
"""
        ...
        demisto.results(fileResult(configuration_name, json.dumps(raw_instance, indent=4)))

        doc_file_name = f"{instance_name}_summary.md"
        demisto.results(fileResult(doc_file_name, doc))
        context = {
            "TroubleshootAggregateResults": {"configuration_file_name": configuration_name, "summary_file_name": doc_file_name}
        }
