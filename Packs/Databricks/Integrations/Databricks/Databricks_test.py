...
from Databricks import (
    ...
    fetch_incidents,
    test_module as databricks_test_module,
)
...
class TestClusterCommands:
    ...
    def test_cluster_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"clusters": [copy.deepcopy(CLUSTER_RESPONSE)]})
    ...
    def test_cluster_create(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"cluster_id": "new-123"})
        result = cluster_create_command(
            client,
            {
...
class TestJobCommands:
    ...
    def test_job_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"jobs": [copy.deepcopy(JOB_RESPONSE)]})
        result = job_list_command(client, {})
        assert len(result.outputs) == 1

    def test_job_create(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"job_id": 99})
        result = job_create_command(
            client,
            {
                "name": "new-job",
                "tasks": '[{"task_key": "t1"}]',
    ...
    def test_job_run_now(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"run_id": 555, "number_in_job": 1})
...
class TestPipelineCommands:
    ...
    def test_pipeline_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"statuses": [{"pipeline_id": "pipe-1", "name": "p1"}]})
...
class TestWarehouseCommands:
    ...
    def test_warehouse_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"warehouses": [WAREHOUSE_RESPONSE]})
...
class TestSQLCommands:
    def test_sql_statement_execute(self, client, mocker):
        ...
        mocker.patch.object(client, "_http_request", return_value=response)
        result = sql_statement_execute_command(client, {"warehouse_id": "wh-001", "statement": "SELECT 1"})
    ...
    def test_sql_query_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"results": [{"id": "q-1", "name": "q"}]})
...
class TestUnityCatalogCommands:
    ...
    def test_catalog_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"catalogs": [{"name": "main"}]})
        result = catalog_list_command(client, {})
        assert len(result.outputs) == 1

    def test_schema_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"schemas": [{"name": "default", "catalog_name": "main"}]})
    ...
    def test_table_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"tables": [{"name": "t1", "table_type": "MANAGED"}]})
        result = table_list_command(client, {"catalog_name": "main", "schema_name": "default"})
        assert len(result.outputs) == 1

    def test_volume_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"volumes": [{"name": "v1", "volume_type": "MANAGED"}]})
...
class TestIAMCommands:
    ...
    def test_user_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"Resources": [{"id": "1", "userName": "user@example.com"}]})
        result = user_list_command(client, {})
        assert len(result.outputs) == 1

    def test_group_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"Resources": [{"id": "1", "displayName": "admins"}]})
        result = group_list_command(client, {})
        assert len(result.outputs) == 1

    def test_service_principal_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"Resources": [{"id": "1", "displayName": "sp1"}]})
...
class TestTokenAndSecretCommands:
    def test_token_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"token_infos": [{"token_id": "t1", "comment": "test"}]})
        result = token_list_command(client, {})
        assert len(result.outputs) == 1

    def test_secret_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"secrets": [{"key": "my-key"}]})
    ...
    def test_secret_scope_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"scopes": [{"name": "my-scope"}]})
...
class TestMiscCommands:
    def test_dashboard_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"dashboards": [{"dashboard_id": "d1", "display_name": "D1"}]})
        result = dashboard_list_command(client, {})
        assert len(result.outputs) == 1

    def test_serving_endpoint_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"endpoints": [{"name": "ep1"}]})
        result = serving_endpoint_list_command(client, {})
        assert len(result.outputs) == 1

    def test_mlflow_model_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"registered_models": [{"name": "model1"}]})
        result = mlflow_model_list_command(client, {})
        assert result.outputs[0]["name"] == "model1"

    def test_dbfs_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"files": [{"path": "/data/file.csv", "is_dir": False}]})
    ...
    def test_workspace_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"objects": [{"object_type": "NOTEBOOK", "path": "/nb"}]})
    ...
    def test_ip_access_list_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"ip_access_lists": [{"list_id": "l1", "label": "test"}]})
    ...
    def test_global_init_script_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"scripts": [{"script_id": "s1", "name": "init"}]})
        result = global_init_script_list_command(client, {})
        assert result.outputs[0]["script_id"] == "s1"

    def test_repo_list(self, client, mocker):
        mocker.patch.object(client, "_http_request", return_value={"repos": [{"id": 1, "url": "https://github.com/test"}]})
        result = repo_list_command(client, {})
        assert len(result.outputs) == 1

    def test_git_credential_list(self, client, mocker):
        mocker.patch.object(
            client, "_http_request", return_value={"credentials": [{"credential_id": 1, "git_provider": "gitHub"}]}
        )
...
class TestTestModule:
    def test_test_module_success(self, client, mocker):
        ...
        result = databricks_test_module(client)
        assert result == "ok"

    def test_test_module_failure(self, client, mocker):
        mocker.patch.object(client, "_http_request", side_effect=Exception("Connection refused"))
        with pytest.raises(Exception, match="Failed to connect"):
            databricks_test_module(client)
