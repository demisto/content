...
class Client(BaseClient):
    ...
    def domain_tags(self, hostname: str = None):
        res = self._http_request("GET", f"domain/{hostname}/tags", ok_codes=(200, 403), timeout=self.timeout)
        return res.get("tags", [])

    def domain_details(self, hostname: str = None):
        return self._http_request("GET", f"domain/{hostname}", ok_codes=(200, 403), timeout=self.timeout)

    def domain_subdomains(self, hostname: str = None, children_only: str = "true"):
        query_string = {"children_only": children_only}
        res = self._http_request(
            "GET", f"domain/{hostname}/subdomains", params=query_string, ok_codes=(200, 403), timeout=self.timeout
        )
        return res

    def associated_domains(self, hostname: str = None, page: int = 1):
        params = {"page": page}
        res = self._http_request("GET", f"domain/{hostname}/associated", params=params, ok_codes=(200, 403), timeout=self.timeout)
        return res
    ...
    def get_company(self, domain: str = None):
        res = self._http_request("GET", f"company/{domain}", ok_codes=(200, 403), timeout=self.timeout)
        return res.get("record", {})

    def get_useragents(self, ip_address: str = None, params: dict = None):
        return self._http_request(
            "GET", f"ips/{ip_address}/useragents", params=params or {}, ok_codes=(200, 403), timeout=self.timeout
        )

    def get_company_associated_ips(self, domain: str = None):
        res = self._http_request("GET", f"company/{domain}/associated-ips", ok_codes=(200, 403), timeout=self.timeout)
        return res.get("record", {})
    ...
    def get_dns_history(self, hostname: str = None, record_type: str = None, page: int = 1):
        params = {"page": page}
        return self._http_request(
            "GET", f"history/{hostname}/dns/{record_type}", params=params, ok_codes=(200, 403), timeout=self.timeout
        )

    def get_whois_history(self, hostname: str = None, page: int = 1):
        params = {"page": page}
        res = self._http_request("GET", f"history/{hostname}/whois", params=params, ok_codes=(200, 403), timeout=self.timeout)
        return res.get("result")

    def get_ip_neighbors(self, ipaddress: str = None):
        res = self._http_request("GET", f"ips/nearby/{ipaddress}", ok_codes=(200, 403), timeout=self.timeout)
    ...
    def query(self, query_type: str = "domain_search", body: dict = None, params: dict = None):
        if query_type == "domain_search":  # noqa: RET503
            ...
            return self._http_request("POST", "domains/stats", json_data=body, ok_codes=(200, 403), timeout=self.timeout)
        elif query_type == "ip_search":
            return self._http_request(
                "POST", "ips/list", params=params, json_data=body, ok_codes=(200, 403), timeout=self.timeout
            )
        elif query_type == "ip_stats":
            return self._http_request(
                "POST", "ips/stats", params=params, json_data=body, ok_codes=(200, 403), timeout=self.timeout
            )

    def sql(self, sql: dict = None, timeout: int = 20):
        return self._http_request("POST", "query/scroll", json_data=sql, timeout=self.timeout)

    def sql_next(self, next_id: str = None, timeout: int = 20):
        return self._http_request("GET", f"query/scroll/{next_id}", timeout=self.timeout)
...
def domain_command(client, args):
    ...
    for domain in domains:
        ...
        admin_contact = [
            {"Name": x.get("name"), "Email": x.get("email"), "Phone": x.get("telephone"), "Country": x.get("country")}
            for x in domain_whois.get("contacts", [])
            if "admin" in x.get("type", "").lower()
        ]
        registrant_contact = [
            {
            ...
            for x in domain_whois.get("contacts", [])
            if "registrant" in x.get("type", "").lower()
        ]
        registrar_contact = [
            {
                "Name": x.get("name", None),
            ...
            for x in domain_whois.get("contacts", [])
            if "registrar" in x.get("type", "").lower()
        ]
        domain_data = {
            "Name": domain,
            "DNS": ",".join([x.get("ip", "") for x in domain_details.get("current_dns", {}).get("a", {}).get("values", [])]),
            "NameServers": ",".join(
                [x.get("nameserver", "") for x in domain_details.get("current_dns", {}).get("ns", {}).get("values", [])]
            ),
        ...
        result = CommandResults(
            outputs_prefix="Domain", outputs_key_field="Name", outputs=domain_data, indicator=domain_indicator, readable_output=md
        )
...
def get_company_details_command(client, args):
    ...
    create_standard_domain_context(
        domain_data={
            ...
            "Registrant": {"Name": res.get("name", None)},
            "WHOIS": {"Registrant": {"Name": res.get("name", None)}},
        }
    )
...
def get_company_associated_ips_command(client, args):
    ...
    output_data = {"name": domain, "associatedips": res, "associatedips_count": len(res)}

    command_results = CommandResults(
        outputs_prefix="SecurityTrails.Domain", outputs_key_field="name", outputs=output_data, readable_output=readable_output
    )
    return_results(command_results)
...
def domain_details_command(client, args):
    ...
    res["name"] = hostname
    output_data = sorted(
        [{"Type": k, "Record Count": len(v.get("values", []))} for k, v in res.get("current_dns", {}).items()],
        key=lambda x: x["Type"],
    )
    readable_output = tableToMarkdown(f"Domain details for {hostname}:", output_data, ["Type", "Record Count"])
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.Domain", outputs_key_field="name", outputs=res, readable_output=readable_output
    )
    return_results(command_results)
...
def domains_subdomains_command(client, args):
    ...
    output_data = {"name": hostname, "subdomains": subdomains, "subdomains_count": len(subdomains)}
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.Domain", outputs_key_field="name", outputs=output_data, readable_output=md
    )
    return_results(command_results)
...
def get_domain_tags_command(client, args):
    ...
    return_results(command_results)
    create_standard_domain_context(domain_data={"Name": hostname, "Tags": tags})
...
def get_whois_command(client, args):
    ...
    if command == "securitytrails-get-domain-whois":
        ...
        command_results = CommandResults(
            outputs_prefix="SecurityTrails.IP", outputs_key_field="ip", outputs=res, readable_output=readable_output
        )

        return_results(command_results)
        ip_data = {"Address": ip_address}
...
def domain_search_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.Domain.Search", outputs_key_field="hostname", outputs=records, readable_output=md
    )
    return_results(command_results)
...
def domain_statistics_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.Domain.Search.DomainStats", outputs_key_field="hostname", outputs=res, readable_output=md
    )
    return_results(command_results)
...
def associated_domains_command(client, args):
    ...
    output_data = {"name": hostname, "associated_domains": records, "associated_domain_count": record_count}
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.Domain", outputs_key_field="name", outputs=output_data, readable_output=md
    )
    return_results(command_results)
...
def get_ssl_certificates(client, args):
    ...
    md = tableToMarkdown(
        f"SSL Certificates for {hostname}",
        table_data,
        [
            "ID",
            "Subject Key ID",
    ...
    output_data = {"name": hostname, "ssl_certiticates": records}
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.Domain", outputs_key_field="name", outputs=output_data, readable_output=md
    )
    return_results(command_results)
    ...
    for record in records:
        for value in record.get("values"):
            if pull_field in value:
                records_list.append({"Record Type": record_type, "Value(s)": value.get(pull_field)})
...
def get_whois_history_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.Domain",
        outputs_key_field="name",
        outputs={"name": hostname, "WHOIS_history": res.get("items", []), "WHOIS_history_count": res.get("count", 0)},
        readable_output=readable_output,
    )
    return_results(command_results)
    ...
    for x in res.get("items", []):
        whois_object = {
            ...
            "CreationDate": datetime.fromtimestamp(x.get("createdDate") / 1000).strftime("%Y-%m-%dT%H:%M:%SZ")
            if x.get("createdDate", None)
            else None,
            "UpdatedDate": datetime.fromtimestamp(x.get("updatedDate") / 1000).strftime("%Y-%m-%dT%H:%M:%SZ")
            if x.get("updatedDate", None)
            else None,
            "ExpirationDate": datetime.fromtimestamp(x.get("expiresDate") / 1000).strftime("%Y-%m-%dT%H:%M:%SZ")
            if x.get("expiresDate", None)
            else None,
        }
        if admin_contact:
...
def get_ip_neighbors_command(client, args):
    ...
    command_results = CommandResults(
        ...
        outputs={"ip": ipaddress, "blocks": res},
        readable_output=readable_output,
    )
    return_results(command_results)
    create_standard_ip_context(ip_data=[{"Address": x.get("ip").split("/")[0]} for x in res])
...
def ip_search_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.IP.Search", outputs_key_field="ip", outputs=records, readable_output=md
    )
    return_results(command_results)
    create_standard_ip_context(
        ip_data=[
            {"Address": x.get("ip"), "Hostname": x.get("ptr"), "Ports": ", ".join([str(y["port"]) for y in x.get("ports")])}
            for x in records
        ]
    )
...
def ip_statistics_command(client, args):
    ...
    table_data = {"Top PTRs Count": len(top_ptrs), "Ports": len(ports), "Total": total}
    md = tableToMarkdown("IP Statistics:", table_data)
    command_results = CommandResults(outputs_prefix="SecurityTrails.IP.Search.IPStats", outputs=res, readable_output=md)
    return_results(command_results)
...
def get_useragents_command(client, args):
    ...
    output_data = {"ip": ip_address, "useragents": records, "useragent_records_count": record_count}
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.IP", outputs_key_field="ip", outputs=output_data, readable_output=md
    )
    return_results(command_results)
...
def query_sql_command(client, args):
    ...
    command_results = CommandResults(
        outputs_prefix="SecurityTrails.SQL", outputs_key_field=["query", "id"], outputs=output, readable_output=readable_output
    )
    return_results(command_results)
...
def main() -> None:
    ...
    try:
        ...
        client = Client(base_url=base_url, verify=verify_certificate, headers=headers, proxy=proxy, timeout=timeout)
