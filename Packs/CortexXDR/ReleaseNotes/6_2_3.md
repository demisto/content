#### Integrations

##### Palo Alto Networks Cortex XDR - Investigation and Response

- **Breaking Changes**: Set **Remove legacy incident fields** to default true- which means that the incident data before going through the mapper is in the following format word1_word2 (and not word1word2) for example: alert_id and not alertid, and that in the context data keys in alerts, file_artifacts and network_artifacts will appear in the word1_word2 format if are originated from the API request.
- **Breaking Changes**: Added the **Minimize Alert Information** parameter in order to support not fetching certain fields of alerts from the API, can either choose null_values or any alert field. Default value is set to null_values.
- **Breaking Changes**: Updated the **Minimize Incident Information** to include the additionaldata field- if checked, also the additional data field will be removed (relevant to Cortex XDR - lite and Malware Investigation and Response incident types).

#### Mappers

##### XDR - Incoming Mapper

- Updated the Mapping for **Cortex XDR - Lite** incident type to map only unmapped field to lables.
- Updated the Mapping for **Cortex XDR - Lite** incident type:
  - The following fields will use word1_word2 format as the source of the field:
    - Additional Data uses additional_data
    - Agent Version uses agent_version
    - Cloud Provider uses cloud_provider
    - Device Name uses host_name
    - Parent Process Name uses alerts.causality_actor_process_image_name
  - The following fields will now populate only unique values:
    - External Category
    - Parent Process Name
    - Parent CMD line
    - Parent Process CMD
    - Parent Process MD5
    - Parent Process Name
    - Parent Process SHA256
    - Process CMD
    - Process MD5
    - Process SHA256
    - XDR Alert Category
    - XDR MITRE Tactics
    - XDR MITRE Techniques
    - Agent Version
    - Agents ID
    - CMD line
    - Device Internal IPs
    - External Category Name
    - File Names
    - File Paths
    - Parent CMD line
- Updated the Mapping for **Cortex XDR Incident** incident type:
  - The following fields will now populate only unique values:
    - XDR Alert Category
    - XDR Alert Name
    - XDR MITRE Tactics
    - XDR MITRE Techniques
- Updated the Mapping for **Malware Investigation and Response** incident type:
  - The following fields will use word1_word2 format as the source of the field:
    - Malware Investigation and Response
  - The following fields will now populate only unique values:
    - Agent Version
    - Agents ID
    - CMD line
    - Device Internal IPs
    - External Category Name
    - File Names
    - File Paths
    - Parent CMD line
    - Parent Process CMD
    - Parent Process MD5
    - Parent Process Name
    - Parent Process SHA256
    - Process CMD
    - Process MD5
    - Process SHA256

#### Incident Fields

##### XDR Alerts

- The following fields will use word1_word2 format as the source of the field:
  - Alert Id uses alert_id
  - Detection Timestamp uses detection_timestamp
  - Action Pretty uses action_pretty
  - Host IP uses host_ip
  - Host Name uses host_name
  - User Name uses user_name
  - MITRE ATTACK TACTIC uses mitre_tactic_id_and_name
  - MITRE ATTACK TECHNIQUE uses mitre_technique_id_and_name