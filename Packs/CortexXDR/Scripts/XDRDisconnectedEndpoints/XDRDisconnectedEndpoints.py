...
def main():
    try:
        ...
        disconnected_endpoints = [endpoint for endpoint in endpoints if endpoint.get('endpoint_status') != 'CONNECTED']
        disconnected_endpoints_num = len(disconnected_endpoints)

        return_results(str(disconnected_endpoints_num))
