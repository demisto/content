...
| resolve_reason | The reason for resolving the case. Can only be provided when status is set to "resolved", and is required when resolving. Possible values are: resolved_known_issue, resolved_duplicate, resolved_false_positive, resolved_true_positive, resolved_security_testing, resolved_other. | Optional |
| resolve_comment | A free-text comment explaining the resolution. Can only be provided when status is set to "resolved". | Optional |
