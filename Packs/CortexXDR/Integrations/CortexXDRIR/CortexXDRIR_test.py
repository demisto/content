...
def test_fetch_only_starred_incidents(requests_mock, mocker):
    ...
    next_run, incidents = fetch_incidents(client, '3 month', 'MyInstance', last_run_obj.get('next_run'), starred=True, starred_incidents_fetch_window='3 days')
    assert len(incidents) == 2
    assert incidents[0]['name'] == "XDR Incident 1 - 'Local Analysis Malware' generated by XDR Agent detected on host" \
                                   " AAAAA involving user Administrator"

    last_run_obj = {'next_run': next_run,
                    'fetched_starred_incidents': {'1': True, '2': True}
                    }
