...
def test_fetch_incidents(requests_mock, mocker):
    ...
    params = {"extra_data": True, "mirror_direction": "Incoming"}
    ...
    next_run, incidents = fetch_incidents(client, '3 month', 'MyInstance', params)
...
class TestFetchStarredIncident:
    ...
    def test_fetch_only_starred_incidents(self, mocker):
        ...
        client = Client(
            base_url=f'{XDR_URL}/public_api/v1', verify=False, timeout=120, proxy=False)
        next_run, incidents = fetch_incidents(client, '3 month', 'MyInstance', params, last_run_obj.get('next_run'),
                                              starred=True,
                                              starred_incidents_fetch_window='3 days')
        assert len(incidents) == 2
