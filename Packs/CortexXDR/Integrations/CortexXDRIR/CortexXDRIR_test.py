...
def test_fetch_incidents(requests_mock, mocker):
    ...
    assert incidents[0]['name'] == "XDR Incident 1 - 'Local Analysis Malware' generated by XDR Agent detected on" \
                                   " host AAAAAA involving user Administrator"

    assert 'network_artifacts' not in json.loads(incidents[0]['rawJSON'])
    assert json.loads(incidents[0]['rawJSON']).pop('last_mirrored_in')
    assert incidents[0]['rawJSON'][17] == json.dumps(modified_raw_incident)[17]
