...
class TestGetRequestsKwargs:
    def test_with_file(self, mocker):
        ...
        ), f"get_requests_kwargs(file_path={path})\n\treturns: {output}\n\t instead: {expected_output}"  # noqa: E501
...
class TestDemistoIOCToXDR:
    ...
    def test_demisto_expiration_to_xdr(self, demisto_expiration, xdr_expiration):
        ...
        assert (
            xdr_expiration == output
        ), f"demisto_expiration_to_xdr({demisto_expiration})\n\treturns: {output}\n\tinstead: {xdr_expiration}"  # noqa: E501
    ...
    def test_demisto_reliability_to_xdr(self, demisto_reliability, xdr_reliability):
        ...
        assert (
            output == xdr_reliability
        ), f"demisto_reliability_to_xdr({demisto_reliability})\n\treturns: {output}\n\tinstead: {xdr_reliability}"  # noqa: E501

    data_test_demisto_types_to_xdr = [("File", "HASH"), ("IP", "IP"), ("Domain", "DOMAIN_NAME"), ("URL", "PATH")]
    ...
    def test_demisto_vendors_to_xdr(self, demisto_vendor, xdr_vendor):
        ...
        assert (
            output == xdr_vendor
        ), f"demisto_vendors_to_xdr({demisto_vendor})\n\treturns: {d_sort(output)}\n\tinstead: {d_sort(xdr_vendor)}"  # noqa: E501
...
class TestXDRIOCToDemisto:
    ...
    def test_xdr_expiration_to_demisto(self, xdr_expiration, demisto_expiration):
        ...
        ), f"xdr_expiration_to_demisto({xdr_expiration})\n\treturns: {output}\n\tinstead: {demisto_expiration}"  # noqa: E501
...
class TestCommands:
    ...
    class TestIOCSCommand:
        def test_iocs_command_with_enable(self, mocker):
            ...
            assert (
                output == "IOCs command: enabled indicators='11.11.11.11'"
        ...
        def test_iocs_command_with_disable(self, mocker):
            ...
            output = outputs.call_args.args[0]
            assert (
                output == "IOCs command: disabled indicators='11.11.11.11'"
    ...
    @pytest.mark.parametrize(
        "zip_value, expected_file_name",
        [pytest.param(False, "xdr-sync-file", id="no zip"), pytest.param(True, "xdr-sync-file-zipped.zip", id="zip")],
    )
...
data_test_test_file_deleted = [(sync, "create_file_sync")]
...
def test_overriding_severity_xsoar():
    ...
    dummy_demisto_ioc = {"value": "1.1.1.1", "indicator_type": "FILE_123", "CustomFields": {custom_severity_field: "critical"}}
...
@pytest.mark.parametrize("value", ("info", "Info", "informational", "INformationAL"))
...
def test_parse_demisto_comments__default_empty():
    ...
    assert _parse_demisto_comments(ioc={}, comment_field_name=Client.xsoar_comments_field, comments_as_tags=False) == [""]
...
def test_parse_demisto_comments__default_as_tag():
    ...
    assert (
        exc.value.message == "When specifying comments_as_tags=True, the xsoar_comment_field cannot be `comments`)."
        "Set a different value."
    )


@pytest.mark.parametrize(
    "comment_value,comments_as_tags,expected",
    (
        ("hello", True, ["hello"]),
        ("hello", False, ["hello"]),
...
def test_parse_demisto_comments__custom_field_empty_value(comments_as_tags: bool):
    ...
    assert _parse_demisto_comments(
        ioc={"CustomFields": {comment_field: ""}}, comment_field_name=comment_field, comments_as_tags=comments_as_tags
    ) == [""]
...
def test_parse_demisto_comments__custom_field_missing(comments_as_tags: bool):
    ...
    assert _parse_demisto_comments(
        ioc={"CustomFields": {}}, comment_field_name="comment_field", comments_as_tags=comments_as_tags
    ) == [""]
...
def test_parse_demisto_comments_url_xsoar_6_default(mocker):
    ...
    assert _parse_demisto_comments(ioc={"id": inc_id}, comment_field_name="", comments_as_tags=False) == [
...
def test_parse_demisto_comments_url_xsoar_8_default(mocker):
    ...
    Client.add_link_as_a_comment = True
    assert XDR_iocs._parse_demisto_comments(ioc={"id": inc_id}, comment_field_name="", comments_as_tags=False) == [
        f"url/indicator/{inc_id}"
    ]
...
def test_parse_demisto_list_of_comments_default(mocker):
    ...
    assert _parse_demisto_comments(
        ioc={Client.xsoar_comments_field: [{"type": "IndicatorCommentRegular", "content": comment_value}], "id": inc_id},
        comment_field_name=Client.xsoar_comments_field,
        comments_as_tags=False,
    ) == [f"{comment_value}, url/#/indicator/{inc_id}"]
...
def test_module_fail_with_fetch_interval(mocker):
    ...
    assert e.value.message == (
        "`Feed Fetch Interval` is set to 14. Setting `Feed Fetch Interval` to less "
...
ioc_example = {
    ...
    "moduleToFeedMap": {"FeedA": {"reliability": "A"}},
    "CustomFields": {
...
def test_xdr_iocs_sync_command_sync_for_fetch(
...
def test_xdr_iocs_sync_command_sync_for_fetch_fails(
...
@patch("XDR_iocs.get_iocs_generator", return_value=[ioc_example])
@patch(
    "XDR_iocs.Client.http_request",
    return_value={
        "reply": {
            "success": True,
            "validation_errors": [{"indicator": "123", "error": "error1"}, {"indicator": "456", "error": "error2"}],
        }
    },
)
@patch("XDR_iocs.set_integration_context", return_value={})
...
@patch(
    "XDR_iocs.get_integration_context",
    return_value={
        "time": "2024-09-10T12:13:57Z",
        "ts": "1234567",
...
def test_update_integration_context(mock_set_integration_context, mock_get_integration_context):
    ...
    update_integration_context_override(
        update_sync_time_with_datetime=fixed_datetime,
        update_is_first_sync_phase="false",
        update_search_after_array=["765", "000"],
    )
    mock_set_integration_context.assert_called_with(
        {"time": "2024-09-10T12:00:00Z", "ts": 1725969600000, "is_first_sync_phase": False, "search_after": ["765", "000"]}
    )
...
@pytest.mark.parametrize(
    "indicators_input, search_results, expected_iocs, expected_warning, expected_info",
    [
        (
...
@pytest.mark.parametrize(
    "xsoar_comment_field, expected_result",
    (
...
@pytest.mark.parametrize(
    "xsoar_comment_field, informative_message",
    (
        (
...
def test_no_override_with_default_severity():
    ...
    """
    ioc = {"value": "test_indicator", "indicator_type": "IP", "CustomFields": {}}
...
def test_no_override_with_custom_field_severity():
    ...
    """
    ioc = {"value": "test_indicator", "indicator_type": "IP", "CustomFields": {"severity_field": "LOW"}}
...
def test_old_override_with_custom_field_severity():
    ...
    """
    ioc = {"value": "test_indicator", "indicator_type": "IP", "CustomFields": {"severity_field": "MEDIUM"}}
...
def test_old_override_without_custom_severity_field(mock_get_score):
    ...
    """
    ioc = {"RULE_INDICATOR": "test_indicator", "IOC_TYPE": "IP", "RULE_STATUS": "ENABLED", "RULE_EXPIRATION_TIME": 1700000000}
