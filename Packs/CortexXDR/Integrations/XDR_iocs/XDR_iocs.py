...
BATCH_SIZE: int = 4000
xdr_types_to_demisto: dict = {"DOMAIN_NAME": "Domain", "HASH": "File", "IP": "IP"}
...
xdr_reputation_to_demisto: dict = {"GOOD": 1, "SUSPICIOUS": 2, "BAD": 3}
demisto_score_to_xdr: dict[int, str] = {1: "GOOD", 2: "SUSPICIOUS", 3: "BAD"}
...
def info_log_for_fetch(
    last_modified_time: str | None, indicator_value: str | None, new_search_after: list[str] | None, ioc_count: int = 1
):
    if new_search_after:
        demisto.info(
            f"Fetched {ioc_count} indicators from xsoar. last modified that was synced "
...
def demisto_ioc_to_xdr(ioc: dict) -> dict:
    try:
        ...
        if comment := _parse_demisto_comments(
            ioc=ioc, comment_field_name=Client.xsoar_comments_field, comments_as_tags=Client.comments_as_tags
        ):
...
def update_integration_context_override(
    update_sync_time_with_datetime: datetime | None = None,
...
def sync_for_fetch(client: Client, batch_size: int = 200):
    ...
    try:
        ...
        request_data = list(map(demisto_ioc_to_xdr, get_iocs_generator(size=batch_size, stop_iteration=True, query=full_query)))
        if request_data:
...
def push_iocs(client, iocs, path="tim_insert_jsons/"):
    ...
    for i, single_batch_iocs in enumerate(batch_iocs(iocs, batch_size=MAX_INDICATORS_TO_SYNC)):
        ...
        requests_kwargs: dict = get_requests_kwargs(_json=list(map(demisto_ioc_to_xdr, single_batch_iocs)), validate=True)
    ...
    try:
        ...
        # If tim_insert_jsons is called from fetch_indicators
            ...
            while True:
                ...
                query = (
                    create_query_with_end_time(to_date=current_run)
                ...
                iocs = list(map(demisto_ioc_to_xdr, get_iocs_generator(size=BATCH_SIZE, stop_iteration=True, query=query)))
                if iocs:
...
def xdr_ioc_to_timeline(iocs: list) -> dict:
    ioc_time_line = {"Value": ",".join(iocs), "Message": "indicator updated in XDR.", "Category": "Integration Update"}
...
def xdr_ioc_to_demisto(ioc: dict) -> dict:
    ...
    comments = _parse_xdr_comments(raw_comment=ioc.get("RULE_COMMENT", ""), comments_as_tags=Client.comments_as_tags)
    ...
    if Client.xsoar_comments_field == "tags":
        ...
        tag_comment_fields = {"tags": Client.tag, Client.xsoar_comments_field: list_of_single_to_str(comments)}
...
def module_test(client: Client):
    ...
    if params.get("feed") and feed_fetch_interval and feed_fetch_interval < 15:
        raise DemistoException(
            f"`Feed Fetch Interval` is set to {feed_fetch_interval}. Setting `Feed Fetch Interval` to less "
...
def fetch_indicators(client: Client, auto_sync: bool = False):
    ...
    if ((not last_run) or (last_run.get("is_first_sync_phase", False))) and auto_sync:
        if not last_run:
            sync_time = datetime.now(UTC)
...
def xdr_iocs_sync_command(
    client: Client, first_time: bool = False, is_first_stage_sync: bool = False, called_from_fetch: bool = False
):
...
def main():  # pragma: no cover
    ...
    if params.get("feed") and feed_fetch_interval and feed_fetch_interval < 15:
        demisto.info(
            f"`Feed Fetch Interval` is set to {feed_fetch_interval}. Setting `Feed Fetch Interval` to less then 15 "
