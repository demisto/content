...
| parse_result_file_to_context | If set to 'true', if a query returns more than 1000 results, it will be extracted as JSON data to the context, instead of being returned as a .gz file. If set to 'false' and the query results are greater than 1000, it will return the .gz file without extracting the results to context. | Optional |  
