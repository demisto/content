...
def results_return(command, thingtoreturn):
    results = CommandResults(outputs_prefix="Gophish." + str(command), outputs_key_field="", outputs=thingtoreturn)
...
def main():
    ...
    try:
        client = BaseClient(base_url=base_url, headers=headers, verify=verify_certificate, proxy=proxy)
        ...
        # User Management commands
            ...
            results_return(
                "ModifiedUser", client._http_request("PUT", "users/" + str(demisto.args().get("id")), json_data=payload)
            )
        ...
        # Landing page commands
            ...
            payload = {"url": demisto.args().get("url"), "include_resources": bool(demisto.args().get("include_resources"))}
        ...
        # Templates commands
            ...
            payload = {"content": demisto.args().get("content"), "convert_links": bool(demisto.args().get("convert_links"))}
        ...
        # Campaign commands
            ...
            results_return(
                "CampaignResults", client._http_request("GET", "campaigns/" + str(demisto.args().get("id")) + "/results")
            )
        elif demisto.command() == "gophish-get-campaign-summary":
            results_return(
                "CampaignSummary", client._http_request("GET", "campaigns/" + str(demisto.args().get("id")) + "/summary")
            )
            ...
            results_return(
                "CompletedCampaign", client._http_request("GET", "campaigns/" + str(demisto.args().get("id")) + "/complete")
            )
        ...
        # User Groups related commands
            ...
            payload = {"name": demisto.args().get("name"), "targets": formtargets(demisto.args().get("targets"))}
