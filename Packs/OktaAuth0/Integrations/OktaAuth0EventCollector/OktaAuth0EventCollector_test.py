...
@pytest.mark.parametrize(
    ...
    [
        (
            {"limit": "1", "since": "2022-08-01T09:00:00Z"},
            {},
            {"q": "date:[2022-08-01T09:00:00Z TO *]", "sort": "date:1", "per_page": 100},
        ),
        ({"since": "2022-08-01T09:00:00Z"}, {"last_id": "11111"}, {"from": "11111", "sort": "date:1", "take": 100}),
    ],
)
...
def test_test_module(mocker):
    ...
    assert (
        test_module_command(Client(base_url=CORE_URL, client_id="", client_secret="", verify=False, proxy=False), {}, {}) == "ok"
    )
...
def test_fetch_events(requests_mock):
    ...
    events, last_run = fetch_events_command(
        Client(base_url=CORE_URL, client_id="", client_secret="", verify=False, proxy=False),
        params={"since": "3 days", "limit": 3},
        last_run={},
    )
...
def test_get_events(mocker, requests_mock):
    ...
    _, results = get_events_command(
        Client(base_url=CORE_URL, client_id="", client_secret="", verify=False, proxy=False), args={"limit": 4}
    )
