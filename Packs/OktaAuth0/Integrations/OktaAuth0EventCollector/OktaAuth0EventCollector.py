...
def prepare_query_params(params: dict, last_run: dict = {}) -> dict:
    ...
    if not last_run:  # requesting by time query
        ...
        query_params = {"q": f"date:[{since} TO *]", "sort": "date:1", "per_page": 100}

    else:  # requesting by log_id
        query_params = {"from": last_run.get("last_id"), "sort": "date:1", "take": 100}
...
def get_access_token_from_context() -> Optional[str]:
    ...
    if (
        "token_creation_time" not in int_context
        or int(time.time()) - int_context.get("token_creation_time", 1) > TOKEN_TIME_TO_EXPIRE
    ):
        return None
...
class Client(BaseClient):
    ...
    def get_access_token_request(self):
        ...
        raw_response = self._http_request(method="POST", url_suffix="/oauth/token", data=body)
    ...
    def get_access_token(self):
        ...
        if not (access_token := get_access_token_from_context()):
            ...
            set_integration_context({"access_token": access_token, "token_creation_time": int(time.time())})
...
def get_events_command(client: Client, args: dict) -> tuple[list, CommandResults]:
    ...
    results = CommandResults(
        raw_response=events,
        readable_output=tableToMarkdown("Okta Auth0 Events", events, date_fields=["date"], removeNull=True),
    )
