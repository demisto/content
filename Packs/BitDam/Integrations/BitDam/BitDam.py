...
STATUS_DONE = "DONE"
AUTH_HEADERS = {"Authorization": "{} {}".format(TOKEN_PREFIX, API_TOKEN)}
...
def parse_scan_file_response(response):
    ...
    if "sha1" not in response_json:
        raise Exception("Scan file failed. Bad response json - {}".format(response.content))
...
def parse_get_file_verdict_response(response):
    ...
    if RESPONSE_CODE_OK != response.status_code:
        raise Exception(
            "Get file verdict failed. Response code -{}, Data- '{}'".format(str(response.status_code), response.content)
        )
    response_json = json.loads(response.content)
    status = ""
    verdict = ""
...
def test_module():
    ...
    if RESPONSE_CODE_OK == response.status_code:
        return True
    raise Exception("Status code - {}, Error- '{}'".format(str(response.status_code), response.content))
