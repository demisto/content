...
def get_client_mock():
    ...
    return AzureFirewallClient(
        subscription_id=SUBSCRIPTION_ID, resource_group=RESOURCE_GROUP_NAME, client_id=CLIENT_ID, verify=False, proxy=False
    )
...
def test_azure_firewall_rules_collection_list_command_for_policy(requests_mock):
    ...
    """
    from AzureFirewall import azure_firewall_rules_collection_list_command, get_policy_rule_collection_name, get_policy_rule_name
    ...
    assert dict_safe_get(result.outputs[0], ["properties", "ruleCollections"])[0].get("rules")[0].get("ruleType") == rule_key
    assert dict_safe_get(result.outputs[0], ["properties", "ruleCollections"])[0].get("ruleCollectionType") == collection_key
...
def test_azure_firewall_policy_create_command(requests_mock):
    ...
    command_arguments = {
        ...
        "threat_intelligence_mode": "Turned-off",
        "location": "eastus",
        "tier": "Standard",
        "enable_proxy": "False",
    }
...
def test_azure_firewall_ip_group_delete_command(requests_mock):
    ...
    assert result[0].outputs is None
    assert result[0].outputs_prefix is None
    assert result[0].readable_output == f"IP Group {ip_group_name} delete operation accepted and will complete asynchronously."
...
def test_azure_firewall_network_rule_collection_create_command_for_firewall(requests_mock):
    ...
    mock_response = json.loads(load_mock_response("test_data/network_rule/firewall_network_rule_collection_create.json"))
    requests_mock.put(url, json=mock_response)
    ...
    command_arguments = {
        ...
        "destinations": "189.160.40.11,189.160.40.11",
        "firewall_name": firewall_name,
        "protocols": "UDP,TCP",
        "rule_name": "my-ip-rule",
        "source_ips": "189.160.40.11,189.160.40.11",
...
def test_azure_firewall_network_rule_collection_create_command_for_policy(requests_mock):
    ...
    mock_response = json.loads(load_mock_response("test_data/network_rule/policy_network_rule_collection_create.json"))
    requests_mock.put(url, json=mock_response)
...
def test_azure_firewall_network_rule_create_command_for_policy(requests_mock):
    ...
    mock_response = json.loads(load_mock_response("test_data/policy/policy_rule_collection_get.json"))
    requests_mock.get(url, json=mock_response)
...
def test_azure_firewall_network_rule_collection_update_command_for_policy(requests_mock):
    ...
    command_arguments = {"action": "Deny", "collection_name": collection_name, "policy": policy_name, "priority": "201"}
...
def test_azure_firewall_network_rule_collection_delete_command_for_firewall(requests_mock):
    ...
    mock_response = json.loads(load_mock_response("test_data/firewall/firewall_update.json"))
    requests_mock.put(url, json=mock_response)
...
def test_azure_firewall_network_rule_remove_command_for_policy(requests_mock):
    ...
    command_arguments = {"collection_name": collection_name, "policy": policy_name, "rule_names": "my-ip-rule,not-exists-rule"}
...
def test_azure_firewall_network_rule_update_command_policy(requests_mock):
    ...
    command_arguments = {
        ...
        "destination_type": "ip_address",
        "destinations": "189.160.40.11",
        "new_rule_name": "new-name",
        "policy": policy_name,
        "protocols": "UDP",
        "rule_name": "my-ip-rule",
