...
class AzureFirewallClient:
    def __init__(
        ...
        scope = (
            ...
            if is_credentials
            else "https://management.azure.com/user_impersonation offline_access user.read"
        )
    ...
    def azure_firewall_list_request(self, resource: str, next_link: str = None) -> dict:
        ...
        response = self.ms_client.http_request(
            "GET", full_url=full_url, params=self.default_params, resp_type="json", timeout=100
        )
    ...
    def azure_firewall_get_request(self, firewall_name: str) -> dict:
        ...
        response = self.ms_client.http_request(
            "GET", url_suffix=url_suffix, params=self.default_params, resp_type="json", timeout=100
        )
    ...
    def azure_firewall_update_request(self, firewall_name: str, firewall_data: dict) -> dict:
        ...
        response = self.ms_client.http_request(
            "PUT", url_suffix=url_suffix, params=self.default_params, json_data=firewall_data, resp_type="json", timeout=100
        )

        return response

    def azure_firewall_policy_create_request(
        self,
        policy_name: str,
        ...
        data = remove_empty_elements(
            {
                ...
                "properties": {
                    ...
                    "threatIntelWhitelist": {"ipAddresses": ip_address, "fqdns": domain_address},
                    "snat": {"privateRanges": None},
                    "dnsSettings": {"servers": dns_servers, "enableProxy": enable_proxy},
        ...
        response = self.ms_client.http_request(
            "PUT", url_suffix=url_suffix, params=self.default_params, json_data=data, resp_type="json", timeout=100
        )
    ...
    def azure_firewall_policy_update_request(self, policy_name: str, policy_data: dict) -> dict:
        ...
        response = self.ms_client.http_request(
            "PUT", url_suffix=url_suffix, params=self.default_params, json_data=policy_data, resp_type="json", timeout=100
        )
    ...
    def azure_firewall_policy_delete_request(self, policy_name: str) -> Response:
        ...
        response = self.ms_client.http_request(
            "DELETE", url_suffix=url_suffix, params=self.default_params, resp_type="response", timeout=100
        )
    ...
    def azure_firewall_policy_rule_collection_create_or_update_request(
        self, policy_name: str, collection_name: str, collection_data: dict
    ) -> dict:
        """
        ...
        response = self.ms_client.http_request(
            "PUT", url_suffix=url_suffix, params=self.default_params, resp_type="json", json_data=collection_data, timeout=100
        )
    ...
    def azure_firewall_policy_rule_collection_get_request(self, policy_name: str, collection_name: str) -> dict:
        """
    ...
    def azure_firewall_policy_network_rule_collection_create_request(
        self, policy_name: str, collection_priority: int | None, collection_name: str, action: str, rule_information: dict
    ) -> dict:
        """
        ...
        response = self.ms_client.http_request(
            "PUT", url_suffix=url_suffix, params=self.default_params, json_data=payload, resp_type="json", timeout=100
        )
    ...
    def azure_firewall_ip_group_create_request(self, ip_group_name: str, location: str, ip_address: list = None) -> dict:
        """
        ...
        """
        payload = remove_empty_elements({"location": location, "properties": {"ipAddresses": ip_address}})
    ...
    def azure_firewall_ip_group_update_request(self, ip_group_name: str, ip_group_data: dict) -> dict:
        ...
        response = self.ms_client.http_request(
            "PUT", url_suffix=url_suffix, params=self.default_params, json_data=ip_group_data, resp_type="json", timeout=100
        )
    ...
    def azure_firewall_subscriptions_list_request(self) -> dict:
        ...
        return self.ms_client.http_request("GET", full_url=full_url, params=self.default_params, resp_type="json", timeout=100)
...
def create_scheduled_command(command_name: str, interval: int, timeout: int, **kwargs):
    ...
    """
    polling_args = {"interval": interval, "polling": True, "timeout": timeout, **kwargs}
...
def generate_firewall_command_output(response: dict | list, readable_header: str, output_key: str = None) -> CommandResults:
    ...
    """
    outputs = (
        copy.deepcopy(response.get(output_key, [])) if output_key and isinstance(response, dict) else copy.deepcopy(response)
    )
...
def azure_firewall_list_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    readable_message = get_pagination_readable_message(header="Firewall List:", limit=limit, page=page)
    ...
    return generate_firewall_command_output(response.get("value", [])[start_offset:end_offset], readable_header=readable_message)
    ...
    for firewall in firewall_names:
        try:
            ...
            if scheduled and state not in ["Succeeded", "Failed"]:
                # schedule next poll
                scheduled_command = create_scheduled_command(
                    command_name="azure-firewall-get", interval=interval, timeout=timeout, firewall_names=firewall
                )
                ...
                command_results_list.append(
                    CommandResults(
                        ...
                        readable_output=generate_polling_readable_message(resource_type_name="Firewall", resource_name=firewall),
                    )
                )

            else:
                command_results = generate_firewall_command_output(response, readable_header=f"Firewall {firewall} information:")

                command_results_list.append(command_results)

        except Exception as exception:
            error = CommandResults(readable_output=f"An error occurred while retrieving {firewall}.\n {exception}")
...
def get_policy_rule_name(rule_type: str) -> str:
    ...
    """
    rule_types = {"network_rule": "NetworkRule", "application_rule": "ApplicationRule", "nat_rule": "NatRule"}

    return rule_types.get(rule_type, "")


def generate_rule_collection_output(
    rule_collection_response: dict, readable_header: str, outputs: list, is_firewall_collection: bool
) -> CommandResults:
    """
    ...
    readable_output = tableToMarkdown(
        readable_header, readable_data, headers=["name", "action", "priority"], headerTransform=pascalToSpace
    )
...
def generate_rule_output(response: dict, readable_header: str, outputs: list) -> CommandResults:
    """
...
def filter_policy_rules_collection(rules_collections: list, rule_type: str) -> list:
    ...
    for collection in rules_collections:
        ...
        if isinstance(current_collections, list) and len(current_collections) > 0:
            ...
            if (
                rule_collection.get("ruleCollectionType") == collection_key
                and isinstance(rule_collection.get("rules"), list)
                and len(rule_collection.get("rules")) > 0
                and rule_collection.get("rules")[0].get("ruleType") == rule_key
            ):
...
def azure_firewall_rules_collection_list_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    readable_message = get_pagination_readable_message(header=f"{resource} Rule Collections List:", limit=limit, page=page)
    ...
    if firewall_name:
        ...
        while not complete_requests:
            ...
            if not response.get("nextLink"):
                ...
                response = client.azure_firewall_policy_rule_collection_list_request(
                    policy_name=policy, next_link=response.get("nextLink")
                )

        filtered_rules = filter_policy_rules_collection(total_response.get("value", []), rule_type)[start_offset:end_offset]

    return generate_rule_collection_output(
        rule_collection_response=response,
        readable_header=readable_message,
        outputs=filtered_rules,
...
def get_policy_collection_rules(client: AzureFirewallClient, policy: str, collection_name: str) -> tuple:
    ...
    """
    rules = []
    response = client.azure_firewall_policy_rule_collection_get_request(policy_name=policy, collection_name=collection_name)
...
def azure_firewall_rules_list_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    if firewall_name:
        ...
        readable_message = get_pagination_readable_message(
            header=f"Firewall {firewall_name} {rule_type} Rules List:", limit=limit, page=page
        )
        ...
        readable_message = get_pagination_readable_message(
            header=f"Policy {policy} {rule_type} Rules List:", limit=limit, page=page
        )

        response, rules = get_policy_collection_rules(client=client, policy=policy, collection_name=collection_name)

    return generate_rule_output(response=response, readable_header=readable_message, outputs=rules[start_offset:end_offset])
...
def azure_firewall_rule_get_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    return generate_rule_output(response=response, readable_header=f"Rule {rule_name} Information:", outputs=rule_data)
...
def azure_firewall_policy_create_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    if should_poll and state not in ["Succeeded", "Failed"]:
        # schedule next poll
        scheduled_command = create_scheduled_command(
            command_name="azure-firewall-policy-get", interval=interval, timeout=timeout, policy_names=policy_name
        )

        return CommandResults(
            scheduled_command=scheduled_command,
            readable_output=generate_polling_readable_message(resource_type_name="Policy", resource_name=policy_name),
        )
    ...
    for policy in policy_names:
        try:
            ...
            if scheduled and state not in ["Succeeded", "Failed"]:
                # schedule next poll
                scheduled_command = create_scheduled_command(
                    command_name="azure-firewall-policy-get", interval=interval, timeout=timeout, policy_names=policy
                )
                ...
                command_results_list.append(
                    CommandResults(
                        scheduled_command=scheduled_command,
                        readable_output=generate_polling_readable_message(resource_type_name="Policy", resource_name=policy),
                    )
                )
            ...
            error = CommandResults(readable_output=f"An error occurred while retrieving {policy}.\n {exception}")
            command_results_list.append(error)
...
def azure_firewall_policy_delete_command(client: AzureFirewallClient, args: Dict[str, Any]) -> list:
    ...
    for policy in policy_names:
        try:
            ...
            error = CommandResults(readable_output=f"An error occurred while deleting {policy}.\n {exception}")
            command_results_list.append(error)
...
def azure_firewall_policy_list_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    readable_message = get_pagination_readable_message(header="Policy List:", limit=limit, page=page)
    ...
    while not complete_requests:
        ...
        if len(total_response["value"]) >= end_offset or not response.get("nextLink"):
            ...
            response = client.azure_firewall_policy_list_request(resource=resource, next_link=response.get("nextLink"))

    return generate_policy_command_output(
        total_response.get("value", [])[start_offset:end_offset], readable_header=readable_message
    )
...
def azure_firewall_policy_attach_command(client: AzureFirewallClient, args: Dict[str, Any]) -> list:
    ...
    for firewall in firewall_names:
        try:
            ...
            error = CommandResults(readable_output=f"An error occurred while updating {firewall}.\n {exception}")
            command_results_list.append(error)
...
def delete_rule_collection(
    client: AzureFirewallClient,
    collection_name: str,
    rule_type: str,
    ...
    if firewall_name:
        ...
        if should_poll and state not in ["Succeeded", "Failed"]:
            # schedule next poll
            scheduled_command = create_scheduled_command(
                command_name="azure-firewall-get", interval=interval, timeout=timeout, firewall_names=firewall_name
            )

            return CommandResults(
                scheduled_command=scheduled_command,
                readable_output=generate_polling_readable_message(resource_type_name="Firewall", resource_name=firewall_name),
            )

        else:
            return generate_firewall_command_output(response, readable_header=f'Successfully Updated Firewall "{firewall_name}"')

    else:
        if not policy:
            raise Exception("One of the arguments: `firewall_name` or `policy` must be provided.")

        response_delete = client.azure_firewall_policy_rule_collection_delete_request(
...
def add_rule_to_policy_collection(
    client: AzureFirewallClient, policy: str, collection_name: str, rule_object: dict, rule_name: str
) -> dict:
    """
    ...
    return client.azure_firewall_policy_rule_collection_create_or_update_request(
        policy_name=policy, collection_name=collection_name, collection_data=collection_information
    )


def remove_rule_from_collection(
    client: AzureFirewallClient,
    collection_name: str,
    rule_type: str,
    rule_names: list,
    ...
    if firewall_name:
        ...
        for rule_name in rule_names:
            ...
            if rule_index == -1:
                error = CommandResults(readable_output=f"Rule {rule_name} is not exists.")
                command_results_list.append(error)
                continue
        ...
        response = client.azure_firewall_policy_rule_collection_create_or_update_request(
            policy_name=policy, collection_name=collection_name, collection_data=collection_information
        )
...
def create_firewall_collection(client: AzureFirewallClient, firewall_name: str, rule_type: str, collection_object: dict) -> dict:
    """
...
def validate_predefined_argument(argument_name: str, argument_value: object, argument_options: list) -> bool:
    ...
    for value in argument_value:
        if value not in argument_options:
            raise Exception(
                f"Invalid {argument_name} argument. Please provide one of the following options:" f"{argument_options!s}"
            )

    return True


def validate_network_rule_properties(
    source_type: str, destination_type: str, protocols: list, ip_source_address: list = None, source_ip_group_ids: list = None
) -> bool:
    """
    ...
    validate_predefined_argument(
        argument_name="protocols", argument_value=protocols, argument_options=["TCP", "UDP", "ICMP", "Any"]
    )

    validate_predefined_argument(
        argument_name="source_type", argument_value=source_type, argument_options=["ip_address", "ip_group"]
    )

    validate_predefined_argument(
        argument_name="destination_type",
        argument_value=destination_type,
...
def create_firewall_network_rule_object(
    rule_name: str,
    description: str,
    protocol: list,
    source_type: str,
    ...
    ip_source_address: list | None = None,
    source_ip_group_ids: list | None = None,
    is_firewall_rule: bool = False,
) -> dict:
    """
...
def azure_firewall_network_rule_collection_create_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    """
    Create network rule collection in firewall or policy.
    Args:
        client (AzureFirewallClient): Azure Firewall API client.
    ...
    ip_source_address = argToList(
        args.get("source_ips", [])
    )  # Must be provided when 'source_type' argument is assigned to 'ip_address'.

    source_ip_group_ids = argToList(
        args.get("source_ip_group_ids", [])
    )  # Must be provided when 'source_type' argument is assigned to 'ip_group'.
    ...
    validate_network_rule_properties(
    ...
    rule_information = create_firewall_network_rule_object(
        rule_name=rule_name,
        description=description,
        protocol=protocol,
        source_type=source_type,
    ...
    if firewall_name:
        collection_object = remove_empty_elements(
            {
                "name": collection_name,
                "properties": {"priority": collection_priority, "action": {"type": action}, "rules": [rule_information]},
            }
        )

        response = create_firewall_collection(
            client=client, firewall_name=firewall_name, rule_type="network_rule", collection_object=collection_object
        )
        ...
        response = client.azure_firewall_policy_network_rule_collection_create_request(
            policy_name=policy,
...
def update_policy_rule_collection(
    client: AzureFirewallClient, policy: str, collection_name: str, priority: int = None, action: str = None
) -> dict:
    """
...
def azure_firewall_network_rule_collection_update_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    """
    ...
    if firewall_name:
        firewall_data, filtered_rules = get_firewall_rule_collection(client, firewall_name, rule_type="network_rule")
        ...
        response = update_policy_rule_collection(
            client=client, policy=policy, collection_name=collection_name, priority=priority, action=action
        )
...
def azure_firewall_network_rule_collection_delete_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    """
...
def add_rule_to_firewall_collection(
    client: AzureFirewallClient, firewall_name: str, collection_name: str, rule_type: str, rule_object: dict
) -> dict:
    """
...
def azure_firewall_network_rule_create_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    if firewall_name:
        response = add_rule_to_firewall_collection(
            client=client,
            firewall_name=firewall_name,
            collection_name=collection_name,
            rule_type="network_rule",
            rule_object=rule_information,
        )
        ...
        response = add_rule_to_policy_collection(
            client=client, policy=policy, collection_name=collection_name, rule_object=rule_information, rule_name=rule_name
        )
...
def update_firewall_collection_rule(
    client: AzureFirewallClient,
    firewall_name: str,
    collection_name: str,
    rule_name: str,
    rule_type: str,
    rule_fields_mapper: dict,
    update_fields: dict,
) -> dict:
    """
...
def update_policy_collection_rule(
    client: AzureFirewallClient, policy: str, collection_name: str, rule_name: str, rule_fields_mapper: dict, update_fields: dict
) -> dict:
    """
...
def azure_firewall_network_rule_update_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    update_fields = assign_params(description=description, destination_port=destination_port, protocol=protocol)
    ...
    if firewall_name:
        ...
        response = update_firewall_collection_rule(
            client=client,
            firewall_name=firewall_name,
            collection_name=collection_name,
            rule_name=rule_name,
        ...
        response = update_policy_collection_rule(
            client=client,
            policy=policy,
            collection_name=collection_name,
            rule_name=rule_name,
...
def azure_firewall_network_rule_remove_command(client: AzureFirewallClient, args: Dict[str, Any]) -> list:
    ...
    return remove_rule_from_collection(
        client=client,
        collection_name=collection_name,
        rule_type="network_rule",
...
def azure_firewall_service_tag_list_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    readable_message = get_pagination_readable_message(header="Service Tag List:", limit=limit, page=page)
    ...
    while not complete_requests:
        ...
        if len(total_response["value"]) >= end_offset or not response.get("nextLink"):
            ...
            response = client.azure_firewall_service_tag_list_request(location=location, next_link=response.get("nextLink"))
...
def azure_firewall_ip_group_create_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    response = client.azure_firewall_ip_group_create_request(
        ip_group_name=ip_group_name, location=location, ip_address=ip_address
    )
    ...
    if should_poll and state not in ["Succeeded", "Failed"]:
        # schedule next poll
        scheduled_command = create_scheduled_command(
            command_name="azure-firewall-ip-group-get", interval=interval, timeout=timeout, ip_group_names=ip_group_name
        )

        return CommandResults(
            scheduled_command=scheduled_command,
            readable_output=generate_polling_readable_message(resource_type_name="IP-Group", resource_name=ip_group_name),
        )

    return generate_ip_group_command_output(response, readable_header=f'Successfully Created IP Group "{ip_group_name}"')
...
def azure_firewall_ip_group_list_command(client: AzureFirewallClient, args: Dict[str, Any]) -> CommandResults:
    ...
    readable_message = get_pagination_readable_message(header="IP Group List:", limit=limit, page=page)
    ...
    while not complete_requests:
        ...
        if len(total_response["value"]) >= end_offset or not response.get("nextLink"):
            ...
            response = client.azure_firewall_ip_group_list_request(resource=resource, next_link=response.get("nextLink"))

    return generate_ip_group_command_output(
        total_response.get("value", [])[start_offset:end_offset], readable_header=readable_message
    )
    ...
    for ip_group in ip_group_names:
        try:
            ...
            if scheduled and state not in ["Succeeded", "Failed"]:
                # schedule next poll
                scheduled_command = create_scheduled_command(
                    command_name="azure-firewall-ip-group-get", interval=interval, timeout=timeout, ip_group_names=ip_group
                )
                ...
                command_results_list.append(
                    CommandResults(
                        scheduled_command=scheduled_command,
                        readable_output=generate_polling_readable_message(resource_type_name="IP-Group", resource_name=ip_group),
                    )
                )
            ...
            error = CommandResults(readable_output=f"An error occurred while retrieving {ip_group}.\n {exception}")
            command_results_list.append(error)
...
def azure_firewall_ip_group_delete_command(client: AzureFirewallClient, args: Dict[str, Any]) -> list:
    ...
    for ip_group in ip_group_names:
        try:
            ...
            error = CommandResults(readable_output=f"An error occurred while deleting {ip_group}.\n {exception}")
            command_results_list.append(error)
...
def main() -> None:
    ...
    if (
        ...
        and ((private_key and not certificate_thumbprint) or (certificate_thumbprint and not private_key))
    ):
