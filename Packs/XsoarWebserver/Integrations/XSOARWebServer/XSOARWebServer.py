...
def run_setup_simple_action():
    ...
    integration_context[entry_uuid] = {
        "type": "simple_action",
        "inc_id": inc["id"],
    ...
    set_integration_context(integration_context)
    to_return = CommandResults(
        outputs_prefix="WS-ActionDetails(val.job_uuid==obj.job_uuid)", outputs=integration_context[entry_uuid]
    )
...
def run_get_action_status():
    ...
    if lookup:
        ...
        to_return = CommandResults(
            outputs_prefix="WS-ActionStatus(val.job_uuid==obj.job_uuid)",
            readable_output="fetching current status",
            outputs=output_to_ret,
        )
        return_results(to_return)
    else:
        return_results(data)
