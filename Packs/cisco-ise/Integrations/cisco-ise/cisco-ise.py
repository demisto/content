...
def http_request(method, url_suffix, params=None, data=None, headers=None, is_admin_api=False):
    ...
    try:
        ...
        response = requests.request(
            method, url, auth=(USERNAME, PASSWORD), headers=headers, verify=USE_SSL, params=params, data=data
        )
...
def get_groups():
    ...
    for group in groups:
        context_dict = {"ID": group.get("id"), "Name": group.get("name"), "Description": group.get("description")}
        hr_dict = dict(context_dict)
        context.append(context_dict)
        humanreadable.append(hr_dict)

    entry_context = {"CiscoISE.Group(val.ID === obj.ID)": context}

    return_outputs(
        tableToMarkdown("Cisco ISE Groups", humanreadable, ["ID", "Name", "Description"], removeNull=True),
        entry_context,
        groups_data,
    )
    return None
...
def get_endpoint_id_command():
    ...
    entry_context = {"Endpoint(val.ID === obj.ID)": {"ID": endpoint_id, "MACAddress": mac_address}}
...
def get_endpoint_details_command():
    ...
    if endpoint_details:
        ...
        ec = {
            "Endpoint(val.ID === obj.ID)": {"ID": endpoint_details["id"], "MACAddress": endpoint_details["mac"]},
...
def reauthenticate_endpoint_command():
    ...
    psn_address = (
        json.loads(xml2json(mac_address_psn))
        .get("restAuthStatusOutputList", {})
        .get("authStatusList", {})
        .get("authStatusElements", {})[0]
        .get("acs_server")
    )
    if not psn_address:
        return (
            "Couldn't find psn address for mac: "
            + mac_address
            + " response from psn endpoint was: "
            + json.dumps(mac_address_psn)
        )
...
def get_endpoints_command(return_bool: bool = False):
    ...
    for endpoint in endpoints:
        context_dict = {"ID": endpoint.get("id"), "MACAddress": endpoint.get("name")}
        hr_dict = dict(context_dict)
        context.append(context_dict)
        hr.append(hr_dict)
    if return_bool:
        return True
    entry_context = {"Endpoint(val.ID == obj.ID)": context, "CiscoISE.Endpoint(val.ID == obj.ID)": context}

    return_outputs(tableToMarkdown("Cisco ISE Endpoints", hr, ["ID", "MACAddress"], removeNull=True), entry_context, endpoints)
    return None
...
def update_endpoint_custom_attribute_command():
    ...
    try:
        ...
        if not update_result:
            demisto.results(
                "Update failed for endpoint " + endpoint_id + ". Please check if the custom "
                "fields are defined in the system. "
                "Got the following response: " + update_result.get("ERSResponse", {}).get("messages", [])
            )
...
def get_policies():
    ...
    for policy in policies:
        data.append({"ID": policy.get("id"), "Name": policy.get("name")})

    context = {"CiscoISE.Policy(val.ID && val.ID === obj.ID)": data}

    return_outputs(tableToMarkdown("CiscoISE Adaptive Network Control Policies", data, removeNull=True), context, policies_data)
    return None
...
def get_policy():
    ...
    if policy_data:
        data.append({"Name": policy_data.get("name"), "Action": policy_data.get("actions")})
...
def create_policy():
    ...
    data = {"ErsAncPolicy": {"name": policy_name, "actions": [policy_actions]}}
    create_policy_request(data)
    policy_context = {"Name": policy_name, "Action": [policy_actions]}

    context = {"CiscoISE.Policy(val.Name && val.Name === obj.Name)": policy_context}
...
def assign_policy_to_endpoint():
    ...
    data = {
        "OperationAdditionalData": {
            "additionalData": [{"name": "macAddress", "value": mac_address}, {"name": "policyName", "value": policy_name}]
        }
    }
    assign_policy_request(data)

    endpoint_context = {"MACAddress": mac_address, "PolicyName": policy_name}
...
def get_blacklist_endpoints():
    ...
    for endpoint in endpoints:
        data.append({"ID": endpoint.get("id"), "Name": endpoint.get("name"), "GroupName": "Blacklist"})

    context = {"CiscoISE.Endpoint(val.ID && val.ID === obj.ID)": data}
...
def get_all_nodes_command():
    ...
    context = {"CiscoISE.NodesData": data}
...
def create_new_endpoint_command():
    ...
    data = {"ERSEndPoint": {"mac": mac_address, "customAttributes": {"customAttributes": attr_map}}}
    ...
    context = {"CiscoISE.Endpoint(val.Name && val.Name === obj.Name)": endpoint_context}
...
def get_session_data_by_ip_request(ip):
    ...
    context = {"CiscoISE.Endpoint(val.ID && val.ID === obj.ID)": session_data}
