...
class Client(BaseClient):
    ...
    def register(self, first_name: str, last_name: str, email: str, org: str):
        ...
        json_data = {"firstName": first_name, "lastName": last_name, "email": email, "organization": org}
    ...
    def info(self):
        ...
        res = self._http_request(
            method="GET", raise_on_status=True, url_suffix="/info", headers={"Content-Type": "application/json"}
        )
    ...
    def analyze(
        ...
        res = self._http_request(method="GET", raise_on_status=True, url_suffix=url_suffix)
        outputs = []
        polling_args = {
            "host": host,
            "publish": publish,
        ...
        if res["status"] != "READY":
            ...
            scheduled_command = ScheduledCommand(
                command=cmd, next_run_in_seconds=interval_in_secs, args=polling_args, timeout_in_seconds=polling_timeout
            )
            command_results = CommandResults(scheduled_command=scheduled_command, readable_output=f"Scan Status: {status}")
        ...
        return CommandResults(
            outputs_prefix="sslLabs.analysis",
            outputs=outputs,
...
def register_email_command(client: Client, first_name: str, last_name: str, email: str, org: str) -> CommandResults:
    ...
    if res:
        result = {"message": res.get("message"), "status": res.get("status")}
        markdown = "### SSL Labs Registration\n"
        markdown += tableToMarkdown("Response", result)
        results = CommandResults(
            readable_output=markdown, outputs_prefix="SslLabs.Registation", outputs_key_field="name", outputs=result
        )
        return results
    return CommandResults(entry_type=EntryType.ERROR, readable_output="Could not register email")
...
def info_command(client: Client) -> CommandResults:
    ...
    if res:
        ...
        markdown += tableToMarkdown("Response", result)
        results = CommandResults(
            readable_output=markdown, outputs_prefix="SslLabs.Info", outputs_key_field="name", outputs=result
        )
        return results
    return CommandResults(entry_type=EntryType.ERROR, readable_output="Could not retrieve client info")
