...
def prepare_result(response: dict, args: dict, readable_comment: str) -> CommandResults:
    ...
    if argToBoolean(args.get("echo_spreadsheet")):
        ...
        markdown += tableToMarkdown(table_title, human_readable, headers=["spreadsheet Id", "spreadsheet url"])
        markdown += "\n"
    ...
    results = CommandResults(
        readable_output=markdown, outputs_prefix="GoogleSheets.Spreadsheet", outputs_key_field="spreadsheetId", outputs=outputs
    )
...
def markdown_single_get(response: dict) -> str:
    ...
    markdown = tableToMarkdown(
        response.get("properties", {}).get("title", {}), human_readable, headers=["spreadsheet Id", "spreadsheet url"]
    )
...
def markdown_single_get_include_grid_data(response: dict) -> str:
    ...
    markdown = tableToMarkdown(
        response.get("properties", {}).get("spreadsheetTitle", {}), human_readable, headers=["spreadsheet Id", "spreadsheet url"]
    )
    markdown += "\n"
...
def default_ranges_if_not_specified(spreadsheet: str, ranges: str, include_grid_data: bool, service: Resource) -> str | None:
    """
    Args:
...
def create_spreadsheet(service: Resource, args: dict) -> CommandResults:
    ...
    spreadsheet = {
        "properties": {
            ...
            "defaultFormat": {
                ...
                "backgroundColor": {"red": rgb_format[0], "green": rgb_format[1], "blue": rgb_format[2], "alpha": rgb_format[3]},
                "textFormat": {
    ...
    markdown = tableToMarkdown(
        "Successfully created a spreadsheet", human_readable, headers=["spreadsheet Id", "spreadsheet title"]
    )
    results = CommandResults(
        readable_output=markdown, outputs_prefix="GoogleSheets.Spreadsheet", outputs_key_field="spreadsheetId", outputs=response
    )
    return results
...
def get_spreadsheet(service: Resource, args: dict) -> CommandResults:
    ...
    if len(spread_sheets_ids) > 1:
        ...
        request = service.spreadsheets().get(spreadsheetId=spread_sheets_ids[0], ranges=ranges, includeGridData=include_grid_data)
        ...
        "requests": [
            {
                "addSheet": {
                    "properties": {
                        ...
                        "tabColor": {"red": rgb_format[0], "green": rgb_format[1], "blue": rgb_format[2], "alpha": rgb_format[3]},
...
def copy_to_sheet(service: Resource, args: dict) -> CommandResults:
    ...
    request = (
        ...
        .sheets()
        .copyTo(
            spreadsheetId=spreadsheet_id_to_copy, sheetId=sheet_id_to_copy, body=copy_sheet_to_another_spreadsheet_request_body
        )
    )
    request.execute()  # we don't save the response because there is no need for output or HR
    results = CommandResults(readable_output="### Successfully copied the sheet")
...
def clear_sheet(service: Resource, args: dict) -> CommandResults:  # pragma: no cover
    ...
    request = service.spreadsheets().values().clear(spreadsheetId=spreadsheet_id, range=ranges, body={})
    request.execute()  # we don't save the response because there is no need for output or HR
    results = CommandResults(readable_output="### Successfully cleared the sheet")
    return results
...
def value_update_sheets(service: Resource, args: dict) -> CommandResults:
    ...
    request = (
        service.spreadsheets()
        .values()
        .update(spreadsheetId=spreadsheet_id, range=ranges, valueInputOption=input_option, body=value_range_body)
    )
    request.execute()
