...
def test_make_markdown_matrix_with_none_value():
    ...
    assert (
        GoogleSheets.make_markdown_matrix(
            util_load_json("test_data/helper_functions/test_make_markdown_matrix/sheets_with_none_value.json")
        )
        == Path("test_data/helper_functions/test_make_markdown_matrix/result_with_none_value.md").read_text()
    )
...
def test_prepare_result_with_echo(mocker):
    ...
    mocker.patch.object(
        GoogleSheets,
        "create_list_id_title",
        return_value=[{"SheetId": 783932040, "Sheet title": "new sheet"}, {"SheetId": 0, "Sheet title": "Sheet1"}],
    )
...
handle_values_input_parametrized_exception = [("1,2,3"), ("[4,5"), ("{3,4}"), ("(1,2,3)"), (None)]
...
grid_ranges_combinations = [
    (True, "Sheet1!A1:D5", "Sheet1!A1:D5"),
...
@pytest.mark.parametrize(
    "path", ["test_data/get_spreadsheet/single_spreadsheet/", "test_data/get_spreadsheet/single_spreadsheet_include_grid_data/"]
)
...
def test_get_multiple_spreadsheets():
    ...
    args = {"spreadsheet_id": "13YRXawxY54RI0uPjD_BQmw31zwaAYQ53I0mxbWlhTy8,1btQWA8icPTiVd-HIXOLpzetcoXFo77deZ3tExukEk-w"}
...
@pytest.mark.parametrize(
    "path", ["test_data/update_spreadsheet/test_sheet_create/", "test_data/update_spreadsheet/test_sheet_create_no_echo/"]
)
