...
def fetch_incidents(
    ...
    if "hits" in raw_data and "hits" in raw_data["hits"]:
        for document in raw_data["hits"]["hits"]:
            ...
            if incident_date_obj.replace(tzinfo=datetime.UTC).timestamp() > \
                    newest_incident_date_obj.replace(tzinfo=datetime.UTC).timestamp():
                newest_incident_date_obj = incident_date_obj

    demisto.setLastRun({"last_run_timestamp": newest_incident_date_obj.replace(tzinfo=datetime.UTC).timestamp()})
    return incidents
