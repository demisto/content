...
def coralogix_search_command(
    ...
    request_data = {"query": {"bool": {"must": [{"query_string": {"query": query}}]}}}
    if since_timestamp is not None:
        request_data["query"]["bool"]["must"].append(
            {"range": {"coralogix.timestamp": {"gt": since_timestamp, "lt": to_timestamp}}}
        )
    ...
    r = http.request(
        "POST", cgx_endpoint_url, body=encoded_data, headers={"token": cgx_private_key, "Content-type": "application/json"}
    )
    ...
    if "hits" in results_raw and "hits" in results_raw["hits"]:
        ...
        return CommandResults(
            ...
            readable_output=tableToMarkdown(
                "Coralogix Search Results", formatted_data, columns_to_include_in_human_readable, removeNull=True
            ),
        ...
        return CommandResults(
            raw_response=results_raw,
            readable_output=tableToMarkdown(
                "Coralogix Search Results", [], columns_to_include_in_human_readable, removeNull=True
            ),
            outputs_key_field="_id",
            outputs_prefix="Coralogix.SearchResults",
            outputs=[],
...
def coralogix_tag_command(
    private_key, coralogix_url, application_name, subsystem_name, tag_name, tag_timestamp="", tag_icon_url=""
):
    """
    ...
    """
    query_string_params = {"key": private_key, "application": application_name, "subsystem": subsystem_name, "name": tag_name}
    ...
    try:
        ...
        raise ValueError(
            "Failed to tag the following timestamp "
            + tag_timestamp
            + " under the name "
            + tag_name
...
def test_module(cgx_private_key, cgx_endpoint_url):
    ...
    request_data = {"query": {"bool": {"must": [{"range": {"coralogix.timestamp": {"gte": "now-15m", "lt": "now"}}}]}}}
    ...
    try:
        ...
        http = urllib3.PoolManager()
        r = http.request(
            "GET", cgx_endpoint_url, body=encoded_data, headers={"token": cgx_private_key, "Content-type": "application/json"}
        )
        results_raw = json.loads(r.data.decode("utf-8"))
    ...
    if results_raw is not None and "hits" in results_raw:
        ...
        return (
            "Test failed (No result was received from Coralogix or the response was unexpected `" + json.dumps(results_raw) + "`)"
        )
...
def fetch_incidents(
    ...
    if "hits" in raw_data and "hits" in raw_data["hits"]:
        for document in raw_data["hits"]["hits"]:
            ...
            if incident_date_obj.replace(tzinfo=UTC).timestamp() > newest_incident_date_obj.replace(tzinfo=UTC).timestamp():
...
def main():
    ...
    incidents_first_fetch_range_raw = strip_or_empty(
        demisto.params().get("incidents_first_fetch_range", default_incidents_first_fetch_range)
    )
