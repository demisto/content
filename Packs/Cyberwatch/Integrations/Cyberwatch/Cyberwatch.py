...
class Client(BaseClient):
    ...
    def get_sysadmin_one_asset(self, params):
        ...
        path = "/api/v3/assets/servers/" + str(params["id"])
        response = self._http_request(method="GET", url_suffix=path, params=params)
        return response
    ...
    def get_compliance_one_asset(self, params):
        ...
        Returns:
        {
            ...
            "hostname": "string",
            "description": "string",
            "last_communication": "1970-01-01T00:00:00.000Z",
            "reboot_required": false,
            "boot_at": "1970-01-01T00:00:00.000Z",
            "category": "no_category",
            "created_at": "1970-01-01T00:00:00.000Z",
            "environment": {
                "id": 0,
                "name": "string",
                "confidentiality_requirement": "confidentiality_requirement_undefined",
                "integrity_requirement": "integrity_requirement_undefined",
                "availability_requirement": "availability_requirement_undefined",
                "ceiling_cvss_v2": "string",
                "ceiling_cvss_v3": "string",
                "ceiling_cvss_v4": "string"
            },
            "os": {
                "arch": "string",
                "eol": "1970-01-01",
            ...
            "rules": [
                ...
                "code": "string",
                "status": "no_status",
                "last_verification": "1970-01-01T00:00:00.000Z"
                }
            ]
        }
        """
        ...
        path = "/api/v3/compliance/servers/" + str(params["id"])
        response = self._http_request(method="GET", url_suffix=path, params=params)
        return response
...
def send_declarative_data_asset_command(client: Client, args: Dict[str, Any]):
    ...
    Required: hostname, data (JSON string)
    """
    hostname = args.get("hostname")
    if not hostname:
        raise DemistoException("Argument 'hostname' is required.")

    # Check hostname exists in the system (avoid creating a new server)
    ...
    server_id_known = matches[0]["id"]

    data_json = args.get("data", "{}")

    blob = _build_declarative_payload(hostname, data_json)
    result = client.upload_declarative_data(blob)

    # Preserve API response but add the matched server id
    result = result or {}
    result.setdefault("matched_server_id", server_id_known)

    # API returns JSON; we surface server_id if present
...
def get_declarative_data_asset_command(client: Client, args: Dict[str, Any]):
    ...
    if not server_id:
        raise DemistoException("Argument 'id' is required.")
...
def list_sysadmin_assets_command(client: Client, args: Dict[str, Any]):
    ...
    if len(assets) == 0:
        raise DemistoException("No Sysadmin assets found")
...
def fetch_sysadmin_asset_command(client: Client, args: Dict[str, Any]):
    ...
    if len(asset) == 0:
        raise DemistoException("Sysadmin asset not found")
...
def list_compliance_assets_command(client: Client, args: Dict[str, Any]):
    ...
    if len(assets) == 0:
        raise DemistoException("No Compliance assets found")
...
def fetch_compliance_asset_command(client: Client, args: Dict[str, Any]):
    ...
    if len(asset) == 0:
        raise DemistoException("Compliance asset not found")
