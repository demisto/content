...
def test_send_declarative_data_asset_command_missing_hostname(mocker):
    from Cyberwatch import send_declarative_data_asset_command

    mocker.patch.object(Client, "_http_request")  # should not be called

    try:
        send_declarative_data_asset_command(client, {"data": "{}"})
    except Exception as e:
        assert "Argument 'hostname' is required." in str(e)
...
def test_get_declarative_data_asset_command_missing_id(mocker):
    from Cyberwatch import get_declarative_data_asset_command

    mocker.patch.object(Client, "_http_request")  # should not be called
    try:
        get_declarative_data_asset_command(client, {})
    except Exception as e:
        assert "Argument 'id' is required." in str(e)
...
def test_fetch_sysadmin_asset_command_missing_id(mocker):
    from Cyberwatch import fetch_sysadmin_asset_command

    try:
        fetch_sysadmin_asset_command(client, {})
    except Exception as e:
        assert "Please provide an asset ID" in str(e)
...
def test_fetch_compliance_asset_command_missing_id(mocker):
    from Cyberwatch import fetch_compliance_asset_command

    try:
        fetch_compliance_asset_command(client, {})
    except Exception as e:
        assert "Please provide an asset ID" in str(e)
