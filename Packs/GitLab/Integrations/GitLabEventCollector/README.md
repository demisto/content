...
| The maximum number of events to fetch for each event type | Each fetch will bring the `limit` number of events for each type (audits, groups and projects) and each group/project ID. For example, if `limit` is set to 1000 and groups/projects IDs are given as well, then the fetch will bring 1000 audit events and 1000 group/project events for each group/project ID. | False |
