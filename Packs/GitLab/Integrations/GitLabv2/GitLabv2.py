...
PIPELINE_SCHEDULE_FIELDS_TO_EXTRACT = {
    "id",
    "description",
    "ref",
    "next_run_at",
...
class Client(BaseClient):
    ...
    def create_issue_request(self, labels: str, title: str, description: str) -> dict:
        headers = self._headers
        suffix = f"/projects/{self.project_id}/issues"
        params = assign_params(labels=labels, title=title, description=description)
    ...
    def delete_merged_branches_request(self) -> dict:
        headers = self._headers
        response = self._http_request(
            "DELETE", f"/projects/{self.project_id}/repository/merged_branches", headers=headers, ok_codes=[200, 202, 204]
        )
        return response
    ...
    def file_delete_request(self, file_path: str, branch: str, commit_message: str) -> dict:
        ...
        response = self._http_request(
            "DELETE", suffix, headers=headers, params=params, ok_codes=[200, 202, 204], resp_type="text"
        )
    ...
    def merge_request_create_request(
        self,
        ...
        allow_collaboration: str | None,
        allow_maintainer_to_push: str | None,
        approvals_before_merge: str | None,
        squash: str | None,
    ) -> dict:
        headers = self._headers
        suffix = f"/projects/{self.project_id}/merge_requests"
    ...
    def merge_request_update_request(
        self,
    ...
    def get_pipeline_request(self, project_id: str, pipeline_id: Optional[str], ref: Optional[str], status: Optional[str]):
        headers = self._headers
    ...
    def get_pipeline_schedules_request(self, project_id: str, pipeline_schedule_id: Optional[str]):
        ...
        return self._http_request("get", final_suffix, headers=headers)
...
def response_according_pagination(client_function: Any, limit: int, page_number: int, params: dict, suffix_id: str | None):
    """
...
def partial_response_fields(object_name: str):
    ...
    if object_name == "Commit":
        return {"id": None, "short_id": None, "title": None, "message": None, "author": ["name"], "created_at": None}

    if object_name == "Issue Note":
        return {"id": None, "created_at": None, "updated_at": None, "body": None, "noteable_iid": None, "author": ["name", "id"]}
...
def merged_branch_delete_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    command_results = CommandResults(
        readable_output="Merged branches Deleted successfully", outputs=response, raw_response=response
    )
    return command_results
...
def get_raw_file_command(client: Client, args: dict[str, Any]) -> list:
    ...
    results = CommandResults(outputs_prefix="GitLab.File", readable_output=human_readable, outputs=outputs, raw_response=response)
...
def file_create_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    if not entry_id and not file_content and not file_path:
        ...
        file_path = encode_file_path_if_needed(file_path)
    response = client.file_create_request(
        file_path, branch, commit_msg, author_email, author_name, file_content, execute_filemode
    )
    return CommandResults(
        outputs_prefix="GitLab.File",
...
def file_update_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    response = client.file_update_request(
        file_path,
        branch,
        start_branch,
        encoding,
...
def commit_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    if commit_id:
        ...
        params = assign_params(
            ...
            until=return_date_arg_as_iso(args.get("created_before")),
            since=return_date_arg_as_iso(args.get("created_after")),
            path=args.get("path"),
...
def issue_note_delete_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(readable_output="Issue note deleted successfully")
...
def merge_request_create_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    response = client.merge_request_create_request(
        source_branch,
        target_branch,
        title,
        assignee_ids,
...
def merge_request_update_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    response = client.merge_request_update_request(
        merge_request_id,
        target_branch,
        title,
        assignee_ids,
...
def merge_request_note_list_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    params = assign_params(sort=args.get("sort"), order_by=args.get("order_by"))
    response = response_according_pagination(
        client.merge_request_note_list_request, limit, page_number, params, merge_request_iid
    )

    for merge_request_note in response:
        merge_request_note_edit = {
            "Id": merge_request_note.get("id", ""),
...
def merge_request_note_delete_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(readable_output="Merge request note deleted successfully")
...
def code_search_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    response = response_according_pagination(client.codes_search_request, limit, page_number, params, None)
    human_readable = tableToMarkdown("Code Search Results", response, removeNull=True, headers=headers)
    return CommandResults(outputs_prefix="GitLab.Code", readable_output=human_readable, outputs=response, raw_response=response)
...
def gitlab_artifact_get_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    outputs = {"job_id": job_id, "artifact_path_suffix": artifact_path_suffix, "artifact_data": response}
    if len(response) <= 100:
...
def main() -> None:  # pragma: no cover
    ...
    commands = {
        "gitlab-group-project-list": group_project_list_command,
    ...
    try:
        ...
        return_error(f"Failed to execute {demisto.command()} command. Error: {error_message}")
