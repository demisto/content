...
### Filters
1. **CVSS Severity Filter** - Filters CVEs by their CVSS severity (LOW, MEDIUM, HIGH, CRITICAL).
2. **CVSS Versions** - When a severity filter is set, controls which CVSS versions (v2, v3, v4) are queried. Defaults to v3 + v4.
3. **Keyword Search** - Uses a keyword (or a sentence) to filter out CVEs by their description. You can test this prior to setup using [NIST website](https://nvd.nist.gov/vuln/search).
4. **KEV Only** - Returns the CVE that appears in CISA's [Known Exploited Vulnerabilities (KEV) Catalog](https://www.cisa.gov/known-exploited-vulnerabilities-catalog).
5. **Max Indicators Per Fetch** - Limits the number of indicators fetched per interval to prevent timeouts during initial sync.
