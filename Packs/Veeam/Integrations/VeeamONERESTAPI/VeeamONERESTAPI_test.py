...
def test_add_property_with_collation(filter_builder):
    filter_builder.add_property("alarmTemplateId", Operation.EQUALS, "330", collation="ignorecase")
    assert len(filter_builder.items) == 1
...
def test_add_node(filter_builder):
    ...
    assert len(filter_builder.items) == 1
    assert filter_builder.items[0] == {"property": "triggeredTime", "operation": "greaterThanOrEqual", "value": "last_fetch_time"}
...
@pytest.mark.parametrize(
    ...
    [("12.1.0.1", "Exception"), ("12.2.0.3770", None), ("12.3.0", None), ("12.1.12.2", "Exception"), ("12.2.0", None)],
)
...
TRIGGERED_ALARMS_INCIDENTS = [
    {
        ...
        "rawJSON": (
            '{"triggeredAlarmId": 4, "name": "VM with no replica", "alarmTemplateId": 330, '
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (datetime.now(), set(), DEFAULT_FETCH, TRIGGERED_ALARMS, (TRIGGERED_ALARMS_INCIDENTS, {"4"}, {"4"})),
        # 2 test case: max_events_for_fetch = 0, => we expect 0 events cause we have 'max_events_for_fetch' = 0
        (datetime.now(), set(), 0, TRIGGERED_ALARMS, ([], set(), set())),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("2024-05-28T15:42:22.198Z", TRIGGERED_ALARMS[0], "2024-05-28T15:42:22.198Z"),
        # 2 test case: 'last_fetch_time' = 2019-08-24T14:15:22Z => we expect 'last_fetch_time' change to the alarm 'triggeredTime'
        ("2019-08-24T14:15:22Z", TRIGGERED_ALARMS[0], "2024-05-27T14:28:52.66Z"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (100, 0, RESPONSE_DATA, [{"id": 1}, {"id": 2}, {"id": 3}, {"id": 4}, {"id": 5}], [{"Offset": 0, "Limit": 100}]),
        (4, 2, RESPONSE_DATA, [{"id": 1}, {"id": 2}], [{"Offset": 0, "Limit": 2}]),
        (3, 3, RESPONSE_DATA, [{"id": 1}, {"id": 2}, {"id": 3}], [{"Offset": 0, "Limit": 3}]),
    ],
)
...
def test_handle_command_with_token_refresh_attempts(client, mocker):
    mock_getIntegrationContext = mocker.patch(
        "VeeamONERESTAPI.demisto.getIntegrationContext", side_effect=[{}, {}, {}, {}, {"token": "valid"}]
...
def test_handle_command_with_token_refresh_attempts_exception(client, mocker):
    mock_getIntegrationContext = mocker.patch(
        "VeeamONERESTAPI.demisto.getIntegrationContext", side_effect=[{}, {}, {}, {}, {}, {}]
    )
    mock_setIntegrationContext = mocker.patch("VeeamONERESTAPI.demisto.setIntegrationContext")
    mock_get_api_key = mocker.patch("VeeamONERESTAPI.get_api_key", return_value="new_api_key")
    mock_set_api_key = mocker.patch("VeeamONERESTAPI.set_api_key")
...
def test_handle_command_with_token_refresh(client, mocker):
    mock_getIntegrationContext = mocker.patch(
        "VeeamONERESTAPI.demisto.getIntegrationContext", side_effect=[{}, {"token": "new_api_key"}, {}, {"token": "new_api_key"}]
    )
    mock_setIntegrationContext = mocker.patch("VeeamONERESTAPI.demisto.setIntegrationContext")
    mock_get_api_key = mocker.patch("VeeamONERESTAPI.get_api_key", return_value="new_api_key")
    mock_set_api_key = mocker.patch("VeeamONERESTAPI.set_api_key")
