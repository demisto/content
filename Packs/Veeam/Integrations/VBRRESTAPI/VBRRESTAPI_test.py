...
from VBRRESTAPI import (
    ...
    fetch_incidents,
    fetch_malware_events,
    fetch_repository_space_incidents,
...
MALWARE_EVENTS_WITHOUT_NEEDED_TYPE = [
    {
        ...
        "machine": {"displayName": "string", "uuid": "string", "backupObjectId": "e5daa78c-c0bb-44d5-8a9c-04130e3d324a"},
        "state": "Created",
        "details": "string",
...
def test_get_malware_incidents(client, mocker, start_time, existed_ids, max_events_for_fetch, response_data, expected_results):
    ...
    malware_incidents, malwareIds, last_fetch_time = get_malware_incidents(client, start_time, existed_ids, max_events_for_fetch)

    assert malware_incidents == expected_results[0]
    assert malwareIds == expected_results[1]

    malware_incidents, malwareIds, last_fetch_time = get_malware_incidents(client, start_time, malwareIds, max_events_for_fetch)
...
def test_get_configuration_backup_incident(
    ...
    backup_incident, backupDate = get_configuration_backup_incident(client, last_successful_backup_date, backup_older_then_days)
    if backup_incident:
    ...
    backup_incident, backupDate = get_configuration_backup_incident(client, backupDate, backup_older_then_days)
...
def test_fetch_malware_events(client, mocker, last_run, max_results, errors_by_command, expected_result):
    ...
    malware_incidents, malwareIds, _ = fetch_malware_events(client, last_run, last_fetch, max_results, errors_by_command)
...
def test_fetch_configuration_backup_incident(client, mocker, last_run, errors_by_command, expected_result):
    ...
    fetch_configuration_backup_incident(client, last_run, backup_older_then_days, errors_by_command)
