...
from VBRRESTAPI import DATE_FORMAT, MAX_INT, Client, FetchClient, \
    search_with_paging, overwrite_last_fetch_time, validate_ipv4, validate_ipv6, validate_time, validate_uuid, validate_bool, \
    validate_int, validate_float, is_api_version_higher_or_equal_than, process_command, handle_command_with_token_refresh, validate_filter_parameter, \
    convert_to_json, get_vcentername, process_error, get_inventory_objects_command, \
    ...
    start_instant_recovery_hyperv_vm_command, start_instant_recovery_hyperv_vm_customized_command, \
    get_session_command, get_session_logs_command, create_malware_event_command, get_access_token, \
    get_security_analyzer_best_practices_command, start_security_analyzer_command, \
    get_security_analyzer_last_run_command, get_job_states_command, get_authorization_events_command, \
    start_vsphere_quick_backup_command, get_yara_rules_command, start_malware_backup_scan_command, \
    mount_entra_id_tenant_command, unmount_entra_id_tenant_command, get_entra_id_items_command, \
    get_entra_id_item_restore_points_command, compare_entra_id_item_properties_command, \
    start_disk_publishing_command, stop_disk_publishing_command, get_disk_publishing_mount_point_command, \
    general_api_request_command, check_license
...
@pytest.mark.parametrize(
    ...
    [
        ({"type": "Subscription", "package": "Suite"}, None),
        ({"type": "Subscription", "edition": "Standard"}, "Exception")
    ]
)
...
def test_start_instant_recovery_hyperv_vm_command(client, mocker, response, expected_command_results, expected_mode):
    mock_request = mocker.patch(
        "VBRRESTAPI.Client.start_instant_recovery_hyperv_vm_request",
        return_value=response
    )
    mocker.patch("VBRRESTAPI.validate_uuid")
    mocker.patch("VBRRESTAPI.validate_bool")
...
def test_start_instant_recovery_hyperv_vm_customized_command(client, mocker, response, expected_command_results, expected_mode):
    mock_request = mocker.patch(
        "VBRRESTAPI.Client.start_instant_recovery_hyperv_vm_customized_request",
        return_value=response
    )
    mocker.patch("VBRRESTAPI.validate_uuid")
    mocker.patch("VBRRESTAPI.validate_bool")
...
SECURITY_ANALYZER_INCIDENT = [
    {
        "name": "Veeam - Configuration is not compliant with security best practices. Affected parameter: Ensure backup encryption is enabled",
        "occurred": "",
        "rawJSON": (
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"a1b2c3d4-e5f6-7890-abcd-ef1234567890"}, SECURITY_ANALYZER_EVENTS,
            ([], {"a1b2c3d4-e5f6-7890-abcd-ef1234567890"})
        ),
        # 3: only non-violation statuses => no incident
        (
            set(),
            [{"id": "b2c3d4e5-f6a7-8901-bcde-f12345678901", "bestPractice": "Configure email notifications",
              "status": "OK", "note": ""}],
            ([], set())
        ),
        # 4: violation with empty note => note replaced with N/A
        (
            set(), SECURITY_ANALYZER_EVENTS_EMPTY_NOTE,
            (SECURITY_ANALYZER_INCIDENT_EMPTY_NOTE, {"a1b2c3d4-e5f6-7890-abcd-ef1234567890"})
        ),
    ]
)
def test_get_security_analyzer_incident(
    client, mocker, existed_ids, response_data, expected_results
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            set(), SURE_BACKUP_EVENTS,
            (SURE_BACKUP_INCIDENTS, {"f1e2d3c4-b5a6-7890-fedc-ba9876543210"})
        ),
        # 2: failed job id already in existed_ids => no new incident, id kept
        (
            {"f1e2d3c4-b5a6-7890-fedc-ba9876543210"}, SURE_BACKUP_EVENTS,
            ([], {"f1e2d3c4-b5a6-7890-fedc-ba9876543210"})
        ),
        # 3: non-failed job => no incident
        (
            set(),
            [{"id": "a0b1c2d3-e4f5-6789-abcd-ef0123456789", "name": "SureBackup Job 2",
              "description": "Desc", "objectsCount": 3, "lastResult": "Success",
              "lastRun": "2024-05-16T07:21:32Z", "status": "Stopped", "type": "SureBackupContentScan"}],
            ([], set())
        ),
    ]
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"sure_backup_ids": []},
            {"error_count_in_sure_backup": 1},
            [{"type": "incident_on_error"}]
        ),
    ]
)
...
def test_fetch_security_analyzer_incidents(
    client, mocker, last_run, errors_by_command, expected_result
):
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"security_ids": []},
            {"error_count_in_security_analyzer": 1},
            [{"type": "incident_on_error"}]
        ),
    ]
)
def test_fetch_security_analyzer_incidents_with_exception(
    client, mocker, last_run, errors_by_command, expected_incidents
):
...
def test_fetch_incidents(client, mocker, last_run, first_fetch_time, response_data, expected_results):
    ...
    next_run, incidents = fetch_client.fetch_incidents(
        first_fetch_time=first_fetch_time, max_malware_events_for_fetch=DEFAULT_FETCH,
        max_repos_space_events_for_fetch=DEFAULT_FETCH, backup_older_then_days=30,
        free_space_less_then=200, fetch_malware_incidents=True, fetch_backup_repository_events=True,
        fetch_configuration_backup_events=True, fetch_security_analyzer_events=True,
