...
@pytest.mark.parametrize(
    "data, expected",
    [
        ([{"urn": "hostsystem:foo"}, {"urn": "hostsystem:bar"}], "foo"),
        ([{"urn": "hostsystem:baz"}, {"urn": "folder:123"}], "baz"),
        ([{"urn": "folder:321"}, {"urn": "folder:789"}], ""),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"data": [{"urn": "hostsystem:foo"}, {"urn": "hostsystem:bar"}]},
            "foo",
            {"outputs_prefix": "Veeam.HOST", "outputs": {"parsed_value": "foo"}},
        )
    ],
)
