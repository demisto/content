...
def prisma_access_logout_user(computer: str, domain: str, user: str, tenant: str) -> dict[str, str]:
    if apiConfigured:
        ...
        if tenant:
            ...
            cmd = cmd.replace("<gpcs>", f"<gpcs>{tenant_entry}").replace(
                "</logout_mobile_user>", "</logout_mobile_user></multi-tenant>"
            )
        params = {"type": "op", "key": API_KEY, "cmd": cmd}
        result = http_request(URL, "GET", params=params)
        return result
    else:
        ...
        demisto.results(
            {
                ...
                "ReadableContentsFormat": formats["markdown"],
                "HumanReadable": hr,
                "EntryContext": {"PrismaAccess.LogoutUser": res},
            }
        )
    else:
