...
@pytest.mark.parametrize(
    # Write and define the expected
    "args",
    [{"folder": "Shared", "position": "pre", "tsg_id": "1234567"}],
)
...
@pytest.mark.parametrize(
    # Write and define the expected
    "args",
    [{"folder": "Shared", "position": "pre", "rule_id": "####385c-1c8a-42fc-94e4-####cbd148b9"}],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            {"rule_id": "####ec11-b599-4372-a0d7-####ecb8203", "action": "deny", "to": "any", "overwrite": False},
            {"action": "deny", "to": ["any"]},
        ),
        (
            {"rule_id": "####ec11-b599-4372-a0d7-####ecb8203", "action": "deny", "to": "to", "overwrite": True},
            {"action": "deny", "to": ["to"]},
        ),
        (
            {"rule_id": "####ec11-b599-4372-a0d7-####ecb8203", "action": "deny", "to": "to", "overwrite": False},
...
@pytest.mark.parametrize(
    # Write and define the expected
    "args",
    [{"rule_id": "####385c-1c8a-42fc-94e4-####cbd148b9", "tsg_id": "1234567"}],
)
...
@pytest.mark.parametrize(
    # Write and define the expected
    "args",
    [{"id": "####f837-379e-4c48-a967-####7a52ec14", "tsg_id": "1234567"}],
)
...
@pytest.mark.parametrize(
    # Write and define the expected
    "args",
    [{"folder": "Shared", "limit": "20", "tsg_id": "1234567"}],
)
...
@pytest.mark.parametrize(
    ...
    [
        ({"id": "id1", "overwrite": True, "value": "www.test.com"}, ["www.test.com"]),
        ({"id": "id1", "overwrite": False, "value": "www.test.com"}, ["www.google.com", "www.test.com"]),
    ],
)
...
@pytest.mark.parametrize(
    # Write and define the expected
    "args, expected_results",
    [({"id": "1", "overwrite": True, "source_url": "www.test1.com", "frequency": "hourly"}, EXPECTED_DYNAMIC_LIST)],
)
...
@pytest.mark.parametrize(
    "input, expected_output", [("external-dynamic-list.json", "five_minute"), ("external-dynamic-list2.json", None)]
)
...
@pytest.mark.parametrize(
    ...
    "args, expected_result",
    [
        ({"type": "ip", "source_url": "test.com"}, "test.com"),
        ({"type": "predefined_url", "predefined_url_list": "panw-auth-portal-exclude-list"}, "panw-auth-portal-exclude-list"),
        ({"type": "predefined_ip", "predefined_ip_list": "panw-torexit-ip-list"}, "panw-torexit-ip-list"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    "args, expected_result",
    [
        ({"type": "ip", "predefined_url_list": "test.com"}, "Please provide the source_url"),
        ({"type": "predefined_url", "source_url": "panw-auth-portal-exclude-list"}, "Please provide the predefined_url_list"),
        ({"type": "predefined_ip", "source_url": "panw-torexit-ip-list"}, "Please provide the predefined_ip_list"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"frequency": "daily", "frequency_hour": "00"}, {"daily": {"at": "00"}}),
        (
            {"frequency": "weekly", "day_of_week": "sunday", "frequency_hour": "00"},
            {"weekly": {"day_of_week": "sunday", "at": "00"}},
        ),
        ({"frequency": "monthly", "day_of_month": "1", "frequency_hour": "00"}, {"monthly": {"day_of_month": "1", "at": "00"}}),
    ],
)
...
@pytest.mark.parametrize(
    ...
    "args, expected_result",
    [
        ({"frequency": "daily"}, "Please provide the frequency_hour"),
        ({"frequency": "weekly", "frequency_hour": "00"}, "Please provide the day_of_week"),
        ({"frequency": "weekly", "day_of_week": "1"}, "Please provide the frequency_hour"),
        ({"frequency": "monthly", "frequency_hour": "00"}, "Please provide the day_of_month"),
    ],
)
...
@pytest.mark.parametrize(
    # Write and define the expected
    "args, expected_result",
    [({"page": "2", "page_size": "2"}, {"limit": 2, "offset": 2}), ({"limit": "2"}, {"limit": 2}), ({}, {"limit": 50})],
)
...
def test_get_cie_user(mocker):
    ...
    client = create_mocked_client()
    args = {
        "attributes_to_filter_by": "Distinguished Name, Unique Identifier, Common-Name, Name, User Principal Name",
...
def test_get_cie_user_prepare_args():
    ...
    expected_result = {
        "domain": "example.com",
        "attrs": ["Unique Identifier", "Common-Name", "Distinguished Name", "User Principal Name", "Name", "Manager"],
        "name": {
            "attrNameOR": ["Distinguished Name", "Unique Identifier", "Common-Name", "Name", "User Principal Name"],
            "attrValue": "CN=Test,UID=TestID,DC=example,DC=com",
            "match": "equal",
        },
