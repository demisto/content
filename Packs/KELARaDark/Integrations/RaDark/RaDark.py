...
class Client(BaseClient):
    def __init__(self, base_url: str, verify: bool, headers: dict[str, str], proxy: bool, api_key: str, monitor_id: str):
    ...
    def search_alerts(self, start_time: int | None, max_results: int, incident_types: list[str]):
        ...
        params: dict[str, dict] = {
            "filters": {"and": {"start_date": start_time}},
            "sort": {"by": "incident_date", "order": "asc"},
        }
        ...
        try:
            return self._http_request(
                method="POST",
                url_suffix=FETCH_INCIDENTS_API.format(MONITOR_ID=self.monitor_id, API_KEY=self.api_key, max_results=max_results),
                json_data=params,
                headers=headers,
            )
    ...
    def incident_get_items(self, incident_id: str) -> dict[str, Any] | None:
        try:
            return self._http_request(
                method="GET",
                url_suffix=FETCH_ITEMS_API.format(incident_id=incident_id, API_KEY=self.api_key, MONITOR_ID=self.monitor_id),
            )
        except Exception as e:
            return_error(str(e))
            return None
    ...
    def email_enrich_data(self, email: str) -> dict[str, Any] | None:
        ...
        params = {
            ...
            "getAll": False,
            "filters": {"and": {"emails": [email], "indices": [{"identifier": "leaked_credentials"}]}},
        }
        headers = {"Content-Type": "application/json"}
        try:
            return self._http_request(
    ...
    def message_on_incident(self, message: dict[str, Any], room: dict[str, str]) -> dict[str, Any] | None:
        ...
        try:
            return self._http_request(
                method="POST", url_suffix=MESSAGE_API.format(API_KEY=self.api_key), json_data=params, headers=headers
            )
        except Exception as e:
            return_error(str(e))
            return None
    ...
    def get_mention_list(self) -> dict[str, Any] | None:
        try:
            return self._http_request(
                method="GET", url_suffix=MENTIONS_LIST_API.format(API_KEY=self.api_key, MONITOR_ID=self.monitor_id)
            )
        except Exception as e:
            return_error(str(e))
            return None
...
def parse_incident_markdown_table(data: dict, incident_id: str):
    ...
    if aggr_type == "reports":
        ...
        details = (
            "incident may contain textual context or screenshots that include the company's assets. "
...
def parse_hacking_discussions_markdown_table(items: list[dict[str, Any]], aggr: dict[str, Any]):
    ...
    for i in range(len(lines)):
        ...
        if len(tag_catched) > 0:
            snippet_lines = []
            for j in range(
                max(0, i - HACKING_DISCUSSIONS_LINES_SNIPPET), min(len(lines), i + 1 + HACKING_DISCUSSIONS_LINES_SNIPPET)
            ):
...
def get_first_time_fetch(first_fetch: str) -> int | None:
    first_fetch_time = arg_to_datetime(
        arg=first_fetch if first_fetch else DEFAULT_FIRST_TIME_TO_FETCH, arg_name="First fetch time", required=True
    )
...
def fetch_incidents(
    client: Client, max_results: int, last_run: dict[str, int], first_fetch_time: int | None, incident_types: list[str]
    ...
    alerts = client.search_alerts(start_time=last_fetch, max_results=max_results, incident_types=incident_types)["data"][
        "aggregations"
    ]
    ...
    for alert in alerts:
        ...
        if sub_type in SUPPORTED_SUB_TYPES:
            ...
            alert["incident_url"] = INCIDENT_URL.format(MONITOR_ID=client.monitor_id, incident_id=alert["feed_property_id"])
...
def incident_get_items_command(client: Client, args: dict[str, Any]) -> CommandResults:
    try:
        ...
        if (
            ...
            and isinstance(incident_data.get("data"), dict)
            and isinstance(incident_data["data"].get("incidents"), list)
            and len(incident_data["data"]["incidents"]) > 0
        ):
...
def email_enrich_command(client: Client, args: dict[str, Any]) -> CommandResults:
    try:
        ...
        if (
            ...
            and isinstance(email_data.get("data"), dict)
            and isinstance(email_data["data"].get("incidents"), list)
            and len(email_data["data"]["incidents"]) > 0
        ):
...
def item_handle_command(client: Client, args: dict[str, Any]) -> CommandResults:
    try:
        ...
        if isinstance(action_res, dict) and isinstance(action_res.get("data"), dict) and action_res["data"].get("value"):
...
def item_purchase_command(client: Client, args: dict[str, Any]) -> CommandResults:
    try:
        ...
        if (
            isinstance(mentions_list_res, dict)
            and isinstance(mentions_list_res.get("data", ""), list)
            and len(mentions_list_res["data"]) > 0
            and isinstance(mentions_list_res["data"][0], dict)
            ...
            if isinstance(item_res, dict) and isinstance(item_res.get("data", ""), dict):
                ...
                if (
                    isinstance(incident_res, dict)
                    and isinstance(incident_res.get("data", ""), dict)
                    and isinstance(incident_res["data"].get("chat", ""), dict)
                    and isinstance(incident_res["data"]["chat"].get("room", ""), dict)
                ):
                ...
                if (
                    isinstance(action_res, dict)
                    and isinstance(action_res.get("data", ""), dict)
                    ...
                    if (
                        isinstance(message_res, dict)
                        and isinstance(message_res.get("data", ""), dict)
...
def main() -> None:
    ...
    try:
        # Initialize Client
        client = Client(
            base_url=BASE_URL, verify=verify_certificate, headers={}, proxy=proxy, api_key=API_KEY, monitor_id=MONITOR_ID
        )
