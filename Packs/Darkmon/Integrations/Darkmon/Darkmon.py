register_module_line('Darkmon', 'start', __line__())
CONSTANT_PACK_VERSION = '1.0.0'
demisto.debug('pack id = Darkmon, pack version = 1.0.0')
...
def classification_to_dbot_score(classification: Optional[str]) -> int:
...
def _redact_rows(rows: List[Dict[str, Any]], redact: bool,
                 sensitive: set = SENSITIVE_FIELDS_FOR_REDACTION) -> List[Dict[str, Any]]:
    if not redact:
        return rows
    return [
...
def build_dbot_outputs(value: str, indicator_type: str,
    ...
    """
    classification = next(
        (item.get('classification') for item in (search_results or [])
...
def extract_features_to_dict(item: dict) -> dict:
    ...
    if indicator_type == 'domain':
        ...
        result.update({
            'jarm_fingerprint': item.get('jarm_fingerprint')
        })
...
class Client(BaseClient):
    ...
    def global_search(self, query: str, indicator_type: str, page: int = 0, size: int = DEFAULT_SIZE) -> dict:
        allowed_types = [
            "Domain", "IP", "URL", "Hash", "CVE", "Email", "Username",
            "Malware", "Source", "Keyword",
    ...
    def get_compromised_data(self, data_type: str, page: int = 0, size: int = DEFAULT_SIZE,
        ...
        if data_type not in endpoint_map:
            raise ValueError(
                f"Unsupported compromised data type: {data_type}. "
                f"Supported types: {', '.join(endpoint_map.keys())}"
            )
        ...
        return self._http_request(
            method='GET',
            url_suffix= endpoint_map[data_type],
            params=params
        )
    ...
    def get_vpn(self, page: int = 0, size: int = DEFAULT_SIZE,
                sort: Optional[str] = None) -> dict:
        params: Dict[str, Any] = {'page': page, 'size': size}
        if sort:
            params['sort'] = sort
    ...
    def get_proxy(self, page: int = 0, size: int = DEFAULT_SIZE,
                  sort: Optional[str] = None) -> dict:
        params: Dict[str, Any] = {'page': page, 'size': size}
        if sort:
            params['sort'] = sort
    ...
    def get_nrd(self, page: int = 0, size: int = DEFAULT_SIZE,
                sort: Optional[str] = None) -> dict:
        params: Dict[str, Any] = {
            'page': page,
            'size': size,
    ...
    def get_tbf(self, page: int = 0, size: int = DEFAULT_SIZE,
                sort: Optional[str] = None) -> dict:
        params: Dict[str, Any] = {
            'page': page,
            'size': size,
    ...
    def get_ransomware(self, mentions: bool = False, page: int = 0,
                       size: int = DEFAULT_SIZE, sort: Optional[str] = None) -> dict:
    ...
    def get_board_protection_requests(self, page: int = 0, size: int = DEFAULT_SIZE,
    ...
    def get_board_leaks(self, leak_type: str, email: str, page: int = 0,
                        size: int = DEFAULT_SIZE, term: Optional[str] = None) -> dict:
        endpoint_map = {
        ...
        if leak_type not in endpoint_map:
            raise ValueError(
                f"Unsupported board leak type: {leak_type}. "
                f"Supported types: {', '.join(endpoint_map.keys())}"
            )
...
def dmontip_get_indicators_command(client: Client, args: dict) -> CommandResults:
    ...
    outputs = {
        'Darkmon.Indicator(val.id == obj.id)': indicators
    }
...
def dmontip_get_compromised_command(client: Client, args: dict) -> CommandResults:
    ...
    outputs: Dict[str, Any] = {
        f'Darkmon.Compromised.{singular}': content_items
    }
    if page_obj:
    ...
    priority_columns: Dict[str, List[str]] = {
        'accounts': [
            'id', 'username', 'password', 'url', 'firstSeen', 'firstSeenDate',
            'firstCompromiseDate', 'lastCompromiseDate', 'state', 'valid',
            'compromiseSourcesCount', 'countries', 'sources', 'stealers'
        ],
        'employees': [
            'id', 'username', 'password', 'url', 'firstSeen', 'firstSeenDate',
            'firstCompromiseDate', 'lastCompromiseDate', 'state', 'valid',
        ...
        'combo-lists': [
            'id', 'username', 'password', 'source', 'firstSeen', 'messageTime',
            'firstSeenDate', 'state', 'valid'
        ],
        'bank-cards': [
            'id', 'cardNumber', 'cardHolders', 'cvvs', 'expiry', 'firstSeen',
            'firstSeenDate', 'state', 'valid', 'compromiseSourcesCount',
            'countries', 'sources', 'stealers', 'chatIds', 'chatNames',
            'chatUsernames'
        ],
        'public-breaches': [
            'id', 'source', 'breachTime', 'name', 'username', 'password',
            'emails', 'phoneNumbers', 'address', 'country', 'gender',
            'birthDate', 'facebookUsername', 'githubUsername', 'linkedinUsername',
            'twitterUsername', 'firstSeen', 'firstSeenDate', 'state', 'valid',
            'photoUrl'
        ]
    }
...
def dmontip_get_vpn_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        ...
        headers=ordered_headers,
        removeNull=True
    ) if rows else 'No VPN data found'
...
def dmontip_get_proxy_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        ...
        headers=ordered_headers,
        removeNull=True
    ) if rows else 'No proxy data found'
...
def dmontip_get_cve_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        ...
        headers=ordered_headers,
        removeNull=True
    ) if rows else 'No vulnerability data found'
...
def dmontip_get_nrd_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        ...
        headers=ordered_headers,
        removeNull=True
    ) if rows else 'No newly-registered domains found'
...
def dmontip_get_tbf_command(client: Client, args: dict) -> CommandResults:
    ...
    readable_output = tableToMarkdown(
        ...
        headers=ordered_headers,
        removeNull=True
    ) if rows else 'No Telnet Brute Force IOCs found'
...
def dmontip_get_ransomware_command(client: Client, args: dict) -> CommandResults:
    ...
    priority = [
        'id', 'victimName', 'victimDomain', 'threatActor', 'description',
        'publishedAt', 'updatedAt', 'firstSeen', 'state', 'valid',
        'matchedKeywordsLength', 'matchedKeywords', 'screenShotUrl', 'articleSourceId'
    ]
    ...
    readable_output = tableToMarkdown(
        ...
        headers=ordered_headers,
        removeNull=True
    ) if rows else f'No ransomware {title_type.lower()} found'
...
def dmontip_get_landscape_command(client: Client, args: dict) -> CommandResults:
    ...
    priority = [
        'id', 'title', 'link', 'publicationDate', 'source', 'author',
        'firstSeen', 'firstSeenDate', 'state', 'valid', 'categories',
        'description', 'matchedKeywords', 'matchedKeywordsLength'
    ]
    ...
    readable_output = tableToMarkdown(
        ...
        headers=ordered_headers,
        removeNull=True
    ) if rows else f'No landscape {title_type.lower()} found'
...
def dmontip_get_boardprotection_command(client: Client, args: dict) -> CommandResults:
    ...
    priority = [
        'value', 'type', 'state', 'firstName', 'middleName', 'lastName',
        'reason', 'createdBy', 'createdAt', 'updatedAt', 'tokens', 'id',
    ]
...
def dmontip_get_boardemails_command(client: Client, args: dict) -> CommandResults:
    ...
    priority_columns: Dict[str, List[str]] = {
        'accounts': [
            'email', 'id', 'compromiseDate', 'username', 'password', 'url',
            'machineUsername', 'ip', 'country', 'stealer', 'source',
        ],
        'combo-lists': [
            'email', 'id', 'messageTime', 'username', 'password', 'source',
        ],
        'public-breaches': [
            'email', 'id', 'breachTime', 'source', 'name', 'username', 'password',
            'address', 'country', 'birthDate', 'gender', 'firstSeen', 'firstSeenDate',
            'facebookUsername', 'githubUsername', 'linkedinUsername', 'twitterUsername',
            'photoUrl',
        ],
    }
...
def _search_each(client: Client, args: dict, arg_key: str, type_label: str,
                 missing_msg: str) -> List[CommandResults]:
    ...
    for v in values:
        cr = dmontip_global_search_command(client, {
            'query': v,
            'type': type_label,
...
register_module_line('Darkmon', 'end', __line__())
