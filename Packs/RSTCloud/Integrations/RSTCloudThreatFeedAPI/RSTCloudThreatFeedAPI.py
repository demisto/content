...
URLREGEX = (
    r"^(?:(?:(?:https?|ftps?):)?\/\/)?((?:\S+(?::\S*)?@)?(?:(?!(?:10|127)(?:\.\d{1,3}){3})(?!(?:169\.254|192\.168)"
...
class RSTFile(Common.File):
    ...
    def to_context(self):
        ret_value = super().to_context()
        file_context = {"MD5": self.md5, "SHA1": self.sha1, "SHA256": self.sha256}
        if self.rstscore:
...
class Client:
    ...
    def get_indicator(self, value):
        ...
        r = requests.get(apiurl, headers=headers, verify=self.verify, proxies=self.proxy)
        return r.json()
    ...
    def submit_indicator(self, value, desc="manual submission"):
        ...
        headers = {"Accept": "*/*", "X-Api-Key": self.apikey}
        r = requests.post(apiurl, json=payload, headers=headers, verify=self.verify, proxies=self.proxy)
        return r.json()
    ...
    def submit_falsepositive(self, value, desc="manual submission"):
        ...
        apiurl = self.api_url + endpoint
        headers = {"Accept": "*/*", "X-Api-Key": self.apikey}
        r = requests.put(apiurl, json=payload, headers=headers, verify=self.verify, proxies=self.proxy)
        return r.json()
...
def test_module(client: Client) -> str:
    ...
    params = [
        ...
        "threshold_url",
        "threshold_hash",
        "indicator_expiration_ip",
        "indicator_expiration_domain",
...
def ip_command(client: Client, args: dict[str, str]) -> tuple[list, list, list]:
    ...
    for ip in addresses:
        ...
        if "error" in indicator:
            ...
            markdown_item += tableToMarkdown(
                f'RST Threat Feed IP Reputation for: {indicator["ioc_value"]}\n', table, removeNull=True
            )
            markdown.append(markdown_item)
            raw_results.append(parse_indicator_response(indicator, "IP"))
...
def domain_command(client: Client, args: dict[str, str]) -> tuple[list, list, list]:
    ...
    for domain in domains:
        ...
        if "error" in indicator:
            ...
            markdown_item += tableToMarkdown(
                f'RST Threat Feed Domain Reputation for: {indicator["ioc_value"]}\n', table, removeNull=True
            )
            markdown.append(markdown_item)
            raw_results.append(parse_indicator_response(indicator, "Domain"))
            indicators.append(result)
...
def url_command(client: Client, args: dict[str, str]) -> tuple[list, list, list]:
    ...
    for url in urls:
        ...
        if "error" in indicator:
            ...
            result = RSTUrl(url=indicator["ioc_value"], dbot_score=dbot_score, tags=indicator.get("tags", "").get("str", ""))
            human_readable_score = ""
            if calc_score == 3:
                human_readable_score = "Malicious"
            if calc_score == 2:
            ...
            markdown_item += tableToMarkdown(
                f'RST Threat Feed URL Reputation for: {indicator["ioc_value"]}\n', table, removeNull=True
            )
            markdown.append(markdown_item)
            raw_results.append(parse_indicator_response(indicator, "URL"))
            indicators.append(result)
...
def file_command(client: Client, args: dict[str, str]) -> tuple[list, list, list]:
    ...
    for hash in hashes:
        ...
        if "error" in indicator:
            ...
            result = Common.File(md5=indicator["md5"], sha1=indicator["sha1"], sha256=indicator["sha256"], dbot_score=dbot_score)
            human_readable_score = ""
            if calc_score == 3:
                human_readable_score = "Malicious"
            if calc_score == 2:
            ...
            markdown_item += tableToMarkdown(
                f'RST Threat Feed File Reputation for: {indicator["ioc_value"]}\n', table, removeNull=True
            )
            markdown.append(markdown_item)
            raw_results.append(parse_indicator_response(indicator, "File"))
            indicators.append(result)
