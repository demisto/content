...
import pytest
from AWSEKS import list_clusters_command, validate_args
from test_data.test_response import (
...
def test_validate_args_one_arg():
    ...
    logging_arg = {"clusterLogging": [{"types": ["api", "authenticator", "audit"], "enabled": True}]}
...
def test_validate_args_no_args():
    ...
    try:
        ...
        assert (
            str(e) == "Please provide exactly one of the following arguments: resources_vpc_config, logging or "
            "authentication_mode."
        )
...
def test_list_clusters_command_no_clusters(mocker):
    ...
    Then:
        - assert that the readable output is correct.
    """
    response = {"clusters": [], "nextToken": None}
    mocker.patch.object(Boto3Client, "list_clusters", return_value=response)
...
def test_list_clusters_command_with_pagination(mocker):
    ...
    assert result.outputs == {"ClustersNames": ["cluster_name_1", "cluster_name_100", "cluster_name_101"], "NextToken": None}
...
def test_update_cluster_config_authentication_mode_command(mocker):
    ...
    http_request = mocker.patch.object(
        Boto3Client, "update_cluster_config", return_value=UPDATE_CLUSTER_CONFIG_ACCESS_CONFIG_RESPONSE
    )
    client = Boto3Client()
    args = {"cluster_name": "cluster_name", "authentication_mode": "true"}
    access_config = {"authenticationMode": "API_AND_CONFIG_MAP"}
    update_cluster_config_command(client, args)
    http_request.assert_called_with(name="cluster_name", accessConfig=access_config)
...
def test_create_access_entry_command(mocker):
    ...
    client = Boto3Client()
    args = {"cluster_name": "cluster_name", "principal_arn": "principal_arn"}
...
def test_associate_access_policy_command(mocker):
    ...
    client = Boto3Client()
    args = {"cluster_name": "clusterName", "principal_arn": "principalArn", "policy_arn": "policyArn", "type": "cluster"}
...
def test_associate_access_policy_command_namespaces():
    ...
    args = {"cluster_name": "clusterName", "principal_arn": "principalArn", "policy_arn": "policyArn", "type": "namespace"}
    try:
        client = Boto3Client()
...
def test_associate_access_policy_command_namespaces_assert_called_with(mocker):
    ...
    access_scope = {"type": "namespace", "namespaces": ["namespace1"]}
    associate_access_policy_command(client, args)
    http_request.assert_called_with(
        clusterName=args.get("cluster_name"),
...
ARGS_CLUSTER = {"cluster_name": "cluster_name", "policy_arn": "policy_arn", "principal_arn": "principal_arn", "type": "cluster"}
ARGS_NAMESPACES = {
    "cluster_name": "cluster_name",
...
ACCESS_SCOPE_CLUSTER = {"type": "cluster", "namespaces": []}
ACCESS_SCOPE_NAMESPACES = {"type": "namespace", "namespaces": ["namespace_1"]}


@pytest.mark.parametrize("args, access_scope", [(ARGS_CLUSTER, ACCESS_SCOPE_CLUSTER), (ARGS_NAMESPACES, ACCESS_SCOPE_NAMESPACES)])
def test_associate_access_policy_command_type_cluster(mocker, args, access_scope):
    ...
    http_request = mocker.patch.object(
        Boto3Client, "associate_access_policy", return_value=UPDATE_CLUSTER_CONFIG_ACCESS_CONFIG_RESPONSE
    )
    client = Boto3Client()
    associate_access_policy_command(client, args)
    http_request.assert_called_with(
