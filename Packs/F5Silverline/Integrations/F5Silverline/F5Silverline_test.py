...
IP_ADDRESSES_TO_DELETE = [
    ...
    ({"list_type": "allowlist", "object_id": "850f7418-2ac9", "note": "test"}, "", False),
    (
...
IP_OBJECT_GET_LIST = [
    ({"list_type": "denylist", "object_id": ["id1"]}, "ip_object_list_by_id.json"),
    ({"list_type": "denylist"}, "ip_object_list_no_id.json"),
    ({"list_type": "denylist", "page_number": "1", "page_size": "1"}, "ip_object_list_no_id.json"),
    ({"list_type": "denylist", "page_number": "1", "page_size": "1", "object_id": ["id1"]}, "ip_object_list_by_id.json"),
]
...
def test_add_ip_objects_command(mocker, args, expected_output):
    ...
    client = create_client(base_url="https://portal.f5silverline.com/api/v1/ip_lists", verify=False, headers={}, proxy=False)
...
def test_delete_ip_objects_command(mocker, args, expected_output, is_object_exist):
    ...
    if not is_object_exist or not result:
        ...
        assert_called_with_expected_values: dict[str, Any] = {
            "body": {},
            "method": "DELETE",
            "url_suffix": f"{list_type}/ip_objects/850f7418-2ac9",
            "params": {"list_target": "proxy"},
            "resp_type": "content",
        }
...
def test_get_ip_objects_list_command(mocker, args, response_json):
    ...
    assert results.outputs["IPObjectList"][0].get("attributes") == {
        "ip": "1.2.3.4",
        "mask": "32",
        "duration": 0,
        "expires_at": "None",
