...
def fetch_notables(service, cache_object=None, enrich_notables=False):
    ...
    extensive_log('[SplunkPy] Size of last_run_fetched_ids after removing old IDs: {}'.format(len(last_run_fetched_ids)))
    ...
    if len(incidents) == 0:
        ...
        extensive_log('[SplunkPy] SplunkPy - Next run time with too many incidents:  {}'.format(last_run_time))
        new_last_run = {
            'time': last_run_time,
