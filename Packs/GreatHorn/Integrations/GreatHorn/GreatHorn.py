...
class Client(BaseClient):
    ...
    def get_policy(self, policy_id: Optional[str] = None) -> Dict[str, Any]:
        if policy_id:
            return self._http_request(method="GET", url_suffix="/policy/{}".format(policy_id))
        else:
            return self._http_request(method="GET", url_suffix="/policy")
    ...
    def set_policy(self, policy_id: str, update_method: str, policy_json: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return self._http_request(method=update_method, url_suffix="/policy/{}".format(policy_id), json_data=policy_json)

    def search_events(self, args: Optional[dict]) -> Dict[str, Any]:
        return self._http_request(method="POST", url_suffix="/search/events", json_data=args)

    def remediate_message(self, action: str, action_args: Optional[dict]) -> Dict[str, Any]:
        return self._http_request(method="POST", url_suffix="/remediation/{}".format(action), json_data=action_args)

    def revert_remediate_message(self, action: str, action_args: Optional[dict]) -> Dict[str, Any]:
        return self._http_request(method="POST", url_suffix="/remediation/revert/{}".format(action), json_data=action_args)
...
def gh_search_message_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=events_md, outputs_prefix="GreatHorn.Message", outputs_key_field="eventId", outputs=result
    )


def gh_revert_remediate_message_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    valid_actions: Dict[str, list] = {
        "banner": [],
        "quarantinerequest": [],
        "quarantinerelease": [],
        "quarantinedeny": [],
    ...
    return CommandResults(
        readable_output=human_readable, outputs_prefix="GreatHorn.Remediation", outputs_key_field="eventId", outputs=results
    )
...
def gh_set_policy_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(
        readable_output=human_readable, outputs_prefix="GreatHorn.Policy", outputs_key_field="id", outputs=results
    )
...
def gh_get_policy_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    return CommandResults(readable_output=policies_md, outputs_prefix="GreatHorn.Policy", outputs_key_field="id", outputs=results)
...
def gh_get_message_command(client: Client, args: Dict[str, Any]) -> CommandResults:
    ...
    if len(result.get("results", [])) > 0:
        ...
        for link in message.get("links"):
            link_dict = {"Text": link.get("text"), "Url": link.get("url"), "Tags": ", ".join(link.get("tags", []))}
            if showalllinks:
        ...
        for file in message.get("files"):
            f = {"Name": file.get("fileName"), "Type": file.get("fileType"), "SHA256": file.get("fileHash")}
            files.append(f)
        ...
        if message.get("flag") is not None:
            for policy_id in message.get("flag"):
                ...
                for action in policy.get("actions"):
                    actions.append(action.get("type"))
                p = {"ID": policy.get("id"), "Name": policy.get("name"), "Actions": ",".join(actions)}
                policies.append(p)
        ...
        return CommandResults(readable_output="GreatHorn event not found", outputs={})
