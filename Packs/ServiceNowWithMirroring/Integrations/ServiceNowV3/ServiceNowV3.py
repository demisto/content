...
def fetch_incidents(client: Client):
    ...
    mirror_tags = demisto.params().get('entry_tags', []).split(",")
    ...
    for result in res.get('result', []):
        ...
        # Extend the incident to contain data relevant for mirroring
        result['dbotMirrorInstance'] = instance_name
        result['dbotMirrorDirection'] = mirror_direction
        result['dbotMirrorTags'] = mirror_tags
...
def get_remote_data_command(client: Client, args: dict = None, params: dict = None):
    record_id = args.get('id')
    table_name = params.get('ticket_type')
    last_update = dateparser.parse(args.get('lastUpdate')).timestamp()
    result = client.get(table_name, record_id)
    case = result.get('result', {})
    entries = list()
    ...
    sys_param_query = f'element_id={record_id}^element=comments^ORelement=work_notes'
    result = client.query('sys_journal_field', 500, 0, sys_param_query)
    notes = result.get('result', [])
    for note in notes:
        note_timestamp = int(dateparser.parse(note.get('sys_created_on')).timestamp())
        if note_timestamp > last_update:
            entries.append({
                'Type': entryTypes['note'],
                'Category': 'chat',
                'Contents': f'### ServiceNow "{note.get("element")}" entry from "{note.get("sys_created_by")}"\n\n  ({note.get("sys_created_on")}Z): *{note.get("value")}*',
                'ContentsFormat': formats['markdown']
            })
    return [case] + entries


def update_remote_system_command(client: Client, args: dict = None, params: dict = None):
    data = args.get('data', {})
    ...
    record_id = args.get('remoteId')
    entry_tags = params.get('entry_tags', 'work_notes,comments').split(",")
    table_name = params.get('ticket_type')
    custom_fields = {}

    if incident_changed:
        client.update(table_name, record_id, changes, custom_fields)
    ...
    if entries and len(entries) > 0:
        for entry in entries:
            ...
            if not key:
                continue
            ticket_type = client.get_table_name(table_name)
            user = entry.get('user', 'dbot')
            comment = entry.get('contents', '')
            note = f"({user}): {comment}"
            result = client.add_comment(record_id, ticket_type, key, note)

    return record_id


def get_mapping_fields_command(client: Client, args: dict = None, params: dict = None):

    # Build a default schema that covers all fields common across various incident types
    schema_options = ['change_request', 'incident', 'problem', 'sc_req_item', 'sc_request', 'sc_task']
    instance_schema = params.get('ticket_type')
    default_schema = dict()
    current_schema = dict()
    for schema_type in schema_options:
        res = client.get_table_fields(schema_type)
        if "result" in res:
            result = res['result'][0]
            if schema_type == instance_schema:
                current_schema = result
            if not default_schema:
                default_schema = result
            else:
                this_schema_fields = result.keys()
                default_schema = {k: v for k, v in default_schema.items() if k in this_schema_fields}
    instance_name = demisto.integrationInstance()
    mirror_direction = demisto.params().get('mirror')
    mirror_direction = None if mirror_direction == "Disabled" else mirror_direction
    mirror_tags = params.get('entry_tags', []).split(",")
    default_schema['dbotMirrorInstance'] = instance_name
    default_schema['dbotMirrorDirection'] = mirror_direction
    default_schema['dbotMirrorTags'] = mirror_tags
    current_schema['dbotMirrorInstance'] = instance_name
    current_schema['dbotMirrorDirection'] = mirror_direction
    current_schema['dbotMirrorTags'] = mirror_tags
    return {
        "default Schema": default_schema,
        f"{instance_schema} Schema": current_schema
    }
...
def main():
    ...
    try:
        ...
        elif command == 'servicenow-test-schema':
