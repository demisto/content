...
FIRST_FETCH_LOOKBACK_MINUTES = 5
...
def fetch_events_command(client: Client, max_fetch: int, last_run: dict):
    # Without a stored LastRun, look back FIRST_FETCH_LOOKBACK_MINUTES instead of "now" (the API returns
    # events at/after start_date_time and events arrive delayed, so "now" would yield nothing).
    start_date_time = last_run.get("LastRun")
    if not start_date_time:
        first_fetch_dt = datetime.utcnow() - timedelta(minutes=FIRST_FETCH_LOOKBACK_MINUTES)
        start_date_time = first_fetch_dt.strftime(DATE_FORMAT)
