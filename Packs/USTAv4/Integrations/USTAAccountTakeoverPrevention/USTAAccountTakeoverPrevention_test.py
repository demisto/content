...
from USTAAccountTakeoverPrevention import (
    Client,
    check_module,
    compromised_credentials_search_command,
...
@pytest.mark.parametrize(
    "username, mock_response_file, expected_output_file",
    [
        (
            "user1",
            "test_data/compromised_credentials_search_response.json",
            "test_data/compromised_credentials_search_response.json",
        ),
        ("user2", "test_data/search_empty_response.json", "test_data/search_empty_response.json"),
    ],
)
...
def test_fetch_incidents(mocker):
    ...
    next_run, incidents = fetch_incidents(client=client, max_results=100, last_run={}, first_fetch_time="3 days")
...
def test_subsequent_run(mocker):
    ...
    next_run, incidents = fetch_incidents(client=client, max_results=3, last_run=last_run, first_fetch_time=first_fetch)
...
def test_main_search_cmd(mocker):
    ...
    mocker.patch.object(
        Client,
        "compromised_credentials_search_api_request",
