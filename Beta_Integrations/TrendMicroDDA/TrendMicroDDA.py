...
def calculate_checksum(api_key, headers, body=''):
    ...
    temp += body
    return hashlib.sha1(temp.encode('utf-8'))
...
def http_request(uri, method, headers, body={}, params={}, files={}):
    ...
    url = '{}/{}'.format(str(SERVER_URL), str(uri))
...
def build_report(res, threshold, status, verbose):
    ...
    hr_headers = {
        ...
        'Image Type': reports.get('IMAGE_TYPE', {}).get('TYPE', {}).get('#text', 'Unknown'),
