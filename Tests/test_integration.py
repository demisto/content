...
def __create_incident_with_playbook(client, name, playbook_id, integration_name):
    # create incident
    ...
    if inc_id == 'incCreateErr':
        print_error('Failed to create incident for integration name: {} and playbookID: {}.'
                    'Possible reasons are:\nMismatch between playbookID in conf.json and '
                    'the id of the real playbook you were trying to use,'
                    'or schema problems in the TestPlaybook.'.format(integration_name, playbook_id))
        return False, -1
...
def test_integration(client, integrations, playbook_id, options=None, is_mock_run=False):
    ...
    incident, inc_id = __create_incident_with_playbook(client, 'inc_%s' % (playbook_id, ), playbook_id, integration_name)
