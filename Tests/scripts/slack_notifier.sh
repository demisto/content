...
if [ "$4" = "gitlab" ]; then
  python3 ./Tests/scripts/slack_notifier.py -n $IS_NIGHTLY -u "$CI_PIPELINE_URL" -b "$CI_PIPELINE_ID" -s "$SLACK_TOKEN" -c "$TEMP_GITLAB_SLACK_TOKEN" -t "$1" -f "$2" -bu $IS_BUCKET_UPLOAD -j "$CI_JOB_NAME" -ca "$ARTIFACTS_FOLDER" -ch "$3" -g "$CI_SERVER_URL" -gp "$CI_PROJECT_ID"
else
