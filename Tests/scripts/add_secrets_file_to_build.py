...
def get_secrets_from_gsm(branch_name: str, options: argparse.Namespace, yml_pack_ids: list[str]) -> dict:
    ...
    master_secrets = secret_conf_prod.list_secrets(options.gsm_project_id, labels_filter_master,
                                                   name_filter=yml_pack_ids, with_secrets=True)
    branch_secrets = secret_conf_dev.list_secrets(options.gsm_project_id, labels_filter_branch,
                                                  name_filter=yml_pack_ids, with_secrets=True)
