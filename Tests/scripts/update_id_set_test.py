...
def test_find_duplicates():
    ...
    temp_report = tempfile.NamedTemporaryFile(mode="w+", prefix='report-', suffix='.json', dir='Reports')  # disable-secrets-detection
    json.dump(REPORT_DATA, temp_report)
    temp_report.flush()
    os.fsync(temp_report.fileno())
    temp_report2 = tempfile.NamedTemporaryFile(mode="w+", prefix='report-', suffix='.json', dir='Reports')  # disable-secrets-detection
    json.dump(REPORT_DATA, temp_report2)
    temp_report2.flush()
    os.fsync(temp_report2.fileno())

    # create duplicate Widgets
    temp_widget = tempfile.NamedTemporaryFile(mode="w+", prefix='widget-', suffix='.json', dir='Widgets')  # disable-secrets-detection
    json.dump(WIDGET_DATA, temp_widget)
    temp_widget.flush()
    os.fsync(temp_widget.fileno())
    temp_widget2 = tempfile.NamedTemporaryFile(mode="w+", prefix='widget-', suffix='.json', dir='Widgets')  # disable-secrets-detection
    json.dump(WIDGET_DATA, temp_widget2)
    temp_widget2.flush()
    os.fsync(temp_widget2.fileno())

    # create duplicate Classifier
    temp_classifier = tempfile.NamedTemporaryFile(mode="w+", prefix='classifier-', suffix='.json', dir='Classifiers')  # disable-secrets-detection
    json.dump(WIDGET_DATA, temp_classifier)
    temp_classifier.flush()
    os.fsync(temp_classifier.fileno())
    temp_classifier2 = tempfile.NamedTemporaryFile(mode="w+", prefix='classifier-', suffix='.json', dir='Classifiers') # disable-secrets-detection
    json.dump(WIDGET_DATA, temp_classifier2)
    temp_classifier2.flush()
    os.fsync(temp_classifier2.fileno())

    # create duplicate Layout
    temp_layout = tempfile.NamedTemporaryFile(mode="w+", prefix='layout-', suffix='.json', dir='Layouts')  # disable-secrets-detection
    ...
    temp_dashboard = tempfile.NamedTemporaryFile(mode="w+", prefix='dashboard-', suffix='.json', dir='Dashboards')  # disable-secrets-detection
    json.dump(DASHBOARD_DATA, temp_dashboard)
    temp_dashboard.flush()
    os.fsync(temp_dashboard.fileno())
    temp_dashboard2 = tempfile.NamedTemporaryFile(mode="w+", prefix='dashboard-', suffix='.json', dir='Dashboards')  # disable-secrets-detection
    ...
    temp_incident_field = tempfile.NamedTemporaryFile(mode='w+', prefix='incidentfield-', suffix='.json',  # disable-secrets-detection
    ...
    temp_indicator_field = tempfile.NamedTemporaryFile(mode='w+',prefix='incidentfield-', suffix='.json',   # disable-secrets-detection
    ...
    temp_id_set = tempfile.NamedTemporaryFile(mode="w+", prefix='temp_id_set-', suffix='.json', dir='Tests/scripts')  # disable-secrets-detection
