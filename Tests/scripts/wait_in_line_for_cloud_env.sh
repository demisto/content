...
python3 ./Tests/scripts/utils/lock_cloud_machines.py --service_account $GCS_ARTIFACTS_KEY --gcs_locks_path $GCS_LOCKS_PATH  --ci_job_id $CI_JOB_ID  --test_machines_list "test-machines-ga"  --gitlab_status_token $GITLAB_STATUS_TOKEN --lock_machine_name "$LOCK_MACHINE_NAME" --number_machines_to_lock 1 --response_machine CloudEnvVariables
