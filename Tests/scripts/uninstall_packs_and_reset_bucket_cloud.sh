...
if [[ -z ${CLOUD_CHOSEN_MACHINE_ID} ]]; then
  ...
  sleep 120
  python3 ./Tests/Marketplace/search_and_uninstall_pack.py --cloud_machine $CLOUD_CHOSEN_MACHINE_ID --cloud_servers_path $CLOUD_SERVERS_PATH --cloud_servers_api_keys "cloud_api_keys.json" --unremovable_packs $UNREMOVABLE_PACKS --one-by-one --build-number "$CI_PIPELINE_ID" --is_nightly $IS_NIGHTLY
fi
