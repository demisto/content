...
all_dir=`find . -type d -not \( -path "./.*" -o -path "./Templates*" -o -path "./node_modules*" -o -path "./venv*" -o -path "./Packs*" -o -path "*infrastructure_tests*" -o -path "*scripts/awsinstancetool*" \) | xargs`
dir_for_pylint=`find . -maxdepth 1 -type d -not \( -path "./.*" -o -path "./Templates*" -o -path "./node_modules*" -o -path "./venv*" -o -path "./Packs*" \)`
...
#echo "Run pylint"
#python -m pylint --disable=C0301,C0116,R0801 $dir_for_pylint || errors=$?
