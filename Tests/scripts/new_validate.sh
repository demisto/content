...
if [[ $CI_COMMIT_BRANCH = master ]] || [[ -n "${NIGHTLY}" ]] || [[ -n "${BUCKET_UPLOAD}" ]] || [[ -n "${DEMISTO_SDK_NIGHTLY}" ]]; then
    if [[ -n "${PACKS_TO_UPLOAD}" ]]; then
        ...
        for item in $PACKS_TO_UPLOAD_SPACED; do
            python3 -m demisto_sdk validate --skip-old-validate -i Packs/"$item" --post-commit --graph --skip-pack-dependencies --run-new-validate
        done       
    else
        ...
        python3 -m demisto_sdk validate --skip-old-validate -a --graph --skip-pack-dependencies --prev-ver $PREV_VER --run-new-validate
    fi
elif [[ $CI_COMMIT_BRANCH =~ pull/[0-9]+ ]]; then
    python3 -m demisto_sdk validate --skip-old-validate -g --post-commit --graph --skip-pack-dependencies --run-new-validate
elif [[ $CI_COMMIT_BRANCH = demisto/python3 ]]; then
    python3 -m demisto_sdk validate --skip-old-validate -g --post-commit --no-conf-json --allow-skipped --graph --skip-pack-dependencies --run-new-validate
else
    python3 -m demisto_sdk validate --skip-old-validate -g --post-commit --graph --skip-pack-dependencies --run-new-validate
fi