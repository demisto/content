...
CLOUD_SERVERS_PATH1=$(cat xsiam_servers_path)
...
if [[ -z ${XSIAM_CHOSEN_MACHINE_ID} ]]; then
  ...
  python3 ./Tests/Marketplace/search_and_uninstall_pack.py --xsiam_machine $XSIAM_CHOSEN_MACHINE_ID --xsiam_servers_path $CLOUD_SERVERS_PATH1 --xsiam_servers_api_keys $CLOUD_API_KEYS
fi
