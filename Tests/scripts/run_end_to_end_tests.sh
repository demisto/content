...
if [[ -n "$test_path" ]]; then
  if [[ -z "${CLOUD_CHOSEN_MACHINE_IDS}" ]]; then
    ...
    for CLOUD_CHOSEN_MACHINE_ID in "${CLOUD_CHOSEN_MACHINE_ID_ARRAY[@]}"; do
      ...
      python3 -m pytest "$test_path" -v --cloud_machine "${CLOUD_CHOSEN_MACHINE_ID}" --cloud_servers_path "${CLOUD_SERVERS_PATH}" --cloud_servers_api_keys "cloud_api_keys.json" --integration_secrets_path "${SECRET_CONF_PATH}" --disable-warnings --junitxml=${ARTIFACTS_FOLDER}/end_to_end_tests_result.xml
