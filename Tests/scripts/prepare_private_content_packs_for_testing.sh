...
if [ ! -f $CONTENT_PACKS_TO_INSTALL_FILE ]; then
  ...
  if [[ -z "$CONTENT_PACKS_TO_INSTALL" ]]; then
    ...
    python3 ./Tests/Marketplace/upload_packs.py -b $GCS_TESTING_BUCKET -pb $GCS_PRIVATE_TESTING_BUCKET -n $$GITHUB_RUN_NUMBER -e $NEW_EXTRACT_FOLDER -sb $PUBLIC_TARGET_PATH
