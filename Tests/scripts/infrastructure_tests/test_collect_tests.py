...
def _test(monkeypatch, case_mocker: CollectTestsMocker, collector_class: Callable,
    ...
    assert collected.packs_to_install == set(expected_packs or ())
...
def test_nightly(monkeypatch, case_mocker: CollectTestsMocker, collector_class: Callable, expected_tests: set[str],
    ...
    """
    expected_packs = expected_packs + ALWAYS_INSTALLED_PACKS
...
@pytest.mark.parametrize(
    ...
    (
            # (0) change in a sanity-collection-triggering file,
            # expecting xsoar sanity tests to be collected
            ...
            (MockerCases.C, None, ('bothMarketplacesPack', 'bothMarketplacesPackOnlyXSIAMIntegration'), None, None,
             XSOAR_BRANCH_ARGS, (), ('bothMarketplacesPack', 'bothMarketplacesPackOnlyXSIAMIntegration'), None),
...
def test_branch(
    ...
    if expected_packs_to_upload is None:
        expected_packs_to_upload = expected_packs + ALWAYS_INSTALLED_PACKS
...
def test_no_file_type_and_non_content_dir_files_are_ignored(mocker, monkeypatch):
    ...
    _test(monkeypatch, case_mocker=MockerCases.A_xsoar, collector_class=BranchTestCollector, expected_tests=(),
          expected_modeling_rules_to_test=(), expected_packs=ALWAYS_INSTALLED_PACKS, expected_packs_to_upload=(),
...
def test_only_collect_pack(mocker, monkeypatch, file_type: collect_tests.FileType):
    ...
    _test(monkeypatch, case_mocker=MockerCases.H, collector_class=BranchTestCollector,
          expected_tests=(), expected_packs=expected_packs + ALWAYS_INSTALLED_PACKS,
...
def test_invalid_content_item(mocker, monkeypatch):
    ...
    _test(monkeypatch, case_mocker=MockerCases.H, collector_class=BranchTestCollector,
          expected_tests=(), expected_packs=ALWAYS_INSTALLED_PACKS, expected_packs_to_upload=(), expected_machines=None,
...
def test_upload_all_packs(mocker, monkeypatch):
    """
    given:  The override_all_packs flag.
    when:   Collecting tests.
    then:   Make sure all packs are collected, and no exception is raised.
    """

    _test(monkeypatch, case_mocker=MockerCases.C, collector_class=UploadAllCollector,
          expected_tests=(), expected_packs=(), expected_packs_to_upload=(
            'myXSOAROnlyPack', 'bothMarketplacesPack', 'CoreAlertFields', 'bothMarketplacesPackOnlyXSIAMIntegration',
            'myXSIAMOnlyPack', 'Whois'), expected_machines=None,
          expected_modeling_rules_to_test=None,
          collector_class_args=(MarketplaceVersions.XSOAR, None))
