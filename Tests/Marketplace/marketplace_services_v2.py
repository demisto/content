...
class Pack:
    ...
    def __init__(self, pack_name, pack_path, is_modified=None):
        ...
        self._user_metadata = {}  # initialized in load_pack_metadata function
        ...
        self._tags = set()  # initialized in enhance_pack_attributes function
        self._modules = None
    ...
    def _load_pack_dependencies_metadata(self, index_folder_path):
        ...
        return dependencies_metadata_result
    ...
    def load_pack_metadata(self):
        ...
        try:
            ...
            with open(metadata_path) as metadata_file:
                metadata = json.load(metadata_file)  # loading user metadata
                # part of old packs are initialized with empty list
                metadata = {} if isinstance(metadata, list) else metadata

            # store important user metadata fields
            self.support_type = metadata.get(Metadata.SUPPORT, Metadata.XSOAR_SUPPORT)
            self.current_version = metadata.get(Metadata.CURRENT_VERSION, '')
            self.hidden = metadata.get(Metadata.HIDDEN, False)
    ...
    def _collect_pack_tags(self, trending_packs) -> set:
        tags = set()
