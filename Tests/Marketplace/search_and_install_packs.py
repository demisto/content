...
def install_all_content_packs(client: demisto_client, host: str, server_version: str):
    ...
    for pack_id in os.listdir(PACKS_FULL_PATH):
        if pack_id not in IGNORED_FILES:
            ...
            # Check if the server version is greater than the minimum server version required for this pack or if the pack is hidden (deprecated):
            if ('Master' in server_version or LooseVersion(server_version) >= LooseVersion(server_min_version)) and not hidden:
                all_packs.append(get_pack_installation_request_data(pack_id, pack_version))
