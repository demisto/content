...
def test_remove_old_pack_from_private_id_set():
    workday_pack = {"Workday": {"pack": "Workday"}}

    abuse_pack = {"abuse.ch SSL Blacklist Feed": {"pack": "Feedsslabusech"}}
    ...
    assert abuse_pack not in private_id_set['integrations']

    private_id_set = remove_old_pack_from_private_id_set(os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                                                      "test_data", "id_set.json"), '')
    assert abuse_pack in private_id_set['integrations']
