...
function sendRequestInSession(method, uri, body) {
    if (!SESSION_DATA || !SESSION_DATA.access_token) {
        throw "Field to get access token for Salesforce integration.";
    }
...
function userToEntry(raw_info, title, userMapping) {
    ...
    return createEntry(raw_info, {
        contextPath: 'SalesForce.GetUser(val.ID && val.ID == obj.ID)',
        title: title,
        data: [
            {to: 'ID', from: 'Id', humanReadable: false},
            {to: 'Alias', from: 'Alias'},
...
function searchToEntry(searchRecords) {
    ...
    if (case_ids.length > 0) {
        ...
        var casesA = queryObjects(properties, "Case", condition).records;
        entries.push(casesToEntry(casesA, 'Cases:', userMapping));
    }
...
function getUser(oid, caseNumber) {
    ...
    if (caseNumber !== undefined) {
        ...
        users = JSON.parse(sendRequestInSession('GET', 'sobjects/User/'+ cases[0].OwnerId ).Body);

        var conditionA = "Id='" +  cases[0].OwnerId + "'";
        propertiesA = ['Id', 'Name', 'Alias', 'CommunityNickname', 'Email','FirstName','Username'];
        var usersA = queryObjects(propertiesA, "User", conditionA).records;
        return usersToEntry(usersA, 'User #' + cases[0].OwnerId + ':', getUserNames());


    }

    if (oid !== undefined) {
        users = JSON.parse(sendRequestInSession('GET', 'sobjects/'+'User').Body);
        return userToEntry(users.records, 'User #' + oid + ':', getUserNames());
    }
