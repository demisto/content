...
HOSTS_HEADERS = ['id','name','entity','os','hostStatus','location','riskLevel','threatLevel','dateUpdated','hostZone']
...
def http_request(method, url_suffix, data = None, headers = HEADERS):
    ...
    if res.status_code not in {200, 201, 207}:
        return_error('Error in API call to {}, status code: {}, reason: {}'.format(BASE_URL + '/' + url_suffix, res.status_code, res.json()['message']))
...
def add_host(dataArgs):
    ...
    return_outputs(readable_output = dataArgs.get('name') + " added successfully to " + dataArgs.get('entity-name'), outputs = None, raw_response = res)
