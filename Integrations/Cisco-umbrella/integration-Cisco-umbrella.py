...
# each field has Name,Info & ContextKey  - that just looks scary
SECURITY_RESULT_INFO = {
    'dga_score': { 'Name': 'DGA', 'ContextKey': 'DGA', 'Info': 'Domain Generation Algorithm. This score is generated based on the likeliness of the domain ' +
    ...
    'ks_test': { 'Name': 'Kolmogorov-Smirnov Test', 'ContextKey': 'KolmogorovSmirnovTest', 'Info': "Kolmogorov-Smirnov test on geodiversity. 0 means that the client traffic matches what is expected for this TLD" },
...
def get_domain_categorization_command():
    ...
    if categorization:
        ...
        if categorization.get('security_categories'):
            ...
            contents['Malware Categories'] = security_categories
...
def get_domain_security_command():
    ...
    if res:
        ...
        for key in SECURITY_RESULT_INFO:
            context_key = SECURITY_RESULT_INFO[key].get('ContextKey', format_string_to_context_key_format(SECURITY_RESULT_INFO[key]['Name']))
...
def get_ip_dns_history_command():
    ...
    results.append ({
        ...
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('RRS:', response_contents, headers) + tableToMarkdown('Features:', features_contents, headers),
        'EntryContext': context
    })
...
def get_domain_command():
    ...
    whois = {
        'Name': whois['domainName'],
        'Registrar Name': whois['registrantName'],
        'Last Retrieved': timestamp_to_date(whois['timeOfLatestRealtimeCheck']),
        'Created': whois['created'],
        'Updated': whois['updated'],
        'Expires': whois['expires'],
        'IANAID': whois['registrarIANAID'],
        'Last Observed': whois['auditUpdatedDate']
    }
    ...
    'Type': entryTypes['note'],
        ...
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('"Umbrella Investigate" Domain Reputation for: ' + domain, contents, headers) +
        ...
        tableToMarkdown('Emails:', { 'Emails': emails }, headers) +
...
def get_related_domains_command():
    ...
    if related_domains:
        ...
        if is_co_occurences:
            ...
            if contents_co_occurences:
                co_occurences_md = tableToMarkdown('"Umbrella Investigate" Domain Co-occurences for: ' + domain, contents_co_occurences, headers)
                if related_domains:
    ...
    results.append ({
        ...
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('"Umbrella Investigate" Related Domains for a Domain: ', contents_related_domains, headers) + co_occurences_md,
        'EntryContext': context
    })
...
def get_domain_query_volume_command():
    ...
    if response_object:
        ...
        start_date = dates[0]
        stop_date = dates[1]
        # Query timestamp/hour needs to be calculated manually, every entry represents 1 hour (3600 secods). ts is in milliseconds
    ...
    results.append ({
        ...
        'Contents': contents,
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('"Umbrella Investigate" Domain Volume: ' + domain + '\nStart Date ' + timestamp_to_date(start_date) + ' - Stop Date ' + timestamp_to_date(stop_date), contents, headers),
        'EntryContext': context
    })
...
def get_domains_for_nameserver_command():
    ...
    for nameserver in nameservers:
        if re.match('^(([a-zA-Z]|[a-zA-Z][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z]|[A-Za-z][A-Za-z0-9\-]*[A-Za-z0-9])$', nameserver) == None:
...
def get_whois_for_domain_command():
    ...
    results.append ({
        ...
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('"Umbrella Investigate" WHOIS Record Data for: ' + whois['Name'], table_whois, headers) + tableToMarkdown('Nameservers: ', contents_nameserver, headers) + tableToMarkdown('Email Addresses: ', contents_email, headers),
        'EntryContext': context
    })
...
def get_malicious_domains_for_ip_command():
    ...
    if res:
        ...
        context[outputPaths['domain']] = context_malicious
        context[outputPaths['dbotscore']] = context_dbotscore
    ...
    results.append ({
        ...
        'Contents': contents,
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('"Umbrella Investigate" Malicious Domains for an IP: ' + ip, contents, headers),
        'EntryContext': context
    })
...
def get_domain_using_regex_command():
    ...
    results.append ({
        ...
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('"Umbrella Investigate" Domain Pattern Search for: ' + regex, title_contents, headers) + tableToMarkdown('Matches: ', contents, headers),
        'EntryContext': context
    })
...
def get_domain_timeline_command():
    ...
    # Get vars
    domain = extract_domain_name(demisto.args()['domain'])
    if re.match('[a-zA-Z\d-]{,63}(\.[a-zA-Z\d-]{,63})*', domain) == None:
...
try:
    if demisto.command() == 'test-module':
        ...
        demisto.results('ok')
        sys.exit(0)
    elif demisto.command() == 'investigate-umbrella-domain-categorization' or demisto.command() ==  'umbrella-domain-categorization':
    ...
    elif demisto.command() == 'investigate-umbrella-domain-co-occurrences' or demisto.command() == 'umbrella-domain-co-occurrences':
    ...
    elif demisto.command() == 'investigate-umbrella-domain-dns-history' or demisto.command() == 'umbrella-domain-dns-history':
    ...
    elif demisto.command() == 'investigate-umbrella-ip-malicious-domains' or demisto.command() == 'umbrella-ip-malicious-domains':
