...
''' GLOBALS '''
VERBOSE = True
SERVER = demisto.params().get('server').rstrip('/') + '/'
...
def http_request(method, url_suffix, params=None, headers=None, threshold=THRESHOLD):
    if headers is None:
        headers = HEADERS

    LOG('running request with url=%s' % (SERVER + url_suffix))
    try:
        analysis = requests.request(method, SERVER + url_suffix, headers=headers, params=params, verify=not INSECURE)
...
def analysis_to_entry(info, threshold=THRESHOLD, verbose=VERBOSE):
    ...
    for analysis in info:
        ...
        if verbose:
            reports = sum([report_dict.get("categories") for report_dict in analysis.get("reports")], [])
