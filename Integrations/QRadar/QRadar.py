...
def upload_indicators_command():
    ...
    try:
        ...
        if len(indicators_values_list) == 0:
            return_error("No indicators found, Reference set {0} didn't change".format(reference_name))
        else:
    ...
    # Gets an error if the user tried to add indicators that dont match to the reference set type
        ...
        if '1005' in str(e):
            return_error("You tried to add indicators that dont match to reference set type")
