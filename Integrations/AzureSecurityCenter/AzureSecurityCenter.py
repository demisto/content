...
def format_jit_port_rule(ports):
    ...
    for port in ports:
        ...
        p_src_addr = str(port.get('allowedSourceAddressPrefix')) if port.get(
            'allowedSourceAddressPrefix') != '*' else 'any'
...
def get_alert(resource_group_name, asc_location, alert_id):
    ...
    cmd_url += '/providers/Microsoft.Security/locations/{}/alerts/{}?api-version={}'.format(asc_location, alert_id,
                                                                                            ALERT_API_VERSION)
...
def list_alerts_command(args):
    ...
    alerts = list_alerts(resource_group_name, asc_location, filter_query, select_query, expand_query).get('value')
    outputs = list()
    for alert in alerts:
        properties = alert.get('properties')
        if properties:
...
def update_alert_command(args):
    ...
    response = update_alert(resource_group_name, asc_location, alert_id, alert_update_action_type)
    outputs = {
        "ID": response.get("id"),
...
def get_atp_command(args):
    ...
    outputs = {
        "ID": response.get('id'),
        "Name": response.get('name'),
        "IsEnabled": response["properties"]["isEnabled"] if response.get('properties') and response.get(
            'properties').get('isEnabled') else None
    }
    md = tableToMarkdown(
...
def list_aps_command():
    ...
    for setting in settings:
        outputs.append({
            'Name': setting.get('name'),
            'AutoProvision': setting.get('properties').get('autoProvision') if setting.get(
                'properties') and setting.get('properties').get('autoProvision') else None,
            'ID': setting.get('id')
        })
...
def get_aps_command(args):
    ...
    outputs = [{
        'Name': setting.get('name'),
        'AutoProvision': setting.get('properties').get('autoProvision') if setting.get('properties') and setting.get(
            'properties').get('autoProvision') else None,
        'ID': setting['id']
    }]
    md = tableToMarkdown(
    ...
    if policies:
        for policy in policies:
            ...
            outputs.append({
                ...
                'InformationTypes': policy.get('properties').get('informationTypes') if policy.get(
                    'properties') and policy.get('properties').get('informationTypes') else None,
                'ID': policy['id']
            })
        md = tableToMarkdown(
...
def list_ipp(management_group=None):
    ...
    if management_group:
        cmd_url += '/providers/Microsoft.Management/managementGroups/{}'.format(management_group)
...
def get_ipp(policy_name, management_group):
    ...
    cmd_url += '/providers/Microsoft.Security/informationProtectionPolicies/{}?api-version={}'.format(policy_name,
                                                                                                      IPP_API_VERSION)
...
def get_jit_command(args):
    ...
    property_table_output = [{
        'Name': policy.get('name'),
        'Kind': policy.get('kind'),
        'ProvisioningState': policy.get('properties').get('provisioningState') if policy.get(
            'properties') and policy.get('properties').get('provisioningState') else None,
        'Location': policy.get('location'),
        'Rules': policy.get('properties').get('virtualMachines') if policy.get('properties') and policy.get(
            'properties').get('virtualMachines') else None,
        'Requests': policy.get('properties').get('requests') if policy.get('properties') and policy.get(
            'properties').get('requests') else None,
        'ID': policy.get('id')
    }]
    md = tableToMarkdown(
