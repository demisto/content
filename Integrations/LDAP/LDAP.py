...
class Client():
    def __init__(self, host, port, username, password, secure, getInfo="SCHEMA"):
        ...
        self.conn = Connection(self.server, user=self.username, password=self.password, raise_exceptions=True)
        self.bind = None
...
def search_command(client):
    ...
    client.search(base, inputFilter, attributes)
...
def checkAttribute_command(client):
    ...
    return_results(
        'Check Attribute {}:'.format(attribute),
        entry,
        'LDAP.Attribute.Check(val.dn && val.dn == obj.dn && val.attribute && val.attribute == obj.attribute)')
