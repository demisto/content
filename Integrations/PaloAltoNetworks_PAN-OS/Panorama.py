...
def http_request(uri, method, headers={}, body={}, params={}, files=None):
    ...
    if jresult['response']['@status'] != 'success':
        if 'msg' in jresult['response'] and 'line' in jresult['response']['msg']:
            ...
            elif isinstance(jresult['response']['msg']['line'], str) and jresult['response']['msg']['line'].find(
                    'job') != -1 and jresult['response']['msg']['line'].find('not found') != -1:
            ...
            # catch already registered ip tags and return this as an entry.note
                ...
                if isinstance(jresult['response']['msg']['line']['uid-response']['payload']['register']['entry'], list):
                    ips = [o['@ip'] for o in
                           jresult['response']['msg']['line']['uid-response']['payload']['register']['entry']]
                else:
...
def prepare_security_rule_params(api_action=None, xpath=None, rulename=None, source=None, destination=None,
                                 ...
                                 description=None, target=None, log_forwarding=None):
...
def panorama_commit_status_command():
    ...
    demisto.results({
        ...
        'Contents': result,
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('Commit status:', commit_status_output, ['JobID', 'Status', 'Details'], None,
                                         True),
...
def panorama_push_status_command():
    ...
    demisto.results({
        ...
        'HumanReadable': tableToMarkdown('Push to Device Group status:', push_status_output,
                                         ['JobID', 'Status', 'Details'], None, True),
...
def panorama_get_address_command():
    ...
    demisto.results({
        ...
        'Contents': address,
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('Address:', address_output, ['Name', 'IP_Netmask', 'IP_Range', 'FQDN'], None,
                                         True),
        'EntryContext': {
...
def panorama_create_address(address_name, fqdn=None, ip_netmask=None, ip_range=None, description=None):
    ...
    params['element'] = add_argument(fqdn, 'fqdn', False) + add_argument(ip_netmask, 'ip-netmask',
                                                                         False) + add_argument(ip_range, 'ip-range',
                                                                                               False) + add_argument(
...
def panorama_list_address_groups_command():
    ...
    demisto.results({
        ...
        'HumanReadable': tableToMarkdown('Address groups:', address_groups_output,
                                         ['Name', 'Type', 'Addresses', 'Match', 'Description'], None, True),
        'EntryContext': {
...
def panorama_create_dynamic_address_group(address_group_name, match, description=None):
    params = {
        ...
        'type': 'config',
        'xpath': XPATH_OBJECTS + "address-group/entry[@name='" + address_group_name + "']",
        'element': "<dynamic>" + add_argument(match, 'filter', False) + "</dynamic>" + add_argument(description,
                                                                                                    'description',
                                                                                                    False),
        'key': API_KEY
    }
...
def panorama_list_services_command():
    ...
    demisto.results({
        ...
        'HumanReadable': tableToMarkdown('Services:', services_output,
                                         ['Name', 'Protocol', 'SourcePort', 'DestinationPort', 'Description'], None,
                                         True),
        'EntryContext': {
...
def panorama_create_service(service_name, protocol, destination_port, source_port=None, description=None):
    ...
    params['element'] = '<protocol>' + '<' + protocol + '>' + add_argument(destination_port, 'port',
                                                                           False) + add_argument(source_port,
                                                                                                 'source-port',
                                                                                                 False) + '</' + protocol + '>' + '</protocol>' + add_argument(
        description, 'description', False)
...
def panorama_get_service_group_command():
    ...
    demisto.results({
        ...
        'Contents': result,
        'ReadableContentsFormat': formats['markdown'],
        'HumanReadable': tableToMarkdown('Service group:', prettify_service_group(result), ['Name', 'Services'], None,
                                         True),
        'EntryContext': {
...
def panorama_get_custom_url_category_command():
    ...
    demisto.results({
        ...
        'HumanReadable': tableToMarkdown('Custom URL Category:', custom_url_category_output,
                                         ['Name', 'Sites', 'Description'], None, True),
        'EntryContext': {
...
def panorama_edit_custom_url_category(custom_url_category_name, sites, description=None):
    params = {
        ...
        'type': 'config',
        'xpath': XPATH_OBJECTS + "profiles/custom-url-category/entry[@name='" + custom_url_category_name + "']",
        'element': "<entry name='" + custom_url_category_name + "'>" + add_argument(description, 'description',
                                                                                    False) + add_argument_list(sites,
                                                                                                               'list',
                                                                                                               True) + "</entry>",
        'key': API_KEY
    }
...
def panorama_edit_url_filter(url_filter_name, element_to_change, element_value, add_remove_element=None):
    ...
    if element_to_change == 'description':
        params[
            'xpath'] = XPATH_OBJECTS + "profiles/url-filtering/entry[@name='" + url_filter_name + "']/" + element_to_change
...
def panorama_get_pcap_command():
    ...
    if result.headers['Content-Type'] != 'application/octet-stream':
        return_error(
            'PCAP download failed. Most likely cause is the file size limitation. For information on how to download manually, see the documentation for this integration.')
