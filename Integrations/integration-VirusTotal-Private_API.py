...
def get_domain_report_command():
    ...
    if categories is not None:
        ...
        md += tableToMarkdown("Domain Categories", [{'Categoty': c} for c in categories])
    ...
    if (positives >= threshold):
        ec.update({
            outputPaths['url']: {
                'Data': url,
                'Malicious': {
                    'Description': 'Detections / Total: ' + positives + ' / ' + response.get('total', ''),
                    'Vendor': 'VirusTotal - Private API'
                },
            },
        })
...
def get_ip_report_command():
    ...
    ec['DBotScore'] = []
    dbotScore = 0
    bad_downloads_amount = len(undetected_downloaded_samples) if undetected_downloaded_samples else 0
...
try:
    if demisto.command() == 'test-module':
        ...
        if get_url_report('www.google.com', 0):
            demisto.results('ok')
        else:
