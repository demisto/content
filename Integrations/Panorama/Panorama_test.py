...
def test_build_policy_match_query():
    ...
    expected = '<test><security-policy-match><source>1.1.1.1</source><destination>2.3.4.5</destination>' \
...
def test_prettify_matching_rule():
    ...
    matching_rule = {'action': 'my_action1', '@name': 'very_important_rule', 'source': '3.4.5.6', 'destination': 'any'}
    response = prettify_matching_rule(matching_rule)
    expected = {'Action': 'my_action1', 'Name': 'very_important_rule', 'Source': '3.4.5.6', 'Destination': 'any'}
    assert response == expected
