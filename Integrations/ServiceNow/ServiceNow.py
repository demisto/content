...
DEFAULTS = {"limit": 10, "offset": 0, "fetch_limit": 10}
...
def send_request(path, method="get", body=None, params=None, headers=None, file=None):
    ...
    if res.status_code < 200 or res.status_code >= 300:
        raise Exception(
            "Got status code {} with url {} with body {} with headers {}".format(
                str(res.status_code), url, str(res.content), str(res.headers)
            )
        )
...
def create_ticket_context(data, ticket_type):
    ...
    if "closed_by" in data:
        context["ResolvedBy"] = (
            data["closed_by"]["value"] if "value" in data["closed_by"] else ""
        )
    if "opened_by" in data:
        context["OpenedBy"] = (
            data["opened_by"]["value"] if "value" in data["opened_by"] else ""
        )
        context["Creator"] = (
            data["opened_by"]["value"] if "value" in data["opened_by"] else ""
        )
    if "assigned_to" in data:
        context["Assignee"] = (
            data["assigned_to"]["value"] if "value" in data["assigned_to"] else ""
        )
    ...
    if "priority" in data:
        ...
        if demisto.command() in DEPRECATED_COMMANDS:
            ...
            context["Priority"] = TICKET_PRIORITY.get(
    ...
    if "state" in data:
        ...
        if demisto.command() not in DEPRECATED_COMMANDS:
            if ticket_type in TICKET_STATES:
                mapped_state = TICKET_STATES[ticket_type].get(
...
def get_ticket_command():
    ...
    entry = {
        ...
        "HumanReadable": tableToMarkdown(
            "ServiceNow ticket", hr, headers=headers, removeNull=True
        ),
        "EntryContext": {
...
def get_record_command():
    ...
    if fields:
        ...
        entry["HumanReadable"] = tableToMarkdown(
            "ServiceNow record", record, removeNull=True
        )
        entry["EntryContext"] = {
        ...
        mapped_record = {
            DEFAULT_RECORD_FIELDS[k]: record[k]
            for k in DEFAULT_RECORD_FIELDS
            if k in record
        }
        entry["ReadableContentsFormat"] = formats["markdown"]
        entry["HumanReadable"] = tableToMarkdown(
            "ServiceNow record" + record_id, mapped_record, removeNull=True
        )
        entry["EntryContext"] = {
...
def get(table_name, record_id, number=None):
    ...
    if record_id:
        ...
        raise ValueError(
            "servicenow-get-ticket requires either ticket ID (sys_id) or ticket number"
        )
...
def get_ticket_attachments(ticket_id):
    path = "attachment"
    query_params = {"sysparm_query": "table_sys_id=" + ticket_id}
...
def get_ticket_attachment_entries(ticket_id):
    ...
    if "result" in attachments_res and len(attachments_res["result"]) > 0:
        ...
        links = [
            (attachment["download_link"], attachment["file_name"])
...
def update_ticket_command():
    ...
    entry = {
        ...
        "EntryContext": {"ServiceNow.Ticket(val.ID===obj.ID)": context},
    }
...
def update_record_command():
    ...
    mapped_record = {
        DEFAULT_RECORD_FIELDS[k]: result[k]
        for k in DEFAULT_RECORD_FIELDS
        if k in result
    }
    entry = {
        ...
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown(
            "ServiceNow record updated successfully", mapped_record, removeNull=True
        ),
        "EntryContext": {
            "ServiceNow.Record(val.ID===obj.ID)": createContext(mapped_record)
        },
...
def create_record_command():
    ...
    entry = {
        ...
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown(
            "ServiceNow record created successfully", mapped_record, removeNull=True
        ),
        "EntryContext": {
            "ServiceNow.Record(val.ID===obj.ID)": createContext(mapped_record)
        },
...
def add_link_command():
    ...
    key = (
        "comments"
        if demisto.args().get("post-as-comment", "false").lower() == "true"
        else "work_notes"
    )
    text = demisto.args().get("text", demisto.args()["link"])
    link = (
        '[code]<a class="web" target="_blank" href="'
        + demisto.args()["link"]
        + '" >'
        + text
        + "</a>[/code]"
...
def get_ticket_notes_command():
    ...
    comments_query = (
        "element_id=" + ticket_id + "^element=comments^ORelement=work_notes"
    )
    ...
    entry = {
        ...
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown(
            "ServiceNow notes for ticket " + ticket_id,
            mapped_notes,
            headers=headers,
...
def query_tickets_command():
    ...
    entry = {
        ...
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown(
            "ServiceNow tickets", hr, headers=headers, removeNull=True
        ),
        "EntryContext": {
            "Ticket(val.ID===obj.ID)": context,
            "ServiceNow.Ticket(val.ID===obj.ID)": context,
...
def query_table_command():
    ...
    if fields:
        ...
        records = [
            dict(filter(lambda kv_pair: kv_pair[0] in fields, r.iteritems()))
            for r in res["result"]
        ]
        for r in records:
            r["ID"] = r.pop("sys_id")
        ...
        entry["ReadableContentsFormat"] = formats["markdown"]
        entry["HumanReadable"] = tableToMarkdown(
            "ServiceNow records", records, removeNull=True
        )
        entry["EntryContext"] = {
        ...
        mapped_records = [
            {DEFAULT_RECORD_FIELDS[k]: r[k] for k in DEFAULT_RECORD_FIELDS if k in r}
            for r in result
        ]
        entry["ReadableContentsFormat"] = formats["markdown"]
        entry["HumanReadable"] = tableToMarkdown(
            "ServiceNow records", mapped_records, removeNull=True
        )
        entry["EntryContext"] = {
...
def get_computer_command():
    ...
    if "support_group" in computer:
        ec["support_group"] = (
            computer["support_group"]["value"]
            if "value" in computer["support_group"]
            else ""
        )
    ...
    entry = {
        ...
        "EntryContext": {"ServiceNowComputer(val.sys_id==obj.sys_id)": ec},
    }
...
def query_computers_command():
    ...
    mapped_computers = [
        {
            ...
            "DisplayName": "{} - {}".format(
                computer.get("asset_tag", ""), computer.get("name", "")
            ),
            ...
            "State": COMPUTER_STATUS.get(
                computer.get("install_status", ""), computer.get("install_status")
            ),
            "Cost": "{} {}".format(
                computer.get("cost", ""), computer.get("cost_cc", "")
            ).rstrip(),
    ...
    entry = {
        ...
        "EntryContext": {
            "ServiceNow.Computer(val.ID===obj.ID)": createContext(
                mapped_computers, removeNull=True
            )
        },
    }
...
def query_groups_command():
    ...
    entry = {
        ...
        "EntryContext": {
            "ServiceNow.Group(val.ID===obj.ID)": createContext(
                mapped_groups, removeNull=True
            )
        },
    }
...
def query_users_command():
    ...
    entry = {
        ...
        "EntryContext": {
            "ServiceNow.User(val.ID===obj.ID)": createContext(
                mapped_users, removeNull=True
            )
        },
    }
...
def get_groups_command():
    ...
    for group in res["result"]:
        if group["name"] == group_name:
            ...
            context_groups.append(
                {"GroupId": group["sys_id"], "GroupName": group["name"]}
            )
    ...
    entry = {
        ...
        "EntryContext": {"ServiceNowGroups(val.GroupId==obj.GroupId)": context_groups},
    }
...
def list_table_fields_command():
    ...
    entry = {
        ...
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown(
            "ServiceNow Table fields - " + table_name, fields
        ),
        "EntryContext": {"ServiceNow.Field": createContext(fields)},
    }
...
def get_table_name_command():
    ...
    entry = {
        ...
        "ReadableContentsFormat": formats["markdown"],
        "HumanReadable": tableToMarkdown(
            "ServiceNow Tables for label - " + label,
            mapped_tables,
            headers=headers,
            headerTransform=pascalToSpace,
        ),
...
def fetch_incidents():
    ...
    if "time" not in last_run:
        time = FETCH_TIME if FETCH_TIME else "10 minutes"
        snow_time, _ = parse_date_range(time, "%Y-%m-%d %H:%M:%S")
    else:
    ...
    for result in res["result"]:
        ...
        current_time = result.get("opened_at")

        # Enforce time
        if current_time and (
            datetime.strptime(current_time, "%Y-%m-%d %H:%M:%S")
            < datetime.strptime(snow_time, "%Y-%m-%d %H:%M:%S")
        ):
            continue
        ...
        if get_attachments:
            ...
            for file_result in file_entries:
                if file_result["Type"] == entryTypes["error"]:
                    raise Exception(
                        "Error getting attachment: " + str(file_result["Contents"])
                    )
                file_names.append(
                    {"path": file_result["FileID"], "name": file_result["File"]}
                )
...
try:
    if demisto.command() == "test-module":
        ...
        demisto.command() == "servicenow-get"
        or demisto.command() == "servicenow-incident-update"
        or demisto.command() == "servicenow-get-ticket"
    ):
        demisto.results(get_ticket_command())
    elif (
        demisto.command() == "servicenow-update"
        or demisto.command() == "servicenow-incident-update"
        or demisto.command() == "servicenow-update-ticket"
    ):
        demisto.results(update_ticket_command())
    elif (
        demisto.command() == "servicenow-create"
        or demisto.command() == "servicenow-incident-create"
        or demisto.command() == "servicenow-create-ticket"
    ):
        demisto.results(create_ticket_command())
    elif demisto.command() == "servicenow-delete-ticket":
        demisto.results(delete_ticket_command())
    elif (
        demisto.command() == "servicenow-add-link"
        or demisto.command() == "servicenow-incident-add-link"
    ):
        demisto.results(add_link_command())
    elif (
        demisto.command() == "servicenow-add-comment"
        or demisto.command() == "servicenow-incident-add-comment"
    ):
        demisto.results(add_comment_command())
    elif (
        demisto.command() == "servicenow-query"
        or demisto.command() == "servicenow-incidents-query"
        or demisto.command() == "servicenow-query-tickets"
    ):
        demisto.results(query_tickets_command())
    elif (
        demisto.command() == "servicenow-upload-file"
        or demisto.command() == "servicenow-incident-upload-file"
    ):
