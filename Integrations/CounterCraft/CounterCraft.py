...
def http_request(request_method, path, data={}, params=""):
    ...
    signature = get_signature(request_method, headers, path, urllib.parse.urlencode(params).encode('utf-8'), SECRET_KEY)
...
def return_results(title, content, context, headers):
    ...
    if len(content) == 0:
        return_outputs(readable_output="There is no output result", outputs=context, raw_response=content)
