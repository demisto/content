...
def upload_sample():
    ...
    with open(fileData['path'], 'rb') as f:
        r = requests.request('POST', URL + SUB_API + 'samples',
                             files={'sample': (encode_sample_file_name(demisto.getArg('filename')), f)}, data=args)
        if r.status_code != requests.codes.ok:
...
def get_summary_by_id():
    ...
    md = tableToMarkdown('ThreatGrid - Sample Summary for ' + sample_id,
                         [sample['AnalysisSummary']], ['RegistryCount', 'FileName', 'SHA256', 'SampleType', 'FirstSeen', 'LastSeen'])
...
def search_samples():
    ...
    """
    if demisto.getArg('checksum'):
        search_checksum_samples()
        return
...
def search_checksum_samples():
    params = {'api_key': API_KEY, 'term': 'checksum', 'q': demisto.getArg('checksum')}
    r = req('GET', SUB_API + 'search/submissions', params=params)
    samples = []
    for sample in demisto.get(r.json(), 'data.items'):
        samples.append({
            'ID': demisto.get(sample, '_id')
        })
    demisto.results({
        'Type': entryTypes['note'],
        'EntryContext': {'ThreatGrid.Sample': samples},
        'HumanReadable': tableToMarkdown('ThreatGrid - Sample Search', samples, ['ID']),
        'ContentsFormat': formats['json'],
        'Contents': r.json()
    })
