...
def alert_to_incident(alert):
    ...
    for key in alert:
        if key == 'extraData':
            continue
        alert_labels.append({'type': key, 'value': str(alert[key])})

    for key in alert['extraData']:
        alert_labels.append({'type': f'extraData__{key}', 'value': str(alert['extraData'][key])})
...
def fetch_incidents():
    ...
    if FETCH_INCIDENTS_FILTER:
        ...
        filter_string = ', '\
            .join([f'{filt["key"]} {filt["operator"]} "{urllib.parse.quote(filt["value"])}"' for filt in alert_filters])
    ...
    for event in events:
        ...
        for ed in event['extraData']:
            event['extraData'][ed] = urllib.parse.unquote(event['extraData'][ed])
