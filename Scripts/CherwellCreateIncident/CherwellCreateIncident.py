...
# After you added the argument you will need to add his value to this `FIELDS` object by using
# `args.get('argument_name').
# Make sure the key name must be exactly equal to the field name in Cherwell system.
# For example: we wished to create an incident with a requesting customer in it, thus, we added a customer_display_name
# argument to this script, we pulled the value of the argument by using`args.get('customer_display_name')` and we set
# the key name to be `CustomerDisplayName` - as it is appear in Cherwell system.
#
# Expert tip: if you don't want to keep Demisto's arguments naming convention, you can name your arguments exactly as
# they appear in Cherwell system, this will shorten the `FIELDS` constant to be `FIELDS = args`
