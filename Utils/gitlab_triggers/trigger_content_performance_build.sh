#!/usr/bin/env bash
# This script triggers a performance job in gitlab-CI.
...
# 1. Utils/gitlab_triggers/trigger_content_performance_build.sh <trigger-token> <branch-name>
# 2. Utils/gitlab_triggers/trigger_content_performance_build.sh <trigger-token>
if [[ "$#" -lt 1 ]]; then
...
curl "$BUILD_TRIGGER_URL" -F "ref=$_branch" -F "token=$_gitlab_token" -F "variables[PERFORMANCE]=true" -F "variables[IFRA_ENV_TYPE]=Performance_Master" | jq
