...
def get_pack_support_type_from_pr_metadata_file(pr_metadata_filename: str, pr: PullRequest):
    ...
    metadata_file: ContentFile = contributor_repo.get_contents(path=pr_metadata_filename, ref=branch.commit.sha)  # type: ignore
    metadata_file_content: dict = json.loads(base64.b64decode(metadata_file.content))  # type: ignore
