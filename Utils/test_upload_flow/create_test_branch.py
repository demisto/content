...
def get_pack_content_paths(pack_path: Path, marketplace="xsoar"):
    ...
    """
    create_artifacts_command = [
        "demisto-sdk",
        "prepare-content",
        "-i",
        f"Packs/{pack_path.name}",
        "-o",
        ".",
    ]
    ...
    try:
        logging.debug(
            f"Running the SDK prepare-content command for pack {pack_path.name} - "
            f"Command: `{' '.join(create_artifacts_command)}`"
        )
        res = subprocess.run(create_artifacts_command, capture_output=True, check=True)
        logging.debug(
            f"Result from prepare-content - stdout: [{str(res.stdout)}] stderr: [{str(res.stderr)}]"
        )
    except subprocess.CalledProcessError as se:
        logging.error(
            f"Subprocess exception: {se}. stderr: [{se.stderr}] stdout: [{se.stdout}]"
        )
        raise
    ...
    for content_item_type in sub_dirs:
        if content_item_type not in ["ReleaseNotes", "TestPlaybooks"]:
            content_dict[content_item_type] = [
                "/".join(p.parts[1:])
                for p in Path(
                    os.path.join(str(pack_artifacts_path), content_item_type)
                ).glob("*")
            ]
...
def add_dependency(base_pack: Path, new_depndency_pack: Path, mandatory: bool = True):
    ...
    base_metadata.setdefault("dependencies", {}).update(
        {new_pack_name: {"mandatory": mandatory, "display_name": new_pack_name}}
    )
...
def update_existing_release_notes(pack: Path):
    ...
    latest_pack_version = str(
        max(
            [
                Version(file_name.name.replace(".md", "").replace("_", "."))
...
def create_failing_pack(pack: Path):
    ...
    base_metadata["currentVersion"] = ".".join(
        [splited_pack_version[0], str(int(splited_pack_version[1]) + 1)]
    )
    json_write(str(metadata_json), base_metadata)
    return pack, base_metadata["currentVersion"], None
...
def modify_modeling_rules_path(modeling_rule: Path, old_name: str, new_name: str):
    ...
    modify_item_path(
        modeling_rule / f"{old_name}_schema.json", f"{new_name}_schema.json"
    )
    ...
    return (
        pack_path,
        get_current_version(pack_path),
        get_pack_content_paths(pack_path, marketplace="marketplacev2"),
    )
...
def do_changes_on_branch(packs_path: Path):
    ...
    modify_modeling_rules_path(
        packs_path / "AlibabaActionTrail/ModelingRules/AlibabaModelingRules",
        "AlibabaModelingRules",
        "Alibaba",
    )

    # Case 11: Verify script path - CortexXDR
    modify_script_path(
        packs_path / "CortexXDR/Scripts/XDRSyncScript",
        "XDRSyncScript",
        "XDRSyncScript_new_name",
    )

    # case 12: Verify setting hidden dependency does not add this dependency to the metadata - MicrosoftAdvancedThreatAnalytics
    add_dependency(
        packs_path / "MicrosoftAdvancedThreatAnalytics",
        packs_path / "Microsoft365Defender",
        mandatory=False,
    )
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "-a", "--artifacts_path", help="Path to store the script's output", default="."
    )
    parser.add_argument(
        "-g",
        "--gitlab-mirror-token",
        help="Gitlab mirror token for pushing commits " "directly to gitlab repo",
    )
...
def main():
    ...
    try:
        new_branch_name = (
            args.test_branch
            if args.test_branch
            else f"{original_branch}_upload_test_branch"
        )
        content_path = Path(__file__).parent.parent.parent
        ...
        repo.git.push(
            ...
            f"code.pan.run/xsoar/content.git",
            branch,
            push_option="ci.skip",
        ...
        repo.git.checkout(original_branch)
        json_write(
            os.path.join(args.artifacts_path, "packs_items.json"), pack_items_dict
        )
        json_write(
            os.path.join(args.artifacts_path, "versions_dict.json"), versions_dict
        )
