...
test_inputs_query = [(response_mock, "Guy Freund", context_output), ([], "Found no results for given query.", {})]
...
class TestDatabase:
    ...
    def test_query(self, requests_mock, req_input, expected_md, expected_context):
        ...
        readable_output, context, _ = query_command(self.client, {"query": "qqq", "columns": "ID,Timestamp,Name,Urgency"})
        assert expected_md in readable_output
        assert expected_context == context

    @pytest.mark.parametrize(
        "req_input,last_run,fetch_time,table_name,columns,date_name,incidents_len,new_last_run", test_inputs_fetch_incidents
    )
    def test_fetch_incidents(
        self, requests_mock, req_input, last_run, fetch_time, table_name, columns, date_name, incidents_len, new_last_run
    ):
