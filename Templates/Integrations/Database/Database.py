...
def fetch_incidents_command(
    client: Client, last_run_dict: dict | None, first_fetch_time: str, table_name: str, columns: str, date_name: str
) -> tuple[dict, list]:
    ...
    incidents = [
        {"name": f"Database incident: {row[0]}", "occurred": row[1], "rawJSON": json.dumps(raw_response)} for row in raw_response
    ]
...
def main():
    ...
    if command == "fetch-incidents":
        ...
        last_run, incidents = fetch_incidents_command(client, demisto.getLastRun(), fetch_time, table_name, columns, date_name)
