    ...
    "event": [
        {
            ...
            "isActive": True,
            "assignee": [{"name": "DBot Demisto", "id": "11"}, {"name": "Demisto DBot", "id": "12"}],
        },
        {
            "eventId": "ab123",
            "description": "Phishing email",
...
EVENT_LIST_OUTPUT = {
    "Event": [
        {
            "Assignee": [{"ID": "11", "Name": "DBot Demisto"}, {"ID": "12", "Name": "Demisto DBot"}],
...
class TestFetchIncidends:
    ...
    def test_fetch_incidents_first_time(self, requests_mock, mocker):
        ...
        requests_mock.get(BASE_URL + "event?sinceTime=2010-01-01+00%3A00%3A00", json=EVENT_LIST_INPUT)
        mocker.patch(
            "AnalyticsAndSIEM.parse_date_range", return_value=(datetime.strptime(self.last_fetch, self.time_format), None)
        )
...
class TestEvents:
            ...
            "event": [
                {
                    ...
                    "createdAt": "2019-09-09T08:30:07.959533",
                    "isActive": True,
            ...
            "event": [
                {
                    ...
                    "createdAt": "2019-09-09T08:30:07.959533",
                    "isActive": False,
                    "assignee": [{"name": "user1", "id": "123"}, {"name": "user2", "id": "124"}],
                }
            ]
        }
        requests_mock.post(BASE_URL + "event", json=request_json)
        human_readable, context, raw_response = create_event_command(client, {"description": "Test event", "assignee": "123,124"})
    ...
    def test_query(self, requests_mock):
        ...
        event__to_query = {"event": [{"eventId": "1", "assignee": [{"id": 123}]}, {"eventId": "2", "assignee": [{"id": 123}]}]}
        requests_mock.get(BASE_URL + "query?eventId=1&eventId=2&eventId=3&assignee=123", json=event__to_query)
        human_readable, context, _ = query_command(client, {"event_id": "1,2,3", "assignee": "123"})
    ...
    def test_query_empty(self, requests_mock):
        ...
        human_readable, _, _ = query_command(client, {"event_id": "1,2,3", "assignee": "123"})
