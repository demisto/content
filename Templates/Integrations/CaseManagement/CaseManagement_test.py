...
class TestInputs:
        ...
        "CaseManagement.Ticket(val.ID && val.ID === obj.ID)": [
            {
                ...
                "Timestamp": "2010-01-01T00:00:00",
                "IsOpen": True,
                "Assignee": [{"ID": "user1", "Name": "User Name1"}, {"ID": "user2", "Name": "User Name2"}],
            }
        ]
    }
    ...
    CREATE_TICKET_INPUT = [(TICKET_MOCK, "Ticket has been successfully created", EXCEPTED_CONTEXT_ONE_TICKET)]
    ...
    EXCEPTED_CONTEXT_USER_LIST = {
        "CaseManagement.User(val.ID && val.ID === obj.ID)": [
            {"Username": "user1ftw", "ID": "111"},
            {"Username": "dasIstMe", "ID": "1337"},
        ]
    }
    LIST_USERS_INPUT = [(USER_LIST, "Users list", EXCEPTED_CONTEXT_USER_LIST), ({}, "Could not find", {})]
    INCIDENT = [{"name": "Case Management Integration - ticket number: 111", "rawJSON": json.dumps(TICKET_MOCK)}]
