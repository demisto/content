...
def build_raw_tickets_to_context(tickets: dict | list):
    ...
    return {
        ...
        "Assignee": [{"ID": assignee.get("id"), "Name": assignee.get("name")} for assignee in tickets.get("assignee", [])],
    }
...
def build_raw_users_to_context(users: list | dict):
    ...
    return {"ID": users.get("id"), "Username": users.get("username")}
...
class Client(BaseClient):
    ...
    def list_tickets(
        self, ticket_id: AnyStr | None = None, limit: AnyStr | None = None, from_time: datetime | None = None
    ) -> dict:
        ...
        params.update(assign_params(id=ticket_id, fromTime=from_time.strftime(TIME_FORMAT) if from_time else None))
...
def get_ticket_command(client: Client, args: dict) -> tuple[str, dict, dict]:
    ...
    if tickets:
        ...
        human_readable = tableToMarkdown(title, context_entry, headers=["ID", "Name", "Timestamp", "Description", "Assignee"])
...
def list_users_command(client: Client, *_) -> tuple[str, dict, dict]:
    ...
    if raw_response:
        ...
        context = {f"{INTEGRATION_CONTEXT_NAME}.User(val.ID && val.ID === obj.ID)": context_entry}
...
def close_ticket_command(client: Client, args: dict) -> tuple[str, dict, dict]:
    ...
    if tickets and tickets[0].get("id") == ticket_to_lock and not tickets[0].get("isOpen"):
        ...
        raise DemistoException(f"{INTEGRATION_NAME} - Could not close" f" ticket `{ticket_to_lock}`.\nResponse: {raw_response}")
...
def list_tickets_command(client: Client, args: dict) -> tuple[str, dict, dict]:
    ...
    if tickets:
        ...
        context_entry = build_raw_tickets_to_context(tickets)
        context = {f"{INTEGRATION_CONTEXT_NAME}.Ticket(val.ID && val.Name ==== obj.ID)": context_entry}
...
def fetch_incidents_command(client: Client, last_fetch: dict, fetch_time: str) -> tuple[list, dict]:
    ...
    if tickets:
        for ticket in tickets:
            incidents.append({"name": f'{INTEGRATION_NAME} - ticket number: {ticket.get("id")}', "rawJSON": json.dumps(ticket)})
