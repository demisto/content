...
def account_response_to_context(credentials: dict | list) -> dict | list:
    ...
    return {"Username": credentials.get("username"), "Name": credentials.get("name"), "IsLocked": credentials.get("isLocked")}
...
def build_credentials_fetch(credentials: dict | list) -> dict | list:
    ...
    return {"user": credentials.get("username"), "name": credentials.get("name"), "password": credentials.get("password")}
...
def build_vaults_context(vaults: list | dict) -> list[dict] | dict:
    ...
    return {"ID": vaults.get("vaultId"), "IsLocked": vaults.get("isLocked")}
...
def fetch_credentials(client: Client) -> list:
    ...
    if raw_credentials:
        ...
        raise DemistoException(
            f"`fetch-incidents` failed in `{INTEGRATION_NAME}`, no keyword `credentials` in" f" response. Check API"
        )
...
def lock_account_command(client: Client, args: dict) -> tuple[str, dict, dict]:
    ...
    if accounts and accounts[0].get("username") == username and accounts[0].get("isLocked") is True:
        ...
        context = {f"{INTEGRATION_CONTEXT_NAME}.Account(val.Username && val.Username === obj.Username)": context_entry}
