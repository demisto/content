...
'''
    ...
    # no such header
    table_no_headers = tableToMarkdown('tableToMarkdown test with no headers', data, headers=['no', 'header', 'found'],
                                       removeNull=True)
...
def test_camelize():
    assert str(camelize([{'chookity_bop': 'asdasd'}, {'ab_c': 'd e', 'fgh_ijk': 'lm', 'nop': 'qr_st'}],
                        '_')) == "[{u'ChookityBop': 'asdasd'}, {u'AbC': 'd e', u'Nop': 'qr_st', u'FghIjk': 'lm'}]"
    assert str(camelize({'ab_c': 'd e', 'fgh_ijk': 'lm', 'nop': 'qr_st'},
                        '_')) == "{u'AbC': 'd e', u'Nop': 'qr_st', u'FghIjk': 'lm'}"
...
def test_return_error():
    return_error(readable_output="foo", outputs={"foo": "foo1"})
...
class TestGetError(unittest.TestCase):
    ...
    def test_get_error_need_raise_error_on_non_error_input(self):
        ...
        try:
            ...
            self.assertEquals(str(e), "execute_command_result has no error entry. before using get_error use is_error")
