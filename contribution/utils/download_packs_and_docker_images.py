...
def download_and_save_packs(
    pack_names: dict, id_set_json: dict, output_path: str, verify_ssl: bool, use_defaut_filter: bool = False
) -> None:
    ...
    try:
        for pack_d_name, pack_name in pack_names.items():
            ...
            if use_defaut_filter and should_filter_out_pack(
                id_set_packs[pack_name], fields={"author": "Cortex XSOAR"}, remove_deprecated=True
            ):
            ...
            r = requests.request(
                method="GET", url=f"{BUCKET_PACKS_URL}/{pack_name}/{pack_version}/{pack_name}.zip", verify=verify_ssl
            )
...
def options_handler():
    ...
    parser.add_argument("--insecure", help="Skip certificate validation.", dest="feature", action="store_true")
...
def main():
    ...
    if not options.skip_packs and pack_names:
        download_and_save_packs(
            pack_names, id_set_json, os.path.join(output_path, "packs"), verify_ssl, use_defaut_filter=not bool(packs)
        )
    else:
